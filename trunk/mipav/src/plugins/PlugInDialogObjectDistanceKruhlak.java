 ...
 * 
 * $Logfile: /mipav/src/plugins/PlugInDialogObjectDistanceKruhlak.java $
 * $Revision: 6 $ $Date: 1/25/06 4:59p $
 * 
 */
public class PlugInDialogObjectDistanceKruhlak extends JDialogBase implements AlgorithmInterface {
    ...
    /**
     * Creates new dialog for distances between 2 3D centers of mass using a
     * plugin.
     * 
     ...
     * 
     * @param event
     *            a checkbox-generated event
     */
    public void itemStateChanged(ItemEvent event) {
        Object source = event.getSource();

        if ((colorsPresent == 3) && ((source == redCheckBox) || (source == greenCheckBox) || (source == blueCheckBox))) {
            ...
            if (((redCheckBox.isSelected()) && (greenCheckBox.isSelected()) && (!blueCheckBox.isSelected()))
                ...
                if (useRed) {
                    labelThreshold1
                            .setText("Red Threshold (" + String.valueOf(minR) + "-" + String.valueOf(maxR) + ")");
                    threshold1 = minR;
                } else {
            ...
            if (objectDistAlgo.isCompleted() == true) {
                if (userInterface.isScriptRecording()) {
                    userInterface.getScriptDialog().append(
                            "ObjectDistanceKruhlak " + userInterface.getScriptDialog().getVar(image.getImageName())
                                    + " " + useRed + " " + useGreen + " " + useBlue + " " + threshold1 + " "
                                    + threshold2 + "\n");
                }
            }
            dispose();
