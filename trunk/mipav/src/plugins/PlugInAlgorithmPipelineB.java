...
public class PlugInAlgorithmPipelineB extends AlgorithmBase {
    ...
    /** DOCUMENT ME! */
    public static int subcutfatCount, fatCount, muscleCount, boneCount, bone_marrowCount, total_thighCount, xDim, yDim,
                      zDim, sliceSize, volSize;
     ...
     */
    public void convert(ModelImage destImage, ModelImage tempImageOne, ModelImage srcImage, int tempIntensity,
                        int newIntensity) {
                    ...
                    for (x = 3; x < (xDim - 3); x++) {
                        ...
                        if ((imgBuffer2[i - 3] == 0) && (imgBuffer2[i - 2] == 0) && (imgBuffer2[i - 1] == 0) &&
                            ...
                            for (cc = 0; cc < (xDim - x - 1); cc++) { // go up til edge of image
                                ...
                                if ((imgBuffer2[i + cc - 3] == 1) && (imgBuffer2[i + cc - 2] == 1) &&
                                        (imgBuffer2[i + cc - 1] == 1) && (imgBuffer2[i + cc] == 0) &&
                                        (imgBuffer2[i + cc + 1] == 0) && (imgBuffer2[i + cc + 2] == 0)) {
                                    xmin = cc;
                                    xmax = 0;
                                    ...
                                    for (dd = 0; dd < (xDim - x - cc - 1); dd++) {
                                        ...
                                        if ((imgBuffer2[i + cc + dd - 3] == 0) && (imgBuffer2[i + cc + dd - 2] == 0) &&
                                                ...
                                                (imgBuffer2[i + cc + dd + 1] == 1) &&
                                                (imgBuffer2[i + cc + dd + 2] == 1)) { // looking for another bkrd to
                                                                                      // bone boundary
                                            xmax = cc + dd;
                                            cc = xDim;
                                            dd = xDim;
    ...
    public ModelImage isolatingBone(ModelImage srcImage, int BoneIntensity) {
        ...
        Dilate6(BoneID);
        Close24(BoneID);
        IDObjects(BoneID, zDim * 1000 / 20, zDim * 30000 / 20); // ShowImage(BoneID, "thresholded IDobjected opened
                                                                // closed idobjected --");
        ...
        for (bb = 0; bb < zDim; bb++) {
            ...
            try {
                ...
                for (cc = 1; cc <= numObjects[bb]; cc++) {
                    ...
                    distFromCent[cc - 1] = Math.abs((centroidX[cc - 1] - (xDim / 2)) ^
                                                        (2 + (centroidY[cc - 1] - (yDim / 2))) ^ 2);
                }
    ...
    public ModelImage N3(ModelImage destImage1) {
        ...
        destImage2 = new ModelImage(destImage1.getType(), destImage1.getExtents(), "destImage2",
                                    destImage1.getUserInterface());

        AlgorithmIHN3Correction ihn3Algo1 = null;
        ihn3Algo1 = new AlgorithmIHN3Correction(destImage2, fieldImage, destImage1, 100f, 150, 0.0001f, 33.3f, 4f, 0.2f,
    ...
    /**
     * public ModelImage processFat(ModelImage fatImage0, ModelImage fatImage1, ModelImage fatImage2, ModelImage
     * fatImage3){.
     *
    ...
    public ModelImage processFat(ModelImage fatImage0, ModelImage fatImage1, ModelImage fatImage2) {
        ...
        fatImage = new ModelImage(fatImage1.getType(), fatImage1.getExtents(), "fatImage",
                                  fatImage1.getUserInterface());
    ...
    public void runAlgorithm() {
        ...
        for (aa = 1; aa <= 2; aa++) {
            ...
            // AVGsubcutfatCount, AVGfatCount, AVGmuscleCount,AVGmuscleCount, AVGbone_marrowCount, AVGtotal_thighCount
            // = 0;
            for (bb = 0; bb < zDim; bb++) {
