...
public class PlugInAlgorithmAnonymizeDICOM extends AlgorithmBase{
    ...
    private RandomAccessFile sourceRaFile = null;
	...
	//	~ Constructors -----------------------------------------------------------------------------------------
	public PlugInAlgorithmAnonymizeDICOM(File inputFile) {
    ...
    public void loadTagBuffer() {
        ...
        try {
            sourceRaFile = new RandomAccessFile(selectedFile, "r");
            if (sourceRaFile != null) {
                fLength = sourceRaFile.length();
            } else {
                return;
            }
            ...
            sourceRaFile.readFully(tagBuffer);
        } catch (IOException ioE) { }
    }
    ...
    public boolean anonymizeDicom() {
    	...
    	//try {  
    	//	if (bPtr == ID_OFFSET) { // Preamble and DICM present. 
    	//		buffer = new byte[ID_OFFSET];
        //    	sourceRaFile.seek(0);
        //    	sourceRaFile.readFully(buffer);
        //    	destRaFile.seek(0);
        //    	destRaFile.write(buffer);
        //    }
