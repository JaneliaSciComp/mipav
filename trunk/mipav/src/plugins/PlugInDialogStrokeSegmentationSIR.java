...
public class PlugInDialogStrokeSegmentationSIR extends JDialogStandaloneScriptablePlugin implements AlgorithmInterface {
...
//    private JTextField cerebellumAggressiveSliceMaxField;
    ...
    private boolean flairImageMultifile = false;
    private ModelImage flairImage = null;
...
//    private int minPerfusionObjectSize = 100;
    ...
    private PlugInAlgorithmStrokeSegmentationSIR segAlgo = null;
    ...
    private static final String flairFileStub = "FLAIR";
    ...
    private boolean allowNoFlair = false;

    /**
     * Constructor used for instantiation during script execution (required for dynamic loading).
     */
    public PlugInDialogStrokeSegmentationSIR() {
        super(false);
    ...
    public PlugInDialogStrokeSegmentationSIR(final StrokeSegmentationDicomReceiverPWI parent, final String dicomDir, boolean noFlair) {
        ...
        if ((flairPath != null && !flairPath.equals("")) && !allowNoFlair) {
            final File flairFile = new File(flairPath);
            flairImage = openImage(flairFile, flairImageMultifile);
            
            if (flairImage == null) {
                System.err.println("Error opening FLAIR volume from file: " + dwiPath);
                return;
            }
        } else if (allowNoFlair) {
        	// do nothing
        } else {
            System.err.println("No FLAIR volume selected.");
            return;
        }
        
        if (adcImage != null && dwiImage != null && (flairImage != null || allowNoFlair)) {
            callAlgorithm();
        }
    }
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        } else if (command.equals("BrowseFlair")) {
            if (browseFlairImage()) {
                fileMethodRadio.setSelected(true);
            }
...
//            sigmaYText.setEnabled(spatialSmoothingCheckBox.isSelected());
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        PlugInAlgorithmStrokeSegmentationSIR segAlgo = (PlugInAlgorithmStrokeSegmentationSIR) algorithm;
        
        if (segAlgo instanceof PlugInAlgorithmStrokeSegmentationSIR) {
            System.err.println("Stroke segmentation Elapsed: " + segAlgo.getElapsedTime());
            
            double resFactorCC = PlugInAlgorithmStrokeSegmentationSIR.getResolutionFactorCC(adcImage);
    ...
    protected boolean setVariables() {
        ...
        if (doDirMethod) {
            ...
            flairPath = flairImageFileField.getText();
        ...
        if (flairPath != null && !flairPath.equals("")) {
            final File flairFile = new File(flairPath);
            flairImage = openImage(flairFile, flairImageMultifile);
            
            if (flairImage == null) {
                MipavUtil.displayError("Error opening FLAIR volume from file: " + flairPath);
                return false;
            }
        } else {
            MipavUtil.displayError("No FLAIR volume selected.");
            return false;
        }
...
//        ventricleMeanThresh = Float.parseFloat(ventricleMeanThreshField.getText());
    ...
    protected void callAlgorithm() {
        ...
        try {
            segAlgo = new PlugInAlgorithmStrokeSegmentationSIR(dwiImage, adcImage, flairImage, adcThreshold, doFilter, doCerebellumSkip, cerebellumSkipSliceMax, 
            		...
            		requireMinCoreSize, minCoreSizeCC, outputDir);
...
//        
        ...
        dirMethodRadio = new JRadioButton("Select directory with ADC, DWI, and FLAIR files/subdirectories");
        ...
        JLabel labelFlair = new JLabel("FLAIR scan");
        labelFlair.setForeground(Color.black);
        labelFlair.setFont(serif12);
        mainPanel.add(labelFlair, gbc);
        
        flairImageFileField = new JTextField(50);
        //flairImageFileField.setText(lastAdc);
        gbc.gridx++;
        mainPanel.add(flairImageFileField, gbc);
        
        JButton flairImageFileButton = new JButton("Browse");
        flairImageFileButton.setActionCommand("BrowseFlair");
        flairImageFileButton.addActionListener(this);
        flairImageFileButton.setForeground(Color.black);
        flairImageFileButton.setFont(serif12B);
        gbc.gridx++;
        mainPanel.add(flairImageFileButton, gbc);
    ...
    private boolean browseFlairImage() {
        final ViewFileChooserBase fileChooser = new ViewFileChooserBase(true, false);
        fileChooser.setMulti(ViewUserInterface.getReference().getLastStackFlag());
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            flairImageFileField.setText(file.getAbsolutePath());
            flairImageMultifile = isMultiFile;
    ...
    private boolean findVolumesInDir(final String dir) {
        ...
        for (File file : dirContents) {
            ...
            if (nameNoExt.equalsIgnoreCase(adcFileStub) || nameNoExt.equalsIgnoreCase(adcFileStub + "-register")) {
                if (file.isDirectory()) {
            ...
            } else if (nameNoExt.equalsIgnoreCase(dwiFileStub) || nameNoExt.equalsIgnoreCase(dwiFileStub + "-register")) {
                if (file.isDirectory()) {
                    File[] subContents = file.listFiles();
                    if (subContents.length > 0) {
            ...
            } else if (nameNoExt.equalsIgnoreCase(flairFileStub)) {
                if (file.isDirectory()) {
                    File[] subContents = file.listFiles();
                    if (subContents.length > 0) {
                        flairPath = subContents[0].getAbsolutePath();
        ...
        if (!foundADC || !foundDWI || !foundFlair) {
        ...
        if (!foundFlair && !allowNoFlair) {
            MipavUtil.displayError("No FLAIR files found in directory: " + dir);
            return false;
        } else if (!foundFlair && allowNoFlair) {
        	System.err.println("No FLAIR files found - running DWI/ADC coretool only: " + dir);
        }
    ...
    private void findDicomFilesRecursive(final File dirFile) {
        ...
        for (File file : dirContents) {
            ...
            if (file.isDirectory()) {
                ...
                if (!checkedFirstFile && !file.getName().contains(PlugInAlgorithmStrokeSegmentationSIR.outputLabel)) {
