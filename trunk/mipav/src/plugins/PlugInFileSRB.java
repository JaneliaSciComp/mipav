...
public class PlugInFileSRB implements PlugInFile, ScriptableInterface {
    ...
    private final static int TRANSFER_DIALOG = 2;
    ...
    public void readImage() {
        ...
        /**
         * Uses the JargonFileChooser to retrieve the file that the user wants to
         * open.
         */
        ...
        if(returnValue == JargonFileChooser.APPROVE_OPTION){
            /**
             * According to the files selected by user, tries to create the
             * srb file list.
             */
            ...
            /**
             * Gets the local temporary diretory, if it doesn't exist, then it
             * will be created.
             */
    ...
    public Vector copyFilesFromSRB(SRBFile srbFile){
        ...
        if (extension.equalsIgnoreCase("")) {
            ...
            SRBFile f = new SRBFile((SRBFile)srbFile.getParentFile(), srbFile.getName().replaceFirst("\\.xml$", ".raw"));
            srbFileList.add(f);
        } else if (extension.equalsIgnoreCase(".img")) {
            SRBFile f = new SRBFile((SRBFile)srbFile.getParentFile(), srbFile.getName().replaceFirst("\\.img$", ".hdr"));
            srbFileList.add(f);
        }
    ...
    public void readImageFromURI(URI uri) {
        ...
        if (isSRBURI(uri)) {
            ...
            /**
             * If there is no srb file system information, then try to use the
             * uri to create the srb file system. If the srb file system can't
             * be created by uri, then use the srb server login dialog to get
            ...
            try{
                ...
                e.printStackTrace(System.err);
                MipavUtil.displayError("Error encountered opening file: "
                        + e.getMessage());
                return;
            }
        }
    ...
    /**
     * Creates the srb file system from the given uri, if it fails, then use the srb login
     * dialog to get the needed information to create srb file systme.
     * @param uri
     * @return
     */
    public static SRBFileSystem createSRBFileSystem(URI uri) {
        ...
        try {
            ...
            if (srbAccount == null) {
                JDialogLoginSRB loginDialog = new JDialogLoginSRB(
                        "Connect to");
            ...
            e.printStackTrace(System.err);
            MipavUtil.displayError("Error encountered creating SRBAccount from URI("
    ...
    public void writeImageToSRBFile(SRBFile srbFile){
         ...
         * 
         * The idea is try to use the save function save the files to different 
         * directory.
         */
         ...
         */
        FileWriteOptions opts = new FileWriteOptions(
                ((LocalFile) sourceFileList.get(0)).getName(), localTempDir.getPath()+"//", false);
    ...
    /**
     * Gets the file name list that the current image contains according to
     * the file info.
     * @param fileInfoList
     * @return
     */
    ...
    /**
     * Transfers the images between the source URI and the destination
     * URI, which could be local or SRB file.
    ...
    private static final String getExtension(SRBFile srbFile){
        if(srbFile == null){
            return null;
        }
    ...
    private class JDialogGetURI extends JDialog implements ActionListener {
        ...
        public JDialogGetURI(int dialogType, String dialogTitle) {
            ...
            if(dialogType == OPEN_DIALOG){
                ...
                manager.add(WidgetFactory.buildTextButton("Save", "Save the active imagefile indicated by URI", "Save", this));
            } else {
                manager.add(WidgetFactory.buildLabel("Enter Source URI "));
                sourceURIField = WidgetFactory.buildTextField("srb://");
                sourceURIField.setColumns(40);
                ...
                manager.add(WidgetFactory.buildTextButton("Browse", "Browse srb or local filesystem (determined by which is in URI field).", "destinationBrowse", this));
                manager.add(WidgetFactory.buildTextButton("Transfer", "Push file from source URI to destination URI", "Transfer", this));
            }
            if(dialogTitle == null){
