...
public class PlugInAlgorithmAnonymizeDicom extends AlgorithmBase {
    ...
    private class ReadDicom extends FileDicomInner {
        ...
        private final HashMap<FileDicomKey, FileDicomSQ> sequenceTags;

        private final HashMap<FileDicomKey, Object> privateTags;

        private final HashMap<FileDicomKey, Object> anonymizeTags;

        public ReadDicom(final String fName, final String fDir) throws IOException {
            super(fName, fDir);
        ...
        public HashMap<FileDicomKey, FileDicomSQ> storeSequenceTags(
                final HashMap<FileDicomKey, FileDicomSQ> prevSliceSequenceTags) {
            ArrayList<FileDicomKey> ar;
            final String anonymized = PlugInAlgorithmAnonymizeDicom.ANONYMIZED + "\t";
            String seqPrefix = "";
            Collections.sort(ar = new ArrayList<FileDicomKey>(sequenceTags.keySet()), new KeyComparator());
            final Iterator<FileDicomKey> sqItr = ar.iterator();
            while (sqItr.hasNext()) {
                final FileDicomKey key = sqItr.next();
                final FileDicomSQ sq = sequenceTags.get(key);
                final FileDicomSQ sqPrev = prevSliceSequenceTags.get(key);
                ...
                final Vector<String> differentEntries = new Vector<String>();
                ...
                if (v.size() > 0 && !allSame) {
                    ...
                    for (int i = 0; i < v.size(); i++) {
                        final String tagCode = v.get(i).substring(1, 10);
                        final String prefix = tagExistInAnonymizeTagIDs(tagCode) ? anonymized : seqPrefix;
        ...
        public HashMap<FileDicomKey, Object> storeAnonymizeTags(
                final HashMap<FileDicomKey, Object> prevSliceAnonymizeTags) {
            ArrayList<FileDicomKey> ar;
            Collections.sort(ar = new ArrayList<FileDicomKey>(anonymizeTags.keySet()), new KeyComparator());
            final Iterator<FileDicomKey> prItr = ar.iterator();
            while (prItr.hasNext()) {
                final FileDicomKey key = prItr.next();
                final Object obj = anonymizeTags.get(key);
                String name = DicomDictionary.getName(key);
                if (name == null) {
                    name = "Private Tag";
                }
                if (prevSliceAnonymizeTags.get(key) == null
                        || (prevSliceAnonymizeTags.get(key) != null && !printObject(prevSliceAnonymizeTags.get(key))
                                .equals(printObject(obj)))) {
        ...
        public HashMap<FileDicomKey, Object> storePrivateTags(final HashMap<FileDicomKey, Object> prevSlicePrivateTags) {
            ...
            while (prItr.hasNext()) {
                final FileDicomKey key = prItr.next();
                final Object obj = privateTags.get(key);
                if (prevSlicePrivateTags.get(key) == null
                        || (prevSlicePrivateTags.get(key) != null && !printObject(prevSlicePrivateTags.get(key))
                                .equals(printObject(obj)))) {
        ...
        private String printObject(final Object obj) {
            if (obj instanceof Object[]) {
                String s = new String();
         ...
         * handles the various tags that are present at the beginning of a DICOM image file. It also sets the important
         * File Info variables based on what it finds.
         * 
         * <p>
         * The method will return with a failure code if it finds the tag &quot;0000,0000&quot; or it mis-reads the
         * header and starts reading from an odd byte.
         * </p>
         * 
         * <p>
         ...
         * 
         * <p>
         * Any special handling based on type occurs for each tag (@see FileInfoDicom), then it is added to the DICOM
         * tags table. Each tag is checked against a small list of individual tags, as some tags have an effect on the
         * way the following tags are interpreted.
         * </p>
         * 
         * <p>
    ...
    /**
     * This class reads and writes DICOM files. The DICOM file format consists of header information marked by tags,
     * with the pixel data as the last tag. Each tag has a length field that contains the length of the tag, so it is
     * possible to skip over tags that are unrecognized. This class is entirely based on DICOM version 3.0 published by
     * ACR/NEMA and so tests for all the tags in the Data Dictionary (DICOM v. 3.0 Part 6). It stores these in a table
     ...
     * contains many
     * privatehttp://www.google.com/search?hl=en&client=firefox-a&channel=s&rls=org.mozilla%3Aen-US%3Aofficial&hs=ptT&q=Agatston+method+aortic+calcium&btnG=Search
     * tags that are commented out. If the user wishes a specific private tag to be recognized by this program, he or
     * she should edit the dictionary file. The tag will then be displayed as any other standard tag would be displayed.
     * Otherwise, all tags are read in, but if their value representation is unrecognized (only the case with tags not
     * defined in dictionary file) their value is stored as a string. When FileInfoDicom displays the tag information,
     * it shows the name of the tag as private and the value as the string. The string may contain valid data or it may
     * contain junk. There is no way of knowing how to properly read in a private tag without a valid value
     * representation (VR). So if the user wishes to know private tag information, he or she should specify the proper
     * VR in the dictionary file and be sure that their file conforms to that VR.
     * </p>
     * 
     * <p>
    ...
    private class FileDicomInner extends FileDicomBaseInner {
        ...
        /**
         * When dicom image data is 'encapsulated,' it may be in pieces, or 'fragments.' don't know quite why, or if
         * pieces should be kept together. If in fragments, the image data may span several slices, called a 'frame.'
         */
        ...
        /**
         * Number of bytes following this File Meta Element (end of the Value field) up to and including the last File
         * Meta Element of the Group 2 File Meta Information.
         * 
         * <p>
        ...
        /**
         * If the file is <i>quiet</i> no user-interaction is performed. Useful for determining whether or not to
         * display the MipavUtil.displayError() is to be called. This allows the option of leaving the user-interaction
         * and notification of an error to occur here or to be handled somewhere else (as in, when an IOException is
         * thrown but we'd prefer to notify the user once, rather than for each exception.) based on programming
         * preferences or user-debug preference settings.
         */
        ...
        /**
         * DICOM reader/writer constructor. Creates the access files and ensures that the files are opened for
         * read-write access; it will fall back to read-only if read-write access to the image file is not granted. The
         * file info gets the DICOM dictionary set, as well as the endianess property is set. The image itself is not
         * read.
         * 
        ...
        /**
         * Prepares this class for cleanup. Calls the <code>finalize</code> method for existing elements, closes any
         * open files and sets other elements to <code>null</code>. Exceptions which occur while this method runs
         * (for instance, the possibility of getting a <code>IOException</code> when closing the image file) is
         * ignored quietly.
         */
        public void finalize() {
            fileName = null;
        ...
        /**
         * returns the ModelLUT, if there, which might be specified by the <code>0028,1201</code>,
         * <code>0028,1202</code>, <code>0028,1203</code> tags.
         * 
         * @return DOCUMENT ME!
         */
        @SuppressWarnings("unused")
        ...
        /**
         * Dicom version 2: Does not have a preamble in which to search for "DICM" So...the solution will be to search
         * that it has at least a couple of beginning "08" tags However, we do not know if its Little Endian or Big
         * Endian..so need to handle both
         * 
         * @return
         * @throws IOException
         */
         ...
         * </p>
         * 
         * @return whether this class should consider itself quiet, and by internal inspection, not notify the user.
         *         <code>
         *          True</code> is to consider itself to not notify the user. <code>False</code> is to notify
        ...
        /**
         * Reads a length of the data and deposits it into a single Short or an array of Short as needed by the tag's
         * VM.
         * 
         * @return Object
         * 
         ...
         */
        protected Object getByte(final int vm, final int length, final boolean endianess) throws IOException {
        ...
        /**
         * Reads a length of the data and deposits it into a single Double or an array of Double as needed by the tag's
         * VM.
         * 
         * @return Object
         * 
         ...
         *            indicates little-endian.
         */
        protected Object getDouble(final int vm, final int length, final boolean endianess) throws IOException {
            int len = (elementLength == UNDEFINED_LENGTH) ? 0 : elementLength;
            int i = 0;
        ...
        /**
         * Reads a length of the data and deposits it into a single Float or an array of Float as needed by the tag's
         * VM.
         * 
         * @return Object
         * 
        ...
        /**
         * Reads a length of the data and deposits it into a single Integer or an array of Integer as needed by the
         * tag's VM.
         * 
         * @return Object
         * 
         ...
         *            indicates little-endian.
         */
        protected Object getInteger(final int vm, final int length, final boolean endianess) throws IOException {
            int len = (elementLength == UNDEFINED_LENGTH) ? 0 : elementLength;
            int i = 0;
            Object readObject = null;
        ...
        /**
         * Reads in four integers, then tests for implicit Value Representation(VR) or explicit VR. If explicit, it
        ...
        /**
         * Increments the location, then reads the elementWord, groupWord, and elementLength. It also tests for an end
         ...
         * @throws IOException DOCUMENT ME!
         */
        protected void getNextElement(final boolean bigEndian) throws IOException {
            ...
            if (fileInfo.vr_type == FileInfoDicom.EXPLICIT) {
                ...
                /*
                 * explicit tags carry an extra 4 bytes after the tag (group, element) information to describe the type
                 * of tag. the element dictionary describes this info, so we skip past it here. (apr 2004)
                 */
                final String tagname = convertGroupElement(groupWord, elementWord);

                if (tagname.equals(FileDicomInner.SEQ_ITEM_BEGIN) || tagname.equals(FileDicomInner.SEQ_ITEM_END)
                        || tagname.equals(FileDicomInner.SEQ_ITEM_UNDEF_END) || tagname.equals("FFFE,EEEE")) // reserved
                {
         ...
         * standard. There is usually an "item" tag, then a dataset encoded exactly like other tags, then a tag
         ...
         * @see DicomSQ
         */
        protected Object getSequence(final boolean endianess, final int seqLength, final String name)
                throws IOException {
            ...
            try {
                ...
                if ( (seqLength == UNDEFINED_LENGTH) || (seqLength == ILLEGAL_LENGTH)) {
                    ...
                    while ( !nameSQ.equals(FileDicomInner.SEQ_ITEM_UNDEF_END)) {

                        if (nameSQ.equals(FileDicomInner.SEQ_ITEM_BEGIN)) {
                    ...
                    while ( (getFilePointer() - seqStart) < seqLength) {
                        ...
                        if (nameSQ.equals(FileDicomInner.SEQ_ITEM_BEGIN)) { // this should always be true
         ...
         *            indicates little-endian.
         */
        protected Object getShort(final int vm, final int length, final boolean endianess) throws IOException {
            int len = (elementLength == UNDEFINED_LENGTH) ? 0 : elementLength;
            int i = 0;
            Object readObject = null; // the Object we read in
        ...
        /**
         * Writes out a sequence tag and its data. The routine writes the Sequence as undefined-length and each of the
         * items as undefined length. The appropriate write-methods are used to output the various tags to the file.
         * 
    ...
    /**
     * FileDICOMBase is an class that supports the reading/writing of DICOM files. It reads in a buffer of tags that can
     * be parsed more quickly than continued random accesses to the harddrive.
     * 
    ...
    private class FileDicomBaseInner {
        ...
        /**
         * Setups the allocation of memory for the byte buffer to load tags. This tag buffer is a cache to speed the
