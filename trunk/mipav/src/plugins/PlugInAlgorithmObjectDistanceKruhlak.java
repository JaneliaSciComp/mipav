 ...
 * threshold will be counted toward the center of mass. The thresholds are specified for each color by the user in the
 * dialog box. Be sure to hit the new VOI button for each new VOI. Note that a single VOI can contain multiple contours
 * so if mulitple curves are used without hitting the new VOI button, then the interiors of all these curves will belong
 * to the same VOI.
 * 
 * @version June 3, 2004
 * @author
 * @see AlgorithmBase
 * 
 * $Logfile: /mipav/src/plugins/PlugInAlgorithmObjectDistanceKruhlak.java $ $Revision: 7 $ $Date: 1/25/06 4:59p $
 */
    ...
    private void calc2D() {
        ...
        for (i = 0; i < nVOIs; i++ ) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                xPos2 /= count2;
                yPos2 /= count2;
                centerToCenter = (float) Math.sqrt( (xPos1 - xPos2) * (xPos1 - xPos2) * xRes * xRes + (yPos1 - yPos2)
                        * (yPos1 - yPos2) * yRes * yRes);
                if (useRed) {
    ...
    private void calc3D() {
        ...
        for (i = 0; i < nVOIs; i++ ) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                zPos2 /= count2;
                centerToCenter = (float) Math.sqrt( (xPos1 - xPos2) * (xPos1 - xPos2) * xRes * xRes + (yPos1 - yPos2)
                        * (yPos1 - yPos2) * yRes * yRes + (zPos1 - zPos2) * (zPos1 - zPos2) * zRes * zRes);
                if (useRed) {
                    UI.setDataText("VOI ID = " + i + " with red weighted voi center of mass = " + "(" + xPos1 + ", "
                            + yPos1 + ", " + zPos1 + ")\n");
                } else {
