...
public class PlugInDialogDrosophilaStandardColumnRegistration extends JDialogStandalonePlugin implements AlgorithmInterface {
    ...
    public ArrayList<float[]> swcFilamentCoords = new ArrayList<float[]>();
    ...
    public JRadioButton _27APointsRadio, _75APointsRadio;
   ...
   //public static final int _27POINTSA = 27;
   //public static final int _75POINTSA = 75;
   ...
   private String numPointsString;
	...
	public void init() {
        ...
        _27APointsRadio = new JRadioButton("9 points per layer (27)");
        _75APointsRadio = new JRadioButton("25 points per layer (75)");
        _27APointsRadio.setSelected(true);

        //pointsGroup.add(_27PointsRadio);
        //pointsGroup.add(_75PointsRadio);
        //pointsGroup.add(_147PointsRadio);
        
        pointsGroup.add(_27APointsRadio);
        pointsGroup.add(_75APointsRadio);
        ...
        pointsPanel.add(_27APointsRadio);
        pointsPanel.add(_75APointsRadio);
			 ...
			 JFileChooser chooser = new JFileChooser();
		        ...
		        if (returnValue == JFileChooser.APPROVE_OPTION) {
		        	...
		        	if(!readSurfaceFile(surfaceFile)) {
		        	    ...
		        	    success = swcFilamentCoords.size() > 1;
		        	    if (success == false) {
		        	    	swcFilamentCoords = new ArrayList<float[]>();	
		        	    } else {
		        		...
		        		success = determineIfProperlyConnected();
		        		if(success == false) {
		        			allFilamentCoords = new ArrayList <ArrayList<float[]>>();
	...
	public boolean readSurfaceFile(File surfaceFile) {
		...
		String fileName = surfaceFile.getName();
		int pos = fileName.lastIndexOf(".");
	    haveSWC = false;
		if ((pos >= 0) && (fileName.substring(pos+1).equalsIgnoreCase("SWC"))) {
		    haveSWC = true;	
		}
		try {
	...
	private boolean setVariables() {
		...
		/*if(_27PointsRadio.isSelected()) {
			numPointsString = _27POINTS;
		}else if (_75PointsRadio.isSelected()) {
			numPointsString = _75POINTS;
		}else if (_147PointsRadio.isSelected()) {
			numPointsString = _147POINTS;
		}*/
		
		if(_27APointsRadio.isSelected()) {
			numPointsString = _27APOINTS;
		}else if (_75APointsRadio.isSelected()) {
			numPointsString = _75APOINTS;
		}
	...
	public void callAlgorithm() {
        ...
        if (haveSWC) {
        	alg = new PlugInAlgorithmDrosophilaStandardColumnRegistration(neuronImage,pointsMap,swcFilamentCoords,
		    ...
		    alg = new PlugInAlgorithmDrosophilaStandardColumnRegistration(neuronImage,pointsMap,allFilamentCoords,
		    		surfaceFile,samplingRate,cityBlockImage,pointsFile,outputTextArea,flipXCB.isSelected(),
		    		flipYCB.isSelected(), flipZCB.isSelected(),greenThreshold,subsamplingDistance,rigRadio.isSelected(),
		    		doSWC,rvldRadio.isSelected(),numPointsString);
        }
