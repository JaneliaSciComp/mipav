...
public class PlugInAlgorithmDrosophilaRetinalRegistration extends AlgorithmBase {
			 ...
			 for(int y=0;y<512;y++) {
				 for(int x=0;x<512;x++) {
					 ...
					 if(!transform3FilePathTextField.getText().trim().equals("")){ //if nlt file is inputted
				        ...
				        int iIndexTrg = (int)tPt1[0] + ((int)tPt1[1] * imageYExtents[0]) + ((int)tPt1[2] * imageYExtents[0]*imageYExtents[1]);
	...
	private void createRedAndGreenChannelsImages() {
		...
		int[] extents = {512,512,512};
		redChannelsImage = new ModelImage(ModelImage.ARGB, extents,"redChannelsImage-Rxreg-Ry-Rcomp");
		 float[] redChannelsImageResols = new float[3];
		 ...
		 byte[] redChannelsBuffer = new byte[512*512*512*4];
		 
		 greenChannelsImage = new ModelImage(ModelImage.ARGB, extents,"greenChannelsImage-Gxreg-Gy-Gcomp");
		 float[] greenChannelsImageResols = new float[3];
		 ...
		 byte[] greenChannelsBuffer = new byte[512*512*512*4];
        ...
        for(int i=0;i<redChannelsBuffer.length;i=i+4){
		 ...
		 String redImageFileName = redChannelsImage.getImageName();
		 ...
		 String greenImageFileName = greenChannelsImage.getImageName();
		 opts = new FileWriteOptions(true);
	     opts.setFileType(FileUtility.ICS);
	     opts.setFileDirectory(dir);
