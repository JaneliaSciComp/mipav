...
public class PlugInAlgorithmKidneySegmentation extends AlgorithmBase {
    ...
    private void calc3D() {
        ...
        for (i = 0; i < 360; i++) {
            ...
            ptArray[i].x = (float)(m10 + costh*xp + sinth*yp);
            ptArray[i].y = (float)(m01 - sinth*xp + costh*yp);
        ...
        spinePt.x = (float)(m10 + 0.12*sinth*b);
        spinePt.y = (float)(m01 + 0.12*costh*b);
        ...
        // In Automated Segmentation Method of Kidney Using Statistical Information 2 rectangles are used to localize the kidneys.  The rectangles
        // go from 1/9 of the major axis from the end to 2/5 of the major axis to the end and from the bottom of the minor axis to 2/3 above the 
        // bottom of the minor axis
        // So let's use (1/9 + 2/5)/2 from major axis end = (23/90)*major axis = (23/45)*a from major axis end as ellipse center and (1 - 1/3)/2 * b
        // = (1/3)*b below major axis as first guess for confining ellipses.   Make ellipses with short axis length of 0.4*majorAxis,
        // long axis length of 0.6*minorAxis, and 60 degrees tilt.
        // Then experiment and adjust
        // x left kidney center = m10 - (20/45)*a*cos(theta) + (7.0/30.0)*b*sin(theta)
        // x right kidney center = m10 + (20/45)*a*cos(theta) + (7.0/30.0)*b*sin(theta)
        // y left kidney center = m01 + (20/45)*a*sin(theta) + (7.0/30.0)*b*cos(theta)
        // y right kidney center = m01 - (20/45)*a*sin(theta) + (7.0/30.0)*b*cos(theta)
