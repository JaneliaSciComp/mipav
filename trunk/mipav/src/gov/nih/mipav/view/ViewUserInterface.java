 ...
 * It keeps a list of all the image frames presently being displayed and keeps a hash table
 * of all the images (ModelImage) open in the applicaiton. In addition, this class keeps a
 * reference to the main MIPAV frame  and the message frame with much of the imaging results
 * are output.
 *
...
public class ViewUserInterface
    ...
    /**
     * Message area for multi-line text output. The message area
     * has the ability to save/copy/paste the data to a file or clip board.
     * This frame contains the Global Data, Data, Logging and Debug text areas.
     * The Global and Debug text areas are common to all images (i.e. there is only
     * one). The Data and logging window are unique and associated to their respect image.
     */
    ...
    /**
     * Dialog that allows changes to the amount of heap requested of the system by the java Virtual
     * Machine for MIPAV during java start.
     */
    ...
    /**
     * Reference to the DICOM receiver that listens for DICOM formatted images
     * sent by a DICOM server.
     */
    ...
    /**
     * A list of image file names and multifile tags, set by the command line,
     * to be opened (and processed by a script, if one is used).
     */
    ...
    /**
     * Indicates whether the user is currently recording a new keyboard
     * shortcut using the shortcut editor dialog.
     */
    private boolean shortcutRecording = false;

    /**
     * Indicates the user's last choice of whether to open images as
     * multi-file (stack) or single file in the file open dialog.
     */
    private boolean lastStackFlag = false;

    /**
     * Constructs main UI frame.  Accesses the .preferences file to set up variables.  Sets up
     * DICOM hashtable for reading in DICOM files and starts the catcher, if appropriate. This
     * method cannot be called directly. To use this constructor, you must call createReference.
     * This class is a singleton, which means that only one type of this class is allowed to
     * be instantiated in a single VM.
    ...
    /**
     * This method should only be called once, and it should only be called
     * by MipavMain to during the initialization of MIPAV.
     *
     * @param args String[]
     * @return ViewUserInterface
     */
     ...
     * Starts by reading out some starting options out of the preferences file;
     * setting up the main frame; building the message frame and setting the
     * message frame into the preferences; building the menu; setting controls;
     * performing a Macintosh JDK check; initialising the DICOM dictionary;
     * setting up the message field that is used in the main frame; setting the
     * application titles; showing the main frame and acting on the starting
     * argument.
     * <p>
     * Recommendations for how to most easily perform modifications to the
     * initialisation procedure are included with the individual method comments,
     * although in most cases, it suffices to simply over-ride an individual
     * method.
     *
     * @see #initPrefsFile()
    ...
    /**
     * Create the panel containing components which show the application title initially,
     * and will later be used to show image coordinates and intensities.
     */
    ...
    /**
     * This method is a callback method. The purpose is to update the memory usage readout every one second.
     * A separate thread calls this method every one second.
     */
    ...
    /** Sets the layout of the Main Frame with the given, preferred layout
     * manager; makes the main frame resizable as given; makes this class a
     * window-listener; sets the default operation to do nothing on close;
     * and the icon is set.  The image icon is set by the preferences.
     * <p>
     * Over-riding classes can alter the
     * preferred layout most easily by over-riding this method, but calling
     * this method with a different layout and desired size-setting.  To change
     * the default close-operation with an over-riding class, call this
     * super method, then set the default close operation, rather than
     * creating a custom re-implemention of this method.
     *
     ...
     */
    protected void initSetMainFrameDefaults(LayoutManager prefLayout,
                                            boolean resize) {
    ...
    /** Performs a variety of start-up operations based on user-preferences;
     * First, it tries to read from the preferences file; if it cannot, it
     * sets the user directory.  It then follows from the preferences file:
     * <ol>
     *  <li>splash screen</li>
     ...
     *  <li>sets the TRIM</li>
     * </ol>
     * Over-riding classes should not over-ride this method unless one of these
     * intermediate operations is not desired or more operations are needed.
     * To modify the defaults used in creating a preferences file, over-ride
     * one of the called methods.
     *
    ...
    /**
     * Gets the TRIM and TRIM_FLAG values from the preferences file,
     * and if it cannot find them, it sets to some default value:
     * TRIM will be zero-point-three, and TRIM_FLAG will be true.
     * <p>
    ...
    /**
     * Method checks to verify that when running on a Machintosh, the JDK
     * version is at least 1&#0x2e;4.  It sets the
     * start up file "Info.plist" to use java1.4 if it can.
     * The check for these routines is
     * System.getProperty("os.name").indexOf("Mac"), and we simply return doing
     * nothing if the property for <tt>os.name</tt> is something different.
     * If the os.name does not contain "Mac", and the Info.plist file is found,
     *
     * "Info.plist" file to hold its startup arguments, so we need that
     * file anyway.  All exceptions caught
     * here are not propogated.
     */
    protected void initMacintoshJDKversionCheck() {
        ...
        try {
                ...
                if (answer == JOptionPane.YES_OPTION) {
                    ...
                    if (needsJavaVersionSpecifier) {
                        ...
                        MipavUtil.displayInfo(
                            "Java 1.4.1 was detected, and the Startup file was \n" + " modified to use it.\n"
                    ...
                    MipavUtil.displayWarning(
                        "This computer doesn't appear to have Java 1.4 installed.\n"
            ...
            Preferences.debug(
                "MemoryAllocation could not open a file, and caused an "
            ...
            Preferences.debug(
                "ViewUserInterface: A null pointer exception " + "was caught while "
     ...
     * For other applications, extend this method to call the super method with the appropriate title.
     *
        ...
        if (args.length == 1) {
            if (!args[0].startsWith("-")) {
                ...
                MipavUtil.displayError(
                    "To open files starting with \"-\", use the \"-i\" option.\nSee the usage message for more information.");
                printUsageAndExit();
            }
            return;
        }
    ...
    /**
     * Displays command line help information on usage to standard out and
     * then into an informational dialog box then exits the MIPAV application.
     * Help display just shows the different options, display help, load image,
     * load script, load VOI, and hide menu bar, as well as examples of use.
     */
    ...
    /**
     * Tells the progress bar to say "Loading" rather than "Opening" for images being loaded
     * if true
    ...
    /**
     * Gets the application title from the preference file and prepends to the
     * string passed into the method and displays the resultant string in the
    ...
    /**
     * Change whether the GUI should be visible.  The order is strage because we want
     * the main frame to be first on the taskbar, but also focused when the windows show up.
     ...
     * If the top frame is not an image frame, then the frame vector
     * is iterated until an image frame is found.  If none is found
     ...
     * make it unique.  The image name will be reset with the new
     * name.  Classes calling this method need to check the returned
     * key (image name) from this method.
     *
    ...
    /**
     * Register image model by adding it to the image hashtable.
     * Use the String provided as the key.  If the key is not unique
     * then the <code>CustomHashtable</code> will attempt to
     * make it unique.  The actual key used will be returned.
     *
     ...
     * Assume that the imageName is the key to the image.
     * Display an error if the image key is not found.
    ...
    /**
     * Unregister image model by removing it from the image hashtable
     * given the key to the hashtable.
     * Display an error if the image key is not found.
    ...
    /**
     * Return a registered image from the image hashtable based
     * on the name of the image.  This works as long as the image
     * was registered and the name is unique.
     *
     ...
     * @param frame  Frame to be registered with this the main UI.
     *               The zero element frame is the active image.
     *               Any new image registered is made the active window.
     */
    ...
    /**
     * Test for lax memory sizes being the same as last run in preferences:
     * displays a user-warning that the preferences &amp; LAX files disagree
     * and presents the JDialogMemoryAllocation dialog with "use preference"
     * buttons to quicken the matching process.
     * <p>
    ...
    private void checkLaxAgainstPreferences() {
        ...
        try {
            ...
            if (!Preferences.getProperty("StartingHeapSize").equals(mems[0])
                ...
                MipavUtil.displayWarning(
                    "Heap size settings in the " + "environment startup file do not match \n"
            ...
            MipavUtil.displayWarning(
                "Heap size settings in the " + "environment startup file either do not match \n"
     ...
     * <p>
     * The &quot;license.html&quot; file is read (using the
     * <code>GetPath</code> class) and displayed as HTML with a JDialogText.
     * If the file is not found, or there is a problem opening it, a notation
     * is made in the <code>Preferences.debug</code> window and is otherwise
     * ignored.  A warning box is displayed when the license dialog cannot
     * be created (and throws a <code>NullPointerException</code>).  Finally,
     * the main frame does not record this item in its list of windows, so
     * many instances of this window may be made.
     */
    ...
    public void activeImageRegistryMonitoring() {
        ...
        /*
         ...
         activeImageMonitorFrame.dispose();
         }
         */
        if (activeImageMonitorFrame != null) {
    ...
    /**
     * Display Options dialog for all mipav options (including image specific options)
     * to allow user to adjust display in one place
     */
    public void options() {
    ...
    /**
     * Sets the Preference to use XOR'ing when doing VOI operations, and updates the
     * frames so that the checkboxes will reflect this change
    ...
    /**
     * Sets the Preference to use VOI Names instead of labels, then updates all frames to reflect
     * this change with their VOIs
    ...
    /**
     * Pass the key event to the selected image frame (if one exists).
     * If not, check the shortcut table and attempt to handle it here.
     * @param e a key event generated by the user
     */
       ...
       public void actionPerformed(ActionEvent event) {
            ...
            */ else if (command.equals("BrowseDICOM")) {
               ...
               if (scriptDialog != null) {
                   MipavUtil
                       .displayError("Already recording.  You must save or exit the first script to begin a new one.");
                   return;
               }
               else {
        ...
        if (reply == JOptionPane.YES_OPTION) {
            ...
            while (names.hasMoreElements()) {
                try {
                    ...
                    // supplied name.\n" );
                    Preferences.debug(
                        "Illegal Argument Exception in " + "ViewUserInterface.windowClosing(). "
                        + "Somehow the Image list sent an incorrect name to " + "the image image hashtable. " + "\n",
                        1);
                    Preferences.debug("Bad argument.", Preferences.DEBUG_MINOR);
                }
            ...
            for (int i = 0; i < imageFrameVector.size(); i++) {
                ...
                if (object != null && object instanceof ViewJFrameImage) {
                    ( (ViewJFrameImage) object).close(); // this removes object from imageFrameVector, thus the need for the i--
                    i--;
                }
            }
     ...
     * (ie- restore other frames to their lastState).
    ...
    protected JMenu buildFileMenu() {
        ...
        fileMenu.add(ViewMenuBuilder.buildMenuItem("Open new image(A)", "OpenNewImage",
            0, this, "open.gif", true));

        fileMenu.add(ViewMenuBuilder.buildMenuItem("Open Leica series", "loadLeica",
            0, this, "open.gif", true));

        fileMenu.add(ViewMenuBuilder.buildMenuItem("Open image sequence", "openImgSeq",
            0, this, "open.gif", true));
        ...
        fileMenu.add(ViewMenuBuilder.buildMenuItem("Image browser", "Browse",
            0, this, null, true));

        fileMenu.addSeparator();

        fileMenu.add(ViewMenuBuilder.buildMenuItem("DICOM browser", "BrowseDICOM",
            0, this, null, true));

        fileMenu.add(ViewMenuBuilder.buildMenuItem("Anonymize DICOM directory", "AnonymizeDirectory",
            0, this, null, true));

        itemQueryDatabase = ViewMenuBuilder.buildMenuItem("DICOM database access", "QueryDatabase",
            0, this, "database.gif", true);

        fileMenu.add(itemQueryDatabase);

        itemDicom = ViewMenuBuilder.buildCheckBoxMenuItem("DICOM receiver on/off", "Dicom",
            this, Preferences.is(Preferences.PREF_AUTOSTART_DICOM_RECEIVER));
        ...
        menu.add(ViewMenuBuilder.buildMenuItem("Record script", "RecordScript",
                                               0, this, null, false));

        menu.add(ViewMenuBuilder.buildMenuItem("Run Script", "RunScript",
                                               0, this, null, false));
        ...
        fileMenu.add(ViewMenuBuilder.buildMenuItem("Open graph", "OpenNewGraph",
            0, this, "open.gif", true));
        ...
        fileMenu.add(ViewMenuBuilder.buildMenuItem("Convert old XML", "convertXML",
            0, this, null, true));

        fileMenu.add(ViewMenuBuilder.buildMenuItem("DCCIE image conversion", "dccieconvert",
            0, this, null, true));

        fileMenu.addSeparator();

        fileMenu.add(ViewMenuBuilder.buildMenuItem("Exit - MIPAV", "Exit",
            0, this, null, true));
    ...
    /**
     * Called by either userInterface (this) or by another actionlistener (ViewJFrameImage)
     * to build the plugins menu bar.
     *
    ...
    protected JMenu buildHelpMenu() {
        ...
        helpMenu.add(ViewMenuBuilder.buildMenuItem("About...", "About",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("MIPAV License", "License",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("About Java", "About Java",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Help Topics", "Help Topics",
            0, this, null, true));
        helpMenu.addSeparator();
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Memory Usage", "MemoryUsage",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Memory Allocation", "MemoryAdjust",
            0, this, null, true));
        helpMenu.addSeparator();
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Program Options", "Options",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Shortcut Options", "Shortcuts",
            0, this, null, true));
        helpMenu.add(ViewMenuBuilder.buildMenuItem("Image Registry Monitor", "ImageRegistryMonitor",
            0, this, null, true));

        helpMenu.addSeparator();
        itemOutput = ViewMenuBuilder.buildCheckBoxMenuItem("Show/hide Output window", "ShowOutput",
            this, Preferences.is(Preferences.PREF_SHOW_OUTPUT));
     ...
     * @param fileInfo This object contains the enough image information to build
     *                 a ModelImage with nothing inside (eg. blank image).
     */
