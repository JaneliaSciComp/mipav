...
public class JDialogACPC extends JDialogBase {
    ...
    public JDialogACPC( ViewJFrameTriImage theParentFrame, ModelImage im ) {
        ...
        if ( pt.x != Float.POSITIVE_INFINITY ) {
            ...
            ( (ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A) ).setReferenceXY(
                    ViewJComponentTriImage.SUPERIOR_EDGE, pt );
            setSuperiorEdge( pt );
        }
        if ( image.getFileInfo()[0].getFileFormat() == FileBase.AFNI ) {
        ...
        if ( pt.x != Float.POSITIVE_INFINITY ) {
            pt = frame.toDicom( pt, im );
            ( (ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A) ).setReferenceXY(
                    ViewJComponentTriImage.POSTERIOR_MARGIN, pt );
        ...
        if ( pt.x != Float.POSITIVE_INFINITY ) {
            pt = frame.toDicom( pt, im );
            ( (ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A) ).setReferenceXY(
                    ViewJComponentTriImage.INFERIOR_EDGE, pt );
            setInferiorEdge( pt );
        }
        if ( image.getFileInfo()[0].getFileFormat() == FileBase.AFNI ) {
        ...
        if ( pt.x != Float.POSITIVE_INFINITY ) {
            pt = frame.toDicom( pt, im );
            ( (ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A) ).setReferenceXY( ViewJComponentTriImage.FIRST_PT, pt );
            setFirstPt( pt );
        }
        if ( image.getFileInfo()[0].getFileFormat() == FileBase.AFNI ) {
     ...
     *   @param panel  the main panel of the
     */
    ...
    private void setSuperiorEdge( Point3Df pt ) {
        ...
        setACPCButton.setEnabled( false );
        clearACPCButton.setEnabled( true );
        if ( ( haveSuperiorEdge ) && ( havePosteriorMargin ) && ( haveInferiorEdge ) && ( haveFirstPt )
                && ( haveAnotherPt ) ) {
    ...
    private boolean convertToACPC() {
            ...
            if ( ( unit == FileInfoBase.INCHES ) || ( unit == FileInfoBase.CENTIMETERS )
                ...
                if ( orgResol[i] <= 0.0f ) {
                    MipavUtil.displayWarning(
                            "resolution[" + i + "] was recorded as " + orgResol[i] + " It is being changed to 1.0" );
                    orgResol[i] = 1.0f;
                }
        ...
        if ( ( orient[0] != FileInfoBase.ORI_R2L_TYPE ) || ( orient[1] != FileInfoBase.ORI_A2P_TYPE )
            ...
            for ( int x = 0; x < xDim; x++ ) {
                ...
                for ( int y = 0; y < yDim; y++ ) {
                    ...
                    for ( int z = 0; z < zDim; z++ ) {
                        ...
                        dicomBuffer[newX + newXDim * newY + newSliceSize * newZ] = datasetBuffer[x + xDim * y + sliceSize * z];
                    } // for (z = 0; z < zDim; z++)
                } // for (y = 0; y < yDim; y++)
            } // for (x = 0; x < xDim; x++)
            datasetBuffer = null;
            ...
            dicomBuffer = null;
            progressBar.dispose();
        } // if ((orient[0] != FileInfoBase.ORI_R2L_TYPE) || (orient[1] != FileInfoBase.ORI_A2P_TYPE) || (orient[2] != FileInfoBase.ORI_I2S_TYPE))
        ...
        /* At this point we have the new origin of Talairach coordinates in rr; new axes
         directions in alpha, beta, gamma.  Now construct the transformation between the
        ...
        // calculate the offset 3 vector bvec for forward transformation needed to place
        // the origin at the appropriate place in the image.  Subtract bvec to obtain the
        // actual coordinates.  However, our interpolation routines map from output to input
        // so we do a backward transformation and do not use bvec
        //bvec.x = alpha.x * rr.x + alpha.y * rr.y + alpha.z * rr.z;
        //bvec.y = beta.x * rr.x + beta.y * rr.y + beta.z * rr.z;
        //bvec.z = gamma.x * rr.x + gamma.y * rr.y + gamma.z * rr.z;
        //Preferences.debug("bvec = " + bvec.x + "," + bvec.y + "," + bvec.z + "\n");
        ...
        /* X = (i*oXres*T00 + j*oYres*T01 + k*oZres*T02 + T03)/iXres
         ...
         Wish to find TCenter.x,TCenter.y,TCenter.z from rr.x,rr.y,rr.z */
        ...
        /* b.set(0,0,rr.x - Mat[0][3]/orgResol[0]);
         b.set(1,0,rr.y - Mat[1][3]/orgResol[1]);
         b.set(2,0,rr.z - Mat[2][3]/orgResol[2]);
         X = Ac.solve(b);
        ...
        Preferences.debug(
                "Transformed Talairach origin coordinates = " + TCenter.x + "  " + TCenter.y + "  " + TCenter.z + "\n" );
