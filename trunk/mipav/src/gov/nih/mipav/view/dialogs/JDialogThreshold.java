...
public class JDialogThreshold
   ...
   */
  public void runFromLUTFrame(ModelImage im, float thres1, float thres2,
                              float fillV, boolean isBinary, boolean isInverse) {
      this.image = im;
  ...
  private void init() {
    ...
    String tempStr = new String("Lower limit ( " +
                                makeString(min, 6) + " - " +
                                makeString(max, 6) + " ):");
    ...
    tempStr = new String("Upper limit ([lower limit] - " +
                         makeString(max, 6) + " ).");
  ...
  private void callAlgorithm() {
    ...
    //System.err.println("Name is: " + name + " lower thresh: " + thresholds[0] + " upper thresh: " + thresholds[1]);
    ...
    if (displayLoc == NEW) {
      try {
        ...
        thresholdAlgo = new AlgorithmThresholdDual(resultImage, image,
            thresholds,
            fillValue, binaryFlag, regionFlag, isInverse);
      ...
      try {
        ...
        thresholdAlgo = new AlgorithmThresholdDual(image, thresholds, fillValue,
            binaryFlag, regionFlag, isInverse);
        // This is very important. Adding this object as a listener allows the algorithm to
        // notify this object when it has completed of failed. See algorithm performed event.
        // This is made possible by implementing AlgorithmedPerformed interface
        thresholdAlgo.addListener(this);
  ...
  //*******************************************************************
   //************************* Item Events ****************************
    //*******************************************************************
     ...
     /**
          *	Sets the fill value text box to enabled or disabled depending on if the
      *	binary checkbox is selected.
