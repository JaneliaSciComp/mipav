...
public class JDialogSnake extends JDialogBase implements AlgorithmInterface {
    ...
    */
    private void init() {
		...
		gbc.gridwidth = 1; gbc.gridheight = 1; gbc.anchor = gbc.WEST; gbc.weightx = 1; gbc.insets = new Insets(3,3,3,3);
   ...
   */
   public void actionPerformed(ActionEvent event) {
      ...
      if ( source == OKButton ) {
        ...
        if (singleSlice.isSelected())     propagationType = AlgorithmSnake.PROP_SINGLE;
       	else if (propagate.isSelected())  propagationType = AlgorithmSnake.PROP_ALL;
        ...
        if (resolutionCheckbox.isSelected())  scaleZ = scaleZ*normFactor;
        ...
        if (image.getNDims() == 2 ) {                  // source image is 2D
            ...
            try{
                // Make algorithm
                snakeAlgo = new AlgorithmSnake(image, sigmas, boundaryIterations,
                                               smoothness, srcVOI, boundaryDir);
