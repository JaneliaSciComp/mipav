 ...
 *
 */
public class JDialogUnsharpMask
            ...
            if (userInterface.isScriptRecording()) {
                ...
                if (displayLoc == NEW) {
                    ...
                    userInterface.getScriptDialog().append(userInterface.
                        ...
                        regionFlag + " " + image25D + " " + scaleX + " " + scaleY +
                        " " + scaleZ + " " + weight + "\n");
                }
                else {
                    userInterface.getScriptDialog().append(userInterface.
    ...
    private void init() {
        ...
        if (image.getNDims() == 3) { // Source image is 3D, thus show correction factor
            ...
            labelCorrected = new JLabel("      Corrected scale = " +
                                        String.valueOf(normFactor *
                Float.valueOf(textGaussZ.
                              getText()).floatValue()));
        ...
        if (image.getNDims() == 2) { // source image is 2D
            ...
            if (displayLoc == NEW) {
                try {
                    ...
                    // Make algorithm
                    unsharpMaskAlgo = new AlgorithmUnsharpMask(resultImage, image, sigmas,
                        weight, regionFlag, false);
                    ...
                    MipavUtil.displayError(
                        "Dialog Unsharp Mask: unable to allocate enough memory");
                    if (resultImage != null) {
     ...
     *	@param scale	Value to set weight to.
     */
      ...
      //*******************************************************************
       ...
       public void itemStateChanged(ItemEvent event) {
           ...
           if (source == resolutionCheckbox) {
               if (resolutionCheckbox.isSelected()) {
                   tempNum = normFactor *
                       Float.valueOf(textGaussZ.getText()).floatValue();
                   labelCorrected.setText("      Corrected scale = " +
                                          makeString(tempNum, 3));
               }
               else {
