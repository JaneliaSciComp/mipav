 ...
 *   histogram matched to the base image. Algorithms are executed in their own thread.
 *
 */
public class JDialogHistogramMatch
    extends JDialogBase
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (UI.isScriptRecording()) {
              ...
              UI.getScriptDialog().append("HistogramMatch " +
                                          UI.getScriptDialog().
                                          getVar(matchImage.getImageName()) +
                                          " " +
                                          UI.getScriptDialog().getVar(baseImage.
                                                                      getImageName()) + " ");
              if (displayLoc == NEW) {
  ...
  */
  ...
  private void callAlgorithm() {
    ...
    if (displayLoc == NEW) {
      try {
        resultImage = new ModelImage(matchImage.getType(),
                                     matchImage.getExtents(),
                                     name, matchImage.getUserInterface());

        // Make algorithm
        matchHistogramAlgo = new AlgorithmHistogramMatch(resultImage,
            matchImage, baseImage);
        ...
        if (runInSeparateThread) {
          ...
          matchHistogramAlgo.setActiveImage(isActiveImage);
        ...
        MipavUtil.displayError(
            "Dialog Histogram match: unable to allocate enough memory");
        return;
      }
    }
    else {
