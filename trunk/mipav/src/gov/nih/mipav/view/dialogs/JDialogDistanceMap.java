 ...
 *
 */
public class JDialogDistanceMap
    extends JDialogBase
      ...
      if (displayLoc == NEW) {
        try {
          ...
          distanceMapAlgo2D = new AlgorithmMorphology2D(resultImage, kernel, 0,
              AlgorithmMorphology2D.DISTANCE_MAP, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
          ...
          if (runInSeparateThread) {
            ...
            distanceMapAlgo2D.setActiveImage(isActiveImage);
          ...
          MipavUtil.displayError(
              "Dialog distance map: unable to allocate enough memory");
          if (resultImage != null) {
      ...
      if (displayLoc == NEW) {
        try {
          ...
          distanceMapAlgo3D = new AlgorithmMorphology3D(resultImage, kernel, 0,
              AlgorithmMorphology3D.DISTANCE_MAP, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
          ...
          if (runInSeparateThread) {
            ...
            distanceMapAlgo3D.setActiveImage(isActiveImage);
            if (!userInterface.isAppFrameVisible()) {
