 ...
 * @author Ruida Cheng
 */
public class JDialogLoadOtherOrientationVOIs extends JDialogBase {
    ...
    VOIVector voiAxialDicom;
    VOIVector voiSagittalDicom;
    VOIVector voiCoronalDicom;
     ...
     */
    public JDialogLoadOtherOrientationVOIs(final Frame theParentFrame) {
		...
		while (images.hasMoreElements()) {
	    	...
	    	if ( name.contains("Axial") || name.contains("ax")) {
	    		imageAxial = tempImage;
	    	} else if ( name.contains("Sagittal") || name.contains("sag")) {
	    		imageSagittal = tempImage;
	    	} else if ( name.contains("Coronal") || name.contains("cor")) {
    ...
    public void drawVOI() {
    	...
    	Vector3f centerAxial = imageAxial.getImageCenter();
    	for ( int z = 0; z < zDim; z++ ) {    		
    		Vector3f ptImg = new Vector3f(centerAxial.X, centerAxial.Y, z);
    	...
    	System.err.println("Sagittal image processing ...");
    	zDim = imageSagittal.getExtents()[2];
    	VOI voiSagittal = new VOI((short)0, "blank");
    	imageSagittal.registerVOI(voiSagittal);
    	Vector3f centerSagittal = imageSagittal.getImageCenter();
    	for ( int z = 0; z < zDim; z++ ) {    		
    		Vector3f ptImg = new Vector3f(centerSagittal.X, centerSagittal.Y, z);
			Vector3f ptDicom = new Vector3f();
	        MipavCoordinateSystems.fileToScanner(ptImg, ptDicom, imageSagittal);
	        zDicom = ptDicom.Z;
    		System.err.println("zSlice = " + z + " zDicom = " + zDicom);
    		findVOIsInDicomSpace(z, zDicom, imageSagittal, AxialVOIs, CoronalVOIs);
    	}
    	
    	// Draw Sagittal image VOI
    	System.err.println("Coronal image processing ...");
    	zDim = imageCoronal.getExtents()[2];
    	VOI voiCoronal = new VOI((short)0, "blank");
    	imageCoronal.registerVOI(voiCoronal);
    	Vector3f centerCoronal = imageCoronal.getImageCenter();
    	for ( int z = 0; z < zDim; z++ ) {    		
    		Vector3f ptImg = new Vector3f(centerCoronal.X, centerCoronal.Y, z);
			Vector3f ptDicom = new Vector3f();
	        MipavCoordinateSystems.fileToScanner(ptImg, ptDicom, imageCoronal);
	        zDicom = ptDicom.Z;
    		System.err.println("zSlice = " + z + " zDicom = " + zDicom);
    		findVOIsInDicomSpace(z, zDicom, imageCoronal, SagittalVOIs, AxialVOIs);
    	}
    ...
    public void findVOIsInDicomSpace(int zSlice, float zDicom, ModelImage image, InstanceVOI firstVOI, InstanceVOI secondVOI ) {
         ...
         for (int i = 0; i < firstVOI.myContourVector.size(); i++) {
             ...
             if ( zDicom <= (v.Z + 3.0f) && zDicom >= (v.Z - 3.0f) ) {
         ...
         // tracing the VOI boundary in clock wise order, sorted.  
         int size = pointsFind.size();
         float xMin = 999f, yMin = 999f;
         float xMax = -1f, yMax = -1f;
 		 ...
 		 if (size > 0) {
			...
			float[] zPtsZero = new float[size];
 			for (int i = 0; i < size; i++) {
 				v = pointsFind.get(i);
 				Vector3f ptImage = new Vector3f();
 				MipavCoordinateSystems.scannerToFile(v, ptImage, image);
 				xPts[i] = ptImage.X;
				yPts[i] = ptImage.Y;
				zPts[i] = zSlice;
 				// In image space now. find min, max bounding box
 				if ( ptImage.X < xMin ) {
 					xMin = ptImage.X;
 				}
 				if ( ptImage.X > xMax ) {
 			...
 			image.getVOIs().VOIAt(0).importCurve(xPts, yPts, zPts);
 			if ( true ) return;
 			/************************************************************************/
 			// get the VOI in image space
 			VOI voi = new VOI((short)0, "voi");
		    voi.importCurve(xPts, yPts, zPtsZero);
		    // get the 2D image
		    int[] extents = imageAxial.getExtents();
		    int sliceSize = extents[0] * extents[1];
		    int[] newExtents = new int[2];
		    newExtents[0] = extents[0];
		    newExtents[1] = extents[1];
		    ModelImage testImage2D = null;
		    
		    int start = zSlice * sliceSize;
		    float[] sourceBuffer = new float[sliceSize];
		    try {
		    	imageAxial.exportData(start, sliceSize, sourceBuffer);
		        testImage2D = new ModelImage(imageAxial.getType(), newExtents, "test_" + zSlice);
		        testImage2D.importData(0, sourceBuffer, true);
		        // add VOI to testImage2D
				testImage2D.registerVOI(voi);
				
				// new ViewJFrameImage(testImage2D);
				// now, crop the testImage2D with VOI bounding box.
				int[] xBounds = new int[2];
				int[] yBounds = new int[2];
				xBounds[0] = (int)xMin;   xBounds[1] = (int)xMax;
				yBounds[0] = (int)yMin;   yBounds[1] = (int)yMax;
				int[] destExtents = new int[2];
				destExtents[0] = Math.abs(xBounds[1] - xBounds[0]) + 1;
				destExtents[1] = Math.abs(yBounds[1] - yBounds[0]) + 1;
				
				ModelImage cropImage2D = new ModelImage(testImage2D.getType(), destExtents, "crop_" + zSlice);
				...
				AlgorithmAddMargins cropAlgo = new AlgorithmAddMargins(testImage2D, cropImage2D, xCrop, yCrop, zCrop);
				...
				// new ViewJFrameImage(cropImage2D);
				// Now, trace the VOI to generate the clock wise traversing path. 
	            AlgorithmProstateVOIBoundaryFinding voiSortAlgo = new AlgorithmProstateVOIBoundaryFinding(cropImage2D);
	            voiSortAlgo.run();
	            VOI result = voiSortAlgo.getResultVOI();
	            // cropImage2D.getVOIs().removeAllElements();
	            // cropImage2D.registerVOI(result);
	            // new ViewJFrameImage(cropImage2D);
	            // transform the VOI to image slice
	            Vector<VOIBase>[] vArray = result.getSortedCurves(VOIBase.ZPLANE, 1);
	            VOIBase vResult = vArray[0].get(0);
	            if ( vResult != null ) {
	            	VOIBase vTemp = (VOIBase)vResult.clone();
	            	int nPts = vTemp.size();
	            	xPts = new float[nPts];
	            	yPts = new float[nPts];
	            	zPts = new float[nPts];
	            	...
	            	for ( int u = 0; u < nPts; u++ ) {
	            		xPts[u] = xPts[u] + xBounds[0];
	            		yPts[u] = yPts[u] + yBounds[0];
	            		zPtsZero[u] = zSlice;
	            	}
	            	
	            	vTemp.importArrays(xPts, yPts, zPtsZero, nPts);
	            	
	            	VOIVector voiVectorNew = imageAxial.getVOIs();
	            	voiVectorNew.get(0).importCurve(vTemp);
	            }
