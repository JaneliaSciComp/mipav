 ...
 *
 */
public class JDialogSkeletonize
    extends JDialogBase
   ...
   *	@param flag		<code>true</code> indicates the image should also be pruned, <code>false</code> indicates the image should not be pruned.
   */
    ...
    if (image.getNDims() == 2) { // source image is 2D
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          if (pruning) {
            skeletonizeAlgo2D = new AlgorithmMorphology2D(resultImage, kernel,
                0,
                AlgorithmMorphology2D.SKELETONIZE, 0, 0, pruningPixels, 0,
                regionFlag);
          }
          else {
            skeletonizeAlgo2D = new AlgorithmMorphology2D(resultImage, kernel,
                0,
                AlgorithmMorphology2D.SKELETONIZE, 0, 0, 0, 0, regionFlag);
          }
          if (regionFlag == false)
          ...
          MipavUtil.displayError(
              "Dialog Skeletonize: unable to allocate enough memory");
          if (resultImage != null) {
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          if (pruning) {
            skeletonizeAlgo3D = new AlgorithmMorphology3D(resultImage, kernel,
                0,
                AlgorithmMorphology3D.SKELETONIZE, 0, 0, pruningPixels, 0,
                regionFlag);
          }
          else {
            skeletonizeAlgo3D = new AlgorithmMorphology3D(resultImage, kernel,
                0,
                AlgorithmMorphology3D.SKELETONIZE, 0, 0, 0, 0, regionFlag);
          }
          if (regionFlag == false)
          ...
          MipavUtil.displayError(
              "Dialog Skeletonize objects: unable to allocate enough memory");
          if (resultImage != null) {
                ...
                try {
                    ...
                    if (pruning) {
                        skeletonizeAlgo25D = new AlgorithmMorphology25D(resultImage, kernel, 0,
                                AlgorithmMorphology25D.SKELETONIZE, 0, 0, pruningPixels, 0, regionFlag);
                    }
                    else {
                        skeletonizeAlgo25D = new AlgorithmMorphology25D(resultImage, kernel, 0,
                                AlgorithmMorphology25D.SKELETONIZE, 0, 0, 0, 0, regionFlag);
                    }
                    if ( regionFlag == false ) {
