...
/**
 *  Dialog for calculating statistics of a (set of)
 *  Volumes of Interest.  User selects a Volume of Interest
 *  (or more than one) to act on, selects the statistics from
 *  a selectable checklist panel.  The User selects
 *  "Calculate"; the output from the statistics is reported
 *  in a "log-panel" and may be sent to a file.
 *
                    ...
                    ScriptableInterface
{
  ...
  public JDialogVOIStatistics(ViewUserInterface ui, VOIVector voiList) {
    ...
    everything.insertTab("VOI selection",
                         null,
                         buildVOIPanel(voiList), //we must store this panel so we can create a new listing later
                         "Choose VOIs and statistics file",
                         VOI_TAB);
    ...
    everything.insertTab("Statistics Options",
                         null,
                         statPanel,
                         "Statistic Selection",
                         STAT_TAB);

    everything.insertTab("Logging",
                         null,
                         buildLogPanel(),
                         "Output Log",
                         LOG_TAB);
   ...
   */
   ...
   /**
    * The method is called by the scriptRun().  It initializes variables in order to run the script.
    * The method should function as the constructor, however, the script calling mechanism force it
    * can not be invoked at the beginning.   So, scriptRun() call this method directly.   More work
    * need to be done for the calling mechanism in the future.
  ...
  /** creates the source panel for the VOI tab which
   *   consists of the directory line, the browse
   *   button, and a check box approving the anonymize
   *   in sub-directories.  Also includes the file-format
   *   selection for the output file.
   */
  private JPanel buildVOIPanel(VOIVector VOIlist) {
    ...
    fileSelectorPanel = new JPanelFileSelection(
        new File(image.getFileInfo( 0 ).getFileDirectory() + File.separator + image.getImageName() + ".table"),
   ...
   *   OKAY button and the Cancel button.
   */
  ...
  /**
   *   Builds a small menu with "Clear log" and "Overwrite"
   *   options.
   */
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    if (command.equals("clear log")) {
      ...
      Preferences.setProperty("OverwriteStatistics",
                              String.valueOf(overwrite.isSelected()));
    }
    ...
    if (selectedList.getModel().getSize() == 0) {
      if (noisyProcess) {
        MipavUtil.displayWarning(
            "No volumes of interest were selected!  Select a VOI.");
      }
      return false;
    }
  ...
  private void callAlgorithm() {
    ...
    //da.addTextUpdateListener(this);   // unimplemented -- meant to permit messaging between running thread and this' logging pane.
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (ui.isScriptRecording()) {
              ...
              String line = "VOIStatistics " + ui.getScriptDialog().getVar(image.getImageName()) + " " + getParameterString(" ") + "\n";;
  ...
  /** checks to see if the checklist has had
   *   any selections made to it.  If it hasn't, then
   *   the "Statistics Options" tab is brought to the front (it
   *   contains the checklist), and the warning
   *   message
   *   <i>"No statistics were selected!  Select a statistic."</i>
   *   is displayed.
  ...
  private boolean isStatisticSelectionOkay() {
    if (checkBoxPanel.getNumberSelected() == 0) {
      ...
      MipavUtil.displayWarning(
          "No statistics were selected!  Select a statistic.");
      return false;
    }
    return true;
  }
    ...
    if (calculator.isCalculatedBySlice() || calculator.isCalculatedByContour()) {
      ...
      for (int i = 0; i < VOIStatisticList.numberOfStatistics; i++) {
        if (checkBoxPanel.getSelectedList(VOIStatisticList.statisticDescription[i])) {
          ...
          if (calculator.isColor() &&
              VOIStatisticList.statisticDescription[i].indexOf("Intensity") !=-1) {
            totalCount += 2;
          }
        }
      }
      ...
      for (int i = 0; i < VOIStatisticList.numberOfStatistics; i++) {
        if ( checkBoxPanel.getSelectedList(VOIStatisticList.statisticDescription[i]) ) {
          ...
          // total count used for total # of data elemets, need to add 3 if color image and intensity related (R,G,B)
          totalCount++;
  ...
  /** places the input String at the end of the output log;
  ...
  /** Any further writes to the keyLog
   *   will be writing into a fresh, new keyLog.  Writing to the keyFile later will
  ...
  private void writeStatisticFile() {
    ...
    try {
      ...
      if (noisyProcess) {
        MipavUtil.displayError("security violation incurred while creating \"" +
                               tableDestination.getName() + "\"; \n" +
                               "is destination directory still writable?  " +
                               "Table file not written.");
      }
      Preferences.debug("security violation incurred while creating \"" +
                        tableDestination.getName() + "\";\n");
      return;
    }
    ...
    try {
      ...
      Preferences.debug("IOexception error in creating statFile!  threw "+
                         ...
                         io);
      io.printStackTrace();
      Preferences.debug("IO exception while writing VOIStatistic's \"" +
    ...
    try {
      ...
      if (noisyProcess)
        MipavUtil.displayError("error writing the logging to \"" +
                               tableDestination.getAbsolutePath() + "\""); //figure out where to store somewhere else?
    }
   ...
   */
  private File setupNew(File selected)
    throws  IOException,
            SecurityException
  {
    ...
    if (!selected.canWrite()) {
      if (!selected.createNewFile()) {
        throw new IOException("Error in creating destination.  " +
                              "Write rights maybe?");
      }
    }
    return selected;
  ...
  /** resets the volumes list to the current VOIVector.
   *   adds the highlighter to the new VOI.
   */
  ...
  /** resets the volumes list to the current VOIVector.
   *   removes the highlighter from the removed VOI.
   */
  public void removedVOI(VOIVectorEvent voiEvent) {
     ...
     * VOIs in the image. -- voiEvent.getSource() is null when 'removeAll' is
     * called.
     * getVOI is to return, specifically, the new VOI that has changed.  Since
     * all VOIs are now null, we must recognise that the VOI that is new is the
     * empty VOI.  (one of these a-ha! moments.  silly comments left undone.)
     */
  ...
  public class      JPanelAddRemoveVOI
         extends    JPanelListController
  {
    ...
    /** Checks if all super's action commands are used, and ensures that
     * the delete button removes items from listB, and that duplicate items
     * in listB are not repeated.
     */
    public void actionPerformed(ActionEvent ae) {
  ...
  /** An active listener for VOIEvents and ListSelectionEvents, this class
   * will ensure the selected state of the VOI and its name in a dialog's list
   * is the same.  It provides that when the name of the VOI is selected in a
   * dialog's list, the VOI itself is selected inside the image.  Likewise, a
   * selected VOI will highlight the name in the list of the appropriate
   * dialog.
   *
   */
  public class VOIHighlighter
      implements    ListSelectionListener,
                    VOIListener
  {
      /** Goes through the list selection event's JList source to find
       * selected VOI list items.  The selected VOIs are then instructed
       * to be &quot;active&quot;.  Any other VOIs in the list are set to be
       * not active.
       */
     ...
     * This panel has a border labelled, "Statistics Options" and contains a
     * set of radio buttons for selecting which properties about a VOI are to
     * be reported: by contour and slice, by slice, or by total VOI; an option
     * exists to display all totals, as well.  There is also an exclusion
     * selector panel.  The panel only permits programmatic setting of
     * slice-only calculation, but there is a retrieval facility available for
     * other options.
     */
    public  class JPanelStatisticsOptions
            extends JPanel
    {
        ...
        /** Creates a default view of the panel, including all options
         * displayed.  The option to calculate for the total VOI only is
         * selected.
         */
        ...
        public Float getMinimumExclude() {
            try {return excluder.getLowerBound();}
        ...
        public Float getMaximumExclude() {
            try {return excluder.getUpperBound();}
        ...
        public void setMaximumExclude(String floatValue) {
          try { excluder.setUpperBound(floatValue);}
          catch (NullPointerException noUpper) {
              return;
          }
        }
        ...
        public void setMinimumExclude(String floatValue) {
          try { excluder.setLowerBound(floatValue);}
            catch (NullPointerException noLower) {
                return;
            }
        }
    ...
    /** A dialog to permits choosing a range (positive or negative, and
     * floating-point) of values with the intention of the selection indicating
     * a range of values.  Includes &quot;Between&quot; cut-off value ranges.
     * It is coded to display the ranges as values for exclusion, but minor
     * rewriting this class to allow operation specific titles.
     */
    public class JPanelExclusionSelector
        extends JPanel
        implements  ActionListener
    {
        ...
        /** Creates an exclusion panel which has a checkbox to make the range controls
         * available, a selector to choose the range controls (&quot;Between&quot;,
         * &quot;Above&quot; and &quot;Below&quot;), and the range inputs for these
         * controls.
         */
        ...
        /** Returns the upper bound text as a number.  May be too positive for
         * some applications.
         * <p>
         * There is a side-effect in that when the permitExclusion checkbox is
         * unchecked, the upper bound returned is <CODE>null</CODE>.  This
         * means that relying on the upperbound also means relying on the
         * text fields being editable as well.
         * @return upper bound text as a Float; null is returned if the panel
         *      is not set to be used or one of the text entries is empty or
         *      not a number.
         */
        ...
        /** Returns the lower bound text as a number.  May be too negative for
         * some applications.
         * <p>
         * There is a side-effect in that when the permitExclusion checkbox is
         * unchecked, the lower bound returned is <CODE>null</CODE>.  This
         * means that relying on the upperbound also means relying on the
         * text fields being editable as well.
         * @return lower bound text as a Float; null is returned if the panel
         *      is not set to be used or one of the text entries is empty or
         *      not a number.
         */
        ...
        /** When state changes in some elements of the panel, the panel must
         * make display changes; these changes are registered here.
         * When state the state of the checkbox changes (from checked to un- or
         * vice-versa), the excluded selection's state is changed and the
         * Exlusion range property is reset.
         * <p>
         * Checks state of: <ul>
         * <li>Enables or disables the exclusion drop-down and the text boxes
         * based on the state of the checkbox</li>
         * <li>Changes the visibility of the text-boxes based on the state of
         * the exclusion dropdown; &quot;Between&quot; displays both text
         * boxes, &quot;Above&quot; only displays the lower cutoff box, and
         ...
         * It only does so if there are valid (that is, numbers and that they are
         * neither infinite nor at the maximum or minimum value.
         * @see Float.MAX_VALUE
         * @see Float.MIN_VALUE
         */
        ...
        /** Sets the fields' value and accessability based on the state of the
         * drop-down.  &quot;Above&quot; will display an uneditable
         * &quot;max&quot; value and the lesser of the two values, &quot;Below&quot;
         * will display an uneditable &quot;min&quot; value and the larger of
         * the two values and &quot;Between&quot; will display the largest
         * possible values, if the fields have not been set, or will reset the
         * fields to the stored values.
         */
        public void selectRangeInput() {
            ...
            if (excludeSelection.getSelectedItem().equals("Above")) {
                ...
                try {boundA.setText(lowerLimit.toString());}
                catch (NullPointerException noLower) {
                ...
                try {boundB.setText(upperLimit.toString());}
                catch (NullPointerException noUpper) {
                    boundB.setText(Float.toString(Float.MAX_VALUE));
                }
            }
    ...
    /** File-format options panel to select output format for the display of
     * the statistics output file.  Bordered with &quot;Output format&quot;,
     * it permits options for tab-delimited or XML-formatted output file.
     */
  public class JPanelStatisticFileFormatOptions  extends JPanel {
    ...
    /** Creates a default layout, of the radio button options  laid-out
     * vertically.
     * Currently, &quot;XML&quot; is not selectable.
     */
