...
public class JDialogDilate
   ...
   *	@param im	Source image.
   */
  public JDialogDilate(ViewUserInterface UI, ModelImage im) {
    super();
    if (im.getType() != ModelImage.BOOLEAN &&
        im.getType() != ModelImage.UBYTE &&
        im.getType() != ModelImage.USHORT) {
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (userInterface.isScriptRecording()) {
              ...
              userInterface.getScriptDialog().append("Dilate " +
              ...
              if (displayLoc == NEW) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.
                                                         ...
                                                         regionFlag + " " + iters + " " + kernel + " " + kernelSize +
                                                         " " +
                                                         do25D + "\n");
              } else {
                  userInterface.getScriptDialog().append(userInterface.
  ...
  private void init() {
    ...
    imageVOIPanel.setBorder(buildTitledBorder("Dilate"));
  ...
  private boolean setVariables() {
    ...
    float max = (float) (image.getExtents()[0] *
                         image.getFileInfo()[0].getResolutions()[0] / 5);
    //if ( max < 10 ) max = 10;
    ...
    if (image.getNDims() == 2) { // source image is 2D
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          dilateAlgo2D = new AlgorithmMorphology2D(resultImage, kernel,
              kernelSize,
              AlgorithmMorphology2D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
          ...
          MipavUtil.displayError(
              "Dialog dilate: unable to allocate enough memory");
          if (resultImage != null) {
        ...
        try {
          ...
          dilateAlgo2D = new AlgorithmMorphology2D(image, kernel, kernelSize,
              AlgorithmMorphology2D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
            dilateAlgo2D.setMask(image.generateVOIMask());
            // This is very important. Adding this object as a listener allows the algorithm to
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          dilateAlgo3D = new AlgorithmMorphology3D(resultImage, kernel,
              kernelSize,
              AlgorithmMorphology3D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
        ...
        try {
          // Make algorithm
          dilateAlgo3D = new AlgorithmMorphology3D(image, kernel, kernelSize,
              AlgorithmMorphology3D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
            dilateAlgo3D.setMask(image.generateVOIMask());
            // This is very important. Adding this object as a listener allows the algorithm to
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          dilateAlgo25D = new AlgorithmMorphology25D(resultImage, kernel,
              kernelSize,
              AlgorithmMorphology25D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
        ...
        try {
          // No need to make new image space because the user has choosen to replace the source image
          // Make the algorithm class
          dilateAlgo25D = new AlgorithmMorphology25D(image, kernel, kernelSize,
              AlgorithmMorphology25D.DILATE, iters, 0, 0, 0, regionFlag);
          if (regionFlag == false)
            dilateAlgo25D.setMask(image.generateVOIMask());
            // This is very important. Adding this object as a listener allows the algorithm to
