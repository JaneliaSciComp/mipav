...
/**
 *   Dialog to get options for running the time series registration.
 *   Cost function, interpolation, mean or center volume, etc.
 *
 *   @author Neva Cherniavsky
 *   @see    AlgorithmRegistrationTSOAR
 */
public class JDialogRegistrationTSOAR
    extends JDialogBase
  ...
  /**
   *   Creates new dialog with given parent frame and
   *   image to register.
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (UI.isScriptRecording()) {
              ...
              UI.getScriptDialog().append("RegistrationTSOAR " +
                                          UI.getScriptDialog().getVar(image.
                                                                      getImageName()) + " ");
              if (resultImage != null) {
                  UI.getScriptDialog().append(UI.getScriptDialog().getVar(resultImage.
                                                                          getImageName()) + " ");
              } else {
                  UI.getScriptDialog().append(UI.getScriptDialog().getVar(image.
                                                                          getImageName()) + " ");
              }
              UI.getScriptDialog().append(cost + " " +
                                          DOF + " " +
                                          interp + " " +
                                          reference + " " +
                                          refImageNum + " " +
   ...
   *   @param doSubsample if true graph result of translations and rotations
   */
  ...
  private void init() {
    ...
    sampleCheckBoxLevel2 = new JCheckBox(
    ...
    referenceButton = new JRadioButton("Register to reference volume(1-" +
                                       String.valueOf(image.getExtents()[3]) +
                                       ")");
    ...
    refImageNumText = new JTextField(String.valueOf( (image.getExtents()[3] / 2) +
        1), 3);
  ...
  /**
   *   Sets the variables based on what was selected in
   *   the GUI.  Returns flag indicating success.
...
//	        case 7:  interp = AlgorithmTransform.NEAREST_NEIGHBOR;	 break;
  ...
  private void callAlgorithm() {
    algoReg = new AlgorithmRegTSOAR(image, cost, DOF, interp, reference,
                                    refImageNum,
                                    sincNormalizedCrossCorrelation, doGraph,
                                    doSubsample, finalRegistrationAtLevel2);
    ...
    if (algorithm instanceof AlgorithmRegTSOAR) {
      if (algoReg.isCompleted()) {
        ...
        if (doGraph) {
          ...
          ViewJFrameGraph rotGraph = new ViewJFrameGraph(posR, rot, "Rotations",
              "Slice number", "Degrees");
          rotGraph.makeRangeSymmetric();
          rotGraph.showXYZLegends();
          rotGraph.setDefaultDirectory(image.getUserInterface().
                                       getDefaultDirectory());
          ...
          ViewJFrameGraph transGraph = new ViewJFrameGraph(posT, trans,
              "Translations",
              "Slice number", "Translations in " +
              FileInfoBase.getUnitsOfMeasureAbbrevStr(image.getFileInfo(0).
              getUnitsOfMeasure(0)));
          transGraph.makeRangeSymmetric();
          transGraph.showXYZLegends();
          transGraph.setDefaultDirectory(image.getUserInterface().
                                         getDefaultDirectory());
