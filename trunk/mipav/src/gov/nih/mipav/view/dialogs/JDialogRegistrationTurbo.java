...
/**
     *   This code is derived from the Turbo Registration code of Philippe Thevenaz,
 ...
 *
 */
public class JDialogRegistrationTurbo
    extends JDialogBase
  ...
  private void init() {
    ...
    automaticCheckBox = new JCheckBox(
        "Automatically nudge initial source landmarks");
      ...
      if (automaticCheckBox.isSelected()) {
        if (comboBoxInterp.getSelectedIndex() == 1) {
          MipavUtil.displayWarning(
              "Nearest neightbor interpolation cannot be used in automatic mode.");
          comboBoxInterp.setSelectedIndex(0);
        }
      }
    }
  ...
  private void callAlgorithm() {
    ...
    try {
        ...
        if ( (targetImage.getNDims() == 3) && (sourceImage.getNDims() == 3)) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
              ...
              for (s = 0; s < targetImage.getExtents()[2]; s++) {
                ...
                for (j = 0; j < tmpptA.length; j++) {
                  ptNum = (int) (Short.valueOf( ( (VOIPoint) curves[s].
                                                 elementAt(j)).getLabel()).
                                 shortValue()) - 1;
          ...
          for (i = 0; i < nVOIs; i++) {
            if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
              ...
              for (j = 0; j < tmpptA.length; j++) {
                ptNum = (int) (Short.valueOf( ( (VOIPoint) curves[0].elementAt(
                    j)).getLabel()).shortValue()) - 1;
                targetPt[ptNum][0] = tmpptA[j].x;
                targetPt[ptNum][1] = tmpptA[j].y;
              }
            } // if (VOIs.VOIAt(i).getCurveType() == VOI.POINT)
      ...
      resultImage[0] = new ModelImage(ModelStorageBase.FLOAT,
                                      ...
                                      " registered",
                                      ( (ViewJFrameBase) (parentFrame)).
                                      getUserInterface());

      resultImage[1] = new ModelImage(ModelStorageBase.BOOLEAN,
                                      targetImage.getExtents(),
                                      sourceImage.getImageName() +
                                      " output mask",
                                      ( (ViewJFrameBase) (parentFrame)).
      ...
      turbo = new AlgorithmRegTurbo(resultImage, targetImage, entireTarget,
                                    sourceImage,
                                    entireSource, targetPt, sourcePt,
                                    transformation,
                                    interpolation, automan);
    }
    catch (OutOfMemoryError x) {
      MipavUtil.displayError(
          "Dialog RegistrationTurbo: unable to allocate enough memory");
      if (resultImage != null) {
        for (int i = 0; i < 2; i++) {
