...
/**
 *   Dialog to call the extractSlices. This dialog will not be
 *   visible because it does not require user input at this time. It
 *   was made a dialog object to be consistent with the dialog/algorithm
 *   paradigm.
 *
 ...
 *
 */
public class JDialogExtractSlices
    extends JDialogBase
   ...
   */
  public JDialogExtractSlices(Frame theParentFrame, ModelImage image,
                              Vector eList) {
   ...
   */
  public JDialogExtractSlices(ViewUserInterface UI, ModelImage image,
                              Vector eList) {
    super(false);
    userInterface = UI;
    srcImage = image;
    ...
    try {
      ...
      if (numDestSlices == srcImage.getExtents()[2]) {
        MipavUtil.displayError(
            "Extract Slices: Cannot extract all slices from image.");
        return;
      }
      else if (numDestSlices == 0) {
        MipavUtil.displayError(
            "Extract Slices: Must select slices to extract from image.");
        return;
      }
      ...
      System.gc();
      MipavUtil.displayError(
          "Dialog Extract Slices: unable to allocate enough memory");
      return;
    }
  }
               ...
               if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
                   ...
                   for (int i = 0; i < extractList.size(); i++) {
                       ...
                       if (index == -1) {
                           ...
                           Preferences.debug("slice " +
                              (Integer.valueOf(sliceStr).intValue() + 1) +
                              " volume " +
