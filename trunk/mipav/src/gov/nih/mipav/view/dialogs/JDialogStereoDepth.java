 ...
 *   Dialog to input left and right stereo images for calculating
 *   stereo depth
 *
 */
public class JDialogStereoDepth
    extends JDialogBase
    ...
    while (names.hasMoreElements()) {
      ...
      if (!name.equals(image.getImageName())) {
        ...
        if ( (image.isColorImage() == nextImage.isColorImage()) &&
            (UI.getFrameContainingImage(nextImage) != null) &&
  ...
  private void callAlgorithm() {
    ...
    try {
      resultImage = new ModelImage(ModelImage.FLOAT, firstImage.getExtents(),
                                   name, firstImage.getUserInterface());

      // Make algorithm
      stereoDepthAlgo = new AlgorithmStereoDepth(resultImage, leftImage,
                                                 rightImage);
      ...
      MipavUtil.displayError(
          "Dialog Stereo Depth: unable to allocate enough memory");
      return;
    }
     ...
     public void algorithmPerformed(AlgorithmBase algorithm) {
       ...
       if (stereoDepthAlgo.isCompleted() == true) {
         if (UI.isScriptRecording()) {
           ...
           UI.getScriptDialog().append("Stereo Depth " +
                                       UI.getScriptDialog().
                                       getVar(leftImage.getImageName()) +
                                       " " +
                                       UI.getScriptDialog().getVar(rightImage.
               getImageName()) +
                                       " " +
