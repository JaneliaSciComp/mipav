...
public class JDialogNoise
   ...
   *  @param message        string to be displayed in dialog frame
   */
  ...
  private void init() {
    ...
    imageRange = new JLabel("Image: ( min = " + image.getMin() +
                            "  max = " + image.getMax() + " )");
      ...
      if (displayLoc == NEW) {
        try {
          ...
          if (runInSeparateThread) {
            ...
            randomAlgo.setActiveImage(isActiveImage);
          ...
          MipavUtil.displayError(
              "Dialog random: unable to allocate enough memory");
          return;
        }
      }
      else {
