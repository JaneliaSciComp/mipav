 ...
 *
 */
public class JDialogConstrainedOAR3D
    extends JDialogBase
  ...
  private JPanel translateRangePanelX, translateRangePanelY,
  ...
  private JLabel labelTranslateRangeX = new JLabel(
  ...
  // when done debugging:
  // private boolean limitTranslation = false, oneTransLimit = false;
   ...
   *	@param im	Source image.
   */
  public JDialogConstrainedOAR3D(ViewUserInterface _UI, ModelImage im) {
    super();
    UI = _UI;
    matchImage = im;
  ...
  public void scriptRun(AlgorithmScriptParser parser) throws
    ...
    try {
      ...
      setAdvancedSettings(parser.getNextInteger(), parser.getNextInteger(),
                          parser.getNextInteger());
    }
    catch (Exception e) {
      throw new IllegalArgumentException();
    }
    ...
    if (algo.isCompleted()) {
      if (UI.isScriptRecording()) {
        ...
        UI.getScriptDialog().append(UI.getScriptDialog().getVar(matchImage.
            getImageName()) + " ");
        UI.getScriptDialog().append(UI.getScriptDialog().getVar(refImage.
            getImageName()) + " ");
        ...
        if (weighted) {
          ...
          UI.getScriptDialog().append(UI.getScriptDialog().getVar(
              refWeightImage.getImageName()) + " ");
        }

        UI.getScriptDialog().append(UI.getScriptDialog().getVar(resultImage.
            getImageName()) + " ");
        UI.getScriptDialog().append(DOF + " " + interp + " " + cost + " " +
                                    rotateBeginX + " " + rotateEndX + " " +
                                    numCoarseX + " " +
                                    rotateBeginY + " " + rotateEndY + " " +
                                    numCoarseY + " "  +
                                    rotateBeginZ + " " + rotateEndZ + " " +
                                    numCoarseZ + " "  +
                                    displayTransform + " " + interp2 + " " +
                                    maxOfMinResol + " " + doSubsample + " " +
                                    fastMode + " " + calcCOG + " " +
                                    bracketBound + " " + maxIterations + " " +
                                    numMinima + "\n");
      }
    }
  }
   ...
   *	@param	numMinima
   */
  public void setAdvancedSettings(int bracketBound, int maxIterations,
                                  int numMinima) {
  ...
  private void init() {
    ...
    calcCOGCheckbox = new JCheckBox(
        "Initialize registration process by aligning the COG's.");
    ...
    // Radio buttons
    universalRotationCheckbox = new JCheckBox(
        "Apply same rotations to all dimensions.", true);
    ...
    JLabel labelCoarseX = new JLabel(
        "Number of angles to sample in sample in coarse grid: ");
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (command.equals("OK")) {
      ...
      advancedDialog = buildAdvancedDialog(bracketBound, maxIterations,
                                           numMinima,
                                           limitTranslation, oneTransLimit);
    }
  ...
  public void itemStateChanged(ItemEvent event) {
    ...
    else if ( (event.getSource() == xRRadio) ||
             (event.getSource() == yRRadio) ||
             (event.getSource() == zRRadio)) {
   ...
   */
  private JDialog buildAdvancedDialog(int bracketBound, int maxIter,
                                      int numMinima, boolean limitTrans,
                                      boolean oneTrans) {
    ...
    JLabel bracketBoundLabel = new JLabel(
        "Multiple of tolerance to bracket the minimum: ", JLabel.LEFT);
    ...
    JLabel bracketInstruct = new JLabel("Recommended values 10-60.",
                                        JLabel.RIGHT);
    ...
    JLabel maxIterationsLabel = new JLabel("Number of iterations: ",
                                           JLabel.LEFT);
    maxIterPanel.add(maxIterationsLabel, BorderLayout.WEST);
    maxIterPanel.setToolTipText(
        "Used for levelOne. Other levels are multiples of this #.");
    ...
    JLabel numMinLabel = new JLabel(
        "Number of minima from Level 8 to test at Level 4: ", JLabel.LEFT);
    numMinPanel.add(numMinLabel, BorderLayout.WEST);
    numMinPanel.setToolTipText(
        "Increasing will significantly increase processing time.");
    ...
    universalTranslationCheckbox = new JCheckBox(
        "Apply same translations limits to all dimensions.");
...
//	        case 7:  interp = AlgorithmTransform.NEAREST_NEIGHBOR;	 break;
    ...
    if (numCoarseX == 1) {
      int response = JOptionPane.showConfirmDialog(this,
          "Warning: There will only be 1 sampling.  Continue?",
  ...
  private void callAlgorithm() {
    ...
    if (weighted) {
      reg3 = new AlgorithmConstrainedOAR3D(refImage, matchImage, refWeightImage,
                                           inputWeightImage, cost, DOF, interp,
                                           rotateBeginX, rotateRangeX,
                                           rotateBeginY, rotateRangeY,
                                           rotateBeginZ, rotateRangeZ,
                                           numCoarseX, numCoarseY, numCoarseZ,
                                           transLimits, maxOfMinResol,
                                           doSubsample, fastMode, calcCOG,
                                           bracketBound, maxIterations,
                                           numMinima);
    }
    else {
      ...
      reg3 = new AlgorithmConstrainedOAR3D(refImage, matchImage, cost, DOF,
                                           interp,
                                           rotateBeginX, rotateRangeX,
                                           rotateBeginY, rotateRangeY,
                                           rotateBeginZ, rotateRangeZ,
   ...
   //************************** Algorithm Events ****************************
    //************************************************************************
     ...
     public void algorithmPerformed(AlgorithmBase algorithm) {
       ...
       ModelImage inputTempWeightImage = null;
       ModelImage refTempWeightImage = null;
       ...
       if (algorithm instanceof AlgorithmConstrainedOAR3D) {
         if (reg3.isCompleted()) {
           if (displayTransform) {
             ...
             transform = new AlgorithmTransform(matchImage, reg3.getTransform(),
                                                interp2,
           ...
           for (int m = 0; m < matchImage.getExtents()[2]; m++) {
             matchImage.getFileInfo()[m].setTransformID(FileInfoBase.
           ...
           reg3.getTransform().saveMatrix(UI.getDefaultDirectory() +
