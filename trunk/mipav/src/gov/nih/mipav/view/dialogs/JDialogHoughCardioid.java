 ...
 * binary image, where sqrt((x - x0)**2 + (y - y0)**2) = rad*(1 + cos(theta))
 */
public class JDialogHoughCardioid extends JDialogBase implements AlgorithmInterface, ItemListener, WindowListener {
    ...
    private void callAlgorithm() {
        ...
        try {
            ...
            // Make algorithm
            hAlgo = new AlgorithmHoughCardioid(resultImage, image, x0, y0, theta0, rad, numCardioids);
    ...
    private void init() {
        ...
        mainLabel = new JLabel("sqrt((x - x0)**2 + (y - y0)**2) = rad*(1 + cos(theta))");
