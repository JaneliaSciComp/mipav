  	...
  	*/
  	public JDialogTalairach(ViewJFrameTriImage theParentFrame, ModelImage im) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.ANTERIOR_PT,pt);
            setAnteriorPt(pt);
        }
        if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.POSTERIOR_PT,pt);
            setPosteriorPt(pt);
        }
        if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.SUPERIOR_PT,pt);
            setSuperiorPt(pt);
        }
        if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.INFERIOR_PT,pt);
            setInferiorPt(pt);
        }
        if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.LEFT_PT,pt);
            setLeftPt(pt);
        }
        if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
        ...
        if (pt.x != Float.POSITIVE_INFINITY) {
            ((ViewJComponentTriImage) frame.getTriImage(ViewJFrameTriImage.AXIAL_A)).setReferenceXY(ViewJComponentTriImage.RIGHT_PT,pt);
            setRightPt(pt);
        }
		pack();
		setVisible(true);
    ...
    */
	public void actionPerformed(ActionEvent event) {
      ...
      if (command.equals("setTalairach")) {
            pt = new Point3Df(frame.getSagittalComponentSlice(),
                              frame.getCoronalComponentSlice(),
    ...
    */
    private void convertToTalairach() {
        ...
        if ((orgResol[0] != orgResol[1]) || (orgResol[0] != orgResol[2])) {
            MipavUtil.displayError("Voxels are not cubic as required. Have res[0] = " +
                                    orgResol[0] + " res[1] = " + orgResol[1] + " res[2] = " +
                                    orgResol[2]);
            return;
        }
        vlength = orgResol[0];
        ...
        if ((orient[0] != FileInfoBase.ORI_R2L_TYPE) || (orient[1] != FileInfoBase.ORI_A2P_TYPE) ||
            ...
            MipavUtil.displayError("Orientation is dicom as required. Have orient[0] = " +
            orient[0] + " orient[1] = " + orient[1] + " orient[2] = " +  orient[2]);
            return;
        }
        ...
        Preferences.debug("Talairach center before Talairach conversion = " +
        ...
        if ((dist_ant/ATLAS_FRONT_TO_AC < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_ant + " Standard ATLAS_FRONT_TO_AC = " + ATLAS_FRONT_TO_AC);
            return;
        }
        ...
        if ((dist_med/ViewJFrameTriImage.ATLAS_AC_TO_PC < MIN_ALLOWED_DEVIATION) ||
            ...
            MipavUtil.displayError("Intercommissural distance outside allowed range dist = " +
            dist_med + " Standard ViewJFrameTriImage.ATLAS_AC_TO_PC = " + ViewJFrameTriImage.ATLAS_AC_TO_PC);
            return;
        }
        ...
        if ((dist_pos/ATLAS_PC_TO_BACK < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_pos + " Standard ATLAS_PC_TO_BACK = " + ATLAS_PC_TO_BACK);
            return;
        }
        ...
        if ((dist_inf/ATLAS_BOT_TO_AC < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_inf + " Standard ATLAS_BOT_TO_AC = " + ATLAS_BOT_TO_AC);
            return;
        }
        ...
        if ((dist_sup/ATLAS_AC_TO_TOP < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_sup + " Standard ATLAS_AC_TO_TOP = " + ATLAS_AC_TO_TOP);
            return;
        }
        ...
        if ((dist_lef/ATLAS_AC_TO_LAT < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_lef + " Standard ATLAS_AC_TO_LAT = " + ATLAS_AC_TO_LAT);
            return;
        }
        ...
        if ((dist_rig/ATLAS_AC_TO_LAT < MIN_ALLOWED_DEVIATION) ||
            ...
            + dist_rig + "Standard ATLAS_AC_TO_LAT = " + ATLAS_AC_TO_LAT);
            return;
        }
        ...
        /* Compute the 12 linear maps:
           ...
           These are maps from AC-PC aligned coordinates to the Talairach
           system. */
       ...
       imgBuffer = new float[bufferSize];
	    ...
	    for(int i = 0; i < oZdim; i++){
	        fileInfo[i] = new FileInfoAfni(image.getFileInfo()[0].getFileName(), image.getFileInfo()[0].getFileDirectory(), FileBase.AFNI);
        ...
        /*
        TCenterOriginal.x = TCenterNew.x * Sx + center.x + Sx*dx
        dx = (TCenter.x - ViewJFrameTriImage.ATLAS_BBOX_LAT*Sx - center.x)/Sx
        TCenterOriginal.y = TCenterNew.y * Sy + center.y + Sy*dy
        dy = (TCenter.y - ViewJFrameTriImage.ATLAS_BBOX_ANT*Sy - center.y)/Sy
        pcieOriginal.y = pcieNew.y * Sy + center.y + Sy*dy2
        dy2 = (pcie.y - (ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC)*Sy - center.y)/Sy
        */
	    ...
	    if (image.getFileInfo()[0].getFileFormat() == FileBase.AFNI) {
            ...
            progressBar = new ViewJProgressBar(afniInfo.getFileName(), "Transformation pass #"+1, 0, 100, false, null, null);
            ...
            for (int i = 0; i < 12; i++) {
                ...
                switch(y) {
                    ...
                    case M:
                        ...
                        topY[i] = (int)Math.round((ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC)/vlength);
                        rrArray[i].x = rr.x;
                        rrArray[i].y = rr.y;
                        rrArray[i].z = rr.z;
                        break;
                    case P:
                        Sy = 1.0f/scale_P;
                        Ty = (pcie.y - (ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC)*Sy - center.y)/Sy;
                        botY[i] = (int)Math.round((ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC)/vlength);
                        topY[i] = oYdim - 1;
                        rrArray[i].x = posrr.x;
                        rrArray[i].y = posrr.y;
                        rrArray[i].z = posrr.z;
                ...
                xfrm.setTranslate((TCenter.x - ViewJFrameTriImage.ATLAS_BBOX_LAT*Sx - center.x)/Sx,
                            Ty,
                            (TCenter.z - ViewJFrameTriImage.ATLAS_BBOX_INF_NEW*Sz -center.z)/Sz);
                Mat = xfrm.getMatrix();

                transformTalairachTrilinear(imgBuffer, Mat, vlength, image.getExtents()[0],
                image.getExtents()[1],image.getExtents()[2],
                oXres,oYres,oZres,oXdim,oYdim,oZdim,
                botX[i],botY[i],botZ[i],topX[i],topY[i],topZ[i], progressBar, talairachImage);
                ...
                Preferences.debug("alphaArray[" + i + "]= " + alphaArray[i].x + " " +
                                alphaArray[i].y + " " + alphaArray[i].z + "\n");
                Preferences.debug("betaArray[" + i + "]=" + betaArray[i].x + " " +
                                betaArray[i].y + " " + betaArray[i].z + "\n");
                Preferences.debug("gammaArray[" + i + "]=" + gammaArray[i].x + " " +
                                gammaArray[i].y + " " + gammaArray[i].z + "\n");
            } // for (i = 0; i < 12; i++)
           ...
           /* xfrm.identity();
            ...
            xfrm.setZoom(Sx,Sy,Sz);
            xfrm.setTranslate((TCenter.x - ViewJFrameTriImage.ATLAS_BBOX_LAT*Sx - center.x)/Sx,
                            (TCenter.y - ViewJFrameTriImage.ATLAS_BBOX_ANT*Sy - center.y)/Sy,
                            (TCenter.z - ViewJFrameTriImage.ATLAS_BBOX_INF_NEW*Sz -center.z)/Sz);
            Mat = xfrm.getMatrix();
            Matrix Ac = new Matrix(3,3);
            Ac.set(0,0,oXres*Mat[0][0]/vlength);
            Ac.set(0,1,oYres*Mat[0][1]/vlength);
            Ac.set(0,2,oZres*Mat[0][2]/vlength);
            Ac.set(1,0,oXres*Mat[1][0]/vlength);
            Ac.set(1,1,oYres*Mat[1][1]/vlength);
            Ac.set(1,2,oZres*Mat[1][2]/vlength);
            Ac.set(2,0,oXres*Mat[2][0]/vlength);
            Ac.set(2,1,oYres*Mat[2][1]/vlength);
            Ac.set(2,2,oZres*Mat[2][2]/vlength);
            Matrix b = new Matrix(3,1);
            b.set(0,0,TCenter.x - Mat[0][3]/vlength);
            b.set(1,0,TCenter.y - Mat[1][3]/vlength);
            b.set(2,0,TCenter.z - Mat[2][3]/vlength);
            Matrix X = Ac.solve(b);
            // Note that the final tranformed Talairach origin in millimeters should be at
            // (ViewJFrameTriImage.ATLAS_BBOX_LAT, ViewJFrameTriImage.ATLAS_BBOX_ANT, ViewJFrameTriImage.ATLAS_BBOX_INF_NEW) = (80,80,65)
            Preferences.debug("Non rounded T = " + X.get(0,0) + "," +
                                            X.get(1,0) + "," + X.get(2,0) + "\n");
            NTCenter.x = (int)Math.round(X.get(0,0));
            NTCenter.y = (int)Math.round(X.get(1,0));
            NTCenter.z = (int)Math.round(X.get(2,0));
            Preferences.debug("Transformed Talairach origin = " + NTCenter.x + "  " + NTCenter.y +
                            "  " + NTCenter.z + "\n");
            // Now check the location of the new transformed pcie y coordinate.  This should be at
            // (ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC) = 103
            xfrm.identity();
            xfrm.setTranslate(center.x, center.y, center.z);
            Sx = 1.0f/scale_R;
            Sy = 1.0f/scale_P;
            ...
            xfrm.setZoom(Sx,Sy,Sz);
            xfrm.setTranslate((TCenter.x - ViewJFrameTriImage.ATLAS_BBOX_LAT*Sx - center.x)/Sx,
                            (pcie.y - (ViewJFrameTriImage.ATLAS_BBOX_ANT + ViewJFrameTriImage.ATLAS_AC_TO_PC)*Sy - center.y)/Sy,
                            (TCenter.z - ViewJFrameTriImage.ATLAS_BBOX_INF_NEW*Sz -center.z)/Sz);
            Mat = xfrm.getMatrix();
            Ac = new Matrix(3,3);
            Ac.set(0,0,oXres*Mat[0][0]/vlength);
            ...
            Ac.set(2,2,oZres*Mat[2][2]/vlength);
            b = new Matrix(3,1);
            b.set(0,0,pcie.x - Mat[0][3]/vlength);
            b.set(1,0,pcie.y - Mat[1][3]/vlength);
            b.set(2,0,pcie.z - Mat[2][3]/vlength);
            X = Ac.solve(b);

            Preferences.debug("Non rounded pcie = " + X.get(0,0) + "," +
                                            X.get(1,0) + "," + X.get(2,0) + "\n");
            tpcie.x = (int)Math.round(X.get(0,0));
            tpcie.y = (int)Math.round(X.get(1,0));
            tpcie.z = (int)Math.round(X.get(2,0));
            Preferences.debug("Transformed pcie = " + tpcie.x + "  " + tpcie.y +
                            "  " + tpcie.z + "\n"); */
