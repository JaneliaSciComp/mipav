...
import gov.nih.mipav.model.file.FileProject;
...
public class JDialogSaveMergedVOIs extends JDialogBase {
	 ...
	 */
	public JLabel labelAxialVOI, labelSagittalVOI, labelCoronalVOI,
			labelPlyFile;

	/**
	 * Text field for each VOI and ply file.
	 */
	public JTextField textFieldAxialVOI, textFieldSagittalVOI,
			textFieldCoronalVOI, textFieldPlyFile;
	...
	public void writePlyFile() {
		...
		int ptSize = AxialVOIs.myContourVector.size()
				+ SagittalVOIs.myContourVector.size()
	...
	private void init() {
		...
		VOIsPanel
				.setBorder(buildTitledBorder("Choose the 3 VOIs (Axial, Sagittal, Coronal)"));
 ...
 * We assume only read XML type ( voi_coord.xsd ) voi file. 
 */
class InstanceVOI {
	...
	public boolean readXML() {
		...
		try {
			...
			saxParser.setProperty(SAXParserImpl.JAXP_SCHEMA_LANGUAGE,
					W3C_XML_SCHEMA);
			...
			saxParser.setProperty(SAXParserImpl.JAXP_SCHEMA_SOURCE, xsdURL
					.toExternalForm());
...
/**
 * Handle events generated while parsing the XML file.   This MyXMLHandler is a special one.   We just 
 * need to save the Pts from the VOI xml file into one chunk vector. We don't need to save the point 
 * coordinates into the VOI structure. 
 * 
 */
	...
	public void endElement(String namespaceURI, String localName, String qName)
		...
		if (currentKey.equals("Unique-ID")) {
			...
			try {
				...
				Preferences.debug("Point is incorrectly formatted: "
			...
			try {
				...
				Preferences.debug(
						"Error reading pt: " + nfex.toString() + "\n",
						Preferences.DEBUG_FILEIO);
			}
			...
			for (index = 0; index < contourVector.size(); index++) {
			    x[index] = ((Vector3f) contourVector.elementAt(index)).X;
			    y[index] = ((Vector3f) contourVector.elementAt(index)).Y;
			    z[index] = ((Vector3f) contourVector.elementAt(index)).Z;
			}
			 */
		}
...
class XMLErrorHandler implements ErrorHandler {
	...
	public void warning(SAXParseException spe) throws SAXException {
		Preferences.debug("Warning: " + getParseExceptionInfo(spe),
				Preferences.DEBUG_FILEIO);
	}
