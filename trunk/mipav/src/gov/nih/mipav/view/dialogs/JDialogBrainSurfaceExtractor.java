...
public class JDialogBrainSurfaceExtractor extends JDialogBase
    ...
    public JDialogBrainSurfaceExtractor( Frame theParentFrame, ModelImage im ) {
        ...
        // make the kernel encompas about 6 pixels total (diameter) (+1 to make sure that we get the last pixel inside the kernel)
        closeKernelSize = Math.max(
                im.getFileInfo( 0 ).getResolutions()[0], im.getFileInfo( 0 ).getResolutions()[1] )
                        * closeKernelPixels
                                + 1;
    ...
    public void callAlgorithm() {
        ...
        try {
            ...
            extractBrainAlgo = new AlgorithmBrainSurfaceExtractor( image, filterIterations, filterGaussianStdDev,
                    edgeKernelSize, erosion25D, erosionIterations, closeKernelSize, closeIterations,
                    showIntermediateImages, fillHoles, useSeparable, extractPaint );
    ...
    public void algorithmPerformed( AlgorithmBase algorithm ) {
        ...
        if ( algorithm instanceof AlgorithmBrainSurfaceExtractor ) {
            ...
            if ( extractBrainAlgo.isCompleted() == true ) {
                userInterface.setDataText(image.getImageName() + " volume = \t" + extractBrainAlgo.getBrainVolume() / 1000.0f + " cc\n" );
