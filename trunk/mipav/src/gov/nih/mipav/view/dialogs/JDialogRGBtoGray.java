...
public class JDialogRGBtoGray
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if ( (source == equalButton) || (source == graphicsButton) ||
        (source == userButton)) {
  ...
  /**
       *   Sets the red, green, and blue values to the given parameters.  The values
   *   represent the proportion of one color to another (i.e. they must add up to 1.0).
  ...
  /**
   *   Sets the red, green, and blue values to equal values.  The values
   *   represent the proportion of one color to another (i.e. they must add up to 1.0).
   */
  ...
  /**
       *   Sets the red, green, and blue values to computer graphic values.  The values
   *   represent the proportion of one color to another (i.e. they must add up to 1.0).
   */
  ...
  /**
       *  Sets whether threshold averaging is performed - Only include those components
  ...
  /**
   *   Sets whether intensity averaging is performed to determine which
   *   which channels should be used in calculating the gray
   */
  ...
  public void callAlgorithm() {
    ...
    if (displayLoc == NEW) {
      try {
        ...
        // Make algorithm
        RGBAlgo = new AlgorithmRGBtoGray(resultImage, imageA, redValue,
                                         greenValue,
                                         blueValue, thresholdAverage, threshold,
                                         intensityAverage);
      ...
      try {
        ...
        // Make algorithm
        RGBAlgo = new AlgorithmRGBtoGray(imageA, redValue,
                                         greenValue,
                                         blueValue, thresholdAverage, threshold,
                                         intensityAverage);
        // This is very important. Adding this object as a listener allows the algorithm to
        ...
        System.gc();
        MipavUtil.displayError(
        "Dialog RGBtoGRAY: unable to allocate enough memory");
        return;
      }
    } // displayLoc == REPLACE
  }
