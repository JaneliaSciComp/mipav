...
public class JDialogAnisotropicDiffusion extends JDialogBase
            ...
            if ( userInterface.isScriptRecording() ) {
                ...
                if ( displayLoc == NEW ) {
                    userInterface.getScriptDialog().append(
                            "AnisotropicDiffusion " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
    ...
    /**
     *	Performs some error checking, then closes dialog box when the OK button is pressed and
     *   calls the algorithm.
    ...
    private void callAlgorithm() {
        ...
        if ( image.getNDims() == 2 ) { // source image is 2D
            ...
            if ( displayLoc == NEW ) {
                ...
                try {
                    // Make algorithm
                    diffusionAlgo = new AlgorithmAnisotropicDiffusion( image, sigmas, iters, konst, regionFlag, image25D );
                    // This is very important. Adding this object as a listener allows the algorithm to
                    // notify this object when it has completed of failed. See algorithm performed event.
                    // This is made possible by implementing AlgorithmedPerformed interface
                    diffusionAlgo.addListener( this );
