 ...
 */
public class JDialogEdgeNMSuppression extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "EdgeNMSuppression " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
                    userInterface.getScriptDialog().putVar( resultImage.getImageName() );
                    userInterface.getScriptDialog().append(
                            userInterface.getScriptDialog().getVar( resultImage.getImageName() ) + " " + regionFlag
                            + " " + image25D + " " + scaleX + " " + scaleY + " " + scaleZ );
                } else {
                    userInterface.getScriptDialog().append(
    ...
    private void callAlgorithm() {
        ...
        if ( image.getNDims() == 2 ) { // source image is 2D
            ...
            /*else {
             ...
             nmSupAlgo = new AlgorithmEdgeNMSuppression(image, sigmas, regionFlag);
             // This is very important. Adding this object as a listener allows the algorithm to
             // notify this object when it has completed of failed. See algorithm performed event.
             // This is made possible by implementing AlgorithmedPerformed interface
             nmSupAlgo.addListener(this);
