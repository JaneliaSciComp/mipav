 ...
 *    all 3D DICOM images to compressed AVI (using java or native codecs) or
 *    to Tiff (multifile)</p>
...
public class JDialogConvertDicom
    extends JDialogBase
            ...
            try {
              ...
              if (testFloat > 1 || testFloat < .01) {
                MipavUtil.displayWarning(
                    "Enter a valid value for compression quality (0.01 - 1.00).");
                return false;
              }
            }
            catch (Exception ex) {
              MipavUtil.displayWarning(
                  "Please enter float value for compression quality.");
              return false;
            }
          }
