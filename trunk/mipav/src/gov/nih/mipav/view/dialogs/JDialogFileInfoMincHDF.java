...
public class JDialogFileInfoMincHDF extends JDialogScriptableBase implements ActionListener {
     ...
     */
    public static void showTags(ViewTableModel tagsModel, FileInfoMincHDF mincInfo) {
        Enumeration e;
        String name;
        FileDicomKey key;
        Object[] rowData = { "", "", "" };
        Hashtable tagsList = mincInfo.getDicomTable();
        ...
        for (ii = 0, e = tagsList.keys(); e.hasMoreElements(); ii++) {
            ...
            rowData[2] =  ((FileDicomTag) tagsList.get(key)).getValueRepresentation();
    ...
    public void displayAboutInfo(ModelImage _image, FileInfoMincHDF _info, int sIndex) {
        ...
        tagsModel.addRow(rowData);
        tagsModel.setValueAt("DICOM information", i, 1);
        i++;
        tagsModel.addRow(rowData);
        
        showTags(tagsModel, mincInfo);
