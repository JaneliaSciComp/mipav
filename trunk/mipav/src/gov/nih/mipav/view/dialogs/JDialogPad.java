...
/**
 * Creates the dialog to pad pixels with a value entered by the user around the image.
 *
 * <p>User selects:</p>
 *
 * <ol>
 ...
 *   <li>Value to pad the image</li>
 * </ol>
 *
 * <p>A new image or replacement of the old image may be selected.</p>
 */
public class JDialogPad extends JDialogScriptableBase implements AlgorithmInterface {
     ...
     */
    public JDialogPad(Frame theParentFrame, ModelImage im) {
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        ...
        if (algorithm instanceof AlgorithmCrop) {
            ...
            if ((padAlgo.isCompleted() == true) && (resultImage != null)) { // in StoreInDest; "new Image"
            ...
            } else if ((padAlgo.isCompleted() == true) && (resultImage == null)) {

                image = padAlgo.getSrcImage();
            ...
            } else if (padAlgo.isCompleted() == false) {
    ...
    /**
     * Accessor that sets the pad value.
     *
     * @param  x  Value to pad with.
     */
        ...
        if (displayLoc == NEW) {
            ...
            try {
                ...
                if (image.getNDims() == 2) {
                    ...
                    destExtents[0] = Math.abs(xBounds[1]);
                    destExtents[1] = Math.abs(yBounds[1]);
                } else if (image.getNDims() == 3) {

                    destExtents = new int[3];
                    destExtents[0] = Math.abs(xBounds[1]);
                    destExtents[1] = Math.abs(yBounds[1]);
                    destExtents[2] = Math.abs(zBounds[1]);
                ...
                resultImage = new ModelImage(image.getType(), destExtents,
                                             makeImageName(image.getImageName(), "_pad"));
                resultImage.setAll(padValue);

                padAlgo = new AlgorithmPad(resultImage, image, xBounds, yBounds, zBounds);

                // This is very important. Adding this object as a listener allows the algorithm to
                // notify this object when it has completed of failed. See algorithm performed event.
                // This is made possible by implementing AlgorithmedPerformed interface
                padAlgo.addListener(this);

                createProgressBar(image.getImageName(), padAlgo);
                ...
                if (isRunInSeparateThread()) {
                    ...
                    if (padAlgo.startMethod(Thread.MIN_PRIORITY) == false) {
                ...
                MipavUtil.displayError("Dialog Pad: unable to allocate enough memory");
            ...
            try {
                padAlgo = new AlgorithmPad(image, padValue, xBounds, yBounds, zBounds);
                padAlgo.addListener(this);
    ...
    private void init() {
        ...
        JLabel valueLabel = new JLabel("Pad value:");
        backLabel.setFont(serif12);
        backLabel.setForeground(Color.black);
        backLabel.setRequestFocusEnabled(false);
        gbc.gridwidth = 2; // GridBagConstraints.RELATIVE;
        ...
        optionPanel.add(Box.createHorizontalStrut(10));
        valueInput = new JTextField("0", 4);
        valueInput.addActionListener(this);
        MipavUtil.makeNumericsOnly(valueInput, false);
        gbc.gridwidth = GridBagConstraints.REMAINDER;
        gbl.setConstraints(valueInput, gbc);
        optionPanel.add(valueInput);
    ...
    private boolean setVariables() {
        ...
        xBounds[0] = leftSide;
        xBounds[1] = image.getExtents()[0] + leftSide + rightSide;
        yBounds[0] = topSide;
        yBounds[1] = image.getExtents()[1] + topSide + bottomSide;

        if (image.getNDims() >= 3) {
            zBounds[0] = front;
            zBounds[1] = image.getExtents()[2] + front + back;
        }
