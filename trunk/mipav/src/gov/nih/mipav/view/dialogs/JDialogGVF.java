 ...
 *   the degree of blurring in x and y dimensions. It should be noted,
 *   that the algorithms are executed in their own thread.
 *
 *       @see        AlgorithmGVF
 *
 */
public class JDialogGVF
    extends JDialogBase
   ...
   *   @param _zSlice          Slice of the image.
   */
      ...
      if (image.getNDims() == 2) { // source image is 2D
        ...
        try {
          if (checkboxDisplay.isSelected() == true) {
            ...
            gvfAlgo = new AlgorithmGVF(resultImage, image, sigmas,
                                       gvfIterations, boundaryIterations,
                                       kValue, srcVOI, do25D);
          }
          else {
            gvfAlgo = new AlgorithmGVF(null, image, sigmas,
                                       gvfIterations, boundaryIterations,
                                       kValue, srcVOI, do25D);
          }
        ...
        try {
          if (checkboxDisplay.isSelected() == true) {
            resultImage = new ModelImage(ModelStorageBase.FLOAT, extents,
                                         image.getImageName() + "_gvf",
                                         userInterface);
            // Make algorithm
            gvfAlgo = new AlgorithmGVF(resultImage, image, sigmas,
