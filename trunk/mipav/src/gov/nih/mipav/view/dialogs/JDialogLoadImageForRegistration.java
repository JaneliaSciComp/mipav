...
public class JDialogLoadImageForRegistration
    extends JDialogBase {
  ...
  public JDialogLoadImageForRegistration(ViewJFrameImage component) {
    ...
    setTitle("Load " + imageA.getImageName() +
             " and Image(B) into Registration");
    ...
    if (command.equalsIgnoreCase("frame")) {
      ...
      if (frameButton.isSelected()) {
        ...
        for (int i = uiV.size() - 1; i >= 0; i--) { // get the first image-index which has this title
          try {
            if ( ( (ViewJFrameImage) uiV.elementAt(i)).getComponentImage().
                getActiveImage().getImageName().equals(imageChooser.
                getSelectedItem())) {
              elem = i;
              break;
            }
        ...
        if (fileTFld.getText() != null &&
            !fileTFld.getText().equalsIgnoreCase("")) {
          if (directory == null) {
          ...
          /*
          int [] extentsB = new int[2];
          int [] extentsA = new int[2];

          extentsB = importImage.getExtents();
          extentsA = imageA.getExtents();

          System.out.println("First image: " + extentsA[0] + "," + extentsA[1]);
          System.out.println("Second image: " + extentsB[0] + "," + extentsB[1]);

          if (extentsA[0] != extentsB[0] || extentsA[1] != extentsB[1]) {
            //error
            MipavUtil.displayError("Chosen image does not have the same dimensions as " + imageA.getImageName());
            importImage.disposeLocal();
            importImage = null;
            return;
          }
          if (importImage.getType() != imageA.getType()) {
            MipavUtil.displayError("Images must have the same type");
            importImage.disposeLocal();
            importImage = null;
            return;
          }
