 ...
 * Goal for 1.1 is to make JDialogDICOMTagEditor a subclass of this.
 */
public class JDialogEditor extends JDialogBase {
    ...
    public static final int ANALYZE_AXIS_ORIENTATION = 12; // see JPanelEditAxisOrientation (drop down) // this is a MIPAV hack to the analyze format to overcome a difficulty of display.  (uses the unused variables)
    ...
    /*   public JDialogEditor(Object tag, String old) {
     ...
     oldPanel.add(oldValue);
     values.add(oldPanel);
     newValue = new JPanelEdit[1];
     newValue[0] = new JPanelEditDefault(old);
     newValue[0].setBorder(buildTitledBorder("New Value"));
     values.add(newValue[0]);
    ...
    /** sadly, the dialog doesn't know how to figure out what
     *   editor panels to apply to the different inputs, so
     *   old values must be sent as an array of objects.
     *   @param tag a unique identifier for this editor (use getKey())
     *   @param old[] an array of Object that is the coded value for
     *                each of the values.  The Old values will be
     *                printed out seperated by spaces.
     *   @param editorType[] an array specifying how each of the
     *                       old values are to be edited.
     *                       Ie.,
     */
    ...
    /** creates a JPanel to hold the Okay and Cancel buttons.
     *   in a FlowLayout; presets all the listeners to the buttons,
     ...
     *   <p>
     *   It might be required to subclass this method to have the seperator
     *   character be something else.
     */
    ...
    /**
     *   panel may have many seperate fields of entry for the user to
     *   speeratetly enter some values.  For instance, if the date
     *   were entered in several different fields <i>[month] [day] [year]</i>,
     *   then the return values could be the set {01, 01, 01}, instead of
     *   {jan, 01, 2001}.  The way the returns <em>is</em> implementation
     *   specific, but that this permits a panel to interperet data for
     *   the caller.
     *   @return The value of the Panel which can be in any format
     *           appropriate to the type, containing as many fields
     *           as needed to represent each distinct field of entry
     *           in the panel.
    ...
    /** Selects the panel for a given inputted edType, filling it
     *   the given string.  If the editor Type is not found,
     *   the JPanelEditDefault is returned.
