 ...
 */
public class JDialogExtractSlicesVolumes extends JDialogBase
    ...
    private void init() {
        ...
        if ( ( srcImage.getNDims() == 2 ) || ( srcImage.getExtents()[2] == 1 ) ) {
            MipavUtil.displayError(
                    "Extract Individual Slices does not make sense for single-slice (2-D)\n"
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "ExtractSlicesVolumes " + userInterface.getScriptDialog().getVar( srcImage.getImageName() ));
            ...
            if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
                ...
                if (srcImage.getNDims() == 3) {
                    Preferences.debug("from " +
                                      srcImage.getFileInfo(0).getExtents()[2]
                                      + " slice 3D " +
                    ...
                    Preferences.debug("from " +
                                      srcImage.getFileInfo(0).getExtents()[2] +
                                      " slice " +
                                      srcImage.getFileInfo(0).getExtents()[3]
                                      + " volume 4D " +
                                      srcImage.getImageName() + "\n");
                }
                ...
                if (srcImage.getNDims() == 3) {
                    ...
                    if (numExtracted > 1) {
                        Preferences.debug(numExtracted + " " +
                                          srcImage.getFileInfo(0).getExtents()[3]
                                          + " slice 3D images\n");
                    } else {
                        Preferences.debug(numExtracted + " " +
