...
public class ViewJFrameDICOMParser extends ViewImageDirectory implements WindowListener {
     ...
     */
    private void addSeriesData(FileInfoDicom fileInfo, FileInfoDicom fileInfoRef, int length) {
    ...
    private void parse(File file, boolean addStudy, boolean addSeries, FileInfoDicom fileInfoRef) {
        ...
        File[] grandparent = new File[] { file };
        ...
        try {
            ...
            while (test != null) { // If test != null then we are not at the image level.
                imageLevel = false;
          ...
          //  progressPanel.getProgressBar().setBackground(Color.DARK_GRAY);
            ...
            if (!imageLevel) {
                // progressBar = new ViewJProgressBar("Reading directory information", "Loading...", 0, 100, false,
                    ...
                    for (int i = 0; (parent != null) && (i < parent.length); i++) {
                        ...
                        if ((test != null) && !test[n].isDirectory()) {
                            ...
                            try {
                                ...
                                if (first) {
                                    imageFile = new FileDicom(test[n].getName(),
                                                              test[n].getParent() + File.separatorChar);
                                } else {
                            ...
                            if (addSeries && success) {
                                ...
                                for (int ii = 0; ii < test.length; ii++) {
                                    ...
                                    if (imageFilter.accept(test[ii]) && !test[ii].isDirectory()) {
                                        count++;
                                    }
                                }
    ...
    private class TableListener implements MouseListener {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (source.equals(studyTable)) {
                ...
                /*
                 * FileInfoDicom fileInfo = (FileInfoDicom) studyTableModel.getValueAt(     studyTable.getSelectedRow(),
                 * 5);
                 *
                 * seriesTableModel.setRowCount(0); parse((new File(fileInfo.getFileDirectory()).getParentFile()), false,
                 * true, fileInfo);
                 */
            } else if (source.equals(seriesTable)) {
                FileInfoDicom fileInfo = (FileInfoDicom) seriesTableModel.getValueAt(seriesTableSorter.modelIndex(seriesTable.getSelectedRow()),
                                                                                     6);
