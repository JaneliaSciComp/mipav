...
/**
 * GUI for DICOM Query and retreival from DICOM compliant servers. There are
 * three tabbed panels in the GUI. The first one is DICOM Query Panel. It has a
 * panel for entering the query information, a panel for setting the dates of
 * the query, a table that displays the results of the query, and a table that
 * displays the status of any move requests. The user may either double click on
 * the query table, hit the "Send Query" button, or hit the "Down" button to
 * send a query to the next level. To go up a level, the user can click the "Up"
 * button. To move an image from any level, the user can select the item to move
 * on and click the "Move Image" button. At the images level, double clicking on
 * an image will move that image. When a move request is sent, it is put in a
 * separate thread. That thread updates the message table as to the progress of
 * the move request.
 * 
 * @version 1.0
...
public class ViewJFrameDICOMQuery extends JFrame implements ActionListener, ListSelectionListener, MouseListener,
    ...
    /**
     * Builds the DICOM Query panel, one of the three tabbed Panels in the
     * DICOMQuery GUI. Inside, there are three different elements: The patient
     * information panel - patient name, ID, study dates ... The Query result
     * panel - display the result of the query The message area - display the
     * status of the query
     * 
     * @return the DICOM Query Panel which contains the patient info and query
     *         result
     */
    ...
    /**
     * Builds the host panel by calling methods to build the server and storage
     * panels.
     * 
    ...
    private JPanel buildListingPanel() {
        ...
        /*
         * String strs[] = new String[serverTable.getRowCount()]; for (int r =
         * 0; r < serverTable.getRowCount(); r++) { test =
         * (Boolean)serverTable.getValueAt(r, 0); str =
         * (String)serverTable.getValueAt(r,1); if (test.booleanValue() == true) {
         * strs[0] = str;} else if (r == 0) { strs[1] = str; } else { strs[r] =
         * str;} }
         */
    ...
    /**
     * A helper method for adding a component using GridBagLayout, so we don't
     * have to set up the x, y, width, and height over and over again.
     * 
     * @param x
    ...
    /**
     * Builds the label for the textField, try to standardize the label
     * appearance in the GUI.
     * 
     * @param s
     *            a string which encode the name of the label
     ...
     * 
     * @param s
     *            a string carried the name of the border
     ...
     * 
     * @param s
     *            The name of the radio button: today, one week, one month ...
     * @param flag
     *            Whether the radio button is active by default or not.
     * @return The constructed radio button with appropriated attributes
     *         attached.
     */
     ...
     * It contains a panel of radiobuttons, and two rows of pull down boxes to
    ...
    /**
     * Constructs the panel that contains the query result table. There are four
     * action-attached buttons to facilitate different levels of query: up,
     * down, send query, and move images. Up moves the query up a level; down
     * moves it down a level; send query sends a query; and move images moves an
     * image or a set of images at that level.
     * 
     * @return The constructed panel.
     */
    ...
    /**
     * Converts the combo box item to its corresponding integer. Year and date -
     * are numbers as they represented. Month - will be converted to integer,
     ...
     * 
     * @param date
     *            The source Calendar class which will be copied from.
     * @return The newly constructed calendar class with the same info as
     *         source.
     */
     ...
     *            The source calendar
     * @param targetC
     *            The target calendar that will copy the values of the source
     *            Calendar.
    ...
    /**
     * Sets the specified button to enabled or disabled, depending on the
     * boolean parameter.
     * 
     * @param button
     *            button to set
    ...
    /**
     * Gets all the starting date of the patient's study from the all three
     * comboboxes
     * 
    ...
    /**
     * Gets all the ending date of the patient's study from the all three
     * comboboxes
     * 
     ...
     * 
     * @param year
     *            an integer of the starting date: year
     */
     ...
     * 
     * @param month
     *            an integer of the starting date: month
     */
     ...
     * 
     * @param day
     *            an integer of the starting date: day
     */
     ...
     * @param month
     *            month of the date to check
     * @param day
     *            day of the date to check
     * @param start
     *            flag that says if this is the start date or end date
     ...
     * 
     * @param tmp
     *            the date to get the years before of
     * @param num
     ...
     * 
     * @param tmp
     *            the date to get the months before of
     * @param num
     ...
     * 
     * @param date
     *            the date to get the days before of
     * @param num
    ...
    /**
     * Constructs the helping manual in the help panel - one of the three panels
     * in the tabbed panel. Reads the help information from a file
     * "DICOMhelp.txt" that should be in the classpath. If it is not, throws up
     * an error message and displays nothing in the tabbed pane.
     */
    ...
    /**
     * Builds the panel that contains the server table and four buttons. The
     * server table consists of five columns with information the user needs to
     * query the server. The buttons are "Create", which creates a new server;
     * "Edit", which edits the currently selected server; "Delete", which
     * deletes the currently selected server; and "Set As Default", which sets
     * the current server as default. To get the data in the table, the table
     * model reads the .preferences file and parses the data. When changes are
     * made, they are saved to the .preferences file.
     * 
     * @return the server panel
     */
    ...
    /**
     * Builds the panel that contains the storage table and four buttons. The
     * storage table consists of five columns with information the user needs to
     * move images. The buttons are "Create", which creates a new destination;
     * "Edit", which edits the currently selected destination; "Delete", which
     * deletes the currently selected destination; and "Set As Default", which
     * sets the current destination as default. To get the data in the table,
     * the table model reads the .preferences file and parses the data. When
     * changes are made, they are saved to the .preferences file.
     * 
    ...
    /**
     * Construct the Query retrieval message panel. It contains a table for
    ...
    /**
     * This method sets up the information for the DICOM query depending on the
     * type, sends the query, and parses the information that the server sends
     * back. On a study query, it filters out studies before the start date or
     * after the end date.
     * 
     * @param type
     *            type of query: PATIENT, STUDY, SERIES, or IMAGE.
     */
    private void sendQuery(int type) {
        ...
        case PATIENT_LEVEL:
            setEnabled(up, false);
            dataObject = dicomQuery.setQueryStudyData(originalName(getPtName().trim()).concat("*"), getPtID().trim(),
    ...
    public void displayQueryResults(int type) {
        ...
        case SERIES_LEVEL:
            ...
            for (int i = 0; i < queryPDU.findResults.size(); i++) {
                ...
                try {
                    uids[i] = new DICOM_UID((String) results.getStr(DICOM_RTC.DD_SeriesInstanceUID), (String) results
        ...
        case IMAGE_LEVEL:
            ...
            for (int i = 0; i < queryPDU.findResults.size(); i++) {
                ...
                try {
                    uids[i] = new DICOM_UID((String) results.getStr(DICOM_RTC.DD_SeriesInstanceUID), (String) results
                            .getStr(DICOM_RTC.DD_SOPInstanceUID), (String) results
                            .getStr(DICOM_RTC.DD_StudyInstanceUID));
                } catch (OutOfMemoryError error) {
                    MipavUtil.displayError("Out of memory: ViewJFrameDICOMQuery.sendQuery");
                    return;
    ...
    /**
     * Sends a move request at the current level, setting up the data objects
     * appropriately. It runs the move request in its own thread.
     * 
     * @param type
     *            type of move request: PATIENT, STUDY, SERIES, or IMAGE.
     */
    private void sendMoveRequest(int type) {
        ...
        case SERIES_LEVEL:
            ...
            break;
        case IMAGE_LEVEL:
            dataObject = move.setMoveImageData(getPtID().trim(), studyInstanceUID, seriesInstanceUID, SOPInstanceUID);
            messageTable.updateRow();
            break;
        ...
        String[] serverInfo = DICOM_PDUService.parseServerInfo(Preferences.getProperty(Preferences
                .getDefaultServerKey()));
        String[] storageInfo = DICOM_PDUService.parseServerInfo(Preferences.getProperty(Preferences
    ...
    /**
     * Sorts the column of the table model. If reverse is true, sorts in reverse
     * order.
     * 
     * @param model
     *            table model to sort on
     * @param col
     *            column to sort on
     * @param reverse
    ...
    // ************************************************************************
    /**
     * A List Selection Listener event for changes in the tables. If this is the
     * query table, it will enable the down and move images buttons and set up
     * the UIDs for querying. If this is the server or storage table, it will
     * enable the edit, delete, and set as default buttons.
     * 
     * @param e
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals("ClearPtInfo")) {
            ...
            if (((String) (getPtName())).equals("") && ((String) (getPtID())).equals("")
                ...
                int option = JOptionPane.showConfirmDialog(this, "Query on all studies on all patients from "
    ...
    /**
     * When the user double clicks on a selection, sends a query at the next
     * query level; when the user clicks on a header, sorts the column.
     * 
     * @param e
     *            event that triggered this method
     */
    ...
    public boolean cancelPendingMoves() {
        ...
        for (i = 0; i < nRows; i++) {
            ...
            if (pending != null && (pending.trim().equals("Saving images") || pending.trim().equals("Sending request"))) {
    ...
    /**
     * Closes frame is requested. The user is notified if there are any pending
     * image move requests. The user is able to
     * 
     * @param event
     ...
     * 
     * @param dir
     *            the directory from which to build the listing
     */
    ...
    /**
     * When the user clicks the mouse out of a text field, resets the neccessary
     * variables.
     * 
     * @param event
