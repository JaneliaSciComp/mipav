...
public class Polynomial1f {
    ...
    public Float bisection(float fXMin, float fXMax, int iDigitsAccuracy) {
        ...
        if (Math.abs(fP0) <= Mathf.EPSILON) {
            return new Float(fXMin);
        }

        if (Math.abs(fP1) <= Mathf.EPSILON) {
    ...
    public Polynomial1f getDerivative() {
        ...
        if (m_iDegree > 0) {
            kDeriv = new Polynomial1f(m_iDegree - 1);
            ...
            kDeriv = new Polynomial1f(); // invalid in, invalid out
        }
    ...
    public int getRootsOnInterval(float fXMin, float fXMax, float[] afRoot, int iDigitsAccuracy) {
        ...
        Polynomial1f kDeriv = getDerivative();
        ...
        if (iNewCount > 0) {
            ...
            for (int i = 1; i < iNewCount; i++) {
                ...
                if (Math.abs(afNewRoot[i] - afNewRoot[i - 1]) > Mathf.EPSILON) {
     ...
     */
    public void mul(Polynomial1f kPoly1, Polynomial1f kPoly2) {
    ...
    public int rootsDegree3(float[] afRoot) {
        ...
        if (Math.abs(fDiscr) <= Mathf.EPSILON) {
            fDiscr = 0.0f;
        }
     ...
     */
    public void set(Polynomial1f kPoly) {
