...
import gov.nih.mipav.view.WildMagic.LibGraphics.Rendering.Bindable.Info;
    ...
    implements StreamInterface
{
    ...
    protected Info m_kInfo = null;

    public ResourceIdentifier GetIdentifier (Renderer pkUser)
    {
        if ( !m_bShared )
        {
            return super.GetIdentifier(pkUser);
        }
        if ( m_kInfo != null )
        {
            return m_kInfo.ID;
                           ...
                           ResourceIdentifier pkID)
    {
        if ( !m_bShared )
        {
            super.OnLoad(pkUser, oRelease, pkID);
        }
        else
        {
            m_kInfo = new Info();
