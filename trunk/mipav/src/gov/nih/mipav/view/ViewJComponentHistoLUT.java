...
/**
 * This class extends the ViewJComponentHLUTBase class and used to coordinate
 ...
 * Note y inversion in transfer segment because graphical origin
 ...
 * </pre>
 *
 * The transfer function ( member of the LUT class) dictates how
 * the image intensity values are converted into the LUT.
 *
...
public class ViewJComponentHistoLUT extends ViewJComponentHLUTBase {
     ...
     */
    public ViewJComponentHistoLUT( HistoLUTParent _histoFrame, ModelHistogram _histo,
            ModelLUT _lut, ModelImage _image ) {
    ...
    public void checkProximityToTransferFunction( float px, float py, boolean remove ) {
        ...
        for ( int i = 0; i < nPts - 1; i++ ) {
            ...
            y1N = (int) ( y[i + 1] + 0.5 );
            if ( MipavMath.distance( mx, xN, my, yN ) + MipavMath.distance( mx, x1N, my, y1N ) - MipavMath.distance( xN, x1N, yN, y1N ) < 0.5 ) { // Are we close to a line.
        ...
        if ( mode == ALPHA ) {
            ...
            if ( index == 0 ) {
                ...
                /*
                 ...
                 //adjust LUT
                 lut.makeGrayTransferFunctions();
                 // System.err.println("INDEX FOR LUT IS : " + (256 - ((int)my + 1)));
                 lut.makeThresholdLUT(256 - ((int)my + 1));
        ...
        if ( g != null ) {
            ...
            if ( logFlag == true ) { // add ticks and values on right side of histogram component (log)
                ...
                for ( int i = 0; i < 4; i++ ) {
                    ...
                    g.drawString( String.valueOf( (int) ( Math.exp(histogramMaxLog * number) + 0.5 ) ),
                            offsetX + 265, yLog + 6 );
                    number = number * 0.5;
                }
    ...
    public void updateCursorXPos( float _mx, float _my, int _index ) {
        ...
        if ( mode == RED ) { // Drag point of red transfer function
            ...
            if ( _index == lut.getTransferFunction().size() - 1 || _index == 0 ) {} else {
                if ( _index != 0 ) {
                    x[_index] = mx;
                }
            }
    ...
    public void updateCursor( float _mx, float _my, int _index ) {
        ...
        /*
         lut.getTransferFunction().exportArrays( x, y, z );
         nPts = lut.getTransferFunction().size();
