...
*   In addition, a 2nd image can be overlayed and an alpha value
*   can be adjusted to control the amount of each image that is
*   displayed. The user can control how the images are tiled by
*   specifing the number of row or cols, border thickness and size,
*   and magnification. The options can changed using the dialog
*   accessed via the options menu.
*
*/

public class ViewJFrameLightBox  extends ViewJFrameBase
                                 implements ItemListener {

    /** default values indicating minimum, maximum sizes are in PIXELS on a side for
     * a magnification.  this is to fix the magnification problems of very small
     * images for which the arbitrary limits of lightbox image sizes of 25%-75%
     * of the full size of an image.
     * in practice the default max will reference the side with greatest magnitude
     *  ...
     */
    ...
    * <p>the default is the number of rows is the dependent variable, and is
    * dependent apon the total number of images and the number of columns.
    */
    ...
    /**
     * The selectedImages contains the *real* slice numbers -- not
     *  the index of a slice on a page.  So the maximum number in the
     * vector will be numTotalSlices - 1.
     */
    private     Vector                  selectedImages = new Vector();


    /**
     * The selected Time Slices vector contains the real slice number and time slice number
     * in the format (slice #).(time slice #)
     * the maximum number in the vector will be numTotalSlices * t
     */
    ...
    /**
     * Value for the number of rows (number of images along the y-axis) and
     * columns (number of images along the x-axis) in the light-box.
     * <p> Note that both row and column set to 1 cannot make sense,
     * but that one or the other will be the 'dependent variable' as defined
     * by the row_dependent boolean var.
     *
     * A value of 0 means that the dependent variable will automatically be sized
     * to its maximum size that can fit nicely on the screen.
     */
     ...
     * <p>the default is the number of rows is the dependent variable, and is
     * dependent apon the total number of images and the number of columns.
     * Don't confuse the reading to be "This display is (t/f) dependent on the row."
     * It -could- be read as "The display is a column-independent (t/f) matrix".
     */
    ...
    /**
     * Current info -- this slice info refers to the *real* slice number,
     * not the page index for a slice on a page
     */
    ...
    */
    public ViewJFrameLightBox(ViewJFrameImage imgFrame, String _frameTitle,
                              ModelImage _imageA, ModelLUT _LUTa,
                              ModelImage _imageB, ModelLUT _LUTb,
                              float resX, float resY, Dimension loc,
        ...
        if (imgFrame == null || imageA == null) return;
        ...
        try {
            ...
            if (imageB != null) imageB.addImageDisplayListener(this);
        ...
        */
        ...
        /**
        *   Calculate the number of pages.  This is dependent on the
        *   number of visible slices and the number of total slices.
        *   It's possible that the last page will not be completely
        *   filled with images.
        */
        ...
        /**
        *   Calculate the size of the page Panel.  First the maximum size of
        *   the pagePanel must have been determined.  Once the maximum page
        *   panel size is determined, then the actual size may be reduced to
        *   nicely fit the current image panel size.  This will also determine
        *   how many images will fit into the page panel.
        */
        public void calcPagePanelSize()  {
            ...
            if (row_dependent == true) {
                ...
                if (tmpCols < 1) tmpCols = 1;   // force it to be valid
                ...
                int tmpRows = maxRows;
                if(gridRow < tmpRows) tmpRows = gridRow;
                ...
                int tmpRows = gridRow;
                if (tmpRows < 1) tmpRows = 1;  // force it to be valid
                ...
                int tmpCols = maxCols;
                if(gridColumn < tmpCols) tmpCols = gridColumn;
                while (tmpCols >= 1) {
                    // get width from columns first
                    tmpSize = tmpCols*(imagePanelSizeX + gridSpacing + 2*borderSize + 2*selectedBorderSize)
                                - gridSpacing;   // subtract one spacing, since spaces only occur between image panels
    ...
    /**
     *   Calculate the maximum page panel size based on the screen size,
     *   the size of the menubar and toolbars and whether the columns or
    ...
    /**
     *   Calculate the individual image panel size based on the current
    ...
    /**
     *   Builds the panels for a page and adds them to the
     *   page.
     */
    public void buildPage () throws OutOfMemoryError {
        ...
        for (int i = 0; i < numVisibleSlices; i++) {
            ...
            componentImage[i] = new ViewJComponentEditImage(this, imageA, LUTa, null,
                                                            imageB, LUTb, null,
                                                            null, magnification/100,
                                                            extents, false, ViewJComponentEditImage.NA,
                                                            false, imageA.getFileInfo()[0].getAxisOrientation());
            componentImage[i].setFrameControls(controls);
            componentImage[i].setLocation(borderSize + selectedBorderSize,
            ...
            if (componentImage[i].MAGR_MAG < 1)  componentImage[i].MAGR_MAG = 1;
    ...
    /**
     *   Loads the currentPage images
     *   see loadPage(int page)
     */
    ...
    /**
     *   Loads the images for a page into component images and then loads the
     *   page.
     */
    ...
    /**
     *   Setup the light box view. A scrollpane is added to the frame.
     *   A panel with a grid layout is added to the scrollpane. Bordered panels
     *   the size of the image are added to each position of the grid. Lastly
     *                   image slices are added to each bordered panel.
    ...
    private void setupLightBox(ModelLUT LUTa, ModelLUT LUTb) {
        ...
        try {
            ...
            frameHeight = pagePanelSizeY + getInsets().top  + getInsets().bottom
                + getJMenuBar().getHeight() + toolbarPanel.getHeight();
     ...
     */
    public int      getGridColumn()                { return gridColumn;  }

    /**
     *   Returns the number of images in the volume.
     *   @return           the number of image (slices).
     */
    public int      getNumTotalSlices()           { return numTotalSlices; }
    ...
    /**
     *    Changes the layout of the light box (i.e. row or col)
     *        1. reset the layout
     *        2. reset the size of panel and pane
     *        3. resize accordingly
     *
     *   This will work for simple layout updates --- where the new page size
     *   can fit on the screen.  If the number of pages decrease, then
    ...
    private void updateLayout() {
        ...
        try {
            ...
            frameWidth  = pagePanelSizeX  + getInsets().left + getInsets().right;
            frameHeight = pagePanelSizeY  + getInsets().top  + getInsets().bottom
                + menuBar.getHeight() + toolbarPanel.getHeight();
            setSize(frameWidth, frameHeight);
    ...
    /**
     *   This method creates the compound image border array for each image in
     *   the light box.  The outer border is a LineBorder that uses the border
     *   color defined in the settings.  The inner border is also a Line Border, however
     *   it's color depends on whether an image is selected or not (see the selectedImages
     *   vector).
    ...
    /**
     *   This method is called when there is a change to the light box row number
     *   and then updates light box. Verifies that the width value is within bounds row value.
     *   If width is outside of bounds, method fails quietly.
     *        1. recalculate the columns of the light box
     *        2. recalculate the size of the scrollPanel and scrollPane
     *        3. change the layout accordingly
    ...
    /**
     *   This method is called when there is a change to the light box setting
     *   for rows, columns, or row_dependency
    ...
    /**
     *   This method is called when there is a change to the light box column number
     *   verifies that the width value is within bounds.
     *   If width is outside of bounds, method fails quietly.
     *        1. recalculate the rows of the light box
     *        2. recalculate the size of the scrollPanel and scrollPane
     *        3. change the layout accordingly
    ...
    /**
     *   Set the distance between adjacent images and updates light box.
     *   verifies that the width value is within bounds.
     *   If width is outside of bounds, method fails quietly.
    ...
    public void updateBorderSize(int size) {
        if ((size < 0) || (size > MAX_GRID_BORDER)) return;
        if (size == borderSize) return;
        borderSize = size;
    ...
    public void updateBorderColor (Color col) {
        if (borderColor.equals(col) && !singleTSlice) return;
    ...
    /**
     *  This method is called when there is a change to the light box row
     *  number--in the limiting case.  It ensures that the row value is within bounds.
     *   @param row  the number of rows to be used to display the images
     */
    ...
    /**
     *   This method is called when there is a change to the
     *   light box column number--in the limiting case.  It
     *   ensures that the column value is within bounds.
     *   @param row  the number of columns to be used to display the images
     */
    ...
    /**
     *   Sets the distance between adjacent images.
     *                       verifies that the width value is within bounds.
     *                       If width is outside of bounds, method fails quietly.
     *   @param width  the width of space between images.
     */
    ...
    public void setSelectedBorderSize(int size) {
        ...
        if (size == selectedBorderSize) return;
        selectedBorderSize = size;
    }


    /** Sets the maximum magnification to the image.
     *   will ensure that the current magnification is not
     *   larger than the maximum.
     */
    protected void setMagMax() {
        magMax = ((float)this.DEFAULT_IMAGE_MAX)/Math.max(imageA.getExtents()[0], imageA.getExtents()[1]) * 100;
        if (magnification > magMax) {magnification = magMax;}
    }

    /** Sets the minimum magnification to the image.
     *   will ensure that the current magnification is not
     *   smaller than the minimum.
     */
    protected void setMagMin() {
        magMin = ((float)this.DEFAULT_IMAGE_MIN)/Math.max(imageA.getExtents()[0], imageA.getExtents()[1]) * 100;
        if (magnification < magMin) {magnification = magMin;}
    }

    /**
     *   Sets the magnification of the images
     *   will ensure that the current magnification is not
     *   larger than the maximum.
     *   @param num        number between magMin and magMax
     */
    ...
    public void setGridColor (Color col) {
        if (gridColor != null && gridColor.equals(col)) return;
        gridColor = col;
    }
    ...
    public void setBorderColor (Color col) {
        if (borderColor != null && borderColor.equals(col)) return;
        borderColor = col;
    }
    ...
    public void setSelectedBorderColor (Color col) {
        if (selectedBorderColor != null && selectedBorderColor.equals(col)) return;
    ...
    /**
     * Gets the string from the preferences and verifies the upper-left-hand
     * verifies the upper-left-hand corner is within the screen boundaries.
     * So long as it is within screen boundaries, sets the frame to the given location.
    ...
    private void updateLightBoxLocation(String ulCorner) {
        ...
        try {
            if (corner.countTokens() != 2) {
                ...
                if ( ((horiz < -9) || (vert < -9)) ||  // -1, -2 ... values are easy to do in practice.  double digits should be difficult
    ...
    /**
     *   Sets the menu and controls (i.e. toolbars) of the  main frame! This puts the menus and
     *   controls needed to controls the operations of this frame. Different
     *   image frames have different menu and controls.
     *   <p>
     *   This function no longer loads its controls into the main control window
     *   however, this method is required because this class extends ViewJFrameBase
     */
     ...
     */
    public ViewControlsImage getControls() { return null;}//controls; }
        ...
        if (displayMode == IMAGE_A){
            if (imageA.getNDims() > 3) { // Setup the title
                str    = imageA.getImageName() + "  " +
                    String.valueOf(currentTSlice+1) + "/" + String.valueOf(numTSlices) +  " M:" +
                    makeString(componentImage[0].getZoomX(),2);
                setTitle(str);
            }
            else {
     ...
     *  @return     image
     */
    public ModelImage getImageA(){ return imageA; }

    /**
     *  Returns the reference to imageB
     *  @return     imageB
     */
    public ModelImage getImageB(){ return imageB; }
    ...
    /**
     *   (Part of ViewImageUpdateInterface)
     ...
     */
    public boolean updateImageExtents(){return false;}

    /**
     *   This methods calls the componentImage's update method
     *   to repaint the screen.  This will set the requested highlight
     *   when the repaint is issued.  Use setSlice() to update a single
     *   frames highlight, as this will not set the current slice.
     *   @param slice     the slice of the image to update -- NOT the
     *                    index into the visibile slices.
     *   @param highlightFlag      requests to paint the highlight
     *                             around the component image
     *   @return       boolean confirming successful update
     *   see setSlice
     ...
     *   This needs to be called whenever a new page is loaded.
     */
    ...
    public void setTimeSlice(int tslice)  {
        ...
        if (((imageA.getNDims() <= 3) && (imageB == null)) ||
            ((imageA.getNDims() <= 3) && (imageB != null) && (imageB.getNDims() <= 3))) return;
     ...
     *  @param imageA image to set the frame to
     */
    ...
    /**
     *   Updates the image selection list by toggling the selection of
     *   the Z(3rd dimension) plane and the time (4th dimesion) volume
     *   @param plane image plane that is to displayed
     *   @param timeVolume the volume in which the plane the is to be displayed from
     *
     */
    ...
    /**
     *   Method to set the status of the delete and extract
     *   menuItems and Buttons based on the selectedImages vector.
     */
...
*/
    ...
    public void windowClosing(WindowEvent       event){
        ...
        int reply = JOptionPane.showConfirmDialog(this,
                                                  "Do you really want to close the Light Box?",
                                                  "Light Box close",
    ...
    /**
     *  When switching the active image, copy the paintBitmap of the previous
     *  active image into the paintBitmap of the new active image
     */
    ...
    private void resize() {
        ...
        height = getSize().height - getInsets().top  - getInsets().bottom
            - getJMenuBar().getSize().height - toolbarPanel.getHeight();
        ...
        int newGridRow    = (int)Math.round((double)height/(double)(imagePanelSizeY + gridSpacing + 2*borderSize + 2*selectedBorderSize));
        int newGridColumn = (int)Math.round((double)width /(double)(imagePanelSizeX + gridSpacing + 2*borderSize + 2*selectedBorderSize));
    ...
    */
    ...
    /**
     * Takes a java.awt.Color and forms a string representing its color.
     * the string appears as 6 hex digits and looks like the color coding used in
     * html files, as in: "RRGGBB".
     * @param aColor the color to be converted to a hexidecimal
     * @return   hexa-decimal string representing the 8-bit values of an RGB color,
    ...
    /**
     * Makes a series of ints, corresponding to a color string stored in the Mipav.preferences file
     * which looks like a color string defined in web pages ("RRGGBB"). and returns a
     * java.awt.Color based on those values.
     * <p>Call with extractColor(Preferences.getProperty("OneOfThoseColors"));
     * <p>if preferencesColorString is null, or incomplete, returns black.
    ...
    private Color extractColor(String preferencesColorString) {
        ...
        try {
            ...
            Preferences.debug(
                              "ViewJFrameLightBox: Color string was improper.  String was '"+preferencesColorString+"'");
    ...
    private void buildMenuBar() {
        ...
        /*  the print method cannot be used with the lightbox until a new version of
            the PrintJob class is released
            JMenuItem itemPrint = new JMenuItem("Print");
            itemPrint.addActionListener((ActionListener)this);
            itemPrint.setActionCommand("Print");
            itemPrint.setFont(font12B);
            optionMenu.add(itemPrint);
        */
    ...
    private void buildToolbar() {
        ...
        /*
        goToSlicePanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 10));
        goToSliceLabel = new JLabel ("Go to slice: ", SwingConstants.RIGHT);
        goToSliceLabel.setFont(MipavUtil.font12B);
        goToSliceLabel.setForeground(Color.black);
        goToSlicePanel.add(goToSliceLabel);

        goToSliceText = new JTextField (4);
        goToSliceText.setHorizontalAlignment(JTextField.LEFT);
        goToSliceText.addActionListener((ActionListener)this);
        goToSliceText.setActionCommand("GoToSlice");
        goToSliceText.setFont(MipavUtil.font12B);
        goToSliceText.setToolTipText("Go to slice");
        goToSlicePanel.add(goToSliceText);
        pagingTBar.add(goToSlicePanel);
        */
    ...
    /**
     *   Enable or disable the buttons on the paging toolbar and the
     *   view menu items based on the current page and the number of pages.
     */
    ...
    /**
     *  call the ExractSlices algorithm, building the dest image based on
     *   the type of the src image.  The ExtractSlices algorithm always
     *   extracts slices to a new dest image.
     */
    ...
    /**
     *  call the RemoveSlices algorithm, building the dest image based on
     *   the type of the src image.  The RemoveSlices algorithm always
     *   deletes slices from the src image, replacing the old src image.
     */
