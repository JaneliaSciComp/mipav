...
/**
*   This class is used to represent a file system in
*   a tree.  This is a node of the file system.  It
*   has a java.io.File as its user object.
*   <p>
*   Nodes in the tree are expanded by calling this
*   class's explore method.  You can set the explore
*   method to return only directories by setting the
*   directoriesOnly flag.  Additionally you can call
*   explore with a file filter, and only nodes that
*   satisfy that file filter will be returned.  Directories
*   are verified to contain valid files before they
*   are added to the tree.
*   <p>
*   Basic structure of class taken from Sun's Graphic
*   Java Swing "Trees" section.
*
...
public class ViewFileTreeNode extends DefaultMutableTreeNode {
    ...
    *   This method permits telling the node
    *   that the file is a root for the
    *   filesystem.
    *   <p>
    *   It can be useful to remember whether
    *   or not the given file was a root.
    *   (which is not information normally
    *   stored by the File.)
    *   @param node     File for tree leaf.
    ...
    */
    ...
    /**
    *   Sets the FileNode to view only directories
    *   when it goes exploring.
    *   @param directoryChildren The permission to view only directories.
    */
    public void exploreDirectoriesOnly(boolean directoryChildren) {directoriesOnly = directoryChildren;}
    ...
    */
    public boolean exploreDirectoriesOnly() {return directoriesOnly;}
    ...
    */
    ...
    /**
    *   Whether or not this node has been 'explored', and its
    *   subdirectories and file-children have been found.
    *   @return     whether or not this FileNode knows that is
    *               known.
    */
    public boolean isExplored() { return explored;}

    /**
    *   Returns <code>true</code> if this fileNode is a
    ...
    */
    ...
    /**
    *   Gets the name of the File.  When this FileNode
    *   is a FileNode for a root of the filesystem, the
    *   String returned is the absolute path (that is so
    *   on Windows systems,the user sees "C:\"); otherwise,
    *   the string returned is the filename.
    *   @return     Name of the file.
    */
    public String toString() {return (rootfile)? getAbsolutePath() : getName();}

    /**
    *   The name of the FileNode,
    *   as taken from its File.
    *   @return     The File's name.
    */
    public String getName() {return getFile().getName();}


    /** The directory of the FileNode,
    *   as taken from its File.
    *   @return     The directory's name.
    */
    public String getDirectory() {return getFile().getParent();}

    /**
    *   The absolute path of the FileNode,
    *   as taken from its File.
    *   @return     The File's name.
    */
    public String getAbsolutePath() { return getFile().getAbsolutePath();}
    ...
    *   directory or if the node has already been explored.
    *   If directories only are to be explored, then the
    *   only children to be added will be directories;
    *   otherwise, all files in the directory will be
    *   added to the node.
    *   <p>
    *   Implementation of this is different from the Sun Books' code.
    */
    ...
    /**
    *   Checks if this directory ought to be added to the list based
    *   on whether it has any children that satisfy the filter.
    ...
    */
    private boolean hasValidChildren(ViewImageFileFilter filter) {
        ...
        if (!isDirectory()) {
            if (filter.accept(getFile()))   return true;
            else                            return false;
        }
        File file = getFile();
        File children[] = file.listFiles();
        ...
        for (int i=0; i < children.length; i++) {
            ...
            if (node.hasValidChildren(filter)) return true;
        }
        return false;
    }
}
