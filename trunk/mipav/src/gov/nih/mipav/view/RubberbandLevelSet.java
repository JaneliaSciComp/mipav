...
/**
 * Does not really "rubberband" levelset but I wanted to keep the
 * same naming convention. Builds a polygon of a levelset given
 * a level and a starting position.
 *
 * @version 1.0, 6/23/99
 ...
 */
 ...
 public class RubberbandLevelSet implements
    ...
    */
    public Polygon getLevelSetPolygon() { return levelSet;}
            ...
            if ( ((ViewJComponentEditImage)(component)).isNewVoiNeeded(VOI.CONTOUR)) {
                try {
                    ...
                    */
                    ...
                    ((VOIContour)(tempContours[component.getSlice()].elementAt(0))).trimPoints(
	            ...
	            for (i = 0 ; i < nVOI; i++) {
	                if (VOIs.VOIAt(i).getID() == ((ViewJComponentEditImage)(component)).getVOI_ID()) {
	                    if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
	                        ...
	                        ((VOIContour)(VOIs.VOIAt(i).getCurves()[component.getSlice()].lastElement())).trimPoints(
	                                Preferences.getTrim(), Preferences.getTrimAdjacient());
	                    }
	                    else {
    ...
    */
    ...
    /**
    *   Takes a VOI contour from an adjacent slice and the intensity level at that
    *   slice and tries to find the equivalent level set at this slice.
    ...
    */
    public Polygon findNextLevelSet(float origLevel, Polygon polygon) {
        ...
        for (int i = 0; i < polygon.npoints; i++) {
            ...
            if (tmpLevel >= origLevel - diff && tmpLevel <= origLevel + diff) {
                ...
                if (levelSet != null) {
                    ...
                    Vector tempContours[] = newVOI.getCurves();
                    ((VOIContour)(tempContours[0].elementAt(0))).trimPoints(
                        Preferences.getTrim(), Preferences.getTrimAdjacient());
        ...
        for (int i=0; i < potentialGons.size(); i++) {
            ...
            temp = Math.abs(bounds.width - test.width) + Math.abs(bounds.height - test.height) + 2*(testLevel-level);
            if (temp < min) {
                solution = potential;
                min = temp;
                tmpLevel = testLevel;
    ...
    */
    ...
    /**
    *  Generates the possible paths of the level set and pushes them onto
    *  a stack. Looks in the 8 neighborhood directions for the possible paths.
    *  @param index   image location
    *  @param i
    *  @param xDim    direction not to be followed
    */
    private final void paths(int index, int i) {
    ...
    /**
    *  Creates a single level set. Takes a starting point and finds a closed path along
    *  the levelset back to the starting point.
    ...
    */
    private void singleLevelSet(float startPtX, float startPtY, float level) {
        ...
        if (imageBufferActive == null) return;
        ...
        if (startPtX >= xDim-1) return;
        if (startPtY >= yDim-1) return;
        ...
        do {
            ...
            if (x >= 2 && x < xDim-2 && y >= 2 && y < yDim-2) {
                ...
                if(avgPix(index-xDim) >= level &&
                    (avgPix(index-xDim+1) < level  ||
                     avgPix(index)        < level  ||
                     avgPix(index-xDim-1) < level  ||
                     avgPix(index-2*xDim) < level) &&
                     map.get(index-xDim) == false) {
                     dir = 1;
                     diff = Math.abs(avgPix(index-xDim) - avgPix(index));
                }
                if (avgPix(index-xDim+1) >= level &&
                        (avgPix(index-xDim+2)   < level ||
                         avgPix(index+1)        < level ||
                         avgPix(index-xDim)     < level ||
                         avgPix(index-2*xDim+1) < level ) &&
                ...
                if (avgPix(index+1) >= level  &&
                        (avgPix(index+2)       < level ||
                         avgPix(index+xDim+1)  < level ||
                         avgPix(index)         < level ||
                         avgPix(index-xDim+1)  < level) &&
                        map.get(index+1) == false) {
            ...
            if (distance < 2.1 && levelSetStack.size() < 10) distance = 10;
        }
        while (distance > 2.1);
    ...
    */
    private final float avgPix(int index) {
