...
/**
 *   Four lines that are the border of a slice in the surface
 *   renderer.  Color of the lines can be changed.
 */
public class ViewJComponentBoxSlice
  ...
  /**
   *   Sets verticies based on dimensions and mode.  Whichever mode
   *   this is, that dimension will be the same for all four verticies
   *   so it's in the proper plane.
   *   @param x    X dimension to use.
   *   @param y    Y dimension to use.
   *   @param z    Z dimension to use.
   *   @param mode One of X_SLICE, Y_SLICE, or Z_SLICE.
  ...
  public void setSlices(float x, float y, float z, int mode) {
    if (mode == X_SLICE) {
      // X slice bounds
         ...
         * consistent with the other slices:
         * verts[0] ---- verts[1]
         *   |             |
         *   |             |
         *   |             |
         * verts[3] ---- verts[2]
         *
         * For the X_SLICE the z values are considered to be inverted
         * (negative z is out of the screen).
         */
