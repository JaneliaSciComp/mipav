...
public class JPanelMousePlotter extends JPanelRendererBase
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals("Save")) {
            ...
            if (!text.isCancelled()) {
                ...
                MouseEventVectorPlotter vector = new MouseEventVectorPlotter(
                        text.getName(), t3D, false, parentScene.getSceneState(),
            ...
            if (index > -1) {
                // go to view
                parentScene.getSceneRootTG().setTransform(
                        ((MouseEventVectorPlotter) events.elementAt(index)).getView());
            ...
            */
        ...
        } else if (command.equals("X") || command.equals("Y")
                || command.equals("Z")) {// need to save state.  need to debug slider problems.  I think it's because stateChanged is getting
            ...
            /*
             ...
             parentScene.getSceneRootTG().getTransform(t3D);
             // store name and view together
             MouseEventVectorPlotter vector = new MouseEventVectorPlotter(command + " slice" +
             boxCount, t3D, first, parentScene.getSceneState(),
             parentScene.getMouseMode());
             if (first == true) {
             first = false;
             }
             vector.add(event);
             listModel.addElement(command + " slice" + boxCount);
             boxCount++;
             events.add(vector);
    ...
    private void init() {
        ...
        try {
            vector = new MouseEventVectorPlotter("Original view", resetTransform,
                    true, parentScene.getSceneState(),
                    ((SurfacePlotter)parentScene).getMouseMode());
    ...
    private JToolBar buildToolBar() {
        ...
        try {
            ...
            recordButton = new JToggleButton(MipavUtil.getIcon("record.gif"),
                    false);
            playButton = new JToggleButton(MipavUtil.getIcon("play.gif"), false);
            contButton = new JToggleButton(MipavUtil.getIcon("rightcont.gif"),
                    false);
            stepButton = new JToggleButton(MipavUtil.getIcon("step.gif"), false);
            pauseButton = new JToggleButton(MipavUtil.getIcon("pause.gif"),
                    false);
     ...
     *	the mouse events stored in the vector within MouseEventVectorPlotter are played.  For more
     *	information, see MouseEventVectorPlotter.
     *	@param event	Original mouse event.
     */
    public void mousePressed(MouseEvent event) {
        ...
        if (mode == RECORD_MODE) {
            ...
            try {
                ...
                eventVector = new MouseEventVectorPlotter("Mouse " + mouseCount,
                        t3d, first, parentScene.getSceneState(),
                        ((SurfacePlotter)parentScene).getMouseMode());
                if (first == true) {
                    first = false;
                }
    ...
    public void mouseReleased(MouseEvent event) {
        if (mode == RECORD_MODE && transformChange) {
            ...
            // ((MouseEventVectorPlotter)events.elementAt(current)).add(event);
    ...
    private void save()
        ...
        try {
            ...
            if (ViewUserInterface.getReference().getDefaultDirectory()
                    != null) {
                chooser.setCurrentDirectory(
                        new File(
                                ViewUserInterface.getReference().getDefaultDirectory()));
            } else {
                chooser.setCurrentDirectory(
     ...
     *	<ol>
     *	<li>As in play(), we cycle through each mouse event by going through each MouseEventVectorPlotter and
     *	looking at each mouse event stored in the hidden vector within each MouseEventVectorPlotter.</li>
    ...
    private synchronized void saveAVI()
        ...
        try {
            ...
            chooser.addChoosableFileFilter(
                    new ViewImageFileFilter(new String[] { "avi" }));
            if (((ViewJFrameBase) parentFrame).getUserInterface().getDefaultDirectory()
                    != null) {
                chooser.setCurrentDirectory(
                        new File(
                                ((ViewJFrameBase) parentFrame).getUserInterface().getDefaultDirectory()));
            } else {
                chooser.setCurrentDirectory(
                        new File(System.getProperties().getProperty("user.dir")));
            }
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            directory = String.valueOf(chooser.getCurrentDirectory())
                    + File.separatorChar;
            ((ViewJFrameBase) parentFrame).getUserInterface().setDefaultDirectory(
                    directory);
        } else {
            return;
        }
        ...
        try {
            recorderToAVI = new RecordMouse(subSample, frameRate,
                    new FileAvi(((ViewJFrameBase)
                    parentFrame).getUserInterface(), fileName, directory),
     ...
     *	in each MouseEventVectorPlotter and puts it in the events vector (and puts the name in the list).
     */
    private void load()
        throws IOException {
        String fileName;
        ...
        public void run() {
            ...
            do { // execute at least once.
                ...
                for (int i = index; i < stop; i++) {
                    ...
                    currentEventVector = (MouseEventVectorPlotter) events.elementAt(
                            i);
                    ...
                    if (vector.getName().substring(0, 5).equals("Mouse")) {
                        ...
                        parentScene.getSceneRootTG().setTransform(
                                vector.getView());
                        // ruida
                        // ( (ViewJFrameSurfacePlotter) (parentScene)).updateCubicTransform(vector.getView());
                    }
                    ...
                    for (int j = 0; j < vector.getMouseEvents().size(); j++) {
                        ...
                        long diff;
                        EventObject event = (EventObject) vector.getMouseEvents().elementAt(
                                j);

                        if (vector.getMouseEvents().elementAt(j) instanceof MouseEvent) {
                            process = 1;
                            currentObject = vector.getStateVector().elementAt(j); // NEW
                            diff = ((MouseEvent) vector.getMouseEvents().elementAt(j)).getWhen()
                                    - when;
                            if (j == 0) {
                                diff = 1;
                            }
                            newWhen = ((MouseEvent) vector.getMouseEvents().elementAt(j)).getWhen();
                            ((SurfacePlotter) parentScene).setSceneState(
                                    vector.getStateVector().elementAt(j));
                            parentScene.setGUI(
                                    vector.getStateVector().elementAt(j));
                        } else {
                            process = 2;
                        ...
                        if (firstTimePlay) {
                            ...
                            initTransform.setRotation(
                                    new AxisAngle4f(1, 1, 1, (float) .52));
                            ...
                            transformChanged(MouseBehaviorCallback.ROTATE,
                                    initTransform);
                            firstTimePlay = false;
                        }
                        ((SurfacePlotter)parentScene).dispatchSavedEvent(event);
    ...
    class RecordMouse extends Thread {
        ...
        public RecordMouse(int subSample, double frameRate, FileAvi aviFile) {
            ...
            try {
                ...
                extents[0] = xScale;
                extents[1] = yScale;
                javaImage = new ImageComponent2D(ImageComponent.FORMAT_RGB, xDim,
                        yDim); // needed to capture screen
                ...
                raster = new javax.media.j3d.Raster(new Point3f(),
                        javax.media.j3d.Raster.RASTER_COLOR, 0, 0, xDim, yDim,
                        javaImage, null);
            } catch (OutOfMemoryError e) {
                MipavUtil.displayError("Out of memory in mouse recorder.");
                return;
        ...
        public void run() {
            ...
            try {
                ...
                progress = new ViewJProgressBar("Saving as AVI", "Playing back",
            ...
            synchronized (this) {
                while (isPlaying) {
                    ...
                    if (process == -1) {} else if (process == 0) {
                        ...
                        ((SceneStatePlotter) (currentObject)).transform.set(
                                currentTransform);
                ...
                try {
                    ...
                    if (obj instanceof java.lang.String) {
                        ...
                        if (((SceneStatePlotter) (obj)).transform != null) {
                            parentScene.getSceneRootTG().setTransform(
                                    ((SceneStatePlotter) (obj)).transform);
                        }
                        ((SurfacePlotter) parentScene).setSceneState(
                                obj);
                    ...
                    MipavUtil.displayError(
                            "Error while trying to capture screen in recorder.");
                    return;
                }
                progressNum++;
            ...
            if (compression != 0) {
                ...
                if (!file2delete.exists()) {
                    System.err.println(
                            "File does not exist: cancelling transcoding");
                    return;
                }
                if (compression == 2) {
                    fileExt = ".mov";
                ...
                try {
                    ...
                    System.err.println(
                            "MalformedURLException in MouseRecorder transcoding");
                    return;
                }
                at.run();
        ...
        private void captureScreen()
            ...
            imagePix = raster.getImage().getImage().getScaledInstance(xScale,
                    yScale, Image.SCALE_REPLICATE);
            PixelGrabber pgTest = new PixelGrabber(imagePix, 0, 0, xScale,
                    yScale, pixels, 0, xScale);
