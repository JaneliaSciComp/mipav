...
public class ViewJFrameVolumeViewWildMagic extends ViewJFrameVolumeViewWM implements MouseListener, ItemListener, ChangeListener {
    ...
    /** DOCUMENT ME! */
    private GPUVolumeRenderWM raycastRenderWM;
     ...
     */
    public ViewJFrameVolumeViewWildMagic(ModelImage _imageA, ModelLUT LUTa, ModelRGB _RGBTA, ModelImage _imageB, ModelLUT LUTb,
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals("HistoLUT")) {
            ...
            clipBox.setVisible(true);
            raycastRenderWM.getClipPanel().invokeClippingPlanes();
            insertTab("Clip", clipPanel);
            insertSurfaceTab("Clip", clipPanel);
            insertRaycastTab("Clip", clipPanel);
        } else if (command.equals("DisableClipping")) {
            clipBox.setVisible(true);
            raycastRenderWM.getClipPanel().disable6Planes();
            insertTab("Clip", clipPanel);
            insertSurfaceTab("Clip", clipPanel);
            insertRaycastTab("Clip", clipPanel);
    ...
    public void buildClipPanel() {
        ...
        clipBox = raycastRenderWM.getClipPanel().getMainPanel();
    ...
    public void buildDisplayPanel() {
        ...
        displayPanel.add(raycastRenderWM.getDisplayPanel().getMainPanel());
    ...
    public void buildLightPanel() {
        ...
        lightPanel.add(raycastRenderWM.getLightPanel().getMainPanel());
    ...
    public void buildOpacityPanel() {
        ...
        gbLayout.setConstraints(raycastRenderWM.getVolOpacityPanel().getMainPanel(), gbConstraints);
        opacityPanel.add(raycastRenderWM.getVolOpacityPanel().getMainPanel());
...
//         maxPanelWidth = Math.max(raycastCameraPanel.getPreferredSize().width, maxPanelWidth);
    ...
    public void buildSculpt() {
        ...
        m_kSculptPanel.add(raycastRenderWM.getSculptorPanel().getMainPanel());
        ...
        try {
            ...
            if (isSurfaceRenderEnable) {
                ...
                */
                ...
                raycastRenderWM = new GPUVolumeRenderWM(imageA, LUTa, RGBTA, imageB, LUTb, RGBTB, this);
                TransferFunction kTransfer = raycastRenderWM.getVolOpacityPanel().getCompA().getOpacityTransferFunction();
    ...
    public int getBlendValue() {
    	JPanelVolOpacityBaseWM opacityPanel = raycastRenderWM.getVolOpacityPanel();
    ...
    public void itemStateChanged(ItemEvent event) {
        ...
        if (raycastRenderWM != null) {
            if (radioMIP.isSelected() && (source == radioMIP)) {
                ...
                updateRayTracingSteps();
                raycastRenderWM.getLightPanel().refreshLighting();
...
//                 }
        	...
        	if ((raycastRenderWM != null) && (source == raycastRenderWM.GetCanvas())) {
...
//                 }
    ...
    public void resetAxisX() {
    	...
    	Transform3D transRotation = new Transform3D();
        ...
        transRotation.setScale(1.0f);
        
        float[] data = new float[16];
        transRotation.get(data);
        raycastRenderWM.transformUpdate(data);
...
//         }
    ...
    public boolean updateImages(boolean forceShow) {
       ...
       /*
        ...
        if (raycastRenderWM != null) {
            ViewJComponentVolOpacityBaseWM kSelectedComp = raycastRenderWM.getVolOpacityPanel().getSelectedComponent();
            if ( imageB != null )
            {
                if ( kSelectedComp == raycastRenderWM.getVolOpacityPanel().getCompA() )
                {
                    TransferFunction kTransfer = raycastRenderWM.getVolOpacityPanel().getCompA().getOpacityTransferFunction();
                    raycastRenderWM.updateImages(0, kTransfer);
                }
                ...
                {
                    TransferFunction kTransfer = raycastRenderWM.getVolOpacityPanel().getCompB().getOpacityTransferFunction();
            ...
            else
            {
                ...
                {
                    TransferFunction kTransfer = raycastRenderWM.getVolOpacityPanel().getCompA_GM().getOpacityTransferFunction();
...
//         gbc.weighty = 1;
    ...
    private void resizePanel() {
        ...
        if (isSurfaceRenderEnable) {
            ...
            if (raycastRenderWM.getVolOpacityPanel() != null) {
            	raycastRenderWM.getVolOpacityPanel().resizePanel(maxPanelWidth, height);
            }
