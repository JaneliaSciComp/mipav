...
/*
 ...
 The old if statement says if the mouse is dragged and neither the right nor
 middle button are pressed .
 The new if statement says if the mouse is dragged and the right button
 and not the middle button is pressed.

 Made Additions under the mouse conditions for picking

 Added the algorithm to rotate the object by multiplying the inverse of the viewers
 rotation then applying the users rotation (just like when looking normal to XY plane)
 then multiply back in the original users rotation to put object back with the newly
 applied rotations.
        ...
        while ( criteria.hasMoreElements() ) {
            ...
            if ( wakeup instanceof WakeupOnAWTEvent ) {
                    ...
                    if ( ( ( buttonPress ) && ( ( flags & MANUAL_WAKEUP ) == 0 ) )
                            ...
                            )
                        {
                            ...
                            if ( !reset ) {
                                 ...
                                 * rotation model. Because the probe is
                                 * displayed within the object space of the 3
                                 * orthogonal slices, which are also rotated
                                 * by trackball rotation, the two trackball
                                 * transformations must be independent of each
                                 * other, so that the probe rotation works
                                 * regardless of whether it and the 3
                                 * orthogonal slices have been rotated or
                                 * not. */
                ...
                if ( (callback != null) && (iCallback != -1) ) {
                        callback.transformChanged( iCallback,
                                                   currXform );
                }
