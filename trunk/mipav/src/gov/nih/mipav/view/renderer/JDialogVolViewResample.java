...
public class JDialogVolViewResample extends JDialogBase {
    ...
    /** Radio button of the brainsurface flattener render mode in the right
     * panel. */
     ...
     * Used by RFAST to open up the RFA simulation immediately.
     */
    private String startupCommand = null;

    /**
     * Pad button is used for inserting blank images to the end of the image.  This approach creates the
     * power of 2 image.  Pad Button is only enabled when the power of 2 value is greater than
     * the slices number.
     */
    ...
    public void actionPerformed( ActionEvent event ) {
        ...
        if ( command.equals( "Resample" ) ) {
             ...
             * a type that is definted after bilinear is selected, increment
             * the m_iFilter value so it matches the AlgotithmTransform type
    ...
    public void exec() {
        try {
            ...
            if ( sr.getProbeDialog() != null ) {
                // need to update the rfa target labels in case there were attached surfaces that we should show info about
            ...
            Preferences.debug(
                    "ViewJFrameSurfaceRenderer cannot be called; encountered "
            ...
            Preferences.debug(
                    "ViewJFrameSurfaceRenderer cannot be called as there was " + "not enough memory allocated.  \n"
                    + "The error is: \n" + notEnoughError.getLocalizedMessage() );
            MipavUtil.displayError(
                    "The surface renderer requires more memory " + "than is currently available;\n"
