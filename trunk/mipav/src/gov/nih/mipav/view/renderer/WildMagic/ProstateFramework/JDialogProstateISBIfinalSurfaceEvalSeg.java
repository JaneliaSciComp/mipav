 ...
 * "EvaluateSegmentation" software takes the list to run shell script for performance measure. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstateISBIfinalSurfaceEvalSeg extends JDialogBase implements AlgorithmInterface {
	 ...
	 */
	public JDialogProstateISBIfinalSurfaceEvalSeg(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_1 = new File("/scratch/gitrepo/ProstateSeg/ISBI2017/result3D/fold4");
	...
	private void printNIItable() {
	    ...
	    for ( String orientationKey : orientationKeys ) {
	    	...
	    	for ( String hashIDKey : hashIDKeys ) {
	    		...
	    		String segName = fileNameTable.get(orientationKey).get(hashIDKey).get("seg3D");
	    		if ( gtName != null && segName != null ) {
	...
	private void traverse_nii(File secondLayer, String orientation, String hashID ) {
		...
		for ( int i = 0; i < children.length; i++ ) {
			...
			if ( children[i].startsWith("seg3D") && children[i].endsWith(".nii")) {
				 fileNameTable.get(orientation).get(hashID).put("seg3D",
						secondLayer.getAbsolutePath() +
						File.separator + children[i]);
				System.err.println(secondLayer.getAbsolutePath() +
						File.separator + children[i]);
