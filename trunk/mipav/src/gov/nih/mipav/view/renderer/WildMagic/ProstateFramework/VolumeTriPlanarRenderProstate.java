...
public class VolumeTriPlanarRenderProstate extends GPURenderBase
implements GLEventListener, KeyListener, MouseMotionListener,  MouseListener
{

	protected VolumeTriPlanarInterfaceProstate m_kParent = null;
    ...
    /** 3D Slice renderer. */
    private VolumeSlices m_kSlices_Axial = null;   
    private VolumeSlices m_kSlices_Sagittal = null;
    private VolumeSlices m_kSlices_Coronal = null;
     ...
     * Default Constructor.
     */
    public VolumeTriPlanarRenderProstate()
    {
        super();
    }
     ...
     */
    public VolumeTriPlanarRenderProstate( VolumeTriPlanarInterfaceProstate kParent, Animator kAnimator, 
            VolumeImage kVolumeImageAxial, VolumeImage kVolumeImageSagittal, 
            VolumeImage kVolumeImageCoronal, VolumeImage kVolumeImageB  )
    {
        super();
        ...
        m_kVolumeImageA = kVolumeImageAxial;
        m_kVolumeImageAxial = kVolumeImageAxial;
        m_kVolumeImageSagittal = kVolumeImageSagittal;
        m_kVolumeImageCoronal = kVolumeImageCoronal;
    ...
    public void display(GLAutoDrawable arg0) {
        ...
        if ( m_bFirstRender )
        {
            ...
            m_kSlices_Axial.SetDisplay(false);   
            m_kSlices_Sagittal.SetDisplay(false);   
            m_kSlices_Coronal.SetDisplay(false);   
        }
        MeasureTime();
        Move();
        Pick();
        ...
        if ( m_bFirstRender )
        {
            ...
            m_kSlices_Axial.SetDisplay(true);   
            m_kSlices_Sagittal.SetDisplay(true); 
            m_kSlices_Coronal.SetDisplay(true); 
    ...
    {
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.SetDisplay( bDisplay );
        }
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.SetDisplay( bDisplay );
        }
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.SetDisplay( bDisplay );
        }
    ...
    public void init(GLAutoDrawable arg0) {	
        ...
        System.err.println("m_iWidth = " + m_iWidth + " m_iHeight = " + m_iHeight);
        // set up camera
    ...
    public void setABBlend( float fValue )
    {
        ...
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.setABBlend(fValue);
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.setABBlend(fValue);
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.setABBlend(fValue);
        }
    ...
    public void setBackgroundColor( ColorRGBA kColor )
    {
        ...
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.SetBackgroundColor( kColor );
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.SetBackgroundColor( kColor );
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.SetBackgroundColor( kColor );
        }
    }    
    ...
    public void setBoundingBoxColor( int i, ColorRGB kColor )
    {
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.SetBoundingBoxColor( i, kColor );
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.SetBoundingBoxColor( i, kColor );
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.SetBoundingBoxColor( i, kColor );
        }
    ...
    public void setCenter( Vector3f kCenter )
    {
        if ( m_kSlices_Axial != null )
        {
            int[] aiExtents = m_kVolumeImageAxial.GetImage().getExtents();
            m_kSlices_Axial.SetCenter( new Vector3f( (kCenter.X / (aiExtents[0] -1)),
        ...
        if ( m_kSlices_Sagittal != null )
        {
            int[] aiExtents = m_kVolumeImageSagittal.GetImage().getExtents();
            m_kSlices_Sagittal.SetCenter( new Vector3f( (kCenter.X / (aiExtents[0] -1)),
                                               (kCenter.Y / (aiExtents[1] -1)),
                                               (kCenter.Z / (aiExtents[2] -1))  ) );
        ...
        if ( m_kSlices_Coronal != null )
        {
            int[] aiExtents = m_kVolumeImageCoronal.GetImage().getExtents();
            m_kSlices_Coronal.SetCenter( new Vector3f( (kCenter.X / (aiExtents[0] -1)),
                                               (kCenter.Y / (aiExtents[1] -1)),
                                               (kCenter.Z / (aiExtents[2] -1))  ) );
    ...
    public void setRGBTA(ModelRGB RGBT) {
        ...
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.setRGBTA(RGBT);
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.setRGBTA(RGBT);
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.setRGBTA(RGBT);
        }
    }
    ...
    public void setRGBTB(ModelRGB RGBT) {
        ...
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.setRGBTB(RGBT);
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.setRGBTB(RGBT);
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.setRGBTB(RGBT);
        }
    }
    ...
    public void setSliceOpacity( int i, float fAlpha )
    {
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.SetSliceOpacity( i, fAlpha );
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.SetSliceOpacity( i, fAlpha );
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.SetSliceOpacity( i, fAlpha );
        }
    }
    ...
    public void showBoundingBox( int i, boolean bShow )
    {
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.ShowBoundingBox( i, bShow );
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.ShowBoundingBox( i, bShow );
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.ShowBoundingBox( i, bShow );
        }
    }
    ...
    public void showSlice( int i, boolean bShow )
    {
        if ( m_kSlices_Axial != null )
        {
        	m_kSlices_Axial.ShowSlice( i, bShow );
        }
        
        if ( m_kSlices_Sagittal != null )
        {
            m_kSlices_Sagittal.ShowSlice( i, bShow );
        }
        
        if ( m_kSlices_Coronal != null )
        {
            m_kSlices_Coronal.ShowSlice( i, bShow );
        }
    }
    ...
    private void CreateScene (GLAutoDrawable arg0)
    {
        ...
        */
        ...
        m_kSlices_Axial = new VolumeSlices( m_kVolumeImageAxial, m_kVolumeImageB, m_kTranslate, m_fX, m_fY, m_fZ );
        
        m_kSlices_Sagittal = new VolumeSlices( m_kVolumeImageSagittal, m_kVolumeImageB, m_kTranslate, m_fX, m_fY, m_fZ );
        
        m_kSlices_Coronal = new VolumeSlices( m_kVolumeImageCoronal, m_kVolumeImageB, m_kTranslate, m_fX, m_fY, m_fZ );
        
        displayVolumeSlices( true );
        
        m_kDisplayList.add(m_kDisplayList.size(), m_kSlices_Sagittal);
        // m_kDisplayList.add(m_kDisplayList.size(), m_kSlices_Sagittal);
        // m_kDisplayList.add(m_kDisplayList.size(), m_kSlices_Coronal);
        ...
        m_kParent.addSlices(m_kSlices_Axial, m_kSlices_Sagittal, m_kSlices_Coronal);
    }
