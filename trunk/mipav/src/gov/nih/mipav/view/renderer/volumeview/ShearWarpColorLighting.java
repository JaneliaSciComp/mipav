...
/**
 * A renderer for RGBA+lighting volume data.  See the document
    ...
    extends ShearWarpColor
{
    ...
    /**
     * Create an RGBA+lighting color renderer.  The input color channel type
     * is 'byte', a signed 8-bit quantity, but the data represents unsigned
     * 8-bit values.
     *
     ...
     * sets the current eye position to always be in front of the box along
     * the current renderer view direction.  This function also saves the
     * current permutation for use in computing voxel positions for dynamic
     * lighting.
     *
     ...
     *
     * @param iDS The number of slices to increment during the resampling
     *    phase.  The value should be one or larger.  If one, all slices of
     *    the volume data are resampled.  If two, only every other slice is
     *    resampled.  An input larger than one is used to allow fast
     *    rendering during rotation of the volume data.  Once the rotation
     *    terminates, a composite with input of one should be called.
     */
    protected void resampleAll( int iDS ) {
        ...
        if ( m_bCompositeForward ) { // front to back
            ...
            for (/**/; m_iSlice <= m_aiSliceMax[2]; m_iSlice += iDS ) {
                if ( ( m_iSlice ) % mod == 0 ) {
                    ViewJFrameVolumeView.getRendererProgressBar().setValue( Math.round( (float) ( m_iSlice ) / ( m_aiSliceBound[2] - 2 ) * 50 ) );
            ...
            for (/**/; m_iSlice >= m_aiSliceMin[2]; m_iSlice -= iDS ) {
                if ( ( m_aiSliceBound[2] - 1 - m_iSlice ) % mod == 0 ) {
                    ViewJFrameVolumeView.getRendererProgressBar().setValue( Math.round( (float) ( m_aiSliceBound[2] - 1 - m_iSlice ) / ( m_aiSliceBound[2] - 2 ) * 50 ) );
                    ViewJFrameVolumeView.getRendererProgressBar().update( ViewJFrameVolumeView.getRendererProgressBar().getGraphics() );
                }
    ...
    protected void resampleSingle() {
        ...
        for ( int i1 = m_aiSliceMin[1]; i1 < m_aiSliceMax[1]; i1++ ) {
            ...
            for ( m_iSIndex = 0; asIndex[m_iSIndex] >= 0; m_iSIndex++ ) {
                ...
                // bilerp position
                m_kVertexProperty.setPosition(
                    fS00*m_kP00.x + fS01*m_kP01.x + fS10*m_kP10.x + fS11*m_kP11.x,
                    fS00*m_kP00.y + fS01*m_kP01.y + fS10*m_kP10.y + fS11*m_kP11.y,
                    fS00*m_kP00.z + fS01*m_kP01.z + fS10*m_kP10.z + fS11*m_kP11.z);
                ...
                m_kVertexProperty.setDiffuse(
                    (fS00*fR00 + fS01*fR01 + fS10*fR10 + fS11*fR11) / 255.0f,
                ...
                m_kVertexProperty.setNormal(
                    fS00*kN00.x + fS01*kN01.x + fS10*kN10.x + fS11*kN11.x,
