...
public class ViewJFrameRenderCamera extends ViewJFrameBase
    ...
    public void addImage( ModelImage _imageA ) {
        ...
        Point t = new Point();
        int fwidth = (int) Math.round(
                (double) gridColumn
                        * (double) ( imagePanelSizeX + gridSpacing + 2 * borderSize + 2 * selectedBorderSize ) )
                                + gridSpacing;
    ...
    public final boolean updateImages( boolean forceShow ) {
        ...
        ( ( (ViewJComponentEditImage) componentImageVector.elementAt( currentIndex ) ) ).
            setStringOverride(String.valueOf( currentIndex+1 ));
        ( ( (ViewJComponentEditImage) componentImageVector.elementAt( currentIndex ) ) ).show( 0, 0, null,
            null, true, -1);
    ...
    public final boolean updateImages( ModelLUT _LUTa, ModelLUT _LUTb, boolean forceShow, int interpMode ) {
        ...
        for ( int i = 0; i < componentImageVector.size(); i++ ) {
            ...
            if ( slice < numTotalSlices ) {
                if ( ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).show( 0, 0, LUTa,
                        LUTb, true, interpMode )
                        == false ) {
                    return false;
                }
                if ( slice == userInterface.getActiveImageFrame().getViewableSlice() ) {
        ...
        for ( int i = 0; i < componentImageVector.size(); i++ ) {
            if ( ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ) != null ) {
                ...
                if ( slice < numTotalSlices ) {
                    ...
                    ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).paintComponent(
                            ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).getGraphics() );
                } else {
                    ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).showBlank();
                }
    ...
    public void updateImageBorder( int i ) {
        ...
        try {
            ...
            ( ( (JPanel) imagePanelVector.elementAt( i ) ) ).setBorder(
                    (CompoundBorder) ( imageBorderVector.elementAt( i ) ) );
        } catch ( ArrayIndexOutOfBoundsException aioobe ) {
    ...
    public void actionPerformed( ActionEvent event ) {
        ...
        if ( event.getActionCommand().equals( "SelectAll" ) ) {
            ...
            for ( int i = 0; i < numTotalSlices; i++ ) {
                ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).setMode(
                        ViewJComponentEditImage.MAG_REGION );
            }
        } else if ( event.getActionCommand().equals( "Pointer" ) ) {
            for ( int i = 0; i < numTotalSlices; i++ ) {
                ( ( (ViewJComponentEditImage) componentImageVector.elementAt( i ) ) ).setMode(
                        ViewJComponentEditImage.DEFAULT );
            }
    ...
    private void saveAVI() {
        ...
        if (imageA.getAnimateFrame() == null) {
           ...
           dialogAnimate = new JDialogAnimate(this, destImage,
                                              LUTa,
                                              null,
                                              null,
                                              userInterface, false);
    ...
    public void windowClosing( WindowEvent event ) {
        ...
        int reply = JOptionPane.showConfirmDialog( this, "Do you really want to close the camera capture frame?",
                "Camera capture frame close", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE );
     ...
     * The extraction takes the first slice in each componentImageVector,
     * then binding them into the resulting modelImage model.
     */
    private void extractImages() {
        ...
        for ( int i = 0; i < sliceList.length; i++ ) {
            ...
            try {
                // try copying the zSrc slice out of srcImage, making it the zDest in destImage
                ModelImage tempImage = ( (ViewJComponentEditImage) ( componentImageVector.elementAt( sliceList[i] ) ) ).getImageA();
    ...
    /**
     * Only allow resizing the frame horizontally.  Vertial not allow.
     * Dr. Ben's  contribution.
     * @param frame
     */
            ...
            public void componentResized( ComponentEvent event ) {
                ...
                if ( frameStartLocation.getX() + frameWidthLimit >= xScreen ) {
                    gridColumnLimit = (int) ( (double) ( xScreen - frameStartLocation.getX() )
                            / (double) imagePanelSizeX )
                                    - 1;
                }
                gridColumnMinimized = (int) Math.round(
                        (double) frame.getSize().width
                                / (double) ( imagePanelSizeX + gridSpacing + 2 * borderSize + 2 * selectedBorderSize ) );
                ...
                // System.out.println("gridColumnMinimized = " + gridColumnMinimized + " gridColumn = " + gridColumn + " gridColumnLimit = " + gridColumnLimit);
                widthFrame = (int) Math.round(
                        (double) availColumn
                                * (double) ( imagePanelSizeX + gridSpacing + 2 * borderSize + 2 * selectedBorderSize ) )
                                        + gridSpacing;
    ...
    public void disposeLocal( boolean flag ) {
       ...
       imageBufferA = null;
       pixBuffer = null;
       if (pagePanel != null) pagePanel.removeAll();
       pagePanel = null;
       menuBar = null;
       if (toolbarPanel != null) toolbarPanel.removeAll();
       toolbarPanel = null;
       LUTa = null;
       LUTb = null;
