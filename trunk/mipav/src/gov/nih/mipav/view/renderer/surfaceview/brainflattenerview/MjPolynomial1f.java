...
{
    static private final float ms_fInvLog2 = 1.0f/(float)Math.log(2.0f);
    static private final float ms_fLog10 = (float)Math.log(10.0f);
    static private final float ms_fThird = 1.0f/3.0f;
    static private final float ms_fSqrt3 = (float)Math.sqrt(3.0f);
    static private final float ms_fTwentySeventh = 1.0f/27.0f;
    ...
    // set this polynomial to have the same coefficients as those
    ...
    public void sub (MjPolynomial1f kPoly1, MjPolynomial1f kPoly2)
    {
        ...
        if ( kPoly1.m_iDegree > kPoly2.m_iDegree )
        {
            ...
            for (int i = 0; i <= kPoly2.m_iDegree; i++)
                kDiff.m_afCoeff[i] =
                kPoly1.m_afCoeff[i] - kPoly2.m_afCoeff[i];
    ...
    public void mul (MjPolynomial1f kPoly1, MjPolynomial1f kPoly2)
    {
        ...
        MjPolynomial1f kProd =
                new MjPolynomial1f (kPoly1.m_iDegree + kPoly2.m_iDegree);
        ...
        for (int i1 = 0; i1 <= kPoly1.m_iDegree; i1++)
        {
            ...
            {
                kProd.m_afCoeff[i1+i2] +=
                          kPoly1.m_afCoeff[i1]*kPoly2.m_afCoeff[i2];
            }
        }
    ...
    // Returns Float which is null if no root is found; otherwise
    // contains the value of the root.
    ...
    // afRoot should have length 2; returns number roots found, which are
    // returned in the afRoot array
    public int getRootsOnInterval (float fXMin, float fXMax,
                                   float [] afRoot, int iDigitsAccuracy)
    {
        int iCount;
        Float kRoot;
        ...
        iCount =
                kDeriv.getRootsOnInterval(fXMin,fXMax,afRoot,iDigitsAccuracy);
    ...
    /* Called by computeRadius, finds the root of the <= 8 degree polynomial
    ...
    // afRoot should have length 3 which is used to return the
    // real roots found;
    // returns 0 if no root can be found, otherwise returns the
    ...
    public int rootsDegree3 (float [] afRoot)
    {
        ...
        float fB =
                ms_fTwentySeventh*(2.0f*afCoeff[2]*afCoeff[2]*afCoeff[2]
        ...
        else if ( fDiscr < 0.0f )
        {
            ...
            float fAngle =
                    ms_fThird*(float)Math.atan2(Math.sqrt(-fDiscr),-fHalfB);
