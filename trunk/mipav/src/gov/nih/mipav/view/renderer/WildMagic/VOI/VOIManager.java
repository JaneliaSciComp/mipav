...
public class VOIManager implements ActionListener, KeyListener, MouseListener, MouseMotionListener {
     ...
     * a problem because we have a unique identifier in the parameter <code>position</code>. New nodes of the same
     * cost as an existing node are inserted to the left of the first existing node of the same cost. The children of
     * the first existing node are transferred to the left of the new node. The new node is not put all the way to the
     * left of a chain of nodes of the same cost so as to save tree traversal time. This convention must be followed in
     * the remove and pop or else some nodes get lost and others are never removed.
     * 
     * <p>
     * The main reason for using a Binary Search Tree is that the cost of a "find" (required for remove, pop, and
    ...
    public class ActiveTree {
         ...
         */
        public void insert(final int position, final float cost) {
         ...
         */
        private void inOrder(final TreeNode node, final int level) {
    ...
    public class TreeNode {
         ...
         */
        public TreeNode(final int position, final float cost) {
            right = null;
            left = null;
    ...
    private int m_iNearStatus = VOIManager.NearNone;
    ...
    private final float grad_weight = 0.20f; // used to remember gradient weight
    ...
    public VOIManager(final VOIManagerInterface kParent) {
        m_kParent = kParent;
    }
    ...
    public void actionPerformed(final ActionEvent event) {
        ...
        if (command.equals(VOIManager.DELETE_INTENSITY_LINE)) // handling the popup menu for the VOI intensity line
        {
            m_kParent.deleteVOI(m_kCurrentVOI);
            m_kParent.setDefaultCursor();
        } else if (command.equals(VOIManager.SHOW_INTENSITY_GRAPH)) // handling the popup menu for the VOI intensity
                                                                    // line
        {
    ...
    public void add(final VOIBase kVOI, final float fHue) {
    ...
    public boolean add(final VOIBase kVOI, final int iPos, final Vector3f kNewPoint, boolean bIsFile) {
        if (kVOI.isFixed()) {
            return false;
        }
    ...
    public boolean add(final VOIBase kVOI, final Vector3f kNewPoint, final boolean bIsFile) {
        return add(kVOI, kVOI.size() - 1, kNewPoint, bIsFile);
    }

    public void anchor(final int iX, final int iY, final boolean bSeed) {
        final Vector3f kNewPoint = new Vector3f(iX, iY, m_iSlice);
        if (m_kCurrentVOI == null) {
            final Vector<Vector3f> kPositions = new Vector<Vector3f>();
    ...
    public void anchorPolyline(final int iX, final int iY, boolean bFinished) {
        final Vector3f kNewPoint = new Vector3f(iX, iY, m_iSlice);
        if (m_kCurrentVOI == null) {
            final Vector<Vector3f> kPositions = new Vector<Vector3f>();
            kPositions.add(kNewPoint);
    ...
    public boolean contains(final VOIBase kVOI, final int iX, final int iY, final int iZ) {
    ...
    public void deleteVOI(final VOIBase kVOI) {
        m_kParent.deleteVOI(kVOI);
    }

    public int deleteVOIActivePt(final VOIBase kVOI) {
        final int iPos = kVOI.getSelectedPoint();
        if (iPos < 0) {
            return kVOI.size();
        }
    ...
    public void doVOI(final String kCommand, final boolean isDrawCommand) {
        if (m_bFirstVOI) {
            m_bFirstVOI = false;
        ...
        if (kCommand.equals(CustomUIBuilder.PARAM_VOI_DEFAULT_POINTER.getActionCommand())) {
            m_bPointer = true;
        } else if (kCommand.equals(CustomUIBuilder.PARAM_VOI_NEW.getActionCommand()) || kCommand.equals("ResetVOI")) {
            m_kCurrentVOI = null;
    ...
    public void drawNext(final int iX, final int iY) {
        if (m_kCurrentVOI == null) {
            return;
        }
        ...
        final Vector3f kFile = new Vector3f();
        m_kDrawingContext.screenToFile(iX, (int) fY, m_iSlice, kFile);
        final Vector3f kLocalPt = fileCoordinatesToPatient(kFile);
        final int xDim = m_aiLocalImageExtents[0];
        ...
        final int iAnchor = m_kCurrentVOI.getAnchor();
        while (liveWirePtIndex != seedPoint) {
            // add mouseIndex point:
            x = mouseIndex % xDim;
            y = (mouseIndex - x) / xDim;
            kNewPoint = new Vector3f(x, y, m_iSlice);
            final Vector3f kVolumePt = patientCoordinatesToFile(kNewPoint);
    ...
    public void drawNextPolyline(final int iX, final int iY) {
        if (m_kCurrentVOI == null) {
            return;
        }
        ...
        final float fY = iY;
        final Vector3f kNewPoint = new Vector3f(iX, fY, m_iSlice);
        if (add(m_kCurrentVOI, kNewPoint, false)) {
            m_kParent.updateDisplay();
        }
    }

    public Vector3f fileCoordinatesToPatient(final Vector3f volumePt) {
        final Vector3f kPatient = new Vector3f();
    ...
    public void init(final ModelImage kImageA, final ModelImage kImageB, final Component kComponent,
            final ScreenCoordinateListener kContext, final int iOrientation, final int iSlice) {
        ...
        setSlice(iSlice);
        final int iPlane = MipavCoordinateSystems.getAxisOrder(kImageA, iOrientation)[2];
        switch (iPlane) {
            case 0:
                m_iPlane = VOIBase.XPLANE;
                break;
            case 1:
                m_iPlane = VOIBase.YPLANE;
                break;
            case 2:
                m_iPlane = VOIBase.ZPLANE;
                break;
        }
    }
    ...
    public void keyPressed(final KeyEvent e) {
        ...
        switch (keyCode) {
            case KeyEvent.VK_UP:
                m_kParent.doVOI("MoveUP");
                return;
            case KeyEvent.VK_DOWN:
                m_kParent.doVOI("MoveDown");
                return;
            case KeyEvent.VK_LEFT:
                m_kParent.doVOI("MoveLeft");
                return;
            case KeyEvent.VK_RIGHT:
                m_kParent.doVOI("MoveRight");
                return;
        }
    ...
    public void liveWire(final int iSelection) {
    ...
    public void mouseClicked(final MouseEvent kEvent) {
        ...
        if (m_bDrawVOI && (m_iDrawType == VOIManager.LIVEWIRE || m_iDrawType == VOIManager.POLYLINE)
                && (kEvent.getClickCount() > 1)) {
            showSelectedVOI(kEvent.getX(), kEvent.getY());
            if (m_iNearStatus == VOIManager.NearPoint) {
                ...
                if (m_kCurrentVOI.getGroup() != null) {
                    if (m_kCurrentVOI.getGroup().getCurveType() != m_kCurrentVOI.getType()) {
                        final VOI kGroup = m_kCurrentVOI.getGroup();
            ...
            if (m_iDrawType == VOIManager.LIVEWIRE) {
            ...
            m_bDrawVOI = false;
            m_iNearStatus = VOIManager.NearNone;
            m_kParent.setDefaultCursor();
        }
    }
    ...
    public void mouseExited(final MouseEvent arg0) {
        if (m_bDrawVOI && (m_iDrawType == VOIManager.LEVELSET)) {
            if (m_kCurrentVOI != null) {
                m_kParent.deleteVOI(m_kCurrentVOI);
            }
        }
    ...
    public void mouseMoved(final MouseEvent kEvent) {
        ...
        if (m_bPointer) {
            showSelectedVOI(kEvent.getX(), kEvent.getY());
        } else if (m_bDrawVOI && (m_iDrawType == VOIManager.LEVELSET)) {
            createVOI(kEvent.getX(), kEvent.getY());
        } else if ( (m_kCurrentVOI != null) && m_bDrawVOI && (m_iDrawType == VOIManager.LIVEWIRE)) {
            drawNext(kEvent.getX(), kEvent.getY());
        } else if ( (m_kCurrentVOI != null) && m_bDrawVOI && (m_iDrawType == VOIManager.POLYLINE)) {
    ...
    public void mousePressed(final MouseEvent kEvent) {
        ...
        if (kEvent.getButton() == MouseEvent.BUTTON1) {
            if (m_bPointer) {
                ...
                } else if (m_iNearStatus == VOIManager.NearLine) {
    ...
    public void mouseReleased(final MouseEvent kEvent) {
        ...
        if (m_bDrawVOI && ( (m_iDrawType == VOIManager.POINT) || (m_iDrawType == VOIManager.POLYPOINT))) {
            createVOI(kEvent.getX(), kEvent.getY());
        } else if (m_bDrawVOI && (m_iDrawType == VOIManager.TEXT)) {
            createTextVOI(kEvent.getX(), kEvent.getY());
        } else if (m_bDrawVOI && (m_iDrawType == VOIManager.RECTANGLE3D)) {
            m_kCurrentVOI.setActive(true);
            m_kParent.doVOI(CustomUIBuilder.PARAM_VOI_PROPAGATE_ALL.getActionCommand());
            m_bDrawVOI = false;
            return;
        } else if (m_bDrawVOI && (m_iDrawType == VOIManager.LUT)) {
            m_kParent.quickLUT(m_kCurrentVOI);
            m_bDrawVOI = false;
            return;
        } else if (m_bDrawVOI && (m_iDrawType == VOIManager.SPLITLINE)) {
            ...
            m_kParent.setDefaultCursor();
            return;
        } else if (m_bDrawVOI && ( (m_iDrawType == VOIManager.POLYLINE) || (m_iDrawType == VOIManager.LIVEWIRE))) {
            if (m_kCurrentVOI != null && m_kCurrentVOI.size() > 2) {
                showSelectedVOI(kEvent.getX(), kEvent.getY());
                if ( (m_iNearStatus == VOIManager.NearPoint)
                        && ( (m_kCurrentVOI.getNearPoint() == 0) || (m_kCurrentVOI.getNearPoint() == m_kCurrentVOI
                                .size() - 2))) {
        ...
        m_iNearStatus = VOIManager.NearNone;
        if ( !kEvent.isShiftDown() || (m_iDrawType == VOIManager.TEXT) || (m_iDrawType == VOIManager.LINE)
                || (m_iDrawType == VOIManager.PROTRACTOR) || (m_iDrawType == VOIManager.POLYLINE)
                || (m_iDrawType == VOIManager.LIVEWIRE) || (m_iDrawType == VOIManager.RECTANGLE3D)
                || (m_iDrawType == VOIManager.SPLITLINE)) {
            m_bDrawVOI = false;
        } else {
            m_kCurrentVOI = null;
    ...
    public boolean testMove(final Vector3f kDiff, final Vector3f[] akMinMax) {
        final Vector3f kScreenMin = m_kDrawingContext.fileToScreen(akMinMax[0]);
        kScreenMin.Add(kDiff);

        final Vector3f kScreenMax = m_kDrawingContext.fileToScreen(akMinMax[1]);
        kScreenMax.Add(kDiff);
        final Vector3f kTemp = new Vector3f(kScreenMin);
        kScreenMin.Min(kScreenMax);
        kScreenMax.Max(kTemp);
        if (m_fMouseX < kScreenMin.X || m_fMouseX > kScreenMax.X || m_fMouseY < kScreenMin.Y
                || m_fMouseY > kScreenMax.Y) {
            return false;
        }
        final Vector3f kTestMin = new Vector3f();
        final Vector3f kTestMax = new Vector3f();
    ...
    public void move(final VOIBase kVOI, final Vector3f kDiff) {
        if (kVOI.isFixed() || kDiff.equals(Vector3f.ZERO)) {
            return;
        }
        final int iNumPoints = kVOI.size();
        if (iNumPoints > 0) {
            final Vector3f kVolumeDiff = new Vector3f();
            for (int i = 0; i < iNumPoints; i++) {
                if (i == 0) {
                    final Vector3f kPos = kVOI.elementAt(i);
    ...
    public boolean nearPoint(final VOIBase kVOI, final int iX, final int iY, final int iZ) {

        final Vector3f kVOIPoint = new Vector3f(iX, iY, iZ);
        for (int i = 0; i < kVOI.size(); i++) {
            final Vector3f kFilePos = kVOI.get(i);
    ...
    public void pasteAllVOI(final VOIBase kVOI) {
        m_kCopyVOI = kVOI;
    ...
    public void pasteVOI(final VOIBase kVOI) {
        m_kCopyVOI = kVOI;
        pasteVOI(m_iSlice);
    }

    public Vector3f patientCoordinatesToFile(final Vector3f patientPt) {
    ...
    public void setActiveImage(final int iActive) {
        if (m_akImages[iActive] != null) {
            m_kImageActive = m_akImages[iActive];
        }
    }

    private void setCanvas(final Component kComponent) {
    ...
    public void setCurrentVOI(final VOIBase kCurrentVOI) {
        m_kCurrentVOI = kCurrentVOI;
    }

    public void setDrawingContext(final ScreenCoordinateListener kContext) {
    ...
    public void setPopupPt(final ViewJPopupPt kPopupPt) {
        m_kPopupPt = kPopupPt;
    }

    public void setPopupVOI(final ViewJPopupVOI kPopupVOI) {
     ...
     * @param fSlice
     */
    public void setSlice(final float fSlice) {
    ...
    private void addPopup(final VOIBase kVOI) {
     ...
     */
    private int convertGraphToInt(final int next, int xDim) {
    ...
    private void createTextVOI(final int iX, final int iY) {
        final int colorID = 0;
        final VOI newTextVOI = new VOI((short) colorID, "annotation3d.voi", 1, VOI.ANNOTATION, -1.0f);
        ...
        final int[] extents = m_aiLocalImageExtents;
        ...
        final String prefColor = Preferences.getProperty(Preferences.PREF_VOI_TEXT_COLOR);
        Color textColor;
    ...
    private VOIBase createVOI(final int iType, final boolean bClosed, final boolean bFixed,
            final Vector<Vector3f> kPositions) {

        VOIBase kVOI = null;

        switch (iType) {
            case POINT:
                kVOI = new VOIPoint(VOI.POINT);
                break;
            case POLYPOINT:
                kVOI = new VOIPoint(VOI.POLYLINE_SLICE);
                break;
            case RECTANGLE:
            case RECTANGLE3D:
            case LUT:
            ...
            case POLYLINE:
            case LIVEWIRE:
                kVOI = new VOIContour(bFixed, bClosed);
                break;
            case LINE:
            case SPLITLINE:
                kVOI = new VOILine();
                break;
            case PROTRACTOR:
                kVOI = new VOIProtractor();
                ((VOIProtractor) kVOI).setPlane(m_iPlane);
                break;
        }
    ...
    private void createVOI(final int iX, final int iY) {
        final float fYStart = m_fMouseY;
        final float fY = iY;

        final VOIBase kOld = m_kCurrentVOI;
        if ( (m_iDrawType == VOIManager.POINT) || (m_iDrawType == VOIManager.POLYPOINT)) {
            final Vector<Vector3f> kPositions = new Vector<Vector3f>();
            kPositions.add(new Vector3f(iX, fY, m_iSlice));

            m_kCurrentVOI = createVOI(m_iDrawType, false, false, kPositions);

        } else if (m_iDrawType == VOIManager.RECTANGLE || m_iDrawType == VOIManager.RECTANGLE3D
                || m_iDrawType == VOIManager.LUT) {
            if (m_kCurrentVOI == null) {
                final Vector<Vector3f> kPositions = new Vector<Vector3f>();
        ...
        } else if (m_iDrawType == VOIManager.OVAL) {
        ...
        } else if (m_iDrawType == VOIManager.LEVELSET) {
            initLiveWire(m_iSlice, false);
            final VOIBase kTemp = singleLevelSet2(iX, fY);
            // VOIBase kTemp = singleLevelSet(iX, fY);
            if (kTemp == null) {
                return;
            }
            m_kCurrentVOI = kTemp;
        } else if (m_iDrawType == VOIManager.POLYLINE) {
            if (m_kCurrentVOI == null) {
                final Vector<Vector3f> kPositions = new Vector<Vector3f>();
                kPositions.add(new Vector3f(m_fMouseX, fYStart, m_iSlice));
                kPositions.add(new Vector3f(iX, fY, m_iSlice));
            ...
            m_fMouseX = iX;
            m_fMouseY = iY;
        } else if ( (m_iDrawType == VOIManager.LINE) || (m_iDrawType == VOIManager.SPLITLINE)) {
            if (m_kCurrentVOI == null) {
                final Vector<Vector3f> kPositions = new Vector<Vector3f>();
                kPositions.add(new Vector3f(m_fMouseX, fYStart, m_iSlice));
                kPositions.add(new Vector3f(iX, fY, m_iSlice));
                ...
                m_kCurrentVOI = createVOI(m_iDrawType, false, false, kPositions);
                if (m_iDrawType == VOIManager.SPLITLINE) {
            ...
            m_fMouseX = iX;
            m_fMouseY = iY;
        } else if (m_iDrawType == VOIManager.PROTRACTOR) {
            if (m_kCurrentVOI == null) {

                final Vector3f kStart = new Vector3f(m_fMouseX, fYStart, m_iSlice);
                final Vector3f kEnd = new Vector3f(iX, fY, m_iSlice);
                final Vector3f kMiddle = new Vector3f();
                ...
                final Vector3f kStart = m_kDrawingContext.fileToScreen(m_kCurrentVOI.get(1));
                final Vector3f kEnd = new Vector3f(iX, fY, m_iSlice);
                final Vector3f kMiddle = new Vector3f();
                kMiddle.Sub(kEnd, kStart);
                kMiddle.Scale(.4f);
    ...
    private void drawArrow(final VOIText kVOI, final Graphics2D g2d, final int xCenter, final int yCenter, final int x,
            final int y, final float stroke) {
    ...
    private void drawGeometricCenter(final VOIBase kVOI, final Graphics g) {
        int xS, yS;
        ...
        final Vector3f gcFilePt = kVOI.getGeometricCenter();
        // Profile.clear();
        // Profile.start();
        // for ( int i = 0; i < 100; i++ )
        // {
        ...
        final Vector3f gcPt = m_kDrawingContext.fileToScreen(gcFilePt);
        xS = (int) gcPt.X;
        yS = (int) gcPt.Y;
        g.drawLine(xS, yS - 3, xS, yS + 3);
        g.drawLine(xS - 3, yS, xS + 3, yS);

        final int iContourID = kVOI.getContourID();
        if (iContourID != -1) {
     ...
     */
    private void drawLength(final Graphics g, final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure) {
        final String tmpString = kVOI.getTotalLengthString(resols, unitsOfMeasure);

        final Vector3f gcFilePt = kVOI.getGeometricCenter();
        final Vector3f pt = m_kDrawingContext.fileToScreen(gcFilePt);
    ...
    private void drawTickMarks(final VOIBase kVOI, final Graphics g, final Color color, final int[] unitsOfMeasure,
            final int xD, final int yD, final float[] res) {
        g.setFont(MipavUtil.font12);

        final Vector3f kStart = m_kDrawingContext.fileToScreen(kVOI.get(0));
        final Vector3f kEnd = m_kDrawingContext.fileToScreen(kVOI.get(1));
    ...
    private void drawTickMarks(final VOIBase kVOI, final Graphics g, final int[] unitsOfMeasure, final int xD,
        ...
        final Vector3f kStart = m_kDrawingContext.fileToScreen(kVOI.get(0));
        final Vector3f kMiddle = m_kDrawingContext.fileToScreen(kVOI.get(1));
        final Vector3f kEnd = m_kDrawingContext.fileToScreen(kVOI.get(2));
        ...
        final String degreeString = ((VOIProtractor) kVOI).getAngleString(res);
        ...
        if (showLengths) {
            ...
            final String lengthString = kVOI.getLengthString(0, 1, res, unitsOfMeasure);
            if (close == true) {
     ...
     */
    private void drawVertices(final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure, final Graphics g,
            final int thickness, final boolean boundingBox) {
        Polygon gon = null;
        int j;
        ...
        /*
         * if (boundingBox == true) { int x0, x1, y0, y1; x0 = (int) ((xBounds[0] * zoomX * resolutionX) + 0.5f); x1 =
         * (int) ((xBounds[1] * zoomX * resolutionX) + 0.5f); y0 = (int) ((yBounds[0] * zoomY * resolutionY) + 0.5f); y1 =
         * (int) ((yBounds[1] * zoomY * resolutionY) + 0.5f); g.setColor(Color.yellow.darker()); g.drawRect(x0, y0, x1 -
         * x0, y1 - y0);
         *  // draw corners of bounding box to make handles for resizing VOI g.fillRect(x0 - 2, y0 - 2, 5, 5);
         * g.fillRect(x1 - 2, y0 - 2, 5, 5); g.fillRect(x0 - 2, y1 - 2, 5, 5); g.fillRect(x1 - 2, y1 - 2, 5, 5);
         *  // draw mid points of bounding box to make handles for resizing VOI g.fillRect(MipavMath.round(x0 + ((x1 -
         * x0) / 2) - 2), y0 - 2, 5, 5); g.fillRect(x1 - 2, Math.round(y0 + ((y1 - y0) / 2) - 2), 5, 5);
         * g.fillRect(MipavMath.round(x0 + ((x1 - x0) / 2) - 2), y1 - 2, 5, 5); g.fillRect(x0 - 2, Math.round(y0 + ((y1 -
         * y0) / 2) - 2), 5, 5); g.setColor(Color.yellow.brighter());
         * 
         * switch (nearBoundPoint) {
         * 
         * case 1: g.fillRect(x0 - 2, y0 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 - 2, y0 - 2, 4, 4); break;
         * 
         * case 2: g.fillRect(x1 - 2, y0 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x1 - 2, y0 - 2, 4, 4); break;
         * 
         * case 3: g.fillRect(x1 - 2, y1 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x1 - 2, y1 - 2, 4, 4); break;
         * 
         * case 4: g.fillRect(x0 - 2, y1 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 - 2, y1 - 2, 4, 4); break;
         * 
         * case 5: g.fillRect(MipavMath.round(x0 + ((x1 - x0) / 2) - 2), y0 - 2, 5, 5); g.setColor(Color.black);
         * g.drawRect(MipavMath.round(x0 + ((x1 - x0) / 2) - 2), y0 - 2, 4, 4); break;
         * 
         * case 6: g.fillRect(x1 - 2, MipavMath.round(y0 + ((y1 - y0) / 2) - 2), 5, 5); g.setColor(Color.black);
         * g.drawRect(x1 - 2, MipavMath.round(y0 + ((y1 - y0) / 2) - 2), 4, 4); break;
         * 
         * case 7: g.fillRect(MipavMath.round(x0 + ((x1 - x0) / 2) - 2), y1 - 2, 5, 5); g.setColor(Color.black);
         * g.drawRect(MipavMath.round(x0 + ((x1 - x0) / 2) - 2), y1 - 2, 4, 4); break;
         * 
         * case 8: g.fillRect(x0 - 2, MipavMath.round(y0 + ((y1 - y0) / 2) - 2), 5, 5); g.setColor(Color.black);
         * g.drawRect(x0 - 2, MipavMath.round(y0 + ((y1 - y0) / 2) - 2), 4, 4); break; } }
         */
    }

    public Vector3f drawBlendContour(final VOIBase kVOI, final int[] pixBuffer, final float opacity, final Color color,
            final int slice) {
        if ( (m_iPlane != (m_iPlane & kVOI.getPlane())) || (getSlice(kVOI) != slice)) {
            return null;
        }
        ...
        for (int i = 0; i < iNumPoints; i++) {
            final Vector3f kVolumePt = kVOI.get(i);
            final Vector3f kPt = fileCoordinatesToPatient(kVolumePt);
        ...
        final Vector3f kVolumePt = kVOI.get(0);
        final Vector3f kPt = fileCoordinatesToPatient(kVolumePt);
        ...
        int numPts = 0;
        final Vector3f kCenterMass = new Vector3f();
        ...
        for (int iX = iXMin; iX < iXMax; iX++) {
            ...
            for (int iY = iYMin; iY < iYMax; iY++) {
                ...
                if (bInside == true) {
                     ...
                     */
                    if (pixBuffer != null) {
                        final int index = iY * m_aiLocalImageExtents[0] + iX;
                        int opacityInt = (int) (opacity * 255);
                        opacityInt = opacityInt << 24;

                        final int colorInt = color.getRGB() & 0x00ffffff;
    ...
    private void drawVOI(final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure, final Graphics g,
            final int thickness) {
        Polygon gon = null;
        int j;
        new DecimalFormat(".##");
        ...
        if (kVOI.isActive() /*
                             * && getSType() != VOIManager.LEVELSET && getSType() != VOIManager.LIVEWIRE
                             */) {
            // if active draw little boxes at points
            for (j = 0; j < kVOI.size(); j++) {
            ...
            if ( (kVOI.getSelectedPoint() >= 0) && (kVOI.size() > kVOI.getSelectedPoint())) {
                ...
                g.fillRect((int) (gon.xpoints[kVOI.getSelectedPoint()] - 1.5 + 0.5f), (int) (gon.ypoints[kVOI
                        .getSelectedPoint()] - 1.5 + 0.5f), 3, 3);
            }

            /*
             * if (boundingBox == true) { int x0, x1, y0, y1; x0 = (int) ((xBounds[0] * zoomX * resolutionX) + 0.5); x1 =
             * (int) ((xBounds[1] * zoomX * resolutionX) + 0.5); y0 = (int) ((yBounds[0] * zoomY * resolutionY) + 0.5);
             * y1 = (int) ((yBounds[1] * zoomY * resolutionY) + 0.5); g.setColor(Color.yellow.darker()); g.drawRect(x0,
             * y0, x1 - x0, y1 - y0);
             *  // draw corners of bounding box to make handles for resizing VOI g.fillRect(x0 - 2, y0 - 2, 5, 5);
             * g.fillRect(x1 - 2, y0 - 2, 5, 5); g.fillRect(x0 - 2, y1 - 2, 5, 5); g.fillRect(x1 - 2, y1 - 2, 5, 5);
             *  // draw mid points of bounding box to make handles for resizing // VOI g.fillRect(x0 + ((x1 - x0) / 2) -
             * 2, y0 - 2, 5, 5); g.fillRect(x1 - 2, y0 + ((y1 - y0) / 2) - 2, 5, 5); g.fillRect(x0 + ((x1 - x0) / 2) -
             * 2, y1 - 2, 5, 5); g.fillRect(x0 - 2, y0 + ((y1 - y0) / 2) - 2, 5, 5);
             *  // display the height/width of the bounding box above (or below) // the top // midpoint and to the right
             * of (or left of) the right midpoint String widthString, heightString; String measuredWidthString,
             * measuredHeightString; String upperLeftLocationString; String lowerXYmmString; int width = (int)
             * ((xBounds[1] - xBounds[0]) + 0.5f); int height = (int) ((yBounds[1] - yBounds[0]) + 0.5f); widthString =
             * String.valueOf(width); heightString = String.valueOf(height); measuredWidth = (xBounds[1] - xBounds[0]) *
             * resols[0]; measuredHeight = (yBounds[1] - yBounds[0]) * resols[1]; xUnitsString = FileInfoBase
             * .getUnitsOfMeasureAbbrevStr(unitsOfMeasure[0]); yUnitsString = FileInfoBase
             * .getUnitsOfMeasureAbbrevStr(unitsOfMeasure[1]); measuredWidthString =
             * String.valueOf(nf.format(measuredWidth)) + " " + xUnitsString; measuredHeightString = String
             * .valueOf(nf.format(measuredHeight)) + " " + yUnitsString;
             *  // System.err.println("width: " + widthString + " height: " + // heightString); float lowerXmm = originX +
             * (resols[0] * xBounds[0]); float lowerYmm = originY + (resols[1] * yBounds[0]); lowerXYmmString = "(" +
             * String.valueOf(nf.format(lowerXmm)) + " " + xUnitsString + ", " + String.valueOf(nf.format(lowerYmm)) + " " +
             * yUnitsString + ")"; upperLeftLocationString = "(" + String.valueOf((int) (xBounds[0] + 0.5f)) + "," +
             * String.valueOf((int) (yBounds[0] + 0.5f)) + ")"; g.setColor(Color.black);
             *  // System.err.println(xBounds[0] + " " + xBounds[1] + " " + // yBounds[0] + " " + yBounds[1]); if ((y1 -
             * 45) < 0) { g.drawString(measuredWidthString, (x0 + ((x1 - x0) / 2)) - 20, y1 + 21);
             * g.drawString(measuredWidthString, (x0 + ((x1 - x0) / 2)) - 20, y1 + 19);
             * g.drawString(measuredWidthString, (x0 + ((x1 - x0) / 2)) - 21, y1 + 20);
             * g.drawString(measuredWidthString, (x0 + ((x1 - x0) / 2)) - 19, y1 + 20); g.drawString(widthString, (x0 +
             * ((x1 - x0) / 2)) - 20, y1 + 36); g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 20, y1 + 34);
             * g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 21, y1 + 35); g.drawString(widthString, (x0 + ((x1 -
             * x0) / 2)) - 19, y1 + 35); g.setColor(Color.white); g.drawString(measuredWidthString, (x0 + ((x1 - x0) /
             * 2)) - 20, y1 + 20); g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 20, y1 + 35); } else {
             ...
             * g.drawString(measuredWidthString, (x0 + ((x1 - x0) / 2)) - 19, y1 - 25); g.drawString(widthString, (x0 +
             * ((x1 - x0) / 2)) - 20, y1 - 9); g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 20, y1 - 11);
             * g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 21, y1 - 10); g.drawString(widthString, (x0 + ((x1 -
             * x0) / 2)) - 19, y1 - 10); g.setColor(Color.white); g.drawString(measuredWidthString, (x0 + ((x1 - x0) /
             * 2)) - 20, y1 - 25); g.drawString(widthString, (x0 + ((x1 - x0) / 2)) - 20, y1 - 10); }
             * 
             * g.setColor(Color.black);
             * 
             * if ((x0 - 40) < 0) { g.drawString(measuredHeightString, x0 + 10, y0 + 10 + ((y1 - y0) / 2) + 1);
             * g.drawString(measuredHeightString, x0 + 10, y0 + 10 + ((y1 - y0) / 2) - 1);
             * g.drawString(measuredHeightString, x0 + 9, y0 + 10 + ((y1 - y0) / 2)); g.drawString(measuredHeightString,
             * x0 + 11, y0 + 10 + ((y1 - y0) / 2)); g.drawString(heightString, x0 + 10, y0 + 25 + ((y1 - y0) / 2) + 1);
             * g.drawString(heightString, x0 + 10, y0 + 25 + ((y1 - y0) / 2) - 1); g.drawString(heightString, x0 + 9, y0 +
             * 25 + ((y1 - y0) / 2)); g.drawString(heightString, x0 + 11, y0 + 25 + ((y1 - y0) / 2));
             * g.setColor(Color.white); g.drawString(measuredHeightString, x0 + 10, y0 + 10 + ((y1 - y0) / 2));
             * g.drawString(heightString, x0 + 10, y0 + 25 + ((y1 - y0) / 2)); } else {
             * g.drawString(measuredHeightString, x0 - 35, y0 + 10 + ((y1 - y0) / 2) + 1);
             * g.drawString(measuredHeightString, x0 - 35, y0 + 10 + ((y1 - y0) / 2) - 1);
             * g.drawString(measuredHeightString, x0 - 36, y0 + 10 + ((y1 - y0) / 2));
             * g.drawString(measuredHeightString, x0 - 34, y0 + 10 + ((y1 - y0) / 2)); g.drawString(heightString, x0 -
             * 35, y0 + 25 + ((y1 - y0) / 2) + 1); g.drawString(heightString, x0 - 35, y0 + 25 + ((y1 - y0) / 2) - 1);
             * g.drawString(heightString, x0 - 36, y0 + 25 + ((y1 - y0) / 2)); g.drawString(heightString, x0 - 34, y0 +
             * 25 + ((y1 - y0) / 2)); g.setColor(Color.white); g.drawString(measuredHeightString, x0 - 35, y0 + 10 +
             * ((y1 - y0) / 2)); g.drawString(heightString, x0 - 35, y0 + 25 + ((y1 - y0) / 2)); }
             * 
             * g.setColor(Color.black);
             * 
             * if (((x0 - 40) <= 0) && ((y0 - 45) <= 0)) { g.drawString(lowerXYmmString, x0 + 10, y0 + 11);
             * g.drawString(lowerXYmmString, x0 + 10, y0 + 13); g.drawString(lowerXYmmString, x0 + 9, y0 + 12);
             * g.drawString(lowerXYmmString, x0 + 11, y0 + 12); g.setColor(Color.white); g.drawString(lowerXYmmString,
             * x0 + 10, y0 + 12); g.setColor(Color.black); g.drawString(upperLeftLocationString, x0 + 10, y0 + 29);
             * g.drawString(upperLeftLocationString, x0 + 10, y0 + 17); g.drawString(upperLeftLocationString, x0 + 9, y0 +
             * 28); g.drawString(upperLeftLocationString, x0 + 11, y0 + 28); g.setColor(Color.white);
             * g.drawString(upperLeftLocationString, x0 + 10, y0 + 28); } else if (((x0 - 40) <= 0) && ((y0 - 45) > 0)) {
             * g.drawString(lowerXYmmString, x0 + 10, y0 - 25); g.drawString(lowerXYmmString, x0 + 10, y0 - 27);
             * g.drawString(lowerXYmmString, x0 + 9, y0 - 26); g.drawString(lowerXYmmString, x0 + 11, y0 - 26);
             * g.setColor(Color.white); g.drawString(lowerXYmmString, x0 + 10, y0 - 26); g.setColor(Color.black);
             * g.drawString(upperLeftLocationString, x0 + 10, y0 - 9); g.drawString(upperLeftLocationString, x0 + 10, y0 -
             * 21); g.drawString(upperLeftLocationString, x0 + 9, y0 - 10); g.drawString(upperLeftLocationString, x0 +
             * 11, y0 - 10); g.setColor(Color.white); g.drawString(upperLeftLocationString, x0 + 10, y0 - 10); } else if
             * (((x0 - 40) > 0) && ((y0 - 45) <= 0)) { g.drawString(lowerXYmmString, x0 - 35, y0 + 11);
             * g.drawString(lowerXYmmString, x0 - 35, y0 + 13); g.drawString(lowerXYmmString, x0 - 36, y0 + 12);
             * g.drawString(lowerXYmmString, x0 - 34, y0 + 12); g.setColor(Color.white); g.drawString(lowerXYmmString,
             * x0 - 35, y0 + 12); g.setColor(Color.black); g.drawString(upperLeftLocationString, x0 - 35, y0 + 29);
             * g.drawString(upperLeftLocationString, x0 - 35, y0 + 17); g.drawString(upperLeftLocationString, x0 - 36,
             * y0 + 28); g.drawString(upperLeftLocationString, x0 - 34, y0 + 28); g.setColor(Color.white);
             * g.drawString(upperLeftLocationString, x0 - 35, y0 + 28); } else { g.drawString(lowerXYmmString, x0 - 35,
             * y0 - 25); g.drawString(lowerXYmmString, x0 - 35, y0 - 27); g.drawString(lowerXYmmString, x0 - 36, y0 -
             * 26); g.drawString(lowerXYmmString, x0 - 34, y0 - 26); g.setColor(Color.white);
             * g.drawString(lowerXYmmString, x0 - 35, y0 - 26); g.setColor(Color.black);
             * g.drawString(upperLeftLocationString, x0 - 35, y0 - 9); g.drawString(upperLeftLocationString, x0 - 35, y0 -
             * 11); g.drawString(upperLeftLocationString, x0 - 36, y0 - 10); g.drawString(upperLeftLocationString, x0 -
             * 34, y0 - 10); g.setColor(Color.white); g.drawString(upperLeftLocationString, x0 - 35, y0 - 10); }
             *  // System.err.println("height: " + heightString + " width: " + // widthString);
             * g.setColor(Color.yellow.brighter());
             * 
             * switch (nearBoundPoint) {
             * 
             * case 1: g.fillRect(x0 - 2, y0 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 - 2, y0 - 2, 4, 4);
             * break;
             * 
             * case 2: g.fillRect(x1 - 2, y0 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x1 - 2, y0 - 2, 4, 4);
             * break;
             * 
             * case 3: g.fillRect(x1 - 2, y1 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x1 - 2, y1 - 2, 4, 4);
             * break;
             * 
             * case 4: g.fillRect(x0 - 2, y1 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 - 2, y1 - 2, 4, 4);
             * break;
             * 
             * case 5: g.fillRect(x0 + ((x1 - x0) / 2) - 2, y0 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 + ((x1 -
             * x0) / 2) - 2, y0 - 2, 4, 4); break;
             * 
             * case 6: g.fillRect(x1 - 2, y0 + ((y1 - y0) / 2) - 2, 5, 5); g.setColor(Color.black); g.drawRect(x1 - 2,
             * y0 + ((y1 - y0) / 2) - 2, 4, 4); break;
             * 
             * case 7: g.fillRect(x0 + ((x1 - x0) / 2) - 2, y1 - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 + ((x1 -
             * x0) / 2) - 2, y1 - 2, 4, 4); break;
             * 
             * case 8: g.fillRect(x0 - 2, y0 + ((y1 - y0) / 2) - 2, 5, 5); g.setColor(Color.black); g.drawRect(x0 - 2,
             * y0 + ((y1 - y0) / 2) - 2, 4, 4); break; } }
             */
        }
    }

    private void drawVOILine(final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure, final Graphics g,
            final int thickness) {
        ...
        if (kVOI.isActive()) {
            ...
            if ( (kVOI.getSelectedPoint() >= 0) && (kVOI.size() > kVOI.getSelectedPoint())) {
                g.setColor(Color.GREEN);
                g.fillRect((int) (x[kVOI.getSelectedPoint()] - 1.5 + 0.5f),
                        (int) (y[kVOI.getSelectedPoint()] - 1.5 + 0.5f), 3, 3);
            }

            if ( !kVOI.isSplit()) {
                drawTickMarks(kVOI, g, currentColor, unitsOfMeasure, m_kDrawingContext.getWidth(), m_kDrawingContext
                        .getHeight(), resols);
            }
        }
    }

    private void drawVOIPoint(final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure, final Graphics g) {
    ...
    private void drawVOIPoint(final VOIBase kVOI, final Graphics g, final String label) {
        final boolean doName = (Preferences.is(Preferences.PREF_SHOW_VOI_NAME) && kVOI.getName() != null);

        final Vector3f kFile = kVOI.get(0);
        final Vector3f kScreen = m_kDrawingContext.fileToScreen(kFile);
        final int x = Math.round(kFile.X);
        final int y = Math.round(kFile.Y);
        final int xS = Math.round(kScreen.X);
        final int yS = Math.round(kScreen.Y);
        ...
        if (kVOI.getType() != VOI.POLYLINE_SLICE) {
            ...
            int type = 0;
            final String typeStr = Preferences.getProperty(Preferences.PREF_VOI_POINT_DRAW_TYPE);
            if (typeStr != null) {
                try {
            ...
            final VOIPoint kPolyPoint = (VOIPoint) kVOI;
    ...
    private void drawVOIPolyLineSlice(final VOIPolyLineSlice kVOI, final float[] resols, final int[] unitsOfMeasure,
            final Graphics g, final int slice, final int orientation) {
        final Color color = g.getColor();

        final String totalDistance = kVOI.getTotalLengthString(resols, unitsOfMeasure);
        ...
        for (int i = 0; i < kVOI.size(); i++) {
            ...
            final int sliceI = getSlice(kVOI.getPoints().get(i));
            if (sliceI == m_iSlice) {
        ...
        for (int i = 0; i < kVOI.size() - 1; i++) {
            final int sliceI = getSlice(kVOI.getPoints().get(i));
            final int sliceIP1 = getSlice(kVOI.getPoints().get(i + 1));
            if (sliceI == sliceIP1 && sliceI == m_iSlice) {
                final Vector3f kStart = m_kDrawingContext.fileToScreen(kVOI.get(i));
                final Vector3f kEnd = m_kDrawingContext.fileToScreen(kVOI.get(i + 1));
                g.setColor(color);
    ...
    private void drawVOIProtractor(final VOIBase kVOI, final float[] resols, final int[] unitsOfMeasure,
            final Graphics g) {
        ...
        if (kVOI.isActive()) {
            this.drawTickMarks(kVOI, g, unitsOfMeasure, m_kDrawingContext.getWidth(), m_kDrawingContext.getHeight(),
                    resols);
        }
    }

    private void drawVOIText(final VOIText kVOI, final Graphics g) {
        ...
        final int xS2 = Math.round(kScreen.X);
        final int yS2 = Math.round(kScreen.Y);

        // draw the arrow if useMarker is true
        if (kVOI.useMarker()) {
            // determine the width/height of the TEXT (for marker line location)
            final int width = (g.getFontMetrics(kVOI.getTextFont()).stringWidth(kVOI.getText()));
            final int ascentValue = (int) (g.getFontMetrics(kVOI.getTextFont()).getStringBounds(kVOI.getText(), g)
                    .getHeight() / 2);
        ...
        if ( (kVOI.getTextFont() != null) && (kVOI.getTextFont().getName() == kVOI.getFontName())
                && (kVOI.getTextFont().getStyle() == kVOI.getFontDescriptors())) {
    ...
    private void getCoordsLine(final float[] linePtsX, final float[] linePtsY, final double fraction,
            final float[] coords) {
        float x1, y1;
    ...
    private void getCoordsProtractor(final float[] x, final float[] y, final double fraction, final float[] coords) {
        float x1, y1;
        double vector1, vector2, tmp;
        double length;
        x1 = (x[0] + x[1]) / 2;
    ...
    private void getEndLinesLine(final float[] linePtsX, final float[] linePtsY, final int line, final float[] coords) {
        double vector1, vector2, tmp;
        double length;
        vector1 = (linePtsX[1] - linePtsX[0]);
        vector2 = (linePtsY[1] - linePtsY[0]);
    ...
    private void getEndLinesProtractor(final float[] x, final float[] y, final int line, final float[] coords) {
        double vector1, vector2, tmp;
        double length;
        vector1 = (x[1] - x[0]);
        vector2 = (y[1] - y[0]);
    ...
    private int getSlice(final VOIBase kVOI) {
    ...
    private void handleIntensityLineBtn3(final MouseEvent mouseEvent) {
        ...
        menuItem.setActionCommand(VOIManager.SHOW_INTENSITY_GRAPH);
        menuItem = new JMenuItem("Delete this intensity line");
        popupMenu.add(menuItem);
        menuItem.addActionListener(this);
        menuItem.setActionCommand(VOIManager.DELETE_INTENSITY_LINE);
        popupMenu.show(m_kComponent, mouseEvent.getX(), mouseEvent.getY());
    }

    private void initLiveWire(final int iSlice, final boolean bLiveWire) {
        if ( !m_bLiveWireInit) {
            if (m_iPlaneOrientation != m_kImageActive.getImageOrientation()
                    && m_iPlaneOrientation != FileInfoBase.UNKNOWN_ORIENT) {
                final int[] aiAxisOrder = MipavCoordinateSystems.getAxisOrder(m_kImageActive, m_iPlaneOrientation);
                final boolean[] abAxisFlip = MipavCoordinateSystems.getAxisFlip(m_kImageActive, m_iPlaneOrientation);
            ...
            final int nSlices = m_aiLocalImageExtents.length > 2 ? m_aiLocalImageExtents[2] : 1;
        ...
        final int xDim = m_kLocalImage.getExtents()[0];
        final int yDim = m_kLocalImage.getExtents()[1];
        ...
        if (bLiveWire) {
            final ViewJProgressBar progressBar = new ViewJProgressBar(m_kImageActive.getImageName(),
                    "Livewire: Computing cost function ...", 0, 100, false, this, null);
            localCosts = RubberbandLivewire.getLocalCosts(m_kLocalImage, m_iLiveWireSelection, imageBufferActive,
    ...
    private void pasteVOI(final int iSlice) {
        ...
        for (int i = 0; i < m_kCopyVOI.size(); i++) {
            final Vector3f kPos = fileCoordinatesToPatient(m_kCopyVOI.get(i));
            kPos.Z = iSlice;
    ...
    private void processLeftMouseDrag(final MouseEvent kEvent) {
        ...
        if (m_bDrawVOI
                && ! ( ( (m_iDrawType == VOIManager.POINT) || (m_iDrawType == VOIManager.POLYPOINT)
                        || (m_iDrawType == VOIManager.LIVEWIRE) || (m_iDrawType == VOIManager.LEVELSET) || (m_iDrawType == VOIManager.TEXT)))) {
            createVOI(kEvent.getX(), kEvent.getY());
        } else if (m_bPointer) {
            if (m_iNearStatus == VOIManager.NearPoint) {
                ...
                m_kParent.moveVOI(this, new Vector3f(kEvent.getX() - m_fMouseX, kEvent.getY() - m_fMouseY, 0),
                        m_iPlane, m_bFirstDrag);
                m_fMouseX = kEvent.getX();
                m_fMouseY = kEvent.getY();
            }
        }
    ...
    private Polygon scalePolygon(final VOIBase kVOI) {
        int i;
        int x;
        int y;
        ...
        for (i = 0; i < kVOI.size(); i++) {
            final Vector3f kVolumePt = kVOI.elementAt(i);
            final Vector3f kScreen = m_kDrawingContext.fileToScreen(kVolumePt);
     ...
     * 
     * <p>
     * The array seededCosts holds the costs so far for a location. If a cost has not been assigned yet, the cost is -2.
     * ActiveList is simply a linked list of Integers; the Integer refers to the location in the seededCosts array.
     ...
     * expense of memory, but it's not too much memory in the overall scheme of things.
     * </p>
     * 
     * <p>
     * The gradient direction component of the cost is added in on the fly. This is because to precalculate would mean
     * an array of 8n, where n is the size of the image. The link from p to q is not the same as the link from q to p.
     * Furthermore, it may never be necessary to calculate some of the links, because the graph would never look at that
     * pair. For more information on how the gradient direction cost is calculated, look at the comments directly above
     * the code.
     * </p>
     * 
     * @param pt Point to seed with.
     */
    private void seed(final int iX, final int iY) {
        final Vector3f kVolumePt = new Vector3f();
        m_kDrawingContext.screenToFile(iX, iY, m_iSlice, kVolumePt);
        final Vector3f kLocalPt = fileCoordinatesToPatient(kVolumePt);

        final int xDim = m_aiLocalImageExtents[0];
        final int yDim = m_aiLocalImageExtents[1];
        int x = (int) kLocalPt.X;
        int y = (int) kLocalPt.Y;
        ...
        final float gdConst = (float) (2f / (3 * Math.PI));
        ...
        while (activeTree.isEmpty() == false) { // while active list has more elements to process
            ...
            for (int iy = -1; iy <= 1; iy++) {
                final int yOffset = (y + iy) * xDim;

                if ( ( (y + iy) >= 0) && ( (y + iy) < yDim)) { // in bounds in the y dimension

                    for (int ix = -1; ix <= 1; ix++) {
                        final int position = yOffset + (x + ix);
                        ...
                        if ( ( (x + ix) >= 0) && ( (x + ix) < xDim) && // in bounds in the x dimension
                            ...
                            if ( (ix != 0) && (iy != 0)) { // diagonal costs more
                                temp = cost + ( (localCosts[position] + (grad_weight * gradDir)) * 1.4142f); // *
                                                                                                                // square
                                                                                                                // root
                                                                                                                // of 2,
                                                                                                                // Euclidean
    ...
    private VOIBase selectVOI(final int iX, final int iY, final boolean bShiftDown) {
        m_kParent.selectAllVOIs(false);
        m_bSelected = false;
        m_kCurrentVOI = null;
        final VOIVector kVOIs = m_kImageActive.getVOIs();
        for (int i = kVOIs.size() - 1; i >= 0; i--) {
            final VOI kVOI = kVOIs.get(i);
            for (int j = kVOI.getCurves().size() - 1; j >= 0; j--) {
                final VOIBase kVOI3D = kVOI.getCurves().get(j);
                if ( (m_iPlane == (m_iPlane & kVOI3D.getPlane())) && (m_iSlice == getSlice(kVOI3D))
    ...
    private void setPosition(final VOIBase kVOI, final int iPos, final float fX, final float fY, final float fZ) {
        setPosition(kVOI, iPos, new Vector3f(fX, fY, fZ));
    }

    private void setPosition(final VOIBase kVOI, final int iPos, final Vector3f kPos) {
        if (kVOI.isFixed()) {
            return;
        }
    ...
    private void showSelectedVOI(final int iX, final int iY) {
        ...
        if (m_kCurrentVOI != null && (m_iSlice == getSlice(m_kCurrentVOI))) {
            if (nearPoint(m_kCurrentVOI, iX, iY, m_iSlice)) {
                ...
                if ( (m_kCurrentVOI.getType() == VOI.CONTOUR) || (m_kCurrentVOI.getType() == VOI.POLYLINE)) {
                    m_kParent.setCursor(MipavUtil.addPointCursor);
                    m_iNearStatus = VOIManager.NearLine;
                } else {
                    m_kParent.setCursor(MipavUtil.moveCursor);
                    m_iNearStatus = VOIManager.NearNone;
                }
    ...
    private VOIBase split(final VOIBase kVOI, final Vector3f kStartPt, final Vector3f kEndPt) {
        if (kVOI.getType() != VOI.CONTOUR) {
            return null;
        }
        final int iVOISlice = (int) fileCoordinatesToPatient(kVOI.get(0)).Z;
        final int iPoints = kVOI.size();
        ...
        for (int iP = 0; iP < (iPoints - 1) && (kSecondIntersectionPt == null); iP++) {
            final Vector3f kLocal1 = fileCoordinatesToPatient(kVOI.get(iP));
            final Vector3f kLocal2 = fileCoordinatesToPatient(kVOI.get(iP + 1));
            final Vector3f kIntersection = new Vector3f();
        ...
        if (kSecondIntersectionPt == null) {
            final Vector3f kLocal1 = fileCoordinatesToPatient(kVOI.lastElement());
            final Vector3f kLocal2 = fileCoordinatesToPatient(kVOI.firstElement());
            final Vector3f kIntersection = new Vector3f();
    ...
    private void splitVOIs(final boolean bAllSlices, boolean bOnlyActive, VOIBase kSplitVOI) {
        final VOIVector kVOIs = m_kImageActive.getVOIs();
        if ( !kSplitVOI.isSplit() || (kVOIs.size() == 0)) {
            return;
        }
        final Vector<VOIBase> kNewVOIs = new Vector<VOIBase>();

        final Vector3f kStartPt = fileCoordinatesToPatient(kSplitVOI.get(0));
        final Vector3f kEndPt = fileCoordinatesToPatient(kSplitVOI.get(1));

        final int iStartSlice = bAllSlices ? 0 : m_iSlice;
        final int nSlices = m_aiLocalImageExtents.length > 2 ? m_aiLocalImageExtents[2] : 1;
        final int iEndSlice = bAllSlices ? nSlices : m_iSlice + 1;
        ...
        for (int i = 0; i < kVOIs.size(); i++) {
            ...
            for (int j = 0; j < kVOI.getCurves().size(); j++) {
                ...
                final int iVOISlice = kVOI3D.slice();
                ...
                final VOIBase kNew = split(kVOI3D, kStartPt, kEndPt);
                if (kNew != null) {
     ...
     */
    private float avgPix(final int index) {
        final int xDim = m_aiLocalImageExtents[0];
     ...
     * @param i DOCUMENT ME!
     */
    private void paths(final int index, final int i, final float level) {
     ...
     * @param level the level of the level set private VOIBase singleLevelSet(float startPtX, float startPtY, float
     *            level) {
     * 
     * int x, y; int index; double distance; stack.removeAllElements();
     * 
     * if (imageBufferActive == null) { return; }
     * 
     * int xDim = m_aiLocalImageExtents[0]; int yDim = m_aiLocalImageExtents[1];
     * 
     * for (int i = 0; i < map.size(); i++) { map.clear(i); }
     * 
     * if (startPtX >= (xDim - 1)) { return; }
     * 
     * if (startPtY >= (yDim - 1)) { return; }
     * 
     * x = (int) (startPtX + 0.5); y = (int) (startPtY + 0.5);
     * 
     * levelSetStack.reset(); levelSetStack.addPoint(x, y); map.set((y * xDim) + x);
     * 
     * int dir = -1; float diff = 100000;
     * 
     * do { index = (y * xDim) + x;
     * 
     * if ((x >= 2) && (x < (xDim - 2)) && (y >= 2) && (y < (yDim - 2))) {
     * 
     * if ((avgPix(index - xDim) >= level) && ((avgPix(index - xDim + 1) < level) || (avgPix(index) < level) ||
     * (avgPix(index - xDim - 1) < level) || (avgPix(index - (2 * xDim)) < level)) && (map.get(index - xDim) == false)) {
     * dir = 1; diff = Math.abs(avgPix(index - xDim) - avgPix(index)); }
     * 
     * if ((avgPix(index - xDim + 1) >= level) && ((avgPix(index - xDim + 2) < level) || (avgPix(index + 1) < level) ||
     * (avgPix(index - xDim) < level) || (avgPix(index - (2 * xDim) + 1) < level)) && (map.get(index - xDim + 1) ==
     * false)) {
     * 
     * if (Math.abs(avgPix(index - xDim + 1) - avgPix(index)) < diff) { dir = 2; diff = Math.abs(avgPix(index - xDim +
     * 1) - avgPix(index)); } }
     * 
     * if ((avgPix(index + 1) >= level) && ((avgPix(index + 2) < level) || (avgPix(index + xDim + 1) < level) ||
     * (avgPix(index) < level) || (avgPix(index - xDim + 1) < level)) && (map.get(index + 1) == false)) {
     * 
     * if (Math.abs(avgPix(index + 1) - avgPix(index)) < diff) { dir = 3; diff = Math.abs(avgPix(index + 1) -
     * avgPix(index)); } }
     * 
     * if ((avgPix(index + xDim + 1) >= level) && ((avgPix(index + xDim + 2) < level) || (avgPix(index + (2 * xDim) + 1) <
     * level) || (avgPix(index + 1) < level) || (avgPix(index + xDim) < level)) && (map.get(index + xDim + 1) == false)) {
     * 
     * if (Math.abs(avgPix(index + xDim + 1) - avgPix(index)) < diff) { dir = 4; diff = Math.abs(avgPix(index + xDim +
     * 1) - avgPix(index)); } }
     * 
     * if ((avgPix(index + xDim) >= level) && ((avgPix(index + xDim + 1) < level) || (avgPix(index + (2 * xDim)) <
     * level) || (avgPix(index + xDim - 1) < level) || (avgPix(index) < level)) && (map.get(index + xDim) == false)) {
     * 
     * if (Math.abs(avgPix(index + xDim) - avgPix(index)) < diff) { dir = 5; diff = Math.abs(avgPix(index + xDim) -
     * avgPix(index)); } }
     * 
     * if ((avgPix(index + xDim - 1) >= level) && ((avgPix(index + xDim) < level) || (avgPix(index + (2 * xDim) - 1) <
     * level) || (avgPix(index + xDim - 2) < level) || (avgPix(index - 1) < level)) && (map.get(index + xDim - 1) ==
     * false)) {
     * 
     * if (Math.abs(avgPix(index + xDim - 1) - avgPix(index)) < diff) { dir = 6; diff = Math.abs(avgPix(index + xDim -
     * 1) - avgPix(index)); } }
     * 
     * if ((avgPix(index - 1) >= level) && ((avgPix(index) < level) || (avgPix(index + xDim - 1) < level) ||
     * (avgPix(index - 2) < level) || (avgPix(index - xDim - 1) < level)) && (map.get(index - 1) == false)) {
     * 
     * if (Math.abs(avgPix(index - 1) - avgPix(index)) < diff) { dir = 7; diff = Math.abs(avgPix(index - 1) -
     * avgPix(index)); } }
     * 
     * if ((avgPix(index - xDim - 1) >= level) && ((avgPix(index - xDim) < level) || (avgPix(index - 1) < level) ||
     * (avgPix(index - xDim - 2) < level) || (avgPix(index - (2 * xDim) - 1) < level)) && (map.get(index - xDim - 1) ==
     * false)) {
     * 
     * if (Math.abs(avgPix(index - xDim - 1) - avgPix(index)) < diff) { dir = 0; // diff =
     * Math.abs(imageBufferActive[index-xDim-1] - imageBufferActive[index]); } }
     * 
     * diff = 1000000;
     * 
     * if (dir == 1) { // x = x; y = y - 1; map.set(index - xDim); paths(index, 1, level); } else if (dir == 2) { x = x +
     * 1; y = y - 1; map.set(index - xDim + 1); paths(index, 2, level); } else if (dir == 3) { x = x + 1; // y = y;
     * map.set(index + 1); paths(index, 3, level); } else if (dir == 4) { x = x + 1; y = y + 1; map.set(index + xDim +
     * 1); paths(index, 4, level); } else if (dir == 5) { // x = x; y = y + 1; map.set(index + xDim); paths(index, 5,
     * level); } else if (dir == 6) { x = x - 1; y = y + 1; map.set(index + xDim - 1); paths(index, 6, level); } else if
     * (dir == 7) { x = x - 1; // y = y; map.set(index - 1); paths(index, 7, level); } else if (dir == 0) { x = x - 1; y =
     * y - 1; map.set(index - xDim - 1); paths(index, 0, level); } else {
     * 
     * if (!stack.empty()) { int ptr = ((int[]) stack.pop())[0]; x = levelSetStack.getPointX(ptr); y =
     * levelSetStack.getPointY(ptr); levelSetStack.setIndex(ptr); } else { x = y = -1; } }
     * 
     * dir = -1; } else { // near edge of image levelSetStack.reset();
     * 
     * break; }
     * 
     * if ((x == -1) || (y == -1)) { levelSetStack.reset();
     * 
     * break; }
     * 
     ...
     * 
     * if ((distance < 2.1) && (levelSetStack.size() < 10)) { distance = 10; } } while (distance > 2.1);
     * 
     * 
     * if (levelSetStack.size() != 0) { return levelSetStack.exportPolygon(); } else { System.err.println(
     * "singleLevelSet return null" ); return null; } }
     * 
     */
    ...
    private VOIBase singleLevelSet2(float startPtX, float startPtY) {
        ...
        final float level = imageBufferActive[y * xDim + x];
        ...
        do {
            ...
            if ( (x >= 2) && (x < (xDim - 2)) && (y >= 2) && (y < (yDim - 2))) {
                ...
                if ( (avgPix(index + 1) >= level)
                        && ( (avgPix(index + 2) < level) || (avgPix(index + xDim + 1) < level)
                                || (avgPix(index) < level) || (avgPix(index - xDim + 1) < level))
