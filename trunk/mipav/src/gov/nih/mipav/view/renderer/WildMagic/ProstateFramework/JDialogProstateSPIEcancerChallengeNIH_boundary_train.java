 ...
 * This class generates the training case for prostate boundary.   
 * 
 * Unfortunately, I never get a chance to run the testing case. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstateSPIEcancerChallengeNIH_boundary_train extends JDialogBase
	...
	public void runScaleIntensity() {
		...
		for (String key : keys) {
			...
			scaleIntensityTable.put(key, scaleIntensity(keyImage));
		...
		try {
			...
			for (int j = 0; j < zDim; j++) {
				...
				try {
					...
					savePNGfile(sliceDir, imgName, targetImageSlice, min, max,
							xDim, yDim, false);

					String maskName = "voi_" + index[0] + ".png";
					savePNGfile(sliceDir, maskName, maskImage, min, max, xDim,
							yDim, true);
