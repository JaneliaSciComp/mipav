...
public class ModelTriangleMeshCurveSegments
{
     ...
     *
     * The ModelTriangleMeshCurveSegments constructor takes a
     * ModelTriangleMesh, a list of Curves, and the point samples for those
     * curves and produces a collection of sub-meshes smaller than the
     * original mesh. The sub-meshes are constructed such that when the
     * viewpoint is inside one of the sub-meshes only the triangles in that
     * sum-mesh are visible. Allowing the program to send a smaller number of
     * triangles through the graphics piplines will increase performance for
     * large triangle meshes.
     *
     * The Algorthim involves three steps:
     *
     * 1). First each curve is divided into segments based on curvature and
     * the length of the segments. Breaking the curves at places of high
     * curvature is a heuristic that works because the TriangleMesh is assumed
     * to be a tube, for example the colon or brachia in the lung. If the tube
     * bends, then triangles on the far side of the curve are not visible.
     *
     * 2). Second the radial distance from the curve through the
     * TriangleMesh tube is calculated for each position on the
     * curve. This distance is used to determine the maximum width of the
     * tube for each position on the FlyPath curve.
     *
     * 3). Third, the triangles are distributed into sub-meshes. Each sub-mesh
     * represents the visible triangles along each of the curve
     * segments. Because triangles associated with the adjacent curve segments
     * are visible at the start and end of each curve segment, each sub-mesh
     * contains the triangles for the previous curve segment, the current
     * curve segment, and the next curve segment. If a branch occurs in the
     * current curve segment, the triangles associated with the first segment
     * on that branch curve are also included in the sub-mesh.
     *
     */
    public ModelTriangleMeshCurveSegments(ModelTriangleMesh kMesh,
                                          FlyPathGraphSamples kGraphSamples,
                                          FlyPathGraphCurve kGraphCurve,
                                          int iPathSamplesReductionFactor)
    {
         ...
         * curvature of the FlyPath using the maximum tube widths to create
         * bounding spherical volumes for the segmentation.
         */
    ...
    /*
     * segmentCurvesByTangent produces a segmentation of the curves stored in
     * the m_akCurvePosition data member. The segmentation is based on how
     * much the tangent of the curve changes from the last segment endpoint to
     * the current position on the curve. If the angle between the tangent at
     * the current position and the vector between the current position and
     * the last segment endpoint is greater than 45 degrees, and the
     * distance along the path between the current point and the last segment
     * end point is greater than a 5% of the overall curve length, then the
     * curve is split, and a new curve segment is started. If the new segment
     * length is greater than 10% of the overall curve length, it is divided into
     * two new segments each with half the length.
     *
     * The Curve segment endpoints, including the first start point at 0.0 and
     * the last endpoint at 1.0, are stored in the data member
     * m_aafBreakPoints as relative distances along the curve. The numbers of
     * break points for each curve are stored in the data member
     * m_aiNumBreakPoints. There will be m_aiNumBreakPoints - 1 mesh segments.
     *
     * The Curve is sampled based on the original voxel path sample
     * points. These points are also used in the findMaxMeshDistance function,
     * so they are stored in the data member m_aakCurvePos, the tangents at
     * the sample points are also stored for later use in the data member
     * m_aakTangent.
     *
     * The relative distances of each segment endpoint are stored in the data
    ...
    protected void segmentCurvesByTangent()
    {
        ...
        /*
         * Loop over the number of curves in the kGraphCurve.
         * Divide each curve into segments based on curvature,
         * and on the length of the curve segment.
         */
        ...
        for (int iCurve = 0; iCurve < m_iNumCurves; iCurve++)
        {
          ...
          /* Retrieve the path sample points from m_kGraphSamples the sample
             * points were used to create the BSplines and are used calculate
             * the relative distance of points along the BSpline.  First the
             * segment path lengths and the total path length are calculated,
             * so that the relative path can be stored for use later
             */
            ...
            for (int iSample = 0; iSample < akSamplePos.length; iSample++)
            {
                ...
                else
                {
                     ...
                     */
                    afTotalLength[iCurve] +=
                        akSamplePos[iSample].distance(akSamplePos[iSample-1]);
             ...
             */
            /*
            ViewJSimpleProgressBar kProgress =
                new ViewJSimpleProgressBar("Segmenting Triangle Mesh",
            ...
            for (int iSample = 0; iSample < akSamplePos.length; iSample++)
            {
                 ...
                 * points.
                 */
                m_aafRelativeLengths[iCurve][iSample] =
                    (aafLengths[iCurve][iSample] / afTotalLength[iCurve]);
                 ...
                 */
                m_aakCurvePos[iCurve][iSample] =
                    m_akCurvePosition[iCurve].getPosition(fTime);

                /* If this is the first time through the loop, save the first
                 * point and index, also set the first CurveBreakPoint to this
                 * fTime.
                */
                if (iSample == 0)
                {
                    ...
                    aafCurveBreakPoints[iCurve][m_aiNumBreakPoints[iCurve]++] =
                        fTime;
                 ...
                 * the curvature from the current point on the Curve to the
                 * last point where the Curve was segmented.
                 */
                else if (iSample != 0)
                {
                    /* Use the tangent of the Curve at the current fTime to
                     * determine curvature. It is stored in a data member for
                     * use in later functions.
                     */
                    m_aakTangent[iCurve][iSample] =
                        m_akCurvePosition[iCurve].getTangent(fTime);

                    /* Calculate the difference vector between the current
                     *  point and the last point where the curve was segmented.
                     */
                    kDifference.x = m_aakCurvePos[iCurve][iSample].x -
                        kPrevCurvePos.x;
                    kDifference.y = m_aakCurvePos[iCurve][iSample].y -
                        kPrevCurvePos.y;
                    kDifference.z = m_aakCurvePos[iCurve][iSample].z -
                        kPrevCurvePos.z;
                     ...
                     * break point */
                    fCurveSegmentPathLength = fTime -
                        m_aafRelativeLengths[iCurve][iPrevIndex];


                    /* If the angle is greater than or equal to 45 degrees,
                     * and the segment length is greater than a5% of
                     * the overall curve length, then segment the curve, and
                         ...
                         )
                    {
                        ...
                        fPreviousTime = aafCurveBreakPoints[iCurve]
                            [m_aiNumBreakPoints[iCurve] - 1];

                        if ((fTime - fPreviousTime) > 0.10)
                        {
                            fHalf =(float)((fTime + fPreviousTime)/2.0);
                            aafCurveBreakPoints[iCurve]
                                [m_aiNumBreakPoints[iCurve]++] = fHalf;
                        }

                        aafCurveBreakPoints[iCurve]
                            [m_aiNumBreakPoints[iCurve]++] = fTime;
            ...
            // kProgress.dispose();
            aafCurveBreakPoints[iCurve]
                [m_aiNumBreakPoints[iCurve]++] = (float)(1.0);
        }
        ...
        for (int iCurve = 0; iCurve < m_iNumCurves; iCurve++)
        {
            m_aafBreakPoints[iCurve] =
                new float[m_aiNumBreakPoints[iCurve]];

            for (int iBreakPoint = 0;
                  iBreakPoint < m_aiNumBreakPoints[iCurve];
                  iBreakPoint++)
            {
                m_aafBreakPoints[iCurve][iBreakPoint] =
                    aafCurveBreakPoints[iCurve][iBreakPoint];
            }
        }
    ...
    /*
     * @param int iDownSample
     *
     * findMaxMeshDistance approximates the width of the TriangleMesh tube for
     * each sample point on each curve.
     *
     * Given the tangent direction, pointing down the tube in the direction of
     * the FlyPath curve, four vectors perpendicular to the FlyPath direction
     * are calculated, one pointing to the "top" of the tube, and ones
     * pointing to the "left", "bottom", and "right" sides of the tube
     * respectively.
     *
     * The distance along each of these four vectors to every vertex in the
     * triangle array is evaluated, and the minimum distance for each vector
     * is stored. The maximum of the four minimum distances is used to
     * approximate the "width" of the tube at the sample point along the Curve
     * path.
     *
     * Once the approximate width of the tube is found for all sample points
     * on all curves, the values are stored in the data member m_aafMaxDist,
     * where each value of m_aafMaxDist is taken as the maximum of three
     * sample points, the current sample point and each point on either side.
     *
     * The values in m_aafMaxDist array are used as the radii of the bounding
     * sphere volumes, which are used to determine which triangles to include
     * in each sub-mesh.
     */
    protected void findMaxMeshDistance(int iDownSample)
    {
        ...
        /*
         * The four minimum distance values in each of the four directions
         * perpendicular to the tangent (viewing) direction.
         */
        ...
        /*
         * The four vectors perpendicular to the tangent (viewing) direction
         * along the FlyPath curve.
         */
        ...
        for (int iCurve = 0; iCurve < m_iNumCurves; iCurve++)
        {
             ...
             * boundary distances from those path positions to the
             * TriangleMesh data.
             */
            akSamplePos =
                m_kGraphSamples.getArrayPointPosition(iCurve);
          ...
          */
                 ...
                 iSample += iDownSample)
            {
                 ...
                 */
                kNormal = m_akCurvePosition[iCurve].
                    getSecondDerivative(fRelativeLength);
                 ...
                 * crossproduct between the tangent direction and the normal
                 * direction.
                 */
                 ...
                 * perpendicular direction are initialized to the maximum
                 * float value.
                 */
                 ...
                 * minimum distance between the point on the FlyPath curve to
                 * each vertex in the four perpendicular directions.
                 */
                for (int iVertex = 0; iVertex < iVertexCount; iVertex++)
                {
                     ...
                     */
                    fDist = m_aakCurvePos[iCurve][iSample].
                        distanceSquared(kVertex0);
             ...
             * final boundary distance used is the maximum of the three values
             * in a window around the current position.
             */
            for (int iSample = 0; iSample < akSamplePos.length; iSample++)
            {
                fMaxTemp1 = 0;
                ...
                {
                    fMaxTemp1 =
                        Math.max(aafMaxDist[iCurve][iSample-iDownSample],
                                 aafMaxDist[iCurve][iSample]);
                }

                if ((iSample + iDownSample) < akSamplePos.length)
                {
                    fMaxTemp2 =
                        Math.max(aafMaxDist[iCurve][iSample],
                                 aafMaxDist[iCurve][iSample + iDownSample]);
                }
     ...
     * @param int iDownSample
     *
     * segmentMesh produces a collection of sub-meshes that each represent the
     * visible triangles for intervals along the FlyPath through the tube.
     *
     * The segmentation is based on the curvature of the FlyPath, taking
     * advantage of the property that when the tube bends far enough,
     * triangles on the far side of the bend are not visible. The segmentation
     * also uses the approximate "width" of the tube at each point along the
     * FlyPath to create bounding spherical volumes. All triangles in the
     * bounding volumes are included in the triangle sub-meshes.
     *
     * The final result is an array of submeshes with fewer triangles than the
     * original mesh, producing increased rendering performance.
     */
    protected void segmentMesh(int iDownSample)
    {
        ...
        for (int iCurve = 0; iCurve < m_iNumCurves; iCurve++)
        {
            ...
            aaaiConn[iCurve] =
                new int[(m_aiNumBreakPoints[iCurve] - 1)][];
            m_aaaiConnectivity[iCurve] =
                new int[(m_aiNumBreakPoints[iCurve] - 1)][];
            aaabFound[iCurve] =
                new boolean[(m_aiNumBreakPoints[iCurve] - 1)][];

            for (int iBreak = 0;
                  iBreak < (m_aiNumBreakPoints[iCurve] - 1);
                  iBreak++)
            {
        ...
        for (int iCurve = 0; iCurve < m_iNumCurves; iCurve++)
        {
                 ...
                 iSample += iDownSample)
            {
                 ...
                 * the curvature and the breakpoints calculated in
                 * segmentCurvesByTangent.
                 */
                fRelativeLength = m_aafRelativeLengths[iCurve][iSample];
                 ...
                 * each time through the loop -- as there are three Vertices
                 * for each triangle. Only one vertex in the triangles is
                 * boundary tested.
                 */
                for (int iIndex = 0; iIndex < iIndexCount; iIndex += 3)
                {
                     ...
                     * curve, both in the BSpline representation and the
                     * original voxel sample position representation are
                     * evaluated. The minimum distance is taken.
                     */
                    fDist = Math.min(
                                     m_aakCurvePos[iCurve][iSample].
                                     distanceSquared(kVertex0),
                     ...
                     * the tube at this point on the path is the triangle
                     * added to the sub-mesh.
                     */
                    if (fDist < (fDISTANCE_SCALE_FACTOR *
                                 m_aafMaxDist[iCurve][iSample]))
                    {
                         ...
                         * either side of this mesh. This takes care of
                         * the fact that when the position on the FlyPath
                        ...
                        if ((iMinPt - 1) >= 0)
                        {
                            if (!aaabFound[iCurve]
                                ...
                                [iTriIndexDiv3])
                            {
                                aaabFound[iCurve][iMinPt-1][iTriIndexDiv3]
                                    = true;

                                iTriIndex = m_aaiNumTris[iCurve][iMinPt-1];
                                aaaiConn[iCurve][iMinPt-1][iTriIndex] =
                                    iVIndex0;
                                aaaiConn[iCurve][iMinPt-1][iTriIndex + 1] =
                                    iVIndex1;
                                aaaiConn[iCurve][iMinPt-1][iTriIndex + 2] =
                                    iVIndex2;
                        ...
                        {
                            iParent =
                                m_kGraphCurve.getBranchParentIndex(iCurve);
                            if (iParent != -1)
                            {
                                fParentT =
                                    m_kGraphCurve.
                                    getBranchParentNormalizedDist(iCurve);
                                iMinParentPt =
                                    findCurveSegment(iParent, fParentT);
                                if ( !aaabFound[iParent]
                                     [iMinParentPt]
                                     [iTriIndexDiv3] )
                                {
                                    aaabFound[iParent]
                                    ...
                                    iTriIndex =
                                        m_aaiNumTris[iParent]
                                        [iMinParentPt];
                                    aaaiConn[iParent]
                                        [iMinParentPt][iTriIndex] =
                                        iVIndex0;
                                    aaaiConn[iParent]
                                        [iMinParentPt][iTriIndex+1] =
                                        iVIndex1;
                                    aaaiConn[iParent]
                                        [iMinParentPt][iTriIndex+2] =
                                        iVIndex2;
                        ...
                        for (int iChild = 0; iChild < fChildren.length;
                             iChild++)
                        {
                            /* If the child's relative distance falls inside
                             * this branch: */
                            iMinChildPt =
                                findCurveSegment(iCurve, fChildren[iChild]);
                            if (iMinChildPt == iMinPt)
                            {
                                /* Get the curve index of the children */
                                iChildren = m_kGraphCurve.
                                    getBranchPointBranches(iCurve, iChild);
                                /* Loop over children */
                                for (int iChildIndex = 0;
                                     iChildIndex < iChildren.length;
                                     iChildIndex++)
                                {
                                    /* Add this triangle to the child's first
                                     * segment ([0]), if it is not already
                                     * added: */
                                    if (!aaabFound[iChildren[iChildIndex]]
                                        [0][iTriIndexDiv3])
                                    {
                                        aaabFound[iChildren[iChildIndex]]
                                            [0][iTriIndexDiv3] = true;

                                        iTriIndex =
                                            m_aaiNumTris[iChildren
                                                         [iChildIndex]][0];

                                        aaaiConn[iChildren[iChildIndex]]
                                            [0][iTriIndex] = iVIndex0;
                                        aaaiConn[iChildren[iChildIndex]]
                                            [0][iTriIndex+1] = iVIndex1;
                                        aaaiConn[iChildren[iChildIndex]]
                                            [0][iTriIndex+2] = iVIndex2;

                                        m_aaiNumTris[iChildren[iChildIndex]]
                                            [0]+=3;
                                    }
                                }
                            }
         ...
         * TriangleMesh index array, and add any Triangles that were not found
         * to every mesh segment.
         */
        /*
        ViewJSimpleProgressBar kProgress =
            new ViewJSimpleProgressBar("Segmenting Triangle Mesh",
                                       "Checking Triangles ...");
        kProgress.setRange((float)0, (float)iIndexCount);
        */
        ...
        for (int iIndex = 0; iIndex < iIndexCount; iIndex += 3)
        {
             ...
             * approach to occlusion culling -- the triangle is added to each
             * mesh. In testing typically > 1% of triangles are not found at
        ...
        {
            for (int iBreakPt = 0;
                 iBreakPt < (m_aiNumBreakPoints[iCurve]-1);
                 iBreakPt++)
            {
                m_aaaiConnectivity[iCurve][iBreakPt] =
                    new int[m_aaiNumTris[iCurve][iBreakPt]];
                iNumTotalTris = 0;
                for (iTriIndex = 0;
                     iTriIndex < m_aaiNumTris[iCurve][iBreakPt];
                     iTriIndex++)
                {
                    m_aaaiConnectivity[iCurve][iBreakPt][iNumTotalTris++] =
                        aaaiConn[iCurve][iBreakPt][iTriIndex];
                }
            }
        }
    ...
    /*
     * @param int iCurve
     * @param float fRelativeDistance
     * @return int index
     *
     * Function findCurveSegment: returns the index for the
     * TriMesh Segment for Curve = iCurve and the distance along the
     * curve equal to fRelativeDistance.
     */
    protected int findCurveSegment(int iCurve, float fRelativeDistance)
    {
         ...
         * until the interval containing the value of
         * fRelativeDistance is found.
    ...
    /**
     * Delete all member variables needed for the mesh segmentation, but not
     * needed for getTriangleArrayIndices.
     */
    public void cleanUp()
    {
        m_aaiNumTris = null;
     ...
     * @param fT float
     * @return int[]
     * getTriangleArrayIndices, returns the submesh associated with the
     * Curve = iCurve and the relative position on that curve equal to
     * fT. If
     */
     ...
     * One per curve segment. */
    private int[][][] m_aaaiConnectivity;

    /* The aray of points (defined in time) where the curvature of the spline
     * is highest. */
    ...
    /* Values that are used in multiple functions, are stored so that they are
     * calculated only once. */
    /* Relative position on the curve */
