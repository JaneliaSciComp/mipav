...
public class RendererImageData {
     ...
     * @param   kOpacityGradMag    ViewJComponentVolOpacity Input gradient magnitude opacity map (optional by specifying
     *                             a null reference).
     ...
     */
    public boolean updateRenderer(Renderer kRayTracer, ModelLUT kLUT, TransferFunction transferFunction,
                                  ViewJComponentVolOpacity kOpacityGradMag, int iTimeSlice, boolean bUpdatedTimeSlice,
                                  boolean bUpdatedLut, boolean bForceShow) {
        ...
        for (iZ = 0; iZ < iZBound; iZ++) {
            ...
            for (iY = 0; iY < iYBound; iY++) {
                ...
                for (iX = 0; iX < iXBound; iX++) {
                    ...
                    float fDX = (bMinX ? afData[i] : afData[i - iOffX]) - (bMaxX ? afData[i] : afData[i + iOffX]);
                    float fDY = (bMinY ? afData[i] : afData[i - iOffY]) - (bMaxY ? afData[i] : afData[i + iOffY]);
                    float fDZ = (bMinZ ? afData[i] : afData[i - iOffZ]) - (bMaxZ ? afData[i] : afData[i + iOffZ]);
    ...
    private void modulateAlphaImageBytes(float[] afData, TransferFunction transferFunction) {
        ...
        for (int i = 0; i < afData.length; i++) {
            int iValue = (int) ((afData[i] - fImageMin) * fScale);
            m_acImageA[i] = (byte) ((transferFunction.getRemappedValue(afData[i], 256)) * ((int) m_acImageA[i] & 0xFF) /
                                        255);
        }
    }
     ...
     * @param  kOpacityRGB           ModelRGB Input opacity map to apply to values.
     */
    private void modulateAlphaImageBytes(float[] afData, RendererMapColor kMapColorToIntensity, ModelRGB kOpacityRGB) {
