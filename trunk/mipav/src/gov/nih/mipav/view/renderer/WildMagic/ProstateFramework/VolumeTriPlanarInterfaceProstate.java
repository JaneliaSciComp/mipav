...
public class VolumeTriPlanarInterfaceProstate extends VolumeTriPlanarInterface {
    ...
    protected ModelRGB RGBTA_coronal = null;
    protected ModelRGB RGBTA_sagittal = null;
    ...
    protected JPanel histoLUTPanelAxial;
    protected JPanel histoLUTPanelSagittal;
    protected JPanel histoLUTPanelCoronal;
    ...
    protected PlaneRenderProstate[] m_akPlaneRender;
    ...
    protected JPanelHistoLUT panelHistoLUTAxial;
    protected JPanelHistoLUT panelHistoLUTSagittal;
    protected JPanelHistoLUT panelHistoLUTCoronal;

    /** RGB control panel of the color image. */
    protected JPanelHistoRGB panelHistoRGBAxial;
    protected JPanelHistoRGB panelHistoRGBSagittal;
    protected JPanelHistoRGB panelHistoRGBCoronal;
    ...
    protected VolumeImage m_kVolumeImageAxial;
    
    /** VolumeImage contains data and textures for ModelImage A. */
    protected VolumeImage m_kVolumeImageCoronal;
    /** VolumeImage contains data and textures for ModelImage A. */
    protected VolumeImage m_kVolumeImageSagittal;
    ...
    protected Animator m_kAnimatorCoronal;
    protected Animator m_kAnimatorSagittal;
    
    /** Volume/Slice/Surface renderer. */
    protected VolumeTriPlanarRenderProstate raycastRenderWM;
    ...
    protected ModelImage imageAxial = null;
    protected ModelImage imageSagittal = null;
    protected ModelImage imageCoronal = null;
    
    // protected ModelLUT LUTa;
    protected ModelLUT LUTa_coronal;
    protected ModelLUT LUTa_sagittal;
     ...
     */
    public VolumeTriPlanarInterfaceProstate()
    {
    	// super(null, null);
    }
   

    public VolumeTriPlanarInterfaceProstate(ModelImage _imageA, ModelImage _imageCoronal, ModelImage _imageSagittal, ModelImage _imageB)
    {
    	...
    	int[] extents = _imageA.getExtents();
    	boolean bDirExists = true;
    	String kImageName = ModelImage.makeImageName( _imageA.getFileInfo(0).getFileName(), "");
        String m_kParentDir = _imageA.getFileInfo()[0].getFileDirectory().concat( kImageName + "_RenderFiles" + File.separator);
        ...
        m_kVolumeImageAxial = new VolumeImage(  _imageA, "A", bDirExists, m_kParentDir, 0, extents );
        
        imageAxial = m_kVolumeImageAxial.GetImage();
        RGBTA = m_kVolumeImageAxial.GetRGB();
        this.LUTa = m_kVolumeImageAxial.GetLUT();
        
        
        
        int[] extentsCoronal = _imageCoronal.getExtents();
    	boolean bDirExistsCoronal = true;
    	String kImageNameCoronal = ModelImage.makeImageName( _imageCoronal.getFileInfo(0).getFileName(), "");
        String m_kParentDirCoronal = _imageSagittal.getFileInfo()[0].getFileDirectory().concat( kImageName + "_RenderFiles" + File.separator);
       
         File kDirCoronal = new File( m_kParentDirCoronal );
         if ( !kDirCoronal.exists() )
         {
             bDirExistsCoronal = false;
             try {
                 kDirCoronal.mkdir();
             } catch (SecurityException e) {}
         }
        
        m_kVolumeImageCoronal = new VolumeImage(  _imageCoronal, "A", bDirExistsCoronal, m_kParentDirCoronal, 0, extentsCoronal );
        imageCoronal = m_kVolumeImageCoronal.GetImage();
        RGBTA_coronal = m_kVolumeImageCoronal.GetRGB();
        this.LUTa_coronal = m_kVolumeImageCoronal.GetLUT();
        
        

        int[] extentsSagittal = _imageSagittal.getExtents();
    	boolean bDirExistsSagittal = true;
    	String kImageNameSagittal = ModelImage.makeImageName( _imageSagittal.getFileInfo(0).getFileName(), "");
        String m_kParentDirSagittal = _imageSagittal.getFileInfo()[0].getFileDirectory().concat( kImageName + "_RenderFiles" + File.separator);
       
         File kDirSagittal = new File( m_kParentDirSagittal );
         if ( !kDirSagittal.exists() )
         {
             bDirExistsSagittal = false;
             try {
                 kDirSagittal.mkdir();
             } catch (SecurityException e) {}
         }
        
        m_kVolumeImageSagittal = new VolumeImage(  _imageSagittal, "A", bDirExistsSagittal, m_kParentDirSagittal, 0, extentsSagittal );
        imageSagittal = m_kVolumeImageSagittal.GetImage();
        RGBTA_sagittal = m_kVolumeImageSagittal.GetRGB();
        this.LUTa_sagittal = m_kVolumeImageSagittal.GetLUT();
        
        
         // createImage(_imageA, _imageSagittal, _imageCoronal);
        
    }
    
    public void createImage(ModelImage imageAxial, ModelImage imageSagittal, ModelImage imageCoronal) {
    	ModelImage imageResult = createEmptyImage(null);
    	float intAxial, intSagittal, intCoronal;
    	int count = 0;
    	float intensity;
    	int length = 510 * 556 * 319;
    	
    	for ( int i = 0; i < length; i++ ) {
    		count = 0;
    		intAxial = imageAxial.getFloat(i);
    		if ( intAxial != 0f ) {
    			count++;
    		}
    		intSagittal = imageSagittal.getFloat(i);
    		if ( intSagittal != 0f ) {
    			count++;
    		}
    		intCoronal = imageCoronal.getFloat(i);
    		if ( intCoronal != 0f ) {
    			count++;
    		}
    		// if ( count == 0 ) count = 1;
    		intensity = ( intAxial + intSagittal + intCoronal ) / (float)count;
    		imageResult.set(i, intensity);
    	}
    	imageResult.calcMinMax();
    	imageResult.addImageDisplayListener(this);
    	new ViewJFrameImage(imageResult, null, new Dimension(610, 200));
    ...
    public ModelImage createEmptyImage(FileInfoBase fileInfo) {
        ...
        final int[] extents = {510, 556, 319};
        final int[] units = {7, 7, 7, -1, -1};
        final float[] res = {0.2734375f, 0.2734375f, 0.2734375f, 1.0f, 1.0f};
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals("Extract")) {
            ...
            insertTab("AxialLUT", histoLUTPanelAxial);
            insertTab("SagittalLUT", histoLUTPanelSagittal);
            insertTab("CoroanlLUT", histoLUTPanelCoronal);
            ...
            VolumeImageViewerPoint.main(null, m_kVolumeImageAxial, true);
        }
    ...
    public void addSlices(VolumeSlices kSlicesAxial, VolumeSlices kSlicesSagittal, VolumeSlices kSlicesCoronal)
    {
            if ( m_akPlaneRender[0] != null )
            {
                m_akPlaneRender[0].addSlices(kSlicesAxial);
            }
            
            if ( m_akPlaneRender[1] != null )
            {
                m_akPlaneRender[1].addSlices(kSlicesSagittal);
            }
            
            if ( m_akPlaneRender[2] != null )
            {
                m_akPlaneRender[2].addSlices(kSlicesCoronal);
            }
    }
    ...
    public void buildHistoLUTPanel() {
        histoLUTPanelAxial = new JPanel();

        if (imageAxial.isColorImage()) {
            histoLUTPanelAxial.add(panelHistoRGBAxial.getMainPanel());
        } else {
            histoLUTPanelAxial.add(panelHistoLUTAxial.getMainPanel());
        }

        maxPanelWidth = Math.max(histoLUTPanelAxial.getPreferredSize().width, maxPanelWidth);
        
        // sagittal
        histoLUTPanelSagittal = new JPanel();

        if (imageSagittal.isColorImage()) {
            histoLUTPanelSagittal.add(panelHistoRGBSagittal.getMainPanel());
        } else {
            histoLUTPanelSagittal.add(panelHistoLUTSagittal.getMainPanel());
        }

        maxPanelWidth = Math.max(histoLUTPanelSagittal.getPreferredSize().width, maxPanelWidth);
        
        // Coronal 
        histoLUTPanelCoronal = new JPanel();

        if (imageCoronal.isColorImage()) {
            histoLUTPanelCoronal.add(panelHistoRGBCoronal.getMainPanel());
        } else {
            histoLUTPanelCoronal.add(panelHistoLUTCoronal.getMainPanel());
        }

        maxPanelWidth = Math.max(histoLUTPanelCoronal.getPreferredSize().width, maxPanelWidth);
    ...
    public void buildMultiHistogramPanel() {
        ...
        multiHistogramGUI = new JPanelMultiDimensionalTransfer(this, m_kAnimator, m_kVolumeImageAxial);
    ...
    public void buildOpacityPanel() {
        ...
        if (imageAxial.isColorImage()) {
            m_kVolOpacityPanel = new JPanelVolOpacityRGB(this, imageAxial, imageB);
        } else {
            m_kVolOpacityPanel = new JPanelVolOpacity(this, imageAxial, imageB);
        }
    ...
    public void constructRenderers() {
        ...
        try {
            ...
            m_akPlaneRender = new PlaneRenderProstate[3];
            m_akPlaneRender[0] = new PlaneRenderProstate(this, m_kAnimator, m_kVolumeImageAxial, m_kVolumeImageB, FileInfoBase.AXIAL);
           
            m_akPlaneRender[1] = new PlaneRenderProstate(this, m_kAnimator, m_kVolumeImageSagittal, null, FileInfoBase.SAGITTAL);
           
            m_akPlaneRender[2] = new PlaneRenderProstate(this, m_kAnimator, m_kVolumeImageCoronal, null, FileInfoBase.CORONAL);
            
            progressBar.setMessage("Constructing gpu renderer...");

            
            // ruida
            raycastRenderWM = new VolumeTriPlanarRenderProstate( this, m_kAnimator, m_kVolumeImageAxial,
            		m_kVolumeImageSagittal, m_kVolumeImageCoronal,  m_kVolumeImageB);


            TransferFunction kTransfer = m_kVolOpacityPanel.getCompA().getOpacityTransferFunction();
            m_kVolumeImageAxial.UpdateImages(kTransfer, 0, null);
            if ( imageB != null )
            {
            ...
            if (imageAxial.isColorImage()) {
                panelHistoRGBAxial = new JPanelHistoRGB(imageAxial, imageB, RGBTA, RGBTB, true);
            } else {
                panelHistoLUTAxial = new JPanelHistoLUT(imageAxial, imageB, LUTa, LUTb, true, true);
            }

            if (imageSagittal.isColorImage()) {
                panelHistoRGBSagittal = new JPanelHistoRGB(imageSagittal, imageB, RGBTA, RGBTB, true);
            } else {
                panelHistoLUTSagittal = new JPanelHistoLUT(imageSagittal, imageB, LUTa, LUTb, true, true);
            }

            if (imageCoronal.isColorImage()) {
                panelHistoRGBCoronal = new JPanelHistoRGB(imageCoronal, imageB, RGBTA, RGBTB, true);
            } else {
                panelHistoLUTCoronal = new JPanelHistoLUT(imageCoronal, imageB, LUTa, LUTb, true, true);
            }
            ...
            if ( panelHistoLUTAxial != null ) 
                panelHistoLUTAxial.updateComponentLUT(); 
            if ( panelHistoRGBAxial != null ) 
            {
                panelHistoRGBAxial.updateHistoRGB(imageAxial, imageB, false);
                panelHistoRGBAxial.updateFrames(false);
            }
            
            if ( panelHistoLUTSagittal != null ) 
                panelHistoLUTSagittal.updateComponentLUT(); 
            if ( panelHistoRGBSagittal != null ) 
            {
                panelHistoRGBSagittal.updateHistoRGB(imageSagittal, imageB, false);
                panelHistoRGBSagittal.updateFrames(false);
            }
            
            
            if ( panelHistoLUTCoronal != null ) 
                panelHistoLUTCoronal.updateComponentLUT(); 
            if ( panelHistoRGBCoronal != null ) 
            {
                panelHistoRGBCoronal.updateHistoRGB(imageCoronal, imageB, false);
                panelHistoRGBCoronal.updateFrames(false);
            }
    ...
    public void disposeLocal(boolean flag) {
        ...
        histoLUTPanelAxial = null;
        histoLUTPanelCoronal = null;
        histoLUTPanelSagittal = null;
        ...
        if (m_kVolumeImageAxial != null) {
        	m_kVolumeImageAxial.dispose();
        	m_kVolumeImageAxial = null;
        }
        ...
        if (panelHistoLUTAxial != null) {
            panelHistoLUTAxial.disposeLocal();
            panelHistoLUTAxial = null;
        }
        
        if (panelHistoLUTSagittal != null) {
            panelHistoLUTSagittal.disposeLocal();
            panelHistoLUTSagittal = null;
        }
        
        if (panelHistoLUTCoronal != null) {
            panelHistoLUTCoronal.disposeLocal();
            panelHistoLUTCoronal = null;
        }
        

        if (panelHistoRGBAxial != null) {
            panelHistoRGBAxial.disposeLocal();
            panelHistoRGBAxial = null;
        }
        
        if (panelHistoRGBSagittal != null) {
            panelHistoRGBSagittal.disposeLocal();
            panelHistoRGBSagittal = null;
        }
        
        if (panelHistoRGBCoronal != null) {
            panelHistoRGBCoronal.disposeLocal();
            panelHistoRGBCoronal = null;
        }
        ...
        if (imageAxial != null) {
        	imageAxial.removeImageDisplayListener(this);
        	imageAxial.disposeLocal();
        	imageAxial = null;
        }

        if (imageSagittal != null) {
        	imageSagittal.removeImageDisplayListener(this);
        	imageSagittal.disposeLocal();
        	imageSagittal = null;
        }
        
        if (imageCoronal != null) {
        	imageCoronal.removeImageDisplayListener(this);
        	imageCoronal.disposeLocal();
        	imageCoronal = null;
        }
    ...
    public ModelImage getHistoLUTActiveImage() {
        ...
        if (panelHistoLUTAxial != null) {
            ...
            if (panelHistoLUTAxial.getDisplayMode() == JPanelHistoLUT.IMAGE_A) {
                return imageAxial;
            } 
            return imageB;
        }
    ...
    public ModelImage getHistoRGBActiveImage() {
        ...
        if (panelHistoRGBAxial != null) {
            ...
            if (panelHistoRGBAxial.getDisplayMode() == JPanelHistoRGB.IMAGE_A) {
                return imageAxial;
            } 
            return imageB;
        }
     ...
     */
    public VolumeTriPlanarRenderProstate getVolumeGPUrender()
    {
    ...
    public void setGradientMagnitude( boolean bShow )
    {
        ...
        m_kVolumeImageAxial.UpdateImages(kTransfer, 2, m_kVolOpacityPanel.getGradMagA());
        if ( imageB != null )
        {
    ...
    {
    	imageAxial.setRadiologicalView(bOn);
    ...
    public void setRGBTA(ModelRGB RGBT) {
        if (m_kVolumeImageAxial != null) {
            m_kVolumeImageAxial.SetRGBT(RGBT);
        }
    ...
    public void setTimeSlice(int slice) { 
        ...
        m_kVolumeImageAxial.SetTimeSlice( slice );
        setModified();
    }
    ...
    public void updateData() {
        if ( m_kVolumeImageAxial != null )
        {
            m_kVolumeImageAxial.UpdateData(imageAxial);
        }
        if ( m_kVolumeImageSagittal != null )
        {
            m_kVolumeImageSagittal.UpdateData(imageSagittal);
        }
        if ( m_kVolumeImageCoronal != null )
        {
            m_kVolumeImageCoronal.UpdateData(imageCoronal);
        }
        if ( m_kVolumeImageB != null )
        {
    ...
    public boolean updateImages(ModelLUT LUTa, ModelLUT LUTb, boolean forceShow, int interpMode) {
        if (m_kVolumeImageAxial != null) {
            m_kVolumeImageAxial.UpdateImages(LUTa);    
            setModified();
        }
        
        if (m_kVolumeImageSagittal != null) {
            m_kVolumeImageSagittal.UpdateImages(LUTa);    
            setModified();
        }
        
        if (m_kVolumeImageCoronal != null) {
            m_kVolumeImageCoronal.UpdateImages(LUTa);    
            setModified();
        }
    ...
    protected JMenuBar buildMenu() {
        ...
        if ( m_kVolumeImageAxial != null && m_kVolumeImageAxial.GetImage() != null )
        {
            menuObj.setMenuItemEnabled("4D toolbar", m_kVolumeImageAxial.GetImage().is4DImage() );
        }
    ...
    protected void configureFrame( boolean bVolumeViewer ) {
        ...
        imageAxial.setImageOrder(ModelImage.IMAGE_A);
        ...
        imageAxial.addImageDisplayListener(this);
        imageSagittal.addImageDisplayListener(this);
        imageCoronal.addImageDisplayListener(this);
    ...
    protected void create3DVOI( boolean bIntersection )
    {
        ...
        if ( m_akPlaneRender != null )
        {
            ModelImage kImage = new ModelImage( ModelStorageBase.INTEGER, 
                    m_kVolumeImageAxial.GetImage().getExtents(), "Temp" );
            kImage.copyFileTypeInfo(m_kVolumeImageAxial.GetImage());
            for (int i = 0; i < 3; i++)
            {
                if ( m_akPlaneRender[i] != null )
                {
            ...
            int[] direction = MipavCoordinateSystems.getModelDirections(m_kVolumeImageAxial.GetImage());
            float[] startLocation = m_kVolumeImageAxial.GetImage().getFileInfo(0).getOrigin();
...
//              Get the adjacent triangles:
    ...
    protected void set3DModelPosition(Vector3f position) {
        ...
        float fMaxX = (m_kVolumeImageAxial.GetImage().getExtents()[0] - 1) * m_kVolumeImageAxial.GetImage().getFileInfo(0).getResolutions()[0];
        float fMaxY = (m_kVolumeImageAxial.GetImage().getExtents()[1] - 1) * m_kVolumeImageAxial.GetImage().getFileInfo(0).getResolutions()[1];
        float fMaxZ = (m_kVolumeImageAxial.GetImage().getExtents()[2] - 1) * m_kVolumeImageAxial.GetImage().getFileInfo(0).getResolutions()[2];
        ...
        fX *= position.X/(m_kVolumeImageAxial.GetImage().getExtents()[0] - 1);
        fY *= position.Y/(m_kVolumeImageAxial.GetImage().getExtents()[1] - 1);
        fZ *= position.Z/(m_kVolumeImageAxial.GetImage().getExtents()[2] - 1);
    ...
    protected void setPatientSlicePosition(Vector3f position) {
        ...
        MipavCoordinateSystems.fileToPatient(position, axial, imageAxial, FileInfoBase.AXIAL);

        Vector3f coronal = new Vector3f();
        MipavCoordinateSystems.fileToPatient(position, coronal, imageCoronal, FileInfoBase.CORONAL);

        Vector3f sagittal = new Vector3f();
        MipavCoordinateSystems.fileToPatient(position, sagittal, imageSagittal, FileInfoBase.SAGITTAL);
    ...
    private void deleteVOISurface( String kVOIName )
    {
        ...
        m_iVOITotal--;
        int[] aiExtents = imageAxial.getExtents();
        int length = aiExtents[0] * aiExtents[1] * aiExtents[2];
