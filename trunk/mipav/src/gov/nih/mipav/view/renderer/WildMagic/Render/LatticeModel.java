...
//    	new ViewJFrameImage(blurb);
     ...
     * @param imageA
     */
    public LatticeModel( ModelImage imageA )
	{
		this.imageA = imageA;
	}
     ...
     * @param lattice
     */
    public LatticeModel( ModelImage imageA, VOI lattice )
	{
		this.imageA = imageA;
     ...
     * @param doAnnotation
     */
    public LatticeModel( ModelImage imageA, VOI annotation, boolean doAnnotation )
	{
		this.imageA = imageA;
		this.lattice = null;
...
//		centerTangents = null;
     ...
     * At this point a lattice must be defined, outlining how the worm curves in 3D.  
     * A lattice is defined ad a VOI with two curves of equal length marking the left-hand and right-hand sides or the worm.
     * @param displayResult, when true intermediate volumes and results are displayed as well as the final straighened image.
     */
    public void interpolateLattice( boolean displayResult )
	{
    	...
    	// save the updated lattice positions, including any volume estimation for the marker segmentat at that lattice point:
...
//	}
    ...
    /**
     * As part of growing the contours to fit the worm model, this function
     * checks that all annotations are included in the current version of the model.
     * @param model
     * @return
     */
...
//    		System.err.println( text.getText() + " " + position + "  " + value );
    ...
    /**
     * Creates the worm model based on the segmented left-right marker images and the current lattice and natural splines fitting the lattice.
     * @param imageA
     ...
     * @param displayResult
     */
    private void createWormModel( ModelImage imageA, VOI samplingPlanes, 
    		Vector<Ellipsoid3f> ellipseBounds, Vector<Float> diameters, int diameter, boolean displayResult  )
    {
		...
		// 7. The set of ellipses from the head of the worm to the tail defines an approximate outer boundary of the worm in 3D.  
		// The centers of each ellipse are spaced one voxel apart along the center line curve of the worm, and each ellipse 
		...
		// Voxels where multiple ellipses intersect are labeled as conflict voxels.  Once all ellipses have been evaluated, 
    	...
    	for ( int i = 0; i < samplingPlanes.getCurves().size(); i++ )
    	{
    		...
    		if ( i < (samplingPlanes.getCurves().size() - 1) )
    		{
    			...
    			{
    				initializeModelandConflicts( imageA, model, insideConflict, 0, i, resultExtents, cornersSub, 
		...
		// corresponding region of the worm volume. This prevents a slice from extending beyond the worm boundary and capturing the 
		// adjacent fold of worm. Because the marker segmentation image only segments the left-right markers it is not possible
		...
		for ( int i = 0; i < samplingPlanes.getCurves().size(); i++ )
		{
			...
			mapSliceIDstoMarkerIDs( model, markerSegmentation, sliceIDs, markerIDs, completedIDs, currentID, 0, i,  resultExtents, corners, ellipseBounds.elementAt(i) );
		}
...
//		}		
		...
		// 9. The last step is an attempt to ensure that as much of the worm data is captured by the algorithm as possible. 
    	// Using the marker segmentation image where possible as a guide to the worm boundary, each slice of worm model is 
    	// grown outward.  The points on the boundary are expanded in an iterative process until the point comes in contact
    	// with another edge of the worm. For areas of the worm where it folds back on itself this results in a flattened 
    	// cross-section where the folds press against each other. 
    	// For areas of the worm where the cross-section does not contact other sections of the worm the 2D contour extends 
...
//    	}
	 ...
	 * @param ellipseBound
	 */
	private void fillMarkerSegmentationImage( ModelImage model, ModelImage markerSegmentation,
			float[] sliceIDs, 
    ...
    private void generateCurves( )
    {
    	...
    	// 1. The center line of the worm is calculated from the midpoint between the left and right points of the lattice.
	    ...
	    // curves, the 2D sampling plane can be defined.  The center point of the plane is the current point on the center curve.  
	    ...
	    // This method fully defines the sample plane location and orientation as it sweeps through the 3D volume of the worm.
		...
		// The ellipse short axis is in the direction of the plane vertical axis; the length is set to ½ the length 
...
//		rightPositions.setVolumeDisplayRange(minRange);
	...
	/**
	 * Grows the outer edge of the worm model outward to capture more of the worm, using the segmented marker image as a guide.
	 * @param model
	 * @param markers
	 * @param sliceIDs
	 * @param step
...
//				diff.scale(0.5f);
    ...
    /**
     * First pass generating the worm model from the simple ellipse-based model.
     * Any voxels that fall inside overlapping ellipses are set as conflict voxels.
     * @param image
     * @param model
     ...
     * @param value
     */
    private void initializeModelandConflicts( ModelImage image, ModelImage model, ModelImage insideConflict,
    		final int tSlice, final int slice, final int[] extents,
...
//    	}
     ...
     * @param ellipse
     */
    private void makeEllipse2D( Vector3f right, Vector3f up, Vector3f center, float diameterA, float scale, VOIContour ellipse  )
	{
		int numPts = 32;
		float diameterB = diameterA/2f;// + (1-scale) * diameterA/4f;
		for ( int i = 0; i < numPts; i++ )
     ...
     * @param ellipse
     */
    private void makeSelectionFrame( Vector3f right, Vector3f up, Vector3f center, float diameter, VOIContour ellipse  )
	{
		int numPts = 12;
		for ( int i = 0; i < numPts; i++ )
		{
    ...
    /**
     * Writes out slice IDs that match the segmented marker IDs, set the value for every slice in the straightened image to the corresponding marker ID for 
     * the marker that intersects that slice. Only markers that are inside the current worm model are included -- even when multiple markers intersect 
     * the same slice.
     * the markers that are inside the worm model are included.
     * @param model
     * @param markerSegmentation
     * @param sliceIDs
     * @param markerIDs
     ...
     * @param ellipseBound
     */
    private void mapSliceIDstoMarkerIDs( ModelImage model, ModelImage markerSegmentation,
			float[] sliceIDs,
...
//		System.err.println( slice + " " + markerPt + " " + markerIDs[ currentID[0] ] );
    ...
    /**
     * Converts the marker segmentation image into a color image where each marker is colored based on the corresponding lattice ID
     * @param image
     * @param displayResult
     */
    private void markerImageToColor( ModelImage image, boolean displayResult )
    {
		...
		float numColors = (float) image.getMax();
...
//		}
    	...
    	int dimX = colorImage.getExtents().length > 0 ? colorImage.getExtents()[0] : 1;
    	int dimY = colorImage.getExtents().length > 1 ? colorImage.getExtents()[1] : 1;
    	int dimZ = colorImage.getExtents().length > 2 ? colorImage.getExtents()[2] : 1;
    	for ( int z = 0; z < dimZ; z++ )
    	{
    		for ( int y = 0; y < dimY; y++ )
    		{
    			for ( int x = 0; x < dimX; x++ )
    			{
    				...
    				if ( value > 0 )
    				{
    					Color color = new Color( Color.HSBtoRGB( value / numColors, 1, 1) );
...
//					System.err.println( imageA.getImageName() + " originToStraight " + text + " " + pt + " OK ");
     ...
     * @param value
     */
    private void resolveModelConflicts( ModelImage model, ModelImage markerSegmentation,
			float[] sliceIDs, 
    		final int tSlice, final int slice, final int[] extents,
...
//        System.err.println( image.getImageName() + ".xml" );
     ...
     * @param postFix
     */
    private void saveLatticePositions( ModelImage image, ModelImage model, ModelImage originToStraight, VOIContour left, VOIContour right, int[][] volumes, String postFix )
    {    	
    	String imageName = image.getImageName();
    	if ( imageName.contains("_clone") )
    	{
...
//        	}
    ...
    /**
	 * Produces an image segmentation of the left-right fluorescent markers using the lattice points as a guide.
	 * The fluorescent markers in the worm volume are automatically segmented, using the positions of the lattice as a guide. 
	 * During segmentation the voxels that fall within the segmented regions are labeled with a corresponding marker ID.  
	 * In the event that not all 10 markers are segmented, which may occur if the input lattice points are placed just outside 
	 * the fluorescent marker boundaries, the lattice points will be modified to be inside the segmented boundaries.
	 * @param image
	 * @param left
	 * @param right
	 * @param markerIDs
	 ...
	 * @return
	 */
	private ModelImage segmentMarkers( ModelImage image, VOIContour left, VOIContour right, int[] markerIDs, int[][] markerVolumes, boolean segAll )
	{
    	String imageName = image.getImageName();
    	if ( imageName.contains("_clone") )
    	{
...
//		new ViewJFrameImage((ModelImage)markerSegmentation.clone());
     ...
     * Sets the time values for each point on the curve.
     * @param curve
     * @param time
     * @return
     */
    ...
    /**
     * Generates the Natural Spline curves for the left and right curves for the lattice.
     * The time points are passed to the spline and correspond to the time points along the center-line curve
     * so that all three curves match in time-space.
     * @param curve
     * @param time
     * @return
     */
    ...
    /**
     * Once the 3D model of the worm is finalized, a 2D slice plane is swept through the model. At each sample-point along the 3D 
     * center spline of the worm, the 2D plane is intersected with the original 3D volume. Voxels that fall outside the updated 2D
     *  worm contour are set to the image minimum value (typically = 0).  Voxels that fall inside the 2D worm contour are copied 
     *  into the output slice.  The set of 2D slices from the worm head to tail are concatenated to form the final 3D straightened volume.
     *  
     *  During the straightening step as well as during the model-building process or anytime the 2D sample plane is intersected 
     *  with the 3D volume steps are taken by the algorithm to minimize sampling artifacts.  Due to the twisted configuration of 
     *  the worm, sampling the volume along the outer-edge of a curve will result under-sampling the data while the inside edge of 
     *  the curve will be over-sampled.  
     *  
     *  To reduce sampling artifacts the sample planes are interpolated between sample points, using the maximum distance between 
     *  points along consecutive contours to determine the amount of super-sampling.  The multiple sample planes are averaged 
     *  to produce the final slice in the straightened image. In addition, each contour is modeled as having the thickness of 
     *  one voxel and sample points that fall between voxels in the volume are trilinearly interpolated.
     *  
     * @param image
     * @param resultExtents
     * @param baseName
...
//					writeDiagonal( image, model, originToStraight, 0, i, resultExtents, corners, values, dataOrigin);		        	
    ...
    /**
     * Tests and updates the origin-to-straight image so that each voxel in the original twisted image
     * has a corresponding output voxel in the straightened image.
     * @param model
     * @param originToStraight
     */
...
//    			System.err.println("updateLinks showSelected ");
    ...
    /**
     * Updates the VOI displaying which point (lattice or annotation) is currently selected
    ...
    /**
     * Called from the straightening function.
     * Exports the finalized worm model slice-by-slice into the straightened image.
     * @param image
     * @param model
     * @param originToStraight
     * @param tSlice
