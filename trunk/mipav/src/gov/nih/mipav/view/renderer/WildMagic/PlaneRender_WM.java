 ...
 * 
 */
public class PlaneRender_WM extends GPURenderBase implements GLEventListener, ScreenCoordinateListener {
    ...
    private final Matrix4f m_kPVWMatrix = new Matrix4f();
     ...
     * @param iPlane
     */
    public PlaneRender_WM(final VolumeTriPlanarInterface kParent, final Animator kAnimator,
    ...
    public void mouseDragged(final MouseEvent kEvent) {
        ...
        final Vector4f kMouseVec = new Vector4f(kEvent.getX(), kEvent.getY(), m_iSlice, 1);
        final Vector4f kMouseWorld = new Vector4f();
    ...
    public void reshape(final GLAutoDrawable arg0, final int iX, final int iY, final int iWidth, final int iHeight) {
        ...
        if (iWidth > 0 && iHeight > 0) {
            ...
            final float[] afData = new float[16];
            m_pkRenderer.SetConstantWVPMatrix(2, afData);
            m_kPVWMatrix.Set(afData[0], afData[1], afData[2], afData[3], afData[4], afData[5], afData[6], afData[7],
                    afData[8], afData[9], afData[10], afData[11], afData[12], afData[13], afData[14], afData[15]);
    ...
    public void setMouseActive(final boolean bActive) {
    ...
    protected void setOrientation() {
        ...
        final float fMaxZ = (m_aiLocalImageExtents[2] - 1) * m_afResolutions[2];
    ...
    private void drawAxes() {
        if (m_bDrawAxes) {
            ...
            if (m_iPlaneOrientation == FileInfoBase.AXIAL) {
                ...
                drawText(m_iLabelX_SpacingX, m_iHeight - m_iLabelX_SpacingY, kXSliceHairColor, m_kLabelXDisplay
                        .toCharArray());
    ...
    protected void drawText(final int iX, final int iY, final ColorRGBA kColor, final char[] acText) {
    ...
    private void drawVOIs(final VOIVector kVOIs) {
        for (int i = 0; i < kVOIs.size(); i++) {
            final VOI kVOI = kVOIs.get(i);
                ...
                if (element != null) {
                    for (int k = 0; k < element.size(); k++) {
                        ...
                        if (kVOI3D instanceof LocalVolumeVOI) {
                            ((LocalVolumeVOI) kVOI3D).draw(this, m_pkRenderer, m_kCuller, m_afResolutions, m_aiUnits,
                                    m_aiAxisOrder, m_kCenter, m_iSlice, m_iPlaneOrientation, m_kVolumeScale,
                                    m_kTranslate);
                        }
                    }
                }
     ...
     */
    public void LocalToPatient(final Vector3f localPt, final Vector3f patientPt) {
    ...
    public void PatientToLocal(final Vector3f patientPt, final Vector3f localPt) {
    ...
    public boolean screenToFile(final int iX, final int iY, final int iZ, final Vector3f volumePt) {
        final Vector3f localPt = new Vector3f();
        final boolean bClipped = this.ScreenToLocal(iX, iY, iZ, localPt);
    ...
    public boolean screenToFile(final Vector3f kScreen, final Vector3f kFile) {
        final Vector3f localPt = new Vector3f();
        final boolean bClipped = this.ScreenToLocal((int) kScreen.X, (int) kScreen.Y, (int) kScreen.Z, localPt);
    ...
    public Vector3f fileToScreen(final Vector3f volumePt) {
        final Vector3f patientPt = new Vector3f();
    ...
    public Vector3f patientToScreen(final Vector3f patientPt) {
        final Vector3f localPt = new Vector3f();
     ...
     */
    public boolean ScreenToLocal(int iX, int iY, final int iZ, final Vector3f kLocal) {
    ...
    public void LocalToScreen(final Vector3f kLocal, final Vector3f kScreen) {
