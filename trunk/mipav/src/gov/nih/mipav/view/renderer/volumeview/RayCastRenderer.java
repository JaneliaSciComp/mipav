...
public abstract class RayCastRenderer extends Renderer {
     ...
     * @param  iSpacing          A positive value indicating how often to sample the rendered image. The typical value
     *                           is 1 indicating that all rays corresponding to all pixels should be processed. If the
     *                           value is 2, rays corresponding to pixels (x,y) with (x mod 2) = 0 and (y mod 2) = 0 are
     *                           processed. The other pixels are assigned values of the modulo 2 neighbor pixel. Similar
     *                           reduced sampling occurs with larger values of iSpacing. VolumeRenderer sets the spacing
     *                           to be 2 when the image is being rotated by the virtual track ball. Once the image is
     *                           finished rotating, a final trace is made with a spacing of 1.
     */
        ...
        for (iY = 0; iY < m_iRBound; iY += iSpacing) {
            ...
            if ((((iY) % mod) == 0) || (iY == (m_iRBound - 1))) {
                ...
                if (iY == (m_iRBound - 1)) {
                    ...
                    ViewJFrameVolumeView.getRendererProgressBar().setValue(Math.round((float) (iY) / (m_iRBound - 1) *
                                                                                          100));
        ...
        if (bluring) {
            ...
            for (iY = 1; iY < (m_iRBound - 1); iY++) {
                ...
                for (int iX = 1; iX < (m_iRBound - 1); iX++) {
                    ...
                    tempImage[iIndex] = 
                                        // (((int)255   & 0xff) << 24) |
                                        (((int) (fTrgR) & 0xff) << 16) | (((int) (fTrgG) & 0xff) << 8) |
                                            ((int) (fTrgB) & 0xff);
     ...
     *
     * @return  The interpolated value for the point. If kP is not within the bounding box of the image, a value of -1
     *          is returned.
     */
     ...
     */
    private boolean clipped(float fDen, float fNum) {
     ...
     * axis-aligned box.
     *
     * <p>The end points of the intersecting line segment are stored in class members m_kP0 and m_kP1 as image
     * coordinates. The bounding box is centered at the origin. The image coordinates are just a translation of box
     * coordinates so that the center of the box becomes the center of the image. The class members m_fT0 and m_fT1 are
     * the corresponding line segment parameters. They are class members only because Java does not support
     * call-by-reference semantics. In another language, the parameters can be local and passed to 'clipped' with the
     * potential to be changed by 'clipped'.</p>
     *
     * @return  true if and only if the ray intersects the box
     */
    private boolean intersectsBox() {
