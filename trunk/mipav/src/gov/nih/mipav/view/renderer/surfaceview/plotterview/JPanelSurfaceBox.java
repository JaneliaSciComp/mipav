...
/**
  ...
  *   @param  allowFloatingPoint  <code>true</code> will force the text field to also allow
  *                               the use of the '.' key to permit entering floating point
  *                               numbers
  */
 /**
 protected void makeNumericsOnly(JTextField txt, boolean allowFloatingPoint) {
   if (allowFloatingPoint) {
     txt.addKeyListener(new KeyAdapter() { // make the field
       public void keyTyped(KeyEvent evt) { // not accept letters
         ...
         //negative signs are not allowed
         else if ( ( (ch < '0') || (ch > '9'))
                  &&
                  ( (ch != KeyEvent.VK_DELETE) && (ch != KeyEvent.VK_BACK_SPACE))) {
           // if is the case that ch is outside the bounds of a number AND it is the case that ch is neither a BS or a DE, then...
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (source instanceof JButton && source != cancelButton) {
            colorChooser = new ViewJColorChooser(new Frame(),
                                             "Pick color",
                                             new OkColorListener((JButton)source),
                                             new CancelListener());
        }
