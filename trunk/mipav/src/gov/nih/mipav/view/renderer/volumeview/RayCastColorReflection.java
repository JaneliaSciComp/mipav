...
/**
 * Ray traced rendering of the level surface within the voxel for
 * the fixed level value of 0.0.
 */

public class RayCastColorReflection
   ...
   * this class is MjVolumeRenderer.
   *
   ...
   * @param fShininess float Non-negative shininess value to use for
   * specular lighting.  If this value is zero, then specular lighting
   * is not computed.  The larger the value, the "sharper" will be
   ...
   * This method uses the Trapezoid Rule to numerically integrates the
   * image along the line segment.  The number of integration samples is
   * chosen to be proportional to the length of the line segment.
   *
   ...
   * B | (G << 8) | (R << 16).  This method returns a yellowish value
   * if the ray intersects a region bounded by the level surface indicated
   * by the level value class member.  Otherwise a gray scale value is
   * returned.
   *
  ...
  protected void processRay(int iIndex, int rayTraceStepSize) {
    ...
    for (iStep = 0; iStep < iNumSteps; iStep++) {
      ...
      if ( (iX >= 0 && iX <= m_iXBoundM2) &&
        ...
        // Interpolate alpha
        float fSrcA =
            fS000 * (m_acImageA[i000] & 0x0ff) +
            fS001 * (m_acImageA[i001] & 0x0ff) +
            fS010 * (m_acImageA[i010] & 0x0ff) +
            fS011 * (m_acImageA[i011] & 0x0ff) +
            fS100 * (m_acImageA[i100] & 0x0ff) +
            fS101 * (m_acImageA[i101] & 0x0ff) +
