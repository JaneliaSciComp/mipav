...
public class JDialogProstateTestFeatures extends JDialogScriptableBase
    ...
    private JPanel edgeDetectionPanel;
	...
	public void itemStateChanged(ItemEvent event) {
		...
		if (source == image25DCheckbox) {
			enable3DComponents(!image25DCheckbox.isSelected());
		}
	}
    ...
    protected void callAlgorithm() {
        ...
        boolean doneEdgeDetection;
        int zDim;
        int tDim;
        int newExtents[];
        int operationAdditional = 1;
        ...
        try {
            ...
            for (i = 0; i < numDirections; i++) {
                ...
                doneEdgeDetection = false;
                
    
                for (j = 0; j < ( numOperators + operationAdditional); j++) {
                    index = j + (i * ( numOperators + operationAdditional));
                    ...
                    if (contrast && (!doneContrast)) {
                        ...
                        donePromenance = true;
                    } else if (edgeDetection && ( !doneEdgeDetection )) {
                        opString = "_EdgeNMSuppression";
                    	doneEdgeDetection = true;
                    }
        ...
        if (concatenate) {
            ...
            if (image.isColorImage()) {
                textureAlgo = new AlgorithmProstateFeatures(resultImage, image, RGBOffset, windowSize, offsetDistance,
                        ...
                        concatenate, edgeDetection, sigmas, true, image25D);    
            }
            else {
                textureAlgo = new AlgorithmProstateFeatures(resultImage, image, windowSize, offsetDistance, greyLevels, ns, nesw, ew,
                                                           senw, invariantDir, contrast, dissimilarity, homogeneity,
                                                           inverseOrder1, asm, energy, maxProbability, entropy, mean,
                                                           variance, standardDeviation, correlation, shade, promenance,
                                                           concatenate, edgeDetection, sigmas, true, image25D);
    ...
    private void init() {
        ...
        buildHaralickPanel();
        buildEdgeDetectionPanel();
        buildSavedFilePanel();
        
        JPanel otherFeatures = new JPanel();
        otherFeatures.setLayout(new BorderLayout());
        
        otherFeatures.add(edgeDetectionPanel, BorderLayout.NORTH);
    ...
    private void buildSavedFilePanel() {
		...
		savedFilePanel.setBorder(buildTitledBorder("Saved Test Features File"));
    ...
    private void buildEdgeDetectionPanel() {
    	int ypos = 0;
    	
    	edgeDetectionPanel = new JPanel();
    	edgeDetectionPanel.setBorder(buildTitledBorder("Edge Detection"));
    	edgeDetectionPanel.setLayout(new GridBagLayout());
         ...
         gbc.gridx = 0;
         gbc.gridy = ypos++;
         scalePanel.add(WidgetFactory.buildLabel("X dimension (0.0 - 10.0) "), gbc);
         gbc.gridx = 1;
         scalePanel.add(textGaussX, gbc);
         
         gbc.gridx = 0;
         gbc.gridy = ypos++;
         scalePanel.add(WidgetFactory.buildLabel("Y dimension (0.0 - 10.0) "), gbc);
         gbc.gridx = 1;
         ...
         gbc.gridx = 0;
         gbc.gridy = ypos++;
         scalePanel.add(correctionFactorCheckbox, gbc);
         ...
         if (image.getNDims() >= 3) { // if the source image is 3D then allow
             ...
             gbc.gridx = 0;
             gbc.gridy = ypos++;
             scalePanel.add(correctionFactorLabel, gbc);
        ...
        gbc.gridy = 0;
 		...
 		edgeDetectionPanel.add(scalePanel, gbc);
 		gbc.gridy = 1; 		
 		edgeDetectionPanel.add(optionsPanel, gbc);
    ...
    private void buildHaralickPanel() {
    	int ypos = 0;
         ...
         gbc.gridx = 0;
         gbc.gridy = ypos++;
         haralickPanel.add(outputPanel, gbc);
