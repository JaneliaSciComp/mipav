 ...
 *
 */
public abstract class ViewJComponentRenderImage
   ...
   protected RendererImageData m_kRendererImageDataB=null;
     ...
     */
    public ViewJComponentRenderImage(
     ...
     * @return boolean True if the update of lighting triggers a need
     * to redraw the images.
     */
    ...
    *  @return           boolean to indicate if the show was successful
     */
    public synchronized boolean show( int tSlice, ModelLUT _LUTa, ModelLUT _LUTb, boolean forceShow, boolean bQualityHigh  ) {
        ...
        if ( null != rayTracerA ) {
            ...
            if ( !m_kImageA.isColorImage() ) {
                ...
                if ( !m_kRendererImageDataA.updateRenderer(
                    rayTracerA,
                    LUTa,
                    ((ViewJComponentVolOpacity)( (JPanelVolOpacity)(m_kVolumeRenderer.getVolOpacity())).getCompA()).getOpacityTransferFunction(),
                    ((ViewJComponentVolOpacity)( (JPanelVolOpacity)(m_kVolumeRenderer.getVolOpacity())).getCompA_GM()),
                    timeSliceA,
                    bUpdatedTimeSlice,
                    bUpdatedLut,
                    forceShow ) ) {
                ...
                if ( !m_kRendererImageDataA.updateRenderer(
                    rayTracerA,
                    m_kRGBTA,
                    ( (JPanelVolOpacityRGB)(m_kVolumeRenderer.getVolOpacity())).getRGB_OpacityA(),
                    m_kVolumeRenderer.getVolOpacity().getGradMagA(),
                    ( (JPanelVolOpacityRGB)(m_kVolumeRenderer.getVolOpacity())).getRGB_OpacityA_GM(),
                    timeSliceA,
                    bUpdatedTimeSlice,
                    forceShow ) ) {
                    return false;
        ...
        if ( null != m_kImageB && null != rayTracerB ) {
            ...
            if ( !m_kImageB.isColorImage() ) {
                ...
                if ( !m_kRendererImageDataB.updateRenderer(
                    rayTracerB,
                    LUTb,
                    ((ViewJComponentVolOpacity)( (JPanelVolOpacity)(m_kVolumeRenderer.getVolOpacity())).getCompB()).getOpacityTransferFunction(),
                    ((ViewJComponentVolOpacity)( (JPanelVolOpacity)(m_kVolumeRenderer.getVolOpacity())).getCompA_GM()),
                    timeSliceB,
                    bUpdatedTimeSlice,
                    bUpdatedLut,
                ...
                if ( !m_kRendererImageDataB.updateRenderer(
                    rayTracerB,
                    m_kRGBTB,
                    ( (JPanelVolOpacityRGB)(m_kVolumeRenderer.getVolOpacity())).getRGB_OpacityB(),
                    m_kVolumeRenderer.getVolOpacity().getGradMagB(),
                    ( (JPanelVolOpacityRGB)(m_kVolumeRenderer.getVolOpacity())).getRGB_OpacityB_GM(),
                    timeSliceB,
                    bUpdatedTimeSlice,
                    forceShow ) ) {
                    return false;
    ...
    /**
     * A virtual track ball.  This method generates a rotation matrix from
     * (x0,y0) and (x1,y1), two points in [-1,1]^2.  If either point is
     * outside the unit circle, it is projected onto the unit circle.  The
     * corresponding sphere points (x0,y0,z0) and (x1,y1,z1) are computed.
     * The cross product is used as the axis of rotation.  The angle between
     * the vectors is used as the angle of rotation.  The rotation matrix is
     * used to rotate the oriented bounding box of the 3D image.
     */
    public boolean moveTrackBall( float fX0, float fY0, float fX1, float fY1 ) {
        // (x0,y0) and (x1,y1) are in [-1,1]^2.  The track ball is the unit
    ...
    /**
     * Call by the surface volume render to update transform changes.
     * Composite the slice trace by the specified space size.
    ...
    /**
     * One of the overrides necessary to be a MouseListener.  When a mouse
     * button is pressed, the application is starting a drag operation to
     * rotate the virtual track ball.  The initial mouse location is
     * recorded for use by 'moveTrackBall', stored as a point (x0,y0) in
     * [-1,1]^2.
     *
    ...
    /**
     * One of the overrides necessary to be a MouseListener.  When a mouse
     * button is released, the application is finishing a drag operation to
     * rotate the virtual track ball.  The final mouse location is
     * recorded for use by 'moveTrackBall', stored as a point (x1,y1) in
     * [-1,1]^2.  The virtual track ball is moved into its final position
     ...
     * member handles rotating the virtual track ball during the dragging
     * of the mouse after a mouse-press event but before a mouse-release
     * event.  Each drag event causes the virtual track ball to be rotated.
     * Each such rotation causes a ray trace to occur, but the trace is
    ...
    /**
     * One of the overrides necessary to be a MouseMotionListener.  This
     * member only exists to satisfy the conditions of being a
     ...
     *  @return float Amount in range [0,1] to apply to image A.
     */
    ...
    /* Updates the BufferedImage, Dimension, and render buffers when the
     * render target image size changes so that the image can be redisplayed
     * at the new target resolution. */
                              ...
                              int[] rvolBufferA, int[] rvolBufferB  )
    {
         ...
         * size: */
        m_kImage = new BufferedImage( iMaxExtent, iMaxExtent,
