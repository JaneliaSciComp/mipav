...
/**
 *  This class holds the necessary information for each "mouse event" that
 * shows up in the list.  In reality, the "mouse event" listed is a vector of
 * mouse events that begins with a mousePressed and ends with a mouseReleased.
 * In between are mouseDragged events.  In this way all events that happen on
 * the canvas are accounted for.  If we were to put every event that actually
 * happened in the list, the list would soon contain hundreds of elements.  The
 * mouseEvents vector can be empty, as is the case when a view is saved.  Then
 * just the name and the view are saved; the vector holding mouseEvents remains
 * empty.
 */
public class MouseEventVectorPlotter
    implements Serializable
{
  ...
  /**
   * Creates a new structure to hold necessary information, with
   * the name in the list and the beginning view saved.  The
   * Vector containing the rest of the mouseEvents is also created
   * but begins empty.  Elements are added with the add method, below.
   *
   *   @param	name	Name of this view or mouse event.
   *   @param	view	Transform representing the view.
   *   @param  first    If this is the first-time accessed use <code>true</code>
   *                    otherwise, use <code>false</code>.
   *   @param  state    The state being recorded.
   *   @param  mode     Current mode.
   */
  public MouseEventVectorPlotter(String name, Transform3D view, boolean first,
                          Object state, int mode)
  {
    this.name = name;
    this.view = view;
    this.first = first;
  ...
  /**
   * Returns flag indicating if this is the first mouse event vector in a
   * series.
   *	@return		<code>true</code> if the first mouse event vector,
   ...
   *	@return		Vector containing all the mouse events that happened
   *				after (and including) the mousePressed event.
   */
   ...
   *	@return		Vector containing all the states parallel to
   *               mouse events.
   */
  ...
  /**
   *	Called by some native invokeMethod.  Provides a way to read this
   *	class out as a object stream.  First, the size of the vector is
   *	read, then each mouseEvent into the vector, then the name, then the
   *	view.  The view is read in as a String because it is not serializable.
   *	The read method corresponds to the write method, below.
   *	@param stream	Object stream to read from.
   *    @throws IOException     If the matrix being read in by the
   *                            JDialogMouseRecorder has the incorrect size.
   */
    ...
    for (int i = 0; i < size; i++) {
      try {
        ...
        if (eventRead instanceof MouseEvent) {
          ...
          MouseEvent newEvent = new MouseEvent(new Container(),
                                               event.getID(),
                                               event.getWhen(),
                                               event.getModifiers(),
                                               event.getX(),
  ...
  /**
   *	Called by some native invokeMethod.  Provides a way to write this
   *	class out as a object stream.  First, the size of the vector is
   *	written, then each mouseEvent in the vector, then the name, then the
   *	view.  The view is written out as a String because it is not serializable.
   *	The write method corresponds to the read method, above.
