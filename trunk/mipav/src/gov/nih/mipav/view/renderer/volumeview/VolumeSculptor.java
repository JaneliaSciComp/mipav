...
/**
 *      Implementation of Volume Sculpting for the RayCast and ShearWarp
 *      Volume Renderers. See Sculptor.java.
 *
...
public class VolumeSculptor extends Sculptor {
    ...
    /**
     * Constructor:
     * @param VolumeRenderer, reference to current renderer
     * @param int, Canvas Width
     * @param int, Canvas Height
     */
    public VolumeSculptor( VolumeRenderer kVolumeRenderer,
            int iSculptWidth, int iSculptHeight ) {
    ...
    /*
     * applySculpt: called by ViewJFrameVolumeView when the user presses the
     * "Apply Sculpt" button.
     *
     * This function has several steps:
     *
     * 1. Getting the ModelImage volume data and correctly calculating the
     * center of the volume and the spacing of the volume.
     *
     * 2. Calculating the viewing Transformations.
     *
     * 3. Determining which voxels in the volume fall within the sculpt region
     drawn on the screen, and setting those values to the minimum voxel
     value.
     */
    public boolean applySculpt() {
        ...
        /*
         * 1. Access the volume data and calculate the bounds of the data,
         center, and spacing parameters:
         */
         ...
         * of the volume in 3Space. The ModelImage volume slices may be spaced
         * differently than is represented by indexing into the ModelImage
         * volume array -- an implied spacing of 1,1,1 in each of the x,y,z
         * directions. The actual spacing may be different in any of the
         * directions, and that spacing is important in the raytracing step
         * for producing a correctly rendered volume. The values must be
         * reproduced here, since the rotation of the volume is reversed
         * before projecting the volume onto the 2D Sculpt Image for
         * sculpturing. */
         ...
         *
         * Rotation is applied to the volume as a whole, and is performed in
         * the raytracing step. The rotation matrix is retrieved from the
         * ViewJComponentRayCastRenderImage componentImageXY in the
         * VolumeRenderer.
         *
         * Once the volume is raytraced, the 2D image is texture-mapped onto a
         * 2D polygon and displayed in 3D world coordinates. Translation and
         * Zoom are preformed by translating the polygon in the x,y, or
         * z-directions (for zoom). These values are retrieved from the
         * TransformGroup objTransXY in the VolumeRenderer.
         */
         ...
         * volume. Rotation is preformed in the raytracer, before raytracing
         * the volume. The endresult of the raytraced volume is a 2D texture,
         * which is texture-mapped onto a 2D polygon in world
         * space. Translation and Zoom (scale) are performed as operations on
        ...
        kRotateMatrix = new Matrix3d( kRenderImageXY.getRayTracerA().getAxis( 0 ).x,
                kRenderImageXY.getRayTracerA().getAxis( 0 ).y, kRenderImageXY.getRayTracerA().getAxis( 0 ).z,

                kRenderImageXY.getRayTracerA().getAxis( 1 ).x, kRenderImageXY.getRayTracerA().getAxis( 1 ).y,
                kRenderImageXY.getRayTracerA().getAxis( 1 ).z, kRenderImageXY.getRayTracerA().getAxis( 2 ).x,
                kRenderImageXY.getRayTracerA().getAxis( 2 ).y, kRenderImageXY.getRayTracerA().getAxis( 2 ).z );
        kRotateMatrix.invert();

        /* Zooming and Translating is done with the TransformGroup
         * objTransXY. Both are done after the volume is raytraced, and the
         * operations are performed on a texture-mapped polygon in
         * world-space. matrixZoomTranslate is the final value used in this
         * function:  */
        ...
        /*
         * Zooming is done as a translation in Z, the value is an ofset from
         * the original Z-position, which is stored in the TransformGroup
         * setup in the simple universe. This step gets that transform, and
         * copies the final value into matrixNormalizedCoords.
         */
         ...
         * produced by the raytracer is texture mapped onto the polygon. Since
         * all textures are mapped onto a square of size 2, larger textures
         ...
         * translation of the polygon in Z, relative to the original
         * translation, which is stored in kZoomMatrix.m23. The
         * amount of scale is proportional to the change in the z-distance to
         * the polygon. */
    ...
    /*
     * undoSculpt: called by the ViewJFrameVolumeView object when the user
     * presses the "Undo Sculpt" button. It resets the volume data back to the
     * original values, using the data stored in the m_aiImage_backup data
     * members.
     */
    public void undoSculpt() {
        ...
        if ( m_kVolumeRenderer != null ) {
            kRenderImageXY = (ViewJComponentRenderImage)
                    ( m_kVolumeRenderer.getComponentImageXY() );
            kImageAref = m_kVolumeRenderer.getImageA();
            kImageBref = m_kVolumeRenderer.getImageB();
        }
    ...
    /* This sequence causes the m_kVolumeRenderer to rerender the volume
     * data, from the current ModelImage, not the data that is stored in
     * the Renderer. */
    ...
    /* Update the underlying volume and rerender. This function is called when
    ...
    public void save( FileWriteOptions options, int filterType ) {
        ...
        if ( options.isSaveAs() ) {
            ...
            if ( options.isSet() ) {
                ...
                try {
                    ViewFileChooserBase fileChooser = new ViewFileChooserBase( ViewUserInterface.getReference(), true, true );
