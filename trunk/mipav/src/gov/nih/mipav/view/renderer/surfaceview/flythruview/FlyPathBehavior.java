 ...
 */
public class FlyPathBehavior extends Behavior
    implements KeyListener {

    /**
     * Instances which want to be notified of updates to this behavior
     * should implement this interface by providing the viewChanged
     * method implementation.
     */
    static public interface Callback {

        /**
         * @param kBehavior reference to this MjFlyPathBehavior in which
         * the view changed.
         * @param iEvent Bitmask identifies the event(s) which caused the view
         * to change.  Bitmask created from OR of EVENT_* defintions.
         */
        public void viewChanged( FlyPathBehavior kBehavior, int iEvent );
    }

    static public final int EVENT_CHANGE_ALL = 0xffffffff;
    static public final int EVENT_CHANGE_POSITION = 0x00000001;
    static public final int EVENT_CHANGE_ORIENTATION = 0x00000002;
    static public final int EVENT_CHANGE_BRANCH = 0x00000004;

    /**
     * Setup to fly along the specified path and look around.
     * @param kFlyPathGraph FlyPathGraphCurve contains the information
     * regarding the graph representation of 3D path segments represented
     * by Curve3 instances
     * @param kAnnotateList FlyPathAnnotateList contains the list of
     * annotation points.
     * @param kTransformPosition TransformGroup contains the Transform3D
     * for the current viewing position.
     * @param kTransformDirection TransformGroup contains the Transform3D
     * for the current viewing direction.
     * @param kTransformOrientation TransformGroup contains the Transform3D
     * for the current viewing orientation.
    ...
    public FlyPathBehavior( FlyPathGraphCurve kFlyPathGraph,
            ...
            TransformGroup kTransformPosition,
            TransformGroup kTransformDirection,
            TransformGroup kTransformOrientation,
     ...
     * The viewChanged method of the Callback interface will be called
     * whenever anything about this behavior changes.
     * @param kCallback reference to instance implementing the Callback
     * interface; may pass null reference to disable the callback
     */
    ...
    /**
     * Access the index of the curve in the path graph currrently
     * positioned along.
     ...
     * @return int Index of currently selected branch; -1 if not currently
     * at a branch point.
     */
    ...
    /**
     * Access the normalized distance traveled from the beginning of
     * the current branch path.  Also represents the minimum of the range
     * that has been unvisited.
     * @return float Normalized distance along the current branch path
     * that is minimum of the unvisited range.
     */
    ...
    /**
     * Access the normalized distance traveled from the end of
     * the current branch path.  Also represents the maximum of the range
     * that has been unvisited.
     * @return float Normalized distance along the current branch path
     * that is maximum of the unvisited range.
     */
     ...
     * If this distance is zero, then the view direction is the tangent
     * to the path curve at the current position.
    ...
    /**
     * Get indication of whether the movement along the current path
     * is in the forward or reverse direction.
     * @return boolean True if the movement is along the forward direction
     * along the path.
     */
    ...
    /**
     * Retrieve the current relative orientation transform matrix for viewing
     * which is relative to the current base direction transform for viewing.
    ...
    /**
     * Implementation of the Behavior abstract method.  Process events
     * in the criteria which are the ones we setup for.
     * @param kCriteria collection of WakeupCriterion to which this
     * Behavior is responding to events.
     */
    ...
    /**
     *	Class used to send Standard key events to the canvas.  Must subclass Thread
     *	because a single <code>keyPressed</code> event on one of the mouse
     *	buttons needs to generate <code>keyReleased</code> events on the canvas until
    ...
    class StandardKey extends Thread {
        ...
        /**
         *	Runs the thread.  While the button is pressed, dispatches mouse dragged events at a rate
         *	consistent with the velocity slider.  Once the mouse is released, <code>pressed</code> will be
         *	set to false and the loop will stop.
         */
        public synchronized void run() {
            while ( pressed ) {
                ...
                evt = new KeyEvent( parentScene.getCanvas(), id, when, 0, currentEvent.getKeyCode(), currentEvent.getKeyChar(), currentEvent.getKeyLocation() );
            }
        }
    }
     ...
     * @param _kBranchState BranchState Instance which describes the
     * state of the branch.
     */
    ...
    /**
     * Loop through all of the possible branch choices and select
     * the one that is the closest to the current view direction
     * vector.  Take a vector from the current view point to a
     * point along each choice branch.  Compute the dot-product
     * between the current view direction vector and each of these
     * branch direction vectors and take the one with the largest
     * positive value, i.e., most in alignment.
     */
    ...
    /**
     * Make the specified branch be the current state for following.
     * @param kBranchState BranchState Instance which describes the
     * state of the branch.
     */
     ...
     * That is, remove all yaw/pitch/roll.
     */
    ...
    /**
     * Take the specified number of steps along the current path using
     * the current path step size.  If the number of steps is negative,
     * then the steps are taken in reverse.
    ...
    /**
     * Create the array of branch choices and set the mode which forces
     * the user to select from among the branch choices.
     * @param iBranchParent int Index which identifies the parent branch.
     * @param iBranchPoint int Index which identifies the point along
     * the branch path where the branching occurs for the choices.
     */
     ...
     * If the value is outside that range, then sound a beep.
    ...
    /**
     * Update the camera position along the path based on the specified
     * distance from the beginning.
     * @param fNormalizedDist normalized distance from the beginning of the
     * path for the location of the camera for viewing
     */
    ...
    /**
     * Set the camera to the specified be located at the specified
     * view point and looking in the specified direction.
     * @param kViewPoint coordinates of the camera view point
     * @param kViewdirVector coordinates of the camera view direction vector.
    ...
    /**
     * Call the implementation of the callback's notification method,
     * if a callback instance has been defined.
     * @param iEvent Bitmask identifies the event(s) which caused the view
     * to change.  Bitmask created from OR of EVENT_* defintions.
     */
    ...
    private class BranchState extends Object
        ...
        // List of branch points (normalized path distances where
        // one or more branches starts).
         ...
         * @param kFlyPathGraph FlyPathGraphCurve Data structure which
         * contains all of the information about each branch and its
         * connections.
         */
        ...
        /**
         * Create a copy of this instance which has the same information
         * except that the state of the moving forward is inverted.
         * @return BranchState New instance which is a copy of this
         * instance except that the moving forward flag is inverted.
         */
        ...
        /**
         * Reset parameters to start at the beginning of the path
        ...
        /**
         * Return the normalized path distance of a point further
         * down the branch curve in the current heading.
         * @param fForwardDist float Distance further down the branch
         * curve in the current heading.  This value can be negative
         * for a point in the reverse heading.
         * @return float Normalized path distance in the [0,1] range
        ...
        /**
         * Return the position of the curve of the point further down
         * the curve the specified distance in the current heading.
         * @param fDist float Distance further down the branch
         * curve in the current heading.  This value can be negative
         * for a point in the reverse heading.
         * @return Point3f Coordinates of the 3D point further down
         * along the curve.
         */
        ...
        /**
         * Update the range of normalized path distances that have
         * not been visited.  Call this method before changing the
         * current normalized path distance!
         * @param fNewNormalizedPathDistance float Normalized path
         * distance about to be set for this branch.
         */
        ...
        /**
         * Return an index which identifies segment the specified
         * normalized path distance belongs.
         * @param fNormalizedPathDist float Normalized path distance
         * in the range [0,1] along the branch path.
         * @return int An index of zero is returned if the distance
         * is before the first branch point or if there are no
         * branch points.  An index of one is returned if the distance
         * is greater than or equal to the first branch point but
         * less than or equal to second branch point.
         */
    ...
    // View direction component for this view transform.  This assumes
    // a viewup vector (see m_kVectorViewup member).
    // This transform causes the view to look down the path when the
    // yaw=pitch=roll=0.
    ...
    // What is the increment in distance along the path that each
    // step takes.  Increment may be negative indicating that the
    // path is being followed in the opposite direction.
    public float m_fPathStep;

    // How far ahead down the path should the view be aimed.  If zero,
    // then the view direction is the tangent to the curve.  If positive,
    // the the view direction is to look at that point along the path.
    ...
    // When a branch point along the current branch is reached,
    // this flag is set until the user selects a branch to follow.
    private boolean m_bChooseBranch = false;

    // When a branch point along the current branch is reached,
    // this array is filled with the possible BranchState instances
    // that the user can select from for following.  Once movement
    // is made along one of those selected from the list, then this
    // list is cleared.  An index is maintained to indicate which
    // branch in the list is currently selected.
    private BranchState[] m_akBranchChoice = null;
    private int m_iBranchChoiceIndex = -1;

    // The desired view up vector is a normalized average of these
    // two orthogonal axes vectors.  The problem is that if just one desired
    // view up vector is chosen, then when the view direction vector is
    // "aligned" with that vector, some other view up vector would have
    // to be chosen and some rule would have to be defined for that.
    ...
    // Instance which implements the Callback interface whose
    // viewChanged method is to be called whenever anything about
