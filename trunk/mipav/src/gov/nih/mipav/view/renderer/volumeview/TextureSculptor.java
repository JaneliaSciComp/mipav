...
public class TextureSculptor extends Sculptor
{
    ...
    /*
     * Constructor:
     * @param SurfaceRender, reference to container class
     */
     ...
     * m_kSurfaceRender's mouse response, so the mouse can be used to draw the
     * sculpt outline.
     */
    ...
    public boolean applySculpt()
    {
         ...
         * relative scale in screen space -- in the View.screenscale
         * variable. The scale is relative to the original scale, which is
         * stored in m_kSurface.getOriginalScreenScale. The amount of scale is
        ...
        else
        {
            ...
            kTransGroup = m_kSurface.getUniverse().
            ...
            fZoomScale = (float)(kZoomMatrix.m23/
                                 (kZoomMatrix.m23-kMatrixZoomTranslate.m23));
            fZoomScale /= 2.0f;
        ...
        float fXTranslate = (float)(kMatrixZoomTranslate.m03 *
                                    fScale *
                                    (float)iCanvasWidth);
        float fYTranslate = (float)(kMatrixZoomTranslate.m13 *
                                    fScale *
                                    (float)iCanvasWidth);
        ...
        for ( int iZ = 0; iZ < iZBound; iZ++ )
        {
            for ( int iX = 0; iX < iXBound; iX++ )
            {
                for ( int iY = 0; iY < iYBound; iY++ )
                {
                    ...
                    double x = (((double)iX/(double)(iXBound-1) -
                                 kCoordMapX.w) /
                                kCoordMapX.x);
                    double y = (((double)iY/(double)(iYBound-1) -
                                 kCoordMapY.w) /
                                (kCoordMapY.y));
                    double z = (((double)iZ/(double)(iZBound-1) -
                                 kCoordMapZ.w) /
                                (kCoordMapZ.z));
                    kPoint.x = x; kPoint.y = y; kPoint.z = z;
                    ...
                    int iScreenX = (int)(m_iSculptImageWidth -
                                        (kPoint.x * m_iSculptImageWidth +
                                         m_iSculptImageWidth/2));
                    int iScreenY = (int)((kPoint.y * m_iSculptImageWidth +
                                         m_iSculptImageHeight/2));
                    ...
                    if ( ( iScreenX >= 0 ) &&
                         ( iScreenX < m_iSculptImageWidth ) &&
                         ( iScreenY >= 0 ) &&
                         ( iScreenY < m_iSculptImageHeight )   )
                    {
                        /* If the voxel index falls inside the sculpt
                         * region: */
                        if ( m_kSculptImageOpaque.getRGB( iScreenX, iScreenY )
                             == m_iColorSculpt )
