 ...
 * This panel loads the mask image and surface file.  It also adjusts
 * the endoscopy view parameters.
 */
public class JPanelVirtualEndoscopySetup extends JPanelRendererBase {
    ...
    /** Keep a copy of the loaded triangle mesh and any other properties
    ...
    /**
     * Update the appropriate controls based on the current settings
     * of the position-based information in the FlyPathBehavior instance.
     * @param kFlyPathBehavior FlyPathBehavior Instance which contains
     * the current state of position along the branches.
     */
    ...
    /**
     * Update the appropriate controls based on the current settings
     * of the orientation-based information in the FlyPathBehavior instance.
     * @param kFlyPathBehavior FlyPathBehavior Instance which contains
     * the current state of orientation along the branches.
     */
    ...
    /**
     * Update the display to show the current gaze distance for
     * looking ahead down the path.
     * @param fStepDist magnitude is the distance increment along the path
     * for moving; sign is the direction of moving along the path from
     * one end to the other or vice versa.
     * @param fGazeDist distance ahead for looking down the path.
     */
    protected void setStepGazeDist( float fStepDist, float fGazeDist ) {
        m_kTextStepGaze.setText(
                m_kDecimalFormat.format( Math.abs( fStepDist ) ) + " / " + m_kDecimalFormat.format( fGazeDist ) );
    }

    /**
     * Update the display to show the current branch and path direction
     * (forward or reverse) for moving along the length of the path.
     * @param iBranch index which identifies the branch currently on
     * @param bPathMoveForward flag set if moving forward, as opposed to
     * in reverse, along the length of the path.
     */
    protected void setBranchInfo( int iBranch, boolean bPathMoveForward ) {
        m_kTextBranch.setText(
                ( bPathMoveForward ? "Start-to-End" : "End-to-Start" ) + " on " + Integer.toString( iBranch ) );
    }

    /**
     * Update the display to show the current distance along the
     * length of the path.
    ...
    protected void setPathDistance( float fDist, float fTotalDist ) {
        m_kDecimalFormat.setMinimumFractionDigits( 1 );
        m_kDecimalFormat.setMaximumFractionDigits( 1 );
        m_kTextDistance.setText(
                m_kDecimalFormat.format( fDist ) + " of " + m_kDecimalFormat.format( fTotalDist ) + "  ("
                + m_kDecimalFormat.format( 100.0f * fDist / fTotalDist ) + "%)" );
    }

    /**
     * Update the display to show the coordinates of the current
    ...
    protected void setPathPosition( Point3f kPosition ) {
        m_kDecimalFormat.setMinimumFractionDigits( 1 );
        m_kDecimalFormat.setMaximumFractionDigits( 1 );
        m_kTextPosition.setText(
                m_kDecimalFormat.format( kPosition.x ) + " " + m_kDecimalFormat.format( kPosition.y ) + " "
    ...
    /**
     * Update the display to show the current base viewing direction
     * at the current point along the path.
    ...
    protected void setViewDirection( Vector3f kVector ) {
        ...
        m_kTextDirection.setText(
                m_kDecimalFormat.format( kVector.x ) + " " + m_kDecimalFormat.format( kVector.y ) + " "
                + m_kDecimalFormat.format( kVector.z ) );
    }

    /**
     * Update the display to show the current viewing orientation which is
     * always relative to the base viewing direction.
    ...
    protected void setViewOrientation( Matrix3f kMatrix ) {
        ...
        m_kTextOrientation.setText(
                m_kDecimalFormat.format( fRotateX ) + " " + m_kDecimalFormat.format( fRotateY ) + " "
    ...
    private void init() {
        ...
        JLabel kLabelPercentBSplineNumControlPoints = new JLabel(
                "BSpline fit num control points fraction of branch samples" );
        ...
        JLabel kLabelSegmentSurfaceBranchSamplesReductionFactor = new JLabel(
                "Surface segmentation branch samples reduction factor" );
    ...
    private void loadingSurface() {
        // Display a file dialog to select the surface file to load.
        // Instantiate each time in order to cause a re-scan of
        // the files in the directory.
        // ViewJSimpleProgressBar kProgress = new ViewJSimpleProgressBar( kFile.getName(), "Loading surface file ..." );
        ...
        try {
            ...
            double dMeanCurvatureStddev = Math.sqrt(
                    ( dMeanCurvatureSum2 - dNumVertices * dMeanCurvatureMean * dMeanCurvatureMean )
                            / ( dNumVertices - 1.0 ) );
