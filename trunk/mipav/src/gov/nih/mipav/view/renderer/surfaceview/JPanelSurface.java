...
import gov.nih.mipav.model.file.FileInfoSurfaceXML;
...
/**
 *	A Java3D-based dialog for surfaces represented as triangle
 *	meshes.  The surfaces are displayed in the 3D viewer that already
 *	contains a 3D image.  The mouse behavior is the same for the image
 *	as it is for the surfaces, so that when the user zooms or rotates,
 *	it looks like the surfaces are part of the tri-image.
 *
 *	@author David Eberly
...
public class JPanelSurface extends JPanelRendererBase
    implements
            ListSelectionListener, // for surface list box
            MouseListener, // for picking operations
            MouseMotionListener, // for picking static bulb operations
            ChangeListener { // for slider changes (LOD change)
    ...
    /**
     *	The parent object that holds necessary the structures to which the surfaces
     *	will be added and modified.
     */
    ...
    /** The material options button, which launches the material editor
     * window. */
    private JButton m_kAdvancedMaterialOptionsButton;

    /** Stereo render button, launches the JStereoWindow for viewing the
     * ModelTriangleMesh in stereo: */
    ...
    /**
     * The index of the lights in JPanelLights is different from
     * the order maintained here.  This array maps from our index to JPanelLights' index.
     */
    private final int[] m_aiMapIndexToJPanelLightsIndex;

    /**
     * The index of the lights in JPanelLights is different from
     * the order maintained here.  This array maps from JPanelLights' index to our index.
     */
     ...
     *	stores the index into the list for the selected item.  If no items
     *	are selected or if multiple items are selected, the value is -1.
     */
    ...
    /**
     *	The colors for the first six surfaces are fixed.  For the seventh and
     *	later surface, the colors are randomly generated.  This member provides
    ...
    private static Color3f[] fixedColor = { new Color3f( 0.0f, 0.0f, 0.5f ), // blue
    ...
    /**
     *	An array of eight lights that can be used for illuminating
     *	the surfaces in the scene.  The lights at indices 0 and 7
     *	are initially enabled; the other lights are initially
     *	disabled.  The default positions for the lights are at the
     *	8 corners of the cube [0,1]^3.  Light 0 is at (0,0,0)
     *	and light 7 is at (1,1,1), thereby providing suitable
     *	lighting for the visible surfaces in the scene.  Other
     *	lights can be enabled as desired through the light attribute
     *	dialog.
     */
    ...
    /**
     *	Java3D does not expose an intensity member for lights, but it
     *	does support attenuation for point lights.  The linear and
     *	quadratic attenuation coefficients are set to zero.  The
     *	constant attenuation coefficient is then the inverse of the
     *	light intensity.  To avoid divisions by numbers close to zero,
     *	the intensity is clamped to a minimum and maximum intensity.
     */
    ...
    /**
     *	The PickCanvas object that ties together the canvas and the
     *	surfaces branch group to allow pick operations on the scene.
     */
     ...
     * Defaults to MIPAV default directory.
     */
    ...
    protected Point3f[/* m_iVQuantity */] m_akVertex;

    /** Surface vertext conectin array. */
    protected int[/* 3*m_iTQuantity */] m_aiConnect;
    ...
    /**
     *	Creates the Java3D scene graph for the application.  The surfaces are
     *	all children of the BranchGroup surfaceRootBG, which is in turn a child of
     *	the TransformGroup in ViewJFrameTriImage3D.  That transform group
     *	controls how both the surfaces and the three image planes are displayed;
     *	by making the surfaces a child of the transform, the surfaces and image
     *	planes move together.  The BranchGroup created here is used for the
     *	PickCanvas, so that the pick tool ignores the image planes when the mouse is
     *	clicked and just looks for the surfaces.
     *	<p>
     *	This constructor also adds lights to the scene.  Because of the texture maps
     *	on the image planes, lights don't make a difference in how the image planes
     *	are displayed.  Therefore, the lights are only necessary for the surfaces.
     *
     ...
     */
    public JPanelSurface( SurfaceRender parent, Canvas3D canvas, TransformGroup surfaceRoot, float xBox,
            float yBox, float zBox ) {
     ...
     *	specified color.  A Shape3D object is created whose geometry is the
     *	triangle mesh and whose appearance contains a material.  The material's
     *	diffuse and specular colors are set to the specified color.  A
     *	BranchGroup node is created as the parent of the Shape3D object.
     *	The parent node is necessary to allow dynamic modification of a
     *	compiled scene graph.
     *
     *  Create outVoxels and outerVoxels byte arrays with sizes equal to the
     *  size of the volume and all the values initialized to zero.
     *  The surface is designed to go thru the voxel center so that the
     *  target points will be enclosed.  Calculate the surface point in
     *  standard voxel space that goes from 0 to xDim - 1, 0 to yDim - 1, and
     *  0 to zDim - 1.  Find the floor and ceilings for these x,y, and z values -
     *  this gives a total of 8 voxels.  Set these 8 voxels to 1 in the
     *  outVoxels array.  The verticies for a triangle in this standard voxel
     *  space may be too far apart and crate gaps.  To prevent gaps from occurring
     *  recursively divide a triangle into 4 subtriangles if any 2 of the verticies
     *  are a distance >= 1 apart.  In creating 4 subtriangles the
     *  midpoints of the original triangle segments are added to the outVoxels
     *  array.  Set all edge voxels of outVoxels that do not equal 1 to 2.
     *  Recursively set all zero valued outVoxels voxels that have a 6-connected
     *  neighbor with a value of 2 to 2.  Then, in outVoxels all voxels outside
     *  the surface will have a value of 2, all voxels serving as floors or
     *  ceilings to the surface points will have a value of 1, and all voxels
     *  inside the surface will have a value of 0.  For all the 1 values in
     *  outVoxels that represent a rounding toward the inside of the surface,
     *  set the corresponding values in outerVoxels to 1.
     *
    ...
    private void addSurface( File file, Color4f color, String name, float opacity, int idx, boolean isVisible ) {
            ...
            try {
                ...
                for ( i = 0; i < voi.length; i++ ) {
                    ...
                    SurfaceAttributes surface = new SurfaceAttributes( sphereRoot, filePath, fileName,
                            new Color4f( voi[i].getColor() ), 64, 100, PolygonAttributes.POLYGON_FILL, numTriangles,
                            volume, area, ( iType != 0 ), opacity, true, getSurfaceMask() );
                    if ( idx == -1 ) {
        ...
        try {
            if ( iType == 0 ) {
                ...
                for ( i = 0; i < iQuantity; i++ ) {
                    if ( isSur == true ) {
                        akComponent[i] = ModelTriangleMesh.loadTMesh( in, progress, i * 100 / iQuantity, iQuantity, true );
                    } else {
                        if ( i == 0 ) {
                    ...
                    for ( j = 0; j < aiConnect.length; ) {
                        ...
                        d1 = (float) Math.sqrt(
                                ( sx1 - sx2 ) * ( sx1 - sx2 ) + ( sy1 - sy2 ) * ( sy1 - sy2 )
                                + ( sz1 - sz2 ) * ( sz1 - sz2 ) );
                        d2 = (float) Math.sqrt(
                                ( sx1 - sx3 ) * ( sx1 - sx3 ) + ( sy1 - sy3 ) * ( sy1 - sy3 )
                                + ( sz1 - sz3 ) * ( sz1 - sz3 ) );
                        d3 = (float) Math.sqrt(
                                ( sx2 - sx3 ) * ( sx2 - sx3 ) + ( sy2 - sy3 ) * ( sy2 - sy3 )
                                + ( sz2 - sz3 ) * ( sz2 - sz3 ) );
        ...
        SurfaceAttributes surface = new SurfaceAttributes( root, file.getPath(), name, color, 64, 100,
                PolygonAttributes.POLYGON_FILL, numTriangles, volume, area, ( iType != 0 ), opacity, center, getSurfaceMask() );
        if ( idx == -1 ) {
            surfaceVector.add(surface);
        } else {
            surfaceVector.set(idx, surface);
     ...
     * @param sz3 float
     */
    private void subTriangles( float sx1, float sy1, float sz1, float sx2,
            float sy2, float sz2, float sx3, float sy3,
            float sz3 ) {
        ModelImage imageA = parentScene.getImageA();
        int[] extents = imageA.getExtents();
        int xDim = extents[0];
        ...
        d1 = (float) Math.sqrt(
                ( sx1 - sx4 ) * ( sx1 - sx4 ) + ( sy1 - sy4 ) * ( sy1 - sy4 ) + ( sz1 - sz4 ) * ( sz1 - sz4 ) );
        d2 = (float) Math.sqrt(
                ( sx1 - sx5 ) * ( sx1 - sx5 ) + ( sy1 - sy5 ) * ( sy1 - sy5 ) + ( sz1 - sz5 ) * ( sz1 - sz5 ) );
        d3 = (float) Math.sqrt(
                ( sx4 - sx5 ) * ( sx4 - sx5 ) + ( sy4 - sy5 ) * ( sy4 - sy5 ) + ( sz4 - sz5 ) * ( sz4 - sz5 ) );
        if ( ( d1 >= 1.0f ) || ( d2 >= 1.0f ) || ( d3 >= 1.0f ) ) {
        ...
        d1 = (float) Math.sqrt(
                ( sx4 - sx5 ) * ( sx4 - sx5 ) + ( sy4 - sy5 ) * ( sy4 - sy5 ) + ( sz4 - sz5 ) * ( sz4 - sz5 ) );
        d2 = (float) Math.sqrt(
                ( sx4 - sx6 ) * ( sx4 - sx6 ) + ( sy4 - sy6 ) * ( sy4 - sy6 ) + ( sz4 - sz6 ) * ( sz4 - sz6 ) );
        d3 = (float) Math.sqrt(
                ( sx5 - sx6 ) * ( sx5 - sx6 ) + ( sy5 - sy6 ) * ( sy5 - sy6 ) + ( sz5 - sz6 ) * ( sz5 - sz6 ) );
        if ( ( d1 >= 1.0f ) || ( d2 >= 1.0f ) || ( d3 >= 1.0f ) ) {
            subTriangles( sx4, sy4, sz4, sx5, sy5, sz5, sx6, sy6, sz6 );
        }
        d1 = (float) Math.sqrt(
                ( sx3 - sx4 ) * ( sx3 - sx4 ) + ( sy3 - sy4 ) * ( sy3 - sy4 ) + ( sz3 - sz4 ) * ( sz3 - sz4 ) );
        d2 = (float) Math.sqrt(
                ( sx3 - sx6 ) * ( sx3 - sx6 ) + ( sy3 - sy6 ) * ( sy3 - sy6 ) + ( sz3 - sz6 ) * ( sz3 - sz6 ) );
        d3 = (float) Math.sqrt(
                ( sx4 - sx6 ) * ( sx4 - sx6 ) + ( sy4 - sy6 ) * ( sy4 - sy6 ) + ( sz4 - sz6 ) * ( sz4 - sz6 ) );
        if ( ( d1 >= 1.0f ) || ( d2 >= 1.0f ) || ( d3 >= 1.0f ) ) {
        ...
        d1 = (float) Math.sqrt(
                ( sx2 - sx5 ) * ( sx2 - sx5 ) + ( sy2 - sy5 ) * ( sy2 - sy5 ) + ( sz2 - sz5 ) * ( sz2 - sz5 ) );
        d2 = (float) Math.sqrt(
                ( sx2 - sx6 ) * ( sx2 - sx6 ) + ( sy2 - sy6 ) * ( sy2 - sy6 ) + ( sz2 - sz6 ) * ( sz2 - sz6 ) );
        d3 = (float) Math.sqrt(
                ( sx5 - sx6 ) * ( sx5 - sx6 ) + ( sy5 - sy6 ) * ( sy5 - sy6 ) + ( sz5 - sz6 ) * ( sz5 - sz6 ) );
        if ( ( d1 >= 1.0f ) || ( d2 >= 1.0f ) || ( d3 >= 1.0f ) ) {
    ...
    /**
     * ReplaceMesh is used by the Geodesic when a mesh is cut along either an
     * open or closed geodesic curve, the original mesh is changed, but not
     * deleted and no new mesh is added. The orginal mesh, kOld, is replced by
     * the new mesh, kNew
    ...
    /**
     * Called when a mesh is changed by the Geodesic class, when a mesh is cut
     * along the geodesic curve it may be divided into two or more
     * meshes. This function allows the Geodesic class to add meshes to the
     * scene renderer. It follows the pattern of adding a surface through the
     * surface dialog.
    ...
    public void addMesh( ModelTriangleMesh kOld,
        ...
        SurfaceAttributes kSurfaceAttributes = new SurfaceAttributes( root, "", kName, surfaceColor, 64, 100,
                PolygonAttributes.POLYGON_FILL, iNumTriangles, 1.0f, 1.0f, false, 1.0f, new Point3f(), null );
        ...
        for ( iIndex = 0; iIndex < aiSelected.length; iIndex++ ) {
            ( (SurfaceRender) parentScene ).setColor( aiSelected[iIndex], ( (SurfaceAttributes)
                    surfaceVector.get( aiSelected[iIndex] ) ).color );
        }
    }
     ...
     *	viewer window.  This member is called via the mouse listener callback.
     *	If a surface is picked, the list box is updated to show that the surface
     *	is selected and the corresponding information is set for the color
     *	button, the detail slider, and the number of triangles.
     *
     * @param kEvent	The mouse event generated by the picking operation.
     * @return			The picked surface, or null if no surface is located at the
     *      			pick location.
     */
    ...
    /**
     *	Create and initialize the nine lights in the scene graph.  The first eight
     *	lights are positioned at the eight vertices of the cube [-1,1]^3.
     *	Light 0 is at (-1,-1,-1) and light 7 is at (1,1,1), both enabled by
     *	default.  The default color for all lights is white.  The default intensity
     *	is 1.  All surfaces in the scene are illuminated by all enabled lights.  The
     *	ninth light is ambient light, light that seems to come from all directions.
    ...
    /**
     *	The number of lights supported by the viewer (currently 9).  The
     *	only caller of this method is the light attribute dialog, an object
     *	from class JDialogSurfaceLight.  The dialog uses the count to create
     *	a list of lights in the scene.  If the number of supported lights
     *	ever changes, the dialog code does not have to change.
     *
     ...
     * @param iIndex int Index of the bulb in the array as stored
     * in JPanelLights.
     */
    ...
    /**
     *	The only caller of this method is the light attribute dialog, an
     *	object from class JDialogSurfaceLight.  The dialog calls this
     *	to initialize its color button.
     *
    ...
    /**
     * The action taken when the Add button is clicked in the list box.
     * A file dialog is launched that allows the user to select new surfaces
     * to load from disk.
    ...
    private void addSurfaces() throws IOException {
        ...
        ViewUserInterface.getReference().setDefaultDirectory(
                String.valueOf( chooser.getCurrentDirectory() ) + File.separatorChar );
        ...
        for ( int i = 0; i < files.length; i++ ) {
            ...
            {
                kSurfaceXML =
                    new FileSurfaceXML( ViewUserInterface.getReference(),
                                        kName, files[i].getParent(), false );
                kFileInfo =
                    kSurfaceXML.readSurfaceXML( kName, files[i].getParent() );
                if ( kFileInfo != null )
                {
                    int iSurface = addModelTriangleMesh( kFileInfo.getMesh(),
                                                         kFileInfo.getMaterial(),
                                                         files[i], kName, 1.0f );
            ...
            if ( parentScene.getParentFrame().getImageOriginal().getFileInfo()[0] instanceof FileInfoImageXML ) {
                for ( int s = 0; s < parentScene.getParentFrame().getImageOriginal().getExtents()[2]; s++ ) {
                    ( (FileInfoImageXML) parentScene.getParentFrame().getImageOriginal().getFileInfo()[s] ).addSurface(
                            surfaceDirectoryName + kName );
                }
            }
        }
     ...
     */
    private int addModelTriangleMesh( ModelTriangleMesh kMesh, Material kMaterial, File file, String name, float opacity )
    {
        ...
        akMeshes[0] = kMesh;
        BranchGroup root = createSurface( akMeshes, kDiffuse4,
        ...
        SurfaceAttributes surface =
            new SurfaceAttributes( root, file.getPath(), name, kDiffuse4, 64, 100,
                                   PolygonAttributes.POLYGON_FILL,
                                   numTriangles, volume, area,
     ...
     * @throws IOException
     */
    public void addSurfaces( String dir, File file )
        throws IOException {
     ...
     *	All selected surfaces in the list box are removed from the scene
     *	graph.
     */
    private void removeSurfaces() {
        ...
        for ( i = 0; i < aiSelected.length; i++ ) {
            ...
            if ( parentScene.getParentFrame().getImageOriginal().getFileInfo()[0] instanceof FileInfoImageXML ) {
                for ( int s = 0; s < parentScene.getParentFrame().getImageOriginal().getExtents()[2]; s++ ) {
                    ( (FileInfoImageXML) parentScene.getParentFrame().getImageOriginal().getFileInfo()[s] ).removeSurface(
                            ( (SurfaceAttributes) removeSurfaces.get( i ) ).fullPath );
                }
            }
        }
        Vector surfaceNames = new Vector();
     ...
     * @throws IOException
     */
    private void savePortableMesh( ModelTriangleMesh[] meshes, PrintWriter kOut, Color3f color )
        throws IOException {
        int i;
        ModelTriangleMesh[] meshesCopy = new ModelTriangleMesh[meshes.length];
    ...
    /**
     *	The override necessary to be an ActionListener.  This callback is
     *	executed whenever the Add or Remove buttons are clicked, or when
     *	the color button or light button is clicked, or when the combo box changes.
     *	If the Add button is clicked, a file dialog is launched to allow the user
     *  to select new surface meshes to load from disk.  If the Remove
     *  button is clicked, the currently selected surfaces in the list box
     *  are removed from the scene graph.
     *
        ...
        if ( command.equals( "Add" ) ) {
            ...
            for ( int i = 0; i < aiSelected.length; i++ ) {
                ...
                if ( !( (SurfaceAttributes) surfaceVector.get( iIndex ) ).isVOIPt ) {
                    ...
                    new JStereoWindow( new ModelTriangleMesh( m_kSurface.getVertexCopy(),
            ...
            for ( int i = 0; i < aiSelected.length; i++ ) {
                ...
                if ( !( (SurfaceAttributes) surfaceVector.get( iIndex ) ).isVOIPt ) {
                    ...
                    new JFrameSurfaceMaterialProperties( this, iIndex, m_akLights,
                                                         fOpacity, material );
                }
            }
            ...
            {
                BranchGroup root =
                    ( (SurfaceAttributes) surfaceVector.get( iIndex ) ).surface;
                Shape3D shape = (Shape3D) root.getChild( 0 );
            ...
            for ( Enumeration e = surface.getAllChildren(); e.hasMoreElements(); ) {
                ...
                try {
                    ...
                    shape.getAppearance().getPolygonAttributes().setPolygonMode( mode );
                     ...
                     Object obj2;
                     // won't let me read these branch groups.
                     for (Enumeration e2 = ((BranchGroup)obj).getAllChildren(); e2.hasMoreElements();) {
                     obj2 = e2.nextElement();
                     if (obj2 instanceof BranchGroup) {
                     Object obj3;
                     for (Enumeration e3 = ((BranchGroup)obj2).getAllChildren(); e3.hasMoreElements();) {
                     obj3 = e3.nextElement();
                     }
                     }
                     }
    ...
    /**
     *	Takes the image's default transformation and transforms the surface by that
     *	transformation.  This can be changed in the future so that the user can have
     *	a better way of setting the transform.
     */
    ...
    /**
     *	Sets the surface options GUI panel to enabled or disabled.  If there
     *	are 0 or multiple surfaces selected, all the options should be disabled.
    ...
    /**
     *	The override necessary to be a ListSelectionListener.  This callback
     *	is executed whenever the user selects a new item (or items) in the
     *	list box.  If a single item is selected, then the selection index is
     *	remembered in <code>iSelect</code> and the color button, detail slider,
     *	and polygon mode are initialized with the appropriate values corresponding
     *	to the selected surface.  If multiple items are selected, then the
     *	selection index is -1 and the color button is set to the background color
     *	and disabled.  The slider and polygon mode are set to the values found
     *	in the minimum selected surface but are still enabled.  The dialog does not
     *	support changing the color for multiple surfaces at one time but does
     *	support changing level of detail and polygon mode for multiple surfaces at
    ...
    /**
     *	This is called when the user chooses a new color for the
     *	surface.  It changes the color of the surface.
        ...
        for ( int i = 0; i < aiSelected.length; i++ ) {
            ...
            if ( ( (SurfaceAttributes) surfaceVector.get( iIndex ) ).isVOIPt ) {
                ...
                for ( int k = 0; k < root.numChildren(); k++ ) {
                    Shape3D shape = (Shape3D) ( ( (Sphere) ( ( (TransformGroup) ( root.getChild( k ) ) ).getChild( 0 ) ) ).getShape() );
                    Appearance appearance = shape.getAppearance();
    ...
    /**
     *	One of the overrides necessary to be a MouseListener.  The surface
     *	attribute dialog was added as a listener to the 3D canvas on which
     *	the scene is drawn.  Whenever a picking operation occurs on the
     *	canvas, this member function is called.  The pick operation returns
     *	the Shape3D object (null, if no object is situated at the pick
     *	location).  The root of the subtree for the picked surface is
     *	looked up within the surfaces list.  If found, the list box, color
     *	button, detail slider, triangle text, and polygon mode are updated
     *	to display the selected surface.<br><br>
     *
     *	<b>Note</b>.  The use of Shape3D by the picker and BranchGroup by the
     *	surfaces list appears to be more complicated than necessary, and
     *	instead both should use Shape3D or BranchGroup.  In fact it is
     *	necessary to structure the code as it currently is.  The picker needs
     *	to locate actual geometry, so Shape3D objects must enable themselves
     *	to be picked with ray-triangle intersection testing.  If the surfaces
     *	in the list were set up to be Shape3D instead of BranchGroup, then
     *	all add/remove operations would have to be based on Shape3D, not on
     *	BranchGroups.  These operations are not allowed in compiled scene
     *	graphs.<br><br>
     *
     *	<b>Note.</b>  The iteration over the surface list checking for the
     *	BranchGroup whose child is the picked surface is also necessary.
     *	Simpler would be to query the Shape3D object for its parent.  Such a
     *	query is also not supported in a compiled scene graph.
     *
        ...
        if ( ( (SurfaceRender) renderBase ).getProbeDialog() != null ) {
            ...
            if ( findIndex != -1 ) {
                ...
                if ( sphereShowing == false ) {
                    ( (BranchGroup) ( (BranchGroup) ( burnRoot.getChild( findIndex ) ) ).getChild( 0 ) ).addChild(
                            sphereBranch );
                    sphereShowing = true;
                }
                return;
    ...
    /**
     *	One of the overrides necessary to be a MouseListener.  This member
     *	only exists to satisfy the conditions of being a MouseListener.  It
     ...
     *	only exists to satisfy the conditions of being a MouseMotionListener.  It
     *	does nothing when invoked.
     *
     *	@param kEvent The mouse event.
     */
    ...
    public synchronized void mousePressed( MouseEvent kEvent ) { /* stub */
        ...
        if ( myMouseDialog.isRecording() && source == opacitySlider ) {
            ...
            surfaceOpacityEvents = new MouseEventVector( "surfaceOpacitySlider" + surfaceSliderCount, t3D,
                    myMouseDialog.first, parentScene.getSceneState(), parentScene.getMouseMode() );
    ...
    /**
     *	The override necessary to be a ChangeListener for a JSlider.  When
     *	a change occurs, the user has requested that the level of detail be
     *	changed for the currently active surface.  The slider range is [0,100]
     *	and is treated as a percent of maximum level of detail.  A change has
     *	no effect on ModelTriangleMesh objects, but it does change the level of
     *	detail for an ModelClodMesh object.  The number of triangles is also updated.
     *
        ...
        if ( event.getSource() == detailSlider ) {
            ...
            for ( int i = 0; i < aiSelected.length; i++ ) {
                if ( ( (SurfaceAttributes) surfaceVector.get( aiSelected[i] ) ).isVOIPt == false ) {
                    ...
                    float volume = 0;
                    float area = 0;
                    BranchGroup root = ( (SurfaceAttributes) surfaceVector.get( aiSelected[
                            i] ) ).surface;
    ...
    private void importA() {
        ...
        try {
            ...
            SurfaceAttributes surface = new SurfaceAttributes( root, directory, fileName, color, 64, 100,
                    PolygonAttributes.POLYGON_FILL, numTriangles, volume, area, ( iType != 0 ), getSurfaceMask() );
     ...
     */
    private static final String readLine(RandomAccessFile file) throws IOException {
    ...
    /**
     * Get the reference to the surface list.  This method
     * is called by the probePanel.
    ...
    protected void maskInsideVoxels( ModelTriangleMesh kMesh, boolean bHasVertexColor ) {
        ...
        /*
         ModelImage newImage = new ModelImage( parentScene.getImageA().getType(), parentScene.getImageA().getExtents(),
         "Mask Image", parentScene.getImageA().getUserInterface() );
         try {
         ...
         new ViewJFrameImage( newImage, null, new Dimension( 610, 200 ), parentScene.getUserInterface() );
         */
    }
        ...
        if ( fInfo != null && fInfo instanceof FileInfoImageXML ) {
            ...
            while ( e.hasMoreElements() ) {
                ...
                if ( surfaceFile == null || !surfaceFile.exists() ) {
                    ...
                    surfaceFileName = parentScene.getImageA().getUserInterface().getDefaultDirectory() + File.separator
                            + surfaceFile.getName();
                    ...
                    fileInfo.getSurface( surfaceFileName ).setOpacity(
                            fileInfo.getSurface( originalSurfaceFileName ).getOpacity() );
                    fileInfo.getSurface( surfaceFileName ).setDisplay(
                            fileInfo.getSurface( originalSurfaceFileName ).getDisplay() );
    ...
    /**
     * Called from the JPanelSurfaceMAterialProperties.java dialog when the
     * dialog is used to change the material properties of a surface. The
     * surface is determined by the index iIndex. The color button is set to
