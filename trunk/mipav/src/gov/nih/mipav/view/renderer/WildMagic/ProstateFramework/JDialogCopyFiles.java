 ...
 * 
 */
public class JDialogCopyFiles extends JDialogBase
	...
	Hashtable<String, Vector<String>> voiNameHashtableNathan = new Hashtable<String, Vector<String>>(); 
	 ...
	 */
	public JDialogCopyFiles(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		*/
		...
		File fileDir_2 = new File("/scratch/data/Data/Prostate/ForNathan");
		traverse_Layer_ForNathan(fileDir_2); 
		copyFiles();
	}
	
	private void copyFiles() {
	
		Set<String> keys = voiNameHashtable.keySet();
		System.err.println("key size = " + keys.size());
		int count = 0;
		for (String key : keys ) {
		    System.err.println(key);
			
		    Vector<String> voiNames = voiNameHashtable.get(key);
		    Vector<String> savedDir = voiNameHashtableNathan.get(key);
			
		    String annotateName = savedDir.get(0);
		    String voiName = voiNames.get(0);
		    
				int idx = voiName.lastIndexOf(File.separator);
				String voi = voiName.substring(idx+1, voiName.length());
				Path source = Paths.get(voiName);
				
				Path destination = Paths.get(annotateName + File.separator + voi);
					
				try {
						System.err.println("voiName: " + voiName);
						System.err.println("targetName: " + annotateName + File.separator + voi);
	...
	private void traverse_Layer_ForNathan(File dir) {
		...
		if (dir.isDirectory() ) {
		    ...
		    for (int i = 0; i < children.length; i++) {
				...
				voiNameHashtableNathan.put(children[i], new Vector<String>());
				traverse_firstLayer_ForNathan(dir, children[i]);
			}
		}
	}
	
	private void traverse_firstLayer_ForNathan(File firstDir, String child) {
    	File firstLayer = new File(firstDir, child);
    	traverse_secondLayer_ForNathan(firstLayer, child);
    }
	
	private void traverse_secondLayer_ForNathan(File firstLayer, String hashID) {
    	String[] children = firstLayer.list();
    	// System.err.println(firstLayer.toString());
    	for ( int i = 0; i < children.length; i++ ) {
    		// traverse_voiLayer_ForNathan(new File(firstLayer, children[i]), hashID);
    		traverse_thirdLayer_ForNathan(new File(firstLayer, children[i]), hashID);
    		
    	}
    }
	
	 private void traverse_thirdLayer_ForNathan(File secondLayer, String hashID) {
		    
		    System.err.println("2nd layer: " + secondLayer.toString());
	    	// String[] children = secondLayer.list();
			voiNameHashtableNathan.get(hashID).add(secondLayer.toString());
	    }
	
	 private void traverse_voiLayer_ForNathan(File secondLayer, String hashID) {
	    	
	    	int count = 0;
	    	
	    		// System.err.println("voiLayer: " + secondLayer.toString());
	    		String voiString = secondLayer.toString();
	...
	private void traverse_Layer(File dir) {
		// processDir_folder_1(dir);
		if (dir.isDirectory() && !dir.toString().contains("ForNathan") ) {
			String[] children = dir.list();
			// for (int i = 0; i < 1;  i++) {
		    for (int i = 0; i < children.length; i++) {
				// System.err.println(dir + File.separator+ children[i]);
		    	if ( !children[i].contains("ForNathan")) {
    ...
    private void traverse_thirdLayer(File secondLayer, String hashID) {
    	...
    	for ( int i = 0; i < children.length; i++ ) {
    		...
    		if ( children[i].equals("Annotations")) {
    			...
    			// voiNameHashtable.get(hashID).add(secondLayer + File.separator + children[i]);
    			// System.err.println("3rd layer: " + voiNameHashtable.get(hashID));
    		}
