 ...
 *   This control panel is the control LUT of the gray scale images.
 *
 *   @version    1.0
 */
...
*/
    ...
    /**
     * If focus is lost from either threshold field (lower/upper), update the new value
     * on the LUT
     * @param e FocusEvent
     */
        ...
        if ( event.getActionCommand().equals( "grayLUT" ) ) {
            ...
            if ( tabbedPane.getSelectedComponent() == panelA ) {
                if ( getHistoLUTComponentA() != null ) {
                    ...
                    threshLowerF.setText(
                            Float.toString( ( (Point2Df) ( getLUTa().getTransferFunction().getPoint( 1 ) ) ).x ) );
                    threshUpperF.setText(
                            Float.toString( ( (Point2Df) ( getLUTa().getTransferFunction().getPoint( 4 ) ) ).x ) );
                    histoPanelA.updateLUTRecorder();
                }
            } else {
                if ( getHistoLUTComponentB() != null ) {
            ...
            if ( tabbedPane.getSelectedComponent() == panelA ) {
                if ( getHistoLUTComponentA() != null ) {
                    ...
                    threshLowerF.setText(
                            Float.toString( ( (Point2Df) ( getLUTa().getTransferFunction().getPoint( 1 ) ) ).x ) );
                    threshUpperF.setText(
                            Float.toString( ( (Point2Df) ( getLUTa().getTransferFunction().getPoint( 3 ) ) ).x ) );
                    histoPanelA.updateLUTRecorder();
                ...
                if ( getHistoLUTComponentB() != null ) {
                    ...
                    // threshLowerBF.setText(
                    // Float.toString(
                    // ( (Point3Df) ( getLUTb().getTransferFunction().elementAt( 1 ) ) ).x ) );
                    // threshUpperBF.setText(
                    // Float.toString(
                    // ( (Point3Df) ( getLUTb().getTransferFunction().elementAt( 3 ) ) ).x ) );
                    // threshFillBF.setText( Double.toString( imageB.getMin() ) );
                }
            }
        } else if ( event.getActionCommand().equals( "ctPresetsLUT" ) ) {/* if ( tabbedPane.getSelectedComponent() == panelA ) {
             if ( getHistoLUTComponentA() != null ) {
             ctDialogA = new JDialogCT( userInterface.getActiveImageFrame() );
             ctDialogA.setVisible( true );
             }
             } else {
             if ( getHistoLUTComponentB() != null ) {
             ctDialogB = new JDialogCT( userInterface.getActiveImageFrame() );
             ...
             */} else if ( event.getActionCommand().equals( "ChangeNColors" ) ) {/*
             ...
             if ( tabbedPane.getSelectedComponent() == panelA ) {
             nColorsATextF.setText( String.valueOf( dialog.getNColors() ) );
             LUTa.makeLUT( dialog.getNColors() );
             setLUTA( LUTa );
             } else if ( tabbedPane.getSelectedComponent() == panelB ) {
             nColorsBTextF.setText( String.valueOf( dialog.getNColors() ) );
             LUTb.makeLUT( dialog.getNColors() );
             setLUTB( LUTb );
             }
             updateFrames( false );
             }
     ...
     *   are loaded.
     *   @param  boolean loadAll    boolean indicating that both lut and
     *                                transfer functions should be loaded.
     *                                If false, then only transfer functions
     *                                are loaded.
     *   @param  String filename    filename to save LUT as
     *   @param  String dirName     directory to save LUT to
     *   @param  boolean quietMode  if true indicates that warnings
