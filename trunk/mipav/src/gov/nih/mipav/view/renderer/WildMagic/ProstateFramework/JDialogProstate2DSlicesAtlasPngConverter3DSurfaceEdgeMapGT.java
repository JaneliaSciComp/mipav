 ...
 * This class uses ground truth to build the VOIs for performance evaluation.     
 * 
 * ------------------------------------------------------------------
 * Redundant file.  Need to remove after SVN check-in
 * ------------------------------------------------------------------
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstate2DSlicesAtlasPngConverter3DSurfaceEdgeMapGT extends JDialogBase implements AlgorithmInterface {
	 ...
	 */
	public JDialogProstate2DSlicesAtlasPngConverter3DSurfaceEdgeMapGT(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_1 = new File("/scratch/ISBI2017/prostate3D");
		traverse_Layer(fileDir_1);
		
		File fileDir_5_test = new File("/scratch/ISBI2017/resultPNGnii/hed_prostate_mask_iter_16000_axial");
	...
	private void generateContours() {
	    ...
	    for ( String hashID : hashKeys ) {
	    	...
	    	int index = hashID.lastIndexOf("_");
	    	String id = hashID.substring(0, index);
	    	
	    	Hashtable<String, ModelImage> imageHash = origImageTable.get(hashID);
	    	ModelImage srcImage = imageHash.get("axial");
	    	
	    	
	    	String subDir = saveImageDirectory + File.separator + "axial" + File.separator;
	    	File subDirFile = new File(subDir);
			if (!subDirFile.isDirectory())
				subDirFile.mkdir();
			subDir += hashID + File.separator;
			...
			// save image and VOI
			try {
				FileVOI fileVOI = new FileVOI("voi_" + id + ".voi", subDir, srcImage);
				fileVOI.writeVOI(voiNewFinal, true);
				srcImage.saveImage(subDir, "image" + id + ".xml" , FileUtility.XML, false);
			} catch (IOException e ) {
				e.printStackTrace();
		    }
	...
	public void readImages() {
		...
		Set<String> idSet = maskImageTable.keySet();

		for (String hashID : idSet) {
			Vector<String> stringVec = maskImageTable.get(hashID);
			...
			maskTable.put(hashID, imageVec);

		}
		System.err.println("finish I/O ruida");
