...
public class VolumeTriPlanarRenderBase extends GPURenderBase implements
		GLEventListener, KeyListener, MouseMotionListener, MouseListener,
		ChangeListener {
	 ...
	 * back-facing polygons of the proxy-geometry are shown instead of the
	 * volume:
	 */
	...
	protected ColorRGB[] m_akSlicesColor = new ColorRGB[] {
			new ColorRGB(1, 0, 0), new ColorRGB(0, 1, 0), new ColorRGB(1, 1, 0) };
	 ...
	 * 
	 * @param kVolumeImageA
	 *            volume data and textures for ModelImage A.
	 */
	public VolumeTriPlanarRenderBase(VolumeImage kVolumeImageA) {
		...
		m_pkRenderer = new OpenGLRenderer(m_eFormat, m_eDepth, m_eStencil,
				m_eBuffering, m_eMultisampling, m_iWidth, m_iHeight);
	 ...
	 * 
	 * @param kParent
	 *            parent user-interface and frame.
	 * @param kAnimator
	 *            animator used to display the canvas.
	 * @param kVolumeImageA
	 *            volume data and textures for ModelImage A.
	 * @param kVolumeImageB
	...
	public VolumeTriPlanarRenderBase(VolumeTriPlanarRenderBase kShared,
			GLCanvas kCanvas, VolumeImage kVolumeImageA,
			VolumeImage kVolumeImageB) {
		super();
		// m_eBuffering = FrameBuffer.BufferingType.BT_QUAD_STEREO;
		m_pkRenderer = new OpenGLRenderer(m_eFormat, m_eDepth, m_eStencil,
				m_eBuffering, m_eMultisampling, m_iWidth, m_iHeight, kCanvas);
		((OpenGLRenderer) m_pkRenderer).GetCanvas().addGLEventListener(this);
		((OpenGLRenderer) m_pkRenderer).GetCanvas().addKeyListener(this);
		((OpenGLRenderer) m_pkRenderer).GetCanvas().addMouseListener(this);
		((OpenGLRenderer) m_pkRenderer).GetCanvas()
	 ...
	 * 
	 * @param kSurface
	 *            the surface the geodesic component is added to.
	 * @param kNew
	 *            the new geodesic component.
	 * @param iGroup
		...
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetMesh() != null) {
				if (m_kDisplayList.get(i).GetMesh() == kSurface) {
					((VolumeSurface) (m_kDisplayList.get(i))).AddGeodesic(kNew,
							iGroup);
				}
			}
		}
	...
	public void addSurface(SurfaceState kSurface) {
		VolumeSurface kVolumeSurfaces = new VolumeSurface(m_kVolumeImageA,
				m_kVolumeImageB, m_kTranslate, m_fX, m_fY, m_fZ, kSurface);
	...
	public void addTract(VOIContour kContour, Polyline kLine, int iGroup) {
		if (m_kDTIDisplay == null) {
			m_kDTIDisplay = new VolumeDTI(m_kVolumeImageA, m_kTranslate, m_fX,
					m_fY, m_fZ);
	...
	public boolean applySculpt(boolean bAll) {
		...
		int iTSize = (m_kVolumeImageA.GetImage().is4DImage() && bAll) ? m_kVolumeImageA
				.GetImage().getExtents()[3] : 1;
		for (int i = 0; i < iTSize; i++) {
			if (m_kVolumeImageA.GetVolumeTarget().GetImage().GetData() != null) {
				m_kSculptor.setTextureImageDataA(m_kVolumeImageA
						.GetVolumeTarget().GetImage().GetData());
			}

			if (m_kVolumeImageB.GetImage() != null) {
				if (m_kVolumeImageB.GetVolumeTarget().GetImage().GetData() != null) {
					m_kSculptor.setTextureImageDataB(m_kVolumeImageB
							.GetVolumeTarget().GetImage().GetData());
				}
			}
		...
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					...
					((VolumeSurface) (m_kDisplayList.get(i)))
							.SetOpacity(fValue);
				}
			}
		}
	...
	protected void Move() {
		...
		if (m_bTestFrameRate) {
			...
			for (int i = 0; i < m_kDisplayList.size(); i++) {
				m_kDisplayList.get(i).GetScene().Local
						.SetRotateCopy(m_spkScene.Local.GetRotate());
			}
		}
	}
		...
		if (m_bSurfaceUpdate) {
			...
			for (int i = 1; i < m_kDisplayList.size(); i++) {
				if (m_kDisplayList.elementAt(i) instanceof VolumeSurface) {
					((VolumeSurface) m_kDisplayList.elementAt(i))
							.SetPerPixelLighting(m_pkRenderer, true);
				}
			}
			UpdateSceneRotation();
	...
	public void toggleDisplaySurface() {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i) instanceof VolumeSurface) {
				m_kDisplayList.get(i).SetDisplay(
						!m_kDisplayList.get(i).GetDisplay());
			}
		}
	}
	...
	public boolean drawVOI(VOIBase kVOI, VolumeTriPlanarRenderBase kDisplay,
			VolumeImage kVolumeImage, Vector3f kTranslate) {
	 ...
	 */
	public void enableArbitraryClipPlane(boolean bEnable, boolean bDisplay,
			ColorRGB kColor) {
		if (m_kVolumeClip != null) {
	 ...
	 */
	public void enableEyeClipPlane(boolean bEnable, boolean bDisplay,
			ColorRGB kColor) {
		if (m_kVolumeClip != null) {
	 ...
	 */
	public void enableEyeInvClipPlane(boolean bEnable, boolean bDisplay,
			ColorRGB kColor) {
		if (m_kVolumeClip != null) {
		...
		if (!bEnable) {
			setEyeInvClipPlane(m_kVolumeImageA.GetImage().getExtents()[2] - 1,
					bDisplay, bEnable);
		}
	}
	 ...
	 *            painting on/off.
	 * @param bPaint
	 *            when true apply paint.
	 * @param bDropper
	 *            when true do dropper mode.
	 * @param bPaintCan
	 *            when true do paint can mode.
	 * @param bErase
	 *            when true erase.
	 */
	public void enablePaint(ColorRGBA kPaintColor, int iBrushSize,
			boolean bEnabled, boolean bPaint, boolean bDropper,
			boolean bPaintCan, boolean bErase) {
	...
	public void enableSculpt(boolean bSculpt) {
		if (m_kSculptor == null) {
			m_kSculptor = new Sculptor_WM(
					((OpenGLRenderer) m_pkRenderer).GetCanvas());
			m_kSculptor.setImage(m_kVolumeImageA.GetImage(),
					m_kVolumeImageB.GetImage());
		}
	 ...
	 * GLCanvas is first displayed. Called again if the GLCanvas is removed from
	 * the frame and added to another window or panel. Initializes the display.
	 * 
	 * @param arg0
	 *            GLCanvas
	 */
	...
	public void keyPressed(KeyEvent e) {
		...
		case 'c':
			for (int i = 0; i < m_kDisplayList.size(); i++) {
				if (m_kDisplayList.get(i) instanceof VolumeSurface) {
					VolumeSurface kSurface = (VolumeSurface) m_kDisplayList
							.get(i);
	...
	public void removeGeodesic(TriMesh kSurface, int iNode, int iGroup) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetMesh() != null) {
				if (m_kDisplayList.get(i).GetMesh() == kSurface) {
					((VolumeSurface) (m_kDisplayList.get(i))).RemoveGeodesic(
							iNode, iGroup);
				}
			}
		}
	 ...
	 * added. This function allows the Geodesic object to replace the original
	 * mesh with the sliced mesh in the surface renderer. ReplaceMesh is also
	 * used to undo cutting operations.
	 * 
	 * @param kOld
	 *            TriMesh old surface mesh
	 * @param kNew
	...
	@Override
	public void reshape(GLAutoDrawable arg0, int iX, int iY, int iWidth,
			int iHeight) {
		if (m_iWidth != iWidth || m_iHeight != iHeight) {
			m_bDisplay = false;
			super.reshape(arg0, iX, iY, iWidth, iHeight);
			if (isNavigationEnabled) {
				// During navigation mode fly-thru, have to update the camera
				// near-plane
	...
	public void saveCroppedImage() {
		...
		OpenCLAlgorithmVolumeCrop oclCrop = new OpenCLAlgorithmVolumeCrop(
				kImage);
		oclCrop.setClip(clip, clipI, doClip);
		Matrix3f rot = GetSceneRotation();
		Matrix4f rot4 = new Matrix4f(rot.M00, rot.M01, rot.M02, 0, rot.M10,
				rot.M11, rot.M12, 0, rot.M20, rot.M21, rot.M22, 0, 0, 0, 0, 1);
		float[] rotMatrix = new float[16];
		rot4.getData(rotMatrix);
		oclCrop.setClipEyeArb(clipE.getClipEye(), clipE.getClipEyeInv(),
				clipE.getClipArb(), rotMatrix);
		oclCrop.run();
		...
		result.calcMinMax();
		int fileType = m_kVolumeImageA.GetImage().getFileInfo(0)
				.getFileFormat();
		String suffix = FileTypeTable.getFileTypeInfo(fileType)
				.getDefaultExtension();
		ModelImage.saveImage(result, result.getImageName() + suffix,
				result.getImageDirectory());

		// System.err.println( result.getImageName() + suffix + "   " +
		// result.getImageDirectory() );
	...
	public void setBoundingBoxColor(int i, ColorRGB kColor) {
		...
		} else if (m_akSlicesColor != null && i < m_akSlicesColor.length
				&& m_akSlicesColor[i] != null) {
	 ...
	 *            the clip plane to set.
	 * @param fValue
	 *            the new position of the clip plane (the same value as the
	...
	public void SetCustomBlend(int iBlendEquation, int iLogicOp, int iSrcBlend,
		...
		m_kVolumeRayCast.SetCustomBlend(iBlendEquation, iLogicOp, iSrcBlend,
				iDstBlend, kColor);
	}
	 ...
	 *            .
	 */
	public void setDTIImage(ModelImage kDTIImage, ModelImage kEigenVectorImage,
		...
		m_kDTIDisplay.setDTIImage(kDTIImage, kEigenVectorImage,
				kEigenValueImage, m_pkRenderer);
		m_bSurfaceUpdate = true;
	}
	 ...
	 * 
	 * @param f4
	 *            clip position (same value as sSlice in JPanelClip)
	 * @param bDisplay
	 *            on/off.
	 */
	 ...
	 * 
	 * @param f4
	 *            clip position (same value as sSliceInv in JPanelClip)
	 * @param bDisplay
	 *            on/off.
	 */
	...
	public void setMaterial(String kSurfaceName, MaterialState kMaterial,
		...
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) (m_kDisplayList.get(i))).SetMaterial(
							kMaterial, bUpdate);
					m_kVolumeImageA.setSurfaceMaskColor(kSurfaceName,
							kMaterial.Diffuse);
					m_bSurfaceMaskUpdate = true;
				}
			}
	 ...
	 *            FILL, LINE, or POINT.
	 */
	public void setPolygonMode(String kSurfaceName,
			WireframeState.FillMode eMode) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
	 ...
	 * 
	 * @param iGroup
	 *            the fiber bundle group to set.
	 * @param kColor
	 *            the new polyline color for the specified fiber bundle tract
	 *            group.
	 */
	...
	/**
	 * Turns on surface texture display for the given surface. The user can use
	 * a separate ModelImage and LUT than the one displayed in the volume
	 * renderer.
	 * 
	 * @param kSurfaceName
	 *            the name of the surface to texture.
	 * @param bOn
	 *            texture on/off.
	 * @param bUseNewImage
	 *            when false use the current ModelImage, when true the user
	 *            specifies the model image.
	 * @param bUseNewLUT
	 *            when false use the current LUT, when true the user specifies
	 *            the LUT.
	 */
	public void setSurfaceTexture(String kSurfaceName, boolean bOn,
			boolean bUseNewImage, boolean bUseNewLUT) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) m_kDisplayList.get(i)).SetSurfaceTexture(
							bOn, bUseNewImage, bUseNewLUT);
				}
			}
		}
	}
	 ...
	 * 
	 * @param i
	 *            the plane index (0-3) in file coordinates.
	 * @param bShow
	 ...
	 * 
	 * @param kSurfaceName
	 *            the name of the surface to smooth.
	 * @param iteration
	 *            smooth iterations.
	 * @param alpha
	 *            smooth factor.
	 * @param volumeLimit
	 *            whether to use a volume % change limit.
	 * @param volumePercent
	 *            the % volume change limiting factor
	 */
	public void smoothMesh(String kSurfaceName, int iteration, float alpha,
			boolean volumeLimit, float volumePercent) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) (m_kDisplayList.get(i))).smoothMesh(
							iteration, alpha, volumeLimit, volumePercent);
				}
			}
		}
	}
	 ...
	 *            smooth factor.
	 */
	public void smoothThree(String kSurfaceName, int iteration, float lambda,
			float mu) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) (m_kDisplayList.get(i))).smoothThree(
							iteration, lambda, mu);
				}
			}
		}
	 ...
	 *            the % volume change limiting factor.
	 */
	public void smoothTwo(String kSurfaceName, int iteration, float fStiffness,
			boolean volumeLimit, float volumePercent) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) (m_kDisplayList.get(i))).smoothTwo(
							iteration, fStiffness, volumeLimit, volumePercent);
				}
			}
		}
	}
	...
	public void toggleGeodesicPathDisplay(String kSurfaceName, int iWhich) {
		for (int i = 0; i < m_kDisplayList.size(); i++) {
			if (m_kDisplayList.get(i).GetName() != null) {
				if (m_kDisplayList.get(i).GetName().equals(kSurfaceName)) {
					((VolumeSurface) (m_kDisplayList.get(i)))
							.ToggleGeodesicPathDisplay(iWhich);
				}
			}
		}
	}
	 ...
	 * semi-transparent surfaces. Creates the screen-space polygon and
	 * OrderIndTransparencyEffect that display the final result. The first
	 * render target is for the solid objects. Second render target is for
	 * translucent objects. Third render target is for accumulating the count of
	 * translucent objects per-pixel.
	 * 
	 * @param arg0
	 * @param iWidth
	 * @param iHeight
	...
	protected void CreateRenderTarget(GLAutoDrawable kDrawable, int iWidth,
		...
		for (int i = 0; i < m_akSceneTarget.length; i++) {
			...
			GraphicsImage pkSceneImage = new GraphicsImage(
					GraphicsImage.FormatMode.IT_RGBA32, iWidth, iHeight,
					(float[]) null, "ColorTex" + i);
	...
	protected void CreateScene() {
		...
		if (m_kSlices == null) {
			m_kSlices = new VolumeSlices(m_pkRenderer, m_kVolumeImageA,
					m_kVolumeImageB, m_kTranslate, m_fX, m_fY, m_fZ);
		...
		m_kVolumeClip = new VolumeClip(m_kVolumeImageA, m_kTranslate, m_fX,
				m_fY, m_fZ);
		m_kDisplayList.add(m_kDisplayList.size(), m_kVolumeClip);

		m_kVolumeBox = new VolumeBoundingBox(m_kVolumeImageA, m_kTranslate,
				m_fX, m_fY, m_fZ);
		m_kDisplayList.add(m_kDisplayList.size(), m_kVolumeBox);

		m_kVolumeCube = new VolumeOrientationCube(m_kVolumeImageA,
				m_kTranslate, m_fX, m_fY, m_fZ);
	...
	public static int getHistogramLUTTextureIndex(String kCommand) {
		...
		if (kCommand.equals(CustomUIBuilder.PARAM_LUT_GRAY_BLUE_RED
				.getActionCommand())) {
			return 4;
		}
		if (kCommand.equals(CustomUIBuilder.PARAM_LUT_HOTMETAL
				.getActionCommand())) {
			return 5;
		}
		if (kCommand.equals(CustomUIBuilder.PARAM_LUT_SPECTRUM
				.getActionCommand())) {
			return 6;
		}
		if (kCommand.equals(CustomUIBuilder.PARAM_LUT_COOL_HOT
				.getActionCommand())) {
			return 7;
		}
		...
		if (kCommand.equals(CustomUIBuilder.PARAM_LUT_STRIPED
				.getActionCommand())) {
			return 10;
		}
		// ---------------------- customized LUTs
	...
	private void initLUT(Texture[] m_akLUTTexture, ModelLUT kLUT, int index) {
		...
		GraphicsImage kColorMap = new GraphicsImage(
				GraphicsImage.FormatMode.IT_RGBA8888, 256, aucData, new String(
						"ColorMap" + index));
	...
	private void loadShared() {
		...
		kLUT.makeRedTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);

		kLUT.makeGreenTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeBlueTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeGrayBRTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeHotMetalTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeSpectrumTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeCoolHotTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeSkinTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeBoneTransferFunctions();
		kLUT.makeLUT(256);
		initLUT(ms_akLUTTexture, kLUT, index++);
		kLUT.makeStripedLUT();
		initLUT(ms_akLUTTexture, kLUT, index++);
	...
	protected void Render(GLAutoDrawable kDraw) {
		...
		if (m_iStereo == 0) {
			...
			if (!m_bDisplaySecond) {
				 ...
				 * m_pkRenderer.SetBackgroundColor( m_kBackgroundColor );
				 * m_pkRenderer.ClearBuffers(); m_pkRenderer.Draw( m_pkPlane );
	 ...
	 * following steps: 1. A FrameBuffer Object (FBO) is created with three
	 * render-to-texture targets, and a depth buffer. 2. The first render target
	 * is activated. Solid objects are rendered: writing color and depth
	 * information to the FBO target1. The color information displays the solid
	 * objects, the depth information enables the solid objects to properly
	 * occlude the translucent objects. 3. The second and third render targets
	 * are activated. Translucent objects are rendered: writing accumulated
	 * color and alpha to the second target and the number of overlapping
	 * surfaces per pixel to the third target. In this pass the depth buffer is
	 * read-only. 4. Rendering to the three texture targets is disabled. 5. A
	 * screen-space polygon is rendered, it reads the three textures generated
	 * in the previous pass. The first texture contains the color from the solid
	 * objects, the next two textures contain the color information and
	 * weighted-average for rendering the translucent textures.
	 * 
	 * @param bPreRender
	...
	protected void updateCenterOnDisplay() {
		...
		if (m_kSlices != null) {
			int[] aiExtents = m_kVolumeImageA.GetImage().getExtents();
			m_kSlices.SetCenter(new Vector3f(
					(m_kNewCenter.X / (aiExtents[0] - 1)),
	...
	/**
	 * Picking. If a display list object has picking enabled, find the picked
	 * polygon based on the mouse position.
	 */
	protected void Pick() {
			...
			if (m_spkCamera.GetPickRay(m_iXPick, m_iYPick, GetWidth(),
				...
				for (int i = 0; i < m_kDisplayList.size(); i++) {
					if (m_kDisplayList.get(i).GetPickable()) {
						m_kPicker.Execute(m_kDisplayList.get(i).GetScene(),
								kPos, kDir, 0.0f, Float.MAX_VALUE);
						if (m_kPicker.Records.size() > 0) {
							// System.err.println( kPos.X() + " " + kPos.Y() +
							// " " + kPos.Z() );
							// System.err.println( kDir.X() + " " + kDir.Y() +
							// " " + kDir.Z() );
							if (m_bPaintEnabled) {
								// System.err.println("Picked " +
								// m_kDisplayList.get(i).getClass().getName());
								if (m_bPaint) {
									m_kDisplayList.get(i).Paint(m_pkRenderer,
											m_kPicker.GetClosestNonnegative(),
											m_kPaintColor, m_iBrushSize);
								} else if (m_bDropper || m_bPaintCan) {
									ColorRGBA kDropperColor = new ColorRGBA();
									Vector3f kPickPoint = new Vector3f();
									m_kDisplayList.get(i).Dropper(
											m_kPicker.GetClosestNonnegative(),
											kDropperColor, kPickPoint);
									// m_kParent.setDropperColor( kDropperColor,
									// kPickPoint );
								} else if (m_bErase) {
									m_kDisplayList.get(i).Erase(m_pkRenderer,
											m_kPicker.GetClosestNonnegative(),
											m_iBrushSize);
								}
							}
							if (m_bGeodesicEnabled) {
								// m_kParent.setGeodesic(
								// m_kDisplayList.get(i).GetMesh(),
								// m_kPicker.GetClosestNonnegative() );
							}
							if (m_bPickCorrespondence) {
								PickRecord kRecord = m_kPicker
										.GetClosestNonnegative();
								// m_kParent.PickCorrespondence( kRecord.iV0,
								// kRecord.iV1, kRecord.iV2 );
							}
						}
					}
				}
	...
	void NormalKernel(float[] a, float[] b, int width, int height, int depth,
		...
		for (int iGID = 0; iGID < a.length; iGID++) {
			...
			float fDX = (float) (0.71f * (values[0] - values[1])
					+ (values[2] - values[3]) + .71 * (values[4] - values[5]));
			float fDY = (float) (0.71f * (values[0] - values[4])
					+ (values[6] - values[7]) + .71 * (values[1] - values[5]));
			float fDZ = (float) (0.71f * (values[8] - values[9])
					+ (values[10] - values[11]) + .71 * (values[12] - values[13]));
	...
	/**
	 * Creates the containing JPanel that will display this object. The JPanel
	 * contains a scroll pane with both horizontal and vertical scroll bars and
	 * a horizontal slider for changing where the slice plane intersects the
	 * volume in depth.
	 * 
	...
	public void stateChanged(ChangeEvent arg0) {
		if (arg0.getSource() == opacitySlider) {
			...
			opacityLabel.setText(new Float(opacitySlider.getValue() / 100f)
					.toString());
		}
	}
	...
	public void toggleNavigation(boolean _isNavigationEnabled) {
		...
		if ( isNavigationEnabled ) {
			((OpenGLRenderer) m_pkRenderer).GetCanvas().removeKeyListener(this);
		} else {
			((OpenGLRenderer) m_pkRenderer).GetCanvas().addKeyListener(this);
		}
