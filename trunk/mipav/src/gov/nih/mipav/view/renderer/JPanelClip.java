 ...
 * <p>Description: Clip panel control of the surface renderer.   The clipping control
 * includes x, y, z clipping sliders; the arbitrary and static eye clipping sliders.</p>
 * @author Ruida Cheng
 */
public class JPanelClip extends JPanelRendererBase
    ...
    /** Rotation axis to sync up the action for both ModelClip arbitrary clipping plane
     *  and the clipSliceA frame box transformGroup. */
    ...
    public void buildPanelA() {
        ...
        cpGBC.fill = GridBagConstraints.BOTH;
        labelA = new JLabel(
                " A (" + String.valueOf( -xDim ) + " - " + String.valueOf( (int) ( xDim * radicalRatio ) ) + ")" );
    ...
    public void actionPerformed( ActionEvent event ) {
        ...
        if ( source == extractButtonA || source == extractButtonS ) {
            ...
            int length = (int) Math.round(
                    Math.sqrt(
                            ( pts[2].x - pts[0].x ) * ( pts[2].x - pts[0].x )
    ...
    /**
     * Swap ModelClip Branch group between the 6 clipping plane and the
     * arbitray clipping plane.
     * @param flag   <code>true</code> enable the 6 clipping planes,
     *               <code>false</code> enable the arbitrary clipping plane.
     */
     ...
     * @return is6PlaneClipping  <code>true</code> 6 clipping plane active,
     *                           <code>false</code> arbitrary clippiing plane active.
     */
    ...
    /**
     *	Sets the arbitray clip slider and the labels.
    ...
    /**
     *	Sets the static slider and the labels beside and beneath it to the
     *	state given by <code>flag</code>.
     *	@param flag if <code>true</code> enable, otherwise disable.
     */
    ...
    /**
     *	Sets the static inverse slider and the labels beside and beneath it to the
     *	state given by <code>flag</code>.
     *	@param flag if <code>true</code> enable, otherwise disable.
     */
    ...
    public void mouseReleased( MouseEvent event ) {
        ...
        /*
         if ( arbiMouseRotateBehavior != null ) {
         enableClipArbiBehavior(false);
         }
         */
    }
    ...
    /**
     * Called by the JDialogSurface to find the current mouse event
     * is from the arbitrary frame box moving or not.
     * @param pickedObject   Shape3D picked object from the JDialogSurface.
    ...
    /**
     * During mouse recorder diaplay, setup the rotatino axis for
     * both arbiTG and mcArbiTG.
     ...
     * @param flag   <code>true</code> enable arbitrary clipping mouse behavior, disable
     *               the scene graph mouse behavior;
     *               <code>false</code> disable arbitrary clipping mouse behavior, enable
     *               the scene graph mouse behavior.
     */
    ...
    /**
     *   Build cliping planes tree structure.  This method being called
     *   when clipping plane menu selected in the surface render.
     */
    public void buildClipPlanesTree() {
        ...
        clipSliceStatic = new ViewJComponentBoxSlice( xBox * 0.5f, yBox * 0.5f, zBox, ViewJComponentBoxSlice.S_CLIPSLICE );
    ...
    public void updateClipSliceA() {
        if ( getAVisible() ) {
            ...
            clipSliceAIndicator.setSlices( -maxBox + 2 * ( (float) aSlice / ( maxDim - 1 ) ) * maxBox,
                    maxBox * radicalRatio, maxBox * radicalRatio, ViewJComponentBoxSlice.A_CLIPSLICE );
            Shape3D shape = new Shape3D( clipSliceA, null );
    ...
    public void cropVolume() {
        ...
        xB = xBox; yB = yBox; zB = zBox; maxB = xBox;
    ...
    // xBox range between 0 - 1.
        ...
        x = eqnX.w; xInv = eqnXInv.w;
        y = eqnY.w; yInv = eqnYInv.w;
        z = eqnZ.w; zInv = eqnZInv.w;
    ...
    public void updateClipSliceStatic() {
        if ( getStaticVisible() ) {
            ...
            clipSliceStatic.setSlices( xBox * 0.5f, yBox * 0.5f,
                    zBox - 2 * ( (float) sSlice / ( zDim - 1 ) ) * zBox + 0.001f, ViewJComponentBoxSlice.S_CLIPSLICE );
            Shape3D shape = new Shape3D( clipSliceStatic, null );
