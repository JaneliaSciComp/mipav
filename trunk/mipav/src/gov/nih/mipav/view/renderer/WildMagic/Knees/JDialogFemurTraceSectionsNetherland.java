...
public class JDialogFemurTraceSectionsNetherland extends JDialogBase implements AlgorithmInterface {
	...
	private GrowingPlatePattern growingPlate = new GrowingPlatePattern();
	...
	private Vector2f traceSection2_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
		...
		double class2IntensityGre = class2ImageGre.getDouble((int) posX, (int) posY);
		double class3IntensityGre = class3ImageGre.getDouble((int) posX, (int) posY);
	...
	private boolean findGrowingPlate(int x, int y, ModelImage imageSlice, int range, double intensity, int type) {
		...
		int findEdge = growingPlate.searchEdge(map, startX, startY, xDim, yDim, range);
		if (findEdge == 1) {
			return true;
		} else {
			return false;
