...
/**
 * Basic displayable image object in MIPAV.  Contains the viewable objects
 * such as VOIs and carries listeners to most any action that can happen in
 * a window.
 *
 * @version    0.1 Nov 18, 1997
 * @author     Matthew J. McAuliffe, Ph.D.
 */
public class ViewJComponentEditImage
    extends ViewJComponentBase implements MouseMotionListener,
    MouseWheelListener,
    MouseListener,
    KeyListener,
    PaintGrowListener {
     ...
     *   Images which are not active will not have the recangle drawn.
     */
    ...
    /* * In painting COMPLEX images the COMPLEX image is assumed to have been a result of
     transforming real data.  x(n1,n2) real <-> X(k1,k2) = X*(-k1,-k2) for 2D and
     x(n1,n2,n3) real <-> X(k1,k2,k3) = X*(-k1,-k2,-k3).  Therefore, when one pixel is
     painted in a COMPLEX image, the above defined complementary pixel is also painted.
     The zero point for k1, k2, k3 occur such that there is one more pixel below the
     zero point than there is above it.  For example, if the FFT is 2048 by 2048 by 2048,
     the center occurs at (1024,1024,1024) so that (0,0,0) is further away than
     (2047,2047,2047) */
    ...
    /** Rubberband tool for VOIs.            */
    protected Rubberband rbRect, rbPolyline, rbEllipse, rbLine, rbRectQuick,
        rbProtractor, rbLivewire;
    ...
    /**
     *   Buffer used to store ARGB images of the image
    ...
    /**
     * The previous paint brush we were using when the user temporarily changes
     * the paint brush size using keyboard shortcuts (by pressing 1, 2, 3, 4).
     */
    ...
    // regionGrow parameters
    // fuzzyConnectedness is not used if fuzzyThreshold is less than 0.
    // When fuzzyConnectedness is used, fuzzyThreshold ranges from 0 to 1.
    ...
    /**
     *	VOI clipboard used to undo the VOI. This VOI is a copy of
     *   last VOI selected.
     */
    ...
    // in setPaintBuffers and setPaintBuffers zero out buffer
    ...
    // last slice the image was at when win region was ON. its necessary because otherwise,
    // a new cleanImageB would have to be created upon every repaint. should be initialized to
    // a negative number
     ...
     *   @param _hasOrientation true if the image is known to be in axial orientation and
     *                            is displayed in ViewJFrameTriImage
     *   @param _orient a 3 integer array containing the orientation of each axis
     */
    public ViewJComponentEditImage(ViewJFrameBase _frame,
        ...
        if (crosshairName.equalsIgnoreCase("default")) {
            ...
            try {
                ...
                this.setCrosshairCursor(
                    toolkit.createCustomCursor(MipavUtil.getIcon(crosshairName).getImage(), new Point(15, 15),
                                               crosshairName));
            }
                ...
                this.setCrosshairCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
                Preferences.debug(
                    "ViewJComponentEditImage: Crosshair icon \"" + crosshairName + "\" cannot be found.  "
        ...
        try { // this try does not work... sun didn't bother to propogate the exception ... maybe someday ...
            ...
            magRegionCursor = Toolkit.getDefaultToolkit().createCustomCursor(cursorImage, new Point(12, 12),
                "Magnification");
            blankCursor = Toolkit.getDefaultToolkit().createCustomCursor(cursorImage, new Point(12, 12), "Blank Cursor");
        }
        ...
        // small pointer cursor : TRY to get the image cursor.  if you can't (and coders may have EXTRA probs with this!)
        // then notify the user of the prob and get a default.  This try must be seperate for all diff cursors
        try { // this try does not work... sun didn't bother to propogate the exception ... maybe someday ...
    ...
    /** Sets this component to paint the "on top" high-light.  It is not
     *   required to use the coloured rectangle to high-light the on-top
     *   component, but the component is not aware of its position in the
     *   user-interface, and thus, that the user-interface controls point
     *   to it.
     *   @param hilite   <code>true</code> will set this component to
     *                   paint the 'on-top' high-light the next time it is
     *                   redrawn.
     *                   <code>false</code>, of course, will not let the
     *                   component paint the coloured rectangle when
     *                   repainted.
     */
    ...
    /**
     * Sets the image to display in "Checkerboard" mode with
     * the given numbers of rows and columns
     ...
     *  @param rowCheckers     number of checkers in a row
     *  @param columnCheckers  number of checkers in a column
     */
    ...
    /**
     *   The frame in which the image(s) is displayed, allocates the
     *   memory and uses this method to pass the references to the buffers.
     *   @param imgBufferA   storage buffer used to display image A
     *   @param imgBufferB   storage buffer used to display image B
     ...
     *  assumes dimensionality same as image B's  for now.
     *  @param image    imageA
     */
     ...
     * !!!!!! assumes dimensionality same as image A's  for now
     * will fix soon.
     *  @param image     imageB
     */
     ...
     *   @return           boolean to indicate if the show was successful
     */
    public boolean setPaintBuffers(int tSlice, int zSlice,
                                   ModelLUT _LUTa, ModelLUT _LUTb,
                                   boolean forceShow) {
        float imageMinA = 0;
        int xDim, yDim, zDim;
        ...
        if ( (orientation == AXIAL) || (orientation == NA)) {
            ...
            if (slice != zSlice || timeSlice != tSlice || forceShow == true) {
                ...
                try {
                    ...
                    Preferences.debug(
                        "IOException in ViewJComponentEditImage:setPaintBuffers (else if orientation == XZ)"); // Need to fix this
                    return false;
                }
            } // end of if ( slice != zSlice || timeSlice != tSlice || forceShow == true)
            if (imageB == null) {
    ...
    /**
     * The purpose of this method is to examine both LUTs to determine if they are zero-based, that is,
     * if they map values of zero to the color R=0, G=0, B=0. If the LUT does, in fact, do this, then
     * this method ensures that those pixels are completely transparent so that no blending takes place
     * for those pixels.
     */
    ...
    /**
     * Builds the overlay Strings from the tag's value.  Concatenates the
     * output strings from the tags and ensures that any properly read-in string
     * has usable (if empty) values.
     *
     * @param inf The FileInfo with DICOM tags to display.
     * @param dicomKey Key of tag to display.
     * @returns null when value is not a String or when the tag does not exist.
     */
    private final String buildOverlayStrings(FileInfoDicom inf, String name, String dicomKey) {
        try {
            ...
            Preferences.debug(
                "Creating strings for DICOM overlay for " + dicomKey + " but encountered a ClassCastException.\n", 4);
            return null;
        }
        catch (NullPointerException noTag) {
            Preferences.debug(
                "Creating strings for DICOM overlay for " + dicomKey + " but encountered a NullPointerException.\n",
                4);
            return null;
        }
    }
     ...
     * @return String
     */
    private final String buildOverlayStrings(FileInfoBase inf, String name, String attribString) {
    ...
    /**
     * Draws the gradicules (the half-image lines which demarcate lengths in
     * the image).
    ...
    protected void drawGradicules(Graphics g, float xRes, float yRes) {
        ...
        if (getZoomX() >= 1.0 && getZoomY() >= 1.0) {
            ...
            for (int i = 1; i < XGradcmLen; i++) {
                g.setColor(Color.white);
                g.drawLine(XGradpixLen + (int) (i / XCMPerPix), bottomOffset - 30,
                           XGradpixLen + (int) (i / XCMPerPix), bottomOffset - 35);
                g.setColor(Color.black);
                    ...
                    if (VOIs.VOIAt(i).getLevel() == VOIs.VOIAt(i).NOT_A_LEVELSET) {
                        ...
                        if (gon == null) {
                            MipavUtil.displayError(
                                "Active propogation returned no viable contour.\nUse regular propogation.");
                            return false;
                        }
     ...
     *   @return true if active VOI is found
     *   --- Must add to JDialog base - should return null if nothing selected else return VOI selected!!!!
     */
    ...
    /**
     *  Places the active VOI and places it in clipboard.
     *  <br>Note: this method does not actually "cut" voi, but rather
     *   just "copies it" (the voi is not deleted with this method)
     ...
     * Places the active VOI and places it in clipboard.
     * <br>
     * Note: this method does not actually "cut" voi, but rather just "copies it" (the voi is not deleted with this method)
     */
    ...
    public void pasteVOI() {
        ...
        for (int k = 0; k < numClipped; k++) {
            ...
            if (imageXExt <= (x[1] - x[0]) || imageYExt <= (y[1] - y[0]) || imageXExt <= x[0] || imageXExt <= x[1]
                || imageYExt <= y[0] || imageYExt <= y[1]) { // checks to make sure if voi can fit
    ...
    /** deactivates, or deselects, all active VOIs.  will not deselect
     *   the VOI which is set at Last Point.
     */
    ...
    /**
     *   Generates and displays a 1D graph of the image intensities
     *   underlying the contour of the VOI
     */
                        ...
                        if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR
                            ...
                            for (j = 0; j < VOIs.VOIAt(i).getCurves()[slice].size(); j++) {
                                if ( ( (VOIContour) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).isActive()) {
                                    length = (int) MipavMath.round(
                                        ( (VOIContour) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).getLengthPtToPt(
                                            getActiveImage().getFileInfo(0).getResolutions()));

                                    if (getActiveImage().getFileInfo(0).getResolutions()[0] > 0) {
                                        length = (int) MipavMath.round(
                                            length / getActiveImage().getFileInfo(0).getResolutions()[0]);
                                    }
                                    else {
                                ...
                                if ( ( (VOIContour) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).isActive()) {
                                    ...
                                    pt = ( (VOIContour) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).
                                        findPositionAndIntensity(
                                            position, intensity, getActiveImageBuffer(),
                                            imageActive.getFileInfo()[slice].getResolutions(),
                                            getActiveImage().getExtents()[0], getActiveImage().getExtents()[1]);
                                    ...
                                    contourGraph.setUnitsInLabel(
                                        FileInfoBase.getUnitsOfMeasureAbbrevStr(
                                            imageActive.getFileInfo(0).getUnitsOfMeasure(0)));
                                    contourGraph.setDefaultDirectory(
                                        getActiveImage().getUserInterface().getDefaultDirectory());
                                    contourGraph.setVisible(true);
                                    return;
                                }
                            }
    ...
    /**
     *  Displays the dialog for calculating point area average intensities
     *  (for a fixed Point location (x,y)) , and pops up
     *  the associated graph when "OK" is pressed
     */
                        ...
                        for (int j = 0; j < VOIs.VOIAt(i).getCurves()[slice].size(); j++) {
                            if ( ( (VOIPoint) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).isActive()) {
                                graphPointVOI(VOIs.VOIAt(i),
                                              (VOIPoint) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j)), j);
                            }
                        }
                    }
                ...
                for (s = 0; s < imageActive.getExtents()[2]; s++) {
                    for (int c = 0; c < 3; c++) {
                        ...
                        ptRGBIntensities[c][s] = imageActive.getFloat(
                            (4 * (s * imageActive.getSliceSize() + (int) pt.y * imageActive.getExtents()[0] +
                                  (int) pt.x) + c + 1));
                    }
                }
                ...
                try {
                    ...
                    for (s = 0; s < imageActive.getExtents()[2]; s++) {
                        ...
                        ptIntensity[s] = imageActive.getFloat(
                            (int) (s * imageActive.getSliceSize() + pt.y * imageActive.getExtents()[0] + pt.x));
                    }
                    if (v.getContourGraph() == null) {
            ...
            try {
                ...
                for (t = 0; t < imageActive.getExtents()[3]; t++) {
                    ...
                    ptIntensity[t] = imageActive.getFloat(
                        (int) (t * xDim * yDim * zDim + pt.z * xDim * yDim + pt.y * xDim + pt.x));
                }
                if (v.getContourGraph() == null) {
                    ...
                    if (imageActive.getNDims() == 3) {
                        if (imageActive.isColorImage() == true) {
                            ...
                            for (int s = 0; s < imageActive.getExtents()[2]; s++) {
                                ...
                                ptPosition[s] = s;
                                ptIntensity[s] = imageActive.getFloat(
                                    (int) (s * imageActive.getSliceSize() + pt.y * imageActive.getExtents()[0]
                                           + pt.x));
                            }
    ...
    /**
     *   Generates and displays a 1D graph of the average or
     *   total intensity of 2.5 VOI of 2.5D image (3D)
     *   @param totalIntensity     if true calculates total sum of the intensity else calculates
                        ...
                        if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible() == true) {
                            ...
                            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                                            ...
                                            for (j = 0, intensitySum = 0; j < VOIs.VOIAt(i).getCurves()[s].size(); j++) {
                                                ...
                                                if (useThreshold) {
                                                    intensitySum +=
                                                        ...
                                                        graphImgBuff, imageActive.getExtents()[0], c, threshold);
                                                }
                                                else {
                                                    intensitySum +=
                                                        ( (VOIContour) (VOIs.VOIAt(i).getCurves()[s].elementAt(j))).
                                ...
                                ViewJFrameGraph contourGraph = new ViewJFrameGraph(rgbPositions, rgbIntensities,
                                    "Intensity Graph", v,
                                    FileInfoBase.getUnitsOfMeasureAbbrevStr(
                                        imageActive.getFileInfo(0).getUnitsOfMeasure(2)));
                        ...
                        if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible() == true) {
                            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                                ...
                                for (s = 0; s < imageActive.getExtents()[2]; s++) {
                                    try {
                                        numPixels = 0;
                                        imageActive.exportData(s * length, length, graphImgBuff); // locks and releases lock
                                        for (j = 0, intensitySum = 0; j < VOIs.VOIAt(i).getCurves()[s].size(); j++) {
                                            ...
                                            if (useThreshold) {
                                                intensitySum +=
                                                    ...
                                                    graphImgBuff, imageActive.getExtents()[0], threshold);
    ...
    /**
     *   If a VOI is selected the VOI properties dialog is displayed
     *   with the current properites of the selected VOI.
     */
     ...
     *   VOI Update listeners, this method is no longer
     *   supported.
    ...
    /**
     *  A mouse event.  When the mouse is clicked in the image,
     *  several different things may happen.  If a Volume of
     *  Interest (VOI) is selected and the click count is 2, a
     *  VOI dialog should pop up.  If the click count is 1 and the
     *  mouse is in an VOI, it should select the VOI.  In all other
     *  cases, a click within the image but not in an VOI should
     *  deselect all VOIs.
    ...
    /**
     * Removes (if there is an active Polyline or polygon) the active
     * point (a single point) from the contour
     */
     ...
     *
     * Arrow key = move active VOI
     * CTRL + left/right = cycle through VOI's contours
     * CTRL + up/down = cycle through VOIs
     * Shift + arrow key = move active contour's active point
     * CTRL + Shift + arrow key = cycle through active contour's active points
     * @param e KeyEvent
     */
    ...
    /**
     *  A mouse event.  Sets the mode of the program
     *  depending on the cursor mode.  If the mode is move,
     *  activates the contour or line and enables the
     *  delete button.
     *  @param mouseEvent   event that triggered function
     */
            ...
            if ((mouseEvent.getModifiers() & MouseEvent.BUTTON3_MASK) != 0) {
                ...
                for (i = 0; i < nVOI; i++) {
                    if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible() == true) {
                        if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                            for (j = 0; j < VOIs.VOIAt(i).getCurves()[slice].size(); j++) {
                                if ( ( (VOIContour) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).contains(xS,
                                    yS, true)) {
                                    return;
                                }
                            }
                        ...
                        if (VOIs.VOIAt(i).getCurveType() == VOI.LINE) {
                            ...
                            for (j = 0; j < VOIs.VOIAt(i).getCurves()[slice].size(); j++) {
                                if ( ( (VOILine) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).nearLine(xS,
                                    yS)) {

                                    //if CTRL was held down, then print out the Line's information to the
                                    // DATA window
                                    if ( (mouseEvent.getModifiers() & (MouseEvent.CTRL_DOWN_MASK | MouseEvent.CTRL_MASK)) !=
                                        0) {
                                        Point3Df startPt = null;
                                        Point3Df endPt = null;

                                        startPt = (Point3Df)( (VOILine) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).elementAt(0);
                                        endPt = (Point3Df)( (VOILine) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).elementAt(1);

                                        double angle = ( (VOILine) (VOIs.VOIAt(i).getCurves()[slice].elementAt(j))).getAngle();
                                        ...
                                        if (imageActive.getFileInfo(0).getOrigin()[0] != 0 ||
                                            ...
                                            String[] startValues = setScannerPosition(fileInfo, (int) startPt.x, (int) startPt.y, slice);
                                            String[] endValues = setScannerPosition(fileInfo, (int) endPt.x, (int) endPt.y, slice);

                                            if (startValues != null && endValues != null) {
                                                frame.getUserInterface().setDataText("Line: " +
                                                    tmpString2 + ", Position(start): " + startValues[0] + " " + startValues[1] +
                                                    " " + startValues[2] + ", Position(end): " + endValues[0] + " " +
                                                    endValues[1] + " " + endValues[2] + "\n");
                                            } else {
                                                frame.getUserInterface().setDataText("Line: " +
                                                    tmpString2 + ", Position(start): " + (int)(startPt.x + 1) + " " + (int)(startPt.y + 1) +
                                                    ", Position(end): " + (int)(endPt.x + 1) + " " +
                                                    (int)(endPt.y + 1) + "\n");
                                    ...
                                    if (imageActive.isColorImage() == true) {
                                        ...
                                        position = new float[length * 2 + 1];
                                        intensity = new float[length * 2 + 1];
                                        int pt = VOIs.VOIAt(i).findPositionAndIntensity(slice, j, position,
                                            intensity,
                                        ...
                                        if (VOIs.VOIAt(i).getContourGraph() == null) {
                                            lineGraph = new ViewJFrameGraph(pos, inten, "Line VOI Graph",
                                                VOIs.VOIAt(i),
                                                FileInfoBase.getUnitsOfMeasureAbbrevStr(
                                                imageActive.getFileInfo(0).getUnitsOfMeasure(0)));
                                            lineGraph.setDefaultDirectory(
                                                imageActive.getUserInterface().getDefaultDirectory());
            ...
            if (imageActive.getFileInfo(0).getOrigin()[0] != 0 || imageActive.getFileInfo(0).getOrigin()[1] != 0
                ...
                if (values != null) {
                    frame.setMessageText(
                        "  X: " + String.valueOf( (xS + 1)) + " Y: " + String.valueOf( (yS) + 1)
                        + "  Intensity:  "
    ...
    public void paintComponent(Graphics graphics) {
        try {
            ...
            if (paintImageBuffer == null || paintImageBuffer.length != (imageDim.width * imageDim.height)) {
                paintImageBuffer = new int[imageDim.width * imageDim.height]; // make the buffer that will hold the paint
            }
            else {
            ...
            offscreenGraphics2d.setClip(visibleRect); // clip graphics to visible area on screen - this saves rendering time
            ...
            if (memImageA == null) { // create imageA if it hasn't already been created
                memImageA = new MemoryImageSource(imageDim.width,
                                                  imageDim.height,
                                                  pixBuffer,
                                                  0,
                                                  imageDim.width);
            ...
            // draw image A
            offscreenGraphics2d.drawImage(img,
                                          0, 0,
                                          zoomedWidth,
                                          zoomedHeight,
                                          0, 0,
                                          img.getWidth(this), img.getHeight(this), null);
            ...
            if (imageB != null) {
                ...
                if ( !(frame instanceof ViewJFrameLightBox) ) cleanBuffer(BOTH);
                if (!isCheckerboarded()) {
                    adjustOpacityFor000Color();
                    offscreenGraphics2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1 - alphaBlend));
                }
                else {
                    makeCheckerboard();
                }
                ...
                // draw image B
                offscreenGraphics2d.drawImage(imgB,
                                              0, 0,
                                              zoomedWidth,
                                              zoomedHeight,
                                              0, 0,
                                              imgB.getWidth(this), imgB.getHeight(this), null);

            }

            memImage = new MemoryImageSource(imageDim.width, imageDim.height, paintImageBuffer, 0,
                                             imageDim.width);
            ...
            offscreenGraphics2d.drawImage(paintImage,
                                          0, 0,
                                          zoomedWidth,
                                          zoomedHeight,
                                          0, 0,
            ...
            if (mode == WIN_REGION && (lastMouseX != OUT_OF_BOUNDS || lastMouseY != OUT_OF_BOUNDS) && shiftDown == false) {
                if (interpMode == INTERPOLATE_B || interpMode == INTERPOLATE_BOTH) {
                    offscreenGraphics2d.setRenderingHints(new RenderingHints(RenderingHints.KEY_INTERPOLATION,
                        RenderingHints.VALUE_INTERPOLATION_BILINEAR));
                }
                ...
                if (lastWinRegionSlice != slice || cleanImageB == null)
                {
                    ...
                    MemoryImageSource memImageSource = new MemoryImageSource(imageDim.width,
                        imageDim.height,
                        cleanImageBufferB,
                        0,
                        imageDim.width);
    ...
    protected void makePaintImage(int[] paintImageBuffer) {
        ...
        } // should be changed later to a more elegant solution, since this always fails when 'frame' is a ViewJFrameLightBox
            ...
            if (mode == LEVELSET) {
                if (rbLevelSet.getLevelSetPolygon() != null) {
                    ...
                    graphics.drawPolygon(
                        zoomPolygon(rbLevelSet.getLevelSetPolygon(), getZoomX() * resolutionX,
    ...
    /**
     * Paints the image and calls drawSelf for all VOIs
     * and also resizes the image if it is too big for printer.
    ...
    /**
     *  Switches modes based on the variable mode.  Sets
     *  the number of pixels to be drawn when painting.
    ...
    /**
     *   Sets the mode to Live wire and sets the cost function
     *   of the live wire rubberband.
     ...
     */
    public void setModeLivewire(float gradSigma, float edgeLap, float gradWeight,
                                float kernWeight, boolean smoothVOIFlag) {
        this.mode = LIVEWIRE;
        rubberband.setActive(false);
    ...
    /**
     *	Resets the rubberband live wire to null.
     *	When the rubberband live wire is null, the grad mag field is
     * recaculated the next time live wire mode is selected.
     */
    ...
    /**
     *  Switches modes based on the variable mode.  Sets
     *  rubberband activity and the cursor.
    ...
    /**
     * Sets position data to display in message bar - for DICOM and MINC images,
     * gives patient position as well. The image's associated transformation must be
     * FileInfoBase.TRANSFORM_SCANNER_ANATOMICAL or the function returns null.
    ...
    /**
     *    Grows a region based on a starting supplied. A voxel is added to the
     *    the paintBitmap mask if its intensity is between the the bounds which
     *    are also supplied.  Used on black and white images.
     * <p>
     *    when click <code>false</code>, adds points in the newly grown
     *    region which weren't in the old one remove points which
     *    were in the old region but aren't in the new one (and
     *    which weren't in the region painted before the last
     *    click), otherwise, the regions are simply added into
     *    the new set.
     *
        ...
        try {
            ...
            if (imageActive.getNDims() == 2) {
                count = regionGrowAlgo.regionGrow2D(seedPaintBitmap, new Point(saveX, saveY), fuzzyThreshold, useVOI,
                    displayFuzzy, growDialog,
                    saveValue - less, saveValue + more, sizeLimit, maxDistance,
                    variableThresholds);
                ...
                if (growDialog instanceof JDialogPaintGrow && ( (JDialogPaintGrow) growDialog).boundsConstrained()) {
                    ...
                    regionGrowBounds = ( (ViewJComponentTriImage) ( (ViewJFrameTriImage) frame).getTriImage(
                        ViewJFrameTriImage.AXIAL_A)).
                        getBoundedVolume();
                }
                else {
                ...
                count = regionGrowAlgo.regionGrow3D(seedPaintBitmap, new Point3Ds(saveX, saveY, saveZ),
                    fuzzyThreshold, useVOI, displayFuzzy, growDialog,
                    saveValue - less, saveValue + more, sizeLimit,
                    maxDistance, variableThresholds, timeSlice, regionGrowBounds);
                showRegionInfo(count, str);
            }
        }
        catch (OutOfMemoryError error) {
     ...
     *    Grows a region based on a starting supplied. A voxel is added to the
     *    the paintBitmap mask if its intensity is between the the bounds which
     *    are also supplied.  Used on color images.
     * <p>
     *    when click <code>false</code>, adds points in the newly grown
     *    region which weren't in the old one remove points which
     *    were in the old region but aren't in the new one (and
     ...
     *	  @param click  whether this region grow was initiated by a click on the image
     */
    public void regionGrow(short x, short y, short z, float valueR,
                           float valueG, float valueB, String str, boolean click) {
        Cursor cursor = getCursor();
        ...
        try {
            ...
            if (imageActive.getNDims() == 2) {
                count = regionGrowAlgo.regionGrow2D(seedPaintBitmap, new Point(saveX, saveY), fuzzyThreshold, useVOI,
                    displayFuzzy, growDialog,
                    saveValueR - lessR, saveValueR + moreR,
                    saveValueG - lessG, saveValueG + moreG,
                ...
                count = regionGrowAlgo.regionGrow3D(seedPaintBitmap, new Point3Ds(saveX, saveY, saveZ),
                    ...
                    saveValueR - lessR, saveValueR + moreR,
                    saveValueG - lessG, saveValueG + moreG,
                    saveValueB - lessB, saveValueB + moreB, sizeLimit,
                    maxDistance, timeSlice, regionGrowBounds);
                showRegionInfo(count, str);
            }
        }
        catch (OutOfMemoryError error) {
     ...
     * Grows a region based on a starting supplied. A voxel is added to the
     * the paintBitmap mask if its intensity is between the the bounds which
     * are also supplied.  Used for black and white images.
     * <p>
     * When click is <code>false</code>, adds points in the newly grown
     * region which weren't in the old one remove points which
     * were in the old region but aren't in the new one (and
     * which weren't in the region painted before the last
     * click), otherwise, the regions are simply added into
     ...
     * @param click       whether this region grow was initiated by a click on the image
     */
    public void regionGrow(short x, short y, short z, float value,
                           ModelImage image, String leadString, boolean click) {
        Cursor cursor = getCursor();
        ...
        try {
            ...
            if (image.getNDims() == 2) {
                ...
                if ( ( (JDialogPaintGrow) growDialog).boundsConstrained()) {
                    // constrain bounds to cropping volume
                    regionGrowBounds = ( (ViewJComponentTriImage)
                                        ( (ViewJFrameTriImage) frame).getTriImage(ViewJFrameTriImage.AXIAL_A)).
                        getBoundedVolume();
                }
                else {
                    // bounds are not constrained by cropping volume, use image extents as bounds
     ...
     * Grows a region based on a starting supplied. A voxel is added to the
     * the paintBitmap mask if its intensity is between the the bounds which
     * are also supplied.  Used for color images.
     * <p>
     * When click is <code>false</code>, adds points in the newly grown
     * region which weren't in the old one remove points which
     * were in the old region but aren't in the new one (and
     ...
     * @param click       whether this region grow was initiated by a click on the image
     */
    public void regionGrow(short x, short y, short z,
                           float valueR, float valueG, float valueB,
                           ModelImage image, String leadString, boolean click) {
        Cursor cursor = getCursor();
        ...
        try {
            ...
            if (image.getNDims() == 2) {
                count = regionGrowAlgo.regionGrow2D(seedPaintBitmap, new Point(saveX, saveY), fuzzyThreshold, useVOI,
                    ...
                    saveValueR - lessR, saveValueR + moreR,
                    saveValueG - lessG, saveValueG + moreG,
                    saveValueB - lessB, saveValueB + moreB,
                    sizeLimit, maxDistance);
                showRegionInfo(count, leadString);
            }
            else if ( (image.getNDims() == 3) || (image.getNDims() == 4)) {
                CubeBounds regionGrowBounds;
        ...
        try {
            ...
            if (imageActive.getNDims() == 2) {
                ...
                if (leadString != null) {
                    ...
                    frame.getUserInterface().setDataText(
                        "Region grow: pixels = " + count + "\t  area = " + area + str + "\n");
                }
                ...
                if (leadString != null) {
                    ...
                    frame.getUserInterface().setDataText(
                        "Region grow: pixels = " + count + "\t  volume = " + volume + str + "\n");
                }
            }
        }
        catch (OutOfMemoryError error) {
     ...
     * @param voiColor131   the new voi color
     ...
     *   are loaded.
     *   @param  loadAll boolean     boolean indicating that both lut and
     ...
     *   @param  quietMode boolean   if true indicates that warnings
     *                                should not be displayed.
     */
    public void loadOnlyLUTFrom(boolean loadAll, String filename, String dirName,
                                boolean quietMode) {
        ModelRGB rgb;
        ModelLUT lut;
        ModelImage img;
     ...
     *   @param imageData actual image data used to display intensities on the image
     *   @param imageType image type
     *   @param imageXDim x dimension of the image
     */
    public void paintMagComponent(Graphics2D graphics2d) {
        ...
        if (img != null) {
            ...
            graphics2d.drawImage(img, x1, y1, xw1, yh1,
                                 x2, y2,
                                 sX + (int) (sIWidth / 2), sY + (int) (sIHeight / 2),
                                 this);
            ...
            if (imageB != null && imgB != null) {
                ...
                graphics2d.drawImage(imgB, x1, y1, xw1, yh1,
                                     x2, y2,
                                     sX + (int) (sIWidth / 2), sY + (int) (sIHeight / 2),
                                     this);
