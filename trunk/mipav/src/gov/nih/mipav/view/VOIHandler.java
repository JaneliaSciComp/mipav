 ...
 * @version 1.0
 */
public class VOIHandler extends JComponent implements MouseListener,
		MouseMotionListener {
	...
	/**
	 * VOI clipboard used to undo the VOI. This VOI is a copy of last VOI
	 * selected.
	 */
	protected VOI undoVOI;

	/**
	 * Dialog for displaying/calculating VOI statistics as well as the VOI
	 * Browser.
	 */
	...
	public VOIHandler(ViewJComponentEditImage comp) {
		...
		if (compImage.getFrame() != null) {
			voiDialog = new JDialogVOIStats(compImage.getFrame(), compImage
					.getActiveImage(), null);
	...
	/**
	 * Calls the VOI Function (for polyline slices) to calculate and display
	 * distances.
	 */
	public void calcPLineSliceDistances() {
		...
		for (i = 0; i < nVOI; i++) {
			...
			if ((VOIs.VOIAt(i).isActive() == true)
				...
				VOIs.VOIAt(i).calcPLineDistances(
	...
	/**
	 * New all-in-one function for changing the order of whole VOIs and VOI
	 * contours.
	 * 
	 * @param doContour
	 *            boolean contour only (true) or entire VOI (false)
	 * @param direction
	 *            int (FORWARD, BACKWARD, FRONT, BACK)
	 */
	public void changeVOIOrder(boolean doContour, int direction) {
		...
		try {
					...
					if (!doContour) {
						...
						if (direction == FORWARD) {
							...
							if (i == (nVOI - 1)) {
								...
								VOIs.insertElementAt((VOI) VOIs.VOIAt(i)
										.clone(), i + 2);
							}
						...
						for (int j = 0; j < curves[compImage.getSlice()].size(); j++) {
							selectedCurve = (VOIBase) curves[compImage
									.getSlice()].elementAt(j);

							if (selectedCurve.isActive()) {
								foundActive = true;

								VOIBase curveClone = (VOIBase) selectedCurve
										.clone();
								...
								if (direction == FORWARD) {
									...
									if ((j - 1) >= 0) {
										curves[compImage.getSlice()]
												.insertElementAt(curveClone,
														j - 1);
									} else {
										curves[compImage.getSlice()]
												.insertElementAt(curveClone, 0);
									}
			...
			System.gc();
			MipavUtil
					.displayError("Out of memory: ViewJComponentEditImage.changeVOIOrder.");
	 ...
	 * Note: this method does not actually "cut" voi, but rather just
	 * "copies it" (the voi is not deleted with this method)
	 */
	...
	public boolean copyVOItoClipBrd() {
		...
		boolean doLPS = Preferences.is(Preferences.PREF_VOI_LPS_SAVE)
				&& !isFrom2D;
		...
		if (!doLPS) {
				...
				if (VOIs.VOIAt(i).isActive() == true) {
					...
					for (int s = 0; s < zDim; s++) {
						...
						for (int j = numCurves - 1; j >= 0; j--) {
							...
							if (!((VOIBase) (curves[s].elementAt(j)))
									.isActive()) {
					...
					ViewUserInterface.getReference().addClipped2DVOI(voi,
							compImage.getSlice());
					didCopy = true;
				}
			}
			...
			for (i = 0; i < nVOI; i++) {
				...
				if (VOIs.VOIAt(i).isActive() == true) {
					...
					for (int s = 0; s < zDim; s++) {
						...
						for (int j = 0; j < numCurves; j++) {
							...
							for (int k = 0; k < numPoints; k++) {
								...
								MipavCoordinateSystems.fileToScanner(inPoint,
										outPoint, compImage.getActiveImage());
					...
					// add the voi to the clipboard
					ViewUserInterface.getReference().addClippedScannerVOI(voi,
							compImage.getSlice(), scannerVector);
					didCopy = true;
	 ...
	 *            volume of interest
	 * @param zSlice
	 *            the compImage.getSlice() presently displayed from a 3D dataset
	 *            or 0 for 2D
	 */
			...
			if (VOIs.VOIAt(i).isActive() == true) {
				...
				if (contoursOnly) {
					...
					if (VOIs.VOIAt(i).isEmpty() == true) {
						...
						int id = (compImage.getActiveImage().getVOIs().size() > 0) ? (((VOI) (compImage
								.getActiveImage().getVOIs().lastElement()))
								.getID() + 1)
								: 0;
						int lastUID = (compImage.getActiveImage().getVOIs()
								.size() > 0) ? (((VOI) (compImage
								.getActiveImage().getVOIs().lastElement()))
								.getUID() + 1) : -1;
	...
	/**
	 * Removes (if there is an active Polyline or polygon) the active point (a
	 * single point) from the contour.
	 */
				...
				if ((curveType == VOI.CONTOUR) || (curveType == VOI.POLYLINE)) {
					...
					if (allActive) {
						...
						for (int sl = 0; sl < end; sl++) {
							...
							for (int j = 0; j < VOIs.VOIAt(i).getCurves()[sl]
									.size(); j++) {

								if (((VOIContour) (VOIs.VOIAt(i).getCurves()[sl]
										.elementAt(j))).isActive()) {
									((VOIContour) (VOIs.VOIAt(i).getCurves()[sl]
											.elementAt(j))).removeActivePt();
		...
		if (compImage.getOrientation() == FileInfoBase.UNKNOWN_ORIENT) {
			...
			if (VOIs != null) {
				...
				if (compImage.getSlice() != -99) {
					if ((compImage.getActiveImage().getFileInfo(0) != null)
						...
						float originX = (float) compImage.getActiveImage()
								.getFileInfo(0).getOrigin()[0];
						float originY = (float) compImage.getActiveImage()
								.getFileInfo(0).getOrigin()[1];

						for (int i = 0; i < nVOI; i++) {
							VOIs.VOIAt(i).drawSelf(
									compImage.getZoomX(),
									compImage.getZoomY(),
									...
									compImage.getActiveImage().getFileInfo(0)
											.getUnitsOfMeasure(),
									compImage.getSlice(),
									compImage.getOrientation(),
									compImage.getActiveImage().getFileInfo(0),
									compImage.getActiveImage().getNDims(),
									graphics);
						}
					}
				}
			...
			if (mode == ViewJComponentEditImage.LEVELSET) {
				...
				if (rbLevelSet.getLevelSetPolygon() != null) {
					...
					graphics.drawPolygon(zoomPolygon(rbLevelSet
							.getLevelSetPolygon(), compImage.getZoomX()
							* compImage.getResolutionX(), compImage.getZoomY()
							* compImage.getResolutionY()));
				}
			}
	...
	/**
	 * Gets the VOI Popup menu (used for enabling/disabling and mouse
	 * listening).
	 * 
	...
	public Polygon getZoomedLevelSetPolygon() {
		return zoomPolygon(rbLevelSet.getLevelSetPolygon(), compImage
				.getZoomX(), compImage.getZoomY());
	}
	...
	public void graph25VOI_CalcInten(boolean totalIntensity,
		...
		if (compImage.getActiveImage().getNDims() == 3) {
			...
			if (compImage.getActiveImage().isColorImage() == true) {
				...
				try {
					rgbPositions = new float[3][compImage.getActiveImage()
							.getExtents()[2]];
					rgbIntensities = new float[3][compImage.getActiveImage()
							.getExtents()[2]];
					...
					for (i = 0; i < nVOI; i++) {
						...
						if (VOIs.VOIAt(i).isActive()
							...
							if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
								...
								for (s = 0; s < compImage.getActiveImage()
										.getExtents()[2]; s++) {

									try {
										compImage.getActiveImage().exportData(
												s * length * 4, length * 4,
												graphImgBuff); // locks and
																// releases lock

										for (int c = 0; c < 3; c++) {
											numPixels = 0;

											for (j = 0, intensitySum = 0; j < VOIs
													.VOIAt(i).getCurves()[s]
													.size(); j++) {

												if (useThreshold) {
													intensitySum += ((VOIContour) (VOIs
															.VOIAt(i)
															.getCurves()[s]
															.elementAt(j)))
				...
				try {
					position = new float[compImage.getActiveImage()
							.getExtents()[2]];
					intensity = new float[compImage.getActiveImage()
							.getExtents()[2]];
					VOIs = compImage.getActiveImage().getVOIs();
					nVOI = VOIs.size();
			...
			if (compImage.getActiveImage().getFileInfo()[0].getFileFormat() == FileUtility.DICOM) {
				...
				fileInfo = (FileInfoDicom) (compImage.getActiveImage()
						.getFileInfo(0));
				...
				for (i = 0; i < nVOI; i++) {
					...
					if (VOIs.VOIAt(i).isActive()
						...
						if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
							...
							for (int t = 0; t < compImage.getActiveImage()
									.getExtents()[3]; t++) {

								try {
									numPixels = 0;

									for (s = 0, intensitySum = 0; s < compImage
											.getActiveImage().getExtents()[2]; s++) {
										compImage.getActiveImage().exportData(
												(t * xDim * yDim * zDim)
														+ (s * xDim * yDim),
												length, graphImgBuff); // locks
																		// and
																		// releases
																		// lock

										for (j = 0; j < VOIs.VOIAt(i)
												.getCurves()[s].size(); j++) {
											intensitySum += ((VOIContour) (VOIs
													.VOIAt(i).getCurves()[s]
													.elementAt(j)))
									...
									if (useFrameRefTime) {
										fileInfo = (FileInfoDicom) (compImage
												.getActiveImage().getFileInfo(t
												* zDim));
										frameRefTimeString = ((String) fileInfo
												.getTagTable().getValue(
														"0054,1300")).trim();
										if (frameRefTimeString != null) {
											try {
	 ...
	 * 
	 * @param v
	 *            The VOI that the point is from
	 * @param voiPt
	 *            The VOIPoint which the graph is to be of
	 * @param j
	...
	public void graphPointVOI(VOI v, VOIPoint voiPt, int j,
		...
		if ((compImage.getActiveImage().getNDims() != 3)
				&& (compImage.getActiveImage().getNDims() != 4)) {
			return;
		}
		...
		if (compImage.getActiveImage().getNDims() == 3) {
			...
			if (compImage.getActiveImage().isColorImage() == true) {
				ptRGBPositions = new float[3][compImage.getActiveImage()
						.getExtents()[2]];
				ptRGBIntensities = new float[3][compImage.getActiveImage()
						.getExtents()[2]];
				...
				for (s = 0; s < compImage.getActiveImage().getExtents()[2]; s++) {
					...
					for (int c = 0; c < 3; c++) {
						...
						ptRGBIntensities[c][s] = compImage
								.getActiveImage()
								.getFloat(
										((4 * ((s * compImage.getActiveImage()
												.getSliceSize())
												+ ((int) pt.Y * compImage
														.getActiveImage()
				...
				try {
					ptPosition = new float[compImage.getActiveImage()
							.getExtents()[2]];
					ptIntensity = new float[compImage.getActiveImage()
							.getExtents()[2]];
					...
					for (s = 0; s < compImage.getActiveImage().getExtents()[2]; s++) {
						...
						ptIntensity[s] = compImage.getActiveImage().getFloat(
								(int) ((s * compImage.getActiveImage()
										.getSliceSize())
			...
			try {
				...
				for (t = 0; t < compImage.getActiveImage().getExtents()[3]; t++) {
					...
					ptIntensity[t] = compImage.getActiveImage()
							.getFloat(
	...
	/**
	 * Generates and displays a 1D graph of the image intensities underlying the
	 * contour of the VOI.
	 */
	public void graphVOI() {
		...
		if (compImage.getActiveImage().isColorImage() == true) {
			...
			try {
				...
				for (i = 0; i < nVOI; i++) {
					...
					if (VOIs.VOIAt(i).isActive()
						...
						if ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							...
							for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
									.getSlice()].size(); j++) {

								if (((VOIContour) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j))).isActive()) {
									length = (int) MipavMath
											.round(((VOIContour) (VOIs.VOIAt(i)
													.getCurves()[compImage
													.getSlice()].elementAt(j)))
													.getLengthPtToPt(compImage
															.getActiveImage()
															.getFileInfo(0)
															.getResolutions()));

									if (compImage.getActiveImage().getFileInfo(
											0).getResolutions()[0] > 0) {
										...
										MipavUtil
												.displayError("Image resolutions appear to be incorrect!");
									}
									...
									for (int c = 0; c < 3; c++) {
										...
										rgbStdDevInten[c] = (float) Math
												.sqrt(rgbStdDevInten[c] / pt);
									}
								}
							}

							ViewJFrameGraph contourGraph = new ViewJFrameGraph(
									rgbPos, rgbInten, "Intensity Graph");

							contourGraph.setUnitsInLabel(FileInfoBase
									.getUnitsOfMeasureAbbrevStr(compImage
											.getActiveImage().getFileInfo(0)
											.getUnitsOfMeasure(0)));
							contourGraph.setDefaultDirectory(ViewUserInterface
				...
				if (i == nVOI) {
					MipavUtil
							.displayError("Please select a contour or line VOI!");
				}
			} catch (OutOfMemoryError error) {
				System.gc();
				MipavUtil
						...
						if ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							...
							for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
								...
								if (((VOIContour) (VOIs.VOIAt(i).getCurves()[compImage
									...
									stdDevInten = (float) Math.sqrt(stdDevInten
											/ pt);
	 ...
	 * 
	 * <p>
	 * Arrow key = move active VOI CTRL + left/right = cycle through VOI's
	 * contours CTRL + up/down = cycle through VOIs Shift + arrow key = move
	 * active contour's active point CTRL + Shift + arrow key = cycle through
	 * active contour's active points
	 * </p>
	 * 
	 * @param e
	 *            KeyEvent
	...
	/**
	 * Accessor that returns a flag indicating if the rubberband live wire is
	 * null or not.
	 * 
	...
	/**
	 * Compares the current VOI ID and its type to the given voi type. If the
	 * types are different, then set the voi ID to -1, and return true (to
	 * indicate changing the type). If the types are the same, then it's not
	 * necessary to change types so return false.
	 * 
	 * @param voiType
	 *            this should be one of the static ints from <code>VOI</code>
	 *            (for instance <code>
     *                   VOI.LINE</code>).
	 * 
	...
	public void mouseDragged(MouseEvent mouseEvent) {
		...
		if (mode == ViewJComponentEditImage.MOVE) {
			...
			for (i = 0; i < nVOI; i++) {
				...
				if (VOIs.VOIAt(i).isActive()) {
					...
					if (((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							...
							|| (VOIs.VOIAt(i).getCurveType() == VOI.PROTRACTOR) || (VOIs
							.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE))
						...
						if (allActive) {
							VOIs.VOIAt(i).moveVOI(compImage.getSlice(), xDim,
							...
							for (int sl = 0; sl < end; sl++) {
								...
								if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
									...
									if ((sl == compImage.getSlice())
											&& VOIs.VOIAt(i).nearLine(xS, yS,
													sl)) {

										VOIs.VOIAt(i).moveVOI(sl, xDim, yDim,
												zDim, distX, distY, 0);
										compImage.getActiveImage()
												.notifyImageDisplayListeners();
									}
									...
									for (j = 0; j < VOIs.VOIAt(i).getCurves()[sl]
											.size(); j++) {
										...
										if ((curveType == VOI.CONTOUR)
											...
											contains = ((VOILine) (VOIs
													.VOIAt(i).getCurves()[sl]
													.elementAt(j))).contains(
													xS, yS, true);
											isActive = ((VOILine) (VOIs
													.VOIAt(i).getCurves()[sl]
													.elementAt(j))).isActive();
						...
						if ((VOIs.VOIAt(i).getContourGraph() != null)
								&& VOIs.VOIAt(i).getContourGraph().isVisible()
								&& ((curveType == VOI.CONTOUR) || (curveType == VOI.POLYLINE))) {
							VOI v;
							float intensitySum;
							int length = compImage.getActiveImage()
									.getSliceSize();
							int s;
							int numPixels;
							boolean foundCurve;
							...
							float[][] rgbPositions = VOIs.VOIAt(i)
									.getRGBPositions();
							float[][] rgbIntensities = VOIs.VOIAt(i)
									.getRGBIntensities();
							...
							if (compImage.getActiveImage().getNDims() == 3) {
								...
								if (compImage.getActiveImage().isColorImage() == true) {
									...
									try {
										...
										for (s = 0, foundCurve = false; s < compImage
												.getActiveImage().getExtents()[2]; s++) {
									...
									try {
										...
										for (s = 0, foundCurve = false; s < compImage
											...
											try {
												...
												for (j = 0, intensitySum = 0; j < VOIs
														.VOIAt(i).getCurves()[s]
														.size(); j++) {
													boolean isActive = ((VOIContour) (VOIs
															.VOIAt(i)
															.getCurves()[s]
															.elementAt(j)))
															.isActive();
										...
										VOIs.VOIAt(i).getContourGraph().update(
								...
								try {
									...
									for (int t = 0; t < compImage
											.getActiveImage().getExtents()[3]; t++) {
						...
						// asdf
						if (allActive) {
							VOIs.VOIAt(i).moveVOI(-1, xDim, yDim, zDim, distX,
									distY, 0);
						} else {
							VOIs.VOIAt(i).moveVOI(compImage.getSlice(), xDim,
									yDim, zDim, distX, distY, 0);
						...
						if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
							...
							for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
								...
								if (((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j))).isActive()) {
									pt = ((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
											.getSlice()].elementAt(j)))
											.getActivePt();
									compImage.setPixelInformationAtLocation(
											(int) pt.X, (int) pt.Y);

									if (compImage.getActiveImage().getNDims() == 3) {

										if (compImage.getActiveImage()
												.isColorImage() == true) {

											for (int s = 0; s < compImage
													.getActiveImage()
													.getExtents()[2]; s++) {
												pt = ((VOIPoint) (VOIs.VOIAt(i)
														.getCurves()[compImage
														.getSlice()]
														.elementAt(j)))
														.exportPoint();
												...
												for (int c = 0; c < 3; c++) {
													...
													if ((ptRGBPositions != null)
															&& (ptRGBIntensities != null)) {
														ptRGBPositions[c][s] = s;
														ptRGBIntensities[c][s] = compImage
																.getActiveImage()
																.getFloat(
									...
									} else if (compImage.getActiveImage()
											.getNDims() == 4) {
			...
			return;
		} else if (mode == ViewJComponentEditImage.MOVE_POINT) { // rubberband a
																	// point
			...
			if (mouseEvent.isShiftDown() == false) {
				...
				for (i = 0; i < nVOI; i++) {
					...
					if (VOIs.VOIAt(i).isActive()
						...
						VOIs.VOIAt(i).rubberbandVOI(xS, yS,
								compImage.getSlice(), xDim, yDim, false,
								compImage.getZoomX(), compImage.getZoomY(),
								compImage.getResolutionX(),
								compImage.getResolutionY(), g);

						if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
							Vector3f pt = VOIs.VOIAt(i).exportPSlicePoint(
									compImage.getSlice());
				...
				for (i = 0; i < nVOI; i++) {
					...
					if (VOIs.VOIAt(i).isActive()
						...
						for (j = 0; j < sliceNum; j++) {
							VOIs.VOIAt(i).rubberbandVOI(xS, yS, j, xDim, yDim,
									false);
						}
			...
			for (i = 0; i < nVOI; i++) {
				...
				if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible()) {
					...
					if (((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) || (VOIs
							.VOIAt(i).getCurveType() == VOI.POLYLINE))
							&& (mouseEvent.isControlDown() == false)
							&& (mouseEvent.getModifiers() != MouseEvent.BUTTON3_MASK)) {
			...
			for (i = 0; i < nVOI; i++) {
				...
				if (VOIs.VOIAt(i).isActive()) {
					try {
						if (restart) {
							((VOIContour) (VOIs.VOIAt(i)
									.getActiveContour(compImage.getSlice())))
									.resetStart();
							restart = false;
						}
						((VOIContour) (VOIs.VOIAt(i).getActiveContour(compImage
								.getSlice()))).retraceContour(compImage
								.getZoomX(), compImage.getZoomY(), compImage
								.getResolutionX(), compImage.getResolutionY(),
								compImage.getActiveImage().getFileInfo(0)
										.getResolutions(), xS, yS, g, VOIs
										.VOIAt(i).getThickness());
						compImage
								.setCursorMode(ViewJComponentEditImage.RETRACE);
					} catch (Exception e) {
			...
			compImage.performPaint(mouseEvent,
					mouseMods == MouseEvent.BUTTON3_MASK);
	...
	public void mouseMoved(MouseEvent mouseEvent) {
		...
		if ((mode == ViewJComponentEditImage.PAINT_VOI)
		...
		if ((compImage.getGraphics() == null)
				|| (compImage.getModifyFlag() == false)
				|| (compImage.getSlice() == -99)) {
			return;
		}

		if ((compImage.getPixBuffer() == null)
				|| (compImage.getActiveImageBuffer() == null)) {
			compImage.getGraphics().dispose();
		...
		if ((mode == ViewJComponentEditImage.RECTANGLE)
				|| (mode == ViewJComponentEditImage.ELLIPSE)
				|| (mode == ViewJComponentEditImage.LINE)
				|| (mode == ViewJComponentEditImage.RECTANGLE3D)
				|| (mode == ViewJComponentEditImage.POINT_VOI)
				|| (mode == ViewJComponentEditImage.POLYLINE)
				|| (mode == ViewJComponentEditImage.LEVELSET)
				|| (mode == ViewJComponentEditImage.PAINT_VOI)
				|| (mode == ViewJComponentEditImage.DROPPER_PAINT)
				|| (mode == ViewJComponentEditImage.ERASER_PAINT)
				|| (mode == ViewJComponentEditImage.QUICK_LUT)
				|| (mode == ViewJComponentEditImage.PROTRACTOR)
				|| (mode == ViewJComponentEditImage.LIVEWIRE)
				|| (mode == ViewJComponentEditImage.ANNOTATION)
				|| (mode == ViewJComponentEditImage.POLYLINE_SLICE_VOI)
				|| (mode == ViewJComponentEditImage.PAINT_CAN)
			...
			for (i = 0; i < nVOI; i++) {
				...
				if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible()) {
					...
					if (mouseEvent.isShiftDown()
							&& VOIs.VOIAt(i).nearPoint(x, y,
									compImage.getSlice(), compImage.getZoomX(),
									compImage.getResolutionX(),
									compImage.getResolutionY())) {
						...
						if ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							...
							for (j = 0; j < nCurves; j++) {
								...
								if (VOIs.VOIAt(i).nearPoint(x, y,
										compImage.getSlice(), j,
										compImage.getZoomX(),
										compImage.getResolutionX(),
										compImage.getResolutionY())) {
									break;
							...
							// false ) {
							compImage
									.setCursorMode(ViewJComponentEditImage.DELETE_POINT);
							...
							VOIs.VOIAt(i).drawVertices(
									...
									compImage.getActiveImage().getFileInfo(0)
											.getUnitsOfMeasure(),
									compImage.getSlice(),
									compImage.getOrientation(), g, j);
							compImage.getGraphics().dispose();

							return;
						}
					} else if (VOIs.VOIAt(i).nearPoint(x, y,
							compImage.getSlice(), compImage.getZoomX(),
							compImage.getResolutionX(),
							compImage.getResolutionY())
							&& (VOIs.VOIAt(i).getCurveType() != VOI.POINT)
						...
						if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
							...
							compImage
									.setCursorMode(ViewJComponentEditImage.MOVE_POINT);
							compImage.getGraphics().dispose();
							...
							if (VOIs.VOIAt(i).getCurveType() != VOI.LINE) {
								...
								VOIs.VOIAt(i).drawVertices(
										...
										compImage.getResolutionX(),
										compImage.getResolutionY(),
										compImage.getActiveImage().getFileInfo(
												0).getResolutions(),
										compImage.getActiveImage().getFileInfo(
												0).getUnitsOfMeasure(),
										compImage.getSlice(),
						...
						for (j = 0; j < nCurves; j++) {
							...
							if (VOIs.VOIAt(i).nearOuterPoint(x, y,
									compImage.getSlice(), j,
									compImage.getZoomX(),
									compImage.getResolutionX(),
									compImage.getResolutionY())
					...
					// of the contour/polyline/polyline_slice
					else if ((VOIs.VOIAt(i)).nearLine(xS, yS, compImage
							.getSlice())
							&& (mouseEvent.isAltDown() == false)) {
						...
						if ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							...
							for (j = 0; j < nCurves; j++) {
								...
								if (((VOIContour) (curves[compImage.getSlice()]
										.elementAt(j))).isActive()
										&& ((VOIContour) (curves[compImage
												.getSlice()].elementAt(j)))
												.nearLine(xS, yS)) {
									break;
								}
							...
							compImage
									.setCursorMode(ViewJComponentEditImage.NEW_POINT);
							compImage.getGraphics().dispose();
		...
		for (i = 0; i < nVOI; i++) {
			...
			if ((curveType == VOI.CONTOUR) || (curveType == VOI.POLYLINE)
					|| (curveType == VOI.PROTRACTOR) || (curveType == VOI.LINE)
					|| (curveType == VOI.ANNOTATION)) {
				...
				for (j = 0; j < curves[compImage.getSlice()].size(); j++) {
					...
					if ((curveType == VOI.CONTOUR)
							|| (curveType == VOI.POLYLINE)) {
						isContained = ((VOIContour) (curves[compImage
								.getSlice()].elementAt(j))).contains(xS, yS,
								false);
					} else if (curveType == VOI.PROTRACTOR) {
						isContained = ((VOIProtractor) (curves[compImage
								.getSlice()].elementAt(j))).contains(xS, yS,
								false);
						((VOIProtractor) (curves[compImage.getSlice()]
								.elementAt(j))).setShowLength(true);
					} else if (curveType == VOI.LINE) {
						isContained = ((VOILine) (curves[compImage.getSlice()]
								.elementAt(j))).contains(xS, yS, false);
					} else if (curveType == VOI.ANNOTATION) {
						isContained = ((VOIText) (curves[compImage.getSlice()]
								.elementAt(j))).contains(xS, yS, compImage
								.getZoomX(), compImage.getZoomY(), compImage
								.getActiveImage().getFileInfo(0)
								.getResolutions(), g);
					}
				...
				if (VOIs.VOIAt(i).isVisible()
						&& VOIs.VOIAt(i).nearPoint(x, y, compImage.getSlice(),
								compImage.getZoomX(),
								compImage.getResolutionX(),
								compImage.getResolutionY())) {
					compImage.setCursorMode(ViewJComponentEditImage.MOVE);
	...
	public void mousePressed(MouseEvent mouseEvent) {
		...
		try {
			...
			if ((xS < 0) || (xS >= compImage.getActiveImage().getExtents()[0])
					|| (yS < 0)
					...
					if (VOIs.VOIAt(i).isActive()
						...
						if (VOIs.VOIAt(i).getCurveType() == VOI.LINE) {
							...
							for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
								...
								if (((VOILine) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j))).nearLine(
										xS, yS)) {

									// if CTRL was held down, then print out the
									// Line's information to the
									// DATA window
									if ((mouseEvent.getModifiers() & (MouseEvent.CTRL_DOWN_MASK | MouseEvent.CTRL_MASK)) != 0) {
										Vector3f startPt = null;
										Vector3f endPt = null;

										startPt = (Vector3f) ((VOILine) (VOIs
												.VOIAt(i).getCurves()[compImage
												.getSlice()].elementAt(j)))
												.elementAt(0);
										endPt = (Vector3f) ((VOILine) (VOIs
												.VOIAt(i).getCurves()[compImage
												.getSlice()].elementAt(j)))
												.elementAt(1);

										double angle = ((VOILine) (VOIs
												.VOIAt(i).getCurves()[compImage
												.getSlice()].elementAt(j)))
												.getAngle();

										String tmpString2 = String
												.valueOf(angle);
										i = tmpString2.indexOf('.');

										if (tmpString2.length() >= (i + 3)) {
											tmpString2 = tmpString2.substring(
													0, i + 3);
										}
										...
										if ((compImage.getActiveImage()
												...
												|| (compImage.getActiveImage()
														.getFileInfo(0)
														.getOrigin()[2] != 0)) {
											String[] startValues = ViewJComponentEditImage
													.getScannerPositionLabels(
															...
															new Vector3f(
																	...
																	compImage
																			.getSlice()));
											String[] endValues = ViewJComponentEditImage
													.getScannerPositionLabels(
															compImage
																	.getActiveImage(),
															new Vector3f(
			...
			System.gc();
			MipavUtil
					.displayError("Out of memory: ComponentEditImage.mousePressed");
			compImage.setCursorMode(ViewJComponentEditImage.DEFAULT);
	...
	/**
	 * A mouse event. This function sets up and draws the VOI according to the
	 * mode.
	 * 
	 * @param mouseEvent
	...
	public void mouseReleased(MouseEvent mouseEvent) {
		...
		int xS = compImage.getScaledX(mouseEvent.getX()); // zoomed x. Used as
															// cursor
		int yS = compImage.getScaledY(mouseEvent.getY()); // zoomed y. Used as
															// cursor
		...
		if ((mouseEvent.getModifiers() & InputEvent.BUTTON2_MASK) != 0) {
			ViewJFrameTriImage triFrame = compImage.getActiveImage()
					.getTriImageFrame();
		...
		if (mode == ViewJComponentEditImage.POINT_VOI) {
				...
				if (isNewVoiNeeded(VOI.POINT)) { // create new VOI
					...
					try {
						...
						if (compImage.getActiveImage().getVOIs().size() > 0) {
							colorID = ((VOI) (compImage.getActiveImage()
									.getVOIs().lastElement())).getID() + 1;
						}

						if (compImage.getActiveImage().getNDims() > 2) {

							newPtVOI = new VOI((short) colorID, "point3D.voi",
									compImage.getActiveImage().getExtents()[2],
									VOI.POINT, -1.0f);
						} else {
							newPtVOI = new VOI((short) colorID, "point2d.voi",
									1, VOI.POINT, -1.0f);
						}
						...
						System.gc();
						MipavUtil
								.displayError("Out of memory: ComponentEditImage.mouseReleased");
						compImage
								.setCursorMode(ViewJComponentEditImage.DEFAULT);
					...
					((VOIPoint) (VOIs.VOIAt(voiID).getCurves()[compImage
							.getSlice()].elementAt(0))).setActive(true);

					compImage.getActiveImage().notifyImageDisplayListeners();

					graphPointVOI(newPtVOI, ((VOIPoint) (VOIs.VOIAt(voiID)
							.getCurves()[compImage.getSlice()].elementAt(0))),
							0, false);
					...
					for (i = 0; i < nVOI; i++) {
						...
						if (VOIs.VOIAt(i).getID() == voiID) {
							...
							if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
								...
								MipavUtil
										.displayError("Can't add Point VOI to other VOI structure.");
					...
					for (j = 0; j < end; j++) {
						...
						for (k = 0; k < index; k++) {
							((VOIPoint) (VOIs.VOIAt(i).getCurves()[j]
									.elementAt(k))).setActive(false);
						}
					}

					index = VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
							.size();
					((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
							.elementAt(index - 1))).setActive(true);

					compImage.getActiveImage().notifyImageDisplayListeners();

					if (!((VOIs.VOIAt(i).getContourGraph() != null) && (compImage
							.getActiveImage().isColorImage() == true))) {
						graphPointVOI(VOIs.VOIAt(i), ((VOIPoint) (VOIs.VOIAt(i)
								.getCurves()[compImage.getSlice()]
			...
			if ((mouseEvent.getModifiers() & InputEvent.BUTTON1_MASK) != 0) {
				...
				if (isNewVoiNeeded(VOI.POLYLINE_SLICE)) { // create new VOI
					...
					try {
						...
						if (compImage.getActiveImage().getNDims() > 2) {
							newPolySliceVOI = new VOI((short) colorID,
									"Polyline_slice", compImage
											.getActiveImage().getExtents()[2],
									VOI.POLYLINE_SLICE, -1.0f);
						} else {
							MipavUtil
									.displayError("Inter-frame polyline must be used on 2.5/3D images only");
						...
						y[0] = yS;
						z[0] = compImage.getSlice();
						newPolySliceVOI.importCurve(x, y, z, compImage
								.getSlice());
					...
					allActive = true;
					((VOIBase) (newPolySliceVOI.getCurves()[compImage
							.getSlice()].elementAt(0))).setActive(true);

					updateVOIColor(newPolySliceVOI.getColor(), newPolySliceVOI
							.getUID());
					// ( (VOIPoint)
					...
					for (i = 0; i < nVOI; i++) {
						...
						if (VOIs.VOIAt(i).getID() == voiID) {
							...
							if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
								...
								MipavUtil
										.displayError("Can't add POLYLINE_SLICE VOI to other VOI structure.");
			...
			if ((mouseEvent.getModifiers() & InputEvent.BUTTON1_MASK) != 0) {
				...
				if (compImage.getActiveImage().getVOIs().size() > 0) {
					colorID = ((VOI) (compImage.getActiveImage().getVOIs()
							.lastElement())).getID() + 1;
				}
				...
				if (compImage.getActiveImage().getNDims() > 2) {
					...
					newTextVOI = new VOI((short) colorID, "annotation2d.voi",
							1, VOI.ANNOTATION, -1.0f);
				}
				...
				String prefColor = Preferences
						.getProperty(Preferences.PREF_VOI_TEXT_COLOR);
				Color textColor;
				...
				if (prefColor != null) {
					...
					Preferences.setProperty(Preferences.PREF_VOI_TEXT_COLOR,
							MipavUtil.makeColorString(Color.white));
					newTextVOI.setColor(Color.white);
				}

				// pop up a dialog that allows text input, color, and font
				// formatting
				new JDialogAnnotation(compImage.getActiveImage(), newTextVOI,
						compImage.getSlice(), false, false);
		...
		} else if ((mode == ViewJComponentEditImage.POLYLINE)
				|| (mode == ViewJComponentEditImage.LIVEWIRE)) {
			return;
		...
		} else if (mode == ViewJComponentEditImage.NEW_POINT) { // impossible
																// for LINE
			...
			if (((mouseEvent.isShiftDown() == true))) {
				...
				if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
					...
					if (!((mouseEvent.isShiftDown() == true) || Preferences
							.is(Preferences.PREF_CONTINUOUS_VOI_CONTOUR))) {
						VOIs.VOIAt(i).insertPSlicePt(new Vector3f(xS, yS, 0),
								compImage.getSlice());
						compImage.getActiveImage().notifyImageDisplayListeners(
								null, true);
					}
				...
				int index = VOIs.VOIAt(i).getActiveContourIndex(
						compImage.getSlice());

				((VOIContour) (VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
						.elementAt(index))).insertElement(xS, yS, compImage
						.getSlice());
				compImage.getActiveImage().notifyImageDisplayListeners(null,
						true);
					...
					if (((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) || (VOIs
						...
						if ((VOIs.VOIAt(i).getContourGraph() != null)
							...
							if (compImage.getActiveImage().getNDims() == 3) {
								...
								int zDim = compImage.getActiveImage()
										.getExtents()[2];
		...
		} else if (mode == ViewJComponentEditImage.DELETE_POINT) { // impossible
																	// for LINE
			nVOI = VOIs.size();
			...
			for (i = 0; i < nVOI; i++) {
				...
				for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
						.size(); j++) {

					// get the curve referenced by the VOI. We'll check it.
					selectedCurve = ((VOIBase) VOIs.VOIAt(i).getCurves()[compImage
							.getSlice()].elementAt(j));

					if (selectedCurve instanceof VOIPoint) {

						if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
							if (VOIs.VOIAt(i).nearLine(xS, yS,
									compImage.getSlice())
									|| ((VOIPoint) selectedCurve).nearPoint(xR,
											yR, compImage.getZoomX(), compImage
													.getResolutionX(),
											compImage.getResolutionY())) {
								VOIs.VOIAt(i).setActive(true);
								VOIs.VOIAt(i).setAllActive(true);
								...
								VOIs.VOIAt(i)
										.markPSlicePt(compImage.getSlice());

								allActive = true;
								updateVOIColor(VOIs.VOIAt(i).getColor(), VOIs
										.VOIAt(i).getUID());
								voiID = VOIs.VOIAt(i).getID();
								j = VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].size();
						...
						} else if (((VOIPoint) selectedCurve).nearPoint(xR, yR,
								compImage.getZoomX(), compImage
										.getResolutionX(), compImage
										.getResolutionY())) {
							...
							if (mouseEvent.isShiftDown()) {
								...
								Vector3f pt = ((VOIPoint) (selectedCurve))
										.exportPoint();
							...
							&& ((VOIText) selectedCurve).contains(xS, yS,
									compImage.getZoomX(), compImage.getZoomY(),
									compImage.getActiveImage()
											.getResolutions(0), g)) {
						...
						allActive = false;
						VOIs.VOIAt(i).setActive(true);
						updateVOIColor(VOIs.VOIAt(i).getColor(), VOIs.VOIAt(i)
								.getUID());
						...
						if (mouseEvent.getClickCount() == 2) {
							new JDialogAnnotation(compImage.getActiveImage(),
									VOIs.VOIAt(i), compImage.getSlice(), true,
									false);
						}
						...
						if (mousePressIsShiftDown) {
							...
							// the curves
							j = VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
									.size();
						} else {
							allActive = false;
							...
							// active)
							boolean toggle = !selectedCurve.isActive()
									|| wasDragging
						...
						fireVOISelectionChange(VOIs.VOIAt(i), selectedCurve);
							...
							&& !mouseEvent.isPopupTrigger()) { // selected curve
																// was not
			...
			for (i = 0; i < nVOI; i++) {
				...
				if ((VOIs.VOIAt(i).isActive() && (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR))
						|| (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE)) {
					VOIs.VOIAt(i).rubberbandVOI(xS, yS, compImage.getSlice(),
							xDim, yDim, true);

					for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
							.getSlice()].size(); j++) {

						if (((VOIBase) (VOIs.VOIAt(i).getCurves()[compImage
								.getSlice()].elementAt(j))).isActive()) {
							Vector3f pt = ((VOIBase) (VOIs.VOIAt(i).getCurves()[compImage
									.getSlice()].elementAt(j))).getActivePt();
							compImage.setPixelInformationAtLocation((int) pt.X,
									(int) pt.Y);
					...
					VOIs.VOIAt(i).nearPoint(0, 0, compImage.getSlice(),
							compImage.getZoomX(), compImage.getResolutionX(),
							compImage.getResolutionY());
				} else if ((VOIs.VOIAt(i).isActive() && (VOIs.VOIAt(i)
						.getCurveType() == VOI.LINE))
						|| (VOIs.VOIAt(i).getCurveType() == VOI.PROTRACTOR)) {
					...
					for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
						...
						if (((VOIBase) (VOIs.VOIAt(i).getCurves()[compImage
							...
							if (VOIs.VOIAt(i).getCurveType() == VOI.LINE) {
								Vector3f pt = ((VOIBase) (VOIs.VOIAt(i)
										.getCurves()[compImage.getSlice()]
										.elementAt(j))).getActivePt();
								compImage.setPixelInformationAtLocation(
										(int) pt.X, (int) pt.Y);
							}

							if (VOIs.VOIAt(i).getCurveType() == VOI.PROTRACTOR) {
								((VOIProtractor) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j)))
										.setShowLength(true);
							}
		...
		if (VOIs != null) {
			...
			if (compImage.getSlice() != -99) {
				...
				for (int i = nVOI - 1; i >= 0; i--) {
					VOIs.VOIAt(i).drawBlendSelf(1, 1, 1, 1,
							compImage.getSlice(), compImage.getPaintBuffer(),
							graphics,
	...
	public void pasteVOI() {
		...
		if (doLPS) {
			...
			if (((compImage.getActiveImage().getNDims() > 2) && isFrom2D)
				...
				MipavUtil
						.displayInfo("Can not copy/paste LPS-based VOIs between 2D and 2.5D/3D images.");

				return;
			}
		}

		ViewVOIVector clippedVOIs = ViewUserInterface.getReference()
				.getClippedVOIs();
		...
		if (doLPS) {
			scannerVectors = ViewUserInterface.getReference()
		...
		for (int k = 0; k < numClipped; k++) {
			...
			try {
				...
				if (isFrom2D == false && doLPS) {
					...
					Vector[] newCurves = new Vector[compImage.getActiveImage()
							.getExtents()[2]];
					...
					for (int s = 0; s < curves.length; s++) {
						...
						for (int j = 0; j < numCurves; j++) {
							...
							for (int c = 0; c < numPoints; c++, scannerIndex++) {
								inPoint = (Vector3f) scannerVector
										.elementAt(scannerIndex);

								// System.err.println("In point (scanner): " +
								// inPoint);

								// convert from scanner
								MipavCoordinateSystems.scannerToFile(inPoint,
										outPoint, compImage.getActiveImage());
				...
				e.printStackTrace();
				MipavUtil
						.displayInfo("The clipboard is empty, so there is nothing to paste.");

				return;
			} catch (ArrayIndexOutOfBoundsException e) {
				e.printStackTrace();
				MipavUtil
						.displayError("VOI bounds do not match those of current image: aborting.");
			...
			if ((imageXExt <= (x[1] - x[0])) || (imageYExt <= (y[1] - y[0]))
					|| (imageXExt <= x[0]) || (imageXExt <= x[1])
					|| (imageYExt <= y[0]) || (imageYExt <= y[1])) { // checks
																		// to
																		// make
																		// sure
																		// if
				...
				// can fit
				MipavUtil
						.displayError("Cannot paste VOI : Out of image bounds."); // in
																					// image
																					// in
																					// x
																					// and
				...
				return;
			} else if ((newVOI.getCurveType() == VOI.POLYLINE_SLICE)
					&& (compImage.getActiveImage().getNDims() < 3)) {
				MipavUtil
						.displayError("Cannot paste VOI : Polyline-Slice VOIs are restricted to 3D+ images");
				return;
			}
			...
			do {
				...
				for (int i = 0; i < compImage.getActiveImage().getVOIs().size(); i++) {
					...
					if (ID == ((VOI) compImage.getActiveImage().getVOIs()
							.elementAt(i)).getID()) {
						ID++;
						test = 0;
					}
			...
			if (doLPS) {
				...
				newVOI.setColor(Color
						.getHSBColor(hue, (float) 1.0, (float) 1.0));
				...
				int currentSlice = compImage.getSlice();
				outVOI = new VOI((short) ID,
						newVOI.getName() + "_pasted_" + ID, zExt, newVOI
								.getCurveType(), hue);

				/**
				 * special case for doing polyline_slice VOIs... ordering is
				 * specific, not slice-based
				 */
				if ((newVOI.getCurveType() == VOI.POLYLINE_SLICE)) {
					...
					int adjZ = (int) ((VOI.PolyPointHolder) polyVector
							.elementAt(0)).getPoint().Z;
					int diff = currentSlice - adjZ;

					for (int i = 0; i < polyVector.size(); i++) {
						oldPt = ((VOI.PolyPointHolder) polyVector.elementAt(i))
								.getPoint();
						xt[0] = oldPt.X;
						yt[0] = oldPt.Y;
						zt[0] = oldPt.Z + diff;
					...
					VOIText vText = (VOIText) newVOI.getCurves()[(int) z[0]]
							.elementAt(0);
					...
					Vector3f pt1 = new Vector3f(oldPt1.X, oldPt1.Y,
							currentSlice);
					Vector3f pt2 = new Vector3f(oldPt2.X, oldPt2.Y,
							currentSlice);
					...
					for (int cSlice = (int) z[0]; cSlice <= z[1]; cSlice++, currentSlice++) {
						...
						// (currentSlice));
						outVOI.importNewVOI(currentSlice, cSlice, newVOI, zExt,
								false);
					}
	 ...
	 * 
	 * @param direction
	 *            if positive propagate upward, if negative propagate downward
	 * @param active
	 *            Whether or not to propogate a levelset VOI.
	 * 
	...
	public boolean propVOI(int direction, boolean active) {
		...
		if ((compImage.getSlice() < (compImage.getActiveImage().getExtents()[2] - 1))
				&& (direction > 0)) { // Propagate up
			nVOI = VOIs.size();
			...
			for (i = 0; i < nVOI; i++) {
				...
				if ((VOIs.VOIAt(i).isActive() == true)
						&& ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) || (VOIs
								.VOIAt(i).getCurveType() == VOI.POLYLINE))) {
					gons = VOIs.VOIAt(i).exportPolygons(compImage.getSlice());

					break;
				} else if ((VOIs.VOIAt(i).isActive() == true)
						&& (VOIs.VOIAt(i).getCurveType() == VOI.POINT)) {
			...
			if (!doPoint) {
				...
				if (active) {
					...
					if (VOIs.VOIAt(i).getLevel() == VOIs.VOIAt(i).NOT_A_LEVELSET) {
						MipavUtil
								.displayError("You can only use active propogation with a levelset VOI.");

						return false;
					} else {
						RubberbandLevelSet rubber = new RubberbandLevelSet(
								compImage);

						removeMouseListener(rubber);
						removeMouseMotionListener(rubber);
						compImage.show(compImage.getTimeSlice(), compImage
								.getSlice() + 1, null, null, true, compImage
								.getInterpMode());

						Polygon gon = rubber.findNextLevelSet(VOIs.VOIAt(i)
								.getLevel(), gons[j]);
						...
						VOIs.VOIAt(i).importPolygon(gon,
								compImage.getSlice() + 1);
						VOIs.VOIAt(i).setActive(true);
						updateVOIColor(VOIs.VOIAt(i).getColor(), VOIs.VOIAt(i)
								.getUID());
						VOIs.VOIAt(i).setLevel(rubber.getLevel());
						((VOIContour) (VOIs.VOIAt(i).getCurves()[compImage
								.getSlice() + 1].lastElement()))
								.setActive(true);
				...
				((VOIContour) (VOIs.VOIAt(i).getCurves()[compImage.getSlice() + 1]
				...
				for (j = 0; j < nCurves; j++) {
					...
					if (((VOIPoint) (curves[compImage.getSlice()].elementAt(j)))
							.isActive()) {
						break;
					}
				}
				...
				VOIs.VOIAt(i).setActive(true);
				updateVOIColor(VOIs.VOIAt(i).getColor(), VOIs.VOIAt(i).getUID());
				((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage.getSlice() + 1]
						.lastElement())).setActive(true);
				fireVOISelectionChange(VOIs.VOIAt(i), null);
	...
	public boolean propVOIAll() {
		...
		if (!doPoint) {
			...
			for (j = 0; j < nCurves; j++) {
				...
				if (((VOIContour) (curves[compImage.getSlice()].elementAt(j)))
						.isActive()) {
					break;
				}
			}
				...
				for (j = 0; j < VOIs.VOIAt(i).getCurves()[compImage.getSlice()]
					...
					if (compImage.getActiveImage().getNDims() == 3) {
						...
						if (compImage.getActiveImage().isColorImage() == true) {
							...
							for (int s = 0; s < compImage.getActiveImage()
									.getExtents()[2]; s++) {
								pt = ((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j)))
										.exportPoint();
							...
							graphPointVOI(VOIs.VOIAt(i), (VOIPoint) (VOIs
									.VOIAt(i).getCurves()[compImage.getSlice()]
									.elementAt(j)), j, false);
						} else {
						...
						for (int t = 0; t < compImage.getActiveImage()
								.getExtents()[3]; t++) {
							pt = ((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
									.getSlice()].elementAt(j))).exportPoint();
							ptPosition[t] = t;
							ptIntensity[t] = compImage.getActiveImage()
									.getFloat(
											(int) ((t * xDim * yDim * zDim)
						...
						graphPointVOI(VOIs.VOIAt(i),
								(VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
										.getSlice()].elementAt(j)), j, false);
					}
				}
			}
		}
	...
	/**
	 * Resets the rubberband live wire to null. When the rubberband live wire is
	 * null, the grad mag field is recaculated the next time live wire mode is
	 * selected.
	 */
	...
	/**
	 * Selects all contours of the currently active VOI (like holding down Shift
	 * and clicking on a VOI).
	 */
	 ...
	 * 
	 * @param doSelect
	 *            boolean select all or deselect all
	 */
			...
			if (VOIs.VOIAt(i).isActive() && (VOIs.VOIAt(i).isVisible() == true)) {
				...
				if ((VOIs.VOIAt(i).getCurveType() == VOI.POINT)
					...
					if (useFrameRefTime
						...
						for (int j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
								.getSlice()].size(); j++) {
	...
	/**
	 * Function called to set the mode/drawing state for the
	 * VOIHandler/VJCompEditImage The mode change will trigger cursor changes
	 * and rubberbanding actions, as well as prepare the mouse listening for
	 * actions (moving a VOI, adding points to a VOI etc).
	 * 
	 * @param mode
	...
	public void setMode(int mode) {
		...
		case ViewJComponentEditImage.DEFAULT:
			...
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setPointerSelected();
			}
		...
		case ViewJComponentEditImage.RECTANGLE:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("rectvoi");
			}
		...
		case ViewJComponentEditImage.ELLIPSE:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("ellipsevoi");
			}
		...
		case ViewJComponentEditImage.RECTANGLE3D:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("rect3dvoi");
			}
		...
		case ViewJComponentEditImage.POLYLINE:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("polyline");
			}
		...
		case ViewJComponentEditImage.LIVEWIRE:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("livewirevoi");
			}
		...
		case ViewJComponentEditImage.LEVELSET:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("levelsetvoi");
			}
		...
		case ViewJComponentEditImage.PROTRACTOR:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("protractor");
			}
		...
		case ViewJComponentEditImage.POLYLINE_SLICE_VOI:
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("polyslice");
			}
		...
		case ViewJComponentEditImage.ANNOTATION:
			...
			if (isImageFrame) {
				compImage.getFrame().getControls().getTools()
						.setToggleButtonSelected("TextVOI");
			}
			break;
	...
	/**
	 * Sets the mode to Live wire and sets the cost function of the live wire
	 * rubberband.
	 * 
	 * @param selection
	...
	/**
	 * Sets the mode to Livewire and provides parameters to the livewire cost
	 * function.
	 * 
	 * @param gradSigma
	 *            std dev of gaussian function for the gradient
	 * @param edgeLap
	 *            std dev of function for the edge lapacian
	 * @param gradWeight
	 *            weight to apply to gradient vector (gradWeight+2*kernWeight ==
	 *            1)
	 * @param kernWeight
	 *            weight to apply to the small kernel (gradWeight+2*kernWeight
	 *            == 1)
	 * @param smoothVOIFlag
	 *            whether to smooth out the voi contour after it is finished
	 */
	public void setModeLivewire(float gradSigma, float edgeLap,
			float gradWeight, float kernWeight, boolean smoothVOIFlag) {
		this.mode = ViewJComponentEditImage.LIVEWIRE;
		rubberband.setActive(false);
		rbLivewire = new RubberbandLivewire(this, new float[] { gradSigma,
				gradSigma }, new float[] { edgeLap, edgeLap }, kernWeight,
				gradWeight, smoothVOIFlag);
		rubberband = rbLivewire;
		rubberband.setActive(true);
		compImage.setCursor(compImage.crosshairCursor);
	}
	...
	/**
	 * Displays the dialog for calculating point area average intensities (for a
	 * fixed Point location (x,y)) , and pops up the associated graph when "OK"
	 * is pressed.
	 */
			...
			if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
				...
				for (int j = 0; j < VOIs.VOIAt(i).getCurves()[compImage
					...
					if (((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
						...
						pt = ((VOIPoint) (VOIs.VOIAt(i).getCurves()[compImage
								.getSlice()].elementAt(j))).exportPoint();
						new JDialogPointArea(compImage.getFrame(), compImage
	...
	/**
	 * If a VOI is selected the VOI properties dialog is displayed with the
	 * current properites of the selected VOI.
	 * 
	 */
	...
	public void showColorDialog() {
		...
		for (i = 0; i < nVOI; i++) {
			...
			if ((VOIs.VOIAt(i).isActive() == true)
					&& ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR)
							|| (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE)
							|| (VOIs.VOIAt(i).getCurveType() == VOI.POINT)
							|| (VOIs.VOIAt(i).getCurveType() == VOI.LINE) || (VOIs
							.VOIAt(i).getCurveType() == VOI.PROTRACTOR))) {
				break;
			} else if ((VOIs.VOIAt(i).isActive() == true)
					&& (VOIs.VOIAt(i).getCurveType() == VOI.ANNOTATION)) {
				i = -1;
		...
		colorChooser = new ViewJColorChooser(new Frame(), "Pick VOI color",
				new OKVOIColorListener(VOIs.VOIAt(i)), new ActionListener() {
	...
	/**
	 * Function (moved out from vjframeimage, for use by several classes) that
	 * determines if a compatible (current) voi is present (otherwise need a new
	 * one)
	 * 
	 * @param VOIs
	 * @param type
	...
	protected void showOverlay(Graphics g) {
		...
		if (compImage.getActiveImage().getFileInfo(0) instanceof FileInfoDicom) {
			...
			if (compImage.getSlice() >= 0) {
				fileInfo = (FileInfoDicom) (compImage.getActiveImage()
						.getFileInfo())[compImage.getSlice()];
			} else {
				fileInfo = (FileInfoDicom) (compImage.getActiveImage()
						.getFileInfo())[0];
			}
			...
			for (int i = 0; i < 16; i++) {
				...
				if (overlays[i] != null) {
					...
					if (i < 4) {
						compImage.drawStringBW(overlays[i], g, 5,
								(15 * (i + 1)));
					} else if ((i > 3) && (i < 8)) {
						compImage.drawStringBW(overlays[i], g, rightOffset
								- len, (15 * ((i % 4) + 1)));
					} else if ((i > 7) && (i < 12)) {
						compImage.drawStringBW(overlays[i], g, 5, bottomOffset
								- 45 + (i % 4 * 15));
					} else if (i > 11) {
						compImage.drawStringBW(overlays[i], g, rightOffset
			...
			compImage.drawGradicules(g, fileInfo.getResolutions()[0], fileInfo
					.getResolutions()[1]);
			...
			if (compImage.getSlice() >= 0) {
				fileInfo = compImage.getActiveImage().getFileInfo()[compImage
						.getSlice()];
			} else {
	...
	protected void updateVOIColor(Color voiColor, int voiUID) {
		...
		if (compImage.getFrame() instanceof ViewJFrameImage) {
			((ViewJFrameImage) compImage.getFrame()).getControls().setVOIColor(
					voiColor);
	 ...
	 * 
	 * @param voiUID
	 *            the ID of the last VOI selected
	 */
	...
	private String buildOverlayStrings(FileInfoBase inf, String name,
		...
		if (inf instanceof FileInfoDicom) {
			...
			if (attribString.equals(atts[0])) {
				...
				return resultStr
						+ Double.toString(compImage.getActiveImage().getMin());
			} else if (attribString.equals(atts[6])) {
				return resultStr
						+ Double.toString(compImage.getActiveImage().getMax());
	...
	/**
	 * Handles cycling through contours or VOIs left or right = cycle through
	 * contours up or down = cycle through VOIs.
	 * 
	 * @param keyCode
	 *            int directional arrow key
	 */
			...
			if (VOIs.VOIAt(i).isActive()) {
					...
					if (allActive) {
						...
						for (int sl = 0; sl < end; sl++) {
							...
							for (int j = 0; j < size; j++) {
								...
								if (((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
										.elementAt(j))).isActive()) {
									((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
											.elementAt(j))).setActive(false);
									...
									((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
											.elementAt(index))).setActive(true);

									Vector3f pt = ((VOIBase) (VOIs.VOIAt(i)
											.getCurves()[sl].elementAt(index)))
											.getActivePt();
				...
				// end if contourOnly
					...
					for (int sl = 0; sl < end; sl++) {
						...
						if (size > 0) {
							((VOIBase) (VOIs.VOIAt(index).getCurves()[sl]
									.elementAt(0))).setActive(true);

							Vector3f pt = ((VOIBase) (VOIs.VOIAt(index)
									.getCurves()[sl].elementAt(0)))
									.getActivePt();
	 ...
	 * 
	 * @param voiIndex
	 *            int index of active Line VOI
	 * @param contourIndex
	 *            int index of line's contour (can use zero)
	 */
	private void graphLineVOI(int voiIndex, int contourIndex) {
		...
		if (compImage.getActiveImage().isColorImage() == true) {
			...
			length = (int) (Math
					.sqrt(((lineX[1] - lineX[0]) * (lineX[1] - lineX[0]))
							+ ((lineY[1] - lineY[0]) * (lineY[1] - lineY[0]))));
			rgbPositions = new float[3][(length * 2) + 1];
			rgbIntensities = new float[3][(length * 2) + 1];

			for (int c = 0; c < 3; c++) {
				int pt = ((VOILine) (VOIs.VOIAt(voiIndex).getCurves()[compImage
						.getSlice()].elementAt(contourIndex)))
						.findPositionAndIntensityRGB(rgbPositions[c],
								rgbIntensities[c], c, compImage
										.getActiveImageBuffer(), compImage
										.getActiveImage().getResolutions(
												compImage.getSlice()),
								compImage.getActiveImage().getExtents()[0],
								compImage.getActiveImage().getExtents()[1]);
			...
			if (VOIs.VOIAt(voiIndex).getContourGraph() == null) {
				ViewJFrameGraph contourGraph = new ViewJFrameGraph(rgbPos,
						rgbInten, "Intensity Graph", VOIs.VOIAt(voiIndex),
						FileInfoBase.getUnitsOfMeasureAbbrevStr(compImage
								.getActiveImage().getFileInfo(0)
								.getUnitsOfMeasure(0)));
				...
				VOIs.VOIAt(voiIndex).getContourGraph().setUnitsInLabel(
						FileInfoBase.getUnitsOfMeasureAbbrevStr(compImage
								.getActiveImage().getFileInfo(0)
								.getUnitsOfMeasure(0)));
				VOIs.VOIAt(voiIndex).getContourGraph().saveNewFunction(rgbPos,
						rgbInten, contourIndex);
			}
	...
	private void handleVOIActivePt(int keyCode, boolean doMove) {
		...
		for (int i = 0; i < nVOI; i++) {
			...
			if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible()) {
				...
				if (((curveType == VOI.CONTOUR) || (curveType == VOI.POLYLINE)
						|| (curveType == VOI.LINE) || (curveType == VOI.POINT) || (curveType == VOI.POLYLINE_SLICE))) {
					...
					if (allActive) {
						...
						if (VOIs.VOIAt(i).getCurveType() == VOI.POLYLINE_SLICE) {
							...
							if (doMove) {
								...
								VOIs.VOIAt(i).cyclePSlicePt(
										compImage.getSlice(), keyCode);
								compImage.getActiveImage()
										.notifyImageDisplayListeners();
							}
						}
						...
						for (int sl = 0; sl < end; sl++) {
							...
							for (int j = 0; j < VOIs.VOIAt(i).getCurves()[sl]
								...
								if (((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
									...
									if (doMove) {
										((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
												.elementAt(j))).moveActivePt(
												keyCode, xDim, yDim);
									} else {
										((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
												.elementAt(j)))
												.cycleActivePt(keyCode);
									}
	 ...
	 * 
	 * @param keyCode
	 *            int (only up down left and right do anything)
	 */
					...
					if (allActive) {
						...
						for (int sl = 0; sl < end; sl++) {
							...
							for (int j = 0; j < VOIs.VOIAt(i).getCurves()[sl]
								...
								if (((VOIBase) (VOIs.VOIAt(i).getCurves()[sl]
										.elementAt(j))).isActive()) {
									VOIs.VOIAt(i).moveVOI(sl, xDim, yDim, zDim,
											distX, distY, 0);
	...
	/**
	 * Scales the points of the polygon so that it is displayed at the correct
	 * size.
	 * 
	 * @param gon
	...
	private Polygon zoomPolygon(Polygon gon, float zoomX, float zoomY) {
		...
		try {
			...
			System.gc();
			MipavUtil
					.displayError("Out of memory: ComponentEditImage.zoomPolygon");

			return null;
		}

		for (int i = 0; i < gon.npoints; i++) {
			zoomedGon.addPoint(MipavMath.round(gon.xpoints[i] * zoomX),
					MipavMath.round(gon.ypoints[i] * zoomY));
		}
	...
	/**
	 * Listener class (for Color Chooser) that updates the VOI color without the
	 * need of the VOI Properties dialog
	 * 
	 * @author linkb
	 * 
	 */
