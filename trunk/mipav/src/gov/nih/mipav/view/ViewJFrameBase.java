...
public abstract class ViewJFrameBase extends JFrame
    ...
    /**
     * Returns whether an image is a Minc label file by seeing if the file is Minc and then asking the user if it is a
     * label.
     *
    ...
    private boolean isMincLabelFile(ViewJComponentEditImage compImage, ModelImage image) {
        ...
        if ((image.getFileInfo(0).getFileFormat() == FileBase.MINC) &&
            ...
            int opt = JOptionPane.showConfirmDialog(this, "Is this a MINC label file?", "MINC label",
    ...
    /**
     * Takes in a MINC source image and a MINC label image, and returns an image which can be loaded as the MINC source
     * image's Image B. The reason we need this method is that MINC label images are small, only encompassing the
     * relevant information. Where the mask is relative to the source image is based on header information:
     * specifically, the start variables for each dimension. Another tricky aspect of the MINC label files is that the
     * orientation can be strange: for instance, an axial image which normally has the patient's x axis as the image x
     * axis and the patient's y axis as the image y axis can be reversed, so that the label image would appear to be
     * sideways.
     *
     ...
     */
    private ModelImage makeMinc(ModelImage srcImage, ModelImage labelImage) throws IOException {
        ModelImage destImage = new ModelImage(labelImage.getType(), srcImage.getExtents(), labelImage.getImageName(),
                                              userInterface);
        ...
        if ((imageOri[0] != FileInfoBase.ORI_UNKNOWN_TYPE) && (labelOri[0] != FileInfoBase.ORI_UNKNOWN_TYPE)) {

            while (!((imageOri[0] == labelOri[0]) && (imageOri[1] == labelOri[1]) && (imageOri[2] == labelOri[2]))) {
                Preferences.debug("Image axis: " + imageOri[0] + " " + imageOri[1] + " " + imageOri[2] + "\n");
                Preferences.debug("Label axis: " + labelOri[0] + " " + labelOri[1] + " " + labelOri[2] + "\n");
                ...
                if (imageOri[2] == labelOri[2]) {
                    ...
                    if (imageOri[1] == labelOri[1]) {
                        ...
                        AlgorithmFlip flip = new AlgorithmFlip(labelImage2, AlgorithmFlip.Y_AXIS,
                                                               AlgorithmBase.NO_CANCEL);

                        flip.setActiveImage(false);
                        flip.run();
                    } else if (imageOri[1] == FileInfoBase.oppositeOrient(labelOri[1])) {
                        ...
                        if (imageOri[0] == labelOri[0]) {
                            ...
                            AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.Z_AXIS_PLUS,
                                                                      AlgorithmBase.NO_CANCEL);

                            rot.setActiveImage(false);
                            rot.run();
                            labelImage2 = rot.returnImage();
                            rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.Z_AXIS_PLUS,
                                                      AlgorithmBase.NO_CANCEL);
                            rot.setActiveImage(false);
                            rot.run();
                            labelImage2 = rot.returnImage();
                        ...
                        // Z_AXIS_PLUS
                        Preferences.debug("Rotate about the Z axis +90 degrees.\n");
                        ...
                        AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.Z_AXIS_MINUS,
                                                                  AlgorithmBase.NO_CANCEL);
                    ...
                    AlgorithmInverseOrder inv = new AlgorithmInverseOrder(labelImage2, AlgorithmBase.NO_CANCEL);
                    ...
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.Y_AXIS_MINUS,
                                                              AlgorithmBase.NO_CANCEL);
                    ...
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.Y_AXIS_PLUS,
                                                              AlgorithmBase.NO_CANCEL);
                    ...
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.X_AXIS_PLUS,
                                                              AlgorithmBase.NO_CANCEL);
                    ...
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2, AlgorithmRotate.X_AXIS_MINUS,
                                                              AlgorithmBase.NO_CANCEL);
        ...
        labelStart = ((FileInfoMinc) labelImage2.getFileInfo(0)).getStartOrient();
        ...
        // yDiff must be adjusted because MINC's origin is the lower left-hand corner and ours is the upper left-hand
        // corner
        yDiff = (int) (((srcImage.getExtents()[1] - 1) * srcImage.getFileInfo(0).getResolutions()[1]) - yDiff);

        // adjust again because MINC counts positively from the lower and we count positively from the upper
        yDiff = yDiff - (int) ((labelImage2.getExtents()[1] - 1) * labelImage2.getFileInfo(0).getResolutions()[1]);
        ...
        // then just create an image the same size as the source image, and put the label image in at the appropriate
        // places
        Preferences.debug("After transform, xDiff = " + xDiff + " yDiff = " + yDiff + " zDiff = " + zDiff + "\n");
        ...
        for (int i = zDiff; i < (zDiff + labelImage2.getExtents()[2]); i++) {
            ...
            for (int j = yDiff; j < (yDiff + labelImage2.getExtents()[1]); j++) {
                ...
                for (int k = xDiff; k < (xDiff + labelImage2.getExtents()[0]); k++) {

                    if (labelBuffer[index] > 0) {
                        destBuffer[(i * bufferSize) + (j * rowLength) + k] = labelBuffer[index];
                    }
        ...
        for (int i = 0; i < destImage.getExtents()[2]; i++) {
            FileInfoBase fileInfo1 = (FileInfoBase) labelImage2.getFileInfo(0).clone();
