...
public class GetPath {
   ...
   /**
    **  Determine the best path for the indicated file.  The
    **  best path depends on the purpose: reading or writing
    **  the file.  When reading a file, first check the user's
    **  home for a user specific version of the file.  If no file
    **  is found there, then try the working directory (user.dir),
    **  if the file still can't be found, then look in the class path,
    **  and lastly look in the directory containing the main class.
    **
    **  When writing a file, the file should be saved in the user's
    **  home directory (user.home).  The user should always have write
    **  permissions here and this allows for different preferences for
    **  different users.
    **
   ...
   public static String getPath(String fileName, int purpose) {
      ...
      if (purpose == GetPath.FOR_READING) {
         ...
         if (!found) {
            ...
            if (appTmp2 != null) {
               appName = new String(
                   appTmp2.substring(0, 1).toUpperCase()
         ...
         if (!found) {
            Preferences.debug("GetPath.getPath: Unable to write " + fileName + ": " + userHome + " mipav directory not found." + "\n");
         }
