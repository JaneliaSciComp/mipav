...
/**
 * <code>SRBFilePane</code> is the SRB version of sun.swing.FilePane, most of
 * the codes here are copied from sun.swing.FilePane.
 * 
 * @version 1.0, 04/04/06
 * @author Hailong Wang
 */
public class SRBFilePane extends JPanel implements PropertyChangeListener {
    // Constants for actions. These are used for the actions' ACTION_COMMAND_KEY
    // and as keys in the action maps for SRBFilePane and the corresponding UI
    // classes
    ...
    private SRBFile newFolderFile;

    // Used for accessing methods in the corresponding UI class
    private SRBFileChooserUIAccessor fileChooserUIAccessor;

    public SRBFilePane(SRBFileChooserUIAccessor fileChooserUIAccessor) {
    ...
    protected SRBFileChooser getFileChooser() {
        return fileChooserUIAccessor.getFileChooser();
    }

    protected BasicSRBDirectoryModel getModel() {
    ...
    private static void recursivelySetInheritsPopupMenu(Container container,
        ...
        for (int i = 0; i < n; i++) {
            recursivelySetInheritsPopupMenu((Container) container
                    .getComponent(i), b);
        }
    }
    ...
    public void setViewPanel(int viewType, JPanel viewPanel) {
        ...
        case VIEWTYPE_DETAILS:
            detailsTable = (JTable) findChildComponent(viewPanels[viewType],
                    JTable.class);
            detailsTable.setRowHeight(Math.max(
                    detailsTable.getFont().getSize() + 4, 16 + 1));
            if (listSelectionModel != null) {
                detailsTable.setSelectionModel(listSelectionModel);
            }
            break;
    ...
    protected void installDefaults() {
        ...
        listViewBackground = UIManager
                .getColor("FileChooser.listViewBackground");
        listViewWindowsStyle = UIManager
                .getBoolean("FileChooser.listViewWindowsStyle");
        readOnly = UIManager.getBoolean("FileChooser.readOnly");

        // TODO: On windows, get the following localized strings from the OS

        viewMenuLabelText = UIManager.getString(
                "FileChooser.viewMenuLabelText", l);
    ...
    /**
     * Fetches the command list for the SRBFilePane. These commands are useful
...
public Action[] getActions() {      
    if (actions == null) {
        class SRBFilePaneAction extends AbstractAction {
        SRBFilePaneAction(String name) {
            this(name, name);
        }

        SRBFilePaneAction(String name, String cmd) {
            super(name);
            putValue(Action.ACTION_COMMAND_KEY, cmd);
        }
        ...
        actionList.add(new SRBFilePaneAction(ACTION_CANCEL));
        actionList.add(new SRBFilePaneAction(ACTION_EDIT_FILE_NAME));
        actionList.add(new SRBFilePaneAction(refreshActionLabelText, ACTION_REFRESH));
    ...
    public JPanel createList() {
        ...
        final SRBFileChooser fileChooser = getFileChooser();
        final JList list = new JList() {
            public int getNextMatch(String prefix, int startIndex,
                ...
                for (int i = startIndex; backwards ? i >= 0 : i < max; i += (backwards ? -1
                        : 1)) {
                    String filename = fileChooser.getName((SRBFile) model
                            .getElementAt(i));
        ...
        if (listFont != null) {
            // Make sure the font is not a UIResource to avoid the ListUI
            // overriding it
        ...
        getModel().addListDataListener(new ListDataListener() {
            ...
            public void intervalAdded(ListDataEvent e) {
                ...
                int i1 = e.getIndex1();
                if (i0 == i1) {
                    SRBFile file = (SRBFile) getModel().getElementAt(i0);
    ...
    class DetailsTableModel extends AbstractTableModel implements
        ...
        SRBFileChooser chooser;

        ListModel listModel;

        DetailsTableModel(SRBFileChooser fc) {
            this.chooser = fc;
            listModel = getModel();
        ...
        public Object getValueAt(int row, int col) {
            ...
            SRBFile f = (SRBFile) listModel.getElementAt(row);
            switch (col) {
            case COLUMN_FILENAME:
                return f;
        ...
        public void setValueAt(Object value, int row, int col) {
            if (col == COLUMN_FILENAME) {
                ...
                SRBFile f = (SRBFile) getValueAt(row, col);
                if (f != null) {
                    ...
                    if (!newDisplayName.equals(oldDisplayName)) {
                        ...
                        SRBFile f2 = fsv.createFileObject((SRBFile)f.getParentFile(), newFileName);
                        if (!f2.exists() && SRBFilePane.this.getModel().renameFile(f, f2)) {
                            if (fsv.isParent(chooser.getCurrentDirectory(), f2)) {
                                if (chooser.isMultiSelectionEnabled()) {
                                    chooser.setSelectedFiles(new SRBFile[] { f2 });
                                } else {
        ...
        public boolean isCellEditable(int row, int column) {
            SRBFile currentDirectory = getFileChooser().getCurrentDirectory();
    ...
    class DetailsTableCellRenderer extends DefaultTableCellRenderer {
        ...
        DetailsTableCellRenderer(SRBFileChooser chooser) {
        ...
        public Component getTableCellRendererComponent(JTable table,
            ...
            if (listViewWindowsStyle
                    && (table.convertColumnIndexToModel(column) != COLUMN_FILENAME || !table
                            .isFocusOwner())) {
        ...
        public void setValue(Object value) {
            setIcon(null);
            if (value instanceof SRBFile) {
                SRBFile file = (SRBFile) value;
    ...
    public JPanel createDetailsView() {
        final SRBFileChooser chooser = getFileChooser();
            ...
            public Component prepareRenderer(TableCellRenderer renderer,
                ...
                if (comp instanceof JLabel) {
                    if (convertColumnIndexToModel(column) == COLUMN_FILESIZE) {
                        // Numbers are right-adjusted, regardless of component
                        // orientation
                        ((JLabel) comp)
        ...
        detailsTable.setDefaultRenderer(SRBFile.class, cellRenderer);
        ...
        if (!readOnly) {
            ...
            columns[COLUMN_FILENAME].setCellEditor(new DefaultCellEditor(tf) {
                ...
                public Component getTableCellEditorComponent(JTable table,
                    ...
                    if (value instanceof SRBFile) {
                        tf.setText(chooser.getName((SRBFile) value));
                        tf.requestFocus();
                        tf.selectAll();
                    }
                    return comp;
        ...
        if (listViewWindowsStyle) {
            // 4835633.
            // If the mouse is pressed in the area below the Details view table,
            // the
    ...
    private class DelayedSelectionUpdater implements Runnable {
        ...
        DelayedSelectionUpdater(SRBFile editFile) {
    ...
    private void editFileName(int index) {
        ...
        switch (viewType) {
        case VIEWTYPE_LIST:
            editFile = (SRBFile) getModel().getElementAt(index);
    ...
    private void applyEdit() {
        if (editFile != null && editFile.exists()) {
            ...
            if (!newDisplayName.equals(oldDisplayName)) {
                ...
                // rename
                SRBFileSystemView fsv = chooser.getFileSystemView();
                SRBFile f2 = fsv.createFileObject((SRBFile)editFile.getParentFile(),
                        newFileName);
            ...
            newFolderAction = new AbstractAction(newFolderActionLabelText) {
                ...
                // Initializer
                {
                    putValue(Action.ACTION_COMMAND_KEY,
                            SRBFilePane.ACTION_NEW_FOLDER);

                    SRBFile currentDirectory = getFileChooser()
                            .getCurrentDirectory();
                ...
                public void actionPerformed(ActionEvent ev) {
                    if (basicNewFolderAction == null) {
                        basicNewFolderAction = fileChooserUIAccessor
                                .getNewFolderAction();
                    }
                    SRBFileChooser fc = getFileChooser();
                    SRBFile oldFile = fc.getSelectedFile();
                    basicNewFolderAction.actionPerformed(ev);
                    SRBFile newFile = fc.getSelectedFile();
    ...
    protected class FileRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            if (icon != null) {
                ...
                if (getFileChooser().getFileSystemView().isTraversable(file).booleanValue()) {
                    setText(fileName + SRBFile.PATH_SEPARATOR);
                }
            }
    ...
    void setFileSelected() {
        if (getFileChooser().isMultiSelectionEnabled()
                && !isDirectorySelected()) {
            SRBFile[] files = getFileChooser().getSelectedFiles(); // Should be
            // selected
            ...
            try {
                ...
                while (shouldIndex < files.length
                    ...
                    int comparison = files[shouldIndex]
                            .compareTo((SRBFile) selectedObjects[actuallyIndex]);
                    if (comparison < 0) {
    ...
    private void doSelectedFileChanged(PropertyChangeEvent e) {
        applyEdit();
        SRBFile f = (SRBFile) e.getNewValue();
        SRBFileChooser fc = getFileChooser();
        if (f != null
    ...
    private void doSelectedFilesChanged(PropertyChangeEvent e) {
        applyEdit();
        SRBFile[] files = (SRBFile[]) e.getNewValue();
        SRBFileChooser fc = getFileChooser();
        if (files != null
                && files.length > 0
    ...
    private void doDirectoryChanged(PropertyChangeEvent e) {
        SRBFileChooser fc = getFileChooser();
        SRBFileSystemView fsv = fc.getFileSystemView();
    ...
    public void propertyChange(PropertyChangeEvent e) {
        ...
        if (s.equals(SRBFileChooser.SELECTED_FILE_CHANGED_PROPERTY)) {
            doSelectedFileChanged(e);
        } else if (s.equals(SRBFileChooser.SELECTED_FILES_CHANGED_PROPERTY)) {
            doSelectedFilesChanged(e);
        } else if (s.equals(SRBFileChooser.DIRECTORY_CHANGED_PROPERTY)) {
            doDirectoryChanged(e);
        } else if (s.equals(SRBFileChooser.FILE_FILTER_CHANGED_PROPERTY)) {
            doFilterChanged(e);
        } else if (s.equals(SRBFileChooser.FILE_SELECTION_MODE_CHANGED_PROPERTY)) {
            doFileSelectionModeChanged(e);
        } else if (s
                .equals(SRBFileChooser.MULTI_SELECTION_ENABLED_CHANGED_PROPERTY)) {
            doMultiSelectionChanged(e);
        } else if (s.equals(SRBFileChooser.CANCEL_SELECTION)) {
            applyEdit();
        } else if (s.equals("componentOrientation")) {
            ComponentOrientation o = (ComponentOrientation) e.getNewValue();
            SRBFileChooser cc = (SRBFileChooser) e.getSource();
    ...
    public static boolean canWrite(SRBFile f) {
    ...
    // that are not public.
    public interface SRBFileChooserUIAccessor {
