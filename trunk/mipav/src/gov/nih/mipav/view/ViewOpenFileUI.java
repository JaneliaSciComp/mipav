...
/**
 * Opens an image file by calling FileIO.readImage and
 * puts it into an image frame.
 *               @version    0.1 Sept 4, 1997
 *               @author     Matthew J. McAuliffe, Ph.D.
 *               @see            FileIO
 */
public class ViewOpenFileUI
    extends ViewFileChooserBase {

    /**
     *   Indicate what the current filter should be on opening the
     *   the dialog.  The values for this are the static ints in ViewImageFileFilter.
     */
     ...
     *    @param openDialog a boolean that selects whether this UI should be built
     *      and displayed.  <code>true</code> indicates the UI will be displayed,
     *      and <code>false</code> is that it will not be displayed.
     */
     ...
     *   @param  filterType  Indicate what the current filter should be on opening the
     *                       the dialog.  The values for this are the static ints in
     *                       ViewImageFileFilter.
     */
     ...
     *   @param multiFile        Flag to indicate if image is an array of 2D image files (<code>true</code>) or
     *                           if the image is stored in a single file (<code>false</code>).
    ...
    public Vector open(boolean multiFile, boolean imageset) {
        ...
        try {
            ...
            if (linkedImage != null) {
                ...
                if ( ( (image.getFileInfo(0).getModality() == FileInfoBase.RED_FREE &&
                        linkedImage.getFileInfo(0).getModality() ==
                        FileInfoBase.ICG) ||
                      (image.getFileInfo(0).getModality() == FileInfoBase.ICG &&
                       linkedImage.getFileInfo(0).getModality() ==
                       FileInfoBase.RED_FREE)) &&
                    !imageset) {
        ...
        if (secondImage != 0) {
            ...
            if (image2 != null) {
                ...
                try {
                    if (putInFrame == true) {
                        new ViewJFrameImage(image2, LUT2,
        ...
        if (secondImage != 0) {
            ...
            if (UI.isScriptRecording()) {
                if (image2.getFileInfo(0).getFileFormat() != FileBase.RAW &&
                    ...
                    UI.getScriptDialog().putVar(image2.getImageName());
                    if (multiFile) {
                        UI.getScriptDialog().append("OpenMultiFile " + UI.getScriptDialog().getVar(image2.getImageName()) +
                            "\n");
                    }
                    else {
                        UI.getScriptDialog().append("OpenImage " + UI.getScriptDialog().getVar(image2.getImageName()) +
