...
public class ViewJFrameCardiology extends ViewJFrameBase implements KeyListener {
     ...
     */
    public ViewJFrameCardiology(final ModelImage _imageA, final ModelLUT LUTa, final Dimension loc) {
    ...
    public void actionPerformed(final ActionEvent event) {
        ...
        if (command.equals("MagImage")) {
            final float zoom = 2.0f * componentImage.getZoomX();

            updateFrame(zoom, zoom);
        } else if (command.equals("UnMagImage")) {
            final float zoom = 0.5f * componentImage.getZoomX();
    ...
    public synchronized void componentResized(final ComponentEvent event) {
        ...
        if ( (getSize().width >= (ViewJFrameCardiology.xScreen - 20))
                || (getSize().height >= (ViewJFrameCardiology.yScreen - 20))) {
            return;
        }
        ...
        zoom = (int) Math.min( (bigger - 1) / ( (imageA.getExtents()[0] * widthResFactor) - 1), (bigger - 1)
                / ( (imageA.getExtents()[1] * heightResFactor) - 1));
        ...
        if (zoom > componentImage.getZoomX()) {
            ...
            if ( (componentImage.getSize(null).width + 200) > ViewJFrameCardiology.xScreen) {
                width = ViewJFrameCardiology.xScreen - 200;
            } else {
                width = componentImage.getSize(null).width /* + fudgeFactor */;
            }

            if ( (componentImage.getSize(null).height + 200) > ViewJFrameCardiology.yScreen) {
                height = ViewJFrameCardiology.yScreen - 200;
            } else {
    ...
    /*
     * public boolean setZoomA() { int xDim = imageA.getExtents()[0]; int yDim = imageA.getExtents()[1]; float
     * imageTemp[] = null; int pixBufferTemp[] = null; int paintBufferTemp[] = null; try { if (imageA != null) { int
     * bufferFactor = 1; if ( imageA.isColorImage() ) { bufferFactor = 4; } imageTemp = new float [bufferFactor * xDim *
     * yDim]; pixBufferTemp = new int[xDim * yDim]; paintBufferTemp = new int[xDim * yDim]; } } catch (OutOfMemoryError
     * error){ imageTemp = null; pixBufferTemp = null; paintBufferTemp = null; System.gc(); throw(error); } imageBufferA =
     * imageTemp; pixBuffer = pixBufferTemp; paintBuffer = paintBufferTemp; componentImage.setBuffers(imageBufferA,
     * imageBufferB, pixBuffer, paintBuffer); return true; }
     */
     ...
     */
    public void setImageB(final ModelImage imageB) {}
     ...
     */
    public void setLastVOI_UID(final int uid) {
    ...
    public void updateFrame(final float sX, final float sY) {
        ...
        if ( ( (componentImage.getSize(null).width + 200) > ViewJFrameCardiology.xScreen)
                || ( (componentImage.getSize(null).height + 200) > ViewJFrameCardiology.yScreen)) {
            ;
            ...
            final Dimension size = getFrameSize();
            ...
            if ( ( (componentImage.getSize(null).width + 200) < ViewJFrameCardiology.xScreen)
                ...
                removeComponentListener(this);
                scrollPane.setSize(getSize().width - getInsets().left - getInsets().right - 1, getSize().height
                        - getInsets().top - getInsets().bottom - 1);
                addComponentListener(this);
            }
        }
    ...
    protected static float initZoom(final ModelImage img, final float xResFactor, final float yResFactor) {
        ...
        if ( (img.getExtents()[0] * xResFactor) > (ViewJFrameCardiology.xScreen - 300)) {
            zoomX = (ViewJFrameCardiology.xScreen - 300.0f) / (img.getExtents()[0] * xResFactor);
        }

        if ( (img.getExtents()[1] * yResFactor) > (ViewJFrameCardiology.yScreen - 300)) {
            zoomY = (ViewJFrameCardiology.yScreen - 300.0f) / (img.getExtents()[1] * yResFactor);
        }
    ...
    protected int[] createBuffers() throws OutOfMemoryError {
        final int[] extents = ViewJFrameCardiology.initExtents(imageA);

        imageBufferA = ViewJFrameCardiology.initImageBuffer(extents, imageA.isColorImage());
        pixBuffer = ViewJFrameCardiology.initPixelBuffer(extents);
     ...
     */
    protected void initExtentsVariables(final ModelImage img) {
        int[] slices = null;
        int[] numImages = null;

        slices = ViewJFrameCardiology.initSlicePositions(img);
        numImages = ViewJFrameCardiology.initNumSlices(img);
    ...
    protected void initLUT() throws OutOfMemoryError {
        ...
        if (LUTa == null) {
            LUTa = ViewJFrameCardiology.initLUT(imageA);
        }
    }
    ...
    protected void initResolutions() {
        resols = ViewJFrameCardiology.initResolutions(imageA);
        units = ViewJFrameCardiology.initUnits(imageA);

        final float[] factor = ViewJFrameCardiology.initResFactor(resols, units);
    ...
    protected void initZoom() {
        zoom = ViewJFrameCardiology.initZoom(imageA, widthResFactor, heightResFactor);
    } // end initZoom()
     ...
     * @return DOCUMENT ME!
     */
    private boolean checkForVOICompatibility(final int type) {

        final int numVOI = getActiveImage().getVOIs().size();
        int lastType = -1;

        if (numVOI == 0) {
            return true;
        } else if (numVOI == 1) {
            lastType = ( (getActiveImage().getVOIs().elementAt(0))).getCurveType();
        } else {

            for (int i = 0; i < numVOI; i++) {

                if ( ( (getActiveImage().getVOIs().elementAt(i))).getUID() == lastVOI_UID) {
                    lastType = ( (getActiveImage().getVOIs().elementAt(i))).getCurveType();
        ...
        switch (type) {
            ...
            case VOI.POLYLINE:
                if ( (lastType != VOI.CONTOUR) && (lastType != VOI.POLYLINE)) {
                    final int id = ( ( (getActiveImage().getVOIs().lastElement())).getID() + 1);
    ...
    private void init(final ModelLUT LUTa, final Dimension loc, final boolean logMagDisplay) throws OutOfMemoryError {
        ...
        controls.buildToolbar(menuBuilder.isMenuItemSelected("Image toolbar"), false, false, false, componentImage
                .getVOIHandler().getVOI_ID());
     ...
     * @throws OutOfMemoryError if enough memory cannot be allocated for this method
     */
    private void initComponentImage(final int[] extents) throws OutOfMemoryError {
     ...
     */
    private ModelImage prepareImages(ModelImage image2load, final ModelLUT lut, boolean isFromFrame) {
        ...
        if (getActiveImage().isColorImage() && !image2load.isColorImage()) {
            ...
            tempImage = new ModelImage(ModelStorageBase.ARGB, image2load.getExtents(), JDialogBase.makeImageName(
                    image2load.getImageName(), "_rgb"));
            ...
            final float redValue = 1.0f / 3.0f;
            final float greenValue = 1.0f / 3.0f;
            final float blueValue = 1.0f / 3.0f;
            final float threshold = 1.0f;

            try {

                if (image2load.getType() == ModelStorageBase.ARGB) {
                    tempImage = new ModelImage(ModelStorageBase.UBYTE, image2load.getExtents(), (image2load
                            .getImageName() + "Gray"));
                } else if (image2load.getType() == ModelStorageBase.ARGB_USHORT) {
                    tempImage = new ModelImage(ModelStorageBase.USHORT, image2load.getExtents(), (image2load
                            .getImageName() + "Gray"));
                } else if (image2load.getType() == ModelStorageBase.ARGB_FLOAT) {
                    tempImage = new ModelImage(ModelStorageBase.FLOAT, image2load.getExtents(), (image2load
                            .getImageName() + "Gray"));
                }
                ...
                // Make algorithm
                final AlgorithmRGBtoGray RGBAlgo = new AlgorithmRGBtoGray(tempImage, image2load, redValue, greenValue,
        ...
        if ( ( (dims[0] != image2load.getExtents()[0]) || (dims[1] != image2load.getExtents()[1]))) {
            ...
            TransMatrix xfrm;
            final Vector3f center = image2load.getImageCentermm(false);
            ...
            if ( !isFromFrame) {
                ...
                final ViewJFrameImage newFrame = new ViewJFrameImage(image2load, lut);
                ...
                final ViewJFrameImage newFrame = new ViewJFrameImage(tempImage, lut);
     ...
     */
    private ViewJFrameCardiology subSampleImage(final ModelImage _imageA, final ModelImage _imageB) {
        int[] extents;
        float[] res;
        final int[] volExtents = new int[3];
        final float[] resols = new float[3];
        boolean forceResample = false;
        final ViewJFrameCardiology tempFrame;
        ...
        AlgorithmTransform transformImg = new AlgorithmTransform(imageA, new TransMatrix(4),
                AlgorithmTransform.TRILINEAR, resols[0], resols[1], resols[2], volExtents[0], volExtents[1],
