...
/** A collection of static methods and frequently used and useful constants
 *  are in this utility class.
 */
public class MipavUtil extends JComponent {
    ...
    public static final int[] functionKeys = new int[] {
        0, KeyEvent.VK_F1, KeyEvent.VK_F2, KeyEvent.VK_F3, KeyEvent.VK_F4, KeyEvent.VK_F5,
    ...
    /**
     *  Finds the icon of the specified name. Uses the PlaceHolder
     *  class, which is in the same directory as the icons, to locate
     *  the icons.
    ...
    /**
     *  Finds the image of the specified name. Uses the PlaceHolder
     *  class, which is in the same directory as the icons images,
     ...
     *  <p>
     *  Use when an operation has failed, preventing some operation critical
     *  for MIPAV to continue running normally or an operation can neither
     *  be completed nor its errors accomodated.
    ...
    public static void displayError( String error ) {
        if (ViewUserInterface.getReference() != null && ViewUserInterface.getReference().isAppFrameVisible() == true) {
            try {
     ...
     *  <p>
     *  Use when an operation has failed, but the can be completed
     *  but the output may display inaccuratly; ie., errors can be
     *  accomodated.
     ...
     *  <p>
     *  Use for brief notices such as completion of an operation or where
     *  to find logging information.  It is not meant to notify a user of
    ...
    /**
     *  Displays the Java Help dialog indexed directly to the section
     *  identified by the ID passed in.
     *  @param ID the index ID indicating what section the Java Help
     *           dialop should display.
     */
    ...
    /**
     *   Finds the help set file within the JAR file. The help set
     *   file should end in ".hs"
    ...
    /**
     * Takes a text field and forces the text field to accept numbers,
     * backspace and delete-key entries.
     * @param   txt     Text field to modify.
     * @param  allowFloatingPoint  <code>true</code> will force the text field
     *                             to also allow the use of the '.' key to
     ...
     *  numbers, backspace and delete-key entries.  Arguments to the method
     *  can permit the text field to also allow the entry to be negative or
     *  give it a floating point decimal value.  All other characters are
     *  ignored, so if further action is required, it will need to be
     *  custom-assigned by the calling object.
     *  @param  txt                  the text field to make only accept numeric text
     *  @param  allowFloatingPoint   true will forces the text field to also
     *                               allow the use of the '.' key to permit
     *                               entering floating point numbers
     *  @param  allowNegativeNumbers true causes textfield to allow the entry
     *                               of a single minus sign ('-') in front
     *                               of the text.  An additional '-' removes
     *                               minus sign from the field to give it the
     *                               opposite sign.
     */
    public static void makeNumericsOnly(JTextField txt,
                                        boolean allowFloatingPoint,
        ...
        if ( allowFloatingPoint && allowNegativeNumbers ) {
                        ...
                        if ( t.getSelectedText() != null ) {
                            if (t.getText().length() == t.getSelectedText().length()) {
                                ...
                                evt.consume();
                                    ...
                                    (t.getSelectedText().indexOf('.') == -1))
                            { // there is a '.', but not in the selected text
                                evt.consume(); // ignore
                            }
                        }
                        else if ( t.getText().indexOf('.') != -1 ) {
            ...
            txt.addKeyListener(
                ...
                public void keyTyped( KeyEvent evt ) { // not accept letters
                    ...
                    /* else if (ch == '-') {...}*/ // negatives are not allowed
                    else if ( (( ch < '0' ) || ( ch > '9' ) ) &&
                              ( ch != KeyEvent.VK_DELETE) &&
                              (ch != KeyEvent.VK_BACK_SPACE))
                    {
            ...
            txt.addKeyListener(
                ...
                public void keyTyped( KeyEvent evt ) { // not accept letters
                    ...
                    /* else if (ch == '.') {..}*/ // decimal not allowed
                    if (ch == '-') {
                        String text = t.getText().trim();
                        int minusPlace = text.indexOf( '-' );
            ...
            txt.addKeyListener(
                ...
                public void keyTyped( KeyEvent evt ) { // not accept letters
                    ...
                    /* else if (ch == '.') {...}*/ // floating point not allowed
                    /* else if (ch == '-') {...}*/ // negatives are not allowed
    ...
    public static final void setFonts(Component [] comp) {
      ...
      {
          if(comp[x] instanceof Container) setFonts(((Container)comp[x]).getComponents());
          try{comp[x].setFont(MipavUtil.defaultMenuFont);}
  ...
  public static final void setComponentsEnabled(Container container, boolean enabled) {
     ...
     * @return String
     */
    public static final String makeHTMLFontString(Color color, String name, int size, int style,
                                                  boolean doU, String text) {
    ...
    public static Color extractColor( String preferencesColorString ) {
        ...
        try {
            if ( preferencesColorString != null ) {
                ...
                RGB[0] = Integer.parseInt(
                        String.valueOf( preferencesColorString.toCharArray()[0] )
                                + String.valueOf( preferencesColorString.toCharArray()[1] ),
                                16 );
                RGB[1] = Integer.parseInt(
                        String.valueOf( preferencesColorString.toCharArray()[2] )
                                + String.valueOf( preferencesColorString.toCharArray()[3] ),
                                16 );
                RGB[2] = Integer.parseInt(
                        String.valueOf( preferencesColorString.toCharArray()[4] )
                                + String.valueOf( preferencesColorString.toCharArray()[5] ),
                                16 );
            }
        } catch ( Exception npe ) {
    ...
    /**
     * This should only be called once when MIPAV starts, and then
     * if the user changes the font options through Program Options
     * to rebuild the fonts used in GUI building
     */
