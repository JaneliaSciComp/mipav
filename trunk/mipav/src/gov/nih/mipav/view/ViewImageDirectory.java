...
/**
 *   Tree of images beneath the given directory.  When a user clicks on
 *   a filename, a thumbnail of the image appears, along with the header
 *   data.
 *
...
public class ViewImageDirectory
   ...
   *   "Reset file filter", and "Open image" options.
   */
  ...
  protected JPanel buildToolbar() {
    ...
    JButton openToSeparateButton = new JButton(MipavUtil.getIcon(
        "openintoseparate.gif"));
    openToSeparateButton.addActionListener(this);
    openToSeparateButton.setToolTipText(
        "Open selected images into separate frames");
    ...
    openToSeparateButton.setRolloverIcon(MipavUtil.getIcon(
        "openintoseparateroll.gif"));
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (command.equals("New")) {
      ...
      //if there is only one image selected, open that in a separate frame
        ...
        fileName = ( (ViewFileTreeNode) selected[0].getLastPathComponent()).
            getName();
        directory = ( (ViewFileTreeNode) selected[0].getLastPathComponent()).
            getDirectory();
        io.setPBar(progressPanel);
        ModelImage image = io.readImage(fileName,
        ...
        for (i = 0; i < selected.length; i++) {
          newName = ( (ViewFileTreeNode) selected[i].getLastPathComponent()).
              getName();
          newDir = ( (ViewFileTreeNode) selected[i].getLastPathComponent()).
              getDirectory();
        ...
        /**
         * Continue through the hashtable, getting the image names and storing them
         * in an array, sorting them, then matching up any images of like dimensions
         * and opening them up into the same frame.  the names must be sorted so that files
         * with sequel number will be placed in the correct order (001, 002, 003 etc).
        ...
        if (!table.isEmpty()) {
          ...
          if (newImage != null) {
            ...
            else if (openSeparate == true) // open to separate frames
            {
              ...
              ModelImage concatImage = new ModelImage(newImage.getType(),
                  newExtents,
              ...
              for (i = 0; i < matchingImageNames.size(); i++) {
                ...
                newImage = (ModelImage) table.remove(matchingImageNames.
                    elementAt(i));
  ...
  protected TitledBorder buildTitledBorder(String title) {
    return new TitledBorder(new EtchedBorder(), title, TitledBorder.LEFT,
                            TitledBorder.CENTER, MipavUtil.font12B,
                            Color.black);
  }
