...
/**
 *		Progress bar used everywhere for displaying to the user how long the
 *		current process is going to take.  The progress bar looks like this:
 *		<pre>
 *		+-Title------------------------+
 *		| Message                      |
 *		| |--------Bar--------|	percent|
...
public class ViewJProgressBar
    extends JFrame
    implements ActionListener, ProgressBarInterface {
  ...
  /**
   *	Creates a new progress bar with the given title, message, and min and max.  The percentage
   *	is initially set to "0%".  The <code>cancelFlag</code> indicates if there should be a cancel
   *	button - usually this is used for algorithms, which can be stopped, but not loading images.
   ...
   *	@param cancelFlag		<code>true</code> indicates that the user can stop the process by clicking on
   *							the cancel button, and so the button should be added to the frame.
  ...
  public ViewJProgressBar(String _title, String msg, int min, int max,
    ...
    // prototype value is 100%, so that packing doesn't make the size of the label too small.
  ...
  /**
   * Use this method if you are not running in a separate thread, or, you have already set
   * the value of separateThread through the setSeparateThread method. Otherwise, use
   * updateValue(int, boolean);
   * @param value int
   */
  ...
  //  percentage.setText(String.valueOf(value) + "%");
  //  super.setTitle(title + "   " + String.valueOf(value) + "%");
    //update(this.getGraphics()); // Commmenting this out stops progress bar flicker !!!
 // }
   ...
   *	Be sure to provide appropriate leading spacing.
   *   @param msg   Message to be displayed.
   */
  ...
  /**
   *	Closes this progress bar when the cancel button is clicked.  This
   *	doesn't actually get called when the progress bar is running in the
   *	same thread.
