...
public class ActionChangeResolutions extends ActionImageProcessorBase {
    ...
    public void insertScriptLine() {
        ...
        try {
            ...
            parameters.put(ParameterFactory.newParameter(IMAGE_RESOLUTIONS, recordingInputImage.getResolutions(sliceIndex)));
        ...
        if (inputImage.getExtents().length == resolutions.length) {
        	if (changeAll) {
        		for (int i = 0; i < zDim; i++) {
        			...
        			if (isDicom) {
        				((FileInfoDicom) (inputImage.getFileInfo()[i])).setValue("0028,0030", dicomString, dicomString.length());
        			}
    			}
        	} else {
        		if ((nDims > 2 && sliceIndex < inputImage.getExtents()[2]) ||
        				sliceIndex == 0) {
        			...
        			if (isDicom) {
        				((FileInfoDicom) (inputImage.getFileInfo()[sliceIndex])).setValue("0028,0030", dicomString, dicomString.length());
        			}
        		}
        	}
        }
