...
public abstract class AlgorithmBase
    ...
    /** Vector list of AlgorithmInterface objects. When the algorithm has been
        stopped or completed, all listeners in this list are notified.  */
    ...
    /** String for successfully completed algorithms that will go
     * into the history of the model image that was changed (based on destFlag)
     */
     ...
     *  Should be set to false if in a single thread - will force a graphics update of the progress bar.
     */
    ...
    /**
     *   Default constructor which sets thread stopped to false, source
     *   and destination images to null, and destination flag to false.
     */
    ...
    /**
     * Sets the active image flag which controls how the progress bar is updated
     * If true the progress bar is not forced to update.
     * @param activeFlag Should be set to true if NOT in a single thread - will NOT force a graphics update of the progress bar.
     *                   Should be set to false if in a single thread - will force a graphics update of the progress bar.
     */
    ...
    /**
     *   Returns flag that indicates that the algorithm has been
     *   sucessfully completed.
     *   @return <code>true</code> if algorithm completed successfully,
     *           <code>false</code> if it was stopped or had an error.
     */
    ...
    /**
    * Sets the state of the keepProgressBar flag. If false (default state)
    * the progress bar is disposed in the finalized method.
    ...
    */
     ...
     */
    public boolean isKeepProgressBar() { return keepProgressBar; }
    ...
    /**
     *   Sets a flag to indicate the image has dimensions of (i.e. spacial, spacial, time) and
     *   image processing routines should apply 2.5D algorithm.
     *   @param flag    flag to set to
     */
    ...
    /**
     *   Display string pass into the method. If the string is null no message is displayed.
     *   The completed flag is set to <code>false</code> and the progress bar is disposed;
    ...
    /**
     *   Writes the logString to the appropriate history area.
     *   If algorithm was completed successfully, write the history string
     *   to the algorithm (and copy the source history into destination if
     *   there is a destination image)
     */
     ...
     *   If so, it returns false, else it starts the thread and returns true.
     ...
     * Since this class extends the Thread class it can be run in its own thread by invoking object.start();
     * It can also be invoked without a new thread by calling the the run() method directly (ie. object.run()).
     */
    public void run() {
        setStartTime();
     ...
     *   Useful for updating DICOM file information.
     *   @param  str     Some number of strings in an array
     *   @return         An array of floats; the same number of entries in str
    ...
    /**
     *   Add a listener to this class so that when
     *   when the algorithm has completed processing
     *   it can use notifyListener to notify all listeners
     *   that the algorithm has completed.
     *
    ...
    /**
     *   Sets the start time to the current time.  This should be
    ...
    /**
     *   Computes the elapased time as the difference between the
     *   start time and the current time (both of which are in milliseconds).
    ...
    /**
     *   Returns the elapsed time in seconds.  This does not compute a
     *   new elapsed time.
       ...
       public void actionPerformed(ActionEvent event) {
           if (progressBar != null && !progressBar.isComplete()) {
               ...
               if (Preferences.is(Preferences.PREF_LOG)) {
                  String tempStr = new String(this.getClass().getName().substring(
                                  this.getClass().getName().lastIndexOf(".")+1 )+ " failed.");
