 ...
 *  transform is performed, and the exponential of the the data is taken. Optionally,
 *  a specified percentage of the data can be clamped at the low end and/or at the
 *  high end.  Finally, the data is rescaled so that the original minimum and
 *  maximum values are achieved.  Only Butterworth filters are used with homomorphic
 *  filters in this module.  References on homomorphic filters:
 *  1.) Digital Image Processing Second Edition by Rafael C. Gonzalez and Richard E.
 *      Woods, Prentice-Hall, Inc., 2002, Chapter 4.5, pp. 191-194.
 *  2.) "Butterworth equations for homomorphic filtering of images" by Holger. G.
         Adelmann, Computers in Medicine and Biology, Vol. 28, 1998, pp. 169-181.
 *
 ...
 *  original dimensions after applying the inverse fast fourier transform.  The powers of 2 are
 *  not kept identical because symmetrical Fourier pictures are not required.
 *
 ...
 *  3.) realData and imagData are enlarged to the powers of two.
 ...
 *      number that equals or exceeds the original dimension size + kDim - 1, where
 ...
 *  8a.) The ideal kernel and the Hamming window kernel are multiplied together.
 *  9a.) The kernel is zero padded up to the same dimensions that the image data was.
 * 10a.) exec() is run to obtain the FFT of the kernel.
 ...
 * 6b.) The real part of the FFT is set equal to the appropriate filter magnitude and the
 *      imaginary part is set equal to zero.  This filter has the same dimensions as the
 *      padded image data.
 * 11.) The data FFT is set equal to the product of the data FFT and the filter FFT.
 * 12.) The inverse FFT process is invoked.
 ...
 * 13.) If Butterworth or Gaussian filters are used, the center() routine is invoked to
 *      restore the data to its original ordering.
 * 14.) exec() is invoked to run the inverse fast fourier transform algorithm.
 * 15.) The realData now holds the correct response if Gaussian or Butterworth filtering was used.
 ...
 *      only roundoff error.
 * 16.) For FIR filters shift the data back by (kDim - 1)/2 toward the start of each dimension.
 * 17.) Stripping is performed to return the image to its original dimensions.
...
public class AlgorithmFrequencyFilter extends AlgorithmBase {
    ...
    public final static int HOMOMORPHIC = 5;
    public final static int LAPLACIAN = 5;
	 ...
	 */
    public AlgorithmFrequencyFilter(ModelImage destImg, ModelImage srcImg, boolean image25D,
                        boolean imageCrop, int kernelDiameter,
     ...
     */
    public AlgorithmFrequencyFilter(ModelImage destImg, ModelImage srcImg, boolean image25D,
                        float freq1, int butterworthOrder, float lowGain, float highGain,
                        float lowTruncated, float highTruncated) {
    super(destImg, srcImg);
     ...
     * @param highTruncated part of high histogram end truncated
     */
    public AlgorithmFrequencyFilter(ModelImage srcImg, boolean image25D,
                        float freq1, int butterworthOrder, float lowGain, float highGain,
                        float lowTruncated, float highTruncated) {
    super(null, srcImg);
    ...
    private void constructLog() {
        ...
        if (constructionMethod == GABOR) {
          ...
          historyString = new String("FrequencyFilter(" +
                                     ...
                                     String.valueOf(f1) + ", " +
                                     String.valueOf(butterworthOrder) +  ", " +
                                     String.valueOf(lowGain) + ", " +
                                     String.valueOf(highGain) + ", " +
                                     String.valueOf(lowTruncated) + ", " +
    ...
    */
    ...
    /*
    * We tried this used in the Laplacian medialness option of live wire cost function
    * @return resultData
    */
    /*private float[] calc2DMedialness() {
        ...
        makeComplexData();
        sliceSize = dimLengths[0]*dimLengths[1];
        // Perform forward FFT on data
        transformDir = FORWARD;
        exec(realData,imagData,0);
        try {
            realProduct     = new float[newSliceSize];
            imagProduct     = new float[newSliceSize];
            realKernelData  = new float[newSliceSize];
            imagKernelData  = new float[newSliceSize];
            resultData      = new float[sliceSize];
            sData           = new byte[sliceSize];
        }
        catch (OutOfMemoryError e) {
            realProduct = null;
            System.gc();
            displayError("AlgorithmFrequencyFilter: Out of memory creating realProduct");
            progressBar.dispose();
            setCompleted(false);
            return null;
        }
        ...
        for (s = 1; s <= 8 && !threadStopped; s++) {
            ...
            for (i = 0; i < newSliceSize; i++) {
              realProduct[i] = realData[i]*realKernelData[i] - imagData[i]*imagKernelData[i];
              imagProduct[i] = imagData[i]*realKernelData[i] + realData[i]*imagKernelData[i];
            }

            // Perform inverse FFT
            transformDir = INVERSE;
            exec(realProduct,imagProduct,0);

            for (i = 0; i < sliceSize; i++) {
                if (finalData[i] > resultData[i]) {
                    resultData[i] = finalData[i];
                    sData[i] = (byte)s;
                }
            }
    ...
    */
    private void makeKernelData() {
        ...
        if ((ndim == 2) || (image25D)) {
            switch(filterType) {
                case LOWPASS: idealLPKernel2D(f1);
                          break;
                case HIGHPASS: idealHPKernel2D(f1);
                           break;
                case BANDPASS: idealBPKernel2D(f1,f2);
                               break;
                case BANDSTOP: idealBSKernel2D(f1,f2);
                               break;
                default: displayError("makeKernelData: no such filter type");
                         progressBar.dispose();
                         setCompleted(false);
                         return;
            ...
            switch(filterType) {
                case LOWPASS: idealLPKernel3D(f1);
                          break;
                case HIGHPASS: idealHPKernel3D(f1);
                           break;
                case BANDPASS: idealBPKernel3D(f1,f2);
                               break;
                case BANDSTOP: idealBSKernel3D(f1,f2);
                               break;
                default: displayError("makeKernelData: no such filter type");
                         progressBar.dispose();
                         setCompleted(false);
    ...
    */
    ...
    /* This function produces a new image that is either the FFT,
    *   the filtered FFT, or the inverse FFT of the original or filtered image.
    */
                 ...
                 for (k = 0; k < dimLengths[2]; k++)
                   for (j = 0; j < dimLengths[1]; j++)
                     for (i = 0; i < dimLengths[0]; i++)
                       tempData[i + dimLengths[0]*j + dimLengths[0]*dimLengths[1]*k +
                                dimLengths[0]*dimLengths[1]*dimLengths[2]*m +
                                dimLengths[0]*dimLengths[1]*dimLengths[2]*
                                dimLengths[3]*n] = rData[i + newDimLengths[0]*j +
                                newDimLengths[0]*newDimLengths[1]*k +
                                newDimLengths[0]*newDimLengths[1]*newDimLengths[2]*m +
                                newDimLengths[0]*newDimLengths[1]*newDimLengths[2]*newDimLengths[3]*n];
          }
    ...
    */
      ...
      if (ndim == 1) {
        ...
        for (k = 0; k <= end[2] - start[2]; k++)
          for (j = 0; j <= end[1] - start[1]; j++)
            for (i = 0; i <= end[0] - start[0]; i++)
              tempData[i + start[0] + dimLengths[0]*(j + start[1]) + dimLengths[0]*
              dimLengths[1]*(k + start[2])] = finalData[i + dimLengths[0]*j +
              dimLengths[0]*dimLengths[1]*k];
      }
      else if (ndim == 4) {
          ...
          for (k = 0; k <= end[2] - start[2]; k++)
            for (j = 0; j <= end[1] - start[1]; j++)
              for (i = 0; i <= end[0] - start[0]; i++)
                tempData[i + start[0] + dimLengths[0]*(j + start[1]) + dimLengths[0]*
                dimLengths[1]*(k + start[2]) + dimLengths[0]*dimLengths[1]*
                dimLengths[2]*(m + start[3])] = finalData[i + dimLengths[0]*j +
                dimLengths[0]*dimLengths[1]*k + dimLengths[0]*dimLengths[1]*
                dimLengths[2]*m];
      }
      else if (ndim == 5) {
            ...
            for (k = 0; k <= end[2] - start[2]; k++)
              for (j = 0; j <= end[1] - start[1]; j++)
                for (i = 0; i <= end[0] - start[0]; i++)
                  ...
                  dimLengths[1]*(k + start[2]) + dimLengths[0]*dimLengths[1]*
                  dimLengths[2]*(m + start[3]) + dimLengths[0]*dimLengths[1]*
                  dimLengths[2]*dimLengths[3]*(n + start[4])] = finalData[i +
                  dimLengths[0]*j + dimLengths[0]*dimLengths[1]*k +
                  dimLengths[0]*dimLengths[1]*dimLengths[2]*m +
                  dimLengths[0]*dimLengths[1]*dimLengths[2]*dimLengths[3]*n];
      }
    ...
    /*
   *  @param buffer float [] input buffer to be sorted
   ...
   *  pp. 331- 332.
   */
  private final void shellSort(float[] buffer) {
    int i, j, inc;
    float v;
    inc = 1;
