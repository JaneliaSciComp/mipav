 ...
 *  Attributes include: volume, area, number of pixels, center of mass,
 *  average pixel intensity, standard deviation of intensity,
 *  eccentricity, and principalAxis.
 *
 *		@version 0.1 Feb 11, 1998
 *		@author Matthew J. McAuliffe, Ph.D.
 *
 *       $Logfile: /mipav/src/gov/nih/mipav/model/algorithms/AlgorithmVOIProps.java $
 *       $Revision: 44 $
 *       $Date: 12/19/05 4:51p $
 */

public class AlgorithmVOIProps extends AlgorithmBase
    implements VOIStatisticList {
    // both of these have graphic resources,
    // but useful information and references.
    ...
    /**
     *   constructor.  sets the source image of the algorithm, and
     *   presets the algorithm to calculate properties of 3D images
     *   as a volume of interest, rather than by slice.
     ...
     *   constructor
     *   @param srcImg image model that contain the VOI
     *   @param sliceBySlice perform the property calculations for each slice,
     *                       rather than for whole volume of interest (VOI).
     */
    ...
    /**
     *   constructor.  note that if there are no VOIs to
     *   act on, this constructor returns quietly.
     *   @param srcImg image model that contain the VOI
     *   @param list list of items to perform the statistics
     *               operations on.
     *   @param sliceBySlice perform the property calculations
     *                       for each slice, rather than for
     *                       whole volume of interest (VOI).
     */
    public AlgorithmVOIProps( ModelImage srcImg,
            JPanelStatisticsList list,
    ...
    /**
     *   Accessor that indicates if the source image is
     *   a color image.
    ...
    /* not for use.  should be moved to a better location.
     * does NOT clone the VOIs that it find to be active,
     * and inserts into a new ViewVOIVector.  if no VOIs
     *  are active, the ViewVOIVector returned is
     *  <code>null</code>.
     */
    ...
    /** tells the algorithm to perform property calculations for each
     *   slice, rather than for the whole volume of interest (VOI).
     *   This method is not applicable to 2D images, and will merely
     *   reset the algorithm to act on each slice for 2D images.
     */
    ...
    /** tells the algorithm to perform property calculations for each
     *   contour, rather than for the whole volume of interest (VOI).
     */
    ...
    /** reports if this algorithm is performing
     *   property calculations for each slice,
     *   rather than on the volume of interest (VOI)
     *   as a whole.
     *   @return boolean
     */
    ...
    // sets the selected VOIset and the data set
    // data storage sizes based on the number of
    // curves in the Vector.
    ...
    /** method to decide if a particular string is in the
    ...
    private final boolean inRange( float ignoreMin, float ignoreMax, float num ) {
    ...
    /**
     *   Gets the minimum intensity of the VOI
     *   return        average intensity of image defined by the VOI
     */
    ...
    /**
     *   Gets the maximum intensity of the VOI
     *   return        average intensity of image defined by the VOI
     */
    ...
    /**
     *   Gets the minimum intensity of the Red channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMinIntensityRed() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.minIntensity + "Red" ) ).floatValue();
    } // {return minIntenRed;}

    /**
     *   Gets the maximum intensity of the Red channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMaxIntensityRed() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.maxIntensity + "Red" ) ).floatValue();
    } // {return maxIntenRed;}

    /**
     *   Gets the minimum intensity of the Green channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMinIntensityGreen() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.minIntensity + "Green" ) ).floatValue();
    } // {return minIntenGreen;}

    /**
     *   Gets the maximum intensity of the Green channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMaxIntensityGreen() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.maxIntensity + "Green" ) ).floatValue();
    } // {return maxIntenGreen;}

    /**
     *   Gets the minimum intensity of the Blue channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMinIntensityBlue() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.minIntensity + "Blue" ) ).floatValue();
    } // {return minIntenBlue;}

    /**
     *   Gets the maximum intensity of the Blue channel for the VOI
     *   return        average intensity of image defined by the VOI
     */
    public float getMaxIntensityBlue() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.maxIntensity + "Blue" ) ).floatValue();
    } // {return maxIntenBlue;}

    /**
     *   Gets the average intensity of the VOI
     *   return        average intensity of image defined by the VOI
     */
    ...
    /**
     *   Gets the average intensity of the Red channel of VOI
     *   return        average intensity of the Red channel of image defined by the VOI
     */
    public float getAvgIntenR() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.avgIntensity + "Red" ) ).floatValue();
    } // {return avgIntenR;}

    /**
     *   Gets the average intensity of the Green channel of VOI
     *   return        average intensity of the Green channel of image defined by the VOI
     */
    public float getAvgIntenG() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.avgIntensity + "Green" ) ).floatValue();
    } // {return avgIntenG;}

    /**
     *   Gets the average intensity of the Blue channel of VOI
     *   return        average intensity of the Blue channel of image defined by the VOI
     */
    public float getAvgIntenB() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.avgIntensity + "Blue" ) ).floatValue();
    } // {return avgIntenB;}

    /**
     *   Gets the standard deviation of image intensities
     *   return        standard deviation of image intensities defined by the VOI
     */
    ...
    /**
     *   Gets the standard deviation of image intensities (red channel)
     *   return        standard deviation of image intensities defined by the VOI
     */
    public float getStdDevR() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.deviationDescription + "Red" ) ).floatValue();
    } // {return stdDevR;}

    /**
     *   Gets the standard deviation of image intensities (green channel)
     *   return        standard deviation of image intensities defined by the VOI
     */
    public float getStdDevG() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.deviationDescription + "Green" ) ).floatValue();
    } // {return stdDevG;}

    /**
     *   Gets the get standard deviation of image intensities (blue channel)
     *   return        standard deviation of image intensities defined by the VOI
     */
    public float getStdDevB() {
        return Float.valueOf( ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty( VOIStatisticalProperties.deviationDescription + "Blue" ) ).floatValue();
    } // {return stdDevB;}

    /**
     *  Gets the the number of pixels
     *   return        number of pixels defined by the VOI
     */
    ...
    /**
     *   Gets the area of the VOI;
     *   return       area defined by the VOI
     */
    ...
    /**
     *   Gets the volume of the VOI;
     *   return       volume defined by the VOI
     */
    ...
    /**
     *   Gets the the center of mass of the VOI ;
     *   return       center of mass defined by the VOI
     */
    public String getCenterOfMass() {
        return ( (VOIStatisticalProperties) propertyList.firstElement() ).getProperty(
                VOIStatisticalProperties.centerDescription );
    } // {return cMass;}
    ...
    /**
     *   Gets the eccentricity of the VOI: 1 = line, 0 = circle;
     *   return       eccentricity of the VOI
     */
    ...
    /**
     *   Gets the principle axis of VOI (only valid for 2D object);
     *   return       pricipal axis angle of the VOI
     */
    ...
    /**
     *   Gets the major axis of VOI (only valid for 2D object);
     *   return       major axis length of the VOI
     */
    ...
    /**
     *   Gets the minor axis of VOI (only valid for 2D object);
     *   return       minor axis length of the VOI
     */
    ...
    /**
     *   Gets the greatest distance between any two point of the VOI
     *   return       distance
     */
