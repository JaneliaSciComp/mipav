...
public class LseSegShapeDetection2 extends LseSegmenter
{
    //~ Instance fields ------------------------------------------------------
     ...
     */
    public LseSegShapeDetection2 (int iXBound, int iYBound, float fXSpacing,
    ...
    public final void createFeatureImage ()
    {
        ...
        for (iY = 0, i = 0; iY < m_iYBound; iY++)
        {
            for (iX = 0; iX < m_iXBound; iX++, i++)
            {
                ...
                m_afGradMagBlurCurvFlow[i] =
                    (float)Math.sqrt((double)(fUx*fUx + fUy*fUy));
            }
        }
    ...
    public void beginCoarse (int[] aiSeeds)
    {
        ...
        createFeatureImage();
        m_kFastMarcher = new LseFastMarch2(m_iXBound,m_iYBound,m_fXSpacing,
            m_fYSpacing,m_afFeature,aiSeeds);
    }
    ...
    public boolean beginDistanceTransform (float fMaxDistance)
    {
        ...
        float fSpeed = 1.0f;
        m_kFastMarcher = new LseFastMarch2(m_iXBound,m_iYBound,m_fXSpacing,
            m_fYSpacing,fSpeed,m_aiBoundary);
        ...
        float fLaplacianWeight, float fTimeStep)
    {
        ...
        m_kLevelSetEvolver = new LseShapeDetection2(m_iXBound,m_iYBound,
            m_fXSpacing,m_fYSpacing,m_afSignedDistance,m_abSignedDistanceMask,
            m_fBorderValue,LsePdeFilter.PRESERVE_ZERO,m_afFeature,
            fPropagationWeight,fCurvatureWeight);
