...
public class AlgorithmRegOAR3D extends AlgorithmBase {
     ...
     */
    public AlgorithmRegOAR3D( ModelImage _imageA, ModelImage _imageB,
            int _costChoice, int _DOF, int _interp,
            float _rotateBeginX, float _rotateEndX,
            float _coarseRateX, float _fineRateX,
            float _rotateBeginY, float _rotateEndY,
            float _coarseRateY, float _fineRateY,
            float _rotateBeginZ, float _rotateEndZ,
            float _coarseRateZ, float _fineRateZ,
            boolean _maxResol, boolean _doSubsample,
            boolean _fastMode, int _bracketBound,
     ...
     */
    public AlgorithmRegOAR3D( ModelImage _imageA, ModelImage _imageB,
            ModelImage _refWeight, ModelImage _inputWeight,
            int _costChoice, int _DOF, int _interp,
            float _rotateBeginX, float _rotateEndX,
            float _coarseRateX, float _fineRateX,
            float _rotateBeginY, float _rotateEndY,
    ...
    public void runAlgorithm() {
        ...
        if ( resampleInput ) {
            ...
            simpleInput = new ModelSimpleImage( blurredInput.getExtents(),
                    blurredInput.getFileInfo( 0 ).getResolutions(), blurredInput );
        }
        ...
        if ( weighted ) {
            ...
            if ( weightedRefPixels > subMinFactor && weightedInputPixels > subMinFactor
                    && simpleWeightRef.zDim >= minimumZForSub && simpleWeightInput.zDim >= minimumZForSub && doSubsample ) {
        ...
        if ( weighted && fullAnalysisMode ) { // If in fast mode, skip resampling because won't do levelEight, levelFour and levelTwo
            ...
            } else if ( weightedRefPixelsSub2 > subMinFactor && weightedInputPixelsSub2 > subMinFactor && doSubsample ) {
            ...
            if ( weightedRefPixelsSub4 > subMinFactor && weightedInputPixelsSub4 > subMinFactor
                ...
                if ( weightedRefPixelsSub8 > subMinFactor && weightedInputPixelsSub8 > subMinFactor
                        && simpleWeightRefSub8.zDim >= minimumZForSub && simpleWeightInputSub8.zDim >= minimumZForSub
                        && doSubsample ) {
            ...
            } else if ( weightedRefPixelsSub4 > subMinFactor && weightedInputPixelsSub4 > subMinFactor && doSubsample ) {
        ...
        if ( fullAnalysisMode ) {
            ...
            if ( simpleRefSub2.dataSize > subMinFactor && simpleInputSub2.dataSize > subMinFactor && allowLevel4XY
                    && allowLevel4Z && simpleRefSub2.zDim >= minimumZForSub && simpleInputSub2.zDim >= minimumZForSub
                    && doSubsample ) {
            ...
            if ( simpleRefSub4.dataSize > subMinFactor && simpleInputSub4.dataSize > subMinFactor && allowLevel8XY
                    && allowLevel8Z && simpleRefSub4.zDim >= minimumZForSub && simpleInputSub4.zDim >= minimumZForSub
                    && doSubsample ) {
                ...
                // For really big images subsample level 8 again!
                if ( simpleRefSub8.dataSize > subMinFactor && simpleInputSub8.dataSize > subMinFactor && allowLevel16XY
    ...
    private String getConstructionInfo() {
        ...
        s += rotateBeginX + ", " + coarseRateX + ", " + fineRateX + ", " + rotateBeginY + ", " + coarseRateY + ", "
                ...
                + fastMode + ", " + calcCOG + ")\n";
        historyString += rotateBeginX + ", " + coarseRateX + ", " + fineRateX + ", " + rotateBeginY + ", " + coarseRateY
                + ", " + fineRateY + ", " + rotateBeginZ + ", " + coarseRateZ + ", " + fineRateZ + ", " + maxResol
                + ", " + fastMode + ", " + calcCOG + ")\n";
        return s;
    }
    ...
    private Vector[] levelEight( ModelSimpleImage ref, ModelSimpleImage input ) {
         ...
         */
        Preferences.debug(
                "Total number of positions at coarse sampling to be tested: " + coarseNumX * coarseNumY * coarseNumZ
                + ".\n" );
        Preferences.debug(
                "Total number of positions at fine sampling to be tested: " + fineNumX * fineNumY * fineNumZ + ".\n" );
        ...
        for ( int j = 0; j < 7 && !threadStopped; j++ ) {
            for ( int i = 0; i < ( 2 * total - remove ) && !threadStopped; i++ ) {
                ...
                if ( j == 0 ) {
                    ...
                    if ( ( j == 1 ) || ( j == 2 ) ) {
                        Preferences.debug(
                                " by adding " + sign * fineDeltaX + " to initial[0] (" + (int) initial[0] + ").\n" );
                        initial[0] += sign * fineDeltaX;
                    } else if ( ( j == 3 ) || ( j == 4 ) ) {
                        Preferences.debug(
                                " by adding " + sign * fineDeltaY + " to initial[1] (" + (int) initial[1] + ").\n" );
                        initial[1] += sign * fineDeltaY;
                    } else {
                        Preferences.debug(
                                " by adding " + sign * fineDeltaZ + " to initial[2] (" + (int) initial[2] + ").\n" );
