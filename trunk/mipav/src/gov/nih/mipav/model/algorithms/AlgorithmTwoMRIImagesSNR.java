 ...
 * The factor square root(2) is required because the standard deviation used is from the difference image
 * rather than one of the original images
 * The contrast to noise ratio is simply SNR for VOI 1 - SNR for VOI 2.
 *
 * <p>An optional registration may be performed before SNR. In this registration image2 is registered to
 * image. AlgorithmRegOAR2D is used with the cost function being the only registration parameter the
 * user can vary in the dialog box. Correlation ratio is the default cost function, but the user can also select least
 * squares, normalized cross correlation, or normalized mutual information. The SNR will be performed with the
 * registered image2 rather than on the original prebleached image.</p>
 * 
 ...
 * in the dialog box are used to select a red signal 1 or a green signal 2 VOI. Either an
...
public class AlgorithmTwoMRIImagesSNR extends AlgorithmBase {
     ...
     *                          image
     */
    public AlgorithmTwoMRIImagesSNR(ModelImage image, ModelImage image2,
                                            int signalIndex, int signalImage,
                                            int signal2Index, 
    ...
    public void runAlgorithm() {
        ...
        if (register) {
            ...
            if (srcImage.getNDims() == 2) {
                ...
                regAlgo3D = new AlgorithmRegOAR3D(srcImage, image2, cost, DOF, interp,
                ...
                maxResol, doSubsample, fastMode, bracketBound, maxIterations, numMinima);
                regAlgo3D.run();
                transform = new AlgorithmTransform(image2, regAlgo3D.getTransform(), interp2,
                           srcImage.getFileInfo()[0].getResolutions()[0],
                           srcImage.getFileInfo()[0].getResolutions()[1],
                           srcImage.getFileInfo()[0].getResolutions()[2],
                           srcImage.getExtents()[0], srcImage.getExtents()[1], srcImage.getExtents()[2],
                           transformVOI, false,
                           false);
                regAlgo3D.finalize();
                regAlgo3D = null;
    ...
    private void constructLog() {
        historyString = new String("TwoMRIImagesSNR(" + signalIndex + ", " + signalImage + ", " + 
                                   signal2Index + ", " + register + ", " + cost + ", " + createRegImage + "\n");
    }
