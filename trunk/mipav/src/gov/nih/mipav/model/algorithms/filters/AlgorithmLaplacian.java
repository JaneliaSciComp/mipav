...
public class AlgorithmLaplacian extends AlgorithmBase {
    ...
    *                    highpass filter.
    */
	public AlgorithmLaplacian(ModelImage destImg, ModelImage srcImg,float
	                        sigmas[], boolean maskFlag, boolean img25D, float ampFactor) {
    ...
    *                    highpass filter.
    */
        public AlgorithmLaplacian(ModelImage srcImg,float sigmas[],
                                  boolean[] objectBuffer, boolean img25D, float ampFactor) {
        super(null, srcImg);
    ...
    */
	private void makeKernels2D() {
        ...
        for (int i = 0; i < GyyData.length; i++){
            ...
            if (tmp > 0 ) { tmp *= amplificationFactor; }
            GxxData[i] = tmp;
        }
	}
    ...
    */
    private void constructLog() {
        ...
        historyString = new String( "Laplacian(" + sigmaStr        +
    ...
    */
    ...
    /**
    *   Generates a zero crossing mask for a 2D function sets a
    *   ModelImage to 1 if a zero crossing is detected.
    *
        ...
        for (s = 0; s <  nImages && !threadStopped; s++) {
            ...
            for ( i = 0; i < length && !threadStopped; i++){
                ...
                if (entireImage == true || mask.get(i) ) {
                    lap = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                        buffer, kExtents, GxxData);
        ...
        for (s = 0; s < nImages && !threadStopped; s++) {
            ...
            if ( entireImage == false ) {
                for (i = 0; i < length; i++){
                    if ( mask.get(i) ) {
                        ...
                        else if ( buffer[i] < min ) min = buffer[i];
                    }
                }
            }
            ...
            for (i = 0, idx = start; i < length && !threadStopped; i++, idx++){
                ...
                if (entireImage == true || mask.get(i)) {
                    ...
                    if ( entireImage == false ) {
                        if ( mask.get(i) ) {
                            if       ( lap > maxL ) maxL = lap;
                            else if  ( lap < minL ) minL = lap;
                        }
                    }
