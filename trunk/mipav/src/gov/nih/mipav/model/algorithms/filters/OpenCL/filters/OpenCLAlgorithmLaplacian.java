...
public class OpenCLAlgorithmLaplacian extends OpenCLAlgorithmBase {
     ...
     */
    public OpenCLAlgorithmLaplacian(ModelImage srcImg, float[] sigmas, boolean maskFlag, boolean img25D, float ampFactor) {
        this(null, srcImg, sigmas, maskFlag, img25D, ampFactor);
    }
    ...
    private void laplacian25D()
	{
		...
		saveImage(output, 0, srcImage.getType(), srcImage.getImageName() + "_laplacian" );
    ...
    private void laplacian3D( int time )
	{
    	...
    	clEnqueueReadBuffer(commandQueue, outputBuffer, CL_TRUE, 0, Sizeof.cl_float * output.length, Pointer.to(output), 0, null, null);
		saveImage(output, time, srcImage.getType(), srcImage.getImageName() + "_laplacian" );
