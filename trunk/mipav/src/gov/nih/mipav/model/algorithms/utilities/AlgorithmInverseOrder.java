...
/**
 *  Algorithm that inverses the order of 3D image dataset. That
 *  is the last image becomes the first the first image becomes
 *  the last.
 *
...
public class AlgorithmInverseOrder extends AlgorithmBase {
    ...
    /**
     *   Constructs an algorithm object that inverses the order of
     *   3D image dataset.
            ...
            for ( s = 0; s < sliceNumber && !threadStopped; s++ ) {
                if ( isProgressBarVisible() ) {
                    progressBar.updateValue(
                            Math.round( (float) ( t * sliceNumber + s ) / ( tDim * sliceNumber - 1 ) * 100 ),
                            activeImage );
                }
                start = s * length;
                try {
                ...
                for ( i = 0, idx = ( t + 1 ) * threeDLength - length - start; i < length && !threadStopped; i++, idx++ ) {
