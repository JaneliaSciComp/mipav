...
/**
 * A class for segmenting the brain from a 3D MRI.  The algorithm is partially
 * based on the paper:
 * <pre>
 ...
 *
 *      @author David Henry Eberly, Magic Software. Under contract from NIH.
 *      with modifications by Matthew McAuliffe
 */
public class AlgorithmBrainExtractor extends AlgorithmBase {
    ...
    // Tells whether second stage which erodes edge values exceeding median
    // by a factor >= aboveMedian occurs
    ...
    /**
     * Create an extractor for segmenting the brain from a 3D magnetic
     * resonance image.
     *
     * @param srcImg the source image. Should be MR image of the brain
     * @param orientation image orienation
     */
    public AlgorithmBrainExtractor( ModelImage srcImg, int orientation,
            boolean justEllipse, boolean estimateSphere, Point3f centerPoint ) {
        int i;
        float fMax, fMin;
         ...
         *   ellipsoid into a mesh that approximates the brain surface.  The
    ...
    // Reinitialize when secondStageErosion has been performed so that the mask
    // will match the eroded image
    ...
    private void constructLog() {
        historyString = new String( "ExtractBrain(" + orientationFlag + ", " +
                                    onlyEllipse + ", " + useSphere + ", " +
                                    nIterations + ", " + m_iMaxDepth + ", " +
                                    f3Factor + ", " + m_fStiffness + ", " +
    ...
    /**
     * Set the brain selection term, as described in BrainExtraction.pdf,
     * that is part of the image term in the surface evolution.  The default
     * value is 0.5.
     *
     * @param fBrainSelection the new brain selection term
     */
    final public void setBrainSelection( float fBrainSelection ) {
        m_fBrainSelection = fBrainSelection;
    }

    /**
     * Get the current brain selection term, as described in
     ...
     */
    final public float getBrainSelection() {
        return m_fBrainSelection;
    }

    /**
     * Set the maximum depth, as described in BrainExtraction.pdf, that is
     * part of the image term in the surface evolution.  The default value is
     * 7.
     *
     * @param iMaxDepth the new maximum depth
     */
    final public void setMaxDepth( int iMaxDepth ) {
        m_iMaxDepth = iMaxDepth;
        m_iHalfMaxDepth = iMaxDepth / 2;
    }

    /**
     * Get the maximum depth, as described in BrainExtraction.pdf, that is
     * part of the image term in the surface evolution.  The default value is
     * 7.
     *
     ...
     */
    final public void setJustIntialEllipsoid( boolean flag ) {
        onlyEllipse = flag;
    }
     ...
     */
    final public void setUseSphere( boolean flag ) {
        useSphere = flag;
    }
     ...
     */
    final public void setIterations( int nIter ) {
     ...
     *   Sets the number of iterations.
     */
    final public void setImageRatio( float ratio ) {
        f3Factor = ratio;
    }
    ...
    /**
     *  Sets whether or not second stage erosion occurs taking edge values exceeding
     *  median by a factor >= aboveMedian to zero
     *  @param secondStageErosion
     */
    final public void setSecondStageErosion( boolean secondStageErosion ) {
        this.secondStageErosion = secondStageErosion;
    }

    /**
     *  sets the aboveMedian ratio for second stage erosion
     *  Edge values >= median * aboveMedian are taken to zero
     *  @param aboveMedian
     */
    final public void setAboveMedian( float aboveMedian ) {
    ...
    /**
     * Set the dilation size for dilating the voxelized brain surface
    ...
    /**
     * Get the dilation size for dilating the voxelized brain surface
     * obtained by rasterizing the triangle mesh.  The rasterization is
     * designed so that the voxel surface has no holes, thereby allowing it
     * to be flood-filled.  But just in case numerical round-off errors cause
     * a few holes, this parameter is exposed for public use.  The default
    ...
    /**
     * The segmentation function.  Ideally, the only work an application
     * needs to do is create an MjBrainExtractor object and call the method
     * extractBrain().  Various parameters managed by MjBrainExtractor may
    ...
    /**
     * Get the 3D image that represents the extracted brain.  The image is
     * ternary and has the same dimensions as the input MRI.  A voxel value
     * of 0 indicates background.  A voxel value of 1 indicates brain
     * surface.  A voxel value of 2 indicates a voxel inside the brain
     * surface.
     *
    ...
    /**
     * Analyze the histogram of the 10-bit binned 3D MRI.  The function
     * computes a minimum threshold, a maximum threshold, and a background
     * threshold that are used in the image term of the surface evolution.
     * A brightness threshold is also computed that is used for determining
     * the initial ellipsoid that approximates the brain surface.
     */
    ...
    /**
     * Approximate the intial brain surface by an sphere. Find the
     * center of mass and approimate radius
     */
    ...
    protected void generateEllipsoidMesh( int iSubdivisions ) {
        ...
        for ( iStep = 1; iStep <= iSubdivisions; iStep++ ) {
            ...
            while ( kEIter.hasNext() ) {
                ...
                kPMid.add( kP0, kP1 );
                float fInvLen = 1.0f / (float) Math.sqrt( kPMid.x * kPMid.x + kPMid.y * kPMid.y + kPMid.z * kPMid.z );
                kPMid.scale( fInvLen );
    ...
    /**
     * Compute the median intensity of those voxels inside the initial
     * ellipsoid.  This intensity is used in the image term of the surface
     * evolution.
     */
    ...
    protected UnorderedSetInt[/* m_iVQuantity */] m_akAdjacent;
    protected HashMap m_kEMap; // map<Edge,int>
    protected Point3f[/* m_iVQuantity */] m_akVMean;
    protected Vector3f[/* m_iVQuantity */] m_akVNormal;
    protected Vector3f[/* m_iVQuantity */] m_akSNormal;
    protected Vector3f[/* m_iVQuantity */] m_akSTangent;
    protected float[/* m_iVQuantity */] m_afCurvature;
    ...
    // brain mask creation
    protected byte[/* m_iVQuantity */] m_aiMask;
     ...
     * brain surface.  The reason for using an array is to minimize reallocations
