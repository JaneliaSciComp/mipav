...
/** Local Normalisation equalises colour levels among pixels by removing
 *  variations due to lighting; this brings out contrasts in detail.
 *  <p>
 *  This is done by normalizing a surface-detail version of an image with
 *  its slowly varying local average image.  That is to say, each pixel of
 *  the output image is the value at that location of the pixel of the
 *  high-frequency image divided by the pixel at that same location of the
 *  low-frequency image.
 *  <p>
 *  The surface-detail image is generated by using an unsharp masking
 *  algorithm, and the local average image is a low-pass image generated
 *  using a general-purpose (but fast) frequency filter.
 *  <p>
 *  References:
 *  <ol>
 *  <li>Local Normalization.
 *     <a href="http://bigwww.epfl.ch/demo/normalize/desc.html">
...
public class AlgorithmLocalNormalization extends AlgorithmBase
{
    ...
    /** Images that are solely 2D images set this to true;
     *  images with other dimensionality are false
     */
    private boolean is2d;
    /** Colour images set this to true; monochromatic should be false; */
    private boolean isColourImage;
    /** Indicitive of the number of colours per pixel.  Monochrome is 1;
     *  Colour is 4 (alpha, red, green, blue).
     */
    ...
    /** This constructor initialises a Local normalisation algorithm for a
     *  source and destination image, and ensures that the destination image
     *  is <code>ModelStorageBase.FLOAT</code>.
     *  <p>
     *  Currently (8 May 2003), this algorithm does not support replacing
     *  the original data set with that of the locally-normalized image.
     */
    public AlgorithmLocalNormalization(ModelImage dest, ModelImage src,
                float   sigmas[], /* Gaussian's standard deviation in each dimension */
    ...
    /** Standard algorithm run method.  It will not run if the source Image
     *  is <code>null</code>.  The calculation is done and placed in
     *  a separate destination image if it is to be stored there.
     */
    ...
    /** Filters the source image.  Replaces the original image with
     *  the filtered image.
     *  <p>
     *  <em>Does not currently work.</em>
     */
    private void calcInPlace(){
        errorCleanUp("AlgorithmLocalNormalization: "+
                    "Replace Image not yet implemented", false);
        finalize();
        return;
    }

    /**
     * This function produces a local-normalized image into a ModelImage that
     * does not replace the original image-data.
     */
    private void calcStoreInDest() {
        ...
        try {imageData = new float[datalen];}
        catch (OutOfMemoryError oome) {
            errorCleanUp("AlgorithmLocalNormalization: "+
                        "out of memory creating imageData", false);
            finalize();
            return;
        }
        ...
        try {
            ...
            errorCleanUp("AlgorithmLocalNormalization: "+
                        "failure to export imageData", false);
            finalize();
            return;
        }
        ...
        if (tempImage[0] == null) {
            ...
            try {tempImage[0].importData(0, imageData, true);}
            catch (IOException ioe) {
                errorCleanUp("AlgorithmLocalNormalization: "+
                            "failure to import imageData", true);
                finalize();
                return;
            }
        }
        ...
        unsharper = new AlgorithmUnsharpMask(tempImage[0], sigmas,
                                            weightingFactor, true, true);
        ...
        if (!unsharper.isCompleted()) {
            Preferences.debug("AlgorithmLocalNormalization: "+
                            "failure to complete unsharp mask");
         ...
         */
        FFTer = new AlgorithmFrequencyFilter(tempImage[1], !is2d, false,
                                            kernelDiameter,
                                            AlgorithmFrequencyFilter.LOWPASS,
                                            frequency, 0,
                                            AlgorithmFrequencyFilter.GAUSSIAN,
                                            0);
        // don't set to a new Thread just yet.  let run serially.
        // (ie., in current Thread):
...
//        tempImage[1]=null;
    ...
    /** works depending on the extent of the image and the
     *  colourisation of the source image in question.
     *  <p>
    ...
    private void normalize() {
        ...
        float[] unsharpd =
                new float[srcImage.getExtents()[0]*srcImage.getExtents()[1]];
        float[] FFTd =
                new float[srcImage.getExtents()[0]*srcImage.getExtents()[1]];
        ...
        try {
            if (!isColourImage) {
                ...
                for (i = 0; i < nslices; i++) {
                    ...
                    destImage.importData(i*unsharpd.length,
                                doImageDivision(unsharpd, FFTd), false);
...
//                            destImage.getType());
                ...
                errorCleanUp("AlgorithmLocalNormalization: "+
                            "2d and colour not yet supported", false);
                finalize();
            }
            else { // if (!is2d && isColourImage)
                //System.out.println("(else)not yet.");
                errorCleanUp("AlgorithmLocalNormalization: "+
                            "colour not yet supported", false);
                finalize();
            }
        }
        catch (IOException ioe) {
            errorCleanUp("AlgorithmLocalNormalization: "+
                        "IO exception: " + ioe.getMessage(), false);
            ...
            try {
                ...
                Preferences.debug("AlgorithmLocalNormalization: "+
                          "TempImages already null.  Nothing to do, "+
                          "so moving along.\n");
            }
        }
    }
    ...
    /** takes the data of the two images,
     *  one unsharp'd and the other gaussian blur'd,
     *  and divides the two pixel-by-pixel to find the
     *  locally normalized value for each pixel.
     */
    ...
    */
    private void constructLog() {
        ...
        historyString = "local normalization(" +
                        sigmaStr                       + ", " +
                        String.valueOf(kernelDiameter) + ", " +
                        String.valueOf(weightingFactor)+ ", " +
                        String.valueOf(frequency)      + ")\n";
    }

    /**
    *	RGB images are local-normalized by 'channel.'  That is, each colour,
    *   red, blue and green, is filtered independantly of the other two colours.
    *   This filter permits selectively filtering any combination of the
    *   three channels instead of simply filtering all three.  True for any of
    *   the arguments enforces filtering that channel.
    ...
    */
    ...
    /** The ViewUserInterface is held for debugging purposes only.
     *  May be deprecated in the future.
     */
    ViewUserInterface userInterface;  // for debug only

    /** The ViewUserInterface is set for debugging purposes only.
     *  This method may be deprecated in the future.
     */
    public void setUserInterface(ViewUserInterface vui) { userInterface = vui; }
