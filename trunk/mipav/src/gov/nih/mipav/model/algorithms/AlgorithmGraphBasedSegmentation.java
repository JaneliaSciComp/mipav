...
public class AlgorithmGraphBasedSegmentation extends AlgorithmBase {
  ...
  // This code is a port of the image segmentation C++ source code found at
  // Pedro F. Felzenszwalb's website at http://people.cs.uchicago.edu/~pff/.
  // The implementation of this segmentation algorithm is described in:
  // Efficient Graph-Based Image Segmentation by Pedro F. Felzenszwalb and
  // Danile P. Huttenlocher, Internation Journal of Computer Vision,
  // Vol. 59, No. 2, September, 2004.
  ...
  // Larger values for threshold result in larger components in the result.
  ...
  private int minComponentSize = 20;
     ...
     */
    public AlgorithmGraphBasedSegmentation( ModelImage resultImage,
           ModelImage srcImage, float sigma, float thresh, int minComponentSize) {
     ...
     *   @param minComponentSize
     */
    public AlgorithmGraphBasedSegmentation(ModelImage srcImage,
            float sigma, float thresh, int minComponentSize) {
    ...
    private void constructLog() {
        historyString = new String(
                "GraphBasedSegmentation(" + sigma + ", " + thresh + ", " +
                                            minComponentSize + ")\n" );
    }
    ...
    private void segment3Colors2D() {
      ...
      try {
          ...
          buildProgressBar(srcImage.getImageName(),
                           "Performing graph based segmentation...", 0, 100);
          initProgressBar();
          ...
          for (i = 0; i < num; i++) {
              ...
              if ((a != b) && (eltsSize[a] < minComponentSize) &&
                              (eltsSize[b] < minComponentSize)) {
                  if (eltsRank[a] > eltsRank[b]) {
                      eltsP[b] = a;
                      eltsSize[a] += eltsSize[b];
                  }
    ...
    private static void sort1(float x[], int off, int len, int x2[], int x3[]) {
        ...
        int s, n = off + len;
        s = Math.min(a-off, b-a  );  vecswap(x, off, b-s, s, x2, x3);
        s = Math.min(d-c,   n-d-1);  vecswap(x, b,   n-s, s, x2, x3);
