...
public class AlgorithmProstateFeaturesSaveAutoTrain extends AlgorithmBase implements AlgorithmInterface {
     ...
     * @param classImage
     * @param srcImg
     * @param imageIntensityFilter         flag for image intensity filter. 
     * @param coherenceEnhancingDiffusionFilter		flag for CED filter. 
     * @param regisotropicDiffusionFilter		flag for iso-tropic diffusion filter. 
     * @param IHN3CorrectionFilter				flag for N3 correction filter. 
     * @param modeFilter					flag for mode filter. 
     * @param meanFilter                   flag for mean filter. 
     * @param medianFilter	                flag for median filter. 
     * @param invertFilter                   flag for invert filter. 
     * @param haralickFilter                 flag for filter. 
     * @param gaborFilter                    flag for filter. 
     * @param hurstFilter					flag for index filter.
     * @param waveletFilter					flag for filter. 
     * @param gaussianFilter				flag for filter. 
     * @param gmFilter						flag for gradient magnitude filter. 
     * @param numberFeatures				number features. 
     */
   ...
   private void calculateHurstIndex() {
    ...
    private void calculate() {
        ...
        resultBuffer = new float[zDim][numberFeatures][sliceSize];
        resultBufferClass = new float[zDim][numberFeatures][sliceSize];
			...
			try {
				...
				for (i = 0; i < numberFeatures; i++) {
					for (j = 0; j < sliceSize; j++) {
						resultBuffer[z][i][j] = 0;
						resultBufferClass[z][i][j] = 0;
					}
				}
				...
				for (y = yStart; (y <= yEnd) && !threadStopped; y++) {
					...
					for (x = xStart; x <= xEnd; x++) {
						...
						if ( imageIntensityFilter ) {
							...
							resultBuffer[z][currentResult][pos] = (float)cedValue;
							currentResult++;
						}
						
						if (coherenceEnhancingDiffusionFilter) {
							float cedValue = coherenceEnhancingDiffusionBuffer[pos];
							resultBuffer[z][currentResult][pos] = cedValue;
							currentResult++;
						}

						if (regisotropicDiffusionFilter) {
							float adValue = regisotropicDiffusionBuffer[pos];
							resultBuffer[z][currentResult][pos] = adValue;
							currentResult++;
						}
						...
						if ( hurstFilter ) {
							resultBuffer[z][currentResult][pos] = hurstBuffer[pos];
							currentResult++;
						}
