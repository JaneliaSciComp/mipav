 ...
 * http://www.gradetools.com/karlisf. At the start of the program RGB color space is transformed in YCrCb space and at
 * the end of the program YCrCb space is transformed into RGB space. Hence, all of the program operations occur in YcrCb
 * space. Frequently Asked Questions About Color by Charles Poynton at http://www.poynton.com/colorFAQ.html reveals that
 * the RGB to YCrCb and YCrCb to RGB conversion equations used in the Freivalds program are significantly different from
 * the standard ones. Black and white images are not transformed into another space. ----------- Description -----------
 * The filter reduces noise in the image without blurring the edges. The results are much better than can be achieved by
 * using the popular adaptive median or smoothing filters. Like modern compression algorithms (including JPEG) the
 * filter operates in Y,Cr, Cb color space, hence it is useful for removing compression artifacts. ----------------- How
 * does it work? ----------------- It is based on the search of the pixel neighbor graph. The main steps of the
 * algorithm are: - create the pixel neighbor graph, set edge weights based on the differences between the pixels - for
 * the pixel being filtered calculate the shortest path from it to all the other pixels reachable with path length < d -
 * replace the pixel value with the weighted average of all reached pixels, weights are taken as the Gaussian function
 * of the path length In this way only the relevant filtering neighborhood for each pixel is selected. The size and
 * shape of the neighborhood adopt to the image features, including curved ones and sharp corners. Note that the edge
 * graph is created with a median filtered version of the YCrCb space. The median filter only obtains a median of the 3
 * by 3 area if no edge is detected in any of Y, Cr, or Cb spaces. If an edge is detected in any of the 3 spaces, then
 * the Y, Cr, and Cb values are all unchanged. An edge is detected if the difference between the center pixel and any
 * one of its 4 nearest neighbors is greater than a threshold or if the center pixel is larger than all of its four
 * neighbors by threshold/2 or if the center pixel is smaller than all of its four neighbors by threshold/2. The Y, Cr,
 * and Cb spaces have 3 different thresholds. The pixel neighbor or edge graph is an array of arrays, with the first
 * array having a length equal to the number of the pixels in the image and the second array having 8 integers. The 8
 * values are the edge weights calculated from the intensity differences between the center pixel and one of the 8
 * neighboring pixels. edge weight = sqrt((dy*wY)**2 + (dr*wR)**2 + (dB*wB)**2), where dy, dR, and dB are the Y, Cr, and
 * Cb differences and wY, wR, and wB are the Y, Cr, and Cb weights derived from the Y, Cr, and Cb radiuses. A bigger
 * relative radius results in a smaller weight for radiuses >= 0.1, which should typically be the case. After the edge
 * graph is created, the Y, Cr, and Cb spaces are separately filtered. The filter uses the pixel as the center of a
 * square with sides of length 2*range + 1, with range = (int)(radius + 0.999). A priority queue is created with all of
 * the queue elements having a key, a baseKey, and a value. After the priority queue is created, the filter table is
 * created. Then the distMap distance function table is created. The distMap at a given position inside the maxLength =
 ...
 * which had retrieved queue elements are put into a weighted sum. The weight is derived from an index into the filter
 * array given by the queue element key times the invFilterLen. The filter array is generated with a Gaussian function.
 * value gives the location within the maxLength = (2*range+1)*(2*range+1) local area. value = x - minX + (y -
 * minY)*(2*range + 1). baseKey contains a sum of all the edgeGraph values over the low cost path from the center pixel
 * to the selected pixel. key = baseKey + the distMap value for the selected pixel. A position is only added to the
 * priority queue if key < maxDistance. Initially, the seen array is set equal to 0 for all queue elements within the
 * local area. After a queue element has been added and retrieved from the queue, seen is set equal to 2 for that
 * position to prevent later addition to the queue. The variable last gives the number of queue elements in the priority
 * queue. The places array of length maxLength gives the queue element number of that local area position or -1 if the
 * local area position has no element present in the priority queue. The priority queue is implemented as a binary heap
 * and the binary heap is implemented as an array. A binary heap is a "nearly full" binary tree(only the bottom level
 * may not be complete). This binary heap has the property that for every node other than a leaf, its key value is less
 * than or equal to that of its children. In a binary heap the item of highest priority is always at the root of the
 * tree or at node 0. When the highest priority item at the root is removed, the item of highest priority among the
 * remainder moves into the root position. The children of node i are at 2*i + 1 and 2*i + 2. The parent of node i is at
 * (int)((i-1)/2). Adding an element e to the priority queue: The places of e.value returns a number p. p >= 0 if the
 * queue element is already in the priority queue. p = -1 if the queue element is not in the priority queue. If element
 * e is already in the queue with a smaller key value than the key value attached to e, then simply return without
 * putting e in the queue. If e is not in the queue and last, the number of queue elements, has already reached the
 * maximum size, then return with a full queue error. If e is not in the queue and last is less than maxsize, set a new
 * queue position i equal to last and then increment last. If e is already in the queue, set i to p. Then, place e at
 * queue position i. As long as i > 0 and the key of queue element i is less than the key of its parent at position
 * (int)((i-1)/2), then interchange the parent and the child queue elements. Set the places of the moved queue elements
 * to the new queue positions. Getting an element e from the priority queue: The highest priority queue element, that is
 * the queue element with the smallest key value, is always at queue[0], the root of the queue. Hence, queue[0] is
 * always returned. If last == 0, no elements are in the queue, so return with a priority queue is empty error message.
 * If last is not zero, set the places of the queue[0] element to -1 to indicate that it is no longer in the queue.
 * Decrement last. If last now equals zero, then the queue is now empty, so simply return. If last does not equal zero,
 * then move queue[last] into the root queue[0] position. Keep looping as long as the right child position, 2*i + 2, is
 * <= last. If the key of the left child position at j - 1 is less than the key of the right child position at j, then
 * decrement j to select the left child. If j == last, then the right child position is empty, so decrement to select
 * the left child. Otherwise, the right child position at j has the smaller key so do not decrement j. If the key of the
 * parent is greater than the key of the child, then swap the parent and child. Set the places of the moved queue
 * elements to the new queue positions. Stop looping when the key of the parent is <= the key of the child. --------
 * Settings -------- You can adjust several parameters to obtain the best noise reduction level for each particular
 * image: - Filter radius for each Y, Cr, Cb color channel: Larger radius will remove more noise, however some detail
 * also can be lost. - Edge sensitivity: larger values will preserve more edges but sacrifice smoothness in low contrast
 * areas. For very large values banding may appear. - Cr and Cb 2x reduction: The resolution of Cr and Cb channels is
 * reduced two times, which improves the speed of the algorithm for large radiuses with little loss of quality.
 * Recommended settings are. - for high quality scanned images radius Y 2, Cr 2 Cb 2 Edge Sensitivity 3072 Cr and Cb 2x
 * reduction false - for high quality JPEG images radius Y 2, Cr 3 Cb 4 Edge Sensitivity 3072 Cr and Cb 2x reduction
 * false - for low quality JPEG images radius Y 3, Cr 4 Cb 6 Edge Sensitivity 3072 Cr and Cb 2x reduction true
 */
public class AlgorithmAdaptiveNR extends AlgorithmBase {
    ...
    /**
     * Creates a new AlgorithmAdaptiveNR object.
     *
     ...
     * are connected to the center pixel via a low cost path, queue elements corresponding to the neighboring pixels are
     ...
     * queue elements are put into a weighted sum. The weight is derived from an index into the filter array given by
     * the queue element key times the invFilterLen.
     *
     * @param   x  DOCUMENT ME!
     * @param   y  DOCUMENT ME!
     *
     ...
     * elements are put into a weighted sum. The weight is derived from an index into the filter array given by the
