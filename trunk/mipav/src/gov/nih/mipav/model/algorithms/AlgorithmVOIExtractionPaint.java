...
public class AlgorithmVOIExtractionPaint extends AlgorithmBase {
    ...
    private void calcInPlace() {
        ...
        for ( z = 0; z < zDim; z++ ) {
            ...
            for ( y = 0; y < yDim2 && !threadStopped; y++ ) {
                for ( x = 0; x < xDim2 && !threadStopped; x++ ) {
                    ...
                    if ( ( !mask.get( smallPos ) ) && ( srcPaintMask.get( smallPos3D ) == true ) ) {
                        ...
                        ( (VOIContour) ( addedVOI.getCurves()[z].lastElement() ) ).trimPoints( Preferences.getTrim(), Preferences.getTrimAdjacient() );
            ...
            for (smallX = 0; smallX < xDim; smallX++) {
              smallPos = smallX + smallY * xDim;
              smallPos3D = smallX + smallY * xDim + offset;
              if ( (!outMask.get(smallPos)) &&
                   ( srcPaintMask.get( smallPos3D ) == false ) ) {
            ...
            for ( y = 0; y < yDim2 && !threadStopped; y++ ) {
              for (x = 0; x < xDim2 && !threadStopped; x++) {
                ...
                if ( (!outMask.get(smallPos)) && (!mask.get(smallPos)) &&
                   ( srcPaintMask.get( smallPos3D ) == false ) ) {
                    // have found a new contour
                    // Since genContour works on finding a four-connected boundary using
                    // Papert's turtle algorithm, the routine to grow the region must
    ...
    /*private void setRegionMaskR( int xStart, int yStart, int offset ) {
        ...
        mask2.set( 2 * xStart + xDim4 * yStart );
        mask2.set( 2 * xStart + 1 + xDim4 * yStart );
        mask2.set( 2 * xStart + xDim4 * yStart + xDim2 );
        mask2.set( 2 * xStart + 1 + xDim4 * yStart + xDim2 );
        maskAll.set( 2 * xStart + xDim4 * yStart );
        maskAll.set( 2 * xStart + 1 + xDim4 * yStart );
        maskAll.set( 2 * xStart + xDim4 * yStart + xDim2 );
        maskAll.set( 2 * xStart + 1 + xDim4 * yStart + xDim2 );

        if ( ( ( xStart + 1 ) < xDim ) && ( !mask.get( xStart + 1 + xDim * yStart ) )
                && ( srcPaintMask.get( offset + xStart + 1 + xDim * yStart ) == true ) ) {
            setRegionMaskR( xStart + 1, yStart, offset );
        }
        ...
        if ( ( ( yStart + 1 ) < yDim ) && ( !mask.get( xStart + xDim * ( yStart + 1 ) ) )
                && ( srcPaintMask.get( offset + xStart + xDim * ( yStart + 1 ) ) == true ) ) {
            setRegionMaskR( xStart, yStart + 1, offset );
        }
    ...
    private void setRegionMaskI( int xStart, int yStart, int offset ) {
        ...
        while ( maskNumber > 0 ) {
            ...
            for ( i = 0; i < maskNumber; i++ ) {
                ...
                if ( ( ( xt + 1 ) < xDim ) && ( !mask.get( xt + 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt + 1 + xDim * yt ) == true ) ) {
                ...
                if ( ( ( xt - 1 ) > 0 ) && ( !mask.get( xt - 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt - 1 + xDim * yt ) == true ) ) {
                    nextMaskList[nextMaskNumber][0] = xt - 1;
                    nextMaskList[nextMaskNumber++][1] = yt;
                ...
                if ( ( ( yt + 1 ) < yDim ) && ( !mask.get( xt + xDim * ( yt + 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt + 1 ) ) == true ) ) {
                ...
                if ( ( ( yt - 1 ) > 0 ) && ( !mask.get( xt + xDim * ( yt - 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt - 1 ) ) == true ) ) {
                    nextMaskList[nextMaskNumber][0] = xt;
                    nextMaskList[nextMaskNumber++][1] = yt - 1;
    ...
    private void setRegionMaskIFalse( int xStart, int yStart, int offset ) {
        ...
        while ( maskNumber > 0 ) {
            ...
            for ( i = 0; i < maskNumber; i++ ) {
                xt = maskList[i][0];
                yt = maskList[i][1];
                if ( ( ( xt + 1 ) < xDim ) && ( !mask.get( xt + 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt + 1 + xDim * yt ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt + 1;
                    nextMaskList[nextMaskNumber++][1] = yt;
                ...
                if ( ( ( xt - 1 ) > 0 ) && ( !mask.get( xt - 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt - 1 + xDim * yt ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt - 1;
                    nextMaskList[nextMaskNumber++][1] = yt;
                ...
                if ( ( ( yt + 1 ) < yDim ) && ( !mask.get( xt + xDim * ( yt + 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt + 1 ) ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt;
                    nextMaskList[nextMaskNumber++][1] = yt + 1;
                ...
                if ( ( ( yt - 1 ) > 0 ) && ( !mask.get( xt + xDim * ( yt - 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt - 1 ) ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt;
                    nextMaskList[nextMaskNumber++][1] = yt - 1;
    ...
    /**
     * Sets the mask of all points in the 4 connected region with a srcPaintMask
     * equal to false
     * Uses iteration
     *
     * @param xStart    the x coordinate of the starting point
    ...
    private void setRegionOutMaskI( int xStart, int yStart, int offset ) {
        ...
        while ( maskNumber > 0 ) {
            ...
            for ( i = 0; i < maskNumber; i++ ) {
                xt = maskList[i][0];
                yt = maskList[i][1];
                if ( ( ( xt + 1 ) < xDim ) && ( !outMask.get( xt + 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt + 1 + xDim * yt ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt + 1;
                    nextMaskList[nextMaskNumber++][1] = yt;
                    outMask.set( ( xt + 1 ) + xDim * yt );
                }
                if ( ( ( xt - 1 ) > 0 ) && ( !outMask.get( xt - 1 + xDim * yt ) ) && ( srcPaintMask.get( offset + xt - 1 + xDim * yt ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt - 1;
                    nextMaskList[nextMaskNumber++][1] = yt;
                    outMask.set( ( xt - 1 ) + xDim * yt );
                }
                if ( ( ( yt + 1 ) < yDim ) && ( !outMask.get( xt + xDim * ( yt + 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt + 1 ) ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt;
                    nextMaskList[nextMaskNumber++][1] = yt + 1;
                    outMask.set( xt + xDim * ( yt + 1 ) );
                }
                if ( ( ( yt - 1 ) > 0 ) && ( !outMask.get( xt + xDim * ( yt - 1 ) ) ) && ( srcPaintMask.get( offset + xt + xDim * ( yt - 1 ) ) == false ) ) {
                    nextMaskList[nextMaskNumber][0] = xt;
                    nextMaskList[nextMaskNumber++][1] = yt - 1;
                    outMask.set( xt + xDim * ( yt - 1 ) );
                }
