...
/**
 * Extracts a surface using Marching Cube Extraction. Triangle decimation can be
 * invoked to reduce triangle count. The decimation algorithm produces a
 * continuous level of detail (clod) structure that can be used to optimize the
 ...
 * The mask image is then blurred slightly and the level (50) is extracted. A
 * greyscale image may also be input and a surface is extracted given a level.
 * The steps are:
 * 
 * <ol>
 * <li>Build mask image of VOI (i.e. all point interior to VOI are set to 100.
 * All points exterior are = 0.</li>
 ...
 * @author David H. Eberly, Ph.D. wrote all the extraction and decimation code
 *         found in the SurfaceExtraction, SurfaceDecimation and associated
 *         classes, with a little input from Matt with speed and memory
 *         optimizations
...
public class AlgorithmExtractSurfaceCubes extends AlgorithmBase {
	...
	/**
	 * Do not perform triangle consistency checking - all counter clockwise or
	 * all clockwise.
	 */
	...
	/**
	 * If true then the extracted surface is decimated into a continuous level
	 * of detail surface (clod).
	 */
	...
	/**
	 * Path and name of extracted surface file. ".sur" will be appended if
	 * necessary.
	 */
	 ...
	 * 
	 * @param image
	 *            mask image or gray-scale where a level surface is to be
	 *            extracted
	 * @param level
	 *            indicates level surface to be extracted
	 * @param mode
	 *            Indicates mode - VOI, LEVELSET, or MASK.
	 * @param decFlag
	 *            indicates whether or not the decimation into a CLOD should
	 *            take place.
	 * @param blurFlag
	 *            if true then the input image is blurred slightly
	 * @param sigma
	 ...
	 * @param fileName
	 *            path and name of extracted surface file. ".sur" will be
	 *            appended if necessary.
	 * @param smoothFlag
	 *            whether the generated mesh should have smoothing applied to it
	 */
	public AlgorithmExtractSurfaceCubes(ModelImage image, int level, int mode,
			boolean decFlag, boolean blurFlag, float sigma, String fileName) {
		super(null, image);
	...
	public void runAlgorithm() {
		...
		// then extract
		// a surface.
		fireProgressStateChanged(srcImage.getImageName(),
				"Extracting surface ...");
	...
	private void extractSurface() {
		...
		if (blurFlag == true) {
			...
			try {
				blurAlgo = new AlgorithmGaussianBlur(null, maskImage, sigmas,
						true, false);
		...
		// Make storage string
		surfaceFileName = ViewUserInterface.getReference()
				.getDefaultDirectory()
				+ File.separator + surfaceFileName;
		...
		try {
			...
			if (srcImage.getMatrixHolder().containsType(
			...
			SurfaceExtractorCubes kExtractor = new SurfaceExtractorCubes(iXDim,
					iYDim, iZDim, buffer, fXRes, fYRes, fZRes, direction,
			...
			VETMesh kVETMesh = new VETMesh(2 * kMesh.VBuffer
					.GetVertexQuantity(), .9f, 2 * kMesh.IBuffer
					.GetIndexQuantity(), .9f, 2 * kMesh.GetTriangleQuantity(),
					.9f, kMesh.IBuffer.GetData());
			...
			if (decimateFlag == true) {
				...
				kVETMesh = new VETMesh(2 * kMesh.VBuffer.GetVertexQuantity(),
						.9f, 2 * kMesh.IBuffer.GetIndexQuantity(), .9f,
						2 * kMesh.GetTriangleQuantity(), .9f, kMesh.IBuffer
								.GetData());
				...
				for (int i = 0; (i < iNumComponents) && !threadStopped; i++) {
					...
					IndexBuffer kIBuffer = new IndexBuffer(kComponents.get(i)
							.GetTriangles());
					CreateClodMesh kDecimator = new CreateClodMesh(kVBuffer,
							kIBuffer);
					kDecimator.decimate();
					akClod[i] = new ClodMesh(kVBuffer, kIBuffer, kDecimator
							.getRecords());

					fireProgressStateChanged("Saving surface");

					FileSurface_WM.save(kName + i + ".sur", akClod[i], 1,
							akClod[i].VBuffer, true, direction, startLocation,
				...
				fireProgressStateChanged("Saving surface");
				FileSurface_WM
						.save(surfaceFileName, kMesh, 0, kMesh.VBuffer, true,
								direction, startLocation, box,
								inverseDicomMatrix);
	...
	private void init() {
		...
		try {
			...
			if (mode == VOI_MODE) {
				...
				short[] tempImage = new short[destExtents[0] * destExtents[1]
						* destExtents[2]];
				...
				if (tempImage == null) {
					MipavUtil
							.displayError("Error when making mask image from VOI.");
				...
				maskImage = new ModelImage(ModelImage.USHORT, destExtents,
						"Surface image");
				maskImage.getFileInfo()[0].setResolutions(srcImage
			...
			MipavUtil
					.displayError("Algorithm extract surface: Image(s) locked");
			...
			MipavUtil
					.displayError("Dialog extract surface: unable to allocate enough memory");
