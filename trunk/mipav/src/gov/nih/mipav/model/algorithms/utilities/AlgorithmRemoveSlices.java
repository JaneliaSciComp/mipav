...
/**
 *     Algorithm verifies the z-th slice should be in included in the destination
 *     image, as defined in the list, remove.  It copies the slice from the srcImage img
 *     to a buffer, and then from the buffer into the destination img.  Copies the srcImage
 ...
 *
 *
 *       **(as of 1 Nov, does not yet process the more complicated DICOM
 *          images Completely)
...
public class AlgorithmRemoveSlices extends AlgorithmBase {
     ...
     *  The default (for most images) is 2.  DICOM images need to be checked.
     */
     ...
     *  @param  image   -- ModelImage whose offset is being computed
    ...
    /**
     *  Build and return the fileInfo for the current slice based on
     *  the source slice, the destination slice and time slice
     ...
     */
    private FileInfoBase getSliceFileInfo( int srcSlice, int destSlice, int t,
            int[] newExtents ) {
    ...
    /**
     *  Calculates and returns the new extents based on the source image and the
     *  list of slices to be removed.
     */
