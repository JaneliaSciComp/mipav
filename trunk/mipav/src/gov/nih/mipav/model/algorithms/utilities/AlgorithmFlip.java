...
public class AlgorithmFlip extends AlgorithmBase {
    ...
    *   @param nImages      Number of images to be flipped. If 2D image then nImage = 1,
    *                       if 3D or 4D image where each image is to processed independently
    ...
    */
    private void calcInPlace(int nImages){
                  ...
                  for (y = 0; y < yDim && !threadStopped; y++) {
                      for ( x = 0; x < xDim && !threadStopped; x++){
                          ...
                          if ((start+index)%mod==0 && isProgressBarVisible())
                              progressBar.updateValue(Math.round((float)(start+index)/(totalLength-1) * 100), activeImage);
        ...
        if (flipAxis == Y_AXIS) index = 0;
        else                    index = 1;

        float loc    = fileInfo[0].getOrigin(index);
        int   orient = fileInfo[0].getAxisOrientation(index);
        if (orient == FileInfoBase.ORI_L2R_TYPE || orient == FileInfoBase.ORI_S2I_TYPE || orient == FileInfoBase.ORI_P2A_TYPE) {
