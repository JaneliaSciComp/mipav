 ...
 * equivalent, but the AlgorithmEdgeLaplacianSep one has intensity values which are significantly lower than
 * AlgorithmEdgeLaplacian's. This, however, does not seem to affect (much) the edge crossing bitmasks that are generated
 * from the Lapacian image. Therefore, this version should be okay for use in functions which call calcZeroXMaskBitset()
 * (eg - RubberbandLivewire). Trying to figure out why this difference is occurring should still be a priority though (I
 * think it might be because AlgorithmEdgeLaplacian summed the dxx and dyy kernels and multiplied by -1 before
 * convolving, which might be affecting the magnitude of the convolved image (this algo convolves the image with both
 * dxx and dyy and then combines the result images like the other algo combines the kernels)).</p>
 *
...
public class AlgorithmEdgeLaplacianSep extends AlgorithmBase {
    ...
    public BitSet calcZeroXMaskBitset(float[] buffer, int[] extents) {
        ...
        AlgorithmSeparableConvolver xConvolver = new AlgorithmSeparableConvolver(buffer, extents,
                                                                                 GxxData, kExtents, false, Preferences.isMultiThreadingEnabled()); // assume not color
        ...
        AlgorithmSeparableConvolver yConvolver = new AlgorithmSeparableConvolver(buffer, extents,
                                                                                 GyyData, kExtents, false, Preferences.isMultiThreadingEnabled()); // assume not color
        ...
        for (int s = 0; (s < nImages) && !threadStopped; s++) {
            ...
            if ((max - min) > 1) {
                xConvolver = new AlgorithmSeparableConvolver(buffer,
                                                             ...
                                                             }, GxxData, kExtents, false, Preferences.isMultiThreadingEnabled());
            ...
            if ((max - min) > 1) {
                yConvolver = new AlgorithmSeparableConvolver(buffer,
                                                             new int[] {
                                                                 srcImage.getExtents()[0], srcImage.getExtents()[1]
                                                             }, GyyData, kExtents, false, Preferences.isMultiThreadingEnabled());
    ...
    private void calcStoreInDest3D(int detectionType) {
        ...
        if (Math.round(stepPerDimension) > 1) {
            xConvolver = new AlgorithmSeparableConvolver(buffer, srcImage.getExtents(), GxxData,
                                                         kExtents, false, Preferences.isMultiThreadingEnabled()); // assume not color
