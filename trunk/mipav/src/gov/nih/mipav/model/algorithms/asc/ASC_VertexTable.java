...
/**
 * A vertex-edge data structure that is used to store the vertices and edges that are extract for the six faces of a
 * voxel. The data structure supports the ear-clipping algorithm for locating triangles in the mesh. It also supports
 * the triangle fanning that is necessary to deal with crack avoidance on shared faces of two monoregions.
 */
...
public class ASC_VertexTable {
    ...
    /**
     * Get the vertex indices for the edges sharing vertex i. The indices are stored in the class member m_aiAdj[] and
     * the number of indices in this array is stored in the class member m_iAdjQuantity.
     *
    ...
    /**
     * Insert a point into the vertex array. This function is used for those vertices generated by subdividing edges to
     * avoid cracks. The vertex is appended to the array.
     *
    ...
    /**
     * Insert a point into the vertex array at the specified location. This function is used only for those vertices
     * that are on the edges of the cube (12 possibilities) or on the faces because of a branch point (6 possibilities).
     *
    ...
    /**
     * Remove the triangles from the vertex-edge mesh for a box using ear-clipping. This function is used when no
     * subdivision is required on box faces, that is, when no cracking occurs with the natural edges of intersection of
     * box and level surface.
     *
    ...
    /**
     * Remove the triangles from the vertex-edge mesh for a box that needed subdivision of edges to avoid cracking. The
     * simplest triangulation adds a new vertex, the centroid of the vertices on the box faces. The triangles are formed
     * by fanning with common origin at the centroid.
     *
     * @param  kVArray  the vertices of the triangles obtained by ear-clipping
     * @param  kTArray  the triangles obtained by ear-clipping
     */
    ...
    /**
     * Remove a point from the vertex array. The specified vertex must have only two edges sharing it (m_iAdjQuantity is
     * exactly 2).
     *
     * @param  i  the index of the vertex in m_akVertex (0 <= i < 18)
     */
    ...
    /**
     * Clip an ear from the vertex-edge mesh. The triple of indices for the removed triangle are stored in the class
     * members m_i0, m_i1, and m_i2.
     *
     * @return  'true' if and only if a triangle was removed. This function is called repeatedly until it returns
     *          'false', at which point all the triangles have been removed.
     */
    ...
    /**
     * A representation of a vertex in the vertex-edge mesh for a voxel. Each vertex stores an (x,y,z) triple for
     * location. The number of edges sharing the vertex is either 2 (an edge point) or 4 (a face center point), so the
     * adjacency array (of vertex indices) is allocated to the maximum of 4. The validity flag is used when removing
    ...
    public class Vertex {
        ...
        /** 'true' if and only if the vertex is still considered to be in the vertex-edge mesh. */
        boolean m_bValid;
