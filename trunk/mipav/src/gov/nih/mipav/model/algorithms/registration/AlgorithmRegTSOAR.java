...
/**
 *   This is an automatic time series registration method based on MCFLIRT.
 *   MCFLIRT stands for Motion Correction - FMRIB's Linear Image Registration Tool.
 *   For more information on MCFLIRT, visit their homepage at
 *   <a href="http://www.fmrib.ox.ac.uk/fsl/mcflirt/index.html">
 *   http://www.fmrib.ox.ac.uk/fsl/mcflirt/index.html</a>.  Their main paper is:
 *   <p>
 *   Jenkinson, M., Bannister, P., Brady, M., and Smith, S. (2002a).<br>
 ...
 *   <li>Take the subsampled by 8 image (note - it might be subsampled less if the subsampling makes the image too small). </li>
 *   <li>Do this at each level, with varying tolerances:
 *       <ol>
 *       <li>for each volume in time series, starting at refvol + 1 and going up, then starting at refvol - 1 and going down:</li>
 ...
 *       </ol>
 *   </li>
 *   <li>Do the previous with the image subsampled at 4, then again subsampled at 2, and finally at original
 *   size and with a new cost function of normalized cross correlation sinc if normalized cross
 *   correlation were the initial cost function.  The original MCFLIRT
 *   scheme had 1 round at subsampling 8 followed by 3 rounds at
 *   subsampling 4. </li>
 *   <li>The answer is stored as a list of matrices, giving the transformation from one volume to the next in the series.</li>
 *   </ol>
 *
...
public class AlgorithmRegTSOAR extends AlgorithmBase {
    ...
    /** Use NORMALIZED_CROSS_CORRELATION_SINC on nonsubsampled images
     in final iterations */
    private boolean normalizedCrossCorrelationSinc;

    /** if true output graphs for rotations and translations
     Only used for 6 DOF */
    ...
    /** Speed can be an issue. Therefore, if true preform final registration
     at the subsample level 2 */
    ...
    private final static int minimumZForSub = 16;
     ...
     *   @param _reference      <code>true</code> means register to reference volume, <code>false</code> means
     *                       register to average volume.
     *   @param _refNum the reference volume number
     *   @param _normalizedCrossCorrelationSinc if true use
     *                       NORMALIZED_CROSS_CORRELATION SINC on nonsubsampled
     *                       images in final iterations
     ...
     */
    public AlgorithmRegTSOAR( ModelImage image, int _costChoice, int _DOF,
            int _interp,
            boolean _reference, int _refNum,
            boolean _normalizedCrossCorrelationSinc,
        ...
        try {
            ...
            imageRef = new ModelImage( srcImage.getType(), volumeExtents, "Reference image", srcImage.getUserInterface() );
     ...
     *   <li>Rescale reference volume to isotropic voxels.</li>
     *   <li>Subsample the volume to a max subsampling of 8.</li>
     *   <li>Note: if the image is too small to subsample to 8, then the "subsampled by 8" image might actually only be subsampled
     *   by 4, and the "subsampled by 4" image will only be subsampled by 2, etc.</li>
     *   <li>Call register() on the subsampled by 8 image, then 3 times on the subsampled by 4 image.  The tolerances will be
     *   different on the first two calls with the subsampled by 4 image, and on the last call, the cost function will be
     *   different if cross correlation was the initial cost function.</li>
     *   <li>The answer is stored as a list of matrices, giving the transformation from one volume to the next in the series.</li>
     *   </ol>
     */
    public void runAlgorithm() {
        ...
        if ( resample ) {
            transform = new AlgorithmTransform( imageRef, new TransMatrix( 4 ), interp, volumeResolutionsIso[0],
                    volumeResolutionsIso[1], volumeResolutionsIso[2], volumeExtentsIso[0], volumeExtentsIso[1],
                    volumeExtentsIso[2], false, true, false );
        ...
        /* If only one subsampling occurs, only simpleRefSub8 is subsampled.
         If 2 subsamplings occur simpleRefSub8 is subsampled twice and simpleRefSub4
         is subsampled once. */
    ...
    /**
     *   Registers the reference volume to all the other volumes in the 4D time series.
     *   If the reference volume is the center volume, doesn't register the center volume
     *   (because they are the same).  Reslices the input volume to isotropic voxels and
     *   subsamples it if necessary.  Sets up the tolerances based on the given level.  At
     *   level 1, changes the cost function.
     ...
     *   @param previous List of previous transformation matrices.  Null for level 8, otherwise
     *                   the result of a previous call to this method.
     *   @return         List of transformation matrices, [0] to reference, [1] to reference, etc.
     */
    private MatrixListItem[] register( ModelSimpleImage refImage, int level,
            MatrixListItem[] previous ) {
        ...
        for ( int i = 0; i < nVolumes && !threadStopped; i++ ) {
            ...
            if ( i != refNum || reference == false ) {
                ...
                transform = new AlgorithmTransform( imageInput, answer[i].matrix, interpolation, volumeResolutions[0],
                        volumeResolutions[1], volumeResolutions[2], volumeExtents[0], volumeExtents[1], volumeExtents[2],
                        false, true, false );
                transform.setProgressBarVisible( false );
                transform.run();
    ...
    /**
     *   Takes a 4D image buffer and finds the average image
     *   volume from the 4D series.
            ...
            for ( i = 0; i < srcImage.getExtents()[0]; i++ ) {
                for ( j = 0; j < srcImage.getExtents()[1]; j++ ) {
                    for ( k = 0; k < srcImage.getExtents()[2]; k++ ) {
                        ...
                        loc = k * srcImage.getExtents()[1] * srcImage.getExtents()[0] + j * srcImage.getExtents()[0] + i;
                        image[loc] = sum / srcImage.getExtents()[3];
                    }
                }
            }
