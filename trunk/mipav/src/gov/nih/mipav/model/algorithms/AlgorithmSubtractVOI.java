...
/**
 *      Algorithm that subtracts the mean or median value of a VOI from
 *      the image.  If only 1 contour is present, that contour is used
 *      whether or not it is selected.  If multiple contours are present,
 *      at least 1 contour must be selected, and only selected contours
 *      are used.  Note that under MIPAV all selected contours must belong
 *      to the same VOI.  If the new image exceeds the range that
 *      can be stored in an image of that type the data is either clipped
 *      and stored in the original image. Or a new image of a type (int, float...)
 *      that can store the range of new data is generated.  For color the
 *      dialog currently disables promotion for color because MIPAV cannot
...
public class AlgorithmSubtractVOI extends AlgorithmBase {
    ...
    /**
     *   Minimum clipping modes if data result value exceeds
     *   the capacity of image's data type
     */
    private double clipMin;
    private float clipMinC;

    /**
     *   Maximum clipping modes if data result value exceeds
     *   the capacity of image's data type
     */
     ...
     */
    public AlgorithmSubtractVOI( ModelImage destImg, ModelImage srcImg,
            int _averageMode, int _clipMode ) {
     ...
     *   @param _clipMode clamp data to image type range or promote image type to hold new data range.
     */
    public AlgorithmSubtractVOI( ModelImage srcImg, int _averageMode,
            int _clipMode ) {
        super( null, srcImg );
        averageMode = _averageMode;
        clipMode = _clipMode;
     ...
     *  string to the message frame if the logging procedure is turned on.
     */
            ...
            if ( !Double.isInfinite( average ) && !Double.isNaN( average ) ) {
                if ( srcImage.isColorImage() ) {
                    if ( averageMode == MEDIAN ) {
                        Preferences.debug(
                                "median red = " + averageR + " median green = " + averageG + " median blue = "
                                + averageB, Preferences.DEBUG_ALGORITHM );
                    } else {
                        Preferences.debug(
                                "mean red = " + averageR + " mean green = " + averageG + " mean blue = " + averageB, Preferences.DEBUG_ALGORITHM );
                    }
                            ...
                            try {
                                if ( i % mod == 0 && isProgressBarVisible() ) {
                                    progressBar.updateValue(
                                            Math.round( (float) ( i + offset ) / ( totalLength - 1 ) * 50 ), activeImage );
                                }
     ...
     *   <p>
     *   <b>note</b> that if this method is canceled part-way through
     *   a multi-sliced image, it will almost certainly be partially
     *   adjusted.  This is because to save memory buffer-space,
     *   it returns an adjusted slice back to the source image after
     *   processing it; it then takes out the next slice to alter it.
     *   Canceling a job causes the current slice will not
     *   be affected, but all previous slices will be.
     */
                ...
                for ( j = 0; j < z && !threadStopped; j++ ) {
                    ...
                    for ( i = 0; i < length && !threadStopped; i++ ) {
                        try {
                            if ( i % mod == 0 && isProgressBarVisible() ) {
                                progressBar.updateValue(
                                        Math.round( 50 + (float) ( i + offset ) / ( totalLength - 1 ) * 50 ),
                                        activeImage );
                            }
                        } catch ( NullPointerException npe ) {
                            if ( threadStopped ) {
     ...
     *   <p>
     *   This method checks for thread halting actions more infrequently
     *   than the in-place method.
     */
