...
/**
 *      AlgorithmRegChamfer
 *      First slice is template (base image) to which match image
 *          is registered.
 *      Minimize cost, measures distance between surfaces in
 *          base image and match image.
...
public class AlgorithmRegChamfer extends AlgorithmBase {
        ...
        try {
            ...
            if ( image25D == true ) { //for 2.5D time series
                ...
                /*  tMatrixMatchtoBase = new TransMatrix[1];
                 tMatrixMatchtoBase[0] = new TransMatrix(DIM+1);
                 N = getNumberMatchPts();
                 Preferences.debug("N = "+N+"\n");
                 generateCoordList();
                 distanceTransform();
                 search(tMatrixMatchtoBase[0]);*/}
            if ( !threadStopped ) {
                setCompleted( true );
            }
        } catch ( IOException error ) {
     ...
     */
    private void transform25DMatchSlice( int matchSlice, TransMatrix xfrm,
            float[] imgBuf, float[] tImgBuf ) {
        ...
        try {
            ...
            AlgorithmTransform.transformBilinear( imgBuf, tImgBuf, xfrm, xdimM, ydimM,
                    volume25D.getFileInfo( 0 ).getResolutions()[0], volume25D.getFileInfo( 0 ).getResolutions()[1],
                    xdimM, xdimM, volume25D.getFileInfo( 0 ).getResolutions()[0],
    ...
    /**
     *   Chamfer matching: Distance transformation coverts binary surface
     *   image into grey-level image where each pixel's intensity =
     *   distance to nearest surface pixel
     */
    ...
    /**
     *  InitializePandY
     *   p[i][j] = each i is a simplex vertex; each j is a parameter tx, ty, or r
     *   y[i] = cost of position corresponding to row i of p[][]
     */
    ...
    /*
     private void transformSlice(int slice, TransMatrix xfrm,
     float imgBuf[],float tImgBuf[])
     {
     //int interp = AlgorithmTransform.NEAREST_NEIGHBOR;
     int interp = AlgorithmTransform.BILINEAR;
     float xres = volume.getFileInfo(0).getResolutions()[0];
     float yres = volume.getFileInfo(0).getResolutions()[1];
     float tres = volume.getFileInfo(0).getResolutions()[2];

     int sliceIndex = slice*sliceSize;
     try{
     volume.exportData(sliceIndex, sliceSize, imgBuf);//copy volume into 1D array

     AlgorithmTransform.transformNearestNeighbor2D(imgBuf, tImgBuf, xfrm,
     xdim, ydim, xres, yres);
    ...
    /**
     *   Class for specifying optimization function
     *   for AlgorithmSimplexOpt
     */
