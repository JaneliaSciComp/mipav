...
//**** SUPPLY ERROR MESSAGE TEXT FOR ERROR NUMBER IER
		   ...
		   for (IA=1; IA <= TNSUA; IA++) {
		       ...
		       jqloop: for (JQ=1; JQ <= NQ; JQ++) {
		           ...
		           if (Math.abs(XI[1]) < PTHTL && Math.abs(XI[0]) < 1.0+PTHTL) {
		               ...
		               // CONTINUATION OF THE BOUNDARY CORRESPONDENCE FUNCTION TO 
		               // ESTIMATE CANPT.
		               ...
		               PPSBI1(XI,BETA,NQPTS,DEG,AIN,BIN,
		               ...
		               for (I=1; I <= QINTS[0]; I++) {
		                    ...
		                    if (I == 1) {
		                        ...
		                        for (J=1; J <= NQPTS; J++) {
		                            ...
		                            WSPEC[K-1]=RRB*QUWTS[J1-1]*JACSUM(TT,DEG,AIN,BIN,H0VAL[AJT-1],JACOF);
		                            TT=TT*SS;
		                            TSPEC[K-1]=MD+TT*HL;
		                            CT[0]=TSPEC[K-1];
		                            CT[1] = 0.0;
   ...
   private double ARGIN1(double RT1, double RT2, int PT, double DIFF1[], double DIFF2[],
		   double ZZ[], double LIMIT) {
       // INTEGER PT
       // REAL RT1,RT2,LIMIT
   ...
   private double[] CJACSU(double X[], int N, double A[], double B[], double H,
		   double CO[]) {
       // INTEGER N
       // REAL A(*),B(*),H,CO(*)
       // COMPLEX X
   ...
   private void PPSBI1(double ZZ[], double BETA, int NQUAD, int DGPOL, double ACOEF[], double BCOEF[],
       double H0VAL, double SOLUN[][], double TOLIN, double TOLOU[], double XENPT[], int QINTS[],
       int MQIN1, int IER[]) {
	   ...
	   // COMPLEX SOLUN(*),ZZ
		...
		if (XENPT[1] < 1.0) {
		    ...
		    RIGHT[0]=-1.0;
		    FIRST=true;
		    DEPPL9(ZZIN,1,BETA,RIGHT,TAU,T1FXD,NQUAD,DGPOL,ACOEF,
		    ...
		    while (true) {
		        ...
		        if (XENPT[QINTS[0]-1] > RIGHT[0]) {
		          ...
		          FIRST=true;
		          TAU1[0] = XENPT[QINTS[0]-1];
		          DEPPL9(ZZIN,1,BETA,TAU1,TAU,T1FXD,NQUAD,DGPOL,
   ...
   private void GQCANP(double RWORK[], int IER[]) {
    ...
    private void CNDPLT(boolean MAP11[], double RESMN[], double UPHYC[], double UCANP[], double CRRES,
    	...
    	DIAGN4(CCAPH,COCAP,COPHC,CPHCA,EXCAP,EXPHC,ICRVS,IER,
    	     ...
    	     ACOEF,BICOF,BCFSN,BCOEF,H0VAL,
    	     HIVAL,HALEN,JACIN,MIDPT,SOLUN,
    	     LODP,LODW);
    	      for (I = 0; I < NQPTS; I++) {
    ...
    private void DIAGN4(double CCAPH[], double COCAP[], double COPHC[], double CPHCA[], double EXCAP[],
    	...
    	double AL,BT,CC,COF,D,H0,HH,MCHEP,MPT,
    	     PHI,SEND,SINC,SJT,SS,SUM,TERM,TINC,TT,TUPI,X,XX,YMAX,
    	     YMIN,YY;
    ...
    private void DMCANP(int NPTS, double PHYPT[][], double CANPT[][],
    	...
    	CATPH4(NPTS,PHYPT,CANPT,NARCS,NQPTS,TNSUC,DGPOC,
    	     JTYPC,LSUBC,LQSBG,NPPQG,PARNT,
    	     PRNSA,AICOC,ACOFC,BICOC,BCOFC,
    	     H0VLC,HIVLC,HALEN,JAINC,RGEOM[1],
    	     MIDPT,PHPAS,QUPTC,QUWTC,VTARG[0],
    	     VARGC,BFSNC,CENTR,ZSNCA,SOLNC,
    	     WPPQG,ZPPQG,INTER,IER);
    ...
    private void CATPH4(int NPTS, double PHYPT[][], double CANPT[][], int NARCS, int NQPTS,
        int TNSUC, int DGPOC[], int JTYPC[], int LSUBC[], int LQSBG[], int NPPQG[],
        int PARNT[], int PRNSA[], double A1COC[], double ACOFC[], double B1COC[], double BCOFC[],
        double H0VLC[], double H1VLC[], double HALEN[], double JAINC[], double LGTOL,
        double MIDPT[], double PHPAS[], double QUPTC[], double QUWTC[], double THET0,
    	double VARGC[], double BFSNC[][], double CENTR[], double FACTR[], double SOLNC[][],
    	double WPPQG[][], double ZPPQG[][], boolean INTER, int IER[]) {
    	...
    	loopIP: for (IP=1; IP <= NPTS; IP++) {
    	    ...
    	    for (IA=1; IA <= TNSUC; IA++) {
    	        ...
    	        if (DIST >= DELTA) {
    	            ...
    	            // WW IS PATHOLOGICALLY CLOSE TO ARC IA (OR MAYBE JUST CLOSE TO
    	            // ARC IA AND APPARENTLY OUTSIDE THE CANONICAL DOMAIN) AND WE 
    	            // USE THE CONTINUATION OF THE BOUNDARY CORRESPONDENCE FUNCTION
    	            // TO ESTIMATE PHYPT.
