 ...
 * has the same dimensions as the input target image.  A 2D B-Spline is
 ...
 * spaced equally for each dimension.  Bilinear interpolation is used to
 ...
 * restricted from moving outside the polygon formed by its 8 neighboring
 * control points.
 */
public class BSplineRegistration2Df
    ...
    public BSplineRegistration2Df createSameMapping(ModelSimpleImage kImageTrg) {
        ...
        BSplineRegistration2Df kReg = new BSplineRegistration2Df(
            this.m_kImageSrc,
            kImageTrg,
            kBasisX,
            kBasisY,
     ...
     */
    public BSplineRegistration2Df createSameMapping(ModelSimpleImage kImageTrg,
        BSplineBasisf kBasisX, BSplineBasisf kBasisY) {
    ...
    /**
     * Return access to the 2D discretized B-Spline internally created
     ...
     * sample contains the 2D interpolated point coordinates in the
     ...
     * y coordinate for the map.  Each image has the same dimensions which
     * are that of the target image.
     */
    public ModelSimpleImage[] createImageSourceMap() {

        ModelSimpleImage[] akSourceMap = m_kBSpline2D.createImageMap(
        ...
        for (int iY = 0; iY < m_kBSplineBasisY.getNumSamples(); iY++) {
            ...
            for (int iX = 0; iX < m_kBSplineBasisX.getNumSamples(); iX++) {
                ...
                kImageDisplacement.data[iIndex] =
                    (float)Math.sqrt(fDX*fDX+fDY*fDY);
            }
        }
        akSourceMap = null;
     ...
     * location.  The requested location must be inside the polygon formed
     * by its 8 neighboring control points.  If not, then a line segment
     * is created between the current and requested locations and the point
     * where this line segment intersects the bounding polygon is found.
     * This point is offset a little along the line segment so that it
     * is just inside the bounding polygon, and this point is taken as
     ...
     * @param kPoint Point2f Requested new coordinates for the specified
     * control point.
    ...
    public float moveControlPoint(int iControlX, int iControlY, Point2f kPoint) {
        ...
        fDist =
            getControlPointMaxMoveDist(iControlX, iControlY, kDirection, fDist);
     ...
     * the bounding polygon formed by the 8 neighboring control points
     * is reached.
     * @param iControlX int Identifies the control point in the 2D lattice.
     * @param iControlY int Identifies the control point in the 2D lattice.
    ...
    public void minimizeControlPoint(int iControlX, int iControlY, int iMaxSteps, float fStepSize) {
        ...
        fMaxDist =
            getControlPointMaxMoveDist(iControlX, iControlY,
                                       kDirection, fMaxDist);
    ...
    protected void updateSamples(int iMinX, int iMaxX, int iMinY, int iMaxY) {
        ...
        for (int iY = iMinY; iY <= iMaxY; iY++) {
            for (int iX = iMinX; iX <= iMaxX; iX++) {
                ...
                float fRegSrcY0 =
                    m_kImageSrc.data[iX0Y0] * fX0 +
                    m_kImageSrc.data[iX1Y0] * fX1;
                float fRegSrcY1 =
                    m_kImageSrc.data[iX0Y1] * fX0 +
                    m_kImageSrc.data[iX1Y1] * fX1;
     ...
     * @param iControlX int Identifies the control point in the 2D lattice.
     * @param iControlY int Identifies the control point in the 2D lattice.
     * @param kDeriv Vector2f Vector to be filled with the derivative
     ...
     * its 8 neighboring control points such that "folding" does not occur.
     * @param iControlX int Identifies the control point in the 2D lattice.
     * @param iControlY int Identifies the control point in the 2D lattice.
     * @param kRayDirection Vector2f Direction from the current control
     * point location for finding the distance to the bounding polygon.
    ...
    protected float getControlPointMaxMoveDist(
        int iControlX, int iControlY, Vector2f kRayDirection,
        ...
        for (int iPolyPoint = 0; iPolyPoint < 8; iPolyPoint++) {
            ...
            m_kBSpline2D.getControlPoint(
                iControlX + m_scControlPointPolygonX[iPolyPoint],
                iControlY + m_scControlPointPolygonY[iPolyPoint],
                kP0);
            m_kBSpline2D.getControlPoint(
                iControlX + m_scControlPointPolygonX[iPolyPoint+1],
                iControlY + m_scControlPointPolygonY[iPolyPoint+1],
                kP1);
        ...
        while ((fMaxDesiredDist*0.01) < (fIterMoveDistMax-fIterMoveDistMin)) {
            ...
            for (int iPolyPoint = 0; iPolyPoint < 8; iPolyPoint++) {
                ...
                float fDet =
                    a0*b1 - a1*b0 + a1*c0 - a0*c1 + b0*c1 - b1*c0;
                if (fDet < 0.0f) {
                    bFolding = true;
                    break;
                }
    ...
    /**
     * Defines the relative offset in control point indices to the 8
     * neighboring control points which form a bounding polygon.
