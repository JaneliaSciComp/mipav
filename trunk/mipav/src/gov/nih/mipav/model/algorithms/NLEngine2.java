...
import gov.nih.mipav.view.Preferences;
 ...
 /**
  *  This is a port of code from the companion website to the book
  *  Data Reduction and Error Analysis for the Physical Sciences
  *  Third Edition by Philip R. Bevington and D. Keith Robinson,
  *  McGraw-Hill, 2003.  The code covers Chapter 8 Least-Squares
  *  Fit to an Arbitrary Function.  The website is www.mhhe.com/bevington.
  */

  /*Sample usage folows:
  class FitReferenceModel extends NLEngine2 {

    public FitReferenceModel(int nPoints, double xData[], float yData[],
                               double initial[]) {
        ...
        double ymod = 0;
                  try {
                      ymod = atry[0] + atry[1]*Math.exp(atry[2] * x1);
                        ...
                        dyda[2] = atry[1]*atry[2]*Math.exp(atry[2] * x1);
                }
                catch (Exception e) {
...
/*class FitDoubleExponentialModel extends NLEngine2 {
    ...
    */
    /*public double fitToFunction(double x1, double atry[], double dyda[]) {
        ...
        double ymod = 0;
                  try {
                      ymod = 1 - atry[0]*Math.exp(atry[1]*x1)
                               - (1 - atry[0])*Math.exp(atry[2] * x1);
                      // a0 partial derivative
                        dyda[0] = -Math.exp(atry[1]*x1) + Math.exp(atry[2]*x1);
                        // a1 partial derivative
                        dyda[1] = -atry[0]*atry[1]*Math.exp(atry[1] * x1);
                        // a2 partial derivative
                        dyda[2] = -(1 - atry[0])*atry[2]*Math.exp(atry[2] * x1);
                }
                catch (Exception e) {
                        Preferences.debug("function error: " + e.getMessage() + "\n");
                }
   ...
   /* public void dumpResults() {
        ...
        Preferences.debug("Chi-squared: " + String.valueOf(chisq) + "\n");
        Preferences.debug("a0 " + String.valueOf(a[0]) +
                          " +- " + String.valueOf(siga[0]) + "\n");
        Preferences.debug("a1 " + String.valueOf(a[1]) +
                          " +- " + String.valueOf(siga[1]) + "\n");
        Preferences.debug("a2 " + String.valueOf(a[2]) +
                          " +- " + String.valueOf(siga[2]) + "\n");
    }
}*/
...
public abstract class NLEngine2 {
	...
	public final static int GRID_SEARCH = 1;
    public final static int GRADIENT_SEARCH = 2;
    public final static int CHISQ_EXPANSION = 3;
    public final static int MARQUARDT = 4;
    ...
    */
	public void driver() {
	    ...
	    try {
	        ...
	        System.out.println("degrees of freedom = " + nFree);
	        System.out.println("chisq/dof = " + chi2PerDof);
	        System.out.println("prob = " + x2Prob);
	        System.out.println("Fitted parameters: a[i] +- siga[i]");
	        for (i = 0; i < nParams; i++) {
	            System.out.println("a[" + i + "] = " + a[i] + " +- " + siga[i]);
	        }
	        if ((fitMethod == CHISQ_EXPANSION) || (fitMethod == MARQUARDT)) {
	            System.out.println("Error matrix");
	            for (i = 0; i < nParams; i++) {
	                String varLine = "";
	                for (j = 0; j < nParams; j++) {
	                    varLine = varLine + " " + alpha[i][j];
	                }
...
private double	chiProb( double nFree, double chi2)
{
	...
	if (nFree == 1)
	{
		if (chi2 < cLim)						// Integrate expansion of the function
			probTem = 1.0-Math.sqrt(chi2/2./Math.PI)*
			          (2. - chi2*(1./3. - chi2*(1.0/20. - chi2*(1./168. - chi2/1728.))));
		else
...
private double simpson(char functCode,int nIntervals, double loLim, double hiLim, double p1, double p2, double p3)   //2 calcs/interval
{
	double sum = 0.0;
	double x,dx,      t1,t2;
	int i;
...
{	double tem;
	tem = gamma((nFree+1.)/2.)/gamma(nFree/2.) * Math.exp((nFree-2.)/2. *
	                           Math.log(1. - r*r))/Math.sqrt(Math.PI);
	return(tem);
}
...
*  @param dummy
*/
private double studentT_Dens(double  t, double nu, double g, double dummy) //Student's t function. See Stud_Prob for g.
{
	double  x;
