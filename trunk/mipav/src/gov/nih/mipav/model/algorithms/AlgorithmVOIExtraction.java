...
*/
    ...
    /**
     * Calculates the VOI extraction.
     * voi -> mask -> voi -> mask looses 1 corner pixel from nonboundary touching square
     * once seen in lower left and once seen in upper right
     * No loss seen in more calculated calcInPlace
     */
        ...
        for (z = 0; z < zDim; z++) {
            ...
            for (short obj = 1; obj < (srcImage.getMax() + 1); obj++) {
                ...
                for (y = 1; y <= yDim - 2; y++) {
                    for (x = 1; x <= xDim - 2; x++) {
                        present = true;
                        for (yinc = -1; (yinc <= 1) && present; yinc++) {
                            for (xinc = -1; (xinc <= 1) && present; xinc++) {
                                if (objBuffer[x + xinc + xDim*(y + yinc)] != 1) {
                                    present = false;
                                }
                            }
                        }
                        if (present) {
                            erodeBuffer[x + y * xDim] = 1;
                        }
                    } // for (x = 1; x <= xDim - 1; x++)
                } // for (y = 1; y <= yDim - 1; y++)
                
                for (i = 0; i < length; i++) {
                    if ((objBuffer[i] == 1) && (erodeBuffer[i] == 0)) {
                        mask.set(i);
                    }
                }
                
                haveStartPt = false;
                for (i = 0; (i < length) && (!haveStartPt); i++) {
                    if (mask.get(i)) {
                        haveStartPt = true;
                        y = i / xDim;
                        x = i % xDim;
                ...
                if (newGrayScale) {
                    grayScaleArray[grayScaleNumber] = obj;
                    short voiID = (short) (obj - 1);
                    addedVOI = new VOI(voiID, "VOI" + voiID, VOI.CONTOUR, -1.0f);
