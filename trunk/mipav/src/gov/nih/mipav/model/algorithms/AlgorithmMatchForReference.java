...
/**
 * Will take Two 2D images (one Reference, one Adjusted) and
 * use the Reference image's Resolutions to transform the adjusted Image into a
 * new size and resolution that the Reference Image can then be matched to, while
 * preserving all Field of Views (there is no loss of FOV)
 * @author Ben Link
 * @version 1.0
 */
    ...
    public void runAlgorithm() {
        ...
        if ( refImage.isColorImage() && !adjImage.isColorImage() ) {
            ...
            ModelImage tempImage = new ModelImage( ModelImage.ARGB, adjImage.getExtents(),
                    JDialogBase.makeImageName( adjImage.getImageName(), "_rgb" ), userInterface );
            ...
            if ( adjImage.getType() == ModelStorageBase.ARGB ) {
                tempImage = new ModelImage( ModelImage.UBYTE, adjImage.getExtents(),
                        ( adjImage.getImageName() + "Gray" ), userInterface );
            } else if ( adjImage.getType() == ModelStorageBase.ARGB_USHORT ) {
                tempImage = new ModelImage( ModelImage.USHORT, adjImage.getExtents(),
                        ( adjImage.getImageName() + "Gray" ), userInterface );
            } else if ( adjImage.getType() == ModelStorageBase.ARGB_FLOAT ) {
                tempImage = new ModelImage( ModelImage.FLOAT, adjImage.getExtents(),
                        ( adjImage.getImageName() + "Gray" ), userInterface );
            }
