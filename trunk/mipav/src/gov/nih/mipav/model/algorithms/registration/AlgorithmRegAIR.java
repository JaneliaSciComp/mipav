...
public class AlgorithmRegAIR extends AlgorithmBase {
    // This software is a port of portions of version 5.21 of the
    // Automated Image Registration (AIR) package found at the
    // AIR WWW site (currently http://bishopw.loni.ucla.edu/AIR5/.
    // References:
    // 1.) Woods RP, Grafton ST, Holmes CJ, Cherry SR, Mazziotta JC.
    //     Automated image registration: I. General methods and intrasubject,
    //     intramodality validation.  Journal of Computer Assisted
    //     Tomography 1998; 22:141-154.
    // 2.) Woods RP, Grafton ST, Watson JDG, Sicotte NL, Mazziotta JC.
    //     Automated image registration: II. Intersubject validation
    //     of linear and nonlinear methods.  Journal of Computer Assisted
    //     Tomography 1998; 22:155-165.
    /**
     *  If you are working with two dimensional data, be sure to specify a
     *  voxel z size that is greater than or equal to the voxel x and y
     *  sizes.  Otherwise, interpolation to the irrelevant voxel z size will
     *  occur in order to generate cubic voxels.
     */
    ...
    // 2 for least squares
    // 3 for least squares with intensity rescaling
    // 4 for correlation ratio
    // Only standard deviation of ratio image or correlation ratio can be used
    // with intermodality registration - 1 PET and 1 MRI.
     ...
     *   @param costFxn  1 for standard deviation of ratio image, 2 for least squares
     *                   3 for least squares with intensity rescaling
     *                   4 for correlation ratio
     ...
     */
    public AlgorithmRegAIR( ModelImage resultImage, ModelImage targetImage, boolean entireTarget,
            ModelImage sourceImage, boolean entireSource, int transformation,
            int interpolation, float sourceThreshold, float targetThreshold,
            float sourceGaussX, float sourceGaussY, float sourceGaussZ,
            float targetGaussX, float targetGaussY, float targetGaussZ,
            int sourcePartitions, int targetPartitions, int costFxn,
            boolean cubicInterpolation, boolean interaction,
     ...
     *   @param iterations  maximum number of iterations at a given sampling density
     */
    public AlgorithmRegAIR( ModelImage resultImage, ModelImage targetImage, boolean[] targetMask,
            ModelImage sourceImage, boolean[] sourceMask, int transformation,
            int interpolation, float sourceThreshold, float targetThreshold,
            float sourceGaussX, float sourceGaussY, float sourceGaussZ,
            float targetGaussX, float targetGaussY, float targetGaussZ,
            int sourcePartitions, int targetPartitions, int costFxn,
    ...
    public void runAlgorithm() {
        ...
        if ( ( targetImage.getNDims() > 2 )
                ...
                || ( targetUnitsX != sourceUnitsX ) || ( targetUnitsX != sourceUnitsY )
                || ( targetUnitsX != sourceUnitsZ ) ) ) {
            doConvert = true;
        ...
        if ( doConvert ) {
            ...
            if ( ( sourceUnitsX == FileInfoBase.INCHES ) || ( sourceUnitsX == FileInfoBase.CENTIMETERS )
                ...
                if ( sourceVoxelX == 0.0f ) {
                    MipavUtil.displayWarning(
                            "source x resolution was recorded as zero " + " It is being changed to 1.0" );
                    sourceVoxelX = 1.0f;
                } else if ( sourceVoxelX < 0.0f ) {
                    MipavUtil.displayWarning(
                            "source X resolution was recorded as " + sourceVoxelX + " It is being changed to "
                            + -sourceVoxelX );
            ...
            if ( ( sourceUnitsY == FileInfoBase.INCHES ) || ( sourceUnitsY == FileInfoBase.CENTIMETERS )
                ...
                if ( sourceVoxelY == 0.0f ) {
                    MipavUtil.displayWarning(
                            "source y resolution was recorded as zero " + " It is being changed to 1.0" );
                    sourceVoxelY = 1.0f;
                } else if ( sourceVoxelY < 0.0f ) {
                    MipavUtil.displayWarning(
                            "source Y resolution was recorded as " + sourceVoxelY + " It is being changed to "
                            + -sourceVoxelY );
            ...
            if ( inPlane ) {
                ...
                if ( sourceVoxelZ == 0.0f ) {
                    MipavUtil.displayWarning(
                            "source z resolution was recorded as zero " + " It is being changed to 1.0" );
                    sourceVoxelZ = 1.0f;
                } else if ( sourceVoxelZ < 0.0f ) {
                    MipavUtil.displayWarning(
                            "source Z resolution was recorded as " + sourceVoxelZ + " It is being changed to "
                            + -sourceVoxelZ );
            ...
            if ( ( targetUnitsX == FileInfoBase.INCHES ) || ( targetUnitsX == FileInfoBase.CENTIMETERS )
                ...
                if ( targetVoxelX == 0.0f ) {
                    MipavUtil.displayWarning(
                            "target x resolution was recorded as zero " + " It is being changed to 1.0" );
                    targetVoxelX = 1.0f;
                } else if ( targetVoxelX < 0.0f ) {
                    MipavUtil.displayWarning(
                            "target X resolution was recorded as " + targetVoxelX + " It is being changed to "
                            + -targetVoxelX );
            ...
            if ( ( targetUnitsY == FileInfoBase.INCHES ) || ( targetUnitsY == FileInfoBase.CENTIMETERS )
                ...
                if ( targetVoxelY == 0.0f ) {
                    MipavUtil.displayWarning(
                            "target y resolution was recorded as zero " + " It is being changed to 1.0" );
                    targetVoxelY = 1.0f;
                } else if ( targetVoxelY < 0.0f ) {
                    MipavUtil.displayWarning(
                            "target Y resolution was recorded as " + targetVoxelY + " It is being changed to "
                            + -targetVoxelY );
            ...
            if ( inPlane ) {
                ...
                if ( targetVoxelZ == 0.0f ) {
                    MipavUtil.displayWarning(
                            "target z resolution was recorded as zero " + " It is being changed to 1.0" );
                    targetVoxelZ = 1.0f;
                } else if ( targetVoxelZ < 0.0f ) {
                    MipavUtil.displayWarning(
                            "target Z resolution was recorded as " + targetVoxelZ + " It is being changed to "
                            + -targetVoxelZ );
        ...
        if ( ( sourceGaussX != 0.0f ) || ( sourceGaussY != 0.0f ) || ( sourceGaussZ != 0.0f ) ) {
            try {
                ...
                MipavUtil.displayError(
                        "Algorithm Registration Air: unable to allocate enough memory for sourceGaussianImage" );
                disposeProgressBar();
                setCompleted( false );
                return;
            }
        ...
        if ( ( targetGaussX != 0.0f ) || ( targetGaussY != 0.0f ) || ( targetGaussZ != 0.0f ) ) {
            try {
                ...
                finalize();
                MipavUtil.displayError(
                        "Algorithm Registration Air: unable to allocate enough memory for targetGaussianImage" );
                disposeProgressBar();
                setCompleted( false );
                return;
            }
        ...
        if ( direction > -1 ) {
            if ( !cubicInterpolation ) {
                ...
                targetCubicXDim = (int) Math.floor(
                        ( targetXDim - 1 ) * ( targetVoxelX / targetCubicVoxelX ) + 1 + Float.MIN_VALUE );
                targetCubicYDim = (int) Math.floor(
                        ( targetYDim - 1 ) * ( targetVoxelY / targetCubicVoxelY ) + 1 + Float.MIN_VALUE );
                if ( !inPlane ) {
                    targetCubicZDim = (int) Math.floor(
                            ( targetZDim - 1 ) * ( targetVoxelZ / targetCubicVoxelZ ) + 1 + Float.MIN_VALUE );
                } else {
                    targetCubicZDim = targetZDim;
                }
                for ( j = 0; j < 4; j++ ) {
        ...
        if ( direction < 1 ) {
            if ( !cubicInterpolation ) {
                ...
                sourceCubicXDim = (int) Math.floor(
                        ( sourceXDim - 1 ) * ( sourceVoxelX / sourceCubicVoxelX ) + 1 + Float.MIN_VALUE );
                sourceCubicYDim = (int) Math.floor(
                        ( sourceYDim - 1 ) * ( sourceVoxelY / sourceCubicVoxelY ) + 1 + Float.MIN_VALUE );
                if ( !inPlane ) {
                    sourceCubicZDim = (int) Math.floor(
                            ( sourceZDim - 1 ) * ( sourceVoxelZ / sourceCubicVoxelZ ) + 1 + Float.MIN_VALUE );
                } else {
                    sourceCubicZDim = sourceZDim;
                }
        ...
        case FIXED_DETERMINANT2D:
            uv2D5( tps, es, des, ees, sourceVoxelX, sourceVoxelY, sourceVoxelZ, targetVoxelX, targetVoxelY, targetVoxelZ,
                    cubicInterpolation );
            break;
                ...
                if ( direction == 0 ) {
                    ...
                    case UVDERIVSN6:
                        cf = uvderivsN6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, q1, q2, q3, q4, q5, q6, q7, targetPartitions, max5 );
                        break;

                    case UVDERIVSN12:
                        cf = uvderivsN12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, q1, q2, q3, q4, q5, q6, q7, targetPartitions, max5 );
                        break;

                    case QVDERIVSN6:
                        cf = qvderivsN6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, q1, q2, q3, q4, q5, q7, targetPartitions, max5 );
                        break;

                    case QVDERIVSN12:
                        cf = qvderivsN12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, q1, q2, q3, q4, q5, q7, targetPartitions, max5 );
                        break;

                    case UVDERIVSLS6:
                        cf = uvderivsLS6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff );
                        break;

                    case UVDERIVSLS12:
                        cf = uvderivsLS12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff );
                        break;

                    case QVDERIVSLS6:
                        cf = qvderivsLS6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff );
                        break;

                    case QVDERIVSLS12:
                        cf = qvderivsLS12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff );
                        break;

                    case UVDERIVSRS6:
                        cf = uvderivsRS6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, tps[parameters], true );
                        break;

                    case UVDERIVSRS12:
                        cf = uvderivsRS12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, tps[parameters], true );
                        break;

                    case QVDERIVSRS6:
                        cf = qvderivsRS6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, tps[parameters], true );
                        break;

                    case QVDERIVSRS12:
                        cf = qvderivsRS12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, tps[parameters], true );
                        break;

                    case UVDERIVSCOR6:
                        cf = uvderivsCOR6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, q1, q2, q3, q4, q5, q6, q7, targetPartitions, max5 );
                        break;

                    case UVDERIVSCOR12:
                        cf = uvderivsCOR12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, q1, q2, q3, q4, q5, q6, q7, targetPartitions, max5 );
                        break;

                    case QVDERIVSCOR6:
                        cf = qvderivsCOR6( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim, targetThreshold,
                                dcff, ecff, q1, q2, q3, q4, q5, q7, targetPartitions, max5 );
                        break;

                    case QVDERIVSCOR12:
                        cf = qvderivsCOR12( parameters, es, des, ees, sampleFactor, sourceArray, sourceXDim, sourceYDim,
                                sourceZDim, pixel5, pixel5Mask, targetCubicXDim, targetCubicYDim, targetCubicZDim,
                                targetThreshold, dcff, ecff, q1, q2, q3, q4, q5, q7, targetPartitions, max5 );
                        break;
                    } // switch(derivsN)
                    ...
                    case UVDERIVSN6:
                        cf += uvderivsN6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, q1, q2, q3, q4, q5, q6, q7, sourcePartitions, max6 );
                        break;

                    case UVDERIVSN12:
                        cf += uvderivsN12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, q1, q2, q3, q4, q5, q6, q7, sourcePartitions, max6 );
                        break;

                    case QVDERIVSN6:
                        cf += qvderivsN6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, q1, q2, q3, q4, q5, q7, sourcePartitions, max6 );
                        break;

                    case QVDERIVSN12:
                        cf += qvderivsN12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, q1, q2, q3, q4, q5, q7, sourcePartitions, max6 );
                        break;

                    case UVDERIVSLS6:
                        cf += uvderivsLS6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr );
                        break;

                    case UVDERIVSLS12:
                        cf += uvderivsLS12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr );
                        break;

                    case QVDERIVSLS6:
                        cf += qvderivsLS6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr );
                        break;

                    case QVDERIVSLS12:
                        cf += qvderivsLS12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr );
                        break;

                    case UVDERIVSRS6:
                        cf += uvderivsRS6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, tps[parameters], false );
                        break;

                    case UVDERIVSRS12:
                        cf += uvderivsRS12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, tps[parameters], false );
                        break;

                    case QVDERIVSRS6:
                        cf += qvderivsRS6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, tps[parameters], false );
                        break;

                    case QVDERIVSRS12:
                        cf += qvderivsRS12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, tps[parameters], false );
                        break;

                    case UVDERIVSCOR6:
                        cf += uvderivsCOR6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, q1, q2, q3, q4, q5, q6, q7, sourcePartitions, max6 );
                        break;

                    case UVDERIVSCOR12:
                        cf += uvderivsCOR12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, q1, q2, q3, q4, q5, q6, q7, sourcePartitions, max6 );
                        break;

                    case QVDERIVSCOR6:
                        cf += qvderivsCOR6( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim, sourceThreshold,
                                dcfr, ecfr, q1, q2, q3, q4, q5, q7, sourcePartitions, max6 );
                        break;

                    case QVDERIVSCOR12:
                        cf += qvderivsCOR12( parameters, er, der, eer, sampleFactor, targetArray, targetXDim, targetYDim,
                                targetZDim, pixel6, pixel6Mask, sourceCubicXDim, sourceCubicYDim, sourceCubicZDim,
                                sourceThreshold, dcfr, ecfr, q1, q2, q3, q4, q5, q7, sourcePartitions, max6 );
                        break;
                    } // switch(derivsN)
     ...
     * @param cubicInterpolation
     */
    private void rigid_2D_rotatef( double[] e, double[][] de, double[][][] ee, double[] tps,
            float xoom1, float xoom2, float yoom1, float yoom2, double pixel_size1,
            double pixel_size2, int x_dim1, int x_dim2, int y_dim1, int y_dim2, boolean cubicInterpolation ) {
     ...
     * @param cubicInterpolation
     */
    private void gscale2D_rotatef( double[] e, double[][] de, double[][][] ee, double[] tps, float xoom1,
            float xoom2, float yoom1, float yoom2, double pixel_size1, double pixel_size2,
            int x_dim1, int x_dim2, int y_dim1, int y_dim2, boolean cubicInterpolation ) {
    ...
    void uv2D5( double[] tps, double[][] e, double[][][] de, double[][][][] ee,
            float x_size2, float y_size2, float z_size2, float x_size1,
            float y_size1, float z_size1, boolean cubicInterpolation ) {
    ...
    void uv3D6( double[] tps, double[][] e, double[][][] de, double[][][][] ee, int x_dim2, int y_dim2, int z_dim2,
            float x_size2, float y_size2, float z_size2, int x_dim1, int y_dim1, int z_dim1,
     ...
     * @param cubicInterpolation
     */
    void rigid_rotatef( double[] e, double[][] de, double[][][] ee, double[] tps, float xoom1, float xoom2,
            float yoom1, float yoom2, float zoom1, float zoom2,
            double pixel_size1, double pixel_size2, int x_dim1, int x_dim2, int y_dim1, int y_dim2,
            int z_dim1, int z_dim2, boolean cubicInterpolation ) {
    ...
    private void uv3D7( double[] tps, double[][] e, double[][][] de, double[][][][] ee, int x_dim2, int y_dim2, int z_dim2,
            float x_size2, float y_size2, float z_size2, int x_dim1, int y_dim1, int z_dim1,
            float x_size1, float y_size1, float z_size1, boolean cubicInterpolation ) {
     ...
     * @param cubicInterpolation
     */
    void gscale_rotatef( double[] e, double[][] de, double[][][] ee, double[] tps,
            float xoom1, float xoom2, float yoom1, float yoom2, float zoom1, float zoom2,
            double pixel_size1, double pixel_size2, int x_dim1, int x_dim2, int y_dim1,
            int y_dim2, int z_dim1, int z_dim2, boolean cubicInterpolation ) {
    ...
    /**
     *
     * @param tps pitch,roll,yaw,p,q,r,talax,ralay,talaz in radians, half-pixels, and dimensionless scalings
     * @param e corresponding forward unit vectors
     * @param de 1st derivatives of unit vectors with respect to pitch,roll,yaw,p,q,r,talax,talay,talaz
     * @param ee 2nd derivatives of unit vectors with respect to pitch,roll,yaw,p,q,r,talax,talay,talaz
     * @param x_dim2
     * @param y_dim2
     * @param z_dim2
     * @param x_size2
     ...
     * @param cubicInterpolation
     */
    void tala_rotatef( double[] e, double[][] de, double[][][] ee, double[] tps,
            float xoom1, float xoom2, float yoom1, float yoom2, float zoom1, float zoom2,
            double pixel_size1, double pixel_size2, int x_dim1, int x_dim2, int y_dim1,
            int y_dim2, int z_dim1, int z_dim2, boolean cubicInterpolation ) {
    ...
    double uvderivsN6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff, int[] count,
            double[] mean, double[] square, double[][] dmean, double[][] dsquare,
     ...
     * @param max_actual_value
     * This routine computes the first and second derivatives
     *  of the correlation ratio with respect to all
     *  external parameters.
     *
     * The matrix indices have been adjusted to allow for a more
     *  orderly relationship between the matrix and its elements
     *
     * Returns the correlation ratio
     */

    double uvderivsCOR6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff, int[] count,
            double[] mean, double[] square, double[][] dmean, double[][] dsquare,
    ...
    double uvderivsN12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
        ...
        for ( r = 0; r < rTerm; r += sampleFactor ) {
            ...
            /* Notice that dx[0], dx[1], dx[2], dx[3] are equal to dy[4], dy[5], dy[6], dy[7]
             and to dz[8], dz[9], dz[10], dz[11].  Consequently, these are all treated as
            ...
            pix4 = n0 * d * e * f + n1 * a * e * f + n2 * d * b * f + n3 * a * b * f + n4 * d * e * c + n5 * a * e * c
                    + n6 * d * b * c + n7 * a * b * c;

            /*Some intermediate values needed to calculate derivatives efficiently*/

            dxpix4 = ( ( e * f ) * ( n1 - n0 ) + ( b * f ) * ( n3 - n2 ) + ( c * e ) * ( n5 - n4 )
                    + ( b * c ) * ( n7 - n6 ) );
            dypix4 = ( ( d * f ) * ( n2 - n0 ) + ( a * f ) * ( n3 - n1 ) + ( c * d ) * ( n6 - n4 )
                    + ( a * c ) * ( n7 - n5 ) );
            dzpix4 = ( ( d * e ) * ( n4 - n0 ) + ( a * e ) * ( n5 - n1 ) + ( b * d ) * ( n6 - n2 )
                    + ( a * b ) * ( n7 - n3 ) );
     ...
     * @param maxActualValue
     * This routine computes the first and second derivatives
     *  of the correlation raio with respect to all
     *  external parameters.
     *
     * The matrix indices have been adjusted to allow for a more
     *  orderly relationship between the matrix and its elements
    ...
    double uvderivsCOR12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            int[] count, double[] mean, double[] square, double[][] dmean, double[][] dsquare,
     ...
     * @param threshold5
     * @param dcff first derivatives of sd with respect to parameters
     * @param esdf second derivatives of sd with respect to parameters
     * @param count number of pixels in partitions that are above threshold
     * @param mean ratio of pixel in source file/corresponding pixel in target file
     * @param square intermediate computation value
     * @param dmean partial derivatives of mean with respect to parameters
    ...
    double qvderivsN6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff, int[] count,
            double[] mean, double[] square, double[][] dmean, double[][] dsquare,
    ...
    double qvderivsCOR6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff, int[] count,
            double[] mean, double[] square, double[][] dmean, double[][] dsquare,
     ...
     * @param esquare second partial derivatives of square
     * @param partitions
     * @param param_int provides verification of compatibility of calling routine
     * This routine computes the first and second derivatives
     *  of the normalized standard deviation with respect to all
     *  external parameters.
     *
    ...
    double qvderivsN12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            int[] count, double[] mean, double[] square, double[][] dmean, double[][] dsquare,
    ...
    double qvderivsCOR12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            int[] count, double[] mean, double[] square, double[][] dmean, double[][] dsquare,
     ...
     * @param ecff second derivatives of sd with respect to parameters
     * This routine computes the first and second derivatives
     *  of the sum of squares with respect to all
     *  external parameters.
     *
     * The matrix indices have been adjusted to allow for a more
     *  orderly relationship between the matrix and its elements
    ...
    double uvderivsLS6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff ) {
    ...
    double uvderivsLS12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff ) {
    ...
    double qvderivsLS6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff ) {
    ...
    double qvderivsLS12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff ) {
     ...
     *  external parameters.
     *
     * A rescaling parameter is included to allow for data where
     *  intensities require adjustment
     *
     * The matrix indices have been adjusted to allow for a more
     *  orderly relationship between the matrix and its elements
     *
    ...
    double uvderivsRS6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
    ...
    double uvderivsRS12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            double scale, boolean forward ) {
    ...
    double qvderivsRS6( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim, int pixel5YDim,
            int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            double scale, boolean forward ) {
    ...
    double qvderivsRS12( int parameters, double[][] es, double[][][] des, double[][][][] ees,
            int sampleFactor, float[] pixel2, int pixel2XDim, int pixel2YDim, int pixel2ZDim,
            float[] pixel5, boolean[] pixel5Mask, int pixel5XDim,
            int pixel5YDim, int pixel5ZDim, float threshold5, double[] dcff, double[][] ecff,
            double scale, boolean forward ) {
    ...
    private boolean uvrm( int parameters, double[][] ef, double[][] er, double[][][] def,
            double[][][] der, double[][][][] eef, double[][][][] eer,
            boolean cubicInterpolation, float x_size1, float y_size1,
            float z_size1, float x_size2, float y_size2, float z_size2 ) {
    ...
    /**
     * @param a On entry the matrix to be factored
     *          On return an upper triangular matrix and the multipliers
     *          which were used to obtain it.  The factorization can be
     *          written A=L*U where L is a product of permutation and unit
     ...
     * @return an integer with normal value of n, with value
     *         =K<n if U(K,K) == 0.0.  This is not an error condition
     *         for this routine, but it does inidcate that DGESL or
     *         DGEDI will divide by zero if called.  Flags zeros on
     *         diagonal. Returns Integer.MAX_VALUE if n == 0.
    ...
    /**
     * @param a on input the output from DEGCO or GEFFA
     *          on return the inverse of original matrix
     * @param n the order of matrix a
     * @param ipvt the pivot vector from DGECO or DGEFA
     * @param work work vector, contents destroyed
     ...
     *  @param ipvt the pivot vector from MODCHOL
     *
     * On return
    ...
    /* *
     * @param a On entry the matrix to be factored.  Only the diagonal
     * and upper triangle are used
     * On return an upper triangle matrix R so that A = TRANS(R)*R
     ...
     * @param n the order of matrix A
     * @param b On entry the right hand side vector
     ...
     * @return resliced data array if successful,
     *         null if unsuccessful
     *
     * This routine will reslice a file based on the unit vectors es
     *  and achieves greater efficiency than reslicer15() based on
     *  not using perspective transformations.  Trilinear interpolation
     *  is used.
     */

    float[] reslicer( float[] volume, int x_dim1, int y_dim1, int z_dim1,
            int x_dim2, int y_dim2, int z_dim2, double[][] es ) {
        ...
        for ( k = 0, k3 = 0, x_k = e30, y_k = e31, z_k = e32; k < z_dim2; k++, k3 += slice2, x_k += e20, y_k += e21, z_k += e22 ) {
            for ( j = 0, j3 = k3, x_j = x_k, y_j = y_k, z_j = z_k; j < y_dim2; j++, j3 += x_dim2, x_j += e10, y_j += e11, z_j += e12 ) {
                for ( i = 0, i3 = j3, x_i = x_j, y_i = y_j, z_i = z_j; i < x_dim2; i++, i3++, x_i += e00, y_i += e01, z_i += e02 ) {
    ...
    private float[] nnreslicer( float[] volume, int x_dim1, int y_dim1, int z_dim1,
            int x_dim2, int y_dim2, int z_dim2, double[][] es ) {
        ...
        for ( k = 0, k3 = 0, x_k = e30, y_k = e31, z_k = e32, t_k = e33; k < z_dim2; k++, k3 += slice2, x_k += e20, y_k += e21, z_k += e22, t_k += e23 ) {
            for ( j = 0, j3 = k3, x_j = x_k, y_j = y_k, z_j = z_k, t_j = t_k; j < y_dim2; j++, j3 += x_dim2, x_j += e10, y_j += e11, z_j += e12, t_j += e13 ) {
                for ( i = 0, i3 = j3, x_i = x_j, y_i = y_j, z_i = z_j, t_i = t_j; i < x_dim2; i++, i3++, x_i += e00, y_i += e01, z_i += e02, t_i += e03 ) {
                    x_p = x_i / t_i;
    ...
    private float[] sincer( float[] volume, int x_dim1, int y_dim1, int z_dim1,
            int x_dim2, int y_dim2, int z_dim2, double[][] es,
            int xkern, int ykern, int zkern ) {
                    ...
                    if ( x_p >= -0.5 && x_p <= x_max1 ) {
                        ...
                        if ( y_p >= -0.5 && y_p <= y_max1 ) {
                            ...
                            if ( z_p >= -0.5 && z_p <= z_max1 ) {
                                ...
                                for ( k4 = (int) Math.ceil( z_p ), k2 = k4 * slice1; k4 < z_dim1
                                        && k4 < (int) Math.ceil( z_p ) + zkern; k4++, k2 += slice1 ) {
                                    sincz = wsinc( ( z_p - k4 ), zkern );
                                    for ( j4 = (int) Math.ceil( y_p ), j2 = k2 + j4 * x_dim1; j4 < y_dim1
                                            && j4 < (int) Math.ceil( y_p ) + ykern; j4++, j2 += x_dim1 ) {
                                        sincy = wsinc( ( y_p - j4 ), ykern );
                                        for ( i4 = (int) Math.ceil( x_p ), i2 = j2 + i4; i4 < x_dim1
                                                && i4 < (int) Math.ceil( x_p ) + xkern; i4++, i2++ ) {
                                            sincx = wsinc( ( x_p - i4 ), xkern );
                                            total += volume[i2] * sincx * sincy * sincz;
                                        }
                                        for ( i4 = (int) Math.ceil( x_p ) - 1, i2 = j2 + i4; i4 >= 0
                                                && i4 > (int) Math.ceil( x_p ) - 1 - xkern; i4--, i2-- ) {
                                            sincx = wsinc( ( x_p - i4 ), xkern );
                                            total += volume[i2] * sincx * sincy * sincz;
                                        }
                                    }
                                    for ( j4 = (int) Math.ceil( y_p ) - 1, j2 = k2 + j4 * x_dim1; j4 >= 0
                                            && j4 > (int) Math.ceil( y_p ) - 1 - ykern; j4--, j2 -= x_dim1 ) {
                                        sincy = wsinc( ( y_p - j4 ), ykern );
                                        for ( i4 = (int) Math.ceil( x_p ), i2 = j2 + i4; i4 < x_dim1
                                                && i4 < (int) Math.ceil( x_p ) + xkern; i4++, i2++ ) {
                                            sincx = wsinc( ( x_p - i4 ), xkern );
                                ...
                                for ( k4 = (int) Math.ceil( z_p ) - 1, k2 = k4 * slice1; k4 >= 0
                                        && k4 > (int) Math.ceil( z_p ) - 1 - zkern; k4--, k2 -= slice1 ) {
                                    sincz = wsinc( ( z_p - k4 ), zkern );
                                    for ( j4 = (int) Math.ceil( y_p ), j2 = k2 + j4 * x_dim1; j4 < y_dim1
                                            && j4 < (int) Math.ceil( y_p ) + ykern; j4++, j2 += x_dim1 ) {
                                        sincy = wsinc( ( y_p - j4 ), ykern );
    ...
    private void constructLog() {
        historyString = new String(
                "AIR Registration(" + String.valueOf( entireTarget ) + ", " + String.valueOf( entireSource ) + ", "
                + String.valueOf( model ) + ", " + String.valueOf( interpolation ) + ", "
                + String.valueOf( sourceThreshold ) + ", " + String.valueOf( targetThreshold ) + ", "
                + String.valueOf( sourceGaussX ) + ", " + String.valueOf( sourceGaussY ) + ", "
                + String.valueOf( sourceGaussZ ) + ", " + String.valueOf( targetGaussX ) + ", "
                + String.valueOf( targetGaussY ) + ", " + String.valueOf( targetGaussZ ) + ", "
                + String.valueOf( sourcePartitions ) + ", " + String.valueOf( targetPartitions ) + ", "
                + String.valueOf( costFxn ) + ", " + String.valueOf( cubicInterpolation ) + ", "
                + String.valueOf( interaction ) + ", " + String.valueOf( precision ) + ", "
