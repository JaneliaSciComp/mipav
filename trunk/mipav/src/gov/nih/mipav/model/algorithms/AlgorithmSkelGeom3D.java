...
public class AlgorithmSkelGeom3D extends AlgorithmBase {
    ...
    private final static int MAXBLOCKS = 1000;
    private final static int BLOCKSIZE = 100000;
    private final static double DELIFT = 0.0;
    private final static int MAXDIM = 8;
    private final static short CNV = 0; /* status of simplex, if it's on convex hull */
    private final static short VV = 1; /* Status if a regular simplex */
    private final static short SLV = -1; /* if orient3d=0, sliver simplex */
    // DBL_MANT_DIG is I1MACH(14), the number of base-B digits for double
    ...
    private final static int DBL_MANT_DIG = 53;
    // epsilon = D1MACH(4)
    ...
    // Multiply all floating point surface positions by multUp and round them into
    // integers.
    private double multUp = 10.0;
    // If true return Delaunay triangulation from buildConvexHull()
    // Found in hullmain.c
    ...
    // Estimate for sampling density constant.  1.0 or larger turns it off.  (default = 0.6).
    // Used to estimate whether Voronoi cells are "well-shaped", for handling noise and sharp-
    // corners hack.  This is the r in r-sampling, that is, the minimum distance to the nearest
    // sample on the surface, as a fraction of distance to the medial axis.  When r is small
    // the sampling is expected to be very dense, the Voronoi cells are expected to be really
    // long and skinny and the poles of fat Voronoi cells are thrown away.  On noise-free
    // inputs with no sharp corners, you can give a value >= 1, and perhaps get a good
    // reconstruction from a sparser input sample.  Setting estR small might help
    // get good reconstructions from dense but noisy samples.
    private float estR = 0.6f;
    // If true, throw away both poles for cells which are not long and skinny.  Set bad
    // to true on noise-free inputs from surfaces with sharp corners, amd make sure that the
    // estR is less than one ( The smaller estR, the more it will
    // interpolate data near the corners from the data on adjacient smooth surfaces).
    private boolean bad = false;
    // If true, no propagation for 1st pole of Voronoi cells which are not long and skinny.
    // Set to true when reconstructing surfaces with boundaries.  It allows a sample on the
    // boundary to have two outside poles.
    private boolean defer = false;
    // cosine (pi - alpha), where alpha is the angle between deep intersecting balls.
    // (default = 0.4).  When a pole gets labeled as inside or outside, it propagates
    // the same label to any neighboring pole whose polar ball deeply intersects its
    // own.  This value is never changed.
    private float theta = 0.4f;
    // cosine (pi - alpha), where alpha is the angle between deep intersecting balls.
    // Same as theta, but for trying to label unlabeled poles, the second time around.
    // (default = 0.3).  Once you start fooling around with the estR, bad,
    // and/or defer options, some poles might fail to be labeled by the regular
    // algorithm.  This parameter is passed to a second-pass clean-up function which should
    // be a little more liberal in propagating labels.  Make the deep value smaller when
    // you see lots of "unlabeled pole" messages.
     ...
     * @param defer If true, no propagation for 1st pole of Voronoi cells which are
     *               not long and skinny
     * @param deep cosine (pi - alpha) for trying to label unlabled poles, the
     *                second time around
     */
    public AlgorithmSkelGeom3D (ModelImage srcImg, float minObjectValue,
                                   boolean sliceHoleFilling, float estR,
    ...
    public void runAlgorithm() {
        ...
        grayImage = new ModelImage(ModelStorageBase.BYTE, srcImage.getExtents(),
    ...
    private void computeVV(simplex s) {
        ...
        for (j = 0; j < cDim; j++) { /* Compute 1st pole for vertex */
            ...
            if ((s.getNeigh(j).getVert(0) > omaxs[0]) ||
                (s.getNeigh(j).getVert(0) < omins[0]) ||
                (s.getNeigh(j).getVert(1) > omaxs[1]) ||
                (s.getNeigh(j).getVert(1) < omins[1]) ||
                (s.getNeigh(j).getVert(2) > omaxs[2]) ||
                (s.getNeigh(j).getVert(2) < omins[2])) {
                pole1[i] = null;
                continue;
            }
            else {
...
private void tetCircumcenter(double a[], double b[], double c[], double d[],
                             double circumcenter[], double cond[])
{
 ...
 private double orient3dAdapt(double pa[], double pb[], double pc[], double pd[],
                             double permanent) {
 ...
 if ((adxtail[0] == 0.0) && (bdxtail[0] == 0.0) && (cdxtail[0] == 0.0)
     && (adytail[0] == 0.0) && (bdytail[0] == 0.0) && (cdytail[0] == 0.0)
     && (adztail[0] == 0.0) && (bdztail[0] == 0.0) && (cdztail[0] == 0.0)) {
     return det;
 }

 errBound = o3dErrBoundC * permanent + resultErrBound * Math.abs(det);
 det += (adz[0] * ((bdx * cdytail[0] + cdy * bdxtail[0])
                - (bdy * cdxtail[0] + cdx * bdytail[0]))
         + adztail[0] * (bdx * cdy - bdy * cdx))
     + (bdz[0] * ((cdx * adytail[0] + ady * cdxtail[0])
               - (cdy * adxtail[0] + adx * cdytail[0]))
        + bdztail[0] * (cdx * ady - cdy * adx))
     + (cdz[0] * ((adx * bdytail[0] + bdy * adxtail[0])
               - (ady * bdxtail[0] + bdx * adytail[0]))
 ...
 if (adxtail[0] == 0.0) {
     ...
     if (adytail[0] == 0.0) {
         ...
         twoTwoDiff(adxt_bdy1[0], adxt_bdy0[0], adyt_bdx1[0], adyt_bdx0[0],
                      at_blarge, at_b[2], at_b[1], at_b[0]);
         at_b[3][0] = at_blarge[0];
         at_blen = 4;
         twoProduct(adytail[0], cdx, adyt_cdx1, adyt_cdx0);
         twoProduct(adxtail[0], cdy, adxt_cdy1, adxt_cdy0);
         twoTwoDiff(adyt_cdx1[0], adyt_cdx0[0], adxt_cdy1[0], adxt_cdy0[0],
 ...
 if (bdxtail[0] == 0.0) {
     ...
     if (bdytail[0] == 0.0) {
         ...
         twoTwoDiff(bdxt_cdy1[0], bdxt_cdy0[0], bdyt_cdx1[0], bdyt_cdx0[0],
                      bt_clarge, bt_c[2], bt_c[1], bt_c[0]);
         bt_c[3][0] = bt_clarge[0];
         bt_clen = 4;
         twoProduct(bdytail[0], adx, bdyt_adx1, bdyt_adx0);
         twoProduct(bdxtail[0], ady, bdxt_ady1, bdxt_ady0);
         twoTwoDiff(bdyt_adx1[0], bdyt_adx0[0], bdxt_ady1[0], bdxt_ady0[0],
 ...
 if (cdxtail[0] == 0.0) {
     ...
     if (cdytail[0] == 0.0) {
         ...
         twoTwoDiff(cdxt_ady1[0], cdxt_ady0[0], cdyt_adx1[0], cdyt_adx0[0],
                      ct_alarge, ct_a[2], ct_a[1], ct_a[0]);
         ct_a[3][0] = ct_alarge[0];
         ct_alen = 4;
         twoProduct(cdytail[0], bdx, cdyt_bdx1, cdyt_bdx0);
         twoProduct(cdxtail[0], bdy, cdxt_bdy1, cdxt_bdy0);
         twoTwoDiff(cdyt_bdx1[0], cdyt_bdx0[0], cdxt_bdy1[0], cdxt_bdy0[0],
...
private int fastExpansionSumZeroElim(int elen, double e[][], int flen,
                                     double f[][], double h[][]) {  /* h cannot be e or f. */
    double Q;
...
private int scaleExpansionZeroElim(int elen, double e[][],
                           double b[], double h[][]) {  /* e and h cannot be the same. */
    ...
    private void twoOneProduct(double a1, double a0, double b,
...
/*   already been split.  Avoids redundant splitting.                        */
    ...
    private void twoProductPresplit(double a, double b, double bhi, double blo,
                               double x[], double y[]) {
    ...
    private void twoOneDiff(double a1, double a0, double b,
    ...
    private void twoTwoDiff(double a1, double a0, double b1, double b0,
                            double x3[] , double x2[], double x1[], double x0[]) {
        double _j[] = new double[1];
        double _0[] = new double[1];
    ...
    private simplex buildConvexHull() {
            ...
            if (rDim > MAXDIM) {
                MipavUtil.displayError("Dimension bound MAXDIM exceeded rDim = " + rDim +
                                       " pDim = " + pDim);
                errorStatus = -1;
                return null;
            }
            ...
            if (neigh != null) {
                ...
                neigh.setVert(1, p[1]);
                neigh.setVert(2, p[2]);
                neigh.setVertIndex(currentNumSites);
                neigh.setSimp(s);
            } // if (neigh != null)
            neigh = s.getPeak();
            if (neigh != null) {
    ...
    private void buildHull(simplex root) {
        ...
        while (cDim < rDim) {
            ...
            if (outOfFlat(root, p, vertIndex)) {
                ...
                connect(makeFacets(search(root)));
            }
        } // while (cDim < rDim)
    ...
    private void connect(simplex s) {
        ...
        double xb[];
        double xfi[];
        simplex sb = new simplex();
        simplex sf = new simplex();
        simplex seen = new simplex();
        int i;
        neighbor sn;
        int bIndex;
        int fiIndex;
        ...
        for (i = 0, sn = s.getNeigh(0); i < cDim; i++, sn = s.getNeigh(i)) {
            ...
            sb.setSimplex(seen);
            sf.setSimplex(sn.getSimp());
            xf = xfi;
            fIndex = fiIndex;
            if (sf.getPeak().getVert() == null) { /* are we done already */
                sf.setSimplex(opVert(seen, xb, bIndex).getSimp());
                ...
                sb.setSimplex(sf);
                sf.setSimplex(opVert(sb, xb, bIndex).getSimp());
    ...
    private simplex makeFacets(simplex seen) {
        ...
        if (!sees(p, vertIndex, seen)) {
            MipavUtil.displayError("sees returned null in makeFacets");
            disposeProgressBar();
            setCompleted(false);
            System.exit(1);
        }
    ...
    private neighbor opSimp(simplex a, simplex b) {
        ...
        int i;
        neighbor x;
        for (i = 0, x = a.getNeigh(0); (x.getSimp() != b) && (i < cDim); i++,
                            x = a.getNeigh(i)) {
            if (i < cDim) {
                return x;
            } else {
    ...
    private neighbor opVert(simplex a, double b[], int vertIndex) {
        ...
        int j;
        neighbor x;
        for (i = 0, x = a.getNeigh(0); (x.getVertIndex() != vertIndex) && (i < cDim); i++,
                            x = a.getNeigh(i)) {
            if (i < cDim) {
                return x;
            } else {
    ...
    private void visitTriangGen(simplex s) {
        ...
        int tms = 0;
        simplex stTemp[];
        simplex t = new simplex();
        neighbor sn;
        vNum--;
        if (stTri == null) {
            stTri = new simplex[ss + MAXDIM + 1];
        }
        if (s != null) {
            stTri[tms] = new simplex();
            (stTri[tms++]).setSimplex(s);
        }
        while (tms > 0) {
            if (tms > ssTri) {
                ...
                for (i = 0; i < stTri.length; i++) {
                    if (stTri[i] != null) {
                        stTemp[i] = new simplex();
                        stTemp[i].setSimplex(stTri[i]);
                    }
                }
                stTri = new simplex[(ss += ss) + MAXDIM + 1];
                for (i = 0; i < stTemp.length; i++) {
                    if (stTemp[i] != null) {
                        stTri[i] = new simplex();
                        stTri[i].setSimplex(stTemp[i]);
                    }
                }
            } // if (tms > ssTri)
            if (stTri[--tms] == null) {
    ...
    private simplex search(simplex root) {
        ...
        if (root.getPeak().getSimp() != null) {
            st[tms] = new simplex();
            (st[tms++]).setSimplex(root.getPeak().getSimp());
        }
        else {
            st[tms++] = null;
        }
    ...
    private boolean sees (double p[], int vertexIndex, simplex s) {
        ...
        if (vdCh || powerDiagram) {
            if (Double.isInfinite(p[0])) {
                ...
                b.setVecs(infinityBasis.getVecs());
    ...
    private void getNormalSede(simplex s) {
        ...
        for (i = cDim+1; i > 0; i--) {
            ...
            reduce(s.getNormal(), rn.getVert(), s, cDim);
    ...
    private boolean checkPerps(simplex s) {
        ...
        for (i = 1; i < cDim; i++) {
            ...
            if ((vdCh || powerDiagram) && Double.isInfinite(y[0])) {
                ...
                bCheckPerps.setVecs(infinityBasis.getVecs());
            }
            else {
                for (k = 0; k < pDim; k++) {
    ...
    private boolean outOfFlat(simplex root, double p[], int vertIndex) {
        ...
        pNeigh.setVert(0, p[0]);
        pNeigh.setVert(1, p[1]);
        pNeigh.setVert(2, p[2]);
        ...
        getBasisSede(root);
        if ((vdCh || powerDiagram) && (Double.isInfinite(root.getNeigh(0).getVert()[0]))) {
            return true;
        }
    ...
    private void getBasisSede(simplex s) {
        ...
        neighbor t;
        basisS bas;
        if ((vdCh || powerDiagram) && (Double.isInfinite(sn0.getVert(0))) && (cDim > 1)) {
            t = sn0;
            sn0 = sn;
            sn = t;
            bas = sn0.getBasisS();
    ...
    private int reduce(basisS v, double point[], simplex s, int k) {
        double z[];
        neighbor neigh;
        double tt[];
        int i;
        ...
        if (vdCh || powerDiagram) {
            if (Double.isInfinite(p[0])) {
                ...
                v.setVecs(infinityBasis.getVecs());
    ...
    private int reduceInner(basisS v, simplex s, int k) {
        double va[];
        double va2[];
        double vb[];
        double vb2[];
        ...
        vb = v.getVecs();
        va = new double[vb.length-rDim];
        for (i = rDim; i < vb.length; i++) {
            va[i-rDim] = vb[i];
        }
    ...
    private class basisS {
        ...
        double vecs[] = new double[1]; /* The actual vectors */
        ...
        public void setVecs(int index, int num) {
            vecs[index] = num;
        }
    ...
    private class neighbor {
        double vert[] = new double[3]; /* vertex of simplex */
        int vertIndex;
    ...
    private class simplex {
        ...
        short mark;
        double vv[]; /* Voronoi vertex of simplex */
        ...
        /* NOTE!!! neighbor has to be the LAST field in the simplex structure,
        ...
        basisS normal; /* Normal vector pointing inward */
        neighbor peak; /* if null, remaining vertices give facet */
        neighbor neigh[] = new neighbor[1]; /* neighbors of simplex */
        ...
        public void setChRoot(boolean chRoot) {
            this.chRoot = chRoot;
        }

        public boolean getChRoot() {
            return chRoot;
        }
    ...
    private class SimplexList {
        ...
        public void setChRoot(simplex node) {
            simplex current = firstNode;
            while (current != null) {
                if (current != node) {
                    current.setChRoot(false);
                }
                else {
                    current.setChRoot(true);
                }
                current = current.nextNode;
            }
        }

        public simplex getChRoot() {
            simplex current = firstNode;
            while (current != null) {
                if (current.getChRoot()) {
                    return current;
                }
                current = current.nextNode;
            }
