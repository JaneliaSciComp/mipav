...
/*   already been split.  Avoids redundant splitting.                        */
    ...
    private void connect(simplex s) {
        ...
        for (i = 0, sn = s.getNeigh(0); i < cDim; i++, sn = s.getNeigh(i)) {
            ...
            if (sf.getPeak().getVert() == null) { /* are we done already? */
                ...
                if (sf != null && sf. getPeak() != null &&
                        sf.getPeak().getVert() != null) {
                    continue;
                }
            }
            else do {
    ...
    private neighbor opSimp(simplex a, simplex b) {
        ...
        for (i = 0, x = a.getNeigh(0);(((x == null) || (x.getSimp() != b)) && (i < cDim)); i++,
                            x = a.getNeigh(i));
        if (i < cDim) {
            return x;
        } else {
    ...
    private neighbor opVert(simplex a, double b[], int vertIndex) {
        ...
        for (i = 0, x = a.getNeigh(0); (((x == null) || (x.getVertIndex() != vertIndex)) && (i < cDim)); i++,
                            x = a.getNeigh(i));
        if (i < cDim) {
            return x;
        } else {
    ...
    private boolean checkPerps(simplex s) {
        ...
        for (i = 1; i < cDim; i++) {
            if (Math.abs(s.getNeigh(i).getBasisS().getSqb()) < epsilon) {
                ...
                Preferences.debug("s.getNeigh(" + i + ").getBasisS().getSqb() = " + 
                                   s.getNeigh(i).getBasisS().getSqb() + "\n");
                return false;
            }
        } // for (i = 1; i < cDim; i++)
        if (bCheckPerps == null) {
    ...
    private void getBasisSede(simplex s) {
        ...
        neighbor t;
        basisS bas;
        Preferences.debug("s.getNeigh(0).getVert(0) = " + s.getNeigh(0).getVert(0) +
                          " at getBasisSede entry\n");
        if (s.getNeigh(1).getVert() != null) {
        Preferences.debug("s.getNeigh(1).getVert(0) = " + s.getNeigh(1).getVert(0) +
        " at getBasisSede entry\n");
        }
    ...
    private boolean reduceInner(basisS v, simplex s, int k) {
       ...
       for (i = 0; i < 2*rDim; i++) {
        ...
        for (j = 0; j < 250; j++) { 
            ...
            scale = sc(v, s, k, j);
            Preferences.debug("Doing vecScaleTest # " + j + 
                    " in reduceInner\n");
