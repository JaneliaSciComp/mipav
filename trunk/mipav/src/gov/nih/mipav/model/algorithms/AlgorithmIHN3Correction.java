 ...
 *
 ...
 This is based on code which is John G. Sled,
 ...
 Bibliography:
 <p>
 1. J.G. Sled, A.P. Zijdenbos, and A.C. Evans, "A non-parametric method for automatic
 correction of intensity non-uniformity in MRI data," IEEE Transactions on Medical
 Imaging, vol. 17, pp. 87-97, February, 1998.
 <p>
 2. J.G. Sled, A.P. Zijdenbos, and A.C. Evans, "A comparison of retrospective
 intensity non-uniformity correction methods for MRI," in Information Processing in
 Medical Imaging, pp. 459-464, 1997.
 <p>
 3. J.G. Sled, "A non-parametric method for automatic correction of intensity
 non-uniformity in MRI data," Master's thesis, McGill University, Montreal, QC,
 May, 1997.
 <p>
 The N3 method should work with any MR volume including raw (non-stereotaxic) data.
 <p>
 An artifact often seen in MRI is for the signal intensity to vary smoothly across an
 image.  Variously referred to as RF inhomogeneity, shading artifact, or intensity
 non-uniformity, it is usually attributed to such factors as poor radio frequency(RF)
 field uniformity, eddy currents driven by switching of field gradients, and patient
 anatomy both inside and outside the field of view.
 <p>
 This code corrects intensity non-uniformity in MR data without requiring supervision.
 This method can be applied without a tissue intensity or geometric model.  Described as
 Non-parametric Non_uniform intensity Normalization (N3), the method is independent of
 pulse sequence and insensitive to pathological data that might otherwise violate model
 assumptions.  To eliminate the dependence of the field estimate on anatomy, an iterative
 approach is employed to estimate both the multiplicative bias field and the distribution
 of true tissue intensities.  Preprocessing of MR data using N3 has been shown to
 substantially improve the accuracy of anatomical analysis techniques such as tissue
...
public class AlgorithmIHN3Correction extends AlgorithmBase {
    ...
    public final static int spline = 4;  // splines are cubic
    ...
    */
    ...
    /**
    *   The measure used to terminate the iterations is the coefficient of variation
    ...
    *   The distance between adjacent knots in bspline fitting with at least
    *   4 knots going in every dimension.  The default in the dialog is one
    *   third the distance (resolution * extents) of the smallest dimension.
    */
    private float   fieldDistance = 200.0f;

    /**
    *   The factor by which the data is subsampled to a lower resolution in
    *   estimating the slowly varying non-uniformity field. Reduce sampling
    *   in the finest sampling direction by the shrink factor.  Reduce other
    *   sampling directions only if reduced sampling is less.
    */
    private float     shrink           = 4.0f;


    /**
    *   Width of deconvolution kernel used to sharpen the histogram.  Larger values
    *   give faster convergence while smaller values give greater accuracy.
    */
    ...
    *   The performance of the N3 method is enhanced by supplying a mask for the
    *   region of interest.  If true, the N3 method is applied to the entire image.
    */
    private boolean entireImage = true;

    /**
    *   Determines the threshold by histogram analysis
    *   If true a VOI cannot be used and the input
    ...
    */
    ...
    /**
    *   If minimum value of shrunken buffer < 1.0f, amount that
    *   must be added to make it 1.0f.  Otherwise == 0.0f.
    */
    ...
    */
    ...
    private float   bW[][] = {{-1,3,-3,1}, {3, -6, 0, 4}, {-3, 3, 3, 1}, {1, 0, 0, 0}};
    ...
    *   @param _maxIters
    *   @param _endTol
    *   @param _fieldDistance
    *   @param _shrink
    *   @param _kernelfwhm
    *   @param _noise
    *   @param maskFlag
    *   @param _autoThreshold
    *   @param useScript
    */
	public AlgorithmIHN3Correction(ModelImage destImg,     ModelImage fieldImg,  ModelImage srcImg,
	                            float   _threshold,        int _maxIters,
	                            float   _endTol,           float _fieldDistance,
	                            float   _shrink,           float _kernelfwhm,   float _noise,
	                            boolean maskFlag,          boolean _autoThreshold,
                                    boolean useScript) {
    ...
    */
    private void constructLog() {
        historyString = new String( "IHN3Correction(" +
                                    String.valueOf(threshold)          + ", " +
                                    String.valueOf(maxIters)           + ", " +
                                    String.valueOf(endTol)             + ", " +
                                    String.valueOf(fieldDistance)      + ", " +
                                    String.valueOf(shrink)             + ", " +
                                    String.valueOf(kernelfwhm)         + ", " +
    ...
    */
    ...
    private void IHN3Correction2() {
        ...
        try {
            ...
            if (autoThreshold) {
                ...
                for (k = 1; k < nBins; k++) {
                    ...
                    firstMoment1 = firstMoment0 +
                    (((k-histOffset)/histFactor) + halfBin) * hist[k]/histSum;
            ...
            if (autoThreshold  && (!useScript)) {
                MipavUtil.displayInfo("auto threshold = " + threshold + " includes " + j +
                                    " pixels excludes " + (newSliceSize - j) + " pixels");
            }
        ...
        for (iters = 0; ((iters < maxIters) && (stddev >= endTol)) && !threadStopped; iters++) {
            ...
            if (isProgressBarVisible())  progressBar.updateValue(Math.round((float)iters/(maxIters-1) * 100), activeImage);
            ...
            if (((padded_size-1)/2) != (padded_size/2)) { // if size is even fill in middle value
                blurR[padded_size/2] = (float)(fwhmScale * Math.exp(-padded_size * padded_size *
                                                                    fwhmFactor/4.0));
            }
   ...
   */
   private void transformNearestNeighbor2D(float imgBuf[], float xfrm[][]) {
        ...
        T00 = (float)xfrm[0][0]; T01 = (float)xfrm[0][1]; T02 = (float)xfrm[0][2];
        T10 = (float)xfrm[1][0]; T11 = (float)xfrm[1][1]; T12 = (float)xfrm[1][2];
        T20 = (float)xfrm[2][0]; T21 = (float)xfrm[2][1]; T22 = (float)xfrm[2][2];
            ...
            for (j=0; j < orgDim[1]; j++){
                ...
                if ((X>=0) && (roundX<newDim[0])){
                    ...
                    if ((Y>=0) && (roundY<newDim[1])){
                        if ( (roundX==newDim[0]-1) || (roundY==newDim[1]-1) ){//cannot interpolate on last X or Y
                            X0pos = roundX;
                            ...
                            value = x1*y1*imgBuf[Y0pos+X0pos] +
            ...
            for (i = 0; i < nDimensions; i++) {
                for (order = 0; order < 3; order++) {
                    ...
                    if (nArray[i] < 4) {
                       ...
                       "algorithm IHN3Correction bending energy not defined for size < 4");
                        progressBar.dispose();
                        setCompleted(false);
                        return;
                    }
                    ...
                    // e.g. -x^3 + 3x^2 -3x + 1
                    bW[0][0] = -1; bW[0][1] =  3; bW[0][2] = -3; bW[0][3] = 1;
                    bW[1][0] =  3; bW[1][1] = -6; bW[1][2] =  0; bW[1][3] = 4;
                    bW[2][0] = -3; bW[2][1] =  3; bW[2][2] =  3; bW[2][3] = 1;
                    bW[3][0] =  1; bW[3][1] =  0; bW[3][2] =  0; bW[3][3] = 0;
                    ...
                    for (m = 0; m < spline; m++) {
                        for (j = m; j < spline; j++) {
                            ...
                            for (k = 0; k < sizeC; k++) {
                                CArray[k] = 0;
                                endK = k+1;
                                if (sizeC-k < endK) endK = sizeC-k;
                                for (l = 0; l < endK; l++) {
                                    maxK1 = k-spline+1;
                                    if (maxK1 < 0) maxK1 = 0;
                                    maxK2 = spline-1-k;
                                    if (maxK2 < 0) maxK2 = 0;
                                    CArray[k] += bW[m][maxK1+l] *
                                    bW[j][spline-1-l-maxK2];
                                }
                            } // for (k = 0; k < sizeC; k++)
    ...
    */
    private void IHN3Correction3() {
        ...
        try {
            ...
            if (autoThreshold  && (!useScript)) {
            MipavUtil.displayInfo("auto threshold = " + threshold + " includes " + j +
                                    " voxels excludes " + (newVolSize - j) + " voxels");
            }
            if (sMask != null) {
                for (i = 0; i < newVolSize; i++) {
                    if (!sMask.get(i)) {
        ...
        for (i=0; i < newDim[0]; i++){
            ...
            for (j=0; j < newDim[1]; j++){
                ...
                for (k=0; k < newDim[2]; k++){
                    ...
                    roundY = (int)(Y+0.5f);
                    roundZ = (int)(Z+0.5f);
                    if ( (roundX<0)        || (roundX>orgDim[0]-1) || (roundY<0) ||
                         (roundY>orgDim[1]-1) || (roundZ<0)        || (roundZ>orgDim[2]-1) ){
   ...
   */
   private void transformTrilinear(float imgBuffer[], float xfrm[][]) {
        ...
        AlgorithmTransform.transformTrilinear(imgBuffer, fieldBuffer, matrix,
                    newDim[0], newDim[1], newDim[2], newResol[0], newResol[1], newResol[2],
                    orgDim[0], orgDim[1], orgDim[2], orgResol[0], orgResol[1], orgResol[2],
                    null);
   }
   ...
   */
   private void TBSplineVolume() {
        ...
        for (i = 0; i < nDimensions; i++) {
            ...
            last = (domain[i][1] < ((newDim[i]-1)*newResol[i])) ?
                    (int)Math.floor(domain[i][1]/newResol[i])+1 : newDim[i];
   ...
   *    @return the array
   */
   private static final float[][] matrixtoInverseArray(TransMatrix transMatrix)
   {
        int i,j;
        float xfrm [][] = null;
