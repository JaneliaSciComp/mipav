...
/**
 * Snake-like algorithm deriviative using BSplines. The algorithm is supplied
 * a polygon (VOI - contour) and that polygon is allowed to evolve to edge of
 ...
 * a two-and-half dimensional algorithm where resultant contour in a slice is
...
public class AlgorithmBSnake extends AlgorithmBase {
     ...
     */
    public AlgorithmBSnake(ModelImage srcImg, float sigmas[],
                           int boundaryIterations, VOI srcVOI ) {
	   ...
	   if (srcImg.getNDims() == 2){
	        makeKernels2D();
	        resultVOI = new VOI((short)srcImage.getVOIs().size(), "BsnakeVOI",
                                   1, VOI.CONTOUR, -1.0f);
	    }
    ...
    */
    private void calc2D(){
        ...
        try {
            ...
            buildProgressBar(srcImage.getImageName(), "Bspline snake: Evolving boundary ...",
                             0, 100);
            initProgressBar();
          }
        ...
        for(int j = 0; j < nContours; j++){
            if ( ((VOIContour)(contours[0].elementAt(j))).isActive() ) {
                ...
                ((VOIContour)(resultVOI.getCurves()[0].lastElement())).trimPoints(
    ...
    private float runSnake(float xPoints[], float yPoints[], float image[], Polygon resultGon){
        ...
        for (int z = 0; (z < boundaryIterations) && (!threadStopped); z++) {
            ...
            for( i = 0; i < nPts; i++) {
                ...
                ix = AlgorithmConvolver.convolve2DPt(interpPt, srcImage.getExtents(),
                                                          image, kExtents, GxData);
                iy = AlgorithmConvolver.convolve2DPt(interpPt, srcImage.getExtents(),
                                                          image, kExtents, GyData);
                //gradMag = (float)Math.sqrt(ix*ix + iy*iy);
                gradMag = ix*ix + iy*iy;


                ix = AlgorithmConvolver.convolve2DPt(inNormPt, srcImage.getExtents(),
                                                          image, kExtents, GxData);
                iy = AlgorithmConvolver.convolve2DPt(inNormPt, srcImage.getExtents(),
                                                          image, kExtents, GyData);
                //inGradMag = (float)Math.sqrt(ix*ix + iy*iy);
                inGradMag = ix*ix + iy*iy;

                ix = AlgorithmConvolver.convolve2DPt(outNormPt, srcImage.getExtents(),
                                                          image, kExtents, GxData);
                iy = AlgorithmConvolver.convolve2DPt(outNormPt, srcImage.getExtents(),
                                                          image, kExtents, GyData);
            ...
            if ( Math.abs((energy/oldEnergy)-1) < 0.0001) break;
            oldEnergy = energy;
        }
    ...
    private void calc3D(){
        ...
        try {
          ...
          buildProgressBar(srcImage.getImageName(),
                           "Bspline snake: Evolving boundary ...",
                           0, 100);
          initProgressBar();
        }
        catch (OutOfMemoryError e) {
        ...
        if ( propagationType != PROP_PREV ) {
            while(!threadStopped) {
                ...
                //Preferences.debug(" Points = " + nPts + " energy = " + Math.abs(energy/tempEnergy) + " energy/pt = " + Math.abs((energy/nPts)/(baseEnergy/baseNPts)));
