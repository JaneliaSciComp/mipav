...
/**
 * Algorithm Implementation 1. Calculate the input image histogram. 2. Calculate the image values that represent 2% and
 * 98% of the histogram values and clamp the input image data to that range. 3. Set the cerebrospinal fluid (CSF)
 * threshold minimum value to 30 % of the clamped image range and the CSF threshold maximum value to 70% of the clamped
 * image range. 4. Calculate the brain center of gravity (COG) as the weighted sum of all voxels between the CSF minimum
 * and maximum range using the voxel position scaled by intensity value. 5. Estimate the white matter threshold by
 * segmenting the image into 5x5x5 voxel cubes and finding the cube with the highest mean intensity and lowest variance.
 * The mean intensity of the selected cube is used to set the white matter threshold value. 6. Generate a white-matter
 * segmentation of the brain by using a flood-fill algorithm with the white matter threshold seed point as the starting
 * point. The flood-fill algorithm is limited by the CSF threshold values as well as a radius centered around the
 * initial seed point. 7. Take the convex-hull of the white matter segmentation and pass this mesh into a modified
 * version of the AlgorithmBrainExtractor. 8. The output of the AlgorithmBrainExtractor is a segmented mesh representing
 * the surface of the brain. To err on the side of not removing too much data, the convex-hull of the brain surface is
 * generated. 9. The final mask representing the data outside the brain is generated using all voxels outside the
 * convex-hull of the brain surface. 10. (Optional final step). If the user chooses to segment only the face region
 * instead of the entire skull region the final step produces the face segmentation from the convex-hull of the brain
 * surface.
 */
public class AlgorithmSkullRemoval extends AlgorithmBase {
    ...
    private final int[] originalAxis;

    /** the orientation the images are remapped to before processing: */
    private final int[] targetAxis = new int[] {FileInfoBase.ORI_A2P_TYPE, FileInfoBase.ORI_S2I_TYPE, FileInfoBase.ORI_L2R_TYPE};;
    ...
    private final ModelImage originalSrc;

    private ModelImage maskImage;

    private ModelImage whiteMatterImage;
     ...
     */
    public AlgorithmSkullRemoval(final ModelImage srcImg) {
        super(null, srcImg);
        originalSrc = srcImg;

        originalAxis = srcImage.getAxisOrientation();
        final int[] axisOrder = {0, 1, 2, 3};
        final boolean[] axisFlip = {false, false, false, false};
        if (reMappedSrc = MipavCoordinateSystems.matchOrientation(targetAxis, originalAxis, axisOrder, axisFlip)) {
            final AlgorithmRotate rotateAlgo = new AlgorithmRotate(srcImage, axisOrder, axisFlip);
     ...
     */
    public AlgorithmSkullRemoval(final ModelImage srcImg, final ModelImage atlasImage) {
        ...
        if (reMappedTargetAtlas = MipavCoordinateSystems.matchOrientation(targetAxis, originalAxisTargetAtlas, axisOrder, axisFlip)) {
            final AlgorithmRotate rotateAlgo = new AlgorithmRotate(srcImg, axisOrder, axisFlip);
            rotateAlgo.setRunningInSeparateThread(false);
            rotateAlgo.run();
     ...
     */
    public void setOutputOption(final boolean blur, final boolean remove, final boolean face, final boolean showFaceSegmentation,
            final boolean showSkullSegmentation) {
        this.blur = blur;
        this.remove = remove;
        this.face = face;
    ...
    /**
     * Combines the randomized-voxel blurred image with the output image. Only the areas that are in the segmented
     * region are modified by the blur.
     * 
     * @param blurImage
     * @param destImage
     */
    private void addBlur(final ModelImage blurImage, final ModelImage destImage) {
        final int dimX = destImage.getExtents()[0];
        final int dimY = destImage.getExtents()[1];
        final int dimZ = destImage.getExtents()[2];
        for (int z = 0; z < dimZ; z++) {
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
                    final int index = z * dimX * dimY + y * dimX + x;
    ...
    /**
     * Computes a volume mask of the triangle mesh surface. The BitSet mask volume has the same volume dimensions as the
     * current image. The mask is used to show the surface-plane intersections in the slice views.
     * 
     ...
     */
    private BitSet computeSurfaceMask(final TriMesh mesh) {
        BitSet surfaceMask = null;

        final BoxBV kBoundingBox = new BoxBV();
        kBoundingBox.ComputeFromData(mesh.VBuffer);

        final Vector3f[] kBoxCorners = new Vector3f[8];
        kBoundingBox.GetBox().ComputeVertices(kBoxCorners);
        final Vector3f kMaxBB = new Vector3f( -Float.MAX_VALUE, -Float.MAX_VALUE, -Float.MAX_VALUE);
        final Vector3f kMinBB = new Vector3f(Float.MAX_VALUE, Float.MAX_VALUE, Float.MAX_VALUE);
        ...
        final int iTQuantity = mesh.GetTriangleQuantity();

        int iV0, iV1, iV2;
        final int[] aiTris = new int[3];
        final Vector3f kV0 = new Vector3f();
        final Vector3f kV1 = new Vector3f();
        final Vector3f kV2 = new Vector3f();
        ...
        for (int i = 0; i < iTQuantity; i++) {
            ...
            // surface.
            final float iXMin = min.X, iXMax = max.X;
            final float iYMin = min.Y, iYMax = max.Y;
            final float iZMin = min.Z, iZMax = max.Z;
            int ptr;
            final int end = surfaceMask.size();

            for (float iY = iYMin; iY < iYMax; iY = iY + 0.1f) {

                for (float iZ = iZMin; iZ < iZMax; iZ = iZ + 0.1f) {
                    final float iX = getIntersectX(kV0, kV1, kV2, iY, iZ);
            ...
            for (float iX = iXMin; iX < iXMax; iX = iX + 0.1f) {
                ...
                for (float iZ = iZMin; iZ < iZMax; iZ = iZ + 0.1f) {
                    final float iY = getIntersectY(kV0, kV1, kV2, iX, iZ);
            ...
            for (float iX = iXMin; iX < iXMax; iX = iX + 0.1f) {
                ...
                for (float iY = iYMin; iY < iYMax; iY = iY + 0.1f) {
                    final float iZ = getIntersectZ(kV0, kV1, kV2, iX, iY);
     ...
     * @return
     */
    private TriMesh convexHull(final TriMesh mesh) {
        final ConvexHull3f convexHull = new ConvexHull3f(mesh.VBuffer.GetVertexQuantity(), mesh.VBuffer.GetPositionArray(), 0.00001f, true);
        final IndexBuffer iBuffer = new IndexBuffer(convexHull.GetIndices());
        final VertexBuffer vBuffer = new VertexBuffer(mesh.VBuffer);
    ...
    private BitSet createMask(final Plane3f clipPlane, final Vector3f cog) {
        final Vector3f min = new Vector3f(0, cog.Y, 0);
        final Vector3f max = new Vector3f(cog.X, dimY, dimZ);
            ...
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
                    if ( (x >= min.X) && (x <= max.X) && (y >= min.Y) && (y <= max.Y) && (z >= min.Z) && (z <= max.Z)) {
                        pt.set(x, y, z);
                        final float distance = clipPlane.DistanceTo(pt);
                        if (distance < 0) {
    ...
    private BitSet createMask(final TriMesh mesh, final boolean show, final String postScript) {
        final BitSet meshMask = computeSurfaceMask(mesh);
    ...
    private TriMesh createMesh(final BitSet mask) {
        final int length = dimX * dimY * dimZ;
        ...
        final SurfaceExtractorCubes kExtractor = new SurfaceExtractorCubes(dimX, dimY, dimZ, buffer, 1, 1, 1, null, null, null);
        final TriMesh kMesh = kExtractor.getLevelSurface(0);
        return kMesh;

    }

    private TriMesh createMesh(final ModelImage image) {
        if ( (Preferences.isGpuCompEnabled() && OpenCLAlgorithmBase.isOCLAvailable())) {
            final OpenCLAlgorithmMarchingCubes cubesAlg = new OpenCLAlgorithmMarchingCubes(image, 0, false, false, false, 0, null /*
                                                                                                                                   * JDialogBase
                                                                                                                                   * .
                                                                                                                                   * makeImageName
                                                                                                                                   * (
    ...
    private float distance(final Vector3f cog, final int x, final int y, final int z) {
        ...
        final Vector3f temp = new Vector3f(cog.X * xRes, cog.Y * yRes, cog.Z * zRes);
        final float dist = temp.distance(new Vector3f(x * xRes, y * yRes, z * zRes));
        return dist;
    }

    /**
     * 
     * 1. Calculate the input image histogram. 2. Calculate the image values that represent 2% and 98% of the histogram
     * values and clamp the input image data to that range. 3. Set the cerebrospinal fluid (CSF) threshold minimum value
     * to 30 % of the clamped image range and the CSF threshold maximum value to 70% of the clamped image range. 4.
     * Calculate the brain center of gravity (COG) as the weighted sum of all voxels between the CSF minimum and maximum
     * range using the voxel position scaled by intensity value. Steps 1-4: The preprocessed steps generate the center
     * of gravity (COG) and cerebrospinal fluid (CSF) thresholds, which are used to find the initial white-matter seed
     * point. Potential white-matter seed points are limited to a radius around the COG. The minimum variance is
     * selected from potential seed points with a maximum mean-intensity greater than the CSF threshold. These steps
     * prevent the algorithm from finding a seed point in the background, where the variance may be zero but where the
     * intensity is lower than the CSF threshold.
     */
    private void estimateParameters() {
        final double min = srcImage.getMin();
        final double max = srcImage.getMax();

        // Calculate the histogram:
        final HashMap<Float, Integer> map = new HashMap<Float, Integer>();
        for (int z = 0; z < dimZ; z++) {
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
                    final float value = srcImage.getFloat(x, y, z);
                    int count = 0;
        ...
        final int totalCount = dimX * dimY * dimZ;
        final int count_2P = (int) (0.02 * totalCount);
        final int count_98P = (int) (0.98 * totalCount);

        // Sort the Histogram bins:
        final Set<Float> keySet = map.keySet();
        final Iterator<Float> keyIterator = keySet.iterator();
        final float[] keyArray = new float[keySet.size()];
        int count = 0;
        while (keyIterator.hasNext()) {
            final float value = keyIterator.next();
        ...
        final Vector3f posMin = new Vector3f();
        for (int z = 0; z < dimZ; z++) {
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
        ...
        final float scaleMin = (float) (1f / weightSumMin);
        posMin.scale(scaleMin);
        posMin.X = Math.round(posMin.X);
        posMin.Y = Math.round(posMin.Y);
        posMin.Z = Math.round(posMin.Z);
        final short id = (short) destImage.getVOIs().getUniqueID();
        final VOI centerMin = new VOI(id, "centerMin", VOI.POINT, 1f);
        centerMin.importPoint(posMin);
        destImage.registerVOI(centerMin);
        centerMin.setColor(Color.green);

        final float value = destImage.getFloat((int) posMin.X, (int) posMin.Y, (int) posMin.Z);
        Preferences.debug("CSF min = " + posMin + "   value = " + value + "\n", Preferences.DEBUG_ALGORITHM);
        cog = new Vector3f(posMin);
    }

    /**
     * 5. Estimate the white matter threshold by segmenting the image into 5x5x5 voxel cubes and finding the cube with
     * the highest mean intensity and lowest variance. The mean intensity of the selected cube is used to set the white
     * matter threshold value.
     * 
     * @param image image remapped to the robust image range.
     * @return initial white-matter seed point(s).
     */
    private VOI estimateWhiteMatter(final ModelImage image) {
        final Vector3f min = new Vector3f(dimX, dimY, dimZ);
        final Vector3f max = new Vector3f(0, 0, 0);
        getBoundsDistance(min, max, cog, 60, 60);

        final int cubeSize = 5;
        final int cubeHalf = cubeSize / 2;
        ...
        for (int z = (int) min.Z; z < max.Z - cubeSize; z++) {
            for (int y = (int) min.Y; y < max.Y - cubeSize; y++) {
                for (int x = (int) min.X; x < max.X - cubeSize; x++) {
                        ...
                        for (int y2 = y; y2 < (y + cubeSize); y2++) {
                            for (int x2 = x; x2 < (x + cubeSize); x2++) {
                                if ( (x2 < dimX) && (y2 < dimY) && (z2 < dimZ)) {
                                    final float value = srcImage.getFloat(x2, y2, z2);
                                    meanIntensity += value;
                                    count++;
                                }
                            }
                    ...
                    final Vector3f center = new Vector3f(x + cubeHalf, y + cubeHalf, z + cubeHalf);
        ...
        for (int i = 0; i < sortedIntensities.length; i++) {
            if (srcImage.getImageModality() == FileInfoBase.COMPUTED_TOMOGRAPHY) {
                if ( (sortedIntensities[i].X > csfThresholdMin) && (sortedIntensities[i].X < csfThresholdMax) && (sortedIntensities[i].Y < minVar)) {
        ...
        final short id = (short) image.getVOIs().getUniqueID();
        final VOI seedPoints = new VOI(id, "SeedPoints", VOI.POINT, 1f);
        ...
        for (int i = 0; i < cubeCenters.size(); i++) {
            if (cubeIntensities.elementAt(i).X == maxIntensity && cubeIntensities.elementAt(i).Y <= minVar) {
                // System.err.println( "Seed point: " + cubeCenters.elementAt(i) + "       intensity = " +
                // cubeIntensities.elementAt(i).X + " variance = " + cubeIntensities.elementAt(i).Y );
    ...
    private void fill(final ModelImage image, final Vector3f centerBound, final Vector<Vector3f> seedList, final BitSet visited, final BitSet whiteMatter) {
        final int radiusMax = 110;
        final int radiusMin = 90;
        final Vector3f min = new Vector3f(dimX, dimY, dimZ);
        final Vector3f max = new Vector3f(0, 0, 0);
    ...
    private void fill(final Vector<Vector3f> seedList, final BitSet visited, final BitSet mask) {
    ...
    /**
     * 
     * Once the white-matter seed point is found it is used to initialize a flood-fill algorithm that segments the
     * white-matter of the brain. The seed point is typically found in one hemisphere of the brain. The first step in
     * initializing the flood-fill algorithm is to generate a mirror-image seed-point located in the opposite
     * hemisphere. This enables the flood-fill algorithm to work equally well on both hemispheres. The flood-fill
     * algorithm is limited to an ellipsoid shape around the COG, where the long axis of the ellipsoid is
     * anterior-to-posterior and the short axes of the ellipsoid are superior-to-inferior and left-to-right.
     * 
     * @param image
     * @param seedPoints
     */
    private void fillWhiteMatter(final ModelImage image, final VOI seedPoints) {
        if (seedPoints == null) {
            return;
        }
        ...
        final int length = dimX * dimY * dimZ;
        final BitSet visited = new BitSet(length);
        final BitSet whiteMatter = new BitSet(length);

        final Vector<Vector3f> seedList = new Vector<Vector3f>();
        for (int i = 0; i < seedPoints.getCurves().size(); i++) {
            final VOIBase seedVOI = seedPoints.getCurves().elementAt(i);
            for (int j = 0; j < seedVOI.size(); j++) {
                final Vector3f seed = seedVOI.elementAt(j);
                int index = (int) (seed.Z * dimX * dimY + seed.Y * dimX + seed.X);
                visited.set(index);
                whiteMatter.set(index);
                seedList.add(seed);

                final int diffZ = (int) Math.abs(seed.Z - cog.Z);
                if (diffZ == 0) {
                    continue;
                }
                final Vector3f newSeed = new Vector3f(seed);
                if (seed.Z - cog.Z > 0) {
        ...
        final Vector3f centerBound = new Vector3f(seedPoints.getCurves().elementAt(0).elementAt(0));
        centerBound.X = cog.X;
        centerBound.Z = cog.Z;
    ...
    private void floodFill(final BitSet mask, final Vector3f cog) {
        final int length = dimX * dimY * dimZ;
        ...
        final Vector<Vector3f> seedList = new Vector<Vector3f>();
        seedList.add(cog);
        final int index = (int) (cog.Z * dimX * dimY + cog.Y * dimX + cog.X);
        visited.set(index);
        whiteMatter.set(index);
    ...
    /**
     * Algorithm Implementation Steps: 7. Take the convex-hull of the white matter segmentation and pass this mesh into
     * a modified version of the AlgorithmBrainExtractor. 8. The output of the AlgorithmBrainExtractor is a segmented
     * mesh representing the surface of the brain. To err on the side of not removing too much data, the convex-hull of
     * the brain surface is generated. 9. The final mask representing the data outside the brain is generated using all
     * voxels outside the convex-hull of the brain surface. 10. (Optional final step). If the user chooses to segment
     * only the face region instead of the entire skull region the final step produces the face segmentation from the
     * convex-hull of the brain surface.
     * 
     * Once the white-matter has been segmented, a convex-hull of the segmentation is generated in step 7. This mesh is
     * passed to a modified version of the AlgorithmBrainExtractor in MIPAV. The convex-hull of the white-matter often
     * provides a good enough segmentation of the skull, but in a few cases the convex-hull of the white-matter does not
     * include the entire surface of the brain and processing the mesh with the AlgorithmBrainExtractor corrects the
     * mesh.
     * 
     * Steps 8-9 were added to provide a buffer around the brain segmentation. This enables the algorithm to err on the
     * side of removing too little data rather than removing small portions of the brain surface.
     * 
     */
    private void generateConvexHullMask(final ModelImage image) {
        final TriMesh whiteMatterMesh = createMesh(image.getMask());
        BitSet meshCHMask = new BitSet();
        if (whiteMatterMesh != null) {
            final TriMesh convexHullMesh = convexHull(whiteMatterMesh);
            meshCHMask = createMask(convexHullMesh, false, "_ch_mask");
            image.setMask(meshCHMask);

            TriMesh kMesh = createMesh(image.getMask());
            if (kMesh != null) {
                AlgorithmBrainExtractor betAlg = new AlgorithmBrainExtractor(image, faceOrientation, kMesh, (int) getMedianIntensity(image, meshCHMask), cog);
                betAlg.setExtractPaint(true);
                betAlg.extractBrain();
                betAlg = null;
                final TriMesh convexHullBrain = convexHull(kMesh);
                ...
                // the face mask is generated from the convex-hull of the brain. To generate the face mask, the center
                // of
                // gravity (COG) is used to generate two points on the surface of the mesh. A line parallel to the
                // x-axis
                // centered at the COG intersects the convex-hull, and the intersection point farthest from the COG is
                // selected
                // as the first point on the surface. A second line parallel to the y-axis and centered at the COG
                // intersects the
                // convex-hull and the intersection point farthest from the COG is selected as the second point on the
                // surface.
                //
                // The two points on the hull define a clipping plane that intersects and may cut through the
                // convex-hull.
                // The clipping plane is then moved through the convex-hull to find the point where it is tangent to the
                // surface
                // and does not cut through the surface. The plane is truncated where it intersects the two lines from
                // the COG
                // parallel to the x and y-axes. Truncating the plane prevents the spinal column from being clipped.
                if (face || showFaceSegmentation) {
                    final Vector3f front = new Vector3f(0, cog.Y, cog.Z);
                    final Vector3f bottom = new Vector3f(cog.X, dimY, cog.Z);
                    final Segment3f frontSegment = new Segment3f(front, cog);
                    final Segment3f bottomSegment = new Segment3f(bottom, cog);
                    final IntrSegment3Triangle3f intersection = new IntrSegment3Triangle3f();

                    final int[] aiTris = new int[3];
                    final int iTQuantity = kMesh.GetTriangleQuantity();
                    int iV0, iV1, iV2;
                    final Vector3f kV0 = new Vector3f();
                    final Vector3f kV1 = new Vector3f();
                    final Vector3f kV2 = new Vector3f();
                    ...
                    final Vector3f minDistFrontPoint = new Vector3f();
                    float minDistBottom = Float.MAX_VALUE;
                    final Vector3f minDistBottomPoint = new Vector3f();
                    for (int i = 0; i < iTQuantity; i++) {
                        ...
                        final Triangle3f tri = new Triangle3f(kV0, kV1, kV2);
                        ...
                        if (intersection.Find()) {
                            ...
                            final float distance = front.distance(intersectionPoint);
                        ...
                        if (intersection.Find()) {
                            ...
                            final float distance = bottom.distance(intersectionPoint);
                    ...
                    Plane3f clipPlane = new Plane3f(new Vector3f(minDistFrontPoint.X, minDistFrontPoint.Y, 0), new Vector3f(minDistBottomPoint.X,
                            minDistBottomPoint.Y, 0), new Vector3f(minDistFrontPoint.X, minDistFrontPoint.Y, dimZ));

                    float minDist = Float.MAX_VALUE;
                    int minIndex = -1;
                    for (int i = 0; i < kMesh.VBuffer.GetVertexQuantity(); i++) {
                        final float distance = clipPlane.DistanceTo(kMesh.VBuffer.GetPosition3(i));
                        if (distance < minDist) {
                            minDist = distance;
                            minIndex = i;
                        }
                    ...
                    if (minIndex != -1) {
                        final Vector3f pos = kMesh.VBuffer.GetPosition3(minIndex);
                        clipPlane = new Plane3f(clipPlane.Normal, pos);
                        final BitSet clipMask = createMask(clipPlane, cog);
                        if (face || showFaceSegmentation) {
    ...
    private void getBoundsDistance(final Vector3f min, final Vector3f max, final Vector3f centerBound, final float limitX, final float limit) {
        final float xRes = srcImage.getFileInfo(0).getResolutions()[0];
        final float yRes = srcImage.getFileInfo(0).getResolutions()[1];
        final float zRes = srcImage.getFileInfo(0).getResolutions()[2];
        final Vector3f range = new Vector3f(limitX / xRes, limit / yRes, limit / zRes);
        final Vector3f pos = new Vector3f(centerBound);
     ...
     */
    private float getIntersectX(final Vector3f kV0, final Vector3f kV1, final Vector3f kV2, final float iY, final float iZ) {
     ...
     */
    private float getIntersectY(final Vector3f kV0, final Vector3f kV1, final Vector3f kV2, final float iX, final float iZ) {
     ...
     */
    private float getIntersectZ(final Vector3f kV0, final Vector3f kV1, final Vector3f kV2, final float iX, final float iY) {
    ...
    private float getMedianIntensity(final ModelImage image, final BitSet mask) {
        int iIQuantity = 0;
        final float[] intensity = new float[mask.size()];
        for (int z = 0; z < dimZ; z++) {
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
                    final int index = (z * dimX * dimY + y * dimX + x);
     ...
     * @return
     */
    private ModelImage randomize(final ModelImage image, final int stepSize) {
        final int dimX = image.getExtents()[0];
        final int dimY = image.getExtents()[1];

        final ModelImage blurImage = (ModelImage) image.clone();
        blurImage.setImageName(JDialogBase.makeImageName(image.getImageName(), "_blur"));
        final int stepsX = image.getExtents()[0] / stepSize;
        final int stepsY = image.getExtents()[1] / stepSize;
        final int stepsZ = image.getExtents()[2] / stepSize;
        final int length = stepsX * stepsY * stepsZ;

        final float[] blockAvg = new float[length];
        ...
        for (int z = 0; z < stepsZ; z++) {
            for (int y = 0; y < stepsY; y++) {
                for (int x = 0; x < stepsX; x++) {
                    ...
                    final int blockIndex = z * stepsY * stepsX + y * stepsX + x;
        ...
        final boolean[] swapped = new boolean[length];
        final int[] blockIndex = new int[length];
        ...
        for (int z = 0; z < stepsZ; z++) {
            for (int y = 0; y < stepsY; y++) {
                for (int x = 0; x < stepsX; x++) {
                    ...
                    final int randomIndex = z * stepsY * stepsX + y * stepsX + x;
                    ...
                    for (int z1 = targetZ; z1 < targetZ + stepSize; z1++) {
                        ...
                        for (int y1 = targetY; y1 < targetY + stepSize; y1++) {
                            ...
                            for (int x1 = targetX; x1 < targetX + stepSize; x1++) {
                                ...
                                {
                                    // System.err.println( x1 + " " + y1 + " " + z1 + "      " + srcX + " " + srcY + " "
                                    // + srcZ );
     ...
     * @return
     */
    private ModelImage registerImages(final ModelImage target, final ModelImage src) {
        final AlgorithmRegOAR3D reg3 = new AlgorithmRegOAR3D(target, src, 1, 12, 0, -30, 30, 15, 6, -30, 30, 15, 6, -30, 30, 15, 6, true, true, true, false, 2,
                3);
        reg3.setJTEM(false);
        reg3.run();
        ...
        if (reg3.isCompleted()) {
            ...
            final AlgorithmTransform transform = new AlgorithmTransform(src, finalMatrix, AlgorithmTransform.TRILINEAR, xresA, yresA, zresA, xdimA, ydimA,
                    zdimA, true, false, false);

            transform.setUpdateOriginFlag(true);
            transform.setFillValue((float) src.getMin());
            transform.run();
            final ModelImage resultImage = transform.getTransformedImage();
            transform.finalize();
    ...
    private TriMesh removeUnusedVertices(final TriMesh kMesh) {
        final int[] aiIndex = kMesh.IBuffer.GetData();
        final Vector<Vector3f> vertexList = new Vector<Vector3f>();
        final Vector<Integer> triList = new Vector<Integer>();
        int indexCount = 0;
        for (int i = 0; i < aiIndex.length; i++) {
            final Vector3f kPos = kMesh.VBuffer.GetPosition3(aiIndex[i]);
        ...
        final int[] aiNewIndex = new int[triList.size()];
        for (int i = 0; i < triList.size(); i++) {
            aiNewIndex[i] = triList.elementAt(i);
        }
        final IndexBuffer kIBuffer = new IndexBuffer(aiNewIndex);
        final VertexBuffer kVBuffer = new VertexBuffer(vertexList);
        return new TriMesh(kVBuffer, kIBuffer);
    }

    /**
     * Scales the input mesh based on the offset parameter set in the constructor. This enables the user to set a buffer
     * around the segmentation.
     * 
     * @param mesh
     */
    private void scaleMesh(final TriMesh mesh) {
        final float xRes = srcImage.getFileInfo(0).getResolutions()[0];
        final float yRes = srcImage.getFileInfo(0).getResolutions()[1];
        final float zRes = srcImage.getFileInfo(0).getResolutions()[2];

        final Vector3f scale = new Vector3f(offSet / xRes, offSet / yRes, offSet / zRes);

        for (int i = 0; i < mesh.VBuffer.GetVertexQuantity(); i++) {
            final Vector3f pos = mesh.VBuffer.GetPosition3(i);
            final Vector3f normal = mesh.VBuffer.GetNormal3(i);
            normal.mult(scale);
            pos.add(normal);
    ...
    private void skullRemoval() {
        ...
        final VOI seedPoints = estimateWhiteMatter(destImage);
        ...
        if (showFaceSegmentation | showSkullSegmentation) {
            ...
            whiteMatterImage = (ModelImage) destImage.clone();
        ...
        if (reMappedSrc) {
            ...
            if (MipavCoordinateSystems.matchOrientation(originalAxis, targetAxis, axisOrder, axisFlip)) {
                final AlgorithmRotate rotateAlgo = new AlgorithmRotate(destImage, axisOrder, axisFlip);
                rotateAlgo.setRunningInSeparateThread(false);
                rotateAlgo.run();
        ...
        if (atlasBasedImage != null) {
            final int[] axisOrder = {0, 1, 2, 3};
            final boolean[] axisFlip = {false, false, false, false};
            if (MipavCoordinateSystems.matchOrientation(atlasBasedImage.getAxisOrientation(), destImage.getAxisOrientation(), axisOrder, axisFlip)) {
                final AlgorithmRotate rotateAlgo = new AlgorithmRotate(destImage, axisOrder, axisFlip);
                rotateAlgo.setRunningInSeparateThread(false);
                rotateAlgo.run();
                destImage.disposeLocal();
            ...
            if (reMappedTargetAtlas) {
                ...
                if (MipavCoordinateSystems.matchOrientation(originalAxisTargetAtlas, targetAxis, axisOrder, axisFlip)) {
                    final AlgorithmRotate rotateAlgo = new AlgorithmRotate(atlasBasedImage, axisOrder, axisFlip);
                    rotateAlgo.setRunningInSeparateThread(false);
                    rotateAlgo.run();
                ...
                final ModelImage temp = (ModelImage) atlasBasedImage.clone();
        ...
        final BitSet mask = (BitSet) (destImage.getMask().clone());
        destImage.disposeLocal();
        ...
        if (showFaceSegmentation | showSkullSegmentation) {
            ...
            final short id = (short) destImage.getVOIs().getUniqueID();
            final VOI centerGravity = new VOI(id, "centerGravity", VOI.POINT, 1f);
     ...
     * @param image
     */
    private void subtractMask(final ModelImage image) {
        final float min = (float) image.getMin();
        for (int z = 0; z < dimZ; z++) {
            for (int y = 0; y < dimY; y++) {
                for (int x = 0; x < dimX; x++) {
                    final int index = z * dimX * dimY + y * dimX + x;
