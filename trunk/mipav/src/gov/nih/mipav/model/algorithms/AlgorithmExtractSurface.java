 ...
 *      @author  David H. Eberly, Ph.D. wrote all the extration and decimation code found in
 *               the SurfaceExtraction, SurfaceDecimation and associated classes, with a little
 *               input from Matt with speed and memory optimizations
...
public class AlgorithmExtractSurface extends AlgorithmBase {
     ...
     */
    public final static int ADJ_MODE = 1;

    /**
     *	Use smoothing model to perform triangle consistency. Smooths normals.
     */
    public final static int SMOOTH_MODE = 2;
    ...
    /**
     *   If true then the extracted surface is decimated into a
     *   continous level of detail surface (clod)
     */
     ...
     */
    public AlgorithmExtractSurface( ModelImage image, float level, int mode,
            int triMode,
            boolean decFlag, boolean blurFlag,
            float sigma, String fileName,
            boolean smoothFlag ) {
        super( null, image );
    ...
    private void init() {
        ...
        try {
            if ( mode == VOI_MODE ) {
                ...
                maskImage = new ModelImage( ModelImage.USHORT, destExtents, "Surface image", srcImage.getUserInterface() );
    ...
    private void extractSurface() {
        ...
        try {
            ...
            // put two blank slices at the front and back of surface image buffer, then pass this mask into the extractor
            ...
            if (srcImage.getFileInfo()[0].getTransformID() ==
            ...
            if ( decimateFlag == true ) {
                ...
                ModelClodMesh.save( surfaceFileName, akClod, true, direction, origin, box,
                                    inverseDicomArray);
            } else {
                progressBar.updateValue( 75, activeImage );
                progressBar.setMessage( "Saving surface" );
                kMesh.save( surfaceFileName, true, direction, origin, box,
                            inverseDicomArray);
