...
/**********************************************************************\
  ...
  setupBrightness() the LUT is setup. This mallocs enough space
  for *bp and then repositions the pointer to the center of the
...
public class AlgorithmNLNoiseReduction extends AlgorithmBase {
    ...
    *   @param bt       Brightness threshold: this allows discrimination between
    *                   noise and the underlying image.  Ideally, the value should
    *                   be set greater than the noise level and less than the
    *                   contrast of the underlying image.  Edges of contrast smaller
    *                   than this threshold will be blurred whereas those of greater
    *                   contrast will not be.  Reducing bt gives less smoothing.
    *                   bt has a default value = 0.1 * (srcMax - srcMin).
    *   @param maskSD   This determines the spatial extent of the smoothing.  The
    *                   mask is basically Gaussian with standard deviation (in
    *                   image units - e.g. mm.) set by the user.  However, for a
    *                   small, fast, flat response with a 3x3 or 3x3x3 voxel mask,
    *                   set maskSD to zero.  maskSD has a default value equal to the
    *                   x resolution.
    *   @param useMedian If true, the center pixel is not included in the sums.
    *                   When the local neighborhood of similar brightness
    *                   voxels is empty, a local median filter is used.  This allows
    *                   the correction of impulse("salt-and-pepper")noise.  If false,
    *                   this feature is turned off.  In this case, the center pixel
    *                   is included in the sums.  When no neighborhood is found,
    *                   the original intensity of the voxel of interest remains
    *                   unchanged.
    ...
    */
	public AlgorithmNLNoiseReduction(ModelImage destImg, ModelImage srcImg, double bt,
	                        float maskSD, boolean useMedian, boolean img25D) {
    ...
    *                   image units - e.g. mm.) set by the user.  However, for a
    *                   small, fast, flat response with a 3x3 or 3x3x3 voxel mask,
    *                   set maskSD to zero.  maskSD has a default value equal to
    *                   the x resolution.
    *   @param useMedian If true, the center pixel is not included in the sums.
    *                   When the local neighborhood of similar brightness
    *                   voxels is empty, a local median filter is used.  This allows
    ...
    *                   should be processed independently. 2D images disregard this flag.
    */
    public AlgorithmNLNoiseReduction(ModelImage srcImg, double bt, float maskSD,
                            boolean useMedian, boolean img25D) {
       super(null, srcImg);
    ...
    */
    private void constructLog() {
        ...
        historyString = new String( "Nonlinear Noise reduction(" + String.valueOf(brightThres) + ", " +
                                 String.valueOf(maskStdDev)   +  ", " +
                                 String.valueOf(useMedian) + ", " +
    ...
    *
    * The SUSAN brightness function is implemented as a LUT
    * (Look-Up-Table) for speed. In
    * setupBrightness() the LUT is setup. This creates a 257 entry
    * brightness table. The SUSAN function e^-(x^2) is
    * calculated and converted to a byte in the range 0-100, for all
    * possible image brightness differences (including negative
    * ones). Thus brightnessTable[23] is the output for a brightness
    * difference of 23*(srcMax - srcMin)/256.
    * In the smoothing and smoothing3D algorithms this LUT is used as follows:

    * ip = ((i-yMaskSize)*xSize) + j - xMaskSize;
    * ip = (i-1)*x_size + j - 1;
    * ip points to the first image pixel in the circular mask surrounding
    * point (x,y).
    * ip = (k-zMaskSize)*xSize*ySize + (i-yMaskSize)*xSize + j-xMaskSize;
    * ip = (k-1)*xSize*ySize + (i-1)*xSize + j-1;
    * ip points to the first image voxel in the spherical mask surrounding
    * point (x,y,z).
    * brightness is the intensity value at this mask point

    * i*x_size+j points to the center pixel (x,y).
    * k*xSize*ySize+i*xSize+j points to the center voxel (x,y,z).
    * center is the intensity value of this center point

    * tmp = gaussMask[dpt++] * brightnessTable[(int)Math.abs(256*(center-brightness)/(srcMax - srcMin))];
    * brightnessTable gives the SUSAN brightness function of the absolute value of the
    * difference of the 2 intensity values.  dpt gives the current position inside the mask,
    * so the SUSAN brightness function is multiplied by a Gaussian weighting function.
    * In area += tmp, tmp is added to the area
    * and in total += tmp * brightness, the total area*brightness is calculated.
    * After summing over all mask values and performing median replacement if requested,
    * total is divided by area to obtain the filtered intensity value.
    *
    */
    ...
    */
    private void smoothing2D(int z){
        ...
        if ( z == 0 ) {
            for(i =- yMaskSize; i <= yMaskSize; i++) {
                for(j =- xMaskSize; j <= xMaskSize; j++) {
                    gaussMask[maskPt++] = (byte) (100.0 * Math.exp( ((float)(i*i))/yStdDevLocal + ((float)(j*j))/xStdDevLocal ));
                }
            }
        }
        ...
        for (i = maskSize;  i < ySize-maskSize && !threadStopped; i++) {    // use maskSize as enlarge was isotropic
            newProgressValue = 100 * (z*smallSliceSize + (i-maskSize+1)*(xSize-2*maskSize)) / (zSize*smallSliceSize);
            ...
            for (j = maskSize; j < xSize-maskSize && !threadStopped; j++) {
                ...
                for(y =- yMaskSize; y <= yMaskSize; y++) {
                    for(x =- xMaskSize; x <= xMaskSize; x++)  {
                        ...
                        //tmp   = gaussMask[maskPt++] * brightnessTable[(int)Math.abs(256*(center-brightness)/(srcMax - srcMin))];
                        area  += tmp;
    ...
    private void addBorder(float in[], float tmpEnImage[], int border) {
        ...
        for(i=0; i < border; i++)       // copy top and bottom rows; invert as many as necessary
            for (j = 0; j < xSize; j++)
            {
                ...
                tmpEnImage[(ySize+border+i)*(xSize+2*border)+border+j] = in[(ySize-i-1)*xSize+j];
            }

        for(i=0; i<border; i++)         // copy left and right columns
            for(j=0; j < ySize+2*border; j++)
            {
                tmpEnImage[j*(xSize+2*border)+border-1-i]       = tmpEnImage[j*(xSize+2*border) + border+i];
                tmpEnImage[j*(xSize+2*border)+ xSize+border+i]  = tmpEnImage[j*(xSize+2*border) + xSize+border-1-i];
            }
    ...
    private void smoothing3D() {
            ...
            for(i=-yMaskSize; i<=yMaskSize; i++) {
                for(j=-xMaskSize; j<=xMaskSize; j++) {
                    gaussMask[maskPt++] = (byte) (100.0 *
                                        Math.exp( ((float)(i*i))/xStdDev + ((float)(j*j))/yStdDev + ((float)(k*k))/zStdDev ));
                }
            }
        }
        ...
        for (k = maskSize; k < zSize-maskSize && !threadStopped; k++) { // use maskSize as enlarge was isotropic
            ...
            for (i = maskSize; i < ySize-maskSize && !threadStopped; i++) {
                newProgressValue = 100 * ((k-maskSize)*smallSliceSize + (i-maskSize)*(xSize-2*maskSize))/
    ...
    */
    private void addBorder3D(float data[], float tmpEnImage[], int border) {
        ...
        for(k=0; k < zSize; k++)                          // copy data into tmpImage
            for(i=0; i < ySize; i++)
                for (j = 0; j < xSize; j++)
                    tmpEnImage[(k+border)*(xSize+2*border)*(ySize+2*border)+(i+border)*(xSize+2*border)+border+j] =
                        data[k* xSize* ySize+i* xSize+j];

        for(k=0; k<zSize; k++) {
            for(i=0; i<border; i++) {                   // copy top and bottom rows; invert as many as necessary

                for (j = 0; j < xSize; j++) {
                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+(border-1-i)*(xSize+2*border)+border+j] =
                        data[ k* ySize * xSize + i* xSize+j];
                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+(ySize+border+i)*(xSize+2*border)+border+j] =
                        data[ k* ySize * xSize + (ySize-i-1)* xSize+j];
                }
            }
            for(i=0; i < border; i++)  {
                for(j=0; j < ySize+2*border; j++) {
                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+j*(xSize+2*border)+border-1-i]=
	                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+j*(xSize+2*border)+border+i];
                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+j*(xSize+2*border)+ xSize+border+i]=
	                    tmpImage[(k+border)*(xSize+2*border)*(ySize+2*border)+j*(xSize+2*border)+ xSize+border-1-i];
                }
            }
        }
        ...
        for(k=0; k<border; k++) {
            for (j = 0; j < (xSize+2*border)*(ySize+2*border); j++) {
                tmpImage[(border-1-k)*(xSize+2*border)*(ySize+2*border)+j] =
	            tmpImage[(border+k)*(xSize+2*border)*(ySize+2*border)+j];
                tmpImage[(zSize+border+k)*(xSize+2*border)*(ySize+2*border)+j] =
	            tmpImage[(zSize+border-1-k)*(xSize+2*border)*(ySize+2*border)+j];
            }
        }
