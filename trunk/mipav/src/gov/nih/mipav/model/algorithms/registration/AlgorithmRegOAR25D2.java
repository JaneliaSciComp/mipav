 ...
 *
 *   In adjacent mode the first slice above the reference slice is
 *   registered to the reference slice, then the second slice above the
 *   reference slice is registered to the first slice above the
 *   reference slice, and so on until the last slice is registered
 *   the next to last slice.  Then, the first slice below the
 *   reference slice is registered to the reference slice, the second
 *   slice below the reference slice is registered to first slice below
 *   the reference slice, and so on until the first slice is registered
 *   to the second slice.  In reference mode every slice is simply
 *   registered to the reference slice.
 *
 ...
 *        We loop thru a number of iterations equal to the number of slices - 1
 *        in adjacent mode or equal to the number of slices in reference mode with
 *        one slice as the input slice and one slice as the reference slice.  For each
 *        iteration:
 *   4.)  With the slices that were subsampled by 8 or 16, we call levelEight.  This function
 ...
 *        into their original position in the multi-array.  We look at the 3 neighbors of a point:
 *        +, =, or - one fine sample.  If our point has a cost greater than any of these, it is not
 ...
 *   9.)  We now call levelFour with the slices subsampled by 4 and the vectors of minima.  We measure the
 *        costs of the minima on the new slices and sort them.  We take the top numMinima in each vector
 *        (pre-optimization and post-optimization) and optimize them.  We put them all into one vector.<br>
 *   10.) We perturb the rotation by zero and by plus-minus fineDelta.  If it's not a rigid transformation,
 ...
 *   12.) We now call levelTwo with the slices subsampled by 2.  We measure the costs of the minima
 *        at the new slices.  We optimize the best minimum with 4 degrees of freedom, then 5, then 6.
 ...
 *   13.) We call levelOne with the un-subsampled slices.  At levelOne, one optimization run is performed,
 *        with the maximum allowable degrees of freedom, as specified by the user (the max is 6).<br>
 *   14.) The best answer is returned from levelOne.  The inverse of the matrix from this answer
 *        is used to register the input slice to the reference slice.  The registered input slice
 *        data is imported into inputImage.<br>
 *
...
public class AlgorithmRegOAR25D2 extends AlgorithmBase {
    ...
    /** Other images which will not determine the registration,
     but which will undergo the same transformations as input image.
     Useful if registering a color image based on only 1 of the colors */
    ...
    /** If true use adjacent image for registration. If false use image slice reference number
     to indicate the image slice to register all images to.*/
    ...
    // Produce 2 output graphs - 1 for rotation and 1 for
    // 2 translations.  Only can be used for DOF == 3 and
    // register to reference slice
    ...
    /* Flags are true if weighted image is not present or if weighted image subsampling occurred,
     false if weighted image subsampling did not occur */
    ...
    // Arrays used for producing graphs for DOF == 3
    // and register to reference image
     ...
     * @param refImageNum    If register to reference, the slice all other slices are registered
     *                       to.  If register to adjacent, the first slice used as a reference.
     ...
     * @param doGraph        If true produce 2 output graphs - one for rotation
     *                       and one for 2 translations
     ...
     */
    public AlgorithmRegOAR25D2( ModelImage _image, int _costChoice, int _DOF,
            int _interp, int _interp2,
            boolean mode, int refImageNum,
            float _rotateBegin, float _rotateEnd,
            float _coarseRate, float _fineRate,
            boolean doGraph, boolean doSubsample,
     ...
     *	@param _coarseRate	Point at which coarse samples should be taken (i.e., every 45 degrees).
     *	@param _fineRate	Point at which fine samples should be taken (i.e., every 15 degrees).
     *    @param doGraph      If true produce 2 output graphs - 1 for rotation and
     *                        one for 2 translations
     *    @param doSubsample  If true subsample
     *    @param transformVOIs If true, transform VOIs
     *    @param _bracketBound The bracket size around the minimum in multiples of unit_tolerance
     *                         in the first iteration of Powell's algorithm.
     ...
     *    @param _numMinima   Number of minima from level 8 to test at level 4
     */
    public AlgorithmRegOAR25D2( ModelImage _image, ModelImage _inputWeight,
            ...
            float _coarseRate, float _fineRate,
            boolean doGraph,
            boolean doSubsample, boolean transformVOIs,
            int _bracketBound,
    ...
    /**
     inputImage2 is not used to determine the registration, but each slice of
     inputImage2 undergoes the same transformation as inputImage.  This is useful
     for color images where you wish the registration to be determined by only 1
     of the colors
     */
    ...
    /**
     inputImage3 is not used to determine the registration, but each slice of
     inputImage3 undergoes the same transformation as inputImage.  This is useful
     for color images where you wish the registration to be determined by only 1
     of the colors
     */
     ...
     *   the image is transformed into isotropic pixels.  The resolutions
     *   of the two images after the xy isotropic transformation will be the same in the x and y
     *   dimensions.  That resolution will equal the minimum resolution.
     *   If the image is weighted, the weight image is transformed into
     *   isotropic pixels in the same manner as the original.  Then the image is
     *   subsampled by 2, 4, and 8 or 16.  If the image is too small it will not be subsampled
     *   down to the smallest level; if it is too big, it will be subsampled to 16.
     *   The same is done with the weight image if necessary.  The program loops thru
     *   levelEight, levelFour, levelTwo, and levelOne with one slice as the reference
     *   slice and one slice as the input slice.  The function levelEight
     *   is called with the slices subsampled by 8 or 16; it returns two vectors with minima.  Then
     *   the function levelFour is called with slices subsampled by 4 and the two vectors; it
     *   returns one vector of minima.  The function levelTwo is called with slices subsampled
     ...
     *   minimum, which is used to register the input slice to the reference slice.  The
     *   registered input slice data is imported into inputImage.
     */
    public void runAlgorithm() {
        ...
        if ( doColor ) {
            input1 = new ModelImage( ModelImage.ARGB_FLOAT, iExtents, "slice2D", inputImage.getUserInterface() );
            if ( inputImage2 != null ) {
                input2 = new ModelImage( ModelImage.ARGB_FLOAT, iExtents, "slice2DI2", inputImage.getUserInterface() );
            }
            if ( inputImage3 != null ) {
                input3 = new ModelImage( ModelImage.ARGB_FLOAT, iExtents, "slice2DI3", inputImage.getUserInterface() );
            }
        } else {
            input1 = new ModelImage( ModelImage.FLOAT, iExtents, "slice2D", inputImage.getUserInterface() );
            if ( inputImage2 != null ) {
                input2 = new ModelImage( ModelImage.FLOAT, iExtents, "slice2DI2", inputImage.getUserInterface() );
            }
            if ( inputImage3 != null ) {
                input3 = new ModelImage( ModelImage.FLOAT, iExtents, "slice2DI3", inputImage.getUserInterface() );
            }
        }
        ...
        if ( weighted ) {
            inputw_1 = new ModelImage( ModelImage.FLOAT, iExtents, "slice2DW", inputImage.getUserInterface() );
        ...
        for ( int m = 0; m < endIndex; m++ ) {
            ...
            progressBar.updateValue( (int) ( m / (float) ( inputImage.getExtents()[2] - 1 ) * 100 ), activeImage );
     ...
     *   Sets up the cost function with the slices and the weighted slices,
     *   if necessary.
     *	Uses the coarse sampling rate and optimize translations and global
     ...
     *   at the 3 neighbors of a point: +, =, or - one fine sample.  If the point
    ...
    /**
     *   Takes two slices that have been subsampled by a factor of four, and
     *   two vectors of minima.  Sets up the cost function with the slices
     *   and the weighted slices, if necessary.  Adds the level4Factor determined
     ...
     *   them all into one vector.  Perturbs the rotation by zero and by plus-minus fineDelta.
     *   If it's not a rigid transformation, perturbs the scales by 0,
     *   plus-minus .1, then plus-minus .2.
    ...
    private Vector levelFour( ModelSimpleImage ref, ModelSimpleImage input,
            Vector minima, Vector optMinima ) {
        ...
        for ( int j = 0; j < 2 && !threadStopped; j++ ) {
            for ( int i = 0; i < total && !threadStopped; i++ ) {
                ...
                powell.setInitialPoint( initial );
                //powell.setSeparateThread(false);
                powell.run();
                item = new MatrixListItem( powell.getCost(), powell.getMatrix(), powell.getFinal() );
    ...
    /**
     *   Takes two slices that have been subsampled by a factor of 2 and
     ...
     */
    private MatrixListItem levelTwo( ModelSimpleImage ref, ModelSimpleImage input,
            Vector minima ) {
     ...
     *   Takes the two slices, no subsampling.
     *   Sets up the cost function with the slices and the weighted slices.
     *   Adds the level1Factor determined during subsampling.
    ...
    private MatrixListItem levelOne2D( ModelSimpleImage ref, ModelSimpleImage input ) {
        ...
        maxIter = 4 * baseNumIter;
        AlgorithmPowellOpt2D powell = new AlgorithmPowellOpt2D( this, cog, degree, cost, initial, getTolerance( degree ),
                maxIter, rigidFlag, bracketBound );
     ...
     */
    private void interpolate( double x, double[] initial, double[][] tranforms,
            boolean scale ) {
        int ix0, ix1;
     ...
     *   (the length of the tolerance is the degrees of freedom) and
     *   the level of subsampling (1, 2, 4, 8).
     *   @param DOF   Degrees of freedom, will be length of vector.
     *   @param dims	Image dimensions in x, y and z.
     *   @param res	Pixel resolution in x, y and z.
    ...
    /*private double[] getTolerance(int DOF, int dims[], float res, float factor) {
     ...
     int i;
     for (i=0; i<3; i++) {
     rad[i]=(dims[i]*res)/2; // field of view divided by two
     }
     double mean_rad = (rad[0]+rad[1])/2.0; //2.5 D, only rotation about z

     if (DOF == 2) {
     tols[0] = tols[1] = (res/2.0) * factor; // translations
     }
     else if (DOF == 3 && rigidFlag == true) {
     tols[0] = ((180./Math.PI)*res/(2*mean_rad)) * factor; // rotation
     tols[1] = tols[2] = (res/2.0) * factor; // translations
     }
     else if (DOF == 3 && rigidFlag == false) {
     tols[0] =  (res/(2*mean_rad)) * factor; // global scaling
     tols[1] = (tols[2] = res/2.0) * factor; // translations
     }
     else if (DOF == 4) {
     tols[0] = ((180./Math.PI)*res/(2*rad[0])) * factor; // rotation tolerances
     tols[1] = (tols[2] = res/2.0) * factor; // translation tolerance x and y
     tols[3] =  (res/(2*mean_rad)) * factor; // scaling tolerances global
     }
     else if (DOF == 5) {
     tols[0] = ((180./Math.PI)*res/(2*rad[0])) * factor; // rotation tolerances
     tols[1] = tols[2] = (res/2.0) * factor; // translation tolerance x and y
     tols[3] = tols[4] =  (res/(2*mean_rad)) * factor; // scaling tolerance x
     }
     else if (DOF == 6) {
     tols[0] = tols[1] = (res/2.0) * factor; // translation tolerance x and y
     tols[2] = tols[3] = (res/(2*mean_rad)) * factor; // scaling tolerance x
     tols[4] = tols[5] = (res/(2*mean_rad)) * factor; // skew tolerance x
     }
     return tols;
     }*/
    ...
    class MatrixListItem
        ...
        protected TransMatrix matrix = new TransMatrix( 3 );
         ...
         */
        protected MatrixListItem( double _cost, TransMatrix _matrix,
                double[] _initial ) {
            this.cost = _cost;
            this.matrix = _matrix;
