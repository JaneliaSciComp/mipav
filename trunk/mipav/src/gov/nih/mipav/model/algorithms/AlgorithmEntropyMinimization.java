...
public class AlgorithmEntropyMinimization extends AlgorithmBase {
  // This software performs retrospective shading correction based on entropy
  // minimization. If the image is color, the colors are independently processed
  // one at a time and then recombined.
  // The user can select either both multiplicative and additive quadratic
  // noise, multiplicative quadratic noise only, or cubic multiplicative
  // noise only.  Quartic multiplicative noise could not be made to work.
  // Shading distorts an image with spurious intensity variations not present
  // in the original object. These intensity variations can be caused by
  // nonuniform illumination, imperfections in the optics, uneven camera
  // sensitivity, or imperfect specimen preparation.  Let U(x,y) represent
  // the true image, N(x,y) represent the acquired image, SM(x,y) represent
  // the multiplicative shading component, and SA(x,y) represent the additive
  // shading component.  Then:
  // N(x,y) = U(x,y) * SM(x,y) + SA(x,y)
  // Shading correction calculates the corrected image U(x,y) with
  // U(x,y) = (N(x,y) - SA(x,y))/SM(x,y)
  // SA(x,y) and SM(x,y) are modeled by second-order polynomials
  // For 2D:
  // SA(x,y) = a1*(x - ac1)/ad1+ a2*(y -ac2)/ad2 + a3*(x*y -ac3)/ad3
  //           + a4*(x*x - ac4)/ad4 + a5*(y*y - ac5)/ad5
  // SM(x,y) = 1 + m1*(x - mc1)/md1 + m2*(y - mc2)/md2 + m3*(x*y -mc3)/md3
  //           + m4*(x*x - mc4)/md4 + m5*(y*y - mc5)/md5
  // The software clamps SA(x,y) to be between -image maximum/2 and
  // +image maximum/2.  The software clamps SM(x,y) to be between 0.05 and
  // 20.0.

  // The user can choose to use thresholding to exclude the background from
  // any processing.  Excluded pixels will have the same values as
  // the original source image.  All pixels with values below threshold that
  // are 4 or 6 neighbor connected thru below threshold pixels to a boundary
  // pixel which is below threshold will be excluded from processing.

  // By default image subsampling is used in histogram formation since
  // using 5000 images samples is sufficient to form the histogram.  If
  // subsampling is selected, pixelIncrement for both x and y is set equal to:
  // Math.max(1,(int)Math.sqrt(histogramAvailable/5000))
  // If subsampling is not selected, pixelIncrement equals 1

  // Powell's algorithm is used to search for the values of the 10 shading
  // parameters that minimize the entropy of the blurred histogram of the
  // U(x,y) image.  A histogram is formed to represent integers from the
  // floor(image U minimum) to the ceiling(image U maximum).  For each floating
  // point gray value gp in U(x,y) a count of 1 is split between the 2 integer
  // bins encompassing it.  Let the bins be k and k+1.  k+1-gp is added to
  // bin k and gp - k is added to bin k+1.  Gray values in locations
  // corresponding to the image minimum or image maximum in the original input
  // image are not used in creating this histogram, since they may represent
  // regions where saturation effects are occurring.  Then, a blurred histogram
  // is created from the original histogram.  The original histogram is blurred
  // with a triangular window of size 5:
  // blurredHistogram[i] = originalHistogram[i-2] + 2*originalHistogram[i-1]
  // + 3*originalHistogram[i] + 2*originalHistogram[i+1]
  // + originalHistogram[i+2]
  // The blurred histogram is normalized to give probabilities by dividing
  // the count in each bin by the total number of counts in the blurred
  // histogram.
  // The entropy is formed by summing the -probability(n)*log(probability(n))
  // over all bins n with nonzero counts.

  // References:  This software implements the algorithm discussed in:
  // 1.) Retrospective shading correction based on entropy minimization by
  //     B. Likar, J. B. A. Maintz, M. A. Viergever, and F. Pernus, Journal
  //     of Microscopy, Vol. 197, Pt 3., March, 2000, pp. 285-295.
  // 2.) Retrospective Correction of MR Intensity Inhomogeneity by Information
  //     Minimization by Bostjan Likar, Max Viergever, and Franjo Pernus,
  //     IEEE Transactions on Medical Imaging, Vol. 20, No. 12, December,
  //     2001, pp. 1398-1410.
  // 3.) Real-time automated visual inspection of color tablets in
  //     pharmaceutical blisters by Joze Derganc, Bostjan Likar, Rok Bernard,
  //     Dejan Tomazevic, and Franjo Pernus, Real-Time Imaging, Vol. 9, 2003,
  //     pp. 113-124.
  // Powell's algorithm and supporting routines are derived from:
  // 4.) Numerical Recipes in C The Art of Scientific Computing Second Edition
  //     by William H. Press, Saul A. Teukolsky, William T. Vetterling, and
  //     Brian P. Flannery, Cambridge University Press, Chapters 10.1, 10.2,
  //     and 10.5.

  // Quadratic multiplicative and quadratic additive noise
  ...
  // If true don't use pixels below thresholdLevel that are 4 or 6 neighbor
  // connected to boundary pixels below thresholdLevel
...
// Iterations of powell's algorithm
  ...
  private double mc1, mc2, mc3, mc4, mc5, mc6, mc7, mc8, mc9, mc10, mc11, mc12,
                 mc13, mc14, mc15, mc16, mc17, mc18, mc19;
  private double md1, md2, md3, md4, md5, md6, md7, md8, md9, md10, md11, md12,
                 md13, md14, md15, md16, md17, md18, md19;


    /**
     *   @param resultImage     entropy minimized image
     *   @param srcImage        original image
     *   @param thresholdSelected If true, don't use pixels that are 4 or 6
     *                            neighbor connected to boundary pixels
     *                            below thresholdLevel
     *   @param thresholdLevel
     *   @param subsample
     *   @param noiseType
     */
    public AlgorithmEntropyMinimization( ModelImage resultImage,
           ModelImage srcImage, boolean thresholdSelected,
    ...
    private void constructLog() {
        historyString = new String(
                "EntropyMinimization(" + thresholdSelected + ", " +
                                         thresholdLevel +  ", " +
                                         subsample +  ", " +
                                         noiseType + ")\n" );
    }
            ...
            for (c = 1; c <= maxColor; c++) {
              if ((!srcImage.isColorImage()) || ((c == 1) && (doRed)) ||
                ...
                if (subsample) {
                  ...
                  pixelIncrement = Math.max(1,
                                            (int) Math.sqrt(histogramAvailable / 5000));
                } // if (subsample)
                ...
                Preferences.debug("Iterations of Powell's algorithm = "
                                  + iter + "\n");
            ...
            for (c = 1; c <= maxColor; c++) {
              if ((!srcImage.isColorImage()) || ((c == 1) && (doRed)) ||
                ...
                if (subsample) {
                  ...
                  pixelIncrement = Math.max(1,
                  (int) Math.pow((double)(histogramAvailable / 5000),1.0/3.0));
                } // if (subsample)
              ...
              if ((!srcImage.isColorImage()) || ((c == 1) && (doRed)) ||
                ...
                for (y = 0; y < yDim; y++) {
                  ...
                  for (x = 0; x < xDim; x++) {
                    ...
                    i = j + x;
                    if ( (!backgroundPresent) || (objectBuffer[i])) {
                      sm = smy + m1 * (x - mc1) / md1 + m3 * (xy - mc3) / md3
                          + m4 * (xx - mc4) / md4 +
                          m6 * (xx * x - mc6) / md6 +
                          m7 * (xx * y - mc7) / md7 +
                          m8 * (xy * y - mc8) / md8;
    ...
    private void run3Dm3() {
      ...
      double p[] = new double[nParams];
      double xi[][] = new double[nParams][nParams];
      double m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, m13, m14,
             m15, m16, m17, m18, m19;
      int x;
      int y;
      int z;
    ...
    private double entropyFunction(double p[]) {
          ...
          for (y = 0; y < yDim; y += pixelIncrement) {
            ...
            for (x = 0; x < xDim; x += pixelIncrement) {
              ...
              if ( ( (!backgroundPresent) || (objectBuffer[i])) && (validBuffer[i])) {
                sa = say + a1 * (x - ac1) / ad1 + a3 * (xy - ac3) / ad3
                    + a4 * (xx - ac4) / ad4;
                if (sa < -maximum / 2.0) {
                  sa = -maximum / 2.0;
                }
      ...
      blurredHistogram[0] = 1.5*(3.0*originalHistogram[0] +
        2.0*originalHistogram[1] + originalHistogram[2]);
      blurredHistogram[1] = (9.0/8.0)*(2.0*originalHistogram[0] +
        3.0*originalHistogram[1] + 2.0*originalHistogram[2] +
        originalHistogram[3]);
      for (i = 2; i < histogramLength - 2; i++) {
        blurredHistogram[i] = originalHistogram[i-2] +
        2.0*originalHistogram[i-1] + 3.0*originalHistogram[i] +
        2.0*originalHistogram[i+1] + originalHistogram[i+2];
      }
    ...
    // Minimization of entropy function of nParams variables.  Input consists of an
    // initial starting point p[0...nParams-1];
    // an initial matrix xi[0...nParams-1][0...nParams-1],
    // whose columns contain the initial set of directions (usually the nParams unit
    // vectors); and ftol, the fractional tolerance in the function value such
    // that failure to decrease by more than this amount on one iteration
    // signals doneness.  On output, p is set to the best point found, xi is
    // the then-current direction set, fret is the returned function value at
    // p, and iter is the number of iterations taken.
    ...
    // Given an nParams-dimensional point p[0...nParams-1] and an nParams-
    // dimensional direction xi[0...nParams-1], moves and resets p to where
    // entropyFunction(p) takes on a minimum along the direction xi from p,
    // and replaces xi by the actual vector displacement that p was moved.
    // Also returns as fret the value of entropyFunction at the returned located
    // p.  This is actually all accomplished by calling the routines mnbrak
    // and brent.
    ...
    // Given entropyFunction, and given distinct intial points ax and bx, this
    // routine searches in the downhill direction (defined by entropyFunction
    // as evaluated at the initial points) and returns new points ax, bx, cx
    // that bracket a minimum of entropyFunction.  Also returned are the
    // entropyFunction values at the three points, fa, fb, and fc.
    private void mnbrak() {
    ...
    // Given entropyFunction, and given a bracketing triplet of abscissas
    // ax, bx, cx (such that bx is between ax and cx, and entropyFunction(bx)
    // is less than both the entropyFunction(ax) and entropyFunction(cx)),
    // this routine isolates the minimum to a fractional precision of about
    // tol using Brent's method.  The abscissa of the minimum is returned as
    // xmin, and the minimum function value is returned as brent, the
