...
/**
 *    Calculates the histogram for an image. The number of bins
 *    is determined by the extents of the histogram model. The calling
 *    algorithm can specify if the histogram is to be calculated for
 *    whole image or only over VOI regions. The Otsu and Maximum Entrophy
 *    thresholds are also automatically calculated and stored in the
 *    ModelHistogram structure. The Otsu and Max entropy can take time (minutes) to
 *    calculate for histgrams with bins larger than (10,000).
 *
...
public class AlgorithmHistogram extends AlgorithmBase {
    ...
    /** Indicates which channel of the RGB image the histogram should be calculated.
     *  1 = Red channel
     *  2 = Green channel
     *  3 = Blue channel
     */
    ...
    /** Used to indicate the number of histogram bins when using the constructors that do NOT pass in the
     *  histogram object.
     */
     ...
     *   @param maskFlag   Flag that indicates that the histogram will be
    ...
    /**
     *   Sets a flag to indicate if the histogram is to be
     *   calculated for the entire image or just the VOI.
    ...
    public void runData(int bins) {
                ...
                if ( image.isColorImage() ) {
                    if ( RGBOffset == 1 ) {
                        for ( i = 0; i < bins; i++ ) {
                            if ( histoBuffer[i] >= 1 ) {
                                if ( lowValue[i] == highValue[i] ) {
                                    UI.setDataText(
                                            image.getImageName() + " red intensity = " + lowValue[i] + "\t\t count = "
                                            + histoBuffer[i] + "\t area = " + pixelSize * histoBuffer[i] + " square "
                                            + unitsStr + "\n" );
                                } // if (lowValue[i] == highValue[i])
                                else { // lowValue[i] != highValue[i]
                                    UI.setDataText(
                                            image.getImageName() + " red intensities = " + lowValue[i] + " to "
                        ...
                        for ( i = 0; i < bins; i++ ) {
                            if ( histoBuffer[i] >= 1 ) {
                                if ( lowValue[i] == highValue[i] ) {
                                    UI.setDataText(
                                            image.getImageName() + " green intensity = " + lowValue[i] + "\t\t count = "
                                            + histoBuffer[i] + "\t area = " + pixelSize * histoBuffer[i] + " square "
                                            + unitsStr + "\n" );
                                } // if (lowValue[i] == highValue[i])
                                else { // lowValue[i] != highValue[i]
                                    UI.setDataText(
                                            image.getImageName() + " green intensities = " + lowValue[i] + " to "
                                            + highValue[i] + "\t count = " + histoBuffer[i] + "\t area = "
                                            + pixelSize * histoBuffer[i] + " square " + unitsStr + "\n" );
                                } // lowValue[i] != highValue[i]
                            } // if (histoBuffer[i] >= 1)
                        ...
                        for ( i = 0; i < bins; i++ ) {
                            if ( histoBuffer[i] >= 1 ) {
                                if ( lowValue[i] == highValue[i] ) {
                                    UI.setDataText(
                                            image.getImageName() + " blue intensity = " + lowValue[i] + "\t\t count = "
                                            + histoBuffer[i] + "\t area = " + pixelSize * histoBuffer[i] + " square "
                                            + unitsStr + "\n" );
                                } // if (lowValue[i] == highValue[i])
                                else { // lowValue[i] != highValue[i]
                                    UI.setDataText(
                                            image.getImageName() + " blue intensities = " + lowValue[i] + " to "
                                            + highValue[i] + "\t count = " + histoBuffer[i] + "\t area = "
                                            + pixelSize * histoBuffer[i] + " square " + unitsStr + "\n" );
                                } // lowValue[i] != highValue[i]
                            } // if (histoBuffer[i] >= 1)
                    ...
                    for ( i = 0; i < bins; i++ ) {
                        if ( histoBuffer[i] >= 1 ) {
                            if ( lowValue[i] == highValue[i] ) {
                                UI.setDataText(
                                        image.getImageName() + " intensity = " + lowValue[i] + "\t\t count = "
                                        + histoBuffer[i] + "\t area = " + pixelSize * histoBuffer[i] + " square "
                                        + unitsStr + "\n" );
                            } // if (lowValue[i] == highValue[i])
                            else { // lowValue[i] != highValue[i]
                                UI.setDataText(
                                        image.getImageName() + " intensities = " + lowValue[i] + " to " + highValue[i]
                                        + "\t count = " + histoBuffer[i] + "\t area = " + pixelSize * histoBuffer[i]
                                        + " square " + unitsStr + "\n" );
                            } // lowValue[i] != highValue[i]
                        } // if (histoBuffer[i] >= 1)
                    } // for (i = 0; i < bins; i++)
                        ...
                        for ( i = 0; i < bins; i++ ) {
                            if ( histoBuffer[i] >= 1 ) {
                                if ( lowValue[i] == highValue[i] ) {
                                    UI.setDataText(
                                            image.getImageName() + " red intensity = " + lowValue[i] + "\t\t count = "
                                            + histoBuffer[i] + "\t volume = " + pixelSize * histoBuffer[i] + " cubic "
                                            + unitsStr + "\n" );
                                } // if (lowValue[i] == highValue[i])
                                else { // lowValue[i] != highValue[i]
                                    UI.setDataText(
                    ...
                    for ( i = 0; i < bins; i++ ) {
                        if ( histoBuffer[i] >= 1 ) {
                            if ( lowValue[i] == highValue[i] ) {
                                ...
                                UI.setDataText(
                                        image.getImageName() + " intensities = " + lowValue[i] + " to " + highValue[i]
                                        + "\t count = " + histoBuffer[i] + "\t volume = " + pixelSize * histoBuffer[i]
                                        + " cubic " + unitsStr + "\n" );
                            } // lowValue[i] != highValue[i]
                        } // if (histoBuffer[i] >= 1)
                    } // for (i = 0; i < bins; i++)
        ...
        // in this algorithm, we must clear the paint mask, since it was generated during generateVOIMask in the constructor
        // In the future, we might consider using a separate paint mask so that the image's paint mask can remain even
     ...
     * This method is very similar to Otsu's method. Rather than maximising the inter-class variance, it maximizes the
     * inter-class entropy. Entropy is a measure of the uncertainity of an event taking place. You can calculate it
     * as: S = -(sum)p*log2(p) so it is very straightforward to do using the histogram data. (p is the probability of
     * a pixel greyscale value in the image, and (sum) is the greek capital sigma. It is customary to use log in base 2.
     * P.K. Sahoo, S. Soltani, K.C. Wong and, Y.C. Chen "A Survey of Thresholding Techniques", Computer Vision,
