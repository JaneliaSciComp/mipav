...
public class AlgorithmRGBConcat extends AlgorithmBase {
    ...
    /**
     *   Destination image where results are to be stored. Must be
     *   of type RGB.
     */
    private ModelImage destImage;

    /**
     *   Flag indicating whether of not to remap the data. If true and
     *   srcImage data max is < 255 data will be remapped [0-255] else if
     *   image max > 255 data will automatically be remapped [0-255].
     */
     ...
     *   @param remap     if true and srcImage data max is < 255 data will be remapped [0-255] else if
     *                    image max > 255 data will automatically be remapped [0-255].
     */
    public AlgorithmRGBConcat( ModelImage srcImgR, ModelImage srcImgG, ModelImage srcImgB,
            ModelImage destImg, boolean remap ) {
    ...
    /**
     *   Concatinate the image
     *   Must run getImageR after running this routine
     */
    private void calcStoreInPlace() {
        ...
        if ( reMap == true ) {
            for ( i = 0, id = 0; i < totLength && !threadStopped; i += 4, id++ ) {
                if ( i % mod == 0 && isProgressBarVisible() ) {
                    progressBar.updateValue( Math.round( (float) ( i) / ( totLength - 1 ) * 100 ),
                            activeImage );
                }
