...
/**
 *   algorithm to apply an adaptive histogram to an image,
 *   placing it in a new ModelImage, or returning the
 *   changed picture to the same image.
 *   <p>
 *   This is an algorithm to apply within a neighborhood, that
 *   is it selectes a region of adjacent pixels, the number of
 *   adjacent pixels is of a user-selected size, and forms
 *   the histogram from that region.  The region selection
 *   routine is adopted from AlgorithmMedian.  The value of
 *   the cell (the brightness of the pixel at a particular
 *   location) is then mapped to the region.  Over the entire
 *   image, this means that all pixels will still be brighter
 *   or darker than other pixels, but the brightness has been
 *   shifted making minor variations more apparent.  The
 *   cumulative distribution is changed so that it becomes
 *   step-wise linear, making the darker pixels use the lowest
 *   possible values availble to the image, the brightest use
 *   the highest possible values.
 *   <p>
 *   This algorithm may be used to perform a contrast-limited or 'clamped'
 *   histogram equalization.  It is set by supplying a percentage of the
 *   maximum number of pixels in any particular brightness level.
 *   As the regional histograms are tabulated, the brightness with the
 *   maximum number of pixels attributed to it is remembered.
 *   The algorithm will then evenly redistibute the total number
 *   of pixels from any brightness which has a greater number
 *   than the max times this fraction to all other (less populous)
 *   brightnesses.  (If the supplied percentage is 80, for instance,
 *   the maximum number of pixels to any brightness level, will be
 *   four-fifths the largest number number of pixels of any shade
 *   found.
 *   <p>
 *   The principle methodology of this algorithm, is to take each
 *   window, form a histogram; clamp so there is no brightness
 *   which can have more than a certain number of pixels; scale
 *   the brightnesses so that the values are spread evenly
 *   from the maximum to the minimum brightness.
 *   (ie., scale * histogram(i)).
 *   <p>
 *   <ul><li>Primary source: Stark, J Alex. <u>Adaptive Image Contrast
 *       Enhancement Using Generalizations of Histogram Equalization</u>.</li>
 *       <li>Background Information:
 *       <ul><li>Russ, John. <u>Image Processing HandbookM</u>.</li>
 *           <li>Tidestav, Claes. <u>Short Introduction to Adaptive Equalization</u>.</li>
 *           <li>Rabie, Tamer; Rangayan, Rangaraj; Paranjape, Raman.
 *               <u>Adaptive-Neighborhood Image Deblurring</u>.</li>
 *       </ul></li>
 *   </ul>
 *
...
public class AlgorithmAHElocal extends AlgorithmBase {
    ...
    /** a pixel (no matter whether represented as a float or int), has a brightnessLevel,
    ...
    /**
     *   RGB images are histogram equalized by 'channel.'  That is, each
     *   color, red, blue and green, is run independently of the other two
     *   colors.  This permits selectively filtering any combination
     *   of the three channels instead of simply trying to handle all three
     *   at once.  True filters that channel.
     */
    ...
    /**
     *   Redistribute from brightnesses with at a least a specified
     *   percentage of the brightness with the maximum number of pixels
     *   @param beClamped
     */
    ...
    /**
     *   The Clip Level is a percentage of the maximum number of
     *   pixels in any particular brightness level.  This method allows
     *   one to set that percentage.  As the regional histograms are
     *   tabulated, the brightness with the maximum number of pixels
     *   attributed to it is remembered.  The algorithm will then evenly
     *   redistibute the total number of pixels from any brightness
     *   which has a greater number than the max times this fraction
     *   to all other (less populous) brightnesses.
     *   @param sectorPercentage
     */
    ...
    /**
     *   If useThreshold true only copy when both the source
     *   pixel and its local neighborhoold are are less than
     *   minThreshold
     *   @param useThreshold
     */
    ...
    /** sets minimum threshold values.  Pixels
     *   are copied over when both the source pixel
     *   and the local neighborhood surrounding
     *   the pixel are all less than the given minimum
     *   pixel value.
     */
    ...
    /** permit the user to select the scale on which the algorithm
     *   forms the brightness pattern of the histogram.  The default
     *   is scaleOnSlice.
     *   <p>
     *   The options are:<ul>
    ...
    protected void constructLog() {
        ...
        historyString = new String(
                "NeighborhoodAdaptiveHistogramEqualization(" + String.valueOf( kernelSize ) + ", "
    ...
    /**
     *   Histogram Equalization on the source image and replaces the
     *   source image
                            ...
                            try {
                                if ( pBarVisible ) {
                                    progressBar.updateValue(
                                            (int) ( ( (float) (Math.max(0,colorUsed-1)*numberOfSlices + i ) / (numColors * numberOfSlices) ) * 100 ),
                                            activeImage );
                                    if (color == 1 && rChannel) {
    ...
    /** This function produces a new volume image that has been
     *   histogram equalized. The Image is changed by filtering
     ...
     *   Any color image may be processed in this so long as each
     *   color plane is separate and provided one at a time.
     *   This means, extract <tt>[aRGB aRGB ...] </tt>buffer into
     *   3 seperate buffers <tt>[RRRRRRRRRRR...] , [GGGGGG....] and [BBBBBBBB]</tt>
     *   and feed each into the sliceFilter one-at-a-time.
     *   <p>
     *   The benefit is one sliceFilter for all image types;
     *   just break up each RGB image into these seperate monochrome images
     *   sliceFilter and then reassemble int aRGB.
     *   <p>
     *   Only length of one slice (image.getExtents[0] * image.getExtents[1])
     *   Will be copied at a time.
     *   Note that a progressBar must be created first.
    ...
    protected final void monoSliceEqualizer( float[] srcBuffer,
        ...
        /* if (pBarVisible) {
         ...
         for ( i = 0; i < imageSliceLength; i++){
         if (numberOfSlices == 1) {   // 2D image     update progressBar
         if (i%mod == 0 && pBarVisible == true) {
         progressBar.updateValue(Math.round
         ( (  (float)( (pass*imageSliceLength)+i)/(iterations*(imageSliceLength-1))*100)) );
         }
         }
         */
    }

    /** process pixel at pixel <i>pix</i> using data
     *   from the <i>srcBuffer</i>.  builds a list of
     *   monochromatic image values of the neighboring
     *   pixels image values (using a map generated by
     *   the kernelMask), builds the histogram of those
     *   values, clamps when necessary, then scales the
     *   cumulative histogram, getting the brightness
     *   back out.
     *   <p>
     *   This is the entire algorithm on one pixel.
     *   @param pix  the pixel to process; generates
     *               the histogram from the neighbors
     *               around this image-data element.
     *   @param srcBuffer  all the monochromatic image
     *                     data -- which means that
     *                     4-color image data must be
     *                     seperated out into monochrome
     *                     color slices for this method.
     */
    protected final float procPixel( int pix, float[] srcBuffer ) {
        ...
        scale = (float) ( maxScale ) / ( kernel.length ); // largest scale value over the number of positions to fill (compare to makeScaled(...) after makeCumulative(..)
    ...
    /** make histogram
     *   uses the class-global vars <code>kernel</code> and <code>histogram</code>.
    ...
    protected int buildHistogram() {
        ...
        idealWidth = (float) ( 2.0f
                * ( sortBuffer[( 3 * kernel.length / 4 ) - 1] - sortBuffer[( kernel.length / 4 ) - 1] )
        ...
        for ( i = 0; i < kernel.length; i++ ) {
            try {
                ...
                Preferences.debug(
                        "kernel: " + kernel[i] + "; imageOffset: " + imageOffset + "; unitSize: " + unitSize + "\n" );
    ...
    /** CUMULATION   function
     *   converts the PDF of the pixels in the image
     *   to a CDF. -- deprecated
     */
    ...
    /** scale to max height of the histogram
     *   deprecated
     */
    ...
    /**
     *   Copies all values from the srcbuffer to the destination buffer.
     *   the format is intended to envision the organisation of
     *   dest[i] := src[i]
     *   float dest  destination array
     *   float src   source array
     */
    ...
    /** gets the bottom end of the image.  for any image but color images, the
     *   bottom end is defined as the buffer minimum
     *   for all images except color images; there the
     *   offset is defined to be zero
     *   @return offset
     */
    ...
    /**
     *   after generating a histogram, <code>clip</code> will reduce
     *   the number of pixels attributed to that brightness level.
     *   <p>
     *   The idea is to limit any brightness level in the histogram to
     *   a set value; the number of pixels that are above the limit
     *   are tallied.  The total number of pixels that were found above
     *   the limit, is then spread evenly among the brightnesses that
     *   are below limit.
     *   @param histogram        array where the brightness values of all pixels in the
     *                           image have been counted.
     *   @param clipLimit        no brightness may have any more than this number of pixels.
     *                           Obviously, this method will not achieve anthing when
     *                           <code>clipLimit</code> is as large or larger than the
     *                           largest brightness in <code>histogram</code>.
     */
    ...
    /**
     *	Makes the kernel.  Creates out of memory,
    ...
    /**
     *   Compiles a list of the values neighboring the
     *   desired pixel, that are defined in the kernel.
     *   <p>
     *   The Neighbor list reports the monochromatic brightness values.
     *   returned in the neighbor list's kernel.
     *   @param  i       The central pixel to find neighbors for.
     *   @param  data    Image data.  The data is to be
     *                   arranged always as a monochromatic, 2d set.
     *                   Returned in kernel:
     *            		The neighboring pixel list
     */
            ...
            for ( col = -halfK; col <= halfK; col++ ) { // go through all columns
                if ( kernelMask[maskCenter + col + row * kernelSize] != 0 ) {
                    ...
                    try {
                        ...
                        Preferences.debug(
                                "kernlength: " + kernel.length + ";, count: " + count + "; " + "in dataset: "
                                + Integer.toString( pixelLocation ) + "; datalen: " + data.length + "\n" );
                        Preferences.debug( "\n" );
                    }
                }
            }
    ...
    /** gets the image max based on 1st) whether or not the image is color.
     *   2) the color.
     *
     */
