...
protected void finalize() throws Throwable {
	...
	byteArray2   = null;
	byteArray4   = null;
	if (incomingBuffers != null ) incomingBuffers.removeAllElements();
	if (outgoingBuffers != null ) outgoingBuffers.removeAllElements();
...
/*************************************************************************************
*
*   The next two methods must be overloaded. As an option
*       1. make next two methods abstract which requires this class to be abstract.
*          The problems with this involves DICOMCommsLink which must instantiate a
*          a DICOM_Comms and you can't instantiate an abstract class. Therefore,
*          one must try to redesign DICOMCommsLink and others to remove this
*          dependancy. I think this is a good idea since DICOMCommsLink extends
*          DICOMComms and has a DICOMComms - seems a bit odd but it works!.
*
**************************************************************************************/
...
/*************************************************************************************
*
*   The next 4 static methods facilitate the conversion 16 bit and 32 bit values into
*   a byte buffer for I/O (file and socket).
*
**************************************************************************************/
 ...
 */
public final static int bufferToInt16(byte[] buffer, int index, int endianess) {
 ...
 */
public final static int bufferToInt32( byte[] buffer, int index, int endianess ) {
...
/**
 * Copies a 16 bit (2 bytes) integer into the specified portion of
 *  the byte array of proper endianess.
 * @param buffer    destination byte array
 * @param index     index into buffer
 * @param value     16 bit integer value
 * @param endianess endianess
 */
public final static void int16ToBuffer( byte[] buffer, int index, int value, int endianess ) {
    if( endianess == BIG_ENDIAN ) {
...
/**
 * Copies a 32 bit (4 bytes) integer into the specified portion of
 *                 the byte array of proper endianess.
 * @param buffer    destination byte array
 * @param index     index into buffer
 * @param value     16 bit integer value
 * @param endianess endianess
 */
public final static void int32ToBuffer( byte[] buffer, int index, int value, int endianess ) {
    if( endianess == BIG_ENDIAN ) {
...
/**
*   Simple method that take a byte array and call read with
*          two parameters: byte array and array length
...
/**
*   Reads in a nBytes and puts them in a ByteBuffer and puts the byte buffer
...
public void seekToEndOfGroupTwoTags() {
    ...
    if (dataBuffer.data[128] == 0x44 && dataBuffer.data[129] == 0x49 &&
        dataBuffer.data[130] == 0x43 && dataBuffer.data[131] == 0x4D) {
...
public int peekForEndOfSequence() throws DICOM_Exception {
    ...
    int seqLength = 0;   /* if at asome point we decide we don't want to both
                           * reading more buffers we can return -1
                           * or we can throw an Exception (which mightbe better)
                           * (think IOException, or maybe make a new
                           * one, BufferReadException might be nice name.
                           */
    ...
    do {
        ...
        if ( (bufferToInt16(groupWord, 0, inEndianess) == groupEndSq) &&
            ...
            (bufferToInt16(zeroWord, 0, inEndianess) == 0) &&
            (bufferToInt16(zeroWord, 2, inEndianess) == 0)) {
            if (debugFlag)
                Preferences.debug("\nDICOM_COMMS: Data match (" +
                                  Integer.toString(groupEndSq, 0x10) + "," +
            ...
            if ( (dataIndex + 8) <= dataBuffer.data.length) {
                ...
                if (incomingBuffers.size() <= bufferIndex) return -1;  // nothing more to read so FAIL!
                ...
                if (dataBuffer.data.length - dataIndex == 7) {
                     ...
                     * pointers.  All other cases must return pointers into
                     * the almost finished data buffer.
                     *
                     * we can now be sure that all the following words
                     * will be searched from the following buffer.
                     */
                    seqLength += (dataIndex - dataBuffer.startIndex);
...
public void read( byte[] data, int nBytes ) throws DICOM_Exception {
    ...
    while( inBuffersLength < nBytes ) {  readBlock(0);  } // need more data
...
/**
*   This is the method that actually reads in the data
*   from the socket or IO stream.
*   @param useBreakSize the recommended size of the read buffer
*/
private void readBlock( int useBreakSize ) throws DICOM_Exception {
    int        length, iLength;

    if( useBreakSize == 0 ) {  useBreakSize = breakSize; }

    if( useBreakSize < MAX_READ_LENGTH ) iLength = useBreakSize;
    else                                 iLength = MAX_READ_LENGTH;
...
*   @param value the value to be sent. The value is put into a 2 byte
*               "byte" buffer in the order indicated by the specified
*                endianess of this class.
*/
...
*   @param value the value to be sent. The value is put into a 4 byte
*               "byte" buffer in the order indicated by the specified
*                endianess of this class.
*/
...
/**
*   Sends all byte buffers of the outgoing buffer vector out the port.
*   see DICOM PDUService.sendBinary and DICOMSocket
*/
...
/**
*   Sends specified number of bytes out the port.
*   see DICOM PDUService.sendBinary and DICOMSocket
    ...
    if( nBytes > outBuffersLength) {
        ...
        while( nBytes > 0 ) {
            ...
            if( availableBytes > nBytes ) {
                ...
                System.arraycopy(byteBuffer.data, byteBuffer.startIndex + nBytes, byteBuffer.data,
                                 0, availableBytes - nBytes);
                byteBuffer.startIndex = 0;
...
/**
*   Writes a specified number of bytes from the byte buffer into outGoing ByteBuffer list
*   of byte buffers
...
/**
*   Simple inner class to facilitate the memory allocation of
*   of a byte buffer used in DICOM_Comms to support I/O
*/
