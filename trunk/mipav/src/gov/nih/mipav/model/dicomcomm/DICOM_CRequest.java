...
* Most DIMSE C Service messaging classes only require
* setting an appropriate DICOM command field.
*
*/
 ...
 */
public void read( DICOM_PDUService pdu, DICOM_Object dco, DICOM_Object ddo )
    ...
    if( command != COMMAND ) {
        throw new DICOM_Exception( " DIMSE-C Request error : " +
    ...
    if( dataSetType != DICOM_Constants.DSTYPE_NODATAPRESENT ) {
        if( ddo == null ) { throw new DICOM_Exception( "DICOM data object is null" ); }
        if( pdu == null ) { throw new DICOM_Exception( "PDU is null" ); }
        pdu.readInObject( ddo );
    }
    else {
        if( ddo != null ) { ddo.clear(); }  // Information is not present
    }
...
public void readCResponseAlias( DICOM_PDUService pdu, DICOM_Object dco, DICOM_Object ddo) throws DICOM_Exception {
    ...
    if( dco == null ) { throw new DICOM_Exception( "null dco" ); }
    ...
    if( dstype != DICOM_Constants.DSTYPE_NODATAPRESENT ) {
        ...
        if( ddo == null ) { throw new DICOM_Exception( "null ddo" ); }
        if( pdu == null ) { throw new DICOM_Exception( "null pdu" ); }

        pdu.readInObject( ddo );
    }
    else {
        if( ddo != null ) { ddo.clear(); }
    }
    ...
    if( status >= DICOM_Constants.STATUS_ERRORFIRST &&
        ...
        if( errorComment != null ) { str1 = str1 + errorComment + "\n"; }
        throw new DICOM_Exception( str1 );
    }
}
...
public void write ( DICOM_PDUService pdu, String  classUID, String instanceUID, DICOM_Object ddo, byte[] AETitle)
    ...
    if( ddo != null ) { dcor.setInt16( DICOM_RTC.DD_DataSetType, DICOM_Constants.DSTYPE_DATAPRESENT   );}
    else              { dcor.setInt16( DICOM_RTC.DD_DataSetType, DICOM_Constants.DSTYPE_NODATAPRESENT );}

    if( COMMAND == DICOM_Constants.COMMAND_CMoveRQ ) {
        if( AETitle == null ) { throw new DICOM_Exception( "AETitle = Null" ); }
 ...
 *  @param str
 */
public final static String fixMoveDestString( String str ) {
...
*/
public final void setMsgID(int msgID) {  MSG_ID = msgID;  }

/**
*   Accessor to the the message ID
*   @return this objects message ID
*/
public final int getMsgID() {  return MSG_ID;  }
