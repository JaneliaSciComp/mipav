...
/**
 * The PDU_Service class is a wrapper of all the lower level DICOM messaging classes.
 * This class is the true implementor of the DICOM networking protocol.
 *
 */
...
*/
public DICOM_AAssociateRQ getAAssociateRQ() { return associateRQ;}

/**
*   Sets the display viewer
*   @param displayer
*/
public void setDICOMMessageDisplayer( DICOMDisplayer displayer ) { dicomMessageDisplayer = displayer; }
...
*/
public DICOMDisplayer getDICOMMessageDisplayer()  { return dicomMessageDisplayer; }
    ...
    if( !interrogateAAssociateRQ() && socket != null) {
        ...
        if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
            Preferences.debug(DICOM_Util.timeStamper() + " PDU_Service.handleConnectionFromServer:(" + this.hashCode() +
                              ") MIPAV rejected association from " + new String(associateRQ.getCallingAppTitle()) +
                              "\n");
        throw new DICOM_Exception( " MIPAV rejected association from " + new String(associateRQ.getCallingAppTitle()) );
    }
...
/**
 * This is a simple interface to establish an association over
 * a socket from the client (i.e. MIPAV) (SCU) side.
 *
...
public void connectClientToServer( String remoteAppTitle, boolean verification ) throws DICOM_Exception {
    ...
    if (Preferences.debugLevel(Preferences.DEBUG_COMMS) ) {
        ...
        Preferences.debug(DICOM_Util.timeStamper() + "AE Title of server = " + remoteAppTitle +
                          "  IP = " + Preferences.getIP(remoteAppTitle) +
                          "  PORT = " + Preferences.getPort(remoteAppTitle) + "\n");
    }
    ...
    switch( itemType ) {
            ...
            if( !interrogateAAssociateAC() ) {
                ...
                if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                    Preferences.debug(DICOM_Util.timeStamper() + " PDU_Service.connectClientToServer: (" +
                                      this.hashCode() +
                                      ") Association to " + remoteAppTitle + " accepted, but rejected locally.\n");
                throw new DICOM_Exception( "Association to " + remoteAppTitle + " accepted, but rejected locally.\n");
            }
            break;
        case DICOM_PDUTypeBase.PDUTYPE_AAssociateRJ:
            ...
            if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                Preferences.debug(DICOM_Util.timeStamper() + " PDU_Service.connectClientToServer:(" + this.hashCode() +
                                  ") Association to " + remoteAppTitle + " was rejected.\n");
...
/**
 *  The main method for reading incoming DICOM messages.
 *  true:  complete PData unit received, socket still open
 *  false: release request received, then close the socket
 *  @param DICOMobj
 *  @return true    complete PData unit received, socket still open
 *          false   release request received, then close the socket
 */
...
private boolean interrogateAAssociateRQ() throws DICOM_Exception {
    ...
    for( int i = 0; i < arrayPresContexts.size(); i++) {
        ...
        for( int j = 0; j < presContext.trnSyntax.size(); j++) {
            ...
            if( !shouldWeAcceptAbstractSyntax( presContext.absSyntax ) ){
                if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                    Preferences.debug( DICOM_Util.timeStamper() + " PDU_Service.interrogateAAssociateRQ: presentation context NOT accepted = "
                                        + presContext.absSyntax.getUID() + " " + DICOM_Constants.convertUIDToString(presContext.absSyntax.getUID()) +
                                        " presentation context ID = " + pca.presentationContextID + "\n");
                pca.result = 3;                            // 3 = abstract syntaxes not supported by us
            }
            else {
                if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                    Preferences.debug( DICOM_Util.timeStamper() + " PDU_Service.interogateAAssociateRQ: presentation accepted = "
    ...
    if( !atLeastOne ) {
        if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
            Preferences.debug( DICOM_Util.timeStamper() +
              " PDU_Service.interrogateAAssociateRQ:(" + this.hashCode()  +
...
private boolean interrogateAAssociateAC() {
    ...
    for(int i = 0; i < associateAC.getPresentationContextes().size(); i++) {
        ...
        if( pca.result == 0 ) {
            ...
            if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                Preferences.debug( DICOM_Util.timeStamper() + " PDU_Service.interrogateAAssociateAC - ACCEPTED : ID =  " +
...
*   @return The version.
*/
public String getImplementationVersion() { return( "NIH MIPAV 05 1.1" ); }
...
*   @return true if the remoteAETitle has been found in the server host table (host table in the
...
/**
*   Parses the server info string and returns  a tokenized array of strings that
*   where deliniated by ";" (See MIPAV preference file)
*   @param info
*   @return server info
*/
