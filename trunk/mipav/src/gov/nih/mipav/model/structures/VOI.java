...
public class VOI extends ModelSerialCloneable {
    ...
    public void importCurve(float x[], float y[], float z[], int slice) {
        ...
        if (curveType == POINT || curveType == POLYLINE_SLICE) {
            ...
            if (curveType == POLYLINE_SLICE) {
                try {
                    activePolylineSlicePoint = Integer.parseInt(((VOIPoint) (
                            curves[slice].lastElement())).getLabel()) - 2;
                } catch (Exception e) {
    ...
    public void markPSlicePt(int slice) {
        if (curveType == POLYLINE_SLICE) {
            ...
            try {
                activePolylineSlicePoint = Integer.parseInt(((VOIPoint) (curves[
                        slice].elementAt(polygonIndex))).getLabel()) - 1;
            } catch (Exception e) {
                activePolylineSlicePoint = 0;
            }
        }
    ...
    public void calcPLineDistances(FileInfoBase fileInfo) {
        ...
        for (i = 0; i < distanceVector.size() - 1; i++) {
            ...
            secondPoint = ((PolyPointHolder) distanceVector.elementAt(i + 1)).
                          getPoint();
            distance3D = MipavMath.distance(firstPoint.x, firstPoint.y, firstPoint.z,
                                             secondPoint.x, secondPoint.y,
                                             secondPoint.z,
                                             resols);
            distance2D = MipavMath.distance(firstPoint.x, firstPoint.y, 0,
        ...
        displacement2D = MipavMath.distance(firstPoint.x, firstPoint.y, 0,
                                            secondPoint.x, secondPoint.y, 0,
                                            resols);
        displacement3D = MipavMath.distance(firstPoint.x, firstPoint.y, firstPoint.z,
                                            secondPoint.x, secondPoint.y,
                                            secondPoint.z,
                                            resols);
        //round to nearest .01
    ...
    public void moveVOI(int slice, int xDim, int yDim, int zDim, int xM, int yM, int zM) {
        ...
        if (curveType != POINT
            ...
            if (!doWhole) {
                if (pLineSliceIndex < curves[slice].size() &&
                  pLineSliceIndex >= 0) {
