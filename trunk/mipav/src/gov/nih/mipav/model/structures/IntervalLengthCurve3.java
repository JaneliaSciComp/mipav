...
/**
 * This is an abstract interface for a curve in 3D. This interface builds upon that of the Curve3 abstract interface by
 * adding support for computing arc lengths over subintervals of the entire curve. This capability can be used to speed
 * up the reparameterization of the curve for the time value as a function of arc length.
 */
public abstract class IntervalLengthCurve3 extends Curve3 {
    ...
    /**
     * Create a generalized curve over the specified time interval and having the specified number of intervals for
     * computing arc length. Note that the arc lengths cannot be computed until the derived class is fully constructed.
     * This is done by requiring that derived classes call the initializeIntervalLengths method within the body of their
     * constructor. It is at this time that the time values associated with the intervals is specified.
     *
     * @param  afTime  float[] Array of input time values associated with the intervals. These times values must be
     *                 increasing and must be within the getMinTime() and getMaxTime() range.
     */
    ...
    /**
     * Return the accumulated length of the curve for all intervals up to and including the specified interval.
     *
    ...
    public float getIntervalAccumLength(int iInterval) {
        ...
        if (null == m_afAccumLength) {
            ...
            for (int i = 1; i < m_iNumLengthIntervals; i++) {
                m_afAccumLength[i] = m_afAccumLength[i - 1] + getIntervalLength(i);
            }
        }
    ...
    public float getLength(float fT0, float fT1) {
        ...
        if (iInterval0 < iInterval1) {
            ...
            fLength += getIntervalLength(iInterval0, fDt0,
    ...
    public float getTime(float fLength, int iIterations, float fLengthTolerance) {
        ...
        while ((iInterval < m_iNumLengthIntervals) && (fLength >= getIntervalAccumLength(iInterval))) {
            iInterval++;
        }
        ...
        return getTime(m_afIntervalTime[iInterval], m_afIntervalTime[iInterval + 1], fLength, iIterations,
                       fLengthTolerance);
    }
