...
{
    /**
     * Stores the indexes into a linear array for the samples to use
     * in computing finite difference based gradients as follows:
     *
     * grad.x = (array[m_iX1] - array[m_iX0]) / m_fDeltaX;
     * grad.y = (array[m_iY1] - array[m_iY0]) / m_fDeltaY;
     * grad.z = (array[m_iZ1] - array[m_iZ0]) / m_fDeltaZ;
     */
    static public class GradientNeighbors {
        // Indexes into linear array for X-axis finite difference computed
        // by array[m_iX1] - array[m_iX0];
        public int m_iX0;
        public int m_iX1;

        // Indexes into linear array for Y-axis finite difference computed
        // by array[m_iY1] - array[m_iY0];
        public int m_iY0;
        public int m_iY1;

        // Indexes into linear array for Z-axis finite difference computed
        // by array[m_iZ1] - array[m_iZ0];
        public int m_iZ0;
        public int m_iZ1;

        // Scale factors to use for normalizing the finite differences
        // by axis.
    ...
    // Array of exact Euclidean distances (in sample coordinates) to each of
    // the 26-connected neighbors corresponding to the relative indexes in
    // m_aiOffsetNeighbors27 array.
    private final float[] m_afDistanceNeighbors27;

    // Array of exact Euclidean distances to each of the 26-connected
    // neighbors scaled by the size of each sample along each axis and
    // corresponding to the relative indexes in m_aiOffsetNeighbors27 array.
     ...
     * @param fSpacingX spacing between samples along the X axis.
     * Value can be negative to indicated a reverse direction for real coords.
     * @param fSpacingY spacing between samples along the Y axis
     * Value can be negative to indicated a reverse direction for real coords.
     * @param fSpacingZ spacing between samples along the Z axis
     * Value can be negative to indicated a reverse direction for real coords.
     ...
     */
    public ModelImage3DLayout(int iDimX, int iDimY, int iDimZ,
                              float fSpacingX, float fSpacingY, float fSpacingZ,
        ...
        for (int iZ = 0; iZ <= 1; ++iZ) {
            for (int iY = 0; iY <= 1; ++iY) {
                for (int iX = 0; iX <= 1; ++iX) {
                    ...
                    m_aiOffsetNeighborsVoxel[iIndexNeighborsVoxel++] =
                        getIndex(iX, iY, iZ);
                }
            }
        }
        ...
        for (int iZ = -1; iZ <= 1; ++iZ) {
            for (int iY = -1; iY <= 1; ++iY) {
                for (int iX = -1; iX <= 1; ++iX) {
                    ...
                    // in this case where normally the getIndex method
                    // would never be called with negative indexes.
                    m_akOffsetXYZNeighbors27[iIndexNeighbors27] =
                        new Point3i(iX, iY, iZ);
                    m_aiOffsetNeighbors27[iIndexNeighbors27] =
                        getIndex(iX, iY, iZ);
                    ...
                    float fDy = (float) iY;
                    float fDz = (float) iZ;
                    m_afDistanceNeighbors27[iIndexNeighbors27] =
                        (float) Math.sqrt(fDx * fDx + fDy * fDy + fDz * fDz);

                    // Compute the real Euclidean distance to the next sample.
                    fDx *= fSpacingX;
                    fDy *= fSpacingY;
                    fDz *= fSpacingZ;
                    m_afRealDistanceNeighbors27[iIndexNeighbors27] =
                        (float) Math.sqrt(fDx * fDx + fDy * fDy + fDz * fDz);
    ...
    /**
     * Get the amount to increment the linear array index for each
     * increment in the x coordinate index.
    ...
    /**
     * Get the amount to increment the linear array index for each
     * increment in the y coordinate index.
     * @return int Positive linear array index increment.
     */
    ...
    /**
     * Get the amount to increment the linear array index for each
     * increment in the z coordinate index.
     * @return int Positive linear array index increment.
     */
    ...
    /**
     * Compute the index in a linear data array dimensioned to store
     * all the samples of this volume.  The index is determined by the
     * specified 3D coordinate indexes.  No checking is made to ensure
     * that the coordinate indexes are in their respective ranges.
     ...
     * Compute the index in a linear data array dimensioned to store
     * all the samples of this volume.  The index is determined by the
     * specified 3D coordinate indexes.  Each coordinate index is checked
     * to ensure that it is within its respective coordinate ranges.
     * @param iX sample X coordinate index
     * @param iY sample Y coordinate index
     * @param iZ sample Z coordinate index
     * @return 1D index into linear array or -1 if any of the indexes
     * are outside their respective coordinate ranges
     */
    public int getIndexChecked(int iX, int iY, int iZ) {
        if ( (iX < 0) || (iY < 0) || (iZ < 0) ||
            (iX >= m_iDimX) || (iY >= m_iDimY) || (iZ >= m_iDimZ)) {
            return -1;
        }
    ...
    /**
     * Retrieve the indexes for the 6-connected neighbors of the specified
     * sample which are used to compute the gradient.  If the sample is
     * along any border of the volume, then the indexes for a forward/reverse
     * finite difference are returned which would involve the sample itself
     * but only for the axis where the sample is along its corresponding
     * border.  Otherwise, the samples are returned for computing finite
     * central difference.
     *
     * Unexpected results occur, e.g. invalid sample indexes, if
     * the center sample specified by the indexes is for one on the border
     * of the volume.
     ...
     * @param kNeighbors contains the indexes for the 6-connected neighbors
     * used to compute the gradient
     */
    public void getIndexNeighborsGradient(int iX, int iY, int iZ,
                                          GradientNeighbors kNeighbors) {
     ...
     * These indexes are for the specified sample and the indexes
     * of the immediate neighbor samples that comprise a voxel.
     * Of the 8 neighboring samples returned for a voxel, the specified
     * sample will have the smallest of the coordinate indexes, i.e.,
     * the "minimum" corner sample.
     *
     * Unexpected results occur, e.g. invalid sample indexes, if
     * the specified sample is along any of the maximum borders of the volume
     * (given by an sample coordinate index being the maximum for the
     * corresponding axis).
     ...
     * @param aiIndexes must be an array with at least 8 entries where
     * these entries will be filled in with the indexes for linear
     * volume data array for the specified sample and its voxel
     * neighbors.  Note that the [0] entry is for the specified sample, but
     * there is no specified ordering for the other 7 neighbor samples.
     */
    public void getIndexNeighborsVoxel(int iX, int iY, int iZ,
                                       int[] aiIndexes) {
        // Get the index for the "center" sample.
        int iIndexCenter = getIndex(iX, iY, iZ);
     ...
     * Fill an array of indexes for a linear array of volume values.
     * These indexes are for the specified sample and the indexes
     * of the immediate 26-connected neighbors.
     *
     * Unexpected results occur, e.g. invalid sample indexes, if
     * the center sample specified by the indexes is for one on the border
     * of the volume.
     * @param iIndexCenter linear volume data array index center sample
     * @param aiIndexes must be an array with at least 27 entries where
     * these entries will be filled in with the indexes for linear
     * volume data array for the specified sample and its 26-connected
     * neighbors.  Note that the [0] entry is for the center sample, but
     * there is no specified ordering for the 26 neighbor samples.
     */
    ...
    /**
     * Fill in the array of offsets to the 26-connected neighbors for
     * any given sample in the volume.
     * @param akOffsetXYZ must be an array with at least 27 enties where
     * these entries will be filled in with the XYZ index offsets for the
     * 26-connected neighbors of any given sample in the volume.  The
     * ordering of the offsets corresponds to the ordering of the
     * relative linear array indexes returned by the getIndexNeighbors27
     * method.  Note that the [0] entry is for the center sample (always
     * XYZ offsets of zero), and the remaining 26 entries are for the
     * neighbor samples.
     */
    ...
    /**
     * Fill an array of distances to the 26-connected neighbors for
     * any given sample where the distances are the Euclidean
     * distances in sample coordinates.
     * @param afDistances must be an array with at least 27 entries where
     * these entries will be filled in with the Euclidean distances for the
     * 26-connected neighbors of any given sample in the volume.  The
     * ordering of the distances corresponds to the ordering of the
     * relative linear array indexes returned by the getOffsetNeighbors27
     * method.  Note that the [0] entry is for the center sample (always
     * a distance of zero), and the remaining 26 entries are for the
     * neighbor samples distances.
     */
    ...
    /**
     * Fill an array of distances to the 26-connected neighbors for
     * any given sample where the distances are the real Euclidean
     * distances scaled by the size of the sample along each axis.
     * @param afDistances must be an array with at least 27 entries where
     * these entries will be filled in with the Euclidean distances for the
     * 26-connected neighbors of any given sample in the volume.  The
     * ordering of the distances corresponds to the ordering of the
    ...
    /**
     * Compute the "real" coordinates of the sample in the volume at
     * the specified sample coordinate indexes.
     * @param iX sample X coordinate index
     * @param iY sample Y coordinate index
     * @param iZ sample Z coordinate index
     ...
     * @return Point3f  This is a new instance containing the
     * interpolated sample coordinates converted from the input real
     * coordinates.
     */
    ...
    /**
     * Given the coordinates of two points in sample space of the volume,
     * compute the real distance betwen them by taking into consideration
     * the spacing between samples in the volume along each axis.
     ...
     */
    public float getRealDistance(int iX1, int iY1, int iZ1,
