 ...
 * coordinates and the normal vector.  Each type of vertex value is
 * stored in a separate array, however, the vertex values correspond
...
public class ModelTriangleMeshCurvatures {
    ...
    final private Point3f[] m_akCoordinates;

    // Array of vertex normals.
    final private Vector3f[] m_akNormals;

    // Array of triangle vertex connectivity.
    final private int[] m_aiIndices;

    // Array of triangle normals.
    final private Vector3f[] m_akTriangleNormals;

    // Array of mean curvature values at each vertex.
    private float[] m_afMeanCurvatures = null;

    /**
     * Create a set of the min/max curvatures computed for each vertex
     * in the triangle mesh.  The triangle mesh is specified by the
     * coordinates of the vertices, the normal vectors at each vertex,
     * and the vertex array indices which define the triangle connectivity.
     * @param akCoordinates array of vertex coordinates
     * @param akNormals array of vertex normals; Must have at leat the
     * same number of elements as does the akCoordinates array where each
     * element in this normal array corresponds to the vertex coordinate
     * array element at the same index.
     * @param aiIndices array of vertex indices given triangle connectivity
     */
    public ModelTriangleMeshCurvatures(Point3f[] akCoordinates,
                                       Vector3f[] akNormals,
                                       int[] aiIndices) {
        ...
        for (int iTriangle = 0; iTriangle < m_akTriangleNormals.length;
             ++iTriangle) {
     ...
     * Create a set of the min/max curvatures computed for each vertex
     * in the triangle mesh.  The triangle mesh is specified by the
     * coordinates of the vertices and the vertex array indices which
     * define the triangle connectivity.  From this information, the
     * normal vectors of each triangle can be computed and from that
     * the normal vectors at each vertex can be computed.
     * @param akCoordinates array of vertex coordinates
     * @param aiIndices array of vertex indices given triangle connectivity
     */
    public ModelTriangleMeshCurvatures(Point3f[] akCoordinates,
     ...
     * Create a set of the min/max curvatures computed for each vertex
     * in the triangle mesh.  The triangle mesh is specified by the
     * input ModelTrianglMesh instance.
     * @param kMesh ModelTriangleMesh Input triangle mesh which contains
     * the vertex coordinates and vertex triangle connectivity information.
     */
     ...
     * @return Point3f[] Array of 3D vertex coordinates.  The dimension of
     * this array should equal the value returned by getNumVertices().
     */
     ...
     * @return Vector3f[] Array of 3D normal vectors.  The dimension of
     * this array should equal the value returned by getNumVertices().
     */
    ...
    /**
     * Get the array of connectivity among the vertices which define the
     * triangles of the mesh.  Each group of three values in the array
     * contains an index into the vertex array for the vertcies which
     * define a single triangle in the mesh.
     * @return int[] Array of vertex indices.  Each index in the array
     * should be a non-negative value less than the value returned by
     * getNumVertices().  The dimension of this array should equal three
     * times the value returned by getNumTriangles().
     */
     ...
     * @return Vector3f[] Array of 3D normal vectors.  The dimension of
     * this array should equal the value returned by getNumTriangles().
     */
     ...
     * @return array of vertex mean curvature values where individual
     * values may be signed; the dimension of this array should equal the
     * value returned by getNumVertices().
     */
    ...
    /**
     * Compute the normal vector for each triangle as the normal vector
     * to the plane containing the vertices associated with the triangle
     * and considering the vertices to be counterclockwise ordered for
     * a RH rule oriented normal vector.
     * This sets the elements in the m_akTriangleNormals array.
     */
    ...
    /**
     * Compute the normal vector for each vertex as the normalized
     * average normal vector of all the triangles of which the vertex belongs.
     * Do not call this method unless the m_akTriangleNormals array contains
     * valid normalized normal vectors.
     * This sets the elements in the m_akNormals array.
     */
     ...
     * Description of the algorithm is found in the embedded comments.
     */
        ...
        for (int iVertex = 0; iVertex < iVQuantity; iVertex++) {
            ...
            for (int iRow = 0; iRow < 3; iRow++) {
                for (int iCol = 0; iCol < 3; iCol++) {
                    ...
                    aaaWWTrn[iVertex][iRow][iCol] +=
                        aNormal[iRow] * aNormal[iCol];

                    kWWTrn.setElement(iRow, iCol,
                                      aaaWWTrn[iVertex][iRow][iCol]);
                    kDWTrn.setElement(iRow, iCol,
                                      aaaDWTrn[iVertex][iRow][iCol]);
                }
            }
     ...
     */
    private void generateOrthonormalBasis(Vector3f kU, Vector3f kV,
                                          Vector3f kW) {
