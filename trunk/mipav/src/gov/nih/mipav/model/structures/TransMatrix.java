...
/**
 *   Transformation matrix class is an affine homogenous class that
 *   can be used to rotate objects like images and VOIs. It can
 *   constructed as a  2D(3x3) or 3D(4x4) homogenous matrix for transform
 *   (rotation, translation, skew and zoom) images/VOIs.  Skew is not commonly used.
 *   <p>
 ...
 *
 *   <p>
 *   ORDER OF TRANSFORMATIONS = TRANSLATE, ROTATE, ZOOM or
 *							   TRANSLATE, ROTATE, SKEW, ZOOM
 *   <br>
...
public class TransMatrix extends Matrix //                            implements TableModelListener
{
     ...
     *   @param newMatrix  replaces transformation matrix with a new matrix
     *                     assumes users has correct size matrix
     *                     Will add bounds checking in the future
     */
    ...
    /**
     *   Replaces a value in the matrix.  Row and column start indexing with
     *   zero.  Any Array IndexOutOfBoundsExceptions caught, are ignored and
    ...
    public void setTransform( double tX, double tY, double tZ, double rX, double rY, double rZ, double sX, double sY, double sZ, double skX, double skY, double skZ ) {
        ...
        matrix[1][1] = ( ( cosrZ * sinrY * sinrX + sinrZ * cosrX ) * skX - sinrZ * sinrY * sinrX + cosrZ * cosrX ) * sY;
        matrix[1][2] = ( ( cosrZ * sinrY * sinrX + sinrZ * cosrX ) * skY
                + ( -sinrZ * sinrY * sinrX + cosrZ * cosrX ) * skZ - cosrY * sinrX )
                        * sZ;
        matrix[1][3] = tY;
        matrix[2][0] = ( -cosrZ * sinrY * cosrX + sinrZ * sinrX ) * sX;
        matrix[2][1] = ( ( -cosrZ * sinrY * cosrX + sinrZ * sinrX ) * skX + sinrZ * sinrY * cosrX + cosrZ * sinrX ) * sY;
        matrix[2][2] = ( ( -cosrZ * sinrY * cosrX + sinrZ * sinrX ) * skY
                + ( sinrZ * sinrY * cosrX + cosrZ * sinrX ) * skZ + cosrY * cosrX )
                        * sZ;
        matrix[2][3] = tZ;
        for ( j = 0; j < 3; j++ ) {
            matrix[3][j] = 0;
     ...
     *	@param sZ	z scale
     */
    public void setTransform( double tX, double tY, double tZ, double rX, double rY, double rZ, double sX, double sY, double sZ ) {
        int j;
        double sinrX, sinrY, sinrZ, cosrX, cosrY, cosrZ;
        cosrX = Math.cos( ( rX / 180.0 ) * Math.PI );
        sinrX = Math.sin( ( rX / 180.0 ) * Math.PI );
    ...
    /**
     *   Multiplies two matrices together. General in nature for two-dimensional
    ...
    public final void transform( float[] pt, float[] tPt ) {
        ...
        tPt[0] = (float) ( (double) pt[0] * matrix[0][0] + (double) pt[1] * matrix[0][1] + (double) pt[2] * matrix[0][2]
                + matrix[0][3] );

        tPt[1] = (float) ( (double) pt[0] * matrix[1][0] + (double) pt[1] * matrix[1][1] + (double) pt[2] * matrix[1][2]
                + matrix[1][3] );

        tPt[2] = (float) ( (double) pt[0] * matrix[2][0] + (double) pt[1] * matrix[2][1] + (double) pt[2] * matrix[2][2]
                + matrix[2][3] );
    ...
    public final void transform( Point3Df pt, Point3Df tPt ) {
        ...
        tPt.x = (float) ( (double) pt.x * matrix[0][0] + (double) pt.y * matrix[0][1] + (double) pt.z * matrix[0][2]
                + matrix[0][3] );

        tPt.y = (float) ( (double) pt.x * matrix[1][0] + (double) pt.y * matrix[1][1] + (double) pt.z * matrix[1][2]
                + matrix[1][3] );

        tPt.z = (float) ( (double) pt.x * matrix[2][0] + (double) pt.y * matrix[2][1] + (double) pt.z * matrix[2][2]
                + matrix[2][3] );
    ...
    /**
     *   Saves transformation matrix to a text file
     *       MIPAV format
    ...
    /**
     *   Saves transformation matrix and a message to a text file
     *       MIPAV format
     *       4     // number of rows in matrix
     *       4     // number of cols in matrix
     *       0.234 0.33 0.22 5.0  // matrix info separated by a space
    ...
    /**
     *   Saves transformation matrix and message to a text file
     *       MIPAV format
     *       4     // number of rows in matrix
     *       4     // number of cols in matrix
     *       0.234 0.33 0.22 5.0  // matrix info separated by a space
     ...
     *       0.234  0.33  0.22  5.0 // matrix info separated by 2 spaces
     *       0.234  0.33  0.22  5.0 // matrix info separated by 2 spaces
     *       0  0  0  1             // matrix info separated by 2 spaces also note integer values
     *  <p>
     *   @param raFile random access file pointer
     ...
     * Scale(Sx, Sy, Sz)*ShearXY*ShearXZ*ShearYZ*RotateX*RotateY*RotateZ*Translate(tx, ty, tz)*Perspective(Px, Py, Pz, Pw).
    ...
    public boolean decomposeMatrix( Matrix mat ) {
        ...
        if ( locmat.get( 0, 3 ) != 0 || locmat.get( 1, 3 ) != 0 || locmat.get( 2, 3 ) != 0 ) {
...
//            locmat.set(3, i, 0);
        ...
        //System.out.println( "rot x: " + getRotateX() + " rot y: " + getRotateY() + " rot z: " + getRotateZ() );
        //System.out.println( "scl x: " + getScaleX() + " scl y: " + getScaleY() + " scl z: " + getScaleZ() );
        //System.out.println( "trn x: " + getTranslateX() + " trn y: " + getTranslateY() + " trn z: " + getTranslateZ() );
    ...
    private void V4MulPointByMatrix( Vector4Dd pin, Matrix m, Vector4Dd pout ) {
        ...
        pout.x = ( pin.x * m.get( 0, 0 ) ) + ( pin.y * m.get( 1, 0 ) ) + ( pin.z * m.get( 2, 0 ) )
                + ( pin.w * m.get( 3, 0 ) );

        pout.y = ( pin.x * m.get( 0, 1 ) ) + ( pin.y * m.get( 1, 1 ) ) + ( pin.z * m.get( 2, 1 ) )
                + ( pin.w * m.get( 3, 1 ) );

        pout.z = ( pin.x * m.get( 0, 2 ) ) + ( pin.y * m.get( 1, 2 ) ) + ( pin.z * m.get( 2, 2 ) )
                + ( pin.w * m.get( 3, 2 ) );

        pout.w = ( pin.x * m.get( 0, 3 ) ) + ( pin.y * m.get( 1, 3 ) ) + ( pin.z * m.get( 2, 3 ) )
                + ( pin.w * m.get( 3, 3 ) );
    }
