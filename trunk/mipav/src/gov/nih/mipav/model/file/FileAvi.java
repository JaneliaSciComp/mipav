 ...
 *      The Microsoft Video 1 decoding was mostly derived from Mike Melanson's
 *      Open Source Video 1 decoder found at
 *      http://zebra.fh-weingarten.de/~maxi/html/mplayer-dev-eng/2001-11/txt00008.txt
 *      Installing codecs may be done by running the folowing 2 files in mipav/apps:
 *      jmf_2_1_1e-windows-i586.exe
 *      DivX412Codec.exe
 */

public class FileAvi
    extends FileBase {
    ...
    /**
     * 0 for RGB 24 bit per pixel uncompressed
     * 1 for RLE 8 bit per pixel compressed
     * 1296126531 for Microsoft video 1 compression
     *
     */
    ...
    private int readHeader() throws IOException {
        ...
        try {
            ...
            if (CHUNKsignature == 0x5453494C) {
                ...
                raFile.close();
                throw new IOException("AVI read header error signature first LIST subCHUNK = "
                                      + CHUNKsignature);
            }
            ...
            if (CHUNKtype == 0x6C727473) {
                ...
                raFile.close();
                throw new IOException("AVI read header error no strl in first LIST subCHUNK but = " +
                                      CHUNKtype);
            }
            ...
            if (strhSignature == 0x68727473) {
                ...
                raFile.close();
                throw new IOException("AVI read header error no strhSignature found but = " +
                                      strhSignature);
            }
            ...
            else if (handlerString.startsWith("MRLE") ||
                     handlerString.startsWith("mrle") ||
                     handlerString.startsWith("RLE") ||
                     handlerString.startsWith("rle")) {
            /* mrle microsoft run length encoding */
            }
            else if (handlerString.startsWith("MSVC") ||
                     handlerString.startsWith("msvc")) {
                // Microsoft video 1 compression
                doMSVC = true;
            }
            else if (handlerString.startsWith("MP42") ||
                     handlerString.startsWith("mp42")) {
                return AlgorithmTranscode.TRANSCODE_MP42;
            }
            else if (handlerString.startsWith("MJPG") ||
                     handlerString.startsWith("mjpg")) {
                return AlgorithmTranscode.TRANSCODE_MJPG;
            }
            else if (handlerString.startsWith("DIV") ||
                     handlerString.startsWith("div")) {
                return AlgorithmTranscode.TRANSCODE_DIVX;
            }
            else if (handlerString.startsWith("MPG4") ||
                     handlerString.startsWith("mpg4")) {
                return AlgorithmTranscode.TRANSCODE_MPG4;
            }
            else if (handlerString.startsWith("IV32") ||
                     handlerString.startsWith("iv32")) {
                return AlgorithmTranscode.TRANSCODE_IV32;
            }
            else if (handlerString.startsWith("IV41") ||
                     handlerString.startsWith("iv41")) {
                return AlgorithmTranscode.TRANSCODE_IV41;
            }
            else if (handlerString.startsWith("IV50") ||
                     handlerString.startsWith("iv50")) {
                return AlgorithmTranscode.TRANSCODE_IV50;
            }
            else if (handlerString.startsWith("cvid") ||
                     handlerString.startsWith("CVID")) {
                return AlgorithmTranscode.TRANSCODE_CVID;
            }
            else {
                raFile.close();
                throw new IOException("Unrecognized compression handler is " + handlerString);
                // tscc is the TechSmith Screen Capture Codec put out by the Techsmith Corporation
                // for use with their Camtasia Screen "Camcorder" application.  Camtasia is a
                // Microsoft windows application only.  The company has no plans to develop a
                // version for the apple.
                // Could the program be designed to use the tscc.exe codec provided by Techsmith?
            }
            ...
            if ( ( (compression == 0) && ( (bitCount == 8) || (bitCount == 16) ||
            ...
            if ( (compression == 0) && ( (bitCount == 24) || (bitCount == 32)) && (colorsUsed != 0)) {
                ...
                throw new IOException
                    ("For 24 and 32 bit uncompressed data software does not currently support colorsUsed = "
                     + colorsUsed);
            }
            ...
            if (marker < (LIST1Marker + LIST1Size)) {
                ...
                if (strnSignature == 0x6E727473) {
                    ...
                    if (text[strnLength - 1] != 0) {
                        raFile.close();
                        throw new IOException("strn string ends with illegal temination = " +
                                            text[strnLength - 1]);
                    }
                } // if (strnSignature == 0x6E727473)
        ...
        try {
            ...
            if (progressBar == null) {
                progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                                   ViewUserInterface.getReference().getProgressBarPrefix() + "AVI Image ...", 0, 100, false, null, null);
            ...
            // Do first read thru the data to find the actual number of frames used by MIPAV.
            // This must be done before the MIPAV image can be created.
            ...
            while (((!AVIF_MUSTUSEINDEX) && (totalBytesRead < totalDataArea) && chunkRead) ||
                ...
                if (haveMoviSubchunk && (subchunkBlocksRead == streams) &&
                   (totalBytesRead < totalDataArea)) {
                    totalBytesRead += moviSubchunkPosition + LIST2subchunkSize
                    ...
                    if (signature == 0x5453494C) {
                        ...
                        if (CHUNKtype == 0x20636572) {
                            ...
                            raFile.close();
                            throw new IOException("CHunktype for LIST2sbuchunk is an illegal = "
                                                   + CHUNKtype);
                        }
                    }
                    else {
                                    ...
                                    for (blockX = blocksWide; (blockX > 0) && (!bufferFinished); blockX--) {
                                        ...
                                        // check if it's an 8-color block
                                            ...
                                            for (pixelY = 0; pixelY < 4; pixelY++) {
                                                for (pixelX = 0; pixelX < 4; pixelX++) {
                                                    if ((flags & 1) > 0) {
                                                        imgBuffer[pixelIndex++] =
                                                            (byte)255;
                                                        imgBuffer[pixelIndex++] =
                                                            c1r[pixelX >> 1][pixelY >> 1];
                                                        imgBuffer[pixelIndex++] =
                                                            c1g[pixelX >> 1][pixelY >> 1];
                                                        imgBuffer[pixelIndex++] =
                                                            c1b[pixelX >> 1][pixelY >> 1];
                                                    }
                                                    else {
                                                        imgBuffer[pixelIndex++] =
                                                            (byte)255;
                                                        imgBuffer[pixelIndex++] =
                                                            c2r[pixelX >> 1][pixelY >> 1];
                                                        imgBuffer[pixelIndex++] =
                                                            c2g[pixelX >> 1][pixelY >> 1];
                                                        imgBuffer[pixelIndex++] =
                                                            c2b[pixelX >> 1][pixelY >> 1];
                                                    }
     ...
     *   @param alphaBlend
     *   @param paintBitmap
     *   @param compression  -1 = unchosen, 0 = 24 bit uncompressed RGB,
     *                       1 = 8 bit per pixel compressed RLE
     *   @exception IOException  if there is an error writing the file.
     */
    public boolean writeImage(ModelImage _imageA, ModelImage _imageB,
                           ModelLUT _LUTa, ModelLUT LUTb,
                           ModelRGB RGBTA, ModelRGB RGBTB, int red, int green, int blue,
                           float OPACITY, float alphaBlend,
        ...
        if (compression != -1 && compression != 1) {
            ...
            compression = 0;
            JDialogAVIChoice choice =
                new JDialogAVIChoice(imageA.getUserInterface().getMainFrame(), true);
            ...
            if ( (compression != 0) && (compression != 1)) {
                JDialogAVIChoice choice =
                    new JDialogAVIChoice(imageA.getUserInterface().getMainFrame(), false);
                if (!choice.okayPressed()) {
        ...
        if (UI.isScriptRecording()) {
            UI.getScriptDialog().append("SaveImageAs " +
        ...
        if (useNewCompression) {
            ...
            if (leftPadding != 0 || topPadding != 0) {
                //this means we must resize the image to save it with this compression
                // ask the user if he/she wants to resize first
                ...
                if (response == JOptionPane.NO_OPTION) {
                    ...
                    // System.err.println("New extents: " + newExtents[0] + " by " + newExtents[1] + " by " + newExtents[2]);

                    ModelImage paddedImage = new ModelImage(imageA.getType(),
                        newExtents, "TEMPImage", UI);

                    for (int index = 0; index < newExtents.length; index++) {
                        paddedImage.getFileInfo()[0].setResolutions(imageA.getFileInfo()[0].getResolutions()[index], index);
                        paddedImage.getFileInfo()[0].setUnitsOfMeasure(imageA.getFileInfo()[0].getUnitsOfMeasure()[index], index);
                    }

                    AlgorithmAddMargins algoMarg = null;
                    if (imageA.isColorImage()) {
                        //System.err.println("using color add image margins");
                        algoMarg = new AlgorithmAddMargins(imageA, paddedImage,
                            0, 0, 0, leftPadding, 0, topPadding, 0, 0);
                    }
                    else {
                        //System.err.println("using b\\w add image margins");
                        algoMarg = new AlgorithmAddMargins(imageA,
                            paddedImage, 0, leftPadding, 0, topPadding, 0, 0);
                    }
                    ...
                    boolean wasOkay = writeImage(paddedImage, null, _LUTa, null,
                               RGBTA, RGBTB, red, green, blue,
        ...
        progressBar = new ViewJProgressBar(imageA.getImageName(), progressString,
        ...
        int rate = (int) Math.round( (realFrames) / ( (double) totalFrames / (1.0 / ( (double) newMicroSecPerFrame / 1000000.0))));
        ...
        if ( (resXUnit == FileInfoBase.INCHES) || (resXUnit == FileInfoBase.CENTIMETERS) ||
            ...
            (resXUnit == FileInfoBase.MICROMETERS) ||
            (resXUnit == FileInfoBase.MILLIMETERS) || (resXUnit == FileInfoBase.METERS) ||
            (resXUnit == FileInfoBase.KILOMETERS) || (resXUnit == FileInfoBase.MILES)) {
        ...
        if ( (resYUnit == FileInfoBase.INCHES) || (resYUnit == FileInfoBase.CENTIMETERS) ||
            ...
            (resYUnit == FileInfoBase.MICROMETERS) ||
            (resYUnit == FileInfoBase.MILLIMETERS) || (resYUnit == FileInfoBase.METERS) ||
            (resYUnit == FileInfoBase.KILOMETERS) || (resYUnit == FileInfoBase.MILES)) {
    ...
    /**
     *   This method sets up for an AVI image write. This is only used for RGB images
     *   where only an imageA and no imageB will be present.  The image will be written
     *   as a series of 2D images and a series of frames without data used for repeating
     *   existing frames.  This method will use the first 2D image.
     ...
     */
    private void writeAVITriplet(int timeSlice, int slice, ModelLUT LUTb, int red, int green, int blue, float opacityPrime,
                                 float alphaBlend, BitSet paintBitmap) throws IOException {
     ...
     */
    private void writeAVITripletC(int timeSlice, int slice, ModelRGB RGBTA, ModelRGB RGBTB,
                                  int red, int green, int blue, float opacityPrime, float alphaBlend, BitSet paintBitmap) throws
        IOException {
        ...
        try {
            ...
            if (imageB != null) {
                imageB.exportData( (timeSlice * zDimSlices * bufferSize + slice * bufferSize), bufferSize, imageBufferB);
            }
        }
        catch (IOException error) {
    ...
    /**
     *  Write BGR AVI triplet for color image.  Since this is a frame captured on
     *  screen, this method is much simpler.
     */
     ...
     *  @return           boolean to indicate if the show was successful
     */
    private void writeRLE8(int timeSlice, int slice, byte[] pixStore,
                           byte[] lastStore, byte[] encodeStore) throws IOException {
        int pixIndex;
        int count;
        int encodeIndex;
        int i, index;
        ...
        for (int y = 0; y < yDim; y++) {
            for (int x = 0; x < xDim; x++) {
                index = x + y * xDim;
                if ( (pixStore[index] == lastStore[index]) &&
                    ...
                    if (x < sameX) {
                        ...
                        while ( (deltaX > 0) || (deltaY > 0)) {
                            ...
                            encodeStore[encodeIndex++] =
                                (byte) Math.min(deltaX, 255);
                            encodeStore[encodeIndex++] =
                                (byte) Math.min(deltaY, 255);
                            deltaX -= 255;
                            if (deltaX < 0) {
                                deltaX = 0;
                            }
