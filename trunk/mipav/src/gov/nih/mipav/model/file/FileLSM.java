...
/**
 *   Reads Zeiss LSM 510 image files - an extension of the TIFF format.  However, ordinary
 *   TIFF format supports both Little Endian(0x4949 or II) byte order and Big Endian
 *   (0x4d4d or MM) byte order.  LSM supports only Little Endian byte order.
 *   NEW_SUBFILE_TYPE can be either 0 for images or 1 for thumbnails.  The reader will skip
 *   over thumbnails and not include them in the read image.  COMPRESSION can be either 1
 *   for no compression or 5 for LZW compression.
 *   <p>
 *   One big difference between standard TIFF and the LSM format is the inclusion of the
 *   private CZ_LSMINFO tag.  The pixel resolutions must be obtained from the VoxelSizeX,
 *   VoxelSizeY, and VoxelSizeZ fields in this private tag.  This private tag appears only
 *   in the first image directory, so fileInfo.set operations must cycle thru all the slices
 *   using the values obtained when reading the first slice.
 *   <p>
 *   The x and y resolution units are always be in meters.  LSM data may be 1, 2, or 3 channels.
 *   1 data is black and white.  Both 2 and 3 channels are RGB although in 2 channels only 2 of
 *   the 3 colors are used.  LSM format uses either 8 bit unsigned data, 12 bit unsigned data,
 *   or 32 bit floats.  Ths 12 bit unsigned data is stored in 16 bit shorts.  If 2 or 3 channels
 *   use 12 bit unsigned data, then ARGB_USHORT format must be used, a format not found in ordinary
 *   TIFF.  If a LSM file has 4 or more channels, then create another dimension in which to
 *   put each spectrum.  The color map values are scaled to go only to 255.0 since a value of
 *   65535 was observed in the unscaled map.
 *   <p>
 *   Ordinary TIFF uses the SAMPLE_FORMAT FIELD to specify how to interpret data with 1 =
 *   unsigned integer data, 2 = two's complement signed integer data, 3 = IEEE floating
 *   point data, and 4 = undefined data format.  The default is 1, unsigned integer data.
 *   LSM files do not include the SAMPLE_FORMAT field, so if only TIFF rules were used the
 *   LSM 32 bit floating point numbers would be interpreted as unsigned integers.
 *   <p>
 *   The LSM release 2.0 documentation does not completely conform with the observed files.
 *   Table 16 with the CZ-Private tag shows a 488 byte structure, but at least some LSM files
 *   have values of 512 bytes in the s32StructureSize field.  u32OffsetTimeStamps appears to
 *   point to a time stamps structure, but this structure contains the ascii such as Ch1-T1,
 *   Ch2-T2, and Ch3-T3 which does not appear in the table 22 structure for time stamp information.
 *   u32OffsetNextRecording appears to point to the start of a new structure, but it is not a
 *   second file header.
 *
 *
 *       @see        FileIO
...
public class FileLSM
    ...
    /**
     *  Accessor that returns the address of the TIF header of the second image ir present
    ...
    /**
     *   			        	Reads the Tiff header which indicates endianess,
     *                           the TIFF magic number, and the offset in bytes of
     *                           the first IFD.  It then reads all the IFDs.
     *                           This method
     *                           then opens a Model of an image and imports the
     *                           the images one slice at a time. Image slices are
     *                           separated by an IFD.
        ...
        try {
            ...
            if (doTile) {
                ...
                //tileByteCounts[numVectors * 3 - 1] = (int) ( (Index) (dataOffsets[numVectors-1].elementAt(3))).index -
                 //   tileOffsets[numVectors * 3 - 1];
                //System.err.println("Last tile byte count: " + tileByteCounts[numVectors * 3 -1]);
           ...
           if (haveBleachedRectangle && (bleachKnotX[0] >= 0) &&
               (bleachKnotX[0] <= imgExtents[0] - 1) && (bleachKnotY[0] >= 0) &&
               (bleachKnotY[0] <= imgExtents[1] - 1) && (bleachKnotX[1] >= 0) &&
               ...
               if (reply == JOptionPane.YES_OPTION) {
                ...
                photoBleachedVOI = new VOI(id, "Photobleached", zDim,
                                           VOI.CONTOUR, 0.0f);
                int x[] = new int[4];
                int y[] = new int[4];
                int z[] = new int[4];
           ...
           else if (haveBleachedCircle && (bleachKnotX[0] >= 0) &&
               (bleachKnotX[0] <= imgExtents[0] - 1) && (bleachKnotY[0] >= 0) &&
               (bleachKnotY[0] <= imgExtents[1] - 1) && (bleachKnotX[1] >= 0) &&
               (bleachKnotX[1] <= imgExtents[0] - 1) && (bleachKnotY[1] >= 0) &&
               (bleachKnotY[1] <= imgExtents[1] - 1)) {
               ...
               if (((bleachKnotX[0] - radius) >= 0) &&
                   ((bleachKnotX[0] + radius) <= imgExtents[0] - 1) &&
                   ((bleachKnotY[0] - radius) >= 0) &&
                   ((bleachKnotY[0] + radius) <= imgExtents[1] - 1)) {
                   int reply = JOptionPane.showConfirmDialog(UI.getMainFrame(),
                       "Do you want to load the bleached ROI?",
                       "Load Bleached ROI",
                       JOptionPane.YES_NO_OPTION,
                   ...
                   if (reply == JOptionPane.YES_OPTION) {
                       ...
                       int z[] = new int[40];
                       for (i = 0; i < 40; i++) {
                           x[i] = (int)Math.round(bleachKnotX[0] +
                                       radius*Math.cos(2.0*Math.PI*i/40.0));
                           y[i] = (int)Math.round(bleachKnotY[0] +
                                       radius*Math.sin(2.0*Math.PI*i/40.0));
                       }
           ...
           else if (haveBleachedCirclePerim3 && (bleachKnotX[0] >= 0) &&
               (bleachKnotX[0] <= imgExtents[0] - 1) && (bleachKnotY[0] >= 0) &&
               (bleachKnotY[0] <= imgExtents[1] - 1) && (bleachKnotX[1] >= 0) &&
               (bleachKnotX[1] <= imgExtents[0] - 1) && (bleachKnotY[1] >= 0) &&
               ...
               if (((a - radius) >= 0) &&
                   ...
                   if (reply == JOptionPane.YES_OPTION) {
                       ...
                       int z[] = new int[40];
                       for (i = 0; i < 40; i++) {
                           x[i] = (int)Math.round(a +
                                       radius*Math.cos(2.0*Math.PI*i/40.0));
                           y[i] = (int)Math.round(b +
                                       radius*Math.sin(2.0*Math.PI*i/40.0));
                       }
           ...
           else if (haveBleachedEllipse && (bleachKnotX[0] >= 0) &&
               (bleachKnotX[0] <= imgExtents[0] - 1) && (bleachKnotY[0] >= 0) &&
               (bleachKnotY[0] <= imgExtents[1] - 1) && (bleachKnotX[1] >= 0) &&
               (bleachKnotX[1] <= imgExtents[0] - 1) && (bleachKnotY[1] >= 0) &&
               (bleachKnotY[1] <= imgExtents[1] - 1) && (bleachKnotX[2] >= 0) &&
               ...
               if (reply == JOptionPane.YES_OPTION) {
                    ...
                    for (i = 0; i < 40; i++) {
                        ...
                        rp = Math.sqrt(
                             b*b*a*a/(b*b*costhp*costhp + a*a*sinthp*sinthp));
                        xp = rp*costhp;
                        yp = rp*sinthp;
                        if ((costh != 0.0) && (sinth != 0.0)) {
                            num = xp - sinth*yp/costh + xc*costh
                                  + xc*sinth*sinth/costh;
                            denom = costh + sinth*sinth/costh;
                            x[i] = (int)(num/denom + 0.5);
                            num = xp + costh*yp/sinth + yc*sinth
                                  + yc*costh*costh/sinth;
           ...
           int bufferSize;
            ...
            progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                               ViewUserInterface.getReference().getProgressBarPrefix() + "LSM image(s) ...", 0, 100,
                                               false, null, null);
            ...
            switch (tag) {
                ...
                case BITS_PER_SAMPLE:
                    ...
                    if (count == 1) {
                        ...
                        Preferences.debug("BITS_PER_SAMPLE shows " + spectrumNumber +
                                          " spectrums\n");
                    }
                    break;
                case ROWS_PER_STRIP:
                    ...
                    if (chunky) {
                        if (tilesPerImage != count) {
                            throw new IOException
                                ("Count fields do not agree in TILE_OFFSETS and TILE_BYTE_COUNTS");
                        }
                    }
                    else {
                ...
                case MIN_SAMPLE_VALUE:
                    ...
                    for (i1 = 0; i1 < count; i1++) {
                        Preferences.debug("FileTiff.openIFD: minSampleValue[" + i1 + "] = " +
                                          valueArray[i1] + "\n");
                    }
                    break;
                case MAX_SAMPLE_VALUE:
                    ...
                    for (i1 = 0; i1 < count; i1++) {
                        Preferences.debug("FileTiff.openIFD: maxSampleValue[" + i1 + "] = " +
                                          valueArray[i1] + "\n");
                    }
                    break;
                case SOFTWARE:
                ...
                case SAMPLE_FORMAT:
                    ...
                    for (i1 = 0; i1 < count; i1++) {
                        Preferences.debug("FileTiff.openIFD: sampleFormat[ " + i1 + "] = " +
                                          valueArray[i1] + "\n");
                        if (valueArray[i1] == 1) {
         ...
         if (offsetOutputLut != 0) {
          ...
          if (offsetChannelColors != 0) {
              ...
              if (numberColors != czChannels) {
                  Preferences.debug("Number of channels = " + czChannels +
                                ", but number of colors = " + numberColors + "\n");
              }
              numberNames = getInt(endianess);
              if (numberNames != czChannels) {
                  Preferences.debug("Number of channels = " + czChannels +
                                ", but number of names = " + numberNames + "\n");
              }
              ...
              for (i = 0; i < numberColors; i++) {
                  ...
                  Preferences.debug("Channel # " + i + " red = " + redArray[i] + " green = " +
          ...
          if (offsetChannelDataTypes != 0) {
              ...
              for (i = 0; i < czChannels; i++) {
                  ...
                  if (channelDataTypes[i] == 1) {
                      ...
                      Preferences.debug("Channel # " + i + " has illegal data type = " +
                                         channelDataTypes[i] + "\n");
          ...
          Preferences.debug("Zoom factor for image display in the x-direction = " +
                             displayAspectX + "\n");
          if (displayAspectX > 0.0) {
            fileInfo.setDisplayAspectX(displayAspectX);
          }
          ...
          Preferences.debug("Zoom factor for image display in the y-direction = " +
                             displayAspectY + "\n");
          if (displayAspectY > 0.0) {
            fileInfo.setDisplayAspectY(displayAspectY);
          }
          ...
          Preferences.debug("Zoom factor for image display in the z-direction = " +
                             displayAspectZ + "\n");
          if (displayAspectZ > 0.0) {
            fileInfo.setDisplayAspectZ(displayAspectZ);
          }
          ...
          Preferences.debug("Zoom factor for image display in the time-direction = " +
                             displayAspectTime + "\n");
          if (displayAspectTime > 0.0) {
            fileInfo.setDisplayAspectTime(displayAspectTime);
          }
          ...
          if (offsetChannelWavelength != 0) {
             ...
             for (i = 0; i < wavelengthNumber; i++) {
                 ...
                 Preferences.debug("Start wavelength[" + (i+1) + "] = " +
                                    wavelengths[2*i] + "\n");
                 Preferences.debug("End wavelength[" + (i+1) + "] = " +
                                    wavelengths[2*i+1] + "\n");
             }
          ...
          Preferences.debug("Objective sphere correction = " +
    ...
    private void readLut() throws IOException {
        ...
        if (lutKind == INPUT_LUT) {
            if (lutChannels == czChannels) {
                ...
                Preferences.debug(lutString + "handles " + lutChannels +
                " channels, but the number expected is " + czChannels + ",\n");
            ...
            if (lutChannels == 3) {
                ...
                Preferences.debug(lutString + "handles " + lutChannels +
                " channels, but the number expected is 3\n");
            }
        }
        ...
        if (currentChannel == -1) {
            Preferences.debug(lutString +
            "selected all channels for modification in the LUT editor\n");
        }
        else {
            Preferences.debug(lutString + " most recently selected channel " +
            currentChannel + " for modification in the LUT editor\n");
        }
        for (i = 0; i < 9; i++) {
        ...
        for (i = 0; i < subBlockNumber; i++) {
            ...
            Preferences.debug(lutString + subBlockString + " has " + subBlockSize +
                              " bytes\n");
            switch (subBlockType) {
                case 1: // SUBBLOCK_GAMMA
                    ...
                    for (j = 0; j < lutChannels; j++) {
                        ...
                        Preferences.debug(lutString + "channel " + (j+1) +
                        " has exponent " + gamma[j] + "\n");
                    }
                    break;
                    ...
                    if (lutChannels > 0) {
                        ...
                        for (j = 0; j < lutChannels; j++) {
                            for (k = 0; k < knotNumber; k++) {
                                ...
                                Preferences.debug(lutString + "channel " +
                                                  (j + 1) +
                                                  " knot " + (k + 1) + " = (" +
                                                  lutKnotX[j][k] + "," +
                                                  lutKnotY[j][k] + ")\n");
        ...
        if (lutEnd == 0) {
            ...
            Preferences.debug(lutString + "ended with " + lutEnd +
                              " instead of the expected zero\n");
        }
        return;
                ...
                for (i = 0; i < nLONG; i++) {
                    ...
                    switch(entry) {
                        ...
                        case RECORDING_ENTRY_STOP_SCAN_EVENT:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "Button (normal operation button)";
                                    break;
                                case 1:
                        ...
                        case RECORDING_ENTRY_USE_ROIS:
                            switch(intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "ROIs should not be used for the scan process";
                                    break;
                                default:
                                    tempString[index++] = pad +
                                    "ROIs should be used for the scan process";
                            }
                            break;
                        case RECORDING_ENTRY_USE_REDUCED_MEMORY_ROIS:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "Scan memory was provided for the whole plane";
                                    break;
                                default:
                        ...
                        case RECORDING_ENTRY_INTERPOLATIONY:
                            switch (intValue) {
                                case 1:
                                    ...
                                    break;
                                default:
                                    tempString[index++] = pad + "Scan 1 out of every " +
                                    String.valueOf(intValue) + " lines";
                            }
                            break;
                        case RECORDING_ENTRY_CAMERA_SUPERSAMPLING:
                            switch(intValue) {
                        ...
                        case TRACK_ENTRY_MULTIPLEX_TYPE:
                            switch (intValue) {
                                case 2:
                                    tempString[index++] = pad +
                                    "A switch to the next track is done afer a stack";
                                    break;
                                case 1:
                                    tempString[index++] = pad +
                                    "A switch to the next track is done after a plane";
                                    break;
                                case 0:
                                    tempString[index++] = pad +
                                    "A switch to the next track is done after a line";
                                    break;
                                default:
                                    tempString[index++] = pad + String.valueOf(intValue);
                            }
                        ...
                        case TRACK_ENTRY_ACQUIRE:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "The track was not used during scan";
                                    break;
                                default:
                                    tempString[index++] = pad +
                                    "The track was used during scan";
                            }
                            break;
                        case TRACK_ENTRY_IS_BLEACH_TRACK:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "The track setting does not specify bleach parameters";
                                    break;
                                default:
                                    tempString[index++] = pad +
                                    "The track setting does specify bleach parameters " +
                        ...
                        case TRACK_ENTRY_CONDENSOR_FRONTLENS:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "No condensor front lens was in the beam path";
                                    break;
                                case 1:
                                    tempString[index++] = pad +
                                    "The condensor front lens was in the beam path";
                                    break;
                            }
                            break;
                        case LASER_ENTRY_ACQUIRE:
                        ...
                        case DETCHANNEL_ENTRY_ACQUIRE:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "Detection channel was not used during scan";
                                    break;
                                default:
                                    tempString[index++] = pad +
                                    "Detection channel was used during scan";
                            }
                            break;
                        case ILLUMCHANNEL_ENTRY_ACQUIRE:
                            switch (intValue) {
                                case 0:
                                    tempString[index++] = pad +
                                    "The attenuator was disabled";
                                    break;
                                default:
                                    tempString[index++] = pad +
                                    "The attenuator was enabled";
                            }
                            break;
                        case DATACHANNEL_ENTRY_COLOR:
                            ...
                            tempString[index++] = pad +
                            "red = " + String.valueOf(redValue);
                            tempString[index++] = pad +
                            "green = " + String.valueOf(greenValue);
                            tempString[index++] = pad +
                            "blue = " + String.valueOf(blueValue);
                            break;
                        case DATACHANNEL_ENTRY_SAMPLETYPE:
                            switch (intValue) {
                                case 1:
                        ...
                        case DATACHANNEL_ENTRY_RATIO_TYPE:
                            switch (intValue) {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    tempString[index++] = pad + "Online ratio: " +
                                    "(S1 + C1)/(S2 + C2)*C3+C4";
                                    break;
                                case 2:
                                    tempString[index++] = pad + "Online subtraction: " +
                                    "(S1*C1 - S2*C3)/C2 + C4";
                                    break;
                                case 3:
                                    tempString[index++] = pad + "Online ratio: " +
                                    "(S1-S2*C1)/(S1+S2*C2)*C3+C4";
                                    break;
                                case 4:
                                    tempString[index++] = pad + "Online hill function: " +
                                    "C3*(C4/C5)*(S1-C1)/(C2-S2)";
                                    break;
                                case 5:
                                    tempString[index++] = pad + "Online reference ratio: " +
                                    "(S1-S2+C1)/(S2-C2)*C3+C4";
                                    break;
                                case 6:
    ...
    private void readDrawingElement() throws IOException {
        ...
        if (numberDrawingElements == 1) {
            Preferences.debug(drawString +  "has " + numberDrawingElements +
                              " drawing element in the list\n");
        }
        else {
            Preferences.debug(drawString +  "has " + numberDrawingElements +
                              " drawing elements in the list\n");
        }
        blockSize = getInt(endianess);
        ...
        Preferences.debug("The most recently used color had red = " + redValue +
        ...
        Preferences.debug("Angle of each character's base line = " +
        ...
        if (fontCharSet == 0) {
            ...
            Preferences.debug("FontCharSet = " + fontCharSet +
                              " instead of the expected zero\n");
        }
        ...
        if (fontOutPrecision == 0) {
            ...
            Preferences.debug("FontOutPrecision = " + fontOutPrecision +
                              " instead of the expected zero\n");
        }
        ...
        if (fontClipPrecision == 0) {
            ...
            Preferences.debug("FontClipPrecision = " + fontClipPrecision +
                              " instead of the expected zero\n");
        }
        ...
        if (fontQuality == 0) {
            ...
            Preferences.debug("FontQuality = " + fontQuality +
                              " instead of the expected zero\n");
        }
        ...
        if (fontFamily == 0) {
            ...
            Preferences.debug("Font family was a variable stroke width, " +
                              "sans-serified\n");
        }
        else if (fontFamily == 3) {
            Preferences.debug("Font family was a constant stroke width, " +
        ...
        for (i = 0; i < numberDrawingElements; i++) {
            Preferences.debug("Reading information for" + drawString2 + (i+1) +
                              " of " + numberDrawingElements + "\n");
            ...
            switch (elementType) {
                ...
                case 14:
                    ...
                    break;
                case 15:
                    Preferences.debug(drawString + (i+1) +
                                      " is a horizontal or vertical " +
                                      "scale bar with displayed length\n");
                    break;
                case 16:
                    Preferences.debug(drawString + (i+1) +
                                      " is a line and a two line arrow\n");
                    break;
                case 17:
                    Preferences.debug(drawString + (i+1) +
                                      " is a line and a three line arrow\n");
                    break;
                case 18:
                ...
                case 22:
                    Preferences.debug(drawString + (i+1) +
                                      " is a closed bezier spline curve\n");
                    if (drawingElement == BLEACH_ROI) {
                ...
                case 23:
                    Preferences.debug(drawString + (i+1) +
                                      " is an open bezier spline curve\n");
                    break;
                case 24:
                ...
                case 25:
                    Preferences.debug(drawString + (i+1) +
                                      " is a rectangle with color palette\n");
                    break;
                case 26:
                    Preferences.debug(drawString + (i+1) +
                                      " is a open polyline with arrow tip\n");
                    break;
                case 27:
                    Preferences.debug(drawString + (i+1) +
                                      " is an open Bezier spline curve with arrow tip\n");
                    break;
                case 28:
                    Preferences.debug(drawString + (i+1) +
                                      " is two connected lines for angle measurement\n");
                    break;
                case 29:
                    Preferences.debug(drawString + (i+1) +
                                      " is a circle defined by 3 points on the perimeter\n");
                    if (drawingElement == BLEACH_ROI) {
                        bleachedROIShape = CIRCLE;
                        fileInfo.setBleachedROIShape(bleachedROIShape);
                    }
            ...
            Preferences.debug(drawString + (i+1) + " has a block size = " +
                               drawingElementBlockSize + " bytes\n");
            drawingElementBlockStart = drawingElementBlockStart + drawingElementBlockSize;
            lineWidth = getInt(endianess);
            Preferences.debug(drawString + (i+1) + " has line width = " +
                               lineWidth + "\n");
            measure = getInt(endianess);
            if (measure == 0) {
            ...
            Preferences.debug(drawString + (i+1) + " has a horizontal starting " +
                              "point = " + startPointX + "\n");
            startPointY = getDouble(endianess);
            Preferences.debug(drawString + (i+1) + " has a vertical starting " +
                              "point = " + startPointY + "\n");
            color = getInt(endianess);
            redValue = color & 0x000000ff;
            greenValue = (color >> 8) & 0x000000ff;
            blueValue = (color >> 16) & 0x000000ff;
            Preferences.debug(drawString + (i+1) + " had red = " + redValue +
                          " green = " + greenValue + " blue = " + blueValue + "\n");
            valid = getInt(endianess);
            if (valid == 0) {
                Preferences.debug(drawString + (i+1) +
                                 " editing was unexpectedly not completed\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " editing was completed as expected\n");
            }
            knotWidth = getInt(endianess);
            Preferences.debug(drawString + (i+1) + " knotWidth = " + knotWidth + "\n");
            catchArea = getInt(endianess);
            Preferences.debug(drawString + (i+1) +
                              " The size of the mouse catch area was " + catchArea + "\n");
            fontHeight = getInt(endianess);
            Preferences.debug(drawString + (i+1) +
                              " The font height in logical units was " + fontHeight + "\n");
            fontWidth = getInt(endianess);
            if (fontWidth == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " The default font width was used\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " The font width in logical units was " + fontWidth + "\n");
            }
            fontEscapement = getInt(endianess);
            Preferences.debug(drawString + (i+1) +
                              " Angle of each text line = " + (fontEscapement/10.0f) + "\n");
            fontOrientation = getInt(endianess);
            Preferences.debug(drawString + (i+1) +
                              " Angle of each character's base line = " +
                             (fontOrientation/10.0f) + "\n");
            fontWeight = getInt(endianess);
            if (fontWeight == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " The default font weight was used\n");
            }
            else if (fontWeight == 400) {
                Preferences.debug(drawString + (i+1) +
                                  " The font weight was 400, the value for normal\n");
            }
            else if (fontWeight == 700) {
                Preferences.debug(drawString + (i+1) +
                                  " The font weight was 700, the value for bold\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " The font weight was = " + fontWeight + "\n");
            }

            fontItalic = getInt(endianess);
            if (fontItalic == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " An italic font was not specified\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " An italic font was specified\n");
            }
            fontUnderline = getInt(endianess);
            if (fontUnderline == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " An underlined font was not specified\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " An underlined font was specified\n");
            }
            fontStrikeOut = getInt(endianess);
            if (fontStrikeOut == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " A strikeout font was not specified\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " A strikeout font was specified\n");
            }
            fontCharSet = getInt(endianess);
            if (fontCharSet == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " FontCharSet = 0 as expected\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
            ...
            if (fontOutPrecision == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " FontOutPrecision = 0 as expected\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " FontOutPrecision = " + fontOutPrecision +
                                  " instead of the expected zero\n");
            }
            fontClipPrecision = getInt(endianess);
            if (fontClipPrecision == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " FontClipPrecision = 0 as expected\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " FontClipPrecision = " + fontClipPrecision +
                                  " instead of the expected zero\n");
            }
            fontQuality = getInt(endianess);
            if (fontQuality == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " FontQuality = 0 as expected\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
            ...
            if (fontPitch == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " The system default pitch was used\n");
            }
            else if (fontPitch == 1) {
                Preferences.debug(drawString + (i+1) +
                                  " A fixed pitch was used\n");
            }
            else if (fontPitch == 2) {
                Preferences.debug(drawString + (i+1) +
                                  " A variable pitch was used\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
                                  " FontPitch = 3\n");
            ...
            if (fontFamily == 0) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family is a don't care\n");
            }
            else if (fontFamily == 1) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family is a variable stroke width, serified\n");
            }
            else if (fontFamily == 2) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family was a variable stroke width, " +
                                  "sans-serified\n");
            }
            else if (fontFamily == 3) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family was a constant stroke width, " +
                                  "serified or sans-serified\n");
            }
            else if (fontFamily == 4) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family was cursive\n");
            }
            else if (fontFamily == 5) {
                Preferences.debug(drawString + (i+1) +
                                  " Font family was old English\n");
            }
            else {
                Preferences.debug(drawString + (i+1) +
            ...
            switch (elementType) {
                case 13: // text
                    ...
                    Preferences.debug(drawString + "text " + (i+1) +
                                      " has left text start = " + pointOriginX + "\n");
                    pointOriginY = getDouble(endianess);
                    Preferences.debug(drawString + "text " + (i+1) +
                                      " has upper border of text bounding rectangle at = " +
                                      pointOriginY + "\n");
                ...
                case 14: // line
                    ...
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "line " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "line " + (i+1) +
                        " has " + numberKnots + " knots instead of the required 2\n");
                    }
                ...
                case 15: // scale bar
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "scale bar " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "scale bar " + (i+1) +
                ...
                case 16: // open arrow
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "open arrow " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "open arrow " + (i+1) +
                ...
                case 17: // closed arrow
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "closed arrow " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "closed arrow " + (i+1) +
                ...
                case 18: // rectangle
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "rectangle " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "rectangle " + (i+1) +
                ...
                case 19: // ellipse
                    ...
                    knotY = new double[numberKnots];
                    if (numberKnots == 4) {
                        Preferences.debug(drawString +  "ellipse " + (i+1) +
                                          " has the required 4 knots\n");
                    }
                    else {
                        Preferences.debug(drawString +  "ellipse " + (i+1) +
                        " has " + numberKnots + " knots instead of the required 4\n");
                    }
                ...
                case 20: // closed polyline
                    numberKnots = getInt(endianess);
                    Preferences.debug(drawString +  "closed polyline " + (i+1) +
                                      " has " + numberKnots + " verticies\n");
                    knotX = new double[numberKnots];
                    knotY = new double[numberKnots];
                    for (j = 0; j < numberKnots; j++) {
                        knotX[j] =  getDouble(endianess);
                        Preferences.debug(drawString +  "closed polyline " + (i+1) +
                                         " has vertex" + (j+1) + "X = " + knotX[j] + "\n");
                        knotY[j] =  getDouble(endianess);
                        Preferences.debug(drawString +  "closed polyline " + (i+1) +
                                          " has vertex" + (j+1) + "Y = " + knotY[j] + "\n");
                    } // for (j = 0; j < numberKnots; j++)
                    if (drawingElement == BLEACH_ROI) {
                        bleachKnotX = new double[numberKnots];
                        bleachKnotY = new double[numberKnots];
                ...
                case 21: // open polyline
                    numberKnots = getInt(endianess);
                    Preferences.debug(drawString +  "open polyline " + (i+1) +
                                      " has " + numberKnots + " verticies\n");
                    knotX = new double[numberKnots];
                    knotY = new double[numberKnots];
                    for (j = 0; j < numberKnots; j++) {
                ...
                case 22: // closed Bezier spline curve
                    numberKnots = getInt(endianess);
                    Preferences.debug(drawString + "closed Bezier spline curve " + (i+1) +
                                      " has " + numberKnots + " knots\n");
                    knotX = new double[numberKnots];
                    knotY = new double[numberKnots];
                    for (j = 0; j < numberKnots; j++) {
                        knotX[j] =  getDouble(endianess);
                ...
                case 23: // open Bezier spline curve
                    numberKnots = getInt(endianess);
                    Preferences.debug(drawString + "open Bezier spline curve " + (i+1) +
                                      " has " + numberKnots + " knots\n");
                    knotX = new double[numberKnots];
                    knotY = new double[numberKnots];
                    for (j = 0; j < numberKnots; j++) {
                ...
                case 24: // circle
                    ...
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "circle " + (i+1) +
                                          " has the required 2 knots\n");
                    }
                    else {
                        Preferences.debug(drawString + "circle " + (i+1) +
                ...
                case 25: // rectangle with color palette
                    numberKnots = getInt(endianess);
                    if (numberKnots == 2) {
                        Preferences.debug(drawString + "rectangle with color palette " +
                                          (i+1) +  " has the required 2 knots\n");
                    }
                    else {
                ...
                case 26: // open polyline with arrow tip
                    numberKnots = getInt(endianess);
                    Preferences.debug(drawString + "open polyline with arrow tip " + (i+1) +
                                      " has " + numberKnots + " verticies\n");
                    knotX = new double[numberKnots];
                    knotY = new double[numberKnots];
                    for (j = 0; j < numberKnots; j++) {
                ...
                case 27: // open Bezier spline curve with arrow tip
                    ...
                    for (j = 0; j < numberKnots; j++) {
                        knotX[j] =  getDouble(endianess);
                        Preferences.debug(drawString +
                                         "open Bezier spline curve with arrow tip " + (i+1) +
                                         " has knot" + (j+1) + "X = " + knotX[j] + "\n");
                        knotY[j] =  getDouble(endianess);
                        Preferences.debug(drawString +
                                          "open Bezier spline curve with arrow tip " + (i+1) +
                ...
                case 28: // Two connected lines for angle measurement
                    ...
                    if (numberKnots == 3) {
                        Preferences.debug(drawString +
                                          "two connected lines for angle measurement " + (i+1) +
    ...
    */
    private void readOffsetChannelFactors() throws IOException {
        ...
        for (i = 0; i < nChannels; i++) {
            ...
            Preferences.debug("Factor[" + (i+1) + "] = " + factor[i] +
                              " in offsetChannelFactors\n");
            offset[i] = getDouble(endianess);
            Preferences.debug("Offset[" + (i+1) + "] = " + offset[i] +
                              " in offsetChannelFactors\n");
            unit[i] = getInt(endianess);
            Preferences.debug("Unit[" + (i+1) + "] = " + unit[i] +
                              " in offsetChannelFactors\n");
                            ...
                            while (j < nBytes) {
                                ...
                                if ( (byteBuffer[j] & 0x80) == 0) {
                                    ...
                                    for (iNext = 0; iNext < iCount; iNext++, j++) {
                                        if ( (x < xDim) && (y < yDim)) {
                                            if ( (i + progress) % mod == 0)
                                                progressBar.updateValue(Math.round( (float)
                                                    (i + progress) / progressLength * 100), false);
                                            buffer[x + y * xDim] = byteBuffer[j];
                                            i++;
                                        } // if ((x < xDim) && (y < yDim))
                                        x++;
                        ...
                        if (lzwCompression) {
                            ...
                            for (j = 0; j < decomp.length; j += 4) {
                                if ( (x < xDim) && (y < yDim)) {
                                    ...
                                    if (endianess) {
                                        buffer[x +
                                            y * xDim] = ( (b1 << 24) | (b2 << 16) | (b3 << 8) | b4); // Big Endian
                                    }
                                    else {
                                        buffer[x +
                                            y * xDim] = ( (b4 << 24) | (b3 << 16) | (b2 << 8) | b1); // Little Endian
                                    }
                        ...
                        if (lzwCompression) {
                            ...
                            for (j = 0; j < nBytes; j += 4) {
                                if ( (x < xDim) && (y < yDim)) {
                                    ...
                                    if (endianess) {
                                        buffer[x +
                                            y * xDim] = ( ( (long) b1 << 24) | ( (long) b2 << 16) |
                                            ( (long) b3 << 8) | (long) b4); // Big Endian
                                    }
                                    else {
                                        buffer[x +
                                            y * xDim] = ( ( (long) b4 << 24) | ( (long) b3 << 16) |
                                            ( (long) b2 << 8) | (long) b1); // Little Endian
                                    }
                            ...
                            if (lzwCompression) {
                                ...
                                for (j = 0; j < nBytes; j += 3) {
                                    if ( (x < xDim) && (y < yDim)) {
                                        ...
                                        buffer[4 * (x + y * xDim) + 2] = getUnsignedByte(byteBuffer, j + greenOffset);
                                        buffer[4 * (x + y * xDim) + 3] = getUnsignedByte(byteBuffer, j + blueOffset);
                                        i++;
                                    } // if ((x < xDim) && (y < yDim))
                                    x++;
                                    if (x == (xTile + 1) * tileWidth) {
                            ...
                            if (planarRGB < stripsPerImage) {
                                ...
                                for (j = 0; j < nBytes; j++, i += 4) {
                                    if ( (i / 3 + buffer.length / 3 + progress) % mod == 0)
                                        progressBar.updateValue(Math.round( (float) (i / 3 + buffer.length / 3 + progress) /
                                            progressLength * 100), false);
                                    if (greenOffset == 1) {
                                      buffer[i + 2] = getUnsignedByte(byteBuffer, j);
                                    }
