...
public class FileAnalyze
  ...
  /**
   *   Reads an analyze image file by reading the header then making a FileRaw to read the image
    ...
    if (fileInfo == null) { // if no file info yet, make it.
      ...
      if (!readHeader(fileInfo.getFileName(), fileInfo.getFileDirectory())) {
        throw (new IOException(
            "Cannot read image because of analyze header file error"));
      }
    }
  ...
  public boolean readHeader(String imageFileName, String fileDir) throws IOException {
    ...
    try {
      ...
      Preferences.debug("raFile = new RandomAccessFile(fileHeader, r) gave " +
    ...
    try {
      ...
      Preferences.debug("raFile.read(bufferImageHeader gave IOException " +
                          e + "\n");
    ...
    if (fileInfo.getSizeOfHeader() != headerSize) {
      Preferences.debug(
          "FileAnalyze:readHeader Analyze header length = " +
                       fileInfo.getSizeOfHeader()  +
    ...
    if (fileInfo.getRegular() != 'r') {
      Preferences.debug("fileInfo.getRegular() gave " + fileInfo.getRegular() +
                        " instead of expected r\n");
    }
    ...
    for (i = 0; i < dims; i++) {
      ...
      Preferences.debug("FileAnalyze:readHeader. Dimension " + (i + 1) + " = " +
                        analyzeExtents[i] + "\n");
    ...
    Preferences.debug("FileAnalyze:readHeader. Data type = " +
    ...
    Preferences.debug("FileAnalyze:readHeader. bits per pixel = " +
                      fileInfo.getBitPix() + "\n");
    ...
    for (i = 0,j = 0; i < dims+1; i++) {
      ...
      if ((i >= 1) && (analyzeExtents[i-1] > 1)) {
          ...
          Preferences.debug("FileAnalyze:readHeader. Resolutions " + (j + 1) +
                        " = " + resolutions[j] + "\n");
          j++;
      }
    }
    ...
    Preferences.debug("FileAnalyze:readHeader. global max intensity = " +
                      fileInfo.getGLmax() + "\n");

    fileInfo.setGLmin(getBufferInt(bufferImageHeader, 144, endianess));
    Preferences.debug("FileAnalyze:readHeader. global min intensity = " +
                      fileInfo.getGLmin() + "\n");
  ...
  private boolean writeHeader(ModelImage image, int nImagesSaved,
    ...
    Preferences.debug("FileAnalyze:writeHeader - nImagesSaved = " +
                      nImagesSaved + "\n", 2);
    ...
    for (int i = 0; i < analyzeExtents.length; i++) {
      Preferences.debug("FileAnalyze:writeHeader - i = " + i + " dim = " +
                        analyzeExtents[i] + "\n", 2);
    }
    ...
    if (!simple) { // Must be an Analyze file, can set all Analyze information based on fileInfo
      ...
      for (int i = 0; i < analyzeExtents.length; i++) {
        setBufferShort(bufferImageHeader, (short) analyzeExtents[i], 40 + i * 2,
                       endianess);
      }
      ...
      // MIPAV is hacking the analyze format to use the unused
      // variables to store axis orientation.
      setBufferShort(bufferImageHeader, (short) myFileInfo.getAxisOrientation()[0], 64,
                     endianess);
      setBufferShort(bufferImageHeader, (short) myFileInfo.getAxisOrientation()[1], 66,
                     endianess);
      setBufferShort(bufferImageHeader, (short) myFileInfo.getAxisOrientation()[2], 68,
                     endianess);

      Preferences.debug("FileAnalyze:writeHeader(simple): data type = " +
                        fileInfo.getDataTypeCode() + "\n", 2);
      setBufferShort(bufferImageHeader, fileInfo.getDataTypeCode(), 70, endianess);
      switch (image.getType()) {
        case ModelStorageBase.BOOLEAN:
      ...
      Preferences.debug("FileAnalyze:writeHeader(simple): bits per pixel = " +
                        fileInfo.getBitPix() + "\n", 2);
      ...
      for (int i = 0; i < nDims; i++) {
        setBufferFloat(bufferImageHeader, fileInfo.getResolutions()[i], 80 + i * 4,
                       endianess);
      }
  ...
  */
   ...
   *   voxUnits from an Analyze Header
