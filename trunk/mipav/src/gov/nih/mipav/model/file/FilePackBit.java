...
public class FilePackBit extends FileBase {
    ...
    *  @return            the buffer
    */
    public byte[]    getByteBuffer()    { return bufferByte;}

    /**
    *  Gets the short buffer of the image
    *  @return            the buffer
    */
    public short[]   getShortBuffer()   { return bufferShort;}
    ...
    *                     Note that bufferSize is also in units of extents[0]*extents[1]
    *                     This method only applies to BYTE or UBYTE data.
    */
        ...
        switch (image.getType()) {
            ...
            case ModelStorageBase.BYTE:
                try {
                    ...
                    for (i = 0; i < bufferSize; i++) {
                        if (dicom)  tmpByte = (byte)((bufferByte[i]-intercept)*invSlope);
                        else        tmpByte = bufferByte[i];
                    ...
                    for (iy = 0; iy < yDim; iy++) {
                        ...
                        for (ix = 2; ix < xDim;) {
                            if (((repLength < 128) && (litLength < 128)) ||
                              ...
                              if ((repRun == true) && (bufferByte[iStart+ix] == bufferByte[iStart+ix-1])) {
                              ...
                              else if ((repRun == true) && (bufferByte[iStart+ix] != bufferByte[iStart+ix-1])) {
                                iCount = iCount + 2;
                                ...
                                else if ((ix == (xDim - 2)) && (bufferByte[iStart+xDim-2] == bufferByte[iStart+xDim-1])) {
                                    iCount = iCount + 2;
                                    ix = ix + 2;
                                ...
                                else if ((ix < (xDim -2)) && (bufferByte[iStart+ix] == bufferByte[iStart+ix+1])) {
                                        repLength = 2;
                                        ix = ix + 2;
                                }
                                else if ((ix < (xDim - 2)) && (bufferByte[iStart+ix] != bufferByte[iStart+ix+1])) {
                              ...
                              else if ((repRun == false) && (bufferByte[iStart+ix] != bufferByte[iStart+ix-1])) {
                                litLength++;
                                ix++;
                              }
                              else if ((repRun == false) && (bufferByte[iStart+ix] == bufferByte[iStart+ix-1])) {
            ...
            case ModelStorageBase.UBYTE:
                try {
                    ...
                    for (i = 0; i < bufferSize; i++) {
                        if (dicom)  tmpShort = (short)((bufferShort[i]-intercept)*invSlope);
                        else        tmpShort = bufferShort[i];
