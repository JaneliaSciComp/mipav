...
public class FileMicroCat extends FileBase  {
    ...
    */
    ...
    /**
    *   Reads lines of the file and strips comments indicated by the : symbol
    *   until a nonnull String results or the end of the file is reached
    ...
    public ModelImage readImage(boolean quiet, boolean one, String inputName) throws IOException {
        ...
        try {
            ...
            progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                    	    ViewUserInterface.getReference().getProgressBarPrefix() + "Micro Cat image(s) ...", 0, 100,
                                    	    false, null,null);
    ...
    */
    public String[] getFileList(String fileDir, String fileName, boolean quiet) {
        ...
        nImages = j;
        if (nImages == 0) {
            if (!quiet) {MipavUtil.displayError("FileMicroCat: No images that start with " + subName + " and end with .ct");}
            return null;
        }
