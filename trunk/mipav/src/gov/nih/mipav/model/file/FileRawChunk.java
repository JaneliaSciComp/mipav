...
public class FileRawChunk extends FileBase {
    ...
    public final static int RRRGGGBBB = 1;
                ...
                try {
                    ...
                    if (endianess == BIG_ENDIAN) {
                        for (i = 0, index = 0; i < bufferSize; i++ , index += 2) {
                            ...
                            // OR
                            // Fastest
                            bufferShort[i] = (short) ( ( (bufferByte[index] & 0x00ff) << 8) | (bufferByte[index + 1] & 0x00ff));
                            // System.out.println ("here rawchunck.readimagebig = " + bufferShort[i]);
                        }
                    } else {
                        for (i = 0, index = 0; i < bufferSize; i++ , index += 2) {
                            bufferShort[i] = (short) ( ( (bufferByte[index + 1] & 0x00ff) << 8) | (bufferByte[index] & 0x00ff));
     ...
     * @param fileIndex index into FileInfo array; mostly used for DICOM and Minc Note that bufferSize is also in units
     *            of extents[0]*extents[1]
     */
    public void writeImage(ModelImage image, int start, int end, int fileIndex) throws IOException {
        ...
        switch (image.getType()) {
            ...
            case ModelStorageBase.ARGB_USHORT:
                 ...
                 * else { try { image.exportData(4*start, 4*bufferSize, bufferInt); if (endianess == BIG_ENDIAN) { for
                 * (i = 0, j = 0; i < 4*bufferSize; i+=4, j+=1) { bufferByte[j+2*bufferSize] = (byte)(bufferInt[i+1] >>>
