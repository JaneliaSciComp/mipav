...
public class FileMincHDF extends FileBase {
	...
	for (int i = 0; i < infoNode.getChildCount(); i++) {
		...
		Iterator<Attribute> it = ((HObject)currentInfoNode.getUserObject()).getMetadata().iterator();
		while(it.hasNext()) {
		    Attribute attr = it.next();
		    String attrName = attr.getName();
	...
	for (int i = 0; i < childNode.getChildCount(); i++) {
		...
		List<Attribute> imageMetaData = ((HObject)currentNode.getUserObject()).getMetadata();
		Iterator<Attribute> it = imageMetaData.iterator();
		while(it.hasNext()) {
		    Attribute imageAttr = it.next();
	...
	Hashtable<FileDicomKey,FileDicomTag> dTable = fileInfo.getDicomTable();
	Enumeration<FileDicomKey> e = dTable.keys();
