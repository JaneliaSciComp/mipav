...
*   As the parser reads the DICOM dictionary file, it stores new <code>DicomTags
*   </code> in a standard Hashtable.  Then when the DICOM file reader reads a
*   new file, it sets the value attribute of the DicomTag.  The other
*   constructor is used for private tags, because private tags do not have
*   unique attributes.  Thus, the reader can read and display all the tags in a
*   file, but may not read the data properly and does not know the significance
*   of the data.  If the user wishes to know more information about the file
*   that is encoded in the private tags, then he or she needs to edit the
*   DICOM dictionary file to include the necessary private tags.
*
...
public class FileDicomTag implements Serializable, Cloneable {
    ...
    /** Version of the tag.
     *  Usually "2" or "3", although some tags
     *  get strings like "GEM" or "TSH" when the
     *  tag is from a private manufacturer.
     */
    String version;
    ...
    /**
     *   Constructor that takes all the information needed about the tag
     *   except for the value and DICOM revision number
    ...
    public FileDicomTag(FileDicomKey dicomKey, String vr, int vm, String keyword, String name ) {
        this(   dicomKey.getGroupNumber(), dicomKey.getElementNumber(),
                vr, vm, keyword, name);
    }
    ...
    */
    ...
    /**
    *   Constructor that creates a DicomTag empty except for the name field.
    *   this is used for private tags and the name field is the supposed value
    *   of the tag.
    *   @param group    the integer group word (in hexadecimal)
    *   @param element  the integer element word (in hexadecimal)
    ...
    */
    ...
    /**
    *   Sets the value attribute of the DicomTag.  Does NOT parse so
    *   stored value is the same as the one read in.  getValue does
    *   parse.
    *   <p>NOTE: DICOM compliant codes will be accepted.
    ...
    */
    ...
    /**
    *   Sets the value and length attributes of the DicomTag.  Parses value so
    *   the stored value is converted from a standard, English-readable string to
    *   a DICOM v3 form.  If the parsing routine is unavailable, the
    *   value gets stored <b>as is</b>.  Sending strings to set the value of
    *   tags of other types (eg., short) will attempt to convert the string
    *   to the correct type.  Attempting to store a value which is too large
    *   for the given type will throw an exception.  In any case, the proper
    *   size is used.
    *   <p>
    *   For tags with neither value representation nor keyword (a private tag),
    *   this method ignores the value.  To add, the user class <b>must</b> use
    *   setValue().  typeSequence and typeUnknown must also explicitly use
    *   setValue().
    *
    *   @param value  the value to store
    */
    ...
    */
    ...
    /**
    *   Accessor that returns the value multiplicity of the tag.  The
    *           value multiplicity is how many instances of this value representation (VR)
    ...
    */
    ...
    /** Accessor that returns the DICOM version of the tag.  Although not
     * needed to understand a value, it is both important to the
     * dictionary (@see CreateDICOMFiles) .
     ...
     * Returns it as a number so numerical comparisons (greater-than,
     * equality, etc) can be made.  Also, throws a NumberFormatException
     * for private tags.
    ...
    /**
    *   Accessor that returns the real world name of this tag.  For
    *             private tags, the name is the value.
    ...
    */
    ...
    /**
    *   Accessor that returns the value representation of the tag.  The
    *   value representation allows the reader the read and interpret the
    *   tag properly.  Because private tags are not unique, the VR is null
    *   and they may be read and/or displayed improperly.
    ...
    */
    ...
    /** This <b>hack</b> Calculates the number of bytes that the data (the object value) takes
     * to be stored.  This method returns the number of data items times the
     * sizeof the data type.  This method will be <b>REMOVED</b> when (if)
     * the tags are seperated out as individual classes.  <i>Calls sizeof(String)</i>
    ...
    /** This <b>HACK</b> Gets the data size in bytes of the units held in this class.
     * This method will be <b>removed</b> when (if)
     * the tags are seperated out as individual classes.
     */
     ...
     * This method will be so much simpler when (if)
     * the tags are seperated out as individual classes.
     */
    public int sizeof() {
        int retval = 0;
        ...
        else if (   type.equalsIgnoreCase("otherByteString") ||
                    type.equalsIgnoreCase("typeUnknown") )
        {
            Object val[] = getValueList();
            if (val instanceof Byte[]) {
                retval = 1;
    ...
    /**
    *   Accessor that returns the type of tag, depending on the value
    *   representation.  This allows the reader to read in the tags
    *             properly.
     * This method will be so much simpler when (if)
     * the tags are seperated out as individual classes.
    ...
    */
    public static String getType(String vr) {
        try {
           ...
           else return "typeUnknown"; // unknown
        } catch (NullPointerException npe) {
           return "typeUnknown";
        }
    }
    ...
    /**
    *   Translates the tag value into an understandable string so
    *   no other class need understand how to parse this information.
    *   However, values with more than one multiple (vm > 1) are still
    *   encoded with "\" (as in unprocessable DICOM tags) or " \ " as separators.
    *   A single value may then be de-coded using StringBuffer(str, "\"),
    *   and ...nextToken().
    *   <p>
    *   Eg., The age is encoded as '014Y', so getValue() returns '14 Years'.
    *   Coded strings such as patient sex is 'F', which returns 'Female'
    *   <p>
    *   An additional note:  in order to write a DICOM image correctly, the value
    *   should NOT be parsed into readable form.
    *   @param parse  boolean indicating if we should parse (translate) the value
    ...
    */
    ...
    /** Returns the value(s) as an array so that each tag with a value
     *  multiplicity of more than 1 -- ie, TypeString items separated by '\'
     *  -- is its own element in the array.
     * This method will be so much simpler when (if)
     * the tags are seperated out as individual classes.
     */
    public Object[] getValueList() {
        ...
        try {
            ...
            System.out.print("\tFileDicomTag.getValueList(): ??--" + getKeyword()
                             + "--cannot be dealt with.\n");
            Preferences.debug("\"" + getKeyword() + "\" cannot be found as a list;" +
                              " this may be an error.  \n");
        }
        return stuff;
    }

    /** Provides the number of items held by the tag, as opposed the value
     *  specified by Value Multiplicity(VM).  While a VM may specify an
     *  unlimited  number of values, this represents the number of values
     *  actually held.  This is numerically equivalent to the value of
     *  <code>getValueList().length</code>.  Formerly getMultiplicity().
     */
    ...
    /**
    *   This allows us the copy the value of the DicomTag instead of just
    *   the reference.  This is especially important when setting up the
    *   Hashtables in FileInfoDicom.
    *   @return the new DicomTag
    *   @see    FileInfoDicom
    */
    public Object clone() {
        FileDicomTag tag = new FileDicomTag(this.group, this.element,
                       this.version, this.vr, this.vm, this.keyword, this.name);
    ...
    /**
    *   Used for debugging so that the information contained in the tag
    *   can be converted to a readable form.
    *   @return     The tag in readable form
    */
    public String toString() {
        String s = "";
        s = "Group: " + Integer.toString(this.group, 0x10) +
            " Element: " + Integer.toString(this.element, 0x10);
        if (version != null) s+= " VER: "+this.version;
        if (vr != null) s+= " VR: " + this.vr;
        s+= " VM: " + this.vm;
        if (keyword != null) s += " Keyword: " + this.keyword;
        if (name != null)    s += " Name: " + this.name;
        if (value != null)   s += " Value: " + this.value;
        s += " Length: " + Integer.toString(length, 0x10);
        return s;
    }

    /**
    *   Takes the input string and forms a DICOM compliant
    *   decimal string out of it.  Allows multiple values
    ...
    *   The following limitations apply: has to start with numbers, then
    *   follow-up with the word 'days', 'months', 'years' irrespective of
    *   capitalization;  a number may be no larger than 3 digits, or only
    *   the top three digits are accepted.
    ...
    */
    ...
    /**
     * Converts the given string into a DICOM-legal value for a DA
     * value-representation (VR) tag.
     * places a date (yyyymmdd) into value that is in either
     * yyyymmdd (DICOM v3) or yyyy.mm.dd (< DICOM v3) or
     * mm/dd/yyyy format. Assumes that the date is reasonable
     * okay to begin with.  Saves value in DICOM v3 format
     * as a yyyymmdd (DICOM v3).
     *
     *  @param tempValue -- the string that represents the date.  In American std mm/dd/yyyy
     *                      or DIOCM's yyyymmdd or yyyy.mm.dd
        ...
        while (backslashTok.hasMoreTokens() && (multiple <= vm)) {
            ...
            if (temp.length() >= 2) {
                ...
                if ((Integer.parseInt(timeField.toString()) < 0) ||
                    (Integer.parseInt(timeField.toString()) >= 24)) {
                        return "";
                }
    ...
    */
    ...
    /**
    *   From age string to something that makes more sense.
    *   ages are DICOM as nnnT.  That is, three digits preceeeding
    *   a letter indicating the time unit; valid units are D (day),
    *   M (month), and Y (year).
    *   <p>
    ...
    */
        ...
        while (backslashTok.hasMoreTokens() && (multiple <= vm)) {
            ...
            if (age != null && age.length() > 3) {
                try {
                    ...
                    output.append(" ");
                    switch(age.charAt(3)) {
                        case 'D': case 'd': output.append("Days");      break;
                        case 'M': case 'm': output.append("Months");    break;
                        case 'Y': case 'y': output.append("Years");     break;
                    }
                }
    ...
    /**
     *  Takes a DICOM tag with a TM value representation and returns it
     *  in the format HH:MM:SS.frac, or as much as is available to convert
     *  (ie, HH:MM could be all that is currently stored).
     *
     ...
     *  "Male", "M", "Female","F", "Other" and "O" are valid inputs.
     *  Any other input is converted to an empty string (which is valid, since
     *  the Patient Sex tag is Type 2, that is, required to exist, but no
     *  value is required) and returned.
     */
    ...
    /**
     *  Converts a DICOM patient sex tag (eg, "M") into a more understandable
     *  value (eg, "Male").
     *  "M", "F", and "O" are valid inputs.
     *  Any other value representation (VR) is converted to "Other" but
     *  an empty string (which is valid, since the Patient Sex tag is Type 2,
     *  that is, required to exist, but no value is required) returns an empty
     *  string.
     */
    ...
    /** Converts a DICOM person's name (&quot;PN&quot;) tag into a more
     *  understandable value.  Used so the '^' word-separation character
     *  isn't mistakenly removed.
     */
    ...
    private String fromPatientOrientationToVisibleString() {
        ...
        if      (first.equals("A")) s = "Anterior";
        else if (first.equals("P")) s = "Posterior";
        else if (first.equals("R")) s = "Right";
        else if (first.equals("L")) s = "Left";
        else if (first.equals("H")) s = "Head";
        else if (first.equals("F")) s = "Foot";

        s += " ";

        if      (second.equals("A")) s = s + "Anterior";
        else if (second.equals("P")) s = s + "Posterior";
        else if (second.equals("R")) s = s + "Right";
        else if (second.equals("L")) s = s + "Left";
        else if (second.equals("H")) s = s + "Head";
        else if (second.equals("F")) s = s + "Foot";
    ...
    private String fromVisibleStringToPatientOrientation(String tmpValue) {
        ...
        if      (first.equals("Anterior") || first.equals("A"))  s = "A";
        else if (first.equals("Posterior") || first.equals("P")) s = "P";
        else if (first.equals("Right") || first.equals("R"))     s = "R";
        else if (first.equals("Left") || first.equals("L"))      s = "L";
        else if (first.equals("Head") || first.equals("H"))      s = "H";
        else if (first.equals("Foot") || first.equals("F"))      s = "F";

        s += "\\";

        if      (second.equals("Anterior") || second.equals("A"))  s = s + "A";
        else if (second.equals("Posterior") || second.equals("P")) s = s + "P";
        else if (second.equals("Right") || second.equals("R"))     s = s + "R";
        else if (second.equals("Left") || second.equals("L"))      s = s + "L";
        else if (second.equals("Head") || second.equals("H"))      s = s + "H";
        else if (second.equals("Foot") || second.equals("F"))      s = s + "F";
