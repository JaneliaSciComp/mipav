 ...
 *   The header is intended to be "mostly compatible" with the ANALYZE (TM)
 *   7.5 file format. Most of the "unused" fields in that format have been
 *   taken, and some of the lesser-used fields have been co-opted for
 *   other purposes.
 *
 *   MIPAV only has 1 transformation matrix associated with an image.  NIFTI
 *   can have 2 different transformaton matrices associated with an image -
 *   one stored in the qform_code parameters and one stored in the sform_code
 *   parameters.  While MIPAV separately stores axis orientation and matrix
 *   information, NIFTI does not store axis orientation information.  NIFTI
 *   uses a routine to derive axis orientations from the upper 3 by 3 parameters
 *   of the 4 by 4 matrix.  The 4 by 4 matrix in NIFTI transforms i,j,k indexes
 *   to (x,y,z) coordinates where +x = Right, +y = Anterior, +z = Superior.
 *   In MIPAV the 4 by 4 matrix does not imply the axis orientations.
 *
 *   MIPAV ANALYZE uses 6 for 16 bit unsigned short in the datatype field while
 *   NIFTI uses 512 for 16 bit unsigned short in the datatype field.  NIFTI also
 *   has a signed char at 256, an unsigned int at 768, a LONG at 1024,
 *   an unsigned long at 1280, an 128 bit float at 1536, a 128 bit COMPLEX at 1792,
 *   and a 256 bit COMPLEX at 2048 which are not present in ANALYZE.
 *   MIPAV cannot presently handle a 128 bit float or a 256 bit COMPLEX.
 *   If scl_slope != 1.0 or scl_inter != 0.0, then source data types
 *   will be promoted to higher data types if necessary.
 *
 *   At location 56 our hacked anlayze has a 4 byte string for voxel units,
 *   while NIFTI has the float intent_p1.  At location 60 our hacked analyze
 *   has a 4 byte string for cal units, while NIFTI has the float intent_p2.
 *   At locations 64 and 66 our hacked analyze has shorts for axis orientations
 *   0 and 1, while NIFTI has the float intent_p3.  At location 68 our hacked
 *   analyze has the short for axis orientation 2, while NIFTI has the short
 *   for intent_code.
 *
 *   At location 76 both ANALYZE and NIFTI have 8 floats for resolutions.  In
 *   ANALYZE pixdim[0] at location 76 is unused, but in NIFTI if (pixdim[0] >= 0)
 *   qfac is taken as 1.  Otherwise, if (pixdim[0] < 0) in NIFTI qfac is taken as -1.
 *   qfac is used in determining the transformation matrix in method 2.
 *   In both ANALYZE and NIFTI the x resolution pixdim[1] is found at location 80,
 *   the y resolution pixdim[2] is found at location 84, the z resolution pixdim[3]
 *   is found at location 88, and the t resolution pixdim[4] is found at location 92.
 *   In NIFTI L to R, P to A, and I to S are defined as positive directions.
 *   This differs from AFNI and MIPAV where R to L, A to P, and I to S are positive.
 *   ANALYZE 7.5 has R to L, P to A, and I to S positive.  Thus, NIFTI, AFNI, and
 *   MIPAV have right handed coordinate systems, while ANALYZE has a left handed
 *   coordinate system.
 *
 *   At location 112 our hacked analyze has a float for origin[0],
 *   while NIFTI has the float scl_slope.  At location 116 our hacked
 *   analyze has a float for origin[1], while NIFTI has the float
 *   scl_inter.  The data is scaled according to:
 *   scaled_data[i] = scl_slope * unscaled_data[i] + scl_inter
 *   At location 120 our hacked analyze has a float for origin[2],
 *   while the NIFTI location 120 has short slice_end, location 122 has char
 *   slice_code, and location 123 has char xyzt_units.  NIFTI has qoffset_x, qoffset_y,
 *   and qoffset_z in locations 268, 272, and 276.
 *
 *
 *      @see        FileIO
 *      @see        FileInfoNIFTI
...
public class FileNIFTI
    ...
    /**
     *   Method determines if the image format is NIFTI,
     *   so FileIO will call the appropriate FileNIFTI read method
    ...
    /**
     *   Reads a NIFTI image file by reading the header then making a FileRaw to read the image
    ...
    public ModelImage readImage(boolean one) throws IOException, OutOfMemoryError {
        ...
        try {
            if (one) {
                ...
                image = new ModelImage(fileInfo.getDataType(), fileInfo.getExtents(),
                                       fileInfo.getFileName(), UI);
            }
        }
        ...
        try { // Construct a FileRaw to actually read the image.
            ...
            if ( (scl_slope != 0.0) && ( (scl_slope != 1.0f) || (scl_inter != 0.0f)) &&
                ...
                if (doChangeType) {
                    ...
                    changeTypeAlgo = new AlgorithmChangeType(image, newType, imageMin, imageMax,
                        imageMin, imageMax, false);
     ...
     *   Image is not 'flipped', and neither units of measure nor orientation are set.
        ...
        if (fileInfo == null) { // if no file info yet, make it.
            ...
            if (!readHeader(fileInfo.getFileName(), fileInfo.getFileDirectory())) {
                throw (new IOException(
                    "Cannot read image because of NIFTI header file error"));
            }
        }
    ...
    public boolean readHeader(String imageFileName, String fileDir) throws IOException {
        ...
        if (fileInfo.getSizeOfHeader() != headerSize) {
            Preferences.debug(
                "FileNIFTI:readHeader NIFTI header length != 348.\n", 2);
            return false;
        }
        ...
        for (i = 0; i < dims; i++) {
            ...
            Preferences.debug("FileNIFTI:readHeader. Dimension " + (i + 1) + " = " +
                              niftiExtents[i] + "\n", 2);
            if (niftiExtents[i] > 1) {
                numDims++;
            }
        ...
        Preferences.debug("FileNIFTI:readHeader. intentP1 = " +
                          fileInfo.getIntentP1() + "\n");
        intentP2 = getBufferFloat(bufferByte, 60, endianess);
        fileInfo.setIntentP2(intentP2);
        Preferences.debug("FileNIFTI:readHeader. statPar2 = " +
                          fileInfo.getIntentP2() + "\n");
        intentP3 = getBufferFloat(bufferByte, 64, endianess);
        fileInfo.setIntentP3(intentP3);
        Preferences.debug("FileNIFTI:readHeader. intentP3 = " +
                          fileInfo.getIntentP3() + "\n");
        ...
        switch (intentCode) {
            ...
            case FileInfoNIFTI.NIFTI_INTENT_FTEST:
                ...
                Preferences.debug("Numerator degrees of freedom = " +
                                  Math.round(intentP1) + "\n");
                Preferences.debug("Denominator degrees of freedom = " +
                                  Math.round(intentP2) + "\n");
        ...
        Preferences.debug("FileNIFTI:readHeader. source bits per pixel = " +
                          sourceBitPix + "\n", 2);
        ...
        for (i = 0, j = 0; i < dims + 1; i++) {
            ...
            if ( (i >= 1) && (niftiExtents[i - 1] > 1)) {
                ...
                Preferences.debug("FileNIFTI:readHeader. Resolutions " + (j + 1) +
                                  " = " + resolutions[j] + "\n", 2);
                j++;
            }
        }
        ...
        fileInfo.setModality(FileInfoBase.getModalityFromStr(fileInfo.
            getDescription()));
        ...
        if (numDims >= 3) {
            ...
            if (qform_code > 0) {
                ...
                Preferences.debug("axisOrientation = " + axisOrientation[0] + "  " +
                ...
                if ((axisOrientation[0] == FileInfoBase.ORI_I2S_TYPE) ||
                   ...
                   if (origin[2] > 0 ) {
                        newOrigin[2] = origin[2] - (extents[0] - 1) * resolutions[0];
                    } else {
                        newOrigin[2] = origin[2] + (extents[0] - 1) * resolutions[0];
                    }
                }
                    ...
                    newOrigin[1] = origin[1];
                    if (origin[2] > 0 ) {
                        newOrigin[2] = origin[2] - (extents[1] - 1) * resolutions[1];
                    } else {
                        newOrigin[2] = origin[2] + (extents[1] - 1) * resolutions[1];
                    }
                }
                    ...
                    newOrigin[0] = origin[0];
                    if (origin[1] > 0 ) {
                        newOrigin[1] = (origin[1] - (extents[1] - 1) * resolutions[1]);
                    } else {
                        newOrigin[1] = (origin[1] + (extents[1] - 1) * resolutions[1]);
                    }
                ...
                matrix.setMatrix( (double) newOrigin[0], 0, 3);
                matrix.setMatrix( (double) newOrigin[1], 1, 3);
                matrix.setMatrix( (double) newOrigin[2], 2, 3);
                ...
                // Convert from NIFTI RAS to DICOM LPS
                newOrigin = new float[3];
                if ((axisOrientation[0] == FileInfoBase.ORI_I2S_TYPE) ||
                   (axisOrientation[0] == FileInfoBase.ORI_S2I_TYPE)) {
                   newOrigin[0] = origin[0];
                ...
                Preferences.debug("srow_x = " + srow_x[0] + "  " + srow_x[1] + "  " +
                                  srow_x[2] + "  " + srow_x[3] + "\n");
                Preferences.debug("srow_y = " + srow_y[0] + "  " + srow_y[1] + "  " +
                                  srow_y[2] + "  " + srow_y[3] + "\n");
                Preferences.debug("srow_z = " + srow_z[0] + "  " + srow_z[1] + "  " +
                                  srow_z[2] + "  " + srow_z[3] + "\n");
            } // else if (sform_code > 0)
        } // if (numDims >= 3)
    ...
    // Polar decomposition of a 3x3 matrix: finds the closest orthogonal matrix to
    // input A (in both Frobenius and L2 norms).  Algorithm is that from
    // NJ Higham, SIAM JSci Stat Comput, 7:1160-1174.
    ...
    /**
     *   Helper method to calculate the offset for getting only the middle NIFTI
     *   image slice from the 3D file.
     *   @param fileInfo File info.
     *   @return         offset
     */
    private int getOffset(FileInfoNIFTI fileInfo) {
        int offset = fileInfo.getExtents()[0] * fileInfo.getExtents()[1] *
            (fileInfo.getExtents()[2] / 2);
     ...
     *   @see              FileRaw
     */
    public void writeImage(ModelImage image, FileWriteOptions options) throws
        IOException {
        String fhName;
        int index;
        int nImagesSaved;
     ...
     *   @see                   FileInfoNIFTI
     */
    private boolean writeHeader(ModelImage image, int nImagesSaved,
                                int nTimeSaved,
                                String fileName, String fileDir) throws
        IOException {
        ...
        for (i = 0; (i < unitsOfMeasure.length) && (!found); i++) {
            if ( (unitsOfMeasure[i] == FileInfoBase.UNKNOWN_MEASURE) ||
                (unitsOfMeasure[i] == FileInfoBase.INCHES) ||
                ...
                (unitsOfMeasure[i] == FileInfoBase.METERS) ||
                (unitsOfMeasure[i] == FileInfoBase.KILOMETERS) ||
                (unitsOfMeasure[i] == FileInfoBase.MILES)) {
                found = true;
                firstSpatialDim = i;
        ...
        for (i = 0; (i < unitsOfMeasure.length) && (!found); i++) {
            if ( (unitsOfMeasure[i] == FileInfoBase.NANOSEC) ||
                ...
                (unitsOfMeasure[i] == FileInfoBase.MINUTES) ||
                (unitsOfMeasure[i] == FileInfoBase.HOURS) ||
                (unitsOfMeasure[i] == FileInfoBase.HZ) ||
                (unitsOfMeasure[i] == FileInfoBase.PPM) ||
        ...
        Preferences.debug("FileNIFTI:writeHeader - nImagesSaved = " +
                          nImagesSaved + "\n", 2);
        ...
        for (i = 0; i < niftiExtents.length; i++) {
            Preferences.debug("FileNIFTI:writeHeader - i = " + i + " dim = " +
                              niftiExtents[i] + "\n", 2);
        }
        ...
        else if (!isNIFTI) { // not originally NIFTI
            axisOrientation = image.getFileInfo(0).getAxisOrientation();
            if ( (axisOrientation != null) && (axisOrientation[0] != FileInfoBase.ORI_UNKNOWN_TYPE) &&
                (axisOrientation[1] != FileInfoBase.ORI_UNKNOWN_TYPE) &&
                (axisOrientation[2] != FileInfoBase.ORI_UNKNOWN_TYPE)) {
                ...
                matrix.set(2, 2, 0.0);
                newOrigin = new float[3];
                if (image.getFileInfo(extents[2] - 1).getOrigin()[2] >=
                ...
                switch (axisOrientation[0]) {
                    ...
                    case FileInfoBase.ORI_A2P_TYPE:
                        matrix.set(1, 0, resols[0]);
                        newOrigin[1] = origin[0] + (extents[0] - 1) * resols[0];
                        break;
                    case FileInfoBase.ORI_P2A_TYPE:
                        matrix.set(1, 0, -resols[0]);
                        newOrigin[1] = origin[0];
                        break;
                    case FileInfoBase.ORI_I2S_TYPE:
                        matrix.set(2, 0, resols[0]);
                        newOrigin[2] = origin[0];
                        break;
                    case FileInfoBase.ORI_S2I_TYPE:
                        matrix.set(2, 0, -resols[0]);
                        newOrigin[2] = origin[0] + (extents[0] - 1) * resols[0];
                        break;
                } // switch(axisOrientation[0])
                ...
                switch (axisOrientation[1]) {
                    case FileInfoBase.ORI_R2L_TYPE:
                        matrix.set(0, 1, -resols[1]);
                        newOrigin[0] = origin[1] + (extents[1] - 1) * resols[1];
                        break;
                    case FileInfoBase.ORI_L2R_TYPE:
                        matrix.set(0, 1, resols[1]);
                        newOrigin[0] = origin[1];
                        break;
                    case FileInfoBase.ORI_A2P_TYPE:
                        matrix.set(1, 1, -resols[1]);
                        newOrigin[1] = origin[1] + (extents[1] - 1) * resols[1];
                    ...
                    case FileInfoBase.ORI_I2S_TYPE:
                        matrix.set(2, 1, resols[1]);
                        newOrigin[2] = origin[1];
                        break;
                    case FileInfoBase.ORI_S2I_TYPE:
                        matrix.set(2, 1, -resols[1]);
                        newOrigin[2] = origin[1] + (extents[1] - 1) * resols[1];
                        break;
                } // switch(axisOrientation[1])
                ...
                switch (axisOrientation[2]) {
                    case FileInfoBase.ORI_R2L_TYPE:
                        matrix.set(0, 2, -resols[2]);
                        if (increaseResol) {
                            newOrigin[0] = origin[2] + (extents[2] - 1) * resols[2];
                        }
                        else {
                            newOrigin[0] = origin[2] - (extents[2] - 1) * resols[2];
                        }
                    ...
                    case FileInfoBase.ORI_L2R_TYPE:
                        matrix.set(0, 2, resols[2]);
                        newOrigin[0] = origin[2];
                        break;
                    case FileInfoBase.ORI_A2P_TYPE:
                        matrix.set(1, 2, -resols[2]);
                        if (increaseResol) {
                            newOrigin[1] = origin[2] + (extents[2] - 1) * resols[2];
                        }
                        else {
                            newOrigin[1] = origin[2] - (extents[2] - 1) * resols[2];
                        }
                    ...
                    case FileInfoBase.ORI_S2I_TYPE:
                        matrix.set(2, 2, -resols[2]);
                        if (increaseResol) {
                            newOrigin[2] = origin[2] + (extents[2] - 1) * resols[2];
                        }
                        else {
                            newOrigin[2] = origin[2] - (extents[2] - 1) * resols[2];
                        }
        ...
        if (isNIFTI) { // Must be a NIFTI file, can set all NIFTI information based on fileInfo
            ...
            for (i = 0; i < niftiExtents.length; i++) {
                setBufferShort(bufferByte, (short) niftiExtents[i], 40 + i * 2,
                               endianess);
            }
            ...
            for (i = 0; i < nDims; i++) {
                setBufferFloat(bufferByte, niftiResols[i], 80 + i * 4,
                               endianess);
            }
            ...
            Preferences.debug("FileNIFTI:writeHeader(simple): data type = " +
                              sourceType + "\n", 2);
            setBufferShort(bufferByte, sourceType, 70, endianess);
            switch (image.getType()) {
                case ModelStorageBase.BOOLEAN:
            ...
            Preferences.debug("FileNIFTI:writeHeader(simple): bits per pixel = " +
                              fileInfo.getBitPix() + "\n", 2);
    ...
    /**
     * This method is used when saving a 3D image in an array of 2D files. The file name has
     * numbers appended to correctly order the images.
     ...
     */
    private void writeHeader3DTo2D(ModelImage image, String fileName, String fileDir, FileWriteOptions options) throws
        IOException {
        int k, seq;
        int beginSlice = options.getBeginSlice();
        int endSlice = options.getEndSlice();
    ...
    /**
     * This method is used when saving a 4D image in an array of 3D files. The file name has
     * numbers appended to correctly order the images.
     * @param image the image dataset to be saved
     * @param fileName the file name
     * @param fileDir  the file directory
     * @param options  file options indicate how to save the image
     */
    private void writeHeader4DTo3D(ModelImage image, String fileName,
                                   String fileDir, FileWriteOptions options) throws IOException {
        int k, seq;
    ...
    public void scale(ModelImage image) throws IOException {
        try {
            ...
            if (image.getNDims() == 5) {
                nBuffers = image.getExtents()[4] * image.getExtents()[3] *
                    image.getExtents()[2];
            ...
            if (image.isColorImage()) {
                ...
                for (k = 0; k < nBuffers; k++) {
                    ...
                    for (j = 0; j < yDim; j++) {
                        for (i = 0; i < xDim; i += 4) {
                            buffer[j * xDim + i] = 255;
                            buffer[j * xDim + i + 1] =
                                scl_slope * buffer[j * xDim + i + 1] + scl_inter;
                            buffer[j * xDim + i + 2] =
                                scl_slope * buffer[j * xDim + i + 2] + scl_inter;
                            buffer[j * xDim + i + 3] =
                                scl_slope * buffer[j * xDim + i + 3] + scl_inter;
                        }
                    }
                ...
                for (k = 0; k < nBuffers; k++) {
                    ...
                    for (j = 0; j < yDim; j++) {
                        for (i = 0; i < xDim; i++) {
                            buffer[j * xDim + i] =
                                scl_slope * buffer[j * xDim + i] + scl_inter;
                        }
                    }
                    image.importData(k * bufferSize, buffer, false);
                }
     ...
     */
    private void updateUnitsOfMeasure(FileInfoNIFTI fileInfo,
                                      ModelImage image) {
    ...
    private void updateorigins(FileInfoBase fileInfo[]) {
        ...
        /*else if (image.getNDims() == 5) {
          ...
          for (int i = 0;
               ...
               i++) {
            fileInfo[i].setorigins(startLocs);
     ...
     */
    public void flipTopBottom(float buffer[], FileInfoNIFTI fileInfo) throws
        IOException {
        int nBuffers;
        int bufferSize;
        float resultBuffer[] = null;
        ...
        try {
            ...
            if (fileInfo.getExtents().length - 1 == 5) {
                nBuffers = fileInfo.getExtents()[4] * fileInfo.getExtents()[3] *
                ...
                for (k = 0; k < nBuffers; k++) {
                    for (j = 0; j < yDim; j++) {
                        for (i = 0; i < xDim; i += 4) {
                            ...
                            resultBuffer[k * bufferSize + j * xDim + i +
                                2] = buffer[k * bufferSize + (yDim - 1 - j) * xDim + i + 2];
                            resultBuffer[k * bufferSize + j * xDim + i +
                                3] = buffer[k * bufferSize + (yDim - 1 - j) * xDim + i + 3];
                        }
                    }
                }
            }
