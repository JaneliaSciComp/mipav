...
/**
*   The MINC file format is built on top of NetCDF.  It has a small header that holds
*   certain image information:  position and step data for each of the dimensions,
*   real minimum and maximum values, and history.
*   <P>For more information, go to <A HREF="http://www.bic.mni.mcgill.ca/software/minc">the MINC
*   homepage</A> or <A HREF="http://www.unidata.ucar.edu/packages/netcdf/guidec/guidec-18.html">the
*   NetCDF File Format Specification</A>.
*
...
public class FileMinc extends FileBase {
    ...
    */
    public ModelImage readImage(boolean one) throws IOException {
        ...
        progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                           ViewUserInterface.getReference().getProgressBarPrefix() + "image(s) ...", 0, 100, false, null,null);
       ...
       // ModelImage image = new ModelImage(fileInfo.getDataType(), fileInfo.getExtents(), fileName, UI);
        ...
        if      (fileInfo.getDimElem(0).name.equals("zspace")) fileInfo.setImageOrientation(FileInfoBase.AXIAL);
        else if (fileInfo.getDimElem(0).name.equals("xspace")) fileInfo.setImageOrientation(FileInfoBase.SAGITTAL);
        else if (fileInfo.getDimElem(0).name.equals("yspace")) fileInfo.setImageOrientation(FileInfoBase.CORONAL);
        ...
        try {
            rawFile.setImageFile(fileInfo.getFileName(), fileInfo.getFileDirectory(),
            ...
            intercept = 0.0;
            int nImgs = 1;
            if (fileInfo.getExtents().length == 3) nImgs = fileInfo.getExtents()[2];
            if (one) nImgs = 1;
            for(int n = 0; n < nImgs; n++) {
                ...
                if (one) {
                    if (fileInfo.getExtents().length == 3) {
                        rawFile.readImage(buffer, fileInfo.offset+(fileInfo.getExtents()[2]/2*buffer.length*imgTypeFac), fileInfo.getDataType());
                    }
                    else {
                    ...
                    rawFile.readImage(buffer, fileInfo.offset+(n*buffer.length*imgTypeFac), fileInfo.getDataType());
                }
    ...
    *   name is the name of the attribute; nc_type is the type (integer, char, etc.);
    *   values are the values of this attribute.  It seems that attributes are
    *   mostly of type char, so the entire attribute is just a string. <BR>
    *   var := name nelems [dimid...] vatt_array nc_type vsize begin <BR>
    *   name is the name of the variable; don't know what dimid is; vatt_array
    *   is an array of attributes for this variable; nc_type is the type of the variable;
    *   vsize is the size of the variable; begin is where in the file the variable data begins.
    ...
    */
     public FileInfoMinc readHeader() throws IOException {
        ...
        if (!magic.equals("CDF" + '\001')) {
            Preferences.debug("4 byte magic tag = " + magic +
        ...
        if (next == FileInfoMinc.NC_DIMENSION) {
            ...
            Preferences.debug("NC_DIMENSION is followed with number of elements = "
                              + next + "\n", Preferences.DEBUG_FILEIO);
            ...
            for (int i=0; i<fileInfo.getDimArray().length; i++) {
                ...
                Preferences.debug("extents["+(fileInfo.getDimArray().length-1-i)+
                                  "] = " + len + "\n", Preferences.DEBUG_FILEIO);
                location += 4;
        ...
        // ABSENT - two zeros in a row
            ...
            if (next == 0) {
              ...
              location += 8;
                ...
                Preferences.debug("ABSENCE of NC_DIMENSION tag erroneously " +
                                  "followed with number of elements = " + next +
                                  "\n", Preferences.DEBUG_FILEIO);
                throw new IOException("MINC header corrupted");
            }
        }
        else {
            Preferences.debug("Should have NC_DIMENSION = 10 or ABSENCE = 0, " +
                              "but instead have = " + next + "\n", Preferences.DEBUG_FILEIO);
            throw new IOException("MINC header corrupted");
        }
        next = getInt(endianess);
        ...
        if (next == FileInfoMinc.NC_ATTRIBUTE) {
            ...
            next = getInt(endianess);
            Preferences.debug("NC_ATTRIBUTE is followed with " +
                              " number of elements = " + next + "\n", Preferences.DEBUG_FILEIO);
            ...
            for (int i=0; i<fileInfo.getGattArray().length; i++) {
                ...
                switch (type) {
                  ...
                  case 6:
                    ...
                    break;
                  default:
                    Preferences.debug("attr["+i+"] nc_type illegally = " +
                                       type + "\n", Preferences.DEBUG_FILEIO);
                }
                location += 4;
                len = getInt(endianess);
        ...
        // ABSENT - two zeros
            ...
            if (next == 0) {
              ...
              location += 8;
                ...
                Preferences.debug("ABSENCE of NC_ATTRIBUTE tag erroneously " +
          ...
          Preferences.debug("Should have NC_ATTRIBUTE = 12 or ABSENCE = 0, " +
                              "but instead have = " + next + "\n", Preferences.DEBUG_FILEIO);
            throw new IOException("MINC header corrupted");
        }
        next = getInt(endianess);
        ...
        if (next == FileInfoMinc.NC_VARIABLE) {
            ...
            next = getInt(endianess);
            Preferences.debug("NC_VARIABLE is followed with " +
                              " number of elements = " + next + "\n", Preferences.DEBUG_FILEIO);
            ...
            for (int i=0; i<fileInfo.getVarArray().length; i++) {
                ...
                for (int j=0; j<len; j++) {
                    ...
                    Preferences.debug("var["+i+"] dimid["+j+"] = " +
                                      dimid[j] + "\n", Preferences.DEBUG_FILEIO);
                    location+=4;
                }
                ...
                switch (type) {
                  ...
                  case 6:
                    ...
                    break;
                  default:
                    Preferences.debug("var[" + i + "] nc_type illegally = " +
                                      type + "\n", Preferences.DEBUG_FILEIO);
                }
        ...
        // ABSENT - two zeros
            ...
            if (next == 0) {
              ...
              Preferences.debug("ABSENCE of NC_VARIABLE tag erroneously " +
                                "followed by number of elements = " +
                                next + "\n", Preferences.DEBUG_FILEIO);
          ...
          Preferences.debug("Should have NC_VARIABLE = 11 or ABSENCE = 0, " +
                              "but instead have = " + next + "\n", Preferences.DEBUG_FILEIO);
            throw new IOException("MINC header corrupted");
        }
        return fileInfo;
    ...
    */
    public void writeImage(ModelImage _image, FileWriteOptions options) throws IOException {
        ViewJProgressBar progressBar = new ViewJProgressBar("Saving " + fileName, "Writing header", 0, 100, false, null,null);
        ...
        try {
            image = new ModelImage(_image.getFileInfo()[0].getDataType(),
                                   _image.getFileInfo()[0].getExtents(), _image.getImageFileName(), UI);
            ...
            if (!options.isSaveAs()) {
                    ...
                    if (fileInfo.getVarElem(i).name.equals("image")) {
                        ...
                        if (image.getNDims() == 3) {
                            for (int j = 0; j < image.getExtents()[2]; j++){
                                ...
                                progressBar.updateValue(15 + Math.round((float)j/(fileInfo.getExtents()[2]-1) * 35),
                            ...
                            for (int j = 0; j < image.getExtents()[2]; j++){
                                ...
                                progressBar.updateValue(50 + Math.round((float)j/(fileInfo.getExtents()[2]-1) * 50),
                                    options.isActiveImage());
                            }
                        ...
                        for (int j=0; j<fileInfo.getVarElem(i).values.size();j++) {
                            writeNextElem(fileInfo.getVarElem(i).values.elementAt(j), fileInfo.getVarElem(i).nc_type, fileInfo.getEndianess());
                        }
                        writePadding();
                ...
                for (int j = options.getBeginSlice(); j <= options.getEndSlice(); j++) {
                    ...
                    progressBar.updateValue(10 + Math.round( (float) count / (nImages - 1) * 40), options.isActiveImage());
                ...
                //ViewJFrameImage imgFrame = new ViewJFrameImage(image, null, new Dimension(100,100), _image.getUserInterface());
                for (int j = options.getBeginSlice(); j <= options.getEndSlice(); j++) {
                    jp = j - options.getBeginSlice();
                    //System.out.println(" j = " + j);
                    rawChunkFile.writeImage(image, jp * bufferSize, (jp + 1) * bufferSize, jp);
                    progressBar.updateValue(50 + Math.round( (float) count / (nImages - 1) * 50), options.isActiveImage());
                    count++;
                }
    ...
    /**
    *   Writes a header for MINC to MINC. This is much easier than the
    *   other-format-to-MINC writeHeader; in this case, we already have
    *   all the information we need to write.  Therefore, the process is just
    *   to go through the dimArray, gattArray, and varArray and write out the
    *   variables.
    ...
    */
        ...
        if (fileInfo.getVarArray() != null) { // write the variables
            ...
            for (int i=0; i<fileInfo.getVarArray().length; i++) {
                ...
                for (int j=0; j<fileInfo.getVarElem(i).vattArray.length; j++) { // write attribute array
                    ...
                    writeValuesArray(fileInfo.getVarElem(i).vattArray[j].values, fileInfo.getVarElem(i).vattArray[j].nc_type, endianess);
                }
    ...
    /**
     * 	Writes the header for non-MINC to MINC.  MINC has a very structured format.
     *  Many of the things written out I don't understand, but they seem to be
     *  consistent across the MINC files we've seen (e.g., varid, vartype, and version
     *  attributes).  The header begins with a dimension array with a specific ordering of the
     *  spaces which dictates orientation.  Then there's a global attribute array,
     *  which in our case is just the history of how this file was created.  Then there's
     *  a variable array.  Each variable contains an attribute array, a type, a size, and
     *  a begin location.  The actual value of the variable is written at its begin location,
     *  after all other header info is written.  So the rootvariable's value, for example,
     *  is written after the info for image-min, which is the last variable in the array.
     *  It seems that conventionally the image-max and image-min variables are after the
     *  image variable; thus, the values of those variables are written after the "value
     *  of the image" which is the actual image data.  Consequently, this function exits
     *  just before the image is to be written, even though the image-max and image-min
     *  values have yet to be written.
     *  <p>This is a public method because it is called from FileIO.  Previously it was called from
     *	writeImage, but this would create a file even if the dialog was cancelled.  Obviously that is
    ...
    private void writeHeader(FileInfoBase fileInfo, FileWriteOptions options) throws IOException {
        ...
        if (fileInfo.getExtents().length == 3) zSpace = options.getZSpace();
        if (fileInfo.getExtents().length == 3) {
            switch(fileInfo.getImageOrientation()) {
                case FileInfoBase.SAGITTAL:
                    writeSpace(xSpace, options.getXStart(), "xspace", endianess, options.isLeftToRight());
                    writeInt(FileInfoMinc.NC_DOUBLE, endianess);
                    writeInt(8, endianess);
                    writeInt(START3D+4, endianess);
                    writeSpace(zSpace, options.getZStart(), "zspace", endianess, options.isInfToSup());
                    writeInt(FileInfoMinc.NC_DOUBLE, endianess);
                    writeInt(8, endianess);
                    writeInt(START3D+12, endianess);
                    writeSpace(ySpace, options.getYStart(), "yspace", endianess, options.isPosToAnt());
                    writeInt(FileInfoMinc.NC_DOUBLE, endianess);
                    writeInt(8, endianess);
        ...
        else writeName("signed__", 1, endianess);
    ...
    */
    private final Object getNextElem(int type, boolean endianess) throws IOException {
        Object value = null;
        switch (type) {
            case FileInfoMinc.NC_BYTE:  value = new Byte(raFile.readByte());
                                    location++;
                                    break;
            case FileInfoMinc.NC_CHAR:  value = new Character((char)raFile.readByte());
                                    location++;
                                    break;
            case FileInfoMinc.NC_SHORT: value = new Short((short)getUnsignedShort(endianess));
                                    location+=2;
                                    break;
            case FileInfoMinc.NC_INT:   value = new Integer(getInt(endianess));
                                    location+=4;
                                    break;
            case FileInfoMinc.NC_FLOAT: value = new Float(getFloat(endianess));
                                    location+=4;
                                    break;
            case FileInfoMinc.NC_DOUBLE:value = new Double(getDouble(endianess));
                                    location+=8;
                                    break;
            default: MipavUtil.displayError("Invalid type in FileMinc.getNextElement");
        }
        return value;
    }
    ...
    */
    private void getVattArray(int index, FileInfoMinc fileInfo) throws IOException {
        ...
        for (int i=0; i<next; i++) {
            attrString = new String();
            int len = getInt(endianess);
            Preferences.debug("var["+index+"] vatt_array["+i+"] string name length = "
                              + len + "\n", Preferences.DEBUG_FILEIO);
            String name = getString(len);
            Preferences.debug("var["+index+"] vatt_array["+i+"] string name = "
                              + name + "\n", Preferences.DEBUG_FILEIO);
            location += 4 + len;
            padding();
            int type = getInt(endianess);
            switch (type) {
                ...
                case 6:
                  ...
                  break;
                default:
                  Preferences.debug("var["+index+"] vatt_array["+i+"] type illegally = "
                                    + type + "\n", Preferences.DEBUG_FILEIO);
            }
            location += 4;
            len = getInt(endianess);
    ...
    */
    ...
    /**
    *	Write a string name by writing the length of the string plus padding length, writing the string, and
    *	writing the padding.
    ...
    private void writeValuesArray(Object[] values, int type, boolean endianess) throws IOException {
        ...
        switch (type) {
            ...
            case FileInfoMinc.NC_DOUBLE:
                ...
                break;
            default: MipavUtil.displayError("Invalid type in FileMinc.writeValuesArray");
        }
    }
    /**
    ...
    */
    private void writeNextElem(Object value, int type, boolean endianess) throws IOException {
        ...
        switch (type) {
            case FileInfoMinc.NC_BYTE:  raFile.write(((Byte)value).byteValue());
                                    location++;
                                    break;
            case FileInfoMinc.NC_CHAR:  raFile.writeBytes(String.valueOf(((Character)value).charValue()));
                                    location++;
                                    break;
            case FileInfoMinc.NC_SHORT: writeShort(((Short)value).shortValue(), endianess);
                                    location+=2;
                                    break;
            case FileInfoMinc.NC_INT:   writeInt(((Integer)value).intValue(), endianess);
                                    location+=4;
                                    break;
            case FileInfoMinc.NC_FLOAT: writeFloat(((Float)value).floatValue(), endianess);
                                    location+=4;
                                    break;
            case FileInfoMinc.NC_DOUBLE:writeDouble(((Double)value).doubleValue(), endianess);
                                    location+=8;
                                    break;
            default: MipavUtil.displayError("Invalid type in FileMinc.writeNextElement");
        }
    }
    ...
    */
    ...
    /**
    *	Returns the amount of padding needed (to the nearest 4 byte boundary)
    *	after a variable of length <code>size</code>.
    ...
    */
    ...
    /**
    *	Writes the "space", as in xspace, yspace, zspace.  The space gives information on that dimension,
    *	such as length (what we call extents), step, and start.  Start tells the real talirach value of where
    *	the space starts, so -50 would be something like 50 inches away from the center of the brain to the right, depending
    *	on which space we're in.  Step is the step you take per slice from the start, so the third slice in would be
    *	start + 3*step.
    *	@param step		Step, see above.
    *	@param start	Start, see above.
    *	@param endianess	Big or little.
    */
    private void writeSpace(double step, double start, String space, boolean endianess, boolean isNormal) throws IOException {
        ...
        writeInt(FileInfoMinc.NC_CHAR, endianess);
        writeName("mm", 1, endianess); // should get this from file, but don't know what other measurements are -inches "in"?
