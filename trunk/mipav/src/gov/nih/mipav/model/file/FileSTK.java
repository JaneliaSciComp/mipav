...
*   Note that the IMAGE_DESCRIPTION and STRIP_OFFSETS tags are handled differently
*   in STK file format than in TIFF file format.  Also note that the UIC1Tag,
*   UIC2Tag, UIC3Tag, and UIC4Tag are found in STK but not in TIFF.
*   While TIFF files can be either big-endian or little-endian, STK files must
*   be little endian.
*   Only packed bit compression is supported at this time.
*   Note that although EchoTech has a tResolution field, there is no provision for 4D in TIFF.
*
*       @see        FileIO
*       @see        FileInfoTiff
*       @see        FileRaw
...
public class FileSTK extends FileBase{
    ...
    */
    public FileSTK(ViewUserInterface _UI, String fileName, String fileDir) throws IOException {
    ...
    */
    public ModelImage readImage(boolean multiFile, boolean one)  throws IOException {
        ...
        try {
            ...
            if (numberImages > 1) {
                ...
                for (i = 1;i < numberImages; i++) {
                    ...
                    for (j = 0; j < nIndex; j++) {
                        ...
                        ((Index)(dataOffsets[i].elementAt(j))).byteCount =
                        ((Index)(dataOffsets[0].elementAt(j))).byteCount;
                    }
                }
            } // if (numberImages > 1)
            ...
            if (multiFile == false) {
                if (one) {
                    image = new ModelImage(fileInfo.getDataType(), new int[] {imgExtents[0], imgExtents[1]}, fileInfo.getFileName(), UI);
                }
                else {
            ...
            while (moreIFDs){
                ...
                if (imgExtents.length > 2) {
                    fileInfo.setUnitsOfMeasure(FileInfoBase.MICROMETERS,2);
                }
            ...
            if (imgBuffer == null) { imgBuffer = new float[bufferSize]; }

            progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                               ViewUserInterface.getReference().getProgressBarPrefix() + "STK image(s) ...", 0, 100,
                                                false, null,null);
    ...
    */
    private boolean openIFD(FileInfoSTK fileInfo) throws IOException {
        ...
        if (nDirEntries <= 0) throw new IOException("First 2 IFD bytes are an illegal " + nDirEntries);
        Preferences.debug( "\nOpenIFD: Entries = " + nDirEntries + "\n");
        for (i = 0; i < nDirEntries; i++){

            tag     = getUnsignedShort(endianess);
            if (tag == 0) throw new IOException("Tiff Zero Tag Error");
            ...
            switch (tag) {
                ...
                case BITS_PER_SAMPLE:
                    ...
                    if (count == 1){
                        ...
                        switch((int)valueArray[0]){
                            case  1: fileInfo.setDataType(ModelStorageBase.BOOLEAN);
                                    break;
                            case  4: fileInfo.setDataType(ModelStorageBase.BYTE);
                                    break;
                            case  8: fileInfo.setDataType(ModelStorageBase.UBYTE);
                                    break;
                            case 16: fileInfo.setDataType(ModelStorageBase.USHORT);
                                    break;
                            case 32: fileInfo.setDataType(ModelStorageBase.UINTEGER);
                                    break;
                            default:
                ...
                case IMAGE_DESCRIPTION:
                  ...
                  for (i1 = 0; i1 < count; i1++) {
                    if (imageDescription[i1] == 0) {
                        ...
                        Preferences.debug("FileSTK.openIFD: imageDescription for plane number " +
                                          planeNumber +":\n" + str + "\n");
                        j = i1 + 1;
                        planeNumber++;
                    }
                ...
                case MIN_SAMPLE_VALUE:
                  ...
                  for (i1 = 0; i1 < count; i1++) {
                    Preferences.debug("FileSTK.openIFD: minSampleValue[" + i1 + "] = " +
                                          valueArray[i1] + "\n");
                  }
                  break;
                case MAX_SAMPLE_VALUE:
                  ...
                  for (i1 = 0; i1 < count; i1++) {
                    Preferences.debug("FileSTK.openIFD: maxSampleValue[" + i1 + "] = " +
                                          valueArray[i1] + "\n");
                  }
                  break;
                case SOFTWARE:
                ...
                case SAMPLE_FORMAT:
                  ...
                  for (i1 = 0; i1 < count; i1++) {
                    Preferences.debug("FileSTK.openIFD: sampleFormat[ " + i1 + "] = " +
                                      valueArray[i1] + "\n");
                    if (valueArray[i1] == 1) {
                          ...
                          while (j < nBytes) {
                                ...
                                if ((byteBuffer[j] & 0x80) == 0) {
                                    ...
                                    for (iNext = 0; iNext < iCount; iNext++,j++) {
                                        if ((x < xDim) && (y < yDim)) {
                                            if ((i+progress)%mod==0) progressBar.updateValue( Math.round((float)
        ...
        for (a = 0; a < nIndex; a++, idx++) {
            try {
                ...
                switch (fileInfo.getDataType()) {
                    ...
                    case ModelStorageBase.BYTE:
                        if (packBit == false) {
                          if (byteBuffer == null) byteBuffer =  new byte[buffer.length];
                          raFile.read(byteBuffer, 0, nBytes);
                          progress = slice*buffer.length;
                          progressLength = buffer.length*imageSlice;
                          mod = progressLength/10;
                    ...
                    case ModelStorageBase.UINTEGER:
                        ...
                        for (j =0; j < nBytes; j+=4, i++ ) {
                            ...
                            if (endianess) {
                                buffer[i]=(((long)b1 << 24) | ((long)b2 << 16) |
                                           ((long)b3 << 8) | (long)b4);  // Big Endian
                            }
                            else {
                                buffer[i]=(((long)b4 << 24) | ((long)b3 << 16) |
                                           ((long)b2 << 8) | (long)b1);  // Little Endian
                            }
                        }
                        break;
                    case  ModelStorageBase.ARGB:
                        if (chunky == true) {
                          ...
                          if (planarRGB < stripsPerImage) {
                                ...
                                if ((i/3+progress)%mod==0) progressBar.updateValue( Math.round((float)(i/3+progress)/
                                                                        progressLength * 100), false);
                                    buffer[i]   = 255;
                                    buffer[i+1] = getUnsignedByte(byteBuffer, j);
                                }
                                ...
                                for (j = 0; j < nBytes; j++, i+=4 ) {
                                    ...
                                    progressBar.updateValue( Math.round((float)(i/3+ 2*buffer.length/3 + progress)/
                                                                            progressLength * 100), false);
    ...
    */
    public void writeImage(ModelImage image, ModelLUT LUT, FileWriteOptions options) throws IOException{
        ...
        try {
            ...
            progressBar = new ViewJProgressBar("Saving TIFF image", "Saving Image " + fileName,
            ...
            for(s = sBegin,seq = options.getStartNumber(); s < sEnd; s++,seq++ ) {
                if (oneFile) {
                	...
                	if (options.isSaveAs()) {
                    	if (options.getDigitNumber() == 1) {
                        	...
                        	if (seq < 10) file = new File(fileDir + prefix + "0" + Integer.toString(seq) + fileSuffix);
                        	else file = new File(fileDir + prefix + Integer.toString(seq) + fileSuffix);
                    	}
                    	else if (options.getDigitNumber() == 3) {
                        	if ( seq < 10) file   = new File(fileDir + prefix + "00" + Integer.toString(seq) + fileSuffix );
                        	else if ( seq < 100)  file   = new File(fileDir + prefix + "0"  + Integer.toString(seq) + fileSuffix );
                        	else file = new File(fileDir + prefix + Integer.toString(seq) + fileSuffix);
                    	}
                    	else if (options.getDigitNumber() == 4) {
                        	if (seq < 10) file = new File(fileDir + prefix + "000" + Integer.toString(seq) + fileSuffix);
                        	else if (seq < 100) file = new File(fileDir + prefix + "00" + Integer.toString(seq) + fileSuffix);
                        	else if (seq < 1000) file = new File(fileDir + prefix + "0" + Integer.toString(seq) + fileSuffix);
                        	else file = new File(fileDir + prefix + Integer.toString(seq) + fileSuffix);
                    	}
                    }
                    else file = new File(fileDir + image.getFileInfo(s).getFileName());
                    raFile = new RandomAccessFile(file, "rw");
                    raFile.setLength(0); // necessary so that if this is an overwritten file there isn't junk at the end
                    if (options.isWritePackBit() == false)
                ...
                if (oneFile == true)  { // one file with one or more images
                    for (k = options.getBeginSlice(), m=0; k <= options.getEndSlice(); k++, m++){
                        ...
                        progressBar.updateValue( Math.round((float)(k - options.getBeginSlice() + 1)/(options.getEndSlice() - options.getBeginSlice() + 1) * 100),
                                                 options.isActiveImage());
                        progressBar.setTitle("Saving image " + k);
                        if (options.isWritePackBit()) {
                          stripCount = filePB.getStripSize(image,
                              timeOffset + k*bufferSize,timeOffset + k*bufferSize + bufferSize);
                        ...
                        if (!options.isWritePackBit()) {
                        imgOffset = 8 + (m+1)*(2+nDirEntries*12 + resolutionCount + 4 + rgbCount + intAlign
                                      + zResCount + tResCount) +
                        ...
                        try {
                            if (!options.isWritePackBit()) {
                              fileRW.writeImage(image,
                                  timeOffset + k*bufferSize, timeOffset + k*bufferSize + bufferSize, 0);
                            }
                            else {
                              filePB.writePackBitImage(image,
                                  timeOffset + k*bufferSize, timeOffset + k*bufferSize + bufferSize);
                            }
                        }
                        catch (IOException error) {
                    ...
                    imgOffset = 8 + (2+nDirEntries*12 + resolutionCount + 4 + rgbCount + intAlign
                                + zResCount + tResCount);
	...
	*/
    private void writeIFDs(int imageOffset, int nextIFD, int index, int theStripCount, boolean writePackBit) throws IOException {
        ...
        if ( (resXYUnit != FileInfoBase.UNKNOWN_MEASURE) &&
            (resXYUnit != FileInfoBase.INCHES)
            && (resXYUnit != FileInfoBase.CENTIMETERS) &&
            (resXYUnit != FileInfoBase.MILLIMETERS) &&
            (resXYUnit != FileInfoBase.METERS) &&
            (resXYUnit != FileInfoBase.ANGSTROMS) &&
            (resXYUnit != FileInfoBase.NANOMETERS) &&
            (resXYUnit != FileInfoBase.MICROMETERS) &&
            (resXYUnit != FileInfoBase.MILES) &&
            (resXYUnit != FileInfoBase.KILOMETERS)) {
        ...
        tResOffset = (int)raFile.getFilePointer() + (2 + nDirEntries*12 + 4 + resolutionCount + rgbCount
		                        + zResCount);
	    ...
	    writeIFD(STRIP_BYTE_COUNTS, LONG,   1,
	            theStripCount*bytesPerSample*samplesPerPixel,0);
