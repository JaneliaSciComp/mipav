...
public class FileTiff extends FileBase {
     ...
     * @param multiFile <code>true</code> if a set of files each containing a separate 2D image is present
     *            <code>false</code> if one file with either a 2D image or a stack of 2D images
    ...
    public ModelImage readImage(boolean multiFile, boolean one) throws IOException {
        ...
        try {
            ...
            if (multiFile == false) {
                if (one) {
                    image = new ModelImage(fileInfo.getDataType(), new int[] {imgExtents[0], imgExtents[1]}, fileInfo
                            .getFileName(), UI);
                } else {
            ...
            if (progressBar == null) {
                progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                        ViewUserInterface.getReference().getProgressBarPrefix() + "TIFF image(s) ...", 0, 100, false,
                        null, null);
        ...
        if (doColor) {
            ...
            if (progressBar == null) {
                progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix()
                        + series.getName(), ViewUserInterface.getReference().getProgressBarPrefix() + "series(s) ...",
                            ...
                            if (planarRGB < stripsPerImage) {
                                ...
                                for (j = 0; j < nBytes; j++ , i += 4) {
                                    if ( (i / 3 + progress) % mod == 0)
                                        progressBar.updateValue(Math.round((float) (i / 3 + progress) / progressLength
                                                * 100), false);
                                    buffer[i] = 255;
                                    buffer[i + 1] = getUnsignedByte(byteBuffer, j + currentIndex);
                                }
                                ...
                                for (j = 0; j < nBytes; j++ , i += 4) {
                                    if ( (i / 3 + buffer.length / 3 + progress) % mod == 0)
                                        progressBar.updateValue(Math
                                                .round((float) (i / 3 + buffer.length / 3 + progress) / progressLength
                                                        * 100), false);
            ...
            for (s = sBegin, seq = options.getStartNumber(); s < sEnd; s++ , seq++ ) {
                        ...
                        if (image.getFileInfo(0).getPhotometric() == 3) {
                            ...
                            if (oneFile == true) {
                                LUTOffset = 8
                                        ...
                                        * (options.getEndSlice() - options.getBeginSlice() + 1) + bufferSize
                ...
                if (oneFile == true) { // one file with one or more images
                    for (k = options.getBeginSlice(), m = 0; k <= options.getEndSlice(); k++ , m++ ) {
                        progressBar
                                .updateValue(Math.round((float) (k - options.getBeginSlice() + 1)
                                        / (options.getEndSlice() - options.getBeginSlice() + 1) * 100), options
                                        .isActiveImage());
                        progressBar.setTitle("Saving image " + k);
                        if (options.isWritePackBit()) {
                            stripCount = filePB.getStripSize(image, timeOffset + k * bufferSize, timeOffset + k
                        ...
                        if (k == options.getEndSlice()) {
                            ...
                            nextIFD += ( (2 + nDirEntries * 12 + resolutionCount + 4 + rgbCount + rgbFormat + intAlign
                        ...
                        if ( !options.isWritePackBit()) {
                            imgOffset = 8
                                    ...
                                    * (2 + nDirEntries * 12 + resolutionCount + 4 + rgbCount + rgbFormat + intAlign
                                            + zResCount + tResCount) + m * bufferSize * bytesPerSample
                                    * samplesPerPixel;
                        } else if (options.isWritePackBit()) {
                            imgOffset = 8
                                    + (m + 1)
                        ...
                        try {
                            if ( !options.isWritePackBit()) {
                                // adjust for intAlign ????
                                fileRW.writeImage(image, timeOffset + k * bufferSize, timeOffset + k * bufferSize
                                        + bufferSize, 0);
                            } else {
                                filePB.writePackBitImage(image, timeOffset + k * bufferSize, timeOffset + k
                                        * bufferSize + bufferSize);
                            }
                        } catch (IOException error) {
                            throw error;
                    ...
                    progressBar.updateValue(Math.round((float) s / (options.getEndSlice()) * 100), options
                            .isActiveImage());
     ...
     * @param IFD offset to next IFD. If equal to zero then end of images
    ...
    private void interpretBytesAsFloats(byte byteArray[], float floatArray[], int floatCount) {
        ...
        if (endianess == FileBase.BIG_ENDIAN) {
            for (int i = 0; i < floatCount; i++ ) {
                int value = (int) ( ( (byteArray[j++ ] & 0xff) << 24) | ( (byteArray[j++ ] & 0xff) << 16)
                        | ( (byteArray[j++ ] & 0xff) << 8) | (byteArray[j++ ] & 0xff));
                floatArray[i] = Float.intBitsToFloat(value);
            }
        } else {
            for (int i = 0; i < floatCount; i++ ) {
                int value = (int) ( (byteArray[j++ ] & 0xff) | ( (byteArray[j++ ] & 0xff) << 8)
                        | ( (byteArray[j++ ] & 0xff) << 16) | ( (byteArray[j++ ] & 0xff) << 24));
                floatArray[i] = Float.intBitsToFloat(value);
            }
        }
    }
