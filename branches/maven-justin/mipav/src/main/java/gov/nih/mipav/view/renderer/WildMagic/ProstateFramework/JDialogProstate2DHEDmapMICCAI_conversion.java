...
/**
 * This class generates the VOIs from HED prediction maps.  Only takes MRI transformed images
 ...
 * 
 */
public class JDialogProstate2DHEDmapMICCAI_conversion extends JDialogBase
	 ...
	 */
	public JDialogProstate2DHEDmapMICCAI_conversion(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_5_test = new File("/data/ruida/MICAI2012/test_prostate_mask_ced_iter_30000");
	...
	private void generateContours() {
        ...
        for(int i = 0; i < size; i++ ){
			...
			int imageVecSize = srcExtents[2];
			for (int j = 0; j < imageVecSize; j++) {
				ModelImage image = imageVec.get(j);
				ModelImage cedImage = imageVec.get(j + imageVecSize);
			    ModelImage maxMaskImage = new ModelImage(image.getType(), image.getExtents(),
                        makeImageName(image.getImageName(), "_calc"));
			    int opType = 6;
			    int clipMode = 0;
			    String adOpString = null;
			    AlgorithmImageCalculator mathAlgo = new AlgorithmImageCalculator(maxMaskImage, image, cedImage, 
			    		opType, clipMode, true, adOpString);
			    mathAlgo.run();
			...
			try {
				FileVOI fileVOI = new FileVOI("case_" + i + "_segmentation.voi", directory, srcImage);
