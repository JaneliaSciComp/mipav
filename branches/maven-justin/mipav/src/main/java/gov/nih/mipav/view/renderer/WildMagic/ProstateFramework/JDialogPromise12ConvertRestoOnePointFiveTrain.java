 ...
 * to 0.5mm x 0.5mm x 1.5 mm images.   For training case only. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogPromise12ConvertRestoOnePointFiveTrain extends JDialogBase implements
		AlgorithmInterface {
	...
	private Vector<ModelImage> keyImagesN4Transform = new Vector<ModelImage>();
	 ...
	 */
	public JDialogPromise12ConvertRestoOnePointFiveTrain(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		// After N4 correction, we process the images with MR image and corresponding Mask images. 
		File fileDir_1 = new File("/scratch/miccai/N4Train");
		File fileDir_2 = new File("/scratch/miccai/Preprocessing/N4TrainMaskTransform");
	...
	public void transform() {
		...
		for (int imageIndex = 0; imageIndex < size; imageIndex++) {
			...
			ModelImage transKeyImageN4 = calculateTransform(keyImageN4);
			keyImagesN4Transform.add(imageIndex, transKeyImageN4);
	...
	public void saveTestedImages() {
			...
			for (int imageIndex = 0; imageIndex < size; imageIndex++) {
				...
				if (!sliceDirFile.isDirectory())
					sliceDirFile.mkdir();
				sliceDir += File.separator + "N4Train1_5" + File.separator;
			...
			size = keyImagesN4Transform.size();
            System.err.println("size = " + size);
			for (int imageIndex = 0; imageIndex < size; imageIndex++) {
				
				ModelImage targetImageSlice = keyImagesN4Transform.get(imageIndex);
				// 1) save image
				String sliceDir = saveImageDirectory;
				File sliceDirFile = new File(sliceDir);
				if (!sliceDirFile.isDirectory())
					sliceDirFile.mkdir();
				sliceDir += File.separator + "N4TrainMask1_5" + File.separator;
				File dir = new File(sliceDir);
				if (!dir.isDirectory()) {
					dir.mkdir();
				}
