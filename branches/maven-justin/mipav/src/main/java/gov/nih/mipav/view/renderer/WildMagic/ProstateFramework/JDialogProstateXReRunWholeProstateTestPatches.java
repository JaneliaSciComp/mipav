...
/*
 * This class simply read the MR, CED images, converting them into MR and CED slices.  
 * Those slices run against the HED deep learning model to generate the energy map.  
 * This class prepare the HED testing folds for cross validation test. 
 */
public class JDialogProstateXReRunWholeProstateTestPatches extends JDialogBase implements
		AlgorithmInterface {
	 ...
	 */
	public JDialogProstateXReRunWholeProstateTestPatches(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_2 = new File("/scratch/prostateXdata/trainImageCg/121_146/sagittal");
	...
	public void saveHED2DsliceCED() {
		...
		for (String key : keys) {
			...
			// if (count >= startIndex && count <= endIndex) {
				...
				saveImage(scaledAxialImage, axialImageMask, "sagittal", key);
				...
				saveImage(scaledAxialImageCED, axialImageMaskCED, "sagittal", key);
	...
	public void saveImage(ModelImage image, ModelImage imageMask, String orientation, String key) {
		...
		for (int j = 0; j < zDim; j++) {
			...
			try {
				...
				String imageDir = sliceDir + key + File.separator;
				...
				if (imgName != null && maskName != null) {
					...
					savePNGfile(imageDir, imgName, imageSlice, min, max, xDim, yDim, false);
					savePNGfile(imageDir, maskName, maskSlice, min, max, xDim, yDim, true);
	...
	public ModelImage scaleIntensity(ModelImage image) {
		...
		// ModelImage resultImage = new ModelImage(dataType, destExtents,
		// "scale_inten");
