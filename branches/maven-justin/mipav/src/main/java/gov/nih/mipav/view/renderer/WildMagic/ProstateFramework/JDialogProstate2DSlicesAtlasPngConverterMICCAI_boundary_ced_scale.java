 ...
 * 2. generate the prostate boundary binary mask. 
 * 3. run CED to MRI image. 
 * 4. Transform both CED and MRI images to isotropic resolution(x, y resolution) images
 * 5. converts the MRI and CED slices into png file format.  
 * 
 * For training image only. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstate2DSlicesAtlasPngConverterMICCAI_boundary_ced_scale extends JDialogBase
	...
	private Vector<ModelImage> maskVOIImages = new Vector<ModelImage>();
	 ...
	 */
	public JDialogProstate2DSlicesAtlasPngConverterMICCAI_boundary_ced_scale(Frame theParentFrame) {
	...
	public void runTransform() {
		...
		for (int i = 0; i < maskVOIImages.size(); i++) {
			try {
				ModelImage voiImage = maskVOIImages.get(i);
				transVOIImages.add(calculateTransform(voiImage));
			} catch ( OutOfMemoryError e ) {
				e.printStackTrace();
			}
		}
	...
	public void runMask() {
		...
		for (int i = 0; i < keyImageVOIs.size(); i++) {
			try {
				
				ModelImage maskImage = keyImageVOIs.get(i);
	            ...
	            maskVOIImages.add(diffImage);
				// maskVOIImages.add(calculateTransform(voiImage));
			    // new ViewJFrameImage(transVOIImages.get(i));
			} catch ( OutOfMemoryError e ) {
				e.printStackTrace();
			}
		}
	...
	public void readFile() {
		...
		try {
			...
			for (int imageIndex = start; imageIndex < len; imageIndex++) {
				...
				keyImageVOIs.add(currentIndex, keyVOIio.readImage(voiFileName, voiDirectory));
				keyImageVOIs.get(currentIndex).calcMinMax();
