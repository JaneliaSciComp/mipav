...
//        muscleBundleImage = new ModelImage(ModelStorageBase.USHORT, srcImage.getExtents(), "muscleBundleImage");
    ...
    private VOI makeRightThighVOI(VOI totalVOI) {
    	...
    	int size = 0;
    	for(int i=0; i<zDim; i++) {
    		size = tempVOI.getCurvesTemp()[i].size();
    ...
    private boolean makeThighTissueVOI() {
        ...
        for (int idx = 0; idx < zDim; idx++) {
            numCurves = theVOI.getCurvesTemp()[idx].size();
            
            int idx2 = 0;
            while(idx2 < numCurves) {
                curve = ((VOIContour)theVOI.getCurvesTemp()[idx].get(idx2));
                numPoints = curve.size();
                if (numPoints < 10) {
                    // remove the curve
                    theVOI.getCurvesTemp()[idx].remove(idx2);
                    numCurves--;
                } else {
                    idx2++;
                }
...
*/        
        ...
        for (int sliceIdx = 0; sliceIdx < zDim; sliceIdx++) {
            leftThighVOI.getCurvesTemp()[sliceIdx].removeAllElements();
            rightThighVOI.getCurvesTemp()[sliceIdx].removeAllElements();
        }

        // split the thigh curves when the legs touch (there are only 3 curves, not 4)
        for (int sliceIdx = 0; sliceIdx < zDim; sliceIdx++) {
            if (theVOI.getCurvesTemp()[sliceIdx].size() == 3) {
                
                // find the curve with the greatest area (the thigh curve)
                float maxArea = ((VOIContour)theVOI.getCurvesTemp()[sliceIdx].get(0)).area();
                int maxIdx = 0;
                for (int idx = 1; idx < theVOI.getCurvesTemp()[sliceIdx].size(); idx++) {
                    if (((VOIContour)theVOI.getCurvesTemp()[sliceIdx].get(idx)).area() > maxArea) {
                        maxIdx = idx;
                    }
                }
...
//                System.out.println("Slice num: " +sliceIdx +"   thigh curve idx: " +maxIdx);
                ...
                VOIContour maxContour = ((VOIContour)theVOI.getCurvesTemp()[sliceIdx].get(maxIdx));
...
//                System.out.println(xVals[upperCurveMinIdx] +"  " +yVals[upperCurveMinIdx] +"   and  " +xVals[lowerCurveMaxIdx] +"  " +yVals[lowerCurveMaxIdx]);
            ...
            } else if (theVOI.getCurvesTemp()[0].size() != 4) {
                ...
                rightCurve = ((VOIContour)rightThighVOI.getCurvesTemp()[0].get(0));
                ...
                leftCurve = ((VOIContour)leftThighVOI.getCurvesTemp()[0].get(0));
