 ...
 * @see DecoderSpecs
 * */
public class DecoderRAW extends Decoder {//implements Runnable {
     ...
     * display the image.
     * */
    public DecoderRAW(ParameterList pl, ImgScrollPane isp) {
	    super(pl,isp);
    	this.pl = pl;
     ...
     * values).
     * */
    public DecoderRAW(ParameterList pl) {
        this(pl,null);
    }
...
//	    imwriter = new ImgWriter[1];
    ...
    public void runAllSlices(){
  	...
  	FileInputStream f;
        ...
        f = rawhd.readRawJP2Header(infile);
...
//        int nslice = rawhd.getNumOfSlices();
        int[] imgExtents = rawhd.getImgExtents();
        int nslice =  imgExtents[2];
        try {
        	byte[] b=new byte[1];
            ...
            for (int si=0;si<nslice;si++){
            	...
            	slc = run1Slice(inStream);
            	if (si==0){
            		wr = new ImgWriterRAW(outf,slc ,0,false);
            	}else {
            		wr.setOffset(si);
            		wr.setSrc(slc);
