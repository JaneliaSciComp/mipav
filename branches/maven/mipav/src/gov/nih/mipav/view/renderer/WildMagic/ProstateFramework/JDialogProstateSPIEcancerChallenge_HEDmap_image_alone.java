...
/**
 * This class generates the VOI contours from HED predicted energy maps. This is a try-and-error approach to use
 * MRI image alone.  For backup purpose only. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstateSPIEcancerChallenge_HEDmap_image_alone extends JDialogBase
	 ...
	 */
	public JDialogProstateSPIEcancerChallenge_HEDmap_image_alone(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_5_test = new File("/scratch/backup/SPIEChallenge/NetherlandTrainDataPng/cg_150_mask/");
	...
	private void generateContours() {
        ...
        for(int i = 0; i < size; i++ ){
			...
			for (int j = 0; j < imageVecSize; j++) {
				...
				// new ViewJFrameImage(blankImage);
				...
				AlgorithmThresholdDual thresholdAlgo = new AlgorithmThresholdDual(resultImage, transformedMaskImage, thresholds, fillValue, outputType, regionFlag, isInverse);
				thresholdAlgo.run();
