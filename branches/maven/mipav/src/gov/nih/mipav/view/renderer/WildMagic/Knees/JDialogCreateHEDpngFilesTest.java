...
/*
 * This file convert all the 3D images (MRI GRE Images, CED Images)   
 * into 2D png files for test images. 9 folds cross validation. 
 */
public class JDialogCreateHEDpngFilesTest extends JDialogBase implements
		AlgorithmInterface {
	...
	private void readKeyImageDir() {
		...
		File fileDir_2 = new File("/home/ruida/kneesBackup/downSample/sagittal");
		traverse_folder(fileDir_2, null);
	...
	public void saveHED2DsliceCED() {
		...
		for (String key : keys) {
			...
			if (count >= startIndex && count <= endIndex) {
				...
				saveImage(scaledAxialImage, "sagittal", count);

				ModelImage axialImageCED = keyImagesOrientationCED.get(key);
				ModelImage scaledAxialImageCED = scaleIntensity(axialImageCED);
				saveImage(scaledAxialImageCED, "sagittal", count);

				// ModelImage, don't call disposeLocal to avoid garbage collection 
				// during batch image processing to slow down the speed.  
	...
	public void saveImage(ModelImage image, String orientation, int count) {
		...
		for (int j = 0; j < zDim; j++) {
			...
			try {
				...
				String sliceDir = saveImageDirectory + count + File.separator;
