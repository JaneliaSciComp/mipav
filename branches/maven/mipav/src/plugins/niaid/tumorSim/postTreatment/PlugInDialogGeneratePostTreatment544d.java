...
import niaid.tumorSim.createMap.PlugInAlgorithmCreateSpheres701a;
 ...
 */
public class PlugInDialogGeneratePostTreatment544d extends JDialogScriptableBase implements AlgorithmInterface {
    ...
    private PlugInAlgorithmGeneratePostTreatment544d generatePostAlgo = null;
     ...
     */
    public PlugInDialogGeneratePostTreatment544d() { }
     ...
     */
    public PlugInDialogGeneratePostTreatment544d(Frame theParentFrame, ModelImage im) {
...
//  ~ Methods --------------------------------------------------------------------------------------------------------
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        if (algorithm instanceof PlugInAlgorithmGeneratePostTreatment544d) {
    ...
    protected void callAlgorithm() {
        ...
        try {
            ...
            generatePostAlgo = new PlugInAlgorithmGeneratePostTreatment544d(image1, image1Tumor, image1Intensity, image1IntensityStd, image1Scale, image1Noise, image1ThresholdLower, image1ThresholdUpper, image1cVOI, 
    ...
    private void init() {
        ...
        setTitle("Generate post treatment map 544d");
        try {
    ...
    public JPanel buildMainPanel(boolean doOKCancel, GuiBuilder gui) {
        ...
        try {
            int loc = intenSearch.lastIndexOf(PlugInAlgorithmCreateSpheres701a.INTENSITY1);

            intensity1 = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.INTENSITY1.length(), loc = intenSearch.indexOf('\t', loc)).trim());
            loc++;
            intensity1Std = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.STD_DEV.length(), loc = intenSearch.indexOf(';', loc)).trim());
            loc+=2;
            intensity2 = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.INTENSITY2.length(), loc = intenSearch.indexOf('\t', loc)).trim());
            loc++;
            intensity2Std = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.STD_DEV.length(), loc = intenSearch.indexOf(';', loc)).trim());
            loc+=2;
            normalTissue = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.NORMAL_TISSUE.length(), loc = intenSearch.indexOf('\t', loc)).trim());
            loc++;
            normalTissueStd = Double.valueOf(intenSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.STD_DEV.length(), intenSearch.indexOf(';', loc)).trim());
        } catch(Exception e) {
            intensity1 = 109;
        ...
        try {
            int loc = noiseSearch.lastIndexOf(PlugInAlgorithmCreateSpheres701a.NOISE_LEVEL);
            double noise = Double.valueOf(noiseSearch.substring(loc+PlugInAlgorithmCreateSpheres701a.NOISE_LEVEL.length(), noiseSearch.indexOf(';', loc)).trim());
            noise1 = noise2 = noise;
        } catch(Exception e) {
            noise1 = noise2 = .05;
        }
