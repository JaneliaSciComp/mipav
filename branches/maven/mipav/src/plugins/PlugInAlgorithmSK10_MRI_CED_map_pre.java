 ...
 * 
 */
public class PlugInAlgorithmSK10_MRI_CED_map_pre extends AlgorithmBase {
    ...
    private PluginDialogSK10_MRI_CED_map_pre parentDialog;
     ...
     */
    public PlugInAlgorithmSK10_MRI_CED_map_pre(String _inputDirImage, String _inputDirMap, String _outputDir, 
    		PluginDialogSK10_MRI_CED_map_pre _parentDialog) {
	...
	private void generateContours() {
		...
		for (String key : keys) {
			...
			for (int j = 0; j < imageDim; j++) {
				...
				// get the MRI image predicted map
				ModelImage image = imageVec.get(j);
				// get the CED image predicteed map
				ModelImage cedImage = imageVec.get(j + imageDim);

				// blank image to hold the MRI prediction. 
				...
				// blank image to hold the CED prediction.
				...
				// max Mask image to merge the MRI and CED prediction, and thredhold the 
				// probablity map intensity range from 240 to 255 for femur mask.
				// Threshold range from 250 to 255 will also do.  
				...
				AlgorithmImageCalculator mathAlgo = new AlgorithmImageCalculator(maxMaskImage, blankImage,
						blankImage_ced, opType, clipMode, true, adOpString);
				mathAlgo.run();
				...
				AlgorithmThresholdDual thresholdAlgo = new AlgorithmThresholdDual(resultImage, maxMaskImage, thresholds,
				...
				if (resultImage.getVOIs() != null) {
					...
					for (int idx = 0; idx < resultImage.getVOIs().size(); idx++) {
						...
						int s = current_va.size();

						for (int q = 0; q < s; q++) {
							VOIBase current_v = current_va.get(q);
	...
	public void readFile() {
		...
		try {
			...
			// read the HNN predicted probablity maps
	...
	private void processDir_folder_map(File dir, int index) {
		...
		if (dirName.substring(begin, end).startsWith("image") && dirName.substring(begin, end).endsWith(".png")
	...
	private void read_image_name(File dir) {
		...
		if (fileName.startsWith("image") && fileName.endsWith("xml") && !fileName.contains("ced")
