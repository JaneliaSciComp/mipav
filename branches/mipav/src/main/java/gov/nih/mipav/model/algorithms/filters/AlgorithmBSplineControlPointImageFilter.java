...
import gov.nih.mipav.util.MatrixUtil;
	    ...
	    public void setSplineOrder(int[] order) {
	    	...
	    	for (int i = 0; i < nDims; i++ ) {
	            ...
	            if (doMultiLevel) {
	                ...
	                RMat = RMat.transpose();
	                RMat = MatrixUtil.flipud(RMat);
	                Matrix SMat = new Matrix(S);
	                SMat = SMat.transpose();
	                SMat = MatrixUtil.flipud(SMat);
	                ...
	                refinedLatticeCoefficients[i] = MatrixUtil.extract(2, SMat.getColumnDimension(), QMat);
	            } // if (doMultiLevel)
	    	} // for (i = 0; i < nDims; i++)
	    }
