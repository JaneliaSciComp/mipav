 ...
 * into image pairs ( MR slice with binary mask, CED slice with binary mask ).  Those image pairs are
 * used to train the HED model.  This class prepare the HED training folds for cross validation test. 
 * 
 * @author Ruida Cheng
 *
 */
public class JDialogProstateXReRunWholeProstateTrainPatches extends JDialogBase implements
		AlgorithmInterface {
	 ...
	 */
	public JDialogProstateXReRunWholeProstateTrainPatches(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
		...
		File fileDir_2 = new File("/scratch/prostateXdata/trainImageCg/121_146/coronal");
	...
	public void saveHED2DsliceCED() {
		...
		for (String key : keys) {
			...
			// if (count >= startIndex && count <= endIndex) {
				// normalize the image in range of [0, 1000]
				...
				saveImage(scaledAxialImage, axialImageMask, "coronal");
				...
				saveImage(scaledAxialImageCED, axialImageMaskCED, "coronal");
	...
	public void readFile() {
		...
		try {
			...
			for (String key : keys) {
				System.err.println("coronal ced key = " + key);
			...
			for (String key : keys) {
				System.err.println("coronal mask key = " + key);
