 ...
 * */
public class ImgReaderRAW extends ImgReader {

    /** DC offset value used when reading image */
    public static int DC_OFFSET = 32768;//128;
     ...
     * */
    public ImgReaderRAW(File file) throws IOException {
     ...
     * @exception IOException If an error occurs while opening the file.
     * */
    public ImgReaderRAW(String fname) throws IOException {
        
    	this(new RandomAccessFile(fname.substring(0,fname.lastIndexOf('.')) + ".raw","r"));
    }
     ...
     * */
    public ImgReaderRAW(RandomAccessFile in) throws EOFException, IOException {
        this.in = in;
        this.w = 256;
        this.h = 256;
/*
...
*/
    ...
    public ImgReaderRAW(ModelImage image){
		int[] imgExtents;
		...
		this.w = imgExtents[0];
		this.h = imgExtents[1];
		this.numSlice = imgExtents[2];
		
		//this.dataType = image.getType();
...
*/
	...
	if (barr == null || barr.length < blk.w*blk.h) {
        ...
        try {
            // Read the 1st "layer" (or component, in old term)line by line
            mi = blk.uly + blk.h;
            for (i = blk.uly; i < mi; i++) {
                ...
                for (k = (i-blk.uly)*blk.w+blk.w-1, j = 2*blk.w-1;
                    ...
                    // barr[k] = ((((int)buf[j])&0xFF)*256) + ((int)buf[j-1]&0xFF) -DC_OFFSET;
                	barr[k] = ((((buf[j--]&0xFF)<<8)|(buf[j--]&0xFF)));
                    //barr[k] = barr[k]/2; 
                }
            }
        }
