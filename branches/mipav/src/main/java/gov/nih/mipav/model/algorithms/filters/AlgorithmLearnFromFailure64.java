...
public class AlgorithmLearnFromFailure64 extends AlgorithmBase implements AlgorithmInterface {
	...
	public AlgorithmLearnFromFailure64(ModelImage _srcImage, int _imageNum, int _sliceNum, float min, float max, boolean train, 
	...
	public void runAlgorithm() {
		...
		/*
		else if ( srcImage.getVOIs().size() == 2 ) {
			smoothVOI128double(srcImage, srcImage);
			generatePatches();
		    learnFromFailure();
		}
		*/
	...
	public void smoothVOI128double(ModelImage maskImage, ModelImage resultImage) {
	...
	private void learnFromFailure() {
		...
		// System.err.println("nPtsAAM = " + nPtsAAM);
		
		if ( nPtsGT != 128 || nPtsAAM != 128) return;
	    ...
	    // for (i = 10; i < 11; i++) {
			...
			for (int k = 0; k < 19; k++ ) {
				...
				if ( training ) {
					...
					if ( k == 0 ) {
						extractImage8bitsTest((int) newXPts[0], (int) newXPts[2], (int) newYPts[0], (int) newYPts[2], ClassType.NEG,outNormPt.X, outNormPt.Y);
						// squareOuter.importArrays(newXPts, newYPts, newZPts, 4);
						// squareVOINeg.importCurve(squareOuter);
					} else {
						extractImage8bitsTest((int) newXPts[0], (int) newXPts[2], (int) newYPts[0], (int) newYPts[2], ClassType.UNDEFINED, outNormPt.X + k * normStep.X / 19f, outNormPt.Y + k * normStep.Y / 19f);
						// squareOuter.importArrays(newXPts, newYPts, newZPts, 4);
						// squareVOIUncertain.importCurve(squareOuter);
					}
				}
			...
			if ( training ) {
				...
				extractImage8bitsTest((int) newXPts[0], (int) newXPts[2], (int) newYPts[0], (int) newYPts[2], ClassType.POS, interpPt.X, interpPt.Y);
			}
			patchNum++;
			...
			for (int k = 18; k >=0; k--) {
				...
				if (training) {
					...
					if ( k == 0 ) {
						extractImage8bitsTest((int) newXPts[0], (int) newXPts[2], (int) newYPts[0], (int) newYPts[2], ClassType.NEG,inNormPt.X, inNormPt.Y);
						// squareInner.importArrays(newXPts, newYPts, newZPts, 4);
						// squareVOINeg.importCurve(squareInner);
				
					} else {
						extractImage8bitsTest((int) newXPts[0], (int) newXPts[2], (int) newYPts[0], (int) newYPts[2], ClassType.UNDEFINED, inNormPt.X - k * normStep.X / 19f, inNormPt.Y - k * normStep.Y / 19f);
						// squareInner.importArrays(newXPts, newYPts, newZPts, 4);
						// squareVOIUncertain.importCurve(squareInner);
					}
				}
