...
/*  This is a port of the SUSAN Nonlinear Noise reduction program.  SUSAN noise
    reduction uses nonlinear filtering to reduce noise in an image (2D or 3D)
    while preserving the underlying structure.  It does this by only averaging
    a voxel with local voxels which have similar intensity. The original code
    could only handle integer data going from 0 to 255.  2 main changes were
    made to allow any type of black and white data.  In setupBrightness
    temp = ((float)k)/(float)brightThres);
    was changed to:
    temp=(float)((k*(srcMax-srcMin))/(bpSize * brightThres));
    In smoothing and smoothing3d
    bp[Math.abs(center-brightness)]
    was changed to:
    bp[(int)Math.abs(256*(center-brightness)/(srcMax - srcMin))];
    Also note that using the Math.abs allowed changing the bp
    allocation from bpSize*2 + 1 to bpSize + 1. In the dialog
    the default value of brightThres was set equal to
    0.1 * (srcMax - srcMin).

    The area of a mask excluding the center which has a same or similar intensity value as the
    center of the mask is known as the USAN.  The SUSAN filter works by taking
    an average of the pixels in the locality which lie in the USAN.
    The SUSAN filter uses a Gaussian in both the brightness and spatial domains.
    If useMedian is true, the sums taken over the local neighborhod do not include the center pixel
    itself.  This allows good reduction of impulse noise. If the USAN area is
    zero and useMedian is true, then the median of the pixel's 8 nearest neighbors
    in 2D or 26 nearest neighbors in 3D is used to estimate the pixel's correct value.
    If useMedian is false, then the sums include the center pixel.  If useMedian is false and
    the USAN is zero, then the pixel remains unchanged.

    For useMedian == true the complete equation for the SUSAN filter is:
    J(x,y) = num/denom,
    where num = sum(i,j)!=(0,0)I(x+i,y+j)*exp[-(r*r/2*sigma*sigma) - (I(x+i,y+j) - I(x,y))**2/thresh*thresh]
    denom = sum(i,j)!=(0,0) exp[-(r*r/2*sigma*sigma) - (I(x+i,y+j) - I(x,y))**2/thresh*thresh]
    with r = sqrt(i*i + j*j)
    and the above replacement rule is used when the denominator or USAN is zero.
    If useMedian == false, the restriction (i,j) != (0,0) in the above is removed.

    The SUSAN filter tends to preserve edges.  Pixels are pulled toward the neighboring
    region to which they are closest in value.
 ...
 * Computer Vision", pages = "45--78", volume = "23", number = "1", month = "May", year = 1997} To be registered for
 * automatic (bug) updates of SUSAN, send an email. Note: FDT is the image data type Note: edge and corner finding have
 * been taken out of the 3D version of SUSAN - it is not thought that they would be of great value. See following
 * section for different machine information. Please report any bugs (and fixes). There are a few optional changes that
 * can be made in the "defines" section which follows shortly. This code is written using an emacs folding mode, making
 * moving around the different sections very easy. This is why there are various marks within comments and why comments
 * are indented. SPATIAL CONTROL: d In SUSAN smoothing d controls the size of the Gaussian mask; its default is 4.0.
 * Increasing d gives more smoothing. In edge finding, a fixed flat mask is used, either 37 pixels arranged in a
 * "circle" (default), or a 3 by 3 mask which gives finer detail. In corner finding, only the larger 37 pixel mask is
 * used; d is not variable. In smoothing, the flat 3 by 3 mask can be used instead of a larger Gaussian mask; this gives
 * low smoothing and fast operation. BRIGHTNESS CONTROL: t In all three algorithms, t can be varied (default=20); this
 * is the main threshold to be varied. It determines the maximum difference in greylevels between two pixels which
 * allows them to be considered part of the same "region" in the image. Thus it can be reduced to give more edges or
 * corners, i.e. to be more sensitive, and vice versa. In smoothing, reducing t gives less smoothing, and vice versa.
 * Set t=10 for the test image available from the SUSAN web page. ITERATIONS: With SUSAN smoothing, more smoothing can
 * also be obtained by iterating the algorithm several times. This has a different effect from varying d or t.
 * BRIGHTNESS FUNCTION LUT IMPLEMENTATION: (Only read this if you are interested in the C implementation) The SUSAN
 * brightness function is implemented as a LUT (Look-Up-Table) for speed. The resulting pointer-based code is a little
 * hard to follow, so here is a brief explanation. In setupBrightness() the LUT is setup. This mallocs enough space for
 * *bp and then repositions the pointer to the center of the malloced space. The SUSAN function e^-(x^6) or e^-(x^2) is
 * calculated and converted to a unsigned char in the range 0-100, for all possible image brightness differences
 * (including negative ones). Thus bp[23] is the output for a brightness difference of 23 greylevels. In the SUSAN
 * algorithms this LUT is used as follows: p=in + (i-3)*x_size + j - 1; p points to the first image pixel in the
 * circular mask surrounding point (x,y). cp=bp + in[i*x_size+j]; cp points to a position in the LUT corresponding to
 * the brightness of the center pixel (x,y). now for every pixel within the mask surrounding (x,y), n+=*(cp-*p++); the
 * brightness difference function is found by moving the cp pointer down by an amount equal to the value of the pixel
 * pointed to by p, thus subtracting the two brightness values and performing the exponential function. This value is
 * added to n, the running USAN area. in SUSAN smoothing, the variable height mask is implemented by multiplying the
 * above by the moving mask pointer, reset for each new center pixel. tmp = *dpt++ * *(cp-brightness);
 */
...
public class AlgorithmNLNoiseReduction extends AlgorithmBase {
    ...
    /** DOCUMENT ME! */
    private double brightThres; // brightness threshold
    ...
    /** DOCUMENT ME! */
    private float maskStdDev; // spatial size of smoothing - maskStdDev
     ...
     * @param  bt         Brightness threshold: this allows discrimination between noise and the underlying image.
     *                    Ideally, the value should be set greater than the noise level and less than the contrast of
     *                    the underlying image. Edges of contrast smaller than this threshold will be blurred whereas
     *                    those of greater contrast will not be. Reducing bt gives less smoothing. bt has a default
     *                    value = 0.1 * (srcMax - srcMin).
     * @param  maskSD     This determines the spatial extent of the smoothing. The mask is basically Gaussian with
     *                    standard deviation (in image units - e.g. mm.) set by the user. However, for a small, fast,
     *                    flat response with a 3x3 or 3x3x3 voxel mask, set maskSD to zero. maskSD has a default value
     *                    equal to the x resolution.
     ...
     *                    impulse("salt-and-pepper")noise. If false, this feature is turned off. In this case, the
     *                    center pixel is included in the sums. When no neighborhood is found, the original intensity of
     *                    the voxel of interest remains unchanged.
     ...
     *                    2D images disregard this flag.
     */
    public AlgorithmNLNoiseReduction(ModelImage destImg, ModelImage srcImg, double bt, float maskSD, boolean useMedian,
                                     boolean img25D) {
    ...
    private void addBorder(float[] in, float[] tmpEnImage, int border) {
        ...
        for (i = 0; i < border; i++) { // copy top and bottom rows; invert as many as necessary
            ...
            for (j = 0; j < xSize; j++) {
                ...
                tmpEnImage[((ySize + border + i) * (xSize + (2 * border))) + border + j] = in[((ySize - i - 1) *
                                                                                                   xSize) + j];
            }
        }
        ...
        for (i = 0; i < border; i++) { // copy left and right columns
            ...
            for (j = 0; j < (ySize + (2 * border)); j++) {
                tmpEnImage[(j * (xSize + (2 * border))) + border - 1 - i] = tmpEnImage[(j * (xSize + (2 * border))) +
                                                                                       border + i];
                tmpEnImage[(j * (xSize + (2 * border))) + xSize + border + i] = tmpEnImage[(j *
                                                                                                (xSize + (2 * border))) +
                                                                                           xSize + border - 1 - i];
            }
        }
    ...
    private void addBorder3D(float[] data, float[] tmpEnImage, int border) {
        ...
        for (k = 0; k < border; k++) {
            ...
            for (j = 0; j < ((xSize + (2 * border)) * (ySize + (2 * border))); j++) {
                tmpImage[((border - 1 - k) * (xSize + (2 * border)) * (ySize + (2 * border))) + j] = tmpImage[((border +
                                                                                                                k) *
                                                                                                                   (xSize +
                                                                                                                        (2 *
                                                                                                                             border)) *
    ...
    private float median3D(float[] data, int x, int y, int z) {
        ...
        do {
            ...
            for (i = inc + 1; i <= N; i++) {
                ...
                while (medianVal[j - inc] > tmp) {
                    medianVal[j] = medianVal[j - inc];
                    j -= inc;
    ...
    private void smoothing2D(int z) {
        ...
        if (z == 0) {
            ...
            for (i = -yMaskSize; i <= yMaskSize; i++) {
                ...
                for (j = -xMaskSize; j <= xMaskSize; j++) {
                    gaussMask[maskPt++] = (byte) (100.0 *
                                                      Math.exp((((float) (i * i)) / yStdDevLocal) +
                                                                   (((float) (j * j)) / xStdDevLocal)));
                }
            }
        }
        ...
        double index;
        double constant = 256.0 / (srcMax - srcMin);
        ...
        for (k = maskSize; (k < (zSize - maskSize)) && !threadStopped; k++) { // use maskSize as enlarge was isotropic
            ...
            for (i = maskSize; (i < (ySize - maskSize)) && !threadStopped; i++) {
                newProgressValue = 100 *
                                       (((k - maskSize) * smallSliceSize) +
                                            ((i - maskSize) * (xSize - (2 * maskSize)))) /
