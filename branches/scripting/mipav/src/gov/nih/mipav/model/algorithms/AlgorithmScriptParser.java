...
/**
 * This class parses a script and runs it. A script is parsed based on the newline charater; thus, each line starts with
 * either a command or a "#", the "#" indicating a comment. Any lines beginning with a "#" are ignored. Commands are
 * strings like " Image". Tokens necessary for running the command, such as "$image1" for "OpenImage", are separated by
 * spaces or tabs.
 *
...
public class AlgorithmScriptParser extends AlgorithmBase {
     ...
     */
    public boolean getNextBoolean() throws Exception {
        boolean value;
     ...
     * @throws  Exception  DOCUMENT ME!
     */
    public int getNextInteger() throws Exception {
        int value;
        ...
        if ((scriptDir != null) && (scriptName != null)) {
            ...
            while (st.hasMoreTokens()) {
                ...
                if (!line.startsWith("#")) {
                    ...
                    if (command.equals("OpenImage") || command.equals("OpenMultiFile") || command.equals("LoadImage")) {
                        numImages++;
                    }
                }
            }
    ...
    /**
     * Runs the script parser. Opens the script file and calls <code>parseLine</code> for each line in the script.
     */
    ...
    /**
     * Sets the directory for current script parsing. This method is called by JDialogMultiple, which goes through a
     * series of files and runs a script on them.
     *
    ...
    /**
     * Sets the file name for current script parsing. This method is called by JDialogMultiple, which goes through a
     * series of files and runs a script on them.
     *
    ...
    /**
     * Sets the file type of the image for current script parsing. No one is calling this right now but we made need it
     * in the future.
     *
    ...
    /**
     * Sets the save extension for current script parsing. This method is called by JDialogMultiple, which goes through
     * a series of files and runs a script on them. When the files are saved, this will be prepended to the file name
     * minus the extension.
     *
    ...
    public void setVoiNames(Vector names) {
        ...
        /*
         * voiNames = new Vector[names.length]; for (int i = 0; i < names.length; i++) { voiNames[i] = new Vector(); }
         */
        voiNames = names;
    }
    ...
    /**
     * * Sets up the write options for a save call. Reads in the necessary information from the string* tokenizer.*
     *
     ...
     * @throws  Exception  DOCUMENT ME!
     */
    protected FileWriteOptions getWriteOptions(String name, boolean saveAs) throws Exception {
        ...
        else if ((num == 3) && (image.getNDims() == 4) &&
                     ((fileType == FileBase.TIFF) || (fileType == FileBase.MINC))) {
            opts.setBeginSlice(getNextInteger());
            opts.setEndSlice(getNextInteger());
     ...
     */
    protected ScriptableInterface loadScriptableClass(String algoString) throws ClassNotCreatedException {
        ...
        try {
            ...
            try {
                ...
                throw new ClassNotCreatedException("Unable to instantiate class " + err.getMessage() +
            ...
            throw new ClassNotCreatedException("Unable to instantiate class " + e.getMessage() +
                                               ". An empty constructor is likely needed.");
        } catch (ClassCastException e) {
            throw new ClassNotCreatedException("JDialog" + algoString +
                                               " does not allow itself to be scripted.  See ScriptableInterface.");
        }
    ...
    /**
     * Parses the line with a series of "if...else..." statements on the command. The command is the first token of the
     * string <code>line</code>. If the line begins with a "#", everything after that is ignored; "#" is meant to
     * indicate a comment.
     *
        ...
        if (command.equals("Prefix")) { }
            ...
            try {
                ...
                if (io.getFileType((String) fileNames.elementAt(currFileIndex),
                    ...
                    name = fileUI.open(((String) fileDirs.elementAt(currFileIndex)) +
                                       ((String) fileNames.elementAt(currFileIndex)), false, null);
                }
            ...
            try {
                ...
                if (io.getFileType((String) fileNames.elementAt(currFileIndex),
                    ...
                    name = fileUI.open(((String) fileDirs.elementAt(currFileIndex)) +
                                       ((String) fileNames.elementAt(currFileIndex)), true, null);
                }
            ...
            try {
                ...
                e.printStackTrace();
                MipavUtil.displayError("Error in script file near \"SaveImage\".  Image " + name +
            ...
            try {
                ...
                // Ruida
                /*
                 * ViewOpenVOIUI voiUI = new ViewOpenVOIUI(UI); VOI vois[], firstVoi; vois = voiUI.open(image); firstVoi
                 * = vois[0]; String voiName = firstVoi.getName();
                 */
                key = getNextString();
    ...
    protected class ClassNotCreatedException extends Exception {
        ...
        private static final long serialVersionUID = 4694534827480286162L;
    ...
    class VariableHashtable extends Hashtable {
        ...
        /** Use serialVersionUID for interoperability. */
        private static final long serialVersionUID = 9072617555105150270L;
        ...
        /**
         * Constructs a new hashtable with the same mappings as the given Map. The hashtable is created with a capacity
         * of twice the number of entries in the given Map or 11 (whichever is greater), and a default load factor,
         * which is 0.75.
         *
        ...
        /**
         * Override the get method for Hashtable. Get the image assigned to the variable and return it. A special case
         * is the $active variable which returns the name of the current active image.
         *
