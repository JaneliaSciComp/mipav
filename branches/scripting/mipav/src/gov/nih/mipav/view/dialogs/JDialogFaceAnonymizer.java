...
/**
 * DialogFaceAnonymizer class creates a dialog which launches the FaceAnonymizer Algorithm class. In the dialog the the
 * user can specify the direction in the (x,y) plane that the face points toward - default is +x. The user also
 * specifies the min/max values for "skin", as well as a maximum thickness and blur factor.
 *
 * @version  May 17, 2004
...
public class JDialogFaceAnonymizer extends JDialogBase implements AlgorithmInterface, ScriptableInterface {
    ...
    private static final long serialVersionUID = -280179402911953866L;
    ...
    /**
     * boolean contains state of the dialog this is done so that the dialog can stay open while the Algorithm is finding
     * and bluring the results so that the user can undo either operation.
     */
    ...
    /** Radio buttons to specify the direction in (x,y) plane that the face is pointing toward:. */
    ...
    public void callAlgorithm() {
        ...
        try {
            ...
            if (m_bAlgorithmLaunched == false) {
                m_kFaceAnonymizerAlgorithm = new AlgorithmFaceAnonymizer(image, m_aiAxis, m_iSkinMin, m_iSkinMax,
            ...
            if (isRunInSeparateThread() && (m_bAlgorithmLaunched == false)) {
                ...
                if (m_kFaceAnonymizerAlgorithm.startMethod(Thread.MIN_PRIORITY) == false) {
            ...
            System.gc();
            MipavUtil.displayError("Dialog Anonymize Face : unable to allocate enough memory");
     ...
     * DOCUMENT ME!
     *
     * @param  m_iSkinThickness  DOCUMENT ME!
     */
    ...
    private void init() {
        ...
        optionsPanel.setBorder(buildTitledBorder("Face Posterior To Anterior Orientation"));
         ...
         * JLabel kBlurLabel = new JLabel("Blur in voxel neighborhood"); kBlurLabel.setFont(serif12);
         * paramPanel.add(kBlurLabel, gbc);
         *
         * gbc.gridx = 1; m_kBlurFactor = new JTextField(); m_kBlurFactor.setText("3");
         * m_kBlurFactor.setFont(serif12);paramPanel.add(m_kBlurFactor, gbc);*/
        ...
        // Blur face button
        /*gbc.gridx = 2;
         * JButton blurButton = new JButton( BLUR_BUTTON ); paramPanel.add( blurButton, gbc
    ...
    private boolean setVariables() {
        ...
        if (testParameter(tmpStr, 1, Math.min(image.getExtents()[0], image.getExtents()[1]))) {
         ...
         * if ( testParameter( tmpStr, 1,                 Math.min( image.getExtents()[0],
         * image.getExtents()[1] )/2.0 ) ) { m_iBlurFactor = Integer.valueOf( tmpStr ).intValue(); } else {
         * m_kBlurFactor.requestFocus(); m_kBlurFactor.selectAll(); return false;}*/
