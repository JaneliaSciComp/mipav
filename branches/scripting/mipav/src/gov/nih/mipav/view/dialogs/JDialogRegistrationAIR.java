 ...
 * DOCUMENT ME!
 */
public class JDialogRegistrationAIR extends JDialogBase implements AlgorithmInterface, ScriptableInterface {

    //~ Static fields/initializers -------------------------------------------------------------------------------------

    /** Use serialVersionUID for interoperability. */
    private static final long serialVersionUID = -1760047244477302856L;
    ...
    /** DOCUMENT ME! */
    private JComboBox comboBoxDOF, comboBoxInterp, comboBoxCostFunct, comboBoxPart;
        ...
        if (algo.isCompleted()) {
            ...
            if (UI.isScriptRecording()) {
                ...
                UI.getScriptDialog().append(UI.getScriptDialog().getVar(matchImage.getImageName()) + " " +
                                            ...
                                            entireSource + " " + entireTarget + " " + transformation + " " +
                                            interpolation + " " + sourceThreshold + " " + targetThreshold + " " +
                                            sourceGaussX + " " + sourceGaussY + " " + sourceGaussZ + " " +
                                            targetGaussX + " " + targetGaussY + " " + targetGaussZ + " " +
                                            sourcePartitions + " " + targetPartitions + " " + costFxn + " " +
                                            cubicInterpolation + " " + interaction + " " + precision + " " +
                                            iterations + "\n");
            }
        }
    }
    ...
    /**
     * Method to handle item events. If image name was changed in the combo box, reset labels for target threshold. If
     * partition was changed, disable the necessary check boxes.
     *
            ...
            switch (comboBoxPart.getSelectedIndex()) {
                ...
                case 1: // forward fit, 0 source partitions
                    if (sourceVOICheckBox.isSelected()) {
                        ...
                        MipavUtil.displayWarning("Masking of the source files is disabled because you have" +
                ...
                case 2: // reverse fit, 0 target partitions
                    if (targetVOICheckBox.isSelected()) {
                        ...
                        MipavUtil.displayWarning("Masking of the target files is disabled because you have" +
                                                 " set its number of partitions to zero");
                    }
     ...
     *
     * @param  flag  <code>true</code> means resample to cubic voxels.
     */
    ...
    private void callAlgorithm() {
        ...
        if (cubicInterpolation) {
            ...
            if ((refImage.getNDims() > 2) && ((targetUnitsX != targetUnitsY) || (targetUnitsX != targetUnitsZ))) {
                doConvert = true;
            } else if ((refImage.getNDims() == 2) && (targetUnitsX != targetUnitsY)) {
                doConvert = true;
            }
            ...
            if (doConvert) {
                ...
                if ((targetUnitsZ == FileInfoBase.INCHES) || (targetUnitsZ == FileInfoBase.CENTIMETERS) ||
        ...
        try {
            ...
            air = new AlgorithmRegAIR(resultImage, refImage, entireTarget, matchImage, entireSource, transformation,
            ...
            MipavUtil.displayError("Dialog RegistrationAIR: unable to allocate enough memory");
    ...
    private void init() {
        ...
        if (matchImage.getNDims() == 2) {
            cubicInterpolationCheckBox = new JCheckBox("Interpolation to square pixels");
        } else {
            cubicInterpolationCheckBox = new JCheckBox("Interpolation to cubic voxels");
        }
    ...
    private boolean setVariables() {
         ...
         * Use 256 partitions for intermodality, MRI to PET or for correlation ratio. Bidirectional fits are generally
         * recommended.  However, if you have edited the target image to remove areas of pathology that you think will
         * interfere with registration, choose forward fit. Likewise, if you have edited the source image to remove
         * pathology,choose reverse fit */
