 ...
 */
public class JDialogRunScriptView extends JDialogBase implements Observer{
    ...
    DropJTreeLister dropListener;    
    public JTree tree = null;       
    public MipavScriptTreeNode root;    
    JDialogRunScriptController controller;   
    int numberOfExecuters = 0;     
     
    public  int FRAME_WIDTH = 725;
    public  int FRAME_HEIGHT = 800;
    
   // Dimension initial_frame_size = new Dimension(725,800);
    Dimension tree_size = new Dimension();
    Dimension scroll_size = new Dimension();
    
    
    Dimension frame_size;
    
    final String SCRIPTNODE = "Script Exectuer";
    final String IMAGENODE = "ImageNode";
    final String VOINODE = "VioNode";
    final String ROOTNODE = "Root";


    /*
    public  int TREE_WIDTH = (int)(FRAME_WIDTH * .6);//450;
    public  int TREE_HEIGHT = (int)(FRAME_HEIGHT * .8);//700;
    public  int SCROLL_WIDTH = (int)(FRAME_WIDTH * .30);//250;
    public  int SCROLL_HEIGHT = (int)(FRAME_HEIGHT * .36);//175;*/

    MipavScriptTreeNode[] imageNodes;
    MipavScriptTreeNode voi = null;    
    String[] imagePlaceHolders;
    int[] numberofVOIs;
    JScrollPane treeScroll;   
    String scriptFile;   
    int centralBuffer;    
    int[] selectedListIndicies;    
    JComponent dropSourceComponent;   
    JDialogRunScriptModel model;
    DefaultListModel listModel;
    
    public JDialogRunScriptView(String scriptFile, JDialogRunScriptController controller, JDialogRunScriptModel model) {
    ...
    private void displayView(String scriptFile) {
        ...
        createScriptTree(scriptFile, model.getImagePlaceHolders(), model.getNumberOfVOIs());
        
        String[] listName = {"Images List","VOIs from above image List"};//, "VOI List"};
    ...
    private void addScrollList(String name){
     ...
     public void createScriptTree(org.w3c.dom.Document savedScript){
        ...
        for (int n=root.getChildCount()-1;n>=0;n--){
            ((DefaultTreeModel)tree.getModel()).removeNodeFromParent((MipavScriptTreeNode)root.getChildAt(n));
        }
        ...
        for (int i=1, j=0;i<savedScript.getDocumentElement().getChildNodes().getLength();i+=2, j++){
             ((DefaultTreeModel)tree.getModel()).insertNodeInto(
                    restoreSavedExecuter(savedScript.getDocumentElement().getChildNodes().item(i)),root, j);
    ...
    public void createScriptTree(String scriptFile, String[] imagePlaceHolders, int[] numberofVOIs) {
        ...
        treeScroll = new JScrollPane(tree,JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED){
             public Dimension getPreferredSize() {
                changeSize();
               return tree_size;
                }
 ...
 */
    ...
    private void setLayoutConstraints(Component component){
        ...
        if (component.getName().equalsIgnoreCase("Script Execution Setup")) {
            ...
            setSprings(component, getComponentByName("Script Execution Setup"),getComponentByName("Images List: scroll"), 0, 0);
        }

        else if (component.getName().equalsIgnoreCase("VOIs from selected image")) {
             layout.putConstraint(SpringLayout.NORTH, component, 5, SpringLayout.SOUTH, getComponentByName("Add image from file"));
            layout.putConstraint(SpringLayout.WEST, component,  0, SpringLayout.WEST, getComponentByName("Images List: scroll"));      
        }
        ...
        layout.putConstraint(SpringLayout.NORTH, component, 5 ,SpringLayout.SOUTH, getComponentByName("Script Tree: scroll"));
        layout.putConstraint(SpringLayout.WEST, component, 0 ,SpringLayout.WEST, getComponentByName("Script Tree: scroll"));      
        }
       
        
        else if (component.getName().equalsIgnoreCase("Add new script executer")){
            layout.putConstraint(SpringLayout.NORTH, component, 0 ,SpringLayout.NORTH, getComponentByName("Run Script"));
            ...
            layout.putConstraint(SpringLayout.NORTH, component, 5 ,SpringLayout.SOUTH, getComponentByName("Images List: scroll"));
             layout.putConstraint(SpringLayout.WEST, component, 0 ,SpringLayout.WEST, getComponentByName("Images List: scroll"));      
            }
            ...
            layout.putConstraint(SpringLayout.NORTH, component, 0 ,SpringLayout.NORTH, getComponentByName("Run Script"));
            layout.putConstraint(SpringLayout.WEST, component,  centralBuffer, SpringLayout.EAST, getComponentByName("Script Tree: scroll"));      
            }
        
        
        else if (component.getName().equalsIgnoreCase("Images List: scroll")){
            layout.putConstraint(SpringLayout.NORTH, component, 0, SpringLayout.NORTH, getComponentByName("Script Tree: scroll"));
            layout.putConstraint(SpringLayout.WEST, component,  centralBuffer, SpringLayout.EAST, getComponentByName("Script Tree: scroll"));      
        }
        
        else  if (component.getName().equalsIgnoreCase("VOI List: scroll")){
           layout.putConstraint(SpringLayout.SOUTH, component, 0, SpringLayout.SOUTH, getComponentByName("Script Tree: scroll"));
           layout.putConstraint(SpringLayout.NORTH, component, 20, SpringLayout.SOUTH, getComponentByName("VOIs from above image List: scroll"));
           layout.putConstraint(SpringLayout.WEST, component,  0, SpringLayout.WEST, getComponentByName("Images List: scroll"));      
        }
       
        else if (component.getName().equalsIgnoreCase("VOIs from above image List: scroll")){
           layout.putConstraint(SpringLayout.NORTH, component, 20, SpringLayout.SOUTH, getComponentByName("Add image from file"));
           layout.putConstraint(SpringLayout.WEST, component,  0, SpringLayout.WEST, getComponentByName("Add image from file"));      
       }
       
        else if (component.getName().equalsIgnoreCase("VOIs from File")){
           layout.putConstraint(SpringLayout.NORTH, component, 5, SpringLayout.SOUTH, getComponentByName("VOIs from above image List: scroll"));
           layout.putConstraint(SpringLayout.WEST, component,  0, SpringLayout.WEST, getComponentByName("Add image from file"));      
       }
    ...
    public void update(Observable o, Object arg){
        ...
        if ((JScrollPane)getComponentByName("Images List: scroll") == null) return;
        
        JList imageList = ((JList)((JScrollPane)getComponentByName("Images List: scroll"))
                .getViewport().getView());
        ...
        if(imageList.getSelectedIndex() == -1) {
            ...
            JList voiList = ((JList)((JScrollPane)getComponentByName("VOIs from above image List: scroll"))
                    .getViewport().getView());
  ...
  */   
    ...
    private void updateNode(MipavScriptTreeNode node, String text){
        MipavScriptTreeNode newNode = new MipavScriptTreeNode(text, node.getNodeType());
        MipavScriptTreeNode parentNode = (MipavScriptTreeNode)node.getParent();
        while (node.children().hasMoreElements()){ newNode.add((MipavScriptTreeNode)node.children().nextElement());}
 ...
 /*
  * Checks tree to see that all place holders have been replaced with actual images and VOIs
  * if not, it prompts the user with a warning message, and highlights the first node that 
  * still contains a placeholer
  */
    protected boolean parseTreeForPlaceHolders(){
        Enumeration bf = root.breadthFirstEnumeration();        
        while(bf.hasMoreElements()){            
            MipavScriptTreeNode node = node = (MipavScriptTreeNode) bf.nextElement();
             if ((node.toString().contains("$Image")) || (node.toString().contains("VOI Needed"))){
                 tree.addSelectionPath(new TreePath(node.getPath()));
                JOptionPane.showMessageDialog(frame, "PlaceHolders still found.", "MIPAV Warning", JOptionPane.WARNING_MESSAGE);
                 return true;
            }
        }
        return false;
    ...
    */   
    private MipavScriptTreeNode restoreSavedExecuter(Node executer){
       ...
       MipavScriptTreeNode newNode = new MipavScriptTreeNode("Script Executer", this.SCRIPTNODE);      
       MipavScriptTreeNode[] imageNodes = new MipavScriptTreeNode[executer.getChildNodes().getLength()];
       MipavScriptTreeNode voi = null;
       ...
       System.out.println("first Child: " + executer.getFirstChild().getNodeName());
         ...
         //loop over images
           for (int i=1;i<executer.getChildNodes().getLength();i+=2){
               ...
               imageNodes[ii] = new MipavScriptTreeNode(executer.getChildNodes().item(i).getAttributes().getNamedItem("name").getNodeValue(), this.IMAGENODE);
               ...
               //imageNodes[ii] = new MipavScriptTreeNode(executer.getChildNodes().item(i).getNodeName().replace("__", " "), this.IMAGENODE);
    ...
    private MipavScriptTreeNode createNewExecuter(String[] imagePlaceHolders, int[] numberofVOIs){
        MipavScriptTreeNode newNode = new MipavScriptTreeNode("Script Executer", this.SCRIPTNODE);
        this.numberOfExecuters++;
        MipavScriptTreeNode[] imageNodes;
        MipavScriptTreeNode voi = null;
        ...
        for (int i = 0; i < imagePlaceHolders.length; i++) {
            imageNodes[i] = new MipavScriptTreeNode(imagePlaceHolders[i], this.IMAGENODE);
   ...
   public void addExecuter(String[] imagePlaceHolders, int[] numberofVOIs){
       ...
       MipavScriptTreeNode newNode = createNewExecuter(imagePlaceHolders, numberofVOIs);
  ...
  //TODO This no longer works, need to update how it determines which node it is 
   private void applyToAllNodes(MipavScriptTreeNode selectedNode){
       ...
       for (int j=0;j<numberOfExecuters;j++){
                  ...
                  MipavScriptTreeNode node = 
               ((MipavScriptTreeNode)((MipavScriptTreeNode)((DefaultTreeModel)
                       tree.getModel()).getRoot()).getChildAt(j).getChildAt(index[1]));
          ...
          if (selectedNode.getNodeType().equalsIgnoreCase(this.IMAGENODE)){
              ...
              if (selectedNode.getNodeType().equalsIgnoreCase(this.VOINODE)){
   ...
   */  
   /*
    * class used as a DefaultMutableTreeNode, except it stores a default name that
    * can be recalled at any time if the users deletes the image/voi
    */
...
/*
     protected class ModelImageForScripting{
         ModelImage modelImage;
         
         ModelImageForScripting(ModelImage modelImage){
         ...
         public String toString(){
             return modelImage.getImageName();
         }
     }*/
    ...
    private class MyRenderer extends DefaultTreeCellRenderer {
        ...
        public MyRenderer() {
            ...
            this.scriptIcon = gov.nih.mipav.view.MipavUtil.getIcon("script.gif");
        }
        ...
        public Component getTreeCellRendererComponent(
            ...
            if (node.getUserObject().toString().equalsIgnoreCase(node.getDefaultName())) this.setForeground(Color.GRAY);
            if ((node.getUserObject().toString().contains("$Image")) || (node.getUserObject().toString().contains("$image")))setIcon(imageIcon);
            else if (model.isImage(node.getUserObject().toString())) setIcon(imageIcon); 
            else if (!(node.isRoot()) && (((MipavScriptTreeNode)node.getParent()).isRoot())) {
                setIcon(scriptIcon);
                this.setForeground(Color.BLACK);
            }
            else setIcon(voiIcon);
    ...
    private class DropJTreeLister extends DropTarget {
        public void drop(DropTargetDropEvent dtde){
            ...
            /*
             * Finds the data flavor for transfering plain text,
             * uses that to retreive the text of the image/voi being transfered
             */
            ...
            MipavScriptTreeNode oldNode = (MipavScriptTreeNode)pathTarget.getLastPathComponent();             
            String oldNodeName = (String)oldNode.getUserObject();            
            if (oldNodeName.contains("Script")) return;
            
            MipavScriptTreeNode newNode = new MipavScriptTreeNode(text, oldNode.getNodeType());
            MipavScriptTreeNode parentNode = (MipavScriptTreeNode)oldNode.getParent();
             ...
             * this image, a warning message is shown, and no action if performed
             */
            if (oldNode.getNodeType().equalsIgnoreCase(VOINODE)){
                if (!(((JList)((JScrollPane)getComponentByName("Images List: scroll"))
                    .getViewport().getView()).getSelectedValue().toString().equalsIgnoreCase
                    (parentNode.getUserObject().toString()))){
            
                    JOptionPane.showMessageDialog(frame, "VOIs must be from same image.", "MIPAV Warning", JOptionPane.WARNING_MESSAGE);
                return;
                }
             }
            ...
            if ((oldNode.getNodeType().equalsIgnoreCase(IMAGENODE)) && 
                    ((oldNode.getChildCount() == 
                        ((JList)((JScrollPane)getComponentByName("VOIs from above image List: scroll"))
                    ...
                    for (int i=0;i<oldNode.getChildCount();i++){
                        ...
                        MipavScriptTreeNode voiChildNode = new MipavScriptTreeNode(voiName,VOINODE);
                    ...
                    while (oldNode.children().hasMoreElements()){
                        newNode.add((MipavScriptTreeNode)oldNode.children().nextElement());
                   }  
                }
    ...
    private class DragMouseAdapter extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if (((Component)e.getSource()).getName().equalsIgnoreCase("Images List")) {               
                ...
                if ((selectedListIndicies.length > 1) && (e.getModifiers() != e.BUTTON3_MASK)) return;
               
                if ((selectedListIndicies.length > 1) && (e.getModifiers() == e.BUTTON3_MASK)){                   
                   if (root.getChildAt(0).getChildCount() == 1){ 
                    ...
                    sequentialMenuItem.addActionListener(
                            new java.awt.event.ActionListener()
                            {
                                ...
                                {                                   
                                    ...
                                    updateNode((MipavScriptTreeNode)root.getChildAt(j).getChildAt(0), imageName);
                                    }
                 ...
                 //ModelImageForScripting image = (ModelImageForScripting)((JList)e.getSource()).getSelectedValue();
                JDialogRunScriptModel.ScriptModelImage image = (JDialogRunScriptModel.ScriptModelImage)((JList)e.getSource()).getSelectedValue();
                ...
                for (int i=0;i<frame.getContentPane().getComponents().length;i++){
                    ...
                    if ((components[i].getName() != null) && (components[i].getName().equalsIgnoreCase("VOIs from above image List: scroll"))){
                        voiList = ((JList)((JScrollPane)frame.getContentPane().getComponents()[i]).getViewport().getView());
           ...
           if (((Component)e.getSource()).getName().equalsIgnoreCase("Script Tree")) {
               if (e.getModifiers() == e.BUTTON3_MASK){
                   Point pt = e.getPoint();
                   TreePath pathTarget = tree.getPathForLocation(pt.x, pt.y);
                   selectedNode = (MipavScriptTreeNode)pathTarget.getLastPathComponent();
                   parentNode = (MipavScriptTreeNode)selectedNode.getParent();
                   selectedNodeName = (String)selectedNode.getUserObject();
 
                    if (((String)selectedNode.getUserObject()).indexOf("$Image") != -1) return;
                    if (((String)selectedNode.getUserObject()).indexOf("VOI Needed") != -1) return;
                           ...
                           new java.awt.event.ActionListener()
                           {
                               public void actionPerformed(java.awt.event.ActionEvent e)
                               {
                                   if (selectedNodeName.contains("Script")) {
                                       ...
                                       resetNode = new MipavScriptTreeNode(selectedNode.getDefaultName(), selectedNode.getNodeType());
                                   }

                                   while (selectedNode.children().hasMoreElements()){
                                       resetNode.add((MipavScriptTreeNode)selectedNode.children().nextElement());
                                   }
                                   
                                   ((DefaultTreeModel)tree.getModel()).insertNodeInto(resetNode,parentNode, parentNode.getIndex(selectedNode));
                                   ((DefaultTreeModel)tree.getModel()).removeNodeFromParent(selectedNode);
                    ...
                    JMenuItem applyAllMenuItem = new JMenuItem("Apply to all script executers");
                     applyAllMenuItem.addActionListener(
                            new java.awt.event.ActionListener()
                            {
                                public void actionPerformed(java.awt.event.ActionEvent e)
                                {
            ...
            if (selectedListIndicies.length == 1) handler.exportAsDrag(c, e, TransferHandler.COPY);
