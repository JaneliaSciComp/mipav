...
public class ViewJComponentTriSliceImage {
    ...
    /**
     * OPACITY - opacity value used by the paint brush. value = 1.0 - opaque value = 0.25 - default (mostly see through)
     */
     ...
     */
    public ViewJComponentTriSliceImage(SurfaceRender _frame, ModelImage _imageA, ModelLUT _LUTa, float[] imgBufferA,
                                       ModelImage _imageB, ModelLUT _LUTb, float[] imgBufferB, int[] extents,
                                       int _orientation) {
        frame = _frame;
        imageA = _imageA;
        imageB = _imageB;
        ...
        if ((orientation == XY) || (orientation == NA)) {
            ...
            if (imageB == null) {
                ...
                for (index = 0; index < bufferSize; index++) {
                    ...
                    img.setRGB(index % xDim, index / xDim,
                               (0xFF000000) | ((int) Ra << 16) | ((int) Ga << 8) | (int) Ba);
                } // end of for (index=0; index < bufferSize; index++)
            } // end of else for imageB != null
        } // end of if ((orientation == XY) || (orientation == NA))
        else if (orientation == XZ) {
    ...
    public boolean showDiagonal(Transform3D kTransform, Point3f[] akVerts) {
        ...
        /*
         * 1. Get the HistoLUT or RGB lookup tables for grayscale or color images, depending on the ModelImage data
         * type. This first step initializes the lookup tables for either type of image, and for both imageA and imageB
         * if both are defined:
         */
        int[] RGBIndexBufferA = new int[256];
        int[] RGBIndexBufferB = new int[256];
        ...
        /*
         * 2. The second step is to calculate the bounding box of the plane that intersects the ModelImage data, and
         * transforming the four corners of the plane to the new rotated coordinates. The four transformed/rotated
         * coordinates are used for steping through and interpolating the ModelImage data along the diagonal directions:
         */
        float xBox = (xDim - 1) * Math.abs(imageA.getResolutions(0)[0]);
        float yBox = (yDim - 1) * Math.abs(imageA.getResolutions(0)[1]);
        float zBox = (zDim - 1) * Math.abs(imageA.getResolutions(0)[2]);
        float maxBox = xBox;
        ...
        /*
         * 3. The third step is to step through the image we are writing the data in, using the four transformed points
         * to step through the ModelImage along the diagonal directions, read the corresonding point in the ModelImage
         * and write the value into the image array. If m_bInterpolate is set to true, the ModelImage data for
         * non-interger vertices is interpolated using tri-linear interpolation. Note: there is one loop for steping
         * though he data, no matter which type of plane this object represents (XY, XZ, or ZY).
         */

        /* imageBufferA and imageBufferB have size iYRange * iXRange, step
         * through the imageBuffers while determinging which point along the diagonal plane is shown at that point,
         * accessing the ModelImage at
         * that diagonal point:*/
        for (int j = 0; j < iYRange; j++) {
            ...
            for (int i = 0; i < iXRange; i++) {
                ...
                if (((x < 0) || (x >= xDim)) || ((y < 0) || (y >= yDim)) || ((z < 0) || (z >= zDim))) {
                    ...
                    iMIIndex = (((int) z) * iZFactor) + (((int) y) * iYFactor) + (((int) x) * iXFactor);
                    ...
                    if ((iMIIndex < 0) || (iMIIndex >= iMISize)) {
                        ...
                        for (int ib = 0; ib < bufferFactor; ib++) {
                            ...
                            if (!m_bInterpolate) {
                                ...
                                if ((imageB != null) && (imageBufferB != null)) {
                                    imageBufferB[index + ib] = imageB.getFloat(iMIIndex + ib);
                                }
                            }
                            /* tri-linear interpolation: */
                            else {
                                imageBufferA[index + ib] = interpolate(imageA, x, y, z, iXFactor, iYFactor, iZFactor,
                                                                       ib, iMISize);

                                if ((imageB != null) && (imageBufferB != null)) {
                                    imageBufferB[index + ib] = interpolate(imageB, x, y, z, iXFactor, iYFactor,
                                                                           iZFactor, ib, iMISize);
                                }
                            }
                        }
                ...
                if (!(imageA.isColorImage())) {
                    ...
                    if (RGBTA.getBOn()) {
                        Gb = (RGBIndexBufferA[(int) ((imageBufferA[index + 3] + offsetAB) * normColorA)] & 0x000000ff);
                    } else {
                        Gb = 0;
                    }
                } // end of if (RGBTA != null)
    ...
    /**
     * Interpolates the ModelImage data for the non-interger data point (x,y,z) through tri-linear interpolation. The
     * iColor is used for interpolating the color data RGB:
     *
     * @param   kImage    DOCUMENT ME!
     * @param   x         DOCUMENT ME!
     * @param   y         DOCUMENT ME!
     * @param   z         DOCUMENT ME!
     * @param   iXFactor  DOCUMENT ME!
     * @param   iYFactor  DOCUMENT ME!
     * @param   iZFactor  DOCUMENT ME!
     * @param   iColor    DOCUMENT ME!
     * @param   iSize     DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private float interpolate(ModelImage kImage, float x, float y, float z, int iXFactor, int iYFactor, int iZFactor,
                              int iColor, int iSize) {
         ...
         * input point (x,y,z): */
        int iX1L = iColor + (int) ((fXFloor * iXFactor) + (fYFloor * iYFactor) + (fZFloor * iZFactor));

        int iX2L = iColor + (int) ((fXFloor * iXFactor) + (fYFloor * iYFactor) + (fZCeil * iZFactor));

        int iX3L = iColor + (int) ((fXFloor * iXFactor) + (fYCeil * iYFactor) + (fZCeil * iZFactor));

        int iX4L = iColor + (int) ((fXFloor * iXFactor) + (fYCeil * iYFactor) + (fZFloor * iZFactor));

        int iX1R = iColor + (int) ((fXCeil * iXFactor) + (fYFloor * iYFactor) + (fZFloor * iZFactor));

        int iX2R = iColor + (int) ((fXCeil * iXFactor) + (fYFloor * iYFactor) + (fZCeil * iZFactor));

        int iX3R = iColor + (int) ((fXCeil * iXFactor) + (fYCeil * iYFactor) + (fZCeil * iZFactor));

        int iX4R = iColor + (int) ((fXCeil * iXFactor) + (fYCeil * iYFactor) + (fZFloor * iZFactor));

        /* Bounds checking: */
        if ((iX1L < 0) || (iX1L >= iSize) || (iX1R < 0) || (iX1R >= iSize) ||

            (iX2L < 0) || (iX2L >= iSize) || (iX2R < 0) || (iX2R >= iSize) ||

            (iX3L < 0) || (iX3L >= iSize) || (iX3R < 0) || (iX3R >= iSize) ||

            (iX4L < 0) || (iX4L >= iSize) || (iX4R < 0) || (iX4R >= iSize)) {
            return 0;
        }
