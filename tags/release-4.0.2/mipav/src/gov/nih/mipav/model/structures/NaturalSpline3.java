...
/**
 * This is an implementation for a curve in 3D defined by a natural spline. The spline is defined to pass through an
 * input set of control points with an associated set of time values (any range).
 */
public class NaturalSpline3 extends MultipleCurve3 {
    ...
    /**
     * Create a natural spline curve in 3D through the set of input points. Automatically compute the "times" associated
     * with each point based on the accumulated distance between the points.
     *
    ...
    public NaturalSpline3(float[] afTime, Point3f[] akPoint) {
        ...
        for (i = 1; i < iSegments; i++) {
            ...
            kNumer.x = 3.0f * ((afDt[i - 1] * m_akA[i + 1].x) - (afD2t[i] * m_akA[i].x) + (afDt[i] * m_akA[i - 1].x));
            kNumer.y = 3.0f * ((afDt[i - 1] * m_akA[i + 1].y) - (afD2t[i] * m_akA[i].y) + (afDt[i] * m_akA[i - 1].y));
            kNumer.z = 3.0f * ((afDt[i - 1] * m_akA[i + 1].z) - (afD2t[i] * m_akA[i].z) + (afDt[i] * m_akA[i - 1].z));
    ...
    public Point3f getPosition(int iSegment, float fDt) {
        ...
        kResult.x = m_akA[iSegment].x +
                    (fDt * (m_akB[iSegment].x + (fDt * (m_akC[iSegment].x + (fDt * m_akD[iSegment].x)))));
        kResult.y = m_akA[iSegment].y +
                    (fDt * (m_akB[iSegment].y + (fDt * (m_akC[iSegment].y + (fDt * m_akD[iSegment].y)))));
        kResult.z = m_akA[iSegment].z +
                    (fDt * (m_akB[iSegment].z + (fDt * (m_akC[iSegment].z + (fDt * m_akD[iSegment].z)))));
    ...
    protected Vector3f getFirstDerivative(int iSegment, float fDt) {
        ...
        kResult.x = m_akB[iSegment].x + (fDt * ((2.0f * m_akC[iSegment].x) + (3.0f * fDt * m_akD[iSegment].x)));
        kResult.y = m_akB[iSegment].y + (fDt * ((2.0f * m_akC[iSegment].y) + (3.0f * fDt * m_akD[iSegment].y)));
        kResult.z = m_akB[iSegment].z + (fDt * ((2.0f * m_akC[iSegment].z) + (3.0f * fDt * m_akD[iSegment].z)));
