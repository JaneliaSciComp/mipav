...
public class Algorithm3DMosaicTo4DSlices extends AlgorithmBase {
     ...
     */
    public Algorithm3DMosaicTo4DSlices(ModelImage srcIm, ModelImage dest) {
        super(dest, srcIm);
        srcImage = srcIm; 
    }
    ...
    if (srcImage.is2DImage()) {
            ...
            try {
                ...
                destImage = null;
                errorCleanUp("Algorithm3DMosaicTo4DSlices. srcImage locked", true);
            ...
            for (y = 0; ((y + subYDim - 1) < yDim) && (sliceNum < numberOfImagesInMosaic); y += subYDim) {
                for (x = 0; ((x + subXDim - 1) < xDim) && (sliceNum < numberOfImagesInMosaic); x += subXDim) {
                    ...
                    try {
                        ...
                        destImage.disposeLocal(); // Clean up memory of result image
                        destImage = null;
                        errorCleanUp("Algorithm3DMosaicTo4DSlices. destImage locked", true);

                        return;
                    }
                    sliceNum++;
                    System.err.println( "still ok" );
                }
            }   
                    ...
                    for (y = 0; ((y + subYDim - 1) < yDim) && (sliceNum < subTDim); y += subYDim) {
                        for (x = 0; ((x + subXDim - 1) < xDim) && (sliceNum < subTDim); x += subXDim) {
                            for (z = 0; ((z + subZDim - 1) < zDim) && (sliceNum < subTDim); z += subZDim) {
                                for (ys = 0; ys < subYDim; ys++) {
                                    for (xs = 0; xs <  subXDim; xs++) {
                                        for (zs = 0; zs <  subZDim; zs++) {
                                            for (c = 0; c < cFactor; c++) {
                                                subIndex = c + cFactor*(xs + ys*subXDim);
                                                orgIndex = c + cFactor*(x + xs + (y + ys)*xDim);
                                                subBuffer[subIndex] = buffer[orgIndex];
                                ...
                                try {
                                    ...
                                    destImage.disposeLocal(); // Clean up memory of result image
                                    destImage = null;
                                    errorCleanUp("Algorithm3DMosaicTo4DSlicess destImage locked", true);
                    ...
                    System.err.println( "still going" );
            }     
    System.err.println( "still ahead" );                      
