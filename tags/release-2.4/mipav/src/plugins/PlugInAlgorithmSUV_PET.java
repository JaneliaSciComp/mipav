 ...
 *
 * The radionuclide total dose is administered at the injection time or radiopharmaceutical start time which comes
...
public class PlugInAlgorithmSUV_PET extends AlgorithmBase {
    ...
    public PlugInAlgorithmSUV_PET(ModelImage srcImg, boolean male, float height, float weight, long dose,
            int decayCorrection, String radStartStr, float radStartTime, String acqTimeStr, float acqTime, long halfLife) {
        super(null, srcImg);
        this.male = male;
        this.height = height;
        this.weight = weight;
    ...
    private void constructLog() {
        historyString = new String("SUV_PET(" + male + ", " + height + ", " + weight + ", " + dose + ", "
                + decayCorrection + ", " + radStartStr + ", " + radStartTime + ", " + acqTimeStr + ", " + acqTime
                + ", " + halfLife + ")\n");
    }
    ...
    private void calc2D() {
        ...
        srcImage.getUserInterface().getFrameContainingImage(srcImage).saveAllVOIsTo(
                fileDirectory + File.separator + voiName);
        progressBar.dispose();
        setCompleted(true);
    }
