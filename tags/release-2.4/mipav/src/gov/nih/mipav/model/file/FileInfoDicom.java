...
/**
 *   This class contains DICOM header information.  It uses a Hashtable to
 *   store all the information about the tags.  The standard Hashtable listing
 *   all known tags with empty values is in CreateDICOMFiles.  The tagsList Hashtable
 *   in this file is initialized to that Hashtable.  Then as the tags are read in
 *   by FileDicom, they are stored here.  Also stored here is the offset of the image,
 *   the image number, the resolutions, some pixel data, the bytes allocated, and the
 *   dimensions of the image.
 *
 *   <p>This file also contains a table used for displaying the tag information.  There
 *   is an option to display just the standard tags or both the standard and the private
 *   tags.
 *
...
public class FileInfoDicom extends FileInfoBase {
    ...
    /** these are the DICOM tag ID numbers corresponding to the
     *   tags which are anonymized.  They are kept here for simplicity
     *   of updating (when it is decided that other tags are must be
     *   anonymized) and to the advantage of calling objects.
     */
    public final static String anonymizeTagIDs[] = {
    ...
    /**  Hashtable to store all the information about the DICOM tags. The hashtable created in the class
     *   CreateDICOMFiles and is a list of all the DICOM tags where initially all the tags are empty of values.
     *   Then as the tags are read in by FileDicom and stored here the tagList hashtable.
     */
    ...
    /**
     *  DICOM file information constructor.  <b>Does not import the DICOM
     *  data dictionary and its associated tags list.</b>
    ...
    public final void setLength(String name, int length) {
        ...
        if (entry != null) entry.setLength(length);
    }
     ...
     *   This function also sets modality and other important file
     *   information.
        ...
        if (length != 0) {
            if (key.equals("0018,0050")){ // type 2
                ...
                if (firstHalf != null && firstHalf.length() > 0) {
                    ...
                    if (f1 != null) resolutions[0] = f1.floatValue();
                }
                else {
                    resolutions[0] = 1.0f;
                }
                ...
                if (secondHalf != null && secondHalf.length() > 0) {
                    ...
                    if (f2 != null) resolutions[1] = f2.floatValue();
                }
                else {
                    resolutions[1] = 1.0f;
                }
     ...
     *   information.
     *   <p>
     *   uses the DicomTag method "setValue(obj)" to automagically:
     *   <ul>
     *   <li>determine the length of the tag's value, </li>
     *   <li>convert a "human-readable" string value into a DICOM
     *       compliant code,<li>
     *   <li>and when value is a string, but the key specifies some other
     *       type of data element.</li>
     *   </ul>
     *   Private tags are ignored, as are "sequence" and "unknown" tags.
     *   Throws an exception when the value is too large for the given
     *   tag type.
    ...
    public void setValue(FileDicomKey key, Object value) {
        ...
        if (entry.getType().equals("typeSequence") ||
            entry.getType().equals("typeUnknown")  ||
    ...
    /**
     *   accessor that returns the value matching the key
     *   as a meaningful (ie., non-coded) string .   This is
     *   a direct accessor to <code>getEntry(name).getValue(true)</code>.
     *
    ...
    /**
     * Resets the reference to the dictionary.
     * Sets the DICOM tags table, offered by the
     * <CODE>CreateDICOMFiles</CODE> dictionary.
     * Setting the dictionary has the effect of
     * wiping the tags list of its tags as well.
     */
     ...
     *   Uses the local method getValue to look at the Object
     *   held in the tag and decipher it for the user class
     ...
     *   @return     the array of Strings that were coded into the tag; a single string
     *               is given if the string in the tag has no "\" seporators.  Each String
     *               in the array is a single value, and there are value multiplicity number of Strings
     *               in the array.
     *               NOTE: user class must convert this list to the correct type;
     *                     (which does indicate user class knows what returned string is)
     *               If null then there were zero tokens in the tag
     */
    ...
    /**
     *
     *   removes any and all personal information in the info that
     *   a doctor, patient, researcher may want to have deleted from
     *   the image, such as patient name or patient ID number--from this
     *   slice given by this fileInfoDicom.
     *   <p>
     *   In addition, it allows the change or removal of a whole list of
     *   information, such as: ("0010,0010") Patient name;
     *   ("0010,0020") patient ID number; ("0010,0050") patient Insurance
     *   plan code sequence; ("0010,1000") other patient IDs; ("0010,1001")
     *   other patient names; ("0010,1040") patient address; ("0010,1060")
     *   patient mother's birth name; ("0010,2154") patient telephone numbers;
     *   ("0010,2160") patient ethnic group; ("0010,21D0") patient's last
     *   menstrual date; ("0010,21F0") patient religious preference;
     *   and ("0010,4000") patient comments.
     *
     *   @param list the list of tags to remove; it MUST correspond to
     *           anonymizeTagIDs list (or spurious errors result), and
     *           it must be of the same length, or this will throw an
    ...
    protected final void setModality(Object value) {
        if (value.equals("BI"))         super.setModality(BIOMAGENETIC_IMAGING);
        else if (value.equals("CD"))    super.setModality(COLOR_FLOW_DOPPLER);
        else if (value.equals("CR"))    super.setModality(COMPUTED_RADIOGRAPHY);
        else if (value.equals("CT"))    super.setModality(COMPUTED_TOMOGRAPHY);
        else if (value.equals("DD"))    super.setModality(DUPLEX_DOPPLER);
        else if (value.equals("DG"))    super.setModality(DIAPHANOGRAPHY);
        else if (value.equals("DX"))    super.setModality(DIGITAL_RADIOGRAPHY);
        else if (value.equals("ES"))    super.setModality(ENDOSCOPY);
        else if (value.equals("GM"))    super.setModality(GENERAL_MICROSCOPY);
        else if (value.equals("HC"))    super.setModality(HARDCODY);
        else if (value.equals("IO"))    super.setModality(INTRAORAL_RADIOGRAPHY);
        else if (value.equals("LS"))    super.setModality(LASER_SURFACE_SCAN);
        else if (value.equals("MA"))    super.setModality(MAGNETIC_RESONANCE_ANGIOGRAPHY);
        else if (value.equals("MG"))    super.setModality(MAMMOGRAPHY);
        else if (value.equals("MR"))    super.setModality(MAGNETIC_RESONANCE);
        else if (value.equals("MS"))    super.setModality(MAGNETIC_RESONANCE_SPECTROSCOPY);
        else if (value.equals("NM"))    super.setModality(NUCLEAR_MEDICINE);
        else if (value.equals("OT"))    super.setModality(OTHER);
        else if (value.equals("PT"))    super.setModality(POSITRON_EMISSION_TOMOGRAPHY);
        else if (value.equals("PX"))    super.setModality(PANORAMIC_XRAY);
        else if (value.equals("RF"))    super.setModality(RADIO_FLUOROSCOPY);
        else if (value.equals("RG"))    super.setModality(RADIOGRAPHIC_IMAGING);
        else if (value.equals("RTDOSE"))   super.setModality(RADIOTHERAPY_DOSE);
        else if (value.equals("RTIMAGE"))  super.setModality(RADIOTHERAPY_IMAGE);
        else if (value.equals("RTPLAN"))   super.setModality(RADIOTHERAPY_PLAN);
        else if (value.equals("RTRECORD")) super.setModality(RADIOTHERAPY_RECORD);
        else if (value.equals("RTSTRUCT")) super.setModality(RADIOTHERAPY_STRUCTURE_SET);
        else if (value.equals("SM"))    super.setModality(SLIDE_MICROSCOPY);
        else if (value.equals("ST"))    super.setModality(SINGLE_PHOTON_EMISSION_COMPUTED_TOMOGRAPHY);
        else if (value.equals("TG"))    super.setModality(THERMOGRAPHY);
        else if (value.equals("US"))    super.setModality(ULTRASOUND);
        else if (value.equals("XA"))    super.setModality(XRAY_ANGIOGRAPHY);
        else if (value.equals("XC"))    super.setModality(EXTERNAL_CAMERA_PHOTOGRAPHY);
        else                            super.setModality(UNKNOWN_MODALITY);
    }


    /**
     *   This method extracts directional cosines from the DICOM
     *   image header (tag = "0020, 0037"). Since the third row of
     *   transformation matrix is not given in the tag but can be
     *   derived by taking the cross product. The result is then
     *   put into a MIPAV transformation matrix object and returned.
     *   @return the transformation object extracted from the DICOM header. If this tag
     *           is null then the class returns null.
     */
    public final TransMatrix getPatientOrientation() {
        ...
        try {
            ...
            for (int i=0; i<orientation.length(); i++) {
                if (orientation.charAt(i) == '\\') {
                    if (index1 == notSet) index1 = i;
                    else if (index2 == notSet) index2 = i;
                    else if (index3 == notSet) index3 = i;
                    else if (index4 == notSet) index4 = i;
                    else index5 = i;
                }
            }
