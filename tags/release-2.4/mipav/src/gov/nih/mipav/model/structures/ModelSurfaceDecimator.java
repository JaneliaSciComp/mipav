...
/**
 * A triangle decimator that is used for continuous level of detail of a
 * triangle mesh.  The algorithm is based on edge collapses of the mesh.
 * A detailed discussion of the algorithm is found in
...
{
    /**
     * The decimator constructed by this method is designed to be reused, if
     * necessary, for decimating multiple meshes.  If used this way, the
     * input parameters measure the maximum quantities over all the meshes.
     *
     * @param iVQuantity  the maximum number of vertices for all meshes to be
     *        decimated by this object
     * @param iTQuantity  the maximum number of triangles for all meshes to
     *        be decimated by this object
     */
    ...
    /**
     * A triangle decimator for a triangle mesh.  After decimation, the
     * vertex and triangle connectivity arrays have been reordered and should
     * be used as input in creating an ModelClodMesh object.  The collapse
     * records constructed in this function can be accessed via getRecords().
     *
    ...
    public void decimate (Point3f[] akVertex, int[] aiConnect, ViewJProgressBar progressBar, int added, int total)
    {
        ...
        {
            Triangle kT = new Triangle(aiConnect[3*i],aiConnect[3*i+1],
                aiConnect[3*i+2]);
            insertTriangle(kT);
    ...
    /**
     * DEBUGGING.  After a decimation, the collapse records are applied from
     * highest to lowest resolution to test if the vertex indices for the
     * triangles are within range.
     */
    ...
    /**
     * The collapse records that have been created by the constructor for
     * ModelSurfaceDecimator.  After construction, the application uses the
     * vertex and triangle connectivity arrays that were passed to the
     * constructor (arrays are now reordered) and should call this function
     * to access the collapse records.  All three data structures are then
     * used to create an ModelClodMesh object.
     *
    ...
    /**
     * An override of the base class ModelSurfaceTopology member function.  This
     * callback is used to keep track of vertices deleted during an edge
     * collapse.
     *
    ...
    /**
     * An override of the base class ModelSurfaceTopology member function.  This
     * callback is used to keep track of heap changes due to edge collapses.
     *
        ...
        EdgeAttribute kEAttr)
    {
        ...
        // -INFINITY so that it has the minimum value of all edges.  The
        // update call bubbles the edge to the root of the heap.  The edge is
        // then removed from the root.
    ...
    /**
     * An override of the base class ModelSurfaceTopology member function.  This
     * callback is used to assign integer indices to triangles in the mesh.
     *
    ...
    /**
     * An override of the ModelSurfaceTopology member function.  This function
     * keeps track of the order of triangle removal to support reordering
     * of the connectivity array.  Once done, then the base class function is
     * called to remove the triangle from the mesh.
     *
    ...
    /**
     * When an edge is collapsed, all triangles sharing the edge have been
     * removed.  All remaining triangles that contain VThrow as a vertex
     * must have VThrow replaced by VKeep.  The old triangles are removed
     * from the mesh and the modified triangles are inserted.
     *
    ...
    private void doCollapse ()
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            VertexAttribute kVAttr = (VertexAttribute) m_kVMap.get(
                new Vertex(iV));
            int j;
    ...
    /**
     * A look-ahead test to determine if the requested edge collapse will
     * cause the mesh to fold over itself.
     *
     * @param iVKeep the vertex to keep in the edge collapse
     * @param iVThrow the vertex to be thrown away in the edge collapse
    ...
    {
        VertexAttribute kVTAttr = (VertexAttribute) m_kVMap.get(
            new Vertex(iVThrow));
        ...
        for (int j = 0; j < kVTAttr.m_kTSet.size(); j++)
        {
            ...
            if ( kCollapse == new Edge(kT.m_iV0,kT.m_iV1)
            ||   kCollapse == new Edge(kT.m_iV1,kT.m_iV2)
    ...
    /**
     * The internal edge collapse function.  This is called as long as the
     * suggested edge does not cause folding.
     *
     * @param iVKeep the vertex to keep in the edge collapse
     * @param iVThrow the vertex to be thrown away in the edge collapse
     */
    private void collapseEdge (int iVKeep, int iVThrow)
    {
        ...
        ModelCollapseRecord kCR = new ModelCollapseRecord(iVKeep,iVThrow,
            m_kVDelete.size(),iTDeletions);
        m_kEDelete.add(kCR);
    }
    
    /**
     * Computes the weight associated with an edge.  The weight is based on
     * the length of the edge, the area of the adjacent triangles, and the
     * angle between the triangles.  The smaller any of these quantites are,
     * the more likely the edge should be removed.  That is, attempt to
     * collapse short edges whose triangles have small area and are nearly
     * coplanar.  Only manifold edges have finite weight.  Other edges are
     * assigned infinite weight to preserve topology of the mesh (boundary
     * edges and junction edges should not be collapsed).
     *
    ...
    /**
     * When all edges have infinite weight, no further edge collapses are
     * allowed.  At this point the remaining vertices in the map are written
     * to the reordering/permutation arrays and the edge collapse process is
     * finished.
     */
    ...
    /**
     * When all edges have infinite weight, no further edge collapses are
     * allowed.  At this point the remaining triangles in the map are written
     * to the reordering array and the edge collapse process is finished.
     */
    ...
    /**
     * After edge collapses are finished, the actual vertex locations, the
     * triangle connectivity array, and the collapse record information must
     * be reordered to support fast dynamic change in level of detail.
     */
    ...
    /**
     * All collapse information is known when this is called.  What remains
     * to be done is to determine which indices in the triangle connectivity
     * must be changed when a collapse record is applied to the mesh.  This
     * is done to make the index changes fast in the application.  Without
     * the look-up tables in the collapse record, the application would have
     * to constantly search the connectivity array and do the work in the
     * current function <b>every time level of detail is changed</b>.
     */
    ...
    /**
     * The representation of a heap element.  The important quantity for
     * heap updates is the metric value.  However, the HIndex value is used
     * for O(1) lookups of an edge whose weight needs to be modified as a
    ...
    /**
     * Allocate the heap and initialize all the heap elements.  This is an
     * O(N) process for N edges.
     */
    ...
    /**
     * After the heap is initialized, it must be sorted.  This is an
     * O(N log N) process for N edges.
     */
    private void sort ()
    {
    ...
    /**
     * New edges that occur because of modified triangles being added to the
     * mesh must be added to the heap and the heap must be updated.  This is
     * an O(log N) process for N edges.
     *
     * @param fMetric The new edge weight to be added to the heap.  The
     *  other heap element information was set up by the OnEdgeInsert
     *  callback.
     */
     ...
     * Once removed, the heap must be updated.  This is an O(log N) process
     * for N edges.
     */
    private void remove ()
    {
    ...
    /**
     * If an edge weight changes for an edge whose heap element is interior
     * to the heap tree, the heap must be updated.  This is an O(log N)
     * process for N edges.
     *
     * @param iHIndex the index of the heap element corresponding to the
     *   modified edge (the heap is a binary tree but is stored in an array)
