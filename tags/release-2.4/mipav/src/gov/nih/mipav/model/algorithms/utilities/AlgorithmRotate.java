...
/**
 *      An Algorithm to rotate 3D or 4D dataset 90 degrees about X, Y, or Z axis.
 *      2D Images can also be rotated.  A new rotated image with modified
 *      dimensions and resolutions created and can be accessed through returnImage().
...
public class AlgorithmRotate
     ...
     *	to the message frame if the logging procedure is turned on.
     */
    ...
    /**
     *	Runs the rotation algorithm.  The algorithm is run in place so automatically replaces
     *	the source model image.  Resets image orientation, axis orientations, and start locations.
     */
     ...
     *	Two dimensional images call this method.
     */
    private void calcInPlace2D() {
                    ...
                    for (j = 0; j < oldDimExtents[0] && !threadStopped; j++) {
                        for (k = oldDimExtents[1] - 1; k >= 0 && !threadStopped; k--) {
                            ...
                            for (w = 0; w < 4 && !threadStopped; w++) {
                                if (i % mod == 0 && isProgressBarVisible()) {
                                    progressBar.updateValue(Math.round( (float) (i) / (length - 1) * 100),
                                        activeImage);
                                }
        ...
        if (srcImage.getFileInfo(0).getAxisOrientation()[1] != FileInfoBase.ORI_UNKNOWN_TYPE) { // if were set to some nontrivial value
            newFileInfo.setAxisOrientation(
                srcImage.getFileInfo(0).getAxisOrientation()[2], 2);
            if (rotateAxis == Z_AXIS_PLUS) {
                newFileInfo.setAxisOrientation(srcImage.getFileInfo(0).getAxisOrientation()[0], 1);
                newFileInfo.setAxisOrientation(
                    FileInfoBase.oppositeOrient(srcImage.getFileInfo(0).getAxisOrientation()[1]), 0);
            }
            else { // Z_AXIS_MINUS
                newFileInfo.setAxisOrientation(srcImage.getFileInfo(0).getAxisOrientation()[1], 0);
                newFileInfo.setAxisOrientation(
                    FileInfoBase.oppositeOrient(srcImage.getFileInfo(0).getAxisOrientation()[0]), 1);
            }
        }
    ...
    /**
     *	Calculates the rotated image and replaces the source image with the rotated image.
     *	Three dimensional and higher dimensional images call this method.
     */
    private void calcInPlace3D() {
        ...
        try {
            ...
            for (i = 0; i < 3; i++) {
                ...
                if (oldAxisOrients[j] == FileInfoBase.ORI_A2P_TYPE || oldAxisOrients[j] == FileInfoBase.ORI_R2L_TYPE
                ...
                if (oppositeOrients[i] == true) {
                    ...
                    //            + direct[j] * ( oldDimExtents[j] - 1 ) * oldResolutions[j];
                }
                else {
                    //     newStartLocations[i] = oldStartLocations[j];
                }
                if (newAxisOrients[i] == FileInfoBase.ORI_A2P_TYPE || newAxisOrients[i] == FileInfoBase.ORI_R2L_TYPE
                    ...
                    for (index = 0; index < length && !threadStopped; index++) {
                        if ( (start + index + tOffset) % mod == 0 && isProgressBarVisible()) {
                            progressBar.updateValue(
                                Math.round( (float) (start + index + tOffset) / (totalLength - 1) * 100),
                                activeImage);
                        }
                            ...
                            for (j = 0; j < oldDimExtents[1] && !threadStopped; j++) {
                                for (k = 0; k < oldDimExtents[2] && !threadStopped; k++) {
                                    ...
                                    for (w = 0; w < 4 && !threadStopped; w++) {
                                        ...
                                        buffer[i] = srcImage.getFloat(
                                            (tmpStart * 4) + (k * stride * 4) + w + tOffset);
                                        if (buffer[i] > max) {
                                            max = buffer[i];
                                        }
                                        if (buffer[i] < min) {
                            ...
                            for (j = 0; j < oldDimExtents[0] && !threadStopped; j++) {
                                for (k = oldDimExtents[1] - 1; k >= 0 && !threadStopped; k--) {
                                    ...
                                    for (w = 0; w < 4 && !threadStopped; w++) {
                                        ...
                                        buffer[i] = srcImage.getFloat(
                                            s * (oldDimExtents[0] * oldDimExtents[1] * 4)
                                            + (k * oldDimExtents[0] * 4) + ( (4 * j) + w) + tOffset);
                                        if (buffer[i] > max) {
                                            max = buffer[i];
                                        }
                                        if (buffer[i] < min) {
        ...
        if (newAxisOrients[2] != FileInfoBase.ORI_UNKNOWN_TYPE) {
            ...
            else if (newAxisOrients[2] == FileInfoBase.ORI_S2I_TYPE || newAxisOrients[2] == FileInfoBase.ORI_I2S_TYPE) {
                orientation = FileInfoBase.AXIAL;
            }
            else if (newAxisOrients[2] == FileInfoBase.ORI_R2L_TYPE || newAxisOrients[2] == FileInfoBase.ORI_L2R_TYPE) {
        ...
        if (srcImage.getFileInfo(0).getFileFormat() != FileBase.DICOM) {
            ...
            for (i = 0; i < newDimExtents[2]; i++) {
                ...
                if (newFileInfo[i].getFileFormat() == FileBase.MINC) {
                    newFileInfo[i].setRescaleSlope(
                        ( (FileInfoMinc) newFileInfo[i]).calculateSlope(maximums[i], minimums[i]));
                    newFileInfo[i].setRescaleIntercept(
                        ( (FileInfoMinc) newFileInfo[i]).calculateIntercept(minimums[i],
            ...
            if (rotateAxis != Z_AXIS_PLUS && rotateAxis != Z_AXIS_PLUS && tmp != null) {
                newTagPixelSpc = new StringBuffer(
                    Float.toString(newResolutions[0]) + "\\" + Float.toString(newResolutions[1]));
            ...
            for (i = 0; i < newDimExtents[2]; i++) {
                ...
                if (newTagSliceSpc != null) {
                    ...
                    newDicomInfo[i].setValue(
                        "0018,0088", newTagSliceSpc.toString(), newTagSliceSpc.length()); // spacing between slices
                }
    ...
    private void calcStartLocations(float[] newLoc) {
        ...
        if (srcImage.getNDims() == 3) {
            ...
            if (srcImage.getFileInfo(0).getImageOrientation() == FileInfoBase.SAGITTAL) {
                if (rotateAxis == X_AXIS_PLUS ||
                    rotateAxis == Y_AXIS_MINUS) {
                    newLoc[0] = oldLoc[0] - ( (oldDims[2] - 1) * oldRes[2]);
                }
                else if (rotateAxis == X_AXIS_MINUS ||
                         rotateAxis == Z_AXIS_PLUS) {
    ...
    /**
     *   Convert the matrix to the String format (decimal string) to be stored in the
     *   DICOM tag (0020,0037) image (patient) orientation.
     *   @param matrix	Transformation matrix to be converted.
     *   @return 		The string version of the transformation matrix (i.e. the
     *					directional cosines for the first two rows of the matrix delimited by "\")
     */
    private String matrixToDICOMString(TransMatrix matrix) {
        ...
        strMatrix = nf.format(dMatrix[0][0]) + "\\" + nf.format(dMatrix[0][1]) + "\\" + nf.format(dMatrix[0][2])
            + "\\" + nf.format(dMatrix[1][0]) + "\\" + nf.format(dMatrix[1][1]) + "\\"
            + nf.format(dMatrix[1][2]);

        return strMatrix;
    }

    /**
     *	Convert the image position to the String format (decimal string) to be stored in the
     *   DICOM tag (0020,0032) image (patient) orientation.
