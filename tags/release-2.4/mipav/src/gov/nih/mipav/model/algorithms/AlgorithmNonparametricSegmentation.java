  ...
  //     pp. 391-400.
    ...
    private void segment3Colors() {
      ...
      try {
          ...
          buildProgressBar(srcImage.getImageName(),
                           "Performing Nonparametric segmentation...", 0, 100);
          initProgressBar();
          ...
          /*featureImage = new ModelImage(ModelImage.FLOAT,smoothExtents,
                                        "smoothedHistogram",
                                  srcImage.getUserInterface());
          featureImage.importData(0, featureSpace, true);*/
              ...
              for (i = 0, k = 0; i < nVOIs; i++) {
                  ...
                  if (points != null) {
                      for (j = 0; j < points.length; j++) {
                          ...
                          principalLabel[k++] = label[blueIndex*redGreen + greenIndex*redBins
                                                      + redIndex];
                      }
                  }
              } // for (i = 0, k = 0; i < nVOIs; i++)
                  ...
                  for (j = 0; j < nPoints; j++) {
                      for (k = 0, x = 0, y = 0, z = 0; k < nBins; k++) {
                          if (label[k] == principalLabel[j]) {
                              testDistance = (x - meanx[i])*(x - meanx[i]) +
                                         (y - meany[i])*(y - meany[i]) +
                                         (z - meanz[i])*(z - meanz[i]);
          ...
          if (label2 != null) {
              for (i = 0; i < totLength; i++) {
                  ...
                  greenIndex = (int) ((greenBuffer[i] - greenMin) / greenDivide);
                  blueIndex = (int) ((blueBuffer[i] - blueMin) / blueDivide);
                  segBuffer[i] = label2[blueIndex * redGreen + greenIndex * redBins +
                                        redIndex];
              }
              ...
              for (i = 0; i < totLength; i++) {
                  ...
                  greenIndex = (int) ((greenBuffer[i] - greenMin) / greenDivide);
                  blueIndex = (int) ((blueBuffer[i] - blueMin) / blueDivide);
                  segBuffer[i] = label[blueIndex * redGreen + greenIndex * redBins +
                                       redIndex];
              }
          }
          if (destImage != null) {
    ...
    private void segmentBW() {
        ...
        try {
            ...
            lapAlgo = new AlgorithmLaplacian(image2, lapSigmas,
                                             wholeImage, image25D, ampFactor);
