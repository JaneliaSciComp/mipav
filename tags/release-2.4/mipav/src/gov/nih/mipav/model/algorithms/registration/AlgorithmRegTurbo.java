...
/**
 *   This code is derived from the Turbo Registration code of Phillipe Thevenaz,
 *   speicifcally code from the module TurboReg_.java.
 ...
 *   The source image is transformed until it is
 *   registered to the target image. Algorithms are executed in their own thread.
 *
 *   In automatic mode, the landmark points of the source image are automatically
 *   refined to minimize the mean-square difference between the target and the warped
 *   source image.  For automatic mode if apply to only VOI region is not selected,
 *   then every pixel within the image is considered relevant and should participate
 *   in the mean-square computation.  If apply to only VOI region is selected, then
 *   only those data inside the VOI region should participate in the mean-square
 *   computation.
 *
 *   Two output images are returned.  The first is the warped source image.  The second
 *   is a boolean mask image formed from the warped source VOI.
 *
 */
...
public class AlgorithmRegTurbo extends AlgorithmBase {
    ...
    // In 2D a pair of points determines the combination of a translation, of a rotation, and
    // of an isotropic scaling.  Angles are conserved.
    private static final int SCALED_ROTATION = 2;
    // In 2D three points generate an affine transformation, which is any combination of
    // translation, rotation, isotropic scaling, anisotropic scaling, shearing, and skewing.
    // An affine transformation maps parallel lines onto parallel lines.
    private static final int AFFINE = 3;
    // In 2D four points describe a bilinear transformation, where a point of coordinates(x,y) is
    // mapped on a point of coordinates (u,v) such that u = p0 + p1*x + p2*y + p3*x*y and
    // v = q0 + q1*x + q2*y + q3*x*y.  Thus, both u and v are both linear in x, and in y as well.
    private static final int BILINEAR_DISTORTION = 4;
    // The numerical value of transformation is used in scaleBottomDownLandmarks
    ...
    // Create an output image that is distorted in such a way that the landmarks of the source
    // are made to coincide with those of the target.
    private static final int MANUAL = 1;
    // Refine the landmarks of the source image in such a way that the least-squares error between
    // the source image and target is minimized.
    ...
    /*********************************************************************
     * Minimal size of an image in the multiresolution pyramid.
     * To use automatic mode all all dimensions of both the source and target images must be >=
     * 2*MIN_SIZE.  If any dimension of either image is less than 2*MIN_SIZE, processing will
     * be NEAREST_NEIGHBOR interpolation in MANUAL mode.
     ********************************************************************/
    ...
    private void constructLog() {
        historyString = new String(
                "Turbo Registration(" + String.valueOf( entireTarget ) + ", " + String.valueOf( entireSource ) + ", "
                + String.valueOf( transformation ) + ", " + String.valueOf( interpolation ) + ", "
                + String.valueOf( automan ) + ")" + "\n" );
    }
    ...
    private void getPyramidDepth(
        ...
        int sh = sourceHeight;
        pyramidDepth = 0;
        while ( ( ( 2 * MIN_SIZE ) <= tw ) && ( ( 2 * MIN_SIZE ) <= sw ) && ( ( 2 * MIN_SIZE ) <= th )
                && ( ( 2 * MIN_SIZE ) <= sh ) ) {
            tw /= 2;
            sw /= 2;
            th /= 2;
    ...
    /*------------------------------------------------------------------*/
    private boolean[] getHalfMask2D(
            final boolean[] fullMask,
            final int fullWidth,
            final int fullHeight
            ) {
        int halfWidth = fullWidth / 2;
