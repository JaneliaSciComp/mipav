...
/**
 *   This structure contains the basic information that
 *   describes how the image is stored on disk.
 *   <p>
 *   Subclasses add additional information which is particular
 *   to that image-format.
 *
 *   This class needs work
 *
 *       1. fixing (making consistance what to do when null pointers
 *       encountered. see getStartLocation and getUnitsOfMeasure
 *
...
public abstract class FileInfoBase extends ModelSerialCloneable {
    ...
    /** Array of all units  --- the first value is unknown since all of the
     **                          static definitions start at 1 instead of 0.
     */
    private static final String[] allUnits = {
        UNKNOWN_STRING, UNKNOWN_STRING, INCHES_STRING, CENTIMETERS_STRING,
        ANGSTROMS_STRING, NANOMETERS_STRING, MICROMETERS_STRING, MILLIMETERS_STRING, METERS_STRING, KILOMETERS_STRING,
        MILES_STRING, NANOSEC_STRING, MICROSEC_STRING, MILLISEC_STRING, SECONDS_STRING, MINUTES_STRING, HOURS_STRING,
        HZ_STRING, PPM_STRING, RADS_STRING };

    /** Array of all abbreviated units  --- the first value is unknown since all of the
     **                          static definitions start at 1 instead of 0. Each string
     **                          can be no more than 4 characters.
     */
    private static final String[] allAbbrevUnits = {
        "unk", "unk", "in", "cm", "A", "nm", "um", "mm", "m", "km", "mi",
        "nsec", "usec", "msec", "sec", "min", "hr", "hz", "ppm", "rads" };

    /** Array of space units: inches, mm, etc. */
    public static final String[] sUnits = {
        "Unknown", "Unknown", "Inches", "cm", "A", "nm", "um", "mm", "m", "km",
        "miles" };

    /** Array of time units: seconds, minutes, etc. */
    public static final String[] tUnits = {
        "nano seconds", "micro seconds", "milli seconds", "seconds", "minutes",
    ...
    private static final String[] modalityStr = {
        "Unknown Modality", "Biomagenetic Imaging", "Color Flow Doppler",
        ...
        "Radiographic Imaging", "Radiotherapy Dose", "Radiotherapy Image", "Radiotherapy Plan", "Radiotherapy Record",
    ...
    private static final String[] axisOrientationStr = {
        "Unknown", "Right to Left", "Left to Right",
    ...
    private static final String[] transformIDStr = {
        "Unknown", "Scanner Anatomical", "Another Dataset",
     ...
     *   Intel, DEC Alpha   *****  LSB first byte                LITTLE_ENDIAN (false)
     *   Motorola (MAC), SPARC (SUN), SGI IRIX  MSB first byte   BIG_ENDIAN    (true)
     */
    ...
    /** axis orientation used to support image ordering and display for medical images. We
     *   support the right hand rule where the origin is the upper left hand of the image with
     *   the positive axis.
     *
     *   x - left to right
     *   y - top to botton
    ...
    /** the origin to support image locations (ie. DICOM, MINC ...)
     *   it is relative to the image origin.
     *   the positive axis are (right hand rule.
     *
     *   x - left to right
     *   y - top to botton
     *   z - into the screen
     ...
     *   @throws ArrayIndexOutOfBoundsException when the index is not 0, 1, 2, or 3.
     *   @throws NullPointerException when axisOrientation does not yet exist.
     */
     ...
     *   @return  boolean <code>false</code> = litteEndian format
     *            <code>true</code>  = bigEndian format
     */
    ...
    /**
     *   Photometric interpretion <p>
     *   <table border=true>
     *                    <tr><td>1 indicates</td>   <td>0 is black</td></tr>
     *                    <tr><td>0 indicates</td>   <td>0 is white</td></tr>
     *                    <tr><td>2</td>             <td> RGB</td></tr>
     *                    <tr><td>3</td>             <td>indexed color LUT is saved with image</td></tr>
     *                    <tr><td>4</td>             <td>Transparency Mask</td></tr>
     *   </table>
    ...
    /**
     * Get the direction for accessing each axis of data.  This is
     * based on the values in the axisOrientation array.
     * @return int[] Array of +/-1 values with one entry for each axis.
     * A -1 indicates that the direction is reversed.
     */
    ...
    /**
     *   Sets the start location in the specified axis.  creates
     *   the startLocations if the arrray has not yet been created.
    ...
    /**
     *   Sets photometric interpretation <p>
     *   <table border=true>
     *       <tr><td>1 indicates</td><td>0 is black</td></tr>
     *       <tr><td>0 indicates</td><td>0 is white</td></tr>
     *       <tr><td>2</td>          <td>RGB</td></tr>
    ...
    public String getVolumeUnitsOfMeasureStr() {
        ...
        if (measure == FileInfoBase.INCHES)           {mStr = " inches^3";}
        else if (measure == FileInfoBase.ANGSTROMS)   {mStr = " A^3";}
        else if (measure == FileInfoBase.NANOMETERS)  {mStr = " nm^3";}
        else if (measure == FileInfoBase.MICROMETERS) {mStr = " um^3";}
        else if (measure == FileInfoBase.MILLIMETERS) {mStr = " mm^3";}
        else if (measure == FileInfoBase.CENTIMETERS) {mStr = " cm^3";}
        else if (measure == FileInfoBase.METERS)      {mStr = " m^3";}
        else if (measure == FileInfoBase.KILOMETERS)  {mStr = " km^3";}
        else if (measure == FileInfoBase.MILES)       {mStr = " miles^3";}
        else {mStr = "Unknown";}
    ...
    public String getAreaUnitsOfMeasureStr() {
        ...
        measure = getUnitsOfMeasure(0);
        if (measure == FileInfoBase.INCHES)           {mStr = " inches^2";}
        else if (measure == FileInfoBase.ANGSTROMS)   {mStr = " A^2";}
        else if (measure == FileInfoBase.NANOMETERS)  {mStr = " nm^2";}
        else if (measure == FileInfoBase.MICROMETERS) {mStr = " um^2";}
        else if (measure == FileInfoBase.MILLIMETERS) {mStr = " mm^2";}
        else if (measure == FileInfoBase.CENTIMETERS) {mStr = " cm^2";}
        else if (measure == FileInfoBase.METERS)      {mStr = " m^2";}
        else if (measure == FileInfoBase.KILOMETERS)  {mStr = " km^2";}
        else if (measure == FileInfoBase.MILES)       {mStr = " miles^2";}
        else {mStr = "Unknown";}
    ...
    public static boolean getEndianessFromStr( String s ) {
        ...
        } else if ( ( s.indexOf( "Little" ) != -1 ) || ( s.indexOf( "little" ) != -1 )
                || ( s.indexOf( "LITTLE" ) != -1 ) ) {
    ...
    /**
     *   Abstract method which is used by the extending class to display
    ...
    /**
     *   Method called by many extending classes to display basic information
     *   in the dialog common to all images.
     *   @param dialog   Area where image information is to be displayed.
     *   @param matrix   Transformation matrix
     */
    ...
    /**
     *   Helper method to determine if axis A and axis B are the same axis: that is,
     *   if both are the patient x-axis, they will be either R2L or L2R.
    ...
    /**
     *   Helper method that returns the opposite axis orientation of the one sent in;
     *   that is, R2L for L2R, A2P for P2A, etc.
    ...
    /**
     * Helper method to copy core information from one fileinfo into another,
     * this method also has a list of fileinfos NOT to copy (used by JDialogRemoveSlices)
