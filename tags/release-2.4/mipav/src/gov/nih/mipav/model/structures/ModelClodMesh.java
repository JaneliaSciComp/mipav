...
/**
 * A triangle mesh that represents a level surface.  The mesh is stored in
 * its highest level of detail.  The mesh also has an associated sequence
 * of collapse records that store incremental changes in the vertex quantity,
 * the triangle quantity, and the triangle connectivity array.  These records
 * are used to dynamically change the level of detail of the mesh.  The
 * surface viewer creates a Shape3D object whose geometry can be objects
 * generated from an ModelClodMesh object.  The surface color is provided by
 * attaching to the Shape3D object an appearance that contains a material.
 * The vertex normals in ModelClodMesh are used by the lighting system in
 * conjunction with the surface material to produce the surface color.
 *
 *
 */
...
public class ModelClodMesh
{
    ...
    * normal vectors that are computed from the geometry of the mesh itself.
    * The collapse records form a sequence of incremental changes that are
    * used to dynamically change the level of detail by incrementing or
    * decrementing vertex and triangle quantities.
    *
    ...
    * @param akRecord  Array of collapse records that are computed by
    *   ModelSurfaceDecimator.
    */
    public ModelClodMesh (Point3f[] akVertex, int[] aiConnect,
    ...
    /**
     * Accessor for the triangle mesh that corresponds to the current level
     * of detail in the ModelClodMesh.
     *
    ...
    /*
     * The maximum level of detail supported by the mesh.  The minimum
     * level of detail is always zero (the first collapse record).
     *
     ...
     * @return a triangle mesh corresponding to the selected LOD
     */
     ...
     *            writing
     */
    public void print (String kName)
        throws IOException
    {
     ...
     *            writing
     */
    static public void print (String kName, ModelClodMesh[] akComponent)
        throws IOException
    {
     ...
     * records to the file in the form:
     * <pre>
     ...
     * @param flip if the y axis should be flipped - true in extract and
     ...
     * @exception IOException if there is an error writing to the file
     */
    static public void save (String kName, ModelClodMesh[] akComponent, boolean flip,
    ...
    protected void save (RandomAccessFile kOut, boolean flip, int[] direction,
        ...
        throws IOException
    {
        m_kMesh.save(kOut, flip,direction,startLocation,box,
                     inverseDicomArray);
    ...
    /**
     * Load the clod mesh from a binary file.  The caller must have
     ...
     */
    static public ModelClodMesh loadCMesh (RandomAccessFile kIn, ViewJProgressBar progress, int added, int piece)
        throws IOException
    {
        try
        {
            ...
            for (i = 0, index=0; i < iVertexCount; i++)
            {
                ...
                if (prog != 0 && i%prog == 0) progress.updateValueImmed(added + i/prog);
            }
            ...
            for (i = 0, index=0; i < iVertexCount; i++)
            {
                ...
                if (prog != 0 && i%prog == 0) progress.updateValueImmed(Math.round(25/piece) + added + i/prog);
            }
            ...
            for (i = 0, index = 0; i < iIndexCount; i++) {
                ...
                if (prog != 0 &&  i%prog == 0) progress.updateValueImmed(Math.round(50/piece) + added + i/prog);
            }
            ...
            for (i = 0; i < iRecordCount; i++)
            {
                ...
                akRecord[i].load(kIn);
                if (prog != 0 && i%prog == 0 ) progress.updateValueImmed(Math.round(75/piece) + added + i/prog);
            }
     ...
     * @exception IOException if there is an error reading from the file
     */
    static public ModelClodMesh loadCMesh (RandomAccessFile kIn, JProgressBar pBar, int added, int piece)
        throws IOException
    {
        try
        {
     ...
     *  Used for smoothing.
     *  @param veriticies   New vertices for clod mesh.
     */
    ...
    // current level of detail record,
    //   0 <= m_iCurrentRecord <= m_akRecord.length-1
    ...
    // Java3D supports limiting the vertices to be processed by the call
    // setValidVertexCount(int).  However, there is no equivalent call that
    // supports limiting the triangles [setValidIndexCount(int) is needed].
    // The dynamic edge collapse partially depends on changing the number of
    // active triangles.  Since this is not supported, the setLOD(int) method
    // is designed to create an ModelTriangleMesh object on each change.  Not
    // very efficient.  Better would be to just allow us to change the valid
    // index count.
