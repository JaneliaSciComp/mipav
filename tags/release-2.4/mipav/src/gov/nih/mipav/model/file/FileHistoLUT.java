...
/**
 * HistoLUT reader/writer. This classes incorporates the ability to write/read
 * ModelLUT and ModelRGB information in two ways. The first saves the LUT
 * arrays (not available with ModelRGB), and the second saves the transfer
 * and RGB functions.  The LUT transfer Line is saved by default (and
 * likewise opened) when the saveLUT method is called.
 *
 * NOTE:  Reading a LUT does not return a ModelLUT.  Rather, it updates the arrays
 *        and transfer functions in a provided ModelLUT.
 *
 * @version    May 14, 2002
...
public class FileHistoLUT extends FileBase {
     ...
     */
    private static final float[] parseString( String s, int len ) throws Exception {
        StringTokenizer str;
        float[] array;
    ...
    public void writeOpacityFunctions() throws IOException {
        ...
        /*if ( useLUT ) {
            writeTransferFunction( raFile, lut.getRedFunction(), "Red" );
            writeTransferFunction( raFile, lut.getRedFunction(), "Green" );
            writeTransferFunction( raFile, lut.getRedFunction(), "Blue" );
        } else {
            writeTransferFunction( raFile, rgb.getRedFunction(), "Red" );
            writeTransferFunction( raFile, rgb.getRedFunction(), "Green" );
            writeTransferFunction( raFile, rgb.getRedFunction(), "Blue" );
        }*/
