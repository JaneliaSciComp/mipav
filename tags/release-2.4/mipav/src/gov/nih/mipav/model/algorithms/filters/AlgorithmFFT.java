 ...
 *  Processing images by filtering in the frequency domain is a 3 step process:
 *  1.) Performing a forward fast fourier transform to convert a spatial image into
 *      its frequency image.
 *  2.) Enhancing some frequency components of the image and attenuating other
 *      frequency components of the image by mulitplying by a lowpass, highpass,
 *      bandpass, or bandstop filter.  Frequency filters may be constructed with
 *      1 of 3 methods - finite impulse response filters constructed with Hamming
 *      windows, Gaussian filters, and Butterworth filters.  However, for the Gaussian filters
 *      only lowpass and highpass filters are available.
 *  3.) Performing an inverse fast fourier transform to convert from the frequency
 ...
 *  this algorithm on datasets with arbitrary dimensions, the data is zero padded to
 *  powers of 2 before applying the forward fast fourier transform and stripped down to the
 *  original dimensions after applying the inverse fast fourier transform.
 *  If unequalDim is equal to true, the fourier pictures are allowed to have unequal dimensions so
 *  as to save memory.  If unequalDim is equal to false, the pictures have equal dimensions so as
 *  to maximize symmetry.
 *
 *  The typical full sequence is as follows:
 *  1.) Data from a real spatial image is exported into a float array realData.
 *  2.) An equally sized float array called imagData is created and filled with zeros.
 *  3.) realData and imagData are enlarged to the same length in every dimension.
 *      If finite impulse repsonse filters are constructed
 *      with Hamming windows and no cropping, the new dimension size is equal to the minimum power of two
 *      number that equals or exceeds the maximum original dimension size + kDim - 1, where
 *      kDim is the diameter of a circular or spherical convolution kernel.  If finite impulse
 *      response filters with Hamming windows and cropping or infinite impulse response Gaussian
 *      or Butterworth filters are used, the new dimension size is equal to the minimum power of two
 *      number that equals or exceeds the maximum original dimension size.
 *      The data is padded with zeros at the end of each dimension.
 *  4.) exec() is invoked to run the fast fourier transform algorithm.
 *  5.) The center() algorithm is invoked to reorder the data for display. The Fourier
 *      transform of real data is conjugate symmetric; the real parts are even and the
 *      imaginary parts are odd.  In other words, the magnitude is even and the phase
 *      is odd.  Since images display the magnitude or log10(1 + magnitude) a symmetry
 *      reflected across the center of the image is observed.
 *  6.) The complex data is imported to the display image with the image minimum and
 *      maximum being calculated.  If logMagDisplay is set equal to false, the
 *      minimum and maximum of sqrt(realData(i)*realData(i) + imagData(i)*imagData(i))
 *      will be used as the image minimum and maximum in other modules.  If logMagDisplay
 *      is set equal to true, the log10(1 + sqrt(realData(i)*realData(i) + imagData(i)*imagData(i)))
 *      will be used as the image minimum and maximum in other modules.  Note that
 *      while this module stores the complex data in the source or destination image, the display
 *      oriented modules in the MIPAV package will use the magnitude or log magnitude
 *      as indicated above in the screen displays since complex numbers cannot be directly
 *      displayed.
 *  7.) The command setOriginalExtents is invoked so that the original dimensions of the
 *      source image are available along with the display image.  The original dimensions
 *      are needed so that the original source dimensions can be restored after
 *      the inverse fast fourier transform.
 *  If finite impulse response filters with windows are used:
 *  8a.) An ideal filter kernel is constructed in the spatial domain.
 *  9a.) A Hamming window kernel is constructed in the spatial domain.
 * 10a.) The ideal kernel and the Hamming window kernel are multiplied together.
 * 11a.) The kernel is zero padded up to the same dimensions that the image data was.
 * 12a.) exec() is run to obtain the FFT of the kernel.
 * 13a.) The center() algorithm is run to reorder the data.
 * If Gaussian or Butterworth filters are used:
 * For Gaussian and Butterworth filters the transfer functions affect the real and imaginary
 * parts of the FFT of the image in exactly the same manner.  These filters are zero-phase-
 * shift filters because they do not alter the phase of the transform.  Since these filters
 * are conjugate symmetric, the inverse FFTs of these filters are purely real.  Since this filtering
 * is equivalent to convolving to real 2D data sets, the result must be purely real.
 * 8b.) The real part of the FFT is set equal to the appropriate filter magnitude and the
 *      imaginary part is set equal to zero.  This filter has the same dimensions as the
 *      padded image data.
 * 14) The data FFT is set equal to the product of the data FFT and the filter FFT.
 * 15.) The inverse FFT process is invoked.
 *      The complex data is exported into the 2 float arrays realData and imagData.  There
 *      should be no need for zero padding at this point since the dimensions should already
 *      be all powers of 2 from before.
 * 16.) The center() algorithm is invoked to restore the data to its original ordering.
 * 17.) exec() is invoked to run the inverse fast fourier transform algorithm.
 * 18.) The realData now holds the correct response if Gaussian or Butterworth filtering was used.
 *      If FIR filtering with windows was used then realData holds a version of the correct response
 *      shifted by (kDim - 1)/2 toward the end of each dimension.  The imagData should contain
 *      only roundoff error.
 * 19.) For FIR filters shift the data back by (kDim - 1)/2 toward the start of each dimension.
 * 20.) Stripping is performed to return the image to its original dimensions.
 * 21.) The realData() is imported into the the new spatial image.
 *
...
//  Variables used in Gabor filter
     ...
     *  @param imageCrop        if true crop image if largest image dimension + kDim - 1 exceeds the
     *                          smallest power of 2 number >= the largest dimension
     ...
     *  @param freq1            cutoff frequency in LOWPASS and HIGHPASS
     *                          lower frequency in BANDPASS and BANDSTOP
     *  @param freq2            higher frequency in BANDPASS and BANDSTOP
     *                          User inputs f1 and f2 going from 0.0 to 1.0.  Program multiplies these
     *                          numbers by PI for FIR filters and by (newLength - 1)/2 for Gaussian
    ...
    public AlgorithmFFT(ModelImage destImg, ModelImage srcImg, int transformDir,
                        ...
                        boolean image25D, boolean imageCrop, int kernelDiameter,
    ...
    public AlgorithmFFT(ModelImage srcImg, int transformDir, boolean logMagDisplay,
                        boolean unequalDim, boolean image25D, boolean imageCrop,
        ...
        if ( (transformDir == FORWARD) && (doCrop)) {
            ...
            if (ndim == 1) {
                ...
                for (k = start[2]; k <= end[2]; k++)
                    for (j = start[1]; j <= end[1]; j++)
                        for (i = start[0]; i <= end[0]; i++)
                            tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                dimLengths[1] * (k - start[2])] = realData[i + dimLengths[0] * j +
                ...
                for (m = start[3]; m <= end[3]; m++)
                    for (k = start[2]; k <= end[2]; k++)
                        for (j = start[1]; j <= end[1]; j++)
                            for (i = start[0]; i <= end[0]; i++)
                                tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                    dimLengths[1] * (k - start[2]) + dimLengths[0] * dimLengths[1] *
                                    dimLengths[2] * (m - start[3])] = realData[i + dimLengths[0] * j + dimLengths[0] *
                                    dimLengths[1] * k + dimLengths[0] * dimLengths[1] * dimLengths[2] * m];
            }
            else if (ndim == 5) {
                        ...
                        for (k = start[2]; k <= end[2]; k++)
                            for (j = start[1]; j <= end[1]; j++)
                                for (i = start[0]; i <= end[0]; i++)
                                    tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                        dimLengths[1] * (k - start[2]) + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * (m - start[3]) + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * dimLengths[3] * (n - start[4])] = realData[i +
                                        dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * m + dimLengths[0] * dimLengths[1] * dimLengths[2] * dimLengths[3] * n];
            }
            for (i = 0; i < arrayLength; i++)
            ...
            if (ndim == 1) {
                ...
                for (k = start[2]; k <= end[2]; k++)
                    for (j = start[1]; j <= end[1]; j++)
                        for (i = start[0]; i <= end[0]; i++)
                            tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                dimLengths[1] * (k - start[2])] = imagData[i + dimLengths[0] * j +
                                dimLengths[0] * dimLengths[1] * k];
            }
            else if (ndim == 4) {
                for (m = start[3]; m <= end[3]; m++)
                    for (k = start[2]; k <= end[2]; k++)
                        for (j = start[1]; j <= end[1]; j++)
                            for (i = start[0]; i <= end[0]; i++)
                                tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                    dimLengths[1] * (k - start[2]) + dimLengths[0] * dimLengths[1] *
                                    dimLengths[2] * (m - start[3])] = imagData[i + dimLengths[0] * j + dimLengths[0] *
                                    dimLengths[1] * k + dimLengths[0] * dimLengths[1] * dimLengths[2] * m];
            }
            else if (ndim == 5) {
                for (n = start[4]; n <= end[4]; n++)
                            ...
                            for (j = start[1]; j <= end[1]; j++)
                                for (i = start[0]; i <= end[0]; i++)
                                    tempData[i - start[0] + dimLengths[0] * (j - start[1]) + dimLengths[0] *
                                        dimLengths[1] * (k - start[2]) + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * (m - start[3]) + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * dimLengths[3] * (n - start[4])] = imagData[i +
                                        dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k + dimLengths[0] * dimLengths[1] *
                                        dimLengths[2] * m + dimLengths[0] * dimLengths[1] * dimLengths[2] * dimLengths[3] * n];
            }
            for (i = 0; i < arrayLength; i++)
        ...
        if (zeroPad && isProgressBarVisible()) {
            ...
            if (ndim == 1) {
                ...
                for (k = 0; k < dimLengths[2]; k++)
                    for (j = 0; j < dimLengths[1]; j++)
                        for (i = 0; i < dimLengths[0]; i++)
                            tempData[i + newDimLengths[0] * j +
                                newDimLengths[0] * newDimLengths[1] * k] = realData[i +
                                dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k];
            }
            else if (ndim == 4) {
                for (m = 0; m < dimLengths[3]; m++)
                    for (k = 0; k < dimLengths[2]; k++)
                        for (j = 0; j < dimLengths[1]; j++)
                            for (i = 0; i < dimLengths[0]; i++)
                                tempData[i + newDimLengths[0] * j +
                                    newDimLengths[0] * newDimLengths[1] * k +
                            ...
                            for (j = 0; j < dimLengths[1]; j++)
                                for (i = 0; i < dimLengths[0]; i++)
                                    tempData[i + newDimLengths[0] * j +
                                        ...
                                        newDimLengths[0] * newDimLengths[1] * newDimLengths[2] * newDimLengths[3] * n] =
                                        realData[i + dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k +
                                        dimLengths[0] * dimLengths[1] * dimLengths[2] * m +
                                        dimLengths[0] * dimLengths[1] * dimLengths[2] * dimLengths[3] * n];
            }
            try {
            ...
            if (ndim == 1) {
                ...
                for (k = 0; k < dimLengths[2]; k++)
                    for (j = 0; j < dimLengths[1]; j++)
                        for (i = 0; i < dimLengths[0]; i++)
                            tempData[i + newDimLengths[0] * j +
                                newDimLengths[0] * newDimLengths[1] * k] = imagData[i +
                                dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k];
            }
            else if (ndim == 4) {
                for (m = 0; m < dimLengths[3]; m++)
                            ...
                            for (j = 0; j < dimLengths[1]; j++)
                                for (i = 0; i < dimLengths[0]; i++)
                                    tempData[i + newDimLengths[0] * j +
                                        ...
                                        newDimLengths[0] * newDimLengths[1] * newDimLengths[2] * m +
                                        newDimLengths[0] * newDimLengths[1] * newDimLengths[2] * newDimLengths[3] * n] =
                                        imagData[i + dimLengths[0] * j + dimLengths[0] * dimLengths[1] * k +
                                        dimLengths[0] * dimLengths[1] * dimLengths[2] * m +
                                        dimLengths[0] * dimLengths[1] * dimLengths[2] * dimLengths[3] * n];
            }
            try {
            ...
            for (z = 0; z <= kDim - 1; z++) {
                for (y = 0; y <= kDim - 1; y++) {
                    for (x = 0; x <= kDim - 1; x++) {
                        ...
                        realKernelData[newDimLengths[0] * newDimLengths[1] * z + newDimLengths[0] * y + x] =
                            iKernel[pos] * wKernel[pos];
                    }
                }
            }
            try {
    ...
    /* This function replaces the original image with a new image that is
     *  either the FFT, the filtered FFT, or the inverse FFT of the original or filtered image.
     */
        ...
        if ( (transformDir == FORWARD) || (transformDir == INVERSE)) {
            ...
            if (image25D) {
                ...
                for (z = 0; z < newDimLengths[2]; z++) {
                    ...
                    if (isProgressBarVisible()) {
                        progressBar.updateValue(Math.round(10 +
                                (float) (z + 1) / newDimLengths[2] * 80),
                                             activeImage);
                    }
                    if (transformDir == FORWARD) {
                        for (i = 0; i < newSliceSize; i++) {
    ...
    /**
     *  This function produces a new image that is either the FFT, the filtered FFT,
     *  or the inverse FFT of the original or filtered image.
     */
    ...
    private void phase(float phase[]) {
        ...
        for (i = 0; i < arrayLength; i++) {
            phase[i] = (float) java.lang.Math.atan2(
                imagData[i], realData[i]);
        }
    }
            ...
            if (filterType == LOWPASS) {
                ...
                for (z = 0; z <= upperZ; z++)
                    for (y = 0; y <= newDimLengths[1] - 1; y++)
                        for (x = 0; x <= newDimLengths[0] - 1; x++) {
                            pos = z * newSliceSize + y * newDimLengths[0] + x;
                            coeff = (float) (1.0 - Math.exp( - (x - xcenter) * (x - xcenter) / xexpDenom) *
                                             Math.exp( - (y - ycenter) * (y - ycenter) / yexpDenom));
                            realData[pos] *= coeff;
                            imagData[pos] *= coeff;
                        }
            ...
            if (filterType == LOWPASS) {
                for (z = 0; z <= upperZ; z++)
                    for (y = 0; y <= newDimLengths[1] - 1; y++)
                        for (x = 0; x <= newDimLengths[0] - 1; x++) {
                            pos = z * newSliceSize + y * newDimLengths[0] + x;
                            distsq = (x - xcenter) * (x - xcenter) / xnorm +
                                (y - ycenter) * (y - ycenter) / ynorm;
    ...
    /* l'Hopitals rule if f(a) = g(a) = 0 and if the limit of the ratio f'(t)/g'(t) as t approaches a exists,
       ...
       ((R*R)/(2*PI))*J(R*sqrt(n1*n1 + n2*n2))/(R*sqrt(n1*n1 + n2*n2)).  The taylor series for J1(x) =
       (x/2) - ((x**3)/16) + ...  Therefore, the derivative of J1(x) at x = 0 equals 1/2.  Hence, the
       hlp(n1,n2)low pass impulse response is equal to (R**2)/(4*PI) at n1=n2=0. */
    ...
    private void idealLPKernel3D(float cutoffFreq) {
        ...
        for (z = 0; z <= kDim - 1; z++) {
            for (y = 0; y <= kDim - 1; y++) {
                for (x = 0; x <= kDim - 1; x++) {
                    pos = z * kDim * kDim + y * kDim + x;
                    distance = (float) Math.sqrt( (x - halfKDim) * (x - halfKDim) +
                                                 (y - halfKDim) * (y - halfKDim) + (z - halfKDim) * (z - halfKDim));
     ...
     * Note that a frequency filter operation performs a forward FFT
    ...
    private void center(float[] rData, float[] iData) {
        ...
        // center data is buffer to hold 1/8 of data as we center the FFT
            ...
            for (k = 0; k < zdimHalf; k++) {
                for (j = 0; j < ydimHalf; j++) {
                    for (i = 0; i < xdimHalf; i++) {
                        centerData[centerSliceSize * k + xdimHalf * j + i]
                            = rData[sliceSize * k + xdim * j + i];
                    }
                }
            }
            ...
            for (k = 0; k < zdimHalf; k++) {
                for (j = 0; j < ydimHalf; j++) {
                    for (i = 0; i < xdimHalf; i++) {
                        ...
                        ynew = j + ydimHalf;
                        znew = k + zdimHalf;
                        rData[sliceSize * znew + xdim * ynew + xnew] =
                            centerData[centerSliceSize * k + xdimHalf * j + i];
                    }
                }
            }
             ...
             for (k = 0; k < zdimHalf; k++) {
                 for (j = 0; j < ydimHalf; j++) {
                     for (i = xdimHalf; i < xdim; i++) {
                         ...
                         ynew = j;
                         znew = k;
                         centerData[centerSliceSize * znew + xdimHalf * ynew + xnew]
                             = rData[sliceSize * k + xdim * j + i];
                     }
                 }
             }
            ...
            for (k = 0; k < zdimHalf; k++) {
                for (j = 0; j < ydimHalf; j++) {
                    for (i = 0; i < xdimHalf; i++) {
                        ...
                        ynew = j + ydimHalf;
                        znew = k + zdimHalf;
                        iData[sliceSize * znew + xdim * ynew + xnew] =
                            centerData[centerSliceSize * k + xdimHalf * j + i];
                    }
                }
            }
    ...
    private void shiftBack(float[] rData) {
        ...
        if (ndim == 1) {
            ...
            for (j = 0; j < halfKDim; j++) {
                for (i = 0; i < halfKDim; i++)
                    tempData[i + newDimLengths[0] - halfKDim + newDimLengths[0] * (j + newDimLengths[1] - halfKDim)]
                        = rData[i + newDimLengths[0] * j];
                for (i = halfKDim; i < newDimLengths[0]; i++)
            ...
            for (k = 0; k < halfKDim; k++) {
                for (j = 0; j < halfKDim; j++) {
                    for (i = 0; i < halfKDim; i++)
                        tempData[i + newDimLengths[0] - halfKDim + newDimLengths[0] * (j + newDimLengths[1] -
                            halfKDim) + newDimLengths[0] * newDimLengths[1] * (k + newDimLengths[2] - halfKDim)] =
                            rData[i + newDimLengths[0] * j + newDimLengths[0] * newDimLengths[1] * k];
                    for (i = halfKDim; i < newDimLengths[0]; i++)
                        tempData[i - halfKDim + newDimLengths[0] * (j + newDimLengths[1] - halfKDim) +
                            newDimLengths[0] * newDimLengths[1] * (k + newDimLengths[2] - halfKDim)] =
                            rData[i + newDimLengths[0] * j + newDimLengths[0] * newDimLengths[1] * k];
                }
                for (j = halfKDim; j < newDimLengths[1]; j++) {
                    for (i = 0; i < halfKDim; i++)
            ...
            for (k = halfKDim; k < newDimLengths[2]; k++) {
                for (j = 0; j < halfKDim; j++) {
                    for (i = 0; i < halfKDim; i++)
                        tempData[i + newDimLengths[0] - halfKDim + newDimLengths[0] * (j + newDimLengths[1] -
                            halfKDim) + newDimLengths[0] * newDimLengths[1] * (k - halfKDim)] =
                            rData[i + newDimLengths[0] * j + newDimLengths[0] * newDimLengths[1] * k];
                    for (i = halfKDim; i < newDimLengths[0]; i++)
                        tempData[i - halfKDim + newDimLengths[0] * (j + newDimLengths[1] - halfKDim) +
                            newDimLengths[0] * newDimLengths[1] * (k - halfKDim)] =
                            rData[i + newDimLengths[0] * j + newDimLengths[0] * newDimLengths[1] * k];
                }
                for (j = halfKDim; j < newDimLengths[1]; j++) {
                    for (i = 0; i < halfKDim; i++)
    ...
    private void edgeStrip(float[] rData, int z) {
        ...
        if (ndim == 1) {
            ...
            for (k = 0; k < originalDimLengths[2]; k++)
                for (j = 0; j < originalDimLengths[1]; j++)
                    for (i = 0; i < originalDimLengths[0]; i++)
                        tempData[i + originalDimLengths[0] * j + originalDimLengths[0] * originalDimLengths[1] * k] =
                            rData[i + newDimLengths[0] * j +
                            newDimLengths[0] * newDimLengths[1] * k];
        }
        else if (ndim == 4) {
    ...
    /**
     *   If a cropping operation was performed, shift the data back to the center and put zeros
     *   around the edges
     */
    private void zeroAround() {
        ...
        if (ndim == 1) {
            ...
            for (j = 0; j <= end[1] - start[1]; j++)
                for (i = 0; i <= end[0] - start[0]; i++)
                    tempData[i + start[0] + originalDimLengths[0] * (j + start[1])] = finalData[i + originalDimLengths[0] * j];
        }
        else if (ndim == 3) {
            for (k = 0; k <= end[2] - start[2]; k++)
                for (j = 0; j <= end[1] - start[1]; j++)
                    for (i = 0; i <= end[0] - start[0]; i++)
                        tempData[i + start[0] + originalDimLengths[0] * (j + start[1]) + originalDimLengths[0] *
                            originalDimLengths[1] * (k + start[2])] = finalData[i + originalDimLengths[0] * j +
            ...
            for (m = 0; m <= end[3] - start[3]; m++)
                for (k = 0; k <= end[2] - start[2]; k++)
                    for (j = 0; j <= end[1] - start[1]; j++)
                        for (i = 0; i <= end[0] - start[0]; i++)
                            tempData[i + start[0] + originalDimLengths[0] * (j + start[1]) + originalDimLengths[0] *
                                originalDimLengths[1] * (k + start[2]) + originalDimLengths[0] * originalDimLengths[1] *
                                originalDimLengths[2] * (m + start[3])] = finalData[i + originalDimLengths[0] * j +
                                originalDimLengths[0] * originalDimLengths[1] * k + originalDimLengths[0] * originalDimLengths[1] *
                    ...
                    for (k = 0; k <= end[2] - start[2]; k++)
                        for (j = 0; j <= end[1] - start[1]; j++)
                            for (i = 0; i <= end[0] - start[0]; i++)
                                tempData[i + start[0] + originalDimLengths[0] * (j + start[1]) + originalDimLengths[0] *
                                    originalDimLengths[1] * (k + start[2]) + originalDimLengths[0] * originalDimLengths[1] *
                                    originalDimLengths[2] * (m + start[3]) + originalDimLengths[0] * originalDimLengths[1] *
                                    originalDimLengths[2] * originalDimLengths[3] * (n + start[4])] = finalData[i +
                                    originalDimLengths[0] * j + originalDimLengths[0] * originalDimLengths[1] * k +
                                    originalDimLengths[0] * originalDimLengths[1] * originalDimLengths[2] * m +
                                    originalDimLengths[0] * originalDimLengths[1] * originalDimLengths[2] * originalDimLengths[3] *
                                    n];
        }
