 ...
 *      some user specified value. In addition, the square root or log of
 *      an image can be calculated. If the new image exceeds the range that
 ...
 *      that can store the range of new data is generated.
 *
...
public class AlgorithmImageMath
    ...
    private static final String[] opString = {
        "ABSOLUTE_VALUE", "ADD", "AVERAGE", "CONSTANT", "DIVIDE",
        "LOG", "MULTIPLY", "SQUARE", "SQRT", "SUBTRACT", "SUM"};
     ...
     */
    public AlgorithmImageMath(ModelImage destImg, ModelImage srcImg, int type,
                              double val, double valI, int _clipMode, boolean maskFlag) {
    ...
    public AlgorithmImageMath(ModelImage srcImg, int type, double val, double valI,
                              int _clipMode, boolean maskFlag) {
        super(null, srcImg);
        entireImage = maskFlag;
        opType = type;
        value = val;
        ...
        for (m = 0; m < f && !threadStopped; m++) {
            for (k = 0; k < t && !threadStopped; k++) {
                for (j = 0; j < z && !threadStopped; j++) {
                    ...
                    for (i = 0; i < length && !threadStopped; i++) {
                        ...
                        if (entireImage == true || mask.get(i + offset)) {
                            switch (opType) {
                                ...
                                case SQRT:
                                    mag = Math.sqrt(Math.sqrt(buffer[i] * buffer[i] +
                                        bufferI[i] * bufferI[i]));
                    ...
                    else if ( (clipMode == PROMOTE) &&
                             (srcImage.getType() == ModelStorageBase.COMPLEX)) {
                    ...
                    if (needPromote) {
                        ...
                        try {
                            srcImage.exportDComplexData(0, totalLength, totBuffer, totBufferI); // locks and releases lock
                        }
                        catch (IOException error) {
                            displayError("Algorithm ImageMath : Image(s) locked");
                            setCompleted(false);
                    ...
                    if (clipMode == CLIP) {
                        ...
                        for (i = 0; i < length; i++) {
                            if ( (buffer[i] > Float.MAX_VALUE) || (buffer[i] < -Float.MAX_VALUE) ||
                             ...
                             (destImage.getType() == ModelStorageBase.COMPLEX)) {
                        for (i = 0; i < length && !needPromote; i++) {
                            if ( (buffer[i] > Float.MAX_VALUE) || (buffer[i] < -Float.MAX_VALUE) ||
                                (bufferI[i] > Float.MAX_VALUE) || (bufferI[i] < -Float.MAX_VALUE)) {
                                needPromote = true;
                    ...
                    if (needPromote) {
                        ...
                        if (buffersDone > 0) {
                            try {
                                ...
                                System.gc();
                                displayError(
                                    "Algorithm ImageMath reports: Out of memory when creating total image buffer");
                                setCompleted(false);
                                return;
                            }
                            try {
                                destImage.exportDComplexData(0, buffersDone * length,
                                    totBuffer, totBufferI); // locks and releases lock
                            }
                            catch (IOException error) {
                                displayError(
                                    "Algorithm ImageMath : Image(s) locked");
                                setCompleted(false);
                                return;
                            }
                        } // if (buffersDone > 0)
                        ...
                        if (buffersDone > 0) {
                            try {
                                ...
                                destImage.importDComplexData(0, totBuffer,
                                    totBufferI, false, true);
                            }
                            catch (IOException error) {
                                displayError(
                                    "Algorithm ImageMath: Image(s) locked");
