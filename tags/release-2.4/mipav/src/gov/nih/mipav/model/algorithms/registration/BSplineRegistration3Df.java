 ...
 * The images do not have to have the same dimensions, but they must
 * both contain single-channel data.  The resulting registered source image
 * has the same dimensions as the input target image.  A 3D B-Spline is
 * used to map the coordinates of the registered image to coordinates
 * of the input source image.  A separate B-Spline basis is setup for each
 * axis given the number of control points and the degree of the basis
 * functions to use.  Each B-Spline is uniform open with control points
 * spaced equally for each dimension.  Trilinear interpolation is used to
 * determine which input source image value(s) to assign to the resulting
 * registered source image.  The "error" measure between the input target
 * image and the current registered source image is the sum of squared
 * differences, where the root mean squared error is also computed.  Control
 * points are moved one at a time by means of gradient descent minimization
 * in order to minimize the error.  The gradient is approximated at each
 * control point by means of finite differences.  Control points are
 * restricted from moving outside the polyhedron formed by its 26 neighboring
 * control points.
 */
public class BSplineRegistration3Df
    ...
    /**
     * Create instance to be used for registration.  Upon construction,
     * the initial state of the registered source image is computed
     * along with the error in comparing it to the reference target image.
     ...
     * @param kBasisX BSplineBasisf B-spline basis for X axis which contains
     * the B-spline degree and number of control points.
     * @param kBasisY BSplineBasisf B-spline basis for Y axis which contains
     * the B-spline degree and number of control points.
     * @param kBasisZ BSplineBasisf B-spline basis for Z axis which contains
     * the B-spline degree and number of control points.
    ...
    public BSplineRegistration3Df(ModelSimpleImage kImageSrc,
                                  ...
                                  BSplineBasisf kBasisX,
                                  BSplineBasisf kBasisY,
                                  BSplineBasisf kBasisZ,
        ...
        updateSamples(
            0, kImageTrg.extents[0] - 1,
            0, kImageTrg.extents[1] - 1,
            0, kImageTrg.extents[2] - 1);
    }

    /**
     * Create a new registration with the same mapping as this one and
     * which uses the same source image and the same BSpline basis degree
     * and number of control points.  The only thing that changes is that
     * the discrete [0,1] interval is resampled.
     * @param kImageTrg ModelSimpleImage Resampled target image.
     * @return BSplineRegistration2Df New registration of the same source
     * image referenced to the resampled target image.
     */
    ...
    /**
     * Create a new registration with the same mapping as this one and
     * which uses the same source image and a possibly resampled
     * target image, however, this time the BSpline basis degree and/or the
     * number of control points may be different.
     ...
     */
    public BSplineRegistration3Df createSameMapping(ModelSimpleImage kImageTrg,
        BSplineBasisf kBasisX, BSplineBasisf kBasisY, BSplineBasisf kBasisZ) {
        ...
        if (kBasisX.isSameAs(this.m_kBSplineBasisX) &&
            kBasisY.isSameAs(this.m_kBSplineBasisY) &&
    ...
    /**
     * Return access to the 3D discretized B-Spline internally created
     * given the size of tbe input target image, and the specified number
     * of control points and degree of basis function for each B-Spline
     * basis axis.
    ...
    /**
     * Generate an image the size of the target image in which each
     * sample contains the 3D interpolated point coordinates in the
     * source image which "maps" or is registered with the corresponding
     ...
     * for the map.  Each image has the same dimensions which
     * are that of the target image.
     */
    public ModelSimpleImage[] createImageSourceMap() {

        ModelSimpleImage[] akSourceMap = m_kBSpline3D.createImageMap(
            m_kImageTrg.extents[0],
            m_kImageTrg.extents[1],
    ...
    /**
     * Generate an image which contains the deformation value at each
     * sample in the target image as it relates to how the source image
     * is transformed as a result of the registration.  The image of
     * interpolated source image coordinates for each target image sample
     * is generated and from that the Jacobian is generated at each
     * sample using finite differences.  The deformation value is then
     * the determinant of the Jacobian.
     * @return ModelSimpleImage New instance which contains the properties and
     * data for the deformation image.  This image has the same dimensions as
     * and corresponds to the sample in the target image.
     */
    public ModelSimpleImage createImageDeformation() {
        ...
        ModelSimpleImage kDeformation = new ModelSimpleImage(
            m_kImageTrg.extents, m_kImageTrg.resolutions);
    ...
    /**
     * Generate an image which contains the magnitude of the displacement
     * vector at each sample in the target image as it relates to how the
     * source image is transformed as a result of the registration.  The image
     * of interpolated source image coordinates for each target image sample
     * is generated and the displacement is computed from the source image
     * coordinates for the identity registration transformation.
     * @return ModelSimpleImage New instance which contains the properties and
     * data for the deformation image.  This image has the same dimensions as
     * and corresponds to the sample in the target image.
     */
    public ModelSimpleImage createImageDisplacement() {
        ...
        ModelSimpleImage kImageDisplacement = new ModelSimpleImage(
            m_kImageTrg.extents, m_kImageTrg.resolutions);
        ...
        for (int iZ = 0; iZ < m_kBSplineBasisZ.getNumSamples(); iZ++) {
            ...
            for (int iY = 0; iY < m_kBSplineBasisY.getNumSamples(); iY++) {
                ...
                for (int iX = 0; iX < m_kBSplineBasisX.getNumSamples(); iX++) {
                    ...
                    kImageDisplacement.data[iIndex] =
    ...
    /**
     * Move the control point from its current location to the requested
     * location.  The requested location must be inside the polyhedron formed
     * by its 26 neighboring control points.  If not, then a line segment
     * is created between the current and requested locations and the point
     * where this line segment intersects the bounding polyhedron is found.
     * This point is offset a little along the line segment so that it
     * is just inside the bounding polyhedron, and this point is taken as
     * the new location for the specified control point.
     ...
     * @param kPoint Point3f Requested new coordinates for the specified
     * control point.
     * @return float The distance between the updated control point position
     * and its position before being modified.
     */
    public float moveControlPoint(int iControlX, int iControlY, int iControlZ, Point3f kPoint) {
        ...
        fDist =
            getControlPointMaxMoveDist(iControlX, iControlY, iControlZ,
                                       kDirection, fDist);
    ...
    /**
     * Move the specified control point using a gradient descent approach
     * to minimize the error.  The gradient descent minimization is performed
     * by sampling the error function at specified intervals.  A maximum
     * number of samples is specified, but sampling always terminates when
     * the bounding polyhedron formed by the 26 neighboring control points
     * is reached.
     * @param iControlX int Identifies the control point in the 3D lattice.
     * @param iControlY int Identifies the control point in the 3D lattice.
     * @param iControlZ int Identifies the control point in the 3D lattice.
     * @param iMaxSteps int Maximum number of samples of the error function
     * by moving the control point in the direction of the negative
     * gradient.
     * @param fStepSize float Spacing between the samples of the error
     * function by moving the control point in the direction of the
     * negative gradient.
     */
    ...
    /**
     * Update the registered source image and the computed error as a
     * result of making changes to a single control point.  This is the
     * local control aspect of the B-Spline which means only those image
     * samples which are affected by the control point need to be updated.
     * @param iControlX int Identifies the control point in the 3D lattice.
     * @param iControlY int Identifies the control point in the 3D lattice.
     * @param iControlZ int Identifies the control point in the 3D lattice.
     */
    protected void updateControlPointSamples(int iControlX, int iControlY, int iControlZ) {
        updateSamples(
            m_kBSplineBasisX.getControlPointSampleIndexMin(iControlX),
    ...
    /**
     * Update the registered source image and the computed error but
     * only for the specified range of samples.
        ...
        for (int iZ = iMinZ; iZ <= iMaxZ; iZ++) {
            for (int iY = iMinY; iY <= iMaxY; iY++) {
                for (int iX = iMinX; iX <= iMaxX; iX++) {
                    ...
                    float fRegSrcY0Z0 =
                        m_kImageSrc.data[iX0Y0Z0] * fX0 +
                        m_kImageSrc.data[iX1Y0Z0] * fX1;
                    float fRegSrcY1Z0 =
                        m_kImageSrc.data[iX0Y1Z0] * fX0 +
                        m_kImageSrc.data[iX1Y1Z0] * fX1;
                    float fRegSrcY0Z1 =
                        m_kImageSrc.data[iX0Y0Z1] * fX0 +
                        m_kImageSrc.data[iX1Y0Z1] * fX1;
                    float fRegSrcY1Z1 =
                        m_kImageSrc.data[iX0Y1Z1] * fX0 +
                        m_kImageSrc.data[iX1Y1Z1] * fX1;
    ...
    /**
     * Computes the derivative of the error function at a control point
     * by approximating with finite differences where the control point
     * is moved small amounts in each orthgonal direction.
     * @param iControlX int Identifies the control point in the 3D lattice.
     * @param iControlY int Identifies the control point in the 3D lattice.
     * @param iControlZ int Identifies the control point in the 3D lattice.
     * @param kDeriv Vector3f Vector to be filled with the derivative
     * computed along each axis.
     */
    ...
    /**
     * Compute the maximum distance the control point can be moved in the
     * specified direction while keeping the control point relative to
     * its 26 neighboring control points such that "folding" does not occur.
     * @param iControlX int Identifies the control point in the 3D lattice.
     * @param iControlY int Identifies the control point in the 3D lattice.
     * @param iControlZ int Identifies the control point in the 3D lattice.
     * @param kRayDirection Vector3f Direction from the current control
     * point location for finding the distance to the bounding polyhedron.
     * @param fMaxDesiredDist float Maximum distance along the ray direction
     * that should be considered.  At most, the code would consider
     * moving the control point.
     * @return float Computed distance.
     */
    protected float getControlPointMaxMoveDist(
        int iControlX, int iControlY, int iControlZ, Vector3f kRayDirection,
        ...
        for (int iTriangle = 0; iTriangle < 48; iTriangle++) {
            ...
            m_kBSpline3D.getControlPoint(
                ms_aaaiPolyhedronTriangleControlPointOffset[0][0][iTriangle] +
                iControlX,
                ms_aaaiPolyhedronTriangleControlPointOffset[1][0][iTriangle] +
                iControlY,
                ms_aaaiPolyhedronTriangleControlPointOffset[2][0][iTriangle] +
                iControlZ,
                kP0);
            m_kBSpline3D.getControlPoint(
                ms_aaaiPolyhedronTriangleControlPointOffset[0][1][iTriangle] +
                iControlX,
                ms_aaaiPolyhedronTriangleControlPointOffset[1][1][iTriangle] +
                iControlY,
                ms_aaaiPolyhedronTriangleControlPointOffset[2][1][iTriangle] +
                iControlZ,
                kP1);
            m_kBSpline3D.getControlPoint(
                ms_aaaiPolyhedronTriangleControlPointOffset[0][2][iTriangle] +
                iControlX,
                ms_aaaiPolyhedronTriangleControlPointOffset[1][2][iTriangle] +
                iControlY,
                ms_aaaiPolyhedronTriangleControlPointOffset[2][2][iTriangle] +
                iControlZ,
                kP2);
    ...
    /**
     * Called by the constructor to add the relative control point offset
     * indices for triangle mesh representation of the bounding polyhedron
     * formed by the neighboring 26 control points.  This method is only
     * called for the 8 triangles on a "face" which result from holding
     * one of the axes at a fixed offset of -1 or +1.  Hence this method
     * would need to be called once for each of the six "faces".  All of
     * the triangles go into a single table where order does not matter.
     * @param iTriangleOffset int Offset into triangle array for current
     * "face".  Should be one the following values: {0,8,16,24,32,40}.
     * @param iAxisA int Index {0,1,2} for {X,Y,Z} axis to be assigned
     * to "face" relative A (X) axis.
     * @param iAxisB int Index {0,1,2} for {X,Y,Z} axis to be assigned
     * to "face" relative B (Y) axis.
     * @param iAxisC int Index {0,1,2} for {X,Y,Z} axis to be assigned
     * to "face" relative C (Z) axis.
     * @param iOffsetAxisC int Fixed control point index relative offset value
     * -1 or +1 to be assigned to "face" relative C (Z) axis.
     */
    private static void initPolyhedronTrimeshConnectivity(int iTriangleOffset,
        int iAxisA, int iAxisB, int iAxisC, int iOffsetAxisC) {
        ...
        aaiTriangleAxisA[0] =
            ms_aaaiPolyhedronTriangleControlPointOffset[iAxisA][0];
        aaiTriangleAxisB[0] =
            ms_aaaiPolyhedronTriangleControlPointOffset[iAxisB][0];
        aaiTriangleAxisC[0] =
            ms_aaaiPolyhedronTriangleControlPointOffset[iAxisC][0];
        if (iOffsetAxisC < 0) {
            // for negative face indexes, reverse the ordering of the
            // second and third points to maintain consistent
            // counterclockwise ordering
            aaiTriangleAxisA[1] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisA][2];
            aaiTriangleAxisB[1] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisB][2];
            aaiTriangleAxisC[1] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisC][2];

            aaiTriangleAxisA[2] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisA][1];
            aaiTriangleAxisB[2] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisB][1];
            aaiTriangleAxisC[2] =
                ms_aaaiPolyhedronTriangleControlPointOffset[iAxisC][1];
        }
        else {
    ...
    /**
     * Defines the relative offset in control point indices for the triangle
     * mesh associated with the bounding polyhedron formed by the 26
     * neighboring control points.  Triangle vertices are consistently
     * counterclockwise ordered.
     * First index: 0 for X axis, 1 for Y axis, 2 for Z axis
     * Second index: index of vertex within triangle
     * Third index: index for triangle (there are 48 -- 8 per face; see
     * initPolyhedronTrimeshConnectivity method for description)
     */
    protected static int[][][] ms_aaaiPolyhedronTriangleControlPointOffset = new int[3][3][48];

    /**
     * This is a static method called to initialize the class.
     * This method is used to initialize other static members.
     * Create the (relative) trimesh connectivity for the polyhedron of
     * neighboring control points.  There are six faces each having
     * 8 triangles.  The ordering of the triangles is important and
     * must be counterclockwise.  This table is used where we initially
     * know each point is inside its neighboring control point polyhedron
     * and we will not move the center control point any distance further
     * than the distance to the triangle in the direction of movement.
     */
    static {
