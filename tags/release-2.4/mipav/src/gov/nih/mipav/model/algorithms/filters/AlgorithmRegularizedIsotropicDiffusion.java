 ...
 *  and Pattern Recognition.  The book is published by Academic Press, ISBN
 *  0-12-379772-1
 *
 * <P> Here is a listing of the author's terminology used in the title.
 * <UL>
 * <LI><B>Regularized</B> the computed image is blurred with a Gaussian kernel
 * prior to taking finite derivatives
 * <LI> <B>Isotropic</B> the gradient and flux are parallel
 * <LI> <B>Nonlinear</B> the weights used for combining pixels change over the image,
 * space variant.
 * <LI><B>Diffusion</B> this model uses the heat equation
 * </UL>
 *
 *
...
public class AlgorithmRegularizedIsotropicDiffusion extends AlgorithmBase {
  ...
  */
 public AlgorithmRegularizedIsotropicDiffusion(ModelImage destImg, ModelImage srcImg,
                                                int numI, float stdDev, float contrast,
                                                boolean do25D) {
  ...
  *
  *   @param numImages  int    number of images to be blurred. If 2D image then nImage = 1,
  *                       if 3D image where each image is to processed independently
  *                       then nImages equals the number of slices in the volume.
  */
  ...
  /**
  *   Iterates the Regularized Isotropic Nonlinear Diffusion algorithm for 2D and 2.5D
  *   color images
  *   @param numImages  int    number of images to be blurred. If 2D image then nImage = 1,
  *                       if 3D image where each image is to processed independently
  *                       then nImages equals the number of slices in the volume.
  ...
  *   @param gradBuffer float []    The initialized buffer containing the magnitude of the
  *                     gradient of the Gaussian blurred input image
  */
  private void upDateImage(float[] resltBuffer, float[] srcBuffer, float[] gradBuffer) {
    ...
    for (int idx = 1; idx < length; idx++) {
      if(gradBuffer[idx] > maxGrad) { maxGrad = gradBuffer[idx]; }
    } // for (int idx = 1; ...)
    ...
    for (row = 0; row < yDim; row++) {
      ...
      for (col = 0, cIndex += col; col < xDim; cIndex++, col++) {
        ...
        resltBuffer[cIndex] = centerImgVal + 0.5f * timeStep * (
        topImgVal*(topDiffVal + centerDiffVal) +
        leftImgVal*(leftDiffVal + centerDiffVal) -
        centerImgVal*(leftDiffVal + rightDiffVal + bottomDiffVal + topDiffVal
                       + 4.0f*centerDiffVal) +
 ...
 private float getVal(float[] buffer, int col, int row, int slice) {
  ...
  /**
   *   Computes the gradient magnitude of the buffer passed in.  This version computes
   *   values for the image borders and corners
