 ...
 *
 *  The code here is a port of the MATLAB routine Two_D_Sensitized_FRET provided
 *  by Dr. Stephen Lockett.
 *
 *  The code here requires 3 images - image donor and acceptor dyes taken with a
 *  donor filter, image with donor and acceptor dyes taken with a FRET filter,
 *  and image with donor and acceptor dyes taken with an acceptor filter.
 *  Only 1 color will be used from a color image.
 *
 *  The code here requires the user to provide 4 bleed thru parameters.
 *  AlgorithmFRETBleedThrough will be used to obtain the 4 bleed thru parameters.
 *  The DFP into FRET bleed thru and the DFP into AFP bleed thru will be
 *  obtained from 1 run of AlgorithmFRETBleedThrough on a set of 3 images
 *  containing donor dye only.  The AFP into FRET bleed thru and the AFP into
 *  DFP bleed thru will be obtained from 1 run of AlgorithmFRETBleedThrough
 *  on a set of 3 images containing acceptor dye only.  Then
 *  AlgorithmFRETEfficiency is run with a set of 3 images containing both
 *  donor and acceptor dye, so obtaining the FRET efficiency requires a set
 *  of 9 images.
 *
 *  The code here assumes the presence of 2 or more VOI regions - a backgorund
 *  region and 1 or more active regions whose areas are used for the
 *  efficiency calculations.  Pixels with saturation values in any of the
 *  3 images are excluded from all calculations.
 *  The VOIs must all be placed in the source image and the source image must
 *  be the image with donor and acceptor dyes taken with a donor fluorescent
 *  peak filter.
 *  Radio buttons in the dialog box are used to select an active VOI
 *  or a blue background VOI.
 *  Either an ellipse VOI, rectangle VOI, levelset VOI, or
 *  polyline VOI will be selected from the top MIPAV toolbar.
 *  Hit the NEW_VOI button to create other
 *  active VOIs.  Just do not use blue as an active VOI color.
 *  The background region will have a smaller average intensity than the
 *  active region.
 *
 *  This program outputs the FRET efficiency and the adjusted donor and
 *  adjusted acceptor intensities for each active VOI region.
 *  Optionally, the user may create floating point images with the
 *  adjustedFRET/adjusted donor, adjustedFRET/adjusted acceptor or
 *  efficiency given at every pixel location.
 *
 *  Reference:
...
public class AlgorithmFRETEfficiency extends AlgorithmBase {
     ...
     */
    public AlgorithmFRETEfficiency( ModelImage donorImage, ModelImage FRETImage,
            ModelImage acceptorImage,
            boolean useRed, boolean useGreen, boolean useBlue,
            float DFPintoFRET, float AFPintoFRET, float DFPintoAFP,
            float AFPintoDFP, ModelImage fDivDImage,
    ...
    private void constructLog() {
        historyString = new String(
                "FRETEfficiency(" + useRed + ", " + useGreen + ", " +
                                      useBlue + ", " +
                                      DFPintoFRET + ", " +
                                      AFPintoFRET + ", " +
                                      DFPintoAFP + ", " +
    ...
    public void runAlgorithm() {
      ...
      if (useRed || useGreen || useBlue) {
        ...
        if (acceptorImage.isColorImage()) {
          try {
            ...
            MipavUtil.displayError("IOException " + e +
                                   " on acceptorImage.exportRGBData");
            setCompleted(false);
            disposeProgressBar();
            return;
          }
      ...
      for (i = 0; i < imageSize; i++) {
        if ((donorBuffer[i] == saturation) || (FRETBuffer[i] == saturation) ||
            (acceptorBuffer[i] == saturation)) {
          mask[i] = -1;
        }
      } // for (i = 0; i < imageSize; i++)
      ...
      Preferences.debug("Image with DFP filter\t" +
                        "Image with FRET filter\t" +
                        "Image with AFP filter\n");
      Preferences.debug(srcImage.getImageName() + "\t\t" +
                        FRETImage.getImageName() + "\t\t" +
                        acceptorImage.getImageName() + "\n\n");
      UI.setDataText("Image with DFP filter\t" +
                     "Image with FRET filter\t" +
                     "Image with AFP filter\n");
      UI.setDataText(srcImage.getImageName() + "\t\t" +
                     FRETImage.getImageName() + "\t\t" +
                     acceptorImage.getImageName() + "\n\n");

      Preferences.debug(
      "VOI \t\t FRET efficiency \tDonor intensity \tAcceptor intensity \tEfficiency * Donor\n");
      UI.setDataText(
      "VOI \t\t FRET efficiency \tDonor intensity \tAcceptor intensity \tEfficiency * Donor\n");
      totalActivePixels = 0;
      ...
      for (i = 0; i < nBoundingVOIs - 1; i++) {
        ...
        averageAdjustedDonor = averageAdjustedDonor +
                               activePixels[i] * adjustedDonor;
        adjustedAcceptor = (acceptor[i] - DFPintoAFP * donor[i])/denom;
        averageAdjustedAcceptor = averageAdjustedAcceptor +
                                  activePixels[i] * adjustedAcceptor;
        adjustedFRET = FRET[i] - DFPintoFRET * adjustedDonor
                        - AFPintoFRET * adjustedAcceptor;
        averageAdjustedFRET = averageAdjustedFRET +
                              activePixels[i] * adjustedFRET;
        efficiency = adjustedFRET/ adjustedDonor / adjustedAcceptor;
        Preferences.debug(activeNumber[i] + "\t\t" +
                          String.valueOf(nf.format(efficiency)) + "\t\t" +
      ...
      if (nBoundingVOIs > 2) {
        ...
        efficiency = averageAdjustedFRET/ averageAdjustedDonor/
                     averageAdjustedAcceptor;
        Preferences.debug("WeightedAverage\t" +
              String.valueOf(nf.format(efficiency)) + "\t\t" +
        ...
        UI.setDataText("WeightedAverage\t" +
              String.valueOf(nf.format(efficiency)) + "\t\t" +
              String.valueOf(nf.format(averageAdjustedDonor)) + "\t\t" +
              String.valueOf(nf.format(averageAdjustedAcceptor)) + "\t\t" +
              String.valueOf(nf.format(efficiency*averageAdjustedDonor)) + "\n\n");
      ...
      for (i = 0; i < imageSize; i++) {
              ...
              adjustedFRET = pixelFRET - DFPintoFRET * adjustedDonor
                             - AFPintoFRET * adjustedAcceptor;
              if (fDivDImage != null) {
    ...
    private void buildProgressBar() {
        try {
            if ( pBarVisible == true ) {
                progressBar = new ViewJProgressBar( srcImage.getImageName(), "Performing FRETEfficiency ...", 0, 100, true, this,
                        this );
