...
/**
 * A class for segmenting objects.  The algorithm is partially
 * based on the paper:
 * <pre>
 ...
 * </pre>
 * See the document BrainExtraction.pdf for a detailed description of the
 * algorithm as implemented in this class.  A few modifications to the
 * original algorithm were made.
 */
public class AlgorithmObjectExtractor extends AlgorithmBase {
     ...
     */
    public AlgorithmObjectExtractor (ModelImage srcImg, VOI voi, boolean justInit,
                                     boolean saveGVF, ModelTriangleMesh triMesh,
                                     float uVal[], float vVal[], float wVal[])
    {
        int i;
        onlyInit     = justInit;
        ...
        *   ellipsoid into a mesh that approximates the object surface.  The
        *   number of triangles in the mesh is 8*pow(4,S) where S is the
        *   subdivision parameter.  A reasonable choice is 5, leading to a mesh
        *   with 8192 triangle. */
        ...
        for (i = 0; i <= 2; i++) {
            if (axisOrientation[i] == ORI_L2R_TYPE ||
                axisOrientation[i] == ORI_P2A_TYPE ||
    ...
    private void calcGradMag() {
        ...
        try {
                energyImage = new ModelImage(ModelImage.FLOAT, image.getExtents(),
                                            image.getImageName() + "_gm",  image.getUserInterface());
                ...
                if (energyImage != null) energyImage.disposeLocal();
                energyImage = null;
                System.gc();
                setCompleted(false);
                return;
    ...
    */
    ...
    /**
     * Set the spacing along the vertex normal rays, as described in
     * BrainExtraction.pdf, that is part of the image term in the surface
     * evolution.  The default value is 1.0.
     *
     * @param fRayDelta  the new spacing along the vertex normal rays
     */
    final public void setRayDelta (float fRayDelta)
    {
        m_fRayDelta = fRayDelta;
    }

    /**
     * Get the spacing along the vertex normal rays, as described in
     * BrainExtraction.pdf, that is part of the image term in the surface
     * evolution.  The default value is 1.0.
     *
     * @return the current spacing
     */
    final public float getRayDelta ()
    {
        return m_fRayDelta;
    }

    /**
     * Set the stiffness of the mesh, as described in BrainExtraction.pdf,
     * that is part of the surface normal term in the surface evolution.  The
     * default value is 0.1.
     *
     * @param fStiffness the new stiffness
     */
    final public void setStiffness (float fStiffness)
    {
    ...
    /**
     * Indicates if surface evolution should be skipped. This is helpful
     * when determining if the initial ellipsoid is a good estimate.
     *
     * @param flag  whether the surface evolution should be skipped
     */
    final public void setJustIntial (boolean flag)
    {
        onlyInit = flag;
    }
     ...
     */
    final public float getStiffness ()
    {
        return m_fStiffness;
    }

    /**
     * Set the reduction factor for estimating the initial ellipsoid,
     * as described in BrainExtraction.pdf.  The default value is 0.6.
     *
     * @param fReduction the amount to reduce the axis of the ellipsoid.
     */
    final public void setReductionX (float fReduction)
    {
    ...
    /**
     * Set the reduction factor for estimating the initial ellipsoid,
     * as described in BrainExtraction.pdf.  The default value is 0.5.
     *
     * @param fReduction the amount to reduce the axis of the ellipsoid.
     */
    final public void setReductionY (float fReduction)
    {
     ...
     * @param fReduction the amount to reduce the axis of the ellipsoid.
     */
    final public void setReductionZ (float fReduction)
    {
     ...
     /**
     * Set the reduction factor for estimating the initial ellipsoid,
     * as described in BrainExtraction.pdf.  The default value is 0.75.
     *
     * @return the current reduction factor
     */
    final public float getReductionX ()
    {
     ...
     * @return the current reduction factor
     */
    final public float getReductionY ()
    {
     ...
     * @return the current reduction factor
     */
    final public float getReductionZ ()
    {
     ...
     * designed so that the voxel surface has no holes, thereby allowing it
     * to be flood-filled.  But just in case numerical round-off errors cause
     * a few holes, this parameter is exposed for public use.  The default
     * value is 0 (no dilation).  If the value is D > 0, the dilation mask
     * is a cube of size (2*D+1)x(2*D+1)x(2*D+1).
     *
     * @param iDMax the new dilation size
     */
    final public void setDilationSize (int iDMax)
    {
        m_iDMax = iDMax;
    }
     ...
     */
    final public int getDilationSize ()
    {
        return m_iDMax;
    }

    /**
     * The heart of the segmentation.  This function is responsible for the
     * evolution of the triangle mesh that approximates the object surface.
     * The update has a tangential component, a surface normal component,
     * and a vertex normal component for each vertex in the mesh.  The first
     * two components control the geometry of the mesh.  The last component
     * is based on the MRI data itself.  See BrainExtraction.pdf for a
     * detailed description of the update terms.
     */
    public void updateMesh ()
    {
        ...
        for (int i = 0; i < m_iVQuantity; i++)
        {
            ...
            if (kVertex.x < 0 ) kVertex.x = 0;
            if (kVertex.y < 0 ) kVertex.y = 0;
            if (kVertex.z < 0 ) kVertex.z = 0;

            if (kVertex.x > xDim-1 ) kVertex.x = xDim-1;
            if (kVertex.y > yDim-1 ) kVertex.y = yDim-1;
            if (kVertex.z > zDim-1 ) kVertex.z = zDim-1;
        }
    }

    /**
     * The segmentation function.  Various parameters may
     * be modified, if necessary, before the call.
     */
    ...
    /**
     * Internal support for 'void save (String)' and
     * 'void save (String, ModelTriangleMesh[])'.  ModelTriangleMesh uses this
     * function to write vertices, normals, and connectivity indices to the
     * file.  ModelClodMesh overrides this to additionally write collapse
     * records to the file
     *
     * @param flip if the y axis should be flipped - true for extract, false for
    ...
    protected void saveMesh(boolean flip) throws IOException
    {
        ...
        String kName = image.getUserInterface().getDefaultDirectory()
        + image.getImageName() + "_object.sur";
        ...
        if (image.getFileInfo()[0].getTransformID() ==
          ...
          for (i = 0; i < m_iVQuantity; i++) {
            m_akVertex[i].x = m_akVertex[i].x * m_fXDelta * direction[0] +
                startLocation[0];
            m_akVertex[i].y = m_akVertex[i].y * m_fYDelta * direction[1] +
                startLocation[1];
            m_akVertex[i].z = m_akVertex[i].z * m_fZDelta * direction[2] +
                startLocation[2];
          }
        } // else
        ...
        ModelTriangleMesh.save(kName, newMesh,flip, direction, startLocation,
                               box, inverseDicomArray);
    }


    /**
     * Get the 3D image that represents the extracted object.  The image is
     * ternary and has the same dimensions as the input MRI.  A voxel value
     * of 0 indicates background.  A voxel value of 1 indicates object
     * surface.  A voxel value of 2 indicates a voxel inside the object
     * surface.
     *
     * @return the image that represents the extracted object
     */
    final public byte[] getObjectMask ()
    {
        return m_aiMask;
    }
    ...
    /**
     * Approximate the brain surface by an ellipsoid.  The approximation is
     * based on locating all voxels of intensity larger than a brightness
     * threshold and that are part of the upper-half of the head.  The idea
     * is that the scalp voxels in the upper-half form lie approximately on
     * an ellipsoidal surface. <br><br>
     *
     * NOTE.  The assumption is that the traversal from bottom to top of head
     * is in the y-direction of the 3D image.  It does not matter if the
     * top of the head has y-values smaller/larger than those for the bottom
     * of the head.  If this assumption is not met, the image should be
     * permuted OR this code must be modified to attempt to recognize the
    ...
    protected void estimateEllipsoid ()
    {
        ...
        if ((m_iYBound-1)*m_fYDelta > fBMax ) fBMax = (m_iYBound-1)*m_fYDelta;
        if ((m_iZBound-1)*m_fZDelta > fBMax ) fBMax = (m_iZBound-1)*m_fZDelta;
    ...
    /**
     * Tessellate a unit sphere centered at the origin.  Start with an
     * octahedron and subdivide.  The final mesh is then affinely mapped to
     * the initial ellipsoid produced by estimateEllipsoid().  The subdivision
     * scheme is described in BrainExtraction.pdf.
     */
    protected void generateEllipsoidMesh ()
    {
        ...
        m_aiConnect[ 0] = 4;  m_aiConnect[ 1] = 0;  m_aiConnect[ 2] = 2;
        m_aiConnect[ 3] = 4;  m_aiConnect[ 4] = 2;  m_aiConnect[ 5] = 1;
        m_aiConnect[ 6] = 4;  m_aiConnect[ 7] = 1;  m_aiConnect[ 8] = 3;
        m_aiConnect[ 9] = 4;  m_aiConnect[10] = 3;  m_aiConnect[11] = 0;
        m_aiConnect[12] = 5;  m_aiConnect[13] = 2;  m_aiConnect[14] = 0;
        m_aiConnect[15] = 5;  m_aiConnect[16] = 1;  m_aiConnect[17] = 2;
        m_aiConnect[18] = 5;  m_aiConnect[19] = 3;  m_aiConnect[20] = 1;
        m_aiConnect[21] = 5;  m_aiConnect[22] = 0;  m_aiConnect[23] = 3;
        ...
        for (iStep = 1; iStep <= iSubdivisions; iStep++)
        {
            ...
            while ( kEIter.hasNext() )
            {
                ...
                kPMid.add(kP0,kP1);
                float fInvLen = 1.0f/(float)Math.sqrt(kPMid.x*kPMid.x +
                    kPMid.y*kPMid.y + kPMid.z*kPMid.z);
                kPMid.scale(fInvLen);
        ...
        float resXFactor = m_fXDelta/(float)Math.sqrt(
              m_kRotate.m00*m_kRotate.m00*m_fXDelta*m_fXDelta
            + m_kRotate.m01*m_kRotate.m01*m_fYDelta*m_fYDelta
            + m_kRotate.m02*m_kRotate.m02*m_fZDelta*m_fZDelta);
        float resYFactor = m_fYDelta/(float)Math.sqrt(
              m_kRotate.m10*m_kRotate.m10*m_fXDelta*m_fXDelta
            + m_kRotate.m11*m_kRotate.m11*m_fYDelta*m_fYDelta
            + m_kRotate.m12*m_kRotate.m12*m_fZDelta*m_fZDelta);
        float resZFactor = m_fZDelta/(float)Math.sqrt(
              m_kRotate.m20*m_kRotate.m20*m_fXDelta*m_fXDelta
    ...
    /**
     * Compute the vertex normals of the triangle mesh.  Each vertex normal
     * is the unitized average of the non-unit triangle normals for those
     ...
     * VMean[i], the average of the immediate neighbors of V[i].  Define
     * S[i] = VMean[i] - V[i].  The function also computes a surface normal
     * SNormal[i], the component of S[i] in the vertex normal direction.
     * STangent[i] = S[i] - SNormal[i] is computed as an approximation to
     * a tangent to the surface.  Finally, Curvature[i] is an approximation
     * of the surface curvature at V[i].
     */
    ...
    /**
     * Compute the coefficient of the surface normal for the update of the
     * mesh vertex V[i] in the SNormal[i] direction.  See BrainExtraction.pdf
     * for a description of the update.
     *
    ...
    */
    private void calcGVF3D() {
        ...
        progressBar = new ViewJProgressBar(image.getImageName(), "Calculating 3D GVF ...",
            ...
            for (y = 0; y < yDim && (!threadStopped); y++) {
                ...
                for (x = 0; x < xDim && (!threadStopped); x++) {
                    ...
                    if (sigmas[2] != 0.0) {
                        ...
                        if (z == 0) {
                            iz = imgBuffer[x + y*xDim + sliceSize]
                                 - imgBuffer[x + y*xDim];
                        }
                        else if (z == (zDim - 1)) {
        ...
        for (z = 0; z < zDim; z++) {
            for (y = 0; y < yDim; y++) {
                for (x = 0; x < xDim; x++) {
                    expGvfBuffer[(x+1) + (y+1)*(xDim+2) + (z+1)*expSliceSize] =
                    gvfBuffer[x + y*xDim + z*sliceSize];
                }
            }
        }
        ...
        expGvfBuffer[(xDim+2)*(yDim+2)-1] =
                        gvfBuffer[(yDim-2)*xDim + xDim - 2 + sliceSize];
        expGvfBuffer[(xDim+2)*(yDim+2)*(zDim+1)] =
                   gvfBuffer[1 + xDim + sliceSize*(zDim-2)];
        expGvfBuffer[(xDim+2)*(yDim+2)*(zDim+1) + xDim+1] =
                   gvfBuffer[(xDim-2) + xDim + sliceSize*(zDim-2)];
        expGvfBuffer[(xDim+2)*(yDim+2)*(zDim+1)+(yDim+1)*(xDim+2)] =
                   gvfBuffer[(yDim-2)*xDim + 1 + sliceSize*(zDim-2)];
        expGvfBuffer[(xDim+2)*(yDim+2)*(zDim+1)+(xDim+2)*(yDim+2)-1] =
                        gvfBuffer[(yDim-2)*xDim + xDim - 2 + sliceSize*(zDim-2)];

        // Create a mirror at the corner line segments from x = 1 to x = xDim slices
        for (x = 0; x < xDim; x++) {
            expGvfBuffer[x+1] = gvfBuffer[x + xDim + sliceSize];
            expGvfBuffer[x+1 + (yDim+1)*(xDim+2)] =
                gvfBuffer[x + (yDim-2)*xDim + sliceSize];
            expGvfBuffer[x+1 + (zDim+1)*expSliceSize] =
                gvfBuffer[x + xDim + (zDim-2)*sliceSize];
            expGvfBuffer[x+1 + (yDim+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                gvfBuffer[x + (yDim-2)*xDim + (zDim-2)*sliceSize];
        }
        ...
        for (y = 0; y < yDim; y++) {
            ...
            expGvfBuffer[xDim+1 + (y+1)*(xDim+2)] =
                gvfBuffer[xDim-2 + y*xDim + sliceSize];
            expGvfBuffer[(y+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                gvfBuffer[1 + y*xDim + (zDim-2)*sliceSize];
            expGvfBuffer[xDim+1 + (y+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                gvfBuffer[xDim-2 + y*xDim + (zDim-2)*sliceSize];
        }
        ...
        for (z = 0; z < zDim; z++) {
            ...
            expGvfBuffer[xDim+1 + (z+1)*expSliceSize] =
                     gvfBuffer[(xDim-2) + xDim + z*sliceSize];
            expGvfBuffer[(yDim+1)*(xDim+2) + (z+1)*expSliceSize] =
                     gvfBuffer[(yDim-2)*xDim + 1 + z*sliceSize];
            expGvfBuffer[(xDim+2)*(yDim+2)-1 + (z+1)*expSliceSize] =
                     gvfBuffer[(yDim-2)*xDim + xDim - 2 + z*sliceSize];
        }

        // Mirror left and right x boundaries
        for (z = 0; z < zDim; z++) {
            for (y = 0; y < yDim; y++) {
                expGvfBuffer[(y+1)*(xDim+2)+(z+1)*expSliceSize] =
                         gvfBuffer[z*sliceSize + y*xDim + 1];
                expGvfBuffer[(y+1)*(xDim+2) + xDim + 1 + (z+1)*expSliceSize] =
                         gvfBuffer[z*sliceSize + y*xDim + xDim - 2];
            }
        }
        ...
        for (z = 0; z < zDim; z++) {
            for (x = 0; x < xDim; x++) {
                expGvfBuffer[x+1+(z+1)*expSliceSize] =
                         gvfBuffer[z*sliceSize + xDim + x];
                expGvfBuffer[(yDim+1)*(xDim+2) + x + 1 + (z+1)*expSliceSize] =
                         gvfBuffer[z*sliceSize + (yDim-2)*xDim + x];
            }
        }
        ...
        for (y = 0; y < yDim; y++) {
            for (x = 0; x < xDim; x++) {
                expGvfBuffer[x+1 + (y+1)*(xDim+2)] =
                gvfBuffer[sliceSize + y*xDim + x];
                expGvfBuffer[x+1 + (y+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                         gvfBuffer[(zDim-2)*sliceSize + y*xDim + x];
            }
        }
        ...
        for (z = 0; z < (zDim+2); z++) {
            for (y = 0; y < (yDim+2); y++) {
                for (x = 0; x < (xDim+2); x++) {
                    if (x == 0) {
                        fx[y*(xDim+2)+z*expSliceSize] =
                        expGvfBuffer[1 + y*(xDim+2)+z*expSliceSize]
                        - expGvfBuffer[y*(xDim+2)+z*expSliceSize];
                    }
                    else if (x == (xDim+1)) {
                        fx[xDim+1 + y*(xDim+2)+z*expSliceSize] =
                        expGvfBuffer[xDim+1 + y*(xDim+2)+z*expSliceSize]
                        - expGvfBuffer[xDim + y*(xDim+2)+z*expSliceSize];
                    }
                    else {
                        fx[x + y*(xDim+2)+z*expSliceSize] =
                        (expGvfBuffer[(x+1) + y*(xDim+2)+z*expSliceSize]
                        - expGvfBuffer[(x-1) + y*(xDim+2)+z*expSliceSize])/2.0f;
                    }
                    if (y == 0) {
                        fy[x+z*expSliceSize] =
                        expGvfBuffer[x + (xDim+2)+z*expSliceSize]
                        - expGvfBuffer[x+z*expSliceSize];
                    }
                    else if (y == (yDim + 1)) {
                        fy[x + (yDim+1)*(xDim+2)+z*expSliceSize] =
                        expGvfBuffer[x + (yDim+1)*(xDim+2)+z*expSliceSize]
                        - expGvfBuffer[x + yDim*(xDim+2)+z*expSliceSize];
                    }
                    else {
                        fy[x + y*(xDim+2)+z*expSliceSize] =
                        (expGvfBuffer[x + (y+1)*(xDim+2)+z*expSliceSize]
                        - expGvfBuffer[x + (y-1)*(xDim+2)+z*expSliceSize])/2.0f;
                    }
                    if (z == 0) {
                        fz[x + y*(xDim+2)] =
                        expGvfBuffer[x + y*(xDim+2) + expSliceSize]
                        - expGvfBuffer[x + y*(xDim+2)];
                    }
                    else if (z == (zDim + 1)) {
                        fz[x + y*(xDim+2) + (zDim+1)*expSliceSize] =
                        expGvfBuffer[x + y*(xDim+2) + (zDim+1)*expSliceSize]
                        - expGvfBuffer[x + y*(xDim+2) + zDim*expSliceSize];
                    }
                    else {
                        fz[x + y*(xDim+2) + z*expSliceSize] =
                        (expGvfBuffer[x + y*(xDim+2) + (z+1)*expSliceSize]
                        - expGvfBuffer[x + y*(xDim+2) + (z-1)*expSliceSize])/2.0f;
                    }
                } // for (x = 0; x < (xDim+2); x++)
            } // for (y = 0; y < (yDim+2); y++)
        } // for (z = 0; z < (zDim+2); z++)
        ...
        for (iteration = 0; (iteration < gvfIterations) && (!threadStopped); iteration++) {
            ...
            uVal[(xDim+2)*(yDim+2)-1] =
                uVal[(yDim-1)*(xDim+2) + xDim - 1 + 2*expSliceSize];
            uVal[(zDim+1)*expSliceSize] = uVal[2 + 2*(xDim+2)+ (zDim-1)*expSliceSize];
            uVal[(zDim+1)*expSliceSize + xDim+1] =
                uVal[(xDim-1) + 2*(xDim+2) + (zDim-1)*expSliceSize];
            uVal[(zDim+1)*expSliceSize + (yDim+1)*(xDim+2)] =
                uVal[(yDim-1)*(xDim+2) + 2 + (zDim-1)*expSliceSize];
            uVal[(zDim+1)*expSliceSize + (xDim+2)*(yDim+2)-1] =
                uVal[(yDim-1)*(xDim+2) + xDim - 1 + (zDim-1)*expSliceSize];

            // Create a mirror at the corner line segments from x = 1 to x = xDim
            for (x = 1; x <= xDim; x++) {
                uVal[x] = uVal[x + 2*(xDim+2) + 2*expSliceSize];
                uVal[x + (yDim+1)*(xDim+2)] =
                    uVal[x + (yDim-1)*(xDim+2) + 2*expSliceSize];
                uVal[x + (zDim+1)*expSliceSize] =
                    uVal[x + 2*(xDim+2) + (zDim-1)*expSliceSize];
                uVal[x + (yDim+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                    uVal[x + (yDim-1)*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (y = 1; y <= yDim; y++) {
                ...
                uVal[xDim+1 + y*(xDim+2)] =
                    uVal[xDim-2 + y*(xDim+2) + 2*expSliceSize];
                uVal[y*(xDim+2) + (zDim+1)*expSliceSize] =
                    uVal[2 + y*(xDim+2) + (zDim-1)*expSliceSize];
                uVal[xDim+1 + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    uVal[xDim-1 + y*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (z = 1; z <= zDim; z++) {
                ...
                uVal[z*expSliceSize + xDim+1] =
                    uVal[(xDim-1) + 2*(xDim+2) + z*expSliceSize];
                uVal[z*expSliceSize + (yDim+1)*(xDim+2)] =
                    uVal[(yDim-1)*(xDim+2) + 2 + z*expSliceSize];
                uVal[z*expSliceSize + (xDim+2)*(yDim+2)-1] =
                    uVal[(yDim-1)*(xDim+2) + xDim - 1 + z*expSliceSize];
            }

            // Mirror left and right x boundaries for z = 1 to z = zDim
            for (z = 1; z < (zDim+1); z++) {
                for (y = 1; y < (yDim+1); y++) {
                    uVal[z*expSliceSize + y*(xDim+2)] =
                    uVal[z*expSliceSize + y*(xDim+2) + 2];
                    uVal[z*expSliceSize + y*(xDim+2) + xDim + 1] =
                    uVal[z*expSliceSize + y*(xDim+2) + xDim - 1];
                }
            }
            ...
            for (z = 1; z < (zDim+1); z++) {
                for (x = 1; x < (xDim+1); x++) {
                    uVal[z*expSliceSize + x] =
                    uVal[z*expSliceSize + 2*(xDim+2) + x];
                    uVal[z*expSliceSize + (yDim+1)*(xDim+2) + x] =
                    uVal[z*expSliceSize + (yDim-1)*(xDim+2) + x];
                }
            }
            ...
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    ...
                    uVal[x + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    uVal[x + y*(xDim+2) + (zDim-1)*expSliceSize];
                }
            }
            ...
            vVal[(xDim+2)*(yDim+2)-1] =
                vVal[(yDim-1)*(xDim+2) + xDim - 1 + 2*expSliceSize];
            vVal[(zDim+1)*expSliceSize] = vVal[2 + 2*(xDim+2)+ (zDim-1)*expSliceSize];
            vVal[(zDim+1)*expSliceSize + xDim+1] =
                vVal[(xDim-1) + 2*(xDim+2) + (zDim-1)*expSliceSize];
            vVal[(zDim+1)*expSliceSize + (yDim+1)*(xDim+2)] =
                vVal[(yDim-1)*(xDim+2) + 2 + (zDim-1)*expSliceSize];
            vVal[(zDim+1)*expSliceSize + (xDim+2)*(yDim+2)-1] =
                vVal[(yDim-1)*(xDim+2) + xDim - 1 + (zDim-1)*expSliceSize];

            // Create a mirror at the corner line segments from x = 1 to x = xDim
            for (x = 1; x <= xDim; x++) {
                vVal[x] = vVal[x + 2*(xDim+2) + 2*expSliceSize];
                vVal[x + (yDim+1)*(xDim+2)] =
                    vVal[x + (yDim-1)*(xDim+2) + 2*expSliceSize];
                vVal[x + (zDim+1)*expSliceSize] =
                    vVal[x + 2*(xDim+2) + (zDim-1)*expSliceSize];
                vVal[x + (yDim+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                    vVal[x + (yDim-1)*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (y = 1; y <= yDim; y++) {
                ...
                vVal[xDim+1 + y*(xDim+2)] =
                    vVal[xDim-2 + y*(xDim+2) + 2*expSliceSize];
                vVal[y*(xDim+2) + (zDim+1)*expSliceSize] =
                    vVal[2 + y*(xDim+2) + (zDim-1)*expSliceSize];
                vVal[xDim+1 + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    vVal[xDim-1 + y*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (z = 1; z <= zDim; z++) {
                ...
                vVal[z*expSliceSize + xDim+1] =
                    vVal[(xDim-1) + 2*(xDim+2) + z*expSliceSize];
                vVal[z*expSliceSize + (yDim+1)*(xDim+2)] =
                    vVal[(yDim-1)*(xDim+2) + 2 + z*expSliceSize];
                vVal[z*expSliceSize + (xDim+2)*(yDim+2)-1] =
                    vVal[(yDim-1)*(xDim+2) + xDim - 1 + z*expSliceSize];
            }

            // Mirror left and right x boundaries
            for (z = 1; z < (zDim+1); z++) {
                for (y = 1; y < (yDim+1); y++) {
                    vVal[z*expSliceSize + y*(xDim+2)] =
                    vVal[z*expSliceSize + y*(xDim+2) + 2];
                    vVal[z*expSliceSize + y*(xDim+2) + xDim + 1] =
                    vVal[z*expSliceSize + y*(xDim+2) + xDim - 1];
                }
            }
            ...
            for (z = 1; z < (zDim+1); z++) {
                for (x = 1; x < (xDim+1); x++) {
                    vVal[z*expSliceSize + x] =
                    vVal[z*expSliceSize + 2*(xDim+2) + x];
                    vVal[z*expSliceSize + (yDim+1)*(xDim+2) + x] =
                    vVal[z*expSliceSize + (yDim-1)*(xDim+2) + x];
                }
            }
            ...
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    ...
                    vVal[x + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    vVal[x + y*(xDim+2) + (zDim-1)*expSliceSize];
                }
            }
            ...
            wVal[(xDim+2)*(yDim+2)-1] =
                wVal[(yDim-1)*(xDim+2) + xDim - 1 + 2*expSliceSize];
            wVal[(zDim+1)*expSliceSize] = wVal[2 + 2*(xDim+2)+ (zDim-1)*expSliceSize];
            wVal[(zDim+1)*expSliceSize + xDim+1] =
                wVal[(xDim-1) + 2*(xDim+2) + (zDim-1)*expSliceSize];
            wVal[(zDim+1)*expSliceSize + (yDim+1)*(xDim+2)] =
                wVal[(yDim-1)*(xDim+2) + 2 + (zDim-1)*expSliceSize];
            wVal[(zDim+1)*expSliceSize + (xDim+2)*(yDim+2)-1] =
                wVal[(yDim-1)*(xDim+2) + xDim - 1 + (zDim-1)*expSliceSize];

            // Create a mirror at the corner line segments from x = 1 to x = xDim
            for (x = 1; x <= xDim; x++) {
                wVal[x] = wVal[x + 2*(xDim+2) + 2*expSliceSize];
                wVal[x + (yDim+1)*(xDim+2)] =
                    wVal[x + (yDim-1)*(xDim+2) + 2*expSliceSize];
                wVal[x + (zDim+1)*expSliceSize] =
                    wVal[x + 2*(xDim+2) + (zDim-1)*expSliceSize];
                wVal[x + (yDim+1)*(xDim+2) + (zDim+1)*expSliceSize] =
                    wVal[x + (yDim-1)*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (y = 1; y <= yDim; y++) {
                ...
                wVal[xDim+1 + y*(xDim+2)] =
                    wVal[xDim-2 + y*(xDim+2) + 2*expSliceSize];
                wVal[y*(xDim+2) + (zDim+1)*expSliceSize] =
                    wVal[2 + y*(xDim+2) + (zDim-1)*expSliceSize];
                wVal[xDim+1 + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    wVal[xDim-1 + y*(xDim+2) + (zDim-1)*expSliceSize];
            }
            ...
            for (z = 1; z <= zDim; z++) {
                ...
                wVal[z*expSliceSize + xDim+1] =
                    wVal[(xDim-1) + 2*(xDim+2) + z*expSliceSize];
                wVal[z*expSliceSize + (yDim+1)*(xDim+2)] =
                    wVal[(yDim-1)*(xDim+2) + 2 + z*expSliceSize];
                wVal[z*expSliceSize + (xDim+2)*(yDim+2)-1] =
                    wVal[(yDim-1)*(xDim+2) + xDim - 1 + z*expSliceSize];
            }

            // Mirror left and right x boundaries
            for (z = 1; z < (zDim+1); z++) {
                for (y = 1; y < (yDim+1); y++) {
                    wVal[z*expSliceSize + y*(xDim+2)] =
                    wVal[z*expSliceSize + y*(xDim+2) + 2];
                    wVal[z*expSliceSize + y*(xDim+2) + xDim + 1] =
                    wVal[z*expSliceSize + y*(xDim+2) + xDim - 1];
                }
            }
            ...
            for (z = 1; z < (zDim+1); z++) {
                for (x = 1; x < (xDim+1); x++) {
                    wVal[z*expSliceSize + x] =
                    wVal[z*expSliceSize + 2*(xDim+2) + x];
                    wVal[z*expSliceSize + (yDim+1)*(xDim+2) + x] =
                    wVal[z*expSliceSize + (yDim-1)*(xDim+2) + x];
                }
            }
            ...
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    ...
                    wVal[x + y*(xDim+2) + (zDim+1)*expSliceSize] =
                    wVal[x + y*(xDim+2) + (zDim-1)*expSliceSize];
                }
            }
        ...
        for (z = 1; z < (zDim+1); z++) {
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    gvfBuffer[(x-1) + (y-1)*xDim + (z-1)*sliceSize] =
                    uVal[x + y*(xDim+2) + z*expSliceSize];
                }
            }
        }
        ...
        for (z = 1; z < (zDim+1); z++) {
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    gvfBuffer[(x-1) + (y-1)*xDim + (z-1)*sliceSize] =
                    vVal[x + y*(xDim+2) + z*expSliceSize];
                }
            }
        }
        ...
        for (z = 1; z < (zDim+1); z++) {
            for (y = 1; y < (yDim+1); y++) {
                for (x = 1; x < (xDim+1); x++) {
                    gvfBuffer[(x-1) + (y-1)*xDim + (z-1)*sliceSize] =
                    wVal[x + y*(xDim+2) + z*expSliceSize];
                }
            }
        }
        ...
        if (saveGVF) {
            ModelImage gvfImage = new ModelImage(ModelImage.FLOAT,image.getExtents(),
                                  image.getImageName() + "_uvf",
                                  image.getUserInterface());
            try {
                gvfImage.importData(0,uVal,true);
            }
            catch (IOException error) {
                if (gvfImage != null) gvfImage.disposeLocal();
                gvfImage = null;
            ...
            try {
                gvfImage.saveImage(image.getFileInfo(0).getFileDirectory(),
                                   image.getImageName() + "_uvf",FileBase.XML, true);
            }
            catch (OutOfMemoryError error) {
                if (gvfImage != null) gvfImage.disposeLocal();
                gvfImage = null;
            ...
            try {
                gvfImage.saveImage(image.getFileInfo(0).getFileDirectory(),
                                   image.getImageName() + "_vvf",FileBase.XML, true);
            }
            catch (OutOfMemoryError error) {
                if (gvfImage != null) gvfImage.disposeLocal();
                gvfImage = null;
            ...
            try {
                gvfImage.saveImage(image.getFileInfo(0).getFileDirectory(),
                                   image.getImageName() + "_wvf",FileBase.XML, true);
            }
            catch (OutOfMemoryError error) {
                if (gvfImage != null) gvfImage.disposeLocal();
                gvfImage = null;
    ...
    /**
     * Compute the coefficient of the vertex normal for the update of the
     * mesh vertex V[i] in the VNormal[i] direction.  See BrainExtraction.pdf
     * for a description of the update.
     *
     * @param i  the index of the vertex to update
    ...
    protected void update3 (int i)
    {
        ...
        if      ( x < 0 )          x = 0;
        else if ( x >= m_iXBound-1 ) x = m_iXBound - 1;

        if      ( y < 0 )          y = 0;
        else if ( y >= m_iYBound-1 ) y = m_iYBound - 1;

        if      ( z < 0 )          z = 0;
        else if ( z>= m_iZBound-1 ) z = m_iZBound - 1;
        // nearest neighbor interpolation
        /*iX = (int)(kVertex.x + 0.5f);
        iY = (int)(kVertex.y + 0.5f);
        iZ = (int)(kVertex.z + 0.5f);
        if      ( iX < 0 )          iX = 0;
        else if ( iX >= m_iXBound ) iX = m_iXBound - 1;

        if      ( iY < 0 )          iY = 0;
        else if ( iY >= m_iYBound ) iY = m_iYBound - 1;

        if      ( iZ < 0 )          iZ = 0;
        else if ( iZ >= m_iZBound ) iZ = m_iZBound - 1;
        float oldGVF = gvfBuffer[iX + m_iXBound*(iY+m_iYBound*iZ)];*/
        ...
        kDiff.scaleAdd(-m_fRayDelta, kNormal, kVertex);
        ...
        float newGVF = gvfBuffer[iX + m_iXBound*(iY+m_iYBound*iZ)];
        x = kDiff.x;
        y = kDiff.y;
        z = kDiff.z;
        if      ( x < 0 )          x = 0;
        ...
        float newGVF = getTriLinear(x,y,z,gvfBuffer);
        float fUpdate3;
        if (newGVF >= oldGVF ) {
            fUpdate3 = 0;
            //fUpdate3 = 0.5f * c3Factor * m_fMeanEdgeLength;
        }
        else{
            fUpdate3 = c3Factor;// * m_fMeanEdgeLength;
        }
    ...
    /**
     * A convenience function for mapping the 3D voxel position (iX,iY,iZ)
     * to a 1D array index.  The images are stored as 1D arrays, so this
     ...
     */
    final protected int getIndex (int iX, int iY, int iZ)
    {
        return iX + m_iXBound*(iY + m_iYBound*iZ);
    }

    /**
     * Compute the point of intersection between a line (0,iY,iZ)+t(1,0,0)
     * and the triangle defined by the three input points.  All calculations
     * are in voxel coordinates and the x-value of the intersection point is
     ...
     */
    protected int getIntersectX (Point3f kV0, Point3f kV1, Point3f kV2,
        int iY, int iZ)
    {
    ...
    /**
     * Compute the point of intersection between a line (iX,0,iZ)+t(0,1,0)
     * and the triangle defined by the three input points.  All calculations
     * are in voxel coordinates and the y-value of the intersection point is
     * truncated to an integer.
     *
     * @param kV0 a 3D vertex of the triangle
     * @param kV1 a 3D vertex of the triangle
     ...
     */
    protected int getIntersectY (Point3f kV0, Point3f kV1, Point3f kV2,
        int iX, int iZ)
    {
        // Compute the intersection, if any, by calculating barycentric
        // coordinates of the intersection of the line with the plane of
    ...
    /**
     * Compute the point of intersection between a line (iX,iY,0)+t(0,0,1)
     * and the triangle defined by the three input points.  All calculations
     * are in voxel coordinates and the z-value of the intersection point is
     * truncated to an integer.
     *
     * @param kV0 a 3D vertex of the triangle
     * @param kV1 a 3D vertex of the triangle
     ...
     */
    protected int getIntersectZ (Point3f kV0, Point3f kV1, Point3f kV2,
        int iX, int iY)
    {
        // Compute the intersection, if any, by calculating barycentric
        // coordinates of the intersection of the line with the plane of
    ...
    /**
     * Identify all voxels that are inside or on the mesh that represents
     * the brain surface.  The surface voxels are constructed by rasterizing
     * the triangles of the mesh in 3D.  The centroid of these voxels is
     * used as a seed point for a flood fill of the region enclosed by the
     * surface.
     */
            ...
            for (iY = iYMin; iY <= iYMax; iY++)
            {
                for (iZ = iZMin; iZ <= iZMax; iZ++)
                {
                    ...
                    if ( iX != -1 ) {
                        ...
                        if (ptr >= 0 && ptr < end) m_aiMask[ptr] = 1;
     ...
     * The flood fill is nonrecursive to avoid overflowing the program
     * stack.
     *
    ...
    /**
     * A representation of an edge for the vertex-edge-triangle table.  This
     * class stores the pair of vertex indices for the end points of the edge.
     * The edges <V0,V1> and <V1,V0> are considered to be identical.  To
     * simplify comparisons, the class stores the ordered indices.  The class
     * extends Object to obtain support for hashing into a map of edges.
     */
    protected class Edge extends Object
    {
         ...
         * @return true iff the edges are identical.  Because the class
         *   stores ordered indices, it is not necessary to use the more
         *   expensive test (i0 == other.i0 && i1 == other.i1) ||
    ...
    // The 3D MRI image stored as a 1D array.  The mapping from (x,y,z) to
    // 1D is: index = x + xbound*(y + ybound*z).
    ...
    protected Point3f m_akVertex[/* m_iVQuantity */];
    protected int m_aiConnect[/* 3*m_iTQuantity */];
    protected UnorderedSetInt m_akAdjacent[/* m_iVQuantity */];
    protected HashMap m_kEMap;  // map<Edge,int>
    protected Point3f m_akVMean[/* m_iVQuantity */];
    protected Vector3f m_akVNormal[/* m_iVQuantity */];
    protected Vector3f m_akSNormal[/* m_iVQuantity */];
    protected Vector3f m_akSTangent[/* m_iVQuantity */];
    protected float m_afCurvature[/* m_iVQuantity */];
    ...
    protected byte m_aiMask[/* m_iVQuantity */];
    protected int m_iDMax;  // dilation size


    /**
    * An unordered set of 'int' stored in an array.  The class is used to store
    * adjacency information for the triangles in the mesh representing the
    * object surface.  The reason for using an array is to minimize reallocations
    * during dynamic changes to a mesh.  When an item is deleted from the set,
    * the last element in the array is moved into that location.  The sets for
    * which this class is used are typically small, so the costs for searching
    * the unordered items are not a factor.
    *
    * The class has a static value DEFAULT_GROW that is used to increase the
    * number of elements when a reallocation must occur.  The new storage size
    * is the current maximum quantity plus the growth value.
    */

    private class UnorderedSetInt
    {
        /**
        * Construct an empty unordered set.  The initial maximum quantity and
        * growth values are DEFAULT_GROW.  When
        */
        ...
        /**
        * Construct an empty unordered set with the specified maximum quantity
        * and growth values.
        *
        * @param iMaxQuantity The initial number of elements in the array.  If
        *   the value is nonpositive, the initial number is DEFAULT_GROW.
        * @param iGrow The growth amount for a reallocation.  If a reallocation
        *   occurs, the new number of elements is the current maximum quantity
        *   plus the growth value.  If the input value is nonpositive, the
        *   growth is set to DEFAULT_GROW.
        */
        ...
        */
        ...
        /**
        * Reset the unordered set to its initial state.  The old array is
        * deleted.  The new array has a maximum quantity of DEFAULT_GROW and
        * the growth value is DEFAULT_GROW.
        */
        public void reset ()
        {
            reset(0,0);
        ...
        /**
        * Reset the unordered set to the specified state.  The old array is
        * deleted.  The new array has a maximum quantity and growth value as
        ...
        */
        ...
        /**
        * The maximum quantity of elements in the set.  Not all elements are
        * necessarily used.  The used quantity is provided by getQuantity().
        *
        * @return The maximum quantity of elements in the set.
        */
        final public int getMaxQuantity ()
        {
            return m_iMaxQuantity;
        }

        /**
        * The growth value for reallocations.  If a reallocation must occur,
        * the new maximum quantity is the current maximum quantity plus the
        * growth amount.
        *
        * @return The growth value.
        */
        final public int getGrow ()
        {
            return m_iGrow;
        }

        /**
        * The current number of valid elements in the array.  This number is
        * less than or equal to the maximum quantity.  The elements with indices
        * 0 through getQuantity()-1 are the valid ones.
        *
        ...
        */
        ...
        /**
        * Assign the specified element to array location i.  It is necessary
        * that 0 <= i < getMaxQuantity().
        *
        * @param i The array location to assign to.
        * @param iElement The element to assign to array location i.
        */
        final public void set (int i, int iElement)
        {
            m_aiElement[i] = iElement;
        }

        /**
        * Retrieve the element in the array location i.  It is necessary that
        * 0 <= i < getQuantity() in order to read valid elements.
        *
        ...
        * @return The value is true if and only if the element is found in the
        *    set.
        */
        ...
        * @return The value is true if and only if the element is inserted.  The
        *    input element is not inserted if it already exists in the set.  A
        *    side effect of this call is reallocation of the storage array, if
        *    necessary.
        */
        ...
        * @return The array location that contains the newly appended element.
        *    A side effect of this call is reallocation of the storage array, if
        *    necessary.
        */
        ...
        /**
        * On a call to remove or removeAt, the last element in the array is
        * moved to the array location vacated by the removed element.  The
        * old location of the last element is retrived by this function.  If
        * you need the value, you must call this function before the next call
        * to remove or removeAt.
        *
        * @return The old location of the last element that was moved.
        */
        final public int getOldIndex ()
        {
            return m_iOldIndex;
        }
        ...
        /**
        * On a call to remove or removeAt, the last element in the array is
        * potentially moved to the array location vacated by the removed
        * element.  The new location of the last element is retrived by this
        * function.  However, if the last element is the one that was removed,
        * this function returns -1.  If you need the value, you must call this
        * function before the next call to remove or removeAt.
        *
        * @return The new location of the last element that was moved.
        */
        final public int getNewIndex ()
        {
            return m_iNewIndex;
        }
        ...
        * @return The value is true if and only if the element existed and was
        *    removed.  The last element is potentially moved into the slot
        *    vacated by the specified element.  If needed, the old and new
        *    locations of the last element can be retrieved by calls to
        *    getOldIndex() and getNewIndex().  If the last element was the
        *    one removed, getNewIndex() returns -1.
        */
        ...
        * @return The value is true if and only if the input location is
        *    within the valid index range 0 <= i < getQuantity().  The last
        *    element is potentially moved into the slot vacated by the specified
        *    element.  If needed, the old and new locations of the last element
        *    can be retrieved by calls to getOldIndex() and getNewIndex().  If
        *    the last element was the one removed, getNewIndex() returns -1.
        */
        ...
        /**
        * Use exactly the amount of array storage for the current elements in
        * the set.  After the call, getQuantity() and getMaximumQuantity()
        * return the same value.  This call does cause a reallocation.
        */
        ...
        // The default growth value for reallocations of the array representing
        // the set.  The application can change this to whatever is appropriate
        // for its purposes.
        private int DEFAULT_GROW = 8;

        // The number of valid elements in the array.  The valid indices are
        // 0 <= i < m_iQuantity.
        protected int m_iQuantity;

        // The maximum number of elements in the array.  It is always the case
        // that m_iQuantity <= m_iMaxQuantity.
        protected int m_iMaxQuantity;

        // On a reallocation, the old maximum quantity is incremented by this
        // value.
