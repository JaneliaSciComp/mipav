...
*   changed picture to the same image.  Running this algorithm
*   with a <code>null</code> destination image causes the
*   algorithm to replace the source image with the
*   resultant image.
*   <p>
*   This is a 'regional' algorithm, that is
*   it divides the image buffer into (mostly)
*   equi-sized regions (a 'window') and forms
*   the histogram from that region.  The value of the cell
*   (the brightness of the pixel at a particular location) is
*   then mapped to the region.  Over the entire image, this
*   means that all pixels will still be brighter or darker
*   than other pixels, but the brightness has been shifted
*   making minor variations more apparent.  The cumulative
*   distribution is changed so that it becomes step-wise
*   linear, making the darker pixels use the lowest possible
*   values available to the image, the brightest use the
...
*   brightnesses.  (If the supplied per-centage is 80, for Instance,
...
*       Enhancement Using Generalizations of Histgram Equalization</u>.</li>
...
public class AlgorithmAHE extends AlgorithmBase {
	...
	*   color, red, blue and green, is run independantly of the other two
    ...
    */
    private void constructLog() {
        ...
        historyString = new String("RegionalAdaptiveHistogramEqualization("
                                   + String.valueOf(wDivisions)
                                   + ", "
                                   + String.valueOf(hDivisions)
                                   + ")"
                                   + clFrac
                                   + "\n");
    }
    ...
    */
    private void calcInPlace2D(){
        ...
        int length = srcImage.getExtents()[0]
                    *srcImage.getExtents()[1];// total number of data-elements (pixels) in image
        ...
        try {
          buildProgressBar(srcImage.getImageName(), "Equalizing Histogram ...",
                           0, 100);
          initProgressBar();
            ...
            if (!isColorImage) {
                ...
                try {
                    ...
                    if (threadStopped) {
                        Preferences.debug("somehow you managed to cancel the algorithm and dispose the progressbar between checking for threadStopping and using it.", Preferences.DEBUG_ALGORITHM);
                    }
                }
                ...
                for (color = 1; color < valuesPerPixel && !threadStopped; color++) { // for each color
                    if (  (color == 1 && rChannel)
                        ||(color == 2 && gChannel)    // process only desired channels
        ...
        try {
          ...
          initProgressBar();
            ...
            if (!isColorImage) {
                ...
                for (color = 1; color < valuesPerPixel && !threadStopped; color++) {
                    if (  (color == 1 && rChannel)
                            ||(color == 2 && gChannel)         // and if it is a desired color
                            ...
                            try {
                                if ( pBarVisible ) {
                                    progressBar.updateValue(
                                            (int) ( ( (float) (Math.max(0,colorUsed-1)*nSlices + i ) / (numColors * nSlices) ) * 100 ),
                                            activeImage );
                                    if (color == 1 && rChannel) {
    ...
    *   Note that a progressBar is not changed in this method.
    *   @param  srcBuffer   source buffer
    *   @param  destBuffer  destination Buffer
    *
    */
    private final void monoSliceFilter(float  srcBuffer[],
                                       float  destBuffer[])  {
        ...
        unitSize = ((bufMax - imageOffset)/(totalBins-1)); // one unit is the distance of one totalBin-th in brightness
                ...
                for (y = yStart; y < yEnd; y++) {
                    for (x = xStart; x < xEnd; x++) {
                        try {
                            ...
                            Preferences.debug("srcbuffer: "+srcBuffer[x + y*width] +" bufMin: "+bufMin + " unitSize: "+unitSize+"\n");
                    ...
                    for (x = xStart; x < xEnd; x++) {
                        ...
                        try {
                            destBuffer[x + y*width] = ( // scales the destimage brightness by shifted version
                                  w1 * mapping[brightnessLevel][r][c]
                                + w2 * mapping[brightnessLevel][r][c+1]
                            ...
                            Preferences.debug("y: "+y+", x: "+x+", r: "+r+", c: "+c+", location in buffer: "+Integer.toString(x+y*width)+".\n");
                            Preferences.debug("Y: "+srcImage.getExtents()[1]+", X: "+Integer.toString(srcImage.getExtents()[0])+"\n");
                            Preferences.debug("</HALT>\n");
        ...
        for (c = 0; c < (wDivisions - 1); c++) {
            ...
            for (x = xStart; x < xEnd; x++) {
                ...
                for (y = yStart; y < yEnd; y++) {
                    brightnessLevel = (int) ((srcBuffer[x + y*width] - imageOffset)/(unitSize));
                    try { //if (brightnessLevel < totalBins)
                        destBuffer[x + y*width] = (
                            w1 * mapping[brightnessLevel][0][c]
                ...
                for (y = yStart; y < yEnd; y++) {
                    ...
                    try { // if (brightnessLevel < totalBins)
                        destBuffer[x + y*width] = (
                            w1 * mapping[brightnessLevel][hDivisions - 1][c]
        ...
        for (r = 0; r < hDivisions-1; r++) {
            ...
            for (y = yStart; y < yEnd; y++) {
                ...
                for (x = xStart; x < xEnd; x++) {
                    ...
                    try { // if (brightnessLevel < totalBins)
                        destBuffer[x + y*width] = (
                              w1 * mapping[brightnessLevel][r][0]
                ...
                for (x = xStart; x < xEnd; x++) {
                    ...
                    try { // if (brightnessLevel < totalBins)
                        destBuffer[x + y*width] = (
                              w1 * mapping[brightnessLevel][r][wDivisions - 1]
    ...
    *                           Obviously, this method will not acheive anthing when
