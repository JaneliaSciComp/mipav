 ...
 *	<p>
 *	The initial dialog asks the user for a signal threshold value.  The default value is the image minimum value.
 *	In the centroid calculation only those pixels whose values equal or exceed threshold are used in the
 *	centroid calculaton.  In the hard segmentation pixels whose values are less than threshold
 *	or not in a selected VOI if if the whole image is not used are set to a segmentation value of 0,
 *	meaning that these pixels are outside of the specified classes.
 *	<p>
 *	The initial dialog has a checkbox for Boundary noise cropping which is unchecked by default.
 *	This function finds the smallest bounding box outside of which all image pixel values are below
 *	the image threshold.  Values inside the bounding box are copied to a smaller array to save space
 *	and calculations are performed with this reduced array.  However, these pixels outside the box are
 *	restored for the production of the hard and fuzzy segmented images.  In the hard segmentation case values
 *	outside the box all have a value of 0.  In the fuzzy segmentation case values outside the box all have a
 *	value equal to the image minimum.  If the cropping checkbox is selected, the user is constrained to
 *	select whole image rather than VOI region(s).
 *	<p>
 ...
 *  <p>
 *	References:
 *	This code is a ported subset of C-code written and kindly provided by Dzung Pham.
...
public class AlgorithmFuzzyCMeans extends AlgorithmBase {
    ...
    public AlgorithmFuzzyCMeans( ModelImage[] destImg, ModelImage srcImg,
            ...
            int _jacobiIters1, int _jacobiIters2, float _q,
            float _smooth1, float _smooth2,
            boolean _outputGainField, int _segmentation,
            boolean _cropBackground, float _threshold,
            int _max_iter, float _tolerance,
            boolean _wholeImage ) {
        super( null, srcImg );
     ...
     * out puts the string to the messsage frame if the logging procedure is turned on.
     */
    private void constructLog() {
        historyString = new String(
                ...
                + String.valueOf( qValue ) + ", " + String.valueOf( exponent ) + ", " + String.valueOf( smooth1 ) + ", "
                ...
                try {
                    ...
                    if (destImage != null) {
                        MipavUtil.displayError(
                                "FuzzyCMeans: IOException on destImage[" +
                                destNum + "].importData(0,memsBuffer,true)");
                    }
                    else {
                        MipavUtil.displayError(
                        "FuzzyCMeans: IOException on srcImage.importData(0,memsBuffer,true)");
                    }
                    if ( progressBar != null ) {
                        progressBar.dispose();
                    }
        ...
        if ( segmentation != FUZZY_ONLY ) {
            try {
                ...
                if ( ( units[0] != FileInfoBase.UNKNOWN_MEASURE ) && ( units[0] == units[1] ) ) {
                    ...
                    for ( i = 0; i < nClass; i++ ) {
                        UI.setDataText(
                                srcImage.getImageName() + " intensity = " + range * ( i + 1 ) + " count = "
                                + classNumber[i] + " area = " + pixelSize * classNumber[i] + " square " + unitsStr
                                + "\n" );
                    } // for (i = 0; i < nClass; i++)
                } else {
                    for ( i = 0; i < nClass; i++ ) {
                ...
                if (destImage != null) {
                    MipavUtil.displayError(
                            "FuzzyCMeans: IOException on destImage[" + destNum +
                            "].importData(0,segBuffer,true)");
                }
                else {
                    MipavUtil.displayError(
                            "FuzzyCMeans: IOException on srcImage.importData(0,segBuffer,true)");
                ...
                if (destImage != null) {
                    MipavUtil.displayError(
                            "FuzzyCMeans: Out of memory on destImage[" +
                            destNum + "].importData(0,segBuffer,true)");
                }
                else {
                    MipavUtil.displayError(
                            "FuzzyCMeans: Out of memory on srcImage.importData(0,segBuffer,true)");
    ...
    private void cMeans3() {
        ...
        try {
            ...
            if ( segmentation != FUZZY_ONLY ) {
                ...
                if ( ( units[0] != FileInfoBase.UNKNOWN_MEASURE ) && ( units[0] == units[1] )
                    ...
                    for ( i = 0; i < nClass; i++ ) {
                        UI.setDataText(
                                srcImage.getImageName() + " intensity = " + range * ( i + 1 ) + " count = "
                                + classNumber[i] + " volume = " + voxelSize * classNumber[i] + " cubic " + unitsStr
                                + "\n" );
                    } // for (i = 0; i < nClass; i++)
                } else {
                    for ( i = 0; i < nClass; i++ ) {
            ...
            if (destImage != null) {
                MipavUtil.displayError(
                        "FuzzyCMeans: Out of memory on destImage[" + destNum + "].importData(0,segBuffer,true)");
            }
            else {
                MipavUtil.displayError(
                        "FuzzyCMeans: Out of memory on srcImage.importData(0,segBuffer,true)");
    ...
    /* private void ComputeMultiplier2D(float[] buffer, float[] centroids, float[] mems, float[] mult,
     float smooth1, float smooth2, float qValue) {
     ...
     for (x = 0; x < xDim; x++) {
     for (y = 0; y < yDim; y++) {
     ComputeFW2(f,w,buffer,centroids,mems,x,y,qValue);
     } // for (y = 0; y < yDim; y++)
     } // for (x = 0; x < xDim; x++)
    ...
    /* private void ComputeFW2(float[] f, float[] w, float[] buffer, float[]centroids, float[] mems,
     ...
     w[index] += (float)Math.pow((double)mems[indexM],(double)qValue) * temp1;
     f[index] += (float)Math.pow((double)mems[indexM],(double)qValue) * temp2;
     } // for (c = 0; c < nClass; c++)
     if (buffer[index] < threshold) {
     w[index] = 0.0f;
     } // if (buffer[index] < threshold)
    ...
    /* private void jacobi2(float[] u, int ux, int uy, float[] f, float[] wpyr,
     ...
     // Set up the mask
     mask = new float[25];                                   // Make out permanent to speed up.
    ...
    /**
     *   Convolves a 2D dataset with a 5 x 5 mask
     *   @param in
     *   @param inx
     *   @param iny
     *   @param mask
    ...
    /* private float[] conv2d5(float[] in, int inx, int iny, float[] mask) {
     ...
     outLength = inx*iny;
     out = new float[outLength];               // Make out permanent to speed up.
     ...
     out[offset] += mask[l+2 + 5*(m+2)]*in[lind + mind*inx];
     }
     }
     }
     }
