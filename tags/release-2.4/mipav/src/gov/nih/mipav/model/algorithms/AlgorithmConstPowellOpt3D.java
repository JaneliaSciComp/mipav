...
/**
 *      Runs Powell's method for a 3D image.  Check the parent class comment
...
public class AlgorithmConstPowellOpt3D
   ...
   */
  public AlgorithmConstPowellOpt3D(AlgorithmBase parent, Point3Dd com,
                              int degreeOfFreedom,
                              AlgorithmOptimizeFunctionBase costFunc,
                              double[] initial, double[] tols, int maxIter,
                              int bracketBound) {
    super(parent, degreeOfFreedom, costFunc, initial, tols, maxIter, 3,
          bracketBound);
  ...
  public void setLimits(float[][] limits) {
    if (nDims == 3) {
      ...
      for (int i = 0; i < 3; i++) { // translation x, y, z; i=0, 1, 2
        ...
        Preferences.debug("3D optimization.  For direction " +i  +", the minimum is: "
                          +trLimits[0][i] +" and the maximum is "
                          +trLimits[1][i] +" pixels.\n");
      }
    }
    else if (nDims == 4) {
   ...
   *	@return	vector representing the best transformation in terms of
  ...
  /**
   * Accessor that returns the matrix representing the best tranformation.
   * This transformation contains only the z rotation and the x and y
   * translation, to be used in the midsagittal alignment algorithm.
   ...
   * for both input and reference images, since resampled isotropically).  Since
   * the optimization was done in pixel space, not millimeter space, the translation
   * parameters need to be scaled by the sample value.  This transformation contains
   * only the z rotation and the x and y translation, to be used in the midsagittal
   * alignment algorithm.
   * @param sample the voxel resolution
   * @return matrix representing the best transformation's z rot and x and y trans.
   */
  ...
  /**
   * Accessor that returns the matrix representing the best tranformation.
   * All of the components of the transformation are halved from the
   ...
   * for both input and reference images, since resampled isotropically).  Since
   * the optimization was done in pixel space, not millimeter space, the translation
   * parameters need to be scaled by the sample value.  All of the components of
   * the transformation are halved from the 'best transformation' matrix.
   * @param sample the voxel resolution
   * @return matrix representing the best transformation with its components halved.
   */
  ...
  public void runAlgorithm() {
    ...
    if (count == maxIterations || numFail == nDims) {
      Preferences.debug(
      "Exited main Powell without finding the best minimum because ");
      if (count == maxIterations) {
        Preferences.debug(
            "the Powell loop reached the maximum iterations, " +maxIterations +"\n");
      }
      if (numFail == nDims) {
        Preferences.debug("for the given initial value, a better value could not be found in"
                          +" any of the " +nDims +" dimensions.\n");
      }
    }
   ...
   *	the degrees of freedom originally sent to this algorithm.  Therefore, if
   *	there are 3 degrees of freedom, we set only the translations; 4 means translation
   *	and global scaling; 6 means rotation and translation; 7 means rotation, translation,
   *	and global scaling; 9 means rotation, translation, and scaling; and 12 means rotation,
  ...
  public TransMatrix convertToMatrix(double[] vector) {
    ...
    else if (vector.length == 6 || vector.length == 7 || vector.length == 9 ||
             vector.length == 12) {
      rotX = vector[0];
      rotY = vector[1];
      rotZ = vector[2];
    ...
    matrix.setTransform(transX, transY, transZ, rotX, rotY, rotZ, scaleX,
                        scaleY, scaleZ, skewX, skewY, skewZ);
