...
/**  Addresses and lengths are always contained in 4 bytes.  The first 4 bytes plus 4 give the
    location of ascii text saying UserWndLevels, overlays, or sticks if overlays are present.
    The same region of interest in the same slice may be saved with exactly the same contour
    using 2 different bounding boxes.  The first 90 bytes are header bytes.

    Location 0x24 may
    give a location to the start of information for a given slice.  The first 4 bytes of this
    slice information will give the location of the start of information for another slice.
    This will continue until the first 4 bytes of a slice contains 0x28.  0x28 contains 0.  If
    0x24 does not point to the start of slice information, then it contains 0x28. However, the
    code collecting slice information from 0x24 was commented out since it produced a contour
    in slice 15 in 10080-2-dwi-bo-sw.oly that was not seen in Cheshire.

    With UserWndLevels
    0x8C gives the start of a slice of information and the first 4 bytes of the information slice
    give the start of another slice until finally 1 slice starts with 0.  With overlays and sticks
    an information slice starts 24 bytes after overlays.  This 1 information slice was different
    from other information slices in that the first 8 bytes had the locations of 2 information
    slices instead of just 4 bytes for 1 location like the other information slices.  Also note that
    information slices may be fragmentary - they may not contain all the information promised by
    length bytes.  In these cases just ignore the fragments of the information slices.
*/
...
public class FileCheshireVOI extends FileBase {
    ...
    *   @param image            image model: needed during the read process to ensure
    *                           the VOI "fits" in the image space.
    ...
    */
    ...
    /**   readVOI           - reads an Cheshire overlay file by reading the
    *                          overlay information for each different VOI into
    *                          a different bitmap
    ...
    */
    public VOI[] readVOI() throws IOException, OutOfMemoryError  {
        ...
        byte matchUserWndLevels[] = {0x55, 0x73, 0x65, 0x72, 0x57, 0x6E, 0x64, 0x4C, 0x65, 0x76, 0x65,
                                   0x6C, 0x73};
        byte matchoverlays[] = {0x6F, 0x76, 0x65, 0x72, 0x6C, 0x61, 0x79, 0x73};
        byte matchsticks[] = {0x73, 0x74, 0x69, 0x63, 0x6B, 0x73};

        progressBar = new ViewJProgressBar("Creating VOIs",
                                               "Reading " + fileName, 0, 100,
                                                true, null,null);
      ...
      /*  raFile.seek(0x24L);
      // Comment out the chain starting at 0x24 as this produced a contour in 10080-2-dwi-bo-sw.oly
      // in slice 15 that was not seen in the Cheshire program.
        nextSliceAddress = getInt(endianess);
        while (nextSliceAddress >= 0x90L) {
            sliceAddress[numberSlices++] = nextSliceAddress;
            raFile.seek(nextSliceAddress);
            ...
            for (kk = 0; kk < numberSlices; kk++) {
                if ((nextSliceAddress >= sliceAddress[kk]) && (nextSliceAddress <= (sliceAddress[kk] +
                                                              sliceOffset[kk]))) {
                    // Now feeding into an already found slice address
                    nextSliceAddress = 0;
                    checkOffset = false;
                }
            }
        ...
        for (ii = 0; ii < numberSlices; ii++) { // read slice by slice
            ...
            for (jj = 0; ((jj < ROINumber) && (doSlice)); jj++) { // read ROI by ROI
                ...
                40 bytes of zeros or 31 bytes of zeros
                01 00 00 FF or 00 00 00 FF
                00 00 00 00
                00 00 01 00
                00 00 00 00
                00 00 00 FF
                ...
                MS<sp>Sans<sp>Serif
                20 bytes of zeros
                09 00 00 00 00 00 01 00 01 00 3C 0C 00 00 00 09
                11 bytes of zeros
                90 01
                7 bytes of zeros
                02 02 02 4D
                ...
                currentLocation = raFile.getFilePointer();
                for (kk = 0; kk < numberSlices; kk++) {
                   if ((sliceAddress[kk] < currentLocation + bitMapl + 4) && (sliceAddress[kk] > sliceAddress[ii])){
                       readBytes = false;
                   }
                }
                if (readBytes) {
                        ...
                        for (kk = 0; kk < bytesToRead; kk++) {
                            ...
                            for (mm = 0; mm < 8; mm++) {
                                if ((valueByte & maskValue) != 0){
                                    ...
                                    if ((maskBuffer[position] != 0) && (maskBuffer[position] != (byte)VOINumber)) {
                                        MipavUtil.displayError("Overlapping VOIs in slice = " + slice);
                                        progressBar.dispose();
                                        currentLocation = raFile.getFilePointer();
                                        Preferences.debug("following bytes current location = " + currentLocation);
                                        raFile.close();
                                        return null;
                                    }
                                    else {
                            ...
                            for (mm = 0; mm < 8; mm++) {
                                if ((valueByte & maskValue) != 0) {
                                    ...
                                    if ((maskBuffer[position] != 0) && (maskBuffer[position] != (byte)VOINumber)) {
                                        ...
                                        progressBar.dispose();
                                        currentLocation = raFile.getFilePointer();
                                        Preferences.debug("multiplier bytes current location = " + currentLocation);
                                        raFile.close();
                                        return null;
                                    }
                                    else {
            ...
            for (xx = 0; xx < xDimE; xx++) {
              ...
              // also use four connected neighbors.
                ...
                if (newGrayScale) {
                  ...
                  ((VOIContour)(addedVOI[grayScaleNumber].getCurves()[zz].lastElement())).trimPoints(0.1, true);
                  grayScaleNumber++;
                  if (grayScaleNumber >= 1000) {
                    MipavUtil.displayError
                    ("File Cheshire VOI Extraction: Impossibly high >= 1000 gray scales detected");
                    progressBar.dispose();
                    return null;
	...
	/***
	* setRegionMask set the mask of all points in the 4 connected region with a gray scale value of
	* objectValue
	  ...
	  for (yt = 0; yt < yDimE; yt++) {
            for (xt = 0; xt < xDimE; xt++) {
             ...
             /* if ((neighbors == 0)|| (neighbors == 1)) {
              ...
              } // end of if ((neighbors == 0) || (neighbors == 1)) */
              if (neighbors == 2) {
