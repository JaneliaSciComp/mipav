...
/**
 * AlgorithmMode applies one of the user specified kernels to a 2D or 3D
 * BYTE, SHORT, INTEGER, UBYTE, USHORT, UINTEGER.  AlgorithmMode is a spatial
 * filtering technique that replaces the central kernel pixel with the mode
 * of the pixels under the kernelMask.  The mode of a set of values is the
 * value that occurs most frequently.  If all values are unique, that is,
 * all numbers occur once in the kernelMask, the algorithm does NOT
 * change the value of the central pixel.
 *
 * Mode filtering is currently thought to be useful in filtering a segmented
 * image to get rid of small clusters of one value within a large cluser of
 * a different value.  We are currently investigating mode filtering in
 * cleaning up a C-Means classification of brain images, which were detected
 * using the Brain Extraction Tool (BET).  Mode filtering can be applied to
 * gray scale images to remove some noise, however, we have found that a
 * 3X3X3 cube kernel significantly blurs the image, while an axis kernel
 * appear to clean up some of the noise without too much blurring.  The utility
 * more filtering on gray scale images need more investigation.
 *
 * Giving credit where credit is due, this algorithm and its associated
 * dialog, JDialogMode started with the AlgorithmMedian and JDialogMedian
 * code.  The color image stuff, iteration control, and standard deviation
 * were removed.  Furthermore, all processing is done using integer images
 * and buffers, as finding the mode of floating (real) numbers is not
 * meaningful.
 *
 * @author Paul Hemler
...
public class AlgorithmMode
   ...
   *   @param maskFlag     Flag that indicates that the mode filtering will be
   *                       performed for the whole image if equal to true.
   */
  public AlgorithmMode(ModelImage srcImg, int kSize, int kShape,
                       boolean maskFlag) {
    super(null, srcImg);
   ...
   *                       performed for the whole image if equal to true.
   */
  public AlgorithmMode(ModelImage destImg, ModelImage srcImg,
   ...
   *                       performed for the whole image if equal to true.
   */
  public AlgorithmMode(ModelImage destImg, ModelImage srcImg, int kSize,
                       int kShape,
                       boolean sliceBySlice, boolean maskFlag) {
  ...
  private void constructLog() {
      ...
      if (destImage != null) {
          historyString = new String("Mode(" +
                                     destImage.getImageName() + ", " +
                                     String.valueOf(kernelShape) + ", " +
  ...
  private void calcInPlaceBorder2D() {
    ...
    try {
      ...
      resultBuffer = null;
      errorCleanUp(
          "Algorithm Mode reports: Out of memory when creating image buffer", true);
      return;
    }
    ...
    try {
      ...
      resultBuffer = null;
      errorCleanUp(
          "Algorithm Mode reports: Out of memory when creating border buffer", true);
      return;
    }

    buildProgressBar(srcImage.getImageName(), "",
                     0, 100); // let user know what is going on
    initProgressBar();
   ...
   */
  private void copy2DSrcBufferToBdrBuffer(int srcBuffer[], int bdrBuffer[],
    ...
    for (srcRow = 0; srcRow < srcBufferHeight; ++srcRow) {
      ...
      for (srcCol = 0; srcCol < srcBufferWidth; ++srcCol, srcBufferIdx++,
           bdrBufferIdx++) {
    ...
    for (row = 0; row < halfK; ++row) {
      ...
      for (col = 0; col < halfK; ++col) {
        ...
        bdrBuffer[row * bdrBufferWidth + bdrBufferWidth - 1 -
            col + bdrBufferOffsetIndex] = srcBuffer[srcBufferWidth - 1]; // 6/28
      }
    }

    // last source image row to upper bdrBuffer rows
    int srcBufferFirstColIdx = srcBufferWidth * (srcBufferHeight - 1) +
        srcBufferOffsetIndex;
    int srcBufferLastColIdx = (srcBufferWidth * srcBufferHeight) - 1 +
        srcBufferOffsetIndex;
    for (row = srcBufferHeight + halfK; row < bdrBufferHeight; ++row) {
      ...
      for (col = 0; col < halfK; ++col) {
        ...
        bdrBuffer[row * bdrBufferWidth + bdrBufferWidth - 1 -
            col + bdrBufferOffsetIndex] = srcBuffer[srcBufferLastColIdx]; // 6/28
        bdrBufferIdx++;
      }
    }
    ...
    for (srcRow = 0; srcRow < srcBufferHeight; ++srcRow) {
      ...
      srcBufferIdx = srcRow * srcBufferWidth + srcBufferWidth - 1 +
          srcBufferOffsetIndex;
      bdrBufferIdx = (srcRow + halfK) * bdrBufferWidth + halfK + srcBufferWidth +
          bdrBufferOffsetIndex;
      for (srcCol = 0; srcCol < halfK; ++srcCol) {
        bdrBuffer[bdrBufferIdx++] = srcBuffer[srcBufferIdx];
      }
    }
  ...
  private void copy3DSrcBufferToBdrBuffer(int srcBuffer[], int bdrBuffer[]) {
    ...
    for (sliceIndex = 0; sliceIndex < srcBufferDepth; sliceIndex++) {
      ...
      progressBar.updateValue(Math.round
                           ( ( ( (float) (sliceIndex) / (srcBufferDepth - 1)) *
                              100)), activeImage);

      copy2DSrcBufferToBdrBuffer(srcBuffer, bdrBuffer,
                                 sliceIndex * srcBufferSliceLength,
                                 sliceIndex * bdrBufferSliceLength +
                                 bdrBufferSliceOffset);
    }
    ...
    if (sliceFiltering == false) {
      ...
      for (sliceIndex = bdrBufferDepth - halfK; sliceIndex < bdrBufferDepth;
           sliceIndex++) {
        copyFromIndex = bdrBufferSliceOffset +
            (srcBufferDepth - 1) * bdrBufferSliceLength;
        copyToIndex = sliceIndex * bdrBufferSliceLength;
        for (dataIndex = 0; dataIndex < bdrBufferSliceLength; dataIndex++) {
          bdrBuffer[copyToIndex++] = bdrBuffer[copyFromIndex++];
        }
  ...
  private void calcInPlaceBorder3D() {
    ...
    this.buildProgressBar(srcImage.getImageName(), "Buffering ...", 0,
                          100);
    initProgressBar();
    ...
    if (sliceFiltering) {
      for (currentSlice = 0; currentSlice < numberOfSlices && !threadStopped;
           currentSlice++) {
        ...
        sliceFilterBorder(bdrBuffer, resultBuffer,
                          currentSlice * bdrSliceLength,
  ...
  private void calcStoreInDestBorder2D() {
    ...
    buildProgressBar(srcImage.getImageName(), "Filtering image ...",
                     0, 100); // let user know what is going on
    initProgressBar();
   ...
   */
  private final void sliceFilterBorder(int srcBdrBuffer[],
                                       int destBuffer[],
    ...
    for (destRow = 0; destRow < srcBufferHeight; destRow++) {
      ...
      for (destCol = 0; destCol < srcBufferWidth; destCol++) {
        ...
        destBuffer[destBufferIdx++] = mode(maskedList,
                                           srcBdrBuffer[srcBdrBufferIdx++]);
  ...
  private void volumeFilterBorder(int srcBdrBuffer[], int destBuffer[]) {
    ...
    int srcBrdBufferKernelOffset = halfK * bdrBufferWidth * bdrBufferHeight +
        halfK * bdrBufferWidth + halfK;
    ...
    for (destSlice = 0; destSlice < srcBufferDepth && !threadStopped;
         destSlice++) {

      progressBar.updateValue(Math.round
                           ( ( ( (float) (destSlice) / (srcBufferDepth - 1)) *
                              100)),
                           activeImage);

      srcBdrBufferSliceOffset = destSlice * srcBdrBufferSliceLength +
          srcBrdBufferKernelOffset;
      ...
      for (destRow = 0; destRow < srcBufferHeight; destRow++) {
        ...
        for (destCol = 0; destCol < srcBufferWidth; destCol++) {
          ...
          maskedList = getBorderBufferNeighborList(srcBdrBufferIdx,
              srcBdrBuffer, false);
          shellSort(maskedList);
          destBuffer[destBufferIdx++] = mode(maskedList,
                                             srcBdrBuffer[srcBdrBufferIdx++]);
  ...
  private void makeKernel() {
    try {
      ...
      displayError(
          "Algorithm Mode reports: not enough memory to form a kernel mask.");
      setCompleted(false);
      setThreadStopped(true);
      return;
    }
   ...
   *  image which will be used to find the mode of the
   *  central pixel.  Its length is
   *  <i>(number of pixels to be used to determine mode)</i>.
   *  <p>
   ...
   */
  private final int[] getBorderBufferNeighborList(int i, int[] data,
                                                  boolean is2D) {
      ...
      for (slice = -halfK; slice <= halfK; slice++) {
        for (row = -halfK; row <= halfK; row++) {
          for (col = -halfK; col <= halfK; col++) {
            if (kernel[kCenter + col + row * kernelSize +
                slice * kernelSize * kernelSize] != 0) {
              kernelMask[count++] = data[i + col + row * width +
                  slice * width * height];
            }
          }
   ...
   *  pp. 331- 332.
   */
  private final void shellSort(int[] buffer) {
    int i, j, inc;
    int v;
    inc = 1;
  ...
  /**
   *   Finds the mode value of the list.  If the numbers
   *   in the list are unique, return the value of the
   *   second parameter.  This method assumes the parameter
   *   list is sorted, and counts consecutive array elements
   ...
   */
  private final int mode(int list[], int val) {
    int count = 1;
    int maxCount = 1;
    int rtnVal = val;
