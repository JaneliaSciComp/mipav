...
/**
 * This is an abstract interface for a curve in 3D.  The curve is defined as
 * a function of time over an interval.  The position, 1st derivative, and
 * 2nd derivative is computed for any point along the curve as selected
 * by an input value in the time domain.  The tanget at any point is
 * computed as direction of the 1st derivative vector and the speed
 * is computed as the length of the 1st derivative vector.  The arc length
 * of any subinterval of the curve can be computed.  The curve
 * can be reparameterized as function of arc length where the length
 * at the minimum interval time is zero and the length at the maximum
 * interval time is the total length of the curve.  This means that
 * given an arc length, the corresponding input time within the curve's
 * time interval is determined.  Extensions of this class provide the
    ...
    extends Object
{
    ...
    /**
     * Get the 3D coordinates of the tanget vector at the point along
     * the curve evaluated for the input time value.  The tanget vector
     * is the normalized direction of the 1st derivative vector.
     * @param fTime float Input time value which must be within the range
     * for which this curve is defined.
     * @return Vector3f New instance created to contain the 3D coordinates
     * of the computed tanget vector.  If the time value is less than
     * getMinTime() then the tanget vector at the start of the curve
     * is returned.  If the time value is greater than getMaxTime() then the
     * tanget vector at the end of the curve is returned.
     */
    ...
    /**
     * Get the speed at the point along the curve evaluated for the input
     * time value.  The speed is the length of the 1st derivative vector.
     * @param fTime float Input time value which must be within the range
     * for which this curve is defined.
     * @return float The computed speed.  If the time value is less than
     * getMinTime() then the speed at the start of the curve is returned.
     * If the time value is greater than getMaxTime() then the speed at the
    ...
    /**
     * Reparameterize the curve where the time value is defined as a function
     * of arc length.  Locate the time value within the specified time
     * subinterval that is associated with this arc length along the curve.
     ...
     * @param fLength float Input arc length for which the associated time
     * is to be determined.  This length is measured from the starting
     * point of the curve which is at fTimeMin.
     * @param iIterations int Maximum number of iterations for determining
     * the associated time value.  A bisection method is used to perform
     * the search over the entire range of time values defined for this
     * curve.  If this maximum number of iterations are executed, then
     * the resolution of the time interval is (fTimeMax-fTimeMin)
     * divided by 2^{iIterations}.
     * @param fLengthTolerance float If the length of the current subinterval
     * in the bisection method is smaller than this value, then the search
     * terminates.
    ...
    // total length of curve so that it is computed once;
    // initially set to invalid number to indicate uninitialized
