...
/**
 *  This algorithm anisotropically diffuses an image. That is to say it blurs
 *  over regions of an image where the gradient mag. is relatively small but
 *  diffuses little over areas of the image where the gradient mag is large
 *  (i.e. edges). Therefore, objects are blurred but not edges of objects.
 *  K is a factor that controls the diffusion rate. A large value causes this
 *  algorithm to act like gaussian smoothing where the number of iterations
 *  is related to the sigma of the gaussian. Small values of k reduce
 *  the diffusion across edges.
 *	<p>
 *  The basic equation is the I(x,y,z,t+1) = I(x,y,z,t) + lambda*c(x,y,z,t)*Laplacian(I(x,y,z,t))
 *  I(x,y,z,t) is the brightness function.
 *  It is required that 0 <= lambda <= 0.25 for numerical stability.  In this program
 *  lambda = 0.25 is used.  The conduction coefficient c(x,y,z,t) is a function of the magnitude of
 *  the edge. That is, c = g(||E||).  The gradient of the brightness function gives an
 *  excellent estimate of the edge, E(x,y,z,t) = grad(I(x,y,z,t)).
 *  Thus, c = g (||grad(I(x,y,z,t))||).  In this program g is selected so that
 *  c = 1/(1 + square(||grad(I(x,y,z,t))||/K))
 *	<p>
 *  This program uses the a constant times the square root of the sum of the squares of the
 *  normalized first order derivatives of the Gaussian function as an approximate gradient function.
 *  The constant is selected so that the maximium value of this approximation to the
 *  gradient function is 100.
 *
...
public class AlgorithmAnisotropicDiffusion extends AlgorithmBase {
    ...
    *   K = small reduces blur across edges. Typical K = 5.
    */
    ...
    *                      gaussian smoothing and therefore diffuses across edges.
    *                      K = small reduces blur across edges. Typical K = 5.
    *   @param maskFlag    Flag that indicates that the anisotropic diffusion will be
    ...
    */
	public AlgorithmAnisotropicDiffusion(ModelImage destImg, ModelImage srcImg,
	                                     float sigmas[], int iter, float kValue,
    ...
    *                      should be processed independently. 2D images disregard this flag.
    */
    public AlgorithmAnisotropicDiffusion(ModelImage srcImg, float sigmas[], int iter,
                                         float kValue, boolean maskFlag, boolean img25D) {
       super(null, srcImg);
    ...
    */
    ...
    /**
    *   Creates the derivative kernels used to calculate the gradient
    ...
    */
    ...
    /**
    *   Creates the 3D derivative kernels used to calculate the gradient
    *   magnitude and kernel for the diffusion process.
    */
    ...
    */
    private void constructLog() {
        ...
        for ( int i = 0; i < sigmas.length; i++){
         ...
         historyString = new String( "AnisotropicDiffusion(" + sigmaStr +
                                 String.valueOf(iterations)  +  ", " +
                                 String.valueOf(konsnt)  +  ", " +
    ...
    */
    private void calcInPlace2D(int nImages){
        ...
        try {
            ...
            buildProgressBar(srcImage.getImageName(), "Diffusing image ...", 0, 100);
        }
        ...
        for (s = 0; s <  nImages; s++) {
            ...
            for(n = 0; n < iterations && !threadStopped; n++) {
            	if (isProgressBarVisible() ) {
                	progressBar.updateValue(Math.round((float)(s*iterations+n)/(nImages*iterations-1) * 100), activeImage);
            	}
            	// Normalize GM !!
            	for (i = 0, gmMax = 0, gmMin = Float.MAX_VALUE ; i < length; i++) {
                	if (entireImage == true || mask.get(i) ) {
                    	ix = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                            	imgBuffer, kExtents, GxData);
                    	iy = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                            	imgBuffer, kExtents, GyData);

                    	gmBuffer[i] = (float)Math.sqrt(ix*ix + iy*iy);
                    	if (gmBuffer[i] > gmMax)      gmMax = gmBuffer[i];
                    	else if (gmBuffer[i] < gmMin) gmMin = gmBuffer[i];
                	}
            	}
            	for (i = 0; i < length && !threadStopped; i++) {
                	if (entireImage == true || mask.get(i)) {
                    	...
                    	val = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                        	imgBuffer, klapExtents, lapData);
    ...
    */
        ...
        for(n = 0; n < iterations && !threadStopped; n++) {
            ...
            for (i = 0; i < length && !threadStopped; i++) {
            	if (isProgressBarVisible() ) {
            		...
            		if (i%offset == 0) {
                		progressBar.updateValue(Math.round((float)(50.0*s)/(iterations*srcImage.getExtents()[2])) +
                		                    Math.round(100*((float)(n))/iterations) + Math.round((float)(50.0)/(iterations)), activeImage);
            		}
                ...
                if (entireImage == true || mask.get(i) ) {
                    ...
                    val = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(),
                                                           imgBuffer, klapExtents, lapData);
    ...
    /**
    *   Calculates the diffused image and stores the resultant diffused image
    *   in the destination image model.
    *   @param nImages      number of images to be blurred. If 2D image then nImage = 1,
    *                       if 3D image where each image is to processed independently
    *                       then nImages equals the number of images in the volume.
    */
        ...
        for(n = 0; n < iterations && !threadStopped; n++) {
            ...
            for (i = 0, gmMax = 0, gmMin = Float.MAX_VALUE ; i < length; i++) {
            	if (isProgressBarVisible() ) {
            		int s = i/offset;
            		if (i%offset == 0) {
                		progressBar.updateValue(Math.round((float)(50.0*s)/(iterations*srcImage.getExtents()[2])) + Math.round(100* ((float)(n))/iterations), activeImage);
            		}
            	}
                if (entireImage == true || mask.get(i) ) {
                    ix = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(),
            ...
            for (i = 0; i < length && !threadStopped; i++) {
            	if (isProgressBarVisible() ) {
            		int s = i/offset;
            		if (i%offset == 0) {
                		progressBar.updateValue(Math.round((float)(50.0*s)/(iterations*srcImage.getExtents()[2])) + Math.round(100*((float)(n))/iterations) + Math.round((float)(50.0)/(iterations)), activeImage);
            		}
    ...
    */
    private void calcStoreInDest25D(){
        ...
        for(n = 0; n < iterations && !threadStopped; n++) {
            if (isProgressBarVisible()) progressBar.updateValue(Math.round((float)n/(iterations-1) * 100), activeImage);
