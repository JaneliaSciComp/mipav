 ...
 *
 *      This version of the anisotropic diffusion equation is:
 *      dI/dt = div[g(normalized grad magnitude(image))* grad I],
 *      where g() is the edge stopping function.  The original edge
 *      stopping function used was the Perona and Malik function:
 *      g(x) = 1/(1 + (x**2/k**2)).
 *      However, as outlined in "Robust Anisotropic Diffusion" by Michael J. Black,
 *      Guillermo Sapiro, David H. Marimont, and David Heeger, IEEE Transactions
 *      on Image Processing, Vol. 7, No. 3, March 1998, pp. 421-432, the Perona and
 *      Malik function gives outliers(large values of grad(image)) too much influence.
 *      Thus, the Perona and Malik function was replaced with the Tukey's biweight
 *      function:
 *      g(x) = 0.5 * [1 - (x/k)**2]**2 for abs(x) <= k
 *           = 0 otherwise.
 *
 *      The normalized gradient magnitude of the image goes from 0 to 100.
 *      In the above equation I is a VOI intensity, where initially all the
 *      VOI area is assigned 100 and the rest of the image is assigned 0.
 *      The equation is discretized as:
 *      I(x,y,z,t+1) = I(x,y,z,t) + 0.25*g(normalized grad magnitude(image(x,y,z)))*
 *                          (I(x+gradX,y+gradY,z+gradZ,t) - I(x,y,z,t)),
 *      where gradX, gradY, and gradZ are the normalized gradient measurements
 *      of I.  The value of I is clamped to a maximum value of 100 and the
 *      value of I is not changed if either its value has fallen to be <= 3
 *      or the values of all 4 of its nearest neighbors are <= 3.
 *      Note that the name AlgorithmLevelSet is misleading because the equation
 *      used is a version of the anisotropic diffusion equation and not a version
 *      of the level set equation given by Osher and Sethian:
 *      d(phi)/dt + F*|grad(phi)| = 0, given phi(x,y,z,t=0).
 *      For more information on the Anisotropic diffusion equation see:
 *      Geometry-Driven Diffusion in Computer Vision edited by Bart M.
 *      ter Haar Romeny, Chapter 3 on Anisotropic Diffusion by Pietro
 *      Perona, Takahiro Shiota, and Jitendra Malik, pages 73-92,
...
public class AlgorithmLevelSetDiffusion extends AlgorithmBase {
    ...
    /**
     *   Controls the diffusion rate. A large value causes this algorithms to act like
     *   gaussian smoothing and therefore diffuses across edges.
     *   K = small reduces blur across edges. Typical K = 10.
     */
     ...
     *   @param kValue      K is a factor that controls the diffusion rate.
     *                      A large value causes this algorithms to act like
     *                      gaussian smoothing and therefore diffuses across edges.
     *                      K = small reduces blur across edges. Typical K = 10;
     */
    public AlgorithmLevelSetDiffusion( ModelImage srcImg, float[] sigmas, int iter,
            float kValue ) {
        super( null, srcImg );
     ...
     */
    private final float getBiLinear( float[] imageBuffer, int xDim, float x, float y ) {
     ...
     *   @param imageBuffer buffer containing pixel data
     *   @param xDim        x dimension offset
     *   @param imageSliceSize
     *   @param x           x coordinate
     *   @param y           y coordinate
     *   @param z           z coordinate
     */
    private final float getTriLinear( float[] imageBuffer, int xDim, int imageSliceSize,
