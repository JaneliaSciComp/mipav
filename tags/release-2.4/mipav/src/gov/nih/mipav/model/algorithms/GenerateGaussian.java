...
/**
 *  Calculates an n-dimensional gaussian volume at a given scale, kernel size, and
 *  normalized derivative (through the fourth order) in any of the dimensions.
 *  Explanation of normalized derivative:
 *  The derivative of n = (1/sqrt(2*PI*sigma*sigma))*exp(-x*x/(2*sigma*sigma)) with
 *  respect to x = -(x/(sigma*sigma))*n.  1/2 the integral of the derivative from
 *  -infinity to infinity with respect to x or the integral of the derivative from
 *  0 to infinity with respect to x = 1/(sigma * sqrt(2*PI)).  Hence, the derivative
 *  is normalized by multiplying by the normalizing constant sigma.  GenerateGaussian
 *  returns the Gaussian, sigma times the first derivative of the Gaussian, sigma squared
 *  times the second derivative of the Gaussian, sigma cubed times the third derivative
 *  of the Gaussian.
 *
...
public class GenerateGaussian {
     ...
     *   @param pt       array of ints representing a point where the Gaussian
