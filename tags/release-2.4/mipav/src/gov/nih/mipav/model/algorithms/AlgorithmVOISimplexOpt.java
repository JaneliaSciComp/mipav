...
/**
 * Perform Simplex Optimization on a VOI that has been propagated to a slice
 * to find a transformation which will make an affine "fit" a structure which the VOI
 * fit on the previous slice (used in RFASegTool when propagating liver segmentation VOIs).
 * Minimize cost = -sum of Gradient magnitude under propagated ROI in match image
 *
...
public class AlgorithmVOISimplexOpt extends AlgorithmBase {
    ...
    /**
     *  Set up the algorithm to perform optimizations on an image.  The individual Polygons to optimize will be provided later (use <code>goOptimize()</code>)
    ...
    /**
     * Set up the algorithm to perform optimizations on an image.  To be used in conjunction with the <code>run</code> method.
     * @param image  the image containing the VOI(s) to run the SimplexOpt algorithm on
     ...
     * @param p  the array of simplex vertices p[i][j] where each i is a simplex vertex and each j is a parameter tx, ty, rot, zoomx, zoomy
    ...
    private class CostFunction implements AlgorithmOptimizeFunctionBase {
        ...
        /**
         * Calcuated the cost function for a given simplex vertex.
         * cost = intensity sum of errors = sum_i(I[i] - I'[i])
         * @return the value of the cost function
