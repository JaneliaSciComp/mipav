...
/**
 * Algorithm that matches the transforms a match or source image so as to make
 * its histogram equal to the histogram of a base image.  In order to easily
 * note the great similarity of the histograms, the histogram displays must
 * both be in linear mode rather than log mode so that small differences
 * are not exaggerated.
 * The algorithm is as follows:
 * 1.) Make the histogram for the match image.
 * 2.) Make the histogram cumulative.
 * 3.) Create the scaled cumulative histogram mapping scaled so that the last bin of
 *     mapping has the range of the source image.
 * 4.) For the base image create a histogram with the same number of bins as the
 *     match image.
 * 5.) Make the histogram cumulative.
 * 6.) Create the scaled cumulative histogram cumHist scaled so that the last bin of
 *     cumHist has the range of the source image.
 * 7.) For each value of mapping[i] find the cumHist[j] that is closest in value.
 * 8.) In remap[i] put the value of srcImage corresponding to the jth bin.
 * 9.) For each srcBuffer[i] find the bin number brightnessLevel and replace
 *     the srcBuffer[i] with the remap[brightnessLevel].
 * References:
 * 1.) Digital Image Processing by Rafael C. Gonzalez and Richard E. Woods,
 *     Addison-Wesley Publishing Company, 1992, pp. 173-182.
 * 2.) Two-Dimensional Signal and Image Processing by Jae S. Lim,
 *     Prentice-Hall, Inc., 1990, pp. 453-459.
 *
 *   According to Freedman and Diaconis as summarized in "Recent Developments in
 *   NonParametric Density Estimation" by Alan Julian Izenman, Journal of the
 *   American Statistical Association, March, 1991, Vol. 86, No. 413, pp. 205 - 224:
 *   The ideal histogram bin width W is given by W = 2(IQR)pow(N,-1/3) where
 *   IQR is the inrterquartile range(the 75 percentile minus the 25th percentile)
 *   and N is the number of available samples.
 */
        ...
        try {
            ...
            if ( !isColorImage ) {
                ...
                for ( colour = 0; colour < valuesPerPixel && !threadStopped; colour++ ) { // for each color
                    try {
                        if ( pBarVisible ) {
                            progressBar.updateValue( (int) ( ( (float) ( colour ) / valuesPerPixel ) * 100 ),
                                    activeImage );
...
if (progressBar!= null){
    ...
    /**
     *   This function produces a new image that has had
     *   itself histogram matched to the base image.
     *   places filtered image in the destination image.
     */
     ...
     *   just break up each RGB image into these separate monochrome images
     ...
     *
     */
    private final void filter( float[] srcBuffer, float[] baseBuffer ) {
        ...
        idealWidth = (float) ( 2.0f
                * ( sortBuffer[( 3 * srcBuffer.length / 4 ) - 1] - sortBuffer[( srcBuffer.length / 4 ) - 1] )
        ...
        for ( i = 0; i < srcBuffer.length; i++ ) {
            try {
                ...
                Preferences.debug(
                        "srcbuffer: " + srcBuffer[i] + "; bufMin: " + bufMin + "; unitSize: " + unitSize + "\n" );
        ...
        for ( i = 0; i < baseBuffer.length; i++ ) {
            try {
                ...
                Preferences.debug
                        (
                        "basebuffer: " + baseBuffer[i] + "; offset: " + baseOffset + ";baseUnitSize: " + baseUnitSize
                        + "\n" );
    ...
    /**
     *  Finds the local maximum and minimum values in the given range
     *  in order, as given by the starting and stoping values.
