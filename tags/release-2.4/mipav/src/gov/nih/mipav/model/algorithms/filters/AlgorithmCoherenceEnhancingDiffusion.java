 ...
 *
 * <p>For color form a common structure tensor for all colors by averaging the
 * structure tensors for each color.</p>
 *
 * <p>References:
 * <ol>
 ...
 *      in Handbook of Computer Vision and Applications, Volume 2 Signal Processing
 *      and Pattern Recognition.  The book is published by Academic Press, ISBN 0-12-379772-1</li>
 *
 *  <li>Algorithms for Non-Linear Diffusion Matlab in a Literate Programming
 *      Style by Rein van den Boomgaard, Intelligent Sensory Information
 *      Systems, University of Amsterdam, The Netherlands
 *      http://carol.wins.uva.nl/~rein/nldiffusionweb/material.html</li>
 *
 *  <li>Coherence-Enhancing Diffusion of Colour Images by Joachim Weickert,
 *      A. Sanfeliu, J.J. Villanueva, J. Vitria, editors,
 *      Proc. VII National Symposium on Pattern Recognition and Image Analysis
 *      (VII NSPRIA, Barcelona, April 21-25, 1997), Vol. 1, pp. 239-244, 1997.</li>
 *
 *  <li>Coherence-Enhancing Diffusion of Colour Images by Joachim Weickert,
 *      Image and Vision Computing, Vol. 17, 1999, pp. 201-212.</li>
 *
 *  <li>Coherence-Enhancing Diffusion Filtering by Joachim Weickert,
 *      International Journal of Computer Vision, Vol. 31, No. 2/3,
 *      pp. 111-127, April, 1999</li>
 * </ol>
 * </p>
 *
...
public class AlgorithmCoherenceEnhancingDiffusion extends AlgorithmBase {
    ...
    /**
     * The time parameter step size, called tau in the algorithm description
     * In 2D for stability the time step should be <= 0.25.
     * In 3D for stability the time step should be <= 1/6.
     */
    float timeStep;
    ...
    * Width of the smoothing buffer
    ...
    public AlgorithmCoherenceEnhancingDiffusion( ModelImage destImg, ModelImage srcImg, int numI, float k,
                                                 float derivativeScale, float gaussianScale, boolean do25D, boolean entireImage ) {
     ...
     *   @param entireImage  whether to filter the entire image or just the VOI region
     */
    public AlgorithmCoherenceEnhancingDiffusion( ModelImage srcImg, int numI, float k, float derivativeScale, float gaussianScale, boolean do25D, boolean entireImage ) {
        super( srcImg, srcImg );
    ...
    /**
        ...
        */
        private void constructLog() {
                ...
                historyString = new String( "CoherenceDiffusion(" + ", " +
                                        gaussianScale + ", " +
                ...
                for ( int y = 0; y < yDim; y++ ) {
                    for ( int x = 0; x < xDim; x++, idx++ ) {
                        if ( entireImage || mask.get( sliceOffset + idx ) ) {
                            ...
                            r = -0.25f * ( bmc + bcp ) * Lmp + 0.50f * ( ccp + c[idx] ) * Lcp + 0.25f * ( bpc + bcp ) * Lpp
                                + 0.50f * ( amc + a[idx] ) * Lmc
                        ...
                        if ( entireImage || mask.get( sliceOffset + idx ) ) {
                            ...
                            if ( useRed ) {
                                r = -0.25f * ( bmc + bcp ) * LmpR + 0.50f * ( ccp + c[idx] ) * LcpR
                                    ...
                                    * iterationBufferR[idx]
                                    + 0.50f * ( apc + a[idx] ) * LpcR
                                    + 0.25f * ( bmc + bcm ) * LmmR
                                    + 0.50f * ( ccm + c[idx] ) * LcmR
                                    - 0.25f * ( bpc + bcm ) * LpmR;
                            ...
                            if ( useGreen ) {
                                r = -0.25f * ( bmc + bcp ) * LmpG + 0.50f * ( ccp + c[idx] ) * LcpG
                                    ...
                                    - 0.50f * ( amc + 2 * a[idx] + apc + ccm + 2 * c[idx] + ccp )
                                    * iterationBufferG[idx]
                                    + 0.50f * ( apc + a[idx] ) * LpcG
                                    + 0.25f * ( bmc + bcm ) * LmmG
                                    + 0.50f * ( ccm + c[idx] ) * LcmG
                                    - 0.25f * ( bpc + bcm ) * LpmG;
                            ...
                            if ( useBlue ) {
                                r = -0.25f * ( bmc + bcp ) * LmpB + 0.50f * ( ccp + c[idx] ) * LcpB
                                    ...
                                    - 0.50f * ( amc + 2 * a[idx] + apc + ccm + 2 * c[idx] + ccp )
                                    * iterationBufferB[idx]
                                    + 0.50f * ( apc + a[idx] ) * LpcB
                                    + 0.25f * ( bmc + bcm ) * LmmB
                                    + 0.50f * ( ccm + c[idx] ) * LcmB
                                    - 0.25f * ( bpc + bcm ) * LpmB;
                    ...
                    for ( int x = 0; x < xDim; x++, idx++ ) {
                        if ( entireImage || mask.get( idx ) ) {
                            ...
                            exponent = ( ( ( lambdaOne - lambdaTwo ) * ( lambdaOne - lambdaTwo )
                                           ...
                                           + ( lambdaTwo - lambdaThree ) * ( lambdaTwo - lambdaThree ) )
                                         / (float) k2 );
                            ...
                            r = -0.25f * ( ecpc + eccm ) * Lcpm + 0.25f * ( cmcc + cccm ) * Lmcm
                                ...
                                - 0.50f
                                * ( apcc + amcc + 2 * a[idx] + dcpc + dcmc + 2 * d[idx] + fccp + fccm + 2 * f[idx] )
                                * iterationBuffer[idx]
                                + 0.50f * ( a[idx] + apcc ) * Lpcc
                                + 0.25f * ( bmcc + bcmc ) * Lmmc
                                + 0.50f * ( d[idx] + dcmc ) * Lcmc
                                - 0.25f * ( bpcc + bcmc ) * Lpmc
                                + 0.25f * ( ecpc + eccp ) * Lcpp
                                - 0.25f * ( cmcc + cccp ) * Lmcp
                                + 0.50f * ( f[idx] + fccp ) * Lccp
                                + 0.25f * ( cpcc + cccp ) * Lpcp
                                - 0.25f * ( ecmc + eccp ) * Lcmp;
                        ...
                        if ( entireImage || mask.get( idx ) ) {
                            ...
                            if ( useRed ) {
                                r = -0.25f * ( ecpc + eccm ) * LcpmR + 0.25f * ( cmcc + cccm ) * LmcmR
                                    ...
                                    * ( apcc + amcc + 2 * a[idx] + dcpc + dcmc + 2 * d[idx] + fccp + fccm + 2 * f[idx] )
                                    * iterationBufferR[idx]
                                    + 0.50f * ( a[idx] + apcc ) * LpccR
                                    + 0.25f * ( bmcc + bcmc ) * LmmcR
                                    + 0.50f * ( d[idx] + dcmc ) * LcmcR
                                    - 0.25f * ( bpcc + bcmc ) * LpmcR
                                    + 0.25f * ( ecpc + eccp ) * LcppR
                                    - 0.25f * ( cmcc + cccp ) * LmcpR
                                    + 0.50f * ( f[idx] + fccp ) * LccpR
                                    + 0.25f * ( cpcc + cccp ) * LpcpR
                            ...
                            if ( useGreen ) {
                                r = -0.25f * ( ecpc + eccm ) * LcpmG + 0.25f * ( cmcc + cccm ) * LmcmG
                                    ...
                                    * ( apcc + amcc + 2 * a[idx] + dcpc + dcmc + 2 * d[idx] + fccp + fccm + 2 * f[idx] )
                                    * iterationBufferG[idx]
                                    + 0.50f * ( a[idx] + apcc ) * LpccG
                                    + 0.25f * ( bmcc + bcmc ) * LmmcG
                                    + 0.50f * ( d[idx] + dcmc ) * LcmcG
                                    - 0.25f * ( bpcc + bcmc ) * LpmcG
                                    + 0.25f * ( ecpc + eccp ) * LcppG
                                    - 0.25f * ( cmcc + cccp ) * LmcpG
                                    + 0.50f * ( f[idx] + fccp ) * LccpG
                                    + 0.25f * ( cpcc + cccp ) * LpcpG
                            ...
                            if ( useBlue ) {
                                r = -0.25f * ( ecpc + eccm ) * LcpmB + 0.25f * ( cmcc + cccm ) * LmcmB
                                    ...
                                    * ( apcc + amcc + 2 * a[idx] + dcpc + dcmc + 2 * d[idx] + fccp + fccm + 2 * f[idx] )
                                    * iterationBufferB[idx]
                                    + 0.50f * ( a[idx] + apcc ) * LpccB
                                    + 0.25f * ( bmcc + bcmc ) * LmmcB
                                    + 0.50f * ( d[idx] + dcmc ) * LcmcB
                                    - 0.25f * ( bpcc + bcmc ) * LpmcB
                                    + 0.25f * ( ecpc + eccp ) * LcppB
                                    - 0.25f * ( cmcc + cccp ) * LmcpB
                                    + 0.50f * ( f[idx] + fccp ) * LccpB
                                    + 0.25f * ( cpcc + cccp ) * LpcpB
     ...
     * @returns float The maximum of values a and b
     */
     ...
     */
    private final float getVal( float[] buffer, int col, int row ) {
        float rtnVal;
     ...
     * @return float The pixel value
     */
    private final float getVal( float[] buffer, int col, int row, int slice ) {
        float rtnVal;
     ...
     * @param row int Row index
     */
    private final void setVal( float val, float[] buffer, int col, int row ) {
     ...
     * @param slice int Slice index
     */
    private final void setVal( float val, float[] buffer, int col, int row, int slice ) {
    ...
    /**
     *
     *   @param scale float   scale space parameter, also known as sigma in the equations
     *   @param dx float []      destination buffer
     *   @param src float []     source buffer
     *
     *   <P>Member function computes the derivative of the source buffer in the X-direction
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with the derivative
     *   of a Gaussian kernel in the X-direction and stores the result in an intermediate
     *   buffer.  Once the intermediate buffer is completely filled, a smoothing in the
     *   Y-direction is performed using a 1D Gaussian kernel computed as in the MATLAB
     *   paper.
     *
     */
     ...
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with the derivative
     *   of a Gaussian kernel in the X-direction and stores the result in dx
     *   buffer.  Once dx is completely filled, a smoothing in the
     *   Y-direction is performed using a 1D Gaussian kernel and the intermediate result
     *   is stored in intermediateBuffer.  Once intermediateBuffer is
     *   completely filled, a smoothing in the Z-direction is performed and
     *   the final result is placed in dx.
     */
     ...
     *   @param src float []     source buffer
     *
     *   <P>Member function computes the derivative of the source buffer in the Y-direction
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with a
     *   Gaussian kernel in the X-direction and stores the result in an intermediate
     *   buffer.  Once the intermediate buffer is completely filled, a 1D convolution in the
     *   Y-direction is performed using a 1D Gaussian derivative kernel computed as in the
     *   MATLAB paper.
     */
     ...
     *   <P>Member function computes the derivative of the source buffer in the Y-direction
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D smoothing convolution with
     *   a Gaussian kernel in the X-direction and stores the result in dy
     *   buffer.  Once dy is completely filled, a convolution in the
     *   Y-direction is performed using the derivative of a 1D Gaussian kernel
     *   and the intermediate result is stored in intermediateBuffer.
     *   Once intermediateBuffer is completely filled, a smoothing in the
     *   Z-direction is performed and the final result is placed in dy.
     */
     ...
     *   @param src  float []    source buffer
     *
     *   <P>Member function computes the derivative of the source buffer in the Z-direction
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with
     *   a Gaussian kernel in the X-direction and stores the result in dz
     *   buffer.  Once dz is completely filled, a smoothing in the
     *   Y-direction is performed using a 1D Gaussian kernel and the intermediate result
     *   is stored in intermediateBuffer.  Once intermediateBuffer is
     *   completely filled, a convolution with the derivative of a Gaussian
     *   kernel in the Z-direction is performed and the final result is
     *   placed in dz.
     */
     ...
     *   @param src  float []    source buffer
     *
     *   <P>Member function gaussian smoothes the source buffer in the x and y directions
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with a
     *   Gaussian kernel in the X-direction and stores the result in an intermediate
     *   buffer.  Once the intermediate buffer is completely filled, a 1D convolution in the
     *   Y-direction is performed using a 1D Gaussian kernel.
     */
     ...
     *   @param src  float []    source buffer
     *
     *   <P>Member function gaussian smoothes the source buffer in the x,y, and z directions
     *   using the kernel formulation as specified in the MATLAB paper.  The function uses
     *   seperable convolution, which means it first does a 1D convolution with a
     *   Gaussian kernel in the X-direction and stores the result in gs.  Once the gs
     *   buffer is completely filled, a 1D convolution in the Y-direction is
     *   performed using a 1D Gaussian kernel and the result is stored in
     *   intermediateBuffer.  Once intermediateBuffer is completely filled,
     *   a 1D convolution in the Z-direction is performed using a 1D Gaussian
     *   kernel and the result is placed in gs.
     */
    ...
    private void buildProgressBar() {
        try {
            ...
            if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
                Preferences.debug(
                        "AlgorithmCoherenceEnhancingDiffusion: NullPointerException found while building progress bar." );
            }
        }
