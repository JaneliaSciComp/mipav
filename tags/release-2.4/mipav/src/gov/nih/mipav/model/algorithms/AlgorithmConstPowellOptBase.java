 ...
 *	Basically, it starts at some point P in N-dimensional space, proceeds in a direction, and minimizes
 *	along that line using a 1-dimensional minimization method.  It continues like this until the point
 *      has moved by less than the tolerance.  It starts with the initial point defined in
 *	the constructor and initial directions of (1 0 ... 0) (0 1 0 ... ) ..., the basis vectors.  At the end,
 *	"point" is the best point found and functionAtBest is the value at "point".
 *      <p>
 *      This is an abstract class which contains many of the basic functions needed to run
 *      Powell's method.  2D and 3D versions extend this version.
 *      <p>
 *      This optimization technique is based on FLIRT.  FLIRT stands
 ...
 *      <p>
 *      For further information about Powell's method consult:
 *      <u>Numerical Recipes in C: The Art of Scientific Computing Second Edition</u><br>
 *	Press, William H., Teukolsky, Saul A., Vettering, William T., and Flannery, Brian P.
 *	Cambridge Unversity Press: Cambridge, 1988.
...
public abstract class AlgorithmConstPowellOptBase
  ...
  /** Golden ratio is .38197 and .61803.  Use this second part to magnify
   successive intervals in the bracketing function.  The "1" is for the magnification. */
  ...
  /**
   *	Constructs a new algorithm with the given centers of mass (needed for setting the transformations),
   *	the given cost function (which was constructed with the proper images), the initial point we're looking
   *	at, and some tolerance within that point to look for the minimum.
   *   The initial point contains rotations, translations, scales, and skews.
  ...
  public AlgorithmConstPowellOptBase(AlgorithmBase parentAlgo,
                                     ...
                                     double[] initial,
                                     double[] tols, int maxIter, int coords,
                                     int bracket) {
  ...
  /**
   *	Accessor that returns the final point with translations,
   *	rotations, scales, and skews representing the best tranformation.
   *	@return	A vector representing the best transformation in terms of
   *			translations, rotations, scales, and skews.
   */
  ...
  /**
   *	Accessor that returns the matrix representing the best tranformation.  The
   *	passed in parameter represents the resolution (same in all directions and
   *	for both input and reference images, since resampled isotropically).  Since
   *	the optimization was done in pixel space, not millimeter space, the translation
   *	parameters need to be scaled by the sample value.
  ...
  /**
   *   Helper method that converts a vector with certain values
   *   set to a matrix.  For values not set, uses those in start variable.
  ...
  /**
   *   Minimizes the point along the given vector direction.  Given an initial point
   *   and a guess as to how far from that point we should go to look for a minimum.
   *
   *   Changed from lineMinimization in AlgorithmPowellOptBase because direction is now
   *   assumed ot be one of the original orthogonal directions and can be represented by one integer
   *   instead of a vector.
  ...
  private double estimateMinimum( Bracket bracket ) {
   ...
   /**
    *  Finds the bracket for the minimum value.  A "bracket" is a set of three points
    *  such that a < b < c and f(a) > f(b) and f(c) > f(b).  a and c are said to "bracket"
    *  b if this condition holds.
    *  Original was based on the function mnbrak in Numerical Recipes.  I have completely
    *  changed it so that it doesn't even attempt to use parabolic extrapolation.  -Z Cohen
   ...
   private Bracket minimumBracket(double a, double b, int id, double uTol) {
     ...
     while (bracket.functionAtB > bracket.functionAtC) {
       ...
       if ( (Double.isNaN(newX2)) || ( (newX2 - bracket.a) * dir < 0) ||
           ( (newX2 - maxX2) * dir > 0)) {
     ...
     if (testBounds1D(bracket.c)) {
       ...
       if (count > 1)
         Preferences.debug("It took " + count +
                           " iterations to bracket the minimum.\n");
         // If we never get this message, that means that the entire while loop above isn't
         // necessary, b/c we're never going into it.
     }
     return bracket;
   }
  ...
  /**
   *	Helper class for storing the six variables that come out of
   *	the minimumBracket function.
   */
