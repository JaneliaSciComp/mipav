 ...
 *
 * <p>Title: VOI Text</p>
 * <p>Description: VOI Text is used to hold an annotation that will be displayed on screen
 * with the chosen font size, color, type, and descriptors (BOLD, ITALIC)  it e</p>
...
public class VOIText extends VOIBase {
  ...
  /**
   * The descriptors for the font which start at PLAIN and are additive
   *  (PLAIN + BOLD = bold)  (BOLD+ITALIC = bold and italic) etc
   */
  ...
  /**
   * The font is stored here so that it does not have to be reallocated
   * on each redraw.  It is only new'd at the beginning or
   * if the fontDescriptors variable changes
   */
   ...
   */
   public void getBounds(float x[], float y[], float z[]) {
    ...
    public boolean contains( int _x, int _y, float zoomX, float zoomY, float [] resols, Graphics g) {
      ...
      // the width tells us the width of the font as it has been drawn on screen (not the width of the font's unzoomed size)
      int width = (int)(g.getFontMetrics(textFont).stringWidth(textString) / (zoomX * resols[0]));
      // the ascent value tells us how far the font travels from the baseline to the top of the text, for some reason it needed
      //    to be divided by two
      int ascentValue = (int)(g.getFontMetrics(textFont).getStringBounds(textString, g).getHeight() / (2 * zoomY * resols[1]));

      if (_x < x || _x > (x + width) ||
          _y < (y - ascentValue) || _y > y) {
        return false;
      }
    ...
    public void drawSelf( float zoomX, float zoomY, float resolutionX, float resolutionY,
            ...
            Graphics g, boolean boundingBox ) {
        int j;
