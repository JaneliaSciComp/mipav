...
public class AlgorithmMidsagittal extends AlgorithmBase {
    ...
    /**
     * The number of degrees of freedom to use when registering the original image
    ...
    /**
     * The search angles to use in each dimension (from <code>-searchAngle</code> to 
     * <code>searchAngle</code>) when registering the original image against the flipped image.
     */
    private int searchAngle = 45;
    
    /**
     * The coarse angle step to use in each dimension when registering the original image
     * against the flipped image.
     */
    private int coarseAngle = 15;
    
    /**
     * The fine angle step to use in each dimension when registering the original image
     * against the flipped image.
     */
    private int fineAngle = 6;
    
    /**
     * Whether to use the maximum of the minimum resolutions of the 2 images when respampling 
     * during registration against the flipped image.
     */
    ...
    /**
     * Whether to skip searching of the parameter space when registering the original image
     * against the flipped image.
     */
    private boolean fastMode = false;
    
    /**
     * The bracket size around the minimum in multiples of unit_tolerance for the first
     * iteration of Powell's algorithm when registering the original image against the flipped image.
     */
    private int bracketBound = 10;
    
    /**
     * Limits the number of iterations of Powell's algorithm when registering the original image
     * against the flipped image.
     */
    private int baseNumIter = 2;
    
    /**
     * Number of minima from level 8 to test at level 4 when registering the original image against
     * the flipped image.
     */
    ...
    private void calc() {
        ...
        // register
        AlgorithmRegOAR3D regAlgo = new AlgorithmRegOAR3D( srcImage, flipImage, costFunc, dof, interp,
            -searchAngle, searchAngle, coarseAngle, fineAngle,
            -searchAngle, searchAngle, coarseAngle, fineAngle,
            -searchAngle, searchAngle, coarseAngle, fineAngle,
            maxOfMin, doSubsample, fastMode,
            bracketBound, baseNumIter, numMinima );
        ...
        AlgorithmTransform transformAlgo = new AlgorithmTransform( srcImage, trans, AlgorithmTransform.TRILINEAR, res[0], res[1], res[2], ext[0], ext[1], ext[2], false, false, false );
