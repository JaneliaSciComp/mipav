 ...
 *  from the ACM found at http://www.netlib.org/toms/619.  Ths inversion of
 *  the Laplace transform is done using the Durbin formula in combination
 *  with the Epsilon Algorithm.  This algorithm by R. Piessens and R. Huysmans
 *  appeared in ACM Trans. Math. Software, Vol. 10, No. 3, Sep., 1984,
 *  pp. 348-353.
 */
...
public abstract class InverseLaplace2 {
  ...
  // has to be computed.  time value should be greater than zero.
  private double time[];
  ...
  // The routine tries to satisfy the least stringent of both accuracy
  // requirements.  That is, the routine tries to satisfy:
  // ABS(f(t) - result[0]) <= max(absEps, relEps*ABS(f(t)))
  // where f(t) is the actual time function
  ...
  // Parameter giving information on the termination of the algorithm
  // errStatus[0] = 0 normal and reliable termination of the routine
  // errStatus[0] = 1 The computations are terminated because the bound on the
  //              number of evaluations of the user supplied Laplace function
  //              has been achieved.  This bound is equal to 8*maxBound+ 5.
  //              One can allow more Laplace function evalutions by increasing
  //              the value of maxBound.
  // errStatus[0] = 2 An input time value is less than or equal to zero
  ...
  // emax = 1024, the largest exponent E for double precision, is I1MACH(16)
   ...
   * @param errStatus int[]
   */
  public InverseLaplace2(double time[], double abscissa, double relEps,
                         double absEps, double[] result, double [] estErr,
                         int evaluations[], int errStatus[]) {
                         this.time = time;
  ...
  public void driver() {
    ...
    if (result.length != tPts) {
      MipavUtil.displayError("time.length = " + time.length +
                             " but result.length = " + result.length);
      return;
    }
    if (estErr.length != tPts) {
      MipavUtil.displayError("time.length = " + time.length +
                             " but estErr.length = " + estErr.length);
      return;
    }
    if (evaluations.length != tPts) {
      MipavUtil.displayError("time.length = " + time.length +
                             " but evaluations.length = " +
                             evaluations.length);
      return;
    }
    if (errStatus.length != tPts) {
      MipavUtil.displayError("time.length = " + time.length +
                             " but errStatus.length = " + errStatus.length);
      return;
    }
  ...
  private void dqext() {
    ...
    estErr[it] = Math.abs(result[it] - res3la[2]) +
                 Math.abs(result[it] - res3la[1]) +
   ...
   */
  public abstract void fitToLaplace(double realIn, double imagIn,
                                    double realOut[], double imagOut[]);
}
