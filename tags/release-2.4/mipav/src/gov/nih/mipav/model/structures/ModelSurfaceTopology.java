...
/**
 * This is an implementation of a vertex-edge-triangle table.  It uses hash
 * maps for storing the mesh components.  Basic operations are supported
 * including insertion and removal of triangles and accessing the mesh
 * components and their adjacent components.  More complex operations are
 * also supported including determination if the mesh is connected, closed,
 * or manifold, and allows for computing the connected components of a mesh.
 * With additional computation time, the connected components can be created
 * so that the triangles within each component are consistely ordered.  See
 * the document
 * <a href=../../../LevelSetExtraction.pdf>Level Set Extraction</a>
 * for more details on vertex-edge-triangle tables.
 */

public class ModelSurfaceTopology
{
    /**
     * The default constructor whose job is solely to allocate the hashmaps
     * for the vertex-edge-triangle table.
     */
     ...
     */
    public ModelSurfaceTopology (int iVCapacity, float fVLoad, int iECapacity,
        float fELoad, int iTCapacity, float fTLoad)
    {
    ...
    /**
     * A factory that is used for operations that create new meshes from the
     * current one.  This allows derived class construction within the base
     * class operations.
     *
     * @return a new vertex-edge-triangle table of the correct derived-class
     *         type
     */
    ...
    /**
     * Insert triangle <V0,V1,V2> into the mesh.  The ordering of the indices
     * is relevant.  Triangle <V0,V2,V1> is considered to be a different
     * triangle (reversed ordering from <V0,V1,V2>).
     *
    ...
    /**
     * Remove triangle <V0,V1,V2> from the mesh.  The ordering of the indices
     * is relevant.  Triangle <V0,V2,V1> is considered to be a different
     * triangle (reversed ordering from <V0,V1,V2>).
     *
     * @param iV0 index of triangle
    ...
    /**
     * Access the hash map of vertices.  This is useful for allowing an
     * application to iterate over the vertex/attribute pairs.
     *
    ...
    /**
     * Access the hash map of edges.  This is useful for allowing an
     * application to iterate over the edge/attribute pairs.
     *
    ...
    /**
     * Access the hash map of triangles.  This is useful for allowing an
     * application to iterate over the triangle/attribute pairs.
     *
    ...
    /**
     * Determine if the triangle mesh is manifold.  An edge is a manifold
     * edge if it has at most two triangles sharing it.
     *
    ...
    /**
     * Determine if the triangle mesh is closed.  Each edge must have
     * exactly two triangles sharing it
     *
    ...
    public boolean isConnected ()
    {
        ...
        while ( !kStack.empty() )
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                switch ( i )
                {
                    case 0: iV0 = kT.m_iV0; iV1 = kT.m_iV1; break;
                    case 1: iV0 = kT.m_iV1; iV1 = kT.m_iV2; break;
                    case 2: iV0 = kT.m_iV2; iV1 = kT.m_iV0; break;
                }
    ...
    /**
     * Construct the connected components of the mesh.  The triangle
     * ordering within a single component is not guaranteed to be consistent.
     * That is, it is possible for a triangle to be counterclockwise ordered,
     * but an adjacent triangle to be clockwise ordered.
     *
    ...
    /**
     * Construct the connected components of the mesh by factoring the
     * connectivity array into disjoint subarrays.  The triangle ordering
     * within a single component is not guaranteed to be consistent.  That
     * is, it is possible for a triangle to be counterclockwise ordered, but
     * an adjacent triangle to be clockwise ordered.
     *
     * @param kIndex - An array whose entries indicate the starting point of each
     *   subarray in the returned integer array.  Each entry is of type
     *   Integer.  kIndex[0] always has value 0 since the first subarray
     *   always starts at the beginning.  The next subarray is at kIndex[1].
     *   The number of elements in the first subarray is kIndex[1]-kIndex[0].
     *   The last entry kIndex[kIndex.size()-1] stores the total length of
     *   the returned array, so there are only kIndex.size()-1 subarrays.
     * @return An array of indices that contain the disjoint subarrays in
     *   contiguous order.
     */
     ...
     *
     *     ModelSurfaceTopology kTopo = <some mesh topology>;
     ...
     *     {
     *         int iComponentIQuantity =
     *             kTopo.removeComponent(iIQuantity,aiConnect);
     *         iIQuantity += iComponentIQuantity;
     *         kIndex.add(new Integer(iQuantity));
     *         iNumComponents++;
     *     }
     *
     * The components are stored contiguously in aiConnect.  The starting
     * index for component C is stored in the kIndex[C] for 0 <= C <=
     * iNumComponents.  kIndex[iNumComponents+1] stores the length of the
     * connectivity array.
     *
     * @param iOffset  the starting index in aiConnect where the next
     *        component is to be stored
     * @param aiConnect  the array of connectivity indices that will store
     *        all the components
    ...
    /**
     * Construct the connected components of the mesh.  The triangle
     * ordering within a single component is guaranteed to be consistent.
     * That is, all triangles in the component are counterclockwise ordered,
     * or all are clockwise ordered.  It is not possible to make ordering
     * between two components consistent since this requires geometric
     * information about how the components are placed in space relative to
     * each other (requires specification of an eye point).
     *
    ...
    /**
     * Reverse the order of each triangle in the mesh.  If a triangle in the
     * mesh is <V0,V1,V2>, the returned vertex-edge-table contains a
     * corresponding triangle <V0,V2,V1>.
     *
    ...
    /**
     * Print the vertex-edge-triangle table to a text file.  This function
     * is provided for debugging support and is not needed by the triangle
     * decimation scheme or surface visualizer.
     *
     * @param kFilename the name of the file to which the table should be
     *   printed
     * @exception IOException if the specified file could not be opened for
     *   writing
     */
    public void print (String kFilename)
        throws IOException
    {
        try
        {
    ...
    /**
     * For debugging and testing.  A simple class for storing information
     * about the mesh including the number of vertices in the mesh, the
     * number of edges in the mesh, number of triangles in the mesh, the
     * average number of edges per vertex, the average number of triangles
     * per vertex, the average number of triangles per edge, the maximum
     * number of edges for a vertex, the maximum number of triangles for a
     * vertex, and the maximum number of triangles for an edge.
     */
    static public class Statistics
    {
        /**
         * The constructor and only member function for the class.  The
         * statistics gathering is all done by the constructor.
         *
         * @param kTopo  the mesh topology object whose statistics need to
         *        be calculated
         */
    ...
    /**
     * A representation of a vertex for the vertex-edge-triangle table.  This
     * class just stores the index of the vertex but is unconcerned about the
     * actually vertex 3D coordinate.  The class extends Object to obtain
    ...
    public class Edge extends Object
    {
         ...
         *   expensive test (V0 == other.V0 && V1 == other.V1) ||
     ...
     * This class stores the triple of vertex indices for the vertices of the
     * triangle.  The triangles <V0,V1,V2> and <V0,V2,V1> are considered to
     * be distinct triangles.  The class extends Object to obtain support for
     * hashing into a map of triangles.
     */
    ...
    /**
     * Stub provided for overriding in derived classes.  Each time a vertex
     * is attempted to be inserted into a map, this function is called so
     * that the derived class can perform actions specific to the application.
     *
     * @param kV the vertex that was attempted to be inserted into the map
     * @param bCreate true if the vertex did not exist in the map before
     *   insertion (it is a brand new vertex), false if the vertex already
     *   existed.
     * @param kVAttr the attribute for the vertex
     */
    public void OnVertexInsert (Vertex kV, boolean bCreate,
        VertexAttribute kVAttr)
    {
        // nothing to do
    }
    ...
    /**
     * Stub provided for overriding in derived classes.  Each time a vertex
     * is attempted to be removed from a map, this function is called so that
     * the derived class can perform actions specific to the application.
     *
     * @param kV the vertex that was attempted to be removed from the map
     * @param bDestroy true if the vertex did exist in the map before the
     *   attempted removal and no other mesh components are referencing the
     *   vertex, false if the vertex does exist in the map, but other mesh
     *   components are referencing it.
     * @param kVAttr the attribute for the vertex
     */
    public void OnVertexRemove (Vertex kV, boolean bDestroy,
        VertexAttribute kVAttr)
    {
        // nothing to do
    }
    ...
    /**
     * Stub provided for overriding in derived classes.  Each time an edge
     * is attempted to be inserted into a map, this function is called so
     * that the derived class can perform actions specific to the application.
     *
     * @param kE the edge that was attempted to be inserted into the map
     * @param bCreate true if the edge did not exist in the map before
     *   insertion (it is a brand new edge), false if the edge already
     *   existed.
     * @param kEAttr the attribute for the edge
     */
    public void OnEdgeInsert (Edge kE, boolean bCreate,
        EdgeAttribute kEAttr)
    {
        // nothing to do
    }
     ...
     * @param bDestroy true if the edge did exist in the map before the
     *   attempted removal and no other mesh components are referencing the
     *   edge, false if the edge does exist in the map, but other mesh
     *   components are referencing it.
     * @param kEAttr the attribute for the edge
     */
    public void OnEdgeRemove (Edge kE, boolean bDestroy,
        EdgeAttribute kEAttr)
    {
        // nothing to do
    }
    ...
    /**
     * Stub provided for overriding in derived classes.  Each time a triangle
     * is attempted to be inserted into a map, this function is called so
     * that the derived class can perform actions specific to the application.
     *
     * @param kT the triangle that was attempted to be inserted into the map
     * @param bCreate true if the triangle did not exist in the map before
     *   insertion (it is a brand new triangle), false if the triangle already
     *   existed.
     * @param kTAttr the attribute for the triangle
     */
    public void OnTriangleInsert (Triangle kT, boolean bCreate,
        TriangleAttribute kTAttr)
    {
        // nothing to do
    }
     ...
     * @param bDestroy true if the triangle did exist in the map before the
     *   attempted removal and no other mesh components are referencing the
     *   triangle, false if the triangle does exist in the map, but other mesh
     *   components are referencing it.
     * @param kTAttr the attribute for the triangle
     */
    public void OnTriangleRemove (Triangle kT, boolean bDestroy,
        TriangleAttribute kTAttr)
    {
        // nothing to do
    }
    ...
    /**
     * The attributes associated with a vertex.  These include the set of
     * edges sharing the vertex and the set of triangles sharing the vertex.
     * The data member m_kData allows an application to attach whatever data
     * it wants associated with the vertex.  The attachment/detachment is
     * performed by the application in the OnVertexInsert/OnVertexRemove
     * callbacks.
     */
    protected class VertexAttribute
    {
        /**
         * Construct an attribute for a vertex.  The sets of edges and
         * triangles are initially empty.
         */
        ...
        /**
         * Test if the sets of adjacent edges and triangles are empty.  The
         * function is used to determine the bCreate/bDestroy parameters in
         * the OnVertexInsert/OnVertexRemove callbacks.
         *
    ...
    /**
     * The attributes associated with an edge.  These include the set of
     * triangles sharing the edge.  The data member m_kData allows an
     * application to attach whatever data it wants associated with the edge.
     * The attachment/detachment is performed by the application in the
     * OnEdgeInsert/OnEdgeRemove callbacks.
     */
    protected class EdgeAttribute
    {
        /**
         * Construct an attribute for an edge.  The set of triangles is
         * initially empty.
         */
        ...
        /**
         * Test if the set of adjacent triangles is empty.  The function is
         * used to determine the bCreate/bDestroy parameters in the
         * OnEdgeInsert/OnEdgeRemove callbacks.
         *
    ...
    /**
     * The attributes associated with a triangle.  The triangle does not keep
     * track of any adjacencies (these can be indirectly determined from other
     * mesh components).  The data member m_kData allows an application to
     * attach whatever data it wants associated with the triangle.  The
     * attachment/detachment is performed by the application in the
     * OnTriangleInsert/OnTriangleRemove callbacks.
     */
