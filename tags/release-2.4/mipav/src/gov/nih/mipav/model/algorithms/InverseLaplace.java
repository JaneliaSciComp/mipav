...
import gov.nih.mipav.view.Preferences;
 ...
 /**
  *  This is a port of the MATLAB numerical inverse Laplace transform found
  *  in Hollenbeck, K. J. (1998) INVLAP.M : A matlab function for numerical
  *  inversion of Laplace transforms by the de Hoog algorithm,
  *  http://www.isva.dtu.dk/staff/karl/invlap.htm
  *
  *  The algorithm used is deHoog et al's quotient difference method with
  *  accelerated convergence for the continued fraction expansion published in
  *  "An improved method for numerical inversion of Laplace transforms" by
  *  F. R. de Hoog, J. H. Knight, and A. N. Stokes, SIAM Journal on Scientific
  *  and Statistical Computing, Vol. 3, No. 3, September, 1982, pp. 357-366.
  *
  *  Hollenbeck algorithm modification: The time vector is split in segements
  *  of equal magnitude which are inverted individually.  This gives better
  *  overall accuracy.
  */
...
public abstract class InverseLaplace {
   ...
   private double time[];
   // Don't include a time == 0.  This will give a -inifinity when the log of
   // the time is taken.
   // largest pole of Laplace space function (default zero)
   // Note that InverseLaplace may give incorrect results if the default value
   // of largestPole is used.  For example, the function exp(t) with the
   // Laplace transform 1/(s-1) will give correct results if largestPole is
   // set to 1.0, but will return incorrect negative values for time >= 1.0 if
    ...
    */
	public void driver() {
	    int i, j;
            ...
            double realS;
            int M = 50;
            double imagS[] = new double[2*M+1];
            double realE[][] = new double[2*M+1][M+1];
            double imagE[][] = new double[2*M+1][M+1];
            double realQ[][] = new double[2*M][M+1];
            double imagQ[][] = new double[2*M][M+1];

            double realD[] = new double[2*M+1];
            double imagD[] = new double[2*M+1];
            double realA[][] = null;
            double imagA[][] = null;
            double realB[][] = null;
            double imagB[][] = null;
                  ...
                  for (iLogT = iMinLogT; iLogT <= iMaxLogT; iLogT++) {
                    ...
                    if (segPoints > 0) {
                      ...
                      for (i = 1; i <= 2*M; i++) {
                        ...
                        realQ[i-1][1] = (a[i][0]*a[i-1][0] + a[i][1]*a[i-1][1])/
                                        denom;
                        imagQ[i-1][1] = (a[i][1]*a[i-1][0] - a[i][0]*a[i-1][1])/
                                        denom;
                      }
                      for (r = 2; r <= M+1; r++) {
                        for (j = 0; j <= 2*(M-r+1); j++) {
                          realE[j][r-1] = realQ[j+1][r-1] - realQ[j][r-1]
                                          + realE[j+1][r-2];
                          imagE[j][r-1] = imagQ[j+1][r-1] - imagQ[j][r-1]
                                          + imagE[j+1][r-2];
                        }
                        if (r < (M+1)) {
                          rq = r+1;
                          for (j = 0; j <= 2*(M-rq+1)+1; j++) {
                            ...
                            denom = realE[j][rq-2]*realE[j][rq-2]
                                  + imagE[j][rq-2]*imagE[j][rq-2];
                            realQ[j][rq-1] = (realProd*realE[j][rq-2] +
                                              imagProd*imagE[j][rq-2])/denom;
                            imagQ[j][rq-1] = (imagProd*realE[j][rq-2] -
                                              realProd*imagE[j][rq-2])/denom;
                          } // for (j = 0; j <= 2*(M-rq+1)+1; j++)
                        } // if (r < (M+1))
                      } // for (r = 2; r <= M+1; r++)
                      ...
                      for (n = 2; n <= 2*M+1; n++) {
                        for (j = 0; j < segPoints; j++) {
                          ...
                          realProd2 = realProd*realA[n-2][j] -
                                      imagProd*imagA[n-2][j];
                          imagProd2 = realProd*imagA[n-2][j] +
                                      imagProd*realA[n-2][j];
                          realA[n][j] = realA[n-1][j] + realProd2;
                          imagA[n][j] = imagA[n-1][j] + imagProd2;
                          realProd2 = realProd*realB[n-2][j] -
                                      imagProd*imagB[n-2][j];
                          imagProd2 = realProd*imagB[n-2][j] +
                                      imagProd*realB[n-2][j];
                      ...
                      for (i = 0; i < segPoints; i++) {
                        realH2M = .5 * (1.0 +
                                     (realD[2*M-1] - realD[2*M])*realZ[i]
                                     - (imagD[2*M-1] - imagD[2*M])*imagZ[i]);
                        imagH2M = .5 * ((realD[2*M-1] -realD[2*M])*imagZ[i]
                                     + (imagD[2*M-1] - imagD[2*M])*realZ[i]);
                        ...
                        realDiv = (realZ[i]*realH2MSq + imagZ[i]*imagH2MSq)/
                                  denom;
                        imagDiv = (imagZ[i]*realH2MSq - realZ[i]*imagH2MSq)/
                                  denom;
                        realProd = 1.0 + realD[2*M]*realDiv
                                       - imagD[2*M]*imagDiv;
                        imagProd = realD[2*M]*imagDiv + imagD[2*M]*realDiv;
                        magnitude = Math.sqrt(realProd*realProd
                                              + imagProd*imagProd);
                        ...
                        realDiv = (realA[2*M+1][i]*realB[2*M+1][i] +
                                   imagA[2*M+1][i]*imagB[2*M+1][i])/denom;
                        timeFunction[timeIndex++] =
                            Math.exp(realS*segT[i])* realDiv/T;
                      } // for (i = 0; i < segPoints; i++)
                    } // if (segPoints > 0)
                  } // for (iLogT = iMinLogT; iLogT <= iMaxLogT; iLogT++)
		} // try
  ...
  // Returns 'a' coefficients in power series where the second index is 0 for
  // the real part and 1 for the imaginary part
