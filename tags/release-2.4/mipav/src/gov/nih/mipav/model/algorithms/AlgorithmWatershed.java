...
/**
 *  This program applies the watershed algorithm to the image. It assumes
 *  that the user has identified the starting watershed regions via VOIs.
 *  Future work might be to develop a algorithm that automatically identifies
 *  initial seed points and regions or possibly an automatic method that forms
 *  watersheds - see method at end of file (does not work yet);
 *
...
public class AlgorithmWatershed extends AlgorithmBase {
    ...
    */
    public AlgorithmWatershed(ModelImage destImg, ModelImage srcImg, ModelImage gmImg,
                             float sigmas[], Vector seeds) {
    ...
    */
    ...
    /**
    *   This image is typically the gradient magnitude. Peaks
    *   in this image (function ) are the watersheds boundaries
    ...
    */
    public void runAlgorithm() {
        ...
        if (srcImage  == null) {displayError("Source Image is null"); return;}
        if (destImage  == null) {displayError("Destination Image is null"); return;}
    ...
    /**
    *   Calculates the watershed intialized by the VOI regions. The resultant
    *   watershed regions(basin) are labelled with an integer value. In addition,
    *   each region is separated by connected (8-way) watershed identifier of 1.
    */
    private void calc2D(){
        int nVOI;
        int i, c;
        ...
        if (energyImage == null) {
            try {
                energyImage = new ModelImage(ModelImage.FLOAT, srcImage.getExtents(),
                                            srcImage.getImageName() + "_gm",  srcImage.getUserInterface());
                ...
                energyImage.saveImage(srcImage.getFileInfo(0).getFileDirectory(),
                                      srcImage.getImageName() + "_gm", FileBase.XML, true);
            }
            catch (OutOfMemoryError error){
                if (energyImage != null) energyImage.disposeLocal();
                energyImage = null;
            ...
            for (i = 0; i < nVOI; i++){
                if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                    ...
                    for(c = 0; c < contours[0].size(); c++) {
                        ...
                        try {
                            seedVector.addElement(new Seed (new Point3Df( x,y,0),
        ...
        *
        *       Seed_vector contains the seeds provided by user
        *       current_basin labels a basin corresponding to a seed
        *
        **************************************************************************************/
        Point3Df P;
        int      tempP = 0;
        ...
        while (hQueue.isEmpty() == false){
            ...
            if(destImage.getShort(pixC) == MASK) {
                /*if(currentBasin != destImage.getShort(pixN) &&
                   ...
                   if (destImage.getShort(pixN) >= 0)        destImage.setShort(pixC, destImage.getShort(pixN));
                   else if (destImage.getShort(pixE) >= 0)   destImage.setShort(pixC, destImage.getShort(pixE));
                   else if (destImage.getShort(pixS) >= 0)   destImage.setShort(pixC, destImage.getShort(pixS));
                   else if (destImage.getShort(pixW) >= 0)   destImage.setShort(pixC, destImage.getShort(pixW));
                   else {
                        // maybe add back to queue
                        //destImage.setShort(pixC, (short)50);
    ...
    private void calc3D(){
        ...
        if (energyImage == null) {
            try {
                ...
                gradMagAlgo  = new AlgorithmGradientMagnitudeSep(energyImage, srcImage, sigmas,
                                                        true, false);
                gradMagAlgo.setActiveImage(activeImage);
                gradMagAlgo.run();
        ...
        for (i = 0; i < seedVector.size() && !threadStopped; i++){
            destImage.set((int)((Seed)seedVector.elementAt(i)).point.x,
                          (int)((Seed)seedVector.elementAt(i)).point.y,
                          (int)((Seed)seedVector.elementAt(i)).point.z,
                          ((Seed)seedVector.elementAt(i)).seedValue);
    ...
    */
    /*private void autocalc2D(){
        ...
        //gradMagImage = new ModelImage(ModelImage.FLOAT, srcImage.getExtents(), "Gradient Magnitude", null);
        //gradMagAlgo  = new AlgorithmGradientMagnitudeSep(gradMagImage, srcImage, sigmas, true);
        //gradMagAlgo.run();
        ...
        short    newBasin           = 0;
        short    currentBasin       = 1;
        short    current_distance   = 0;
        int      fictitiousPixel    = -1;
        //Point3Ds P;
        int      tempP = 0;
        int      pixC, pixN, pixS, pixE, pixW;
        //float    tempGM;
        ...
        int      bins  = (int)(srcImage.getMax() - srcImage.getMin());
        //int      bins  = (int)(gradMagImage.getMax() - gradMagImage.getMin());
        int      sensitivity = 75;
        //bins  = 1000;

        IntVector pixelsVector  = new IntVector(10000, 5000);
        IntVector queue         = new IntVector();
        IntVector stack         = new IntVector();
        //HQueue   hQueue = new HQueue((float)gradMagImage.getMin(),
        //                             (float)gradMagImage.getMax(),
        //                             bins);

        HQueue   hQueue = new HQueue((float)srcImage.getMin(),
                                     (float)srcImage.getMax(),
                                     bins);
        ...
        for (i = 0; i < length; i++){
            if ( i%xDim == 0 || i%xDim == (xDim-1)){
                ...
                destImage.setShort(i, INITIAL);
                hQueue.add(i, srcImage.getFloat(i));
            }
        }
        ...
        for (i=0; i < bins-sensitivity; i+=sensitivity ){
            ...
            // i to i+n to MASK
            while (pixelsVector.isEmpty() == false){

                tempP = pixelsVector.lastElement();
                pixelsVector.removeLastElement();
                ...
                if (destImage.getShort(pixC) == BOUNDARY){
                }
                else {
                    destImage.setShort(pixC, MASK);
                    if ((destImage.getShort(pixN) > 0) ||
                        (destImage.getShort(pixS) > 0) ||
                        (destImage.getShort(pixE) > 0) ||
                        (destImage.getShort(pixW) > 0)) {
                        distance[pixC]= 1;
                        // distance of a path connected pixC to the nearest flooded pixel
                        // such that the nodes(pixels) of the path are at the same level,
            ...
            current_distance = 1;
            queue.addElement(fictitiousPixel);
            while (true) {
                tempP = queue.firstElement();
                queue.removeElementAt(0);
                if (tempP == fictitiousPixel){
                ...
                if (destImage.getShort(pixW) > 0 && distance[pixW] < current_distance){
                    if (destImage.getShort(pixC) > 0 ) {
                        if (destImage.getShort(pixC) != destImage.getShort(pixW)){
                            destImage.setShort(pixC, WSHED);
                        }
                        ...
                        destImage.setShort(pixC, destImage.getShort(pixW));
                        currentBasin = destImage.getShort(pixW);           // extend basin to center;
                    }
                }
                else if (destImage.getShort(pixW) == MASK && distance[pixW] == 0){
                    distance[pixW] =(short)(current_distance + 1);
                    queue.addElement(pixW);
                }


                if (destImage.getShort(pixE) > 0 && distance[pixE] < current_distance){
                    if (destImage.getShort(pixC) > 0 ) {
                        if (destImage.getShort(pixC) != destImage.getShort(pixE)){
                            destImage.setShort(pixC, WSHED);
                        }
                ...
                else if (destImage.getShort(pixE) == MASK && distance[pixE] == 0){
                    distance[pixE] = (short)(current_distance + 1);
                    queue.addElement(pixE);
                }


                if (destImage.getShort(pixN) > 0 && distance[pixN] < current_distance){
                    if (destImage.getShort(pixC) > 0 ) {
                        if (destImage.getShort(pixC) != destImage.getShort(pixN)){
                            destImage.setShort(pixC, WSHED);
                        }
                ...
                else if (destImage.getShort(pixN) == MASK && distance[pixN] == 0){
                    distance[pixN] = (short)(current_distance + 1);
                    queue.addElement(pixN);
                }


                if (destImage.getShort(pixS) > 0 && distance[pixS] < current_distance){
                    if (destImage.getShort(pixC) > 0 ) {
                        if (destImage.getShort(pixC) != destImage.getShort(pixS)){
                            destImage.setShort(pixC, WSHED);
                        }
                ...
                else if (destImage.getShort(pixS) == MASK && distance[pixS] == 0){
                    distance[pixS] = (short)(current_distance + 1);
                    queue.addElement(pixS);
                }
            }

             while(hQueue.isEmptyAt(i) == false) {
                tempP = hQueue.firstAt(i);
                distance[tempP]=0;
                if (destImage.getShort(tempP) == MASK){
                    destImage.setShort(tempP, newBasin++);
                    stack.push(tempP);
                    while (stack.isEmpty() == false){
                        ...
                        if (destImage.getShort(pixN) == MASK) {
                            stack.push(pixN);
                            destImage.setShort(pixN, newBasin);
                        }
                        if (destImage.getShort(pixS) == MASK) {
                            stack.push(pixS);
                            destImage.setShort(pixS, newBasin);
                        }
                        if (destImage.getShort(pixE) == MASK) {
                            stack.push(pixE);
                            destImage.setShort(pixE, newBasin);
                        }
                        if (destImage.getShort(pixW)  == MASK){
                            stack.push(pixW);
        ...
        short tempInt;
        for (i = 0; i < length; i++) {
            if (destImage.getShort(i) == MASK){
                destImage.setShort(i, (short)0);
            }
            else if (destImage.getShort(i) == BOUNDARY){
                destImage.setShort(i, (short)0);
            }
            else if (destImage.getShort(i) == WSHED){
                destImage.setShort(i, (short)100);
            }
            else if (destImage.getShort(i) < 0){
                destImage.setShort(i, (short)0);
            }
            else {
                tempInt = destImage.getShort(i);
                destImage.setShort(i, (short)(tempInt+5));
            }
        }

        destImage.calcMinMax();
//        gradMagImage.disposeLocal();
        hQueue.dispose();
        setCompleted(true);
        //progressBar.dispose();
    }
