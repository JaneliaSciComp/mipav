...
/**
 * This class provides an implementation of a second method for segmentation of
 * the brain from a 3D MRI, as opposed to the BET algorithm implemented in
 * AlgorithmBrainExtractor.  The algorithm is partially based on the paper
 * describing the Brain Surface Extraction (BSE) algorithm:
 * <pre>
...
public class AlgorithmBrainSurfaceExtractor extends AlgorithmBase implements AlgorithmInterface {
    ...
    /**
     * The proportion of the total number of pixels in the volume that a region
     * should be before we decide that it's probably the brain.
     */
     ...
     */
    public AlgorithmBrainSurfaceExtractor(ModelImage srcImg, int fIter, float fKernel, float eKernel,
                                          boolean ero25D, int eroIter, float cKernel, int cIter,
                                          boolean showIntermediate, boolean noHoles, boolean doSep,
    ...
    private void constructLog() {
        historyString = new String( "BrainSurfaceExtractor(" + filterIterations + ", " +
                                    filterKernel + ", " + edgeKernel + ", " +
                                    erosion25D + ", " + erosionIterations + ", " +
    ...
    public void extractBrain() {
        ...
        float normFactor = 1;
        float edgeSigmas[] = new float[] {
            edgeKernel, edgeKernel, edgeKernel * normFactor};
        ...
        if (filterIterations > 0) {
            resultImage = new ModelImage(ModelStorageBase.FLOAT, image.getExtents(), imgName + "_temp_results",
                                         userInterface);
            AlgorithmRegularizedIsotropicDiffusion filterAlgo = new
                AlgorithmRegularizedIsotropicDiffusion(resultImage, image,
                filterIterations, filterKernel,
                filterContrast, do25D);
        ...
        if (!erosion25D) {
            ...
            erodeAlgo = new AlgorithmMorphology3D(resultImage, erosionKernel, 0,
                                                  AlgorithmMorphology3D.ERODE, 0, erosionIterations, 0, 0, regionFlag);
        }
        else {
            int erosionKernel = AlgorithmMorphology25D.CONNECTED4;
            erodeAlgo = new AlgorithmMorphology25D(resultImage, erosionKernel, 0,
                AlgorithmMorphology25D.ERODE, 0, erosionIterations, 0, 0, regionFlag);
        }
        ...
        if (!erosion25D) {
            int erosionKernel = AlgorithmMorphology3D.CONNECTED6;
            dilateAlgo = new AlgorithmMorphology3D(resultImage, erosionKernel, 0,
                AlgorithmMorphology3D.DILATE, erosionIterations, 0, 0, 0, regionFlag);
        }
        else {
            int erosionKernel = AlgorithmMorphology25D.CONNECTED4;
            dilateAlgo = new AlgorithmMorphology25D(resultImage, erosionKernel, 0,
                AlgorithmMorphology25D.DILATE, erosionIterations, 0, 0, 0, regionFlag);
        }
        ...
        AlgorithmMorphology25D closeAlgo = new AlgorithmMorphology25D(
            resultImage, closeKernel, closeKernelSize,
            AlgorithmMorphology25D.CLOSE, closeIterations + 1, closeIterations, 0, 0,
            regionFlag);
     ...
     * @return  the paint mask bitset representing the area not
     *          removed from the image during the brain extraction 
     */
     ...
     */
    private static final ModelImage addPadding(ModelImage img) {
        ...
        ModelImage paddedImg = new ModelImage( img.getType(), newExtents, img.getImageName() + "_pad", img.getUserInterface() );
     ...
     */
    private static final ModelImage removePadding( ModelImage paddedImg ) {
        int[] extents = new int[3];
        extents[0] = paddedImg.getExtents()[0];
        extents[1] = paddedImg.getExtents()[1];
        extents[2] = paddedImg.getExtents()[2] - 4;
        ModelImage img = new ModelImage( paddedImg.getType(), extents, paddedImg.getImageName() + "_nopad", paddedImg.getUserInterface() );
