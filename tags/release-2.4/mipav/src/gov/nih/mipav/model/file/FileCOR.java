...
*  Suppose the file base name is table.  There is a header file named
*  table-.info/and or table-.info~ which has the header information in ascii.
*  A separate unsigned byte file exists for each slice of the 3D image -
*  table-001, table-002, and so on until the last slice.  The default is to
*  have xDim = yDim = zDim = 256 and xResolution = yResolution = zResolution =
*  0.001 meter.  Note that all distances in a COR file represent meters.
*
*  The header fields are as follows:
*  imnr0 the first slice number
*  imnr1 the last slice number
*  ptype the data type; 2 for unsigned byte
*  x the x dimension
*  y the y dimension
*  fov the field of view
*  thick the z resolution
*  psiz = a common value for both the x and y resolutions
*  locatn location
*  strtx the minimum x value in meters
*  endx the maximum x value in meters
*  strty the minimum y value in meters
*  endy the maximum y value in meters
*  strtz the minimum z value in meters
*  endz the maximum z value in meters
*  tr T_r
*  te T_e
*  ti T_i
*  xform transform file
*  ras_good_file = 1 if the orientation fields are set, 0 otherwise
*  x_ras x axis orientation
*  y_ras y axis orientation
*  z_ras z axis orientation
*  c_ras volume center coordinates
*  If orientation information is missing, the file is assumed to be coronal
*  with z axis = R to L, y axis = S to I, and z axis = P to A.

*  readInfoImage reads in the header information from the set of FreeSurfer
*  COR volume images and then readImage is called once for each slice that
*  must be read from a COR image file set.
*/
...
public class FileCOR extends FileBase{
    ...
    */
    private String readLine() throws IOException {
        ...
        while ((tempString == null) && (raFile.getFilePointer() < raFile.length() - 1)
               && (!foundEOF)) {
            try {
            ...
            if (parseString[0].equalsIgnoreCase("x_ras")) {
                if (parseString.length == 4) {
                    ...
                    if ((xr[0] == -1.0f) && (xr[1] == 0.0f) && (xr[2] == 0.0f)) {
                        ...
                        raFile.close();
                        throw new IOException("Illegal x_ras values of " + xr[0] + " " +
                                               xr[1] + " " + xr[2]);
                    }
                }
                else {
            ...
            // Read in y axis orientation
                ...
                if (parseString.length == 4) {
                    ...
                    if ((yr[0] == -1.0f) && (yr[1] == 0.0f) && (yr[2] == 0.0f)) {
                        ...
                        raFile.close();
                        throw new IOException("Illegal y_ras values of " + yr[0] + " " +
                                               yr[1] + " " + yr[2]);
                    }
                }
                else {
            ...
            // Read in z axis orientation
                ...
                if (parseString.length == 4) {
                    ...
                    if ((zr[0] == -1.0f) && (zr[1] == 0.0f) && (zr[2] == 0.0f)) {
                        ...
                        raFile.close();
                        throw new IOException("Illegal z_ras values of " + zr[0] + " " +
                                               zr[1] + " " + zr[2]);
                    }
                }
                else {
    ...
    /**
    *                           This method reads one slice from the file
    *                           into byteBuffer and then transfers into
    *                           the float array imgBuffer
    ...
    */
    public void writeImage(ModelImage image, FileWriteOptions options) throws IOException {
        ...
        progressBar = new ViewJProgressBar(fileName, "Writing COR file...",
        ...
        // Otherwise, it is 0.
        if ((image.getNDims() == 3) &&
            (image.getFileInfo(0).getAxisOrientation()[0] !=
            FileInfoBase.ORI_UNKNOWN_TYPE) &&
            (image.getFileInfo(0).getAxisOrientation()[1] !=
            FileInfoBase.ORI_UNKNOWN_TYPE) &&
            (image.getFileInfo(0).getAxisOrientation()[2] !=
            FileInfoBase.ORI_UNKNOWN_TYPE)) {
            line[14] = 49; // 1
        }
        else {
        ...
        for (i = 0; i < image.getNDims(); i++) {
            if (image.getFileInfo(0).getAxisOrientation()[i] !=
                FileInfoBase.ORI_UNKNOWN_TYPE) {
                line = new byte[19];
                switch(i) {
                    case 0:
