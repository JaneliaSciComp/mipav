...
/**
*   Dialog to set the image to Dicom order
*
*   ** replaces image
*
...
public class JDialogDicomOrder extends JDialogBase implements AlgorithmInterface, ScriptableInterface {
    ...
    */
    public void constructDialog() {
        ...
        gbc.gridwidth = 1; gbc.gridheight = 1; gbc.anchor = gbc.WEST; gbc.insets = new Insets(5,5,5,5);

        gbc.gridx = 0; gbc.gridy = 0; gbc.weightx = 0; gbc.fill = gbc.NONE;
        orientPanel.add(labelOrientX, gbc);
        gbc.gridx = 1; gbc.gridy = 0; gbc.weightx = 1; gbc.fill = gbc.HORIZONTAL;
        orientPanel.add(comboBoxOrientX, gbc);
        gbc.gridx = 0; gbc.gridy = 1; gbc.weightx = 0; gbc.fill = gbc.NONE;
        orientPanel.add(labelOrientY, gbc);
        gbc.gridx = 1; gbc.gridy = 1; gbc.weightx = 1; gbc.fill = gbc.HORIZONTAL;
        orientPanel.add(comboBoxOrientY, gbc);
        gbc.gridx = 0; gbc.gridy = 2; gbc.weightx = 0; gbc.fill = gbc.NONE;
        orientPanel.add(labelOrientZ, gbc);
        gbc.gridx = 1; gbc.gridy = 2; gbc.weightx = 1; gbc.fill = gbc.HORIZONTAL;
   ...
   */
   public ModelImage getResultImage() {
    ...
    /**
    *   Sets up the axis orientations based on what the user entered
    *   in the combo boxes.
    *   @return <code>true</code> if the selections make sense (that is,
    *           are consistent with each other); otherwise <code>false</code>.
    */
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        if (algorithm instanceof AlgorithmDicomOrder) {
            if (algorithm.isCompleted() == true) {
                ...
                Point pt;
                if (parentFrame != null) pt = ((ViewJFrameBase)parentFrame).getLocation();
                else pt = new Point(Toolkit.getDefaultToolkit().getScreenSize().width/2, Toolkit.getDefaultToolkit().getScreenSize().height/2);

                imageFrame = new ViewJFrameImage(resultImage, null, new Dimension(pt.x, pt.y));

                insertScriptLine(algorithm);

                if (doClose && (parentFrame != null)) ((ViewJFrameBase)parentFrame).close();
                ...
                if(image.getLightBoxFrame() != null) {
                    ...
                    try {
                        ...
                        lightBoxFrame = new ViewJFrameLightBox(imageFrame, "LightBox", resultImage, imageFrame.getComponentImage().getLUTa(),
                                                imageFrame.getComponentImage().getImageB(),imageFrame.getComponentImage().getLUTb(),
                                                imageFrame.getComponentImage().getResolutionX(), imageFrame.getComponentImage().getResolutionY(),
