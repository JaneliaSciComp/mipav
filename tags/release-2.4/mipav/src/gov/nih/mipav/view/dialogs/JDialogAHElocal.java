 ...
 *   <p>
 *   In should be noted, that the algorithms are executed in their own
 *   thread.
 *
 ...
 *
 */
public class JDialogAHElocal extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "AHElocal " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
                    userInterface.getScriptDialog().putVar( resultImage.getImageName() );
                    userInterface.getScriptDialog().append(
                            userInterface.getScriptDialog().getVar( resultImage.getImageName() ) + " " + clamp + " "
                            + clampValue + " " + kernelSize + " " + kernelShape + " " + threshold + " "
                            + minThresholdValue + " " + wholeImageSelected + " " + scaleMaxValue + " " + red + " "
                            + green + " " + blue + "\n" );
                } else {
                    userInterface.getScriptDialog().append(
                            userInterface.getScriptDialog().getVar( image.getImageName() ) + " " + clamp + " "
                            + clampValue + " " + kernelSize + " " + kernelShape + " " + threshold + " "
                            + minThresholdValue + " " + wholeImageSelected + " " + scaleMaxValue + " " + red + " "
                            + green + " " + blue + "\n" );
                }
    ...
    /**
     *	Sets up the GUI (panels, buttons, etc)
     ...
     *	@param flag		<code>true</code> indicates wholeImageSelected,
    ...
    /**
     *   Creates the panel that allow user to select the
     *   kernel size and shape of the image when building the histogram.
    ...
    /** Creates the editable text comboBox that allows user to
     *   define the number of neighboring pixels
     *   used for the histogram.
     *   the number given may only be odd, as this
     *   is the total number of pixels chosen on
     *   both sides.  For any pixel, there are
     *   floor(<code>kernelSize</code>/2) pixels
     *   in either the horizontal or vertical direction.
     *   Alternatively, it describes a box of the number
     *   here on a side, for which the pixel in question
     *   is at the center.
     *   The drop-box has pre-selected: <ul>
     *   <li>21</li>
     *   <li>45</li>
     *   <li>65</li>
     *   <li>129</li>
     ...
     *   <p>
     *   builds the label to be used, applies it to the
     *   left side of the JPanel <i>holder</i>, then
     *   builds the drop-box so the user may make
     *   a selection, and applies it to the far-right
     *   of the JPanel <i>holder</i>.
     */
    protected void buildKernelSize( JPanel holder, GridBagConstraints gbc,
            GridBagLayout gbl ) {
    ...
    /** Creates the comboBox that allows user to
     *   define the way the neighboring pixels
     *   are chosen for the histogram.
     *   The kernel-shape may be <ul>
     *   <li>square</li>
     ...
     *   of the JPanel <i>holder</i>.
     */
    protected void buildKernelShape( JPanel holder, GridBagConstraints gbc,
            GridBagLayout gbl ) {
     ...
     */
    protected void buildScaleMax( JPanel holder, GridBagConstraints gbc,
            GridBagLayout gbl ) {
    ...
    /** Creates the txt-box that allows user to
     *   insert the bottom-end pixels to skip.
     *   Number entered into the textfield is minimum
     *   value of the pixels considered.  Defaults
     *   to the image minimum.
     */
    protected void buildThreshold( JPanel holder, GridBagConstraints gbc,
            GridBagLayout gbl ) {
        if ( isColorImage ) {
    ...
    /** controls the entire creation of the clamping panel
     *   and all controls related to the display of the
     *   clamping parameter.  places the clamping panel
     *   into the <i>holder</i> panel.
     */
    protected void buildClamping( JPanel holder,
            GridBagConstraints gbc, GridBagLayout gbl ) {
        ...
        clampValueText.setMaximumSize( clampValueText.getPreferredSize() ); // don't let it get any bigger than what it prefers
        clampValueText.setFont(
                serif12 );
    ...
    /** builds the panel which allows user to select
     *   which portion of the image the algorithm is
     *   to operate on:<ul>
     *       <li>the entire image</li>
     *       </li>the currently highlighted volume of interest</li>
     *   </ul>Although specified elsewhere, it should
     *   be noted that if VOI is chosen, although no
     *   currently selected VOI exists, an error notice
     *   will be posted to the user at the time of algorithm
     *   indicating that no VOI had been selected.
    ...
    /** builds the panel which allows user to select
     *   which where the algorithm is to display the
     *   output, or resultant, image:<ul>
    ...
    /**
     *	Once all the necessary variables are set, call the local AHE
    ...
    public void algorithmPerformed( AlgorithmBase algorithm ) {
        ...
        if ( algorithm instanceof AlgorithmAHElocal ) {
            ...
            if ( aheAlgo.isCompleted() && resultImage != null ) {
                ...
                //
                //if (clamp) {
                //    image.setImageName("CLAHElocal ("+shape+", "+kernelSize+", "+ clampValue+ "%): "
                //                +image.getImageName());
                //}
                //else {      // not clipping the histogram
