...
/**
*       This is a custom made Swing component.  It is a graph that takes
*       a panel in the constructor and then draws itself.  The graph has
*       the option of labels for the x and y axis, which are set with an
*       accessor.  Gridlines can also be turned on and off.  The function
*       calling ViewJComponentGraph may specify a number of gridlines to
*       have on the graph.  The number of gridlines is the same as the number
*       of tick marks.  Points are plotted in the graph by calling an
*       accessor with an array of x coordinates and an array of y coordinates.
*       The number of coordinates in each array must be equal.
*
...
*
*/
public class ViewJComponentGraph extends JComponent
    ...
    */
    ...
    /**
    *  Paints the graph.  Checks if the labels are defined and
    *  draws them, checks if the gridlines should be shown and
    ...
    */
    public void paintComponent(Graphics g){
        ...
        g.fill3DRect(graphBounds.x, graphBounds.y, graphBounds.width, graphBounds.height, true);
        ...
        */
        ...
        if (showLegend){
            ...
            for(int j=0; j<numf; j++) if (maxNameLength<fm.stringWidth(name[j])) maxNameLength=fm.stringWidth(name[j]);
            ...
            if (maxNameLength<40) maxNameLength=40;
            ...
            for(int i = 0; i<numf; i++)
            {
              ...
              g.setColor(Color.black);
              if (i==0) g.fillRect(bounds.width-153,23+(14*i),3,3);
              if (i==1) fun2Icon.paintIcon(null,  g, bounds.width-155,19+(14*i));
              if (i==2) fun3Icon.paintIcon(null,  g, bounds.width-155,19+(14*i));
              if (i==3) fun4Icon.paintIcon(null,  g, bounds.width-155,19+(14*i));
              if (i==4) fun5Icon.paintIcon(null,  g, bounds.width-155,19+(14*i));
        ...
        if (showGridLines){
            ...
            for (int i=1; i<xGridLines; i++) {
                g.drawLine(graphBounds.x + (int)Math.round(i*xTick),
                           graphBounds.y - 5,
                           graphBounds.x + (int)Math.round(i*xTick),
    ...
    */
    public void pasteFunct(){
        ...
        if (functions.length != 0 && ( copiedXs.length != functions[0].getOriginalXs().length ||
        ...
        try {
            ...
            tempFittedFuncts[functions.length]   = new ViewJComponentFunct(copiedXs, copiedYs, copiedFunct.getColor(), null);
    ...
    */
    ...
    /*
    * Calculates the default range and domain
    * (according to min and max x and y values).
    * Modifies minRange, maxRange, minDomain, maxDomain accordingly.
    */
    public void calculateDefaultRangeDomain(){
       ...
       for (i=0; i<functions.length; i++) {
           if (functions[i].getXs().length != functions[i].getYs().length) return;
       ...
       for (int k=0; k<functions.length; k++) {
           if (functions[k].getFunctionVisible()) {
                for (i=0; i<functions[k].getXs().length; i++){
                    functVisible = true;
                    if (functions[k].getXs()[i] < xMin) xMin = functions[k].getXs()[i];
                    if (functions[k].getXs()[i] > xMax) xMax = functions[k].getXs()[i];
                    if (functions[k].getYs()[i] < yMin) yMin = functions[k].getYs()[i];
                    if (functions[k].getYs()[i] > yMax) yMax = functions[k].getYs()[i];
                }
           }
           if (functions[k].getFitFunctionVisible()) {
                   for (i=0; i<fittedFunctions[k].getXs().length; i++){
                       if (fittedFunctions[k].getXs()[i] < xMin) xMin = fittedFunctions[k].getXs()[i];
                       if (fittedFunctions[k].getXs()[i] > xMax) xMax = fittedFunctions[k].getXs()[i];
                       if (fittedFunctions[k].getYs()[i] < yMin) yMin = fittedFunctions[k].getYs()[i];
                       if (fittedFunctions[k].getYs()[i] > yMax) yMax = fittedFunctions[k].getYs()[i];
                   }
           }
    ...
    /*
    * Calculates the range and domain
    * (according to min and max y values).
    * and expands range if necessary. (does not shrink range)
    */
    ...
    */
    ...
    /**
    *  Paints the graph for the Printer. Certain things such as background color
    *  and graph dimensions  and location differ from the regular paint method
    *  @param g   Graphics to paint in
    */
    public void paintComponentForPrinter(Graphics g){
        ...
        if ( title != null) {
            ...
            g.setColor(Color.black);
            if (bounds.width>500) {
                if (bounds.height>550)g.drawString(title, (250 - title.length()*4)+40, 60+(inc/2));
                else g.drawString(title, (250 - title.length()*4)+40, 160+(inc/2));
              }
            else {
                if (bounds.height>550)g.drawString(title, (bounds.width/2 - title.length()*4)+40, 60+(inc/2));
                else g.drawString(title, (bounds.width/2 - title.length()*4)+40, 160+(inc/2));
              }
        }
        ...
        if (yLabel != null) {
            ...
            for (int i=0; i<yLabel.length(); i++){
               if (bounds.height>550 && bounds.height<=720)
                    ...
                    g.drawString(String.valueOf(yLabel.charAt(i)), 45, (bounds.height/2-((index-i)*10))+145);
            }
        }
      ...
      if (showLegend){
        ...
        for(int i = 0; i< numf; i++) {
          ...
          if (bounds.width>500){
            if (bounds.height>550) {
              if (i==0) g.fillRect(473,55+(14*i),3,3);
              if (i==1) fun2Icon.paintIcon(this,  g, 470,52+(14*i));
              if (i==2) fun3Icon.paintIcon(this,  g, 470,52+(14*i));
              if (i==3) fun4Icon.paintIcon(this,  g, 470,52+(14*i));
              if (i==4) fun5Icon.paintIcon(this,  g, 470,52+(14*i));
            }
            else {
              if (i==0) g.fillRect(473,155+(14*i),3,3);
              if (i==1) fun2Icon.paintIcon(this,  g, 470,152+(14*i));
              if (i==2) fun3Icon.paintIcon(this,  g, 470,152+(14*i));
              if (i==3) fun4Icon.paintIcon(this,  g, 470,152+(14*i));
              if (i==4) fun5Icon.paintIcon(this,  g, 470,152+(14*i));
            }
            ...
            if (bounds.height>550) {
              if (i==0) g.fillRect(bounds.width-27,55+(14*i),3,3);
              if (i==1) fun2Icon.paintIcon(this,  g, bounds.width-30,52+(14*i));
              if (i==2) fun3Icon.paintIcon(this,  g, bounds.width-30,52+(14*i));
              if (i==3) fun4Icon.paintIcon(this,  g, bounds.width-30,52+(14*i));
              if (i==4) fun5Icon.paintIcon(this,  g, bounds.width-30,52+(14*i));
              }
            else
              if (i==0) g.fillRect(bounds.width-27,155+(14*i),3,3);
              if (i==1) fun2Icon.paintIcon(this,  g, bounds.width-30,152+(14*i));
              if (i==2) fun3Icon.paintIcon(this,  g, bounds.width-30,152+(14*i));
              if (i==3) fun4Icon.paintIcon(this,  g, bounds.width-30,152+(14*i));
              if (i==4) fun5Icon.paintIcon(this,  g, bounds.width-30,152+(14*i));
          }

          g.setColor(getFuncts()[i].getColor());

          if (bounds.width>500){
            if (bounds.height>550 )g.drawString(name[i], 485, 62+(14*i));
            else g.drawString(name[i], 485, 162+(14*i));
          }

          else {
            if(bounds.height>550) g.drawString(name[i], bounds.width-15, 62+(14*i));
            else g.drawString(name[i], bounds.width-15, 162+(14*i));
          }
        }
    ...
    */
    public void setLegendVisible(boolean visible) {showLegend = visible;}

    /**
    *  Accessor that tells whether to show the gridlines
    *  @param visible        boolean to set it to
    */
    public void setGridlinesVisible(boolean visible) {showGridLines = visible;}
    ...
    *  @param visible        boolean to set it to
    */
    public void setMinorTickMarksVisible(boolean visible) {showMinorTickMarks = visible;}
    ...
    *  @param visible        boolean to set it to
    */
    public void setFunctionsVisible(boolean visible)    {showFunctions = visible;}

    /**
    *  Accessor that tells whether to show the gridlines
    *  @param visible        boolean to set it to
    */
    public void setPointsVisible(boolean visible)    {showPoints = visible;}
    ...
    */
    public ViewJComponentFunct[] getFuncts()    {return functions;}

    /**
    *  Accessor that gets the fitted functions for this graph
    *  @return      an array of functions for this graph
    */
    public ViewJComponentFunct[] getFittedFuncts()    {return fittedFunctions;}
    ...
    */
    public boolean getLegendVisible()     {return showLegend;}

    /**
    *  Accessor that gets the visible flag
    *  @return               the boolean visible flag
    */
    public boolean getGridlinesVisible()     {return showGridLines;}

    /**
    *  Accessor that gets the visible flag
    *  @return               the boolean visible flag
    */
    public boolean getFittedFunctionsVisible()     {return showFittedFunctions;}

    /**
    *  Accessor that gets the visible flag
    *  @return               the boolean visible flag
    */
    public boolean getMinorTickMarksVisible()     {return showMinorTickMarks;}
    ...
    *  @return               the boolean visible flag
    */
    public boolean getPointsVisible()     {return showPoints;}

    /**
    *  Accessor that gets the visible flag of the functions
    *  @return               the boolean visible flag
    */
    public boolean getFunctionsVisible()     {return showFunctions;}
    ...
    */
    ...
    /**
    *  Checks to see that the zoom box is valid, then redraws
    *  the graph using the zoom box as bounds
    ...
    */
    public void mouseReleased(MouseEvent mouseEvent) {
        ...
        else oldRubberband = rubberbandRect.getBounds();
        if (rubberbandRect.getBounds().x < graphBounds.x ||
            rubberbandRect.getBounds().x + rubberbandRect.getBounds().width > graphBounds.x + graphBounds.width ||
            rubberbandRect.getBounds().y < graphBounds.y ||
            rubberbandRect.getBounds().y + rubberbandRect.getBounds().height > graphBounds.y + graphBounds.height ||
            rubberbandRect.getBounds().width < 5 ||
    ...
    /**
    *   Changes the cursor to crosshair and enables the zoom box
    *   rubberband if the cursor is in the graph; also sees if cursor
    *   is near a valid point and prints it out
    *  @param mouseEvent  event that triggered this function
    */
        ...
        if (graphBounds.contains(xS, yS)){
            ...
            if (minDistance <= 5){
               ...
               // getGraphics().clearRect(bounds.x+5, bounds.height-26, 120, 20);
                ...
                g.setColor(Color.black);
                try {
                    tmpString = new String("X: "  + makeString(functions[findex].getXs()[xindex],2) +
                                       "  Y: " + makeString(functions[findex].getYs()[xindex],2));
                }
		        catch (OutOfMemoryError error){
    ...
    */
    ...
    /**
    *  Plots the graph by calling PolyLine.  Makes new arrays for
    *  the x and y, scaling them to the graphics x and y.  Also
    ...
    */
    private void plotGraph(Graphics g){
        ...
        for (int k = 0; k<functions.length; k++) {
            ...
            if (functions[k].getFunctionVisible()) g.drawPolyline(newX, newY, functions[k].getXs().length);
        }
        ...
        for (int k = 0; k < fittedFunctions.length; k++) {
            if(functions[k].getFitFunctionVisible()) {
                ...
                for (i=0; i < fittedFunctions[k].getXs().length; i++){
                    newX[i] = (int)Math.round(fittedFunctions[k].getXs()[i]*xScale - xMin*xScale) + graphBounds.x;
        ...
        for (i=0; i<=yGridLines; i++){
            if (newYMax!=-1) tmp = i*(newYMax-newYMin)/yGridLines + newYMin;
            else tmp = i*(yMax-yMin)/yGridLines + yMin;
            ...
            //            50, 30);
            g.fillRect(graphBounds.x - 55,
                       graphBounds.y + (int)Math.round(j*yTick)-10,
                        43, 30);
            g.setColor(Color.blue);
        ...
        for (i=0; i<=xGridLines; i=i+2){
            ...
            tmpString = String.valueOf(tmp);
            index = tmpString.indexOf('.');
            if (index + 3 <= tmpString.length()) tmpString = tmpString.substring(0, index+3);
            g.drawString(tmpString,
                        graphBounds.x + (int)Math.round(i*xTick) - 20,
            ...
            for (int k=0; k<functions.length; k++) {
                if (functions[k].getFunctionVisible()) {
                    for (i=0; i<functions[k].getNewXsLength(); i++){
                        g.setColor(Color.black);
                        if (k == 0) g.fillRect((int)(functions[k].getNewXs()[i]-1), (int)(functions[k].getNewYs()[i]-1), 3, 3);
                        if (k == 1) fun2Icon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 2) fun3Icon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 3) fun4Icon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 4) fun5Icon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                    }
                }
            }
        }
    ...
    /**
    *  Same as the plotGraph method,
    *  except the points used in this method
    *  are readable when sent to the printer
    *  @param g    graphics to draw in
    */
    private void plotGraphForPrinter(Graphics g){
        ...
        for (int k = 0; k<functions.length; k++) {
            ...
            functions[k].setNewYs(newY, functions[k].getYs().length);
            g.setColor(functions[k].getColor());
            if (functions[k].getFunctionVisible()) g.drawPolyline(newX, newY, newX.length);
        }
            ...
            for (int k=0; k<functions.length; k++) {
                if (functions[k].getFunctionVisible()) {
                    for (i=0; i<functions[k].getNewXsLength(); i++){
                        g.setColor(Color.black);
                        if (k == 0) g.fillRect((int)(functions[k].getNewXs()[i]-1), (int)(functions[k].getNewYs()[i]-1), 3, 3);
                        if (k == 1) fun2PrintIcon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 2) fun3PrintIcon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 3) fun4PrintIcon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                        if (k == 4) fun5PrintIcon.paintIcon(null,  g, (int)(functions[k].getNewXs()[i]-3), (int)(functions[k].getNewYs()[i]-3));
                    }
                }
            }
        }
    ...
    /**
    *  Redraws the graph based on the new bounds.  Used for
    *  zooming in on portions of the graph.
    ...
    */
    public void redrawGraph(Rectangle newBounds){
        ...
        try {
            for (int k=0; k<functions.length; k++) {
                if (end[k] < begin[k]) end[k] = functions[k].getNewXsLength() - 1;
                try {
        ...
        try {
            for (int k=0; k<fittedFunctions.length; k++) {
                ...
                if (functions[k].getFitFunctionVisible() ) {
                    if (end[k] < begin[k]) end[k] = fittedFunctions[k].getNewXsLength() - 1;
                    try {
                        zoomX = new float[end[k] - begin[k]+1];
                        zoomY = new float[end[k] - begin[k]+1];
                    }
