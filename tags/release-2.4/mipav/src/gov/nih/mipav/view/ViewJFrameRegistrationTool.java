 ...
 *   ViewJFrameRegistrationTool calls ViewJComponentRegistration.
 *	<p>
...
public class ViewJFrameRegistrationTool
  ...
  private boolean algorithmPerformed = false; //tells the tabbedpane to update once selected after algorithm has been performed
   ...
   */
  public ViewJFrameRegistrationTool(ModelImage _image, ModelImage _image2,
                                    ModelStorageBase _LUT, ModelStorageBase _LUT2) {
    ...
    try {
      ...
      MipavUtil.displayError(
          "ViewJFrameRegistrationTool: IOException Error on exportData - importData sequence");
    }
    init();
  ...
  private void init() {
    ...
    setSize( (int) Math.round( (scrollPaneSize * 2) + 3 + getInsets().left +
                              getInsets().right),
  ...
  private void buildScrollPanes(ViewUserInterface ui) {
    ...
    try {
      ...
      componentImageA = new ViewJComponentSingleRegistration(this, imageA, LUTa,
          imageBufferA, pixBufferCompA, 1, newExtents,
          logMagDisplay, ViewJComponentBase.NA, false,
          imageA.getFileInfo()[0].getAxisOrientation(), true);
      ...
      componentImageB = new ViewJComponentSingleRegistration(this, imageB, LUTb,
          imageBufferB, pixBufferCompA, 1, newExtents,
          logMagDisplay, ViewJComponentBase.NA, false,
          imageA.getFileInfo()[0].getAxisOrientation(), false);
    ...
    scrollPaneSeparateA = new JScrollPane(innerPanelA,
                                          JScrollPane.
    ...
    scrollPaneSeparateB = new JScrollPane(innerPanelB,
                                          JScrollPane.
                                          VERTICAL_SCROLLBAR_AS_NEEDED,
                                          JScrollPane.
                                          HORIZONTAL_SCROLLBAR_AS_NEEDED);
  ...
  public void setTitle() {
    ...
    if (tabbedPane.getSelectedIndex() == 0) {
      ...
      str = image.getImageName() + " (M: " + makeString(componentImageA.getZoomX(),2) + ") + "
          + secondImage.getImageName() + " (M:" +
          makeString(componentImageB.getZoomX(), 2) + ")";
    }
    setTitle(str);
       ...
       if (command.equals("toggleVOIs")) {
        ...
        // }
         ...
         try {
           ...
           MipavUtil.displayError(
               "ViewJFrameRegistrationTool: IOException Error on imageB.importData");
         }
         xfrm.identity();
         imageB.calcMinMax();
         ...
         try {
           ...
           multMatrix(secondImage.getMatrix().getArray(), xfrm.getArray(),
                      xfrmR);
           ...
           if (lsPerformed && xfrmBA != null) {
             multMatrix(secondImage.getMatrix().getArray(), xfrmBA.getArray(),
                        xfrmR);
             for (i = 0; i < 3; i++) {
               for (j = 0; j < 3; j++) {
                 secondImage.getMatrix().set(i, j, xfrmR[i][j]);
  ...
  private void transform() {
    ...
    try {
      ...
      MipavUtil.displayError(
          "ViewJFrameRegistrationTool: IOException Error on importData into imageB");
    }
  ...
  /**
   *  sets values based on knob along slider and catches the tabbed pane
   * switching events
