 ...
 *
 */
public class JDialogSkeletonize3D
    extends JDialogBase
  ...
  private boolean setVariables() {
   ...
   if (minObjectValue <= image.getMin()) {
       MipavUtil.displayError("Minimum object value must exceed image min of " +
                              image.getMin());
       ...
       MipavUtil.displayError("Minimum object value must not exceed image max of " +
                              image.getMax());
       textMinObjectValue.requestFocus();
       textMinObjectValue.selectAll();
       return false;
  ...
  public void callAlgorithm() {
    ...
    try {
        ...
        skeletonize3DAlgo = new AlgorithmSkeletonize3D(image, minObjectValue, sliceHoleFilling,
                                                       saveVF, distCharges, fieldStrength,
                perHDPoints, xvf, yvf, zvf, skPoints, skSegments,
                outputPoints);
        ...
        if (runInSeparateThread) {
            ...
            skeletonize3DAlgo.setActiveImage(isActiveImage);
            ...
            MipavUtil.displayError(
                    "Dialog Skeletonize 3D : unable to allocate enough memory");
            return;
    }
  }
