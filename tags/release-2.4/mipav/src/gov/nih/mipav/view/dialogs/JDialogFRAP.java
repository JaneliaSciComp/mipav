...
*
*/
public class JDialogFRAP extends JDialogBase implements AlgorithmInterface, ItemListener,
                                                        WindowListener {
	...
	*/
	private void init() {
		...
		gbc3.gridwidth = 1; gbc3.gridheight = 1; gbc3.anchor = gbc3.WEST; gbc3.weightx = 1;
            ...
            while (setEnum.hasMoreElements()) {
                ...
                while (paramEnum.hasMoreElements()) {
                    ...
                    if (paramName.equals("firstSliceAfterBleach")) {
                        String firstSliceAfterBleachStr = fileInfoImageXML.getPSet(temp).
                                                getParameter(paramName).getValue();
                        firstSliceAfterBleach = Integer.valueOf(
                                                firstSliceAfterBleachStr).intValue();
                        createInfoLine = true;
                    }
                    else if (paramName.equals("bleachedROIShape")) {
                        String bleachedROIShapeStr = fileInfoImageXML.getPSet(temp).
                                                     getParameter(paramName).getValue();
                        bleachedROIShape = Integer.valueOf(
                                           bleachedROIShapeStr).intValue();
                        createInfoLine = true;
                    }
                    else if ((paramName.length() > 7)&&
                             (paramName.substring(0,6).equals("knotX["))) {
                        ...
                        String knotXString = fileInfoImageXML.getPSet(temp).
                                             getParameter(paramName).getValue();
		...
		if (firstSliceAfterBleach >= 0) {
		    labelFirstSlice = new JLabel("First slice after bleach = " +
		                                    String.valueOf(firstSliceAfterBleach+1));
		    ...
		    switch(bleachedROIShape) {
		        ...
		        case CLOSED_POLYLINE:
		            if (knotX == null) {
		                ...
		                labelShape = new JLabel("Bleached region is a closed polyline " +
		                                " with " + knotX.length + " verticies");
		            }
		            break;
		        case CLOSED_BEZIER:
		            if (knotX == null) {
		                labelShape = new JLabel("Bleached region is a closed bezier " +
		                                        "spline curve");
		            }
		            else {
		                labelShape = new JLabel("Bleached region is a closed bezier " +
		                                        "spline curve with " + knotX.length +
		                                        " knots");
		            }
		            break;
		        case CIRCLE:
		...
		labelRef = new JLabel("First slice after photobleaching (1-" +
                                      String.valueOf(image.getExtents()[2]) + ")");
		...
		gbc5.gridwidth = 1; gbc5.gridheight = 1; gbc5.anchor = gbc5.WEST; gbc5.weightx = 1;
		...
		analysisPanel.add(paramVaryCheckBox, gbc5);
                ...
                gbc6.gridwidth = 1; gbc6.gridheight = 1; gbc6.anchor = gbc6.WEST; gbc6.weightx = 1;
   ...
   */
   public void actionPerformed(ActionEvent event) {
    	...
    	if (command.equals("OK")) {
    	...
    	else if ((source == photoBleachedButton) || (source == wholeOrganButton) ||
    	...
    	else if ((source == bandButton) || (source == circleButton) ||
    	         (source == oneDButton) || (source == singleExpButton)) {
    	    if ((bandButton.isSelected()) && (!circleButton.isSelected()) &&
    	        (!oneDButton.isSelected()) && (!singleExpButton.isSelected())) {
    	        wholeOrganCheckBox.setSelected(true);
    	        wholeOrganCheckBox.setEnabled(false);
                labelRadius.setEnabled(false);
                textRadius.setEnabled(false);
    	    ...
    	    else if ((!bandButton.isSelected()) && (circleButton.isSelected()) &&
    	        (!oneDButton.isSelected()) && (!singleExpButton.isSelected())) {
               wholeOrganCheckBox.setSelected(false);
    	        wholeOrganCheckBox.setEnabled(false);
               labelRadius.setEnabled(true);
    ...
    */
	private boolean setVariables() {
	    ...
	    if (wholeOrganNormalize && (wholeOrganIndex == -1)) {
	        MipavUtil.displayError(
	            "Must specify a whole organ VOI");
	        return false;
	    }
    ...
    private void callAlgorithm() {
        try {
            ...
            componentImage.setPresetHue(-1.0f);
            // Make algorithm
                frapAlgo= new AlgorithmFRAP(image,
                                       useRed, useGreen, useBlue, firstSliceNum,
                                       photoBleachedIndex, wholeOrganIndex,
                                       backgroundIndex,model,
