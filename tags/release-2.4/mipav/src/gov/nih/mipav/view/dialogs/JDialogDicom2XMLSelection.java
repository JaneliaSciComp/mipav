...
public class JDialogDicom2XMLSelection extends JDialogListSaveSelection {
    /**
     * Boolean that determines if the window was closed (hitting the X)
     * wasOkay = true means the [x] was not clicked.
     */
    ...
    /**
     * Creates a single JPanelFileSelector which refers to
     * the file &quot;dicomsave.dictionary&quot; in the user.home (specified by
     * java's properties) directory by default.
    ...
    /**
     * Creates an OK Button, which is relabeled &quot;Save&quot;, an
     * &quot;Ignore&quot; button, a &quot;Cancel&quot; and a &quot;Help&quot;
     * button.
     * @return JPanel The button panel containing a collection of buttons to
    ...
    /**
     * Uses the super method to create a standard OK Button, but then resets
     * the button text to read &quot;Save&quot; leaving the action command
     * string as normal: &quot;OK&quot;.
     */
    ...
    /**
     * Creates a button labelled &quot;Ignore&quot; and connects it to the
     * action listener event list.
     */
    ...
    /**
     * Generates and returns the list data for the Source list from the
     * dicom dictionary as a Vector of <code>DicomTagIdentifier</code>.
    ...
    /**
     * Generates and returns the list data for the Source list from the
     * dicom dictionary specified by the save panel as a Vector of
     * <code>DicomTagIdentifier</code>.
     * The Vector returned is empty if the File listed by the save panel
     * contains no tags, is non-existant, or is unspecified.
     * @return  An ordered list of <code>DicomTagIdentifier</code> of
     *          tags from the DICOM dictionary listed in the save tag file panel.
     *          The Vector returned is empty if the File listed by the save panel
     *          contains no tags, is non-existant, or is unspecified.
     * @see CreateDICOMFiles
    ...
    /**
     * Handles the IGNORE button possibility of the ActionEvent, otherwise
     * passes control to the super event handler.
     */
    public void actionPerformed( java.awt.event.ActionEvent ae ) {
        if ( ae.getActionCommand().equalsIgnoreCase( "ignore" ) ) {
            ignoreAction();
        } else if ( ae.getActionCommand().equalsIgnoreCase( "sortKey" ) ) {} else if ( ae.getActionCommand().equalsIgnoreCase(
                "sortKeyRev" ) ) {} else if ( ae.getActionCommand().equalsIgnoreCase( "sortName" ) ) {} else if ( ae.getActionCommand().equalsIgnoreCase(
                        "sortNameRev" ) ) {} else {
     ...
     * OKAY button, and is probably implementation dependant.
     */
    protected void saveListB() {
        try {
            if ( getSaveTagFilePanel().getSelectedFile().getName().equals( DICOMDictionaryBuilder.DEFAULT_DICTIONARY_FILENAME ) ) {
                Preferences.debug(
                        "Attempted to write DICOM dictionary file "
                ...
                if ( getSaveTagFilePanel().getSelectedFile().canWrite() ) {
                    ...
                    if ( getSelectedList().getModel().getSize() > 0 ) {
                        DICOMDictionaryBuilder.writeFile( getSaveTagFilePanel().getSelectedFile(), getSaveTable(),
                                "These tags were saved while converting a DICOM " + "image into MIPAV's XML format." );
                    }
                }
            }
    ...
    /**
     * Returns a hashtable of the keys and associated DICOM tags that are in
     * the right-list.
     *
    ...
    public Hashtable getSaveTable() {
        ...
        for ( int i = 0; i < saveList.getSize(); i++ ) {
            DicomTagIdentifier tag = (JDialogDicom2XMLSelection.DicomTagIdentifier)
                    saveList.getElementAt( i );
    ...
    /**
     * DicomTagIdentifiers allow collecting both the FileDicomTag with the associated
     * FileDicomKey (the Group and element numbers, properly formatted) of the Dicom
     * Tag in one place, and to ensure that they only display the key and the name
     * string of the Tag in <CODE>toString()</CODE>.
     */
    public class DicomTagIdentifier {
        ...
        /**
         * The Dicom Tag, held here to return the name of the tag, and when we need the
         * value when processing.
         */
        ...
        /**
         * Returns the group and element in parenthesis as displayed by the
         * FileDicomKey and the name of the DICOM tag.
         * <p>
         * Example: <code>(0018,603A) Doppler Sample Volume Y Position</code>
         *
         * @return Returns the group and element in parenthesis as displayed by
         * the FileDicomKey and the name of the DICOM tag.
         */
