...
/**
*   Simple dialog to indicate type, dimensionality, of a raw image or
*   image with a fixed length header in front of the image data.
*   Checks the Preferences file, and will default to the most recently
*   used JDialogRawIO values.  The dialog will save the values chosen
*   when [OK] is struck.
*
...
public class JDialogRawIO extends JDialogBase {
	...
	*/
	private void init() {
        ...
        gbc.insets = new Insets(5,5,5,5); gbc.fill = gbc.NONE;
		...
		gbc.gridx = 0; gbc.gridy = 5; gbc.gridwidth = 3; gbc.fill = gbc.VERTICAL; gbc.weighty = 1;
        ...
        gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 1; gbc.fill = gbc.NONE; gbc.weighty = 0;
        ...
        gbc.weighty = 1; gbc.anchor = gbc.NORTH; gbc.gridy = 5; gbc.fill = gbc.VERTICAL;
        ...
        gbc.gridx = 0; gbc.gridy = 0; gbc.anchor = gbc.NORTH; gbc.weighty = 1; gbc.gridwidth = 1; gbc.fill = gbc.VERTICAL;
...
//            setBigEndian(Boolean.getBoolean(Preferences.getProperty("RawImageBigEndianByteOrder")));
        ...
        else {  // we'll tell the properties that we want to remember these preferences. (use defaults or flag values here!)
	...
	*/
	...
	/**
	*   Accessor that sets the appropriate radio button as
	*   given by the image-data type
	*   @param t    The data type
	*/
	public void setDataType(int t)       {
        if      (t == ModelStorageBase.BOOLEAN) radioByte.setSelected(true);
        else if (t == ModelStorageBase.BYTE)    radioUByte.setSelected(true);
        else if (t == ModelStorageBase.UBYTE)   radioUByte.setSelected(true);
        else if (t == ModelStorageBase.SHORT)   radioShort.setSelected(true);
        else if (t == ModelStorageBase.USHORT)  radioUShort.setSelected(true);
        else if (t == ModelStorageBase.INTEGER) radioInt.setSelected(true);
        else if (t == ModelStorageBase.UINTEGER) radioUInt.setSelected(true);
        else if (t == ModelStorageBase.LONG)    radioLong.setSelected(true);
        else if (t == ModelStorageBase.FLOAT)   radioFloat.setSelected(true);
        else if (t == ModelStorageBase.DOUBLE)  radioDouble.setSelected(true);
        else if (t == ModelStorageBase.ARGB)    radioARGB.setSelected(true);
        else if (t == ModelStorageBase.ARGB_USHORT) radioARGB_USHORT.setSelected(true);
        else                                    radioShort.setSelected(true);
	}
	...
	*/
	public int[]    getExtents()        {return dimExtents;}
	...
	*/
	public String makeUnitString () {
	    return   comboBoxUnitOfMeasure1.getSelectedIndex()+","
	           + comboBoxUnitOfMeasure3.getSelectedIndex()+","
	           + comboBoxUnitOfMeasure4.getSelectedIndex()+","
	           + comboBoxUnitOfMeasure5.getSelectedIndex();
	}

	/**
	*   Inserts the values given by the preference file into
	*   the extents fields of the dialog.
	...
	*/
	...
	/**
	*   Inserts the values given by the preference file into
	*   the resolutions fields of the dialog.
	...
	*/
	...
	/**
	*   Inserts the values given by the preference file into
	*   the units of measure fields of the dialog.
   ...
   */
   public void actionPerformed(ActionEvent event) {
      ...
      if ( source == OKButton ) {
        if      (radioBool.isSelected())     dataType = ModelStorageBase.BOOLEAN;
       	else if (radioByte.isSelected())     dataType = ModelStorageBase.BYTE;
       	else if (radioUByte.isSelected())    dataType = ModelStorageBase.UBYTE;
       	else if (radioShort.isSelected())    dataType = ModelStorageBase.SHORT;
       	else if (radioUShort.isSelected())   dataType = ModelStorageBase.USHORT;
       	else if (radioInt.isSelected())      dataType = ModelStorageBase.INTEGER;
       	else if (radioUInt.isSelected())     dataType = ModelStorageBase.UINTEGER;
       	else if (radioLong.isSelected())     dataType = ModelStorageBase.LONG;
       	else if (radioFloat.isSelected())    dataType = ModelStorageBase.FLOAT;
       	else if (radioDouble.isSelected())   dataType = ModelStorageBase.DOUBLE;
       	else if (radioARGB.isSelected())     dataType = ModelStorageBase.ARGB;
       	else if (radioARGB_USHORT.isSelected()) dataType = ModelStorageBase.ARGB_USHORT;
       	else                                 dataType = ModelStorageBase.BYTE;
       ...
       //	else                        unitOfMeasure = FileInfoBase.INCHES;
        ...
        if (dimExtents.length >= 3) unitsOfMeasure[2] =  comboBoxUnitOfMeasure3.getSelectedIndex()+1;
        if (dimExtents.length >= 4) unitsOfMeasure[3] =  comboBoxUnitOfMeasure4.getSelectedIndex()+1;
        if (dimExtents.length >= 5) unitsOfMeasure[4] =  comboBoxUnitOfMeasure5.getSelectedIndex()+1;
