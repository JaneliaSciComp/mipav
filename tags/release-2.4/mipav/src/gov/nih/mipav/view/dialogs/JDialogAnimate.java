...
/**
 *   Dialog to get scale factor and interpolation method to create initial image
 *   forwarded to ViewJFrameAnimate.  The interpolation is done on a slice by slice
 *   basis only.  Bilinear or bspline may be chosen.  For 4 dimensional images the
 *   user also enters the number of rows and columns for the different z slices, where
 *   z is the third dimension and animation is performed on the fourth time dimension.
 *   For 4D images the user can select whether or not to have a frame border around
 *   each z image and the color of the frame border.
 */
public class JDialogAnimate extends JDialogBase {
     ...
     */
    public JDialogAnimate( Frame theParentFrame, ModelImage _imageA, ModelLUT LUTa,
            ModelImage _imageB, ModelLUT LUTb, ViewUserInterface ui ) {
     ...
     */
    public JDialogAnimate( Frame theParentFrame, ModelImage _imageA, ModelLUT LUTa,
            ModelImage _imageB, ModelLUT LUTb, ViewUserInterface ui, boolean _isVisible ) {
        super( theParentFrame, false );
        this.LUTa = LUTa;
        this.LUTb = LUTb;
        this.ui = ui;
    ...
    /**
     *	Responds to row button, column button, OK button, cancel button,
     *	and borderB button for color.  OK button processes images and
     *    calls ViewJFrameAnimate.
    ...
    public void animate() {
        ...
        if ( Sx != 1.0 ) {
            ...
            try {
                ...
                algoTrans = new AlgorithmTransform( imageA, xfrm, interp, oXres, oYres, oXdim, oYdim,
                                                    false, true, false );
            ...
            if ( imageB != null ) {
                try {
                    // AlgorithmTransform scales the image with the selected interpolation method.
                    algoTransB = new AlgorithmTransform( imageB, xfrm, interp, oXres, oYres, oXdim, oYdim, false,
                            true, false );
                } catch ( OutOfMemoryError x ) {
                    MipavUtil.displayError( "Dialog Transform: unable to allocate enough memory" );
                    return;
                        ...
                        for ( int i = 0; i < oldOXdim; i++ ) {
                            for ( int j = 0; j < oldOYdim; j++ ) {
                                ...
                                transformedImageA.setC( i + 3 + ( oldOXdim + 9 ) * colNum,
                                        j + 3 + ( oldOYdim + 9 ) * rowNum, l, 0, imgBuf[4 * ( i + j * oldOXdim )] );
                                transformedImageA.setC( i + 3 + ( oldOXdim + 9 ) * colNum,
        ...
        try {
            ...
            if ( ( Sx == 1.0 ) && ( DIM == 3 ) ) {
                ...
                frameAnimate = new ViewJFrameAnimate( imageA, LUTa, imageB, LUTb, RGBTA, RGBTB, controlFrame,
                        nRow, nColumn, imageA.getExtents()[2], showFrameBorder, borderCol, false );

            } else if ( DIM == 3 ) {
                // scaled 3D image - pass the scaled resultImageA and possibly resultImageB to ViewJFrameAnimate
                frameAnimate = new ViewJFrameAnimate( resultImageA, LUTa, resultImageB, LUTb, RGBTA, RGBTB,
                        controlFrame, nRow, nColumn, imageA.getExtents()[2], showFrameBorder, borderCol, true );
            } else { // DIM == 4
