 ...
 * The list is updated at a rate which is user-specified.  The frame will allow the user to
 * adjust how frequently the list will be updated.  For efficiency, no updating
 * will occur if the frame has been minimized.
 * <p>
 * The registered images frame provides direct user access to the function
 * Runtime.getRuntime.gc() via a "Garbage Collector" button.
 *</p><p>
 * There are a number of possible supported uses.  There is an unimplemented
 * button to remove un-framed images from the ViewUserInterface image hashtable
 * and there is a button to bring framed-images to the front.
 * </p><p>
 *  This class has a number of inside classes:
 *  <ul>
 *    <li>ImageRegistryMonitor (a type of Thread) to update the list of images
 *        registered with the ViewUserInterface</li>
 *    <li>MouseClickAdapter (a type of MouseAdapter) to attempt to bring the
 *        selected image's frame to the front</li>
 *    <li>ImageCellRenderer (a type of JLabel) to present both the image-name
 *        and a helpful icon depicting whether or not the associated image
 *        is in a frame or not</li>
 * </ul>
 *
...
public class ViewJFrameRegisteredImages extends JFrame
    ...
    private void imageToFront(String selectedName) throws NullPointerException,
        ...
        try {
            ...
            Preferences.debug("Illegal Argument Exception in " +
                "ViewJFrameRegisteredImages.imageToFront() " +
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (source == pauseButton) {
            ...
            try {
                ...
                // attached to a frame
                if (frame == null  && image != null) image.disposeLocal();
                Runtime.getRuntime().gc();
                Runtime.getRuntime().runFinalization();
            } catch (IllegalArgumentException iae) {
	...
	*/
     ...
     *  <p>
     *  calls <code>callToFrontbutton#doClick()</code> when the
     *  list has been double-clicked, to bring the selected
     *  image (and associated frame) to the front.
     */
...
//        System.out.println("ValueChanged");
   ...
   /**
    * Tracks registered images, and notifies anybody who is interested
    *   in finding out about them.
    */
    public class ImageRegistryMonitor      implements Runnable,
                                               FocusListener {
        ...
        public ImageRegistryMonitor(ViewUserInterface ui) {
            this.UI = ui;
        ...
        */
        public void run() {
            ...
            while (thread == me){
                ...
                Enumeration e = UI.getRegisteredImageNames();
    ...
    }  // end class ImageRegistryMonitor
    ...
    private class ImageCellRenderer extends JLabel
