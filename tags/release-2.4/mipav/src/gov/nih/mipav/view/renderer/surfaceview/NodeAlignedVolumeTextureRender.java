...
public  class   NodeAlignedVolumeTextureRender
    ...
    /**
     * This is the only node attached to this TransformGroup.  It will
     * enable either the selection of the "fine" or "coarse" resolution
     *  slice sampling.
     */
    ...
    /**
     * The slicing through the volume will be contained in an OrderedGroup
     * to ensure that the slices are rendered back-to-front for correct
     * blending effect.  One set each is available for fine and coarse
     * resolution quality rendering.
     */
     ...
     * @param kVolumeInfo description of the parameters for the volumes
     * which can be rendered with this node, where all volumes must shared
     ...
     * @return boolean Returns true if "coarse" slice sampling is currently
     * selected.
     */
     ...
     * @param kOrderedGroupSlices OrderedGroup Node which is to be cleared
     * of all children.
     */
    ...
    /**
     * Create the z-axis aligned slices through the volume so that any
     * rotation of the volume does not result in the slices getting
     * clipped by the texture volume.
     * @param kOrderedGroupSlices Node containing the sequenced ordering of
     * the slices for correct back to front rendering when looking down the
     * -Z axis.
