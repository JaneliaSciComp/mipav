...
/**
 * Window for setting the surface material properties. The window displays two
 * sample spheres: a "before" and "after" sphere. Both are initialized to the
 * current material properties and transparency of the surface. The user can
 * change the ambient, diffuse, specular, and emmissive colors with a color
 * control dialog. The user can also change the specular coefficient, or
 * "shininess" through a slider interface.
 *
 * Pressing the "apply" button the user can see the changes applied to sthe
 * surface. Pressing the "reset" button resets the surface and material
 * properties interface to the original surface parameters. Pressing "OK"
 * applies the changes to the surface and closes the window, Pressing "cancel"
 * resets the changes and closes the window.
 *
...
{
    /**
     * Creates the Surface Material Editor window. The two example spheres are
     * displayed with the same lights, opacity, and material properties as the
     * surface.
     *
     * @param GeneralLight[] the array of lights that are used to light the surface
     * @param float, the opacity of the surface
     * @param Material, the original material properties.
     */
    public JFrameSurfaceMaterialProperties( JPanelSurface kParent, int iSurface,
    ...
    public void actionPerformed( ActionEvent kEvent )
    {
         ...
         * Specular, or Emissive, a JColorChooser is created with the modal
         * value set to true. This ensures that only one color is set through
         * the JColorChooser dialog at a time.  The data member m_iWhichButton
         * keeps track of which button is pressed at the time the
         * JColorChooser is created, so that when it is closed with the "OK"
         * command the proper actions can be taken: */
        if ( kCommand.equals( "Ambient" ) )
        {
            m_kColorChooser =
                new JColorChooser( m_kAmbientColorButton.getBackground() );
            JDialog kDialog = m_kColorChooser.createDialog( new Frame(),
                                                       "Set Ambient Color",
                                                       true, m_kColorChooser,
                                                       this, this );
        ...
        {
            m_kColorChooser =
                new JColorChooser( m_kDiffuseColorButton.getBackground() );
            JDialog kDialog = m_kColorChooser.createDialog( new Frame(),
                                                       "Set Diffuse color",
                                                       true, m_kColorChooser,
                                                       this, this );
        ...
        {
            m_kColorChooser =
                new JColorChooser( m_kSpecularColorButton.getBackground() );
            JDialog kDialog = m_kColorChooser.createDialog( new Frame(),
                                                       "Set Specular color",
                                                       true, m_kColorChooser,
                                                       this, this );
        ...
        {
            m_kColorChooser =
                new JColorChooser( m_kEmissiveColorButton.getBackground() );
            JDialog kDialog = m_kColorChooser.createDialog( new Frame(),
                                                       "Set Emissive color",
                                                       true, m_kColorChooser,
                                                       this, this );
         ...
         * m_iWhichButton is used to determine which color parameter is being
         * set, and the new color value is read from the m_kColorChooser
        ...
        {
            int iPreset =
                Integer.valueOf( kCommand.substring( "Preset".length() ) ).intValue();
    ...
    /**
     * Called when the slider values have changed. Reads the new shininess
     * values and sets the Material Properties.
    ...
    /**
     * Reads the current color values from the Material and sets the values
     * for the two spheres displayed in the interface. Saves the original
     * values in case the Reset button is pressed.
    ...
    private void setupMaterials( Material kMaterial )
    {
        ...
        m_kMaterialNew = new Material( m_kAmbient,
                                       m_kEmissive,
                                       m_kDiffuse,
                                       m_kSpecular,
                                       m_fShininess );
        ...
        /* Save the original: */
        m_kMaterialSave = new Material( m_kAmbient,
                                        m_kEmissive,
                                        m_kDiffuse,
                                        m_kSpecular,
                                        m_fShininess  );
    ...
    private void initMaterialPreset()
    {
        ...
        m_akMaterialPreset[0] = /* Diffuse Red */
            new Material( new Color3f( 0f, 0f, 0f ), /* ambient */
        ...
        m_akMaterialPreset[1] = /* Blue plastic (white highlights) */
            new Material( new Color3f( 0f, 0f, 0f ), /* ambient */
                          new Color3f( 0f, 0f, 0f ), /* emissive */
        ...
        m_akMaterialPreset[2] = /* Green metallic (color highlights) */
            new Material( new Color3f( 0f, 0f, 0f ),     /* ambient */
                          new Color3f( 0f, 0f, 0f ),     /* emissive */
        ...
        m_akMaterialPreset[3] = /* Red/orange metallic (color highlights) */
            new Material( new Color3f( 0f, 0f, 0f ),     /* ambient */
                          new Color3f( 0f, 0f, 0f ),     /* emissive */
                          new Color3f( 1f, 0f, 0f ),     /* diffuse */
     ...
     * canvas window with the same lighting, material, and transparency
     * parameters as the original surface. The "Before" sphere only changes
     * when the "Apply" button is presesd, the "After" sphere changes each
     * time one of the color buttons or the shininess slider is changed
    ...
    private void setupScene( float fOpacity, GeneralLight[] akGeneralLights )
    {
        ...
        for ( int iScene = 0; iScene < 2 + m_iNumPreset; iScene++ )
        {
            ...
            akUniverse[iScene].getViewingPlatform().
            ...
            {
                m_kImagePanel[iScene].setMinimumSize( new Dimension( 600/m_iNumPreset,
                                                                     300 ));
            }
        }
    }
    ...
    /**
     * Creates the rendered spheres, setting the apprearance, material
     * properties, and transparency.
     */
    private BranchGroup createSceneGraph( float fOpacity,
                                          Material kMaterial )
    {
        /* Sphere to mark the point being drawn: */
         Shape3D kSphere = new Sphere( .5f,
    ...
    private void setupInterface()
    {
         ...
         * right: */
        m_kImagePane = new JSplitPane( JSplitPane.HORIZONTAL_SPLIT,
                                       true,
                                       m_kImagePanel[BEFORE],
                                       m_kImagePanel[AFTER] );
    ...
    /**
     * Update the color changes to the Before sphere and the original
     * surface.
     */
    ...
    /* Before/After index values for the two displayed spheres, canvases, and
     * display panels: */
    ...
    /* Material properties buttons/slider panel: */
