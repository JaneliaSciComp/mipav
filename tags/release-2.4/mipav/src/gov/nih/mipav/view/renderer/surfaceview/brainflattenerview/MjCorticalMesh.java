...
public class MjCorticalMesh
{
    ...
    public MjCorticalMesh (int iVQuantity, Point3f [] akPoint,
    ...
    /* uniformly scale points to [-1,1]^3, originally in [min,max]^3 */
    /* returns 2D vector with (min,max) as the elements */
    ...
    public void computeMeanCurvature ()
    {
        ...
        MjMeshCurvature kMG =
                new MjMeshCurvature(
                m_iVQuantity,m_akPoint,m_iTQuantity,m_aiConnect);
    ...
    public void computeConformalMapping ()
    {
        ...
        while ( kIter.hasNext() )
        {
            ...
            MjSparseMatrix.Index kIndex =
                    (MjSparseMatrix.Index)kEntry.getKey();
        ...
        m_fRho = computeRadius(new MjVector2f(m_akPlane[iV0]),
                               new MjVector2f(m_akPlane[iV1]),
                               new MjVector2f(m_akPlane[iV2]),
        ...
        {
            m_akCylinder[i].x =
                   -(float)Math.atan2(m_akSphere[i].y,m_akSphere[i].x);
    ...
    /* Vertex-vertex distance measured along the shortest edge-path */
    ...
    /* Computes the distances of vertex pairs <V0,V1> for which an edge-path */
    /* exists with at most iSize edges. */
    ...
    /* Produce polylines that are superimposed on the input cortical mesh, */
    /* sphere, and plane.  The znormal value indicates the z-slice on the */
    /* unit sphere that is mapped to the polyline on the mesh.  This value */
    /* is in (-1,1).  The angle value indicates the theta-slice on the unit */
    /* sphere, theta measured as a positive angle counterclockwise from the */
    /* x-axis towards the y-axis.  This value is in [0,2*pi].  The bias value */
    /* adds a small amount to the polyline to raise it above the mesh surface */
    /* to avoid z-buffer fighting. */

    public Polylines getLatitude (float fZNormal, float fMBias,
                                  float fSBias, float fPBias)
    {
        ...
        kLatitude.m_fRadius =
                (float)Math.sqrt((1.0f+fZNormal)/(1.0f-fZNormal)) *
        ...
        {
            kPolylines.akMVertex[i] =
                     (Point3f)(((Map.Entry)kIM.next()).getValue());
            kPolylines.akSVertex[i] =
                    (Point3f)(((Map.Entry)kIS.next()).getValue());
        }

        kPolylines.akPVertex = new Point3f [2];
        kPolylines.akPVertex[0] =
                new Point3f(+(float)Math.PI,fZNormal,-fPBias);
        kPolylines.akPVertex[1] =
                new Point3f(-(float)Math.PI,fZNormal,-fPBias);

        return kPolylines;
    }

    public Polylines getLongitude (float fAngle, float fMBias,
                                   float fSBias, float fPBias)
    {
    ...
    private float computeRadius (MjVector2f kV0, MjVector2f kV1,
                                 MjVector2f kV2, float fAreaFraction)
    {
    ...
    /* Support for conformal mapping to planar triangle mesh.  Use a linear */
    /* walk to locate a triangle containing P.  The index of such a triangle */
    /* is returned, unless there is no containing triangle in which case the */
    /* return value is -1.  The "start" index is the suggested starting */
    /* triangle.  When testing a lot of points that are ordered in some */
    /* spatial manner, the chance that the triangle containing the previous */
    /* test point will contain the next test point (or a neighbor might */
    /* contain it).  Taking advantage of spatial coherence should speed */
    ...
    /* Support for conformal mapping to planar triangle mesh.  Once the */
    /* containing triangle i is found, compute the barycentric coordinates */
    /* of P relative to the triangle vertices V0, V1, and V2. */
    ...
    /* support for point-in-triangle tests; */
    /* The akVertex array must have length 3. */
    ...
    private void computeDistance (int iSize, int iSource, boolean bInitiator)
    {
        ...
        for (int j = 2; j <= iSize; j++)
        {
            ...
            while ( kIter.hasNext() )
            {
                ...
                float fPathLength =
                        ((Float)(m_kDistance.get(
                        new MjEdgeKey(iSource,iCenter)))).floatValue();
                ...
                for (i = 0; i < kCenter.VQuantity; i++)
                {
                    ...
                    if ( !kInterior.contains(kNbr)
                         && !kBoundary.contains(kNbr) )
                    {
                        /* get distance of edge <center,nbr> */
                        Float kDist =
                                (Float)m_kDistance.get(
                                new MjEdgeKey(iCenter,iNbr));
                        float fDist =
                            (null != kDist)
                            ? kDist.floatValue()
    ...
    private float doInflationStep ()
    {
        ...
        for (int i0 = 0; i0 < m_iVQuantity; i0++)
        {
            ...
            for (int j = 0; j < kVertex.VQuantity; j++)
            {
                ...
                kPoly.setCoeff(2,kPoly.getCoeff(2)+
                               4.0f*fWeight*fB*fB + 2.0f*fA*fTmp1);
    ...
    /* Set the index of the triangle used to "puncture" the mesh before the
     * conformal mapping is computed. The triangle is found based on the three
     * index values that describe the triangle: */
    ...
    public static class Vertex
    {
        ...
        static private final int MV_CHUNK = 8;
    ...
    /* Conformal mapping to a sphere.  rho is the radius of the stereographic */
    /* sphere. */
    ...
    /* The index of the puncture triangle for computing the conformal
     * mapping: */
