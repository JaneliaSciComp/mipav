 ...
 * Class JStereoWindow:
 *
 * Creates a stereo viewing window for both ModelTriangleMesh surfaces and
 * arbitrary Group Nodes, including the NodeVolumeTextureRender. Objects are
 * presented on three canvases: left-eye view, right-eye view, and a anaglyph
 * color view. The user can change the eye separation with a slider that
 * adjusts the focal length.
 *
 * The stereo rendering algorithm for creating the left and right-eye viewing
 * transformations and anaglyph display is based on Paul Bourke's web
 * information at http://astronomy.swin.edu.au/~pbourke/stereographics/
...
public class JStereoWindow extends JFrame
    implements
        ...
        ActionListener, /* Button presses */
        ChangeListener, /* Slider changes */
        ComponentListener, /* Window resize events */
        MouseMotionListener /* mouse moved events */
        
{
    /**
     * Constructs a JStereoWindow with a ModelTriangleMesh and Transform3D
     * representing the model transformation for the surface.
     *
     * @param kMesh, the displayed surface 
     * @param kTransform, the current model transformation (rotation, scale,
     * translation) for the surface.
     */
    public JStereoWindow( ModelTriangleMesh kMesh, Transform3D kTransform )
    {
        ...
        kAppearance.setMaterial( new Material( new Color3f( 0f,0f,0f ),
                                               new Color3f( 0f,0f,0f ),
                                               new Color3f( .8f,.8f,.8f ),
                                               new Color3f( 0f,0f,0f ), 100f ) );
    ...
    /**
     * Constructs a JStereoWindow with two nodes, one copy for each of the
     * left and right eye views. The input nodes are identical.  The
     * Transform3D represents the model transformation for the nodes.
     *
     ...
     * @param kTransform, the current model transformation (rotation, scale,
     * translation) for the nodes.
     * @param kSurface, the parent window.
     */
    public JStereoWindow( Group kNode1, Group kNode2,
                          Transform3D kTransform,
                          SurfaceRender kSurface  )
    {
        ...
        /* Initialize and create the scene graphs and canvases: */
        init( addBranchGroup( kNode1, kTransform ),
              addBranchGroup( kNode2, kTransform ) );
    ...
    /**
     * windowClosing
     * Catch the windowClosing event so that the local data memebrs can be destroyed.
     * @param e WindowEvent
     */
    ...
    /**
     * Called when the slider values have changed. Reads the new focal-length
     * value and recalculates the viewing transformations for the left and
     * right-eye views.
    ...
    /**
     * Creates a BranchGroup containing the input Node and Transform3D, and
     * returns the BranchGroup.
     * @param kNode, the Node to add to the BranchGroup
     * @param kTransform, the transform for the input Node
     * @return the Branch of the SceneGraph containing the node
     * and transform.
     */
    private BranchGroup addBranchGroup( Node kNode, Transform3D kTransform )
    {
        ...
        TransformGroup kTransformGroup =
            new TransformGroup( new Transform3D( kTransform ) );
     ...
     * @return the anaglyph color (packed-int) with the red component
     * from the left-eye view and the green and blue components from the
     * right-eye view, creating a Red-Cyan stereo.
     */
    ...
    /**
     * Blend two BufferedImages, representing the left and right-eye views
     * into a Red-Cyan stereo anaglyph.
     * @param kLeft, the frame buffer from the left-eye rendering.
     * @param kRight, the frame buffer from the right-eye rendering.
     * This function does not return a value, but it sets the BufferedImage
     * member variable m_kStereo and the Dimension member variable m_kD.
     */
    private void blendBuffers( BufferedImage kLeft,
                               BufferedImage kRight )

    {
        ...
        m_kRGStereo = 
            new BufferedImage( iWidthPow2, iHeightPow2, kLeft.getType() );
    ...
    private void setupInterface()
    {
        ...
        m_kDisplayPane = new JSplitPane(  JSplitPane.HORIZONTAL_SPLIT,
                                          true,
                                          m_kLeftImagePanel,
                                          m_kRightImagePanel );
        ...
        m_kStereoPane = new JSplitPane(  JSplitPane.VERTICAL_SPLIT,
                                         true,
                                         m_kDisplayPane,
                                         m_kStereoPanel );
        ...
        m_kMainPane = new JSplitPane(  JSplitPane.VERTICAL_SPLIT,
                                       true,
                                       m_kStereoPane,
                                       kInterfacePanel );
    ...
    /**
     * Reads the left and right-eye buffers from the left and right-eye
     * canvases and blends them into the Red-Cyan Anaglyph stereo buffer,
     * which is then texture-mapped on the a polygon and rendered in the
     * stereo canvas.
     */
    private void updateStereo() {
        /* Get the left-eye rendered image from the left canvas: */
        m_kD = m_kLeftCanvas.getSize();
        BufferedImage kLeft = new BufferedImage( m_kD.width, m_kD.height,
                                                 BufferedImage.TYPE_INT_ARGB );
        getFrameBuffer( kLeft, m_kLeftCanvas );
      
        /* Get the right-eye rendered image from the right canvas: */
        m_kD = m_kRightCanvas.getSize();
        BufferedImage kRight = new BufferedImage( m_kD.width, m_kD.height,
                                                  BufferedImage.TYPE_INT_ARGB );
        ...
        ImageComponent2D kDisplayedImage =
            new ImageComponent2D( ImageComponent.FORMAT_RGBA, m_kRGStereo );

        Texture2D kTexture =
            new Texture2D( Texture.BASE_LEVEL, Texture.RGBA, m_kD.width, m_kD.height );
    ...
    /**
     *  Read the frame buffer from the input VolumeCanvas3D and store the result in
    ...
    private void getFrameBuffer( BufferedImage kImage, VolumeCanvas3D kCanvas )
    {
        ...
        try {
            ...
            MipavUtil.
                displayError( "JStereoWindow: unable to allocate enough memory for RGB image" );
            return;
        } catch ( AWTException error ) {
            MipavUtil.
                displayError( "JStereoWindow: Platform doesn't support screen capture." );
            return;
        }
    ...
    /** 
     * Creates the texture-mapped polygon displayed in the stereo canvas. The
     * left and right-eye views are read from the left and right-eye canvases
     * and the images blended into a stereo view. The blended image is used as
     * a texture and displayed on the texture-mapped polygon created in the
     * function:
     */
    private void createTexturedPolygon()
    {
        ...
        TexCoordGeneration kTexCoordGeneration =
    ...
    /** 
     * Initialize the scene graphs and left, right, and stereo canvases for
     * this JStereoWindow.
    ...
    /**
     * Creates a canvas containing the input Node object, with the input
     * viewing transformation. The canvas is displayed in the input JPanel:
     * @param kNode, the object displayed in this canvas.
     * @param kPanel, the window panel containing this canvas.
     * @return the new canvas.
     */
    private VolumeCanvas3D createCanvas( Node kNode,
                                         JPanel kPanel )
    {
        ...
        light.
            setInfluencingBounds( new BoundingSphere( new Point3d( 0.0f,
                                                                   0.0f,
                                                                   0.0f ),
                                                      100.0f ) );
        ...
        m_akUniverse[ m_iCurrentUniverse ].
    ...
    /** Texture-mapped polygon for displaying the Anaglyph stereo image as a
     * texture: */
