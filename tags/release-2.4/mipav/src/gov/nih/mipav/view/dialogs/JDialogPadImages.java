 ...
 *       Dialog asks the user how to pad the blank images, such as padding
 *       to the front or back of the image, half to front and half to back
 *       of the image.  It gives options to remove or to cancel.
 *		 @author  Ruida Cheng
 */
public class JDialogPadImages extends JDialogBase
    implements AlgorithmInterface, ScriptableInterface {

    public final static int PAD_FRONT = 0;
    public final static int PAD_BACK = 1;
    public final static int PAD_HALF = 2;
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "PadWithSlices " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
            ...
            if ( displayLoc == NEW ) {
                if ( padSlicesAlgo.isCompleted() == true && resultImage != null ) {
                    ...
                    try {
                        ...
                        MipavUtil.displayError( "Pad With Slices reports: out of memory; " + "unable to open a new frame" );
                        successful = false;
                    }
                    if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
                        Preferences.debug("\nHave padded from " + nSlices +
                                          " slices for " + image.getImageName() + " to " +
                                         paddedSlices + " slices for " +
