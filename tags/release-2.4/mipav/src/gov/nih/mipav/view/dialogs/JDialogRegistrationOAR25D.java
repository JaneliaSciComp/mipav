 ...
 *   the reference slice, and so on unitl the first slice is registered
...
public class JDialogRegistrationOAR25D extends JDialogBase
            ...
            if ( UI.isScriptRecording() ) {
                ...
                if ( weighted ) {
                    ...
                    UI.getScriptDialog().append(
                            "RegistrationOAR25D " + UI.getScriptDialog().getVar( matchImage.getImageName() ) + " "
                            + weighted + " " + UI.getScriptDialog().getVar( inputWeightImage.getImageName() ) + " "
                            + getParameterString(" ") + "\n" );
                } else {
                    UI.getScriptDialog().append(
                            "RegistrationOAR25D " + UI.getScriptDialog().getVar( matchImage.getImageName() ) + " "
    ...
    /**
     * Accessor to set whether or not center of gravity should be ignored
    ...
    private void callAlgorithm() {
        ...
        if ( weighted ) {
            reg25 = new AlgorithmRegOAR25D2( matchImage, inputWeightImage, cost, DOF, interp, interp2, doAdjacent,
                    refImageNum, rotateBegin, rotateEnd, coarseRate, fineRate, doGraph, doSubsample, transformVOIs,
                ...
                if ( doGraph ) {
                    ...
                    ViewJFrameGraph transGraph = new ViewJFrameGraph( posT, trans, "Translations", "Slice number",
                            ...
                            + FileInfoBase.getUnitsOfMeasureAbbrevStr(
                                    matchImage.getFileInfo( 0 ).getUnitsOfMeasure( 0 ) ) );
