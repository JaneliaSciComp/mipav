...
/**
 *   Visual Interface to anonymize DICOM images
 *   by the directory-full.  Multiple directories may
 *   be chosen to operate on; it will operate on
 *   sub-directories as well.
 *   <p>
 *   When one directory is chosen as the
 *   source, all anonymized Dicom files in source
 *   are placed directly in the destination.  Any
 *   images in any
 *   subdirectories (when chkChildren is selected)
 *   are sent into the children of the sub-directories.
 *   <p>
 *   More
 *   than one submitted source directory will
 *   be placed into discrete child directories of
 *   the submitted destination directory.  As as
 *   example, /images/a, /images/b are selected, and
 *   /dest is the selected destination directory.
 *   They would get put into /dest/a, and /dest/b.
 *   <p>
 *   Naming can change the way the destination
 *   is made.  Selecting a top-level randomisation,
 *   will randomate the output directory ofthe
 *   selected image directory.  This isalong the
 *   lines in the above example as /images/a
 *   becoming /dest/abcde; and /images/b becoming
 *   /dest/12345.  Selecting only one input directory
 *   will notmakea difference for top-level
 *   randomising.  Child-directory randomising
 *   will give a random name when processing child
 *   direcories.  this amounts to selecting /images
 *   (and recursive processing) and the destination
 *   directory /dest,and getting /images/a becoming
 *   /dest/abcde, and /images/b becoming
 *   /dest/12345.
 */
...
public class JDialogAnonymizeDirectory extends JDialogBase {
    ...
    private JPanel sourcePanel; // remove this panel and rebuild it with a different input var to get a new directory tree.
     ...
     *   <p>
     *   install the panels of
     *   source directory, destination directory, the checkbox
     *   for approving the translation-table file and the
     *   panel containing the ok and cancel buttons.
     *   Installs the checkbox panel.
     */
    public JDialogAnonymizeDirectory( ViewUserInterface ui, String dir ) {
        ...
        everything.insertTab( "Directory", null, filePanel = buildFilePanel( dir ), //we must store this panel so we can create a new directory listing later
                "Choose all source and destination directories",
                DIR_TAB );
     ...
     *   Builds a small menu with "New directory" and "Refresh directory"
     *   options.
     */
    ...
    /** creates the source panel which consists of the
     *   directory line, the browse button, and a check
     *   box approving the anonymize in sub-directories.
     */
    private JPanel buildFilePanel( String dir ) {
        ...
        imageDestDirText.addKeyListener( new KeyAdapter() { // make the field
             ...
             *  is found; the rest of the event is ignored.
             *  transmits the new text from the evt.component (the textbox)
             *  when [bs] or keys are typed.  Also handles removing selected
             *  text.  All handling is done through a StringBuffer.
             */
            public void keyTyped( KeyEvent evt ) { // not accept letters
                ...
                if ( ch != KeyEvent.VK_BACK_SPACE /*&&
                         ...
                         code != KeyEvent.VK_COPY       &&
                         code != KeyEvent.VK_CUT        &&
                         code != KeyEvent.VK_UNDO       &&
    ...
    /** creates the destination panel which consists of the
     *   directory textline, the browse button, and a sub-panel
     *   to provide the name of the randomized image.
     */
    ...
    /** creates the destination panel which consists of the
     *   directory textline, the browse button for the translation
     *   key file.
     */
    ...
    /** puts together the panel of recursive box
     *   and directory-randomization level radio button.
    ...
    /** creates the random-button panel,which are two radio-buttons
     *   determining the naming actions for the output directories.
     */
    ...
    /** Creates the panel which consists of the
     *  OKAY button and the Cancel button.  The
     *  OKAY button is set to read &quot;Run&quot;, and the
     *  Cancel button is set to read &quot;Close&quot;.
     *   <p>
     *   a STOP button to stop the seperate thread
     *  is on the panel after the cancel button.
     *  The stop button reads &quot;Stop&quot;.
     *  <p>
     *  Places a &quot;Help&quot; button to the right
     *  of the &quot;Stop&quot; button.
     */
     ...
     *   <code>directory</code> as the root.  Each leaf of
     *   the tree is populated by <code>ViewFileTreeNode</code>s,
     *   representing directories.
     *   <p>
     *   It is selectable with TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION.
     *   Each item that is selected will be placed into a JTextArea
     *   displaying all the selected items.  And as the tree is expanded,
     *   the leaves will be explored, but as yet, there is no utility
     *   to un-explore them, or refresh their view once explored.
    ...
    private JTree buildNewSourceTree( String directory ) {
         ...
         * each selection on the tree.  on selection, the selected
         * list is cleared, and all current selections of the tree
         * are pasted into the selected list text box.
         *
         ...
         * added, the no nodes will be added, but the tree
    ...
    public void actionPerformed( ActionEvent ae ) {
        ...
        if ( command.equals( "New" ) ) {
            ...
            filePanel.remove( sourcePanel ); // remove the panel and replace it with a panel made with the name of the initial root node.
            filePanel.add(
                    sourcePanel = buildSourcePanel(
                            ( (ViewFileTreeNode) directoryTree.getSelectionPath().getLastPathComponent() ).getAbsolutePath() ),
                            BorderLayout.CENTER );
            ...
            AnonymizeDicomDirectories da = new AnonymizeDicomDirectories( s, imageDestDirectory, genericImageName,
                    sequenceStart, ui, toplevelOnly, changeFilenameBox.isSelected() );
             ...
             * to be available to the user, the user can
             * mouse around to view the logs, the selections
             * and go do other things in MIPAV right now.
             */
    ...
    /** checks to see if the JPanelAnonymizeImage has had
     *   any selections made to it.  If it hasn't, then
     *   the "Tag Options" tab is brought to the front (it
     *   contains the JPanelAnonymizeImage), and the warning
     *   message
     *   <i>"No fields to anonymize were selected!  Select a field." </i>
     *   is displayed.
     *   @return boolean if a selection in the JPanelAnonymizeImage
    ...
    /** Records all paths chosen in the source tree.  If no
     *   selections have been made, the "Directory" tab is
     *   brought to the front, and the warning message
     *   <i>"Select at least one directory to work on."</i>
     *   is displayed.
     *   @return boolean if a selection in the source tree
     *                   has been made, returns
     *                   <code>true</code>.  Otherwise, returns
     *                   <code>false</code>.
     */
    ...
    /** Makes the image destination directory.  If
     *   there is a problem in creating the destination
     *   directory, the "Directory" tab is brought to
     *   the front; the warning message describing the
     *   error is displayed; then the text field with the
     *   destination directory is given focus, and all
     *   available text is displayed.
     *   @return boolean if the selected destination
     *                   has been made, returns
     *                   <code>true</code>.  Otherwise, returns
     *                   <code>false</code>.
     */
    ...
    /** Locates the translation/key-file destination
     *   directory.  If the directory is not there already,
     *   the directory is made.  If there is a problem
     *   in creating the destination directory, the
     *   "Directory" tab is brought to the front;
     *   the warning message describing the error is
     *   displayed; then the text field with the
     *   destination directory is given focus, and all
     *   available text is displayed.
     *   If the translation/key-file is already present
     *   in the chosen directory, there will be an options
     *   dialog allowing the user to Overwrite the old
     *   translation/key-file, to append the the new
     *   translation/key to the end of the current file,
     *   or to cancel the entire operation (to look for
     *   a new location).
     *   @return boolean if the selected destination
     *                   has been made, returns
     *                   <code>true</code>.  Otherwise, returns
        ...
        try {
            if ( keyFile.exists() ) {
                ...
                int result = JOptionPane.showOptionDialog( this,
                        "\"patient.key.doc\" already exists.  What do you want to do with it?", "File exists...",
                        JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, possibilities,
                        new Integer( 0 ) );
                switch ( result ) {
                case 0:
            ...
            if ( noisyProcess ) {
                MipavUtil.displayError(
                        "security violation incurred while creating \"patient.key.doc\"; \n"
    ...
    /** converts the name and sequence JTextFields into
     *   native types.  Ensures that if the textFields are
     *   empty, the empty values don't cause problems.
     *   That means the generic name is the original generated
     *   random string and sequence is 0.
     */
    ...
    /** A psuedo-event-handler. Takes the event
     *   (An instance of AnonymizeDicomDirectories)
     *   resets the enabled status of the OK and
     *   Cancel buttons, appends the key-log text
     *   to the key-log and calls writeKeyFile().
     *
     *   @param AnonymizeDicomDirectories the event
     */
    ...
    /** creates a new keylog, writing which tags are to be
     *   removed from the image information; the table
     *   header for the image read/write logging is added.
     *   the string created here is not automatically turned
     *   into the keylog string.  that must be done by the
     *   caller.
    ...
    /** creates a keyFile named "patient.key.doc" in the destination directory
     *   as specified by the user.  the keyFile contains a tab-delimited table
     *   of original filenames and the anonymous filename which replaced it;
     *   it flushes the keyLog into the keyFile.  Any further writes to the keyLog
     *   will be writing into a new keyLog.  Writing to the keyFile later will
    ...
    /** makes the submitted directory as given by the
     *   inputted File, or the text as given in the given
     *   given textField.  Priority is given to the text
     *   in the textfield, so if the File's path does not
     *   agree with the path in the text field, the
     *   directory will be made with the path in the text
     *   field.  Should the text field not have <i>any</i>
     *   path, an IOException will be thrown indicating this
     *   problem.  If the directory does not exist, it will
     *   be created; if there are errors in creating the
     *   directory, an IOException will be thrown, with
     *   error message describing the problem, with a
     *   possible remedy as a suggestion.
     *
     *   @return File a directory
     *   @exception IOException  -- failure to create the directory
     */
    private File makeDirectory( File selDir, JTextField txtField )
        throws IOException {
    ...
    /**
     *   This thread anonymization processes all
     *   the Dicom files in the submitted source
     *   directories and places them into the
     *   submitted destination directory.  Note that
     *   these selected directories are processed serially.
     *   <p>
     *   When one directory is chosen as the
     *   source, all anonymized Dicom files in source
     *   are placed directly in the destination.  Any
    ...
    public class AnonymizeDicomDirectories extends Thread {
        ...
        private int sequenceStart = 0; // used as part of the image name; represents beginning of sequence for all processed image sets.
        private int sequence = 0; // used as part of the image name; increments for each image set processed -- that is, the next image number
         ...
         *   @param src[]    the list of directories to anonymize.  this
         *          may be as large or as small as needed
         *   @param dest     the output directory.  when the source-list
         *          lists but one file, the destination is the directory
         *          provided.  If the list is more than one, then the
         *          destination is the root of the output directorues.
         *          How the output directories are named depends on the
         *          boolean, toprand.
         *   @param aname    the name to use for all images processed.
         *   @param seqStart number to start the sequence for all names
         *           in the sequence of anonymousnames.
         *   @param ui       the user-interface needed to create the FileIO.
         *   @param toprand  determines which directory to anonymize, if any.
         *          When processing a list of more than one source,
         *          the top-level directory can be randomized (as if the
         *          selected list were the image-directories themselves),
         *          or the first sub-directory level of directories
         *          (as if the selected directories had held
         *          lists of patient directories).
         *          <ul><li>
         *          <code>NO_RANDOM_NAMES</code> indicates no directory names
         *          are to be changed.
         *          </li><li>
         *          <code>RANDOM_DIRECTORY_NAME</code> indicates the top-level
         *          is to be randomised,
         *          </li><li>
         *          <code>RANDOM_SUBDIRECTORY_NAME</code> indicates the first
         *          sub-directory level to be randomised.
         *          </li></ul>
         *          There is (true has??) no effect when only one level
         *          randomised.
         */
        AnonymizeDicomDirectories( File[] src, File dest,
                String aname, int startseq,
                ViewUserInterface ui, int toprand, boolean do_rename ) {
        ...
        /** tells the anonymizer to check and anonymize the
         *   child directories of the selected source directories.
         *   Default is false;
        ...
        /** Specified by Thread.  processes each directory
         *   selected directory, pausing after processing
         *   each selected directory to allow other threads
         *   run.  Notifies the 'anonDialog' of completion.
         */
        public void run() {
            stopThreadRequest = false; // this thread is now running.
            writeToLog( "------------------------------------------", true );
            writeToLog(
                    "Running new anonymization on " + srcDirs.length + " director"
                    + ( ( srcDirs.length == 1 ) ? "y" : "ies" ) + ":",
                    true );
            for ( int i = 0; i < srcDirs.length; i++ ) {
                 ...
                 * destination directory when we are
                 * randomizing on the top-level-directory.
                 * However, although it is unlikely to be true,
                 * we cannot assume that there are no DICOM images
                 * in the top directory.  We must generate an
                 * anonymous name to handle when there are DICOM
                 * images to anonymize there.
                 */
        ...
        /** stopping a thread prematurely is a
         *   co-operative process where the Thread
         *   checks for a stop-flag periodically, and
         *   stops itself by coming to the end of its
         *   <code>run()</code> method early.
         *   This co-operative method of premature
         *   Thread stop permits the Thread to close any
         *   open system resources it needed, and will
         *   use the natural death procedures.
         *   <p>
         *   In this case, in order to accept death,
         *   the Thread must close its open files;
         *   it will then back out of its recursion.
         */
        ...
        /** provides the string key of translations between source filename
         *   and destination filename.  The string is formatted with newlines and
         *   tabs seperating the filenames.
        ...
        /** opens top-level image directory.
         *   anonymizes the images in the top directory,
         *   when the class has been set to dig in recursively,
         *   anonymizes the top directories and calls openSubDirectory(...) to
         *   anonymize the files below.
         */
        private void processMainDirectory( File sourceDirectory, File destinationDirectory ) {
            ...
            if ( checkChildren ) {
                if ( !childDirsVector.isEmpty() ) {
                     ...
                     * so long as a stopThreadRequest has
                     * not come in, make a new random name,
                     * and then create a subdirectory in
                     * the destination direcoty with that
                     * random name; run clean-up, then
                     * append what was done (and to what)
                     * to the logs.
                     */
                    for ( i = 0; i < childDirsVector.size(); i++ ) {
                        ...
                        try {
                            ...
                            if ( noisyProcess ) {
                                MipavUtil.displayError(
                                        "Cannot anonymize \"" + ( (File) childDirsVector.get( i ) ).toString() + "\"" );
                            }
                        }
                        System.gc();
        ...
        /** traverses the directory, anonymizing as it goes.  the
         *   child directories are named after the source child-directory,
         *   and attempted to traverse (by calling processSubDirectory(File dest, File src)).
         *
        ...
        /** actually does the grunt-work of reading the designaated image-file,
         *   anonymizing it, and writing the file to the deisgnated destination
         *   directory.  Note that the anonymous-name is class-public.
         *   <i>don't overwrite it!</i>
         *   This method contains a list to all the random numbers used in making the
         *   image destination due to problems in making File.exists() work correctly
         *   without a SecurityManager.  (haven't tried WITH a SecurityManager).
         *   @param imageFile    a File that <i>could</i> be a DICOM image.  Non-DICOM images
         *                       should fail and simply return out of this method without
         *                       another thought.  DIMC images will be processed and saved into
         *                       the destination directory.
         *   @param destinationDirectory a File which is the directory where a processed DICOM
         *                               image file will be saved.
         *   @param randomSuffixList array of ints, which are filled from 0 to max lengh of
         *                           an unsorted numbers.  each of which becomes the random
         *                           name for the file.
         *   @param numberOfSuffices current number of used slots in randomSuffixList.
         *                           not to be larger than randomSuffixList.length
         *   returns boolean if the given image file was properly written to the
         *                   destination directory, this method returns <code>true</code>.
         *                   <code>false</code> gets returned if the file either did not
         *                   get written correctly (which would generate an exception),
         *                   or if the file was non-DICOM, and was therefor ignored.
         ...
         */
        private boolean anonymizeImage( File imageFile, File destinationDirectory,
                int[] randomSuffixList, int numberOfSuffices ) {
            String anonName;
            int randomNumber;
            ...
            try {
                ...
                if ( mi != null ) {
                    ...
                    try {
                        do {
                            ...
                            // come up with another name, can cause permenant lock if all available
                            // choices are made (ie., more than 10000 imags are in the directory)
                    ...
                    writeOpts.setSaveInSubdirectory( false ); // should be false by default.  we want to preserve current writeOpt structure
                    ( (FileInfoDicom) mi.getFileInfo( 0 ) ).setValue(
                            "0010,0010", anonymousName );
                    ...
                    if (doRename) {
                        ...
                        writeToKey(imageFile.getAbsolutePath(), writeOpts.getFileDirectory() +
                                   writeOpts.getFileName());
                    }
                ...
                Preferences.debug(
                        "anonymizeImage: (Negative Array Size Error " + "while reading: >" + imageFile
        ...
        /** sets the genericname, and the start sequence to use when
         *   generating the anonymousname.  When processed, the
         *   anonymous name will be genericname_[seq].  the following
         *   name will be genericname_[seq+1].
         *   @param aname    the name to use for all images processed.
         *   @param seqStart number to start the sequence for all names
         *                   in the sequence of anonymousnames.
        ...
        /** creates a string to use as a name out of the
         *   generic name, and the sequence.  When processed, the
         *   anonymous name will be genericname_[seq].  the following
         *   name will be genericname_[seq+1].
        ...
        /*private String generateRandString() {
         String rhs = Integer.toHexString(rand.nextInt());
         try {return rhs.substring(0,5);}
         catch(IndexOutOfBoundsException ioobe) {
         return rhs;
         }
         } */

        /** Checks for the existance of the number <code>rnum</code> in the
         *   unsorted array rlist.  The elements of <code>rlist</code> exist
         *   from <code>rlist[0]</code> to <code>rlist[rnumptr-1]</code>.
         *   Of course, if <code>rnumptr</code> is greater than
         *   to <code>rlist.length</code>, an
         *   <code>ArrayIndexOutOfBoundsException</code> will be
         *   thrown.  Check for it.  This of course means that
         *   <code>rnum</code> is not in the full array.  This method
         *   does not check the array against the length of the array,
         *   but rather the number of assigned members, because only a
         *   partially-full list would return unpredicatable results.
         *
         *   @param rnum     number we are interested in finding.
         *   @param rlist    unsorted array of integers at the bottom elements.
         *   @param rnumptr  index into the array to stop searching; also,
         *                   indicates the number of elements with an assigned value.
         *   @return <code>true</code> when <code>rnum</code> is found
         *           between <code>rlist[0]</code> and <code>rlist[rlistptr-1]</code>.
         *           <code>false</code> when <code>rnum</code> is not in the array.
         */
        ...
        /** Method to append the string to the log, adding a newline to
         *   the end, if requested, and appending a colon when not adding
         *   a newline.  The log is not replaced by the incoming
         *   string.
         *   @param  logappendage string to write to the log
         *   @param  completeString  <code>true</code> writes a newline to the log.
         *                           <code>false</code> makes this an unfinished string.
         */
        ...
        /** appends the original string, then two tabs then the newKey,
         *   followed by a newline to the keylog.  old keylog is not
         *   overwritten.
         */
        ...
        /** appends the name string, in between parens trailed by a
         *   colon.  Eg., <i>(anon_name:)"</i>.  To be used before
         *   filtering the tags out of images (as those actions are logged
         *   in sequence.)
         */
        ...
        /** no real need for complete listener capabilities,
         *   so this is to take its place.
        ...
        /** a list of JTextAreas to be notified when a change
         *   in the process/output log is made.  this permits
         *   almost-real-time updates to output devices
         *   this method adds the selected list of JTextAreas
         *   to receieve the output logs.
         */
        ...
        /** called when an update is made to the output log,
         *   this method updates all the process logging
         *   JTextAreas.
         */
