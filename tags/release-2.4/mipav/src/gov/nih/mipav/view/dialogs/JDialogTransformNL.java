...
public class JDialogTransformNL
    extends JDialogBase
    implements AlgorithmInterface, ScriptableInterface, ChangeListener {
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (UI.isScriptRecording()) {
              ...
              UI.getScriptDialog().append("TransformNL " +
                                          UI.getScriptDialog().
              ...
              UI.getScriptDialog().append(UI.getScriptDialog().getVar(resultImage.
                                                                      getImageName()) + " " + targetXDim + " " +
                                          targetYDim + " " +
                                          targetZDim + " " + resultResolutions[0] + " " + resultResolutions[1] + " " +
                                          resultResolutions[2] + " " + targetUnits[0] + " " + targetUnits[1] +
                                          " " + targetUnits[2] + " " + fmodel + " " + coords + " " + coeffp + " ");
              for (int i = 0; i < coords; i++) {
                  for (int j = 0; j < coeffp; j++) {
  ...
  */
  ...
  private void callAlgorithm() {
    ...
    algoTrans = new AlgorithmRegNonlinear(image, targetXDim, targetYDim,
                                          targetZDim, resultResolutions,
                                          targetUnits, fmodel, es, interp,
                                          doClip, transformVOI);
  ...
  public String parametersFileMenu() {
    ...
    try {
      ...
      MipavUtil.displayError(
          "Out of memory: JDialogTransformNL.parametersFileMenu");
      return null;
    }
