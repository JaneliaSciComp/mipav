 ...
 * control the degree of blurring in all dimensions and indicate if a
 * correction factor be applied to the z-dimension to account for differing
 * resolutions between the xy resolutions (intra-plane) and the z resolution
 * (inter-plane). The user has the option to generate a new image or replace
 * the source image. In addition the user can indicate if he/she wishes to have
 * the algorithm applied to whole image or to the VOI regions. It should be
 * noted that the algorithms are executed in their own thread.
 *
