...
/**
 *   Dialog to alter memory allocation of the
 *   runtime environment.  The runtime memory allocation for the
 * InstallAnywhere executable can be found in the
 * LAX file: &quot;<tt>mipav.lax</tt>&quot; or &quot;<tt>iaso.lax</tt>&quot;
 * within most environments or the file &quot;<tt>Info.plist</tt>&quot; in a
 * Darwin/Mac OS 10 environment.
 * <p>
 * Reads the InstallAnywhere start up file
 * then parses it, line-by-line, to come up with the memory options for the
 * dialog.  Waits for user input to declare what memory settings should
 * be applied into the start-up file.  InstallAnywhere will read those
 * settings for the next time it restarts the application.
 *
 */
public class JDialogMemoryAllocation extends JDialogBase {
    ...
    /** Indictes that a particular file is a &quot;lax&quot;-format file.
     *  Literally, <code>LAX</code>
     */
    private static final String laxType = "LAX";

    /** Indictes that a particular file is a &quot;xml&quot;-format file.
     *  Literally, <code>XML</code>
     */
    private static final String xmlType = "XML";

    // java startup options used in XML file
    /** Flag to the java-runtime (as used in the XML-files) to indicate a
     *  memory-option.  Literally, <code>-X</code>.
     */
    private static final String optionFlag = "-X";

    /** Flag to the java-runtime (as used in the XML-files) to indicate an
     *  initial heap-size memory-option.  Literally, <code>ms</code>.
     */
    private static final String initHeapOption = "ms";

    /** Flag to the java-runtime (as used in the XML-files) to indicate an
     *  maximum heap-size memory-option.  Literally, <code>mx</code>.
     */
    private static final String maxHeapOption = "mx";

    // as found in the lax file.  Hope it doesn't change, or none of this will work
    /** Flag to the java-runtime (as used in the LAX-files) to indicate an
     *  initial heap-size memory-option.  Literally,
    ...
    /** Flag to the java-runtime (as used in the LAX-files) to indicate an
     *  maximum heap-size memory-option.  Literally,
                                    ...
                                    boolean             checkOnPreferences)
    {
        ...
        try {startupFile = getStartupFile(ui);}
    ...
    /**
     *  builds the dialog interface to get user to
     *  change the lax-file settings for the JVM to
     *  use on java start-up.  calls to read the InstallAnywhere start up file
     * then parses it, line-by-line to come up with the memory options for the
     * dialog.  Waits for user input to declare what memory settings should
     * be applied into the start-up file.  InstallAnywhere will read those
     * settings for the next time it restarts the application.
     *
     * Failure to read the startup file will dispose the dialog before it is
     * displayed.
     *
     *  @param ui the parent frame of this dialog.  supplies the app-icon
     *            for the upper-right corner of the dialog-window
     */
    public JDialogMemoryAllocation(ViewUserInterface ui) {
        super(ui.getMainFrame(), true);     // enforce modality
     ...
     * Buttons include OK, Cancel, Use Preferences and HELP
            ...
            if (heapStart <= heapMax) {
                ...
                if (os != Preferences.OS_MAC) {
                    int response = JOptionPane.showConfirmDialog(this,
                        "Restart " + progName.toUpperCase() + " to apply memory changes?",
                        "Restart needed",
                    ...
                    if (response == JOptionPane.YES_OPTION) {
                        try {
                            ...
                            MipavUtil.displayError("Error restarting the application (./" + progName + ").  Please exit and start " + progName.toUpperCase() + " again manually to apply the new settings.");
                            dispose();
                            return;
                        }
                    }
    ...
    /** This method returns the startup file which contains the MIPAV start-up
     *  options.  For most systems, the options are kept in a file called
     *  &quot;mipav.lax&quot;.  For Macintosh OS 10 (X), the application
     *  in the application menu is actually a directory &quot;mipav.app&quot;
     *  In this case, the start-up file is in
     *  &quot;mipav.app/Contents/Info.plist&quot;.  This method gets the
     *  application name (&quot;mipav&quot; or &quot;iaso&quot;) and looks for
     *  a .lax file with that name; if it cannot find a .lax file, it then
     *  looks for a file &quot;Info.plist&quot; in the directory with the name
     *  of the application (.app)/Contents/.
     *<p>
     *  Ideally, the GetPath would look in application.app/Contents, but for now
     *  that location is found here.
     *
     *  <p>
     *  @param ui The main user-interface.
     *  @throws FileNotFoundException when the app title is not in the
     *  preferences file or the ViewUserInterface is <code>null</code>.
     */
    public static File getStartupFile(ViewUserInterface ui) throws  FileNotFoundException
    {
        ...
        if (startPath == null) {
            ...
            Preferences.debug("JDialogMemoryAllocation: Looking for " +
                            "Info.plist as the startfile: "+startPath+"\n");
    ...
    /**
     *  Reads the startup file that has already been found.  Searches for,
     *  and sets the values of if found, the initial &amp; maximum heap text
     *  fields.  If the Preferences indicate that debug is on, the entire
     *  startup file is displayed to verify that the dialog indicates what is
     *  in the file.
     * @return if the file can be read from & written to.
     * @exception IOException, FileNotFoundException
     */
    protected boolean readStartupFile()
        throws  IOException,
                FileNotFoundException
    {
        ...
        if (!startupFile.canRead()) {
            MipavUtil.displayError("Not able to read the InstallAnywhere start-" +
                                   "up file: " + startupFile.getAbsolutePath() +
                                   "\n" +
                                   "To alter the memory allocation, either " +
                                   "set the permissions or \n" +
                                   "contact the system administrator.");
            Preferences.debug("JDialogMemoryAllocation:Not able to read the "+
                            "InstallAnywhere start-up file: " +
                            startupFile.getAbsolutePath() + "\n",
                            2);
            return false;   // no point in continuing if we can't do both
        }

        Preferences.debug("JDialogMemoryAllocation: Reading from \""+
                            startupFile.getAbsolutePath() + "\"\n", 5);
    ...
    /** reads the given line of a LAX file to search for the
     *  initial heap size and maximum heap size Strings.
     *  <p>
     *  if the line starts with initial or max size strings
     *  given by initHeapLAX or maxHeapLAX, the value is converted into
     *  megabytes and loaded into the appropriate textBox.
     *  <p>
     *  If the value found is not possible (ie., the conversion threw a
     *  NumberFormatException), the substitute "1" is given.
     *
     *  @param line A line of a LAX file which is to be searched for the
     *              memory strings.
     *  @return the heap string, including LAX file variable name,
     *      the value in the text box as it would be found in the file.
     */
    protected static String[] interpretLAX(String line) {
        ...
        if (line.startsWith(initHeapLAX)) {
            try {
                ...
                Preferences.debug("JDialogMemoryAllocation: Cannot convert "+
            ...
            try {
                ...
                MipavUtil.displayError("JDialogMemoryAllocation: Cannot " +
                                        " convert maximum memory value.\n" +
                                        "Substituting for a known acceptable value.");
                Preferences.debug("JDialogMemoryAllocation: Cannot convert "+
    ...
    /** reads the given line of a plist file to search for the
     *  initial heap size and maximum heap size Strings.
     *  <p>
     *  If the file contains the &quot;-X&quot; java option the value is
     *  parsed out.
     *  the value is converted into megabytes, if needed
     *  and loaded into the appropriate textBox.
     *  <p>
     *  If the value found is not possible (ie., the conversion threw a
     *  NumberFormatException), the substitute "1" is given.
     *
     *  @param line A line of an XML file which is to be searched for the
     *              memory strings.
     *  @return the heap string, including XML formatting,
     *      the value in the text box as it would be found in the file.
     */
    ...
    /**
     * Reads the InstallAnywhere startup file and returns the start up heap
     * memory strings.
     * Method can read either LAX (Window,UNIX) or Info preferences list
     * (Mac OS X) file, finds only the two entries
     *  defined by {@link JDialogMemoryAllocation#initHeap} and
     *  {@link JDialogMemoryAllocation#maxHeap}.  It returns the
     *  values of the associated entries in Megabytes.
     *
     *  @param lax The File referring to the InstallAnywhere startup options
     *              LAX or PLIST file
     *  @return String[0], the initial heap size-text,
     *          String[1], the maximum heap size-text
     *  @throw IOException when the LAX file cannot be read.
     *  @throw FileNotFoundException When the LAX file cannot be found.
     */
    public static String[] readStartupFile(File lax)
        throws  IOException,
                FileNotFoundException
    {
    ...
    /** write startup options file which is used during InstallAnywhere to
     * run the executable.  The starting options file it writes out is
     * the one which would be native to the system (that is, on Windows or UNIX
     * systems, the startup file is a LAX file; on the Macintosh OS 10 systems,
     * it is the Info.plist file) as determined when by {@see getStartupFile}
     * during dialog instantiation.  This method writes initHeapText and
     * maxHeapText to the appropriate start and max values.
     */
    protected void writeStartOptionsFile() throws IOException {
        ...
        // this file is!
        if (startupFile == null)        {
            throw new IOException("Unable to open " + startupFile.getAbsolutePath()
                                + " for writing.");
        }
        ...
        for (i = 0; i < laxContents.size(); i++) {
            ...
            if (fileType == laxType) {
                if (line.indexOf(initHeapLAX) != -1) {
                    try {line = initHeapLAX + convertMBytesToBytes(initHeapText.getText());}
                    catch (NumberFormatException nfe) {
                    ...
                    try {line = maxHeapLAX + convertMBytesToBytes(maxHeapText.getText());}
                    catch (NumberFormatException nfe) {
                        MipavUtil.displayError("JDialogMemoryAllocation: Cannot convert the value.\n" +
                                               "Substituting for a known acceptable value.");
                ...
                if (line.indexOf(initHeapOption) != -1) {
                    line = "\t\t\t\t" +
                            "<string>-Xms"+initHeapText.getText()+"M</string>";
                }
                else if (line.indexOf(maxHeapOption) != -1) {
                    line = "\t\t\t\t" +
                            "<string>-Xmx"+maxHeapText.getText()+"M</string>";
                }
            }
    ...
    /**
     *   convert the number given by the string
     *   (assuming it is a number indicating a
     *   quantity of bytes) to a number indicating
     *   a quantity of megabytes.  If the input
     *   is improperly formatted it throws a
     *   NumberFormatException.
     * @param byteString a number indicating a quantity of bytes
     * @return String a number indicating the rounded value of the input as a
     *                megabyte
     * @throw NumberFormatException if the byteString cannot be represented
     *                              as a number.
     */
    protected static String convertBytesToMBytes(String byteString)
    ...
    *   convert the number given by the string
    *   (assuming it is a number indicating a
    *   quantity of megabytes) to a number indicating
    *   a quantity of bytes.  If the input
    *   is improperly formatted it throws a
    *   NumberFormatException.
    ...
    *                   byte
    */
    protected static String convertMBytesToBytes(String megabyteString)
        throws  NumberFormatException
    {
        long megabyteValue;
        long byteValue;
    ...
    *   convert the number given by the string
    *   (assuming it is a number indicating a
    *   quantity of kilabytes) to a number indicating
    *   a quantity of megabytes.  If the input
    *   is improperly formatted it throws a
    *   NumberFormatException.
    ...
    *                   byte
    */
    protected static String convertKBytesToMBytes(String kilobyteString)
        throws NumberFormatException
    {
        long megabyteValue;
        long kilobyteValue;
    ...
    /** creates the main panel with all the inputs..
     *  argument <code>true</code> includes preferences if available.
...
//                maxPref.setForeground(Color.DARK_GRAY);
     ...
     * Over-rides {@link JDialog#buildButtons()} if includesPrefs is
     * <tt>false</tt>, otherwise, it returns what JDialog#buildButtons()
     * returns.
     * @param includePrefs If the preferences are available, the
     *                      &quot;Use Preferences&quot; button is added to the
     *                      panel when it is returned.
     * @returns A JPanel which holds the buttons for user input.
     */
