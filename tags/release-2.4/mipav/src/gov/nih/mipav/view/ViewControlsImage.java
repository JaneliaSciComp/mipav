 ...
 *   Toolbars are created and added to the panel and the listener for
 *   the toolbars is the image frame. If the image frame is to display
 *   two images then alpha blending tools are added and displayed.
 *
 *   @version    0.1 Dec 15, 1998
 *               update  9 august 2001
 *   @author     Matthew J. McAuliffe, Ph.D.
 */
public class ViewControlsImage extends JPanel
    ...
    /* the slice slider is a JSlider defining an integer number
     * of steps.  That number of steps is given by sliderResolution.
     * Although the major&minor tick marks will not necessarily
     * align with any particular slice, there is built-in
     * ability to have the tick-marks correspond to a particular
     * percentage of an image set.  ie, distance between each major
     * tick will represent 1/10 of image-set, and the minor
     * ticks will be 1/5 of that 1/10.  Should the number of slices
     * the slider must refer to be larger than the resolution
     * the slider will not always refer to the correct slice when moved.
     */
     ...
     *   @param voiIndex     Indicates the index of the currently (or previously) selected VOI
     */
    public void buildToolbar( boolean showImage, boolean showVOI,
            boolean showPaint, boolean showScripting,
            int voiIndex ) {
        ...
        if ( numberOfDimensions == 4 ) {
            ...
            tImageSlider = new JSlider( JSlider.HORIZONTAL, 0, sliderResolutionInt,
                                        sliderResolutionInt/2 );
    ...
    /**
     * Invokes the toolbar builder to update the scripts combo
     *  box based on the new script directory
     *
    ...
    /**
     *	Adds a component to the toolbar panel using the given
     *   gridbag constraints.
    ...
    /**
     *   Places the pointer on the (slice) slider bar to a location
     *   corresponding to the given slice value (within the set)
    ...
    /**
     *   Places the pointer on the (time) slider bar to a location
     *   corresponding to the given slice value (within the set).
    ...
    /**
     *	Displays the panel which controls which image is the
     *   "active image" displayed when imageB is available.
     */
    ...
    /**
     *	Removes the controls to the display of which image is
     *   "active" in the frame when imageB is not available.
     */
     ...
     *   Places the pointer on the (slice) slider bar to a location
     *   corresponding to the given slice value (within the set)
     *   allowing notification of the listeners.
    ...
    /**
     *	Places JLabels under the (slice) slider bar
     *   indicating the minimum, the maximum and the
     *   midpoint of the set.
    ...
    /**
     *	Places JLabels under the (time) slider bar
     *   indicating the minimum, the maximum
     *   and the midpoint of the set.
    ...
    public void stateChanged( ChangeEvent e ) {
        ...
        if ( source == zImageSlider ) {
            newValue = Math.round(zImageSlider.getValue() /
                                  sliderResolutionFloat * ( frame.getImageA().getExtents()[2] - 1 ) - 0.01f );
            ...
            if ( frame.getImageA().getNDims() == 4 ) {
                newValue = Math.round( tImageSlider.getValue() /
                        sliderResolutionFloat * ( frame.getImageA().getExtents()[3] - 1 ) - 0.01f );
            } else {
                newValue = Math.round(tImageSlider.getValue() /
                          sliderResolutionFloat * ( frame.getImageB().getExtents()[3] - 1 ) - 0.01f );
            }
