 ...
 *
 */
public class JDialogImageCalculator
    extends JDialogBase
   ...
   * @param string of an advanced function
   */
      ...
      if (displayLoc == NEW) {
        ...
        try {
          ...
          resultImage = new ModelImage(imageA.getType(), imageA.getExtents(),
                                       makeImageName(imageA.getImageName(),
              "_calc"),
                                       userInterface);
          // Make algorithm
          mathAlgo = new AlgorithmImageCalculator(resultImage, imageA, imageB,
                                                  opType, clipMode, true,
                                                  adOpString);
          ...
          System.gc();
          MipavUtil.displayError(
              "Dialog Image math: unable to allocate enough memory");
          return;
        }
      }
      else {
        try {
          ...
          mathAlgo = new AlgorithmImageCalculator(imageA, imageB, opType,
                                                  clipMode, true, adOpString);
          // This is very important. Adding this object as a listener allows the algorithm to
          // notify this object when it has completed of failed. See algorithm performed event.
          // This is made possible by implementing AlgorithmedPerformed interface
          mathAlgo.addListener(this);
