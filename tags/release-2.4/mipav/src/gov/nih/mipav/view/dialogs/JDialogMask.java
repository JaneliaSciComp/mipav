...
public class JDialogMask
  ...
  // method is meant to be used when the algorithm needs to be performed non-interactively,
  // hence the hardcoding of the variables
  ...
  /**
   *	Once all the necessary variables are set, call the Mask
  ...
  private void callAlgorithm() {
    ...
    if (displayLoc == NEW) {
      try {
        ...
        // Make algorithm
        if (image.isColorImage()) {
            maskAlgo = new AlgorithmMask(resultImage, image, value, valueG, valueB,
                                         polarity, true);
        }
        else {
  ...
  private void setMinMax() {
    ...
    if (image.getType() == ModelStorageBase.BOOLEAN) {
      ...
      max = 127;
             ...
             (image.getType() == ModelStorageBase.ARGB)) {
      min = 0;
      max = 255;
    }
      ...
      max = 32767;
             ...
             (image.getType() == ModelStorageBase.ARGB_USHORT)) {
      min = 0;
      max = 65535;
    }
