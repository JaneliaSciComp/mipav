...
/**
 * MouseRotateExt extends the MouseRotate class by providing public access
 * to certain states of the MouseBehavior that are stored as protected
 * values with no other public accesss.
 *
 * Also overrides the mouseReleased routine so that the callback can be
 * notified one more time of the updated transform, this time with
 * the buttonPress flag being set to false.
 */
...
public class MouseRotateExt extends MouseRotate {
   ...
   /** Mouse behavior callback reference. */
    ...
    /**
     * Return whether the button press state is set which can be used
     * as an indication of whether or not the associated TransformGroup
     * instance is being interactively updated.
    ...
    /**
     * Override mouse release so that we can force another update of
     * the current transform to the callback.
     * @param e MouseEvent
     */
