 ...
 *
 */
public class JDialogExtractObject
    extends JDialogBase
  ...
  public JDialogExtractObject(Frame theParentFrame, ModelImage im) {
    ...
    if (nVOI == 0) {
      MipavUtil.displayWarning
          ("No contour VOI is present, so a .SUR file must be opened");
    }
  ...
  private void open() {
    ...
    if (iQuantity > 1) {
      MipavUtil.displayWarning(iQuantity +
          " ModelTriangleMeshes are present.  Only reading first");
    }

    ViewJProgressBar progress = new ViewJProgressBar("Loading surface",
        "Loading surface", 0, 100, false, null, null);
   ...
   *	All new images should be of the same dimensions.
  ...
  private boolean checkImage(ModelImage testImage) {
    ...
    for (int i = 0; i < image.getNDims(); i++) {
      if ( (testImage != null) &&
          (image.getExtents()[i] != testImage.getExtents()[i])) {
        if (userInterface.isScriptRecording())
          userInterface.getScriptDialog().removeLine();
        MipavUtil.displayError("Error! For dimension = " + i + " " +
                               image.getImageName() +
                               " has length = " + image.getExtents()[i] +
                               " while " + testImage.getImageName() +
  ...
  public void callAlgorithm() {
    ...
    try {
      ...
      // Make algorithm
      extractObjectAlgo = new AlgorithmObjectExtractor(image, srcVOI, justInit,
          saveGVF, triMesh, uvf, vvf, wvf);
      ...
      if (runInSeparateThread) {
        ...
        extractObjectAlgo.setActiveImage(isActiveImage);
