...
public class JDialogClose
   ...
   *	@param im	Source image.
   */
  public JDialogClose(ViewUserInterface UI, ModelImage im) {
    super();
    if (im.getType() != ModelImage.BOOLEAN &&
        im.getType() != ModelImage.UBYTE &&
        im.getType() != ModelImage.USHORT) {
  ...
  public void insertScriptLine (AlgorithmBase algo) {
      if (algo.isCompleted()) {
          if (userInterface.isScriptRecording()) {
              ...
              userInterface.getScriptDialog().append("Close " +
              ...
              if (displayLoc == NEW) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.
                                                         ...
                                                         regionFlag + " " + itersD + " " + itersE + " " + kernel +
                                                         " " + kernelSize + " " + do25D + "\n");
              } else {
                  userInterface.getScriptDialog().append(userInterface.
    ...
    if (image.getNDims() == 2) { // source image is 2D
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          closeAlgo2D = new AlgorithmMorphology2D(resultImage, kernel,
                                                  kernelSize,
                                                  AlgorithmMorphology2D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
          if (regionFlag == false)
          ...
          MipavUtil.displayError(
              "Dialog close: unable to allocate enough memory");
          if (resultImage != null) {
        ...
        try {
          ...
          closeAlgo2D = new AlgorithmMorphology2D(image, kernel, kernelSize,
                                                  AlgorithmMorphology2D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
          if (regionFlag == false)
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          closeAlgo3D = new AlgorithmMorphology3D(resultImage, kernel,
                                                  kernelSize,
                                                  AlgorithmMorphology3D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
          if (regionFlag == false)
        ...
        try {
          // Make algorithm
          closeAlgo3D = new AlgorithmMorphology3D(image, kernel, kernelSize,
                                                  AlgorithmMorphology3D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
          if (regionFlag == false)
      ...
      if (displayLoc == NEW) {
        try {
          ...
          resultImage.setImageName(name);
          // Make algorithm
          closeAlgo25D = new AlgorithmMorphology25D(resultImage, kernel,
                                                  kernelSize,
                                                  AlgorithmMorphology2D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
        ...
        try {
          // No need to make new image space because the user has choosen to replace the source image
          // Make the algorithm class
          closeAlgo25D = new AlgorithmMorphology25D(image, kernel, kernelSize,
                                                  AlgorithmMorphology25D.CLOSE,
                                                  itersD, itersE, 0, 0,
                                                  regionFlag);
          if (regionFlag == false)
            closeAlgo25D.setMask(image.generateVOIMask());
