...
/**
 * Wraps the ViewJProgressBar instance which allows an arbitrary minimum
 * and maximum value to be specified for the ends of the progress bar.
 */
public class ViewJSimpleProgressBar
    extends ViewJProgressBar
{
    // This is the actual resolution of progress bar which we will be using
    // regardless of the actual range specified.
    private final static int RESOLUTION = 100;

    // Remember the range of input values for the progress bar.
    // It is not required that the minimum be less than the maximum.
    private float m_fRangeMin;
    private float m_fRangeMax;

    // Remember the value passed to the setValue method of the JProgressBar.
    // When valid, should be in range [0,RESOLUTION).
     ...
     * Also resets the current amount of progress to the minimum value
     * specified, which is effectively 0% complete.
     ...
     * @param iValue integer which should be within the min/max limits
     * of the progress bar previously specified with by calling setRange
     * method.
     */
    ...
    /**
     * Called to set the current progress bar indicator position.
     * @param fValue floating point value which should be within the
     * min/max limits of the progress bar previously specified with
    ...
    public void setValue (float fValue)
    {
        ...
        if (m_fRangeMin != m_fRangeMax)
        {
            ...
            iMappedValue =
                    Math.round(((fValue-m_fRangeMin)*RESOLUTION)/fRange);
            if (iMappedValue < 0)
            {
                iMappedValue = 0;
            }
