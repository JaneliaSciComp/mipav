 ...
 * align two images with the mouse and then call the AlgorithmRegOAR2D
 * registration algorithm to create a mosaic image with the two aligned
 * images. Multiple images can be added to the mosaic and aligned one at a
 * time.
 */
...
{
    /**
     *   JFrameRegistrationMosaic - Creates new window for manual
     *   (mouse-based) registration of two images.
     */
    ...
    /**
     *	initGUI - Initializes GUI toolbar and buttons and displays the
     *	registration window.
     */
    ...
    /**
     * createTexturedPolygon - Creates a texture-mapped polygon with the
     * BufferedImage displayed as the texture. The texture-mapped polygon is
     * created so that the displayed texture and size of the polygon match the
     * size in pixels of the original image loaded from file -- even when the
     * original image size is not a power of two. The displayed image must
     * match the original data image so that the registration is accurate.
     *
    ...
    /**
     * createMosaicOpenDialog - Creates a file open dialog for image files
     * (.jpg, tiff, etc.). If a new file is opened it is mapped onto a polygon
     * and placed in the scene graph:
    ...
    /**
     * toggleSelectedImage - Toggles which image is currently
     * selected. Changes the color of the image borders.
     */
    ...
    /**
     * registerImages - Registers the reference and tile images based on the
     * how the user positions the images with the mouse.  Two new registered
     * images are created, each containing one registered sub-image. The
    ...
    private boolean registerImages()
    {
         ...
         * with offsets so that the two images are aligned based on the
         * user-positioned transform.
         *
         * (2) Initialize The reference and tile mask images for weighted
         * registration.
         *
         * (3) apply the inverse transformation to the tile image pixels to
         * determine where to place the it in the new overlapped image:
         *
        ...
        try {
            new ViewJFrameImage(kReferenceMask, null, new Dimension(610, 200));
        }
        catch (OutOfMemoryError error) {
            MipavUtil.displayError(
                                   "Out of memory: unable to open new frame");
        ...
        try {
            new ViewJFrameImage(kTile, null, new Dimension(610, 200));
        }
        catch (OutOfMemoryError error) {
            MipavUtil.displayError(
                                   "Out of memory: unable to open new frame");
        ...
        try {
            new ViewJFrameImage(kTileMask, null, new Dimension(610, 200));
        }
        catch (OutOfMemoryError error) {
            MipavUtil.displayError(
                                   "Out of memory: unable to open new frame");
    ...
    /**
     * algorithmPerformed - when the alignment algorithm finishes, the
     * transformed tile is blended with the reference image and displayed:
     *
    ...
    /**
     * backupMosaic -- Backs up the reference & tile polygon shapes, borders,
     * ModelImages, and TransformGroups before the registration is called, so
     * that registration can be undone by the user:
     */
    ...
    /**
     * undoMosaic -- Restores the backed-up ImageTransforms, PolygonShapes,
     * BorderShapes, and ModelImages after an undo button press:
     */
    ...
    /**
     * initData -- Initializes the selected, open, reference and tile
     * indexes. Called on startup and after a closeAllImages()
     */
