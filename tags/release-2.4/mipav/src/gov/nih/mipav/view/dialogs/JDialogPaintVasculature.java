...
import gov.nih.mipav.view.PaintGrowListener;
import gov.nih.mipav.view.ViewJFrameBase;
import gov.nih.mipav.view.ViewJFrameImage;
import gov.nih.mipav.view.ViewJFramePaintVasculature;
...
/**
 * Dialog to facilitate the painting of a region of an image
 * (such as the vascualture in a liver mip).
...
public class JDialogPaintVasculature extends JDialogBase
    ...
    /**
     * Button to undo the last region grow change (either a click of the grow
     * button, or an adjustment of the threshold sliders).
     */
    private JButton undoButton;

    /**
     * Button to indicate that the painting is finished and
     * the dialog and attached image frame should be closed.
     */
    ...
    /**
     * Boolean which indicates that the seed point intensity bounds were changed from
     * one of the text fields and not from one of the sliders.
     */
    ...
    private void initToolbar() {
        ...
        JToggleButton pointButton = toolbarBuilder.buildToggleButton( "PointRegionGrow", "Region grow from auto-generated points which are likely to be vasculature.", "pointROI", modeGroup );
        pointButton.setSelected( true );
        modeBar.add( pointButton );
        modeBar.add( toolbarBuilder.buildToggleButton( "ArbitraryRegionGrow", "Select an arbitrary point to region grow from.", "paintcan", modeGroup ) );
        ...
        if ( command.equals( "Grow" ) ) {
            ...
            for ( int i = 0; i < points.size(); i++ ) {
                if ( ( (VOIPoint) points.get( i ) ).isActive() ) {
                    ...
                    seedPoints.add( pt );
                    rfastFrame.getComponentImage().regionGrow( (short)pt.x, (short)pt.y, (short)pt.z, intensityValue, null, true );
                }
            }
    ...
    public void stateChanged( ChangeEvent event ) {
        ...
        if ( !isCloseOkay && ( source == upperDeltaSlider || source == lowerDeltaSlider ) && ( !upperDeltaSlider.getValueIsAdjusting() && !lowerDeltaSlider.getValueIsAdjusting() ) ) {
            rfastFrame.getComponentImage().setLess( intensityValue - Float.parseFloat( lowerDeltaTF.getText() ) );
            rfastFrame.getComponentImage().setMore( Float.parseFloat( upperDeltaTF.getText() ) - intensityValue );
            ...
            for ( int i = 0; i < seedPoints.size(); i++ ) {
                ...
                rfastFrame.getComponentImage().setRegionGrowVars( (short)pt.x, (short)pt.y, (short)pt.z, intensityValue );
    ...
    /**
     * Allow the user to use left and right keys to adjust the intensity bounds sliders,
     * and update the region grow when the user hits enter from within the bounds text fields.
    ...
    /**
     * Tells the dialog to commit the current set of region grows and prepare
     * for another set region grows, or the closing of the dialog.
    ...
    public void mouseDragged( MouseEvent event ) {
        ...
        short z = (short)( (ViewJFramePaintVasculature) parentFrame ).getMIPZValue( mipX + mipY * rfastFrame.getActiveImage().getExtents()[0] );
     ...
     * @return  -1.
     */
    public float getFuzzyThreshold() { return -1; }

    /**
     * Placeholder.
     * @return  false.
     */
    public boolean getUseVOI() { return false; }

    /**
     * Placeholder.
     * @return  false.
     */
    public boolean getDisplayFuzzy() { return false; }
     ...
     * @return  -1.
     */
    public int getMaxDistance() { return -1; }
}
