 ...
 *
 */
public class JDialogIHN3Correction
    extends JDialogBase
  ...
  public JDialogIHN3Correction(Frame theParentFrame, ModelImage im) {
    ...
    if ( (image.getNDims() == 3) && (image.getExtents()[2] <= 3)) {
      MipavUtil.displayError(
          "3D images must have at least 4 slices to use IHN3Correction");
      return;
    }
  ...
  private void initVars() {
    ...
    if ( (resXUnit == FileInfoBase.INCHES)      ||
         ...
         (resXUnit == FileInfoBase.ANGSTROMS)   ||
         (resXUnit == FileInfoBase.NANOMETERS)  ||
         (resXUnit == FileInfoBase.MICROMETERS) ||
         (resXUnit == FileInfoBase.MILLIMETERS) ||
         (resXUnit == FileInfoBase.METERS)      ||
         (resXUnit == FileInfoBase.KILOMETERS)  ||
         (resXUnit == FileInfoBase.MILES)) {
      orgResol[0] = image.getFileInfo(0).getResolutions()[0];
      if (orgResol[0] <= 0.0f) {
        MipavUtil.displayError("X resolution was recorded as " + orgResol[0] +
      ...
      MipavUtil.displayError(
          "JDialogIHN3Correction - x dimension is not a distance measure");
      return;
    }
    ...
    if ( (resYUnit == FileInfoBase.INCHES)       ||
         ...
         (resYUnit == FileInfoBase.ANGSTROMS)    ||
         (resYUnit == FileInfoBase.NANOMETERS)   ||
         (resYUnit == FileInfoBase.MICROMETERS)  ||
         (resYUnit == FileInfoBase.MILLIMETERS)  ||
         (resYUnit == FileInfoBase.METERS)       ||
         (resYUnit == FileInfoBase.KILOMETERS)   ||
         (resYUnit == FileInfoBase.MILES)) {
      orgResol[1] = image.getFileInfo(0).getResolutions()[1];
      if (orgResol[1] <= 0.0f) {
        MipavUtil.displayError("Y resolution was recorded as " + orgResol[1] +
                               " It is being changed to 1.0");
        orgResol[1] = 1.0f;
      }
      ...
      MipavUtil.displayError(
          "JDialogIHN3Correction - y dimension is not a distance measure");
      return;
    }
    ...
    if (image.getNDims() == 3) {
      ...
      if ( (resZUnit == FileInfoBase.INCHES)       ||
           ...
           (resZUnit == FileInfoBase.ANGSTROMS)    ||
           (resZUnit == FileInfoBase.NANOMETERS)   ||
           (resZUnit == FileInfoBase.MICROMETERS)  ||
           (resZUnit == FileInfoBase.MILLIMETERS)  ||
           (resZUnit == FileInfoBase.METERS)       ||
           (resZUnit == FileInfoBase.KILOMETERS)   ||
        ...
        MipavUtil.displayError(
            "JDialogIHN3Correction - z dimension is not a distance measure");
        return;
      }
    } // if (image.getNDims() == 3)
  ...
  /**
   *	Once all the necessary variables are set, call the IHN3 Correction
   *	algorithm.
   */
  private void callAlgorithm() {
    ...
    try {
      if ( (image.getType() == ModelStorageBase.BYTE) ||
          (image.getType() == ModelStorageBase.UBYTE)) {
        resultImage = new ModelImage(ModelStorageBase.SHORT, destExtents,
      ...
      if (createField) {
        fieldImage = new ModelImage(ModelStorageBase.FLOAT, destExtents,
                                    makeImageName(image.getImageName(),
                                                  "_Field"), userInterface);
      } // if (createField)

      // Make algorithm
      N3Algo = new AlgorithmIHN3Correction(resultImage, fieldImage, image,
                                           threshold, maxIter,
                                           endTol, fieldDistance, shrink,
                                           kernelfwhm, noise, regionFlag,
                                           autoThreshold, useScript);
      ...
      System.gc();
      MipavUtil.displayError(
          "Dialog IHN3Correction: unable to allocate enough memory");
      return;
    }
  }
         ...
         if (N3Algo.isCompleted() == true && resultImage != null) {
           ...
           if (createField) {
             ...
             try {
               ...
               System.gc();
               JOptionPane.showMessageDialog(null,
                   "Out of memory: unable to open new field frame",
                   "Error", JOptionPane.ERROR_MESSAGE);
             }
           } // if (createField)
         }
