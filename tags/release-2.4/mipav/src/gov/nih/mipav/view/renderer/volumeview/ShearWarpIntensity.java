...
/**
 * The base class renderer to support shear-warp rendering of intensity images
 * (gray scale).  The intensity is stored in the blue channel of the base
 * class renderer.  See the document ShearWarpRendering.pdf for a detailed
 * description of the renderer architecture.
 */

public abstract class ShearWarpIntensity extends ShearWarpRenderer
     ...
     * values to colors.
     * @param akMap Look up table for mapping the intensity values to colors.
     */
    ...
    protected void mapIntermediateToFinal() {
        ...
        for ( int i1 = 0; i1 < m_iRBound; i1++ ) {
            if ( ( i1 ) % mod == 0 ) {
                ViewJFrameVolumeView.getRendererProgressBar().setValue( Math.round( (float) ( i1 ) / ( m_iRBound - 1 ) * 50 ) + 50 );
                ViewJFrameVolumeView.getRendererProgressBar().update( ViewJFrameVolumeView.getRendererProgressBar().getGraphics() );
            }
    ...
    // Factor used to scale the intermediate values during the mapping
    // to the final image.
