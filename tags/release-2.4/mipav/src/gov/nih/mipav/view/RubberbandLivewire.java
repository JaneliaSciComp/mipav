...
/**
*   A Livewire rubberband.  It makes segmentation easier by
*   "snapping to" appropriate pixels to outline features.  The pixels
*   it snaps to are those with the minumum cost; cost is a function
*   of gradient magnitude, gradient direction, and zero x-crossings.
*   Whenever an anchor is established, a directed graph of costs is
*   created, so that from any pixel in the image, the shortest path
*   from that pixel to the anchor can be drawn.  Once the user is
*   satified with that path they can put down another anchor, for
*   a new directed cost graph.
*   <p>
*   This is based on the method described by Eric N Mortensen and
*   William A Barrett from Brigham Young University, in their paper
*   "Interactive Segmentation with Intelligent Scissors",
*   Graphical Models and Image Processing, v 60, n 5, September 1998, p.349-384.
*
*   A description of the color gradient direction is given in The Docotoral
*   Dissertation of Eric N. Mortensen: "Simultaneous Multi-Frame Subpixel
*   Boundary Definition using Toboggan-Based Intelligent Scissors for Image
*   and Movie Editing, Department of Computer Science, Brigham Young
*   University, Provo, Utah, December, 2000, Section 4.3.1.1 Domain projection
*   vector: vx.  The description of summing gradient magnitudes of the
*   different color bands is described in Section 3.1.1 Discontinuity Measure:
*   Multi-scale Gradient Magnitude.  This is in contrast to the earlier
*   1998 paper which used the maximum of the color band gradient magnitudes.
*
*   @version 1.0, 4/1/2002
*   @author  Neva Cherniavsky
*   @see     Rubberband
*/
 public class RubberbandLivewire extends Rubberband implements ActionListener,
                                                                WindowListener {
    ...
    /**
    *   Sets up local costs graph by calling AlgorithmGradientMagnitude and
    *   AlgorithmEdgeLaplacianSep.  Initializes necessary global arrays.
    ...
    */
    public RubberbandLivewire(Component component, int selection) {
        ...
        progressBar = new ViewJProgressBar(((ViewJComponentEditImage)component).getActiveImage().getImageName(),
                                            "Livewire: Computing cost function ...",
                                            0, 100, false, this, this);
        ...
        if (selection == GRADIENT_MAG) {
            ...
            //AlgorithmGradientMagnitude magnitude = new AlgorithmGradientMagnitude(null, new float[] {1.75f, 1.75f}, true, false);
            AlgorithmGradientMagnitudeSep magnitude = new AlgorithmGradientMagnitudeSep(null, new float[] {1.75f, 1.75f}, true, false);
            progressBar.updateValueImmed(10);

            if (((ViewJComponentEditImage)component).getActiveImage().isColorImage()) {
                localCosts = magnitude.calcInBuffer2DUnnormalized(((ViewJComponentEditImage)component).getActiveImageSliceBuffer(),
                                            extents,
                ...
                for (int i = 0; i < localCostsTemp.length; i++) {
                    localCostsTemp[i] = (float)Math.sqrt
                                        (localCosts[4*i+1]*localCosts[4*i+1] +
            ...
            if (((ViewJComponentEditImage)component).getActiveImage().isColorImage()) {
                ...
                for (int i = 0; i < length; i++) {
                    if ((smallKernelR.get(i)) || (smallKernelG.get(i)) ||
                        (smallKernelB.get(i))) {
                        smallKernel.set(i);
                    }
                    else {
                    ...
                    if ((largeKernelR.get(i)) || (largeKernelG.get(i)) ||
                        (largeKernelB.get(i))) {
                        largeKernel.set(i);
                    }
                    else {
                ...
                // small kernel edge Laplacian - higher noise, better localization
                //sigmas[0]   = 0.75f;
                //sigmas[1]   = 0.75f;
                //lap         = new AlgorithmEdgeLaplacianSep(null, null, sigmas, true, false, 0, 0);
                //smallKernel = lap.calcZeroXMaskBitset(((ViewJComponentEditImage)component).getActiveImageSliceBuffer(), extents);
                ...
                progressBar.updateValueImmed(75);
                largeKernel = lap.calcZeroXMaskBitset(((ViewJComponentEditImage)component).getActiveImageSliceBuffer(), extents);
                progressBar.updateValueImmed(85);
            } // not color
...
//                }
            ...
            AlgorithmLapMedianess medialness = new AlgorithmLapMedianess(null, new float [] {1.0f, 1.0f}, true, false, 1f, false);
            //progressBar.updateValueImmed(25);
            if (((ViewJComponentEditImage)component).getActiveImage().isColorImage()) {
                float [] localCostsR;
                float [] localCostsG;
                ...
                for (int i = 0; i < length; i++) {
                    localCosts[i] = Math.max(localCostsR[i],Math.max(
                                    localCostsG[i],localCostsB[i]));
                }
            ...
            localCosts = medialness.calcInBuffer2D(
              ((ViewJComponentEditImage)component).getActiveImageSliceBuffer(), extents);
              //progressBar.updateValueImmed(45);
            } // else not color
            /*
            AlgorithmFrequencyFilter freq =
            new AlgorithmFrequencyFilter(((ViewJComponentEditImage)component).getActiveImage(),
                                     false, false);
            localCosts = freq.calc2DMedialness();
            */
            ...
            if (((ViewJComponentEditImage)component).getActiveImage().isColorImage()) {
                float [] localCostsTemp = new float[length];
                for (int i = 0; i < length; i++) {
                    localCostsTemp[i] = localCosts[4*i+1] + localCosts[4*i+2] +
                                     localCosts[4*i+3];
                }
                progressBar.updateValueImmed(25);
                localCosts = new float[length];
        ...
        /*
        ModelImage image = new ModelImage(ModelStorageBase.FLOAT,new int[] {xDim, yDim}, "Test", ((ViewJComponentEditImage)component).getActiveImage().getUserInterface());
        try {
            image.importData(0, localCosts, true);
        }
        catch (IOException e) {
        }
        new ViewJFrameImage(image, null, new Dimension(200, 200), image.getUserInterface());
    */
        if (progressBar != null) {
            progressBar.dispose();
        }
    ...
    /**
    *   Sets up local costs graph by calling AlgorithmGradientMagnitudeSep and
    *   AlgorithmEdgeLaplacianSep.  Initializes necessary global arrays.
    *   Only for non-color images and GRADIENT_MAG path calculation (ie- in RFASegTool).
    *   @param component    component to add to
    *   @param grad_sigmas
    *   @param edgelap_sigmas
    *   @param kern_weight
    *   @param grad_weight
    *   @param loThres
    *   @param hiThres
    ...
    */
    public RubberbandLivewire(Component component, float[] grad_sigmas, float[] edgelap_sigmas, float kern_weight,
        ...
        //sigmas[1]   = 0.75f;
        //lap         = new AlgorithmEdgeLaplacianSep(null, null, sigmas, true, false, 0, 0);
        //smallKernel = lap.calcZeroXMaskBitset(((ViewJComponentEditImage)component).getActiveImageSliceBuffer(), extents, ((ViewJComponentEditImage)component).getActiveImage().getUserInterface());
...
//            }
  ...
  */
    ...
    */
    ...
    /**
    *   Draws a line by following the graph from the current
    *   location back to the anchor point.  The anchor point
    *   has its parent = -1.
    *  @param graphics   Graphics to draw in
    */
    public void drawNext(Graphics graphics) {
       // System.err.println("drawing next");
        ...
        pt.x = Math.round( pt.x / (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()));
        pt.y = Math.round( pt.y / (((ViewJComponentEditImage)(component)).getZoomY() * ((ViewJComponentEditImage)(component)).getResolutionY()));
...
//        System.out.println("Curr contour length: " + count + "  Total cost: " + currContourCost);
        ...
        liveWirePtIndex = mouseIndex + convertGraphToInt(costGraph[mouseIndex]);
        if (liveWirePtIndex == seedPoint) return;
        ...
        while (liveWirePtIndex != seedPoint) {
            ...
            xPoints[count] = (mouseIndex%xDim) * ((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()+1;
            yPoints[count] = (mouseIndex/xDim) * ((ViewJComponentEditImage)(component)).getZoomY() * ((ViewJComponentEditImage)(component)).getResolutionY()+1;
            //System.out.println(" Draw next x: = " + xPoints[count] + " y = " + yPoints[count] + "  seed = " + seedPoint + " location = " +  location);
            mouseIndex = liveWirePtIndex;
            liveWirePtIndex = mouseIndex + convertGraphToInt(costGraph[mouseIndex]);
            count++;
        }
...
*/
    ...
    */
    private final int convertGraphToInt(int next) {
        switch (next) {
            case 0: return (-xDim - 1);
            case 1: return (-xDim);
            case 2: return (-xDim + 1);
            case 3: return (-1);
            case 5: return ( 1);
            case 6: return ( xDim - 1);
            case 7: return ( xDim);
            case 8: return ( xDim + 1);
        }
        return 0;
    }
    ...
    */
    public void mouseReleased(MouseEvent mouseEvent) {
        ...
        int xxS = Math.round( pt.x / (((ViewJComponentEditImage)(component)).getZoomX() *
                                      ((ViewJComponentEditImage)(component)).getResolutionX()));
        int yyS = Math.round( pt.y / (((ViewJComponentEditImage)(component)).getZoomY() *
        ...
        // User clicked with right mouse button. Find where she last clicked and remove points to that click point (undo last click, basically)
        if ( isActive() && clickPoints.size() > 1 && SwingUtilities.isRightMouseButton(mouseEvent) ) {
            for ( i=contour.size()-1; i >=0 && !contour.elementAt(i).equals(clickPoints.elementAt(clickPoints.size()-2)); i-- ) {
                contour.removeElementAt(i);
            }
            clickPoints.removeElementAt(clickPoints.size()-1);
            // reanchor contour at last clicked point
            Point newAnchor = new Point( (int)((int)((Point3Df)(contour.lastElement())).x * (((ViewJComponentEditImage)(component)).getZoomX() *
                                                                                        ((ViewJComponentEditImage)(component)).getResolutionX())),
                                         (int)((int)((Point3Df)(contour.lastElement())).y * (((ViewJComponentEditImage)(component)).getZoomY() *
                                                                                        ((ViewJComponentEditImage)(component)).getResolutionY())));
            ...
            contour.drawSelf( ((ViewJComponentEditImage)(component)).getZoomX(), ((ViewJComponentEditImage)(component)).getZoomY(),
                                ((ViewJComponentEditImage)(component)).getResolutionX(), ((ViewJComponentEditImage)(component)).getResolutionY(),
                                0f, 0f,
                                ...
                                ((ViewJComponentEditImage)(component)).getOrientation(),
                                g, false );
            // draw boxes around the first and last points on the curve
            g.setXORMode(Color.black);
            int ptx = (int)((int)((Point3Df)(contour.elementAt(0))).x *
                        (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()));
            int pty = (int)((int)((Point3Df)(contour.elementAt(0))).y *
                        (((ViewJComponentEditImage)(component)).getZoomY() * ((ViewJComponentEditImage)(component)).getResolutionY()));
        ...
        // User clicked again on last point.  Finish off the contour, leave it open
            ...
            distance (xxS, (int)((Point3Df)(contour.lastElement())).x,
                      yyS, (int)((Point3Df)(contour.lastElement())).y) < 3   /*&& mouseDragged == false*/) {
            open = true;
            done = true;
        }
        // User clicked back again on first point.  Finish off contour and close it.
        else if ( isActive() && contour.size() > 2 &&
                distance (xxS, (int)((Point3Df)(contour.elementAt(0))).x,
                          yyS, (int)((Point3Df)(contour.elementAt(0))).y) < 3 ) {
            open = false;
            done = true;
        }
        else {
        ...
        if (done) {
            ...
            if (!open) {
                ...
                if (xPoints != null) {
                    for (i=0; i<xPoints.length; i++) {
                        ...
                        contour.addElement((xPoints[xPoints.length-1-i]) / (((ViewJComponentEditImage)(component)).getZoomX() *
                                                                                     ((ViewJComponentEditImage)(component)).getResolutionX()),
                                          (yPoints[yPoints.length-1-i]) / (((ViewJComponentEditImage)(component)).getZoomY() *
                                                                                     ((ViewJComponentEditImage)(component)).getResolutionY()),
            ...
            if (((ViewJComponentEditImage)(component)).isNewVoiNeeded(constant)) {
                try {
                     ...
                     */
                    if (image.getNDims() > 2) {
                        newVOI = new VOI((short)colorID, name, image.getExtents()[2],
                                                constant, presetHue);
                    }
                    else {
                    ...
                    if (VOIs.VOIAt(i).getID() == ((ViewJComponentEditImage)(component)).getVOI_ID()) {
                        if (VOIs.VOIAt(i).getCurveType() == constant) {
                            if (smoothVOIFlag) {
                                ...
                                VOIs.VOIAt(i).getCurves()[slice].addElement((VOIContour)smoothAlgo.getResultVOI().getCurves()[slice].lastElement());
                            }
                            else {
                                contour.trimPoints(Preferences.getTrim(), Preferences.getTrimAdjacient());
                                contour.setClosed(!open);
//                                VOIs.VOIAt(i).getCurves()[((ViewJComponentEditImage)(component)).getSlice()].addElement(contour);
        ...
        if (!open) {
            tmpPt.x = Math.round( getEnd().x / (((ViewJComponentEditImage)(component)).getZoomX() *
                                                ((ViewJComponentEditImage)(component)).getResolutionX()));
            tmpPt.y = Math.round( getEnd().y / (((ViewJComponentEditImage)(component)).getZoomY() *
                                                ((ViewJComponentEditImage)(component)).getResolutionY()));
            if( isActive() && testPoint(pt)  && (xS != tmpPt.x || yS != tmpPt.y) ) {
                if (xPoints != null) {
                    clickPoints.addElement(new Point3Df(xPoints[0] / (((ViewJComponentEditImage)(component)).getZoomX() *
                                                                                    ((ViewJComponentEditImage)(component)).getResolutionX()),
                                                        yPoints[0] / (((ViewJComponentEditImage)(component)).getZoomY() *
                                                                                    ((ViewJComponentEditImage)(component)).getResolutionY()),
                                                        ((ViewJComponentEditImage)(component)).getSlice()));
                    for (i=0; i<xPoints.length; i++) {
                        // need to scale back down to image space.
   ...
   /*     Point pt = mouseEvent.getPoint();
        ...
        tmpPt.x = Math.round( getEnd().x / (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()));
        tmpPt.y = Math.round( getEnd().y / (((ViewJComponentEditImage)(component)).getZoomY() * ((ViewJComponentEditImage)(component)).getResolutionY()));
        ...
        if (isActive() && testPoint(pt) && (xS != tmpPt.x || yS != tmpPt.y) ) {
            if (xPoints != null) {
                for (int i=0; i<xPoints.length; i++) {
                    // need to scale back down to image space.
                    contour.addElement(Math.round(xPoints[xPoints.length-1-i] / (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX())),
                                       Math.round(yPoints[yPoints.length-1-i] / (((ViewJComponentEditImage)(component)).getZoomY() * ((ViewJComponentEditImage)(component)).getResolutionY())),
                                       ((ViewJComponentEditImage)(component)).getSlice());
                }
            }
            else {
    ...
    */
    public void mouseMoved(MouseEvent mouseEvent){
        ...
        mouseDragged = false;
        //System.err.println("Active is: " + isActive() + " firstPoint is: " + firstPoint + " testPoint is: " + testPoint(pt));
    ...
    /**
    *   Calls seed on this point to set up costGraph array;
    *   then anchors point.
    ...
    */
    ...
    /**
    *   Sets up directed graph from the seed point.  A point (x,y)
    *   is mapped to its absolute value in the image, y*xDim + x.
    *   This along with its cost is stored in Node.  The structure
    *   costGraph holds the "edges" of the graph.  Each location has an
    *   integer associated with it which represents where that node is
    *   pointing to.  Thus costGraph[7] = 8 would mean the node at 7
    *   (really (0,7)) is pointing to position 8 (really (0,8)).  The only
    *   possibilities for a location in costGraph are the 8 neighbors
    *   surrounding that node.  For this reason we might use a byte array
    *   instead of an integer array.  The seed point points nowhere, to
    *   indicate that it's the seed; costGraph[seed.location] = -1.
    *   We also need to know if a point has been processed, that is,
    *   expanded with the cost set.  For this we use a BitSet whose
    *   size is the same as the number of pixels in the image.  Once
    *   a point is processed, its location in the BitSet is set to
    *   <code>true</code>.
    *   <p>
    *   The array seededCosts holds the costs so far for a location.  If
    *   a cost has not been assigned yet, the cost is -2.  ActiveList is
    *   simply a linked list of Integers; the Integer refers to the location
    *   in the seededCosts array.  ActiveList is sorted by cost, so that the
    *   minimum cost in the ActiveList is the first element of the linked list.
    *   Thus finding the minimum is O(1).  Finding out if an element is in the
    *   ActiveList is also O(1), because for an element to be in the list, it must
    *   have already been assigned a cost.  Therefore, if seededCosts[location] != -2,
    *   it is in the ActiveList.  And finding the cost of an item in the ActiveList
    *   is O(1), because it's just seededCosts[location], where location is the Integer
    *   in the ActiveList.  Obviously we're winning speed at the expense of memory, but
    *   it's not too much memory in the overall scheme of things.
    *   <p>
    *   The gradient direction component of the cost is added in on the
    *   fly.  This is because to precalculate would mean an array of 8n, where
    *   n is the size of the image.  The link from p to q is not the same as the
    *   link from q to p.  Furthermore, it may never be necessary to calculate some
    *   of the links, because the graph would never look at that pair.  For more
    *   information on how the gradient direction cost is calculated, look at the
    *   comments directly above the code.
    ...
    */
    private void seed(Point pt) {
        ...
        int x = Math.round( pt.x / (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()));
        int y = Math.round( pt.y / (((ViewJComponentEditImage)(component)).getZoomX() * ((ViewJComponentEditImage)(component)).getResolutionX()));
                ...
                if ((y+iy) >= 0 && (y+iy) < yDim) {  // in bounds in the y dimension
                    for (int ix = -1; ix <= 1; ix++) {
                        ...
                        if ((x+ix) >= 0 && (x+ix) < xDim &&  // in bounds in the x dimension
                            !processedIndicies.get(position)) {    // not yet processed - this will rule out current node

                            if (selection == GRADIENT_MAG) {
                                // Gradient Direction Cost
                                // Let D(p) be a unit vector of the gradient direction at point p.
                                // Define D'(p) as the unit vector perpendicular to D(p).
                                // In our implementation, D(p) = (xDirections[location], yDirections[location])
                                // where location is the absolute position of point p; that is,
                                // location = p.y*xDim + p.x
                                // So
                                //       D(p) = (xDirections[location], yDirections[location])
                                //       D'(p) = (yDirections[location], -xDirections[location])
                                //
                                // The formulation of the gradient direction feature cost is
                                //      f(p,q) = 2/(3pi) * (acos(dp(p,q) + acos(dq(p,q))))
                                // where
                                //      dp(p,q) = D'(p) dot L(p,q)
                                //      dq(p,q) = L(p,q) dot D'(q)
                                // and
                                //
                            ...
                            else temp = cost + (localCosts[position] + grad_weight*gradDir);   // temp cost

                            if (temp < seededCosts[position]) { // not set seededCosts == -2 which will always be less than temp, temp must be positive
        ...
        /*
        ModelImage image = new ModelImage(ModelStorageBase.FLOAT, new int[] {xDim, yDim}, "Seed", ((ViewJComponentEditImage)component).getActiveImage().getUserInterface());
        try {
            image.importData(0, seededCosts, true);
        }
        catch (IOException e) {
        }
        new ViewJFrameImage(image, null, new Dimension(200, 200), image.getUserInterface());
    ...
    //************************************************************************
    /**
    *   Sets completed to <code>false</code>, stops the thread,
    *   and disposes the progress bar.
    ...
    */
    ...
    /**
    *   Binary search tree, modified so it will work with
    *   elements that have the same cost.  In theory this
    *   should not be a problem because we have a unique
    *   identifier in the parameter <code>position</code>.
    *   New nodes of the same cost as an existing node are
    *   inserted to the left of the first existing node of
    *   the same cost.  The children of the first existing
    *   node are transferred to the left of the new node.
    *   The new node is not put all the way to the left
    *   of a chain of nodes of the same cost so as to save
    *   tree traversal time.  This convention must be
    *   followed in the remove and pop or else some nodes get
    *   lost and others are never removed.
    *   <p>
    *   The main reason for using a Binary Search Tree is that
    *   the cost of a "find" (required for remove, pop, and insert)
    *   is log n if the tree is reasonably balanced.  To insure
    *   balance we could have used a red-black tree, but test
    *   runs indicate that this tree stays fairly balanced
    *   on its own due to the nature of the insertions and
    *   deletions.
    */
    class ActiveTree {
        ...
        */
        ...
        /**
        *   Inserts node with given position and cost
        *   at the appropriate location in the tree.
        *   If the cost is the same as one previously in
        *   the tree, inserts the node on the left of the
        *   original.
        ...
        *   <code>cost</code> is used to find where the node is;
        *   <code>position</code> is used to uniquely identify
        *   the node.
        ...
        */
        ...
        /**
        *   Pops off minimum cost in tree.  Returns position
        *   of minimum cost and also removes that node from the
        *   tree.
        ...
        */
        ...
        /**
        *   Prints out tree in sorted order.  Level allows
        *   user to see the structure of the tree.
        */
        public void inOrder() {
            inOrder(root, 0);
        }
        ...
        /**
        *   Prints out tree in sorted order.  Level allows
        *   user to see the structure of the tree.  Will
        *   print out tree from given node on down.
    ...
    /**
    *   Tree node.  Contains data (the integer
    *   position and float cost) and pointers to
