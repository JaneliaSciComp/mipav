 ...
 *   The user can choose whether or not to remove the original selected
 *   voi and whether or not to apply trimming to remove nearly collinear
 *   points.
 *   The user can choose the number of interpolation points.
 *   In 2D all selected curves of a selected voi are smoothed.
 *   In 3D all selected curves in all slices of a selected voi are smoothed.
 *   The original zslice information is only used so that the default number
 *   of interpolated points comes from a contour in zslice.
 *   If the original voi is not removed, the new voi has a new color.
 *   If the original voi is removed, the new voi has the same color.
 */
public class JDialogBSmooth extends JDialogBase
    ...
    /**
     * algorithmPerformed - this method is required if the AlgorithmPerformed
                    ...
                    for ( slice = 0; slice < sliceNum; slice++ ) {
                        ...
                        for ( element = nContours - 1; element >= 0; element-- ) {
                            if ( ( (VOIContour) ( contours[slice].elementAt( element ) ) ).isActive() ) {
                                System.err.println(
                                        "slice is: " + slice + " element is: " + element + " groupnum is: " + groupNum );
                                VOIs.VOIAt( groupNum ).removeCurve( element, slice );
                                VOIs.VOIAt( groupNum ).importCurve( (VOIContour) resultVOI.getActiveContour( slice ),
                                        slice );
                            }
                        }
                    }
