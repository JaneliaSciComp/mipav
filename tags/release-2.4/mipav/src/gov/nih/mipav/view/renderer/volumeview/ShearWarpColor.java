 ...
 * See the document ShearWarpRendering.pdf for a detailed description of the
 * renderer architecture.
 */

public abstract class ShearWarpColor extends ShearWarpRenderer
    implements RendererInterfaceColor {

    /**
     * Create a color renderer.  The input color channel type is 'byte', a
     * signed 8-bit quantity, but the data represents unsigned 8-bit values.
     *
    ...
    protected void mapIntermediateToFinal() {
        ...
        for ( int i1 = 0; i1 < m_iRBound; i1++ ) {
            if ( ( i1 ) % mod == 0 ) {
                ViewJFrameVolumeView.getRendererProgressBar().setValue( MipavMath.round( (float) ( i1 ) / ( m_iRBound - 1 ) * 50 ) + 50 );
