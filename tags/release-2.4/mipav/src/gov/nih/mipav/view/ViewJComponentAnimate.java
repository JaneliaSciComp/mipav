...
public class ViewJComponentAnimate extends ViewJComponentBase {
    ...
    */
    public ViewJComponentAnimate(ViewJFrameBase _frame,
    ...
    *  @return    active image
    */
    public ModelImage getActiveImage(){ return imageActive;}
    ...
    *  @return          paintBitmap
    */
    public BitSet getpaintBitmap() {return paintBitmap;}
    ...
    *  @param  mask
    */
    public void setPaintMask(BitSet mask) { paintBitmap = mask;}
    ...
    public void setPaintMask() {
        ...
        for (i = 0; i < originalZDim; i++) {
            ...
            for (j = 0; j < zDim; j++) {
              offset = j*xDim*yDim;
              for (k = 0; k < 3; k++) {
                for (l = rowNumber*(originalYDim +9);
                     l < rowNumber*(originalYDim +9) + originalYDim + 6; l++) {
                  paintBitmap.set(offset + k + colNumber*(originalXDim + 9) + l*xDim);
                  paintBitmap.set(offset + k + originalXDim + 3 +
                                colNumber*(originalXDim + 9) + l*xDim);
                }
                for (l = colNumber*(originalXDim + 9);
                     l < colNumber*(originalXDim + 9) + originalXDim + 6; l++) {
                  paintBitmap.set(offset + (k + rowNumber*(originalYDim + 9))*xDim + l);
                  paintBitmap.set(offset + (k + originalYDim + 3 + rowNumber*(originalYDim + 9))*xDim + l);
                }
   ...
   */
    ...
    */
    public boolean buildImageObject(int tSlice, int zSlice, ModelLUT _LUTa, ModelLUT _LUTb, boolean forceShow){
        ...
        if (LUTa == null  && _LUTb == null) return false;
        if (_LUTa != null) LUTa = _LUTa;
        if ( imageB != null && _LUTb != null) LUTb = _LUTb;
    ...
    */
    ...
    // the red component is alphaBlended with zero.
        ...
        if ( slice != zSlice || timeSlice != tSlice || forceShow == true) {
            ...
            try {
                imageA.exportData((timeSliceA*zDimSlices*bufferSize + zSlice*bufferSize), bufferSize, imageBufferA);
                if (imageB != null) {
                    imageB.exportData((timeSliceB*zDimSlices * bufferSize + zSlice*bufferSize), bufferSize, imageBufferB);
                }
            }
            catch (IOException error) {
        ...
        if (imageB == null ) {
            ...
            for (index=0, j=0; j < imageSize; index += 4, j++) {
                if (RGBTA != null) {
                    if (RGBTA.getROn()) redMapped   = (RGBIndexBufferA[(int)(imageBufferA[index+1]*normColorA)] &
                                                        0x00ff0000) >> 16;
                    else      redMapped = 0;
                    if (RGBTA.getGOn()) greenMapped = (RGBIndexBufferA[(int)(imageBufferA[index+2]*normColorA)] &
                                                        0x0000ff00) >> 8;
                    else      greenMapped = 0;
                    if (RGBTA.getBOn()) blueMapped  = (RGBIndexBufferA[(int)(imageBufferA[index+3]*normColorA)] &
                                                        0x000000ff);
                    else      blueMapped = 0;
                ...
                if (paintBitmap.get(offset+j) == true) {
                    ...
                    pixBuffer[j]   = 0xff000000             | ((int)redMapped << 16) |
            ...
            for (index=0, j=0; j < imageSize; index += 4, j++) {
                if ((RGBTA != null) && (RGBTB != null)) {
                    if (RGBTA.getROn()) Ra = (RGBIndexBufferA[(int)(imageBufferA[index+1]*normColorA)] &
                                             0x00ff0000) >> 16;
                    else      Ra = 0;
                    if (RGBTA.getGOn()) Ga = (RGBIndexBufferA[(int)(imageBufferA[index+2]*normColorA)] &
                                             0x0000ff00) >> 8;
                    else      Ga = 0;
                    if (RGBTA.getBOn()) Ba = (RGBIndexBufferA[(int)(imageBufferA[index+3]*normColorA)] &
                                             0x000000ff);
                    else      Ba = 0;


                    if (RGBTB.getROn()) Rb = (RGBIndexBufferB[(int)(imageBufferB[index+1]*normColorB)] &
                                             0x00ff0000) >> 16;
                    else      Rb = 0;
                    if (RGBTB.getGOn()) Gb = (RGBIndexBufferB[(int)(imageBufferB[index+2]*normColorB)] &
                                             0x0000ff00) >> 8;
                    else      Gb = 0;
                    if (RGBTB.getBOn()) Bb = (RGBIndexBufferB[(int)(imageBufferB[index+3]*normColorB)] &
                                             0x000000ff);
                    else      Bb = 0;
                }
                else {
    ...
    */
    public void dispose(boolean gcFlag){
        ...
        frame             = null;
        if (disposeImage) {
          if (imageA != null) imageA.disposeLocal();
          if (imageB != null) imageB.disposeLocal();
        }
        mt = null;
       ...
       int i;
        try {
            ...
            if (img != null) {
                ...
                for (i = nVOI-1; i >= 0; i--) {
                    VOIs.VOIAt(i).drawSelf(zoomX,zoomY,resX, resY, 0,0,
