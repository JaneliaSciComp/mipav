 ...
 * <p>Description: The plotter render view frame.   The plotter render views images
 *  based on each slice intensity values.   Component surface created from a 2D image.
 *  The intensity values are mapped like a relief map, with higher intensities making
 *  peaks and lower intensities forming valleys.  A quad mesh is used to create the
 *  three-dimensional map. The same LUT from the image is used to colors the vertices
 *  of the quad mesh. </p>
...
public class ViewJFramePlotterView extends ViewJFrameBase {
    ...
    /**
     * The frame the rendering the plotter view of images.   The plotter renderer view images based
     * on the image intensity values.
