...
/**
 * Generates a MIP image from a 3D volume.  When the user clicks on a pixel of the MIP,
 * a region grow is performed starting at the point where the clicked intensity occurs.
 *
 * Rotation of the volume to take different MIPs is something that might be useful to add
 * (it's been partially added (see the MIP menu), but it is untested..).
 *
...
public class ViewJFramePaintVasculature extends ViewJFrameBase  {
     ...
     * mipImage intensities as a index into a LUT.
     */
    ...
    /**
     * Frame adjustment to keep the frame from being just small enough for the
     * ScrollPane to display the scroll bars.  Windows seems to find an inset
     * value needing a fudge factor of 3, but Mac OS 10 (X), doesn't, and needs
     * a fudge factor of 7 to not require scroll bars.  Either way, insets +
     * fudge-factor must equal 7.
     */
     ...
     */
    public void initFrame( Dimension loc )
     ...
     * Used to translate points on the MIP to points on the volume.
    ...
    /**
     * Updates the this frame's size the compnents sizes. If the magnified image
    ...
    public void updateFrame( float sX, float sY ) {
        ...
        if ( componentImage.getSize( null ).width + 200 > xScreen
                || componentImage.getSize( null ).height + 200 > yScreen ) {} else if ( getSize().width
                        > componentImage.getSize( null ).width
                                || getSize().height > componentImage.getSize( null ).height ) {
            removeComponentListener( this );
            scrollPane.setSize(
                    componentImage.getSize( null ).width + scrollPane.getInsets().left + scrollPane.getInsets().right,
                    componentImage.getSize( null ).height + scrollPane.getInsets().top + scrollPane.getInsets().bottom );
            ...
            if ( componentImage.getSize( null ).width + 200 < xScreen
                ...
                removeComponentListener( this );
                scrollPane.setSize(
                        componentImage.getSize( null ).width + scrollPane.getInsets().left
     ...
     */
    protected final static double distance(Point3Df pt1, Point3Df pt2){
     ...
     * If points are too close to one another in the volume, only one is marked.
