 ...
 * Extending class must include implementation of the method
 * <code>makeCheckBoxList()</code>.
 */
public abstract class JPanelChecklist extends JPanel
    ...
    // notice that for the next two lists, ith entry on one
    // corresponds to the ith entry on the other.
    // the entire list of choices possible.
     ...
     *   checkboxes to image-specific enabled, use setDicomInfo.
    ...
    abstract protected String[] makeCheckboxLabels();

    /** Abstract to ensure the variable <code>listLength</code> has a value. */
    abstract protected void setListLength();

    /**
     * Builds all checkboxes; prepare for this step by having an array of Strings
     * with the names to use.  Called <code>checkboxLabels</code> one for each of the dicom tags that
     * may be sanitized.  Also sets the properties of all check boxes.
    ...
    protected JCheckBox[] makeCheckBoxList() {
        ...
        JCheckBox[] list = new JCheckBox[listLength]; // selector for the user to choose which slices to remove.  TRUE means remove.
        try {
     ...
     * This box can then be displayed anywhere.
     ...
     * @exception ArrayIndexOutOfBoundsException is thrown when <code>i</code> is
     *                                           greater than the number of checkboxes
    ...
    /**
     * Set the checkboxes to be clickable.  Only useful in applications where the
     * tags which are availble to be used are known before-hand.
     * <p>
     ...
     * @exception IllegalArgumentException is thrown when <code>label</code> is not found in
     ...
     * @exception IllegalArgumentException is thrown when <code>label</code> is
     *                                     is not found in the list of check boxes.
     * @exception NullPointerException if no checkboxes exist when called
     * @return boolean status of checkbox
     */
