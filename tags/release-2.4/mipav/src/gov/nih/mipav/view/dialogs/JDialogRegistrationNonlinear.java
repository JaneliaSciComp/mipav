...
public class JDialogRegistrationNonlinear
    extends JDialogBase
      ...
      if (algo.isCompleted()) {
          if (UI.isScriptRecording()) {
              ...
              UI.getScriptDialog().append(UI.getScriptDialog().getVar(matchImage.
                                          ...
                                          targetGaussZ + " " +
                                          sampleFactor2 + " " + costFxn + " " +
                                          precision + " " +
                                          iterations + " " + writeParameters + "\n");
          }
      }
  }
  ...
  private void init() {
    ...
    if (matchImage.getNDims() == 3) {
      labelInternal = new JLabel("Reference slice (1-" +
                                 String.valueOf(matchImage.getExtents()[2]) +
                                 ")");
  ...
  */
  ...
  private void callAlgorithm() {
    if (!doInternal) {
      try {
        ...
        nl = new AlgorithmRegNonlinear(resultImage, refImage, entireTarget,
                                       matchImage, entireSource,
                                       transformation, interpolation,
                                       clipOutput,
                                       interaction, transformVOI,
                                       sourceThreshold,
                                       targetThreshold, sourceGaussX,
                                       sourceGaussY, sourceGaussZ,
                                       targetGaussX, targetGaussY, targetGaussZ,
                                       sampleFactor2, costFxn,
                                       precision, iterations,
                                       writeParameters, esInitial);
      ...
      try {
        resultImage = new ModelImage(ModelStorageBase.FLOAT,
                                     matchImage.getExtents(),
                                     makeImageName(matchImage.getImageName(),
            "_registered"), UI);

        nl = new AlgorithmRegNonlinear(resultImage, referenceSlice, doAdjacent,
                                       entireTarget, matchImage, entireSource,
                                       transformation, interpolation,
                                       clipOutput,
                                       interaction, transformVOI,
                                       sourceThreshold,
  ...
  public void itemStateChanged(ItemEvent event) {
    ...
    if (source == comboBoxImage) {
      ...
      if (doInternal || refImage.getVOIs() == null ||
          refImage.getVOIs().isEmpty() == true) {
        targetVOICheckBox.setEnabled(false);
        targetVOICheckBox.setSelected(false);
      }
      else {
