 ...
 *
 */
public class JDialogHistogramSummary
    extends JDialogBase
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    else if ((source == redButton) || (source == greenButton) ||
             (source == blueButton)) {
      switch ( image.getType() ) {
        case ModelStorageBase.ARGB:
        case ModelStorageBase.ARGB_USHORT:
  ...
  /**
   *	Once all the necessary variables are set, call the Histogram
   *	algorithm based on whehter the image is color or not
   */
  private void callAlgorithm() {
    try {
      ...
      if (runInSeparateThread) {
        ...
        histAlgo.setActiveImage(isActiveImage);
      ...
      MipavUtil.displayError(
          "Dialog Histogram Summary: unable to allocate enough memory");
      return;
    }
  }
