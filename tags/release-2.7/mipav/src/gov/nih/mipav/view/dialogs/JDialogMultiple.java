...
/**
 * This is a dialog to run a script on multiple files. It allows the user to choose multiple files on which to run the
 * script, then choose the script, then choose a file extension to save result images, if necessary. It calls the script
 * parser on each image file after setting up appropriate variables in the parser. The result image will be saved in the
 * directory that the original image was loaded from. However, the user can specify a different file format save by
 * changing the <code>saveExt</code> field. This should be used with caution, because if the script does not contain the
 * necessary variables for the chosen file format, an error will be displayed and the script will not save the file.
 * However, that's only really an issue with .tif images, which require more information than most image formats.
 *
 * @version  1.0 June 1, 2001
 * @author   Neva Cherniavsky
 * @see      AlgorithmScriptParser
 */
public class JDialogMultiple extends JDialogBase implements ActionListener, ListSelectionListener {

    //~ Static fields/initializers -------------------------------------------------------------------------------------

    /** Use serialVersionUID for interoperability. */
    private static final long serialVersionUID = 1748405241299143647L;
     ...
     * Execute - executes the script by calling <code>AlgorithmScriptParser</code> on each image in turn<br>
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (command.equals("Add") || command.equals("AddMultiple")) {
            ...
            if (Preferences.is(Preferences.PREF_USE_AWT)) {
                ...
                if (fd.getFile() != null) {
                    ...
                    for (int i = 0; i < numToAdd; i++) {
                        ...
                        voiSet[currImageIndex].getVOIVector().addElement(new VOIVector(new String(fd.getDirectory() +
                                                                                                  fd.getFile())));
                    }
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    for (int j = 0; j < numToAdd; j++) {
                        ...
                        for (int i = 0; i < files.length; i++) {
                            imageSet[currImageIndex].addElement(new String("" + chooser.getCurrentDirectory() + "" +
                                                                           File.separatorChar + "" +
                                                                           files[i].getName()));
                            voiSet[currImageIndex].getVOIVector().addElement(new VOIVector(new String("" +
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                for (int j = 0; j < selectIndex.size(); j++) {
                    ...
                    for (int i = 0; i < files.length; i++) {
                        ...
                        ((VOIVector) (voiSet[currImageIndex].getVOIVector().get(idx))).getVOIs().addElement(new String("" +
                                                                                                                       chooser.getCurrentDirectory() +
                                                                                                                       "" +
                                                                                                                       File.separatorChar +
                                                                                                                       "" +
            ...
            if (scriptSelectPanel.isVisible()) {
                ...
                if (numImages == 0) {
                    ...
                    if (userInterface.getActiveImageFrame() == null) {
                        MipavUtil.displayError("This script requires an active image.\n" +
            ...
            imgBorder.setTitle("Select image " + (currImageIndex + 1) +
            ...
            for (int i = 0; i < voiSelections.length; i++) {
                ((VOIVector) (voiSet[currImageIndex].getVOIVector().get(idx))).getVOIs().removeElement(voiSelections[i]);
            }
            ...
            if (Preferences.is(Preferences.PREF_USE_AWT)) {
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    ...
                    scriptName.setText(new String("" + chooser.getCurrentDirectory() + "" + File.separatorChar + "" +
            ...
            if (savePrefix.getText().lastIndexOf(".") != -1) {
                MipavUtil.displayError("Prepended string cannot contain a \".\"  The file extension is set in the script file.");
                savePrefix.requestFocus();
                savePrefix.selectAll();
            }

            if (saveSuffix.getText().lastIndexOf(".") != -1) {
                MipavUtil.displayError("Suffix string cannot contain a \".\"  The file extension is set in the script file.");
    ...
    boolean enoughImages() {
        ...
        for (int i = 1; i < numImages; i++) {
            ...
            if ((imageSet[i].size() == 0) || (imageSet[i].size() != imageSet[i - 1].size())) {
                return false;
            }
        }
    ...
    /**
     * Runs the chosen script on the multiple image files. Calls <code>AlgorithmScriptParser</code> with the appropriate
     * script file and file name and directory parameters to do so.
     */
    ...
    class MyCellRenderer extends JLabel implements ListCellRenderer {
        ...
        /** Use serialVersionUID for interoperability. */
        private static final long serialVersionUID = 1538420930356976891L;
    ...
    class VOISetVector extends Vector {
        ...
        /** Use serialVersionUID for interoperability. */
        private static final long serialVersionUID = 8794522132578756226L;
    ...
    class VOIVector extends Vector {
        ...
        /** Use serialVersionUID for interoperability. */
        private static final long serialVersionUID = 8541986178187126571L;
