 ...
 */
public class ViewJFrameImage extends ViewJFrameBase implements KeyListener, MouseListener, MouseMotionListener, ViewOpenFrameInterface,
    ...
    public void actionPerformed(final ActionEvent event) {
        ...
        if ( (command != null) && userInterface.isShorcutRecording()) {
            ...
            if (activeImageNumDims == 3 || activeImageNumDims == 4) {
                final Enumeration<ModelImage> regImages = ViewUserInterface.getReference().getRegisteredImages();
                // add only the framed ones to a new list...also..dont include the active image
                final int activeImageCurrentSlice = getImageA().getParentFrame().getComponentImage().getSlice();
                while (regImages.hasMoreElements()) {
                    final ModelImage image = regImages.nextElement();
                    // check if it is a framed image...and if its not the active image...also make sure its just imageA
                    if ( (image.getParentFrame() != null) && ( !image.getImageName().equals(getImageA().getImageName()))
            ...
            if ( !Preferences.getData().contains("Position")) {
                final StringBuffer imageBuffer = new StringBuffer().append("Position\t");
                final Enumeration<ModelImage> imageEnum = ViewUserInterface.getReference().getRegisteredImages();
            ...
            final String intenStr = userInterface.getMessageText();
            final int xBegin = intenStr.indexOf("X:");
            final int yBegin = intenStr.indexOf("Y:");
            final int end = intenStr.indexOf("Intensity");
            final int xVal = Integer.valueOf(intenStr.substring(xBegin + 2, yBegin).trim());
            final int yVal = Integer.valueOf(intenStr.substring(yBegin + 2, end).trim());
            final int zVal = this.getComponentImage().getSlice();
            final StringBuffer floatBuffer = new StringBuffer().append(xVal).append(",").append(yVal).append(",").append(zVal).append("\t");
            if (linkedScrolling || isShiftDown) {
                final Enumeration<ModelImage> imageEnum = ViewUserInterface.getReference().getRegisteredImages();
                while (imageEnum.hasMoreElements()) {
                    final ModelImage image = imageEnum.nextElement();
                    final float f = image.getFloat(xVal, yVal, zVal);
                    floatBuffer.append(f).append("\t");
                }
            } else {
                final ModelImage imageA = getImageA();
                float f = imageA.getFloat(xVal, yVal, zVal);
                floatBuffer.append(f).append("\t");
                final ModelImage imageB = getImageB();
            ...
            if (loadImage(fil, componentImage, userInterface.getLastStackFlag(), doOrigins, doOrients, 0, 0, 0, 0, false)) {
                ...
                if ( (imageA.getNDims() == 3) && (imageB.getNDims() == 4)) {
                    ...
                    controls.buildToolbar(menuBuilder.isMenuItemSelected("Image toolbar"), voiManager.getToolBar(), VOIGroup, voiManager.getPointerButton(),
                            menuBuilder.isMenuItemSelected("Paint toolbar"), menuBuilder.isMenuItemSelected("Scripting toolbar"));
                }
            ...
            if (imageB == null) {
                ...
                try {
                    ...
                    final AlgorithmImageCalculator algImageCalc = new AlgorithmImageCalculator(destImage, imageA, imageB, AlgorithmImageCalculator.AND,
            ...
            if (imageB != null) { // ask for confirmation if mask already exists
                ...
                final int response = JOptionPane.showConfirmDialog(this, "This image already has a mask. Discard it?", "Create new mask",
            ...
            int fileFormat;
            int i;
            final JDialogUnknownIO unknownIODialog = new JDialogUnknownIO(userInterface.getMainFrame(), "Choose File Format");
            ...
            while (names.hasMoreElements()) {
                final ModelImage curr = names.nextElement();
                final FileInfoBase fBase[] = curr.getFileInfo();
            ...
            if (imageA.getFileInfo()[0] instanceof FileInfoXML) {
                ...
                if (path != null) {
                    ...
                    final int response = JOptionPane.showConfirmDialog(this, "Maintain link to closed image?", "Linked image", JOptionPane.YES_NO_OPTION);
        ...
        } else if (command.equals("VOIToolbar") || command.equals("PaintToolbar") || command.equals("ImageToolbar") || command.equals("ScriptToolbar")) {
        ...
        // Paint
                ...
                while (names.hasMoreElements()) {
                    ...
                    if ( !getActiveImage().getImageName().equals(name)) {
                        ...
                        if (getActiveImage().getNDims() == img.getNDims()) {
                            if (getActiveImage().getExtents()[0] == img.getExtents()[0] && getActiveImage().getExtents()[1] == img.getExtents()[1]) {
            ...
            final JDialogReplaceBlankSlicesWithAverages rBlankWithAvg = new JDialogReplaceBlankSlicesWithAverages(this, getActiveImage());
         ...
         */
        ...
        } else if (command.equals("EdgePreservingSmoothing")) {
        	new JDialogEdgePreservingSmoothing(this, getActiveImage());
        } else if (command.equals("SymmetricNearestNeighbor")) {
        	new JDialogSymmetricNearestNeighbor(this, getActiveImage());
        ...
        } else if (command.equals("N4Correction")) {

            new JDialogN4MRIBiasFieldCorrection(this, getActiveImage());
        ...
        } else if (command.equals("GLOBAL_PB")) {
            new JDialogGlobalPb(this, getActiveImage());
        } else if (command.equals("PB_BOUNDARY")) {
            new JDialogPbBoundaryDetection(this, getActiveImage());
        } else if (command.equals("HARRIS")) {
            new JDialogHarrisCornerDetector(this, getActiveImage());
        } else if (command.equals("HARRIS_LAPLACE")) {
            new JDialogHarrisLaplace(this, getActiveImage());
        } else if (command.equals("SCALE_SALIENCY")) {
        	new JDialogScaleSaliency(this, getActiveImage());
        } else if (command.equals("ENTROPIC")) {
            new JDialogEntropicEdgeDetection(this, getActiveImage());
        } else if (command.equals("BRISK")) {
        	final ModelImage mi = getActiveImage();
        	if (mi.getMax() > 255) {
        		MipavUtil.displayError("BRISK assumes a maximum value of 255");
        		return;
        	}
        	...
        	new JDialogRegionMergingViaBoundaryMelting(this, getActiveImage());
            ...
            new JDialogThinning2D(this, getActiveImage());
            ...
            if ( ! (type == ModelStorageBase.UBYTE || type == ModelStorageBase.FLOAT || type == ModelStorageBase.USHORT || type == ModelStorageBase.ARGB)) {
                MipavUtil
                        .displayError("<html>Image type must either be UBYTE, USHORT, FLOAT, or ARGB.<br>See Utilities -&gt; Conversion tools -&gt; Convert type.</html>");
                return;
            }
            if (mi.is2DImage()) {
        	...
        	new JDialogGradientInverseWeightedSmoothing(this, getActiveImage());
        ...
        } else if (command.equals("HoughCardioid")) {
            new JDialogHoughCardioid(this, getActiveImage());
        ...
        } else if (command.equals("morReconstruction")) {
            new JDialogMorphologicalReconstruction(this, getActiveImage());
            ...
            if ( (getActiveImage().getNDims() == 2) || ( (getActiveImage().getNDims() == 3) && (getActiveImage().getExtents()[2] == 2))) {
            ...
            final String[] akNamesCompatibleTargetImages = JDialogRegistrationBSpline.getNamesCompatibleTargetImages(componentImage.getActiveImage());
            ...
            if (getActiveImage().getRegistrationFrame() == null) {
                ...
                if (getActiveImage().getNDims() == 2) {
                    ...
                    if (image2load != null) {
                        AlgorithmMatchForReference algoRef = new AlgorithmMatchForReference(getActiveImage(), image2load);
                        ...
                        final ViewJFrameRegistrationTool registrationFrame = new ViewJFrameRegistrationTool(refImage, adjImage, refLUT, adjLUT);
        ...
        } else if (command.equals("ActiveContours")) {
        ...
        // }
        ...
        } else if (command.equals("UnionFindWatershed")) {
        	new JDialogUnionFindWatershed(this, getActiveImage());
        } else if (command.equals("SequentialScanningWatershed")) {
        	new JDialogSequentialScanningWatershed(this, getActiveImage());
        } else if (command.equals("ShortestPathWatershed")) {
        	new JDialogShortestPathWatershed(this, getActiveImage());
        } else if (command.equals("HillClimbingWatershed")) {
        	new JDialogHillClimbingWatershed(this, getActiveImage());
        	...
        	new JDialogLowerCompletion(this, getActiveImage());
        } else if (command.equals("SplitAndMergeWatershed")) {
        	new JDialogSplitAndMergeWatershed(this, getActiveImage());
            ...
            if (componentImage.getImageA().getAnimateFrame() == null) {
                ...
                dialogAnimate = new JDialogAnimate(this, componentImage.getImageA(), componentImage.getLUTa(), componentImage.getImageB(),
            ...
            if ( ( (units[1] != unit) || (units[2] != unit)) || (diffRatio >= 20)) {
                ...
                if (reply == JOptionPane.YES_OPTION) {
                    ...
                    try {
                        final VolumeTriPlanarInterface triVolume = new VolumeTriPlanarInterface(imageA, imageB);
            ...
            final JDialogMagnificationControls magSettings = new JDialogMagnificationControls(this, componentImage, componentImage.getZoomX(), null);
            ...
            try {
                ...
                Preferences.debug("ViewJFrameSurfacePlotter cannot be called; encountered " + "a NoClassDefFoundError.  \nIt is likely that java3D is "
                ...
                Preferences.debug("ViewJFrameSurfacePlotter cannot be called as there was " + "not enough memory allocated.  \n" + "The error is: \n"
         ...
         */
            ...
            if (getActiveImage().getType() == ModelStorageBase.BOOLEAN) {
                ...
                if (imageA.getHistogramFrame() == null) {
                    final ModelStorageBase lutA = imageA.isColorImage() ? componentImage.getRGBTA() : componentImage.getLUTa();
                    final ModelStorageBase lutB = (imageB != null) ? imageB.isColorImage() ? componentImage.getRGBTB() : componentImage.getLUTb() : null;
                    final JFrameHistogram test = new JFrameHistogram(this, imageA, imageB, lutA, lutB);
            ...
            if (componentImage.getImageA().getLightBoxFrame() == null) {
                ...
                try {
                    new ViewJFrameLightBox(this, "LightBox", imageA, componentImage.getLUTa(), imageB, componentImage.getLUTb(),
                            componentImage.getResolutionX(), componentImage.getResolutionY(), new Dimension(50, 200), controls, voiManager);
                } catch (final OutOfMemoryError error) {
            ...
            if (dpDialog == null) {
                dpDialog = new JDialogDataProvenance(this, getActiveImage().getImageName(), getActiveImage().getImageDirectory(), getActiveImage()
                        .getProvenanceHolder(), false);
            } else {
                dpDialog.setVisible();
        ...
        } else if (command.equals("EmbeddedConfidenceEdgeDetection")) {
            new JDialogEmbeddedConfidenceEdgeDetection(this, getActiveImage());
    ...
    public void close() {
        ...
        if (Preferences.is(Preferences.PREF_CLOSE_FRAME_CHECK)) {
            final int reply = JOptionPane.showConfirmDialog(this, "Do you really want to close this frame?", "Close Frame", JOptionPane.YES_NO_OPTION,
    ...
    public void collapseAlltoSinglePaint(final boolean showProgressBar) {
        ...
        if (componentImage != null) {
            ...
            if (showProgressBar) {
                progressBar = new ViewJProgressBar("Converting", "Converting mask to paint...", 0, 100, true, this, this);
        ...
        while (regImages.hasMoreElements()) {
            ...
            if ( (image.getParentFrame() != null) && ( !image.getImageName().equals(getImageA().getImageName()))
                ...
                if (regFramedNumDims == activeImageNumDims) { // same dimensionality required
                    ...
                    int currentSlice;
                    final int activeImageCurrentSlice;
    ...
    public void mouseClicked(final MouseEvent event) {
        ...
        if (event.getButton() == MouseEvent.BUTTON3) {
            ...
            if (event.getSource() instanceof JToggleButton) {
                ...
                if ( ((JButton) event.getSource()).getActionCommand().equals(CustomUIBuilder.PARAM_VOI_QUICK_AND_OP.getActionCommand())
                        || ((JButton) event.getSource()).getActionCommand().equals(CustomUIBuilder.PARAM_VOI_QUICK_NOT_OP.getActionCommand())) {
     ...
     * @return DOCUMENT ME!
     */
    public boolean setAndLoad(final ModelImage image2load, final boolean doOrigins, final boolean doOrients, final double defaultValue,
            final double defaultRed, final double defaultGreen, final double defaultBlue) {

        if (loadImage(image2load, componentImage, false, doOrigins, doOrients, defaultValue, defaultRed, defaultGreen, defaultBlue, false)) {
    ...
    protected void initLUT() throws OutOfMemoryError {
        ...
        if (LUTa == null) {
            ...
            final TransferFunction transfer = LUTa.getTransferFunction();
            int size = transfer.size();
            double localMin = 256;
            double localMax = -1;
            for (int i = 0; i < size; i++) {
                final Vector2f point = transfer.getPoint(i);
    ...
    protected String makeTitle() {
        ...
        if (displayMode == ViewJFrameBase.IMAGE_A) {
            ...
            if (imageA.getNDims() == 4) { // Setup the title for 4D image
                ...
                if (imageNameArray != null) {
                    str = imageNameArray[componentImage.getSlice()] + "      " + String.valueOf(componentImage.getSlice()) + "/" + String.valueOf(nImage - 1)
                            + "     M:" + makeString(componentImage.getZoomX(), 2);
                } else {
    ...
    protected void computeIdealWindowSize() {
        ...
        if ( (getSize().width > ( (ViewJFrameImage.xScreen) * IMAGE_SCREEN_RATIO)) && (componentImage.getSize().width > getSize().width)) {
            addInsets = false;
        ...
        // resize the window, since the only way it could have got that big
        // is if the user manually resized it to be that large
        if ( (getSize().height > ( (ViewJFrameImage.yScreen) * IMAGE_SCREEN_RATIO)) && (componentImage.getSize().height > getSize().height)) {
            addInsets = false;
    ...
    private void init(final ModelLUT LUTa, final Dimension loc, final boolean logMagDisplay) throws OutOfMemoryError {
        ...
        scrollPane = new JScrollPane(componentImage, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    protected void initComponentImage(final int[] extents) throws OutOfMemoryError {
        ...
        componentImage = new ViewJComponentEditImage(this, imageA, LUTa, imageBufferA, null, null, imageBufferB, pixBuffer, zoom, extents, logMagDisplay,
        ...
        while (names.hasMoreElements()) {
            ...
            if ( !getActiveImage().getImageName().equals(name)) {
                ...
                try {
                    ...
                    if ( (getActiveImage().getNDims() == img.getNDims()) || (isForCalculator && ( (getActiveImage().getNDims() == 3) && (img.getNDims() == 2)))) {

                        // System.err.println("two with n-dim equal: " + img.getNDims());
                        if ( (getActiveImage().getNDims() == 3) && (img.getNDims() == 3) && (getActiveImage().getExtents()[2] == img.getExtents()[2])) {

                            // System.err.println("extents 2 are equal");
                            proceed = true;
                        } else if ( (getActiveImage().getNDims() == 4) && (getActiveImage().getExtents()[2] == img.getExtents()[2])
                                && (getActiveImage().getExtents()[3] == img.getExtents()[3])) {
                            proceed = true;
                        } else if (getActiveImage().getNDims() == 2) {
                            proceed = true;
                        } else if ( (getActiveImage().getNDims() == 3) && (img.getNDims() == 2) && (getActiveImage().getExtents()[0] == img.getExtents()[0])
    ...
    protected void initVOI() {
        ...
        voiManager.getVOIManager(0).init(this, imageA, imageB, componentImage, componentImage, componentImage.getOrientation());
     ...
     * 
     * @see
     * gov.nih.mipav.view.renderer.WildMagic.VOI.VOIManagerInterfaceListener#setCenter(WildMagic.LibFoundation.Mathematics
     * .Vector3f)
     */
    @Override
    ...
    public void paintToShortMask() {
        final ModelImage maskImage = ViewUserInterface.getReference().getRegisteredImageByName(componentImage.commitPaintToMask());

        ScriptRecorder.getReference().addLine(new ActionPaintToMask(getActiveImage(), maskImage, ActionPaintToMask.MASK_SHORT));
        ProvenanceRecorder.getReference().addLine(new ActionPaintToMask(getActiveImage(), maskImage, ActionPaintToMask.MASK_SHORT));
    }

    @Override
    public void paintToUbyteMask() {
        final ModelImage maskImage = ViewUserInterface.getReference().getRegisteredImageByName(componentImage.commitPaintToUbyteMask());

        ScriptRecorder.getReference().addLine(new ActionPaintToMask(getActiveImage(), maskImage, ActionPaintToMask.MASK_UBYTE));
        ProvenanceRecorder.getReference().addLine(new ActionPaintToMask(getActiveImage(), maskImage, ActionPaintToMask.MASK_UBYTE));
    }
