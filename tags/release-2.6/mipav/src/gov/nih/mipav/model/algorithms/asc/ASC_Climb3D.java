...
/**
 * A class for extracting multiresolution level surfaces from 3D images. The algorithm is partially based on the paper:
 *
 * <pre>
 ...
 * </pre>
 *
 * @author  David H. Eberly, Ph.D. The algorithm used is an improvement over the one described in this paper. The
 *          merging is done with a simple divide-and-conquer scheme by representing the image as an octree. The
 *          isosurface extraction in this code does not suffer the ambiguitiy problems of the one in the paper. See the
 *          document AdaptiveSkeletonClimbing.pdf for a detailed description of the algorithms as implemented in this
 *          class.
 */
...
public class ASC_Climb3D {
    ...
    /** Indices for the vertex array in MjVertexTable. EI = edge index, FI = face index. */
    ...
    /**
     * Bit-flags for use by getVertices and the functions that connect the vertices by edges. EB = edge bit, FB = face
     * bit.
     */
    ...
    /** output of extractContour public ASC_Vertex3[] m_akVertex; public ASC_Triangle3[] m_akTriangle;. */
     ...
     * surface is constructed. If its value is n where the image size is m-by-m-by-m with m = power(2,n)+1, the finest
     * resolution surface is constructed. This surface corresponds to an extraction at the voxel level. Any value of
     * iDepth between -1 and n may be used. The smaller the value, the coarser the resolution the extracted surface is.
     */
    private int iDepth;
    ...
    /**
     * If set to 'true', image boundary voxels are not allowed to merge with any other voxels. This forces highest level
     * of detail on the boundary. The idea is that an image too large to process by itself can be partitioned into
     * smaller subimages and the adaptive skeleton climbing applied to each subimage. By forcing highest resolution on
    ...
    /**
     * Create a multiresolution extractor for isosurfaces that is based on adaptive skeleton climbing.
     *
     * @param  iN      The input data must contain M-by-M-by-M data values where M = power(2,iN)+1. Also, 0 <= iN <= 7.
     *                 The upper bound is necessary to guarantee that the memory requirements for the octree are within
     *                 reason. The input data organization is lexicographic order for (x,y,z).
     * @param  afData  the image array of the size specified by iN If the image must be partitioned into smaller
     *                 subimages, use ASC_Climb3DDecompose. That class will set bFixBoundary to 'true'.
     */
     ...
     */
    public static ASC_Vertex3[] computeNormals(ASC_Vertex3[] akVertex, Vector kTArray) {
        ...
        for (i = 0; i < akNormal.length; i++) {
            ...
            float fLength = (float) Math.sqrt((kV.m_fX * kV.m_fX) + (kV.m_fY * kV.m_fY) + (kV.m_fZ * kV.m_fZ));
    ...
    /**
     * Generate a unique set of vertices and triangles from the input vertices and triangles. The output vertices and
     * output triangles are stored in kVArray and kTArray.
     *
     * @param  kVArray  The input set of vertices, potentially with duplicates. On output, the set consists of unique
     *                  vertices.
     * @param  kTArray  The input set of triangles, potentially with duplicates. On output, the set consists of unique
     *                  triangles.
     */
    ...
    /**
     * Get the image data object. This is used by ASC_Climb3DDecompose so that it needs only one ASC_Climb3D object to
     * compute level surfaces in a piecewise manner.
     *
    ...
    /**
     * Interpolation of the image gradient values. The resulting gradient is stored in m_fPointX, m_fPointy, m_fPointZ.
     *
    ...
    /**
     * Orient the triangle vertices so the triangle normal is in the direction of the image gradient at its centroid or
     * in the opposite direction depending on the value of bSameDirection.
     *
     * @param  kTri            the triangle to orient
     * @param  bSameDirection  'true' if the normal should be in the direction of the gradient, 'false' if the normal
     *                         should be in the opposite direction of the gradient
     */
    ...
    /**
     * Orient all the triangles so that the triangle normals are in the direction of the image gradient at its centroid
     * or in the opposite direction depending on the value of bSameDirection.
     *
     * @param  bSameDirection  'true' if the normal should be in the direction of the gradient, 'false' if the normal
     *                         should be in the opposite direction of the gradient
     */
     ...
     * duplicate vertices, set this flag to 'true'.
     *
     * @param  bUnique  DOCUMENT ME!
     */
    public void setBUnique(boolean bUnique) {
    ...
    /**
     * Specify whether or not image boundary voxels are allowed to merge. The only client of this call is
     * ASC_Climb3DDecompose. Boundary voxels are not allowed to merge to prevent cracking on shared subimage boundaries.
     *
     * @param  bFixBoundary  set to 'true' if the boundary voxels are not allowed to merge, ''false' if they are allowed
     *                       to merge
     */
    ...
    /**
     * Sets the level value for the desired isosurface. This value should not be an image value. The restriction allows
     * for a large amount of algorithmic simplification. A good choice for fLevel is a non--integral value, perhaps a
     * value of the form v.5 where v is an integer.
     *
     * @param  fLevel  DOCUMENT ME!
     */
     ...
     * @param  _iDepth  DOCUMENT ME!
     */
    public void setIDepth(int _iDepth) {
        this.iDepth = _iDepth;
    }


    /**
     * For the fastest extracction, set this flag to zero. The triangles are extracted for each monoregion without
     * worrying about the orientation of the triangles. In this mode, the display of triangles should have back-face
     * culling disabled. If you need the triangles to be consistently ordered, you have a choice of their normals being
     * in the direction of the image gradients at their centroids (set flag to +1) or in the opposite direction of the
     * image gradients (set flag to -1).
     ...
     * @param  iY0  iDY, iDZ the edge lengths of the box
     * @param  iZ0  iLY, iLZ the indices for the binary trees corresponding to the edges of the box
     * @param  iDX  DOCUMENT ME!
     * @param  iDY  DOCUMENT ME!
     * @param  iDZ  DOCUMENT ME!
     * @param  iLX  DOCUMENT ME!
     * @param  iLY  DOCUMENT ME!
     * @param  iLZ  DOCUMENT ME!
     */
    private void addBox(int iX0, int iY0, int iZ0, int iDX, int iDY, int iDZ, int iLX, int iLY, int iLZ) {
    ...
    /**
     * The function for merging two boxes in the x-direction. See AdaptiveSkeletonClimbing.pdf for details.
     *
     * @param   kR0  kR1 the boxes to attempt merging
     * @param   kR1  the common y-value for the origins of the rectangles
     * @param   iLX  the index of the x-direction binary tree corresponding to the x-line containing iXOrigin
     * @param   iY0  the common z-value for the origins of the rectangles
     * @param   iZ0  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private boolean doXMerge(ASC_MergeBox kR0, ASC_MergeBox kR1, int iLX, int iY0, int iZ0) {

        if (!kR0.m_bValid || !kR1.m_bValid || (kR0.m_iYStride != kR1.m_iYStride) ||
        ...
        for (int iY = iY0; iY <= iY1; iY++) {
            ...
            for (int iZ = iZ0; iZ <= iZ1; iZ++) {
                ...
                switch (m_aakXMerge[iY][iZ].getRootType(iLX)) {
                    ...
                    case ASC_LinearMergeTree.CFG_MULT:
                        return false;

                    case ASC_LinearMergeTree.CFG_INCR:
                        iIncr++;
                        break;

                    case ASC_LinearMergeTree.CFG_DECR:
                        iDecr++;
                        break;
                }
            }
    ...
    /**
     * The function for merging two boxes in the y-direction. See AdaptiveSkeletonClimbing.pdf for details.
     *
     * @param   kR0  kR1 the boxes to attempt merging
     * @param   kR1  the common x-value for the origins of the rectangles
     * @param   iX0  the common z-value for the origins of the rectangles
     * @param   iLY  the index of the y-direction binary tree corresponding to the y-line containing iYOrigin
     * @param   iZ0  DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private boolean doYMerge(ASC_MergeBox kR0, ASC_MergeBox kR1, int iX0, int iLY, int iZ0) {

        if (!kR0.m_bValid || !kR1.m_bValid || (kR0.m_iXStride != kR1.m_iXStride) ||
                (kR0.m_iZStride != kR1.m_iZStride)) {
            return false;
        }
    ...
    /**
     * The function for merging two boxes in the z-direction. See AdaptiveSkeletonClimbing.pdf for details.
     *
     * @param   kR0  kR1 the boxes to attempt merging
     * @param   kR1  the common x-value for the origins of the rectangles
     * @param   iX0  the common y-value for the origins of the rectangles
     * @param   iY0  DOCUMENT ME!
     * @param   iLZ  the index of the z-direction binary tree corresponding to the z-line containing iZOrigin
     *
     * @return  DOCUMENT ME!
     */
    private boolean doZMerge(ASC_MergeBox kR0, ASC_MergeBox kR1, int iX0, int iY0, int iLZ) {
    ...
    /**
     * Compute the points of intersection of the level surface with the edges of the specified monoregion box.
     *
     * @param   kBox    the monoregion box
     * @param   kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     *
     * @return  a bit-flag whose bits store information about whether or not a box edge is intersected by the level
     *          surface
     */
    private int getVertices(ASC_OctBox kBox, ASC_VertexTable kTable) {
        ...
        if (iRoot != -1) {
            iType |= EB_XMIN_YMIN;
            kTable.insert(EI_XMIN_YMIN, (float) kBox.m_iX0, (float) kBox.m_iY0,
        ...
        if (iRoot != -1) {
            iType |= EB_XMIN_YMAX;
            kTable.insert(EI_XMIN_YMAX, (float) kBox.m_iX0, (float) kBox.m_iY1,
        ...
        if (iRoot != -1) {
            iType |= EB_XMAX_YMIN;
            kTable.insert(EI_XMAX_YMIN, (float) kBox.m_iX1, (float) kBox.m_iY0,
        ...
        if (iRoot != -1) {
            iType |= EB_XMAX_YMAX;
            kTable.insert(EI_XMAX_YMAX, (float) kBox.m_iX1, (float) kBox.m_iY1,
        ...
        if (iRoot != -1) {
            iType |= EB_XMIN_ZMIN;
            kTable.insert(EI_XMIN_ZMIN, (float) kBox.m_iX0, getYInterp(kBox.m_iX0, iRoot, kBox.m_iZ0),
                          (float) kBox.m_iZ0);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_XMIN_ZMAX;
            kTable.insert(EI_XMIN_ZMAX, (float) kBox.m_iX0, getYInterp(kBox.m_iX0, iRoot, kBox.m_iZ1),
                          (float) kBox.m_iZ1);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_XMAX_ZMIN;
            kTable.insert(EI_XMAX_ZMIN, (float) kBox.m_iX1, getYInterp(kBox.m_iX1, iRoot, kBox.m_iZ0),
                          (float) kBox.m_iZ0);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_XMAX_ZMAX;
            kTable.insert(EI_XMAX_ZMAX, (float) kBox.m_iX1, getYInterp(kBox.m_iX1, iRoot, kBox.m_iZ1),
                          (float) kBox.m_iZ1);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_YMIN_ZMIN;
            kTable.insert(EI_YMIN_ZMIN, getXInterp(iRoot, kBox.m_iY0, kBox.m_iZ0), (float) kBox.m_iY0,
                          (float) kBox.m_iZ0);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_YMIN_ZMAX;
            kTable.insert(EI_YMIN_ZMAX, getXInterp(iRoot, kBox.m_iY0, kBox.m_iZ1), (float) kBox.m_iY0,
                          (float) kBox.m_iZ1);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_YMAX_ZMIN;
            kTable.insert(EI_YMAX_ZMIN, getXInterp(iRoot, kBox.m_iY1, kBox.m_iZ0), (float) kBox.m_iY1,
                          (float) kBox.m_iZ0);
        }
        ...
        if (iRoot != -1) {
            iType |= EB_YMAX_ZMAX;
            kTable.insert(EI_YMAX_ZMAX, getXInterp(iRoot, kBox.m_iY1, kBox.m_iZ1), (float) kBox.m_iY1,
                          (float) kBox.m_iZ1);
        }
    ...
    /**
     * Interpolate the voxel edge (x,y,z) to (x+1,y,z). The caller of this method guarantees that m_fLevel is between
     * the image values I(x,y,z) and I(x+1,y,z).
     *
     * @param   iX  iY, iZ the starting point of the voxel edge to be interpolated
     * @param   iY  DOCUMENT ME!
     * @param   iZ  DOCUMENT ME!
     *
     * @return  The x-value at which the isosurface intersects the edge. It is guaranteed that x < xIntersect < x+1.
     */
    ...
    /**
     * Compute the edges that connect the vertices on the xmax face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     * @param  iType   the bit-flag whose bits indicate which edges of the box are intersected by the level surface
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getXMaxEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;

        if ((iType & EB_XMAX_YMIN) != 0) {
            iFaceType |= 0x01;
        }

        if ((iType & EB_XMAX_YMAX) != 0) {
            iFaceType |= 0x02;
        }

        if ((iType & EB_XMAX_ZMIN) != 0) {
            iFaceType |= 0x04;
        }

        if ((iType & EB_XMAX_ZMAX) != 0) {
            iFaceType |= 0x08;
        }
        ...
        for (int iZ = kBox.m_iZ0 + 1; iZ < kBox.m_iZ1; iZ++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLY) || kMerge.hasZeroSubedge(kBox.m_iLY)) {
                iRoot = kMerge.getZeroBase(kBox.m_iLY);

                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) kBox.m_iX1, getYInterp(kBox.m_iX1, iRoot, iZ), (float) iZ));
            }
        }
        ...
        for (int iY = kBox.m_iY0 + 1; iY < kBox.m_iY1; iY++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLZ) || kMerge.hasZeroSubedge(kBox.m_iLZ)) {
                iRoot = kMerge.getZeroBase(kBox.m_iLZ);

                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) kBox.m_iX1, (float) iY, getZInterp(kBox.m_iX1, iY, iRoot)));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the xmax face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     * @param  iType   the bit-flag whose bits indicate which edges of the box are intersected by the level surface
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getXMaxEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMAX_YMIN, EI_XMAX_YMAX);
                break;
            ...
            case 12:
                kTable.insert(EI_XMAX_ZMIN, EI_XMAX_ZMAX);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    kTable.insert(FI_XMAX, kTable.getVertex(EI_XMAX_ZMIN).m_fX, kTable.getVertex(EI_XMAX_ZMIN).m_fY,
    ...
    /**
     * Compute the edges that connect the vertices on the xmin face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getXMinEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;

        if ((iType & EB_XMIN_YMIN) != 0) {
            iFaceType |= 0x01;
        }

        if ((iType & EB_XMIN_YMAX) != 0) {
            iFaceType |= 0x02;
        }

        if ((iType & EB_XMIN_ZMIN) != 0) {
            iFaceType |= 0x04;
        }

        if ((iType & EB_XMIN_ZMAX) != 0) {
            iFaceType |= 0x08;
        }
        ...
        for (int iZ = kBox.m_iZ0 + 1; iZ < kBox.m_iZ1; iZ++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLY) || kMerge.hasZeroSubedge(kBox.m_iLY)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) kBox.m_iX0, getYInterp(kBox.m_iX0, iRoot, iZ), (float) iZ));
            }
        }
        ...
        for (int iY = kBox.m_iY0 + 1; iY < kBox.m_iY1; iY++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLZ) || kMerge.hasZeroSubedge(kBox.m_iLZ)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) kBox.m_iX0, (float) iY, getZInterp(kBox.m_iX0, iY, iRoot)));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the xmin face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getXMinEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMIN_YMIN, EI_XMIN_YMAX);
                break;
            ...
            case 12:
                kTable.insert(EI_XMIN_ZMIN, EI_XMIN_ZMAX);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    // plus-sign configuration, add branch point to tessellation
                    kTable.insert(FI_XMIN, kTable.getVertex(EI_XMIN_ZMIN).m_fX, kTable.getVertex(EI_XMIN_ZMIN).m_fY,
    ...
    /**
     * Interpolate the voxel edge (x,y,z) to (x,y+1,z). The caller of this method guarantees that m_fLevel is between
     * the image values I(x,y,z) and I(x,y+1,z).
     *
     * @param   iX  the desired level value for the isosurface
     * @param   iY  iY, iZ the starting point of the voxel edge to be interpolated
     * @param   iZ  DOCUMENT ME!
     *
     * @return  The y-value at which the isosurface intersects the edge. It is guaranteed that y < yIntersect < y+1.
     */
    ...
    /**
     * Compute the edges that connect the vertices on the ymax face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getYMaxEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        if ((iType & EB_YMAX_ZMIN) != 0) {
            iFaceType |= 0x04;
        }

        if ((iType & EB_YMAX_ZMAX) != 0) {
            iFaceType |= 0x08;
        }
        ...
        for (int iX = kBox.m_iX0 + 1; iX < kBox.m_iX1; iX++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLZ) || kMerge.hasZeroSubedge(kBox.m_iLZ)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) iX, (float) kBox.m_iY1, getZInterp(iX, kBox.m_iY1, iRoot)));
            }
        }
        ...
        for (int iZ = kBox.m_iZ0 + 1; iZ < kBox.m_iZ1; iZ++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLX) || kMerge.hasZeroSubedge(kBox.m_iLX)) {
                iRoot = kMerge.getZeroBase(kBox.m_iLX);

                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3(getXInterp(iRoot, kBox.m_iY1, iZ), (float) kBox.m_iY1, (float) iZ));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the ymax face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getYMaxEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMIN_YMAX, EI_XMAX_YMAX);
                break;
            ...
            case 12:
                kTable.insert(EI_YMAX_ZMIN, EI_YMAX_ZMAX);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    // plus-sign configuration, add branch point to tessellation
                    kTable.insert(FI_YMAX, kTable.getVertex(EI_YMAX_ZMIN).m_fX, kTable.getVertex(EI_XMIN_YMAX).m_fY,
    ...
    /**
     * Compute the edges that connect the vertices on the ymin face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getYMinEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        if ((iType & EB_YMIN_ZMIN) != 0) {
            iFaceType |= 0x04;
        }

        if ((iType & EB_YMIN_ZMAX) != 0) {
            iFaceType |= 0x08;
        }
        ...
        for (int iX = kBox.m_iX0 + 1; iX < kBox.m_iX1; iX++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLZ) || kMerge.hasZeroSubedge(kBox.m_iLZ)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) iX, (float) kBox.m_iY0, getZInterp(iX, kBox.m_iY0, iRoot)));
            }
        }
        ...
        for (int iZ = kBox.m_iZ0 + 1; iZ < kBox.m_iZ1; iZ++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLX) || kMerge.hasZeroSubedge(kBox.m_iLX)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3(getXInterp(iRoot, kBox.m_iY0, iZ), (float) kBox.m_iY0, (float) iZ));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the ymin face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getYMinEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMIN_YMIN, EI_XMAX_YMIN);
                break;
            ...
            case 12:
                kTable.insert(EI_YMIN_ZMIN, EI_YMIN_ZMAX);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    // plus-sign configuration, add branch point to tessellation
                    kTable.insert(FI_YMIN, kTable.getVertex(EI_YMIN_ZMIN).m_fX, kTable.getVertex(EI_XMIN_YMIN).m_fY,
                                  kTable.getVertex(EI_XMIN_YMIN).m_fZ);
    ...
    /**
     * Interpolate the voxel edge (x,y,z) to (x,y,z+1). The caller of this method guarantees that m_fLevel is between
     * the image values I(x,y,z) and I(x,y,z+1).
     *
     * @param   iX  the desired level value for the isosurface
     * @param   iY  iY, iZ the starting point of the voxel edge to be interpolated
     * @param   iZ  DOCUMENT ME!
     *
     * @return  The z-value at which the isosurface intersects the edge. It is guaranteed that z < zIntersect < z+1.
     */
    ...
    /**
     * Compute the edges that connect the vertices on the zmax face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getZMaxEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        for (int iX = kBox.m_iX0 + 1; iX < kBox.m_iX1; iX++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLY) || kMerge.hasZeroSubedge(kBox.m_iLY)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) iX, getYInterp(iX, iRoot, kBox.m_iZ1), (float) kBox.m_iZ1));
            }
        }
        ...
        for (int iY = kBox.m_iY0 + 1; iY < kBox.m_iY1; iY++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLX) || kMerge.hasZeroSubedge(kBox.m_iLX)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3(getXInterp(iRoot, iY, kBox.m_iZ1), (float) iY, (float) kBox.m_iZ1));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the zmax face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getZMaxEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMIN_ZMAX, EI_XMAX_ZMAX);
                break;
            ...
            case 12:
                kTable.insert(EI_YMIN_ZMAX, EI_YMAX_ZMAX);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    // plus-sign configuration, add branch point to tessellation
                    kTable.insert(FI_ZMAX, kTable.getVertex(EI_YMIN_ZMAX).m_fX, kTable.getVertex(EI_XMIN_ZMAX).m_fY,
    ...
    /**
     * Compute the edges that connect the vertices on the zmin face of a nonvoxel box that are found by getVertices. An
     * additional vertex is added in the case when the face intersects the level surface in a plus-sign configuration.
     * That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getZMinEdgesM(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        for (int iX = kBox.m_iX0 + 1; iX < kBox.m_iX1; iX++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLY) || kMerge.hasZeroSubedge(kBox.m_iLY)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3((float) iX, getYInterp(iX, iRoot, kBox.m_iZ0), (float) kBox.m_iZ0));
            }
        }
        ...
        for (int iY = kBox.m_iY0 + 1; iY < kBox.m_iY1; iY++) {
            ...
            if (kMerge.isZeroEdge(kBox.m_iLX) || kMerge.hasZeroSubedge(kBox.m_iLX)) {
                ...
                // assert( iRoot != -1 );
                kVSet.add(new ASC_Vertex3(getXInterp(iRoot, iY, kBox.m_iZ0), (float) iY, (float) kBox.m_iZ0));
            }
        }
    ...
    /**
     * Compute the edges that connect the vertices on the zmin face of a 1-by-1-by-1 voxel that are found by
     * getVertices. An additional vertex is added in the case when the face intersects the level surface in a plus-sign
     * configuration. That vertex is the branch point of the plus.
     *
     * @param  kBox    the monoregion box
     ...
     * @param  kTable  the vertex-edge table representing the surface-box intersection as a wireframe
     */
    private void getZMinEdgesS(ASC_OctBox kBox, int iType, ASC_VertexTable kTable) {
        int iFaceType = 0;
        ...
        switch (iFaceType) {
            ...
            case 3:
                kTable.insert(EI_XMIN_ZMIN, EI_XMAX_ZMIN);
                break;
            ...
            case 12:
                kTable.insert(EI_YMIN_ZMIN, EI_YMAX_ZMIN);
                break;
            ...
            case 15: {
                ...
                if (fDet > 0.0f) {
                    ...
                    // plus-sign configuration, add branch point to tessellation
                    kTable.insert(FI_ZMIN, kTable.getVertex(EI_YMIN_ZMIN).m_fX, kTable.getVertex(EI_XMIN_ZMIN).m_fY,
    ...
    /**
     * This function constructs the binary trees for the lines of the image in all three coordinate directions.
     * Afterwards, the octree is traversed to merge the monoregions.
     *
     * @param  iDepth  This parameter controls the resolution of the extracted isosurface. If its value is -1, the
     *                 coarsest resolution surface is constructed. If its value is n where the image size is m-by-m-by-m
     *                 with m = power(2,n)+1, the finest resolution surface is constructed. This surface corresponds to
     *                 an extraction at the voxel level. Any value of iDepth between -1 and n may be used. The smaller
     *                 the value, the coarser the resolution the extracted surface is.
     */
    private void merge(int iDepth) {
        ...
        for (iY = 0; iY < m_iSize; iY++) {
            ...
            for (iZ = 0; iZ < m_iSize; iZ++) {
                ...
                iStride = 1;
                m_aakXMerge[iY][iZ].setLevel(m_fLevel, m_afData, iOffset, iStride);
            }
        }
        ...
        for (iX = 0; iX < m_iSize; iX++) {
            ...
            for (iZ = 0; iZ < m_iSize; iZ++) {
                ...
                iStride = m_iSize;
                m_aakYMerge[iX][iZ].setLevel(m_fLevel, m_afData, iOffset, iStride);
            }
        }
        ...
        for (iX = 0; iX < m_iSize; iX++) {
            ...
            for (iY = 0; iY < m_iSize; iY++) {
                ...
                iStride = m_iSizeSqr;
                m_aakZMerge[iX][iY].setLevel(m_fLevel, m_afData, iOffset, iStride);
            }
        }
    ...
    /**
     * Traverse the image as an octree and merge adjacent monoregions into larger monoregions (if possible).
     *
     * @param   iV      the index of the octree node stored as a complete tree in a 1D array
     * @param   iLX     iLY, iLZ the indices of the binary trees corresponding to the edges of the cube representing the
     *                  octree node
     * @param   iLY     iY0, iZ0 the origin of the cube representing the octree node
     * @param   iLZ     DOCUMENT ME!
     ...
     * @param   iDepth  This parameter controls the resolution of the extracted isosurface. If its value is -1, the
     *                  coarsest resolution surface is constructed. If its value is n where the image size is
     *                  m-by-m-by-m with m = power(2,n)+1, the finest resolution surface is constructed. This surface
     *                  corresponds to an extraction at the voxel level. Any value of iDepth between -1 and n may be
     *                  used. The smaller the value, the coarser the resolution the extracted surface is.
     *
     * @return  DOCUMENT ME!
     */
    private boolean merge(int iV, int iLX, int iLY, int iLZ, int iX0, int iY0, int iZ0, int iS, int iDepth) {
        ...
        if (iS > 1) // internal nodes
        {
            ...
            if (iDepth <= 0) {
                ...
                if (bM000 && bM001) {
                    doZMerge(kR000, kR001, iX0, iY0, iLZ);
                }

                if (bM100 && bM101) {
                    doZMerge(kR100, kR101, iX1, iY0, iLZ);
                }

                if (bM010 && bM011) {
                    doZMerge(kR010, kR011, iX0, iY1, iLZ);
                }

                if (bM110 && bM111) {
                    doZMerge(kR110, kR111, iX1, iY1, iLZ);
                }

                if (bM000 && bM010) {
                    doYMerge(kR000, kR010, iX0, iLY, iZ0);
                }

                if (bM100 && bM110) {
                    doYMerge(kR100, kR110, iX1, iLY, iZ0);
                }

                if (bM001 && bM011) {
                    doYMerge(kR001, kR011, iX0, iLY, iZ1);
                }

                if (bM101 && bM111) {
                    doYMerge(kR101, kR111, iX1, iLY, iZ1);
                }

                if (bM000 && bM100) {
                    doXMerge(kR000, kR100, iLX, iY0, iZ0);
                }

                if (bM010 && bM110) {
                    doXMerge(kR010, kR110, iLX, iY1, iZ0);
                }

                if (bM001 && bM101) {
                    doXMerge(kR001, kR101, iLX, iY0, iZ1);
                }

                if (bM011 && bM111) {
                    doXMerge(kR011, kR111, iLX, iY1, iZ1);
                }
            }
    ...
    /**
     * Construct the triangle meshes that represent the intersection of the level surface with the monoregion boxes.
     *
