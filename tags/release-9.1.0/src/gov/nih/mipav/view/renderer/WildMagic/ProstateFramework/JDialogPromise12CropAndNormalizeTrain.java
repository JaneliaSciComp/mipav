 ...
 * For promise 12 train data only, 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogPromise12CropAndNormalizeTrain extends JDialogBase implements
		AlgorithmInterface {
	 ...
	 */
	public JDialogPromise12CropAndNormalizeTrain(Frame theParentFrame) {
	...
	private void readKeyImageDir() {
	    ...
	    File fileDir_1 = new File("/scratch/miccai/Preprocessing/N4Train1_5");
		File fileDir_2 = new File("/scratch/miccai/Preprocessing/N4TrainMask1_5");
	...
	public void crop() {
		...
		for (int imageIndex = 0; imageIndex < size; imageIndex++) {
			...
			// ModelImage transKeyImage = cropImage(keyImage);
			// keyImagesCrop.add(imageIndex, transKeyImage);
			keyImagesCrop.add(imageIndex, keyImage);
			
			// ModelImage transKeyImageN4 = cropImage(keyImageN4);
			// keyImagesN4Crop.add(imageIndex, transKeyImageN4);
			keyImagesN4Crop.add(imageIndex, keyImageN4);
	...
	public void saveTestedImages() {
			...
			for (int imageIndex = 0; imageIndex < size; imageIndex++) {
				...
				if (!sliceDirFile.isDirectory())
					sliceDirFile.mkdir();
				sliceDir += File.separator + "N4TrainScale" + File.separator;
			...
			for (int imageIndex = 0; imageIndex < size; imageIndex++) {
				...
				if (!sliceDirFile.isDirectory())
					sliceDirFile.mkdir();
				sliceDir += File.separator + "N4TrainMaskScale" + File.separator;
				File dir = new File(sliceDir);
				if (!dir.isDirectory()) {
					dir.mkdir();
				}
