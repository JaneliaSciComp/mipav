 ...
 * 2. convert MR images to CED images. 
 * 3. converts the MRI and CED slices into png slices for both training and testing. 
 * 
 * @author Ruida Cheng
 * 
 */
public class JDialogProstate2DSlicesAtlasPngConverterMICCAI_conversion extends JDialogBase
	 ...
	 */
	public JDialogProstate2DSlicesAtlasPngConverterMICCAI_conversion(Frame theParentFrame) {
	...
	public void saveImagesTrain() {
		...
		for (int i = 0; i < cedImages.size(); i++) {
			try {
				...
				ModelImage keyImage = cedImages.get(i);
                ModelImage keyImageVOI = keyImageVOIs.get(i);
			...
			try {
				...
				for (int j = 0; j <= zDim; j++) {
					...
					try {
						...
						System.err.println(" image number = " + (0 + i) + "   slice number = " + (j+index));
						...
						System.err.println("index = " + (j + index));

						String imgName = "image_" + (j + index) + ".png";
