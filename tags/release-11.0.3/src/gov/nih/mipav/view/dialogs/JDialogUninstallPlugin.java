...
public class JDialogUninstallPlugin extends JDialogBase implements ActionListener {
    ...
    /**
     * Does not create the uninstall dialog, instead performs the install algorithm as if only the <code>name</code> plugin
    ...
    private MutableTreeNode buildPluginsTree() {
        ...
        for (int i = 0; i < plugin.getItemCount(); i++) {
            ...
            } else if (plugin.getItem(i) != null
                    && ! (plugin.getItem(i).getActionCommand().equals("InstallPlugin") || plugin.getItem(i).getActionCommand().equals("UninstallPlugin"))) {
    ...
    private void uninstallPlugins() {
        ...
        for (int i = 0; i < selectedPlugins.length; i++) {
            if (deletePluginDependents(selectedPlugins[i].toString()) && deletePluginFile(selectedPlugins[i].toString())) {
...
//            Class plugin = Class.forName(pluginName);
            Class plugin = PluginUtil.loadPluginClass(pluginName);
            ...
            if (allDep != null) {
                for (int i = 0; i < allDep.size(); i++) {
                    if (isInPluginFolder(allDep.get(i))) {
                        File f = new File(PluginUtil.getDefaultPluginDirectory() + allDep.get(i).getName() + ".class");
...
//        }
    ...
    private boolean isInPluginFolder(Class c) {
        ...
        try {
            ...
            // PluginUtil.getDefaultPluginDirectory() needs to specify a valid location
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean deletePluginFile(String name) {
        String pluginName = "PlugIn" + name;
        File f = new File(PluginUtil.getDefaultPluginDirectory() + pluginName + ".class");
        if (f.exists()) {
            return f.delete();
        }
        return false;
