...
public class JDialogSK10_MRI_CED_map_pre extends JDialogBase implements AlgorithmInterface {
	...
	private void readKeyImageDir() {
		...
		File fileDir_image = new File("/home/ruida/Knee_2010_challenge/train_img_sk10_femur_cart/axial");
		traverse_image_folder(fileDir_image, null);
		
		File fileDir_map = new File("/home/ruida/Knee_2010_challenge/train_img_sk10_femur_cart/test_out/fold10");
	...
	private void generateContours_MRI() {
		...
		for (String key : keys) {
			...
			for (int j = 0; j < imageDim; j++) {
				...
				// blank image to hold the MRI image prediction. 
				ModelImage blankImage = new ModelImage(image.getType(), blankExts, "_blank");
				int[] imageExt = image.getExtents();
				int imageSize = imageExt[0] * imageExt[1];
				int[] maskBuffer = new int[imageSize];
				...
				AlgorithmThresholdDual thresholdAlgo = new AlgorithmThresholdDual(resultImage, blankImage, thresholds,
						fillValue, outputType, regionFlag, isInverse);
				thresholdAlgo.run();
	 ...
	 * @param resultImage     result image to hold the resulting smoothed VOI contour. 
	 */
