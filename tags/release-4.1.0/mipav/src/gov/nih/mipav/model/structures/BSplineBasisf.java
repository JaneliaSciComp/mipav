...
/**
 * This class is an implementation of an open uniform B-Spline given a specified number of control points and a degree
 * for the basis functions. The knot array is interally generated with equally spaced elements.
 */
public class BSplineBasisf {

    //~ Instance fields ------------------------------------------------------------------------------------------------

    /** Storage for the basis functions. The basis array is always allocated by the constructor calls. */
    ...
    /**
     * Create a set of B-Spline uniform open basis functions for the given number of control points and degree of the
     * basis functions. Initially, the knot vectors are uniformly spaced although they can be modified.
     *
    ...
    /**
     * Evaluate basis functions for the given input and return the knot index i for which knot[i] <= fU < knot[i+1]. It
     * is then that indexes i-degree <= k <= i have non-zero basis evaluations.
     *
     * @param   fU     float Input value in the [0,1] range.
     * @param   afBD0  float[] Array to be filled with position basis evaluation at each of the knot values for the
     *                 given fU input. This array must have enough elements to store a value for each control point.
     *                 This array must not be null.
     * @param   afBD1  float[] Array to be filled with first derivative basis evaluation at each of the knot values for
     *                 the given fU input. This array may be null meaning that the first derivative basis evaluation is
     *                 not computed, but if the array is defined it must have enough elements to store a value for each
     *                 control point. If this array is null, then the afBD2 array must also be null.
     * @param   afBD2  float[] Array to be filled with second derivative basis evaluation at each of the knot values for
     *                 the given fU input. This array may be null meaning that the second derivative basis evaluation is
     *                 not computed, but if the array is defined it must have enough elements to store a value for each
     *                 control point. If this array is not null, then the afBD1 array must not be null.
     *
    ...
    public int compute(float fU, float[] afBD0, float[] afBD1, float[] afBD2) {
        ...
        for (int j = 1; j <= m_iDegree; j++) {
            ...
            if (iOrder >= 1) {
                ...
                if (iOrder >= 2) {
                    m_aafBD2[j][i] = ((fN0 * m_aafBD2[j - 1][i]) + (2.0f * m_aafBD1[j - 1][i])) * fInvD0;
                    m_aafBD2[j][i - j] = ((fN1 * m_aafBD2[j - 1][i - j + 1]) - (2.0f * m_aafBD1[j - 1][i - j + 1])) *
                                             fInvD1;
                }
            }
        }
        ...
        for (int j = 2; j <= m_iDegree; j++) {
            ...
            for (int k = i - j + 1; k < i; k++) {
                ...
                if (iOrder >= 1) {
                    m_aafBD1[j][k] = (((fN0 * m_aafBD1[j - 1][k]) + m_aafBD0[j - 1][k]) * fInvD0) +
                                     (((fN1 * m_aafBD1[j - 1][k + 1]) - m_aafBD0[j - 1][k + 1]) * fInvD1);

                    if (iOrder >= 2) {
                        m_aafBD2[j][k] = (((fN0 * m_aafBD2[j - 1][k]) + (2.0f * m_aafBD1[j - 1][k])) * fInvD0) +
                                         (((fN1 * m_aafBD2[j - 1][k + 1]) - (2.0f * m_aafBD1[j - 1][k + 1])) * fInvD1);
                    }
                }
            }
        }
    ...
    /**
     * Return the knot value at the specified control point interval index. The entire set of knot values cannot be
     * accessed since - the knot values in the [0,degree] range are fixed at zero. - the knot valeus in the
     * [numControl,numKnots) range are fixed at one.
     *
     * @param   iControlInterval  int Must be an index in the [0,numControl-1) range. Used to specify the relative
     *                            spacing between control points at iControlInterval and iControlInterval+1.
     *
    ...
    /**
     * Return number of knots which depends on the number of control points and the degree of the basis functions.
     *
     ...
     *
     * @param   that  BSplineBasisf BSpline basis instance to check for similarity.
     *
     * @return  boolean True if the two BSpline basis instances represent the same set of functions.
     */
    public boolean isSameAs(BSplineBasisf that) {
        return (this.m_iDegree == that.m_iDegree) && (this.m_iNumControlPoints == that.m_iNumControlPoints);
    }
     ...
     * @param   iControlInterval  int Must be an index in the [0,numControl-1) range. Used to specify the relative
     *                            spacing between control points at iControlInterval and iControlInterval+1.
     * @param   fValue            float Knot value to set at the specified control point interval index. Note that the
     *                            knot values must not be decreasing.
     *
     * @throws  IllegalArgumentException  DOCUMENT ME!
     */
     ...
     *
     * @param  afKnotValues  float[] Must be an array with at least numControl-1 elements. The values in this array
     *                       should be in the range of [0,1] and must not be decreasing.
     */
