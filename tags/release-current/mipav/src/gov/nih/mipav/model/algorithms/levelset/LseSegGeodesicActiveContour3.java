...
public class LseSegGeodesicActiveContour3 extends LseSegmenter
{
    //~ Instance fields ------------------------------------------------------
     ...
     */
    public LseSegGeodesicActiveContour3 (int iXBound, int iYBound, int iZBound,
        float fXSpacing, float fYSpacing, float fZSpacing, float[] afData,
        boolean[] abMask)
    {
        super(iXBound*iYBound*iZBound,afData,abMask);
        ...
        m_akNegGradient = new float[m_iQuantity][3];
    }
    ...
    void createFeatureImage ()
    {
        ...
        LseCurvatureFlow3 kCFilter = new LseCurvatureFlow3(m_iXBound,
        	m_iYBound,m_iZBound,m_fXSpacing,m_fYSpacing,m_fZSpacing,m_afData,
        	m_abMask,m_fBorderValue,m_eScaleType);
        ...
        for (iZ = 0, i = 0; iZ < m_iZBound; iZ++)
        {
            for (iY = 0; iY < m_iYBound; iY++)
            {
                for (iX = 0; iX < m_iXBound; iX++, i++)
                {
                    m_afCurvFlow[i] = kCFilter.getU(iX,iY,iZ);
                }
            }
        }

        // Apply Gaussian blurring before computing the gradients.
        LseGaussianBlur3 kGFilter = new LseGaussianBlur3(m_iXBound,m_iYBound,
            m_iZBound,m_fXSpacing,m_fYSpacing,m_fZSpacing,m_afCurvFlow,
            m_abMask,m_fBorderValue,m_eScaleType);
        ...
        for (iZ = 0, i = 0; iZ < m_iZBound; iZ++)
        {
            for (iY = 0; iY < m_iYBound; iY++)
            {
                for (iX = 0; iX < m_iXBound; iX++, i++)
                {
                    m_afBlurCurvFlow[i] = kGFilter.getU(iX,iY,iZ);
                }
            }
        }
        ...
        for (iZ = 0, i = 0; iZ < m_iZBound; iZ++)
        {
            for (iY = 0; iY < m_iYBound; iY++)
            {
                for (iX = 0; iX < m_iXBound; iX++, i++)
                {
                    float fUx = kGFilter.getUx(iX,iY,iZ);
                    float fUy = kGFilter.getUy(iX,iY,iZ);
                    float fUz = kGFilter.getUz(iX,iY,iZ);
                    m_afGradMagBlurCurvFlow[i] = (float)Math.sqrt(
                        (double)(fUx*fUx + fUy*fUy + fUz*fUz));
                    m_akNegGradient[i][0] = -fUx;
                    m_akNegGradient[i][1] = -fUy;
                    m_akNegGradient[i][2] = -fUz;
                }
            }
        }
    ...
    /**
     * Get the negative of the gradient.  The result is relevant only after a
     * call to computeFeatureImage or beginCoarse.
     * 
     * @return The gradient magnitude of the blurred curvature flow image.
     */
    float[][] getNegGradientImage ()
    {
         return m_akNegGradient;
    }
        ...
        float fLaplacianWeight, float fTimeStep)
    {
        ...
        m_kLevelSetEvolver = new LseGeodesicActiveContour3(m_iXBound,
            m_iYBound,m_iZBound,m_fXSpacing,m_fYSpacing,m_fZSpacing,
            m_afSignedDistance,m_abSignedDistanceMask,m_fBorderValue,
            LsePdeFilter.PRESERVE_ZERO,m_afFeature,m_akNegGradient,
            fAdvectionWeight,fPropagationWeight,fCurvatureWeight);
