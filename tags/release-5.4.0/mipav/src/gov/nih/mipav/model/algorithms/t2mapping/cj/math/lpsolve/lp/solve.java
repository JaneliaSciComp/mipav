/* $Header: /brain/craig/work/CVS/java/cj/math/lpsolve/lp/solve.java,v 1.5 2002/04/19 16:32:43 craig Exp $ */
/* $Log: solve.java,v $
/* Revision 1.5  2002/04/19 16:32:43  craig
/* MAJOR CHANGES TO DIRECTORY STRUCTURE AND RENAMING
/*
/* Revision 1.4  2002/03/19 04:38:32  craig
/* Converted all short representations of true/false values to boolean. Still
/* need to test this all...
/*
/* Revision 1.3  2002/03/19 03:55:52  craig
/* Changed some loops to Arrays.fill.
/*
  ...
  if(lp.rows > lp.rows_alloc)
    {
      ...
      boolean[] short_ptr = lp.basis;
      lp.basis = new boolean[lp.sum_alloc + 1];
      System.arraycopy(short_ptr, 0, lp.basis, 0, short_ptr.length);

      short_ptr = lp.lower;
      lp.lower = new boolean[lp.sum_alloc + 1];
      System.arraycopy(short_ptr, 0, lp.lower, 0, short_ptr.length);

      short_ptr = lp.must_be_int;
      lp.must_be_int = new boolean[lp.sum_alloc + 1];
      ...
      lp.ch_sign = new boolean[lp.rows_alloc + 1];
  ...
  if(lp.columns >= lp.columns_alloc)
    {
      ...
      boolean[] short_ptr = lp.must_be_int;
      lp.must_be_int = new boolean[lp.sum_alloc + 1];
      System.arraycopy(short_ptr, 0, lp.must_be_int, 0, short_ptr.length);
...
public void add_constraint(lprec lp, double[] row, short constr_type, double rh)
{
  ...
  addtoo = new boolean[lp.columns + 1];
...
public void set_int(lprec lp, int column, boolean must_be_int)
{
  if(column > lp.columns || column < 1)
    System.err.print("Column out of range");
...
boolean is_feasible(lprec lp, double[] values)
{
  int i, elmnr;
  double[] this_rhs;
  double dist;
...
boolean column_in_lp(lprec lp, double[] testcolumn)
{
  int i, j;
  boolean ident;
  double value;
...
static boolean Isvalid(lprec lp)
{
...
private void setpivcol(boolean lower, 
		      int varin,
		      double[]   pcol)
{
  int  i, colnr;
...
//    pcol[i] = 0;
  ...
  frow = new boolean[Rows + 1];
  for(i = 0; i <= Rows; i++)
    frow[i] = true;
  fcol = new boolean[Columns + 1];
  for(i = 0; i < Columns; i++)
    fcol[i] = false;
...
//    colnum[i] = 0;
  ...
  for(j = 1; j <= Columns; j++)
	...
	row_nr = 1;
	while((frow[row_nr] == false || pcol[row_nr] == 0)
		 && row_nr <= Rows)
...
private boolean colprim(Ref colnr,
		     boolean minit,
		     double[]   drow)
{
  int  varnr, i, j;
...
*/
  ...
  return(colnr.value > 0 ? true : false);
} /* colprim */

private boolean rowprim(int colnr,
                     Ref row_nr,
		     Ref theta,
		     double[] pcol)
{
  ...
  return((row_nr.value > 0) ? true : false);
} /* rowprim */

private boolean rowdual(Ref row_nr)
{
  int   i;
  double  f, g, minrhs;
  boolean artifs;
...
private boolean coldual(int row_nr,
		     Ref colnr,
		     boolean minit,
		     double[] prow,
		     double[] drow)
...
private int solvelp()
{
  ...
  Pcol = new double[Rows + 1];
  Arrays.fill(drow, 0.0);
  Arrays.fill(prow, 0.0);
  Arrays.fill(Pcol, 0.0);
//  for (i = 0; i <= Sum; i++) {
//    drow[i] = 0;
//    prow[i] = 0;
//  }
...
//    Pcol[i] = 0;
  ...
  while(i != Rows && primal != false)
    {
      i++;
      primal = (Rhs[i] >= 0 && Rhs[i] <= Upbo[Bas[i]]) ?  true: false;
    }
  if(Lp.trace != false)
    {
      if(primal != false)
  ...
  while(Status == RUNNING)
    {
	              ...
	              if(f > 0)
	                {
	                  ...
	                  if(theta <= Upbo[colnr])
	                    Lower[Bas[row_nr]] = 
				(Lower[Bas[row_nr]] == false)? true:false;
	                }
	              else /* f <= 0 */
	                theta = Rhs[row_nr] / (double) Pcol[row_nr];
	            }
      ...
      if(Doiter != false) {
	ref1.value = theta;
	ref2.value = minit?1:0;
	ref3.value = Lower[colnr]?1:0;
        iteration(row_nr, colnr, ref1, Upbo[colnr], ref2, ref3, primal, Pcol);
	theta = ref1.value;
	minit = (ref2.value>0)?true:false;
	Lower[colnr] = (ref3.value>0)?true:false;
      }
...
private boolean is_int(double value)
{
  double   tmp;
  ...
  if(failure == OPTIMAL)	/* there is a solution */
    {
      ...
      if(Maximise != false)
	is_worse = (Solution[0] <= Best_solution[0]) ? true:false;
      else			/* minimising! */
	is_worse = (Solution[0] >= Best_solution[0]) ? true:false;
      ...
      if(is_worse != false)
	{
      ...
      if(notint != 0)		/* there is at least one value not yet int */
	{
	  /* set up two new problems */
	  double[]   new_upbo, new_lowbo;
	  double   new_bound;
          boolean[]  new_lower, new_basis;
	  int[]    new_bas;
          int     resone, restwo;
	  ...
	  new_lowbo = new double[Sum + 1];
          new_lower = new boolean[Sum + 1];
          new_basis = new boolean[Sum + 1];
	  ...
	  debug_print_bounds(upbo, lowbo);
	      ...
	      else		/* bound feasible */
		{
          ...
          if(resone != OPTIMAL && restwo != OPTIMAL) 
...
public int lag_solve(lprec lp, double start_bound, int num_iter, boolean verbose)
{
  int i, j, result, citer;
  short status;
  boolean OrigFeas, AnyFeas, same_basis;
  ...
  old_lower = new boolean[lp.sum + 1];
      ...
      for(i = 0; i < lp.nr_lagrange; i++)
        {
          ...
          if(lp.lag_con_type[i] == 0 && lp.lambda[i] < 0)
            lp.lambda[i] = 0;
        }
