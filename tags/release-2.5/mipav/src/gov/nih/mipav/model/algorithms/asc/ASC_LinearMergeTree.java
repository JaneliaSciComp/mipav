...
/**
 * The binary tree implementation that stores sign change information for each line of an image.
 */
...
public class ASC_LinearMergeTree {
    ...
    /**
     * The flags indicating if a 1D monoregion has no sign change (CFG_NONE), a negative-to-positive sign change
     * (CFG_INCR), a positive-to-negative sign change (CFG_DECR), or multiple sign changes (CFG_MULT).
     */
     ...
     *
     * @param  iN  The image line has power(2,iN)+1 samples. The binary tree is stored as an array with power(2,iN+1)-1
     *             elements.
     */
     ...
     */
    public final int getRootType(int i) {
        return (int) m_aucNode[i] & CFG_ROOT_MASK;
    }

    /**
     * Get the image base index that is the end point of a 1-voxel wide interval on which the image changes sign.
     *
     * @param   i  the binary tree index for the node
     *
     * @return  The base index. If the returned value is -1, there is no sign change on the interval. Otherwise the
     *          return value is nonnegative and the interval does have a sign change.
     */
    public final int getZeroBase(int i) {
        return (int) m_asZeroBase[i];
    }

    /**
     * Determine if the the binary tree node has a descendant with the CFG_EDGE attribute.
     *
     * @param   i  the binary tree index for the node
     *
     * @return  'true' if and only if a descendant has the CFG_EDGE attribute
     */
    public final boolean hasZeroSubedge(int i) {
     ...
     */
    public final boolean isNone(int i) {
        return ((int) m_aucNode[i] & CFG_ROOT_MASK) == CFG_NONE;
    }

    /**
     * Determine if the sign information for the binary tree node is "incr" or "decr".
     *
     * @param   i  the binary tree index for the node
     *
     * @return  'true' if and only if the sign flag is CFG_INCR or CFG_DECR
     */
    public final boolean isZeroEdge(int i) {
        return ((int) m_aucNode[i] == (CFG_EDGE | CFG_INCR)) || ((int) m_aucNode[i] == (CFG_EDGE | CFG_DECR));
    }

    /**
     * Mark a node as an edge of a monoregion. All ancestors are marked as having a descendant that is an edge.
     *
     * @param  i  the binary tree index for the node
     */
    ...
    /**
     * Compute the sign information for all the nodes of the binary tree. The level set value should be a non-integer
     * value. This constraint greatly reduces the algorithmic complexity for computing level sets.
     *
     * @param  fLevel   the desired level set value
     * @param  afData   the entire image (2D or 3D)
     * @param  iOffset  the starting point for the line of power(2,n)+1 values in the image
     * @param  iStride  the stride of the image samples that correspond to the image line (stride 1 for x direction,
     *                  2^n+1 for y direction, (2^n+1)^2 for z direction
     */
    public final void setLevel(float fLevel, float[] afData, int iOffset, int iStride) {
