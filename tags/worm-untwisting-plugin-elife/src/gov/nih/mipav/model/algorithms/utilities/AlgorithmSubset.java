...
public class AlgorithmSubset extends AlgorithmBase {
     ...
     */
    public AlgorithmSubset(final ModelImage srcImage, final ModelImage destImage, final int removeDim, final int sliceNum) {
    ...
    public void runAlgorithm() {
        ...
        // float[] imagePositionCoords = convertIntoFloat(((FileInfoDicom)
        // srcImage.getFileInfo(0)).parseTagValue("0020,0032"));
        // float[] nextPositionCoords = convertIntoFloat(((FileInfoDicom)
        // srcImage.getFileInfo(1)).parseTagValue("0020,0032"));
        //
        ...
        final int axis3 = (removeDim == REMOVE_T) ? 2 : 3;
        ...
        if (srcImage.getFileInfo()[0] instanceof FileInfoDicom) {
            ...
            for (int z = 0; z < dim3; z++) {
                ...
                destFileInfo[z] = new FileInfoDicom(oldDicomInfo.getFileName(), oldDicomInfo.getFileDirectory(), oldDicomInfo.getFileFormat());
                ...
                ((FileInfoDicom) destFileInfo[z]).getTagTable().importTags(oldDicomInfo);
                ((FileInfoDicom) destFileInfo[z]).getTagTable().removeTag("0019,100A");// Removes NumberofImages in
                                                                                       // Mosaic Tag
                sliceCounter++;
