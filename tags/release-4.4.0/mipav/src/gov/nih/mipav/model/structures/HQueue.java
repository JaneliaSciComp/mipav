...
public class HQueue {
    ...
    public HQueue(float hmin, float hmax, int MAX, int queueSize) {
        ...
        for (int i = 0; i < MAX; i++) {
            ...
            /*if (i < 10){
             *  // allocate larger arrays for small values (i.e. background) intVect[i] = new IntVector(imageSize/20,
             * imageSize/40); } else if ( i >= 10 && i <= 50){ intVect[i] = new IntVector(imageSize/40, imageSize/80); }
             * else if ( i >= 51 && i <= 100){ intVect[i] = new IntVector(4000, 2000); }else {*/
