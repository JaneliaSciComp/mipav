 ...
 * The data we used is the prostate NIH data (cg and wp).  Test case only.
 * 
 ...
 * 
 */
public class JDialogProstate2DSlicesAtlasPngConverterCentralGland_CED_scale_boundary_test extends JDialogBase
	 ...
	 */
	public JDialogProstate2DSlicesAtlasPngConverterCentralGland_CED_scale_boundary_test(Frame theParentFrame) {
	...
	public void saveImagesTest(String key, int []index, String folderName) {
		try {
			...
			int size_3D = xDim * yDim * zDim;
			float[] imageBuffer = new float[size_3D];
			float[] cedBuffer = new float[size_3D];

			try {
			  keyImage.exportData(0, imageBuffer.length, imageBuffer);
			  cedImage.exportData(0, cedBuffer.length, cedBuffer);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			Arrays.sort(imageBuffer);
			Arrays.sort(cedBuffer);

			float min = imageBuffer[minIndex];
			float max = imageBuffer[maxIndex];

			float minCED = cedBuffer[minIndex];
			float maxCED = cedBuffer[maxIndex];
			...
			for (int j = 0; j < zDim; j++) {
				...
				try {
					...
					System.err.println(" image number = " + key + "   slice number = " + (j+zDim));

					ModelImage targetImageSlice = new ModelImage(ModelStorageBase.FLOAT, newExtents, "target" + (j+index[0]));
					float[] targetBuffer = new float[size];
					...
					String maskName = "voi_" + (j + zDim) + ".png";
					savePNGfile(sliceDir, maskName, maskImage, min, max, xDim, yDim, true);
