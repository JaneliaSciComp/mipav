 ...
 *  <p>
 *  1. Generates binary image based on threshold values passed
 *     in by the constructor. Inside the volume of interest:
 *     If the image intensity is greater than or equal to the
 *     threshold[0] and less than or equal to the threshold[1],
 *     the binary image is set to one.  Otherwise, it is set to zero.
 *     Outside the volume of interest:
 *     The binary image is set equal to zero.
 *  <p>
 *  2. Inside the volume of interest:
 *     Preserves all grey scale values from threshold[0] to threshold[1] and
 *     sets all values less than the lower threshold and all values
 *     greater than the upper threshold to the fillValue supplied to
 *     this algorithm via the constructor.
 *     Outside the volume of interest:
 *     Sets all gray scale values to the fill value.
 *
...
public class AlgorithmThresholdDual
    ...
    /**
     *   Inverse threshold:  true means turn all pixels outside of the lower
     *   and upper thresholds to the given fill value,
     *   false means turn all data within the lower and upper thresholds to the fill value
     *   (fill value can also be binary (0))
     */
     ...
     *   @param binaryFlag   if true a binary image is produced, if false gray scale values
     *                       between the thresholds are retained
     *   @param maskFlag     true indicates that the whole image should be processed
     *   @param isInverse    true means turn all pixels outside of the lower
     *                       and upper thresholds to the given fill value,
     *                       false means turn all data within the lower and upper thresholds to the fill value
     */
    public AlgorithmThresholdDual(ModelImage destImg, ModelImage srcImg, float threshold[],
                                  float fillValue, boolean binaryFlag, boolean maskFlag,
                                  boolean isInverse) {
     ...
     *                       false means turn all data within the lower and upper thresholds to the fill value
     */
    public AlgorithmThresholdDual(ModelImage srcImg, float threshold[], float fillValue,
                                  boolean binaryFlag, boolean maskFlag,
                                  boolean isInverse) {
    ...
    private void constructLog() {
        historyString = new String("Threshold(" +
                                   String.valueOf(threshold[0]) + ", " +
                                   String.valueOf(threshold[1]) + ", " +
                                   String.valueOf(fillValue) + ", " +
                                   String.valueOf(binaryFlag) + ", " +
        ...
        for (i = 0; i < length && !threadStopped; i++) {
            ...
            if (isInverse) {
                if ( (entireImage == true || mask.get(i)) &&
                    ...
                    if (binaryFlag == true) buffer[i] = 0;
                    else buffer[i] = fillValue;
                }
            }
            else {
                if ( (entireImage == true || mask.get(i)) &&
        ...
        for (i = 0; i < length && !threadStopped; i++) {
            if (i % mod == 0 && isProgressBarVisible())
                ...
                if (isInverse) {
                    if ( (entireImage == true || mask.get(i)) &&
                        (buffer[i] >= threshold[0] && buffer[i] <= threshold[1])) {
                        if (binaryFlag == true) destImage.set(i, 1);
                        else destImage.set(i, buffer[i]);
                    }
                    else {
                        if (binaryFlag == true) destImage.set(i, 0);
                        else destImage.set(i, fillValue);
                    }
                }
                else {
                    if ( (entireImage == true || mask.get(i)) &&
