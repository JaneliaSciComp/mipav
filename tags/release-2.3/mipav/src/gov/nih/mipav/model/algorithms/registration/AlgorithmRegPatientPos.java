...
public class AlgorithmRegPatientPos extends AlgorithmBase {
    ...
    private void reOrient() {
        ...
        /*
         * Create index arrays that store the reordering of Image B data (i.e. origin, resolution, dimension)
         * to the ordering of Image A.
         * index2Axial, index2ImgA, sign2Axial, and sign2ImgA will be created.
         */
        getIndices();
        ...
        Preferences.debug(
                "Original ImageB startLocation: " + (float) coordB[0] + ", " + (float) coordB[1] + ", "
                + (float) coordB[2] + "\n" );
        ...
        AlgorithmTransform algoTransform = new AlgorithmTransform( imageB, alignmentXfrm, AlgorithmTransform.TRILINEAR,
                (float) resB[index2ImgA[0]], (float) resB[index2ImgA[1]], (float) resB[index2ImgA[2]],
                (int) dimB[index2ImgA[0]], (int) dimB[index2ImgA[1]], (int) dimB[index2ImgA[2]], false, false, true );
        ...
        Preferences.debug(
                "ImageB resolution in reOrient after transform: " + (float) resB[0] + ", " + (float) resB[1] + ", "
                + (float) resB[2] + "\n" );
        Preferences.debug(
                "ImageB dimensions in reOrient after transform: " + (int) dimB[0] + ", " + (int) dimB[1] + ", "
                + (int) dimB[2] + "\n" );
        ...
        Preferences.debug(
                "ImageB startLocation in A order: " + (float) coordB[0] + ", " + (float) coordB[1] + ", "
                + (float) coordB[2] + "\n" );
        ...
        Preferences.debug(
                "New image origin: " + (float) coordB[0] + ", " + (float) coordB[1] + ", " + (float) coordB[2] + "\n" );
        ...
        Preferences.debug(
                "Original image dimensions: " + oldDims[index2ImgA[0]] + " " + oldDims[index2ImgA[1]] + " "
        ...
        Preferences.debug(
                "New orientedB (tempImg) dimensions: " + newNewDims[0] + " " + newNewDims[1] + " " + newNewDims[2]
                + ".\n" );
    ...
    private void matchOrigin() {
        ...
        Preferences.debug(
                "ImageA startLocation in MatchOrigin: " + (float) coordA[0] + ", " + (float) coordA[1] + ", "
                + (float) coordA[2] + "\n" );
        getPatientPosition( coordB, resultImg );
        Preferences.debug(
                "ImageB startLocation in order in MatchOrigin: " + (float) coordB[0] + ", " + (float) coordB[1] + ", "
                + (float) coordB[2] + "\n" );
        ...
        if ( ( coordA[0] == 0.0 && coordA[1] == 0.0 && coordA[2] == 0.0 )
            ...
            MipavUtil.displayInfo(
                    "Start locations for either Image A or Image B are all zeros.  The origin won't be used "
                            + " in registration." );
        } else {
            for ( int i = 0; i < 3; i++ ) {
             ...
             * AlgorithmAddMargins(ModelImage srcImage, ModelImage destImage, double n,
             *				int leftWidth, int rightWidth, int height, int front, int back)
             */
            ...
            resultImg = new ModelImage( imageB.getType(), destExtentsC, imageB.getImageName(), imageB.getUserInterface() );
            AlgorithmCrop algoCrop = new AlgorithmCrop( resultImg, shiftedImgB, 0, boundX, boundY, boundZ );

            /* constructor looks like:
             * AlgorithmCrop(ModelImage destImg, ModelImage srcImg, int _cushion, int x[], int y[], int z[])
             * where x, y, and z are int arrays with 2 elements representing lower and upper bounds.
             */
            algoCrop.setActiveImage( activeImage );
            algoCrop.run();
            if ( algoCrop.isCompleted() == false ) {
    ...
    private void getTransform() {
        ...
        orientMatrixA = new double[][] { { xOrientA[0], xOrientA[1], xOrientA[2], 0 }, {
                yOrientA[0], yOrientA[1],
                yOrientA[2], 0 }, { zOrientA[0], zOrientA[1], zOrientA[2], 0 }, { 0, 0, 0, 1 } };
        ...
        orientMatrixB = new double[][] { { xOrientB[0], xOrientB[1], xOrientB[2], 0 }, {
                yOrientB[0], yOrientB[1],
                yOrientB[2], 0 }, { zOrientB[0], zOrientB[1], zOrientB[2], 0 }, { 0, 0, 0, 1 } };
     ...
     *   @param  image   image
     */
    public void getPatientOrientation( double[] xOrient, double[] yOrient,
            ModelImage image ) {
        if ( image.getFileInfo( 0 ).getFileFormat() == FileBase.DICOM ) {
            FileInfoDicom fileInfo = (FileInfoDicom) ( image.getFileInfo( 0 ) );
    ...
    /**
     *   Get indices for reordering data from Image B to the Axial coordinate system (LPS)
     *	and to the coordinate system of Image A.
     */
