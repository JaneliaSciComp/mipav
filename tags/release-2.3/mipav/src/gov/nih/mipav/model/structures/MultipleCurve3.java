 ...
 * upon that of the IntervalLengthCurve3 abstract interface by adding support
 * for curves defined by segments.  Each segment is defined by time values,
 * the same ones used to define the length intervals used in the
 * IntervalLengthCurve3 superclass.
 */
    ...
    extends IntervalLengthCurve3
{
     ...
     * @param afTime float[] Array of time values associated with
     * the segments.  These times values must be increasing where the
     * first and last values in the array define the minimum and maximum
     * input time values, respectively.
     */
    ...
    /**
     * Compute the accumulated distance between the specified sequence
     * of 3D points.
    ...
    /**
     * Get the offset in time for the input time relative to the starting
     * time for the segment which contains the input time.
     * @param fTime float Input time value.
     ...
     * The input time is expessed in terms of its associated segment.
     * @param iSegment int Index of segment containing the input time.
     * @param fDt float Segment-relative time.
     * @return float The computed speed.  If the fDt value is less than 0
     * then the speed at the start of the segment is returned.  If the fDt
     * value is greater than the segment's width in time, then the speed at the
     * end of the segment is returned.
     */
