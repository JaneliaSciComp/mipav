 ...
 *      First slice is template (base image) to which subsequent
 *          slices are registered.
 *      Trace kidney in 1st slice using polygon VOI.
...
public class AlgorithmRegVOILandmark extends AlgorithmBase {
     ...
     *   @param costFunc
     */
    public AlgorithmRegVOILandmark( ModelImage volume, ModelImage gradMagVol,
            float[] sigmas,
            boolean maskFlag, Vector3Df[] position,
            double minTx, double maxTx, double minTy,
            double maxTy,
            double minRz, double maxRz, double step,
            int opt, int costFunc ) {
        this.volume = volume;
    ...
    private void transformSlice( int slice, TransMatrix xfrm,
        ...
        try {
            ...
            AlgorithmTransform.transformBilinear( imgBuf, tImgBuf, xfrm, xdim, ydim,
                    volume.getFileInfo( 0 ).getResolutions()[
                    0], volume.getFileInfo( 0 ).getResolutions()[
                    1], xdim, ydim, volume.getFileInfo( 0 ).getResolutions()[
                    0], volume.getFileInfo( 0 ).getResolutions()[
                    1], null );
     ...
     *   slice - current fluoroscopy slice being operated on
     */
            ...
            for ( i = 0; i < VOIlength; i++ ) {
                ...
                if ( ( X >= 0 ) && ( X < xdim - 1 ) ) {
                    ...
                    if ( ( Y >= 0 ) && ( Y < ydim - 1 ) ) {
                        ...
                        //  volIndex = (int)matchVOIposition[i].x +
                        //        (int)matchVOIposition[i].y*xdim + slice*sliceSize;
                    }
                }
