...
/**
 *  Calculates the non-maximum suppression of an image at a scale defined
 *  by the user.  This algorithm produces an edge map of the zero crossings
 *  of the non-maximum suppression for 2D images and 2.5D images.
 *  Edges are defined as the union of points for which the gradient magnitude
 *  assumes a maximum in the gradient direction.  Introuduce a local orthonormal
 *  coordinate system (u,v) at any point P0, where the v-axis is parallel to
 *  the gradient direction at P0, and the u-axis is perpindicular.
 *  Iu = sin(a)*Ix - cos(a)*Iy
 *  Iv = cos(a)*Ix + sin(a)*Iy
 *  cos(a) = Ix/(sqrt(Ix*Ix + Iy*Iy)) evaluated at P0
 *  sin(a) = Iy/(sqrt(Ix*Ix + Iy*Iy)) evaluated at P0
 *  Iu = (Ix*Iy - Ix*Iy)/(sqrt(Ix*Ix + Iy*Iy)) = 0
 *  Iv = (Ix*Ix + Iy*Iy)/(sqrt(Ix*Ix + Iy*Iy)) = sqrt(Ix*Ix + Iy*Iy)
 *  Iv*Iv = Ix*Ix + Iy*Iy
 *  Ivv = (cos(a)*Ix + sin(a)*Iy)(cos(a)*Ix + sin(a)*Iy) =
 *        cos(a)*cos(a)*Ixx + 2*cos(a)*sin(a)*Ixy + sin(a)*sin(a)*Iyy =
 *        (Ix*Ix*Ixx + 2*Ix*Iy*Ixy + Iy*Iy*Iyy)/(Ix*Ix + Iy*Iy)
 *  Iv*Iv*Ivv = Ix*Ix*Ixx + 2*Ix*Iy*Ixy + Iy*Iy*Iyy
 *  Ivvv = (cos(a)*cos(a)*Ixx + 2*cos(a)*sin(a)*Ixy + sin(a)*sin(a)*Iyy)(cos(a)*Ix + sin(a)*Iy) =
 *         cos(a)*cos(a)*cos(a)*Ixxx + 3*cos(a)*cos(a)*sin(a)*Ixxy +
 *         3*cos(a)*sin(a)*sin(a)*Ixyy + sin(a)*sin(a)*sin(a)*Iyyy =
 *         (Ix*Ix*Ix*Ixxx + 3*Ix*Ix*Iy*Ixxy + 3*Ix*Iy*Iy*Ixyy + Iy*Iy*Iy*Iyyy)/((Ix*Ix + Iy*Iy)**3/2)
 *  Iv*Iv*Iv*Ivvv = Ix*Ix*Ix*Ixxx + 3*Ix*Ix*Iy*Ixxy + 3*Ix*Iy*Iy*Ixyy + Iy*Iy*Iy*Iyyy
 *  Assuming that the second and third-order directional derivatives of I in the
 *  v-direction are not simultaneously zero, a necessary and sufficient condition for P0
 *  to be a gradient maximum in the gradient direction may be stated as:
 *  Ivv = 0,
 *  Ivvv < 0.
 *  Since only the sign information is important, this condition can be restated as:
 *  Iv*Iv*Ivv = 0,
 *  Iv*Iv*Iv*Ivvv < 0.
 *  Reference:  Geometry-Driven Diffusion in Computer Vision, Bart M. ter Haar Romeny(Ed.),
 *  Chapter2:Linear Scale-Space II: Early Visual Operations by Tony Lindeberg and Bart M.
 *  ter Haar Romeny, Kluwer Academic Publishers, 1994, page 45.
 *
 *  For 3D a similar derivation:
 *  Iv = cos(a)*sin(b)*Ix + sin(a)*sin(b)*Iy + cos(b)*Iz
 *  where spherical coordinates are being used
 *  cos(a) = Ix/(sqrt(Ix*Ix + Iy*Iy)) evaluated at P0
 *  sin(a) = Iy/(sqrt(Ix*Ix + Iy*Iy)) evaluated at P0
 *  cos(b) = Iz/(sqrt(Ix*Ix + Iy*Iy + Iz*Iz)) evaluated at P0
 *  sin(b) = sqrt(Ix*Ix + Iy*Iy)/(sqrt(Ix*Ix + Iy*Iy + Iz*Iz)) evaluated at P0
 *  Iv = (Ix*Ix + Iy*Iy + Iz*Iz)/(sqrt(Ix*Ix + Iy*Iy + Iz*Iz)) =
 *       sqrt(Ix*Ix + Iy*Iy + Iz*Iz)
 *  Iv*Iv = Ix*Ix + Iy*Iy + Iz*Iz
 *  Ivv = (cos(a)*sin(b)*Ix + sin(a)*sin(b)*Iy + cos(b)*Iz)(cos(a)*sin(b)*Ix +
 *         sin(a)*sin(b)*Iy + cos(b)*Iz) =
 *         cos(a)*cos(a)*sin(b)*sin(b)*Ixx + 2*cos(a)*sin(a)*sin(b)*sin(b)*Ixy +
 *         2*cos(a)*cos(b)*sin(b)*Ixz + sin(a)*sin(a)*sin(b)*sin(b)*Iyy +
 *         2*sin(a)*cos(b)*sin(b)*Iyz + cos(b)*cos(b)*Izz =
 *         (Ix*Ix*Ixx + 2*Ix*Iy*Ixy + 2*Ix*Iz*Ixz + Iy*Iy*Iyy + 2*Iy*Iz*Iyz + Iz*Iz*Izz)/
 *         (Ix*Ix + Iy*Iy + Iz*Iz)
 *  Iv*Iv*Ivv = Ix*Ix*Ixx + 2*Ix*Iy*Ixy + 2*Ix*Iz*Ixz + Iy*Iy*Iyy + 2*Iy*Iz*Iyz + Iz*Iz*Izz
 *  Ivvv = (cos(a)*cos(a)*sin(b)*sin(b)*Ixx + 2*cos(a)*sin(a)*sin(b)*sin(b)*Ixy +
 *          2*cos(a)*cos(b)*sin(b)*Ixz + sin(a)*sin(a)*sin(b)*sin(b)*Iyy +
 *          2*sin(a)*cos(b)*sin(b)*Iyz + cos(b)*cos(b)*Izz)(cos(a)*sin(b)*Ix +
 *          sin(a)*sin(b)*Iy + cos(b)*Iz) =
 *          cos(a)*cos(a)*cos(a)*sin(b)*sin(b)*sin(b)*Ixxx +
 *          3*cos(a)*cos(a)*sin(a)*sin(b)*sin(b)*sin(b)*Ixxy +
 *          3*cos(a)*cos(a)*cos(b)*sin(b)*sin(b)*Ixxz +
 *          3*cos(a)*sin(a)*sin(a)*sin(b)*sin(b)*sin(b)*Ixyy +
 *          6*cos(a)*sin(a)*cos(b)*sin(b)*sin(b)*Ixyz +
 *          3*cos(a)*cos(b)*cos(b)*sin(b)*Ixzz +
 *          sin(a)*sin(a)*sin(a)*sin(b)*sin(b)*sin(b)*Iyyy +
 *          3*sin(a)*sin(a)*cos(b)*sin(b)*sin(b)*Iyyz +
 *          3*sin(a)*cos(b)*cos(b)*sin(b)*Iyzz +
 *          cos(b)*cos(b)*cos(b)*Izzz =
 *         (Ix*Ix*Ix*Ixxx + 3*Ix*Ix*Iy*Ixxy + 3*Ix*Ix*Iz*Ixxz + 3*Ix*Iy*Iy*Ixyy + 6*Ix*Iy*Iz*Ixyz +
 *          3*Ix*Iz*Iz*Ixzz + Iy*Iy*Iy*Iyyy + 3*Iy*Iy*Iz*Iyyz + 3*Iy*Iz*Iz*Iyzz + Iz*Iz*Iz*Izzz)/
 *         ((Ix*Ix + Iy*Iy + Iz*Iz)**1.5)
 *  Iv*Iv*Iv*Ivvv = Ix*Ix*Ix*Ixxx + 3*Ix*Ix*Iy*Ixxy + 3*Ix*Ix*Iz*Ixxz + 3*Ix*Iy*Iy*Ixyy + 6*Ix*Iy*Iz*Ixyz +
 *                  3*Ix*Iz*Iz*Ixzz + Iy*Iy*Iy*Iyyy + 3*Iy*Iy*Iz*Iyyz + 3*Iy*Iz*Iz*Iyzz + Iz*Iz*Iz*Izzz
 */

public class AlgorithmEdgeNMSuppression extends AlgorithmBase {
     ...
     *   @param maskFlag Flag that indicates that the EdgeNMSup will be
    ...
    private void constructLog() {
        ...
        historyString = new String(
                "EdgeNMSup(" + sigmaStr + String.valueOf( entireImage ) + ", " + String.valueOf( image25D ) + ")\n" );
     ...
     * sets a Bitset object to 1 is a zero crossing is detected.
    ...
    /**
     * Generates a zero crossing mask for a 2D function sets a ModelImage to 255 if
     * a zero crossing is detected.
     * @param buffer   array in which to find zero crossing
     * @param buffer2  array which ensures that zero crossing is only counted if buffer2
     *                 value at that position is less than zero
        ...
        for ( s = 0; s < nImages && !threadStopped; s++ ) {
            ...
            for ( i = 0, idx = start; i < length && !threadStopped; i++, idx++ ) {
                ...
                if ( entireImage == true || mask.get( i ) ) {
                    ...
                    buffer2[i] = ix * ix * ix * ixxx + 3.0f * ix * ix * iy * ixxy + 3.0f * ix * iy * iy * ixyy
                            + iy * iy * iy * iyyy;
                } // if (entireImage == true || mask.get(i))
                else {
        ...
        for ( s = 0; s < nImages && !threadStopped; s++ ) {
            ...
            for ( i = start; i < start + length && !threadStopped; i++ ) {
                ...
                if ( entireImage == true || mask.get( i ) ) {
                    ...
                    destImage.set( i, nms );
                    ixxx = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxxxData );
                    ixxy = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxxyData );
                    ixyy = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxyyData );
                    iyyy = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GyyyData );
                    ixxz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxxzData );
                    ixzz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxzzData );
                    ixyz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GxyzData );
                    iyyz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GyyzData );
                    iyzz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GyzzData );
                    izzz = AlgorithmConvolver.convolve3DPt( i, srcImage.getExtents(), buffer, kExtents, GzzzData );
                    sliceBuffer2[i - start] = ix * ix * ix * ixxx + 3.0f * ix * ix * iy * ixxy
                            + 3.0f * ix * iy * iy * ixyy + iy * iy * iy * iyyy + 3.0f * ix * ix * iz * ixxz
                            + 3.0f * ix * iz * iz * ixzz + 6.0f * ix * iy * iz * ixyz + 3.0f * iy * iy * iz * iyyz
                            + 3.0f * iy * iz * iz * iyzz + iz * iz * iz * izzz;
                } // if (entireImage == true || mask.get(i))
                else {
