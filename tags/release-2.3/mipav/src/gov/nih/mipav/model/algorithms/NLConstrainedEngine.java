...
import gov.nih.mipav.view.Preferences;
 ...
 /**
  *  This is a port of the program ELSUNC for solving least squares problems
  *  where the unknowns are restricted by lower and upper bounds.
  *  Reference: Gauss-Newton Based Algorithms For Constrained Nonlinear
  *  Least Squares Problems by Per Lindstrom and Per-Ake Wedin, Institute of
  *  Information Processing, University of Umea, S-901 87 Umea, Sweden
  *  This can be downleaded from http://www.cs.umu.se/~perl/reports/alg.ps.gz
  *
  */
...
// DOUBLE EXPONENTIAL ROUTINE
            ...
            double e1, e2;
                      ...
                      try {
                          ...
                          if ((ctrl == -1) || (ctrl == 1)) {
                            ...
                            for (i = 0; i < nPts; i++) {
                              ymodel = a[3] + (1.0 - a[3])*(1 - a[0]*Math.exp(a[1]*xSeries[i])
                          ...
                          // To calculate the Jacobian numerically just set
                          // ctrlMat[0] = 0 and return
                          else if (ctrl == 2) {
                            ctrlMat[0] = 0;
                          } // else if (ctrl == 2)
                    }
    ...
    /* Below is an example used to fit y = alpha - beta*(rho**x)
       This example implements the solution of problem D of chapter 24 of
       Applied Regression Analysis, Third Edition by Norman R. Draper and
       Harry Smith */


    /*public void runTest24D() {
      // Answer is a0 = 72.4326, a1 = 28.2519, a2 = 0.5968
...
public abstract class NLConstrainedEngine {
    ...
    //variables
    // integer scalar containing the number of unknowns
    protected int	            param;
   // integer scalar containing the number of data points
    ...
    /** On completion if exitStatus >= 0 a[] contains the latest (best)
     *  estimate of the solution point */
    protected double            a[];
    protected double dyda[],     stdv;
    protected double            gues[];
    protected int               maxIterations;
    /** The method of Newton is allowed at the end of the iteration in some
        situations (large residuals).  If true, the use of second derivatives
        is permitted */
    protected   boolean           secondAllowed = true;
    /** If true, the columns of the Jacobian are scaled to have unit length.
        If false, internal scaling is not used to compute the Gauss-Newton
        search direction. */
    protected   boolean           internalScaling = false;
    // single relative precision
    private   double            srelpr;
    // pseudo rank tolerance constant used to determine the pseudo rank of
    // the Jacobian J
    protected double            tolerance;
    // relative convergence constant
    // Used to test for convergence by reduction in objective function being
    // less than relativeConvergence squared
    protected double            relativeConvergence;
    // absolute convergence constant
    // Used to test for convergence by the sum of squares being less than
    // the absoluteConvergence squared
    protected double            absoluteConvergence;
    // parameter convergence constant
    // Used to test for convergence by the relative change in a[] being less
    // than the parameterConvergence
    protected double            parameterConvergence;
    /** ctrl can have 3 different values on entry.  If ctrl = 1 or ctrl = -1,
        compute the residuals for all nPts at a certain point a and return
        the residuals = yfit - yseries in the array residuals if the functions are
        computable.  If not successful and ctrl was set to 1, set ctrl = -1.
        If not successful and ctrl was set to -1, set ctrl < -10. If ctrl = 2
        and the Jacobian is supplied analytically,  calculate the
        Jacobian = Jij = dymodi/daj.  The Jacobian is placed in matrix covarMat.
        If the Jacobian cannot be calculated, set ctrl < -10.  If you wish to
        compute the Jacobian numerically on return, set ctrl = 0. */
    private int               ctrl;
    // Wrapper for ctrl or lctrl communication with driver
    protected int ctrlMat[] = new int[1];
    // integer scalar code for assessing the bounds
    // bounds = 0 means an unconstrained problem
    // bounds = 1 means the same lower bounds for all unknowns and the same
    // upper bounds for all unknowns.  The lower and upper bound must be
    // stored in bl[0] and bu[0] respectively.
    // bounds = 2 means that the bounds must be supplied by the user in
    // bl[i] and bu[i] for i = 0,1, param - 1.
    protected int               bounds;
    protected double            bl[];
    protected double            bu[];
    // integer scalar that indicates why the return was taken.  If exitStatus
    // != -1 and exitStatus >= -10, then a[] contains the latest (best) estimate
    // of the solution point.
    // = 1000X convergence because relative predicted reduction in the objective
    //   function = (0.5 * chisquare) is less than (relativeConvergence)**2
    // =  200X convergence because sum of squares is less than
    //   (absoluteConvergence)**2
    // =   30X convergence because the relative change in a is less than
    //   parameterConvergence
    // =    4X convergence because we are computing at noise level
    // X, the last digit in the convergence code, indicates how the last steps
    // were computed
    // X = 0 no trouble (Gauss-Newton the last 3 steps)
    //   = 1 pseudoRank <> param at termination point
    //   = 2 The method of Newton was used (at least) in the last step
    //   = 3 The (last - 2) step was subspace minimization, but the last 2
    //     steps were Gauss-Newton steps
    ...
    // = -1 Invalid starting condition: nPts < param or param <= 0 or nPts <= 0
    //      or maxIterations <= 0 or relativeConvergence < 0 or
    //      relativeConvergence < 0 or absoluteConvergence < 0 or tolerance <= 0
    //      or parameterConvergence < 0 or invalid starting point on entry
    // = -2 Number of iterations has exceeded maximum allowed iterations
    // = -3 The Hessian emanating from the second order method is not
    //      positive definite
    // = -4 The algorithm would like to use second derivatives but is not
    //      allowed to do that
    // = -5 An undamped step with Newton's method is a failure
    // = -6 The latest search direction computed using subspace minimization was not
    //      a descent direction (probably caused by wrongly computed Jacobian)
    // = -7 There is only one feasible point, name a[i] = bl[i] = bu[i];
    //      i = 0,1,...,param-1
    // < -10 Termination due to user stop indicator
    private int                 exitStatus;
    // integer scalar containing the number of iterations until termination
    protected int                 iters;
    // integer scalar containing the total number of function evaluations
    // done inside this routine
    private int                 funcEval;
    // integer scalar containing the number of function evaluations caused
    // by computing Jacobians with difference methods
    private int                 jacobianEval;
    // integer scalar containing the number of function evaluations caused
    // by using second derivatives in the method of Newton
    private int                 secondEval;
    // integer scalar containing the number of function evaluations caused
    // by the linesearch algorithm
    private int                 lineEval;
    // integer scalar containing the pseudo rank of the matrix jhat at the
    // termination point a.  jhat is the Jacobian matrix with columns
    // corresponding to active constraints deleted
    private int                 pseudoRank;
    // Create wrapper for routine gndunc
    private int pseudoRankMat[] = new int[1];
    // double scalar containging the value of the objective function =
    // (0.5 * chisquare) at the termination point a
    private double              phi;
    // double scalar containing an estimate of the linear convergence
    // factor
    private double              speed;
    // 1D array of doubles of dimension nPts containing the value
    // of the residuals at the termination point a;
    protected double            residuals[];
    // 2D array of doubles of dimension nPts by max(4,param) containing
    // the main part of the covariance matrix (Jtranspose * J)inverse,
    // where J is the Jacobian matrix computed at a
    protected double            covarMat[][];
    // double working areas
    // 1D array of length param containing the solution of
    // J * dx = -residuals
    private double              dx[];
    // 1D array of length param containing the gradient of the objective
    ...
    private double              w2[];
    private double              w3[];
    // 1D array of length nPts containing the orthogonal projection of
    // residuals onto space spanned by the frist pseudoRank linear
    // independent columns of the Jacobian
    private double              v[];
    // 1D double array of length param conatiaing the diagonal elements
    // of the diagonal matrix D if internalScaling is true.  Otherwise,
    // undefined.
    private double              diag[];
    // param by param array
    // If secondAllowed if false, gmat can be used as a singly subscripted
    // array of dimension param by setting all second indices to 0.
    private double              gmat[][];
    // array of integers of dimension param containing the permutation matrix
    // E in the decomposition QTranspose * J * D * E = (R)
    //                                                 (0)
    // pivit[i] contains the index of the column that was moved into column i
    private int                 pivit[];
    // 1D integer array of length param containing a code which indicates
    // whether an unknown is active or not
    // aset[i] = 0 when a[i] is free
    //         = +1 when a[i] == bl[i]
    //         = -1 when a[i] == bu[i]
    private int                 aset[];
    // The suffices km2 and km1 in the names of the variables represent
    // time step k-2 and k-1 respectively.  These variables are updated
    // only inside the routine evreuc.
    private int rngkm2, kodkm2, rngkm1, kodkm1;
    private double betkm2, d1km2, fsqkm2, dxnkm2, alfkm2, aupkm2,
                   betkm1, d1km1, fsqkm1, dxnkm1, alfkm1, aupkm1;
    ...
    // Leading dimension of array gmat
    // mdg must be >= param if secondAllowed is true
    private int mdg;
    private int i,j,k;
    private double rootsp, bl0, bu0;
    // Number of active constraints
    private int paramAct;
    // scalar containing the predicted reduction in the objective function
    // if gn-direction is used
    private double d1sqs;
    // scalar containing the norm of the orthogonal projection of residuals
    // onto the space spanned by the columns of the Jacobian
    private double beta;
    // scalar containing the predicted reduction in the objective if
    // pseudoRank from previous step is used
    ...
    private double pk;
    // The original code introudces a beta in lsunc and another beta in
    // subroutine lineuc.  Here the beta in lineuc is called beta2
    private double beta2;
    private double pbeta;
    private int k2;
    // The orginal code introduces a alfkm1 and alfkm2 in the common block preunc
    // and another alfkm1 and alfkm2 in the subroutine lineuc.  Here the
    // alfkm1 and alfkm2 in lineuc are called alfkm1a and alfkm2a.
    ...
    */
	public void driver() {
         try {
          ...
          if ((exitStatus % 10) == 1) {
              ...
              if (!internalScaling) {
                System.out.println("A run with internalScaling = true might " +
                                   "give another solution\n");
              }
              Preferences.debug("Pseudo-rank of the Jacobian is not full\n");
              if (!internalScaling) {
                Preferences.debug("A run with internalScaling = true might " +
                                  "give another solution\n");
              }
          }
          covar();
	...
	private void lsunc() {
        ...
        if ((nPts < param) || (param <= 0) || (nPts <= 0) || (mdg < param) ||
            (maxIterations <= 0) || (tolerance <= 0.0) ||
            (relativeConvergence < 0.0) || (absoluteConvergence < 0.0) ||
            (parameterConvergence < 0.0) || ((relativeConvergence +
            absoluteConvergence + parameterConvergence) <= 0.0)) {
            exitStatus = -1;
            return;
        }
	...
	private void soliuc(double tau) {
          /**
           * @param tau double scalar containing a small positive value used
           *            to determine the pseudo rank of the Jacobian
           */
        ...
        for (i = 0; i < param; i++) {
            ...
            if (pseudoRank < (param - paramAct)) {
                k = 0;
                kk = 0;
                Preferences.debug("Not full rank: pseudoRank = " +
                                   pseudoRank + "\n");
                inds = aset[i];
                aset[i] = 0;
                newunc();
                ...
                if (k != kk) {
                    ...
                    sqrsl(work2,mr,1,scaqr,w2Part,dummy,w2Part,dykk,dummy,
                          dummy,100);
                    for (jj = 0; jj < mr; jj++) {
                ...
                Preferences.debug("SOLIUC:covarMat[0][0] = " +
                                          covarMat[0][0] + "\n");
        ...
        if (pseudoRank < (param - paramAct)) {
            ...
            if (kmax != kk) {
                ...
                sqrsl(work2,mr,1,scaqr,w2Part,w2Part,dummy,dummy,dummy,dummy,
                      10000);
            } // if (kmax != kk)
            for (jj = 0; jj < mr; jj++) {
	...
	private int triunc(double tol, int paramAct) {
           // MAKE A QR-DECOMPOSITION OF THE nPts*param MATRIX covarMat BY USING THE
           // ROUTINE SQRDC FROM LINPACK
           // I.E. DETEMINE MATRICES Q,E,R SO THAT      T
           //                                          Q *covarMat*E = (R)
           //                                                          (0)
           // WHERE  Q IS nPts*nPts ORTHOGONAL
           // E IS param*param PERMUTATION MATRIX
           // R IS param*param UPPER TRIANGULAR
	...
	private double gndunc(boolean internalScaling, int pseudoRankMat[],
	                      double work[]) {
          // COMPUTE THE SOLUTION (DX) OF THE SYSTEM
          //              T
          // R*D*E*DX = -Q *residuals
          // WHERE
          // R IS param*param UPPER TRIANGULAR
          // D IS param*param DIAGONAL MATRIX
          // E IS param*param PERMUTATION MATRIX
          // Q IS nPts*nPts ORTHOGONAL MATRIX
          // residuals IS AN nPts-VECTOR
          // THE SOLUTION WILL BE COMPUTED BY ONLY USING THE FIRST PRANK <= param
          // COLUMNS OF R
	...
	private void sqrsl(double x[][], int n, int k, double qraux[],
	...
	private void sqrdc(double x[][], int n, int p, double qraux[],
	                   int jpvt[], int job) {
	...
	private void analuc() {
        ...
        private void gnavuc() {
            ...
            final double c1 = 0.5, c2 = 0.1, c3 = 4.0,
                       c4 = 10.0, c5 = 0.016;
        ...
        private void dimsub() {
            ...
            if (!restart) {
                ...
                sqrsl(covarMat, nPts, pseudoRank, w0, w1, dummy, w1, dummy, dummy, dummy,
                             1000);
        ...
        private void presub(double work[]) {
            ...
            if ((alfkm1 < stepb) && (pgress <= pgb1*Math.sqrt(d1km1)) &&
                (pgress <= pgb2*betkm1)) {
        ...
        private void secuc() {
            ...
            for (i = 0; i < nn; i++) {
                for (j = 0; j < nn; j++) {
                    Preferences.debug(covarMat[i][j] + " ");
                }
                Preferences.debug("\n");
            }*/
        ...
        private void schdc(int nn, int jpvt[], int job) {
          /** SCHDC COMPUTES THE CHOLESKY DECOMPOSITION OF A POSITIVE DEFINITE
              ...
              covarMat
                 CONTAINS THE MATRIX WHOSE DECOMPOSITION IS TO
                 BE COMPUTED.  ONLT THE UPPER HALF OF covarMat NEED BE STORED.
                 THE LOWER PART OF THE ARRAY covarMat IS NOT REFERENCED.
             ...
             jpvt   INTEGER(nn).
                    ...
                    EACH DIAGONAL ELEMENT covarMat(K,K)
                 ...
                 THE BEGINNING OF THE ARRAY covarMat AND FINAL
                 ELEMENTS TO THE END.  BOTH INITIAL AND FINAL ELEMENTS
                 ARE FROZEN IN PLACE DURING THE COMPUTATION AND ONLY
                 FREE ELEMENTS ARE MOVED.  AT THE K-TH STAGE OF THE
                 REDUCTION, IF covarMat(K,K) IS OCCUPIED BY A FREE ELEMENT
                 IT IS INTERCHANGED WITH THE LARGEST FREE ELEMENT
                 covarMat(L,L) WITH L .GE. K.  JPVT IS NOT REFERENCED IF
                 JOB .EQ. 0.
   ...
   covarMat      covarMat CONTAINS IN ITS UPPER HALF THE CHOLESKY FACTOR
                 OF THE MATRIX covarMat AS IT HAS BEEN PERMUTED BY PIVOTING.

          JPVT   JPVT(J) CONTAINS THE INDEX OF THE DIAGONAL ELEMENT
                 OF covarMat THAT WAS MOVED INTO THE J-TH POSITION,
                 PROVIDED PIVOTING WAS REQUESTED.

          INFO   CONTAINS THE INDEX OF THE LAST POSITIVE DIAGONAL
                 ELEMENT OF THE CHOLESKY FACTOR.

      FOR POSITIVE DEFINITE MATRICES INFO = nn-1 IS THE NORMAL RETURN.
      FOR PIVOTING WITH POSITIVE SEMIDEFINITE MATRICES INFO WILL
      IN GENERAL BE LESS THAN nn-1.  HOWEVER, INFO MAY BE GREATER THAN
      THE RANK OF covarMat-1, SINCE ROUNDING ERROR CAN CAUSE AN OTHERWISE ZERO
      ELEMENT TO BE POSITIVE. INDEFINITE SYSTEMS WILL ALWAYS CAUSE
      INFO TO BE LESS THAN nn-1. */
	...
	private void stepuc() {
        ...
        private void lineuc(double gmod[], double dphize, double alplow) {
          ...
          phik = phiMat[0];
          Preferences.debug("In lineuc no. 1: phik = " + phik + " alfk = " +
                            alfk + "\n");
          k2++;
          iev = k2;
          if (ctrl == -1) {
      ...
      fsumsq(alfk,gmod,fnew,phiMat);
      phik = phiMat[0];
      Preferences.debug("In LINEUC no.2: phik = " + phik + " alfk = " +
                        alfk + "\n");
      if (ctrl < -10) {
        k2 = ctrl;
      }
      ...
      return;
        ...
        private void fsumsq(double alfk, double xnew[], double fnew[],
                            double phiMat[]) {
          // EVALUATE FUNCTION VALUES AT THE POINT a+alfk*dx .
        ...
        private void minrm(double v2[], double xmin) {
          // A SUBROUTINE WHICH FINDS THE POINT a WHERE THE FUNCTION
          // P(X)= 0.5*II residuals+V*X+V2*X**2 II**2  IS MINIMIZED.
          // residuals, v AND v2 BELONG TO R[nPts] AND X IS A SCALAR.
          // THE VALUES OF residuals, v AND v2 ARE BASED ON TW0 FUNCTION
          ...
          // HOWEVER,THE MINIMUM POINT X MUST LIE IN THE INTERVAL (ALFMIN,ALFMAX) .
          // PX IS SET TO P(X) AT THE MINIMUM POINT
          // WHERE P(X) IS THE POLYNOMIAL ABOVE.
          // beta2 IS SET TO THE OTHER MINIMIZER OF P(X)  (IF TWO ARE DETERMINED)
          // OTHERWISE beta2 = X.
          // pbeta = P(beta2)
          ...
          minrm1(v2);
          pprim = pol3(xmin);
          pbiss = 6.0*xmin*xmin*v2norm*v2norm + 6.0*xmin*scv1v2 + 2.0*scv0v2 +
                  v1norm*v1norm;
          h0 = Math.abs(pprim/pbiss);
          dm  = Math.abs(6.0*scv1v2 + 12.0*xmin*v2norm*v2norm) +
                24.0*h0*v2norm*v2norm;
          ...
          do {
            pprim=pol3(x0);
            pbiss = 6.0*x0*x0*v2norm*v2norm + 6.0*x0*scv1v2 + 2.0*scv0v2 +
                    v1norm*v1norm;
            d = -pprim/pbiss;
            alfkp1 = x0 + d;
        ...
        private void oner() {
          ...
          a3rd = 1.0/3.0;
          t = s1*Math.pow(Math.abs(arg1),a3rd) +
              s2*Math.pow(Math.abs(arg2),a3rd);
          alfkp1 = t - a1div3;
          return;
        }
        ...
        private void choose(double xmin, double v2[]) {
          // X1Minrm, X2Minrm AND X3Minrm ARE THREE REAL ROOTS OF A 3:RD DEGREE POLYNOMIAL
          // WHICH TENDS TO MINUS INFINITY WHEN X TENDS TO MINUS INFINITY.
          // CHOOSE ONE OF THE OUTER ROOTS FOR alfkp1.
          // beta2 = THE OTHER OUTER ONE. pbeta = THE VALUE OF THE CORRESPONDING
          // 4:TH DEGREE POLYNOMIAL AT beta2.
        ...
        private void reduc(double pk, double eta,
                           double gmod[], double fnew[]) {
        ...
        private void minrn(double phikm2, double pmax) {
          // PROVIDED THE POINTS alfk, alfkm2a AND alfkm1a ARE NOT TOO CLOSE, THE QUADRATIC
          // PASSING THROUGH (alfk,phik), (alfkm2a,phikm2) AND (alfkm1a,phikm1) IS DETERMINED.
          // alfkp1 = THE MINIMUM POINT OF THIS QUADRATIC.
          // pk = THE VALUE OF THE QUADRATIC AT alfkp1
          ...
          alfkp1 = alfk;
          pk = phik;
          if ((Math.abs(alfkm2a-alfk) < eps) ||
              (Math.abs(alfkm1a-alfk) < eps) ||
        ...
        private void gauc(double gmod[], double dphize, double tau,
                          double pmax) {
          // THIS IS A ROUTINE FOR UNCONSTRAINED LEAST SQUARES PROBLEMS THAT HALVES
          // THE VALUE OF alfk UNTIL A GOLDSTEIN-ARMIJO CONDITION IS SATISFIED OR UNTIL
        ...
        private void chder(double dphize) {
          ...
          if ((Math.abs(dphifo-dphize) > maxdif) &&
              (Math.abs(dphice-dphize) > maxdif)) {
            errorStatus = -1;
          }
          return;
        }
	...
	// fitToFunction communicates with 3 protected variables
        // param, nPts, and ctrlMat
        // ctrlMat is used as a wrapper for ctrl or lctrl.
        // Evaluates the residuals or the Jacobian at a certain a[]
	public abstract void fitToFunction(double a[], double residuals[],
                                           double covarMat[][]);
