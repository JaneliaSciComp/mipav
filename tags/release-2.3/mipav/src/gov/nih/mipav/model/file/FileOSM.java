...
public class FileOSM extends FileBase {
  ...
  /** Based on 1996 files washu.h, write_dataset.c, and read_dataset.c from the
   *  Biomedical Computer Laboratory of Washington University in Missouri.
   *  This file format is a modification of the MRC file format and contains
   *  a similar 1024 byte header.  The header is called the WASHU header and
   *  the dataset structure is called the OSM dataset structure.
   */
    ...
    */
    public ModelImage readImage() throws IOException {
        ...
        try {
            progressBar = new ViewJProgressBar(fileName, "Reading OSM file...",
            ...
            if (magicNumber == 1234) {
              Preferences.debug(
              "Successful read of magic number 1234 at location 152\n");
              Preferences.debug("File is little endian\n");
            }
            else if (magicNumber == -771489792) {
              endianess = FileBase.BIG_ENDIAN;
              Preferences.debug(
              "Successful read of byte swabbed magic number 1234 at location 152\n");
              Preferences.debug("File is big endian\n");
            }
            else {
              MipavUtil.displayError("Magic number at location 152 is an illegal "
                                     + magicNumber);
    ...
    */
    public void writeImage(ModelImage image, FileWriteOptions options) throws IOException {
        ...
        progressBar = new ViewJProgressBar(fileName, "Writing OSM file...",
                                           0, 100, true, null, null);
