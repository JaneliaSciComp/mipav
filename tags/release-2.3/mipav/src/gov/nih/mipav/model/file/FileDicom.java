 ...
 *   of header information marked by tags, with the pixel data as the last tag.
 *   Each tag has a length field that contains the length of the tag, so it is
 *   possible to skip over tags that are unrecognized.  This class is entirely
 *   based on DICOM version 3.0 published by ACR/NEMA and so tests
 *   for all the tags in the Data Dictionary (DICOM v. 3.0 Part 6).  It stores
 *   these in a Hashtable found in FileInfoDicom.
 *
 *   <p>The Hashtable is based upon the default DICOM dictionary which contains
 *   all possible standard DICOM tags.  It also contains many private tags that
 *   are commented out.  If the user wishes a specific private tag to be
 *   recognized by this program, he or she should edit the dictionary file.
 *   The tag will then be displayed as any other standard tag would be displayed.
 *   Otherwise, all tags are read in, but if their value representation is
 *   unrecognized (only the case with tags not defined in dictionary file) their
 *   value is stored as a string.  When FileInfoDicom displays the tag
 *   information, it shows the name of the tag as private and the value as the
 *   string.  The string may contain valid data or it may contain junk.  There
 *   is no way of knowing how to properly read in a private tag without a valid
 *   value representation (VR).  So if the user wishes to know private tag
 *   information, he or she should specify the proper VR in the dictionary
 *   file and be sure that their file conforms to that VR.
...
public class FileDicom extends FileDicomBase {
    /** used in reading; if the preferences are at this level or higher than
     * dicomDebugOutputValue get sent to the debug panel.
     * This is used strictly in readHeader, as the comparison statements are
     * there for speed enhancement (as opposed to relying on the settings in
     * Preferences.debug(String, int)).  Only used for informational statements
     * and not used in failure statements, where the original values are
     * left as they were (around level 3).
     */
     ...
     *  field) up to and including the last File Meta Element of
     *  the Group 2 File Meta Information.
     *  <p>
     ...
     *  'fragments.'
     * don't know quite why, or if pieces should be kept together.
     * If in fragments, the image data may span several slices,
     * called a 'frame.'
     */
    ...
    /** if the file is <i>quiet</i> no user-interaction is performed.
     *  Useful for determining whether or not to display the
     *  MipavUtil.displayError() is to be called.  This allows the option of
     * leaving the user-interaction and notification of an error to occur here
     * or to be handled somewhere else (as in, when an IOException is thrown
     * but we'd prefer to notify the user once, rather than for each
     * exception.) based on programming preferences or user-debug preference
     * settings.
     */
    ...
    /**
     *   DICOM reader/writer constructor.  Creates the access files and ensures
     * that the files are opened for read-write access; it will fall back to
     * read-only if read-write access to the image file is not granted.
     * The file info gets the DICOM dictionary set, as well as the endianess
     * property is set.  The image itself is not read.
     *
    ...
    public FileDicom(String fName, String fDir) throws IOException {
        ...
        try {
            ...
            rawFile = new FileRaw(fileInfo.getFileName(), fileInfo.getFileDirectory(),
                                  fileInfo, false, FileBase.READ);
        }
    ...
    public FileDicom(File ifile) throws IOException {
        ...
        try {
            ...
            fileInfo.resetDictionary();
            fileInfo.setEndianess(LITTLE_ENDIAN);
            rawFile = new FileRaw(fileInfo.getFileName(),
                                fileInfo.getFileDirectory(),
                                fileInfo, false, FileBase.READ);
    ...
    /**
     *   Prepares this class for cleanup.  Calls the <code>finalize</code>
     * method for existing elements, closes any open files and sets other
     * elements to <code>null</code>.  Exceptions which occur while this
     * method runs (for instance, the possibility of getting a
     * <code>IOException</code> when closing the image file) is ignored
     * quietly.
     */
     ...
     */
    public final FileInfoBase getFileInfo() { return fileInfo;}
    ...
    */
    ...
    /**
     *  Accessor that sets the file name and allocates new FileInfo, File and
     * RandomAccess file objects based on the new image file based on the
     * new filename and the old directory.  This method
     * sets the filename property of the FileDicom, recreates a raw file for
     * random access file to read the image file pointed to by the filename
     * (the file is opened for reading only; it cannot be written to).
     * A new file info is created with this file info and the old DICOM
     * dictionary is used.  Finally, the endianess is set to be little-endian.
     *
     ...
     *  Accessor that sets the file name and allocates new FileInfo, File and
     * RandomAccess file objects based on the new image file based on the
     * new filename and the new directory.  This method
     * sets the filename property of the FileDicom, recreates a raw file for
     * random access file to read the image file pointed to by the filename
     * (the file is opened for reading only; it cannot be written to).
     * A new file info is created with this file info and the old DICOM
    ...
    /**
     *  Reads a DICOM image file and stores the data into the given float
     * buffer.  This method reads the image header (@see readHeader())
     * then sets various fields within the FileInfo which are relevant to
     * correctly interpreting the image.  This list includes:
     * <ul>
    ...
    public void readImage(float buffer[], int imageType, int imageNo) throws IOException {
        ...
        if (!encapsulated) {
            try {// rafile (type RandomAccessFile) for header, rawfile (type FileRaw) for image data.
                rawFile.setImageFile(fileInfo.getFileName(),
                                    fileInfo.getFileDirectory(),
                                    fileInfo, FileBase.READ);
            ...
            if (jpegData != null) {
                try {
                    ...
                    MipavUtil.displayError( "Image is smaller than expected.  "+
                                            "Showing as much as available.");
                }
    ...
    /**
     *  Reads a DICOM image file and stores the data into the given short
     * buffer.  This method reads the image header (@see readHeader())
     * then sets various fields within the FileInfo which are relevant to
     * correctly interpreting the image.  This list includes:
     * <ul>
     ...
     * data.  This method is faster than the float buffer version of this
     * method since not as much type-conversion is needed.
     *
     *  @param   buffer          2D buffer used for temporary storage of data
     *  @param   imageType       The type of image (i.e. SHORT, BYTE, ...)
     *  @param   imageNo         For multiFrame images, imageNo >=1. For single slice image imageNo = 0.
        ...
        if (!encapsulated) {
            try {// rafile (type RandomAccessFile) for header, rawfile (type FileRaw) for image data.
                ...
                if (ModelImage.isColorImage(imageType)) {
                    ...
                    rawFile.readImage(buffer, fileInfo.offset + imageNo*buffer.length*fileInfo.bytesPerPixel, imageType);
                    rawFile.raFile.close();
                    rawFile.raFile = null;
                }
            }
    ...
    /**
     *  Reads in all the tags available in the file and stores them in the
     * Hashtable in FileInfoDicom.  This method handles the various tags that
     * are present at the beginning of a DICOM image file.  It also sets the
     * important File Info variables based on what it finds.
     * <p>
     * The method will return with a failure code if it finds the tag
     * &quot;0000,0000&quot; or it mis-reads the header and starts reading from
     * an odd byte.
     * <p>
     * As the reader runs through the tags in the header, it reads them based
     * on the type.  There are 7 types:
     * <ul>
     *  <li>typeString</li>
     *  <li>typeShort</li>
     *  <li>typeInt</li>
     ...
     * </ul>
     *<p>
     * Any special handling based on type occurs for each tag
     * (@see FileInfoDicom), then it is added to the DICOM tags table.  Each
     * tag is checked against a small list of individual tags, as some tags
     * have an effect on the way the following tags are interpreted.
     * <p>
     ...
     * </ul>
     * Display type changes the modality; image length is then also
     * recalculated.
     *
    ...
    public boolean readHeader(boolean loadTagBuffer) throws IOException {
        ...
        try {
            ...
            else {Preferences.debug("Out of memory in FileDicom.readHeader");}
        ...
        while (flag == true) {
            ...
            try {
                if (type.equals("typeString") ) {
                    ...
                    if ( !name.equals("7FE0,0010") ) {
                        ...
                        if (tagVM == 1) {
                            //if ( debug >= dicomDebugOutputValue)
                            //    Preferences.debug( ((FileDicomTag)(fileInfo.getEntry(name))).getName() +
                            //        "\t\t(" + name + ");\t (byte) value = " + ((Byte)(data)).byteValue() +
                            //        " element length = 2" + "\n", 2);
                        }
                    }
                }
                    ...
                    if (tagVM == 1) {
                        if ( debug )
                            Preferences.debug( ((FileDicomTag)(fileInfo.getEntry(name))).getName() +
                                "\t\t(" + name + ");\t (short) value = " + ((Short)(data)).shortValue() +
                                " element length = 2" + "\n");
                    }
                }
                    ...
                    if (tagVM == 1) {
                        if ( debug  )
                            Preferences.debug( ((FileDicomTag)(fileInfo.getEntry(name))).getName() +
                                "\t\t(" + name + ");\t (int) value = " + ((Integer)(data)).intValue() +
                                " element length = 4" + "\n");
                    }
                }
                    ...
                    if (tagVM == 1) {
                        if ( debug  )
                            Preferences.debug( ((FileDicomTag)(fileInfo.getEntry(name))).getName() +
                                "\t\t(" + name + ");\t (float) value = " + ((Float)(data)).floatValue() +
                                " element length = 4" + "\n");
                    }
                }
                    ...
                    if (tagVM == 1) {
                        if ( debug )
                            Preferences.debug( ((FileDicomTag)(fileInfo.getEntry(name))).getName() +
                                "\t\t(" + name + ");\t (double) value = " + ((Double)(data)).doubleValue() +
                                " element length = 8" + "\n");
                    }
                }
                // (type == "typeUnknown" && elementLength == -1) Implicit sequence tag if not in DICOM dictionary.
                    ...
                    //System.err.print( "SEQUENCE DONE: Sequence Tags: (" + name + "); length = " + Integer.toString(len, 0x10) + "\n");

                    try {
                        fileInfo.setValue(new FileDicomKey(name), sq, elementLength);
                    }
                    catch (NullPointerException e) {
                        System.err.println( "Null pointer exception while " +
                                            "setting value.  Trying to put "+
                                            "new tag.");
            ...
            if (name.equals("0002,0000")) { // length of the transfer syntax group
                if (data != null) metaGroupLength = ((Integer)(data)).intValue();
                if ( debug )
                ...
                // red channel LUT
                try {getColorPallete(new FileDicomKey(name));}
                ...
                else fileInfo.displayType = fileInfo.getDataType();
                ...
                try {
                    if (fileInfo.getEntry(name) != null)
                        ...
                        if (debug) {
                            Preferences.debug("Group = " + groupWord + " element = " + elementWord +
                                              " Type unknown" + "; value = " + strValue +
                                          "; element length = " + elementLength + "\n");
                        }
                    ...
                    if (!isQuiet()) {
                        MipavUtil.displayError("Out of memory error while reading \""+
                            fileName + "\".\nThis may not be a DICOM image.");
                        Preferences.debug("Out of memory storing unknown tags in FileDicom.readHeader\n");
                    }
                    else {Preferences.debug("Out of memory storing unknown tags in FileDicom.readHeader\n");}
        ...
        if( (photometricInterp.equals("MONOCHROME1") ||  photometricInterp.equals("MONOCHROME2")) &&
             fileInfo.pixelRepresentation == 0 &&
        ...
        if (fileInfo.getModality() == fileInfo.POSITRON_EMISSION_TOMOGRAPHY) {
            fileInfo.displayType = ModelStorageBase.FLOAT;
            // a bit of a hack - indicates Model image should be reallocated to float for PET image
            // the data is stored as 2 bytes (short) but is "normalized" using the slope parameter
            // required for PET images (intercept always 0 for PET).
        }
        if ((fileInfo.getDataType() == ModelStorageBase.UBYTE   ||
             fileInfo.getDataType() == ModelStorageBase.USHORT) &&
             fileInfo.getRescaleIntercept() < 0) { // if image originally was unsigned and rescale makes a negative, change
        ...
        if (loadTagBuffer == true && raFile != null) raFile.close();
        hasHeaderBeenRead = true;
        return true;
    ...
    /**
     * Looks for the DICM _tag_ in the File header. If present, the image
     * is DICOM 3.0 format.
    ...
    public boolean isDICOM() throws IOException
    {
        ...
        if (raFile.length() <= ID_OFFSET)return false;
     ...
     * This method handles the various tags that
     * are present at the beginning of a DICOM image file.  It also sets the
     * important File Info variables based on what it finds.
     * <p>
     * The method will return with a failure code if it finds the tag
    ...
    public boolean readParserHeader() throws IOException {
        ...
        /**
         * Converts the integer values of the group word and element word into
         * a string that is the hexadecimal representation of group word
         * and element word, separated by a comma.
         ...
         */
        private final String convertGroupElement(int groupWord, int elementWord) {
            String first, second;
         ...
         *   encodings that are detailed in the DICOM standard.  There is usually
         *   an "item" tag, then a dataset encoded exactly like other tags, then a
         *   tag indicating the end of the sequence.
         *   <P>
         ...
         *   @see DicomSQ
         */
        private final Object getSequence(boolean endianess, int seqLength) throws IOException {
            ...
            try {
                if (seqLength == UNDEFINED_LENGTH || seqLength == ILLEGAL_LENGTH) {
                    ...
                    while ((getFilePointer()-seqStart) < seqLength) {
                        ...
                        if ((getFilePointer()-seqStart) < seqLength )
                        {  //  '<=' or just '<'??
                            inSQ = true; // don't add the element length to the location
                            Preferences.debug("[FileDicom.getSequence():]"+Integer.toString(groupWord, 0x10) + "-"+Integer.toString(elementWord, 0x10)+" -- ");
                            getNextElement(endianess);// skipping the tag-length???
...
//                            System.err.println("pulled out next item of seq.  nameSQ: " +nameSQ);
        ...
        /** Reads a length of the data and deposits it into a single Short or
         *  an array of Short as needed by the tag's VM.
         * @return Object
         * @param vm value multiplicity of the DICOM tag data.  VM does not
         *              represent how many to find.
         * @param length number of bytes to read out of data stream; the length
         *              is not used.
         * @param endianess byte order indicator; here <code>true</code>
         *              indicates big-endian and <code>false</code> indicates
         *              little-endian.
         */
        private Object getByte(int vm, int length, boolean endianess)
            throws IOException
        {
         ...
         *              little-endian.
         */
        private Object getShort(int vm, int length, boolean endianess)
            throws IOException
        {
            int len = (elementLength == UNDEFINED_LENGTH)? 0 : elementLength;
            int i = 0;
        ...
        /** Reads a length of the data and deposits it into a single Integer or
         *  an array of Integer as needed by the tag's VM.
         * @return Object
         * @param vm value multiplicity of the DICOM tag data.  VM does not
         *              represent how many to find.
         * @param length number of bytes to read out of data stream; the length
         ...
         *              little-endian.
         */
        private final Object getInteger(int vm, int length, boolean endianess)
            throws IOException
        {
            int len = (elementLength == UNDEFINED_LENGTH)? 0 : elementLength;
            int i = 0;
        ...
        /** Reads a length of the data and deposits it into a single Float or
         *  an array of Float as needed by the tag's VM.
         * @return Object
         * @param vm value multiplicity of the DICOM tag data.  VM does not
         *              represent how many to find.
         * @param length number of bytes to read out of data stream; the length
         ...
         *              little-endian.
         */
        private final Object getFloat(int vm, int length, boolean endianess)
            throws IOException
        {
            int len = (elementLength == UNDEFINED_LENGTH)? 0 : elementLength;
            int i = 0;
        ...
        /** Reads a length of the data and deposits it into a single Double or
         *  an array of Double as needed by the tag's VM.
         * @return Object
         * @param vm value multiplicity of the DICOM tag data.  VM does not
         *              represent how many to find.
         * @param length number of bytes to read out of data stream; the length
         ...
         *              little-endian.
         */
        private final Object getDouble(int vm, int length, boolean endianess)
            throws IOException
        {
            int len = (elementLength == UNDEFINED_LENGTH)? 0 : elementLength;
            int i = 0;
        ...
        /**
         * Reads a set of DICOM tags from a DICOM sequence item,
         * ending with the data-element end tag <code>FFFE,E00D</code>.
         * This list of tags in a DICOM sequence item and places them into
         * a hashtable held within a FileDicomItem.
         *
         ...
         * @see                 FileDicomItem
         */
        private Object getDataSet(int itemLength, boolean endianess)
            throws IOException
        {
            FileDicomItem item = new FileDicomItem();
            ...
            while (!nameSQ.equals("FFFE,E00D")  &&
                   ((getFilePointer() - startfptr) < itemLength) )
            {
                // The following is almost exactly the same as the code in readHeader.  The
                // main difference is the information is stored in a hashtable in DicomItem
        ...
        /**
         * Increments the location, then reads the elementWord, groupWord,
         * and elementLength.  It also tests for an end
         * of file and resets the elementWord if it encounters one.
         * @param bigEndian <code>true</code> indicates big endian byte order,
         *                  <code>false</code> indicates little endian.
         */
        private final void getNextElement(boolean bigEndian) throws IOException {

            groupWord     = getUnsignedShort(bigEndian);
            elementWord   = getUnsignedShort(bigEndian);
            //Preferences.debug("(just found: )"+Integer.toString(groupWord, 0x10) + ":"+Integer.toString(elementWord, 0x10)+" - " );
            //System.err.print("( just found: ) "+
            //      Integer.toString(groupWord, 0x10) + ":"+Integer.toString(elementWord, 0x10)+
            //      " - ");

            if (fileInfo.vr_type == fileInfo.EXPLICIT ) {
                /*  explicit tags carry an extra 4 bytes after the tag (group,
                 *  element) information to describe the type of tag.  the
                 *   element dictionary describes this info, so we skip past it
                 *   here. (apr 2004)
                 */
                    ...
                    tagname.equals("FFFE,EEEE"))   // reserved
                {
                    ...
                    elementLength = getLength(bigEndian, byteBuffer4[0], byteBuffer4[1],
                                                         byteBuffer4[2], byteBuffer4[3]);
        ...
        /**
         * Reads in four integers, then tests for implicit Value
         * Representation(VR) or explicit VR. If explicit, it
         * finds out what the VR is and stores it.
         * <p>
         ...
         */
        private final int getLength(boolean endianess,
            ...
            throws IOException
        {
            ...
            if (fileInfo.vr_type == fileInfo.IMPLICIT || groupWord == 2) {
                if (fileInfo.containsDICM) {
                    ...
                    else implicit = true; //transfer syntax has been read, implicit set
                }
                else {
                    // at this point transfer syntax not read; we know endianess
                    // is little endian but vr may be explicit
            ...
            // explicit VR with 32-bit length
            else if ((b1==79 && b2==66) || (b1==79 && b2==87) || (b1==83 && b2==81) || (b1==85 && b2==78)) {
                ...
                if (b1==83 && b2==81) {  // 'SQ'
                    ...
                    if (byteBuffer4[0] == 255 && byteBuffer4[1] == 255 &&
                        byteBuffer4[2] == 255 && byteBuffer4[3] == 255)
                    {
        ...
        /**
         *   Gets private tags or other tags where the type is unknown;
         *   does not change the data, so it may be written out correctly.
         *   @return  A Byte array of length elementLength with the
         *          data stored in it.
         */
        private final Object readUnknownData() throws IOException {
            byte[] bytesValue;
            Byte[] bytesV;
        ...
        private void writeHeader(RandomAccessFile outputFile, FileInfoDicom fileInfo) throws IOException {
            ...
            if (fileInfo.containsDICM) {
                ...
                for (int k=0; k<skip.length; k++) skip[k] = (byte)0;
            ...
            for (int i=0; i<dicomTags.length; i++) {
                ...
                // Not sure if I need this - efilm adds it - also efilm has tag, 0000 lengths (meta lengths)for tag groups (02, 08, ....)
                //if (gr==2 && fileInfo.getVRType() == fileInfo.IMPLICIT){
                //    fileInfo.setVRType(fileInfo.EXPLICIT);
                //}
                //else if (gr > 2){
                //    fileInfo.setVRType(fileInfo.IMPLICIT);
                //}
                ...
                if (fileInfo.vr_type == fileInfo.EXPLICIT && vr != null) {
                    ...
                    else writeShort((short)length, endianess); // write length as a short
                }
                else { // IMPLICIT VR
                    try {
                        if (vr.equals("SQ")) {
...
//                Preferences.debug( "this is "+dicomTags[i] + "\n", 2);
            ...
            int imageLength = image.getSliceSize() *
                            ((Short)fileInfo.getValue("0028,0100")).shortValue()/8 *   // bits per pixel
        ...
        /**
         * Writes out a sequence tag and its data.  The routine writes the
         * Sequence as undefined-length and each of the items as undefined
         * length.  The appropriate write-methods are used to output the
         * various tags to the file.
         *  @param outputFile   File to write to.
         *  @param vr_type      VR type, explicit or implicit
         *  @param sq           Sequence to write out
         *  @param endianess    Big is <code>true</code> or <code>false</code>
         *                      for little endian byte-order.
         *  @throws IOException  if write fails
         */
        private void writeSequence( RandomAccessFile outputFile,
                                    boolean vr_type,
                                    FileDicomSQ sq,
                                    boolean endianess)
            throws IOException
        {
            ...
            for (int i=0; i<sq.getSequenceLength(); i++) {
                ...
                for (int j=0; j<dataSet.length; j++) {
                    ...
                    //System.out.println("adadadfad length = " + length + " group = " + entry.getGroup() + " element = " + entry.getElement());
                    try {
                        if (vr_type == FileInfoDicom.EXPLICIT && vr != null)
                            type = FileDicomTag.getType(vr);
                        else type = entry.getType();
                    }
         ...
         *	Notify any users opening this file later that it is different from the
         *	original machine which created the image and what changed it.
        ...
        private void stampSecondaryCapture(FileInfoDicom fileInfo) {
            ...
            // mess with tags outside of the fileInfo structure (fileInfo doens't handle dicom formatting), and we want the tags to figure things out
         ...
         * @exception if it cannot return the RGB image data, it'll just throw it.
         */
            ...
            if (v.size() > 1) {
                ...
                for (Enumeration en = v.elements(); en.hasMoreElements(); ) {
                    if (lossy == true) {
                        ...
                        FileDicomJPEG fileReader = new FileDicomJPEG((byte[])en.nextElement(), fileInfo.getExtents()[0], fileInfo.getExtents()[1]);
                ...
                if (lossy == true) {
                    ...
                    FileDicomJPEG fileReader = new FileDicomJPEG((byte[])v.elementAt(0), fileInfo.getExtents()[0], fileInfo.getExtents()[1]);
                    inSQ = false;
         ...
         *   Cuts out JPEG encapsulated data and stores a temp JPEG image
         *   to the temp directory.  Calls the MediaTracker utility to decode
         *   the JPEG image.
        ...
        /**
         * Reads a pallete (look-up table) out of the given DICOM key and
         * interprets the data based on the data-size specification tag.  The
         * DICOM key given defines for which colour table (either red, blue or
         * green) the pallete is to be active.  Only DICOM keys
         * <tt>0028,1201</tt>, <tt>0028,1202</tt> and <tt>0028,1203</tt> are
         * accepted for use in this method and all others cause an exception to
         * be thrown.  If the DICOM color pallete size tag exists in the
         * dictionary and is correct, then the data is read out of the color
         * pallete tag as-specified; if the size in the color pallete tag is
         * invalid for DICOM standard v3, the data is assumed to be 16 bits
         * wide; if the size-specifier tag does not exist, the data in this tag
         * is ignored.  Parsing one particular key does not verify that the
         * other colour channel pallete keys exist.
         * <p>
         * While this method ensures that the tag in the dictionary is set
         * {@link FileInfoDicom#setValue(String, Object, int)}, it does not
         * actually load the ModelLUT {@link ModelLUT}
         *
         * @param palleteKey    The DICOM key which contains some pallete or LUT information.
         * @throws IllegalArgumentException When a DICOM key is provided that
         *                                  is not related to the pallete
         *                                  information; that is, one that is
         *                                  not <tt>0028,1201</tt>,
         *                                  <tt>0028,1202</tt>, or
         *                                  <tt>0028,1203</tt>.
         ...
         */
        private void getColorPallete(FileDicomKey palleteKey)
            throws  IllegalArgumentException, IOException
        {
            if (!palleteKey.equals("0028,1201") &&
                !palleteKey.equals("0028,1202") &&
                !palleteKey.equals("0028,1203"))
            {
            ...
            try {
                ...
                String specKey = palleteKey.getGroup()+","+ Integer.toString(Integer.parseInt(palleteKey.getElement()) - 100);
                ...
                MipavUtil.displayError("This image carries " +
                        t.getName()+
                        ", but specifies it incorrectly.\n"+
                        "The channel will be ignored.");
                Preferences.debug("FileDicom: Error Creating " +
                        t.getName()+
                        "("+palleteKey.getKey()+")"+
                        "; it will be ignored.\n");
            }
         ...
         * <p>
         * The idea is that
         * for all output messages, there should be the option
         * to not bother the user with the message.  Ie., if an
         * exception is thrown, and we normally tell the user that
         * an error occurred, a calling class can set this option
         * so that the calling class can handle (either loudly or
         * quietly, as needed) the error with its own message.
         * this could be upgraded to call with quiet to prevent
         * user-queries from interrupting an automatic process.
         * But that is in the future.
         * <p>
         * Note: In the future, this method and variable is to be
         * moved to FileBase.
         * @param q     whether this class should consider itself
         *              quiet, and by internal inspection, not notify
         *              the user.  <code>True</code> is to consider
         *              itself to not notify the user.  <code>False</code>
         *              is to notify the user, and is the default behaviour.
         */
        public final void setQuiet(boolean q) {quiet = q;}
         ...
         * Note: In the future, this method and variable is to be
         * moved to FileBase.
         * @return whether this class should consider itself
         *         quiet, and by internal inspection, not notify
         *         the user.  <code>True</code> is to consider
         *         itself to not notify the user.  <code>False</code>
         *         is to notify the user, and is the default behaviour.
         */
        public final boolean isQuiet() {return quiet;}


        /** returns the ModelLUT, if there, which might be specified by
         *  the <code>0028,1201</code>, <code>0028,1202</code>,
