...
/**
 *  Calculates the gradient magnitude of an image at a scale
 *  defined by the user.
 *
...
public class AlgorithmGradientMagnitude extends AlgorithmBase {
    ...
    *   false only process the image over the mask areas.
    */
    ...
    *   @param maskFlag  Flag, if true, indicates that the gradient magnitude will be
    ...
    */
    public AlgorithmGradientMagnitude(ModelImage destImg, ModelImage srcImg, float sigmas[],
    ...
    *                   should be processed independently. 2D images disregard this flag.
    */
    public AlgorithmGradientMagnitude(ModelImage srcImg, float sigmas[],
                            boolean maskFlag, boolean img25D) {
       super(null, srcImg);
    ...
    private void constructLog() {
        ...
        for ( int i = 0; i < sigmas.length; i++){
         ...
         if (srcImage.isColorImage() == true) {
             historyString = new String( "GradientMagnitude(" + sigmaStr  +
    ...
    */
    ...
    /**
    *   Calculates the gradient magnitude of a 2D image and returns
    ...
    *   @return Buffer with the gradient magnitude of the image.
    */
    public float[] calcInBuffer2DUnnormalized(float[] buffer, int[] extents, boolean color, float[] xDirs, float[] yDirs) {
        makeKernels2D();
    ...
    *   @param nImages      number of images on which to calculate the gradient magnitude.
    *                       If 2D image then nImage = 1.
    ...
    *   @param yDirections
    */
    private float[] calcInPlace2DBuffer(int nImages, float[] buffer, int[] extents, boolean color, float[] xDirections, float[] yDirections){
            ...
            if (color == true) {
                for ( i = 0; i < length && !threadStopped; i+=4){
                    ...
                    if ( entireImage || mask.get(i/4)) {
                        ...
                        if (red) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(i+1, extents,
                                                                   buffer, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(i+1, extents,
                                                                   buffer, kExtents, Gy2Data);
                        ...
                        if (green) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(i+2, extents,
                                                                   buffer, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(i+2, extents,
                                                                   buffer, kExtents, Gy2Data);
                        ...
                        if (blue) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(i+3, extents,
                                                                   buffer, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(i+3, extents,
                                                                   buffer, kExtents, Gy2Data);
                ...
                for ( i = 0; i < length && !threadStopped; i++){
                    ...
                    if (entireImage || mask.get(i) ) {
                        //System.out.println("buffer length = " + buffer.length + " Gx2Data length = " + Gx2Data.length);
                        //System.out.println("buffer length = " + "buffer length = " + " Gx2Data length = " + "buffer length = " + "buffer length = ");
                        //System.out.println("buffer length = " + "buffer length = " + " Gx2Data length = " + "buffer length = " + "buffer length = ");
    ...
    *   @param yDirections
    */
    private float[] calcInPlace2DBufferUnnormalized(int nImages, float[] buffer, int[] extents, boolean color, float[] xDirections, float[] yDirections){
            ...
            if (color == true) {
                ...
                for ( i = 0; i < length && !threadStopped; i++){
                    ...
                    if (entireImage == true || mask.get(i) ) {
                        ix = AlgorithmConvolver.convolve2DPt(i, extents,
                                                            buffer, kExtents, Gx2Data);
                        iy = AlgorithmConvolver.convolve2DPt(i, extents,
                                                            buffer, kExtents, Gy2Data);
                        resultBuffer[start+i] = (float)Math.sqrt(ix*ix + iy*iy);
                        if (xDirections != null) {
                            if (resultBuffer[start+i] == 0) {
    ...
    /**
    *   Calculates the gradient magnitude image and replaces
    *                   the source image with the new image.
    */
    private void calcInPlace3D(){
        ...
        if (color == true) {
            for ( i = 0; i < length && !threadStopped; i+=4){
                ...
                if ((offsetZ >= 0) && (offsetZ + kExtents[2] - 1 < zDim) ){
                    if ( entireImage == true || mask.get(i/4)) {
                        ...
                        if (red) {
                            ix = AlgorithmConvolver.convolve3DRGBPt(i+1, srcImage.getExtents(),
                                                                    buffer, kExtents, GxData);
                            iy = AlgorithmConvolver.convolve3DRGBPt(i+1, srcImage.getExtents(),
                                                                    buffer, kExtents, GyData);
                            iz = AlgorithmConvolver.convolve3DRGBPt(i+1, srcImage.getExtents(),
                                                                    buffer, kExtents, GzData);

                            resultBuffer[i+1] = (float)Math.sqrt(ix*ix + iy*iy + iz*iz);
                        }
                        else {resultBuffer[i+1] = buffer[i+1];}
                        if (green) {
                            ix = AlgorithmConvolver.convolve3DRGBPt(i+2, srcImage.getExtents(),
                                                                    buffer, kExtents, GxData);
                            iy = AlgorithmConvolver.convolve3DRGBPt(i+2, srcImage.getExtents(),
                                                                    buffer, kExtents, GyData);
                            iz = AlgorithmConvolver.convolve3DRGBPt(i+2, srcImage.getExtents(),
                                                                    buffer, kExtents, GzData);
                        ...
                        if (blue) {
                            ix = AlgorithmConvolver.convolve3DRGBPt(i+3, srcImage.getExtents(),
                                                                    buffer, kExtents, GxData);
                            iy = AlgorithmConvolver.convolve3DRGBPt(i+3, srcImage.getExtents(),
                                                                    buffer, kExtents, GyData);
                            iz = AlgorithmConvolver.convolve3DRGBPt(i+3, srcImage.getExtents(),
                                                                    buffer, kExtents, GzData);
                    ...
                    if (entireImage == true || mask.get(i/4)) {
                        ...
                        resultBuffer[i]  = buffer[i];
                        if (red) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(j+1, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(j+1, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gy2Data);
                            resultBuffer[i+1] = (float)Math.sqrt(ix*ix + iy*iy);
                        }
                        if (green) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(j+2, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(j+2, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gy2Data);
                            resultBuffer[i+2] = (float)Math.sqrt(ix*ix + iy*iy);
                        }
                        if (blue) {
                            ix = AlgorithmConvolver.convolve2DRGBPt(j+3, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DRGBPt(j+3, srcImage.getExtents(),
                                                                buffer2D, kExtents, Gy2Data);
            ...
            for ( i = 0; i < length && !threadStopped; i++){
                ...
                if ((offsetZ >= 0) && (offsetZ + kExtents[2] - 1 < zDim) ){
                    if (entireImage == true || mask.get(i)) {
                        ix = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(),
                                                            buffer, kExtents, GxData);
                        iy = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(),
                                                            buffer, kExtents, GyData);
                        iz = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(),
                                                            buffer, kExtents, GzData);
                    ...
                    if (entireImage == true || mask.get(i)) {
                        ...
                        ix = AlgorithmConvolver.convolve2DPt(j, srcImage.getExtents(),
                                                            buffer2D, kExtents, Gx2Data);
                        iy = AlgorithmConvolver.convolve2DPt(j, srcImage.getExtents(),
                                                            buffer2D, kExtents, Gy2Data);
            ...
            if (color == true) {
                for ( i = 0; i < length && !threadStopped; i+=4){
                    ...
                    if ((offsetZ >= 0) && (offsetZ + kExtents[2] - 1 < zDim) ){
                        ...
                        if (entireImage == true || mask.get(i/4)) {
                            resultBuffer[i]  = buffer[i];
                            if (red) {
                                ix = AlgorithmConvolver.convolve2DRGBPt(i+1, srcImage.getExtents(),
                                                                    buffer, kExtents, Gx2Data);
                                iy = AlgorithmConvolver.convolve2DRGBPt(i+1, srcImage.getExtents(),
                                                                    buffer, kExtents, Gy2Data);
                                resultBuffer[i+1] = (float)Math.sqrt(ix*ix + iy*iy);
                            }
                            if (green) {
                                ix = AlgorithmConvolver.convolve2DRGBPt(i+2, srcImage.getExtents(),
                                                                    buffer, kExtents, Gx2Data);
                                iy = AlgorithmConvolver.convolve2DRGBPt(i+2, srcImage.getExtents(),
                                                                    buffer, kExtents, Gy2Data);
                                resultBuffer[i+2] = (float)Math.sqrt(ix*ix + iy*iy);
                            }
                            if (blue) {
                                ix = AlgorithmConvolver.convolve2DRGBPt(i+3, srcImage.getExtents(),
                                                                    buffer, kExtents, Gx2Data);
                                iy = AlgorithmConvolver.convolve2DRGBPt(i+3, srcImage.getExtents(),
                                                                    buffer, kExtents, Gy2Data);
                                resultBuffer[i+3] = (float)Math.sqrt(ix*ix + iy*iy);
                            }
                        }
                ...
                for ( i = 0; i < length && !threadStopped; i++){
                    ...
                    if ((offsetZ >= 0) && (offsetZ + kExtents[2] - 1 < zDim) ){
                        ...
                        if (entireImage == true || mask.get(i)) {
                            ix = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                                buffer, kExtents, Gx2Data);
                            iy = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                                buffer, kExtents, Gy2Data);
                            resultBuffer[i] = (float)Math.sqrt(ix*ix + iy*iy);
                        }
                        else {
