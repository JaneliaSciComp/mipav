 ...
 * Reference: R. Piessens, E. deDoncker-Kapenga, C. Uberhuber, D. Kahaner
 * Quadpack: a Subroutine Package for Automatic Integration Springer Verlag,
 * 1983.  Series in Computational Mathematics v. 1
 *
 * <p>Title: </p>
 * <p>Description: </p>
...
public abstract class Integration2 {
  ...
  // Integral over (bound, +infinity), (-infinity, bound), or
  // (-infinity, +infinity)
  ...
  // Used in dqagpe
  // This array must always be >= 2 in length.  The first
  // breakPoints.length - 2 points are user provided break points.  If these
  // points are not in an ascending sequence, they will automatically be
  // sorted.
  ...
  // npts = npts2 - 2
  private int npts;
  // If epsabs <= 0.0 and epsrel < Math.max(50*relative machine accuracy,
  //                                         5.0E-29),
  // the routine will exit with an error message
  private double epsabs;
  private double epsrel;
  // Gives an upper bound on the number of subintervals in the partition of
  // lower, upper.
  // In dqagpe limit must be >= npts2.  If limit < npts2 in dqagpe, the routine
  // exits with an error message.
  private int limit;
  // finite bound of integration range used in dqagie
  // (has no meaning if interval is doubly-infinite)
  private double bound;
  // In dqagie indicates the kind of integration range involved
  // inf = 1 corresponds to (bound, +infinity)
  // inf = -1 corresponds to (-infinity, bound)
  // inf = 2 corresponds to (-infinity, +infinity)
  private int inf;
  // In dqage key selects the local integration rule
  ...
  // Estimate of the absolute value of the error, which should equal or
  // exceed abs(actual integral - result)
  private double abserr[] = new double[1];
  // Number of integrand evaluations
  private int neval;
  // errorStatus = 0 for normal and reliable termination of the routine.  It is
  // assumed that the requested accuracy has been achieved.
  // errorStatus > 0 for abnormal termination of the routine.  The estimates
  // for the integral and error are less reliable.  It is assumed that the
  ...
  // taking the according dimension adjustments into account).  However, if
  // this yields no improvement, it is advised to analyze the integrand in order
  // to determine the integration difficulties.  If the position of a local
  // difficulty can be determined( i.e. singularity, discontinuity within the
  // interval), it should be supplied to the routine as an element of the
  // breakPoints array.  If necessary, an appropriate special purpose integrator
  // must be used, which is designed for handling the type of difficulty
  // involved.
  // errorStatus = 2 The occurrence of roundoff error is detected, which
  // prevents the requested tolerance from being achieved.  The error may be
  // under-estimated.
  // errorStatus = 3 Extremely bad integrand behavior occurs at some points of
  // the integration interval.
  // errorStatus = 4 The algorithm does not converge.  Roundoff error is
  ...
  // errorStatus = 5 The integral is probably divergent, or slowly convergent.
  // It must be noted that divergence can occur with any other value of
  // errorStatus > 0.
  // errorStatus = 6 The input is invalid because (epsabs <= 0 and
  // epsrel < max(50.0 * relative machine accuracy, 5.0E-29) or in the
  // case of dqagpe can also happen because npts2 < 2, the breakPoints
  // are specified outside the integration range, or , or limit <= npts.
  private int errorStatus;
  // The left end points of the subintervals in the partition of the given
  // integration range (lower, upper)
  private double alist[];
  // The right end points of the subintervals in the partition of the given
  // integration range (lower, upper)
  ...
  // Integration limits and the break points of the interval in ascending
  // sequence
  private double pts[];
  // level contains the subdivision levels of the subinterval, i.e., if (aa, bb)
  // is a subinterval of (p1, p2) where p1 as well as p2 is a user-provided
  // break point or integration limit, then (aa, bb) has level 1 if abs(bb - aa)
  // = abs(p2 - p1) * 2**(-1).
  private int level[];
  // After the first integration over the intervals (pts[i], pts[i+1]),
  // i = 0, 1, ..., npts2 - 2, the error estimates over some of the intervals
  // may have been increased artificially, in order to put their subdivision
  // forward.  If this happens for the subinterval numbered k, ndin[k] is put
  // to 1, otherwise ndin[k] = 0.
  private int ndin[];
  // The first k elements of iord are pointers to the error estimates over the
  // subintervals, such that elist[iord[0]], ..., elist[iord[k-1]] form a
  // decreasing sequence, with k = last if last <= (limit/2 + 2) , and
  // k = limit + 1 - last otheriwse
  private int iord[];
  // The number of subintervals actually called in the subdivision process
  private int last;
  // epmach = D1MACH(4)
  // Machine epmach is the smallest positive epmach such that
  // (1.0 + epmach) != 1.0.
  // epmach = 2**(1 - doubleDigits) = 2**(1 - 53) = 2**(-52)
  // epmach = 2.224460e-16
  // epmach is called the largest relative spacing
  ...
  *  @param limit
  */
  public Integration2 (double lower, double upper, int routine,
                       double breakPoints[], double epsabs,
                       double epsrel, int limit) {
        double neweps;
        double tol;
        ...
        for (int i = 0; i < npts2 - 2; i++) {
          if (breakPoints[i] < lower) {
            MipavUtil.displayError("Break point " + breakPoints[i] +
                                   " is impossibly < lower");
            errorStatus = 6;
            return;
          }
        ...
        if ((epsabs <= 0.0) && (epsrel < tol)) {
          MipavUtil.displayError("Must set epsabs > 0.0 or must set epsrel >= "
                                 + tol);
          errorStatus = 6;
          return;
        }
        ...
        this.limit = limit;
        if (limit <= npts) {
          MipavUtil.displayError("Must set limit = " + limit +
                                 " > " + npts);
          errorStatus = 6;
          return;
        }
   ...
   * @param limit int
   */
  public Integration2(double bound, int routine, int inf,  double epsabs,
                       double epsrel, int limit) {
     double neweps;
     double tol;
     this.bound = bound;
    ...
    */
	public void dqagpe() {
          ...
          try {
                  ...
                  if ((abserr[0] <= 100.0 * epmach * resabs) &&
                      (abserr[0] > errBnd)) {
                    errorStatus = 2;
                  }
                  if (nint != 1) {
        ...
        /**
         * Computes integral of function over (a,b), with error estimate
         * and computes integral of abs(function) over (a,b)
         * @param a lower limit of integration
         * @param b upper limit of integration
         * @param result Approximation to the integral.  Result is computed
         *               by applying the 21-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 10-point
         *               gauss rule (resg).
         * @param abserr Estimate of the modulus of the absolute error, which
         *               should not exceed abs(actual integral - result)
         * @param resabs Approximation to the integral of abs(function) over
         *               (a,b).
         * @param resasc Approximation to the integral of
         *               abs(function - actual integral/(b-a)) over (a,b)
         */
        private void dqk21(double a, double b, double result[],
          ...
          // 10-point gauss rule
          double xgk[] = new double [] {0.995657163025808080735527280689003,
          ...
          double wgk[] = new double [] {0.011694638867371874278064396062192,
          ...
          double wg[] = new double [] {0.066671344308688137593568809893332,
          ...
          for (j = 0; j <= 9; j++) {
            resasc[0] = resasc[0] + wgk[j] * (Math.abs(fv1[j]-reskh) +
                                              Math.abs(fv2[j]-reskh));
          } // for (j = 0; j <= 9; j++)
          result[0] = resk * hlgth;
          ...
          if ((resasc[0] != 0.0) && (abserr[0] != 0.0)) {
            abserr[0] = resasc[0] * Math.min(1.0, Math.pow(
                        200.0 * abserr[0]/resasc[0], 1.5));
          }
        ...
        /**
         * This routine determines the limit of a given sequence of
         * approximations, by means of the epsilon algorithm of P. Wynn.  An
         * estimate of the absolute error is also given.  The condensed
         * epsilon table is computed.  Only those elements needed for the
         * computation of the next diagonal are preserved.
         * @param n epstab[n[0]-1] contains the new element in the first column
         *          of the epsilon table
         * @param epstab 52 element array containing the elements of the two
         *               lower diagonals of the triangular epsilon table.  The
         *               elements are numbered starting at the right-hand
         *               corner of the triangle
         * @param result resulting approximation to the integral
         *               The element in the new diagonal with the least value
         *               of error.
         * @param abserr estimate of the absolute error computed from result
         *               and the 3 previous results
         * @param res31a array containing the last 3 results
         * @param nres number of calls to the routine (should be zero at
         *             first call)
         */
        private void dqelg(int n[], double epstab[], double result[],
                           double abserr[], double res31a[], int nres[]) {
          double delta1;
          double delta2;
          double delta3;
          double epsinf;
          ...
          abserr[0] = Math.abs(result[0] - res31a[2]) +
                      Math.abs(result[0] - res31a[1]) +
                      Math.abs(result[0] - res31a[0]);
          res31a[0] = res31a[1];
          res31a[1] = res31a[2];
          res31a[2] = result[0];
          abserr[0] = Math.max(abserr[0],5.0*epmach*Math.abs(result[0]));
        ...
        public void dqagie() {
          ...
          try {
            ...
            if ((errorStatus != 0) ||
                ((abserr[0] <= errBnd) && (abserr[0] != resabs[0])) ||
                (abserr[0] == 0.0)) {
              neval = 30 * last - 15;
              if (inf == 2) {
                neval = 2 * neval;
            ...
            for (last = 2; last <= limit; last++) {
              ...
              if ((defab1[0] != error1[0]) && (defab2[0] != error2[0])) {
                if ((Math.abs(rlist[maxErr[0]] - area12) <=
                     1.0E-5 * Math.abs(area12)) && (error12 >= 0.99 *errMax[0])) {
                  if (extrap) {
                    iroff2 = iroff2 + 1;
                  }
                  else {
         ...
         * (0, 1) and (a, b) is a part of (0, 1).  This routine computes the
         * integral of the transformed integrand over (a, b) and the integral
         * of the abs(transformed integrand) over (a, b).
         * @param boun finite bound of original integration range (set to
         *             zero if inf = +2)
         * @param inf If inf = -1, the original interval is (-infinity, bound)
         *            If inf = +1, the original interval is (bound, +infinity)
         *            If inf = +2, the original interval is (-infinity,
         *            +infinity) and the integral is computed as the sum of
         *            two integrals, one over (-infinity, 0) and one over
         *            (0, +infinity)
         * @param a lower limit for integration over subrange of (0, 1)
         * @param b upper limit for integration over subrange of (0, 1)
         * @param result Approximation to the integral.  The result is computed
         *               by applying the 15-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 7-point
         *               gauss rule (resg).
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of transformed integrand - result)
         * @param resabs Approximation to integral of abs(transformed integrand)
         * @param resasc Approximation to the integral of
         *               abs(transformed integrand - integral of transformed
         *                   integrand/(b - a)) over (a, b)
         */
        private void dqk15i(double boun, int inf, double a, double b,
          ...
          // xgk[0], xgk[2], ... abscissae which are optimally added to the
          // 7-point gauss rule.
          double xgk[] = new double[] {0.991455371120812639206854697526329,
          ...
          double wgk[] = new double[] {0.022935322010529224963732008058970,
          ...
          abserr[0] = Math.abs((resk - resg) * hlgth);
          if ((resasc[0] != 0.0) && (abserr[0] != 0.0)) {
            abserr[0] = resasc[0] * Math.min(1.0,
            Math.pow(200.0 * abserr[0]/resasc[0], 1.5));
          } // if ((resasc[0] != 0.0) && (abserr[0] != 0.0))
          if (resabs[0] > uflow/(50.0 * epmach)) {
            abserr[0] = Math.max((50.0 * epmach) * resabs[0], abserr[0]);
          } // if (resabs[0] > uflow/(50.0 * epmach))
        ...
        /**
         * This routine calculates the integral of intFunc over (lower, upper)
         * and can handle end-point singularities at lower and upper.  It tries
         * to satisfy the claim for accuracy abs(actual integral - result) <=
         * max(epsabs, esprel * abs(actual integral))
         */
        private void dqagse() {
          // Variables ending in 1 denote left subinterval and variables
          // ending in 2 denote right subinterval
        ...
        /**
         * Calculates an integral over (lower, upper), hopefully satisfying
         * the abs(actual integral - result) <=
         * max(epsabs, epsresl * abs(actual integral))
         ...
         * rule (res10), or by applying the 43-point rule (res43) obtained by
         * optimal addition of abscissae to the 21-point gauss-kronrod rule, or
         * by applying the 87-point rule (res87) obtained by optimal addition
        ...
        private void dqng() {
          ...
          for (k = 0; k < 5; k++) {
            resasc = resasc + w21a[k] * (Math.abs(fv1[k] - reskh) +
                                         Math.abs(fv2[k] - reskh))
                            + w21b[k] * (Math.abs(fv3[k] - reskh) +
                                         Math.abs(fv4[k] - reskh));
          } // for (k = 0; k < 5; k++)
          abserr[0] = Math.abs((res21 - res10) * hlgth);
          resasc = resasc * dhlgth;
          if ((resasc != 0.0) && (abserr[0] != 0.0)) {
            abserr[0] = resasc * (Math.min(1.0,
                        Math.pow((200.0*abserr[0]/resasc),1.5)));
          } // if ((resasc != 0.0) && (abserr[0] != 0.0))
          ...
          for (k = 0; k < 22; k++) {
            absc = hlgth * x4[k];
            res87 = res87 + w87b[k] * (intFunc(absc + centr) +
                                       intFunc(centr - absc));
          } // for (k = 0; k < 22; k++)
        ...
        /**
         * This routine computes theintegral of the intFunc over (a, b)
         * and the integral of the abs(intFunc) over (a, b).
         ...
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of intFunc over (a,b) - result)
         * @param resabs Approximation to integral of abs(intFunc)
         * @param resasc Approximation to the integral of
         *               abs(intFunc - integral of intFunc/(b - a)) over (a, b)
         */
        private void dqk15(double a, double b,
                            double result[], double abserr[], double resabs[],
                            double resasc[]) {
           // The abscissae and weights are supplied for the interval (-1, 1).
           // Because of symmetry only the positive abscissae and their
           ...
           for (j = 0; j < 3; j++) {
             ...
             resg = resg + wg[j] * fsum;
             resk = resk + wgk[jtw] * fsum;
             resabs[0] = resabs[0] + wgk[jtw] * (Math.abs(fval1) +
                                                 Math.abs(fval2));
           } // for (j = 0; j < 3; j++)
           for (j = 0; j < 4; j++) {
             jtwm1 = 2*j;
             ...
             fsum = fval1 + fval2;
             resk = resk + wgk[jtwm1] * fsum;
             resabs[0] = resabs[0] + wgk[jtwm1] * (Math.abs(fval1) +
                                                   Math.abs(fval2));
           } // for (j = 0; j < 4; j++)
           reskh = 0.5 * resk;
           resasc[0] = wgk[7] * Math.abs(fc - reskh);
         ...
         * @param b upper limit for integration
         * @param result Approximation to the integral.  The result is computed
         *               by applying the 31-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 15-point
         *               gauss rule (resg).
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of intFunc over (a,b) - result)
         ...
         *               abs(intFunc - integral of intFunc/(b - a)) over (a, b)
         */
        private void dqk31(double a, double b,
                            double result[], double abserr[], double resabs[],
                            double resasc[]) {
           // The abscissae and weights are supplied for the interval (-1, 1).
           // Because of symmetry only the positive abscissae and their
           ...
           // xgk[0], xgk[2], ... abscissae which are optimally added to the
           // 15-point gauss rule.
           double xgk[] = new double[] {0.998002298693397060285172840152271,
          ...
          double wgk[] = new double[] {0.005377479872923348987792051430128,
          ...
          double wg[] = new double[] {0.030753241996117268354628393577204,
         ...
         * @param b upper limit for integration
         * @param result Approximation to the integral.  The result is computed
         *               by applying the 41-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 20-point
         *               gauss rule (resg).
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of intFunc over (a,b) - result)
         ...
         *               abs(intFunc - integral of intFunc/(b - a)) over (a, b)
         */
        private void dqk41(double a, double b,
                            double result[], double abserr[], double resabs[],
                            double resasc[]) {
           // The abscissae and weights are supplied for the interval (-1, 1).
           // Because of symmetry only the positive abscissae and their
           ...
           // xgk[0], xgk[2], ... abscissae which are optimally added to the
           // 20-point gauss rule.
           double xgk[] = new double[] {0.998859031588277663838315576545863,
          ...
          double wgk[] = new double[] {0.003073583718520531501218293246031,
          ...
          double wg[] = new double[] {0.017614007139152118311861962351853,
         ...
         * @param b upper limit for integration
         * @param result Approximation to the integral.  The result is computed
         *               by applying the 51-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 25-point
         *               gauss rule (resg).
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of intFunc over (a,b) - result)
         ...
         *               abs(intFunc - integral of intFunc/(b - a)) over (a, b)
         */
        private void dqk51(double a, double b,
                            double result[], double abserr[], double resabs[],
                            double resasc[]) {
           // The abscissae and weights are supplied for the interval (-1, 1).
           // Because of symmetry only the positive abscissae and their
           ...
           // xgk[0], xgk[2], ... abscissae which are optimally added to the
           // 25-point gauss rule.
           double xgk[] = new double[] {0.999262104992609834193457486540341,
          ...
          double wgk[] = new double[] {0.001987383892330315926507851882843,
          ...
          double wg[] = new double[] {0.011393798501026287947902964113235,
         ...
         * @param b upper limit for integration
         * @param result Approximation to the integral.  The result is computed
         *               by applying the 61-point kronrod rule (resk) obtained
         *               by optimal addition of abscissae to the 30-point
         *               gauss rule (resg).
         * @param asberr Estimate of the modulus of the absolute error, which
         *               should equal or exceed
         *               abs(integral of intFunc over (a,b) - result)
         ...
         *               abs(intFunc - integral of intFunc/(b - a)) over (a, b)
         */
        private void dqk61(double a, double b,
                            double result[], double abserr[], double resabs[],
                            double resasc[]) {
           // The abscissae and weights are supplied for the interval (-1, 1).
           // Because of symmetry only the positive abscissae and their
           ...
           // xgk[0], xgk[2], ... abscissae which are optimally added to the
           // 30-point gauss rule.
           double xgk[] = new double[] {0.999484410050490637571325895705811,
          ...
          double wgk[] = new double[] {0.001389013698677007624551591226760,
          ...
          double wg[] = new double[] {0.007968192496166605615465883474674,
