 ...
 *  assumes a maximum in the gradient direction.  Introduce a local orthonormal
...
public class AlgorithmNMSuppression extends AlgorithmBase {
    ...
    *   @param maskFlag Flag that indicates that the non-maximum suppression will be
    ...
    */
	public AlgorithmNMSuppression(ModelImage destImg, ModelImage srcImg,float
	                        sigmas[], boolean maskFlag, boolean img25D) {
    ...
    *                   should be processed independently. 2D images disregard this flag.
    */
    public AlgorithmNMSuppression(ModelImage srcImg, float sigmas[],
    ...
    */
    private void constructLog() {
        ...
        historyString = new String( "Non-maximum suppression(" + sigmaStr +
    ...
    *   @return     - ModelImage of zero crossings ( 2D function
    *                 1 = indicates zero crossing
    */
    public ModelImage getZeroXMask() { return zXMask;}

    /**
    *   Generates a zero crossing mask for a 2D function
    *                  sets a Bitset object to 1 is a zero crossing is
    *                  detected.
    ...
    */
    ...
    /**
    *   Generates a zero crossing mask for a 2D function
    *                  sets a ModelImage to 1 if a zero crossing is
    *                  detected.
    ...
    */
    public void genZeroXMask(float buffer[], float buffer2[]){
        ...
        x0 = buffer[0];
        x2 = buffer[xDim];
        if ((x0>=loThres)&&(x0<=hiThres)) x0 = 0;
        if ((x2>=loThres)&&(x2<=hiThres)) x2 = 0;
        for (j = 0; j < yyDim; j++) {
            indexY = j*xDim;
            for(i = 0; i < xxDim; i++) {
                index = indexY + i;
                x1 = buffer[index+1];
                x3 = buffer[index+1+xDim];
                if ((x1>=loThres)&&(x1<=hiThres))   x1 = 0;
                if ((x3>=loThres)&&(x3<=hiThres))   x3 = 0;
    ...
    /**
    *   Calculates the non-maximum suppression image and replaces the source image
    *                   with the new image.
    ...
    */
            ...
            for ( i = 0; i < length && !threadStopped; i++){
                ...
                if (entireImage == true || mask.get(i) ) {
                    ...
                    if (edgeImage == true && nImages == 1) {
                        ixxx = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                  buffer, kExtents, GxxxData);
                        ixxy = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                  buffer, kExtents, GxxyData);
                        ixyy = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                  buffer, kExtents, GxyyData);
                        iyyy = AlgorithmConvolver.convolve2DPt(i, srcImage.getExtents(),
                                                  buffer, kExtents, GyyyData);
                        resultBuffer2[i] = ix*ix*ix*ixxx + 3.0f*ix*ix*iy*ixxy + 3.0f*ix*iy*iy*ixyy +
                                           iy*iy*iy*iyyy;
                    } // if (edgeImage == true && nImages == 1)
                }
                else {
    ...
    /**
    *   Calculates the non-maximum suppression and replaces the source image
    *                   with the new image.
    */
    private void calcInPlace3D(){
        ...
        for ( i = 0; i < length && !threadStopped; i++){
            ...
            if (entireImage == true || mask.get(i) ) {
                ...
                resultBuffer[i] = ix*ix*ixx + 2.0f*ix*iy*ixy + iy*iy*iyy + 2.0f*ix*iz*ixz + 2.0f*iy*iz*iyz +
                                  iz*iz*izz;
            }
            else {
    ...
    private void calcStoreInDest3D(){
        ...
        for ( i = 0; i < length  && !threadStopped; i++){
            ...
            if (entireImage == true || mask.get(i) ) {
                ...
                iyz = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(), buffer, kExtents, GyzData);
                izz = AlgorithmConvolver.convolve3DPt(i, srcImage.getExtents(), buffer, kExtents, GzzData);
                nms = ix*ix*ixx + 2.0f*ix*iy*ixy + iy*iy*iyy + 2.0f*ix*iz*ixz + 2.0f*iy*iz*iyz +
                      iz*iz*izz;
                destImage.set(i, nms);
            }
            else {
