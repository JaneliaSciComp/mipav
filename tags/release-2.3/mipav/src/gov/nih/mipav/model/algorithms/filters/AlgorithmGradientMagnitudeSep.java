 ...
 *	from AlgorithmGradientMagnitude.  This version should be faster but
 *	uses significantly more memory.
 *
 *	Produces equivalent result images to AlgorithmGradientMagnitude
 *	(aside from some rounding error that seems to be at most 10^-4)
 *
 *
...
public class AlgorithmGradientMagnitudeSep extends AlgorithmBase {
    ...
    */
    public AlgorithmGradientMagnitudeSep(ModelImage destImg, ModelImage srcImg, float sigmas[],
    ...
    private float[] calcInPlace2DBuffer(int nImages, float[] buffer, int[] extents, boolean color, float[] xDirections, float[] yDirections){
        ...
        for (s = 0; s <  nImages; s++) {
            ...
            if (color == true) {
                ...
                AlgorithmSeparableConvolver xConvolver = new AlgorithmSeparableConvolver(
                  xBuffer, buffer, new int[] {extents[0], extents[1]},
                  GxDataDerivative, GyDataRound, color);
                ...
                AlgorithmSeparableConvolver yConvolver = new AlgorithmSeparableConvolver(
                  yBuffer, buffer, new int[] {extents[0], extents[1]},
                  GxDataRound, GyDataDerivative, color);
                ...
                for ( i = 0; i < length && !threadStopped; i+=4 ) {
                    if ( entireImage || mask.get(i/4) ) {
                        ...
                        if (red) {
                            resultBuffer[start+i+1] = (float)Math.sqrt(xBuffer[i+1] * xBuffer[i+1] + yBuffer[i+1] * yBuffer[i+1]);
                            if (xDirections != null) {
                        ...
                        if (green) {
                            resultBuffer[start+i+2] = (float)Math.sqrt(xBuffer[i+2] * xBuffer[i+2] + yBuffer[i+2] * yBuffer[i+2]);
                            if (xDirections != null) {
                        ...
                        if (blue) {
                            resultBuffer[start+i+3] = (float)Math.sqrt(xBuffer[i+3] * xBuffer[i+3] + yBuffer[i+3] * yBuffer[i+3]);
                            if (xDirections != null) {
                ...
                for ( i = 0; i < length && !threadStopped; i++ ) {
                    if ( entireImage || mask.get(i) ) {
                        resultBuffer[start+i] = (float)Math.sqrt(xBuffer[i] * xBuffer[i] + yBuffer[i] * yBuffer[i]);
                        if (xDirections != null) {
    ...
    *   @param yDirections
    */
    private float[] calcInPlace2DBufferUnnormalized(int nImages, float[] buffer,
        int[] extents, boolean color, float[] xDirections, float[] yDirections){
    ...
    */
    private void calcInPlace3D(){
        ...
        if (color) {
            curPercent = 5;
            maxPercent = 20;
            AlgorithmSeparableConvolver xConvolver = new AlgorithmSeparableConvolver(xBuffer,
              buffer, srcImage.getExtents(), GxDataDerivative, GyDataRound,
              GzDataRound, color);
            xConvolver.setActiveImage(activeImage);
            if ( !entireImage ) {
                xConvolver.setMask(mask);
            ...
            maxPercent += 20;
            AlgorithmSeparableConvolver yConvolver = new AlgorithmSeparableConvolver(yBuffer,
              buffer, srcImage.getExtents(), GxDataRound, GyDataDerivative,
              GzDataRound, color);
            yConvolver.setActiveImage(activeImage);
            if ( !entireImage ) {
                yConvolver.setMask(mask);
            ...
            curPercent = maxPercent;
            maxPercent += 20;
            AlgorithmSeparableConvolver zConvolver = new AlgorithmSeparableConvolver(zBuffer,
              buffer, srcImage.getExtents(), GxDataRound, GyDataRound, GzDataDerivative,
              color);
            ...
            for ( i = 0; i < length && !threadStopped; i+=4 ) {
                ...
                if ( (offsetZ >= 0) && (offsetZ + GzDataDerivative.length - 1 < zDim) ) {
                    if ( entireImage || mask.get(i/4) ) {
                        ...
                        if (red) {
                            resultBuffer[i+1] = (float)Math.sqrt(xBuffer[i+1]*xBuffer[i+1] + yBuffer[i+1]*yBuffer[i+1] + zBuffer[i+1]*zBuffer[i+1]);
                        }
                        else {
                            resultBuffer[i+1] = buffer[i+1];
                        }
                        if (green) {
                            resultBuffer[i+2] = (float)Math.sqrt(xBuffer[i+2]*xBuffer[i+2] + yBuffer[i+2]*yBuffer[i+2] + zBuffer[i+2]*zBuffer[i+2]);
                        }
                        else {
                            resultBuffer[i+2] = buffer[i+2];
                        }
                        if (blue) {
                            resultBuffer[i+3] = (float)Math.sqrt(xBuffer[i+3]*xBuffer[i+3] + yBuffer[i+3]*yBuffer[i+3] + zBuffer[i+3]*zBuffer[i+3]);
                        }
                        else {
                    ...
                    if (entireImage || mask.get(i/4)) {
                        if ( cur2DBufferSlice != curSlice ) {
                            ...
                            AlgorithmSeparableConvolver x2Convolver =
                              new AlgorithmSeparableConvolver(x2Buffer, sliceBuffer,
                            ...
                            AlgorithmSeparableConvolver y2Convolver =
                              new AlgorithmSeparableConvolver(y2Buffer, sliceBuffer,
                              new int[] {srcImage.getExtents()[0], srcImage.getExtents()[1]},
                              GxDataRound, GyDataDerivative, color);
                        ...
                        if (red) {
                            resultBuffer[i+1] = (float)Math.sqrt(x2Buffer[j+1]*x2Buffer[j+1] + y2Buffer[j+1]*y2Buffer[j+1]);
                        }
                        if (red) {
                            resultBuffer[i+2] = (float)Math.sqrt(x2Buffer[j+2]*x2Buffer[j+2] + y2Buffer[j+2]*y2Buffer[j+2]);
                        }
                        if (blue) {
                            resultBuffer[i+3] = (float)Math.sqrt(x2Buffer[j+3]*x2Buffer[j+3] + y2Buffer[j+3]*y2Buffer[j+3]);
                        }
                    }
                    else {
                        resultBuffer[i]   = buffer[i];
            ...
            maxPercent += 20;
            AlgorithmSeparableConvolver zConvolver = new AlgorithmSeparableConvolver(zBuffer,
              buffer, srcImage.getExtents(), GxDataRound, GyDataRound,
              GzDataDerivative, color);
            zConvolver.setActiveImage(activeImage);
            if ( !entireImage ) {
                zConvolver.setMask(mask);
            }
            ...
            for ( i = 0; i < length && !threadStopped; i++ ) {
                ...
                if ( (offsetZ >= 0) && (offsetZ + GzDataDerivative.length - 1 < zDim) ) {
                    if ( entireImage == true || mask.get(i) ) {
                        resultBuffer[i]  = (float)Math.sqrt(xBuffer[i]*xBuffer[i] + yBuffer[i]*yBuffer[i] + zBuffer[i]*zBuffer[i]);
                    }
                    else {
                        resultBuffer[i] = buffer[i];
                    }
    ...
    */
    private void calcStoreInDest2D(int nImages){
        ...
        for (s = 0; s <  nImages; s++) {
            ...
            if (color) {
                AlgorithmSeparableConvolver xConvolver = new AlgorithmSeparableConvolver(
                  xBuffer, buffer, new int[] {srcImage.getExtents()[0],
                  srcImage.getExtents()[1]}, GxDataDerivative, GyDataRound, color);
                xConvolver.setActiveImage(activeImage);
                if ( !entireImage ) {
                    xConvolver.setMask(mask);
                }
                ...
                AlgorithmSeparableConvolver yConvolver = new AlgorithmSeparableConvolver(
                  yBuffer, buffer, new int[] {srcImage.getExtents()[0],
                  srcImage.getExtents()[1]}, GxDataRound, GyDataDerivative, color);
                yConvolver.setActiveImage(activeImage);
                if ( !entireImage ) {
                    yConvolver.setMask(mask);
                }
                ...
                for ( i = 0, idx = start; i < length && !threadStopped; i+=4, idx+=4 ) {
                    if ( entireImage || mask.get(i/4) ) {
                        ...
                        if (red) {
                            destImage.set(idx+1, (float)Math.sqrt(xBuffer[i+1] * xBuffer[i+1] + yBuffer[i+1] * yBuffer[i+1]));
                        }
                        else {
                            destImage.set(idx+1, buffer[i+1]);
                        }
                        if (green) {
                            destImage.set(idx+2, (float)Math.sqrt(xBuffer[i+2] * xBuffer[i+2] + yBuffer[i+2] * yBuffer[i+2]));
                        }
                        else {
                            destImage.set(idx+2, buffer[i+2]);
                        }
                        if (blue) {
                            destImage.set(idx+3, (float)Math.sqrt(xBuffer[i+3] * xBuffer[i+3] + yBuffer[i+3] * yBuffer[i+3]));
                        }
                        else {
    ...
    private void calcStoreInDest3D(){
        ...
        if (color) {
            ...
            for ( i = 0; i < length && !threadStopped; i+=4 ) {
                ...
                if ( (offsetZ >= 0) && (offsetZ + GzDataDerivative.length - 1 < zDim) ) {
                    if ( entireImage || mask.get(i/4) ) {
                        ...
                        if (red) {
                            destImage.set(i+1, (float)Math.sqrt(xBuffer[i+1]*xBuffer[i+1] + yBuffer[i+1]*yBuffer[i+1] + zBuffer[i+1]*zBuffer[i+1]));
                        }
                        else {
                            destImage.set(i+1,   buffer[i+1]);
                        }
                        if (green) {
                            destImage.set(i+2, (float)Math.sqrt(xBuffer[i+2]*xBuffer[i+2] + yBuffer[i+2]*yBuffer[i+2] + zBuffer[i+2]*zBuffer[i+2]));
                        }
                        else {
                            destImage.set(i+2,   buffer[i+2]);
                        }
                        if (blue) {
                            destImage.set(i+3, (float)Math.sqrt(xBuffer[i+3]*xBuffer[i+3] + yBuffer[i+3]*yBuffer[i+3] + zBuffer[i+3]*zBuffer[i+3]));
                        }
                        else {
                    ...
                    if (entireImage || mask.get(i/4)) {
                        ...
                        j = i%sliceSize;	// index into 2D x2 and y2Buffer
                        if (red) {
                            destImage.set(i+1, (float)Math.sqrt(x2Buffer[j+1]*x2Buffer[j+1] + y2Buffer[j+1]*y2Buffer[j+1]));
                        }
                        if (red) {
                            destImage.set(i+2, (float)Math.sqrt(x2Buffer[j+2]*x2Buffer[j+2] + y2Buffer[j+2]*y2Buffer[j+2]));
                        }
                        if (blue) {
                            destImage.set(i+3, (float)Math.sqrt(x2Buffer[j+3]*x2Buffer[j+3] + y2Buffer[j+3]*y2Buffer[j+3]));
                        }
                    }
                    else {
            ...
            for ( i = 0; i < length && !threadStopped; i++ ) {
                ...
                if ( (offsetZ >= 0) && (offsetZ + GzDataDerivative.length - 1 < zDim) ) {
                    if ( entireImage || mask.get(i) ) {
                        destImage.set(i, (float)Math.sqrt(xBuffer[i]*xBuffer[i] + yBuffer[i]*yBuffer[i] + zBuffer[i]*zBuffer[i]));
                    }
                    else {
                        destImage.set(i, buffer[i]);
                    }
