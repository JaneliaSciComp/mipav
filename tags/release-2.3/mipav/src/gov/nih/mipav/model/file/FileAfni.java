...
/**
*   Afni file format<p>
*   In Afni 3 .HEAD files may be present with just 1 .BRIK file containing the original
*   Data.  The +orig.BRIK file will contain the data, the +orig.HEAD file will describe
*   transformation to create the original image, the +acpc.HEAD file will describe the
*   transformation needed to create the acpc image, and the +tlrc.HEAD file will describe
*   the transformation needed to create the Talairach image.
*   <p>
*   <b>Special notes:</b><p>
*   The document README.attributes specifies DICOM order for MARKS_XYZ, but the
*   sample file anat+orig.HEAD has dataset order.  The sample file
*   anat+ACPC.HEAD has DICOM order.
*   <p>
*   The original data must be resampled to cubic voxels for +acpc and +tlrc space.
*   In FAQ 67, How can I compare 'small' and 'big' in Talairach box datasets, the text
*   states:<br>
*   If you have used the default 1 mm. voxel size in +tlrc coordinates(as almost everyone
*   does)...
*   <p>
*   The function THD_3dmm_to_dicomm does changes the ordering of x, y, and z, but never inverts
*   them.  That is either R-L or L-R could be present in AFNI's Dicom ordering, either S-I or I-S
*   could be present in AFNI's dicom ordering, and either A-P or P-A could be present in AFNI's
*   dicom ordering.  I perform inversions in my code so that I always have x = R-L,  Y = A-P, and
*   Z = I-S, the standard dicom ordering.
*   <p>
*   To obtain the afni rr, bvec, and svec values, I must not invert axes in dicom ordering, and I
*   must not adjust the marker coordinates for resolution and origin.
*   <p>
*   The function of the SKIP fields is not defined in the AFNI documentation.
*   <p>
*   Note that AFNI code uses mfor for forward transformation and mbac for backward transformation.
*   Our interpolation routines perform a output to input mapping so since I am skipping the
*   transform() section of AlgorithmTransform which has the line
*   xfrm = matrixtoInverseArray(transMatrix);<br>
...
*   <p>
*   Note that in the sample files func+orig.head and anat+orig.head the origin location in
*   millimeters is chosen so that 0,0,0 in millimeters is at the center of the image.  In
*   anat+acpc.head and anat+tlrc.head the origin location in millimeters ensures that the Talairach
*   center is at 0,0,0.
*    <p>
...
*   <p>
*   v1:time+orig.HEAD has 1 = ORI_L2R_TYPE for the third ORIENT_SPECIFIC value, -53 for the third ORIGIN
*   value, and 7 for the third delta value.  However, delta should be negative for ORI_L2R_TYPE.
*   I changed the values to those found in v2:time+orig.HEAD, namely 1 for the third ORIENT_SPECIFIC
*   value, 52 for the third ORIGIN value, and -7 for the third delta value.  The observed overlap was
*   then excellent for v1_time+orig.HEAD and anat+orig.HEAD, anat+ACPC.HEAD, and anat+TLRC.HEAD.
*
...
public class FileAfni extends FileBase {
    ...
    // Default scale factors for functional data threshold values stored as shorts
    // If BRICK_FLOAT_FACS is not supplied and the threshold functional data is
    // stored as shorts, then AFNI will apply a default scale factor unless the
    // data is FUNC_FIM_TYPE with only an intensity value and no threshold value
    // or FUNC_BUCK_TYPE which has a default scale factor of 1.
    ...
    private String          typeString; // one of 3DIM_FileInfoAfni.HEAD_ANAT or 3DIM_FileInfoAfni.HEAD_FUNC or 3DIM_FileInfoAfni.GEN_ANAT or
                                        // 3DIM_FileInfoAfni.GEN_FUNC.  Determines if dataset if of Anat or Func
                                        // type.  If Anat type, and if it is a _FileInfoAfni.HEAD_ dataset in the
                                        // +orig view, then Talairach markers might be attached to it
    ...
    private String  marksLabString[] = new String[10];
                                       // 200 characters give the labels for the markers (20 chars
                                       // per marker, EXACTLY, including the NULS).  A marker whose string
                                       // is empty (all NUL characters) will not be defined or shown by
                                       // AFNI.

    private String  marksHelpString[] = new String[10];
                                       // 2560 characters giving the help strings for the markers
                                       // (256 chars per marker, EXACTLY, including the NULS).
   ...
   private int              slicesWithTimeOffsets = 0; // Obtained from TAXIS_NUMS[1]
    ...
    private float[] brickFloatFacs = null; // There should be subBrickNumber(DATASET_RANK[1] or nvals) values here.
                                 // For the p-th sub-brick, if f = BRICK_FLOAT_FACS[p] is positive, then
                                 // the values in .BRIK should be scaled by f to give their "true" values.
                                 ...
                                 // unscaled.  Possible uses for f < 0 are reserved for the future.
                                 // If this attribute is not present, then the intensity data is unscaled. If you
                                 // store functional threshold data as shorts, but do not supply a
                                 // nonzero scale factor, then a default one is supplied to the data
                                 // unless the functional data is FUNC_BUCK_TYPE for which
                                 // FUNC_BUCK_SCALE_SHORT is 1.
    ...
    private float[] warpData = null; // Data that define the transformation from the warp data to the current
                                 ...
                                 // [mfor] = 3x3 forward transformation matrix [0..8]
                                 // 0,1,2 correspond to alpha.x, alpha.y, alpha.z
                                 // 3,4,5 correspond to beta.x, beta.y, beta.z
                                 // 6,7,8 correspond to gamma.x, gamma.y, gamma.z
                                 // [mbac] = 3x3 backward transformation matrix [0..17]
                                 // [bvec] = 3-vector for forward transformation [18..20]
                                 // [svec] = 3-vector for backward transformation [21..23]
                                 // [bot] [24..26]
                                 // [top] [27..29]
                                 ...
                                 // to the warped dataset (usually +acpc or +tlrc).  The backward transformation
                                 // is just the inverse of the forward transformation, and is stored for
                                 // convenience.  It could be recomputed from the forward transformation
                                 // whenever it was needed.

                                 // The identity BLT would be stored as these 30 numbers:
                                 // 1 0 0
                                 // 0 1 0  [mfor] = I
                                 // 0 0 1
                                 // 1 0 0
                                 ...
                                 // botx boty botz
                                 // topx topy topz
                                 // bot and top numbers depend on the application.

                                 // If the transformation is WARP_TALAIRACH_TYPE, then each BLT type only
                                 ...
                                 // These values are used in the function AFNI_transform_vector() to compute
                                 // the transformation of a 3-vector between +orig and +tlrc coordinates.
                                 // For example, to compute the transformation from +tlrc back to +orig of a
                                 // vector [x_tlrc], the code must scan all 12 [bot]..[top] regions to see
                                 // which BLT to use.  Similarly, to transform [x_orig] from +orig to +tlrc,
                                 // the vector must be transformed with each BLT and the result tested to
                                 // see if it lies within the BLT's [bot]..[top] region.  (if a lower bound
                                 // is supposed to be -infinity, then that element of [bot] is -9999; if an
                                 // upper bound is supposed to be +infinity then that element of [top] is
                                 // +9999.
                                 ...
                                 // However, this fact isn't used much.
    ...
    // Note that the MATVEC attributes are transformations of Dicom-ordered coordinates, and so have to be
    // permuted to transform dataset-ordered xyz-coordinates.  The MATVEC attributes describe the transformation
    // of coordinates from the input dataset to the output dataset in the form
    ...
    // Dicom coordinate order is used for these matrices and vectors, which means that they need to be permuted
    // to dataset order for application
    ...
    private float[] volregMatvec000009 = null;
                                               // For sub-brick #xxxxxx (so a max of 999,999 sub-bricks can be
                                               // used), this stores 12 numbers for the matrix-vector of
                                               // transformation from 3dvolreg
    ...
    */
    public FileAfni(ViewUserInterface _UI, String fileName, String fileDir, boolean loadB,
        ...
        if (doRead) {
            ...
            if ((sameType) &&
                ((viewType == FileInfoAfni.AFNI_ACPC) || (viewType == FileInfoAfni.AFNI_TLRC))) {
    ...
    */
    ...
    /**
    *   readLine()            - reads a line of the file and strips comments indicated
    *                           by the # symbol.
    ...
    */
    ...
    private void reposMarker(int markerNumber) {
        ...
        if (doDicom) {
            ...
            pointMarker = new Point3Df((float)originalMarker[0],(float)originalMarker[1],
                                       (float)originalMarker[2]);
        }
    }
   ...
   */
   private void transformACPCTrilinear(float imgBuffer[], double xfrm[][],
                           ...
                           Z = (j3 + (imm*T20))*invZRes;
                            if ((Z >= 0) && (Z < iZdim1)){
                                    ...
                                    b1 = dy1 * (dx1*imgBuffer[position1]       + dx*imgBuffer[position1+1]) +
                                         dy  * (dx1*imgBuffer[position1+iXdim] + dx*imgBuffer[position1+iXdim+1]);

                                    b2 = dy1 * (dx1*imgBuffer[position2]       + dx*imgBuffer[position2+1]) +
                                         dy  * (dx1*imgBuffer[position2+iXdim] + dx*imgBuffer[position2+iXdim+1]);
   ...
   *   @param xfrm    transformation matrix to be applied
   */
   private void transformTalairachTrilinear(float imgBuffer[], double xfrm[][],
        float iXres, float iYres, float iZres, int iXdim, int iYdim, int iZdim,
        float oXres, float oYres, float oZres, int oXdim,
        int botX, int botY, int botZ, int topX, int topY, int topZ)             {
    ...
    */
    public ModelImage readImage() throws IOException {
        ...
        Preferences.debug("center.x = " + center.x + " center.y = " + center.y + " center.z = " +
                              center.z + "\n");
        ...
        /*
        Tr03 = M[0][0] * Tx + M[0][1] * Ty + M[0][2] * Tz + M[0][3]
        Tr13 = M[1][0] * Tx + M[1][1] * Ty + M[1][2] * Tz + M[1][3]
        Tr23 = M[2][2] * Tx + M[2][1] * Ty + M[2][2] * Tz + M[2][3]
        */
        A = new Matrix(3,3);
        A.set(0,0,M[0][0]);
        A.set(0,1,M[0][1]);
        ...
        frm = xfrm.getMatrix();
        transformACPCTrilinear(imgBuffer, frm,
                iXres, iYres, iZres, iXdim, iYdim, iZdim,
                oXres, oYres, oZres, oXdim, oYdim, oZdim);
        ...
        /* X = (i*oXres*T00 + j*oYres*T01 + k*oZres*T02 + T03)/iXres
           Y = (i*oXres*T10 + j*oYres*T11 + k*oZres*T12 + T13)/iYres
           Z = (i*oXres*T20 + j*oYres*T21 + k*oZres*T22 + T23)/iZres
           Wish to find i,j,k from X,Y,Z
           That is, wish to find TCenter.x,TCenter.y,TCenter.z from rr.x,rr.y,rr.z */
        M = xfrm.getMatrix();
       ...
       /* b.set(0,0,rr.x - M[0][3]/iXres);
        b.set(1,0,rr.y - M[1][3]/iYres);
        b.set(2,0,rr.z - M[2][3]/iZres);
        X = A.solve(b);
        TCenter.x = (float)(X.get(0,0));
        TCenter.y = (float)(X.get(1,0));
        TCenter.z = (float)(X.get(2,0)); */
        TCenter.x = Talx/oXres;
        TCenter.y = Taly/oYres;
        TCenter.z = Talz/oZres;
        Preferences.debug("Transformed Talairach origin = " + TCenter.x + "  " + TCenter.y +
                          "  " + TCenter.z + "\n");
        ...
        /* Now consider the ray from the AC posterior margin (posteriorMarginDicom)
           in the gamma direction, and the ray from the AC superior edge
           (superiorEdgeDicom) in the beta direction.  Nominally, these rays
        ...
        if (alsoAcpc) {
            ...
            if (warpData != null) {
                ...
                xfrm.setTranslate(center.x, center.y, center.z);
                Preferences.debug("center.x = " + center.x + " center.y = " + center.y +
                                  " center.z = " +
                                  center.z + "\n");
                // Since our interpolation routines are doing a output to input mapping, create the mbac
                // transformation matrix which is the transpose(also the inverse) of the mfor matrix.
                ...
                TCenter.y = Taly / oYres;
                TCenter.z = Talz / oZres;
                Preferences.debug("Transformed Talairach origin = " + TCenter.x + "  " +
                                  TCenter.y +
                                  "  " + TCenter.z + "\n");
                ...
                size = (float) (Math.acos((double) size) * 180.0 / Math.PI); // report angle
                Preferences.debug("Angular deviation between AC+PC+mid-sag pts: " + size +
                                  " degrees\n");
                ...
                origAC.x = alignment[0][0] * rr.x + alignment[0][1] * rr.y +
                           alignment[0][2] * rr.z + offset[0];
                origAC.y = alignment[1][0] * rr.x + alignment[1][1] * rr.y +
                           alignment[1][2] * rr.z + offset[1];
                origAC.z = alignment[2][0] * rr.x + alignment[2][1] * rr.y +
                           alignment[2][2] * rr.z + offset[2];
        ...
        if (readTLRC) {
            Point3Df alphaArray[] = {new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f)};
            Point3Df betaArray[] = {new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                    new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                    new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                    new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                    new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                    new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f)};
            Point3Df gammaArray[] = {new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                     new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f)};
            Point3Df rrArray[] = {new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                  new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                  new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                  new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
                                  new Point3Df(0.0f,0.0f,0.0f), new Point3Df(0.0f,0.0f,0.0f),
           ...
           Talz = Math.abs(dicomOrigin[2]);
            ...
            for (i = 0; i < 12; i++) {
                ...
                */
                ...
                xfrm.setTranslate(Tx,Ty,Tz);
                frm = xfrm.getMatrix();
                transformTalairachTrilinear(imgBuffer, frm,
                iXres, iYres, iZres, iXdim, iYdim, iZdim,
                oXres, oYres, oZres, oXdim,
                botX[i], botY[i], botZ[i], topX[i], topY[i], topZ[i]);
                M = xfrm.getMatrix();
                A = new Matrix(3,3);
                A.set(0,0,oXres*M[0][0]/iXres);
                A.set(0,1,oYres*M[0][1]/iXres);
                ...
                b.set(1,0,rrArray[i].y - M[1][3]/iYres);
                b.set(2,0,rrArray[i].z - M[2][3]/iZres);

                X = A.solve(b);
                TalCenter.x = (float)(X.get(0,0));
                TalCenter.y = (float)(X.get(1,0));
                TalCenter.z = (float)(X.get(2,0)); */
    ...
    */
    private void readImage1()  throws IOException {
        ...
        try {
                    ...
                    if (typeAttribute == STRING_ATTRIBUTE) {
                        ...
                        if ((nameString).equalsIgnoreCase("TYPESTRING")) {
                            ...
                            if (typeString.equalsIgnoreCase("3DIM_HEAD_ANAT")) {
                                ...
                                "AFNI Read Header Error: TYPESTRING = " + typeString + "\n");
                            }
                            ...
                            if (tmpString.equalsIgnoreCase("MSB_FIRST")) {
                                ...
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: BYTEORDER_STRING has an illegal " + tmpString);
                            }
                            ...
                            Preferences.debug("VOLREG_GRIDPARENT_IDCODE = " +
                            ...
                            Preferences.debug("VOLREG_GRIDPARENT_NAME = " +
                            ...
                            Preferences.debug("VOLREG_INPUT_IDCODE = " +
                            ...
                            Preferences.debug("VOLREG_INPUT_NAME = " +
                            ...
                            Preferences.debug("VOLREG_BASE_IDCODE = " +
                            ...
                            if (countEntries < 200) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: MARKS_LAB has " + countEntries +
                            ...
                            if (countEntries < 2560) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: MARKS_HELP has " + countEntries +
                        ...
                        if (!countRead) {
                            raFile.close();
                            throw new IOException(
                            "AFNI Read Header Error: For " + nameString + " read " +
                             i + " out of " + countEntries + " integers");
                        } // if (!countRead)
                        if (nameString.equalsIgnoreCase("SCENE_DATA")) {
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: SCENE_DATA has only " +
                                countEntries + " instead of the needed 3");
                            }
                            if (presentViewType != intVar[0]) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: From SCENE_DATA view type = " + intVar[0] +
                                " but from filename presentViewType = " +
                                  presentViewType);
                            } // if (presentViewType != intVar[0])
                            ...
                            if (funcType < 0) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: funcType illegally = " + funcType);
                            }
                            ...
                            if ((typeStringType < 0) || (typeStringType > 3)) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: typeStringType illegally = " + typeStringType);
                            }
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: ORIENT_SPECIFIC has only " +
                                countEntries + " instead of the needed 3");
                            }
                            ...
                            if ((orientSpecific[0] != FileInfoBase.ORI_R2L_TYPE) && (orientSpecific[0] != FileInfoBase.ORI_L2R_TYPE) &&
                                (orientSpecific[1] != FileInfoBase.ORI_R2L_TYPE) && (orientSpecific[1] != FileInfoBase.ORI_L2R_TYPE) &&
                                (orientSpecific[2] != FileInfoBase.ORI_R2L_TYPE) && (orientSpecific[2] != FileInfoBase.ORI_L2R_TYPE)) {
                                    raFile.close();
                                    throw new IOException(
                                    "AFNI Read Header Error: ORIENT_SPECIFIC = " + orientSpecific[0] +
                                    "  " + orientSpecific[1] + "  " + orientSpecific[2]);
                            }
                            if ((orientSpecific[0] != FileInfoBase.ORI_P2A_TYPE) && (orientSpecific[0] != FileInfoBase.ORI_A2P_TYPE) &&
                                (orientSpecific[1] != FileInfoBase.ORI_P2A_TYPE) && (orientSpecific[1] != FileInfoBase.ORI_A2P_TYPE) &&
                                (orientSpecific[2] != FileInfoBase.ORI_P2A_TYPE) && (orientSpecific[2] != FileInfoBase.ORI_A2P_TYPE)) {
                                    raFile.close();
                                    throw new IOException(
                                    "AFNI Read Header Error: ORIENT_SPECIFIC = " + orientSpecific[0] +
                                    "  " + orientSpecific[1] + "  " + orientSpecific[2]);
                            }
                            if ((orientSpecific[0] != FileInfoBase.ORI_I2S_TYPE) && (orientSpecific[0] != FileInfoBase.ORI_S2I_TYPE) &&
                                (orientSpecific[1] != FileInfoBase.ORI_I2S_TYPE) && (orientSpecific[1] != FileInfoBase.ORI_S2I_TYPE) &&
                                (orientSpecific[2] != FileInfoBase.ORI_I2S_TYPE) && (orientSpecific[2] != FileInfoBase.ORI_S2I_TYPE)) {
                                    raFile.close();
                                    throw new IOException(
                                    "AFNI Read Header Error: ORIENT_SPECIFIC = " + orientSpecific[0] +
                                    "  " + orientSpecific[1] + "  " + orientSpecific[2]);
                            ...
                            if (countEntries < 2) {
                                Preferences.debug(
                                "AFNI Read Header Error: MARKS_FLAG has only " +
                                countEntries + " instead of the needed 2\n");
                            }
                            if (countEntries >= 1) {
                              switch (intVar[0]) {
                                case MARKSET_ALIGN:
                                ...
                                case MARKSET_BOUNDING:
                                  Preferences.debug(
                                      "MARKS_FLAG[0] = 2 for MARKSET_BOUNDING\n");
                                  marksFlag = intVar[0];
                                  break;
                                default:
                                  Preferences.debug("MARKS_FLAG[0] = " +
                                                    intVar[0] + "\n");
                                  marksFlag = -1;
                              }
                            ...
                            if (countEntries >= 2) {
                              if (intVar[1] != 1) { // value should always be 1
                                Preferences.debug("MARKS_FLAG[1] = " +
                                                  intVar[1] + "\n");
                              }
                            } // if (countEntries >= 2)
                        } // else if (nameString.equalsIgnoreCase("MARKS_FLAG"))
                            ...
                            if (countEntries < 2) {
                                Preferences.debug(
                                "AFNI Read Header Error: MARKS_FLAGS has only " +
                                countEntries + " instead of the needed 2\n");
                            }
                            if (countEntries >= 1) {
                              switch (intVar[0]) {
                                ...
                                case MARKSET_BOUNDING:
                                  Preferences.debug(
                                      "MARKS_FLAGS[0] = 2 for MARKSET_BOUNDING\n");
                                  marksFlags = intVar[0];
                                  break;
                                default:
                                  Preferences.debug("MARKS_FLAGS[0] = " +
                                                    intVar[0] + "\n");
                                  marksFlags = -1;
                              }
                            ...
                            if (countEntries >= 2) {
                              if (intVar[1] != 1) { // value should always be 1
                                Preferences.debug("MARKS_FLAGS[1] = " +
                                                  intVar[1] + "\n");
                              }
                            } // if (countEntries >= 2)
                        } // else if (nameString.equalsIgnoreCase("MARKS_FLAGS"))
                            ...
                            if (countEntries < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: DATASET_RANK has only " +
                                countEntries + " instead of the needed 2");
                            }
                            if (intVar[0] != 3) { // number of spatial dimensions must be 3
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: DATASET_RANK[0] illegally = " + intVar[0]);
                            }
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: DATASET_DIMENSIONS has only " +
                                countEntries + " instead of the needed 3");
                            }
                            ...
                            if (xDim < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: xDim illegally = " + xDim);
                            }
                            ...
                            if (yDim < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: yDim illegally = " + yDim);
                            }
                            ...
                            if (zDim < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: zDim illegally = " + zDim);
                            }
                            ...
                            for (i = 0; i < countEntries - 1; i++) {
                                if (intVar[i] != intVar[i+1]) {
                                    raFile.close();
                                    throw new IOException(
                                    "AFNI Read Header Error: Cannot currently handle AFNI files " +
                            ...
                            brickType = intVar[0];
                            switch(brickType){
                            case  0: fileInfo.setDataType(ModelStorageBase.UBYTE);
                                    Preferences.debug("DataType = UBYTE\n");
                                    break;
                            case  1: fileInfo.setDataType(ModelStorageBase.SHORT);
                                    Preferences.debug("DataType = SHORT\n");
                                    break;
                            case  2: fileInfo.setDataType(ModelStorageBase.INTEGER);
                                    Preferences.debug("DataType = INTEGER\n");
                                    break;
                            case 3: fileInfo.setDataType(ModelStorageBase.FLOAT);
                                    Preferences.debug("DataType = FLOAT\n");
                                    break;
                            case 4: fileInfo.setDataType(ModelStorageBase.DOUBLE);
                                    Preferences.debug("DataType = DOUBLE\n");
                                    break;
                            case 5: fileInfo.setDataType(ModelStorageBase.COMPLEX);
                                    Preferences.debug("DataType = COMPLEX\n");
                                    break;
                            case 6: fileInfo.setDataType(ModelStorageBase.ARGB);
                                    Preferences.debug("DataType = ARGB\n");
                                    break;
                            default:
                                    raFile.close();
                                    throw new IOException(
                                    "AFNI Read Header Error: BRICK_TYPES[0] illegally = " + brickType);
                            }
                        } // else if (nameString.equalsIgnoreCase("BRICK_TYPES"))
                            ...
                            if ((warpType != WARP_AFFINE_TYPE) && (warpType != WARP_TALAIRACH_12_TYPE)) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: WARP_TYPE[0] illegally = " + intVar[0]);
                            }
                            switch(warpType) {
                                case 0:
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAXIS_NUMS has only " +
                                countEntries + " instead of the needed 3");
                            }
                            tDim = intVar[0];
                            if (tDim < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error:TAXIS_NUMS[0] illegally = " + tDim);
                            }
                            ...
                            if ((timeStepUnit != UNITS_MSEC_TYPE) && (timeStepUnit != UNITS_SEC_TYPE) &&
                                ...
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAXIS_NUMS[2] illegally = " + timeStepUnit);
                            }
                            switch(timeStepUnit) {
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TO3D_ZPAD has only " +
                                countEntries + " instead of the needed 3");
                            }
                            zeroPad = new int[3];
                            zeroPad[0] = intVar[0];
                            zeroPad[1] = intVar[1];
                            zeroPad[2] = intVar[2];
                            Preferences.debug("Zero padding x = " + zeroPad[0] + " y = " + zeroPad[1] +
                                                 " z = " + zeroPad[2]);
                        } // else if (nameString.equalsIgnoreCase("TO3D_ZPAD"))
                            ...
                            if (countEntries < 2) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAGSET_NUM has only " +
                                countEntries + " instead of the needed 2");
                            }
                            tagNumber = intVar[0];
                        ...
                        if (!countRead) {
                            ...
                            throw new IOException(
                            "AFNI Read Header Error: For " + nameString + " read " +
                             i + " out of " + countEntries + " floats ");
                        } // if (!countRead)
                        if (nameString.equalsIgnoreCase("ORIGIN")) {
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: ORIGIN has only " +
                                countEntries + " instead of the needed 3");
                            }
                            origin[0] = floatVar[0];
                            origin[1] = floatVar[1];
                            origin[2] = floatVar[2];
                            Preferences.debug("ORIGIN X = " + origin[0] + " Y = " + origin[1] +
                                              " Z = " + origin[2] + "\n");
                        } // if (nameString.equalsIgnoreCase("ORIGIN"))
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: DELTA has only " +
                                countEntries + " instead of the needed 3");
                            }
                            delta[0] = floatVar[0];
                            delta[1] = floatVar[1];
                            delta[2] = floatVar[2];
                            Preferences.debug("DELTA X = " + delta[0] + " Y = " + delta[1] +
                            " Z = " + delta[2] + "\n");
                            ...
                            if (countEntries < 30) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: MARKS_XYZ has only " +
                                countEntries + " instead of the needed 30");
                            }
                            for (i = 0; i < 30; i++) {
                            ...
                            for (i = 0; i < countEntries; i++) {
                                ...
                                if (brickFloatFacs[i] > 0) {
                                    Preferences.debug("BRICKS_FLOAT_FACS[" + i + "] = " +
                                    brickFloatFacs[i] + " for scaling .BRIK values\n");
                                }
                                else if (brickFloatFacs[i] == 0) {
                                    Preferences.debug("BRICK_FLOAT_FACS[" + i + "] = " +
                                    brickFloatFacs[i] + " indicates unscaled .BRIK values\n");
                                }
                                else {
                                    Preferences.debug("BRICK_FLOAT_FACS[" + i + "] = " +
                                    brickFloatFacs[i] + " is an unexplained negative value\n");
                                }
                            }
                        } // else if (nameString.equalsIgnoreCase("BRICK_FLOAT_FACS"))
                            ...
                            if ((countEntries % 2) != 0) {
                                raFile.close();
                                throw new IOException(
                                "BRICK_STATS has " + countEntries + " values instead of an even number");
                            }
                            ...
                            for (i = 0; i < (countEntries/2); i++) {
                                Preferences.debug("Sub-brick[" + i +"] has minimum = " +
                                brickStats[2*i] + "  maximum = " + brickStats[2*i+1] + "\n");
                            }
                        } // else if (nameString.equalsIgnoreCase("BRICK_STATS"))
                            ...
                            if (presentViewType == FileInfoAfni.AFNI_ORIG) {
                                raFile.close();
                                throw new IOException(
                                "ERROR! Should not have WARP_DATA with a +orig file");
                            }
                            else if (presentViewType == FileInfoAfni.AFNI_ACPC) {
                                if (countEntries < 30) {
                                    raFile.close();
                                    throw new IOException(
                                    "WARP_DATA has only " + countEntries +
                            ...
                            for (i = 0; i < (countEntries/5); i++) {
                                ...
                                warpData[i*5+2] + "\t" + warpData[i*5+3] + "\t" + warpData[i*5+4] + "\n");
                            }
                        } // else if (nameString.equalsIgnoreCase("WARP_DATA"))
                            ...
                            if (countEntries < 5) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAXIS_FLOATS has only " +
                                countEntries + " instead of the needed 5");
                            }
                            ...
                            Preferences.debug("z-axis offset for slice-dependent time offsets = " +
                                              zAxisOffset + "\n");
                            fileInfo.setZAxisOffset(zAxisOffset);
                            zAxisStep = floatVar[4];
                            Preferences.debug("z-axis step for slice-dependent time offsets = " +
                                              zAxisStep + "\n");
                            ...
                            while (i < countEntries) {
                                ...
                                switch(statCode) {
                                    ...
                                    if (followingParms != 3) {
                                        raFile.close();
                                        throw new IOException(
                                        "AFNI Read Header Error: BRICK_STATAUX[ " + i + "] = " + followingParms +
                            ...
                            if (countEntries < 12) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAGALIGN_MATVEC has only " +
                            ...
                            Preferences.debug(tagAlignMatvec[0] + "\t" + tagAlignMatvec[1] + "\t" +
                                                 tagAlignMatvec[2] + "\t" + tagAlignMatvec[3] + "\t" +
                                                 tagAlignMatvec[4] + "\t" + tagAlignMatvec[5] + "n");
                            Preferences.debug(tagAlignMatvec[6] + "\t" + tagAlignMatvec[7] + "\t" +
                                                 tagAlignMatvec[8] + "\t" + tagAlignMatvec[9] + "\t" +
                                                 tagAlignMatvec[10] + "\t" + tagAlignMatvec[11] + "n");
                        } // else if (nameString.equalsIgnoreCase("TAGALIGN_MATVEC"))
                            ...
                            if (countEntries < 12) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: " +  nameString + " has only " +
                                countEntries + " instead of the needed 12");
                            }
                            ...
                            Preferences.debug(nameString + " = \n");
                            Preferences.debug(floatVar[0] + "\t" + floatVar[1] + "\t" +
                                                 floatVar[2] + "\t" + floatVar[3] + "\t" +
                                                 floatVar[4] + "\t" + floatVar[5] + "n");
                            Preferences.debug(floatVar[6] + "\t" + floatVar[7] + "\t" +
                                                 floatVar[8] + "\t" + floatVar[9] + "\t" +
                                                 floatVar[10] + "\t" + floatVar[11] + "n");
                        } // else if (nameString.substring(0,nameString.length() - 6).equalsIgnoreCase
                                                                              //  ("VOLREG_MATVEC_"))
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: VOLREG_CENTER_OLD has only " +
                                countEntries + " instead of the needed 3");
                            }
                            ...
                            if (countEntries < 3) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: VOLREG_CENTER_BASE has only " +
                                countEntries + " instead of the needed 3");
                            }
                            ...
                            if ((countEntries % 5) != 0) {
                                raFile.close();
                                throw new IOException(
                                "AFNI Read Header Error: TAGSET_FLOATS has " + countEntries +
                            ...
                            for (i = 0; i < countEntries/5; i++) {
                                ...
                                if (index >= 0) {
                                Preferences.debug("x = " + tagsetFloats[5*i] + "  y = " + tagsetFloats[5*i+1] +
                                                  "  z = " + tagsetFloats[5*i+2] + "  tag numerical value = " +
                                                  tagnum + "  sub-brick index of tag = " + index + "\n");
                                }
                                else { // index < 0
                                Preferences.debug("x = " + tagsetFloats[5*i] + "  y = " + tagsetFloats[5*i+1] +
                                                  "  z = " + tagsetFloats[5*i+2] + "  tag numerical value = " +
                                                  tagnum + "  not set flag = " + index + "\n");
                                }
                            }
                        } // else if (nameString.equalsIgnoreCase("TAGSET_FLOATS"))
                        else {
            ...
            if ((typeString.equalsIgnoreCase("3DIM_HEAD_ANAT")) && (typeStringType != FileInfoAfni.HEAD_ANAT_TYPE)) {
                throw new IOException(
                "AFNI Read Header Error: typeString = " + typeString + " but typeStringType = " +
                typeStringType);
            }
            ...
            if (subBrickNumber != brickTypeNumber) {
                throw new IOException(
                "AFNI Read Header Error: DATASET_RANK[1] = " + subBrickNumber +
            ...
            if (slicesWithTimeOffsets > 0) {
                if (tAxisOffsets == null) {
                    throw new IOException(
                    "AFNI Read Header Error: slicesWithTimeOffsets = " + slicesWithTimeOffsets +
            ...
            for (i = 0; i < 10; i++) {
                if ((marksXYZ[3*i] >= lowestX) && (marksXYZ[3*i] <= highestX) &&
                    ...
                    if ((marksLabString[i]).equalsIgnoreCase("AC superior edge")) {
                        if (presentViewType == FileInfoAfni.AFNI_ORIG) {
                            Preferences.debug(marksLabString[i] + " at X = " +
                                              marksXYZ[3 * i] +
                                              " Y = " + marksXYZ[3 * i + 1] +
                                              " Z = " + marksXYZ[3 * i + 2] +
                                              "\n");
                          ...
                          if  (presentViewType == FileInfoAfni.AFNI_ORIG) {
                              ...
                              if (doDicom) {
                                  ...
                                  posteriorMarginDicom = dicomMarker(
                                          pointMarker);
                              }
                              if ((!readACPC) && (!readTLRC)) {
                                  fileInfo.setPosteriorMargin(pointMarker);
                                  calcOriginalMarker(i);
                                  fileInfo.setOriginalPosteriorMargin(
                                          pointMarker);
                              }
                          }
                    }
                    ...
                    // points for +acpc to +tlrc transformation
                        ...
                        Preferences.debug("Unrecognized marker " +
                                          marksLabString[i] + " at X = " + marksXYZ[3*i] +
                    ...
                    // points for +orig to +acpc transformation
                    if ((marksLabString[i]).equalsIgnoreCase("AC superior edge")) {
                        Preferences.debug("Out of bounds " +
                                          marksLabString[i] + " at X = " + marksXYZ[3*i] +
                                          " Y = " + marksXYZ[3*i+1] + " Z = " + marksXYZ[3*i+2] + "\n");
                    }
                    else if ((marksLabString[i]).equalsIgnoreCase("AC posterior margin")) {
                    ...
                    // points for +acpc to +tlrc transformation
                        ...
                        Preferences.debug("Out of bounds unrecognized marker " +
                                          marksLabString[i] + " at X = " + marksXYZ[3*i] +
                                          " Y = " + marksXYZ[3*i+1] + " Z = " + marksXYZ[3*i+2] + "\n");
                    }
            ...
            if (!((presentViewType == FileInfoAfni.AFNI_ACPC) && (alsoAcpc))) {
                if ((brickFloatFacs != null) &&
                    (brickFloatFacs.length != subBrickNumber)) {
                    throw new IOException(
                            "AFNI Read Header Error: DATASET_RANK[1] = " +
                            subBrickNumber +
                            ", but BRICK_FLOAT_FACS length = " +
                            brickFloatFacs.length);
                }

                if ((brickStats != null) &&
                    (brickStats.length != 2 * subBrickNumber)) {
                    throw new IOException(
                            "AFNI Read Header Error: DATASET_RANK[1] = " +
                            subBrickNumber +
                ...
                if ((tagsetFloats != null) && (tagsetFloats.length != j)) {
                    throw new IOException(
                            "AFNI Read Header Error: tagsetFloats.length = " +
                            tagsetFloats.length +
    ...
    private ModelImage readImage2() throws IOException {
        ...
        if ((!anatType) && (brikDataType == ModelStorageBase.SHORT) &&
            (funcType != FUNC_FIM_TYPE) && (funcType != FUNC_BUCK_TYPE) &&
            (!havePosFloatFacs1)) {
            havePosFloatFacs = true;
            if (brickFloatFacs == null) {
        ...
        if ((!anatType) && (brikDataType == ModelStorageBase.UBYTE) &&
            ((funcType == FUNC_THR_TYPE) || (funcType == FUNC_COR_TYPE) ||
             (funcType == FUNC_TT_TYPE) || (funcType == FUNC_ZT_TYPE) ||
             (funcType == FUNC_BT_TYPE) || (funcType == FUNC_GT_TYPE)) &&
            (!havePosFloatFacs1)) {
            havePosFloatFacs = true;
            if (brickFloatFacs == null) {
                brickFloatFacs = new float[2];
        ...
        if (image != null) image.disposeLocal();
        image = null;
        if ((havePosFloatFacs) &&
           (brikDataType != ModelStorageBase.FLOAT) &&
           (brikDataType != ModelStorageBase.COMPLEX) &&
           (brikDataType != ModelStorageBase.DOUBLE)) {
    ...
    *   @exception IOException  if there is an error reading the file
    */
    private void readXYZBuffer(int slice, float buffer[], float dicomBuffer[],
                               float scaleFact) throws IOException {
        int i = 0;
        int j;
        int nBytes;
        byte [] byteBuffer;
          ...
          for (x = 0; x < xDim; x++) {
                ...
                for (y = 0; y < yDim; y++) {
                    ...
                    for (z = 0; z < zDim; z++) {
                        ...
                        dicomBuffer[newX + dicomXDim*newY + dicomSliceSize*newZ] =
                        buffer[x + xDim*y + sliceSize*z];
                    } // for (z = 0; z < zDim; z++)
                } // for (y = 0; y < yDim; y++)
            } // for (x = 0; x < xDim; x++)
    ...
    */
    public void writeImage(ModelImage image, FileWriteOptions options) throws IOException {
        ...
        progressBar = new ViewJProgressBar(fileName, "Writing AFNI header file...",
        ...
        if (image.getFileInfo()[0] instanceof FileInfoAfni) {
            ...
            if (AFNIViewType == 0) {
                ...
                if (Float.isInfinite(superiorEdgeX) ||
                    Float.isNaN(superiorEdgeX)) {
                    superiorEdgeX = -999999;
                }
                superiorEdgeY = superiorEdge.y;
                if (Float.isInfinite(superiorEdgeY) ||
                    Float.isNaN(superiorEdgeY)) {
                    superiorEdgeY = -999999;
                }
                superiorEdgeZ = superiorEdge.z;
                if (Float.isInfinite(superiorEdgeZ) ||
                    Float.isNaN(superiorEdgeZ)) {
                    superiorEdgeZ = -999999;
                }
                posteriorMargin = fileInfoAfni.getPosteriorMargin();
                posteriorMarginX = posteriorMargin.x;
                if (Float.isInfinite(posteriorMarginX) ||
                    Float.isNaN(posteriorMarginX)) {
                    posteriorMarginX = -999999;
                }
                posteriorMarginY = posteriorMargin.y;
                if (Float.isInfinite(posteriorMarginY) ||
                    Float.isNaN(posteriorMarginY)) {
                    posteriorMarginY = -999999;
                }
                posteriorMarginZ = posteriorMargin.z;
                if (Float.isInfinite(posteriorMarginZ) ||
                    Float.isNaN(posteriorMarginZ)) {
                    posteriorMarginZ = -999999;
                }
                inferiorEdge = fileInfoAfni.getInferiorEdge();
                inferiorEdgeX = inferiorEdge.x;
                if (Float.isInfinite(inferiorEdgeX) ||
                    Float.isNaN(inferiorEdgeX)) {
                    inferiorEdgeX = -999999;
                }
                inferiorEdgeY = inferiorEdge.y;
                if (Float.isInfinite(inferiorEdgeY) ||
                    Float.isNaN(inferiorEdgeY)) {
                    inferiorEdgeY = -999999;
                }
                inferiorEdgeZ = inferiorEdge.z;
                if (Float.isInfinite(inferiorEdgeZ) ||
                    Float.isNaN(inferiorEdgeZ)) {
                    inferiorEdgeZ = -999999;
                }
                firstPt = fileInfoAfni.getFirstPt();
                firstPtX = firstPt.x;
                if (Float.isInfinite(firstPtX) ||
                    Float.isNaN(firstPtX)) {
                    firstPtX = -999999;
                }
                firstPtY = firstPt.y;
                if (Float.isInfinite(firstPtY) ||
                    Float.isNaN(firstPtY)) {
                    firstPtY = -999999;
                }
                firstPtZ = firstPt.z;
                if (Float.isInfinite(firstPtZ) ||
                    Float.isNaN(firstPtZ)) {
                    firstPtZ = -999999;
                }
                anotherPt = fileInfoAfni.getAnotherPt();
                anotherPtX = anotherPt.x;
                if (Float.isInfinite(anotherPtX) ||
                    Float.isNaN(anotherPtX)) {
                    anotherPtX = -999999;
                }
                anotherPtY = anotherPt.y;
                if (Float.isInfinite(anotherPtY) ||
                    Float.isNaN(anotherPtY)) {
                    anotherPtY = -999999;
                }
                anotherPtZ = anotherPt.z;
                if (Float.isInfinite(anotherPtZ) ||
                    Float.isNaN(anotherPtZ)) {
                    anotherPtZ = -999999;
                }
            } // if (AFNIViewType == 0)
                ...
                if (Float.isInfinite(anteriorPtX) ||
                    Float.isNaN(anteriorPtX)) {
                    anteriorPtX = -999999;
                }
                anteriorPtY = anteriorPt.y;
                if (Float.isInfinite(anteriorPtY) ||
                    Float.isNaN(anteriorPtY)) {
                    anteriorPtY = -999999;
                }
                anteriorPtZ = anteriorPt.z;
                if (Float.isInfinite(anteriorPtZ) ||
                    Float.isNaN(anteriorPtZ)) {
                    anteriorPtZ = -999999;
                }
                posteriorPt = fileInfoAfni.getPosteriorPt();
                posteriorPtX = posteriorPt.x;
                if (Float.isInfinite(posteriorPtX) ||
                    Float.isNaN(posteriorPtX)) {
                    posteriorPtX = -999999;
                }
                posteriorPtY = posteriorPt.y;
                if (Float.isInfinite(posteriorPtY) ||
                    Float.isNaN(posteriorPtY)) {
                    posteriorPtY = -999999;
                }
                posteriorPtZ = posteriorPt.z;
                if (Float.isInfinite(posteriorPtZ) ||
                    Float.isNaN(posteriorPtZ)) {
                    posteriorPtZ = -999999;
                }
                superiorPt = fileInfoAfni.getSuperiorPt();
                superiorPtX = superiorPt.x;
                if (Float.isInfinite(superiorPtX) ||
                    Float.isNaN(superiorPtX)) {
                    superiorPtX = -999999;
                }
                superiorPtY = superiorPt.y;
                if (Float.isInfinite(superiorPtY) ||
                    Float.isNaN(superiorPtY)) {
                    superiorPtY = -999999;
                }
                superiorPtZ = superiorPt.z;
                if (Float.isInfinite(superiorPtZ) ||
                    Float.isNaN(superiorPtZ)) {
                    superiorPtZ = -999999;
                }
                inferiorPt = fileInfoAfni.getInferiorPt();
                inferiorPtX = inferiorPt.x;
                if (Float.isInfinite(inferiorPtX) ||
                    Float.isNaN(inferiorPtX)) {
                    inferiorPtX = -999999;
                }
                inferiorPtY = inferiorPt.y;
                if (Float.isInfinite(inferiorPtY) ||
                    Float.isNaN(inferiorPtY)) {
                    inferiorPtY = -999999;
                }
                inferiorPtZ = inferiorPt.z;
                if (Float.isInfinite(inferiorPtZ) ||
                    Float.isNaN(inferiorPtZ)) {
                    inferiorPtZ = -999999;
                }
                leftPt = fileInfoAfni.getLeftPt();
                leftPtX = leftPt.x;
                if (Float.isInfinite(leftPtX) ||
                    Float.isNaN(leftPtX)) {
                    leftPtX = -999999;
                }
                leftPtY = leftPt.y;
                if (Float.isInfinite(leftPtY) ||
                    Float.isNaN(leftPtY)) {
                    leftPtY = -999999;
                }
                leftPtZ = leftPt.z;
                if (Float.isInfinite(leftPtZ) ||
                    Float.isNaN(leftPtZ)) {
                    leftPtZ = -999999;
                }
                rightPt = fileInfoAfni.getRightPt();
                rightPtX = rightPt.x;
                if (Float.isInfinite(rightPtX) ||
                    Float.isNaN(rightPtX)) {
                    rightPtX = -999999;
                }
                rightPtY = rightPt.y;
                if (Float.isInfinite(rightPtY) ||
                    Float.isNaN(rightPtY)) {
                    rightPtY = -999999;
                }
                rightPtZ = rightPt.z;
                if (Float.isInfinite(rightPtZ) ||
                    Float.isNaN(rightPtZ)) {
                    rightPtZ = -999999;
                }
            } // else if (AFNIViewType == 1)
        } // if (image.getFileInfo()[0] instanceof FileInfoAfni)
        ...
        lineString = new String(String.valueOf(AFNIViewType) + " "
        ...
        for (i = 0; i <= 2; i++) {
            if ((orient[i] == FileInfoBase.ORI_L2R_TYPE) ||
                (orient[i] == FileInfoBase.ORI_P2A_TYPE) ||
        ...
        if (AFNIViewType == 0) {
            ...
            lineString = "        -999999        -999999        -999999" +
                         "        -999999        -999999\n" +
            ...
            lineString = "'AC superior edge~~~~AC posterior margin~" +
                         "PC inferior edge~~~~First mid-sag pt~~~~" +
                         "Another mid-sag pt~~~~~~~~~~~~~~~~~~~~~~" +
                         "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" +
                         "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
            line = lineString.getBytes();
            ...
            if ((marksFlags == MARKSET_ALIGN) || ((marksFlags == 0) &&
                (marksFlag == MARKSET_ALIGN))) {
            ...
            lineString = "'Most anterior point~Most posterior poin~" +
                         "Most superior point~Most inferior point~" +
            ...
            if ((marksFlags == MARKSET_BOUNDING) || ((marksFlags == 0) &&
                (marksFlag == MARKSET_BOUNDING))) {
        ...
        switch(image.getFileInfo()[0].getDataType()) {
            ...
            case ModelStorageBase.ARGB:
                ...
                break;
            default:
                throw new IOException("AFNI cannot write data type = " +
        ...
        if (image.getNDims() == 4) {
            ...
            if ((tAxisOffsets == null) || ((tAxisOffsets != null) &&
                (tAxisOffsets.length != image.getExtents()[2]))) {
