...
/**
 *  Resamples imageB to grid/position of imageA
 *  This code is ported from the AFNI software package located at http://afni.nimh.nih.gov
 *  AFNI is a set of C programs for processing, analyzing, and displaying functional
 *  MRI data.  The AFNI registration algorithm is described in the article Real-Time 3D
 *  Image Registration for Functional MRI by Robert W. Cox and Andrzej Jesmanowicz in
 *  Magnetic Resonance in Medicine 42:1014-1018(1999).  A 3D image is rotated and shifted
 *  using a shear factorization of the rotation matrix.  Combined with gradient descent
 *  (repeated linearization) on a least squares objective function, 3D image alignment
 *  for small movements can be rapidly computed.
 *
 *  If the imageA resolutions and dimensions do not match the imageB
 *  resolutions and dimensions, an AlgorithmTransform is run on imageB
 *  to produce a resampledImageB whose resolutions and dimensions match
 *  those of imageA.
 *
 *  Dr. Cox wrote:
 *  "The earlier (pre 2001) version of 3dvolreg was only good for small
 *  motions.  In late 2000, I added the -twopass options, which allow the
 *  estimation of larger movements.  It is that feature that makes the
 *  small motions statement somewhat dated now.  However, the Fourier
 *  wraparound effect is more apparent when doing large movements, which
 *  is why the Lagrangian interpolation is recommended for that case.
 *  In fact, I personally always use "-heptic -clipit".  "-Fourier" is
 *  the default because that has been true historically, but after a while
 *  I decided it was not the best choice (although it isn't bad, either)."
 *  Hence, in this code the -twopass, -heptic, and -clipit options are used.
 */

public class AlgorithmRegistrationShear extends AlgorithmBase {
    private final static int MAX_ITER = 5;
    private final static float DXY_THRESH = 0.07f; /* pixels */
    private final static float PHI_THRESH = 0.21f; /* degrees */

    private final static int DELTA_AFTER = 1;
    private final static int DELTA_BEFORE = 2;
    private final static int DELTA_FIXED = 3;

    private final static double BIG_NORM = 1.0e+38;

    private final static double PER0 = 1.09e-6;
    private final static double PER1 = 1.22e-6;
    private final static double PER2 = 1.37e-6;

    private final static double DFAC = Math.PI / 180.0;
    ...
    // private boolean clip = true; // clip resultImgB to be within range of input
    // imageB
    // private int zpad = 0; // zero pad around the edges by zpad voxels during rotations
    // These edge values will be stripped off in the output
    // private boolean twopass = true; // Do two passes of the registration algorithm
    // (1) with smoothed imageA and imageB, to
    // get a crude alignment
    // (2) with the input imageA and imageB
    private float twoblur = 2.0f; // blurring factor for pass 1 of 2 pass registration
    // This should be a number >= 2.0 (which is the
    // default).  Larger values would be reasonable if
    // pass 1 has to move imageB a long ways.
    private int twosum = 1; // leave constant at 1
    // private boolean twodup = false; // reset resultImgB origin to be the same as
    // the imageA origin
    private int edperc = 1; // for -1 there is no region around the edges of the base
    // volume A where the default weight is set to 0.
    // for 0 set the same thickness along each base edge to
    // a default weight of 0.  The thickness is given by
    // edging
    // for 1 set the same percentage of each dimension around
    // the base edges.  The percentage is given by edging.
    // For example if edging is 5.0, then 5% of a 256x256x124
    // volume means that 13 voxels on each side of the xy axes
    // will get zero weight, and 6 along the z-axis.  The
    // largest percentage value allowed is 25 percent.
    // The default value is edgperc value is 1.
    ...
    // for the iterations.  coarse_del is the size of
    // these steps in voxels and coarse_num is the number
    // of these steps along each direction (+x,-x,+y,-y,
    // +z,-z).  The default values are coarse_del = 10
    // and coarse num = 2.  If you don't want this step
    // performed, set coarse_num = 0.  Note that the amount
    // of computation grows as coarse_num**3, so don't increase
    // coarse_num past 4, or the program will run forever.
    // The program will prevent the coarse_del parameter
    // from being larger than 10% of the smallest dimension
    // of image B.
    ...
    // For twopass operation the default value is 66.  For 1 pass
    // operation the default value is 9.

    // Iterations converge when maximum movement is less than dxy voxels and maximum
    // rotation is less than dph degrees.
    private float dxy = 0.05f; // voxels
    private float dph = 0.07f; // degrees

    // del is the distance in voxel size used to compute image derivatives using finite
    // differences.
    private float del = 0.70f; // voxels

    // private boolean tshift = false; // If image B is 3D+time and has slice-dependent
    // time-offsets, then tshift = true tells the program
    // to time shift it to the average slice time-offset
    // prior to doing the spatial registration.
    // private int tshift_ignore = 0;  // The number of time points at the beginning to
    // ignore in time shifting.
    ...
    private double old_th1, old_th2, old_th3, old_dx, old_dy, old_dz,
     ...
     *
     */
    public AlgorithmRegistrationShear( ModelImage imageA, ModelImage imageB,
            ModelImage resultImgB, ViewUserInterface userInterface,
    ...
    public void runAlgorithm() {
        ...
        if ( ( nx != nxB ) || ( ny != nyB ) || ( nz != nzB ) || ( xdel != xdelB ) || ( ydel != ydelB )
                || ( zdel != zdelB ) ) {
            ...
            progressBar.setMessage( "Resampling imageB to match imageA dims and resolutions" );
            algoTrans.run();
        ...
        for ( kk = nzbot; kk < nztop; kk++ ) {
            for ( jj = nybot; jj < nytop; jj++ ) {
                for ( ii = nxbot; ii < nxtop; ii++ ) {
                    blurredA[ii + jj * nx + kk * sliceSize] += blurredB[( ii - sx ) + ( jj - sy ) * nx + ( kk - sz ) * sliceSize];
                }
            }
        }
        ...
        gaussianBlurAlgo = new AlgorithmGaussianBlur( blurredImageW, sigmas, true, false );
        gaussianBlurAlgo.setProgressBarVisible( false );
        gaussianBlurAlgo.run();
    ...
    /**
     *  Creates first pass alignment parameters roll_1, pitch_1, yaw_1,
     *  dx_1, dy_1, and dz_1.
    ...
    private void mri_3dalign_one( ModelImage im ) {
        ...
        Preferences.debug(
                "First fit: " + fit[0] + "  " + fit[1] + "  " + fit[2] + "  " + fit[3] + "  " + fit[4] + "  " + fit[5]
                + "  " + fit[6] + "\n" );
        ...
        while ( good ) {
            ...
            THD_rota_vol( tim, (float) ( fit[1] * DFAC ), (float) ( fit[2] * DFAC ), (float) ( fit[3] * DFAC ), fit[4],
                    fit[5], fit[6] );
            ...
            Preferences.debug(
                    "Delta fit: " + dfit[0] + "  " + dfit[1] + "  " + dfit[2] + "  " + dfit[3] + "  " + dfit[4] + "  "
                    + dfit[5] + "  " + dfit[6] + "\n" );
            Preferences.debug(
                    "Total fit: " + dfit[0] + "  " + fit[1] + "  " + fit[2] + "  " + fit[3] + "  " + fit[4] + "  "
                    + fit[5] + "  " + fit[6] + "\n" );
    ...
    /**
     *  Creates second pass alignment parameters roll, pitch, yaw,
     *  ddx, ddy, and ddz and performs the actual alignment
     *  @param im       ModelImage to align to base image
     */
    ...
    /**
     *  @param ModelImage im
     *  calculate initial estimate of alignment parameters
     */
     ...
     * Calculate
     *               (      [                                               2 ] )
     *           min ( sum  [ {c blurredB(i-dx,j-dy,k-dz) - blurredA(i,j,k)}  ] )
     *            c  (  ijk [                                                 ] )
     *
     ...
     *  @parma imwt   = image of weight factors to align to
    ...
    private int mri_3dalign_setup( ModelImage imbase, ModelImage imwt ) {
        ...
        if ( imwt != null ) {
            if ( ( imwt.getExtents()[0] != imbase.getExtents()[0] )
                    || ( imwt.getExtents()[1] != imbase.getExtents()[1] )
        ...
        if ( imwt == null ) {
            ...
            sigmas[2] = 3.0f * zdel;
            // Make algorithm
            gaussianBlurAlgo = new AlgorithmGaussianBlur( imageW, sigmas, true, false );
            gaussianBlurAlgo.setProgressBarVisible( false );
            gaussianBlurAlgo.run();
            //#endif
    ...
    /**
     *  produce the Choleski decomposition
     ...
     *  @param wt     = array holding weight for each data point (length veclen)
     *                  [if NULL, then a vector of all 1's is used]
     *  @param nref   = number of reference vectors, each of length veclen;
     *                  must have 1 <= nref <= veclen
     *  @param ref    = array of pointers to reference vectors, so that
     *                  ref[i][k] is the k-th component of the i-th reference,
     *                  for i=0..nref-1, k=0..veclen-1
     *
     *  @return   1 for success, -1 for failure
     *  fill array chol_fitim which points to memory
    ...
    private void THD_rota_vol( ModelImage vol, float th1, float th2, float th3,
        ...
        if ( ax[0] < 0 ) {
         ...
         if( bot >= top ) EXRETURN ;
         #endif */ /********************************/ /* 02 Feb 2001: include padding */ {
    ...
    /**
     *   Shift one row with heptic Lagrange polynomial interpolation [Nov 1998].
    ...
    void hept_shift( int n, float af, float[] f ) {
        ...
        for ( ii = ibot; ii <= itop; ii++ ) {
            ix = ii + ia;
            lcbuf[ii] = wt_m2 * f[ix - 2] + wt_m1 * f[ix - 1] + wt_00 * f[ix] + wt_p1 * f[ix + 1] + wt_p2 * f[ix + 2]
                    + wt_p3 * f[ix + 3] + wt_m3 * f[ix - 3] + wt_p4 * f[ix + 4];
        }
        ...
        for ( ii = 0; ii < ibot; ii++ ) {
            ...
            lcbuf[ii] = wt_m2 * finsm2 + wt_m1 * finsm1 + wt_00 * fins + wt_p1 * fins1 + wt_p2 * fins2 + wt_p3 * fins3
                    + wt_m3 * finsm3 + wt_p4 * fins4;
        }
    ...
    /**
     *  Shift one row with quintic Lagrange polynomial interpolation [Nov 1998].
     *  Note that heptic interpolation is about the same as Hamming-weighted
    ...
    private shear rot_to_shear( double th1,
            double th2,
            double th3,
        ...
        if ( ax1 == old_ax1 && ax2 == old_ax2 && ax3 == old_ax3 && dcode == old_dcode && th1 == old_th1
                && th2 == old_th2 && th3 == old_th3 && dx == old_dx && dy == old_dy && dz == old_dz && xdel == old_xdel
                && ydel == old_ydel && zdel == old_zdel ) {
            return shr;
        }
     ...
     *            new             3
     *
     *   The function returns a set of shears in the shear struct type.
     *
    ...
    private shear shear_xzyx( double[][] q, double[] xyzdel ) {
        ...
        t69 = t68 * q22;
        t70 = t29 * t51;
        t73 = -t37 * t38 - t36 * q13 * t29 + t37 * q13 * q32 - t44 * t45 + t47 * q23 * q31 + t50 * t51 + t54 - t55 * t11
                - t57 * t5 + t61 + t62 * t38 - t62 * t64 - t67 + t69 - t70 + q31 * t20 * q11;
        t75 = t20 * t22;
        ...
        t107 = t11 * t36;
        t109 = t38 * t22;
        t113 = t86 * t87 - t57 * t90 + 2.0 * t50 * t92 + 2.0 * t37 * t94 + t96 * t22 * t3 + t96 * q31 * t8 - 3.0 * t24
                - t96 * q22 * t102 + 3.0 * t30 + t27 - 2.0 * t36 * t26 * t3 - t19 - t62 * q32 * t107 - 2.0 * t37 * t109
                - t26 * q21 * t64;
        t118 = q32 * q22;
        t119 = t118 * q11;
        t121 = q11 * t36;
        ...
        t134 = t22 * t22;
        t141 = q31 * q13 * q32;
        t145 = -q11 * t15 * t17 * q31 + t23 * t89 + t86 * t119 + t121 * t28 - t123 * t55 + t125 * t45 + t1 * t127
                - t129 * t66 + t131 * t132 + t134 * q13 * q22 - t9 * t134 - 2.0 * t36 * t22 * t8 - t131 * t141
                - t11 * t127 + 2.0 * t47 * t125;
        ...
        t193 = -ydel * q22 * t51 * t26 - t157 * q23 * t134 - t160 * t129 * q33 + t164 * t119 + t163 * t54
                ...
                - 3.0 * ydel * q21 * t75 * q32 + t167 * t8 + 3.0 * ydel * t15 * t17 * q22 * q31 + t185 * t20 * t26
                - ydel * t16 * t18 - t190 * t28 * q31;
        t194 = zdel * q21;
        t195 = t125 * q12;
        t203 = xdel * t18;
        t206 = xdel * t17;
        ...
        t233 = t194 * t195 + t194 * t22 * t12 + t160 * t94 - t194 * q32 * t7 * t22 + t203 * q31 * t15
                ...
                + t210 * t11 * q31 - t220 * t221 * q31 - t194 * t224 + t160 * t207 * q12 + ydel * t25 * t26 - t230 * t8
                - t160 * t109;
        t238 = t194 * t36;
        t240 = ydel * t96;
        t241 = t240 * q21;
        t252 = ydel * t36;
        t264 = -t203 * t36 * t15 + t230 * t12 + 2.0 * t238 * t61 - t241 * t141 - t240 * q22 * t102 + t220 * t221 * t36
                - t160 * q31 * t87 + 2.0 * t206 * t36 * t29 * q32 - 2.0 * t252 * t22 * t8 + t164 * t87
                + t190 * t36 * t17 * t15 - t240 * t67 - 2.0 * t252 * t224 + 2.0 * t252 * q22 * t92 + t241 * t132;
        t267 = t160 * t36;
        t269 = ydel * q31;
        t275 = t252 * q21;
        t292 = -2.0 * t238 * t67 + t240 * t69 + 2.0 * t267 * t132 - t269 * q32 * t90 - t185 * t36 * t20 * t22
                + 2.0 * t275 * t94 + t230 * t10 + 2.0 * t238 * t69 + 2.0 * t252 * t195 - t230 * t4 - 2.0 * t275 * t109
                - 2.0 * t267 * t141 - 2.0 * t238 * t70 + t240 * q31 * t8 - t269 * q21 * t118 * t121
                + t160 * t96 * q13 * q32;
    ...
    /** matrix transpose
     *  @param A input matrix
     *  return B output transposed matrix
     */
