...
/**
 *  	The class generates a triangle or quad  mesh of a 2D dataset (image)
 *  	to be displayed in the surface viewer.  If the image is 3D, takes
 *		the 2D current slice.  The triangle mesh is like a relief map
 *		of the image; the higher intensities are peaks and the lower
 *		intensities are valleys.
 *
...
public class AlgorithmHeightFunction extends AlgorithmBase {
    ...
    /**
     *   Runs the algorithm; calls either calcQuadSurface or calcTriangleSurface, depending on the
     *	mesh parameter set in the constructor.
     */
    ...
    private void calcTriSurface() {
        ...
        for ( y = 0; y < yDim && !threadStopped; y = y + sampleSize ) {
            for ( x = 0; x < xDim; x = x + sampleSize ) {
                ...
                height = ( ( ( buffer[y * xDim + x] - min ) / range ) * maxBox ) - maxBox / 2.0f; // Set function height relative image size
                cVertex[i++] = new Point3f(
                        x - xDim / 2, -( y - yDim / 2 ), height );
            }
        }
