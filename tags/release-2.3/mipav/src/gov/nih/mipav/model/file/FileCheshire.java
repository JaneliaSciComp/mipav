...
public class FileCheshire extends FileBase {
    ...
    */
    public FileCheshire(ViewUserInterface _UI, String fName, String fDir, boolean show)  {
        ...
        progressBar = new ViewJProgressBar(ViewUserInterface.getReference().getProgressBarPrefix() + fileName,
                                                ViewUserInterface.getReference().getProgressBarPrefix() + "Cheshire image(s) ...", 0, 100,
                                                false, null,null);
    ...
    /**
    *	Reads an Cheshire image file by reading the header then making a FileRaw to read the image
    ...
    */
    public FileInfoCheshire  getFileInfo() {return fileInfo;}
    ...
    */
    public boolean readHeader(String imageFileName, String fileDir) throws IOException {
        ...
        if (zDim == 1 || zDim == 0) {
            ...
            mipavExtents[0] = xDim; mipavExtents[1] = yDim;

            mipavResolutions = new float[2];
            mipavResolutions[0] = xRes; mipavResolutions[1] = yRes;
        }
        else if (tDim == 1 || tDim == 0) {
            mipavExtents = new int[3];
            mipavExtents[0] = xDim; mipavExtents[1] = yDim; mipavExtents[2] = zDim;

            mipavResolutions = new float[3];
            mipavResolutions[0] = xRes; mipavResolutions[1] = yRes; mipavResolutions[2] = zRes;
        }
        else {
            mipavExtents = new int[4];
            mipavExtents[0] = xDim; mipavExtents[1] = yDim;
            mipavExtents[2] = zDim; mipavExtents[3] = tDim;

            mipavResolutions = new float[4];
            mipavResolutions[0] = xRes; mipavResolutions[1] = yRes;
            mipavResolutions[2] = zRes; mipavResolutions[3] = tRes;
        }
        ...
        if (nBytesPerVoxel == 1)      fileInfo.setDataType(ModelImage.UBYTE);
        else if (nBytesPerVoxel == 2) fileInfo.setDataType(ModelImage.SHORT);
        if (nBytesPerVoxel == 2 && scaleFactor[0] != 1.0f) fileInfo.setDataType(ModelImage.FLOAT);
