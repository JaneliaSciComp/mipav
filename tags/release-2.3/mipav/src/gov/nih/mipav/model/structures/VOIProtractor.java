...
public class VOIProtractor extends VOIBase {
    ...
    public boolean contains(int _x, int _y, boolean forceReload) {
        ...
        for (i = 0; i < nPts; i++ ) {
            if (yPts[j] <= y && y < yPts[i] && area2(xPts[i], yPts[i], xPts[j], yPts[j], x, y) >= 0 || yPts[i] <= y
                    && y < yPts[j] && area2(xPts[j], yPts[j], xPts[i], yPts[i], x, y) >= 0) {
                isInside = !isInside;
            }
            j = i;
        }
     ...
     */
    private final float area2(float ptAx, float ptAy, float ptBx, float ptBy, float ptCx, float ptCy) {
     ...
     * @param x two element array where x[0] = min extent of the protractor and x[1] = max extent of the Contour in the
     *            x dimension
     * @param y two element array where y[0] = min extent of the protractor and y[1] = max extent of the Contour in the
     *            y dimension
     * @param z two element array where z[0] = min extent of the protractor and z[1] = max extent of the Contour in the
     *            z dimension
     */
    ...
    private void computeAngle(float[] res) {
        ...
        theta = (180.0 / Math.PI) * Math.atan2((double) ( (y[1] - y[0]) * res[1]), (double) ( (x[1] - x[0]) * res[0]));
        if (snap) {
            seg1Length = (float) Math.sqrt((double) ( (x[1] - x[0]) * (x[1] - x[0]) * res[0] * res[0] + (y[1] - y[0])
                    * (y[1] - y[0]) * res[1] * res[1]));
            this.removeAllElements();
