...
/**
 * This class takes an image that is not cubic with dimension a power of
 * two plus one and processes subimage blocks that are cubic and of the
 * correct dimension.
 */
...
public class ASC_Climb3DDecompose extends AlgorithmBase {
     ...
     *   constructed.  If its value is power(2,n) where the subimage size is
     ...
     *   level.  Any value of iDepth between 0 and power(2,n)-1 may be used.
     ...
     * adaptive skeleton climbing.  The image dimensions are not restricted
     * to powers of two plus one.  Level surfaces are extracted from subimage
     * blocks that are cubic with dimension that is a power of two plus one.
     *
     * @param smoothFlag whether the generated mesh should have smoothing applied to it
     */
    public ASC_Climb3DDecompose( ModelImage image, int mode, int triMode,
            boolean blurFlag,
            float sigma, String fileName, boolean smoothFlag ) {
        super( null, image );
    ...
    private void constructLog() {
        historyString = new String(
                "ASC (" + m_iN + ", " + String.valueOf( blurFlag ) + ", " + String.valueOf( blurSigma ) + ", " + surfaceFileName + ")\n" );
    }
    ...
    /**
     * The function that the application calls to extract an isosurface of
                    ...
                    for ( int iZ = iZMin; iZ <= iZMax; iZ++ ) {
                        for ( int iY = iYMin; iY <= iYMax; iY++ ) {
                            for ( int iX = iXMin; iX <= iXMax; iX++ ) {
                                ...
                                int iSubIndex = ( iX - iXMin ) + m_iSize * (
                                        ( iY - iYMin ) + m_iSize * ( iZ - iZMin ) );
        ...
        if ( srcImage.getFileInfo()[0].getTransformID() == FileInfoBase.TRANSFORM_SCANNER_ANATOMICAL ) {
            ...
            for ( i = 0; i < m_kVArray.size(); i++ ) {
                ...
                akVertex[i] = new Point3f( kV.m_fX * fXRes * direction[0] + origin[0],
                        kV.m_fY * fYRes * direction[1] + origin[1], kV.m_fZ * fZRes * direction[2] + origin[2] );
     ...
     *            writing
     */
    public void save( String kName, boolean flip )
        throws IOException {
    ...
    /*protected void save (RandomAccessFile kOut, boolean flip) throws IOException
     ...
     float fResMax = fXRes;
     if (fYRes > fResMax) fResMax = fYRes;
     if (fZRes > fResMax) fResMax = fZRes;
     ...
     ASC_Vertex3[] akVertex = new ASC_Vertex3[m_kVertex.size()];
     int i;
     for (i = 0; i < akVertex.length; i++)
     {
     ASC_Vertex3 kV = (ASC_Vertex3)m_kVertex.get(i);
     akVertex[i] = new ASC_Vertex3(
     kV.m_fX*fXRes*direction[0] + startLocation[0],
     kV.m_fY*fYRes*direction[1] + startLocation[1],
     kV.m_fZ*fZRes*direction[2] + startLocation[2]);
     }
     ...
     byte[] bufferByte = new byte[akVertex.length*24];
     int index, tmpInt;
     for (i = 0, index=0; i < akVertex.length; i++)
     {
     ASC_Vertex3 kVertex = akVertex[i];

     tmpInt = Float.floatToIntBits(kVertex.m_fX);
     ...
     if (flip)
     tmpInt = Float.floatToIntBits(-kVertex.m_fY);
     else tmpInt = Float.floatToIntBits(kVertex.m_fY);
     bufferByte[index++] = (byte)(tmpInt >>> 24);
     bufferByte[index++] = (byte)(tmpInt >>> 16);
     bufferByte[index++] = (byte)(tmpInt >>> 8);
     bufferByte[index++] = (byte)(tmpInt   & 0xff);
     ...
     if (flip)
     tmpInt = Float.floatToIntBits(-kVertex.m_fZ);
     else
     tmpInt = Float.floatToIntBits(kVertex.m_fZ);
     bufferByte[index++] = (byte)(tmpInt >>> 24);
     bufferByte[index++] = (byte)(tmpInt >>> 16);
     ...
     //ASC_Vertex3[] akNormal = computeNormals();
     // write normals
     for (i = 0; i <  akVertex.length; i++)
     {
     //getNormal(i,kNormal);
     ...
     tmpInt = Float.floatToIntBits(kN.m_fX);
     bufferByte[index++] = (byte)(tmpInt >>> 24);
     bufferByte[index++] = (byte)(tmpInt >>> 16);
     bufferByte[index++] = (byte)(tmpInt >>> 8);
     bufferByte[index++] = (byte)(tmpInt   & 0xff);

     tmpInt = Float.floatToIntBits(kN.m_fY);
     bufferByte[index++] = (byte)(tmpInt >>> 24);
     bufferByte[index++] = (byte)(tmpInt >>> 16);
     bufferByte[index++] = (byte)(tmpInt >>> 8);
     bufferByte[index++] = (byte)(tmpInt   & 0xff);

     tmpInt = Float.floatToIntBits(kN.m_fZ);
     bufferByte[index++] = (byte)(tmpInt >>> 24);
     bufferByte[index++] = (byte)(tmpInt >>> 16);
     bufferByte[index++] = (byte)(tmpInt >>> 8);
     bufferByte[index++] = (byte)(tmpInt   & 0xff);
     ...
     // write connectivity
     kOut.writeInt( akTriangle.length * 3 );
     byte[] bufferInt = new byte[ akTriangle.length*3*4];
     for (i = 0, index=0; i <  akTriangle.length; i++) {
     ASC_Triangle3 kT = akTriangle[i];

     bufferInt[index++] = (byte)(kT.m_i0 >>> 24);
     bufferInt[index++] = (byte)(kT.m_i0 >>> 16);
     bufferInt[index++] = (byte)(kT.m_i0 >>> 8);
     bufferInt[index++] = (byte)(kT.m_i0   & 0xff);

     bufferInt[index++] = (byte)(kT.m_i1 >>> 24);
     bufferInt[index++] = (byte)(kT.m_i1 >>> 16);
     bufferInt[index++] = (byte)(kT.m_i1 >>> 8);
     bufferInt[index++] = (byte)(kT.m_i1   & 0xff);

     bufferInt[index++] = (byte)(kT.m_i2 >>> 24);
     bufferInt[index++] = (byte)(kT.m_i2 >>> 16);
     bufferInt[index++] = (byte)(kT.m_i2 >>> 8);
     bufferInt[index++] = (byte)(kT.m_i2   & 0xff);
     }
     ...
     * @exception IOException if there is an error writing to the file
     */
    public void save( RandomAccessFile kOut, boolean flip )
        throws IOException {
