...
public class FileSPM
  ...
  /**
   *	Reads an SPM image file by reading the header then making a FileRaw to read the image
  ...
  public ModelImage readImage(boolean one) throws IOException, OutOfMemoryError {
    ...
    try {
      if (one) {
        ...
        image = new ModelImage(fileInfo.getDataType(), new int[] {extents[0],
                               extents[1]}
                               ,
    ...
    if (fileInfo == null) { // if no file info yet, make it.
      ...
      if (!readHeader(fileInfo.getFileName(), fileInfo.getFileDirectory())) {
        throw (new IOException(
            "Cannot read image because of SPM header file error"));
      }
    }
  ...
  public boolean readHeader(String imageFileName, String fileDir) throws IOException {
    ...
    if (fileInfo.getSizeOfHeader() != headerSize) {
      Preferences.debug(
          "FileSPM:readHeader SPM header length != 348.\n", 2);
      return false;
    }
    ...
    for (i = 0; i < dims; i++) {
      ...
      Preferences.debug("FileSPM:readHeader. Dimension " + (i + 1) + " = " +
                        spmExtents[i] + "\n", 2);
      if (spmExtents[i] > 1) {
        numDims++;
      }
    ...
    Preferences.debug("FileSPM:readHeader. Voxel unit = " +
                      fileInfo.getVoxUnits() + "\n", 2);
    ...
    Preferences.debug("FileSPM:readHeader. Data type = " +
                      fileInfo.getDataTypeCode() + "\n", 2);
    ...
    Preferences.debug("FileSPM:readHeader. bits per pixel = " +
                      fileInfo.getBitPix() + "\n", 2);
    ...
    for (i = 0; i < numDims; i++) {
      ...
      Preferences.debug("FileSPM:readHeader. Resolutions " + (i + 1) +
                        " = " + resolutions[i] + "\n", 2);
    }
    ...
    Preferences.debug("FileSPM:readHeader. global max intensity = " +
                      fileInfo.getGLmax() + "\n", 2);

    fileInfo.setGLmin(getBufferInt(bufferByte, 144, endianess));
    Preferences.debug("FileSPM:readHeader. global min intensity = " +
                      fileInfo.getGLmin() + "\n", 2);
  ...
  /**
       *   Helper method to calculate the offset for getting only the middle SPM
  ...
  private boolean writeHeader(ModelImage image, int nImagesSaved,
    ...
    Preferences.debug("FileSPM:writeHeader - nImagesSaved = " +
                      nImagesSaved + "\n", 2);
    ...
    for (int i = 0; i < spmExtents.length; i++) {
      Preferences.debug("FileSPM:writeHeader - i = " + i + " dim = " +
                        spmExtents[i] + "\n", 2);
    }
    ...
    if (!simple) { // Must be an SPM file, can set all SPM information based on fileInfo
      ...
      for (int i = 0; i < spmExtents.length; i++) {
        setBufferShort(bufferByte, (short) spmExtents[i], 40 + i * 2,
                       endianess);
      }
      ...
      // y dimensions.
      fileInfo.setVoxUnits(FileInfoBase.getUnitsOfMeasureAbbrevStr(fileInfo.
          getUnitsOfMeasure(0)));
      ...
      for (int i = 0; i < nDims; i++) {
        setBufferFloat(bufferByte, fileInfo.getResolutions()[i], 80 + i * 4,
                       endianess);
      }
      ...
      Preferences.debug("FileSPM:writeHeader(simple): data type = " +
                        fileInfo.getDataTypeCode() + "\n", 2);
      setBufferShort(bufferByte, fileInfo.getDataTypeCode(), 70, endianess);
      switch (image.getType()) {
        case ModelStorageBase.BOOLEAN:
      ...
      Preferences.debug("FileSPM:writeHeader(simple): bits per pixel = " +
                        fileInfo.getBitPix() + "\n", 2);
  ...
  public void writeHeader4DTo3D(ModelImage image, String fileName,
                                String fileDir, FileWriteOptions options) throws
      IOException {
    int k, seq;
        ...
        for (k = 0; k < nBuffers; k++) {
          ...
          for (j = 0; j < yDim; j++) {
            for (i = 0; i < xDim; i += 4) {
              ...
              resultBuffer[j * xDim + i + 1] = buffer[ (yDim - 1 - j) * xDim +
                  i + 1];
              resultBuffer[j * xDim + i + 2] = buffer[ (yDim - 1 - j) * xDim +
                  i + 2];
              resultBuffer[j * xDim + i + 3] = buffer[ (yDim - 1 - j) * xDim +
                  i + 3];
            }
          }
   ...
   */
  public void flipTopBottom(float buffer[], FileInfoSPM fileInfo) throws
      IOException {
    int nBuffers;
    int bufferSize;
    float resultBuffer[] = null;
  ...
  /**
   *   updates the units of Measure in the file info based on the
   *   voxUnits from an SPM Header
  ...
  /**
   *   updates the units of Measure in the file info based on the
   *   voxUnits from an SPM Header.  This version simply updates
   *   a single FileInfo.  It does not have an image to attach to.
   *   @param  fileInfo -- an SPM file Info that has already been read
   */
  protected void updateUnitsOfMeasure(FileInfoSPM fileInfo) {
