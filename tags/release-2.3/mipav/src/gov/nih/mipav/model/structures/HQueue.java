...
/**
*   This class is used specifically for the watershed algorithm. It
*   is a version of Hierarcial Queue that greatly increases the speed
*   of the watershed process.
*
...
public class HQueue {
    ...
    */
    public HQueue (float hmin, float hmax, int MAX, int queueSize) {
        ...
        for (int i = 0; i < MAX; i++) {
            /*if (i < 10){
                // allocate larger arrays for small values (i.e. background)
                intVect[i] = new IntVector(imageSize/20, imageSize/40);
            }
            else if ( i >= 10 && i <= 50){
                intVect[i] = new IntVector(imageSize/40, imageSize/80);
            }
            else if ( i >= 51 && i <= 100){
    ...
    *   @param GradImIn float value (gradient magnitude of the image at the
    *                   location imgIndex;
    */
    ...
    *   @param index indicates a specific location in the queue
    *   return  last value (image index) at a specific queue location
    */
    ...
    /*
    *   first   - returns the last value in the first queue location that
    *             has image indexes in it. (LIFO)
    *   @return   last value (image index) at a specific queue location
    */
    public final int first(){

        int pt =  intVect[ptr].lastElement();  /// Get most recent (LIFO)
