...
/**
 * Approximate a set of points by a quadric surface.  The surface is
 * implicitly represented as the quadratic equation
 * <pre>
 *   0 = C[0] + C[1]*X + C[2]*Y + C[3]*Z + C[4]*X^2 + C[5]*Y^2
 *       + C[6]*Z^2 + C[7]*X*Y + C[8]*X*Z + C[9]*Y*Z
 * </pre>
 * The coefficients are estimated using a least-squares algorithm.  There is
 * one degree of freedom in the coefficients.  It is removed by requiring
 * the vector C = (C[0],...,C[9]) to be unit length.  The least-square problem
 * is to minimize E(C) = C^t M C with M = (sum_i V_i)(sum_i V_i)^t and
 * <pre>
 *   V = (1, X, Y, Z, X^2, Y^2, Z^2, X*Y, X*Z, Y*Z)
 * </pre>
 * Minimum value is the smallest eigenvalue of M and C is a corresponding
 * unit length eigenvector. <br><br>
 *
 * The quadratic equation can be factored into P^T A P + B^T P + K = 0 where
 * P = (X,Y,Z), K = C[0], B = (C[1],C[2],C[3]), and A is a 3x3 symmetric
 * matrix with A00 = C[4], A11 = C[5], A22 = C[6], A01 = C[7]/2, A02 = C[8]/2,
 * and A12 = C[9]/2.  Matrix A = R^T D R where R is orthogonal and D is
 ...
 * </pre>
 * The characterization of the surface depends on the signs of the d_i.  For
 * the MRI brain segmentation, the signs d_i are all positive, thereby
 * producing an ellipsoid. <br><br>
 *
 * The quadratic equation can finally be factored into
 * <pre>
 *   (P-G)^T R^T D R (P-G) = L
 * </pre>
 * where G is the center of the surface, R is the orientation matrix, D is
 * a diagonal matrix, and L is a constant.
 */

public class AlgorithmQuadraticFit
{
    /**
     * Fit a set of 3D points with a quadric surface.  The various
     * representations of the surface can be accessed by member functions.
     *
     * @param kPoints a vector of points, each vector element is of type
     *   Point3f
     */
    public AlgorithmQuadraticFit (Vector kPoints)
    {
        ...
        (
                 m_afCoeff[4], 0.5f*m_afCoeff[7], 0.5f*m_afCoeff[8],
            0.5f*m_afCoeff[7],      m_afCoeff[5], 0.5f*m_afCoeff[9],
            0.5f*m_afCoeff[8], 0.5f*m_afCoeff[9],      m_afCoeff[6]
        );
        ...
        m_fConstant = m_kCenter.x*kB.x + m_kCenter.y*kB.y + m_kCenter.z*kB.z -
            m_afCoeff[0];
    ...
    /**
     * Get the coefficients of the quadratic equation that represents the
     * approximating quadric surface.
     *
     * @param i the coefficient index (0 <= i < 10)
     * @return the value of the coefficient
     */
    final public float getCoefficient (int i)
    {
        // assert:  0 <= i < 10
        return m_afCoeff[i];
    }
    ...
    /**
     * Get the center of the quadric surface.  This is the vector G in the
     * factorization (P-G)^T R^T D R (P-G) = L.
     *
     * @return the center point
     */
    final public Point3f getCenter ()
    {
        return m_kCenter;
    }
    
    /**
     * Get the orientation of the quadric surface.  This is the matrix R in
     * the factorization A = R^T D R where A is the coefficient matrix for
     * the quadratic term in the equation.
     *
     * @return the orientation matrix
     */
    final public Matrix3f getOrient ()
    {
        return m_kOrient;
    }
    
    /**
     * Get a diagonal entry for the quadric surface.  This is a diagonal
     * term for the matrix D in the factorization A = R^T D R where A is the
     ...
     */
    final public float getDiagonal (int i)
    {
        // assert:  0 <= i < 3
    ...
    /**
     * Get the constant term for the factored quadratic equation.  This is
     * the L term in (P-G)^T R^T D R (P-G) = L.
     *
    ...
    protected float m_afCoeff[/* 10 */];
    
    // The center G, orientation R, diagonal matrix D, and constant L in
    // the factorization (P-G)^T R^T D R (P-G) = L.
    protected Point3f m_kCenter;
    protected Matrix3f m_kOrient;
    protected float m_afDiagonal[/* 3 */];
