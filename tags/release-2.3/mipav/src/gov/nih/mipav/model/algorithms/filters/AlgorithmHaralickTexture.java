 ...
 *      This code is based on the material in the GLCM Texture Tutorial by Myrka Hall-Beyer
 *      at http://www.fp.ucalgary.ca/mhallbey/tutorial.html.
 *
 *      The Haralick features are based on the frequency of occurrence of the pixel intensity
 *      values of 2 pixels an offset distance apart.  The offset distance will almost always be
 *      1.  The frequency of occurrence is counted over a square window which is placed over each
 *      pixel interior to a band of width (window size - 1)/2 at the edge of the image, since results
 *      are only calculated for pixels over which the entire square window can be placed.  The
 *      values in this outer band have been left at zero.
 *
 *      The user selects 1 or more of the 5 direction possibilities and 1 or more of the 12
 *      operator possibilities.  The number of floating point Haralick texture images created
 *      equals the number of directions selected times the number of operators selected.  The
 *      result image dimensions are the same as the source window dimensions.
 *
 *      The five direction possibilities are north-south, northeast-southwest, east-west,
 *      southeast-northwest, and spatially invariant.  Spatially invariant is created by
 *      counting in the four directions and summing the counts.
 *
 *      The calculations start with the creation of a gray level co-occurrence matrix for each
 *      pixel position interior to the outer band for each direction.  This matrix gives the
 *      probability for each pair of 2 pixels within the square window.
 */
public class AlgorithmHaralickTexture extends AlgorithmBase {
    ...
    // If true spatially invariant direction calculated.  This is created by counting in
    // the 4 directions and summing the counts.
    private boolean invariantDir;
    // If true calculate contrast, also known as sum of squares variance
    // Sum over i,j of Probability(i,j) * (i-j) * (i-j)
    private boolean contrast;
    // If true calculate dissimilarity
    // Sum over i,j of Probability(i,j) * Absolute value(i-j)
    private boolean dissimilarity;
    // If true calculate homogeneity, also called the Inverse Difference Moment of Order 2
    // Sum over i,j of Probability(i,j)/(1 + (i-j)*(i-j))
    private boolean homogeneity;
    // If true calculate inverse difference moment of order 1
    // Sum over i,j of Probability(i,j)/(1 + abs(i-j))
    private boolean inverseOrder1;
    // If true calculate angular second moment
    // Sum over i,j of Probability(i,j) * Probability(i,j)
    private boolean asm;
    // If true calculate energy
    // Energy =  square root(asm)
    private boolean energy;
    // If true calculate maximum probability
    // maximum probability is the largest probability value within the window
    private boolean maxProbability;
    // If true calcuate entropy
    // Sum over i,j of -Probability(i,j) * ln(Probability(i,j))
    private boolean entropy;
    // If true calculate gray level co-occurrence matrix mean
    // Sum over i,j of i * Probability(i,j) = Sum over i,j of j * Probability(i,j)
    private boolean mean;
    // If true calculate glcm variance
    // Sum over i,j of Probability(i,j) * (i - glcm mean) * (i - glcm mean) =
    // Sum over i,j of Probability(i,j) * (j - glcm mean) * (j - glcm mean)
    private boolean variance;
    // If true calculate glcm standard deviation
    // glcm standard deviation = square root(glcm variance)
    private boolean standardDeviation;
    // If true calculate glcm correlation
    // Sum over i,j of Probability(i,j) * (i - glcm mean) * (j - glcm mean)/ glcm variance
    ...
    */
	public AlgorithmHaralickTexture(ModelImage[] destImg, ModelImage srcImg,
	                             int windowSize, int offsetDistance,
                                     boolean ns, boolean nesw, boolean ew,
                                     boolean senw, boolean invariantDir,
                                     boolean contrast, boolean dissimilarity,
                                     boolean homogeneity, boolean inverseOrder1,
                                     boolean asm, boolean energy,
                                     boolean maxProbability, boolean entropy,
                                     boolean mean, boolean variance,
                                     boolean standardDeviation, boolean correlation) {
            super(null, srcImg);
            destImage = destImg;
    ...
    */
    private void constructLog() {
        historyString = new String("HaralickTexture(" +
                                   String.valueOf(windowSize) + ", " +
                                   String.valueOf(offsetDistance) + ", " +
                                   String.valueOf(ns) + ", " +
                                   String.valueOf(nesw) + ", " +
                                   String.valueOf(ew) + ", " +
                                   String.valueOf(senw) + ", " +
                                   String.valueOf(invariantDir) + ", " +
                                   String.valueOf(contrast) + "," +
                                   String.valueOf(dissimilarity) + "," +
                                   String.valueOf(homogeneity) + "," +
                                   String.valueOf(inverseOrder1) + "," +
                                   String.valueOf(asm) + "," +
                                   String.valueOf(energy) + "," +
                                   String.valueOf(maxProbability) + "," +
    ...
    private void calculateHaralick() {
        ...
        try {
            ...
            MipavUtil.displayError(
                    "AlgorithmHaralickTexture: IOException on srcImage.exportData(0,sliceSize,sourceBuffer)" );
            progressBar.dispose();
            setCompleted( false );
            return;
        }
                ...
                for (iDir = 0; iDir < numDirections; iDir++) {
                    ...
                    if ((ns || invariantDir) && (!doneNS)) {
                        ...
                        for (j = y - halfWin; j <= y + halfWin - offsetDistance; j++) {
                            for (i = x - halfWin; i <= x + halfWin; i++) {
                                index = i + j*xDim;
                                nsBuffer[sourceBuffer[index]]
                                        [sourceBuffer[index+xDim]]++;
                                nsBuffer[sourceBuffer[index+xDim]]
                                        [sourceBuffer[index]]++;
                            }
                        }
                        if (ns) {
                    ...
                    if ((nesw || invariantDir) && (!doneNESW)  && (!skip)) {
                        ...
                        for (j = y - halfWin; j <= y + halfWin - offsetDistance; j++) {
                            for (i = x - halfWin + offsetDistance; i <= x + halfWin; i++) {
                                index = i + j*xDim;
                                neswBuffer[sourceBuffer[index]]
                                          [sourceBuffer[index+xDim-1]]++;
                                neswBuffer[sourceBuffer[index+xDim-1]]
                                          [sourceBuffer[index]]++;
                            }
                        }
                        if (nesw) {
                    ...
                    if ((ew || invariantDir) && (!doneEW) && (!skip)) {
                        ...
                        for (j = y - halfWin; j <= y + halfWin; j++) {
                            for (i = x - halfWin; i <= x + halfWin - offsetDistance; i++) {
                                index = i + j*xDim;
                                ewBuffer[sourceBuffer[index]]
                                        [sourceBuffer[index+1]]++;
                                ewBuffer[sourceBuffer[index+1]]
                                        [sourceBuffer[index]]++;
                            }
                        }
                        if (ew) {
                    ...
                    if ((senw || invariantDir) && (!doneSENW) && (!skip)) {
                        ...
                        for (j = y - halfWin; j <= y + halfWin - offsetDistance; j++) {
                            for (i = x - halfWin; i <= x + halfWin - offsetDistance; i++) {
                                index = i + j*xDim;
                                senwBuffer[sourceBuffer[index]]
                                          [sourceBuffer[index+xDim+1]]++;
                                senwBuffer[sourceBuffer[index+xDim+1]]
                                          [sourceBuffer[index]]++;
                            }
                        }
                        if (senw) {
                    ...
                    if (invariantDir && (!doneInvariant) && (!skip)) {
                        ...
                        for (i = 0; i < matrixSize; i++) {
                            for (j = 0; j < matrixSize; j++) {
                                invariantDirBuffer[i][j] = nsBuffer[i][j] + neswBuffer[i][j] +
                                                           ewBuffer[i][j] + senwBuffer[i][j];
                            }
                        }
                    ...
                    if (correlation) {
                        for (i = 0; i < matrixSize; i++) {
                            for (j = 0; j < matrixSize; j++) {
                                resultBuffer[currentResult][pos] +=
                                        glcm[i][j]*(i - glcmMean)*(j-glcmMean)/glcmVariance;
                            }
                        }
                        currentResult++;
                    } // if (correlation)
        ...
        for (i = 0; i < resultNumber; i++) {
            try {
               ...
               MipavUtil.displayError(
               "AlgorithmHaralickTexture: IOException on destImage[" + i +
