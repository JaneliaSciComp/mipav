...
/*  This code is a port of image noise removal filter software by Karlis Freivalds
    found at http://www.gradetools.com/karlisf with major modifications.
    In the original program the weighted sum could be applied to all pixels
    in the (2*range+1)*(2*range+1) local square.  In this program the lowest cost
    pixel at the edge of the (2*range+1)*(2*range+1) local neighborhood is found
    and that pixel and the pixels along the low cost path back to the original
    center pixel are the only pixels used in the weighted sum.  The low cost path
    uses nearest 8 neighbors in 2D or nearest 26 neighbors in 3D.  The program
    also has the option of including the pixels neighboring the low cost path
    if the difference between the neighboring pixel and the low cost pixel is
    less than threshold.  Note that the neigboring pixel is a nearest 4 in
    2D or nearest 6 in 3D.  Another major modification is that the weighting
    scheme is much simpler.  The weight only depends on the Gaussian function of
    the distance from the center pixel.  The original software only handled 2D,
    while this software does 2.5D and 3D.  The old software left boundary pixels
    unfiltered, but this is no longer the case.  The old software used median
    filtering in creating the edge graph, but the median filtering has been
    commented out in this program.

    At the start of the program RGB color space is transformed in YCrCb space
    and at the end of the program YCrCb space is transformed into RGB space.
    Hence, all of the program operations occur in YcrCb space.  Frequently Asked
    Questions About Color by Charles Poynton at http://www.poynton.com/colorFAQ.html
    reveals that the RGB to YCrCb and YCrCb to RGB conversion equations used in
    the Freivalds program are significantly different from the standard ones.
    Black and white images are not transformed into another space.

    -----------
    Description
    -----------

    The filter reduces noise in the image without blurring the edges. The
    results are much better than can be achieved by using the popular adaptive
    median or smoothing filters.  Like modern compression algorithms (including JPEG)
    the filter operates in Y,Cr, Cb color space, hence it is useful for removing
    ...
    It is based on the search of the pixel neighbor graph. The main
    steps of the algorithm are:

    - create the pixel neighbor graph, set edge weights based on the
    differences between the pixels
    - for the pixel being filtered calculate the shortest path from it to
    all the other pixels reachable with path length < d.  If a
    neighboring pixel differs in intensity by threshold or more, then
    the neighboring pixel is not added to the path.  If includeNeighbors is
    true, then the nearest 4 or nearest 6 neigbors of path pixels will also
    be added if they differ from the path pixels by less than threshold.
    However, the nearest neighbor weight will only be half that of a path pixel.
    - replace the pixel value with the weighted average of all pixels in
    the path from the low cost edge pixel to the center pixel with weights
    taken as the Gaussian function of the distance from the center pixel.

    In this way only the relevant filtering neighborhood for each pixel is
    selected. The size and shape of the neighborhood adopt to the image
    features, including curved ones and sharp corners.

    Presently the median filtering step is commented out.  If the median filtering
    should be commented back in:
    Note that the edge graph is created with a median filtered version of the
    YCrCb space.  The median filter only obtains a median of the 3 by 3 area
    if no edge is detected in any of Y, Cr, or Cb spaces.  If an edge is
    detected in any of the 3 spaces, then the Y, Cr, and Cb values are all
    unchanged.  An edge is detected if the difference between the center pixel
    and any one of its 4 nearest neighbors is greater than a threshold or if
    the center pixel is larger than all of its four neighbors by threshold/2 or
    if the center pixel is smaller than all of its four neighbors by
    threshold/2.  The Y, Cr, and Cb spaces have 3 different thresholds in
    isEdgeY - 256*deltaY, 256*deltaCr, and 256*deltaCb.

    The pixel neighbor or edge graph is an array of arrays, with the first array having
    a length equal to the number of the pixels in the image and the second array
    having 8 integers for 2D color or 8 floats for 2D black and white.  The 8
    values for 2D color are the edge weights calculated from the intensity
    differences between the center pixel and one of the 8 neighboring pixels.
    edge weight = sqrt((dy*wY)**2 + (dr*wR)**2 + (dB*wB)**2),
    where dy, dR, and dB are the Y, Cr, and Cb differences and wY, wR, and wB
    are the Y, Cr, and Cb weights derived from the Y, Cr, and Cb radiuses.
    A bigger relative radius results in a smaller weight for radiuses >= 0.1,
    which should typically be the case.  For 2D black and white the edge weights
    are simply the absolute values of the differences between the 2 pixels.

    For 2D and 2.5D the computer memory is adequate for the edge graph to encompass
    the entire image.  However, for 3D with 26 nearest neighbors an edge graph for
    an entire image would have impossibly large memory requirements.  Therefore,
    an edge graph of size (2*range+1)*(2*range+1)*(2*range+1)*26 is separately
    created for every voxel in the image.  Memory requirements are reduced, but
    at the penalty of a much larger execution time.

    After the edge graph is created, the Y, Cr, and Cb spaces are separately
    filtered.  The filter uses the pixel as the center of a square with sides
    of length 2*range + 1, with range = (int)(radius + 0.999).

    A priority queue is created with all of the queue elements having a key,
    a value, and an origin.

    After the priority queue is created, the filter table is created.

    Finally, the function enhancePixel, which returns the filtered pixel value
    at coordinate (x,y), is applied to every pixel in the image.  If neighboring
    pixels are connected to the center pixel via a low cost path, queue
    elements corresponding to the neighboring pixels are added to the priority
    queue.  If no queue element is retrieved from the queue, then the original
    value is passed unchanged. If elements are retrieved from the queue, then
    those Y, Cr, or Cb pixel values which had retrieved queue elements in the
    path from the low cost edge pixel to the center pixel are put into a weighted
    sum.  The weight is derived from the Gaussian of the distance from the
    center pixel with a pixel at a distance of radius from the center having
    a weight of Math.exp(-2).  If includeNeigbors is true, then a nearest 4
    neighbor in 2D or a nearest 6 neighbor in 3D whose difference with the
    path pixel is less than threshold is included in the weighting.  However, a
    neighbor pixel is weighted a factor of 2 less than the path pixel.

    value gives the location within the maxLength = (2*range+1)*(2*range+1) local area.
    value = range + range*(2*range + 1) is the center of the local area.
    key contains a sum of all the edgeGraph values over the low cost path
    from the center pixel to the selected pixel.
    origin is set to the value of the queue elment which provides the neighboring
    pixel on the path back to the center pixel.  The center pixel origin = -1.
    A position is only added to the priority queue if the difference in
    neighboring pixel intensities is less than threshold.  Initially, the
    seen array is set equal to 0 for all queue elements within the local area.
    After a queue element has been added and retrieved from the queue, seen is set
    equal to 2 for that position to prevent later addition to the queue.
    The variable last gives the number of queue elements in the priority queue.
    The places array of length maxLength gives the queue element number of that
    local area position or -1 if the local area position has no element present
    in the priority queue.

    The priority queue is implemented as a binary heap and the binary heap is
    implemented as an array.  A binary heap is a "nearly full" binary tree(only
    the bottom level may not be complete).  This binary heap has the property that
    for every node other than a leaf, its key value is less than or equal to
    that of its children.  In a binary heap the item of highest priority is
    always at the root of the tree or at node 0.  When the highest priority
    item at the root is removed, the item of highest priority among the remainder
    moves into the root position.  The children of node i are at 2*i + 1 and
    2*i + 2.  The parent of node i is at (int)((i-1)/2).

    Adding an element e to the priority queue:
    The places of e.value returns a number p.  p >= 0 if the queue element is
    already in the priority queue.  p = -1 if the queue element is not in the
    priority queue.  If element e is already in the queue with a smaller key
    value than the key value attached to e, then simply return without putting e
    in the queue.  If e is not in the queue and last, the number of queue elements,
    has already reached the maximum size, then return with a full queue error.
    If e is not in the queue and last is less than maxsize, set a new queue
    position i equal to last and then increment last.  If e is already in the
    queue, set i to p.  Then, place e at queue position i.  As long as i > 0 and
    the key of queue element i is less than the key of its parent at position
    (int)((i-1)/2), then interchange the parent and the child queue elements.
    Set the places of the moved queue elements to the new queue positions.

    Getting an element e from the priority queue:
    The highest priority queue element, that is the queue element with the
    smallest key value, is always at queue[0], the root of the queue.  Hence,
    queue[0] is always returned.  If last == 0, no elements are in the queue,
    so return with a priority queue is empty error message.  If last is not
    zero, set the places of the queue[0] element to -1 to indicate that it
    is no longer in the queue.  Decrement last.  If last now equals zero, then
    the queue is now empty, so simply return.  If last does not equal zero,
    then move queue[last] into the root queue[0] position.  Keep looping as
    long as the right child position, 2*i + 2, is <= last.  If the key of the
    left child position at j - 1 is less than the key of the right child position
    at j, then decrement j to select the left child.  If j == last, then
    the right child position is empty, so decrement to select the left child.
    Otherwise, the right child position at j has the smaller key so do not
    decrement j.  If the key of the parent is greater than the key of the
    child, then swap the parent and child.  Set the places of the moved queue
    elements to the new queue positions.  Stop looping when the key of the
    parent is <= the key of the child.

    --------
    Settings
    --------

    You can adjust several parameters to obtain the best noise reduction level
    for each particular image:

    - Filter radius for each Y, Cr, Cb color channel: Larger radius will
    remove more noise, however some detail also can be lost.
    - threshold - neighboring pixels will not be added to the path if
    the intensity difference >= threshold.
    - Cr and Cb 2x reduction: The resolution of Cr and Cb channels is reduced two
    times, which improves the speed of the algorithm for large radiuses with
    little loss of quality.

    Recommended settings are.
    - for high quality scanned images
    radius Y 2, Cr 2 Cb 2
    Cr and Cb 2x reduction false

    - for high quality JPEG images
    radius Y 2, Cr 3 Cb 4
    Cr and Cb 2x reduction false

    - for low quality JPEG images
    radius Y 3, Cr 4 Cb 6
    Cr and Cb 2x reduction true
*/
...
public class AlgorithmAdaptivePathSmooth extends AlgorithmBase {
    ...
    *   @param do25D
    */
    public AlgorithmAdaptivePathSmooth(ModelImage destImage, ModelImage srcImg,
                        float radiusY, float radiusCr, float radiusCb,
                        float threshold, boolean includeNeighbors, boolean reduce,
                        boolean do25D) {
    ...
    */
    ...
    /**
    *   Prepares the data and runs the algorithm for a 2D ARGB image
    *   Convert from RGB to YCrCb space, run process, and convert back
    *   from YCrCb space to RGB space.
    */
    private void calc2D(){
        ...
        try {
            ...
            progressBar = new ViewJProgressBar(srcImage.getImageName(), "Converting to Y,Cr,Cb ...",
    ...
    /**
    *   Prepares the data and runs the algorithm for a 2D black and white image
    *   Run process
    */
    private void calc2DBW(){
        ...
        try {
            ...
            progressBar = new ViewJProgressBar(srcImage.getImageName(), "Performing adaptive path smooth filter ...",
                                            0, 100, true, this, this);
        }
        catch (IOException error) {
            cleanup();
     ...
     /**
    *   Prepares the data and runs the algorithm for a 3D ARGB image
    *   Convert from RGB to YCrCb space, run process, and convert back
    *   from YCrCb space to RGB space.  Processes one slice at a time.
    */
    private void calc25D(){
        ...
        try {
            ...
            Cb = new int[length];
            imgBuffer    = new int[4*length];
            progressBar = new ViewJProgressBar(srcImage.getImageName(), "Processing slice 1...",
                                            0, 100, true, this, this);
        }
        catch (OutOfMemoryError e){
            cleanup();
    ...
    /**
    *   Prepares the data and runs the algorithm for a 3D black and white image
    ...
    */
    private void calc3D() {
        ...
        yDim    = srcImage.getExtents()[1];
        zDim    = srcImage.getExtents()[2];
        if ((radiusY < 0.1) || (radiusCr < 0.1) || (radiusCb < 0.1)) return;
        ...
        for (i = -range; i <= range; i++) {
            for (j =-range; j <= range; j++){
                for (k = -range; k <= range; k++) {
                    distMap[(i+range) + (j+range)*r2 + (k+range)*r22] =
                    (float)Math.exp(-(i*i + + j*j + k*k)/scale);
                }
            }
        }
    ...
    */
    ...
    /**
    *  Create a pixel neighbor graph, with the edge weights based on the
    *  differences between pixels.  Then, if reduce is false, the Y, Cr,
    *  and Cb spaces are separately filtered at full size.  If reduce is
    *  true, only the Y space is filtered at full size, the Y, Cr, and Cb
    *  spaces are shrunk by a factor of 2, a new pixel neighbor graph is
    *  created based on the shrunken spaces, the shrunken Cr and Cb spaces
    *  are filtered, and finally the Cr and Cb spaces are expanded back to
    ...
    */
    private void process3D(int Y[], int Cr[], int Cb[]) {
        ...
        if (!reduce) {
            ...
            AlgorithmTransform.transformTrilinear(Yf, newYf, matrix,
                    xDim, yDim, zDim, xRes, yRes, zRes,
                    newWidth, newHeight, newDepth, newXRes, newYRes, newZRes,
                    null);
            Yf = null;
            ...
            AlgorithmTransform.transformTrilinear(Cbf, newBf, matrix,
                    xDim, yDim, zDim, xRes, yRes, zRes,
                    newWidth, newHeight, newDepth, newXRes, newYRes, newZRes,
                    null);
            Cbf = null;
            ...
            AlgorithmTransform.transformTrilinear(Crf, newRf, matrix,
                    xDim, yDim, zDim, xRes, yRes, zRes,
                    newWidth, newHeight, newDepth, newXRes, newYRes, newZRes,
                    null);
            Crf = null;
            ...
            AlgorithmTransform.transformTrilinear(newBf, Cbf, matrix,
                    newWidth, newHeight, newDepth, newXRes, newYRes, newZRes,
                    xDim, yDim, zDim, xRes, yRes, zRes,
                    null);
            newBf = null;
            ...
            AlgorithmTransform.transformTrilinear(newRf, Crf, matrix,
                    newWidth, newHeight, newDepth, newXRes, newYRes, newZRes,
                    xDim, yDim, zDim, xRes, yRes, zRes,
                    null);
            newRf = null;
    ...
    */
    ...
    /**
    *  Expands a space that has been shrunk by a factor of 2 back to
    *  its original size.
    *  @param outData
    *  @param inData
    */
    ...
    /**
    *  Used to expand the first line in all shrunken spaces and to
    *  expand the last line in shrunken spaces when yDim is even.
    *  @param inData
    *  @param inP
    *  @param outData
    *  @param outP
    */
    ...
    /**
    *  Used to expand all but the beginning and ending lines in
    *  shrunken spaces.
    *  @param inData
    *  @param inP
    *  @param outData
    ...
    */
    ...
    /**
    *  If yDim is odd, used to expand the last line in shrunken space into
    *  2 lines in expanded space.
    *  @param inData
    *  @param inP
    *  @param outData
    *  @param outP
    ...
    */
    ...
    /**
    *  Always used to shrink the first line by a factor of 2 and if yDim is
    *  even used to shrink the last line by a factor of 2.
    *  @param inData
    *  @param inP
    *  @param outData
    *  @param outP
    */
    ...
    /**
    *  Used to shrink all lines but the beginning line for all spaces
    *  and the end line for spaces with even yDim
    *  @param inData
    *  @param inP
    *  @param outData
    *  @param outP
    */
    private void shrinkLine2(int inData[], int inP, int outData[], int outP) {
        ...
        for (int i = 0; i < (xDim-1)/2; i++) {
            outData[outP++] = (inData[inP] + inData[inP+1] + inData[inP+xDim] +
                               inData[inP+xDim+1]) >> 2;
            inP += 2;
        }
    ...
    /**
    *  If on an edge in Y, Cr, or Cb space, no filtering is performed.
    *  If not on an edge, median filter over the 3 by 3 local square.
    *  @param Y
    *  @param Cr
    *  @param Cb
    */
    private void medianFilter(int Y[], int Cr[], int Cb[]) {
        ...
        for (y = 1; y < (yDim - 1); y++) {
            for (x = 1; x < (xDim - 1); x++) {
                counter = x + y*xDim;
                if (isEdgeY(Y1,counter,deltaY) || isEdgeY(Cr,counter,deltaCr) ||
                    isEdgeY(Cb,counter,deltaCb)) {
                }
                else {
                    index = 0;
                    for (j = -1; j <= 1; j++) {
    ...
    /**
    *  If on an edge, do not filter.
    *  If not on an edge, median filter over the local 3 by 3 square.
    *  @param Y
    */
    ...
    /**
    *  Returns true if pixel differs from any of its 4 nearest neighbors by thr or
    *  if pixel differs from all of its 4 nearest neighbors by thr/2.
    *  Returns false otherwise.  Used in color images.
    *  @param Y
    *  @param counter
    *  @param delta
    */
    private boolean isEdgeY(int Y[], int counter, int delta) {
        ...
        if (Math.abs(c - c1) > thr) return true;
        if (Math.abs(c - c2) > thr) return true;
        if (Math.abs(c - c3) > thr) return true;
        if (Math.abs(c - c4) > thr) return true;

        int maxValue = Math.max(Math.max(Math.max(c1,c2),c3),c4);
        if (c >= maxValue + thr/2) return true;
        int minValue = Math.min(Math.min(Math.min(c1,c2),c3),c4);
        if ( c <= minValue - thr/2) return true;
    ...
    *  Returns true if pixel differs from any of its 4 nearest neighbors by thr or
    *  if pixel differs from all of its 4 nearest neighbors by thr/2.
    *  Returns false otherwise.  Used in black and white images.
    *  @param Y
    *  @param counter
    *  @param delta
    */
    ...
    /*
    *  @param buffer input buffer to be sorted
    *  Sort an array into ascending numerical order by Shell's method
    ...
    */
    ...
    /**
    *   Finds the median value of the list.  Median assumes the list of values starts at index 1, not an index of 0.
    *   (ie., 1st element is not included.)
    *   @param  list	List of numbers
    *   @return 		The median.
    *   @author parsonsd
    */
    private final int median(int list[])    {
        int N;
        int med;
    ...
    *   @author parsonsd
    */
    private final float median(float list[])    {
        int N;
        float med;
    ...
    /**
    *  Create the pixel edge graph as an array of arrays, with the first array having
    *  a length equal to the number of the pixels in the image and the second array
    *  having 8 integers.  The 8 values are the edge weights calculated from the
    *  difference between the center pixel and one of the 8 neighboring pixels.
    *  edge weight = sqrt((dy*wY)**2 + (dr*wR)**2 + (dB*wB)**2),
    *  where dy, dR, and dB are the Y, Cr, and Cb differences and wY, wR, and wB
    *  are the Y, Cr, and Cb weights derived from the Y, Cr, and Cb radiuses.
    *  A bigger relative radius results in a smaller weight for radiuses >= 0.1,
    *  which should typically be the case.  Used for color images.
    *  @param Y
    *  @param Cr
    *  @param Cb
    */
    private void createEdgeGraph(int Y[], int Cr[], int Cb[]) {
        ...
        wR *= invWeight;
        wB *= invWeight;
        threshold = (originalThreshold/3.0f)*(float)Math.sqrt(256*256*wY*wY + 358*358*wR*wR +
                                                      456*456*wB*wB);
        ...
        for (y = 0; y < height; y++) {
            for (x = 0; x < width; x++) {
                ...
                if(x>0) {
                    edgeGraph[i][0] =
                        getEdgeWeight(startY-Y[i-1],startR-Cr[i-1],startB-Cb[i-1]);
                }

                if(y>0) {
                    i1 = i-width;
                    edgeGraph[i][1] =
                        getEdgeWeight(startY-Y[i1],startR-Cr[i1],startB-Cb[i1]);
                }
    ...
    *  Create the pixel edge graph as an array of arrays, with the first array having
    *  a length equal to the number of the pixels in the image and the second array
    *  having 8 floats.  The 8 values are the edge weights =  absolute differences
    *  between neighboring pixel values.  Used for black and white images.
    *  @param Y
    */
    ...
    /**
    *  Create the pixel edge graph as an array of arrays, with the first array having
    *  a length = (2*range+1)*(2*range+1)*(2*range+1) and the second array
    *  having 26 integers.  The 26 values are the edge weights calculated from the
    *  difference between the center pixel and one of the 26 neighboring pixels.
    *  edge weight = sqrt((dy*wY)**2 + (dr*wR)**2 + (dB*wB)**2),
    *  where dy, dR, and dB are the Y, Cr, and Cb differences and wY, wR, and wB
    *  are the Y, Cr, and Cb weights derived from the Y, Cr, and Cb radiuses.
    *  A bigger relative radius results in a smaller weight for radiuses >= 0.1,
    ...
    *  Create the pixel edge graph as an array of arrays, with the first array having
    *  a length = (2*range+1)*(2*range+1)*(2*range+1) and the second array
    *  having 26 integers.  The 26 values are the edge weights =  absolute differences
    *  between neighboring pixel values.  Used for black and white images.
    *  @param Y
    */
    ...
    /**
    This method returns the edge weight based on the color differences between
    pixels
    *  @param dy
    *  @param dr
    *  @param db
...
private void filterProcess() {
   ...
   maxLength = (2*range+1)*(2*range+1);
    ...
    for (i = -range; i <= range; i++) {
        for (k = -range; k <= range; k++) {
            distMap[(i+range) + (k+range)*r2] =
            (float)Math.exp(-(i*i + k*k)/scale);
        }
    }
...
/**
*  This method returns the filtered coordinate value at coordinate (x,y)
*  for an ARGB image.  If neighboring pixels are connected to the center
*  pixel via a low cost path, queue elements corresponding to the
*  neighboring pixel are added to the priority queue.  If no queue element
*  is retrieved from the queue,  then the original value is passed unchanged.
*  If elements are retrieved from the queue, then those Y, Cr, or Cb pixel
*  values which had retrieved queue elements in the path from the low cost
*  edge pixel to the center pixel are put into a weighted sum.  The weight
*  is simply the Gaussian function of the distance from the center pixel.
*  If includeNeighbors is true, then any of the low cost path pixels' 4 nearest
*  neighbors that differ from the low cost path pixel by less than threshold
*  are added to the sum with a weighting factor half that of a low cost path
*  pixel.
*  @param x
*  @param y
*  @return result
        ...
        if (includeNeighbors) {
            for (i = 0; i < pathCount; i++) {
                if (((pathX[i] - 1) >= (x - range)) && ((pathX[i] - 1) >= 0) &&
                    (Math.abs(data[pathX[i] + pathY[i]*width] -
                              data[pathX[i] - 1 + pathY[i]*width]) < threshold)) {
                    ...
                    for (j = 0; j < neighborCount && (!alreadyIncluded); j++) {
                        if (((pathX[i] - 1) == xNeighbor[j]) &&
                             (pathY[i] == yNeighbor[j])) {
                            alreadyIncluded = true;
                        }
                    }
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i] - 1 - x + range)
                                         + r2*(pathY[i] - y + range)];
                ...
                if (((pathY[i] - 1) >= (y - range)) && ((pathY[i] - 1) >= 0) &&
                    (Math.abs(data[pathX[i] + pathY[i]*width] -
                              data[pathX[i] + (pathY[i]-1)*width]) < threshold)) {
                    alreadyIncluded = false;
                    for (j = 0; j < pathCount && (!alreadyIncluded); j++) {
                    ...
                    for (j = 0; j < neighborCount && (!alreadyIncluded); j++) {
                        if ((pathX[i] == xNeighbor[j]) &&
                            ((pathY[i] - 1) == yNeighbor[j])) {
                            alreadyIncluded = true;
                        }
                    }
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i]- x + range)
                                         + r2*(pathY[i]- 1 - y + range)];
...
/**
*  This method returns the filtered coordinate value at coordinate (x,y)
*  for a black and white image.  If neighboring pixels are connected to
*  the center pixel via a low cost path, queue elements corresponding to
*  the neighboring pixels are added to the priority queue.  If no queue
*  element is retrieved from the queue, then the original value is passed
*  unchanged. If elements are retrieved from the queue, then those pixel
*  values which had retrieved  queue elements in the path from the low
*  cost edge pixel to the center pixel are put into a weighted sum.  The
*  weight is simply the Gaussian function of the distance from the
*  center pixel.  If includeNeighbors is true, then any of the low cost path
*  pixels' 4 nearest neighbors that differ from the low cost path pixel by less
*  than threshold are added to the sum with a weighting factor half that of a
*  low cost path pixel.
*  @param x
*  @param y
*  @return result
...
private float enhancePixelBW(int x, int y) {
    ...
    if (count == 0) {
        ...
        if (includeNeighbors) {
            for (i = 0; i < pathCount; i++) {
                if (((pathX[i] - 1) >= (x - range)) && ((pathX[i] - 1) >= 0) &&
                    (Math.abs(dataBW[pathX[i] + pathY[i]*xDim] -
                              dataBW[pathX[i] - 1 + pathY[i]*xDim]) < threshold)) {
                    alreadyIncluded = false;
                    for (j = 0; j < pathCount && (!alreadyIncluded); j++) {
                        if (((pathX[i] - 1) == pathX[j]) && (pathY[i] == pathY[j])) {
                            alreadyIncluded = true;
                ...
                if (((pathY[i] - 1) >= (y - range)) && ((pathY[i] - 1) >= 0) &&
                    (Math.abs(dataBW[pathX[i] + pathY[i]*xDim] -
                              dataBW[pathX[i] + (pathY[i]-1)*xDim]) < threshold)) {
                    alreadyIncluded = false;
                    for (j = 0; j < pathCount && (!alreadyIncluded); j++) {
                        if ((pathX[i] == pathX[j]) && ((pathY[i] - 1) == pathY[j])) {
                            alreadyIncluded = true;
...
/**
*  This method returns the filtered coordinate value at coordinate (x,y,z)
*  for an ARGB image.  If neighboring pixels are connected to the center
*  pixel via a low cost path, queue elements corresponding to the
*  neighboring pixel are added to the priority queue.  If no queue element
*  is retrieved from the queue,  then the original value is passed unchanged.
...
*  edge pixel to the center pixel are put into a weighted sum.  The weight
*  is simply the Gaussian function of the distance from the center pixel.
*  If includeNeighbors is true, then any of the low cost path pixels' 6 nearest
*  neighbors that differ from the low cost path pixel by less than threshold,
*  are added to the sum with a weighting factor half that of a low cost path
*  pixel.
*  @param x
            ...
            for (i = 0; i < pathCount; i++) {
                if (((pathX[i] - 1) >= (x - range)) && ((pathX[i] - 1) >= 0) &&
                    (Math.abs(data[pathX[i] + pathY[i]*width + pathZ[i]*sliceSize] -
                              data[pathX[i] - 1 + pathY[i]*width
                                     + pathZ[i]*sliceSize]) < threshold)) {
                    alreadyIncluded = false;
                    for (j = 0; j < pathCount && (!alreadyIncluded); j++) {
                        if (((pathX[i] - 1) == pathX[j]) && (pathY[i] == pathY[j]) &&
                             (pathZ[i] == pathZ[j])) {
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i] - 1 - x + range)
                       + r2*(pathY[i] - y + range) + r22*(pathZ[i] - z + range)];
                        sum += data[pathX[i] - 1 + pathY[i]*width
                                      + pathZ[i]*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i] - 1;
                        yNeighbor[neighborCount] = pathY[i];
                ...
                if (((pathY[i] - 1) >= (y - range)) && ((pathY[i] - 1) >= 0) &&
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i]- x + range)
                        + r2*(pathY[i]- 1 - y + range) + r22*(pathZ[i] - z + range)];
                        sum += data[pathX[i] + (pathY[i]-1)*width
                                     + pathZ[i]*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i];
                        yNeighbor[neighborCount] = pathY[i] - 1;
                ...
                if (((pathZ[i] - 1) >= (z - range)) && ((pathZ[i] - 1) >= 0) &&
                    (Math.abs(data[pathX[i] + pathY[i]*width + pathZ[i]*sliceSize] -
                              data[pathX[i] + pathY[i]*width
                                     + (pathZ[i]-1)*sliceSize]) < threshold)) {
                    alreadyIncluded = false;
                    for (j = 0; j < pathCount && (!alreadyIncluded); j++) {
                        if ((pathX[i] == pathX[j]) && (pathY[i] == pathY[j]) &&
                            ((pathZ[i]-1) == pathZ[j])) {
                            alreadyIncluded = true;
                        }
                    }
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i]- x + range)
                        + r2*(pathY[i]- y + range) + r22*(pathZ[i] - 1 - z + range)];
                        sum += data[pathX[i] + pathY[i]*width
                                     + (pathZ[i]-1)*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i];
                        yNeighbor[neighborCount] = pathY[i];
...
*  weight is simply the Gaussian function of the distance from the
*  center pixel.  If includeNeighbors is true, then any of the low cost path
*  pixels' 6 nearest neighbors that differ from the low cost path pixel by less
*  than threshold are added to the sum with a weighting factor half that of a
*  low cost path pixel.
*  @param x
*  @param y
            ...
            for (i = 0; i < pathCount; i++) {
                if (((pathX[i] - 1) >= (x - range)) && ((pathX[i] - 1) >= 0) &&
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i] - 1 - x + range)
                       + r2*(pathY[i] - y + range) + r22*(pathZ[i] - z + range)];
                        sum += dataBW[pathX[i] - 1 + pathY[i]*xDim
                                      + pathZ[i]*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i] - 1;
                        yNeighbor[neighborCount] = pathY[i];
                ...
                if (((pathY[i] - 1) >= (y - range)) && ((pathY[i] - 1) >= 0) &&
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i]- x + range)
                        + r2*(pathY[i]- 1 - y + range) + r22*(pathZ[i] - z + range)];
                        sum += dataBW[pathX[i] + (pathY[i]-1)*xDim
                                     + pathZ[i]*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i];
                        yNeighbor[neighborCount] = pathY[i] - 1;
                ...
                if (((pathZ[i] - 1) >= (z - range)) && ((pathZ[i] - 1) >= 0) &&
                    ...
                    if (!alreadyIncluded) {
                        weight = distMap[(pathX[i]- x + range)
                        + r2*(pathY[i]- y + range) + r22*(pathZ[i] - 1 - z + range)];
                        sum += dataBW[pathX[i] + pathY[i]*xDim
                                     + (pathZ[i]-1)*sliceSize]*weight/2.0f;
                        divisor += weight/2.0f;
                        xNeighbor[neighborCount] = pathX[i];
                        yNeighbor[neighborCount] = pathY[i];
...
/**
*  This method finds all neighbor pixels within maxDistance from the
*  initial pixel (x,y).  For each pixel the shortest path value is
*  also obtained.  This method is for ARGB images
*  @param x
*  @param y
*  @return count
*/
...
*  This method finds all neighbor pixels within maxDistance from the
*  initial pixel (x,y).  For each pixel the shortest path value is
*  also obtained.  This method is for black and white images.
*  @param x
*  @param y
*  @return count
*/
...
/**
*  This method finds all neighbor pixels within maxDistance from the
*  initial pixel (x,y,z).  For each pixel the shortest path value is
*  also obtained.  This method is for ARGB images
*  @param x
*  @param y
*  @param z
...
private int searchBW(int x, int y, int z) {
    ...
    class PriorityQueue {
        ...
        public QueueElement getElement() {
            ...
            i = 0;
            while((j=2*i+2) <= last){
                if ((queue[j-1].key < queue[j].key)||
                    (j == last)) //  only has one child
                    j--;    //left child
               ...
               else break; // don't push further down
           }
