 ...
 *   for FMRIB's Linear Image Registration Tool 1.3.  For more information on FLIRT,
 ...
 *        Subsampling can be performed for x, y, and z or for only x and y.
 ...
 *        at every 15 degrees, we would optimize at rotations of (-30, -30, -30), (-30, -30, -15),
 *        (-30, -30, 0), etc.  In this case there would be a total of 125 calls to the optimization
 *        method.<br>
 ...
 *        into their original position in the multi-array.  We look at the 9 neighbors of a point:
 *        +, =, or - one fine sample in each of the three directions.  If our point has a cost greater
 *        than any of these, it is not a minima.  Otherwise it is.  We save it in a vector of minima.<br>
 *   8.)  We optimize the minima over rotations as well as translations and global scale.  (Previously
 *        we had not optimized over rotations.)  We return two vectors, one containing the minima
 ...
 *   10.) We perturb the rotations in each dimension by zero and plus-minus fineDelta.  If it's not a rigid
 *        transformation, we then perturb the scaling by factors of 0.8, 0.9, 1.0, 1.1, and 1.2.  <br>
 ...
 *        at the new images.  We optimize the best minimum with 7 degrees of freedom, then 9, then 12.
 *        If the user has limited the degrees of freedom to 6, there will only be one optimization run,
 *        with 6 degrees of freedom.  The function returns the best minimum after optimization.<br>
 *   13.) We call levelOne with the un-subsampled images.  At levelOne, one optimization run is performed,
 *        with the maximum allowable degrees of freedom, as specified by the user (the max is 12).<br>
 ...
 *
 *        Only subsample if 16 or more z slices are present so that the number of z slices will not be
 *        reduced below 8.
 *   @author Neva Cherniavsky
 *	@author Matthew McAuliffe
 */
public class AlgorithmConstrainedOAR3D
  ...
  /** If true this algorithm skips all subsample and goes directly to the
   *   level 1 optimization. This assumes that images are fairly well aligned to
   *   begin with and therefore no sophisticated search is needed.
   */
  private boolean fastMode = false, fullAnalysisMode = true;

  /**
   *  If true calculate the center of gravity (mass) and use the difference to
   *  intialize the translation. If false, images are pretty much aligned then
   *  don't calculated COG.
   */
   ...
   *   If true use the maximum resolution of the two dataset. Throws away information some image
   *   information but is faster.
   *   If false the algorithms uses the minimum of the resolutions when resampling the images. Can
   *   be slower but does not "lose" informaton.
   */
   ...
   * @param _fastMode      If true then searching the parameter space is not conducted and the algorithm
   *                       proceeds to level one immediately
   ...
   */
  public AlgorithmConstrainedOAR3D(ModelImage _imageA, ModelImage _imageB,
                                   int _costChoice, int _DOF, int _interp,
                                   float _rotateBeginX, float _rotateRangeX,
                                   float _rotateBeginY, float _rotateRangeY,
                                   float _rotateBeginZ, float _rotateRangeZ,
                                   int _nCoarseX, int _nCoarseY, int _nCoarseZ,
                                   float[][] _transLimits,
                                   boolean _maxResol, boolean _doSubsample,
                                   boolean _fastMode, boolean _calcCOG,
                                   int _bracketBound, int _baseNumIter,
                                   int _numMinima) {
   ...
   *   reference image; likewise, the input image is blurred if one of the reference resolutions
   ...
   *   dimensions.  If maxResol is true, that resolution will equal the maximum of the minimums of each
   *   image's resolutions: Max( Min (resolutions of ref image, resolutions of input image) ).
  ...
  public void runAlgorithm() {
    ...
    if (refImage.getNDims() != 3) {
      MipavUtil.displayError("" + refImage.getNDims() +
                             "D registration not supported.");
      disposeLocal();
      completed = false;
      return;
    }
    if (inputImage.getNDims() != 3) {
      MipavUtil.displayError("" + inputImage.getNDims() +
                             "D registration not supported.");
      disposeLocal();
      completed = false;
      return;
    ...
    if ( (resRef[0] == resRef[1]) && (resRef[0] == resRef[2]) &&
        (resRef[0] == minSample)) {
      resampleRef = false;
    }
    else {
    ...
    if ( (resInput[0] == resInput[1]) && (resInput[0] == resInput[2]) &&
        (resInput[0] == minSample)) {
      resampleInput = false;
    }
    else {
      resampleInput = true;
    ...
    if (resRef[0]*2. > maxPixSizeX || resRef[1]*2. > maxPixSizeY) allowLevel2XY = false;
    if (resRef[2]*2. > maxPixSizeZ) allowLevel2Z = false;
    ...
    if (resRef[0]*4. > maxPixSizeX || resRef[1]*4. > maxPixSizeY) allowLevel4XY = false;
    if (resRef[2]*4. > maxPixSizeZ) allowLevel4Z = false;
    if (!allowLevel4XY || !allowLevel4Z) {
      Preferences.debug("Image resolution is too low to subsample for level 4, given "
                        + "the prescribed translation limits. \n");
    }
    if (resRef[0]*8. > maxPixSizeX || resRef[1]*8. > maxPixSizeY) allowLevel8XY = false;
    if (resRef[2]*8. > maxPixSizeZ) allowLevel8Z = false;
    if (!allowLevel8XY || !allowLevel8Z) {
      Preferences.debug("Image resolution is too low to subsample for level 8, given "
                        + "the prescribed translation limits. \n");
    }
    if (resRef[0]*16. > maxPixSizeX || resRef[1]*16. > maxPixSizeY) allowLevel16XY = false;
    if (resRef[2]*16. > maxPixSizeZ) allowLevel16Z = false;
    ...
    for (i = 0; i < extentsRefIso.length; i++) {
      extentsRefIso[i] = (int) ( (refImage.getExtents()[i] - 1) /
                                (minSample / resRef[i])) + 1;
      resRefIso[i] = minSample;
      extentsInputIso[i] = (int) ( (inputImage.getExtents()[i] - 1) /
                                  (minSample / resInput[i])) + 1;
    ...
    for (i = 0; i < sigmasRef.length; i++) {
      sigmasRef[i] = ( (resInput[i] - resRef[i]) / resRef[i]) * 0.6f; // 2.36  * sigma = x -->  FWHM  0.424 = 1/2.36
      System.out.println(
          " sigmasRef[" + i + " ] = " + sigmasRef[i]);
      if (sigmasRef[i] < 0.5f) {
        sigmasRef[i] = 0.0f;
      }
      else {
      ...
      sigmasInput[i] = ( (resRef[i] - resInput[i]) / resInput[i]) * 0.6f; // 2.36  * sigma = x -->  FWHM  0.424 = 1/2.36
    ...
    Preferences.debug("Blurring ref = " + sigmasRef[0] + ", " + sigmasRef[1] +
                      ", " + sigmasRef[2] + "\n");
    Preferences.debug("Blurring inp = " + sigmasInput[0] + ", " + sigmasInput[1] +
                      ", " + sigmasInput[2] + "\n");
    constructLog();
    ...
    if (blurRef) {
      if (doColor) {
        blurredRef = new ModelImage(ModelImage.ARGB_FLOAT, refImage.getExtents(),
                                    "BlurRef",
                                    refImage.getUserInterface());
      }
      else {
        blurredRef = new ModelImage(ModelImage.FLOAT, refImage.getExtents(),
                                    "BlurRef",
                                    refImage.getUserInterface());
      }
      // update resolutions
      ...
      AlgorithmGaussianBlur blur = new AlgorithmGaussianBlur(blurredRef,
          refImage, sigmasRef, true, false);
      if (doColor) {
        blur.setRed(true);
        blur.setGreen(true);
        blur.setBlue(true);
    ...
    if (blurInput) {
      if (doColor) {
        blurredInput = new ModelImage(ModelImage.ARGB_FLOAT,
                                      inputImage.getExtents(), "BlurInput",
                                      inputImage.getUserInterface());
      }
      else {
        blurredInput = new ModelImage(ModelImage.FLOAT, inputImage.getExtents(),
                                      "BlurInput",
                                      inputImage.getUserInterface());
      }
      // update resolutions
      ...
      AlgorithmGaussianBlur blur2 = new AlgorithmGaussianBlur(blurredInput,
          inputImage, sigmasInput, true, false);
      if (doColor) {
        blur2.setRed(true);
        blur2.setGreen(true);
        blur2.setBlue(true);
    ...
    if (resampleRef) {
      transform = new AlgorithmTransform(blurredRef, new TransMatrix(4), interp,
                                         resRefIso[0], resRefIso[1],
                                         resRefIso[2], extentsRefIso[0],
                                         extentsRefIso[1], extentsRefIso[2], false, true, false);
      ...
      simpleRef = new ModelSimpleImage(imageRefIso.getExtents(),
                                       imageRefIso.getFileInfo(0).
                                       getResolutions(),
                                       imageRefIso);
    } // if (resampleRef)
    else {
      simpleRef = new ModelSimpleImage(blurredRef.getExtents(),
                                       blurredRef.getFileInfo(0).getResolutions(),
                                       blurredRef);
    }
    ...
    if ( (weighted)
      ...
      transform = new AlgorithmTransform(refWeight, new TransMatrix(4), interp,
                                         resRefIso[0], resRefIso[1],
                                         resRefIso[2], extentsRefIso[0],
                                         extentsRefIso[1], extentsRefIso[2], false, true, false);
      transform.setActiveImage(activeImage);
      ...
      simpleWeightRef = new ModelSimpleImage(imageWeightRefIso.getExtents(),
                                             imageWeightRefIso.getFileInfo(0).
                                             getResolutions(),
                                             imageWeightRefIso);
      ...
      simpleWeightRef = new ModelSimpleImage(refWeight.getExtents(),
                                             refWeight.getFileInfo(0).
                                             getResolutions(),
                                             refWeight);
    }
    ...
    if (resampleInput) {
      transform2 = new AlgorithmTransform(blurredInput, new TransMatrix(4),
                                          interp, resInputIso[0],
                                          resInputIso[1], resInputIso[2],
                                          extentsInputIso[0], extentsInputIso[1],
                                          extentsInputIso[2], false,
                                          true, false);
      ...
      simpleInput = new ModelSimpleImage(imageInputIso.getExtents(),
                                         imageInputIso.getFileInfo(0).
                                         getResolutions(), imageInputIso);
    } // if (resampleInput)
    else {
      simpleInput = new ModelSimpleImage(blurredInput.getExtents(),
                                         blurredInput.getFileInfo(0).
                                         getResolutions(), blurredInput);
    }
    ...
    if ( (weighted)
      ...
      transform2 = new AlgorithmTransform(inputWeight, new TransMatrix(4),
                                          interp, resInputIso[0],
                                          resInputIso[1], resInputIso[2],
                                          extentsInputIso[0], extentsInputIso[1],
                                          extentsInputIso[2], false,
      ...
      simpleWeightInput = new ModelSimpleImage(imageWeightInputIso.getExtents(),
                                               imageWeightInputIso.getFileInfo(
    ...
    if (weighted) {
      ...
      if (weightedRefPixels > subMinFactor &&
          weightedInputPixels > subMinFactor
          && simpleWeightRef.zDim >= minimumZForSub &&
          simpleWeightInput.zDim >= minimumZForSub && doSubsample &&
          allowLevel2XY && allowLevel2Z) {
        simpleWeightRefSub2 = subsampleBy2(simpleWeightRef, false);
        simpleWeightInputSub2 = subsampleBy2(simpleWeightInput, false);
      }
      else if (weightedRefPixels > subMinFactor &&
               weightedInputPixels > subMinFactor && doSubsample &&
               allowLevel2XY) {
    ...
    if (weighted && fullAnalysisMode) { // If in fast mode, skip resampling because won't do levelEight, levelFour and levelTwo
      ...
      if (weightedRefPixelsSub2 > subMinFactor &&
          weightedInputPixelsSub2 > subMinFactor &&
          simpleWeightRefSub2.zDim >= minimumZForSub &&
          simpleWeightInputSub2.zDim >= minimumZForSub && doSubsample &&
          allowLevel4XY && allowLevel4Z) {
        simpleWeightRefSub4 = subsampleBy2(simpleWeightRefSub2, false);
        simpleWeightInputSub4 = subsampleBy2(simpleWeightInputSub2, false);
      }
      else if (weightedRefPixelsSub2 > subMinFactor &&
               weightedInputPixelsSub2 > subMinFactor && doSubsample &&
               allowLevel4XY) {
      ...
      if (weightedRefPixelsSub4 > subMinFactor &&
          weightedInputPixelsSub4 > subMinFactor
          && simpleWeightRefSub4.zDim >= minimumZForSub &&
          simpleWeightInputSub4.zDim >= minimumZForSub
        ...
        if (weightedRefPixelsSub8 > subMinFactor &&
            weightedInputPixelsSub8 > subMinFactor
            && simpleWeightRefSub8.zDim >= minimumZForSub &&
            simpleWeightInputSub8.zDim >= minimumZForSub
            && doSubsample && allowLevel16XY && allowLevel16Z) {
        ...
        else if (weightedRefPixelsSub8 > subMinFactor &&
                 weightedInputPixelsSub8 > subMinFactor
      ...
      else if (weightedRefPixelsSub4 > subMinFactor &&
               weightedInputPixelsSub4 > subMinFactor && doSubsample &&
               allowLevel8XY) {
        ...
        if (weightedRefPixelsSub8 > subMinFactor &&
            weightedInputPixelsSub8 > subMinFactor && doSubsample &&
            allowLevel16XY) {
          ModelSimpleImage simpleWeightRefSub16;
          ModelSimpleImage simpleWeightInputSub16;
          Preferences.debug("Sub sampled level 8 to 16 in XY ***********\n");
      ...
      Preferences.debug(
          "Weighted ref subsampled 2 = " + simpleWeightRefSub2 +
          "Weighted ref subsampled 4 = "
          + simpleWeightRefSub4 + "Weighted ref subsampled 8 = " +
          simpleWeightRefSub8
          + "Weighted input subsampled 2 = " + simpleWeightInputSub2 +
          "Weighted input subsampled 4 = "
          + simpleWeightInputSub4 + "Weighted input subsampled 8 = " +
    ...
    if (fullAnalysisMode) {
      ...
      if (simpleRef.dataSize > subMinFactor &&
          simpleInput.dataSize > subMinFactor &&
          allowLevel2XY && allowLevel2Z &&
          simpleRef.zDim >= minimumZForSub &&
          simpleInput.zDim >= minimumZForSub &&
          doSubsample) {
      ...
      else if (simpleRef.dataSize > subMinFactor &&
               simpleInput.dataSize > subMinFactor &&
               allowLevel2XY &&
               doSubsample) {
        ...
        if (simpleRef.dataSize <= subMinFactor) {
          ...
          if (simpleInput.dataSize <= subMinFactor) {
            ...
            Preferences.debug(
                "resolution too low for user defined translation limits. \n");
          }
        }
      ...
      if (simpleRefSub2.dataSize > subMinFactor &&
          simpleInputSub2.dataSize > subMinFactor && allowLevel4XY
          && allowLevel4Z && simpleRefSub2.zDim >= minimumZForSub &&
          simpleInputSub2.zDim >= minimumZForSub
          && doSubsample) {
      ...
      else if (simpleRefSub2.dataSize > subMinFactor &&
               simpleInputSub2.dataSize > subMinFactor
      ...
      if (simpleRefSub4.dataSize > subMinFactor &&
          simpleInputSub4.dataSize > subMinFactor &&
          allowLevel8XY &&
          allowLevel8Z &&
          simpleRefSub4.zDim >= minimumZForSub &&
          simpleInputSub4.zDim >= minimumZForSub &&
          doSubsample) {
        ...
        // For really big images subsample level 8 again!
        if (simpleRefSub8.dataSize > subMinFactor &&
            simpleInputSub8.dataSize > subMinFactor && allowLevel16XY
            && allowLevel16Z && simpleRefSub8.zDim >= minimumZForSub
        ...
        else if (simpleRefSub8.dataSize > subMinFactor &&
                 simpleInputSub8.dataSize > subMinFactor
                 && allowLevel16XY ) {
          ModelSimpleImage simpleRefSub16;
          ModelSimpleImage simpleInputSub16;
          Preferences.debug("Sub sampled level 8 to 16 in XY ***********\n");
      ...
      else if (simpleRefSub4.dataSize > subMinFactor &&
               simpleInputSub4.dataSize > subMinFactor
      ...
      Preferences.debug(
          "Level 1 factor XY = " + level1FactorXY + "\n" +
          ...
          + "Ref subsampled 2 = " + simpleRefSub2 + "Ref subsampled 4 = " +
          simpleRefSub4
          + "Ref subsampled 8 = " + simpleRefSub8 + "Input subsampled 2 = " +
          simpleInputSub2
          + "Input subsampled 4 = " + simpleInputSub4 + "Input subsampled 8 = " +
          simpleInputSub8);
      ...
      Preferences.debug(
          " Starting level 4 ************************************************\n");
      Vector minima = levelFour(simpleRefSub4, simpleInputSub4, minimas[0],
                                minimas[1]);
      time = System.currentTimeMillis() - time;
      ...
      double diffZ = 0;
      if (calcCOG == true) {
        Point3Dd cog = calculateCenterOfMass3D(simpleInput, simpleWeightInput,
                                               doColor);
        Point3Dd cogR = calculateCenterOfMass3D(simpleRef, simpleWeightRef,
                                                doColor);
        Preferences.debug("Center of mass for the subsampled input image:" +
                          cog + "\n");
        Preferences.debug("Center of mass for the subsampled reference image:" +
                          cogR + "\n");
      ...
      bestGuessLevel2.initial[0] = bestGuessLevel2.initial[1] = bestGuessLevel2.
          initial[2] = 0; // initial rotation
      bestGuessLevel2.initial[3] = diffX; // initial translations
      bestGuessLevel2.initial[4] = diffY;
      bestGuessLevel2.initial[5] = diffZ;
      bestGuessLevel2.initial[6] = bestGuessLevel2.initial[7] = bestGuessLevel2.
          initial[8] = 1; // initial scaling
      bestGuessLevel2.initial[9] = bestGuessLevel2.initial[10] =
          bestGuessLevel2.initial[11] = 0; // initial skewing
    } // end of (fastMode)
...
// image resolution.
  ...
  private String getConstructionInfo() {
    ...
    s = new String("RegistrationOAR3D(" + refImage.getImageName() + ", " +
    ...
    s += rotateBeginX + ", " + coarseRateX + ", " + fineRateX + ", " +
        rotateBeginY + ", " + coarseRateY + ", "
        + fineRateY + ", " + rotateBeginZ + ", " + coarseRateZ + ", " +
        fineRateZ + ", " + maxResol + ", "
        + fastMode + ", " + calcCOG + ")\n";
    historyString += rotateBeginX + ", " + coarseRateX + ", " + fineRateX +
        ", " + rotateBeginY + ", " + coarseRateY
        + ", " + fineRateY + ", " + rotateBeginZ + ", " + coarseRateZ + ", " +
        fineRateZ + ", " + maxResol
        + ", " + fastMode + ", " + calcCOG + ")\n";
  ...
  /**
   * Accessor that returns the z rot and x and y trans from the
   * matrix calculated in this algorithm.
   * @return  z rotation and x and y translations from the matrix found
   ...
   *   of (-30, -30, -30), (-30, -30, -15), (-30, -30, 0), etc.  In this case
   *   there would be a total of 125 calls to the optimization method.
   *   Measures the cost at the fine sampling rate.  Interpolates the translations
   *   and global scale to come up with a good guess as to what the optimized
   *   translation would be at that point.
   *   Takes the top 20% of the points and optimizes them.
   *   Now have a large multi-array of costs.  20% of those have been optimized
   *   and placed back into their original position in the multi-array.  Removes
   *   those items that are outisde the rotation begin and end limits.  Looks
   *   at the 8 neighbors of a point: +, =, or - one fine sample in each of the
   *   three directions.  If the point has a cost greater than any of these, it
   *   is not a minima.  Otherwise it is.  Saves it in a vector of minima.
   *   Optimizes the minima over rotations as well as translations and global scale.
   *   (Previously had not optimized over rotations.)  Returns two vectors, one
  ...
  private Vector[] levelEight(ModelSimpleImage ref, ModelSimpleImage input) {
    ...
    /*
     // To test the amount of time for a single cost evaluation at this level:
     timeNow = System.currentTimeMillis();
     initialCost = cost.cost(tMatrix);
     timeLater = System.currentTimeMillis();
     timeElapsed = timeLater - timeNow;
     Preferences.debug(" LEVEL EIGHT \n");
     Preferences.debug(" Time for single cost function evaluation: " + ( (float) timeElapsed / 1000.0f) +
     " seconds \n");

     // Initial amount of overlap.
     if (costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED_WGT) {
     double overAmt[] = new double[] {0, 0, 0};
     overAmt = cost.getOverlap();
     Preferences.debug(" Initial overlap amount " + (int) (overAmt[0]) + " of " + (int) (overAmt[1])
     + " voxels, yielding " + (int) (overAmt[2]) + " percent.\n");
     }
     */
    ...
    Preferences.debug(
        "Total number of positions at coarse sampling to be tested: " +
        numberOfCoarseAngles + ".\n");
    Preferences.debug(
        "Total number of positions at fine sampling to be tested: " +
         numberOfFineAngles + ".\n");
    ...
    double[][][][] transforms = new double[coarseNumX][coarseNumY][coarseNumZ][
        4];
    ...
    if (DOF > 6) {
      powell = new AlgorithmConstPowellOpt3D(this, cog, 4, cost, initial,
                                             getTolerance(4), maxIter,
                                             bracketBound);
    }
    else {
      powell = new AlgorithmConstPowellOpt3D(this, cog, 3, cost, initial,
                                             getTolerance(3), maxIter,
                                             bracketBound);
    }
    ...
    // Fine sampling
    MatrixListItem[][][] matrixList = new MatrixListItem[fineNumX][fineNumY][
        fineNumZ];
      ...
      for (int j = 0; j < fineNumY && !threadStopped; j++) {
        ...
        for (int k = 0; k < fineNumZ && !threadStopped; k++) {
          ...
          if (testBounds3D(initial, initialMessage)) {
            ...
            Preferences.debug(
                "Setting the cost to the maximum possible for fine increment, " +iG +".\n");
          }
          else {
            powell.setInitialPoint(initial);
            powell.measureCost();
            matrixList[i][j][k] = new MatrixListItem(powell.getCost(),
                powell.getMatrix(), powell.getFinal());
          }
          ...
          for (int itest = -1; itest <= 1 && minimum; itest++) {
            ...
            for (int jtest = -1; jtest <= 1 && minimum; jtest++) {
              for (int ktest = -1; ktest <= 1 && minimum; ktest++) {
                if ( (i + itest >= 0) && (i + itest < fineNumX) &&
                    (j + jtest >= 0)
                    && (j + jtest < fineNumY) && (k + ktest >= 0) &&
                    (k + ktest < fineNumZ)) {
                  if (matrixList[i][j][k].cost >
                      matrixList[i + itest][j + jtest][k + ktest].cost ||
    ...
    maxIter = baseNumIter * 2;
    powell = new AlgorithmConstPowellOpt3D(this, cog, degree, cost, initial,
                                           getTolerance(degree), maxIter,
                                           bracketBound);
    powell.setActiveImage(activeImage);
    powell.setLimits(limits);
    MatrixListItem item;
    for (Enumeration en = minima.elements();
         en.hasMoreElements() && !threadStopped; ) {
      progressBar.updateValue(20 + ( (count + 1) / minima.size() * 5),
                              activeImage);
      ...
      if (testBounds3D(tempInitial, initialMessage)) {
        ...
        powell.setInitialPoint(tempInitial);
        powell.run();
        item = new MatrixListItem(powell.getCost(), powell.getMatrix(),
                                  powell.getFinal());
      }
      optMinima.add(item);
      count++;
   ...
   *   them all into one vector.  Perturbs the rotations in each dimension
   *   by zero and plus-minus fineDelta.  If it's not a rigid
   *   transformation, perturbs the scales by factors of 0.8, 0.9, 1.0, 1.1, and 1.2.
  ...
  private Vector levelFour(ModelSimpleImage ref, ModelSimpleImage input,
    ...
    /*
     ...
     timeLater = System.currentTimeMillis();
     timeElapsed = timeLater - timeNow;
     Preferences.debug(" LEVEL FOUR \n");
     Preferences.debug(" Time for single cost function evaluation: " + ( (float) timeElapsed / 1000.0f) +
     " seconds \n");

     // Test initial amount of overlap.
     if (costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED_WGT) {
     double overAmt[] = new double[] {0, 0, 0};
    ...
    int degree = (DOF < 7) ? DOF : 7;
    maxIter = baseNumIter * 2;
    AlgorithmConstPowellOpt3D powell = new AlgorithmConstPowellOpt3D(this, cog,
        degree, cost, dummy, getTolerance(degree),
    ...
    for (Enumeration en = optMinima.elements();
         en.hasMoreElements() && !threadStopped; ) {
      item = ( (MatrixListItem) en.nextElement());
      powell.setInitialPoint(item.initial);
      powell.measureCost();
    ...
    // Old code: int total = (3 < minima.size()) ? 3 : minima.size();
    // New code: int total = (numMinima < minima.size()) ? numMinima : minima.size();
    // Changed so that the number of minima to test at Level Four is a variable,
    ...
    for (int i = 0; i < total && !threadStopped; i++) { // Loop "total" times.
      ...
      powell.setInitialPoint( ( (MatrixListItem) optMinima.elementAt(i)).
                             initial);
      powell.setActiveImage(activeImage);
      powell.run();
      if (threadStopped) {
    ...
    best4Now = null;
    Preferences.debug(
        "Top optimized minima from Level Eight: (already sorted and recalculated)\n");
    ...
    if (minimumIndex < total) {
      Preferences.debug("New minimum is at index " + (minimumIndex + 1) +
                        " and is " + currentMinimum + "\n");
    }
    else {
      Preferences.debug(
          "New minimum is from optimized minima at index " +
          (minimumIndex - total + 1) + " and is "
          + currentMinimum + "\n");
    }
    ...
    Preferences.debug("Number of minima to test at levelFour: " +
                      (2 * total - remove) + ".\n");
    ...
    for (int j = 0; j < 7 && !threadStopped; j++) {
      for (int i = 0; i < (2 * total - remove) && !threadStopped; i++) {
        progressBar.updateValue(29 +
                                ( ( (j * 2 * total + i + 1) * 3) / (total * 12)),
                                activeImage);

        // Current "initial" is element for this i.
        initial = (double[]) ( (MatrixListItem) newMinima.elementAt(i)).initial.
            clone();
        ...
        if (j == 0) {
          ...
          if ( (j == 1) || (j == 2)) {
            Preferences.debug(
                " by adding " + sign * fineDeltaX + " to initial[0] (" +
                (int) initial[0] + ").\n");
            initial[0] += sign * fineDeltaX;
          }
          else if ( (j == 3) || (j == 4)) {
            Preferences.debug(
                " by adding " + sign * fineDeltaY + " to initial[1] (" +
                (int) initial[1] + ").\n");
            initial[1] += sign * fineDeltaY;
          }
          else {
            Preferences.debug(
                " by adding " + sign * fineDeltaZ + " to initial[2] (" +
                (int) initial[2] + ").\n");
        ...
        powellCost = powell.getCost();
        item = new MatrixListItem(powellCost, powell.getMatrix(),
                                  powell.getFinal());
        perturbList.add(item);

        // Update currentMinimum and minimumIndex so that we can know if level8 minima tend to be the same as level4
        if (powellCost < currentMinimum) {
          currentMinimum = powellCost;
    ...
    Preferences.debug(
        "Best minimum from level four is a perturbed version of " +
        (minimumIndex + 1) + " and is "
        + currentMinimum + ".\n");
    ...
    if (DOF > 6) {
      ...
      for (int j = 0; j < 5 && !threadStopped; j++) {
        for (int i = 0; i < (2 * total - remove) && !threadStopped; i++) {
          progressBar.updateValue(32 +
                                  ( ( (j * 2 * total + i + 1) * 3) / (total * 8)),
                                  activeImage);
          initial = (double[]) ( (MatrixListItem) newMinima.elementAt(i)).
              initial.clone();
          if (j == 1) {
            scaleDelta = 0.9f;
          }
   ...
   *   the images.  Optimizes the best minimum with 7 degrees of
   *   freedom, then 9, then 12.  If the user has limited the degrees
   *   of freedom to 6, there will only be one optimization run,
   *   with 6 degrees of freedom.  Returns the best minimum
   *   after optimization.
  ...
  private MatrixListItem levelTwo(ModelSimpleImage ref, ModelSimpleImage input,
    ...
    /*
     ...
     timeLater = System.currentTimeMillis();
     timeElapsed = timeLater - timeNow;
     Preferences.debug(" LEVEL TWO \n");
     Preferences.debug(" Time for single cost function evaluation: " + ( (float) timeElapsed / 1000.0f) +
     " seconds \n");
     // Amount of overlap for initial position.
     if (costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED_WGT) {
     double overAmt[] = new double[] {0, 0, 0};
    ...
    item = new MatrixListItem(powell.getCost(), powell.getMatrix(),
                              powell.getFinal());
    MatrixListItem itemPtr = new MatrixListItem(powell.getCost(),
                                                powell.getMatrix(input.xRes),
    ...
    if (DOF > 7) {
      ...
      powell = new AlgorithmConstPowellOpt3D(this, cog, degree, cost,
                                             item.initial, getTolerance(degree),
                                             maxIter, bracketBound);
      ...
      item = new MatrixListItem(powell.getCost(), powell.getMatrix(),
                                powell.getFinal());
      itemPtr = new MatrixListItem(powell.getCost(),
                                   powell.getMatrix(input.xRes),
                                   powell.getFinal(input.xRes));
      Preferences.debug("Level 2, after " + degree + " DOF: " + itemPtr + "\n");
      ...
      if (DOF > 9) {
        ...
        powell = new AlgorithmConstPowellOpt3D(this, cog, 12, cost,
                                               item.initial, getTolerance(12),
                                               maxIter,
                                               bracketBound);
   ...
   *   of freedom as specified by the user (the max is 12).  Returns the
   *   best minimum.
  ...
  private MatrixListItem levelOne(ModelSimpleImage ref, ModelSimpleImage input,
                                  MatrixListItem item, int maxIter) {
    ...
    /*
     ...
     timeLater = System.currentTimeMillis();
     timeElapsed = timeLater - timeNow;
     Preferences.debug(" LEVEL ONE \n");
     Preferences.debug(" Time for single cost function evaluation: " + ( (float) timeElapsed / 1000.0f) +
     " seconds \n");

     // To test the initial amount of overlap.
     if (costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED ||
     costChoice == AlgorithmCostFunctions.NORMALIZED_MUTUAL_INFORMATION_SMOOTHED_WGT) {
     double overAmt[] = new double[] {0, 0, 0};
    ...
    item2 = new MatrixListItem(powell.getCost(), powell.getMatrix(input.xRes),
                               powell.getFinal(input.xRes));
  ...
  /**
   *   Calculates the center of mass (gravity) of a 3D image. In image space where
   *   the upper left hand corner of the image is 0,0. The x axis goes left to right,
   *   y axis goes top to bottom and z axis goes into the screen.
   ...
   */
  public static Point3Dd calculateCenterOfMass3D(ModelSimpleImage image,
    ...
    if (isColor) {
        ...
        for (z = 0; z < image.zDim; z++) {
          for (y = 0; y < image.yDim; y++) {
            for (x = 0; x < image.xDim; x++) {
              for (c = 1; c <= 3; c++) {
                voxVal = image.data[4 * (z * sliceSize + y * image.xDim + x) +
                    c];
                cogPt.x += voxVal * x;
                cogPt.y += voxVal * y;
                cogPt.z += voxVal * z;
      ...
      if (wgtImage == null) {
        for (z = 0; z < image.zDim; z++) {
          for (y = 0; y < image.yDim; y++) {
            for (x = 0; x < image.xDim; x++) {
              voxVal = image.data[z * sliceSize + y * image.xDim + x] -
                  image.min;
              cogPt.x += voxVal * x;
              cogPt.y += voxVal * y;
              cogPt.z += voxVal * z;
  ...
  public void calculatePrincipleAxis(ModelImage image, boolean doColor) {
    ...
    double mX = 0, mY = 0, mZ = 0, mXX = 0, mXY = 0, mXZ = 0, mYY = 0, mYZ = 0,
        mZZ = 0;
    ...
    for (z = 0; z < zEnd; z++) {
      for (y = 0; y < yEnd; y++) {
        for (x = 0; x < xEnd; x++) {
          if (doColor) {
            voxVal = (double) (image.getFloatC(x, y, z, 1) +
  ...
  /**
   *   Takes a simple image and subsamples XY by 2, interpolating
   *   so that the new XY values are averages.
   *   @param srcImage Image to subsample.
   *   @param isColor
   *   @return         Subsampled image.
   */
  ...
  /**
   *   Performs a trilinear interpolation on points.  Takes 3 initial points,
   *   a vector of values to set, and an array in which to look at neighbors
   *   of those points.  Sets the appropriate values in the vector.  Does not
   ...
   *   @param initial  Vector to set; if scale is <code>true</code>, set three
   *                   translations and a scale.  Otherwise just set translations.
   *   @param scale    <code>true</code> means set the scale in the vector.
   */
  private void interpolate(double x, double y, double z, double[] initial,
                           double[][][][] tranforms, boolean scale) {
    ...
    if (scale) {
      ...
      initial[3] = ( (x - ix0)
                    * ( (y - iy0)
                    ...
                    + (1 - x + ix0)
                    * ( (y - iy0)
                       * (tranforms[ix0][iy1][iz1][1]
                          * (z - iz0)
                          + tranforms[ix0][iy1][iz0][1]
                          * (1 - z
                             + iz0))
                       + (1
                          - y
                          + iy0)
                       * (tranforms[ix0][iy0][iz1][1]
                          * (z
                             - iz0)
                          + tranforms[ix0][iy0][iz0][1]
                          * (1
                             - z
                             + iz0))));
      // y translation
      initial[4] = ( (x - ix0)
                    * ( (y - iy0)
                    ...
                    + (1 - x + ix0)
                    * ( (y - iy0)
                       * (tranforms[ix0][iy1][iz1][2]
                          * (z - iz0)
                          + tranforms[ix0][iy1][iz0][2]
                          * (1 - z
                             + iz0))
                       + (1
                          - y
                          + iy0)
                       * (tranforms[ix0][iy0][iz1][2]
                          * (z
                             - iz0)
                          + tranforms[ix0][iy0][iz0][2]
                          * (1
                             - z
                             + iz0))));
      // z translation
      initial[5] = ( (x - ix0)
                    * ( (y - iy0)
                    ...
                    + (1 - x + ix0)
                    * ( (y - iy0)
                       * (tranforms[ix0][iy1][iz1][3]
                          * (z - iz0)
                          + tranforms[ix0][iy1][iz0][3]
                          * (1 - z
                             + iz0))
                       + (1
                          - y
                          + iy0)
                       * (tranforms[ix0][iy0][iz1][3]
                          * (z
                             - iz0)
                          + tranforms[ix0][iy0][iz0][3]
                          * (1
                             - z
                             + iz0))));
      // scale
      initial[6] = ( (x - ix0)
                    * ( (y - iy0)
                    ...
                    + (1 - x + ix0)
                    * ( (y - iy0)
                       * (tranforms[ix0][iy1][iz1][0]
                          * (z - iz0)
                          + tranforms[ix0][iy1][iz0][0]
                          * (1 - z
                             + iz0))
                       + (1
                          - y
                          + iy0)
                       * (tranforms[ix0][iy0][iz1][0]
                          * (z
                             - iz0)
                          + tranforms[ix0][iy0][iz0][0]
                          * (1
                             - z
                             + iz0))));
    }
   ...
   *   @param factor Subsampling factor for levels 1, 2, 4, 8.
   ...
   *  Rotation tolerance = (180/PI)*n/(2R)  (converted to degrees because AlgorithmConstPowell works in degrees)
  ...
  protected boolean testBounds3D(double[] point, String message) {
    ...
    for (int i = 0; i < 6; i++) {
      if (point[i] < (limits[0][i] - margin) ||
          point[i] > (limits[1][i] + margin)) {
        test = true;
        i = 6;
      }
    }
