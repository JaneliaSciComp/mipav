...
/**
 *   The class reads and writes raw files of all data types:
 *   boolean, byte, short, int, long, float, and double. For the read
 *   process an offset can be pass as a parameter to identify the location
 *   in the file where the data starts. A number of file formats while not
 *   "raw" have data at a specific location after a fixed length but unknown
 *   header and therefore can be treated as raw.
 *
...
public class FileRaw
   ...
   */
  public FileRaw(String fileName, String fileDir,
                 FileInfoBase fInfo, boolean showProgressBar, int rwFlag) throws IOException {
   ...
   *   @exception IOException   if there is an error making the files
   */
  public FileRaw(String fileName, FileInfoBase fInfo,
                 boolean showProgressBar, int rwFlag) throws IOException {
    fileInfo = fInfo;
  ...
  /**
   * Sets the progress bar panel to be updated (rather than create a new progress bar frame
   * to update status of file opening in a panel instead of frame
   ...
   *   @exception IOException   if there is an error making the files
   */
  public void setImageFile(String fileName, String fileDir,
   ...
   *   @param offset           Points to where the data of the image is located.
   *                           It is equal to the header length.
    ...
    for (k = 0; k < nBuffers; k++) {
      ...
      switch (image.getType()) {
        ...
        case ModelStorageBase.BOOLEAN:
          try {
            fileRW.readImage(ModelStorageBase.BOOLEAN,
                             k * 8 * ( (bufferSize + 63) >> 6) + offset,
                             bufferSize);

            image.importData(k * bufferSize,
                             fileRW.getBitSetBuffer(),
                             false);
          }
        ...
        case ModelStorageBase.BYTE:
          try {
            fileRW.readImage(ModelStorageBase.BYTE,
                             k * bufferSize + offset,
                             bufferSize);

            image.importData(k * bufferSize,
                             fileRW.getByteBuffer(),
                             false);
          }
          catch (IOException error) {
            if (showProgress)
        ...
        case ModelStorageBase.UBYTE:
          try {
            fileRW.readImage(ModelStorageBase.UBYTE,
                             k * bufferSize + offset,
                             bufferSize);

            image.importUData(k * bufferSize,
                              fileRW.getShortBuffer(),
                              false);
          }
          catch (IOException error) {
            if (showProgress)
        ...
        case ModelStorageBase.SHORT:
          try {
            fileRW.readImage(ModelStorageBase.SHORT,
                             k * bufferSize * 2 + offset,
                             bufferSize);
        ...
        case ModelStorageBase.USHORT:
          try {
            fileRW.readImage(ModelStorageBase.USHORT,
                             k * bufferSize * 2 + offset,
                             bufferSize);
        ...
        case ModelStorageBase.INTEGER:
          try {
            fileRW.readImage(ModelStorageBase.INTEGER,
                             k * bufferSize * 4 + offset,
                             bufferSize);
        ...
        case ModelStorageBase.UINTEGER:
          try {
            fileRW.readImage(ModelStorageBase.UINTEGER,
                             k * bufferSize * 4 + offset,
                             bufferSize);
        ...
        case ModelStorageBase.LONG:
          try {
            fileRW.readImage(ModelStorageBase.LONG,
                             k * bufferSize * 8 + offset,
                             bufferSize);

            image.importData(k * bufferSize,
                             fileRW.getLongBuffer(),
                             false);
          }
          catch (IOException error) {
            if (showProgress)
        ...
        case ModelStorageBase.FLOAT:
          try {
            fileRW.readImage(ModelStorageBase.FLOAT,
                             k * bufferSize * 4 + offset,
                             bufferSize);

            image.importData(k * bufferSize,
                             fileRW.getFloatBuffer(),
                             false);
          }
          catch (IOException error) {
            if (showProgress)
        ...
        case ModelStorageBase.DOUBLE:
          try {
            fileRW.readImage(ModelStorageBase.DOUBLE,
                             k * bufferSize * 8 + offset,
                             bufferSize);

            image.importData(k * bufferSize,
                             fileRW.getDoubleBuffer(),
                             false);
          }
          catch (IOException error) {
            if (showProgress)
        ...
        case ModelStorageBase.ARGB:
          try {
            fileRW.readImage(ModelStorageBase.ARGB,
                             k * bufferSize * 3 + offset,
                             bufferSize * 3);
        ...
        case ModelStorageBase.ARGB_USHORT:
          try {
            fileRW.readImage(ModelStorageBase.ARGB_USHORT,
                             k * bufferSize * 6 + offset,
                             bufferSize * 3);
          ...
          try {
            fileRW.readImage(ModelStorageBase.COMPLEX,
                             k * bufferSize * 8 + offset,
                             bufferSize * 2);
            ...
            image.importComplexData(2 * k * bufferSize,
   ...
   *   @see                    FileRawChunk
   */
  public void readImage(float buffer[], int offset, int imageType) throws
      IOException {
    int i;
    int ii;
    ...
    switch (imageType) {
      ...
      case ModelStorageBase.ARGB:
        ii = 0;
        i = 0;
        // This assume that the image on file is just RGB and not ARGB.
        try {
          fileRW.readImage(ModelStorageBase.ARGB, offset, bufferSize / 4 * 3);
      ...
      case ModelStorageBase.ARGB_USHORT:
        ...
        // This assume that the image on file is just RGB and not ARGB.
        try {
          fileRW.readImage(ModelStorageBase.ARGB_USHORT, offset,
                           bufferSize / 4 * 3);
  ...
  /**
   *   This method reads a file and puts the data in the buffer.
   *    Added here to help speed the reading of DICOM images
   *   @param buffer           float buffer where the data will be stored.
   *   @param offset           points to where the data of the image is located.
   *                           It is equal to the header length.
   *   @param imageType        ModelImage type (i.e. boolean, byte ...);
   ...
   *   @see                    FileRawChunk
   */
  public void readImage(short buffer[], int offset, int imageType) throws
      IOException {
    int i;
    int ii;
    int bufferSize = buffer.length;
  ...
  /**
   * Method to write 4D images to an array of 3D image files.  the files will be named sequentially with
   * the given # of digits and given starting #.
   * This method will save ALL slices and the files will be split up by time-period
   *
  ...
  public void writeImage4DTo3D(ModelImage image, FileWriteOptions options, String suffix) throws
      IOException {
    ...
    if (showProgress) {
      pInterface = new ViewJProgressBar("Saving " + fileName,
                                         "Saving image ...", 0, 100,
                                         false, null, null);
    ...
    for (int t = beginTimePeriod, seq = options.getStartNumber();
         t <= endTimePeriod; t++, seq++, prog++) {
  ...
  /**
   * Method to save 3D images to an array of 2D images.  Images will be name sequentially with
   * the given start # and # of digits
   * @param image  Image to be saved (broken down into 2D images)
   * @param options File Write options
   * @param suffix  Suffix for file names.  Example: ".xml", ".img"
   * @throws IOException
   */
  public void writeImage3DTo2D(ModelImage image, FileWriteOptions options, String suffix) throws
      IOException {
    String prefix;
    ...
    for (k = beginSlice, seq = options.getStartNumber(); k <= endSlice; k++,
      ...
      if (showProgress) {
        pInterface.setTitle("Saving image " + fileString);
        pInterface.updateValue(MipavMath.round( (prog / (endSlice - beginSlice + 1)) * 100), options.isActiveImage());
      }
      ...
      try {
        fileRW.writeImage(image, k * bufferSize,
                          k * bufferSize + bufferSize, 0);
        if (compressionType == FileInfoBase.COMPRESSION_NONE) {
            raFile.close();
        }
      }
    ...
    try {
      ...
      for (int t = beginTimePeriod; t <= endTimePeriod; t++) {
        for (k = beginSlice; k <= endSlice; k++) {
          if (showProgress) {
            pInterface.updateValue(MipavMath.round( (float) (t *
                (endSlice - beginSlice) + k) / (nImages) * 100), options.isActiveImage());
