 ...
 * <p>
 * builds a simple dialog which imbeds a JPanelAnonymizeImage. The JDialogAnonymizeImage will gray-out all entries which
 * are not available in the FileInfoDicom that goes with the given image. "Okay" then proceeds to remove or filter the
 * selected tags from the image.
 * @author parsonsd
 * @version 0.9
            ...
            if (checkboxPanel.getNumberSelected() == 0) {
                ...
                anonymizeChoice = JOptionPane.showConfirmDialog(null,
                        "You are about to make a permament change to the entire image.\n"
                                + "Are you sure you want to remove the sensitive data?", "Confirm for Anonymize",
