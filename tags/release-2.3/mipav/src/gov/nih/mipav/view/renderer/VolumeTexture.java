...
/**
 *  Defines 3D texture node for a volume of data. There is a large amount of commented out code.
 *  We were trying to have the texture image updated dynamically (i.e., without having to remove
 *  the texture map from the scene, updating the texture and putting back into the scene. This
 *  is slow and causes flicker. However, it turned out there is a bug in Java3D that makes
 *  it impossible to update the texture dynamically. Therefore we left in the code until the bug
 *  has been fixed.
 */
public class VolumeTexture {
    ...
    /** These vectors are used for mapping each coordinate from its
     * real range into the range [0,1] where real coordinate 0 maps to
     * texture coordinate 0, and real coordinate at the maximum
     * sample index along that axis maps to 1.
     */
    protected Vector4f m_kCoordMapX;
    protected Vector4f m_kCoordMapY;
    protected Vector4f m_kCoordMapZ;

    /** This are the actual dimensions of the texture associated with
     each dimension of the volume. */
    ...
    /** These are declared in the class to avoid allocating temporary
     * instances each call into methods of this class.
     */
    protected Vector4f m_kTempPlane = new Vector4f();

    /**
     * Create texture(s) and automatic texture coordinate generation
     * parameters for each slicing of the specified volume
     * along each of the three axes.
     * @param kVolume volume for which to create the associated texture
     * information
     */
    ...
    /**
     * Access automatic texture coordinate generation parameters associated
     * with volume for a 3D texture.
    ...
    /* Access functions for Mapping from texture coordinates to model
     * coordinates and vice versa: */
    /* getCoordMapX:
     * @return m_kCoordMapX, transformation vector from voxel space to model
     * space: */
    ...
    /* getCoordMapY:
     * @return m_kCoordMapY, transformation vector from voxel space to model
     * space: */
    ...
    /* getCoordMapZ:
     * @return m_kCoordMapZ, transformation vector from voxel space to model
     * space: */
    ...
    */
    private void createTexture() {
        ...
        m_kTexCoordGenerationAxisAligned = new TexCoordGeneration(TexCoordGeneration.OBJECT_LINEAR,
            TexCoordGeneration.TEXTURE_COORDINATE_3, m_kCoordMapX, m_kCoordMapY, m_kCoordMapZ);
        ...
        //int [] aiImageDataX =
        //     ((DataBufferInt)m_kImageX.getRaster().getDataBuffer()).getData();


        // Create the 3D image component for storing in the texture.
        m_kImageComponent = new ImageComponent3D(ImageComponent.FORMAT_RGBA, m_iTextureSizeX, m_iTextureSizeY, m_iTextureSizeZ, true, false);
        ...
        for (int iSliceZ = 0; iSliceZ < iNumSlices; ++iSliceZ) {
            ...
            /*
              m_kImageX = new BufferedImage(m_iTextureSizeX, m_iTextureSizeY,
                                         BufferedImage.TYPE_INT_ARGB);
              if (iSliceZ == 0) System.out.println("first ImageX = " + m_kImageX);
              aiImageDataX = ((DataBufferInt)m_kImageX.getRaster().getDataBuffer()).getData();
              try {
                      image.exportData(bufferSize*iSliceZ, bufferSize, aiImageDataX);
                  }
                  catch (IOException error) {
                      MipavUtil.displayError("" + error);    // Need to fix this
                  }
              for (int i = 0; i < aiImageDataX.length; i++) {
                  aiImageDataX[i] = (60 << 24) | (((int)(100)   << 16) | (((int)(5*iSliceZ) << 8) | ((int)(0))));
              }
    ...
    private class Updater  implements ImageComponent3D.Updater {
      ...
      int imgBuffer[];
      //BufferedImage[] bufImages = null;

      public Updater(BufferedImage m_kImage) {
        imgBuffer = ( (DataBufferInt) m_kImage.getRaster().getDataBuffer()).
            getData();
        //this.bufImages = bufImg;
      }
      ...
      public void updateData(ImageComponent3D imageComponent,
        ...
        if (index == 0)
          System.out.println("updater Comp = " + imageComponent);
        if (index == 0)
          System.out.println("updater ImageX = " + imageComponent.getImage(index));
        if (index == 0)
          System.out.println("updater image = " + m_kImage);

        for (int i = 0; i < imgBuffer.length; i++) {
          ( (DataBufferInt) bufArray[index].getRaster().getDataBuffer()).getData()[
              i] = imgBuffer[i];
        }

        //int temp = ((DataBufferInt)imageComponent.getImage(index).getRaster().getDataBuffer()).getData()[i];
        //System.out.println("Hey before  [" + i + "] = " + temp);
        //System.out.println("Image before  [" + i + "] = " + imgBuffer[i]);
        //((DataBufferInt)imageComponent.getImage(index).getRaster().getDataBuffer()).getData()[i] =  imgBuffer[i];
        //temp = ((DataBufferInt)imageComponent.getImage(index).getRaster().getDataBuffer()).getData()[i];
    ...
    protected void resampleImage() {
        ...
        if (originalVolPowerOfTwo == false) {
            ...
            transformImg = new AlgorithmTransform(image, new TransMatrix(4),
                                                  AlgorithmTransform.CUBIC_LAGRANGIAN, newRes[0], newRes[1], newRes[2], volExtents[0],
                                                  //AlgorithmTransform.TRILINEAR, newRes[0], newRes[1], newRes[2], volExtents[0],
