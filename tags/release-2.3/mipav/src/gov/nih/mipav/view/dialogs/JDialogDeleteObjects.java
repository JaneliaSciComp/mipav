 ...
 *
 */
public class JDialogDeleteObjects
    extends JDialogBase
      ...
      if (displayLoc == NEW) {
        try {
          ...
          deleteAlgo2D = new AlgorithmMorphology2D(resultImage, kernel, 0,
              AlgorithmMorphology2D.DELETE_OBJECTS, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
          ...
          if (runInSeparateThread) {
            ...
            deleteAlgo2D.setActiveImage(isActiveImage);
          ...
          MipavUtil.displayError(
              "Dialog delete: unable to allocate enough memory");
          if (resultImage != null) {
        ...
        try {
          ...
          deleteAlgo2D = new AlgorithmMorphology2D(image, kernel, 0,
              AlgorithmMorphology2D.DELETE_OBJECTS, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
            deleteAlgo2D.setMask(image.generateVOIMask());
          deleteAlgo2D.setMinMax(min, max);
      ...
      if (displayLoc == NEW) {
        try {
          ...
          deleteAlgo3D = new AlgorithmMorphology3D(resultImage, kernel, 0,
              AlgorithmMorphology3D.DELETE_OBJECTS, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
          ...
          if (runInSeparateThread) {
            ...
            deleteAlgo3D.setActiveImage(isActiveImage);
            if (!userInterface.isAppFrameVisible()) {
        ...
        try {
          // Make algorithm
          deleteAlgo3D = new AlgorithmMorphology3D(image, kernel, 0,
              AlgorithmMorphology3D.DELETE_OBJECTS, 0, 0, 0, 0, regionFlag);
          if (regionFlag == false)
            deleteAlgo3D.setMask(image.generateVOIMask());
          deleteAlgo3D.setMinMax(min, max);
