...
/**
 * Dialog to turn slices bounding box of surface renderer on and off,
 * and to change the color of the frame.  This dialog also control
 * the X, Y, Z slices movements.
 * @author Ruida Cheng
 */
public class JPanelProbe extends JPanelRendererBase
    ...
    /** Bone segmentation preset opacity tranform function control button. */
    ;
     ...
     */
    public JPanelProbe( SurfaceRender parent, ViewJFrameBase _parentFrame,
     ...
     */
    private static final JLabel createLabel( String title ) {
     ...
     *	@return 		The new label.
     */
    private static final JLabel createBoldLabel( String title ) {
        JLabel label = new JLabel( title );
    ...
    /**
     * Build the four tabbed panes, probe control pane,
     * burn control pane, burn parameter control pane,
     * and demo pane.
     */
    ...
    /**
     * Build the target surface (tumor) control panel.   This panel can add or remove surfaces, also
     * changes the surface opacity.
    ...
    /**
     * Build the tab which allows the user to pick the current tumor to target, to load/remove
     * tumor surfaces and show tumor position and volume info.
     */
    ...
    /**
     * Build the tab with controls / info used in probe placement before a burn.  This includes
     * rot, trans, in/out, entry pt rotation, warning of intersection of problem regions,
     * and the saving / loading of a problem region mapping.
     */
    ...
    /**
     * Build the tab containing ablation treatment information, including a list of burns;
     * burn removal, position and volume; comparison of current tumor vs current set of burns.
     */
    ...
    private void buildToolBar() {
        ...
        probeTargetButton = toolbarBuilder.buildToggleButton("ProbeTargetPoint", "Picks a probe target point using the Tri-Planar views", "probepoint", cursorGroup );
        ...
        loadVisualizationButton = toolbarBuilder.buildButton("LoadPreset", "Load custom visualization settings.", "userlutopen");
        toolBar.add( loadVisualizationButton );
        saveVisualizationButton = toolbarBuilder.buildButton("SavePreset", "Save custom visualization settings.", "userlutsave");
        toolBar.add( saveVisualizationButton );
        mainPanel.add( toolBar, gbc );
    }

    /**
     * Value changed event, which invoked by probe name list, burn name list,
     * surface name list, burn param name list.
    ...
    public void valueChanged( ListSelectionEvent kEvent ) {
        ...
        if ( kList == probeList ) {
            ...
            String unitStr = parentFrame.getImageOriginal().getFileInfo( 0 ).getUnitsOfMeasureAbbrevStr(
                    parentFrame.getImageOriginal().getFileInfo( 0 ).getUnitsOfMeasure( 0 ) );
            ...
            if ( indices.length == 1 ) {
                ...
                for ( int i = 0; i < surfaces.size(); i++ ) {
                    if ( ( (SurfaceAttributes) surfaces.get( i ) ).name.equals( ( (
                            SurfaceAttributes) surfaceVector.get( selectedIndex ) ).name ) ) {
     ...
     */
    public boolean withinDistance( float centerX, float centerY, float centerZ,
            float ptrX, float ptrY, float ptrZ, double r ) {

        double distance = Math.sqrt(
                ( ptrX - centerX ) * ( ptrX - centerX ) + ( ptrY - centerY ) * ( ptrY - centerY )
    ...
    /**
     * Detect the body tissue component.  If the bone or the vasculature is detected,
     * draw the corresponding color along the probing path, also, show the detection
     * message in the search panel.
     */
    public void detectTissue() {
        ...
        for ( int idx = 0; idx < 300; idx++ ) {
            ...
            if ( result != -1 ) {
                ...
                // draw the sphere voxels kernel, the idea is to draw the voxels only when a bone, or a vasculature detected
                // along the probing path.   This approach saves the time to draw the unnecessary voxels on the path, otherwise,
                ...
                /* if ( probe.isProbeRootParentBGLive() ) {
                 ...
                 for (i = xMin; i < xMax; i = i + stepSize) {
                 for (j = yMin; j < yMax; j = j + stepSize) {
                 for (k = zMin; k < zMax; k = k + stepSize) {
                 // if (withinDistance(x, y, z, i, j, k, r)) {
                 ( (SurfaceRender) renderBase).drawProbingVoxel(i, j, k, result);
                 // }
                 }
                 }
                 }
    ...
    public void actionPerformed( ActionEvent event ) {
        ...
        if ( command.equals( "calcVolume" ) ) {
            ...
            diffVolumeText.setText(
                    " " + MipavUtil.makeFloatString( burnBase.getDiffVolume(), 3 ) + " " + unitStr + "^3" );
            ...
            /* TransferFunction lutFunc = new TransferFunction();
             lutFunc.addPoint( (float) parentFrame.getImageA().getMin(), 255 );
             lutFunc.addPoint( -206, 255 );
             lutFunc.addPoint( 356, 0 );
             lutFunc.addPoint( (float) parentFrame.getImageA().getMax(), 0 );

             TransferFunction opacFunc = new TransferFunction();
             opacFunc.addPoint( (float) parentFrame.getImageA().getMin(), 100 );
             opacFunc.addPoint( -497, 100 );
             opacFunc.addPoint( (float) parentFrame.getImageA().getMax() - 1, 0 );
            ...
            /* TransferFunction lutFunc = new TransferFunction();
             ...
             TransferFunction opacFunc = new TransferFunction();
             opacFunc.addPoint( (float) parentFrame.getImageA().getMin(), 100 );
             opacFunc.addPoint( 79, 100 );
             opacFunc.addPoint( 655, 0 );
             opacFunc.addPoint( (float) parentFrame.getImageA().getMax(), 0 );
            ...
            /* TransferFunction lutFunc = new TransferFunction();
             ...
             TransferFunction opacFunc = new TransferFunction();
             opacFunc.addPoint( (float) parentFrame.getImageA().getMin(), 100 );
             opacFunc.addPoint( 255, 100 );
             opacFunc.addPoint( 718, 0 );
             opacFunc.addPoint( (float) parentFrame.getImageA().getMax(), 0 );
     ...
     * @param swapBurnList  whether to swap in the burn list of the currently selected target surface
     *                      treatment (used when a new selection is made in the target list)
     */
    public void updateTreatmentPanel( boolean swapBurnList ) {
        ...
        if ( burnIndex != -1 ) {
            ...
            volumeText.setText(
                    " " + MipavUtil.makeFloatString( getCurTreatment().getBurnVolume( burnIndex ), 3 ) + " " + unitStr
                    + "^3" );
    ...
    public void startBurn() {
        ...
        BurnAttributes burn = new BurnAttributes( burnBase.getSphereBG(), name, burnBase.getColor(),
                burnBase.getVolume(), opacity, radius, burningTime, center, pickable, clipping, culling,
                burnBase.getMask(), getEntryPoint(), getBurnPoint(), probe.getTransform() );
    ...
    /**
     * Get the burn root branch group.   JPanelSurface calls this method during the
     * geometry picking procedure.
     * @return BranchGroup
     */
    ...
    private void getColorChange( Color color ) {
        ...
        for ( int i = 0; i < aiSelected.length; i++ ) {
            ...
            Appearance appearance = shape.getAppearance();
            Material mat = new Material( new Color3f( color ), new Color3f( color ), new Color3f( color ),
                    new Color3f( color ), 80f );
    ...
    private void loadVisualizationSettings( TransferFunction lut, TransferFunction opac ) {
        ...
        ( (ViewJComponentHistoLUT) parentFrame.getLUTDialog().getHistoLUTComponentA() ).getLUT().setTransferFunction(
                lut );
        if ( imageBLoaded ) {
            ( (ViewJComponentHistoLUT) parentFrame.getLUTDialog().getHistoLUTComponentB() ).getLUT().setTransferFunction(
                    lut );
        }

        if ( ( (SurfaceRender) renderBase ).getDisplayMode3D() ) {
            ( (SurfaceRender) renderBase ).getVolOpacityPanel().getSelectedComponent( JPanelVolOpacity.COMP_A ).updateTransFunc(
                    opac );
            if ( imageBLoaded ) {
                ( (SurfaceRender) renderBase ).getVolOpacityPanel().getSelectedComponent( JPanelVolOpacity.COMP_B ).updateTransFunc(
                        opac );
            }
        }
     ...
     *   @param loadAll    boolean indicating that both lut and transfer functions should be loaded.
     ...
     */
    public void loadVisualizationDataFrom( boolean loadAll, String filename,
     ...
     *   @param  saveAll   boolean indicating that both lut and transfer functions should be saved.
     *                     If false, then only transfer functions are saved.
     *   @param  filename  filename to save LUT as
     *   @param  dirName   directory to save LUT to
     */
    public void saveVisualizationDataAs( boolean saveAll, String filename,
            String dirName ) {
        ModelLUT lut;
        ModelImage img;
        FileHistoLUT fileHistoLUT;
