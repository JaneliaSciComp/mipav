...
/**
 * Implementation of the 3D skeletonization of a binary volume based
 * on the paper "Penalized-distance volumetric skeleton algorithm";
 * I. Bitter, A. Kaufman, M. Sato; IEEE Transactions on Visualization
 * and Computer Graphics, Vol. 7, No. 3, pp. 195-206, July-September 2001.
 */
public class Skeleton3D
{
    // Set this flag to allow the output of volume file which result
     ...
     * @param kVolumeLayout ModelImage3DLayout Contains the mapping between
     * sample coordiantes and real coordinates for the input volume.
     */
    public Skeleton3D (ModelImage kImage, ModelImage3DLayout kVolumeLayout)
    {
        ...
        catch (Exception e)
        {
            throw new IllegalArgumentException (
                "Skeleton3D: IOException on loading data - " + e.getMessage());
        }
        ...
        for (int iZ=0; iZ < m_kVolumeLayout.m_iDimZ; ++iZ)
        {
            for (int iY=0; iY < m_kVolumeLayout.m_iDimY; ++iY)
            {
                for (int iX=0; iX < m_kVolumeLayout.m_iDimX; ++iX)
                {
                    ...
                    if ((0 == iZ) || (m_kVolumeLayout.m_iDimZ-1 == iZ) ||
                        (0 == iY) || (m_kVolumeLayout.m_iDimY-1 == iY) ||
                        (0 == iX) || (m_kVolumeLayout.m_iDimX-1 == iX) ||
                        (0 == m_asVolumeData[iIndex]))
                    {
        ...
        if (DEBUG) {
            ...
            final float fMinSampleScale =
                Math.min(m_kVolumeLayout.m_fSpacingX,
                         Math.min(m_kVolumeLayout.m_fSpacingY,
                                  m_kVolumeLayout.m_fSpacingZ));
            ...
            for (int iIndex = 0; iIndex < iNumSamples; ++iIndex) {
                asVolumeBoundaryDist[iIndex] =
                    (short) Math.round(
                    fScaleDist * m_afBoundarySampleDistMin[iIndex]);
            }
            // rendererProgressBar.setMessage("Saving outputBoundaryDist volume file ...");
            saveVolume("outputBoundaryDist.img", FileBase.XML,
                       asVolumeBoundaryDist);
        }
        ...
        for (int iIndex=0; iIndex < iNumSamples; ++iIndex)
        {
            if (RESULT_MASK_CONNECTED ==
                (RESULT_MASK_CONNECTED & m_asVolumeData[iIndex]))
            {
                iIndexSource = iIndex;
                break;
            }
        ...
        DijkstraMinCostPath kMinCostPath = new DijkstraMinCostPath(
            m_kVolumeLayout,
            m_asVolumeData,
    ...
    /**
     * Given previously determined starting point, compute the tree-structured
     * skeleton path through the volume where no branch is shorter than
     * the specified amount.
     * @param iMaxBranches int Maximum number of branches to extract.
     * @param fMinBranchLength float All branches must be longer than
     * this length.
     * @return FlyPathGraphSamples A tree-structured skeleton path where
     * each path is represented by its samples.
     */
    public FlyPathGraphSamples getPathGraph(
        int iMaxBranches,
        float fMinBranchLength)
    {
        // What is the total number of samples in the volume.
        final int iNumSamples = m_kVolumeLayout.getNumSamples();
        ...
        for (int iIndex = 0; iIndex < iNumSamples; ++iIndex)
        {
            if (RESULT_MASK_CONNECTED ==
                (RESULT_MASK_CONNECTED & m_asVolumeData[iIndex]))
            {
                m_asVolumeData[iIndex] &=
                    ~(RESULT_MASK_SKELETON_NEIGHBOR | RESULT_MASK_SKELETON);
            }
        }
        ...
        for (int iBranch=0; iBranch < iMaxBranches; ++iBranch)
        {
            ...
            for (int iPath = aiIndexPath.length-1; iPath >= 0; iPath--)
            {
                ...
                if (RESULT_MASK_SKELETON ==
                    (RESULT_MASK_SKELETON & m_asVolumeData[iIndex]))
                {
                    break;
                }
            }
            ...
            for (int iPath = 0; iPath < iNumPathSamples; ++iPath)
            {
                // Get linear array index of next sample along the path.
                int iIndex =
                    aiIndexPath[iPath + aiIndexPath.length - iNumPathSamples];
                ...
                // furthest boundaries.
                afPathPointBoundaryMinDist[iPath] =
                    m_afBoundarySampleDistMin[iIndex];
            ...
            if (fPathLength > fMinBranchLength)
            {
                ...
                {
                    markSkeletonPoint (aiPathIndex[iPath],
                                       1.25f * afPathPointBoundaryMinDist[iPath]);
                }
        ...
        if (DEBUG)
        {
            ...
            for (int iIndex = 0; iIndex < iNumSamples; ++iIndex)
            {
                ...
                else if (RESULT_MASK_CONNECTED ==
                         (RESULT_MASK_CONNECTED & m_asVolumeData[iIndex]))
                {
                    asVolumeOutput[iIndex] = 3;
                }
                else if (RESULT_MASK_FLAGGED ==
                         (RESULT_MASK_FLAGGED & m_asVolumeData[iIndex]))
                {
                    asVolumeOutput[iIndex] = 2;
                }
                else if (RESULT_MASK_INSIDE ==
                         (RESULT_MASK_INSIDE & m_asVolumeData[iIndex]))
                {
    ...
    /**
     * Mark the specified volume sample as being part of the skeleton
     * and mark all the connected samples within the specified radius
     * of the specified sample as being neighbors.
     * @param iIndex int Identifies the sample in the volume to be
     * markes as being on the skeleton path.
     * @param fNeighborRadius float Radius of connected neighbors to mark
     * as being skeleton neighbors.
     */
        ...
        for (int iZ = iZMin; iZ <= iZMax; ++iZ) {
            for (int iY = iYMin; iY <= iYMax; ++iY) {
                for (int iX = iXMin; iX <= iXMax; ++iX) {
                    int iIndexNeighbor =
                        m_kVolumeLayout.getIndex(iX, iY, iZ);

                    // Ignore any samples NOT connected.
                    if (RESULT_MASK_CONNECTED !=
                        (RESULT_MASK_CONNECTED &
                         m_asVolumeData[iIndexNeighbor])) {
                        continue;
                    }
    ...
    /**
     * Compute the Euclidean distance from each sample "inside" the volume
     * to the nearest point on the surface surrounding the volume.
     * Based on the algorithm described in "New algorithms for Euclidean
     * distance transformation of an n-dimensional digitized picture with
     * applications", by T. Saito and J. Toriwaki, Pattern Recognition,
     * Vol. 27, No. 11, pp. 1551-1565, 1994.  Fills in the
     * m_afBoundarySampleDistMin linear array of floating point values
     * with the Euclidean distance measures; zero values will
     * be stored in the array corresponding to samples outside the surface.
     * Compute the penalty value associated with each sample based on
     * its relative distance to the boundary.
     * Compute the gradient of the distance field at each sample inside
     * the surface (indicated by a positive distance measure).
     * @param akVolumeBoundaryDistGradient Vector3f[] Array of normalized
     * boundary distance gradients computed at each sample.  The input array is
     * assumed to have all entries null.  Upon return, only those samples
     * that are inside the volume will have a gradient value stored.
     */
    private void computeVolumeBoundaryDist (
        Vector3f[] akVolumeBoundaryDistGradient)
    {
        // What is the total number of samples in the volume.
        final int iNumSamples = m_kVolumeLayout.getNumSamples();
        ...
        for (int k=0, iIndexZ=0; k < N; ++k, iIndexZ += iIndexIncZ)
        {
            for (int j=0, iIndexYZ=iIndexZ; j < M; ++j, iIndexYZ += iIndexIncY)
            {
                ...
                // reverse scan
                float db = 0.0f;
                for (int i=L-1, iIndex=iIndexYZ+(L-1)*iIndexIncX; i >= 0;
                     --i, iIndex -= iIndexIncX)
                {
                    if (0 == m_asVolumeData[iIndex])
                    {
        ...
        ModelImage3DLayout.GradientNeighbors kIndexGradient =
                new ModelImage3DLayout.GradientNeighbors();
        for (int iZ=0; iZ < m_kVolumeLayout.m_iDimZ; ++iZ)
        {
            for (int iY=0; iY < m_kVolumeLayout.m_iDimY; ++iY)
            {
                for (int iX=0; iX < m_kVolumeLayout.m_iDimX; ++iX)
                {
                    ...
                    if (m_fMaxBoundarySampleDistMin <
                        m_afBoundarySampleDistMin[iIndex])
                    {
                    ...
                    // the gradient.
                    m_kVolumeLayout.getIndexNeighborsGradient(iX,iY,iZ,
                    kIndexGradient);
                    ...
                    Vector3f kGradient =
                            new Vector3f(
                            (fX1-fX0)/kIndexGradient.m_fDeltaX,
    ...
    /**
     * Mark all inside samples that have nonuniform boundary distance
     * gradients and then "connect" such samples by following the gradient.
     * Updates the m_asVolumeData linear array of 16-bit signed bit mask
     * values representing the state of each sample in the skeletonziation
     * algorithm.  The RESULT_MASK_* constant bit masks are OR'ed into
     * this field.  Any sample having nonuniform gradients will be marked
     * with the RESULT_MASK_NONUNIFORM_GRADIENT bit mask and the
     * RESULT_MASK_FLAGGED bit mask.  Any sample along the gradient
     * path of connecting the nonuniform gradient samples will be marked
     * only with the RESULT_MASK_FLAGGED bit mask.  This array has values
     * already set upon input and only OR's in additional bit masks.
     * @param akVolumeBoundaryDistGradient linear array of Vector3f
     * instances representing the (unnormalized) boundary distance gradient
     * vectors for inside samples.  The null reference will be stored for
     * outside samples.
     */
    private void markPossibleSkeletonSamples (
        Vector3f[] akVolumeBoundaryDistGradient)
    {
        // What is the total number of samples in the volume.
        final int iNumSamples = m_kVolumeLayout.getNumSamples();
        ...
        for (int iZ=1; iZ < m_kVolumeLayout.m_iDimZ; ++iZ)
        {
            for (int iY=1; iY < m_kVolumeLayout.m_iDimY; ++iY)
            {
                for (int iX=1; iX < m_kVolumeLayout.m_iDimX; ++iX)
                {
                    ...
                    m_kVolumeLayout.getIndexNeighborsVoxel(
                            iX-1,iY-1,iZ-1,aiIndexNeighborsVoxel);
                    ...
                    {
                        if (null == akVolumeBoundaryDistGradient[
                            aiIndexNeighborsVoxel[iIndex]])
                        {
                            bAllInside = false;
                            break;
                        }
                    ...
                    if (bUseAlgorithm1)
                    {
                        ...
                        for (int iIndexA = 0; iIndexA < 8; ++iIndexA) {
                            int iIndexNeighborA =
                                aiIndexNeighborsVoxel[iIndexA];
                            Vector3f kGradientA =
                                akVolumeBoundaryDistGradient[
                                iIndexNeighborA];

                            for (int iIndexB = iIndexA + 1; iIndexB < 8;
                                 ++iIndexB) {

                                int iIndexNeighborB =
                                    aiIndexNeighborsVoxel[iIndexB];
                                Vector3f kGradientB =
                                    akVolumeBoundaryDistGradient[
                                    iIndexNeighborB];
                        ...
                        if (bNonUniform) {
                            for (int iIndex = 0; iIndex < 8; ++iIndex) {
                                m_asVolumeData[aiIndexNeighborsVoxel[iIndex]]
                    ...
                    if (bUseAlgorithm2)
                    {
                        ...
                        for (int iIndex = 0; iIndex < 8; ++iIndex) {
                            kAvgGradient.add(akVolumeBoundaryDistGradient[
                                             aiIndexNeighborsVoxel[iIndex]]);
                        }
                        ...
                        boolean bNonUniform = false;
                        for (int iIndex = 0; iIndex < 8; ++iIndex) {
                            if (kAvgGradient.dot(akVolumeBoundaryDistGradient[
                                                 aiIndexNeighborsVoxel[iIndex]]) <= 0.0f) {
                                bNonUniform = true;
                                break;
                            }
                        }
        ...
        for (int iZ=0; iZ < m_kVolumeLayout.m_iDimZ; ++iZ)
        {
            for (int iY=0; iY < m_kVolumeLayout.m_iDimY; ++iY)
            {
                for (int iX=0; iX < m_kVolumeLayout.m_iDimX; ++iX)
                {
                    ...
                    if (RESULT_MASK_NONUNIFORM_GRADIENT ==
                        (RESULT_MASK_NONUNIFORM_GRADIENT &
                        m_asVolumeData[iIndex]))
                    {
        ...
        while (!kTreeSetFlaggedSamples.isEmpty())
        {
            ...
            while (!kTreeSet.isEmpty())
            {
                ...
                if (kTreeSetFlaggedSamples.remove(kInteger))
                {
                    ...
                    m_kVolumeLayout.getIndexNeighbors27(iIndexCenter,
                        aiIndexNeighbors27);
            ...
            kConnectLabelCount.add(sConnectLabel++,
                                   new Integer(iConnectCount));
        }
        int iLabelMaxCount = 0;
        for (int iLabel=1; iLabel < kConnectLabelCount.size(); ++iLabel)
        {
            int iCountA = ((Integer)
            kConnectLabelCount.get(iLabelMaxCount)).intValue();

            int iCountB =
                    ((Integer)kConnectLabelCount.get(iLabel)).intValue();
     ...
     * @param iFileType type of file selected from one of the
     * constants defined in FileBase class.
     * @param asVolumeData linear array of signed 16-bit values containing
     * the volume data at each sample where the samples are identified
     * by the specified volume layout
     */
    private void saveVolume (String kFilename, int iFileType,
                             short[] asVolumeData)
    {
    ...
    // These are bit masks used in the result volume which identifies
    // how samples were classified at different steps during the
    // processing.
    // RESULT_MASK_INSIDE - subset of all samples; those that are inside
    // RESULT_MASK_NONUNIFORM_GRADIENT - subset of samples marked with
    //   RESULT_MASK_INSIDE that have nonuniform boundary distance gradients
    // RESULT_MASK_FLAGGED - subset of samples marked with RESULT_MASK_INSIDE
    //   and including all samples marked with RESULT_MASK_NONUNIFORM_GRADIENT
    //   are those samples that connected those samples marked with
    //   RESULT_MASK_NONUNIFORM_GRADIENT
    // RESULT_MASK_CONNECTED - subset of samples marked with
    //   RESULT_MASK_FLAGGED that are largest 26-connected collection
    //   of samples marked with RESULT_MASK_FLAGGED
    // RESULT_MASK_SKELETON - subset of samples marked with
    //   RESULT_MASK_CONNECTED that are the samples which make up the
    //   the skeleton of the binary volume
    // RESULT_MASK_SKELETON_NEIGHBOR - the subset of samples marked
    //   RESULT_MARK_CONNECTED that within a certain radius of samples
    //   marked as RESULT_MARK_SKELETON.
    static private final short RESULT_MASK_INSIDE = 0x0001;
    static private final short RESULT_MASK_NONUNIFORM_GRADIENT = 0x0002;
    static private final short RESULT_MASK_FLAGGED = 0x0004;
    static private final short RESULT_MASK_CONNECTED = 0x0008;
    static private final short RESULT_MASK_SKELETON_NEIGHBOR = 0x0010;
    static private final short RESULT_MASK_SKELETON = 0x0020;
    ...
    // The bitmask values are used to classify each sample during
    // the various stages of the skeletonization process.
    private short[] m_asVolumeData;

    // Array of Euclidean distances to the nearest boundary for each
    // inside voluem sample.  Distances are in sample coordinates.
    // Also compute the maximum of these values.
    private float[] m_afBoundarySampleDistMin;
    private float m_fMaxBoundarySampleDistMin;

    // This the linear array index for the sample from which all
    // computed paths will originate.
