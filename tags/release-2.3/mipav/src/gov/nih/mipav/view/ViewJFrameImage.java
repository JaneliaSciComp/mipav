...
public class ViewJFrameImage extends ViewJFrameBase implements KeyListener, MouseListener
{
    ...
    /**
     *   Integer buffer (4 bytes that stores the concatenated Alpha (1 byte), Red (1 byte),
     *   Green ( 1 byte ), Blue (1 byte ) data. The ARGB values are generated by using the
     *   imageA intensities as a index into a LUT.
     */
     ...
     *   Integer buffer (4 bytes that stores the concatenated Alpha (1 byte), Red (1 byte),
     *   Green ( 1 byte ), Blue (1 byte ) data. The ARGB values are generated by using the
     *   imageB intensities as a index into a LUT.
     */
     ...
     * Used primarily for displaying the FFT of an image.
     */
    ...
    /**
     * @deprecated - Reference to ViewUserInterface no longer required.
     * Use ViewJFrameImage(ModelImage, ModelLUT, Dimension) instead.
    ...
    /**
     * @deprecated - Reference to ViewUserInterface no longer required.
     * Use ViewJFrameImage(ModelImage, ModelLUT, Dimension, boolean) instead.
     * Makes a frame and puts an image component into it.
     *   @param _imageA         First image to display
     *   @param LUTa           LUT of the imageA (if null grayscale LUT is constructed)
     *   @param loc            location where image should be initially placed
     ...
     */
    public ViewJFrameImage(ModelImage _imageA, ModelLUT LUTa, Dimension loc,
                           ViewUserInterface ui, boolean logMagDisplay)
    {
    ...
    private void initComponentImage(int[] extents) throws OutOfMemoryError
    {
        ...
        componentImage = new ViewJComponentEditImage(this, imageA, LUTa, imageBufferA, null, null, imageBufferB,
            pixBuffer, zoom, extents, logMagDisplay, ViewJComponentEditImage.NA, false,
    ...
    /**
     * Initializes the variables based on the image extents. (i.e. number of slices,
     ...
     * @return     an array containing the slice in the volume (in the first element) and the time slice (in the second element)
     */
     ...
     * @return     an array containing the number of volume slices (in the first element) and the number of time slices in the image (in the second element)
     */
    ...
    protected static float[] initResFactor(float[] imgResols, int[] imgUnits)
    {
        ...
        if ( (imgResols[1] >= imgResols[0]) && (imgResols[1] < 20.0f * imgResols[0])
     ...
     *   Notify this componentImage that it needs to display
     *   highlight box, notifying user that this componentImage
     *   is the image being controlled
     ...
     *   proper controls.  Since the controls point to this
     *   particular image, when we remove the controls,
     *   turn off the highlight box.
     *   @see    ViewJComponentEditImage#useHighlight(boolean)
     *   @see    ViewJComponentEditImage#repaint()
     */
    public void removeControls()
    {
        ...
        {
            Preferences.debug(
                "NullPointerException in ViewJFrameImage:\n" + npe.getMessage() + "\n in removeControls().\n", 5);
        }
    }
    ...
    /**
     *   When switching the active image, copy the paintBitmap of the previous active image
     *   into the paintBitmap of the new active image
    ...
    /**
     * Updates the checkbox to show whether or not the
     * output window is currently displayed
    ...
    protected String makeTitle()
    {
        ...
        if (displayMode == IMAGE_A)
        {
            ...
            { // Setup the title for 4D image
                str = imageA.getImageName() + "  " + String.valueOf(zSlice + 1) + "/" + String.valueOf(nImage)
                    + "z  " + String.valueOf(tSlice + 1) + "/" + String.valueOf(nTImage) + "t M:"
                    + makeString(componentImage.getZoomX(), 2);
            }
            else if (imageA.getNDims() == 3)
            { // Setup the title for 3D image
                str = imageA.getImageName() + "  " + String.valueOf(zSlice + 1) + "/" + String.valueOf(nImage)
        ...
        else
        {
            ...
            { // Setup the title for 4D image of image B
                str = imageB.getImageName() + "  " + String.valueOf(zSlice + 1) + "/" + String.valueOf(nImage)
                    + "z  " + String.valueOf(tSlice + 1) + "/" + String.valueOf(nTImage) + "t M:"
                    + makeString(componentImage.getZoomX(), 2);
            }
     ...
     *   @return           boolean confirming successful update
     */
    public synchronized boolean updateImages(ModelLUT LUTa, ModelLUT LUTb,
    ...
    public synchronized boolean updateImages()
    {
        ...
        try
        {
            ...
            // componentImage.repaint(); // problems with this method on some machines seems to eat lots of  memory on JVM 1.3
        }
    ...
    public synchronized void incSlice()
    {
        ...
        if (zSlice < imageA.getExtents()[2] - 1)
        {
            ...
            { // move highlight on images in lightbox
    ...
    /**
     *  Updates the this frame's size the components sizes. If the magnified image
     *  fits into the frame that will fit into the screen then frame and image
     *  are sized appropriately. If the frame, to fit the image, exceeds the screen
     *  size the frame remains the same size and the image magnified and placed
     *  in the scroll pane.
    ...
    /** public void updateFrame(float xZoom, float yZoom, int x, int y)
     *  This method is called when the user is zooming in or out.
     *
     ...
     * This method will enlarge or shrink the window size in response to
     * the componentImage being zoomed. It will only resize the window to
     * IMAGE_SCREEN_RATIO of the screen size, in this case 3/5. If the image
     * is zoomed past that size, scrollbars are added. If the user has
     * dragged the window to a size larger than 3/5 of screen size, I
     * assume he wants it that way and the window will not be resized in
     * that case upon zoom-in.
     */
    private void computeIdealWindowSize()
    {
    ...
    public void actionPerformed(ActionEvent event)
    {
        ...
        {
            if (imageB != null)
            { // ask for confirmation if mask already exists
            ...
            if (imageA.getFileInfo()[0] instanceof FileInfoXML)
            {
                ...
                if (path != null)
                {
                    ...
                    int response = JOptionPane.showConfirmDialog(this,
                        "Maintain link to closed image?", "Linked image",
        ...
        else if (command.equals("VOIToolbar") || command.equals("PaintToolbar")
                 || command.equals("ImageToolbar") || command.equals("ScriptToolbar"))
        {
        ...
        else if (command.equals("NewVOI"))
        {
            ...
            /*
             ...
             if (getActiveImage().getVOIs().size() > 0) {
             Color lastColor = ( (VOI) ( getActiveImage().getVOIs().lastElement() ) ).getColor();
             ...
             hsb[0] *= 360.0;
             //  System.err.println("HSB Value: " + hsb[0]);
             hsb[0] = Math.round(hsb[0]/35.0);
             System.err.println("HSB Value (div 35 int): " + hsb[0]);

             hsb[0]++;

             float hue = (float)(((((hsb[0])*35)%360)/360.0));
             getControls().setVOIColor( Color.getHSBColor(hue, (float)1.0, (float)1.0) );
             }
             */
        ...
        else if (command.equals("VOIPropertiesColor"))
        {
            if (getActiveImage().getVOIs().size() > 0)
            {
                ...
                { // there was an annotation selected, do nothing
                }
                else
                {
        ...
        else if (command.equals("BinaryMask"))
        {
            ...
            if (userInterface.isScriptRecording())
            {
                userInterface.getScriptDialog().append(
                    "VOI_to_BinaryMask " + userInterface.getScriptDialog().getVar(getActiveImage().getImageName())
                    + " \n");
            }
        }
        else if (command.equals("ShortMask"))
        {
            ...
            {
                userInterface.getScriptDialog().append(
                    "VOI_to_ShortMask " + userInterface.getScriptDialog().getVar(getActiveImage().getImageName())
                    + " \n");
            }
        }
        ...
        else if (command.equals("PaintToVOI"))
        {
            ...
            AlgorithmVOIExtractionPaint algoPaintToVOI = new
                AlgorithmVOIExtractionPaint(getActiveImage(), componentImage.getPaintBitmap(), xDim, yDim, zDim,
                                            voiID);
        ...
        else if (command.equals("Open all VOIs"))
        {
            ...
            {
                userInterface.getScriptDialog().append(
                    "OpenAllVOIs " + userInterface.getScriptDialog().getVar(getActiveImage().getImageName())
                    + "\n");
            }
        }
        ...
        else if (command.equals("CommitPaint"))
        {
            if (getActiveImage() == imageA)
            {
                componentImage.commitMask(ViewJComponentBase.IMAGE_A, true, true);
            }
            else
            {
                componentImage.commitMask(ViewJComponentBase.IMAGE_B, true, true);
            }
        ...
        else if (command.equals("CommitPaintExt"))
        {
            if (getActiveImage() == imageA)
            {
                componentImage.commitMask(ViewJComponentBase.IMAGE_A, true, false);
            }
            else
            {
                componentImage.commitMask(ViewJComponentBase.IMAGE_B, true, false);
            }
        ...
        else if (command.equals("Clone"))
        {
            ...
            try
            {
                ...
                if (userInterface.isScriptRecording())
                {
                    ...
                    userInterface.getScriptDialog().append(
                        "Clone " + userInterface.getScriptDialog().getVar(getActiveImage().getImageName()) + " ");
                    userInterface.getScriptDialog().putVar(clonedImage.getImageName());
                    userInterface.getScriptDialog().append(
                        userInterface.getScriptDialog().getVar(clonedImage.getImageName()) + "\n");
                }
            }
            catch (OutOfMemoryError error)
            {
        ...
        /*
                  else if (command.equals("subsampleBy2")) {
                  /// makes streaks and i don't know why...
                  if (imageA.getNDims() == 3) {
                  int newExtents[] = new int[imageA.getNDims()];
                  float sigmas[] = new float[imageA.getNDims()];
                  newExtents[0] = imageA.getExtents()[0] / 2;
                  newExtents[1] = imageA.getExtents()[1] / 2;
                  newExtents[2] = imageA.getExtents()[2];
                  sigmas[0] = 1.0f;
                  sigmas[1] = 1.0f;
         sigmas[2] = 1.0f * (imageA.getFileInfo(0).getResolutions()[0] / imageA.getFileInfo(0).getResolutions()[2]);///
                  ModelImage resultImage = new ModelImage(imageA.getType(),
                  newExtents,
                  imageA.getImageName() + "_subsample_2",
                  userInterface);
         AlgorithmSubsample algo = new AlgorithmSubsample(imageA, resultImage, newExtents, sigmas, true, true);
                  algo.setActiveImage(false);
                  algo.run();
                  //resultImage = algo.getResultImage();
        ...
        else if (command.equals("onlyColor"))
        { // JDialogOnlyColor onlyColorDialog = new JDialogOnlyColor(this, getActiveImage());
            // onlyColorDialog.callAlgorithm();
        }
        ...
        else if (command.equals("extractSurfaceCubes"))
        {
            ...
            // } else if (command.equals("adaptiveSkeletonClimbing")) { // Superceded by Extract surface cubes
            // new JDialogClimb3D(this, getActiveImage());
            // }
        }
        ...
        else if (command.equals("EyeSeg"))
        { // new JDialogEyeSeg(this, getActiveImage());
        }
        ...
        {
            String[] akNamesCompatibleTargetImages = JDialogRegistrationBSpline.getNamesCompatibleTargetImages(
                componentImage.getActiveImage(), userInterface);
        ...
        else if (command.equals("Manual"))
        {
            ...
            if (getActiveImage().getRegistrationFrame() == null)
            {
                if (getActiveImage().getNDims() == 2)
                {
                    ...
                    JDialogLoadImageForRegistration dialog = new
                        JDialogLoadImageForRegistration(this);
                    ...
                    if (image2load != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                ViewJFrameImage adjFrame = new ViewJFrameImage(image2load, dialog.getModelLUT(), new Dimension(0, 0));
                            }
                        }
                        algoRef.finalize();
                        algoRef = null;

                        ViewJFrameRegistrationTool registrationFrame = new ViewJFrameRegistrationTool(refImage, adjImage, refLUT, adjLUT);
        ...
        else if (command.equals("Test algorithm"))
        { // new JDialogRegionGrow(this, getActiveImage());
        }
        ...
        /*
                  else if (command.equals("Transform with Padding")) {
                  new JDialogTransformWithPad(this, getActiveImage());
                  }
         */
        ...
        else if (command.equals("LinkFrame"))
        {
            ...
            else
            {
                ...
                {
                    MipavUtil.displayError(
                        "There is no image with the same z dimension as\n" + getActiveImage().getImageName()
                        + " to operate on.");
                }
                else if (getActiveImage().getNDims() == 4)
                {
                    MipavUtil.displayError(
                        "There is no image with the same z and t dimensions as\n" + getActiveImage().getImageName()
                        + " to operate on.");
                }
                return;
            }
            ...
            try
            {
                ...
                if ( (units[1] != unit || units[2] != unit) || diffRatio >= 20)
                {
                    int reply = JOptionPane.showConfirmDialog(this,
                        ...
                        "Adjust Image Resolution/Units", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
        ...
        else if (command.equals("MagSettings"))
        {
            JDialogMagnificationControls magSettings = new
                JDialogMagnificationControls(this, componentImage, componentImage.getZoomX(), null);
        ...
        else if (command.equals("Surface plotter"))
        {
            ...
            {
                Preferences.debug(
                    "ViewJFrameSurfacePlotter cannot be called; encountered "
            ...
            {
                Preferences.debug(
                    "ViewJFrameSurfacePlotter cannot be called as there was " + "not enough memory allocated.  \n"
                    + "The error is: \n" + notEnoughError.getLocalizedMessage());
                MipavUtil.displayError(
                    "The surface plotter requires more memory " + "than is currently available;\n"
                    + "See the Memory Allocation menu");
            }

        }
        /*
                  else if (command.equals("Jogl")) {
                  new InfiniteShadowVolumes().run();
                  }
                  else if (command.equals("Gears")) {
                  new Gears();
                  }
                  else if (command.equals("GearsRotate")) {
                  new GearsRotate();
                  }
                  else if (command.equals("GearsDebug")) {
                  new GearsDebug();
                  }
        ...
        { // new win-level window when it does not exist
            if (getActiveImage() == imageA)
            {
                if (windowLevel[0] == null)
                {
            ...
            if (componentImage.getImageA().getLightBoxFrame() == null)
            {
                ...
                try
                {
                    lightBoxFrame = new ViewJFrameLightBox(this, "LightBox", imageA, componentImage.getLUTa(), imageB,
                        componentImage.getLUTb(), componentImage.getResolutionX(), componentImage.getResolutionY(),
    ...
    /**
     * Quick and simple way for ViewJFrameBase to tell the ViewJFrameImage it created
     * to set the menu (right now only for 2Dimensional) for image B.
    ...
    public void close()
    {
        ...
        {
            userInterface.getScriptDialog().append(
                "CloseFrame " + userInterface.getScriptDialog().getVar(getActiveImage().getImageName()) + "\n");
        }
        ...
        while (names.hasMoreElements())
        {
            ...
            if (!getActiveImage().getImageName().equals(name))
            {
                ...
                catch (IllegalArgumentException iae)
                {
                    ...
                    Preferences.debug(
                        "Illegal Argument Exception in " + "ViewJFrameImage.isMultipleSameSizeImages(). "
    ...
    /**
     *   Helper method to establish if there are other images
     *   so that a dialog can be created.
     *   Used by registration methods, load frame, etc.
     *   @return <code>true</code> if there are images to operate on.
     */
    ...
    private void handleMaskToPaint()
    {
        if (componentImage != null)
        {
            float[] intensityMapB = new float[imageA.getExtents()[0] * imageA.getExtents()[1]]; // make intensity map same size as image dimensions
            ...
            if (imageB != null)
            {
                ...
                try
                {
                    ...
                    for (int currentSlice = 0; currentSlice < numSlices; currentSlice++)
                    {
                        ...
                        // examine every pixel and convert to paint if masked intensity is equal to the toolbar's selected intensity
            ...
            else
            {
                ...
                MipavUtil.displayError(
                    "This function is only useful when the image has a mask. To use this feature, please add a mask.");
            }
        }
        else
        {
