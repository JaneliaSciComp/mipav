...
/**
 * Abstract class used for generating custom components, especially
 * within images.
 *
...
public abstract class ViewJComponentVolOpacityBase
    ...
    /**
     * This method converts a transfer function point value into screen space so that it can
     * be displayed on the histogram. This is necessary because the histogram that is painted
     * on the screen is typically has a different range than the transfer function.
     *
    ...
    /**
     * This method converts a point in screen space into a transfer function point value. This
     * is necessary because the histogram that is painted on the screen is typically has a
     * different range than the transfer function.
    ...
    /**
     * Abstract method to force extended classes to implement
     * a disposal method to clean up memory.
     */
    public void dispose() {
    ...
    protected void drawAxisAccoutrements(Graphics2D graphics)
    {
        ...
        if (logFlag == true)
        { // add ticks and values on right side of histogram component (log)
            double number = 0.5;
            for (int i = 0; i < 4; i++)
            {
                int yLog = (int) (255 - (int) (number * 255 - 0.5));
                graphics.drawLine(254, yLog, 260, yLog);
                graphics.drawString(String.valueOf( (int) (Math.exp(histogramMaxLog * number) + 0.5)), 270
                                    - String.valueOf( (int) (Math.exp(histogramMaxLog * number) + 0.5)).length(),
                                    yLog + 5);
                number = number * 0.5;
        ...
        { // add ticks and values on right side of histogram component - linear scale
     ...
     * have been formatted (packed) properly (ie, aRGB).
    ...
    protected boolean isNearLine(int offsetMouseX, int offsetMouseY)
    {
        for (int i = 0; i < transferFunction.size() - 1; i++)
        {
            ...
            if (MipavMath.distance(offsetMouseX, x1, offsetMouseY, y1) +
                MipavMath.distance(offsetMouseX, x2, offsetMouseY, y2) - MipavMath.distance(x1, x2, y1, y2) < 0.5)
            { // is the mouse cursor close to a line?
    ...
    protected void drawTransferFunctionHandles(Graphics2D graphics, TransferFunction tf)
    {
        ...
        {
            graphics.setColor(Color.black);
            graphics.fillRect( (int) (convertFnPtToScreenSpaceX(tf.getPoint(i).x) - 2), (int) (tf.getPoint(i).y) - 2, 5, 5);
            if (tfActiveIndex == i)
            {
            ...
            graphics.drawRect( (int) (convertFnPtToScreenSpaceX(tf.getPoint(i).x) - 2), (int) (tf.getPoint(i).y) - 2, 5, 5);
        }
    }
    ...
    private void drawTransferFunctionHandleTicks(Graphics2D graphics, TransferFunction tf)
    {
        ...
        for (int i = 0; i < tf.size(); i++)
        {
            ...
            FontMetrics fontMetrics = graphics.getFontMetrics();
            graphics.drawString(valueString, -5 - (fontMetrics.stringWidth(valueString)), (int) tf.getPoint(i).y + (fontMetrics.getAscent() / 2));
            ...
            if (i % 2 == 0) // stagger every other tick to prevent overlap along the horizontal axis
            {
                ...
                graphics.drawString(valueString,
                                    (int) (convertFnPtToScreenSpaceX(tf.getPoint(i).x)) - (fontMetrics.stringWidth(valueString) / 2),
    ...
    /**
     * Filters the histogram through the LUT and produces an int array (aRGB)
     * that can be converted to a java image of the histogram for display purposes.
...
*/
    ...
    public void mousePressed(MouseEvent mouseEvent)
    {
        ...
        for (int i = 0; i < transferFunction.size(); i++)
        {
            ...
            {
                parentPanel.setAdjustersEnabled(!transferFunction.isEndpoint(i)); // user is adjusting a non-endpoint, so enable adjustment components
                addPointFlag = false; // user has clicked on a point, and is therefore not adding one (might be preparing to drag the point)
                tfActiveIndex = i;
    ...
    public void mouseDragged(MouseEvent mouseEvent)
    {
        ...
        /**
     *   Changes the cursor so that function can add points
     *   depending on how near the line it is
     *   @param mouseEvent   event that triggered this function
     */
