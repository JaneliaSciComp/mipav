...
/*
 * Introduction: The goal of this task is to add the ability for the user to
 * draw a region of interest on a triangle mesh surface, by allowing the user
 * to select points on the mesh surface and calculating the connecting
 * polyline on the surface between the points.  Pairs of consecutive points
 * are connected by a geodesics path.  The geodesic path is the shortest
 * surface path between the points.  For a triangle mesh, the final boundary
 * curve for the region of interest is a polyline.
 *
 * To compute the polyline curve I start with Dijkstra's Single-Source
 * Shortest Path graph algorithm to find the shortest path along triangle
 * edges between the pair of points on the mesh.  In the algorithm, the
 * triangle vertices and edges are the nodes and edges of the graph, and the
 * Euclidean distance between vertices connected by an edge serves as the edge
 * weight. The result of Dijkstra's algorithm is a path that follows the edges
 * in the mesh and may be jagged.  I then iterate over the points on the
 * polyline curve and the edges of the triangle mesh the curve crosses, to
 * compute new points on the curve that lie on the triangle edges but not
 * necessarily on the triangle vertices.  This produces a smoother curve.
 *
 * Implementation: Drawing the user-selected region of interest on a surface
 * is implemented through the Geodesic class in Geodesic.java.  The Geodesic
 * class may be used in two different ways. It may be used as a MouseListener
 * that implements picking points on the surface of the mesh with the mouse
 * and drawing the region of interest directly on the mesh.  Or it may be used
 * just to calculate the geodesic curve between two points on the triangle
 * mesh. When the Geodesic class is used just to calculate the geodesic curve
 * between two points on the triangle mesh, the points on the mesh must be
 * specified prior to calculation. The points may be determined through a
 * different picking implementation, or may be determined some other way.  The
 * interface to the Geodesic class allows the programmer to set the end points
 * directly and to read the resulting geodesic curve.
 *
 * Calculating the Geodesic Curve
 *
 * The first step in calculating the geodesic curve on the triangle mesh,
 * whether the Geodesic class implements MouseListener or not, is to
 * initialize the local copy of the triangle mesh and the Single-Source
 * Shortest Path Algorithm data structures based on the locations of the two
 * endpoints of the path.
 *
 * The two endpoints of the path are specified in local mesh coordinates. They
 * are not restricted to vertices in the triangle mesh, but may fall inside a
 * triangle. When the endpoints fall inside a triangle, the triangle is
 * divided into three new triangles, by connecting the new vertex to each of
 * the three triangle vertices.  This modification only affects the local copy
 * of the triangle mesh.
 *
 * In Dijkstra's Single-Source Shortest Path Algorithm, each vertex and edge
 * in the mesh has a weight; vertex weights represent the distance from the
 * source vertex to that vertex, edge weights represent the distance between
 * the two vertices the edge connects. In this implementation of Dijkstra's
 * algorithm the edge weight is the Euclidean distance between the two
 * vertices.
 *
 * Optimization: At the start of the algorithm each vertex is initialized to
 * Float.MAX_VALUE, to represent the distance from the start node to that node
 * has not yet been determined.  Because the nodes and edges fall on a
 * triangle mesh, where distances are based on actual distances in
 * 3-dimensions, we can use an optimization to the standard Dijkstra's
 * algorithm. This optimization uses an additional weight factor, which is the
 * straight-line distance from the vertex to the end vertex.  This is
 * calculated for each vertex in the triangle mesh.
 *
 * Dijkstra's algorithm is a Greedy Algorithm. As it proceeds, points are
 * examined, "relaxed", and potentially added to the final path based on the
 * distance from the start vertex - the vertex not yet on the shortest path
 * with the lowest weight is examined first.  In the optimized version, the
 * vertex with the lowest weight plus the lowest straight-line distance to the
 * end point is examined, or "relaxed" first. This causes the area of vertices
 * that Dijkstra's algorithm searches to be weighted in the direction of the
 * end vertex.  A non-optimized search pattern is symmetrical, and spreads out
 * in a spherical pattern around the start vertex. The optimized search
 * pattern appears conical, and points in the direction of the end vertex.
 *
 * Dijkstra's algorithm returns a list of points on the triangle mesh that
 * connect the start and end vertices. The path travels along edges in the
 * mesh and so may appear jagged. The final step in the Geodesic class
 * algorithm is to smooth the polyline by moving points that fall on triangle
 * vertices along the triangle edges.  The final polyline contains points that
 * are on triangle mesh edges, but are not constrained to the triangle
 * vertices.
 *
 * The smoothing process is iterative.  Given two points in the polyline that
 * are connected by a path that goes through one triangle vertex, the
 * intermediate vertex is moved across the edges that extend from it, until a
 * new position is found that minimizes the distance between the two
 * points. If more than one edge is crossed, then a new vertex for each new
 * edge crossed is added to the polyline.  The smoothing proceeds for each
 * pair of points on the polyline.
 *
 *
 * Added 7/31/05: LiveWire mode.  LiveWire mode enables the user to
 * interactively watch the Dijkstra's path being drawn between the last point
 * placed on the curve to the current mouse location. LiveWire mode does not
 * compute the smoothed geodesic interactively, but instead, waits for the
 * user to place points on Dijkstra's curve and then finish the curve -- by
 * pressing either the "Finish Open" or "Finish Closed" buttons in the
 * interface. Once the curve is finished, then the smoothed geodesic is
 * calculated and the triangle mesh is re-triangulated along the curve.
 *
 * Added: Cutting the mesh along the Geodesic.
 *
...
{
    /**
     * Instantiaion of the Geodesic object, with the objects necessary for the
     * Geodesic to serve as a MouseListener that performs picking and with the
     * Group kGeodesicGroup so that the Geodesic curve can be drawn directly
     * on the ModelTriangleMesh.
     ...
     */
    public Geodesic( PickCanvas kPickCanvas, Group kGeodesicGroup,
                     ModelTriangleMesh kMesh, float fRadius )
    {
    ...
    /**
     * Instantiation without initializing the progress bar, pickCanvas,
     * GeodesicGroup, triangle mesh or sphere radius, each of those can be set
     * through individual member access functions: */
    public Geodesic( )
    {
        initColors();
    }
    ...
    /**
     * Initializes the colors for the first point on the curve, and the
     * successive points.
     */
    ...
    /**
     * cleanUp deletes the data stuctures used for the Dijkstra's search, but
     * does not delete the array of points on the Geodesic curve.
     */
    ...
    /**
     * One of the overrides necessary to be a MouseListener.  Invoked when a
    ...
    public void mousePressed (MouseEvent kMouseEvent)
    {
             ...
             !m_bLastWire  )
        {
             ...
             * prevent multiple mouse clicks at the same location.  If the
             * mouse has been released, then set mousePressed to true and save
            ...
            if ( m_bMouseReleased == true )
            {
                 ...
                 * to finish the curve or to remove the last point added: */
                          ...
                          ( kMouseEvent.getButton() == MouseEvent.BUTTON3 ) )
                {
                    m_bMousePressed = true;
                    m_bMouseReleased = false;
    ...
    public void mouseReleased (MouseEvent kMouseEvent)
    {
        ...
        if ( m_bMousePressed && !m_bMouseReleased )
        {
            ...
            if ( kPickResult != null )
            {
                ...
                Point3f kPickPoint =
                    new Point3f( kPick.getPointCoordinates() );
                ...
                if ( m_iNumPicked == 1 )
                {
                    ModelTriangleMesh kMesh
                        = (ModelTriangleMesh)kPickResult.getGeometryArray();
                    setStartSurface( kMesh );

                    setStartPoint( kPickPoint, true );
                    setStartIndices( kPick.getPrimitiveCoordinateIndices(),
                                     true );
                    ...
                    if ( m_bLivewire )
                    {
                        drawDijkstraEuclidianPoint( m_kStartPoint,
                                                    m_kPickColors[ m_iNumPicked%2 ] );
                    }
                    /* Otherwise add the point to all three scene graphs:
                     * SmoothedGeodesic, Dijkstra, and Euclidian: */
                    else
                    {
                        drawPoint( m_kStartPoint,
                                   m_kPickColors[ m_iNumPicked%2 ] );
                    }
    ...
    /**
     * mouseMoved. In livewire mode the mouseMoved function updates the
     * endpoints of the Dijkstra's geodesic and computes Dijkstra's path
     * between the last point added and the mouse position. Only Dijkstra's
     * and the Euclidian paths are updated, the smoothed geodesic is not
        ...
        if ( m_iNumPicked > 0 )
        {
            ...
            if ( kPickResult != null )
            {
                ...
                else
                {
                    ...
                    /* Get the coordinates of the picked point on the mesh,
                     * this is the closest mesh vertex, not a new vertex
                     * inside a triangle. If the picked point falls inside a
                     * triangle this is handled when the livewire curve is
                     * finished and the smoothed geodesic curve is
                     * calculated: */
    ...
    public void keyReleased(KeyEvent kEvent)
    {
         ...
         * deactivate livewire tracking and stop the dijkstra at the last
         * saved point on the mesh -- which is set in the MouseMoved
         * function. */
        if ( m_bLivewire &&
             !m_bLastWire &&
             (kEvent.getKeyCode() == KeyEvent.VK_CONTROL) )
        {
            m_bMousePressed = true;
            m_bMouseReleased = false;
    ...
    /**
     * Access function to set the pickCanvas. This is necessary for the
     * Geodesic class to do picking with the mouse.
     * @param kPickCanvas PickCanvas
     */
    ...
    /**
     * Access function to set the Group object m_kGeodesicGroup. This is
     * necessary for the Geodesic object to draw the geodesic curve on the
     * mesh. The curve may be drawn in one of three ways: (1) the
     * straight-line Euclidian curve, which is not constrained to lie on the
     * mesh surface, or (2) Dijkstra's path, which falls along the original
     * mesh triangle edges, or (3) the Smoothed Geodesic, which is constrained
     * to lie on the mesh surface, but which may cross triangle edges. All
     * three display modes are represented by a different BranchGroup.
    ...
    /* Access function to set the triangle mesh that the geodesic curve is
     * calculated on. */
    private void setSurface( ModelTriangleMesh kMesh )
    {
        ...
        if ( m_kOriginal == null )
        {
            m_kOriginal = new ModelTriangleMesh( kMesh.getVertexCopy(),
                                                 kMesh.getNormalCopy(),
                                                 kMesh.getIndexCopy() );
    ...
    /**
     * The start and end surfaces ensure that the points on the geodesic curve
     * all fall on one mesh, and that the algorithm isn't trying to find a
     * path between two unconnected meshes.
    ...
    /**
     * Called by the JPanelGeodesic interface to switch between displaying the
     * Smoothed Geodesic, Dijkstra's path, or the Euclidian path.
    ...
    public void setEnable( boolean bEnable )
    {
        ...
        m_iNumPicked = 0;
        if ( ( m_bEnabled == false ) &&
             ( m_kSmoothedGeodesicGroup.numChildren() > 0 ) )
        {
    ...
    /**
     * Set the radius of the spheres used to mark the start and end points on
     * the geodesic curve.
     ...
     * @param boolean bAll, when true clears all the geodesic curves, when
     * false, clears the last point drawn:
     */
    ...
    /**
     * Remove all the geodesic lines and reset to draw the new
     * geodesic lines.
     */
     ...
     * @param boolean, bOpen: when true leaves the geodesic open, when false
     * closes the geodesic by connecting to the first point on the polyline
     * sequence.
     */
        ...
        if ( m_bLastWire )
        {
            ...
            for ( int iPath = 0; iPath < m_kStartEndList.size(); iPath++ )
            {
                ...
                if ( computeGeodesic( 0.99f, true ) )
                {
                    ...
                    if ( iPath == 0 )
                    {
                        drawGeodesicPoint( m_akCoordinates[ m_iEnd ],
                                           m_kPickColors[ 1 ] );
                    }
                    drawGeodesicPoint( m_akCoordinates[ m_iStart ],
                                       m_kPickColors[ 0 ] );
    ...
    public void cut()
    {
        ...
        else
        {
            ...
            iVertexCount +=
                unZip( m_kModified, kClosedPath,
                       iVertexCount, false, kNewClosedPath );
            kGeodesic_Separate.add( kNewClosedPath );
        }


        ModelTriangleMesh kCutMesh =
            createNewMesh( m_kModified, iVertexCount, iOldVertexCount );
        ...
        if ( kGeodesic_Separate.size() > 0 )
        {
            ...
            m_aiIndexShift = null;
            m_abRemoveTris = null;
            int iNumVertsRemoved = findNewMeshes( kCutMesh,
                                                  kGeodesic_Separate );
        ...
        if ( m_kPanel != null )
        {
            ...
            m_kSurfaceBackup = new ModelTriangleMesh( m_kSurface.getVertexCopy(),
                                                      m_kSurface.getNormalCopy(),
            ...
            m_kSurfaceBackup.
                setCapability( IndexedGeometryArray.ALLOW_COLOR_INDEX_READ );
            m_kSurfaceBackup.
                setCapability( IndexedGeometryArray.ALLOW_NORMAL_INDEX_READ );
            m_kSurfaceBackup.
                setCapability( IndexedGeometryArray.ALLOW_TEXCOORD_INDEX_READ );
        ...
        /* Delete local vars: */
        m_kNewVerts.clear();        m_kNewVerts = null;
        m_kNewNormals.clear();      m_kNewNormals = null;
        m_kNewTriangles.clear();    m_kNewTriangles = null;
        m_kRemoveTriangles.clear(); m_kRemoveTriangles = null;
    ...
    /**
     * unZip cuts along the smoothed geodesic path by creating new vertices
     * for each point on the path, and updating the triangles that are
     * connected to the path and that fall on the right of the path to contain
     * the new path vertices. This cuts the mesh by disconnecting the
     * triangles that are on the left and right sides of the geodesic path.
     * @param kMesh ModelTriangleMesh surface mesh
    ...
    private int unZip( ModelTriangleMesh kMesh, LinkedList kPath, int iVertexCount,
                       boolean bOpen, LinkedList kNewPath )
    {
        ...
        {
            if ( ((Point4f)kPath.get( iPath - 1 )).w ==
                 ((Point4f)kPath.get( iPath )).w         )
            {
                kPath.remove( iPath );
            }
            else
        ...
        if ( !bOpen && (kNewPath != null) )
        {
            ...
            kNewPath.add( new Integer( getPathIndex( kPath,
                                                     iVertexCount,
                                                     iPathIndex, bOpen ) ) );
        }
        ...
        for ( iPath = 1; iPath < kPath.size(); iPath++ )
        {
            ...
            if ( iPath == 1 )
            {
                ...
                if ( iNumEndPoints == 2 )
                {
                    if ( onRight( kMesh, iPrevPathIndex, iPathIndex,
                                  ((Integer)kEndPoints.get( 0 )).intValue() ) )
                    {
                        iEndIndex = ((Integer)kEndPoints.get( 0 )).intValue();
                    }
                    else
            ...
            while ( (iEndIndex != iNextPathIndex) &&
    ...
    /**
     * Contains determines if the linked list kTriList contains the input
     * triangle, which is specified by three vertex indices. The LinkedList
     * member function contains is not used because the indices may be in a
     * different order, and the function must return true if any of the
     * triangles in the list match the input triangle regardless of the order
    ...
    private boolean contains( LinkedList kTriList, Point3i kNewTri )
    {
        ...
        for ( int iNewTri = 0; iNewTri < kTriList.size(); iNewTri++ )
        {
            ...
            if ( triangleEquals( kNewTri.x, kNewTri.y, kNewTri.z,
                                 kTri.x, kTri.y, kTri.z ) )
            {
                bReturn = true;
            }
        }
    ...
    /**
     * onRight determines if a given point on a triangle, kSide, is to the
     * right of or to the left of the vector specified by the vertices
     * kPrev - kPoint.
     * @param kMesh ModelTriangleMesh surface mesh
    ...
    private boolean onRight( ModelTriangleMesh kMesh,
                             int iPrevIndex, int iIndex, int iSideIndex )
    {
    ...
    /**
     * getPathIndex returns what the new vertex index should be. If the input
     * index, iIndex, does fall on the cut path, kPath, then that point on the
     * path is going to be duplicated to disconnect the triangles on either
     * side of the cut path. Therefore the input index needs to be translated
     * into a new vertex index, based on the current iVertexCount, the point's
     * position along the path, and wehter or not the path is open or
     * closed.
     * @param kPath LinkedList  path link list
     * @param iVertexCount int   vertex count
    ...
    /**
     * Used in cutting closed paths and creating new meshes to export to the
     * scene graph.
    ...
    private void outputDeletedAsNew(  )
    {
        ...
        kVertices.clear();        kVertices = null;
        kNormals.clear();          kNormals = null;

        ModelTriangleMesh kAddMesh =
            new ModelTriangleMesh( kCoordinates,
                                   kNormalVectors,
                                   aiConnect );
        ...
        if ( m_kPanel != null )
        {
            m_kPanel.addMesh( m_kSurface,
                              kAddMesh,
                              new String( "Geodesic_" + m_iNumNewMeshes++ + ".sur" ) );
        }
    }
    ...
    /**
     * Used when new triangles are added to the mesh, either when the mesh is
     * triangulated along the smoothed geodesic curve, or when the mesh is
     * cut.  sortTriIndex sorts the triangle indices so that the triangle is
     * always front-facing and that the normals are correct for rendering
     ...
     */
    private void sortTriIndex( Point3i aiAddTri,
                               Point3f[] akVertices,
                               Vector3f[] akNormals  )
    {
        int i0 = aiAddTri.x;
    ...
    /**
     * Set the index of the vertex in the triangle mesh where the Geodesic
     * curve is to start. The index must be less than or equal to the number
     * of vertices in the triangle mesh.
     * @param iIndex, int the index of the vertex in the triangle mesh where
     * the Geodesic curve is to start.
     */
    ...
    /**
     * Set the index of the vertex in the triangle mesh where the Geodesic
     * curve is to end. The index must be less than or equal to the number
     * of vertices in the triangle mesh.
     * @param iIndex, int the index of the vertex in the triangle mesh where
     * the Geodesic curve is to end.
     */
    ...
    /**
     * Sets the start point of the geodesic curve on the mesh. The point
     * coordinates must be in local mesh coordinates.
     * @param kPoint the point on the triangle mesh where the geodesic curve
     * is to start, in mesh coordinates.
    ...
    /**
     * In livewire mode the endpoints of each path segment along Dijkstra's
     * curve -- between each of the user-selected points -- are stored so that
     * the smoothed geodesic may be calculated and displayed later.
     */
    private void saveStartEnd()
    {
        Point3i kEndIndex = new Point3i( m_aiEndIndex[0],
                                         m_aiEndIndex[1],
                                         m_aiEndIndex[2]  );

        Point3i kStartIndex = new Point3i( m_aiStartIndex[0],
                                           m_aiStartIndex[1],
                                           m_aiStartIndex[2]  );
     ...
     * In livewire mode the endpoints of each path segment along Dijkstra's
     * curve -- between each of the user-selected points -- are stored so that
     * the smoothed geodesic may be calculated and displayed later. This
     * function retrieves the stored values when the livewire path is
     * finished
    ...
    /**
     * clears the last points added in livewire mode when clear last point is
    ...
    /**
     * clears the all the points added in livewire mode when clear all is
     * called from the user interface.
     */
    ...
    /**
     * Sets the previous start point of the geodesic curve on the mesh.  Used
     * when the last point is deleted, the start point reverts to the
     * previous start point.
     * The point coordinates must be in local mesh coordinates.
     * @param kPoint the point on the triangle mesh where the geodesic curve
     * is to start, in mesh coordinates.
     */
    ...
    /**
     * Sets the indices of the triangle that the previous start point is located
     * in. This is used when the start point falls on the inside of a triangle
     * in the mesh, the indices parameter defines which triangle the start
     * point falls in.
    ...
    /**
     * Sets the end point of the geodesic curve on the mesh. The point
     * coordinates must be in local mesh coordinates.
     * @param kPoint the point on the triangle mesh where the geodesic curve
     * is to end, in mesh coordinates.
     */
    ...
    /**
     * Sets the indices of the triangle that the start point is located
     * in. This is used when the start point falls on the inside of a triangle
     * in the mesh, the indices parameter defines which triangle the start
     * point falls in.
    ...
    /**
     * Sets the indices of the triangle that the end point is located in. This
     * is used when the end point falls on the inside of a triangle in the
     * mesh, the indices parameter defines which triangle the end point
     * falls in.
     * @param iIndices, int[3] array of vertex indices defining the end
     * triangle.
     */
    ...
    /**
     * Calculates and returns the start point normal for a new starting point
     * inside an existing triangle. The new normal is the average of the
     * normals at each point in the triangle the starting point is inside
     * @param kMesh ModelTriangleMesh  surface mesh
    ...
    private Vector3f getNormal( ModelTriangleMesh kMesh, int[] aiIndex )
    {
        ...
        kNormal.x = (kNormal0.x +
                     kNormal1.x +
                     kNormal2.x   )/3.0f ;

        kNormal.y = (kNormal0.y +
                     kNormal1.y +
                     kNormal2.y   )/3.0f ;

        kNormal.z = (kNormal0.z +
                     kNormal1.z +
                     kNormal2.z   )/3.0f ;
    ...
    /**
     * Access to the ith point on the Geodesic curve. All points on the
     * geodesic curve lie on the triangle mesh.
    ...
    /**
     * Access to the all the points on the Geodesic curve. All points on the
     * geodesic curve lie on the triangle mesh.
    ...
    /**
     * Compute the Geodesic curve. The triangle mesh, start, and end points,
     * start and end indices must be defined before this function is called.
     * @param fPercentage, float, the optimization parameter for Dijkstra's
     * shortest-path search. Values between 0-100, (increasing optimization).
    ...
    public boolean computeGeodesic( float fPercentage, boolean bSmoothed )
    {
        ...
        while ( (iSmallest != -1) && (iSmallest != m_iEnd) )
        {
            ...
            if ( m_kPBar != null )
            {
                m_kPBar.setValue( (int)(100 * ((float)iCount++ /
                                               (float)m_iVertexCount)));
     ...
     *
     * If the start and end vertices do not fall on a triangle vertex, then
     * the triangle they fall inside split into three new triangles, by
     * connecting the new vertex to each of the triangle vertices.
     * @param fPercentage float the optimization parameter for Dijkstra's
    ...
    private void initializeGeodesic( float fPercentage )
    {
        ...
        {
            if ( triangleEquals( m_aiStartIndex[0],
                                 m_aiStartIndex[1],
                                 m_aiStartIndex[2],
                                 m_aiEndIndex[0],
                                 m_aiEndIndex[1],
        ...
        if ( bAddEnd )
        {
            iEndEdge = checkOnEdge( m_kSurface,
                                    m_kEndPoint,
                                    m_aiEndIndex,
                                    kEndNormal );
            if ( iEndEdge != -1 )
        ...
        {
            m_aiIndex[ iTriCount * 3 + 0 ] =
                m_iVertexCount - iNumberNewVertices;
    ...
    /**
     * Returns true if the first three triangle indices equal the second three
     * indices
     ...
     */
    private boolean triangleEquals( int i0, int i1, int i2,
                                    int iP0, int iP1, int iP2  )
    {
        if ( (i0 == iP0) &&
             (i1 == iP1) &&
             (i2 == iP2 )   )
    ...
    /**
     * if the two of the first three triangle indices equal the second two
     * indices, then the third index is returned.
     * @param i0 int  first triangle indice 1
     * @param i1 int  first triangle indice 2
     * @param i2 int  first triangle indice 3
     * @param iP0 int second triangle indice 1
     ...
     */
    private int triangleEdge( int i0, int i1, int i2,
                              int iP0, int iP1  )
    {
        if ( (i0 == iP0) &&
             (i1 == iP1)    )
    ...
    /**
     * Determines an appropriate epsilon, based on the size of the triangles
     * in the mesh. The epsilon is used to determine if points are "close
     * enough" to the triangle vertices or edges.
     */
    ...
    /**
     * Given a point which is known to be inside a triangle, and that
     * triangle, this function determines which edge, if any, that point falls
     * on.
     * @param kMesh ModelTriangleMesh surface
                             ...
                             Vector3f kNormal )
    {
        ...
        Vector3f kEdge = new Vector3f( kTri0.x - kTri1.x,
                                       kTri0.y - kTri1.y,
                                       kTri0.z - kTri1.z  );
        kEdge.normalize();
        Vector3f kP_Edge = new Vector3f( kPoint.x - kTri1.x,
                                         kPoint.y - kTri1.y,
                                         kPoint.z - kTri1.z  );
        float fDot = kEdge.dot( kP_Edge );
        Point3f kNewPoint = new Point3f( (fDot * kEdge.x) + kTri1.x,
        ...
        if ( iReturn == -1 )
        {
            kEdge = new Vector3f( kTri0.x - kTri2.x,
                                  kTri0.y - kTri2.y,
                                  kTri0.z - kTri2.z  );
            kEdge.normalize();
            kP_Edge = new Vector3f( kPoint.x - kTri2.x,
                                    kPoint.y - kTri2.y,
                                    kPoint.z - kTri2.z  );

            fDot = kEdge.dot( kP_Edge );
            kNewPoint = new Point3f( (fDot * kEdge.x) + kTri2.x,
        ...
        if ( iReturn == -1 )
        {
            kEdge = new Vector3f( kTri2.x - kTri1.x,
                                  kTri2.y - kTri1.y,
                                  kTri2.z - kTri1.z  );
            kEdge.normalize();
            kP_Edge = new Vector3f( kPoint.x - kTri1.x,
                                    kPoint.y - kTri1.y,
                                    kPoint.z - kTri1.z  );
            fDot = kEdge.dot( kP_Edge );
            kNewPoint = new Point3f( (fDot * kEdge.x) + kTri1.x,
                                     (fDot * kEdge.y) + kTri1.y,
                                     (fDot * kEdge.z) + kTri1.z  );
    ...
    /**
     * This function determines the shortest path from the start vertex
     * through the input vertex to the vertices that neighbor the input
     * vertex.
     *
     * The function relaxEdges looks at all the vertices connected by triangle
     * edges to the input vertex and calculated the weight factors for each of
     * those vertices, adding those vertices that are not yet "relaxed" to the
     * list of vertices on the border.
     *
     * Once the weight factors for each of the vertices connected to the input
     * vertex are set, the input vertex is labeled "relaxed" and removed from
     * the list of vertices on the border.
    ...
    private void relaxEdges( int iNode )
    {
        ...
        for ( int iEdge = 0; iEdge < iNumEdges; iEdge++ )
        {
             ...
             * edge: */
            iNeighbor = ((Integer)
                         m_akEdgeList[ iNode ].get( iEdge )).intValue();
             ...
             * the input vertex. This is done by looking at the smoothed path
             * through the current node to the neighbor. If the neighbor
             * already has a path through it, then save it incase it's
             * smaller: */
            ...
            fPathLength = m_fWeight[ iNode ] +
                distance( m_akCoordinates[ iNode ],
                          m_akCoordinates[ iNeighbor ]   );
    ...
    /**
     * The findSmallest function searches through the list of vertices that
     * are not yet relaxed but that have been visited by the Dijkstra Search,
     * so that the weight factor is not Float.MAX_VALUE, but also is not the
     * minimum value for that node. In Dijkstra's search the vertex with the
     * smallest weight is relaxed first, a greedy algorithm that chooses the
     * locally closest vertex to add to the path. The final weight for the
     * vertex is determined when it is relaxed.
     *
     * This function uses the data member m_kBorder -- the linked list of
     * vertices that have been visited by Dijkstra's search, but which have
     * not yet been relaxed.
    ...
    /**
     * createPath starts with the results of Dijkstra's minimum path
     * algorithm. It smooths the path, and stores the resulting points in the
     * m_kGeodesicPath data member.
    ...
    public boolean createPath( int iStart, int iEnd )
    {
         ...
         * points along the path, except the first and last points, create a
         * list of edges that extend to the left and right of that point. If
         * there are unequal numbers of left and right edges, then repeat the
         * last edge for the side with fewer edges:
         */
        LinkedList kEndPoints = new LinkedList();
        int iNumEndPoints = 0;
        ...
        if ( iPrevious == -1 )
        {
            System.err.println( "error computing geodesic: Start = " +
                                iStart + " End = " + iEnd );
            return false;
        }
        ...
        while ( iPrevious != iStart )
        {
            ...
            else
            {
                ...
                {
                    if ( kLeft.getLast().equals( kEndPoints.get( 0 ) ) ||
                         kRight.getLast().equals( kEndPoints.get( 1 ) )    )
                    {
                        kLeft.add( new Integer( ((Integer)kEndPoints.get( 0 )).intValue() ) );
                        iLeftPrev = iNode;
            ...
            while ( !bDone )
            {
                ...
                if ( iNode != -1 )
                {
                    iNumEndPoints = findTriPoints( iNode,
                                                   iPrevious,
                                                   kEndPoints );
        ...
        while ( iPath < (kMiddle.size() - 1) )
        {
            ...
            if ( iSide != -1 )
            {
                if ( iSide != iPrevSide )
                {
                    ...
                    kNewVert.add( new Point4f( m_akCoordinates[ iCoord ].x,
                                               m_akCoordinates[ iCoord ].y,
                                               m_akCoordinates[ iCoord ].z,
                                               iCoord ) );
                }
            }
            ...
            while ( ((Integer)kMiddle.get( iPath )).intValue()     ==
                    ((Integer)kMiddle.get( iPath + 1 )).intValue()    )
            {
                iPath++;
            }
            iPath++;
        ...
        for ( int j = 0; j < 10; j++ )
        {
            for ( int i = 1; i < (kNewVert.size() - 1); i++ )
            {
                ...
                {
                    findMin( new Point3f( kStart.x, kStart.y, kStart.z ),
                             iMiddle, iSide,
                             new Point3f( kEnd.x, kEnd.y, kEnd.z ),
                             newPoint1 );
        ...
        {
            m_kGeodesicVertices[iVertex] =
                new Point3f( ((Point4f)kNewVert.get(iVertex)).x,
                             ((Point4f)kNewVert.get(iVertex)).y,
                             ((Point4f)kNewVert.get(iVertex)).z  );
            if ( iVertex > 0 )
            {
                m_fSmoothedPathLength += distance( m_kGeodesicVertices[ iVertex ],
                                         m_kGeodesicVertices[ iVertex - 1 ] );
            }
        }
    ...
    {
        Point3f kStart =
            m_akCoordinates[ ((Integer)kMiddle.get( iNode - 1 )).intValue() ];
        ...
        if ( iRight != -1 )
        {
            fRightPathLength += findMin( kStart, iMiddle,
                                         iRight, kEnd,
                                         kNewPoint );
            for ( int iRightNode = iNode + 1;
                  iRightNode < (iNode + iNumSteps);
                  iRightNode++ )
            {
        ...
        if ( iLeft != -1 )
        {
            fLeftPathLength += findMin( kStart, iMiddle,
                                         iLeft, kEnd,
                                         kNewPoint );

            for ( int iLeftNode = iNode + 1;
                  iLeftNode < (iNode + iNumSteps);
                  iLeftNode++ )
            {
        ...
        if ( (bUseRight && bUseLeft && (fRightPathLength <= fLeftPathLength)) ||
             (bUseRight && !bUseLeft) )
        {
        ...
        else if ( (bUseLeft && bUseRight && (fLeftPathLength < fRightPathLength)) ||
                  (bUseLeft && !bUseRight)  )
        {
        ...
        kNewVertTemp.add( new Point4f( kMiddlePoint.x,
                                       kMiddlePoint.y,
                                       kMiddlePoint.z,
                                       iMiddle ) ) ;
        return -1;
    ...
    /**
     * drawPath draws the Geodesic path as a LineArray and adds it to the
     * children of the m_kSmoothedGeodesicGroup object, it also draws the
     * Dijkstra path and Euclidian paths and adds them to the corresponding
    ...
    public void drawGeodesicPath( int iStart, int iEnd )
    {
        ...
        if ( m_bEndpointChanged )
        {
            m_kSmoothedGeodesicGroup.
                removeChild( m_kSmoothedGeodesicGroup.numChildren() - 1 );
            drawGeodesicPoint( m_kEndPoint,
                               m_kPickColors[ m_iNumPicked%2 ] );
        }
        ...
        for ( int iPathNode = 0;
              iPathNode < m_iNumGeodesicVertices; iPathNode++ )
        {
            kSmoothedLineArray.setCoordinate( iPathPointCount,
                                      m_kGeodesicVertices[ iPathNode ] );

            kSmoothedLineArray.setColor( iPathPointCount, kColor );
            iPathPointCount++;

            if ( ( iPathNode != 0 ) &&
                 ( iPathNode != (m_iNumGeodesicVertices - 1) ) )
            {
                kSmoothedLineArray.setCoordinate( iPathPointCount,
                                          m_kGeodesicVertices[ iPathNode ] );
                kSmoothedLineArray.setColor( iPathPointCount, kColor );
    ...
    /**
     * drawPath draws the Dijkstra path and Euclidian paths and adds them to
     * the corresponding m_kDijkstraGeodesicGroup and m_kEuclidianGeodesicGroups.
    ...
    public void drawDijkstraEuclidianPath( int iStart, int iEnd )
    {
        ...
        if ( m_bLivewire && !m_bFirstWire )
        {
            m_kDijkstraGeodesicGroup.
                removeChild( m_kDijkstraGeodesicGroup.numChildren() - 1 );
            m_kEuclidianGeodesicGroup.
                removeChild( m_kEuclidianGeodesicGroup.numChildren() - 1 );
        }
        ...
        if ( m_bEndpointChanged )
        {
            ...
            drawDijkstraEuclidianPoint( m_kEndPoint,
                                        m_kPickColors[ m_iNumPicked%2 ] );
        }
        ...
        while ( iNode != iStart )
        {
            if ( iNode != iStart )
            {
                m_fDijkstraPathLength +=
                    distance( m_akCoordinates[ iNode ],
                              m_akCoordinates[ m_iPrevious[ iNode ] ]);
            }
        ...
        LineArray kEuclidianLineArray = new LineArray( 2,
                                                       GeometryArray.COORDINATES |
                                                       GeometryArray.COLOR_3  );
        ...
        if ( m_kPanel != null )
        {
            float fEuclidian = distance( m_akCoordinates[ iStart ],
                                         m_akCoordinates[ iEnd ]   );
    ...
    private void triangulateMeshPath()
    {
        ...
        LinkedList kOpenPath =
            (LinkedList)m_kGeodesic_Working.get( m_iNumWorking - 1 );
        LinkedList kOpenPathLeft =
            (LinkedList)m_kGeodesic_Working_Left.get( m_iNumWorking - 1 );
        LinkedList kOpenPathRight =
            (LinkedList)m_kGeodesic_Working_Right.get( m_iNumWorking - 1 );
        iVertexCount += triangulate( m_kModified,
                                     kOpenPath,
                                     kOpenPathLeft,
                                     kOpenPathRight,
                                     iVertexCount );
    ...
    /**
     * creates a new mesh after triangulation or when a mesh is cut along the
     * geodesic
     * @param kMesh ModelTriangleMesh surface
                                             ...
                                             int iOldVertexCount )
    {
        ...
        {
            akVertices[ iIndex ] =
                new Point3f( (Point3f)m_kNewVerts.get( iNewIndex ) );
            akNormals[ iIndex ] =
                new Vector3f( (Vector3f)m_kNewNormals.get( iNewIndex ) );
            iIndex++;
        }
        ...
        for ( int iTri = 0; iTri < iNumTris; iTri++ )
        {
            ...
            if ( m_abRemoveTris != null )
            {
                ...
                for ( int iRemoveTri = 0;
                      iRemoveTri < m_kRemoveTriangles.size();
                      iRemoveTri++ )
                {
            ...
            if ( bAdd )
            {
                if ( (iVertexCount < iOldVertexCount) &&
                     (m_aiIndexShift != null)  )
                {
                    int i0 = aiIndex[ iTri * 3 + 0 ];
                    int i1 = aiIndex[ iTri * 3 + 1 ];
        ...
        ModelTriangleMesh kCutMesh =
            new ModelTriangleMesh( akVertices, akNormals, aiConnect );
            ...
            else
            {
                ...
                if ( iPathIndex < iVertexCount )
                {
                    ...
                    if ( triangleExists( kDeleteTri ) )
                    {
                        ...
                        /* Add the next vertex: */
                        m_kNewVerts.
                        add( new Point3f( kNextPoint.x, kNextPoint.y, kNextPoint.z ) );

                        /* Add the new normal: */
                        kNormal = getNormal( kMesh,
                ...
                else
                {
                    ...
                    if ( triangleExists( kDeleteTri ) )
                    {
                        ...
                        /* Add the next vertex: */
                        m_kNewVerts.add( new Point3f( kNextPoint.x,
                                                      kNextPoint.y,
                                                      kNextPoint.z ) );
    ...
    /**
     * findMin finds the point (newpoint) along the edge that connects the
     * points Side-Middle that minimizes the distance Start-newpoint-End.
     ...
     */
    private float findMin( Point3f kStart, int iMiddle,
                           int iSide, Point3f kEnd,
                           Point4f kNewPoint4 )
    {
        if ( (iSide < 0 ) || (iSide >= m_iVertexCount) ||
             (iMiddle < 0 ) || (iMiddle >= m_iVertexCount) )
        {
...
//                     }
    ...
    /**
     * Add the edge to the EdgeList, check to make sure that edge has not
     * already been added.
    ...
    /**
     * Draw the user-selected point as a sphere on the triangle mesh, the
     * sphere is added to all three drawing groups: m_kSmoothedGeodesicGroup,
     * m_kDijkstraGeodesicGroup, and m_kEuclidianGeodesicGroup.
    ...
    {
        return (float)Math.sqrt( (kPoint1.x - kPoint2.x) *
                                 (kPoint1.x - kPoint2.x) +
                                 (kPoint1.y - kPoint2.y) *
                                 (kPoint1.y - kPoint2.y) +
                                 (kPoint1.z - kPoint2.z) *
    ...
    /**
     * Adds the working paths to the finished path lists, combining the
     * segments in the working paths into one path
     * @param bOpen boolean close path or not
     */
    private void finishWorkingLists( boolean bOpen )
    {
        ...
        if ( m_iNumWorking > 1 )
        {
            ...
            for ( int iList = 0; iList < m_iNumWorking; iList++ )
            {
                ...
                LinkedList kGeodesicSegment =
                    (LinkedList)m_kGeodesic_Working.get( iList );
                ...
                for ( int iVert = 0; iVert < kGeodesicSegment.size(); iVert++ )
                {
                    ...
                    if ( (iList == 0) || (iVert != 0) )
                    {
                        if ( !kGeodesic.contains( kGeodesicSegment.get( iVert ) ) ||
                             ( (iList == (m_iNumWorking - 1)) &&
                               (iVert == (kGeodesicSegment.size() - 1)) ) )
                        {
                            kGeodesic.add( iGeodesicCount,
                                           kGeodesicSegment.get( iVert ) );
                            iGeodesicCount++;
                        }
                        else
                        {
                            int iIndex =
                                kGeodesic.indexOf( kGeodesicSegment.get( iVert ) );
    ...
    /* Data members for the Geodesic Class:  */
    /* Triangle mesh: */
    ...
    /* Group for drawing the Geodesic on the triangle mesh, assumes that the
     * Group is created in the same branch tree as the triangle mesh surface,
     * so when the mesh is transformed (rotated,scaled,translated) the
     * polyline drawn and stored in the Group m_kSmoothedGeodesicGroup will be
   ...
   /** Root group for different path. */
    ...
    /* PickCanvas, created by the class that creates the triangle mesh and the
     * geodesic group: */
    ...
    /* Keeps track of the of picking, so that when a pair of points has been
     * picked the Geodesic is calculated:  */
    private int m_iNumPicked = 0;

    /* First point, for closing the Geodesic curve: */
    private Point3f m_kFirstPoint = null;

    /* Start and End points -- pair of points for which a Geodesic is
     * calculated, must be in TriangleMesh coordinates: */
    private Point3f m_kStartPoint = null;
    private Point3f m_kPreviousStartPoint = null;
    private Point3f m_kEndPoint = null;

    /* The index values in the Vertex array for the triangle that the Start,
     * First, and End points fall in, if they are inside a triangle and not on
     * a triangle vertex: */
    ...
    /* Local copies of the Vertex and Index arrays: a local copy is kept so
     * that when the start or end points fall inside a triangle, a new vertex
     * is added to the vertex array, and three new triangles are added to the
     * triangle index array, new normals are added to the Normal array: */
    ...
    /* Data members used in Dijkstra's search. The Edgelist is a list for each
     * vertex of all the vertices that it is connected to by one edge. The
     * vertices are stored in the Edgelist as the vertex index. */
    private LinkedList[] m_akEdgeList = null;

    /* Data memebr for Dijkstra's search. The m_kBorder list stores all the
     * vertices that have been visited by Dijkstra's search, but that have not
     * yet been relaxed. It is used to speed up the search for the non-relaxed
     * vertex with the smallest path distance */
    private LinkedList m_kBorder = null;


    /* Weights, relaxed flags, and previous vertex index for Dijkstra's
     * search: */
    ...
    /* The final list of points in the Geodesic curve. All points are
     * constrained to lie on the triangle mesh, */
    private Point3f[] m_kGeodesicVertices = null;
    private int m_iNumGeodesicVertices = 0;

    /* Flag for clearing the Geodesic curves, if it is false, no curves have
     * been added to the GeodesicGroup. */
    private boolean m_bGroupAdded = false;

    /* Volume renderer progress bar: */
    private JProgressBar m_kPBar = null;

    /* Mouse events. Setting mousePressed and mouseReleased explicitly when
     * the mouse events are received has deals with getting multiply mouse
     * event notifications for the same mouse press. */
    private boolean m_bMousePressed = false;
    private boolean m_bMouseReleased = true;
    private MouseEvent m_kMouseEvent;

    /* Radius of the sphere displayed to mark the points on the Geodesic. This
     * can be set directly by the class using the Geodesic object.  */
    ...
    /* LinkedLists to contain the working paths in progress, and all finished
     * paths, open and closed for the smoothed geodesics and dijkstra's
     * geodesics. These are used in the cutting operations: */

    /* For paths that are not finished, points may still be added to these
     * paths: */
