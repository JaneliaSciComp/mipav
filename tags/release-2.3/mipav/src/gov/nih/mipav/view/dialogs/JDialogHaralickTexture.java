...
public class JDialogHaralickTexture extends JDialogBase
         ...
         if (textureAlgo.isCompleted() == true && resultImage != null) {
           ...
           for (i = 0; i < resultNumber; i++) {
             ...
             resultImage[i].clearMask();
                 try {
                   imageFrame[i] =
                       new ViewJFrameImage(resultImage[i], null, new Dimension(610, 200));
                 }
    ...
    private boolean setVariables() {
        ...
        if (windowSize > image.getExtents()[0]) {
            MipavUtil.displayError("Window size must not excced image width of " +
                                    image.getExtents()[0]);
            textWindowSize.requestFocus();
            textWindowSize.selectAll();
            return false;
        ...
        if (windowSize > image.getExtents()[1]) {
            MipavUtil.displayError("Window size must not excced image height of " +
                                    image.getExtents()[1]);
            textWindowSize.requestFocus();
            textWindowSize.selectAll();
            return false;
    ...
    private void callAlgorithm() {
        ...
        try {
            ...
            for (i = 0; i < numDirections; i++) {
                ...
                for (j = 0; j < numOperators; j++) {
                    ...
                    resultImage[index] = new ModelImage(ModelStorageBase.FLOAT, image.getExtents(),
                                         name[index], userInterface);

                } // for (j = 0; j < numOperators; j++)
            } // for (i = 0; i < numDirections; i++)
            textureAlgo = new AlgorithmHaralickTexture(resultImage, image, windowSize, offsetDistance,
                          ns, nesw, ew, senw, invariantDir, contrast, dissimilarity,
                          homogeneity, inverseOrder1, asm, energy, maxProbability, entropy, mean,
            ...
            if (runInSeparateThread) {
                ...
                textureAlgo.setActiveImage(isActiveImage);
      ...
      MipavUtil.displayError(
          "Dialog Haralick Texture: unable to allocate enough memory");
      return;
    }
