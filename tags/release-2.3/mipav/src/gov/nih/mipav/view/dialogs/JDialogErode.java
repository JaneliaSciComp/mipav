 ...
 *
 */
public class JDialogErode extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "Erode " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
                    userInterface.getScriptDialog().putVar( resultImage.getImageName() );
                    userInterface.getScriptDialog().append(
                            userInterface.getScriptDialog().getVar( resultImage.getImageName() ) + " " + regionFlag
                            + " " + iters + " " + kernelErode + " " + kernelSizeErode + " " + do25D + "\n" );
                } else {
                    userInterface.getScriptDialog().append(
     ...
     *                  AlgorithmMorphology3D.SIZED_SPHERE (or the AlgorithmMorphology25D ones))
     */
        ...
        if ( image.getNDims() == 2 ) { // source image is 2D
            ...
            if ( displayLoc == NEW ) {
                ...
                try {
                    ...
                    erodeAlgo2D = new AlgorithmMorphology2D( image, kernelErode, kernelSizeErode, AlgorithmMorphology2D.ERODE, 0,
                            iters, 0, 0, regionFlag );
                    if ( regionFlag == false ) {
                        erodeAlgo2D.setMask( image.generateVOIMask() );
                    }
                    // This is very important. Adding this object as a listener allows the algorithm to
            ...
            if ( displayLoc == NEW ) {
                ...
                try {
                    // Make algorithm
                    erodeAlgo3D = new AlgorithmMorphology3D( image, kernelErode, kernelSizeErode, AlgorithmMorphology3D.ERODE, 0,
                            iters, 0, 0, regionFlag );
                    if ( regionFlag == false ) {
                        erodeAlgo3D.setMask( image.generateVOIMask() );
                    }
            ...
            if ( displayLoc == NEW ) {
                try {
                    ...
                    if ( runInSeparateThread ) {
                        ...
                        erodeAlgo25D.setActiveImage( isActiveImage );
                        if ( !userInterface.isAppFrameVisible() ) {
                ...
                try {
                    // No need to make new image space because the user has choosen to replace the source image
                    // Make the algorithm class
                    erodeAlgo25D = new AlgorithmMorphology25D( image, kernelErode, kernelSizeErode, AlgorithmMorphology25D.ERODE,
                            0, iters, 0, 0, regionFlag );
                    if ( regionFlag == false ) {
                        erodeAlgo25D.setMask( image.generateVOIMask() );
                    }
                    // This is very important. Adding this object as a listener allows the algorithm to
    ...
    /**
     *   Sets text fields enabled depending upon which item in the combo box
     *   is selected.
     */
