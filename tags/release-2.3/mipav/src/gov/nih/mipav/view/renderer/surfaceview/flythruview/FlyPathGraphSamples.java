...
/**
 * Extension of the FlyPathGraph class which allows for the storage of
 * the samples used to generate the individual curves where curves
 * are the nodes of the graph.
 */
public class FlyPathGraphSamples
    extends FlyPathGraph
{
    ...
    /**
     * Add the following information for a "branch" path of the graph:
     * an array of key values for each point along the path, an array
     * of 3D coordinates assoicated with each point, and an array of
     * distances each point is from its nearest boundary.
     * @param aiPointKey int[] Array of unique integer key values associated
     * with each point along the path.
     * @param akPointPosition Point3f[] Array of 3D positions associated
     * with each point along the path.
     */
    public void add(int[] aiPointKey, Point3f[] akPointPosition)
    {
        ...
        if (null == aiPointKey || null == akPointPosition)
        {
            throw new IllegalArgumentException(
                "FlyPathGraphSamples.add: arrays must not be null.");
        }

        // Arrays must have the same positive size.
        if (aiPointKey.length < 2 ||
            aiPointKey.length != akPointPosition.length)
        {
            throw new IllegalArgumentException(
                "FlyPathGraphSamples.add: arrays must be same positive size.");
        }
        ...
        if (getNumBranches() > 0)
        {
            ...
            LoopBranchSearch:for (int iBranch = 0; iBranch < getNumBranches();
                                  iBranch++)
            {
                ...
                for (int iPoint = 0; iPoint < aiBranchKey.length; ++iPoint)
                {
                    if (iBranchKey == aiBranchKey[iPoint])
                    {
                        ...
                        iBranchPoint = iPoint;
                        akBranchPosition =
                            (Point3f[]) m_kListArrayPointPosition.get(iBranch);
            ...
            if ( -1 == iBranchBranch) {
                throw new IllegalArgumentException(
                    "FlyPathGraphSamples.add: common branch point not found");
            }
            ...
            {
                float fLength =
                    akBranchPosition[iPoint].distance(akBranchPosition[iPoint - 1]);
    ...
    /**
     * Access the array of 3D coordinates for the points which define
