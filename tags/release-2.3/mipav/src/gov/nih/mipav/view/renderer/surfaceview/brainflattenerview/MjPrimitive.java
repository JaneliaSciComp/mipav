...
public class MjPrimitive
{
    ...
    // x0*s - x1*t = c0 and y0*s - y1*t = c1.  The error tests are
    // relative to the size of the direction vectors, |Cross(D0,D1)| >=
    // e*|D0|*|D1| rather than absolute tests |Cross(D0,D1)| >= e.  The
    // quantities P1-P0, |D0|^2, and |D1|^2 are returned for use by
    // calling functions.
    static private int find (Point2f kP0, MjVector2f kD0,
                             Point2f kP1, MjVector2f kD1,
                             MjVector2f kDiff, float [] afT)
    {
    ...
    // The return quantity is 0 if the objects do not intersect, or is
    // 1 or 2 if they do intersect.    If 2, then the returned values are
    // the end points of an interval of intersection.  If an
    // intersection occurs, the returned values are the parameter values for
    // the point(s) of intersection with respect to the first object
    // parameterized by P+t*D (t in [-inf,+inf] for line, t in [0,+inf] for
    // ray, t in [0,1] for segment).
    // The t parameter for the points of intersection are returned in the
    // afT array which must have length 2.
    static public int findIntersection (Ray2f kRay, Segment2f kSegment,
                                        float [] afT)
    {
        MjVector2f kDiff = new MjVector2f();
        float fD0SqrLen = kRay.m_kDirection.lengthSquared();
        int iQuantity = find(kRay.m_kOrigin,kRay.m_kDirection,
                                 kSegment.m_kOrigin,kSegment.m_kDirection,
                                 kDiff,afT);
    ...
    // The t parameter for the points of intersection are returned in the
    // afT array which must have length 2.
    static private int find (Point2f kP, MjVector2f kD,
                             Point2f kC, float fR, float [] afT)
    {
        MjVector2f kDiff = new MjVector2f();
        kDiff.sub(kP,kC);
    ...
    // The value is 0 if the objects do not intersect.  Otherwise, the
    // quantity is 1 or 2 and is the number of valid points in
    // The points of intersection are returned in the akPoint array
    // which must have length 2.
    static public int findIntersection (Segment2f kSegment, Circle2f kCircle,
                                        Point2f [] akPoint)
    {
        float [] afT = new float [2];
        int iQuantity = find(kSegment.m_kOrigin,kSegment.m_kDirection,
                             kCircle.m_kCenter,kCircle.m_fRadius,afT);
        ...
        if ( 0 != iQuantity )
        {
            ...
            for (int i = 0; i < iQuantity; i++)
            {
                ...
                akPoint[i].scaleAdd(
                afT[i],kSegment.m_kDirection,kSegment.m_kOrigin);
            }
        }
