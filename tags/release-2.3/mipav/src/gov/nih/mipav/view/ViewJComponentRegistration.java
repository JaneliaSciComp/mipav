...
public class ViewJComponentRegistration
    ...
    /**
     *   Used to "lock" display when an algorithm
     *                 is in the calculation process.
     */
    ...
    public ViewJComponentRegistration(ViewJFrameRegistration _frame,
        ...
        try { // this try does not work... sun didn't bother to propogate the exception ... maybe someday ...
            ...
            magRegionCursor = Toolkit.getDefaultToolkit().createCustomCursor(img,
                new Point(12, 12), "Magnification");
        }
    ...
    /**
     *  doDrag true enables the adjusted slice to respond to mouse press and drag events
     *  doDrag false restricts the adjusted slice to responding to mouse press and
     *  release events
     */
    ...
    /**
     *   Set the buffers of the frame in which the image(s) is displayed, allocates the
    ...
    public void setBuffers(float imgBufferA[], float imgBufferB[],
                           int pixBuff[], int pixBuffB[], int paintBuff[]) {
     ...
     */
    public boolean show(int tSlice, int zSlice, ModelLUT _LUTa, ModelLUT _LUTb,
                        boolean forceShow) {
        ...
        if (slice != zSlice || timeSlice != tSlice || forceShow == true) {
            ...
            try {
                if (imageA.getType() == ModelStorageBase.COMPLEX) {
                    imageA.exportComplexSliceXY(timeSlice * zDimSlices + slice,
                                                imageBufferA, logMagDisplay);
                }
                ...
                if (imageB != null) {
                    if (imageB.getType() == ModelStorageBase.COMPLEX) {
                        imageB.exportComplexSliceXY(timeSlice * zDimSlices + slice,
                            imageBufferB, logMagDisplay);
                    }
                    else if (orientation == AXIAL || orientation == NA) {
            ...
            if (imageB == null) {
                ...
                for (index = 0; index < bufferSize; index++) {
                    indexA = (short) ( (imageBufferA[index] - imageMinA) * remapConstA +
                                      0.5);
                    indexB = (short) ( (imageBufferB[index] - imageMinB) * remapConstB +
                                      0.5);
                    Ra = iRGB_LUTa[0][indexA];
                    Rb = iRGB_LUTb[0][indexB];
                    Ga = iRGB_LUTa[1][indexA];
    ...
    public boolean show(int tSlice, int zSlice, boolean forceShow) {
        ...
        if (slice != zSlice || timeSlice != tSlice || forceShow == true) {
            ...
            try {
                ...
                //if (imageB != null) {
                //    imageB.exportData((timeSlice*zDimSlices * bufferSize + zSlice*bufferSize), bufferSize, imageBufferB);
                //}
                if (orientation == AXIAL || orientation == NA) {
                    imageA.exportData(timeSlice * zDimSlices * bufferSize +
                                      slice * bufferSize, bufferSize, imageBufferA);
                }
                else if (orientation == SAGITTAL) {
                ...
                if (imageB != null) {
                    if (orientation == AXIAL || orientation == NA) {
                        imageB.exportData(timeSlice * zDimSlices * bufferSize +
                                          slice * bufferSize, bufferSize, imageBufferB);
                    }
                    else if (orientation == SAGITTAL) {
            ...
            if (imageB == null) {
                ...
                for (index = 0, j = 0; j < imageSize; index += 4, j++) {
                    if ( (RGBTA != null) && (RGBTB != null)) {
                        if (RGBTA.getROn())
                            Ra = (RGBIndexBufferA[ (int) imageBufferA[index + 1]] &
                                  0x00ff0000) >> 16;
                        else
                            Ra = 0;

                        if (RGBTB.getROn())
                            Rb = (RGBIndexBufferB[ (int) imageBufferB[index + 1]] &
                                  0x00ff0000) >> 16;
                        else
                            Rb = 0;

                        if (RGBTA.getGOn())
                            Ga = (RGBIndexBufferA[ (int) imageBufferA[index + 2]] &
                                  0x0000ff00) >> 8;
                        else
                            Ga = 0;

                        if (RGBTB.getGOn())
                            Gb = (RGBIndexBufferB[ (int) imageBufferB[index + 2]] &
                                  0x0000ff00) >> 8;
                        else
                            Gb = 0;

                        if (RGBTA.getBOn())
                            Ba = (RGBIndexBufferA[ (int) imageBufferA[index + 3]] &
                                  0x000000ff);
                        else
                            Ba = 0;

                        if (RGBTB.getBOn())
                            Bb = (RGBIndexBufferB[ (int) imageBufferB[index + 3]] &
                                  0x000000ff);
                        else
                            Bb = 0;
                    }
    ...
    /**
     * If a point is selected of the given type (reference or adjusted)
     * delete the point and reset the
    ...
    public void makeReferenceVOI(Point3Df refPoint) {
        ...
        try {
            ...
            MipavUtil.displayError(
                "Out of memory: ComponentRegistration.mouseReleased");
            setMode(DEFAULT);
            return;
        }
        refMark++;
    ...
    //************************************************************************
     //***************************** Mouse Events *****************************
      //************************************************************************
       ...
       /**
        *  When the mouse is clicked in the image,
        *  several different things may happen.  If a Region of
    ...
    public void mousePressed(MouseEvent mouseEvent) {
        ...
        try {
            ...
            System.gc();
            MipavUtil.displayError(
                "Out of memory: ComponentRegistration.mousePressed");
            setMode(DEFAULT);
            return;
        }
        ...
        if (mode == MOVE) {
            ...
            for (i = 0; i < nVOI; i++) {
                ...
                if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) { // curve type is Point
                    if (VOIs.VOIAt(i).nearPoint(x, y, slice, getZoomX(), resolutionX,
                                                resolutionY)) {

                        VOIs.VOIAt(i).setActive(true);
                        ( (VOIPoint) (VOIs.VOIAt(i).getCurves()[slice].elementAt(0))).
                            setActive(true);
                        VOIs.VOIAt(i).drawVOISpecial(slice,
                            getGraphics(),
                            imageActive.getFileInfo(0).
                            getResolutions(),
                            imageActive.getFileInfo(0).
                            getUnitsOfMeasure(),
            ...
            if ( (doCenter) && (rotCenterMark == 0)) {
                ...
                ( (VOIPoint) (centerVOI.getCurves()[0].elementAt(0))).moveVOIPoint(
                    deltaX, deltaY, 0,
                    imageActive.getExtents()[0], imageActive.getExtents()[1], 1);
                xOrg[centerid] = xS;
                yOrg[centerid] = yS;
                xRotation = xS * resolutionX;
                yRotation = yS * resolutionY;
    ...
    public void moveVOIPosition(int deltaX, int deltaY) {
        if (newVOI != null) {
            ...
            if (imageActive.getNDims() >= 3) {
                newVOI.moveVOI(slice, imageActive.getExtents()[0],
                               imageActive.getExtents()[1],
    ...
    public void moveVOITo(int id, int x, int y) {
        ...
        if (imageActive.getNDims() >= 3) {
            VOIs.VOIAt(id).moveVOI(slice, imageActive.getExtents()[0],
                                   imageActive.getExtents()[1],
                                   imageActive.getExtents()[2], deltaX, deltaY, 0);
        }
        else {
            VOIs.VOIAt(id).moveVOI(0, imageActive.getExtents()[0],
                                   imageActive.getExtents()[1],
                                   1, deltaX, deltaY, 0);
        }
    }
    ...
    //************************************************************************
     //************************** Mouse Motion Events *************************
      //************************************************************************
       ...
       /**
        *  Drags an VOI real time by calling a series
       ...
       public void mouseDragged(MouseEvent mouseEvent) {
           ...
           if ( (doDrag == true) && (mode == TRANSLATE || mode == ROTATE)) {
               ...
               if (mode == ROTATE) {
                   ...
                   g.drawLine( (int) (xRotation * getZoomX() + 0.5),
                              (int) (yRotation * getZoomX() + 0.5),
               ...
               for (i = 0; i < nVOI; i++) {
                   ...
                   if (VOIs.VOIAt(i).isActive() && VOIs.VOIAt(i).isVisible()) {
                       ...
                       if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
                           ...
                           newX = xOrg[i] + distX;
                           newY = yOrg[i] + distY;
                           if ( (newX >= 0) && (newX < imageActive.getExtents()[0]) &&
                               (newY >= 0) &&
                               (newY < imageActive.getExtents()[1])) {
                               xOrg[i] = newX;
                               yOrg[i] = newY;
    ...
    /**
     *  If the mode is level set, draws
        ...
        for (i = 0; i < nVOI; i++) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.POINT) {
                if (VOIs.VOIAt(i).isVisible() &&
                    VOIs.VOIAt(i).nearPoint(x, y, slice, getZoomX(), resolutionX,
                                            resolutionY)) {
                    setMode(MOVE);
                    g.dispose();
                    return;
    ...
    public void paintComponent(Graphics g) {
        ...
        try {
            if (g == null) {
                MipavUtil.displayError(
                    "ComponentRegistration.paintComponent: graphics = null");
                return;
            }
                        ...
                        if (slice != -99) {
                            ...
                            for (i = nVOI - 1; i >= 0; i--) {
                                if (markerType[i] == ROTATIONCENTER) {
                                    ...
                                    ( (VOIPoint) (VOIs.VOIAt(i).getCurves()[slice].elementAt(0))).
                                        setLabel(String.valueOf(curRefMark));
                                    VOIs.VOIAt(i).drawSelf(getZoomX(), getZoomY(), resolutionX, resolutionY, 0f, 0f,
                                        imageActive.getFileInfo(0).
                                        getResolutions(),
                                        imageActive.getFileInfo(0).
                                        getUnitsOfMeasure(), slice,
                                        orientation,
                                        g);
                                    curRefMark--;
                                    ...
                                    ( (VOIPoint) (VOIs.VOIAt(i).getCurves()[slice].elementAt(0))).
                                        setLabel(String.valueOf(curAdjMark));
                                    VOIs.VOIAt(i).drawSelf(getZoomX(), getZoomY(), resolutionX, resolutionY, 0f, 0f,
                                        imageActive.getFileInfo(0).getResolutions(),
                                        imageActive.getFileInfo(0).getUnitsOfMeasure(), slice,
                                        orientation,
            ...
            System.gc();
            MipavUtil.displayError(
                "Out of memory: ComponentRegistration.paintComponent.");
        }
    }
    ...
    public void paintWindowComponent(Graphics g, MouseEvent mouseEvent) {
        ...
        if (g == null) {
            MipavUtil.displayError(
                "ComponentRegistration.paintMagComponent: graphics = null");
            return;
        }
        ...
        if (mouseEvent.isShiftDown() == false) {
            ...
            //                        imageBufferActive, imageActive.getType(),
            //                        imageActive.getExtents()[0], getActiveImage().getMin(), getActiveImage().getMax());
