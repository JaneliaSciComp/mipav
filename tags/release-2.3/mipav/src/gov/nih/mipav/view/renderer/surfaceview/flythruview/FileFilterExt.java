...
/**
 * A simple file filter to select only files with the specified extensions
 * for display in a JFileChooser instance.
 */
        ...
        extends FileFilter
{
    ...
    /**
     * Create file filter given no initial extensions.  Until at least
     * one extension is added (see the addExtension method), the accept
    ...
    /**
     * FileFilter override.  This function is called for
     * each file in the current directory of the file dialog to test if it
     * is a file with a valid extension.
     * If so, it is accepted for display in the file dialog.
    ...
    public boolean accept (File kFile)
    {
        if ( null != kFile )
        {
            ...
            {
                for ( int iString = 0; iString < m_kExtensionsVector.size();
                      ++iString )
                {
                    String kExtension =
                                    (String)m_kExtensionsVector.get(iString);
    ...
    /**
     * FileFilter override.  This function is called just
    ...
    /**
     * Add an extension to the list of possible extensions valid for
     * this filter.
     * @param kExtension String containing the valid file extension
    ...
    /**
     * Set the description of this filter as it should be returned by the
     * getDescription override method.
