 ...
 *   the degree of blurring in x and y dimensions. The user also specifies the
 *   the normalization constant and the number of iterations.
 *   The user has the option to use a threshold level.
    ...
    private boolean setVariables() {
        ...
        if (thresholdSelected) {
          ...
          if (testParameter(tmpStr, image.getMin(), image.getMax())) {
            ...
            MipavUtil.displayError("Threshold level must be between " +
                                   image.getMin() + " and " + image.getMax());
    ...
    String name = makeImageName(image.getImageName(), "_mriShadingCorr");
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          mAlgo = new AlgorithmMRIShadingCorrection(resultImage, image, norm,
                       scaleX, scaleY, iters, thresholdSelected, thresholdLevel);
          ...
          MipavUtil.displayError(
              "Dialog MRI Shading Correction: unable to allocate enough memory");
          if (resultImage != null) {
        ...
        try {
          ...
          mAlgo = new AlgorithmMRIShadingCorrection(image, norm,
                       scaleX, scaleY, iters, thresholdSelected,
                       thresholdLevel);
          // This is very important. Adding this object as a listener allows the algorithm to
          // notify this object when it has completed of failed. See algorithm performed event.
          // This is made possible by implementing AlgorithmedPerformed interface
