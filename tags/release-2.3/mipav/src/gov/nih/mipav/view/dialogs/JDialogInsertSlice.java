 ...
 *       Dialog asks the user to enter the slice number before which the
 *       new slice is to be inserted or for the last slice number plus one
 *       if the slice is to be added to the end.  The user may select either
 *       average or blank or original for the new slice.  The slices are assumed to be
 *       parallel and uniformly spaced, so average so calculates the mean of
 *       the 2 surrounding slices or duplicates an end slice if the new slice
 *       is to be placed at the beginnig or end.  Blank creates a new slice
 *       with all the pixels set to zero.  Original inserts a 2D image of the
 *       same dimensions selected from a pop up menu.  Must insert black and
 *       white with black and white and color with color.
 *
 */
public class JDialogInsertSlice
    extends JDialogBase
          ...
          if (userInterface.isScriptRecording()) {
              ...
              if ( (sliceType == ORIGINAL_SLICE) && (insertedImage != null)) {
                  /*
                   ...
                   userInterface.getScriptDialog().append("LoadImage " +
                       userInterface.getScriptDialog().getVar(insertedImage.
                       getImageName()) + "\n");
                   */
              }
              ...
              if ( (sliceType == ORIGINAL_SLICE) && (insertedImage != null)) {
                  ...
                  if (userInterface.getScriptDialog().getImgTableVar(insertedImage.getImageName()) == null) {
                      if (userInterface.getScriptDialog().getActiveImgTableVar(insertedImage.getImageName()) == null) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.getScriptDialog().
                                                         getVar(insertedImage.getImageName()) + " ");
              } else {
                  userInterface.getScriptDialog().append(userInterface.getScriptDialog().
                                                         getVar(image.getImageName()) + " ");
              }
  ...
  private void init() {
    ...
    JLabel sliceLabel = new JLabel("Insert before slice #(1-" +
                                   String.valueOf(nSlices) + ") or enter "
                                   + String.valueOf(nSlices + 1) +
                                   " for new last slice");
  ...
  private void buildComboBox() {
    ...
    while (names.hasMoreElements()) {
      ...
      if ( (!image.getImageName().equals(name)) &&
          (userInterface.getFrameContainingImage(namedImage) != null) &&
          (namedImage.getNDims() == 2) &&
          (namedImage.getExtents()[0] == image.getExtents()[0]) &&
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    else if ( (source == average) || (source == blank) || (source == original) ||
             (source == adjacentUp) || (source == adjacentDown)) {
  ...
  */
  ...
  private boolean setVariables() {
    ...
    if (insertSlice < 1) {
      ...
      MipavUtil.displayError("Slice number cannot exceed " +
                             String.valueOf(nSlices + 1)
                             + " for addition to end");
      textSlice.requestFocus();
      textSlice.selectAll();
  ...
  /**
   *	Once all the necessary variables are set, call the Insert Slice
  ...
  private void callAlgorithm() {
    ...
    try {
      ...
      resultImage = new ModelImage(image.getType(), destExtents,
                                   image.getImageName(), image.getUserInterface());

      // Make algorithm:
      insertSliceAlgo = new AlgorithmInsertSlice(image, resultImage,
                                                 insertSlice, sliceType,
                                                 insertedImage);
      ...
      MipavUtil.displayError(
          "Insert Slice reports: unable to allocate enough memory");
      return;
    }
  }
       ...
       if (algorithm instanceof AlgorithmInsertSlice) {
         if (insertSliceAlgo.isCompleted() == true && resultImage != null) {
           ...
           if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
               ...
               if (image.getNDims() == 3) {
                   Preferences.debug("into " + image.getFileInfo()[0].getExtents()[2] +
                                     " slice 3D " +
                                     image.getImageName() + "\n");
           ...
           if (Preferences.debugLevel(Preferences.DEBUG_ALGORITHM)) {
               ...
               Preferences.debug("as new slice number " + insertSlice + "\n");
               if (image.getNDims() == 3) {
                   Preferences.debug("into " + (image.getFileInfo()[0].getExtents()[2]-1) +
                                     " slice 3D " +
                                     image.getImageName() + "\n");
                   Preferences.debug("to create\n");
                   ...
                   Preferences.debug("into " +
                                     (image.getFileInfo()[0].getExtents()[2]-1) +
                                      " slice " +
                                      image.getFileInfo()[0].getExtents()[3] +
                                      " volume 4D " +
                                      image.getImageName() + "\n");
