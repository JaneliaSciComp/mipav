 ...
 *   Selects image is match image, the image that gets transformed until it is
 *   registered to the base image. Algorithms are executed in their own thread.
 *
...
public class JDialogRegistrationLeastSquares
    ...
    public JDialogRegistrationLeastSquares(Frame theParentFrame, ModelImage _mi,
                                           ModelImage _ri) {
        matchImage = _mi;
        baseImage = _ri;
        DIM = 3;
    ...
    private void callAlgorithm() {
        ...
        try {
                ...
                if ( (baseImage.getNDims() == 3) && (matchImage.getNDims() == 3)) {
                    ...
                    for (s = 0; s < baseImage.getExtents()[2]; s++) {
                        ...
                        for (i = 0; i < tmpptA.length; i++) {
                            ptNum = (int) (Short.valueOf( ( (VOIPoint) curves[s].elementAt(i)).getLabel()).shortValue()) - 1;
                            ptA[ptNum] = tmpptA[i];
                        }
                    }
            ...
            for (i = 0; i < nPtsA; i++) {
                ptAmm[i] = new Point3Dd( (double) (ptA[i].x * xresA),
                                        (double) (ptA[i].y * yresA),
            ...
            for (i = 0; i < nPtsB; i++) {
                ptBmm[i] = new Point3Dd( (double) (ptB[i].x * xresB),
                                        (double) (ptB[i].y * yresB),
       ...
       public void algorithmPerformed(AlgorithmBase algorithm) {
           if (algorithm instanceof AlgorithmRegLeastSquares) {
               if (LSMatch.isCompleted() == true) {
                   ...
                   LSMatch.getTransformBtoA().saveMatrix(userInterface.getDefaultDirectory() +
                   ...
                   if (DIM == 2) {
                       ...
                       if (matchImage.isColorImage() == false) {
                           AlgorithmTransform.transformTrilinear(matchImage, resultImage, LSMatch.getTransformBtoA(), null);
                       }
                       else {
