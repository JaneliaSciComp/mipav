...
public class ViewJComponentColocalizationRegression extends ViewJComponentBase
    ...
    // while zDim and tDim refer to imageA
    ...
    /*  When not in freeRangeMode:
        The point VOI must be dragged along the least squares fit line.
        In mousePressed and mouseDragged the intersection point of the
        perpindicular line from the mouse location to the least squares fit line
        is calculated.  The endpoints of the least squares fit line are at
        (XArray[0],yArray[0]) and (xArray[1],yArray[1]).  The mouse location is
        at (x,y).  The intersection of the perpindicular from (x,y) to
        the least squares fit line occurs at (xIntersect,yIntersect).
        By the sum of the squares formula for a right triangle we have
        (xIntersect - xArray[0])**2 + (yIntersect - yArray[0])**2
        + (xIntersect - x)**2 + (yIntersect - y)**2 =
        (x - xArray[0])**2 + (y - yArray[0])**2
        Likewise:
        (xIntersect - xArray[1])**2 + (yIntersect - yArray[1])**2
        + (xIntersect - x)**2 + (yIntersect - y)**2 =
        (x - xArray[1])**2 + (y - yArray[1])**2
        Subtracting the first equation from the second:
        (xIntersect - xArray[1])**2 - (xIntersect - xArray[0])**2
        + (yIntersect - yArray[1])**2 - (yIntersect - yArray[0])**2 =
        (x - xArray[1])**2 - (x - xArray[0])**2
        + (y - yArray[1])**2 - (y - yArray[0])**2
        With algebraic simplification:
        xIntersect*(xArray[1] - xArray[0]) + yIntersect*(yArray[1] - yArray[0]) =
        x*(xArray[1] - xArray[0]) + y*(yArray[1] - yArray[0])
        Replacing yIntersect with xIntersect*lineSlope + lineOffset,
        (xArray[1] - xArray[0] with xdiff, and (yArray[1] - yArray[0])
        with ydiff:
        xIntersect = (x*xdiff + (y - lineOffset)*ydiff)/(xdiff + lineSlope*ydiff)
        When in free range mode, the point VOI can be dragged anywhere
    ...
    *   @param useGreen
    *   @param useBlue
    *   @param originalLineSlope
    *   @param originalLineOffset
    *   @param haveThreshold
    *   @param rThreshold
    *   @param colocSize
    ...
    *   @param userInterface
    */
    public ViewJComponentColocalizationRegression(AlgorithmColocalizationRegression alg,
                                   ViewJFrameBase _frame,
                                   ViewJFrameColocalizationRegression localizeFrame,
                                   ModelImage _imageA,
                                   ModelImage _imageB,
                                   ModelImage destImage,
                                   ModelLUT LUTdest, float imageBufferDest[],
                                   boolean useRed, boolean useGreen,
                                   boolean useBlue,
                                   ...
                                   double min2, double max2, double scale1,
                                   double scale2, double lineMin1, double lineMax1,
                                   double lineMin2, double lineMax2,
                                   boolean thresholdOn1,
                                   int pixBufferDest[], int paintBufferDest[],
                                   float zoom, int extents[],
                                   boolean logMagDisplay,
                                   boolean regionLinesDisplay,
                                   int leftPad, int rightPad,
                                   int bottomPad, int topPad,
                                   boolean doSecondIteration, float linearCorrelation,
        ...
        ((VOILine)(lineVOI.getCurves()[0].elementAt(0))).exportArrays(
    ...
    /**
    *   Modifies component after mouseReleased in contour VOI indicating
    *   that the VOI has been moved.
    *   @param originalLineSlope
    *   @param originalLineOffset
    *   @param haveThreshold
    *   @param rThreshold
    ...
    *   @param linearCorrelation
    */
    public void setNewVar(float originalLineSlope,
                                   float originalLineOffset,
                                   boolean haveThreshold[], float rThreshold[],
                                   float colocSize[],
                                   float colocIntensity1[], float colocIntensity2[],
                                   double min1, double max1,
                                   double min2, double max2, double scale1,
                                   double scale2, double lineMin1, double lineMax1,
                                   double lineMin2, double lineMax2,
                                   boolean thresholdOn1, float linearCorrelation) {
        this.originalLineSlope = originalLineSlope;
        this.originalLineOffset = originalLineOffset;
        this.haveThreshold = haveThreshold;
        this.rThreshold = rThreshold;
   ...
   */
    ...
    *  @return          paintBitmap
    */
    public BitSet getpaintBitmapDest() {return paintBitmapDest;}

    /**
    *  Accessor that sets the paint mask
    *  @param  mask
    */
    public void setPaintMaskDest(BitSet mask) {paintBitmapDest = mask;}
   ...
   */
    ...
    */
    public boolean buildImageDestObject(ModelLUT _LUTdest, boolean forceShow){
        ...
        if (destImage == null) return false;
        if (LUTdest == null) return false;
        if (_LUTdest != null) LUTdest = _LUTdest;
        ...
        if (imageMaxA - imageMinA < 256  && LUTdest.getLUTType() == ModelLUT.STRIPED) {
            ...
            if (lutHeightA-1 == 0) { remapConstA  = 1; }
            else                   { remapConstA  = (lutHeightA-1)/rangeA;}
        }

        if (frame.getControls() != null) {
          OPACITY = frame.getControls().getTools().getOpacity();
          red = Math.round(frame.getControls().getTools().getPaintColor().getRed() *
                       OPACITY);
          green = Math.round(frame.getControls().getTools().getPaintColor().getGreen() *
                         OPACITY);
          blue = Math.round(frame.getControls().getTools().getPaintColor().getBlue() *
                        OPACITY);
        }
        else {
            red = 128;
        ...
        if (forceShow == true) {
            ...
            if (logMagDisplay) {
                for (i = 0; i < imageBufferDest.length; i++) {
                    imageBufferDest[i] =
                    (float)(0.4342944819*Math.log(1 + imageBufferDest[i]));
                }
            }
    ...
    */
    ...
    // the last image.  createImage must be executed right after MemoryImageSource.
    	...
    	if (data != null) {
    	    memImageDest = null;
    	    try {
                memImageDest = new MemoryImageSource( imageDim.width,
    		                                      imageDim.height,
    		                                      data,
    		                                      0,
    		                                      imageDim.width);
    ...
    /**
    *  If the correlation coefficient and colocalizations have not already been
    *  calculated at the mouse release point, then a call to
    *  AlgorithmColocalizationRegression calculates them.
    */
    public void mouseReleased(MouseEvent mouseEvent) {
        ...
        if (freeRangeMode) {
            ...
            localizeFrame.setCurrentLabels(currentColoc,currentIntensity1,
                        currentIntensity2,currentThreshold);
            ...
            ((VOIPoint)(pointVOI.getCurves()[0].elementAt(0))).locateVOIPoint
            (xS, yS, 0, xDim, yDim, 1);
        } // if (freeRangeMode)
        else { // !freeRangeMode
            // Find the intersection of the perpindicular from the mouse to
            // the least squares fit line
            xIntersectd = (xS*xdiff + (yS - lineOffset)*ydiff)/
                                    (xdiff + lineSlope*ydiff);
            ...
            ((VOIPoint)(pointVOI.getCurves()[0].elementAt(0))).locateVOIPoint
            (xIntersect, yIntersect, 0, xDim, yDim, 1);
        } // !freeRangeMode
    ...
    /**
    *  mouseDragged only updates the histogram display header when the mouse
    *  passes over a point for which the linear correlation coefficient and
    *  colocalizations have already been calculated.
    */
       ...
       int leftCurrentY;
        try {
            ...
            if (imgDest != null) {
                ...
                lineVOI.drawSelf(zoomX,zoomY,resX, resY, 0,0,
                        destImage.getFileInfo(0).getResolutions(),
                        destImage.getFileInfo(0).getUnitsOfMeasure(),
                        0,NA,g);
                ...
                bottomY = (int)((imageDim.height - 1 - (bottomPad - 20))*
                                zoomY*resY + 0.5);
                bottomSPos = Math.min(bottomY + 20,(int)((imageDim.height - 1)*
                                                    zoomY*resY + 0.5));
                ...
                if (imageA.isColorImage()) {
                    g.drawString(String.valueOf(Math.round(firstIndex)), currentX,
                                 bottomCurrentX);
                }
                else {
