...
/**
 *       Creates the dialog to add margins around the image.<p>
 *       User selects:<ol>
 ...
 *       <p>The user chooses a value for the margins, with the image minimum the default.
 *       If Color, values are chosen for red, green, and blue.
 *       A new image or replacement of the old image may be selected.
 *
 */
public class JDialogAddMargins extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "AddMargins " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
     ...
     *	@param scale	Value to set default value to.
     */
     ...
     *	@param scale	Value to set red value to.
     */
     ...
     *	@param scale	Value to set green value to.
     */
     ...
     *	@param scale	Value to set blue value to.
     */
    ...
    private boolean setVariables() {
        try {
            ...
            if ( colorFactor == 1 ) {
                dv = new Double( defaultValueInput.getText() ); // why does the compiler not find static double Double.parseInt()?
            } else {
    ...
    /**
     *	Once all the necessary variables are set, call the Image Margins
                    ...
                    if ( runInSeparateThread ) {
                        ...
                        if ( imageMarginsAlgo.startMethod( Thread.MIN_PRIORITY ) == false ) {
                            MipavUtil.displayError(
                                    "AddMargins reports: A thread is already running on this object [addMarginsAlgo]" );
                        }
                    } else {
