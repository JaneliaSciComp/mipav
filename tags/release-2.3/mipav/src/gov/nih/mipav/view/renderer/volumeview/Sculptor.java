...
/**
 * Volume Sculpturing allows the user to draw a region on the screen in the
 * volume render view, and to remove the parts of the volume covered by that
 * region.  The user outlines a region by holding the left mouse button down
 * and drawing directly on the screen. Volume Sculpturing works in both the
 * Raycast and Shearwarp volume render views through the VolumeSculptor.java
 * implementation and on the VolumeTexture render views through the
 * TextureSculptor.java implemetation.
 *
 * The user draws an outline on the screen, and defines the cut-away or
 * sculpture region.  If the user chooses to apply the sculpture region to the
 * volume, the voxels defined by an extruded volume of the cut-away region are
 * removed.  This is done by projecting the voxels in the volume into screen
 * space and determining which voxels fall inside the sculpture region.
 * Voxels that are inside the sculpture region are set to the minimum voxel
 * value, which is rendered as transparent.  The user may also undo volume
 * sculpturing, returning the volume data to its original values.
 *
 * Drawing the Sculpt Region on the Screen
 *
 * The VolumeSculptor class implements both MouseListener and
 * MouseMotionListener interfaces. Drawing is done through the mousePressed,
 * mouseReleased, and mouseDragged functions.
 *
 * When sculpting is enabled, the viewing transformations (rotation, scale,
 * translation) are disabled, so the left mouse button can be used for drawing
 * on the screen instead of rotating the volume. Second, the view canvas is
 * captured and stored. When the left mouse button is pressed, the
 * VolumeSculptor class captures and stores all subsequent mouse positions,
 * and draws them directly into the canvas image, connecting the mouse
 * positions by drawing 2D lines on the image. On a mouseReleased event the
 * outline is closed by connecting the first and last recorded mouse positions
 * and the outline region is filled in. The line drawing and polygon filling
 * functions are implemented as 2D scan-conversion algorithms and are drawn
 * directly by the VolumeSculptor onto the canvas image. This implementation
 * allows concave and self-intersecting outlines.
 *
 * Multiple outlines may be drawn on the canvas image before applying the
 * sculpt regions to the volume. Because the sculpt regions are in image
 * space, to apply the sculpt region to the volume, it is simply necessary to
 * project the volume voxels onto the canvas image. Voxels that project onto
 * blue pixels in the canvas image are removed, and voxels that project to
 * non-masked pixels are unaltered.
 *
 * Removing Sculpted Voxels
 *
 * The process of applying the sculpt region to the volume data and removing
 * the voxels that fall inside the sculpt region is broken down into three
 * steps. First, the ModelImage data is retrieved and the center and spacing
 * of the data is determined. Second, the viewing transformations are
 * calculated. Third, the voxels are mapped into screen space and the location
 * of the voxel is compared to the corresponding pixel. If the voxel maps to a
 * pixel inside the sculpt region its value is set to the minimum voxel value.
 *
    ...
    implements MouseMotionListener, MouseListener
{
    ...
    /* Green Progress bar at the top right hand corner of the
     * ViewJFrameVolumeView window. */
    protected JProgressBar m_kProgress;

    /*
     * m_bSculptEnabled is turned on when the user presses the "Draw Sculpt
     * Outline" button.
     */
    ...
    /*
     * m_bMousePressed is set to true when the mousePressed callback is
     * activated and the left mouse button is down.
     */
    protected boolean m_bMousePressed;

    /*
     * The original canvas size. This is used if the ViewJFrameVolumeView
    ...
    /*
     * m_kSculptImage is a screen shot of the canvas image. The sculpt region
     * is drawn directly into m_kSculptImage. The outline is filled by
     * alpha-blending the Sculpt region color with the m_kSculptImage
     * backgound image. m_kSculptImageOpaque is filled with solid color, but
     * not displayed, it is used to test whether voxels are inside the sculpt
     * region.
     */
     ...
     * original canvas widths. Disable sculpting and drawing with the mouse.
     *
     * Note - this function should be called once per instance only, as it sets
     * up the MouseMotionListener and MouseListener.
     */
    ...
    /**
     * Invoked when a mouse button is pressed.  When the left mouse button is
     * pressed, and sculpting is enabled, then the user is beginning to
     * outline the sculpt region. This function initializes the outline
     * drawing, and stores the first point(x,y) so the outline can be closed
     * on mouseReleased.  The first and following mouse positions are drawn in
     * outline form using current and previously (last recorded) positions as
     * the start and end points of a 2D line on the canvas.
     *
    ...
    /**
     * Invoked when a mouse button is released. If this happens while drawing
     * the sculpt outline, the mouse release indicates that the outline is
     * complete. The function closes the outline by connecting the first and
     * last points, and then calls processMouseReleased(), which fills the
     * outline.
     *
    ...
    public void mouseReleased( MouseEvent kEvent ) {
        ...
        if ( m_bSculptEnabled && m_bMousePressed &&
             ( kEvent.getButton() == MouseEvent.BUTTON1 ) )
        {
            m_bMousePressed = false;
    ...
    /*
     * Called when the left mouse button is released, indicating that the
     * sculpt outline is complete, or when the outline is being drawn and the
     * mouse is fragged outside the canvas area.  The function closes the
     * outline, using the first and last recorded mouse positions. It then
     * fills the outline, by blending the fill color with the background data
     * on the canvas. Filling is done as a 2D polygon scan conversion so that
     * any closed loop, including concave and self-intersecting loops, can be
     * processed accurately.
     *
     * @param iX int, the x position of the mouse recorded when the mouse
     * button was released.
     * @param iY int, the y position of the mouse recorded when the mouse
     * button was released.
     */
    ...
    /**
     * Invoked when the mouse leaves a component. This function captures the
     * mouseExited event when the button is pressed and the sculpt outline is
     * being drawn, so that the outline is contained within the bounds of the
     * canvas.
     *
    ...
    /**
     * One of the overrides necessary to be a MouseMotionListener. Invoked
     * when the mouse is moved, but no buttons are pressed.
     *
    ...
    /**
     * Invoked when the mouse is dragged while a button is held down. If this
     * occurs while sculpting is enabled, then the mouse positions are stored
     * and the sculpt outline is drawn on the canvas, using the previously
     * recorded mouse position and the current mouse position as the start and
     * end points of a 2D line on the canvas.
     *
    ...
    /*
     * This function draws a 2D line on the canvas image -- ontop of the
     * currently rendered image.
     *
     * To implement the line drawing I use the midpoint line algorithm, in
     * Foley and van Dam, originally Bresenham's algorithm.  The first part of
     * the function sets up the step sizes for x and y depending on what type
     * of line is being drawn. The second part loops over the line, drawing
     * pixels into the canvas image.
     *
     * @param iX0, int start x position
     * @param iY0, int start y position
     * @param iX1, int end x position
     * @param iY1, int end y position
     */
    ...
    /*
     * blendColor: blends two colors in ARGB format from the BufferedImage
     * class, using an alpha value of 0.5.
     *
     * @param iImageColor, int the src ARGB color from the Buffered image
     * @param iBlendColor, int the color to blend ontop of the image color
    ...
    /*
     * fill: fill the sculpt outline drawn by the user. Pixels are determined
     * to be inside or outside the sculpt region based on the parameters,
     * aaiCrossingPoints and aiNumCrossings, using a scan-conversion algorithm
     * that traverses each row and column of the bounding box of the sculpt
     * region coloring inside points as it goes.
     *
     * @param aaiCrossingPoints, int[][] array, specifing where the edge
     * crosses each column in the sculpt region bounding box.
     * @param aiNumCrossings, int[] array, specifies the number of edge
     * crossings per column.
     */
        ...
        for ( int iX = m_iXMin; iX < m_iXMax; iX++ )
        {
            ...
            for ( int iY = m_iYMin; iY < m_iYMax; iY++ )
            {
                ...
                if ( bInside == true )
                {
                     ...
                     * image color from the canvas image and alpha-blend the
                     * sculpt color ontop, storing the result in the canvas
                     * image.
                     */
                     ...
                     * image for point-polygon test later when the sculpt
                     * region is applied to the volume.
                     */
    ...
    /*
     * This function computes the set of spans indicated by column crossings
     * for the sculpt outline drawn by the user, by doing a polygon scan
     * conversion in gridded space. The outline must be closed with last point
     * = first point.
     */
    protected void
    ...
    {
        /*
         * nudge the vertices off of the exact integer coords by a factor of
         * 0.1 to avoid vertices on pixel centers, which would create spans of
         * zero length
         */
        double dNudge = 0.1;
        ...
        {
            aaadEdgeList[ iPoint ][ 0 ][ 0 ] =
                m_aiXPoints[ iPoint ] - dNudge;
            aaadEdgeList[ iPoint ][ 0 ][ 1 ] =
                m_aiYPoints[ iPoint ] - dNudge;
            aaadEdgeList[ iPoint ][ 1 ][ 0 ] =
                m_aiXPoints[ iPoint + 1 ] - dNudge;
            aaadEdgeList[ iPoint ][ 1 ][ 1 ] =
                m_aiYPoints[ iPoint + 1 ] - dNudge;
        }
        ...
        for ( int iColumn = m_iXMin; iColumn <= m_iXMax; iColumn++ )
        {
            ...
            for ( int iPoint = 0; iPoint < m_iNumberPoints - 1; iPoint++)
            {
                ...
                if ( ( dMinX < iColumn ) && ( dMaxX > iColumn ) )
                {
                    ...
                    double dB = ( dDX == 0 ) ?
                        0 : ( ( dX1 * dY0 ) - ( dY1 * dX0 ) ) / dDX;

                    double dYCross = dM * iColumn + dB;
                    double dRound = 0.5;
                    aaiCrossingPoints[ iIndex ][ aiNumCrossings[ iIndex ] ] =
                        ( dYCross < 0 ) ?
                        (int)( dYCross - dRound ) :
                        (int)( dYCross + dRound );
            ...
            sortCrossingPoints( aaiCrossingPoints[ iIndex ],
                                aiNumCrossings[ iIndex ] );
        }
        aaadEdgeList = null;
    }
    ...
    protected boolean getFrameBuffer( )
    {
       ...
       try {
           ...
           MipavUtil.
               displayError( "VolumeSculpt: unable to allocate enough memory for RGB image" );
           return false;
       } catch ( AWTException error ) {
           MipavUtil.
               displayError( "Platform doesn't support screen capture." );
           return false;
       }
    ...
    /*
     * enableSculpt: called by the ViewJFrameVolumeView object when the Draw
     * Sculpt button is pressed. This function deactivates the
     * m_kVolumeRenderer's mouse response, so the mouse can be used to draw
     * the sculpt outline. It also allocates and initializes the
     * m_iSculptImage buffer for drawing.
     */
    public void enableSculpt( boolean bEnabled )
    {
        ...
        if ( (m_iSculptImageWidth <= 0) ||
             (m_iSculptImageHeight <= 0)    )
        {
         ...
         */
        m_kSculptImage = null;
        m_kSculptImage = new BufferedImage( m_iSculptImageWidth,
                                            m_iSculptImageHeight,
                                            BufferedImage.TYPE_INT_ARGB );

        m_kSculptImageOpaque = null;
        m_kSculptImageOpaque = new BufferedImage( m_iSculptImageWidth,
                                                  m_iSculptImageHeight,
                                                  BufferedImage.TYPE_INT_ARGB);
        m_kSavedImage = null;
        m_kSavedImage = new BufferedImage( m_iSculptImageWidth,
                                           m_iSculptImageHeight,
                                           BufferedImage.TYPE_INT_ARGB);
        getFrameBuffer(  );
    }
    ...
    /*
     * clearSculpt: called by ViewJFrameVolumeView when the user presses the
     * "Clear Ouline" button, clearing the sculpt outline from the canvas
     * image. The function disables sculpting and reactivates the mouse events
     * for the m_kVolumeRenderer.
     */
    ...
    /*
     * invertSculpt: called by ViewJFrameVolumeView when the user presses the
     * "Invert Sculpt Region" button, inverting the sculpt region.
     */
        ...
        for ( int iX = 0; iX < m_iSculptImageWidth; iX++ )
        {
            for ( int iY = 0; iY < m_iSculptImageHeight; iY++ )
            {
                ...
                else
                {
                     ...
                     * saved image to blend in the sculpt color for the
                     * SculptImage, and use the m_iColorSculpt for the Opaque
                     * image: */
                    iImageColor = m_kSavedImage.getRGB( iX, iY );
    ...
    /* Called by the TextureSculptor or VolumeSculptor objects. The function
     * calculates the mimumum and maximum intensity and color values in the
     * images. */
    protected void calculateMinMaxValues( ModelImage kImageA,
                                          ModelImage kImageB )
    {
    ...
    /* Called by the TextureSculptor or VolumeSculptor objects. The function
     * sculpts the original volume data by setting the value at input iIndex
     * to the mimimum values calculated in CalculateMinMaxValues.*/
    protected void sculptImage( ModelImage kImageA, ModelImage kImageB,
                                int iIndex )
    {
    ...
    /* Called by the TextureSculptor or VolumeSculptor objects. The function
     * stores the original volume data back to the original values in the
     * m_aiImage_backup data members.*/
                                ...
                                int iIndex )
    {
        ...
        if ( kImageA.isColorImage() )
        {
            ...
            {
                m_aiImageA_backup[ iIndex * iColor + iC ] =
                    kImageA.getInt( iIndex * iColor + iC );
            }
        }
        else
        {
        ...
        if ( kImageB != null )
        {
            if ( kImageB.isColorImage() )
            {
                for ( int iC = 0; iC < iColor; iC++ )
                {
                    m_aiImageB_backup[ iIndex * iColor + iC ] =
                        kImageB.getInt( iIndex * iColor + iC );
                }
            }
            else
            {
                m_aiImageB_backup[ iIndex ] =
                    kImageB.getInt( iIndex );
            }
        }
    }
    ...
    /* Called by the TextureSculptor or VolumeSculptor objects. The function
     * resets the volume data back to the original values, using the data
     * stored in the m_aiImage_backup data members.*/
            ...
            for ( int iX = 0; iX < iXBound; iX++ )
            {
                for ( int iZ = 0; iZ < iZBound; iZ++ )
                {
                    ...
                    if ( kImageA.isColorImage() )
                    {
                        ...
                        {
                            kImageA.set( iIndex * 4 + iColor,
                                         m_aiImageA_backup[ iIndex * 4 + iColor ] );
                        }
                    }
                    else
                    {
                    ...
                    if ( kImageB != null )
                    {
                        if ( kImageB.isColorImage() )
                        {
                            for ( int iColor = 0; iColor < 4; iColor++ )
                            {
                                kImageB.set( iIndex * 4 + iColor,
                                             m_aiImageB_backup[ iIndex * 4 + iColor ] );
                            }
                        }
                        else
                        {
     ...
     * instance is VolumeTextureSculptor or VolumeSculptor
     */
    public abstract boolean applySculpt();

    /*
     * undoSculpt: abstract function, implementation depends on whether the
     * instance is VolumeTextureSculptor or VolumeSculptor
     */
