...
/**
 * This class is used to coordinate
...
public class ViewJComponentVolOpacity extends ViewJComponentVolOpacityBase {
    ...
    /**
     *   Filters the histogram through the LUT and produces
     *   an int array (aRGB) that can be converted to a java image
     *   of the histogram for display purposes.
     */
    ...
    public void showHistogram( boolean repaint ) {
        ...
        for ( i = 0; i < dim.width - 1; i++ ) {
            ...
            if ( index < 0 ) index = 0;
            if ( index > lutIndexBuffer.length-1 ) index = lutIndexBuffer.length-1;
        ...
        if ( myParent instanceof SurfaceRender ) {
            ...
            if ( myMouseDialog.mode == JPanelMouse.RECORD_MODE ) {
                ...
                opacityEvents = new MouseEventVector( "VolOpacity" + opacityCount, t3D, myMouseDialog.first,
                        ( (SurfaceRender) myParent ).getSceneState(), ( (SurfaceRender) myParent ).getMouseMode() );
