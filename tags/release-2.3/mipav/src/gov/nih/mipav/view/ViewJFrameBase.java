 ...
 */
public abstract class ViewJFrameBase
    extends JFrame implements ViewImageUpdateInterface,
    ActionListener,
    WindowListener,
    ...
    /** Indicates the amount of blending when two images
     *  are loaded in the image frame. */
    ...
    /**
     *   Sets the active image (image that should be processed for drawing
     *   VOIs and applying algorithms
    ...
    /**
     *  If true do not getMask on a setActiveImage command so as to
     *  keep the mask from the old active image.
    ...
    /**
     * Removes image B from the frame and resets image A to be the active
     * frame for all image controls.  Ensures the image is removed,
     * disconnected from the frame and un-registered.  Also informs the
     * garbage-collector to release any held memory.
     */
    public void closeImageB() {

        if (imageB != null) {
            imageB.removeImageDisplayListener(this);
            if (imageB.getImageFrameVector() != null &&
                imageB.getImageFrameVector().isEmpty()) {
                imageB.disposeLocal();
    ...
    /**
     * Closes both image A and image B (if it exists).  It ensures the images
     * are un-registered from the main-frame then removes any display listeners.
     */
    public void close() {
        setVisible(false);
    ...
    /**
     * This routine is designed to transform AFNI functional images and AFNI 3D + time anatomical
     * images to match AFNI original 3D anatomical images.  Here the 3 axes of the 2 images
     * are parallel and the spatial orientation of the 2 sets of axes are identical.
     * <p>
     * AFNI:Software for Analysis and Visualization of Functional Magnetic Resonance Neuroimages by
     * Robert W. Cox states:
     * <blockquote>
     * &quot;In some applications, gaps are present between the functional slices.
     ...
     * interpolated nonlinear statistic is a somewhat dubious procedure.&quot;
     *
     * At another point the manual states:
     * &quot;Threshold data in functional datasets is always resampled using the nearest neighbor
     ...
     */
    private void transformAFNI(ModelImage image, float[] imgBuffer,
                               float iXres, float iYres, float iZres,
                               int iXdim, int iYdim, int iZdim, int iTdim,
                               float ilowXmm, float ilowYmm, float ilowZmm,
                               float ihighXmm, float ihighYmm, float ihighZmm,
                               int planeGap, int[] gapArray, boolean doNN,
                               float oXres, float oYres, float oZres,
                               int oXdim, int oYdim, int oZdim,
                               float olowXmm, float olowYmm, float olowZmm,
                               float ohighXmm, float ohighYmm, float ohighZmm) {
        // This routine is designed to transform AFNI functional images and AFNI 3D + time anatomical
        // images to match AFNI original 3D anatomical images.  Here the 3 axes of the 2 images
        // are parallel and the spatial orientation of the 2 sets of axes are identical.
                                    ...
                                    if ( (Z >= 0) && (roundZ < iZdim)) {
                                        if ( ( (int) X < iXdim - 1) &&
                                            ...
                                            if (interp) {
                                                ...
                                                temp7 = y0 * z0;
                                                value = x1 * temp4 *
                                                    imgBuffer[Z0pos + Y0pos +
                                                    X0pos + tOffset]
                                                    +
                                                    x0 * temp4 * imgBuffer[Z0pos +
                                                    Y0pos + X1pos + tOffset]
                                                    +
                                                    x1 * temp5 * imgBuffer[Z0pos +
                                                    Y1pos + X0pos + tOffset]
                                                    +
                                                    x0 * temp5 * imgBuffer[Z0pos +
                                                    Y1pos + X1pos + tOffset]
                                                    +
                                                    x1 * temp6 * imgBuffer[Z1pos +
                                                    Y0pos + X0pos + tOffset]
                                                    +
                                                    x0 * temp6 * imgBuffer[Z1pos +
                                                    Y0pos + X1pos + tOffset]
                                                    +
                                                    x1 * temp7 * imgBuffer[Z1pos +
                                                    Y1pos + X0pos + tOffset]
                                                    +
                                                    x0 * temp7 * imgBuffer[Z1pos +
                                                    Y1pos + X1pos + tOffset];
                                            } // if (interp)
                                        }
                                        else if ( ( (roundX == iXdim - 1) ||
                                            (roundY == iYdim - 1) ||
                                            (roundZ == iZdim - 1))
                                                 &&
                                                 ( (xloc >= olowXmm) &&
                ...
                for (i = 0; i < oXdim; i++) {
                    ...
                    for (j = 0; j < oYdim; j++) {
                        ...
                        for (k = 0; k < oZdim; k++) {
                            ...
                            yloc = roundY * iYres + ilowYmm;
                            zloc = roundZ * iZres + ilowZmm;
                            if ( (roundX < 0) || (roundX > iXdim - 1) ||
                                (roundY < 0) || (roundY > iYdim - 1)
                                || (roundZ < 0) || (roundZ > iZdim - 1) ||
                                (xloc < olowXmm)
                                || (xloc > ohighXmm) || (yloc < olowYmm) ||
                                (yloc > ohighYmm)
                                || (zloc < olowZmm) || (zloc > ohighZmm)) {}
                            else {
                                interp = true;
                                // If values have not been collected in the original functional image
                                // and the gaps are filled in with zeroes, then set the value equal
                                ...
                                if (interp) {
                                    ...
                                    value = imgBuffer[xOffset + yOffset +
                                        zOffset + tOffset];
                                }
                            }
                            if (image.getNDims() == 3) {
     ...
     *   @param oZdim      output z dimension
     */
    private void transformACPC(ModelImage image, float[] imgBuffer,
                               double[][] xfrm,
                               float iXres, float iYres, float iZres,
                               int iXdim, int iYdim, int iZdim, int iTdim,
                               int planeGap, int[] gapArray, boolean doNN,
     ...
     */
    private void transformTalairach(ModelImage image, float[] imgBuffer,
                                    double[][] xfrm,
                                    float iXres, float iYres, float iZres,
                                    int iXdim, int iYdim, int iZdim, int iTdim,
                                    int planeGap, int[] gapArray, boolean doNN,
        ...
        if (!doNN) {
            for (t = 0; t < tLast; t++) {
                ...
                for (i = botX; i <= topX; i++) {
                    if (i % mod == 0) {
                        progressBar.updateValue(
                            (int) ( (float) (t * (topX - botX + 1) + (i - botX))
                                   /
                                   ( (iTdim - 1) * (topX - botX + 1) + (topX - botX)) *
                                   100
                                   + .5), true);
                    }
                    imm = (float) i * oXres;
    ...
    /**
     *   Loads an image into imageB slot of frame.  Matches the imageB resolutions and dimensions to
     *   imageA, and can optionally match the images' origins and orientations.
     *
     *   @param obj  an object which is either a ModelImage to be loaded into the modelimage
     *               (as say, imageB, imported from another frame) or a File to be read in
     *               via FileIO.readImage(...)
     *   @param compImage  image component where image is set so that it can be displayed.
     *   @param stackFlag  flag indicating multi file
     *   @param doOrigins  when calling MatchImages to match resolutions and dimensions,
     *	                   should origins be matched too?
     *   @param doOrients  when calling MatchImages to match resolutions and dimensions,
     *	                   should orientations be matched too?
     *   @return  true if the load was successful, false otherwise
     *
     *   @see JDialogLoadImage
     */
    public boolean loadImage(Object obj,
        ...
        try {
            ...
            if (obj instanceof File) {
                ...
                imageB = fileIO.readImage(file.getName(),
                                          file.getParent() +
                                          File.separator,
                                          stackFlag, null,
                                          true); // read image!
            ...
            if ( ( (imageA.isColorImage() == true && imageB.isColorImage() == false)
                ...
                MipavUtil.displayError(
                    "Image loading failed because the color space is different. Both images must be grayscale or both RGB.");
                return false;
            }

            if (isMincLabelFile(compImage, imageB)) {
                imageB = makeMinc(compImage.getActiveImage(), imageB);
                setImageB(imageB);
                return success = true;
            }
            ...
            if (imageB.getFileInfo(0).getFileFormat() == FileBase.AFNI && imageB.getNDims() > 2 &&
                (axisA[0] != FileInfoBase.ORI_UNKNOWN_TYPE) &&
                (axisB[0] != FileInfoBase.ORI_UNKNOWN_TYPE) &&
    ...
    private boolean isResampleNeeded(ModelImage image) {
        ...
        if (image.getExtents() == null) {
            ...
            Preferences.debug("image.getExtents().length = " +
                              image.getExtents().length, Preferences.DEBUG_MINOR);
        }
        ...
        if (imageA.getExtents() == null) {
            ...
            Preferences.debug("imageA.getExtents().length = " +
                              imageA.getExtents().length, Preferences.DEBUG_MINOR);
        }
    ...
    private boolean isImageResampleable(ModelImage image) {
        ...
        if ( (imageB.getNDims() == imageA.getNDims()) ||
            ( (imageB.getNDims() == 3 || imageB.getNDims() == 4) && (imageA.getNDims() == 3 && imageA.getNDims() == 4))) {
            if (minDims == 4) {
    ...
    /**
     * Match an image against imageA.  Resolutions and extents are always matched.
     * Orientations and origins are optional.
    ...
    private boolean matchImages(ModelImage image, boolean doOrigins, boolean doOrients) {
        ...
        try {
            ...
            if ( (image.getNDims() == 3 && imageA.getNDims() == 3) ||
                ...
                algoMatch = new AlgorithmMatchImages(imageA, image, doOrigins,
                    doDimensions, resByRef);
     ...
     * This image should already be matched against imageA.
    ...
    private void loadResampledImage(ModelImage image) {
        if (image.getNDims() == 3 || image.getNDims() == 4) {
            ...
            AlgorithmTransform transformVol = new AlgorithmTransform(image,
                xfrm, AlgorithmTransform.TRILINEAR,
                oXres, oYres, oZres, oXdim, oYdim, oZdim, false, true, false);
            ...
            AlgorithmTransform transformVol = new AlgorithmTransform(image,
                xfrm, AlgorithmTransform.BILINEAR,
                oXres, oYres, oXdim, oYdim, false, true, false);
            ...
            if (!image.isColorImage()) {
                Preferences.debug(
                    "Done resampling image: min = " + resampledImage.getMin() +
                    "max = "
                    + resampledImage.getMax() + "\n");
            }
            else {
                Preferences.debug(
                    "Done resampling image: minR = " + resampledImage.getMinR() +
                    "minG = "
                    + resampledImage.getMinG() + "minB = " +
                    resampledImage.getMinB() + "maxR = "
                    + resampledImage.getMaxR() + "maxG = " +
                    resampledImage.getMaxG() + "maxB = "
    ...
    /**
     * Returns whether an image is a Minc label file by seeing if the file is Minc
     * and then asking the user if it is a label.
     * @param compImage  the component image contained within this frame (holding imageA)
     * @param image      the image we want to test
     * @return           true if <code>image</code> is a MINC file
     */
    private boolean isMincLabelFile(ViewJComponentEditImage compImage, ModelImage image) {
        if (image.getFileInfo(0).getFileFormat() == FileBase.MINC &&
            compImage.getActiveImage().getFileInfo(0).getFileFormat() == FileBase.MINC) {

            int opt = JOptionPane.showConfirmDialog(this,
                "Is this a MINC label file?", "MINC label",
     ...
     *   // This should be moved to imageModel.save();
     ...
     *   @param operateQuiet informs the FileIO to not notify user of errors or
     *               interrogate for more information.
     */
    public void save(ModelImage img, FileWriteOptions options, int filterType,
        ...
        if (options.isSaveAs()) {
            ...
            if (options.isSet()) {
                ...
                try {
                    ...
                    if (!operateQuiet) {
                        MipavUtil.displayError(
                            "Out of memory: ViewJFrameBase.save");
                    }
                    Preferences.debug("Out of memory: ViewJFrameBase.save\n", 3);
                    return;
                }
        ...
        if (fileName != null) {
            ...
            if ( ( (fileName.endsWith(".avi")) || (fileName.endsWith(".AVI")) ||
                ...
                // (imageA.getType() != ModelStorageBase.UBYTE)) {
                // System.err.println("TYPE: " + imageA.getType());
                // MipavUtil.displayError("Must convert image to ARGB or UBYTE for AVI");
                // return;
                // }
    ...
    /**
     *   This method saves the LUT for the active image. If the image
     *   is not a color image then both the functions and the LUT data
     *   are saved.  If this is a color image, then only the functions
     *   are saved.
     *   @param  saveAll     - boolean indicating that both lut and
     *                         transfer functions should be saved.
     *                         If false, then only transfer functions
     *                         are saved.
     */
     ...
     *                         are saved.
     *
     *                         If either filename or directory is null, then
    ...
    public void saveLUTAs(boolean saveAll, String filename, String dirName) {
        ...
        if (filename == null || dirName == null) {
            ...
            if (saveAll) {
                chooser.addChoosableFileFilter(new ViewImageFileFilter(
                    ViewImageFileFilter.LUT));
            }
            else {
                chooser.addChoosableFileFilter(new ViewImageFileFilter(
                    ViewImageFileFilter.FUNCT));
            }
            int returnVal = chooser.showSaveDialog(this);

            if (returnVal == JFileChooser.APPROVE_OPTION) {
                filename = chooser.getSelectedFile().getName();
                dirName = String.valueOf(chooser.getCurrentDirectory()) +
                    File.separatorChar;
            }
    ...
    /**
     * This method loads the LUT for the active image. If the image
     * is not a color image then both the functions and the LUT data
     * are loaded.  If this is a color image, then only the functions
     * are loaded.
     * @param loadAll    boolean indicating that both lut and
     *                   transfer functions should be loaded.
     *                   If false, then only transfer functions
     *                   are loaded.
     * @param quietMode  if true indicates that warnings
     ...
     * @param loadAll    boolean indicating that both lut and
     *                   transfer functions should be loaded.
     *                   If false, then only transfer functions are loaded.
     * @param filename   filename to save LUT as
     * @param dirName    directory to save LUT to
     * @param quietMode  if true indicates that warnings should not be displayed.
     */
    public void loadLUTFrom(boolean loadAll, String filename, String dirName,
                            boolean quietMode) {
     ...
     *   @param quietMode  if true indicates that warnings should not be displayed.
     */
    public void loadOnlyLUTFrom(boolean loadAll, String filename,
                                String dirName, boolean quietMode) {
    ...
    /**
     *   This method loads all VOIs to the active image from the
     *   default VOI directory for that image.
     *
     *   @param quietMode   if true indicates that warnings should not be displayed.
     */
    public void loadAllVOIs(boolean quietMode) {
        ...
        if (displayMode == IMAGE_A) {
            ...
            if (!quietMode) {
                MipavUtil.displayError(
                    " Cannot open VOIs when viewing both images");
            }
            return;
        }
        ...
        voiDir = new String(fileDir + File.separator + "defaultVOIs_" +
                            imageName + File.separator);
        ...
        try {
            ...
            if (voiFileDir.exists() && voiFileDir.isDirectory()) {
                ...
                for (int k = 0; k < files.length; k++) {
                    if (files[k].getName().endsWith(".voi") ||
                        files[k].getName().endsWith(".xml")) {
                ...
                if (!quietMode) {
                    MipavUtil.displayError("No VOIs are found in directory: " +
                                           voiDir);
                }
                return;
            }
            ...
            for (i = 0; i < filenames.size(); i++) {
                ...
                fileVOI = new FileVOI( (String) (filenames.elementAt(i)),
                                      voiDir, currentImage);
            ...
            if (!quietMode) {
                MipavUtil.displayError("Error loading all VOIs from " + voiDir +
                                       ": " + error);
            }
        }
    ...
    public boolean openVOI(boolean quietMode) {
        ...
        try {
            ...
            if (displayMode == IMAGE_A) {
                ...
                if (!quietMode) {
                    MipavUtil.displayError(
                        " Cannot open VOI when viewing both images");
                }
                return false;
            }
        }
    ...
    /**
     *   This method saves all VOIs for the active image to the
     *   default VOI directory for that image.
     */
    ...
    public void saveAllVOIsTo(String voiDir) {
        ...
        try {
            if (displayMode == IMAGE_A) {
                ...
                MipavUtil.displayError(
                    " Cannot save VOIs when viewing both images");
                return;
            }
            ...
            if (voiFileDir.exists() && voiFileDir.isDirectory()) {
                ...
                if (voiFileDir.getName().startsWith("defaultVOIs_")) {
                    ...
                    if (files != null) {
                        for (int k = 0; k < files.length; k++) {
                            if (files[k].getName().endsWith(".voi") ||
                                files[k].getName().endsWith(".xml")) { // files[k].delete();
                            }
                        }
                    } // if (files != null)
                }
            ...
            for (i = 0; i < nVOI; i++) {
                ...
                fileVOI = new FileVOI(VOIs.VOIAt(i).getName() + ".xml", voiDir,
                                      currentImage);
    ...
    public void saveVOI(boolean saveAllContours) {
        ...
        try {
            if (displayMode == IMAGE_A) {
                ...
                fileVOI = new FileVOI(VOIs.VOIAt(i).getName() + ".xml",
                                      imageA.getFileInfo(0).getFileDirectory(),
                                      imageA);
                ...
                fileVOI = new FileVOI(VOIs.VOIAt(i).getName() + ".xml",
                                      imageB.getFileInfo(0).getFileDirectory(),
                                      imageB);
    ...
    public void about(int zSlice, int tSlice) {
        ...
        if (imageA.getFileInfo()[0] != null) {
            if ( (imageA.getFileInfo()[0]).getFileFormat() == FileBase.DICOM &&
                displayMode == IMAGE_A) {
                dicom = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() == FileBase.XML &&
                     displayMode == IMAGE_A) {
                xml = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() == FileBase.RAW &&
                     displayMode == IMAGE_A) {
                xml = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() ==
                     FileBase.JIMI && displayMode == IMAGE_A) {
                xml = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() ==
                     FileBase.ANALYZE && displayMode == IMAGE_A) {
                analyze = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() ==
                     FileBase.NIFTI && displayMode == IMAGE_A) {
                nifti = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() == FileBase.SPM &&
                     displayMode == IMAGE_A) {
                spm = true;
            }
            else if ( (imageA.getFileInfo()[0]).getFileFormat() ==
                     FileBase.GE_GENESIS && displayMode == IMAGE_A) {
                geSigna = true;
                ...
                if ( (imageB.getFileInfo()[0]).getFileFormat() ==
                    FileBase.DICOM && displayMode == IMAGE_B) {
                    dicom = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() == FileBase.XML &&
                    displayMode == IMAGE_B) {
                    xml = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() == FileBase.RAW &&
                    displayMode == IMAGE_B) {
                    xml = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() == FileBase.JIMI &&
                    displayMode == IMAGE_B) {
                    xml = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() ==
                    FileBase.ANALYZE && displayMode == IMAGE_B) {
                    analyze = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() ==
                    FileBase.NIFTI && displayMode == IMAGE_B) {
                    nifti = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() == FileBase.SPM &&
                    displayMode == IMAGE_B) {
                    spm = true;
                }
                if ( (imageB.getFileInfo()[0]).getFileFormat() ==
                    FileBase.GE_GENESIS && displayMode == IMAGE_B) {
                    geSigna = true;
                }
        ...
        if (dicom) {
            ...
            aboutDialog = new JDialogFileInfoXML(this, "Image information",
                                                 imageA);
        }
        else {
            if (analyze) {
                if (displayMode == IMAGE_A) {
                    aboutDialog = new JDialogFileInfo(this, "Image Information",
                        imageA);
                }
                else {
                    aboutDialog = new JDialogFileInfo(this, "Image Information",
                ...
                if (displayMode == IMAGE_A) {
                    aboutDialog = new JDialogTextGE(this, "Image Information",
                        imageA, zSlice);
                }
                else {
                    aboutDialog = new JDialogTextGE(this, "Image Information",
        ...
        if (!dicom) {
            try {
                ...
                Preferences.debug(
                    "ClassCastException in ViewJFrameBase.about\n", 5);
            }
     ...
     * Implemented by specific frame classes.
    ...
    /**
     *   The extents on this image have changed, so the extents need
     *   to be read in again and menus, panes and slide
        ...
        userInterface, true, false);
            ...
            if (!fileChooser.useAWT()) {
                ...
                if (filter != -1) {
                    ...
                    javax.swing.filechooser.FileFilter found = findFilter(
                        chooser, filter);
     ...
     */
    private javax.swing.filechooser.FileFilter findFilter(JFileChooser chooser,
        int filterType) {
        // get the list of file filters held in chooser
        javax.swing.filechooser.FileFilter[] filters = chooser.
    ...
    /**
     *	Takes in a MINC source image and a MINC label image, and returns an image which can
     *	be loaded as the MINC source image's Image B.  The reason we need this method is that
     *	MINC label images are small, only encompassing the relevant information.  Where the mask
     *	is relative to the source image is based on header information: specifically, the start
     *	variables for each dimension.  Another tricky aspect of the MINC label files is that the
     *	orientation can be strange: for instance, an axial image which normally has the patient's x
     *	axis as the image x axis and the patient's y axis as the image y axis can be reversed, so that
     *	the label image would appear to be sideways.
     *	@param srcImage		MINC source image.
     *	@param labelImage	MINC label image.
     *	@return		        Image to load into source image as Image B.
     *  @throws IOException  if there is a problem importing or creating the Minc
     */
    private ModelImage makeMinc(ModelImage srcImage, ModelImage labelImage) throws
        IOException {
        ModelImage destImage = new ModelImage(labelImage.getType(),
                                              srcImage.getExtents(),
                                              labelImage.getImageName(),
                                              userInterface);
        FileInfoMinc fileInfo = (FileInfoMinc) labelImage.getFileInfo(0);
        float[] srcStart;
        float[] labelStart;
        int bufferSize = srcImage.getSliceSize();
        int xDiff;
        int yDiff;
        int zDiff;
        float[] destBuffer = new float[srcImage.getSize()];
        int[] labelOri;
        int[] imageOri;

        imageOri = srcImage.getFileInfo(0).getAxisOrientation();
        labelOri = fileInfo.getAxisOrientation();
        ModelImage labelImage2 = labelImage;

        Preferences.debug("Beginning MINC label transform.\n");
        // If axis orientations are not unknown, rotate and reorder the label image to have the exact same
        // axis orientations as the original image.
        if (imageOri[0] != FileInfoBase.ORI_UNKNOWN_TYPE &&
            labelOri[0] != FileInfoBase.ORI_UNKNOWN_TYPE) {
            while (! (imageOri[0] == labelOri[0] && imageOri[1] == labelOri[1] &&
                      imageOri[2] == labelOri[2])) {
                Preferences.debug("Image axis: " + imageOri[0] + " " +
                                  imageOri[1] + " " + imageOri[2] + "\n");
                Preferences.debug("Label axis: " + labelOri[0] + " " +
                                  labelOri[1] + " " + labelOri[2] + "\n");
                if (imageOri[2] == labelOri[2]) {
                    if (imageOri[1] == labelOri[1]) {
                        // x orientations are exactly opposite.
                        // Flip horizontally.
                        Preferences.debug("Flip horizontally\n");
                        AlgorithmFlip flip = new AlgorithmFlip(labelImage2,
                            AlgorithmFlip.Y_AXIS,
                            AlgorithmBase.NO_CANCEL);

                        flip.setActiveImage(false);
                        flip.run();
                    }
                    else if (imageOri[1] ==
                             FileInfoBase.oppositeOrient(labelOri[1])) {
                        if (imageOri[0] == labelOri[0]) {
                            // Flip vertically
                            Preferences.debug("Flip vertically\n");
                            FileMinc.flipTopBottom(labelImage2);
                        }
                        else {
                            // do two z+ rotations
                            Preferences.debug("Rotate 180 degrees.\n");
                            AlgorithmRotate rot = new AlgorithmRotate(
                                labelImage2, AlgorithmRotate.Z_AXIS_PLUS,
                                AlgorithmBase.NO_CANCEL);

                            rot.setActiveImage(false);
                            rot.run();
                            labelImage2 = rot.returnImage();
                            rot = new AlgorithmRotate(labelImage2,
                    ...
                    else if (imageOri[1] == labelOri[0]) {
                        // Z_AXIS_PLUS
                        Preferences.debug(
                            "Rotate about the Z axis +90 degrees.\n");
                        AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                            AlgorithmRotate.Z_AXIS_PLUS,
                            AlgorithmBase.NO_CANCEL);
                    ...
                    else { // imageOri[1] == oppositeOrient(labelOri[0])
                        // Z_AXIS_MINUS
                        Preferences.debug(
                            "Rotate about the Z axis -90 degrees.\n");
                        AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                ...
                else if (imageOri[2] == FileInfoBase.oppositeOrient(labelOri[2])) {
                    // do an inverse slice order
                    Preferences.debug("Invert slice order.\n");
                    AlgorithmInverseOrder inv = new AlgorithmInverseOrder(
                        labelImage2, AlgorithmBase.NO_CANCEL);

                    inv.setActiveImage(false);
                    inv.run();
                }
                else if (imageOri[2] == labelOri[0]) {
                    // Y_AXIS_MINUS
                    Preferences.debug("Rotate about the Y axis -90 degrees.\n");
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                        AlgorithmRotate.Y_AXIS_MINUS,
                        AlgorithmBase.NO_CANCEL);
                ...
                else if (imageOri[2] == FileInfoBase.oppositeOrient(labelOri[0])) {
                    // Y_AXIS_PLUS
                    Preferences.debug("Rotate about the Y axis +90 degrees.\n");
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                        AlgorithmRotate.Y_AXIS_PLUS,
                        AlgorithmBase.NO_CANCEL);
                ...
                else if (imageOri[2] == labelOri[1]) {
                    // X_AXIS_PLUS
                    Preferences.debug("Rotate about the X axis +90 degrees.\n");
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                        AlgorithmRotate.X_AXIS_PLUS,
                        AlgorithmBase.NO_CANCEL);
                ...
                else if (imageOri[2] == FileInfoBase.oppositeOrient(labelOri[1])) {
                    // X_AXIS_MINUS
                    Preferences.debug("Rotate about the X axis -90 degrees.\n");
                    AlgorithmRotate rot = new AlgorithmRotate(labelImage2,
                        AlgorithmRotate.X_AXIS_MINUS,
                        AlgorithmBase.NO_CANCEL);
                ...
                labelOri = labelImage2.getFileInfo(0).getAxisOrientation();
            }
        }

        // call special method to get the start variable values based on header's "xspace", "yspace", and "zspace"
        srcStart = ( (FileInfoMinc) srcImage.getFileInfo(0)).getStartOrient();
        labelStart = ( (FileInfoMinc) labelImage2.getFileInfo(0)).
            getStartOrient();
        // Difference in starts in "xspace", "yspace", and "zspace"
        // Difference is in mm.
        xDiff = Math.round(Math.abs(srcStart[0] - labelStart[0]));
        yDiff = Math.round(Math.abs(srcStart[1] - labelStart[1]));
        zDiff = Math.round(Math.abs(srcStart[2] - labelStart[2]));

        // yDiff must be adjusted because MINC's origin is the lower left-hand corner and ours is the upper left-hand corner
        yDiff = (int) ( (srcImage.getExtents()[1] - 1) *
                       srcImage.getFileInfo(0).getResolutions()[1] - yDiff);

        // adjust again because MINC counts positively from the lower and we count positively from the upper
        yDiff = yDiff -
            (int) ( (labelImage2.getExtents()[1] - 1) *
                   labelImage2.getFileInfo(0).getResolutions()[1]);

        float[] labelBuffer = new float[labelImage2.getSize()];

        labelImage2.exportData(0, labelImage2.getSize(), labelBuffer);
        int rowLength = srcImage.getExtents()[0];

        Arrays.fill(destBuffer, 0.0f);
        // xDiff, yDiff, and zDiff are all in mm space.
        xDiff = (int) (xDiff / labelImage2.getFileInfo(0).getResolutions()[0]);
        yDiff = (int) (yDiff / labelImage2.getFileInfo(0).getResolutions()[1]);
        zDiff = (int) (zDiff / labelImage2.getFileInfo(0).getResolutions()[2]);
        // then just create an image the same size as the source image, and put the label image in at the appropriate places
        Preferences.debug("After transform, xDiff = " + xDiff + " yDiff = " +
                          yDiff + " zDiff = " + zDiff + "\n");
        int index = 0;

        // for the slices of the label image
        for (int i = zDiff; i < (zDiff + labelImage2.getExtents()[2]); i++) {
            // rows of label image
            for (int j = yDiff; j < (yDiff + labelImage2.getExtents()[1]); j++) {
                // columns of label image
                for (int k = xDiff; k < (xDiff + labelImage2.getExtents()[0]);
                     k++) {
                    if (labelBuffer[index] > 0) {
                        destBuffer[i * bufferSize + j * rowLength +
                            k] = labelBuffer[index];
                    }
                    index++;
                }
        ...
        destImage.importData(0, destBuffer, false);
        for (int i = 0; i < destImage.getExtents()[2]; i++) {
            FileInfoBase fileInfo1 = (FileInfoBase) labelImage2.getFileInfo(0).
                clone();

            fileInfo1.setOrigin(labelImage2.getFileInfo(0).getOriginAtSlice(i));
            fileInfo1.setExtents(destImage.getExtents());
            destImage.setFileInfo(fileInfo1, i);
        }
        labelImage.disposeLocal();
        labelImage = null;
        if (labelImage2 != null) {
            labelImage2.disposeLocal();
        }
        labelImage2 = null;
     ...
     *   B0 to A1 indicates changing x to y; B0 to A2 indicates a change from x to z.
    ...
    private boolean reorderAfni(ModelImage image, int[] axisA, int[] axisB) {
        ...
        int tDimB;
        float lowXmmB = 0, lowYmmB = 0, lowZmmB = 0, highXmmB = 0, highYmmB = 0,
            highZmmB = 0;
        ...
        try {
            ...
            MipavUtil.displayError(
                "Error on image.exportData(0,xyztSize,imgBuffer)");
            progressBar.dispose();
            if (image != null) {
                image.disposeLocal();
            }
        ...
        try {
            ...
            System.gc();
            MipavUtil.displayError(
                "ViewJFrameBase: Out of memory on image.changeExtents(newExtents)");
            progressBar.dispose();
            if (image != null) {
                image.disposeLocal();
            }
        ...
        for (i = 0; i < newZDim * tDimB; i++) {
            ...
            ( (FileInfoAfni) newFileInfo[i]).setAFNIViewType( ( (FileInfoAfni)
                fileInfo).getAFNIViewType());
            ( (FileInfoAfni) newFileInfo[i]).setAFNITypeString( ( (FileInfoAfni)
                fileInfo).getAFNITypeString());
        ...
        for (t = 0; t < tDimB; t++) {
            for (x = 0; x < xDimB; x++) {
                progressBar.updateValue(Math.round(100 * (x + t * xDimB) /
                    ( (tDimB - 1) * xDimB + xDimB - 1)), true);
                if (xtoy) {
                    newY = x;
                }
                else if (xtoyInvert) {
                ...
                for (y = 0; y < yDimB; y++) {
                    ...
                    for (z = 0; z < zDimB; z++) {
                        ...
                        imgBuffer2[newX + newXDim * newY + newSliceSize * newZ +
                            newVolSize * t] = imgBuffer[x + xDimB * y +
                            sliceSize * z + volSize * t];
                    } // for (z = 0; z < zDimB; z++)
                } // for (y = 0; y < yDimB; y++)
            } // for (x = 0; x < xDimB; x++)
        ...
        try {
            ...
            MipavUtil.displayError(
                "Error on image.importData(0,imgBuffer2,true)");
            progressBar.dispose();
            if (image != null) {
                image.disposeLocal();
            }
    ...
    private int setImageBAfni(ModelImage imageA, ModelImage image) {
        ...
        float lowXmmA = 0, lowYmmA = 0, lowZmmA = 0, highXmmA = 0, highYmmA = 0,
            highZmmA = 0;
        float lowXmmB = 0, lowYmmB = 0, lowZmmB = 0, highXmmB = 0, highYmmB = 0,
            highZmmB = 0;
                   ...
                   && ( ( (AFNITypeStringA == FileInfoAfni.HEAD_ANAT_TYPE)
                       ...
                       || ( (AFNITypeStringA == FileInfoAfni.GEN_ANAT_TYPE)
                           && ( (AFNITypeStringB == FileInfoAfni.GEN_FUNC_TYPE)
                               ...
                               ( (AFNITypeStringB == FileInfoAfni.GEN_ANAT_TYPE)
                                && (image.getNDims() == 4))))))
                 || ( (AFNIViewTypeA == FileInfoAfni.AFNI_TLRC)
                     && (imageA.getNDims() == 3)
                     && (AFNIViewTypeB == FileInfoAfni.AFNI_ORIG)
                     && ( ( (AFNITypeStringA
                             == FileInfoAfni.HEAD_ANAT_TYPE)
                           && ( (AFNITypeStringB
                                 == FileInfoAfni.HEAD_FUNC_TYPE)
                               || ( (AFNITypeStringB
                                     == FileInfoAfni.HEAD_ANAT_TYPE)
                                   && (image.getNDims()
                                       == 4))))
                         || ( (AFNITypeStringA
                               == FileInfoAfni.GEN_ANAT_TYPE)
                             && ( (AFNITypeStringB
                                   == FileInfoAfni.GEN_FUNC_TYPE)
                                 || ( (AFNITypeStringB
                                       == FileInfoAfni.GEN_ANAT_TYPE)
                                     && (image.getNDims()
                                         == 4))))))) {
        ...
        Preferences.debug("lowXmmA = " + lowXmmA + " lowYmmA = " + lowYmmA +
                          " lowZmmA = " + lowZmmA + "\n");
        Preferences.debug("highXmmA = " + highXmmA + " highYmmA = " + highYmmA +
                          " highZmmA = " + highZmmA + "\n");
        Preferences.debug("lowXmmB = " + lowXmmB + " lowYmmB = " + lowYmmB +
                          " lowZmmB = " + lowZmmB + "\n");
        Preferences.debug("highXmmB = " + highXmmB + " highYmmB = " + highYmmB +
                          " highZmmB = " + highZmmB + "\n");
        if ( (AFNITypeStringB == FileInfoAfni.HEAD_FUNC_TYPE) ||
            (AFNITypeStringB == FileInfoAfni.GEN_FUNC_TYPE)) {
            doNN = true;
        }
        else {
        ...
        try {
            ...
            MipavUtil.displayError(
                "ViewJFrameBase: IOException error on exportData");
            ...
            System.gc();
            MipavUtil.displayError(
                "ViewJFrameBase: Out of memory on new image buffer.");
            afniProgressBar.dispose();
            return -1;
        }
        ...
        try {
            image = new ModelImage(imageType, newExtents,
                                   imageBInfo.getFileName(), imageUserInterface);
        }
        catch (OutOfMemoryError e) {
            System.gc();
            MipavUtil.displayError(
                "ViewJFrameBase: Out of memory on new ModelImage");
            afniProgressBar.dispose();
            return -1;
        }
        ...
        if ( (AFNIViewTypeA == FileInfoAfni.AFNI_ORIG) &&
            ...
            afniProgressBar = new ViewJProgressBar(image.getFileInfo()[0].
                getFileName(),
                "Transforming to match AFNI anatomical image ...",
            ...
            transformAFNI(image, imgBuffer, xResB, yResB, zResB, xDimB, yDimB,
                          zDimB, tDimB, lowXmmB, lowYmmB, lowZmmB,
                          highXmmB, highYmmB, highZmmB, planeGap, gapArray,
                          doNN, xResA, yResA, zResA, xDimA, yDimA, zDimA,
                          lowXmmA, lowYmmA, lowZmmA, highXmmA, highYmmA,
                          highZmmA);
            image.calcMinMax();
            minimum = image.getMin();
            maximum = image.getMax();
            ...
            if (planeGap == 0) {
                ...
                for (int i = 0; i < gapArray.length; i++) {
                    for (int x = 0; x < xDimA; x++) {
                        if ( (x * scaleX >= gapArray[i]) &&
                            (x * scaleX < (gapArray[i] + 1)) &&
                            (!found[x])) {
                ...
                for (int i = 0; i < gapArray.length; i++) {
                    for (int y = 0; y < yDimA; y++) {
                        if ( (y * scaleY >= gapArray[i]) &&
                            (y * scaleY < (gapArray[i] + 1)) &&
                            (!found[y])) {
                ...
                for (int i = 0; i < gapArray.length; i++) {
                    for (int z = 0; z < zDimA; z++) {
                        if ( (z * scaleZ >= gapArray[i]) &&
                            (z * scaleZ < (gapArray[i] + 1)) &&
                            (!found[z])) {
            ...
            afniProgressBar = new ViewJProgressBar(image.getFileInfo()[0].
                getFileName(),
                "Transforming to match original anatomical image ...",
                0, 100, false, null, null);
            afniProgressBar.setLocation(xScreen / 2, yScreen / 2);
            afniProgressBar.setVisible(true);
            afniProgressBar.updateValue(0, true);
            ...
            try {
                image = new ModelImage(imageType, newExtents,
                                       imageBInfo.getFileName(),
                                       imageUserInterface);
            }
            catch (OutOfMemoryError e) {
                System.gc();
                MipavUtil.displayError(
                    "ViewJFrameBase: Out of memory on new ModelImage for ACPC rotation");
                afniProgressBar.dispose();
                return -1;
            }
            ...
            // image rr
            double Tr03 = rr.x - TalairachCenter.x * M[0][0] -
                TalairachCenter.y * M[0][1] - TalairachCenter.z * M[0][2];
            double Tr13 = rr.y - TalairachCenter.x * M[1][0] -
                TalairachCenter.y * M[1][1] - TalairachCenter.z * M[1][2];
            double Tr23 = rr.z - TalairachCenter.x * M[2][0] -
                TalairachCenter.y * M[2][1] - TalairachCenter.z * M[2][2];
            ...
            transformACPC(image, imgBuffer, xfrm.getMatrix(), xResB, yResB,
                          zResB, xDimB, yDimB, zDimB, tDimB, planeGap,
                          gapArray, doNN, xResA, yResA, zResA, xDimA, yDimA,
                          zDimA);
            image.calcMinMax();
            minimum = image.getMin();
            maximum = image.getMax();
            ...
            try {
                ...
                System.gc();
                MipavUtil.displayError(
                    "ViewJFrameBase: Out of memory on new ModelImage for Talairach conversion");
                afniProgressBar.dispose();
                return -1;
            }
            ...
            for (int i = 0; i < 12; i++) {
                ...
                double[][] M = xfrm.getMatrix();
                double Tr03 = rrArray[i].x - TalairachCenter.x * M[0][0] -
                    TalairachCenter.y * M[0][1]
                    - TalairachCenter.z * M[0][2];
                double Tr13 = rrArray[i].y - TalairachCenter.x * M[1][0] -
                    TalairachCenter.y * M[1][1]
                    - TalairachCenter.z * M[1][2];
                double Tr23 = rrArray[i].z - TalairachCenter.x * M[2][0] -
                    TalairachCenter.y * M[2][1]
                ...
                xfrm.setTranslate(Tx, Ty, Tz);
                transformTalairach(image, imgBuffer, xfrm.getMatrix(), xResB,
                                   yResB, zResB, xDimB, yDimB, zDimB, tDimB,
                                   planeGap, gapArray, doNN, xResA, yResA,
                                   zResA, xDimA, yDimA, zDimA, botX[i], botY[i],
                                   botZ[i],
                                   topX[i], topY[i], topZ[i]);
            } // for (i = 0; i < 12; i++)
