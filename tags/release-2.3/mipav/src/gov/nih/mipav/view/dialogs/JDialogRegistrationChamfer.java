...
*		Reference image does not change.  Match image is
*   	the image that gets transformed until it is
*   	registered to the base image.
*   	User selects base image using dialog.
...
public class JDialogRegistrationChamfer extends JDialogBase implements AlgorithmInterface {
	...
	*/
	private void init() {
		...
		gbc.gridx = 0; gbc.gridy = 0; gbc.gridheight = 1; gbc.gridwidth = 1; gbc.anchor = gbc.WEST;
	...
	*	@return <code>true</code> if successful in setting variables, <code>false</code> if not and
	*			should cancel algorithm.
	*/
    ...
    /**
    *	Calls the algorithm, assuming the variables are already set up.  Runs
    *	algorithm in separate thread.
    */
