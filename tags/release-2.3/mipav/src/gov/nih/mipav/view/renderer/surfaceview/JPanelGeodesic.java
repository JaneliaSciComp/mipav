 ...
 * <p>Description: Geodesic drawing interface. Drawing the Geodesic curve on the
 * surfaces. Depending on the type of surface displayed: either the
 * SurfaceRender or the FlythruRenderer. </p>
 *
 * This file also includes the interface for cutting the mesh along the
 * geodesic curve. The mesh can be cut along an open curve (which replaces the
 * original mesh in the scene graph) or the mesh can be cut along a closed
 * curve (which replaces the original mesh with two or more new meshes in the
 * scene graph.
 *
 * The interface for drawing geodesics with a "livewire" interface. When
 * livewire mode is selected, by checking the "Livewire Mode" checkbox, then
 * when the user selects points along the geodesic curve with the mouse, the
 * Dijkstra's version of the geodesic is calculated and displayed as the mouse
 * moves.
 *
 * The user may display the geodesic in one of three ways: (1) the smoothed
 * geodesic curve, which is based on a re-triangulation of the surface along
 * Dijkstra's path and a smoothed version of Dijkstra's path. (2) Dijkstra's
 * path along the original mesh vertices and triangle edges. (3) The
 * straight-line distance between points -- Euclidian distance. Of the three
 * display modes the Euclidian path is the only path *NOT* constrained to lie
 * on the surface of the triangle mesh.
 *
 * Distances for all three path types, for the total path lenths and the most
 * recent point-pairs are displayed in the interface as well.
 *
...
public class JPanelGeodesic extends JPanelRendererBase {
    ...
    /*  Geodesic drawing interface */
    /* Toggle button is down while points are added to the curve:  */
    ...
    /* Radio buttons for displaying the smoothed geodesic, dijkstra's path, or
     * the euclidian path: */
    ...
    /** Labels to indicate the path length between the picked points, in
     * Euclidian distance, geodesic smoothed distance, and Dijkstra's path
     * mesh distance. Two labels each, one for the distances between the last
     * two points picked, and one for the total length of the curve, from
    ...
    public JPanelGeodesic(RenderViewBase parent ) {
        ...
        surfaceGeodesic.
            setPickCanvas( surRender.getSurfaceDialog().getPickCanvas() );
        surfaceGeodesic.
            setGeodesicGroup( surRender.getSurfaceDialog().getGeodesicGroup() );
    ...
    public void init() {
        ...
        m_kDrawGeodesicButton =
            new JToggleButton( MipavUtil.getIcon( "drawgeodesic.gif" ) );
        ...
        m_kFinishOpenGeodesicButton =
            new JButton( MipavUtil.getIcon( "finishopen.gif" ) );
        ...
        m_kFinishClosedGeodesicButton =
            new JButton( MipavUtil.getIcon( "finishclose.gif" ) );
        ...
        m_kClearLastGeodesicButton =
            new JButton( MipavUtil.getIcon( "clearpoint.gif" ) );
        ...
        m_kClearAllGeodesicButton =
            new JButton( MipavUtil.getIcon( "clearcurves.gif" ) );
        ...
        m_kGeodesicToggleWireframe =
            new JToggleButton( MipavUtil.getIcon( "wireframe.gif" ) );
        ...
        kGBC.gridx = 0; kGBC.gridy = 0;
        panel2.add(viewToolBar, kGBC);
        kGBC.gridx = 0; kGBC.gridy = 1;
        panel2.add(kDisplayToolBar, kGBC);
        kGBC.gridx = 0; kGBC.gridy = 2;
        ...
        m_kGeodesicSmoothDistance =
            new JLabel( "Geodesic Smoothed:" );
        m_kGeodesicSmoothDistanceValueLast =
            new JLabel( "Last two points picked: " + 0 );
     ...
     * @return boolean <code>true</code> Geodesic drawing enabled,
     * <code>false</code> Geodesic disable.
     */
    ...
    /**
     * Toggles between wireframe and filled polygon drawing for the
     * surfaces. Useful for testing the Geodesic curves:
     */
    ...
    /**
     * Toggles between live wire mode and point and click mode for drawing
     * geodesics on the surfaces. When live wire is active, then the use
     * clicks to add the first point in a curve and then moves the mouse to
     * see Dijkstra's path drawn between that point and the Mesh vertex that
     * is nearest the mouse. When the user clicks again the point is drawn,
     * and the displaay is between the last point clicked and the current
     * mouse point.
     */
    ...
    /**
     * Causes the Geodesic class to switch between displaying the Smoothed
     * Geodesic, Dijkst'ra path along the mesh, or the straight-line Euclidian
     * path between the selected endpoints. Both the Geodesic and Dijkstr'a
     * paths are constrained to lie on the surface of the mesh, whereas the
     * straight-line distance may penetrate the mesh or be through space.
    ...
    /**
     * finishGeodesic, called when the "Finish Closed" or "Finish Open"
     * buttons are pressed, finish the current geodesic polyline.
     * @param boolean bOpen when true leaves the curve open, if false, then
     * closes the curve by connecting the last and first points.
     */
    ...
    /**
     * Enables picking points and drawing the Geodesic curve on the
     * surfaces. Depending on the type of surface displayed: either the
     * SurfaceRender or the FlythruRenderer.
     *
     * For the Geodesic object to perform picking and drawing the pickCanvas,
     * GeodesicGroup, and triangle mesh (Surface) objects must be defined,
     * either through the constructor, or as shown here, by the individual
     * access functions.
     */
     ...
     *  @param bAll, when true deletes all geodesic curves drawn on the
     *  surfaces, when false, deletes the last point drawn
     */
    public void clearGeodesic( boolean bAll ) {
        ...
        m_kEuclidianDistanceValueLast.setText( "Last two points picked: " +
                                               m_fEuclidianCurrent );
        m_kEuclidianDistanceValueTotal.setText( "Curve Total: " +
                                                m_fEuclidianTotal );
        m_kGeodesicSmoothDistanceValueLast.setText( "Last two points picked: " +
                                                    m_fGeodesicSmoothCurrent );
        m_kGeodesicSmoothDistanceValueTotal.setText( "Curve Total: " +
                                                     m_fGeodesicSmoothTotal );
        m_kDijkstraDistanceValueLast.setText( "Last two points picked: " +
                                                  m_fDijkstraCurrent );
        m_kDijkstraDistanceValueTotal.setText( "Curve Total: " +
                                                   m_fDijkstraTotal );
    ...
    /**
     * Calls the Geodesic cut function to cut the triangle mesh along the
     * geodesic curves.
     */
    ...
    /**
     * Displays the Euclidian distance between the last two points picked in
     * the Geodesic class, as well as the running total for the current curve.
    ...
    /**
     * Displays the Geodesic (smoothed) distance between the last two points
     * picked in the Geodesic class, as well as the running total for the
     * current curve.
    ...
    /**
     * Displays the Geodesic (dijkstra's along the mesh) distance between the
     * last two points picked in the Geodesic class, as well as the running
    ...
    /**
     * When a new line segment is added to the geodesic curve, the Geodesic
     * object enables removing the last point added.
    ...
    /**
     * The Geodesic object enables cutting the mesh when the line segments are
     * finished, either finished open or finished closed.
    ...
    /**
     * Since geodesic line only apply to the texture render surface, all the
     * buttons are enabled only if the suface vector size is greater than 0.
     * The method is called from the surfacePanel when add or remove surfaces.
     * @param flag boolean
     */
