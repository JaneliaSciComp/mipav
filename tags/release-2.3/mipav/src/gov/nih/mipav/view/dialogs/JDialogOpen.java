          ...
          if (userInterface.isScriptRecording()) {
              ...
              if (displayLoc == NEW) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.
                                                         ...
                                                         regionFlag + " " + itersE + " " + itersD + " " + kernel +
                                                         " " + kernelSize + " " + do25D + "\n");
              }
              else {
                  userInterface.getScriptDialog().append(userInterface.
    ...
    if (image.getNDims() == 2) { // source image is 2D
      ...
      if (displayLoc == NEW) {
        try {
          ...
          // Make algorithm
          openAlgo2D = new AlgorithmMorphology2D(resultImage, kernel,
                                                 kernelSize,
                                                 AlgorithmMorphology2D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
          ...
          MipavUtil.displayError(
              "Dialog open: unable to allocate enough memory");
          if (resultImage != null) {
        ...
        try {
          ...
          openAlgo2D = new AlgorithmMorphology2D(image, kernel, kernelSize,
                                                 AlgorithmMorphology2D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
      ...
      if (displayLoc == NEW) {
        try {
          resultImage = (ModelImage) image.clone();
          // Make algorithm
          openAlgo3D = new AlgorithmMorphology3D(resultImage, kernel,
                                                 kernelSize,
                                                 AlgorithmMorphology3D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
        ...
        try {
          // Make algorithm
          openAlgo3D = new AlgorithmMorphology3D(image, kernel, kernelSize,
                                                 AlgorithmMorphology3D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
      ...
      if (displayLoc == NEW) {
        try {
          resultImage = (ModelImage) image.clone();
          // Make algorithm
          openAlgo25D = new AlgorithmMorphology25D(resultImage, kernel,
                                                 kernelSize,
                                                 AlgorithmMorphology25D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
        ...
        try {
          // No need to make new image space because the user has choosen to replace the source image
          // Make the algorithm class
          openAlgo25D = new AlgorithmMorphology25D(image, kernel, kernelSize,
                                                 AlgorithmMorphology25D.OPEN,
                                                 itersD, itersE, 0, 0,
                                                 regionFlag);
          if (regionFlag == false)
