...
/**
*   panel contains 3 fields: 2 comboboxes (day, month), 1 text field (year),
*   panel handles proper day/month and leap-year input
*
...
public class JPanelEditDate extends JPanelEdit {
    ...
    /**
    *   Builds the panel with 3 fields.  It is necessary to validate these fields
    *   before accepting the values.
    *   @param initialText the text which will fill the text field whn begun--
    *                       date string will be in mm/dd/yyyy format
    */
    public JPanelEditDate (String initialText, boolean xml) {
        ...
        yearTextField.addKeyListener(new KeyAdapter() {  // make the year field not accept letters
            public void keyTyped(KeyEvent evt) {
                ...
                if (((ch < '0') || (ch > '9')) && (ch != KeyEvent.VK_DELETE) && (ch != KeyEvent.VK_BACK_SPACE) && (ch != KeyEvent.VK_ENTER)) {   // key is not a digit
                    evt.consume();
                }
                else {
                    try {
    ...
    *   In fact, so that it is not necessary to explicitly verify the value.
    */
    ...
    /**
    *   concatonates the date as MM/DD/YYYY; padding any
    *   years < 1000 with leading zeros.
    */
    ...
    */
    ...
    /** Because DICOM requires 4-digit years, we must
    *   check the year field.  If the year-text has
    *   fewer or more than 4 digits, the field must be fixed.
    *   Any year with 4 digits is okay.
    *   <p>
    *   If there is an error, the errorString is set to
    *   describe the problem and errorComponent is set
    *   to point to the JComponent that had an illegal
    *   value.  This method then returns <code>false
    *   </code>.  When processed normally, the caller
    *   finds a <code>true</code>, and the errorString
    *   and errorComponent are set to <code>null</code>
    ...
    */
    ...
    /** sets the panel use the restriction that it
    *   is to allow no dates after today.
    *   @param permission to display dates in the future.
    *           <code>true</code> prevents the panel from displaying a
    *           date in the future;
    *           <code>false</code> (the default) permits selection of a
    *           date in the future.
    */
    ...
    /** sets the panel use the restriction that it
    *   is to allow no dates prior to today.
    *   @param permission to display dates in the past.
    *           <code>true</code> prevents the panel from displaying a
    *           date in the past;
    *           <code>false</code> (the default) permits selection of a
    *           date in the past.
    */
    ...
    /** accessor to find out whether
    *   the panel is set to restrict
    *   the panel-display from displaying
    *   dates in the future.
