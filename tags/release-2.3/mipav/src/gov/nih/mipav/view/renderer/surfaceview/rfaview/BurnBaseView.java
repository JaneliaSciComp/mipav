 ...
 * default burning point's geometry shape, and burning path mark
 * behavior.
 * @author  Ruida Cheng
 */
public class BurnBaseView {
     ...
     */
    public BurnBaseView( SurfaceRender _surfaceRender, JPanelProbe _probePanel,
            BranchGroup _burnRootParentBG ) {
    ...
    protected void generateSphereMesh( int iSubdivisions, boolean isImageSpace ) {
        ...
        /**
         * Volume space -- two pts distance:  Pt ( 0, 0, 0 ) and Pt ( 1, 1, 1) = squreRoot(3)
         * What's the pts distance in the image space.
         */
        float x1 = 1, y1 = 1, z1 = 1, x2 = 10, y2 = 10, z2 = 10;
        double volSpaceLength = Math.sqrt(
        ...
        double imgSpaceLength = Math.sqrt(
                (double) ( ( x2 - x1 ) * ( x2 - x1 ) + ( y2 - y1 ) * ( y2 - y1 ) + ( z2 - z1 ) * ( z2 - z1 ) ) );
        ...
        if ( isImageSpace ) {
            ...
            maskBurnVector.add(new MaskBurnAttribute(new Point3f( centerTrans.x, centerTrans.y, centerTrans.z ), new Transform3D(pos), burnType, 0));
        } else {
        ...
        // System.err.println("m_kCenter.x = " + m_kCenter.x + " m_kCenter.y = " + m_kCenter.y + " m_kCenter.z = " + m_kCenter.z);
    ...
    /*
        ...
        ModelImage newImage = new ModelImage( surfaceRender.getImageA().getType(),
                surfaceRender.getImageA().getExtents(), "Mask Image", surfaceRender.getUserInterface() );
        try {
            newImage.importData( 0, diffMask, true );
            newImage.setFileInfo( surfaceRender.getImageA().getFileInfo() );
  ...
  */
     ...
     // System.err.println("nVoxels = " + nVoxels + " diffVoxels = " + diffVoxels);
     /*
        for ( i = 0; i < maskBurnVector.size(); i++ ) {
            maskBurn = (MaskBurnAttribute)(maskBurnVector.elementAt(i));
            if ( maskBurn.burnType == BurnBase.DEFAULTBURN ) {
                reGenerateSphereMesh( 5, maskBurn.center,  maskBurn.centerTransform);
            } else {
                if ( maskBurn.burnType == BurnBase.REGULARBURN ) {
                    reGenerateEllipsoidMesh( 5, maskBurn.tipLen, true, maskBurn.center, maskBurn.centerTransform );
                } else {
                    reGenerateEllipsoidMesh( 5, maskBurn.tipLen, false, maskBurn.center, maskBurn.centerTransform );
                }
            }
            calcVoxels();
        }

        for ( i = 0; i < volumeMask.size(); i++ ) {
            if ( volumeMask.get(i) ) {
                nVoxels++;
            }
        }
        ...
        surfaceMaskVector = surfaceRender.getSurfaceDialog().getSurfaceMask();
        for ( i = 0; i < surfaceMaskVector.size(); i++ ) {
            surfaceMask.or( (BitSet)(surfaceMaskVector.elementAt(i)));
        }
        int diffVolume= 0;
         // burnMask.xor(surfaceMask);
        surfaceMask.xor(volumeMask);
        for ( i = 0; i < surfaceMask.size(); i++ ) {
    ...
    /**
     * Only used by the default burning point type to reset the image sence graph
     * when the burning sphere radius and time changes.
    ...
    /**
     * Get the burning point center coordinates.   Called by the probe control
     * panel to show the current burning point center.
     ...
     * @param parentBG  the group to attach the text to
    ...
    protected void getInsideVoxels() {
        ...
        for ( int iT = 0; iT < m_iTQuantity; iT++ ) {
            ...
            /*
             kV0.z = (float) zDim - 1f - kV0.z;
             kV1.z = (float) zDim - 1f - kV1.z;
             kV2.z = (float) zDim - 1f - kV2.z;

             kV0.y = (float) yDim - 1f - kV0.y;
             kV1.y = (float) yDim - 1f - kV1.y;
             kV2.y = (float) yDim - 1f - kV2.y;
             */
    ...
    //    System.err.println( "aka  brain = " + nVoxels );
     ...
     /*
        ModelImage newImage = new ModelImage( surfaceRender.getImageA().getType(),
                surfaceRender.getImageA().getExtents(), "Mask Image", surfaceRender.getUserInterface() );
        try {
            newImage.importData( 0, m_aiMask, true );
            newImage.setFileInfo( surfaceRender.getImageA().getFileInfo() );
        } catch ( IOException er ) {
            return;
        }
    ...
    protected void generateEllipsoidMesh( int burnType, float tipLen, int iSubdivisions, boolean isImageSpace ) {
        ...
        if ( isImageSpace ) {
            ...
            maskBurnVector.add(new MaskBurnAttribute(new Point3f( centerTrans.x, centerTrans.y, centerTrans.z ), new Transform3D(pos), burnType, tipLen));
        ...
        System.out.println(
                "m_afLength[0] = " + m_afLength[0] + " m_afLength[1] = " + m_afLength[1] + " m_afLength[2] = "
                + m_afLength[2] );
        // Notes
     ...
     * @param centerTransform burning center transform
     */
    protected void reGenerateEllipsoidMesh( int iSubdivisions, float tipLen, boolean isImageSpace, Point3f center, Transform3D centerTransform ) {
        // Compute the number of vertices, edges, and triangles for an
        // octahedron subdivided to the specified level.  The recursions are
        // V1 = V0 + E0
        // E1 = 2*E0 + 3*T0
