 ...
 */
public class JDialogAutoCorrelation extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( UI.isScriptRecording() ) {
                ...
                UI.getScriptDialog().append(
                        "AutoCorrelation " + UI.getScriptDialog().getVar( image.getImageName() ) + " " );
        ...
        if ( image.isColorImage() ) {
            ...
            if ( colorsPresent == 0 ) {
                MipavUtil.displayError
                        ( "Red, green, and blue are all single valued" );
                return;
                ...
                if ( haveRed ) {
                    limitLabel = new JLabel
                            ( "Only the red channel has more than 1 value" );
                } else if ( haveGreen ) {
                    limitLabel = new JLabel
                            ( "Only the green channel has more than 1 value" );
                } else {
                    limitLabel = new JLabel
                            ( "Only the blue channel has more than 1 value" );
                }
                ...
                if ( haveRed && haveGreen ) {
                    limitLabel = new JLabel
                            ( "Only the red and green channels have more than 1 value" );
                } else if ( haveRed && haveBlue ) {
                    limitLabel = new JLabel
                            ( "Only the red and blue channels have more than 1 value" );
                } else {
                    limitLabel = new JLabel
                            ( "Only the green and blue channels have more than 1 value" );
                }
                limitLabel.setForeground( Color.black );
                limitLabel.setFont( serif12 );
                mainPanel.add( limitLabel, gbc );
    ...
    private void callAlgorithm() {
        ...
        try {
            if ( image.isColorImage() ) {
                ...
                resultImage = new ModelImage( ModelStorageBase.FLOAT, image.getExtents(), name, image.getUserInterface() );
