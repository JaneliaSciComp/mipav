...
/**
 * A panel containing fields which allow the user to input sigmas in three dimensions
 * (of, for example, a gaussian kernel).  Also optionally normalizes the third dimension
...
public class JPanelSigmas extends JPanel implements FocusListener, ItemListener {
     ...
     * This is used to record the values in the text fields when saving dialog defaults and during script recording.
     * @return a 3 element array of un-normalized floating-point sigmas
     */
    public float[] getUnnormalized3DSigmas() {
        return new float[] {Float.parseFloat(textGaussX.getText()), Float.parseFloat(textGaussY.getText()), Float.parseFloat(textGaussZ.getText())};
    }

    /**
     * Returns the sigmas to use in the algorithm, with the z-dim normalized if the user has requested it.
     * @return a 2 sigma array if the image is 2D, 3 sigmas if the input image is 3D (with the 3rd dim optionally normalized)
     */
    public float[] getNormalizedSigmas() {
        if (srcImage.getNDims() == 2) {
            ...
            if (isResolutionCorrectionEnabled()) {
                return new float[] {Float.parseFloat(textGaussX.getText()), Float.parseFloat(textGaussY.getText()), Float.parseFloat(textGaussZ.getText()) * getCorrectionFactor()};
            } else {
                return new float[] {Float.parseFloat(textGaussX.getText()), Float.parseFloat(textGaussY.getText()), Float.parseFloat(textGaussZ.getText())};
            }
        }
