 ...
 *   TransformBSpline dialog
 *   This dialog takes a source image and uses information read in from a
...
public class JDialogTransformBSpline
    extends JDialogBase
        ...
        if (nltName.endsWith("nlt")) {
            try {
                ...
                if (image.getNDims() != nDims) {
                    MipavUtil.displayError("Error! " + image.getImageName() + " is " +
                                           image.getNDims() +
                                           "D, while parameter file nDims is " +
                                           nDims + "D");
                    in.close();
                    return false;
                }
                ...
                if ( (splineDegree < 1) || (splineDegree > 4)) {
                    MipavUtil.displayError("Error! Spline degree has an illegal value = "
                                           + splineDegree);
                    in.close();
                    return false;
                }
                ...
                if (numControlPoints < iNumControlPointsMin) {
                    MipavUtil.displayError("Error! The parameter file specifies " +
                                           numControlPoints + " control points, but "
                                           + iNumControlPointsMin + " are required");
                    in.close();
                    return false;
                }
                if (numControlPoints > iNumControlPointsMax) {
                    MipavUtil.displayError("Error! The parameter file specifies " +
                                           numControlPoints +
                                           " control points, but no more than "
                                           + iNumControlPointsMax + " are allowed");
                    in.close();
                    return false;
                }

                if (!have25D) {
                    int allDimControlPoints = (nDims == 2)
                ...
                MipavUtil.displayError("Read Error on " + nltName + "  +" +
                                       e.getMessage());
                return false;
            }
        }
    ...
    public void callAlgorithm() {
        ...
        try {
            ...
            // Make algorithm
            if (have25D) {
                algoTrans = new AlgorithmTransformBSpline(image, resolutions,
                    splineDegree, numControlPoints, controlMat25D);

            }
            else {
                algoTrans = new AlgorithmTransformBSpline(image, resolutions,
                    destExtents, splineDegree,
            ...
            MipavUtil.displayError(
                "Dialog Extract Brain : unable to allocate enough memory");
            return;
        }
    }
