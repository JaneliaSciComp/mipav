...
*   @see        AlgorithmBase
*   This plug in prints out the two axis lengths of 2D ellipses fitted to
*   the areas of contour VOIs within single slices and the three axis lengths
*   of 3D ellipses fitted to the volumes of contour VOIs extending over more
*   than 1 slice.
*   In 3D the algorithm works as follows:
*   1.) Calculate the first order centroid or center of mass of the volume
*       contained in the VOI.  Pixels are not weighted by gray scale value.
*   2.) Calculate the normalized, second order moment or tensor matrix.
*       The tensor matrix is a symmetric 3 by 3 matrix.
*       Ixx Ixy Izx
*       Ixy Iyy Iyz
*       Izx Iyz Izz
*   3.) Solve for the eigenvalues and eigenvectors of the tensor matrix.
*   4.) The ellipsoid semiaxes are proportional to the square roots of the
*       eigenvalues, so take the square roots of the eigenvalues.
*   5.) Normalize the square roots of the eigenvalues using the actual number
*       of pixels contained in the VOI.  That is,
*       If volume ellipse =
*       (4/3) * PI * sqrt(evalue[0]) * sqrt(evalue[1]) * sqrt(evalue[2])
*       Then the normalizing factor to multiply each sqrt(evalue) by =
*       (number of pixels/ volume ellipse)**(1/3)
*       The largest ellipsoid diameter =
*       2.0 * normalizing factor * sqrt(largest evalue)
*
*   Reference:
*   Object Quantification by Simon Xin Wang at
    ...
    private void calc2D(){
        ...
        if ( fileInfo[0] instanceof FileInfoDicom ) {
          try {
            if ( ( (FileInfoDicom) (fileInfo[0])).getValue("0010,0010") != null) {
              patientName = (String) ( ( (FileInfoDicom) fileInfo[0]).getValue(
                  "0010,0010"));
          ...
          try {
            if ( ( (FileInfoDicom) (fileInfo[0])).getValue("0010,0020") != null) {
              patientID = (String) ( ( (FileInfoDicom) fileInfo[0]).getValue(
                  "0010,0020"));
          ...
          try {
            if ( ( (FileInfoDicom) (fileInfo[0])).getValue("0008,0020") != null) {
              studyDate = (String) ( ( (FileInfoDicom) fileInfo[0]).getValue(
                  "0008,0020"));
          ...
          try {
            if ( ( (FileInfoDicom) (fileInfo[0])).getValue("0008,0030") != null) {
              studyTime = (String) ( ( (FileInfoDicom) fileInfo[0]).getValue(
                  "0008,0030"));
          ...
          try {
            if ( ( (FileInfoDicom) (fileInfo[0])).getValue("0008,1030") != null) {
              studyDescription = (String) ( ( (FileInfoDicom) fileInfo[0]).getValue(
                  "0008,1030"));
          ...
          if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
	        ...
	        progressBar.updateValueImmed(100*(i+1)/nVOIs);
               ...
               for (k = 0; k < nContours; k++) {
                ((VOIContour)(contours[0].elementAt(k))).secondOrderAttributes(
                  xDim, yDim, xRes, yRes, xUnits, yUnits,
                ...
                if (unitsString != null) {
                  UI.setDataText(" Axes    \t= " + majorAxis[0] + ",  " +
                                 minorAxis[0] +
                                 " " + unitsString +"\n");
                  dataString += " Axes    \t= " + majorAxis[0] +
                                 ",  " + minorAxis[0] +
                                 " " + unitsString +"\n";
                }
                else {
        ...
        for (i = 0; i < nVOIs; i++) {
          if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
               ...
               nContours = contours[0].size();
               for (k = 0; k < nContours; k++) {
                ((VOIContour)(contours[0].elementAt(k))).secondOrderAttributeslsq(
                  xRes, yRes, xUnits, yUnits,
                  pAxis, eccentricity, majorAxis, minorAxis);
                if (nContours > 1) {
                  UI.setDataText(" Element number = " + k + "\n");
                  dataString += " Element number = " + k + "\n";
        ...
        for (i = 0; i < nVOIs; i++) {
          if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
               ...
               for (z = 0; z < nSlices; z++) {
                 ...
                 for (k = 0; k < nContours; k++) {
                   ( (VOIContour) (contours[z].elementAt(k))).secondOrderAttributes(
                       xDim, yDim, xRes, yRes, xUnits, yUnits,
                       pAxis, eccentricity, majorAxis, minorAxis);
                   if (nContours > 1) {
                     UI.setDataText(" Element number = " + k + "\n");
                   ...
                   if (unitsString != null) {
                     UI.setDataText(" 2D axes    \t= " + majorAxis[0] + ",  " +
                                    minorAxis[0] +
                                    " " + unitsString + "\n");
                     dataString += " 2D axes    \t= " + majorAxis[0] +
                         ",  " + minorAxis[0] +
                         " " + unitsString + "\n";
                   }
                   else {
               ...
               if (usedSlices > 1) {
                 ...
                 for (k = 0; k < maxElement; k++) {
                   ...
                   ellipVol = (4.0/3.0)*Math.PI*eigenvalue[0]*eigenvalue[1]*
                                                eigenvalue[2];
                  // Calculate a normalizing factor with the actual volume
                  if (useRes3D) {
                    normFactor = Math.pow((xRes*yRes*zRes*volume[k]/ellipVol),
                                          (1.0/3.0));
                  }
                  else {
                  ...
                  if (maxElement > 1) {
                   ...
                   if (unitsString3D != null) {
                     UI.setDataText(" 3D axes    \t= " +
                                      (float)(2.0*eigenvalue[0]) +
                                      ",  " + (float)(2.0*eigenvalue[1]) +
                                      ",  " + (float)(2.0*eigenvalue[2]) +
                                    " " + unitsString3D + "\n");
                     dataString += " 3D axes    \t= " +
                                     (float)(2.0*eigenvalue[0]) +
                                     ",  " + (float)(2.0*eigenvalue[1]) +
        ...
        for (i = 0; i < nVOIs; i++) {
          if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
               ...
               for (z = 0; z < nSlices; z++) {
                 ...
                 for (k = 0; k < nContours; k++) {
                   ( (VOIContour) (contours[z].elementAt(k))).secondOrderAttributeslsq(
                       xRes, yRes, xUnits, yUnits,
                       pAxis, eccentricity, majorAxis, minorAxis);
                   if (nContours > 1) {
                     UI.setDataText(" Element number = " + k + "\n");
                   ...
                   if (usedSlices > 1) {
                     ...
                     for (j = 0; j < gon.npoints; j++) {
                       if (useRes3D) {
                         kVoxel = new Point3f(gon.xpoints[j]*xRes*invMax,
                                              gon.ypoints[j]*yRes*invMax,
                                              z*zRes*invMax);
                       }
                       else {
                         kVoxel = new Point3f(gon.xpoints[j]*invMax,
                                              gon.ypoints[j]*invMax,
                                              z*invMax);
                       }
