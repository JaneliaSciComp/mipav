...
public class PlugInAlgorithmNEISeg extends AlgorithmBase {
    ...
    private void calcStoreInDest() {
        ...
        for (i = 0, id = 0; i < lengthIn; i += 4, id += 4) {
            ...
            try {
                ...
                avgRed = (buffer[i + 1] + buffer[i + 1 - fxDim] + buffer[i + 1 + fxDim] + buffer[i + 1 - 4] + buffer[i + 1 + 4]) / 9.0f;
                avgGreen = (buffer[i + 2] + buffer[i + 2 - fxDim] + buffer[i + 2 + fxDim] + buffer[i + 2 - 4] + buffer[i + 2 + 4]) / 9.0f;
                avgBlue = (buffer[i + 3] + buffer[i + 3 - fxDim] + buffer[i + 3 + fxDim] + buffer[i + 3 - 4] + buffer[i + 3 + 4]) / 9.0f;
        ...
        srcImage.getUserInterface().setDataText(
                " Number of contours excluded: " + numExclude + ", included: " + numInclude + "\n");
        srcImage.getUserInterface().setDataText(" Number of punctate pixels in RGB method: " + numPixels[0] + "\n");
        srcImage.getUserInterface().setDataText(
                " Number of punctate pixels in RGB All Ratios method: " + numPixels[1] + "\n");
        srcImage.getUserInterface().setDataText(
                " Number of punctate pixels in HSB - Hue method: " + numPixels[2] + "\n");
        srcImage.getUserInterface().setDataText(
                " Number of punctate pixels in 2-out-of-3 (voting) method: " + numPixels[3] + "\n\n");
        disposeProgressBar();
        setCompleted(true);
    }
     ...
     * 
     * private boolean includePointRGB_RGRatio(float avgRed, float avgGreen, float avgBlue) {
     * 
     * float diffIncludeRatio = 1000f; float diffExcludeRatio = 1000f;
     * 
     * int includeIndex = -1; int excludeIndex = -1;
     * 
     * float tol = .1f;
     * 
     * float diffRatio = 0f;
     * 
     * float[] avgRGBRatio = new float[] { (avgRed / avgGreen), (avgBlue / avgGreen), (avgBlue / avgRed)};
     * 
     * for (int i = 0; i < excludeMeanRed.length; i++) {
     * 
     * diffRatio = Math.abs( (excludeMeanRed[i] / excludeMeanGreen[i]) - avgRGBRatio[0]);
     * 
     * if (diffRatio < diffExcludeRatio) { excludeIndex = i; diffExcludeRatio = diffRatio; }
     *  }
     * 
     * for (int i = 0; i < includeMeanRed.length; i++) {
     * 
     * diffRatio = Math.abs( (includeMeanRed[i] / includeMeanGreen[i]) - avgRGBRatio[0]);
     * 
     * if (diffRatio < diffIncludeRatio) { includeIndex = i; diffIncludeRatio = diffRatio; } }
     * 
     * if (Math.abs(diffIncludeRatio - diffExcludeRatio) < tol) { diffIncludeRatio = Math.abs(
     * (includeMeanBlue[includeIndex] / includeMeanGreen[includeIndex]) - avgRGBRatio[2]); diffExcludeRatio = Math.abs(
     * (excludeMeanBlue[excludeIndex] / excludeMeanGreen[excludeIndex]) - avgRGBRatio[2]);
     * 
     * if (Math.abs(diffIncludeRatio - diffExcludeRatio) < tol) { diffIncludeRatio = Math.abs(
     * (includeMeanBlue[includeIndex] / includeMeanRed[includeIndex]) - avgRGBRatio[1]); diffExcludeRatio = Math.abs(
     * (excludeMeanBlue[excludeIndex] / excludeMeanRed[excludeIndex]) - avgRGBRatio[1]);
     * 
     * if (diffIncludeRatio < diffExcludeRatio) { return true; } else { return false; } } else if (diffIncludeRatio <
     * diffExcludeRatio) { return true; } else { return false; } } else if (diffIncludeRatio < diffExcludeRatio) {
     * return true; } // closer to exclude else { return false; } }
     */
    /**
    ...
    private boolean includePointRGB_AllRatios(float avgRed, float avgGreen, float avgBlue) {
        ...
        for (int i = 0; i < excludeMeanRed.length; i++ ) {
            diffRatio = (Math.abs(excludeMeanRed[i] / excludeMeanGreen[i] - avgRGRatio)
                    + Math.abs(excludeMeanBlue[i] / excludeMeanGreen[i] - avgBGRatio) + Math.abs(excludeMeanBlue[i]
                    / excludeMeanRed[i] - avgBRRatio));
        ...
        for (int i = 0; i < includeMeanRed.length; i++ ) {
            diffRatio = (Math.abs(includeMeanRed[i] / includeMeanGreen[i] - avgRGRatio)
                    + Math.abs(includeMeanBlue[i] / includeMeanGreen[i] - avgBGRatio) + Math.abs(includeMeanBlue[i]
                    / includeMeanRed[i] - avgBRRatio));
     ...
     * (finding the least difference)... if VOI is include, include the pixel otherwise exclude
     * @param avgHue float average hue
     * @param avgSaturation float average saturation
     * @param avgBrightness float average brightness
     * @return boolean
