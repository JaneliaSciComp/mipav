...
/**
 * This is an implementation for a curve in 3D defined by a set of BSpline basis functions. The basis is defined by a
 * set of 3D control points and a degree. A function is provided to a BSpline-based 3D curve which best fits an input
 * set of 3D data points. Only curves based on open uniform BSplines basis functions are supported. The time values are
 * fixed to be within the [0,1] range.
 */
public class BSplineCurve3 extends IntervalLengthCurve3 {
    ...
    /**
     * Create an open uniform BSpline curve defined on the time interval of [0,1].
     *
     * @param  akCtrlPoint  Point3f[] Array of 3D control points.
     * @param  iDegree      int Degree of the BSpline to use. This must be a positive value less than the number of
     *                      control points.
     */
     ...
     *
     * @param   akPoint            Point3f[] Array of 3D points. The order of the points in the array defines a path.
     * @param   iNumControlPoints  int Number of 3D control points to use when creating the BSpline curve. The location
     *                             of the 3D control points are determined to provide a least squares best fit.
     * @param   iDegree            int Degree of the basis functions to use when creating the BSpline curve.
     *
     * @return  BSplineCurve3 New instance of the best fit BSpline curve.
     */
    public static BSplineCurve3 createApproximation(Point3f[] akPoint, int iNumControlPoints, int iDegree) {
        ...
        for (int iControl = 0; iControl < iNumControlPoints; iControl++) {
            ...
            for (int i = 0; i < iNumControlPoints; i++) {
                akControl[iControl].scaleAdd((float) kA.get(iControl, i), kB[i], akControl[iControl]);
            }
        }
     ...
     * Create a BSpline curve which best fits a set of 3D points.
     *
     * @param   akValue            float[] Array of values. The order of the points in the array defines a path.
     * @param   iNumControlPoints  int Number of control points to use when creating the BSpline curve. The location of
     *                             the control points are determined to provide a least squares best fit.
     * @param   iDegree            int Degree of the basis functions to use when creating the BSpline curve.
     *
     * @return  BSplineCurve3 New instance of the best fit BSpline curve. Even though the fit is for a single set of
     *          values, the curve that is created is for 3D set of values in which the x coordinates are used to
     *          representing the single value curve. The y and z coordinates are undefined.
     */
    public static BSplineCurve3 createApproximation(float[] akValue, int iNumControlPoints, int iDegree) {
    ...
    public float getIntervalLength(int iInterval, float fDt0, float fDt1) {
        ...
        for (int i0 = 2, iP0 = 1; i0 <= iOrder; i0++, iP0 *= 2, fH *= 0.5f) {
            ...
            for (int i2 = 1, iP2 = 4; i2 < i0; i2++, iP2 *= 4) {
                apfRom1[i2] = ((iP2 * apfRom1[i2 - 1]) - apfRom0[i2 - 1]) / (iP2 - 1);
            }
    ...
    /**
     * Called to create an array of equally spaced time intervals over the range of [0,1].
     *
     * @param   iNumControlPoints  int One less than this number is the number of intervals.
     *
     * @return  float[] Array with the the equally spaced times. The size of this array is the number of input control
     *          points.
     */
