...
public class JDialogRegistrationOAR2D
  ...
  /**
   *	Creates new dialog for user to choose type of 2D image
   ...
   *	@param im	Source image.
   */
  public JDialogRegistrationOAR2D(ViewUserInterface _UI, ModelImage im) {
    super();
    UI = _UI;
    matchImage = im;
    if (matchImage.isColorImage()) {
      ...
      if (algo.isCompleted()) {
          if (UI.isScriptRecording()) {
              ...
              if (weighted) {
                  ...
                  //check to see if the input weight image is already in the ImgTable
                  ...
                  //check to see if the ref weight image is already in the ImgTable
                  if (UI.getScriptDialog().getImgTableVar(refWeightImage.getImageName()) == null) {
                      if (UI.getScriptDialog().getActiveImgTableVar(refWeightImage.getImageName()) == null) {
                          UI.getScriptDialog().putActiveVar(refWeightImage.getImageName());
                      }
                  }

                  UI.getScriptDialog().append("RegistrationOAR2D " +
                                              UI.getScriptDialog().getVar(
                      matchImage.getImageName()) + " " +
                                              UI.getScriptDialog().getVar(refImage.
                                                                          getImageName()) + " " + weighted + " " +
                                              UI.getScriptDialog().getVar(
  ...
  private void callAlgorithm() {
    ...
    if (weighted) {
      reg2 = new AlgorithmRegOAR2D(refImage, matchImage, refWeightImage,
                                   inputWeightImage, cost, DOF, interp,
                                   rotateBegin, rotateEnd, coarseRate,
                                   fineRate, doSubsample,
                                   bracketBound, maxIterations, numMinima);
    }
    else {
      reg2 = new AlgorithmRegOAR2D(refImage, matchImage, cost, DOF, interp,
                                   rotateBegin, rotateEnd, coarseRate,
                                   fineRate, doSubsample,
                                   bracketBound, maxIterations, numMinima);
    }
       ...
       if (algorithm instanceof AlgorithmRegOAR2D) {
         if (reg2.isCompleted()) {
           if (displayTransform) {
             ...
             transform = new AlgorithmTransform(matchImage, reg2.getTransform(),
                                                interp2, xresA, yresA, xdimA,
                                                ydimA,
                                                false, false, false);
           ...
           reg2.getTransform().saveMatrix(UI.getDefaultDirectory() +
