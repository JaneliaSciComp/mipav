    ...
    implements ViewImageUpdateInterface
{
     ...
     * @param kView  the engine behind the brainsurfaceFlattener
     *               and one of the parent frames affected by the interface
     * @param kImage  the ModelImage data
     * @param kParentFrame  the parent frame for the panel, contains
     *                      the surfaceRenderer, where the brain model is displayed
     */
    ...
    private void init()
    {
        ...
        m_iGridY = 0;
        gbc.gridx = 0;        gbc.gridy = m_iGridY++;
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if ( command.equals( "Select Surface ..." ) )
        {
            ...
            chooser.
                addChoosableFileFilter( new ViewImageFileFilter( ViewImageFileFilter.SURFACE ) );
            if ( ViewUserInterface.getReference().getDefaultDirectory() != null )
            {
                chooser.
                    setCurrentDirectory( new File( ViewUserInterface.getReference().getDefaultDirectory() ) );
            }
            else
            {
                chooser.
                    setCurrentDirectory( new File( System.getProperties().getProperty( "user.dir" ) ) );
            }
            ...
            m_kSurfaceDir =
                String.valueOf( chooser.getCurrentDirectory() ) +
                File.separatorChar;
        ...
        else if ( command.equals( "Select Image ..." ) )
        {
            ...
            chooser.setMultiSelectionEnabled( true );
            chooser.
                addChoosableFileFilter( new ViewImageFileFilter( ViewImageFileFilter.TECH ) );
            if ( ViewUserInterface.getReference().getDefaultDirectory() != null )
            {
                chooser.
                    setCurrentDirectory( new File( ViewUserInterface.getReference().getDefaultDirectory() ) );
        ...
        else if ( command.equals( "NumLatChanged" ) ||
                  command.equals( "NumLonChanged" )    )
        {
            m_kLatLonLines.setSelected( true );
    ...
    private void loadingImage() {
         ...
         * ModelImage: */
        if ( (m_kImage == null) ||
             ((m_kImage != null) && (!m_kImageFile.equals(m_kImage.getImageName())))
             )
        {
    ...
    private void loadingSurface() {
        ...
        try {
            ...
            for ( int j = 0; j < iVQuantity; j++ ) {
                akVertex[j].x = ( 2.0f * ( akVertex[j].x - startLocation[0] ) /
                                  direction[0]
                                  - ( xDim - 1 ) * resols[0] )
                    / maxBox;
                akVertex[j].y = ( 2.0f * ( akVertex[j].y - startLocation[1] ) /
                                  direction[1]
                                  - ( yDim - 1 ) * resols[1] )
                    / maxBox;
                akVertex[j].z = ( 2.0f * ( akVertex[j].z - startLocation[2] ) /
                                  direction[2]
                                  - ( zDim - 1 ) * resols[2] )
                    / maxBox;
            ...
            m_kCenter = new Point3f(xSum/(float)iVQuantity,
                                    ySum/(float)iVQuantity,
                                    zSum/(float)iVQuantity);
    ...
    private void displayCorticalAnalysis( )
    {
        ...
        else
        {
            ...
            m_kLUTa.makeGrayTransferFunctions();
            m_kLUTa.makeLUT( 256 );
            m_kPanelBrainsurfaceFlattenerLUT.setLUTA( m_kLUTa );
            m_kPanelBrainsurfaceFlattenerLUT.selectLUTa( );
            mainPanel.updateUI(); */
    ...
    private void createLUTFromSurface()
    {
        ...
        m_kLUTImageA =
            new ModelImage(ModelStorageBase.FLOAT, iExtents, "temp", ViewUserInterface.getReference());
        m_kLUTImageA.addImageDisplayListener( this );
        for ( int i = 0; i < 256; i++ )
        {
            for ( int j = 0; j < 256; j++ )
            {
                m_kLUTImageA.set( i, j,
                                 fMin + (float)i/255.0f * (fMax - fMin));
            }
        }
        ...
        m_kPanelBrainsurfaceFlattenerLUT =
            new JPanelHistoLUT( m_kLUTImageA, null, m_kLUTa, null, true );
        JPanel brainsurfaceFlattenerLUTPanel = new JPanel();
        brainsurfaceFlattenerLUTPanel.
            add( m_kPanelBrainsurfaceFlattenerLUT.getMainPanel() );
