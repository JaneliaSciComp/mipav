...
public class JDialogRGBConcat
      ...
      if (algo.isCompleted()) {
          if (userInterface.isScriptRecording()) {
              ...
              userInterface.getScriptDialog().append("RGBConcat " +
                                                     ...
                                                     getVar(imageR.getImageName()) +
                                                     " "
                                                     +
                                                     userInterface.getScriptDialog().
                                                     getVar(imageG.getImageName()) +
                                                     " " +
                                                     userInterface.getScriptDialog().
      ...
      if (UI.getFrameContainingImage(img) != null) {
        if ( (imageR.getNDims() == 2) && (img.getNDims() == 2)) {
          for (j = 0, count = 0; j < 2; j++) {
            if ( (imageR.getExtents()[j] == img.getExtents()[j]) &&
                (img.isColorImage() == false)) {
              count++;
            }
          }
  ...
  private boolean setVariables() {
    String tmpStr;
    ViewUserInterface UI;
    blank = new ModelImage(ModelImage.SHORT, imageR.getExtents(),
                           makeImageName(imageR.getImageName(), ""),
                           userInterface);
  ...
  /**
   *	Once all the necessary variables are set, call the RGBConcat
  ...
  private void callAlgorithm() {
    if (displayLoc == NEW) {
      try {
        ...
        resultImage = new ModelImage(ModelImage.ARGB, imageR.getExtents(),
            ...
            "_rgb"),
                                     userInterface);
        // Make algorithm
        mathAlgo = new AlgorithmRGBConcat(imageR, imageG, imageB, resultImage,
                                          remapMode);
        ...
        MipavUtil.displayError(
            "Dialog RGB concat: unable to allocate enough memory");
        return;
      }
    } // if (displayLoc == NEW)
    else { // displayLoc == REPLACE
      try {
        ...
        // Make algorithm
        mathAlgo = new AlgorithmRGBConcat(imageR, imageG, imageB,
                                          remapMode);
        // This is very important. Adding this object as a listener allows the algorithm to
        // notify this object when it has completed of failed. See algorithm performed event.
        // This is made possible by implementing AlgorithmedPerformed interface
        ...
        System.gc();
        MipavUtil.displayError(
        "Dialog RGBConcat: unable to allocate enough memory");
        return;
      }
