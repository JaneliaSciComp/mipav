 ...
 *
 * This frame can be extended to create a wizard-like frame (see <b>ViewJFrameWizard</b> in
 * the RFAST project.  Here a a number of steps that one would probably need
 * to do to create a wizard:
 * <ul>
 *   <li>Create a number of static strings to identify the stages of the wizard
 *       (these will be used to save and retrieve the step state).</li>
 *   <li>Implement <code>initGUI()</code> to setup the wizard interface (ie - a tabbed pane or card layout).
 *       Should also setup the menubar, frame title, frame size and load the initial step state.</li>
 *   <li>Might need to override the <b>ViewJFrameImage</b> <code>updateImages()</code> methods
 *       (there have been problems with repainting controls).</li>
 *   <li>Override <code>initControls()</code> to change the toolbar controls for different images.</li>
 *   <li>Create parameter panels for actions performed within each step
 *       (see <b>JPanelLiver</b> and <b>JPanelVOISnake</b> in the RFAST project).</li>
 ...
 *   <li>Implement <code>finalize()</code> (to clean up allocated memory) and <code>close()</code>
 *       (for actions required when closing the frame).</li>
 * </ul>
 *
 * @author Evan McCreedy
 * @version 1.0 August 9, 2004
 */
abstract public class ViewJFrameMultiple extends ViewJFrameImage {
    /**
    ...
    /**
     * Table of images being used in this frame.
     * The key for each image indicates which tab it should be used for.
    ...
    /**
     * Table of LUTs to accompany the image table.
     * Stored in <b>LUTa</b>.
     */
    ...
    /**
     * Table of ComponentImages to be displayed on various frames.
     * The current component image is stored in <b>componentImage</b>.
     */
    protected Hashtable componentTable;

    /**
     * Table of the scroll panes, which hold the component images.
     * The current scroll pane is stored in <b>scrollPane</b>.
     */
    protected Hashtable scrollPaneTable;

    /**
     * Table of toolbar image controls.
     * The current controls are stored in <b>controls</b>.
     */
    ...
    /**
     * Sets the magnification of Image B. Allocates the image buffers so that it can
     ...
     * @throws OutOfMemoryError if enough memory cannot be allocated for this method
     */
    protected synchronized static final ViewJComponentEditImage initComponentImage( ViewJFrameBase frame, ModelImage img, ModelLUT imgLUT ) throws OutOfMemoryError {
        ...
        ViewJComponentEditImage imgComp = new ViewJComponentEditImage( frame, img, imgLUT, imageBuf, null, null,
            null, pixBuf, imgZoom, extents, img.getLogMagDisplay(), ViewJComponentEditImage.NA, false,
            img.getFileInfo()[0].getAxisOrientation() );
    ...
    public void saveImageCopy( ModelImage img ) {
        ...
        FileWriteOptions options = new FileWriteOptions( img.getImageName() + ".xml", userInterface.getDefaultDirectory() + File.separator, true );
    ...
    /**
     * Gets the lut of an image with a specified image type
     * (or of the last unclassified image used if no lut with that type).
     * The image corressponding to the requested lut should be accessed before the lut.
     ...
     */
    public synchronized final void setCurrentImageA( String type ) {
     ...
     * @param type  the step to load the current image from
     */
    public synchronized final void setCurrentImageB( String type ) {
     ...
     */
    public synchronized final void setCurrentLUTa( String type ) {
     ...
     * @param type  the step to load the current LUT from
     */
    public synchronized final void setCurrentLUTb( String type ) {
     ...
     */
    public synchronized final void setCurrentComponentImage( String type ) {
    ...
    /**
     * Takes the name from an image (&quot;name&quot;) and appends an underscore
     * and some other text to it (&quot;text&quot;), then returns the result
     * (&quot;imageName_text&quot;).
    ...
    public synchronized boolean saveProjectInfo() {
        ...
        while ( ie.hasMoreElements() ) {
            ...
            String imagePath = getImageA( type ).getFileInfo()[0].getFileDirectory() + getImageA( type ).getImageFileName().substring( 0, i ) + getImageA( type ).getFileInfo()[0].getFileSuffix();

            projectInfo.addImage( imagePath );
            projectInfo.getImage( imagePath ).addInfo( "imageType" );
            projectInfo.getImage( imagePath ).getInfo( "imageType" ).setDescription(
                    "determines how/where this image is used by programs that use it" );
            ...
            projectInfo.getImage( imagePath ).getInfo( "display" ).setDescription(
                    "Whether to display this image in a frame when the project is loaded." );
            ...
            projectInfo.getImage( imagePath ).getInfo( "frameType" ).setDescription(
                    "What type of frame to load this image in." );
        ...
        while ( ie.hasMoreElements() ) {
            ...
            String imagePath = getImageB( type ).getFileInfo()[0].getFileDirectory() + getImageB( type ).getImageFileName().substring( 0, i ) + getImageB( type ).getFileInfo()[0].getFileSuffix();
