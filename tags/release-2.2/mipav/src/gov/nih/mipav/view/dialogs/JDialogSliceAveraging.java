 ...
 *   The program reduces noise in 3D images by averaging together
 *   slices.  3, 5, 7, or all slices can be averaged together.
 *   Algorithms are executed in their own thread.
 *
 *       @see        AlgorithmSliceAveraging
 *
 */
public class JDialogSliceAveraging
    extends JDialogBase
          ...
          if (userInterface.isScriptRecording()) {
              ...
              if (displayLoc == NEW) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.
                                                         ...
                                                         (resultImage.getImageName()) + " " + averagingNumber + "\n");
              } else {
                  userInterface.getScriptDialog().append(userInterface.
                                                         getScriptDialog().getVar
                                                         (image.getImageName()) + " " + averagingNumber + "\n");
              }
          }
      }
  }
  ...
  */
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    else if ( (source == threeButton) || (source == fiveButton) ||
             (source == sevenButton) ||
             (source == allButton)) {
  ...
  private void callAlgorithm() {
    ...
    if (displayLoc == NEW) {
      try {
        ...
        // Make algorithm
        sAverAlgo = new AlgorithmSliceAveraging(resultImage, image,
                                                averagingNumber);
        ...
        MipavUtil.displayError(
            "Dialog Slice Averaging: unable to allocate enough memory");
        if (resultImage != null) {
