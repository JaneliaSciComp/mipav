...
/**
 * The base class renderer to support shear-warp rendering.  See the document
    ...
    extends Renderer
{
    ...
    /**
     * The top level rendering call.  This function calls beforeResampleAll,
     * resampleAll, and mapIntermediateToFinal, all virtual functions that
    ...
    /**
     * Calculate the encoding of the specified channel.  The tables calculated
     * by this function is used to skip transparent voxels during the
     * resampling phase.  This leads to a significant performance improvement
     * for data sets that have a relatively large number of transparent
     * voxels.  Each table corresponds to one of the three permutations of
     * the image indices.  The active permutation is determined by the
     * composite function based on the orientation of the bounding box of
     * the volume data.
     *
     * @param acChannel the blue channel for intensity renderers or the alpha
     *    channel for color renderers
     */
    ...
    /**
     * Calculate the encoding table for the specified permutation.  See the
     * comments for initializeEncodeSkip.  Because bilinear interpolation is
     * used, 2x2 blocks of voxels in the current slice must be analyzed for
     * transparency.  The 2x2 block is tagged as non-transparent if and only
     * if at least one of the voxels is non-transparent.
     * <p>
     * Each one of the indices is a permutation; either one of (0,1,2),
     *  (1,2,0), (2,0,1).
     *
        ...
        for (int i2 = 0; i2 < m_aiBound[k2]; i2++) {
            for (int i1 = 0; i1 < (m_aiBound[k1]-1); i1++) {
                ...
                for (int i0 = 0; i0 < (m_aiBound[k0]-1); i0++) {
                    ...
                    if (acChannel[aiCurrentI[i]] != 0
                        || acChannel[aiCurrentI[i + 1]] != 0
    ...
    /**
     * An array is maintained that keeps track of opaque intermediate pixels
     * during the resampling phase.  Whenever pixels become opaque, they can
     * be skipped if revisited later in the resampling.  As runs of pixels
     * become opaque on a scan line, skipping the run can greatly increase
     * the speed of rendering.  This function is called in resampleSingle by
     * all the renderer classes taking advantage of opacity skipping.
     *
     ...
     * @return The return value is 'true' if and only if there is another
     *    nonopaque pixel to be processed.  If 'true', then m_iI0,
     *    m_iInterIndex, and m_iPixel contain values that are needed for
     *    later calculations in resampleSingle.  The variable m_iInterIndex
     *    stores the offset into the intermediate and skip arrays for the
     *    current nonopaque pixel.  The variable m_iPixel stores the offset
     *    into the volume data for the current nonopaque voxel.  The variable
     *    m_iI0 is one of the indices for the volume data and is only needed
     *    in the MjLightingRenderer class for computing the voxel position
     *    from the permuted indices.
     */
    protected boolean skipToNonopaque(int i1, short[] asIndex) {
        while (true) {
            ...
            if (m_asSkip[m_iInterIndex] == 0) {
                ...
                m_iPixel = m_iI0 + m_aiSliceBound[0] * (i1 +
                    m_aiSliceBound[1] * m_iSlice);
                return true;
            }
    ...
    /**
     * After the compositing step in resampleSingle, the intensity (for gray
     * scale) or alpha value (for color) might become its maximum value.  In
     * this case, the intermediate pixel is opaque.  The skip array must be
     * updated so that this pixel is skipped if revisited later.
     */
    ...
    /**
     * This is a callback to be executed before resampleAll is executed.  The
     * idea is to initialize the necessary parameters needed for the shear
     * warp resampling of the volume data.  The actual shear parameters are
     * computed based on the orientation of the bounding box of the volume
     * data.  The permuted slice bounds, index set, and encoding table are
     * selected based on the input permutation.  The skip array is initialized
     * to zero since before rendering, all intermediate pixels are
     * transparent.  Derived classes may provide additional initializations
     * as needed.
     * <p>
     * Each one of the indices is a permutation; either one of (0,1,2),
     *  (1,2,0), (2,0,1).
    ...
    protected void beforeResampleAll(int k0, int k1, int k2) {
        ...
        // inverse shear warp
        float fDet = m_aafBox[0][k0] * m_aafBox[1][k1] -
            m_aafBox[0][k1] * m_aafBox[1][k0];
    ...
    /**
     * Resample all the slices for the current permuted volume data.  This is
     * a stub for derived classes.  This function must be implemented by at
     * least one class in a derivation chain in order for that class to be
     ...
     * The bilinear interpolation coefficients are computed here and vary
     * with the single, active slice.  The offset into the intermediate image
     * is also computed since this also varies with the active slice.  If
     * encoding of transparent data has occurred in the renderer constructor,
     * then the current slice of the encoding table is initialized for use
     * by resampleSingle.
     */
    ...
    /**
     * Resample a single slice of the permuted volume data.  This is a stub
     * for derived classes.  This function must be implemented in at least
     * one class in a derivation chain in order for that class to be
     * considered non-abstract.
     */
    ...
    /**
     * Map the intermediate images to the final color image.  This is a stub
     * for derived classes.  This function must be implemented in at least
     * one class in a derivation chain in order for that class to be
     * considered non-abstract.
     */
    ...
    /**
     * Convert a 'byte' that represents an unsigned 8-bit integer (uint8) to
     * a signed 32-bit integer (int32).  This is provided to allow 'byte' to
     * represent a color in [0,255], the latter range not supported by Java
     * since it has no unsigned integer types.
     *
     * @param cValue a uint8 value
     * @return the corresponding int32 value
     */
    static final protected int toInt(byte cValue) {
        return (int) (cValue & 0x0ff);
    }

    /**
     * Convert a 32-bit integer (int32) to a 'byte' that represents an
     * unsigned 8-bit integer (uint8).  This is provided to allow 'byte' to
     * represent a color in [0,255], the latter range not supported by Java
     * since it has no unsigned integer types.
     *
     ...
     */
    static final protected byte fromInt(int iValue) {
        return (byte) (iValue & 0x0ff);
    }

    /**
     * Convert a 'byte' that represents an unsigned 8-bit integer (uint8) to
     * a 32-bit float.  This is provided to allow 'byte' to represent a color
     * in [0,255], the latter range not supported by Java since it has no
     * unsigned integer types.
     *
     * @param cValue a uint8 value
     * @return the corresponding float value in [0.0f,255.0f]
     */
    static final protected float toFloat(byte cValue) {
        return (float) (cValue & 0x0ff);
    }

    /**
     * Convert a 32-bit float to a 'byte' that represents an unsigned 8-bit
     * integer (uint8).  This is provided to allow 'byte' to represent a
     * color in [0,255], the latter range not supported by Java since it has
     * no unsigned integer types.
     *
     * @param fValue a float value in [0.0f,256.0f)
     * @return the corresponding uint8 value
     */
    static final protected byte fromFloat(float fValue) {
     ...
     * axis with the specified angle.
     *
     * @param transform The transform and its matrix by which to rotate the
     *                  image.
     */
    ...
    /**
     * Compute the vertex positions for the four voxels in the current slice
     * to be bilinearly interpolated.
     *
     * @param (i0,i1,i2) the current permuted indices of the upper left
     *    voxel in the 2x2 block of voxels in the slice
     */
    ...
    // Parallel viewing model.  Camera direction is always (0,0,1).  The
    // volume image and bounding box axes, center at origin, columns are
    // direction, up, right.  The matrix m_aafM[][] is for inverse warping.
    protected int m_aiBound[ /*3*/], m_iQuantity;
    protected float m_aafBox[ /*3*/][ /*3*/];
    protected float m_aafM[ /*2*/][ /*2*/];
    ...
    protected float m_afShear[ /*2*/], m_afOffset[ /*2*/];
    protected float m_afA[ /*2*/], m_afB[ /*2*/];
    protected int m_aiSliceBound[ /*3*/];
    protected int m_iSliceQuantity, m_iSlice;

    // for clipping
    protected int m_aiClipMin[/*3*/], m_aiClipMax[/*3*/];
    protected int m_aiSliceMin[/*3*/], m_aiSliceMax[/*3*/];

    // encoding of voxels based on transparent runs
    protected short m_aaaasEncode[ /*3*/][][][];
    protected short[][][] m_aaasVolumeEncode;
    protected short[][] m_aasSliceEncode;

    // support for skipping opaque intermediate pixels
    protected short m_asSkip[ /*m_iInterQuantity*/];
    ...
    protected int m_aaiIndex[ /*3*/][ /*m_iQuantity*/];
    protected int[] m_aiCurrentI;

    // intermediate image indicating that the number of times each
    // pixels is processed
    protected int[] m_aiInterC = null;

    // Temporary quantities used in the inner loop of resampleSingle().  These
    // exist since Java does not allow you to pass references to variables
    // to allow more than one to change on return from a function call.
    ...
    // interpolation of a 2x2 block of voxels in the current slice.  The Pij
    // are vertex positions.  The MatColorij are the material colors of the
    // voxels.  The Cij are the final dynamically lit colors of the voxels.
