...
/**
 * Cortical analysis applet and viewer.  This class contains the static 'main'
 * method to launch the application.  This class is an Applet which means that
 * it can be embedded in a browser or it can be attached to a MainFrame when
               ...
               MouseMotionListener
{
    ...
    /* This node contains all of the cortical mesh related nodes */
    /* in the scene which can be manipulated as a group. */
    private TransformGroup m_kTransformGroupPoseMesh = null;

    /* This node contains all of the sphere related nodes in the */
    /* scene which can be manipulated as a group. */
    private TransformGroup m_kTransformGroupPoseSphere = null;

    /* This node contains all of the plane related nodes in the */
    /* scene which can be manipulated as a group. */
    private TransformGroup m_kTransformGroupPosePlane = null;

    /* Currently selected transform that gets modified by the */
    /* handler for the keyboard behavior. */
    private TransformGroup m_kTransformGroupActive = null;

    /* This node contains the latitude/longitude renderings for the mesh */
    /* which can be enabled/disabled. */
    private Switch m_kSwitchLinesMesh = null;

    /* This node contains the latitude/longitude renderings for the sphere */
    /* which can be enabled/disabled. */
    private Switch m_kSwitchLinesSphere = null;

    /* This node contains the latitude/longitude renderings for the plane */
    /* which can be enabled/disabled. */
    ...
    /* BranchGroups for the Meshlines, for deleting and recreating the
     * meshlines when the user changes the number of latitude and longitude
     * lines:*/
    ...
    /* Geometry for the rendering of the cortical mesh is maintained */
    /* here so that the scene can be updated when the mesh is inflated. */
    ...
    /* Flag set to indicate whether curvature colors are rendered */
    /* on the mesh surface.  If not, then average convexity colors */
    /* are rendered. */
    ...
    /**
     * A simple universe is created that contains a viewing platform.  A
     * canvas is created and attached to the simple universe and is added to
     * this instance The canvas is used for the 3D rendering.  An initially
     * empty scene graph for the application is created and assigned to the
     * universe.
     */
    public MjCorticalAnalysis( ModelImage _imageA, ModelImage _imageB,
                               ...
                               ViewJFrameVolumeView kParent )
    {
        ...
        canvas =
            new VolumeCanvas3D(SimpleUniverse.getPreferredConfiguration());
    ...
    {
        DirectionalLight kLightDr = new DirectionalLight( true,
    ...
    private static Point3f [] combine (Point3f [] akPointA,
                                       Point3f [] akPointB)
    {
        Point3f [] akPointCombined =
            new Point3f [akPointA.length+akPointB.length];
        int iIndex=0;
    ...
    private static void setColors (float fCMin, float fCMax,
                                   float [] afCArray, int iVQuantity,
                                   Color3f [] akColor)
    {
    ...
    private static GeometryArray createGeometry (Point3f [] akPoint,
                                                 Color3f [] akColor,
                                                 int [] aiConnect)
    {
        GeometryInfo kGeometryInfo =
    ...
    private static ModelTriangleMesh createTriangleMesh (Point3f [] akPoint,
                                                         Vector3f[] akNormal,
                                                         Color3f [] akColor,
                                                         int [] aiConnect)
    {
        ModelTriangleMesh kTMesh = new ModelTriangleMesh( akPoint, akNormal,
                                                          akColor, aiConnect );
    ...
    /**
     * Perform all initialization prior to executing.  Loads the sample
     * mesh surface and sets up the rendering of the scene.
     */
    public void setup( ModelTriangleMesh kTriangleMesh )
    {
        ...
        m_kCortical = new MjCorticalMesh(
                                         kTriangleMesh.getVertexCount(),
                                         kTriangleMesh.getVertexCopy(),
                                         kTriangleMesh.getIndexCount()/3,
                                         kTriangleMesh.getIndexCopy());
        ...
        /* TriangleMesh: */
        m_kTriangleMesh = createTriangleMesh( m_kCortical.getPoints(),
                                              kTriangleMesh.getNormalCopy(),
                                              akColor, aiConnect );
    ...
    /* Called on setup, when a new triangle mesh is loaded, or when the
     * puncture triangle is selected and the "recalculate conformal" button is
     * pressed by the user: */
    public void calculateConformal()
    {
        ...
        m_kGeometryArraySphere =
            createGeometry(m_kCortical.getSphereCoordinates(),
                           akColor,aiConnect);
        /* m_kShapeSphere: */
        ...
        m_kGeometryArrayPlane =
            createGeometry(m_kCortical.getCylinderCoordinates(),
                           akColor,aiConnect);
        ...
        BoundingSphere kSceneBounds =
            new BoundingSphere(sceneRootTG.getBounds());
        ...
        MouseTranslate behaviorTranslate =
            new MouseTranslate( canvas, sceneRootTG );
        ...
        universe.getLocale().replaceBranchGraph(objBehaviorBG,
                                                kNewSceneRoot);
    ...
    /* Called when the mesh is created, or when the number of latitude or
     * longitude lines is changed by the user. When the user changes the
     * number of latitude or longitude lines, the meshes are removed from the
     * scene and recreated. */
    public void setupLatLon( int iNumLat, int iNumLon )
    {
        ...
        for (int i = -(int)(iNumLat/2); i <= (int)(iNumLat/2); i++)
        {
            ...
            MjCorticalMesh.Polylines kPolylines =
                m_kCortical.getLatitude(fZNormal,fMBias,fSBias,fPBias);
            ...
            /* append the vertices */
            akPolylinePointsMesh =
                combine(akPolylinePointsMesh,kPolylines.akMVertex);
            akPolylinePointsSphere =
                combine(akPolylinePointsSphere,kPolylines.akSVertex);
            akPolylinePointsPlane =
                combine(akPolylinePointsPlane,kPolylines.akPVertex);
        }
        ...
        for (int i = 0; i <= iNumLon-1; i++)
        {
            ...
            MjCorticalMesh.Polylines kPolylines =
                m_kCortical.getLongitude(fAngle,fMBias,fSBias,fPBias);
        ...
        m_kMeshLinesBranchRoot = null;
        LineStripArray kLinesMesh =
            new LineStripArray(
                               akPolylinePointsMesh.length,
                               LineStripArray.COORDINATES,
                               aiPointsMesh);
        kLinesMesh.setCoordinates(0,akPolylinePointsMesh);
        Shape3D kShapeLinesMesh =
            new Shape3D(kLinesMesh,kAppearanceLines);
        ...
        LineStripArray kLinesSphere =
            new LineStripArray(
                               akPolylinePointsSphere.length,
                               LineStripArray.COORDINATES,
                               aiPointsSphere);
        kLinesSphere.setCoordinates(0,akPolylinePointsSphere);
        Shape3D kShapeLinesSphere =
            new Shape3D(kLinesSphere,kAppearanceLines);
        ...
        LineStripArray kLinesPlane =
            new LineStripArray(
                               akPolylinePointsPlane.length,
                               LineStripArray.COORDINATES,
                               aiPointsPlane);
        kLinesPlane.setCoordinates(0,akPolylinePointsPlane);
        Shape3D kShapeLinesPlane =
            new Shape3D(kLinesPlane,kAppearanceLines);
    ...
    /**
     *  Gets the scene state in terms of slices visible, which slice number,
     *  etc.
     ...
     *   @param value amount [0,100] that is the percentage of Image A to be
    ...
    /* For the plane, all vertices have z-value set to zero. Because the mesh
     * is closed, even for the plane, the edge triangles wrap around the back
     * of the mesh, and are co-planar with the forward-facing triangles (the
     * ones that display the data) -- when picking is done, the pickClosest
     * sometimes returns the back-facing triangle. This function returns the
     * correct, visible triangle for the plane: */
    private float closestPlanePointIndex( Point3f kStart, int[] aiIndex )
    {
        ...
        {
            fDistance =
                kStart.distanceSquared( akPlanePoints[ aiIndex[i] ] );
            if ( fDistance < fMinDistance )
            {
                fMinDistance = fDistance;
            }
    ...
    /* Draw the user-selected point, either as a sphere on the triangle mesh,
     * or as a black triangle, depending on which picking is enabled: */
    ...
    private void drawPoint( Point3f kStart, int[] aiIndex, int iWhich )
    {
        ...
        Material kMaterial =
            new Material( m_akPickColors[m_iRunningNumPicked%MAX_POINTS],
        ...
        for ( int i = 0; i < aiIndex.length; i++ )
        {
            ...
            {
                fDistance =
                    kStart.distanceSquared( akSpherePoints[ aiIndex[i] ] );
            }
            ...
            {
                fDistance =
                    kStart.distanceSquared( akMeshPoints[ aiIndex[i] ] );
            }
            if ( fDistance < fMinDistance )
            {
                fMinDistance = fDistance;
    ...
    /* Draw the user-selected triangle as a black triangle, save the original
     * color so it can be restored: */
    ...
    {
        boolean bEnabled =
            Switch.CHILD_ALL == m_kSwitchLinesMesh.getWhichChild();
        int iWhichChild =
            bEnabled ? Switch.CHILD_NONE : Switch.CHILD_ALL;
    ...
    /* perform inflation step and then render shapes using */
    /* the average convexity colors */
    ...
    /* toggle between display of mean curvature-based colors and */
    /* average convexity-based colors */
    ...
    {
        if ( m_bCurvatureColors &&
             ( null != m_kCortical.getAvrConvexity() ) )
        {
            m_bCurvatureColors = false;
            setColorsLUT();
        }
    ...
    private void setColorsLUT()
    {
        ...
        for ( int iLIndex = 0; iLIndex < iLutSize; iLIndex++ ) {
            afXLUTa[ iLIndex ] =
                ((Point2Df)(kLUT.getTransferFunction().getPoint(iLIndex))).x;
            afYLUTa[ iLIndex ] = 255 -
                ((Point2Df)(kLUT.getTransferFunction().getPoint(iLIndex))).y;
        }
        ...
        for (int i = 0; i < iVQuantity; i++)
        {
            ...
            {
                if ( (fValue >=  afXLUTa[ iLut ]) &&
                     (fValue <= afXLUTa[ iLut + 1 ]) )
                {
                    float fSlope = 0;
                    if ( (afXLUTa[ iLut + 1] - afXLUTa[ iLut ]) != 0 )
                    {
                        fSlope = (afYLUTa[ iLut + 1 ] -
                                  afYLUTa[ iLut ]) /
                            (afXLUTa[ iLut + 1 ] - afXLUTa[ iLut ]);
                    }
                    iPixColor = (int)( afYLUTa[iLut] +
                                       fSlope*(fValue - afXLUTa[iLut]) +
                                       0.5);
                    break;
                }
            }
