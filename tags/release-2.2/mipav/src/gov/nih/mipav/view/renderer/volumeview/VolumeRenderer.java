...
/**
 * Common interface for all volume renderers.  Code common to the
 * RayCastVolumeRender and ShearWarpVolumeRender classes was factored into
 * this, the parent class on 11/14/04.
 *
 *	@author		Matthew J. McAuliffe, Ph.D.
 *	@see		ViewJFrameSurface
 */

public abstract class VolumeRenderer extends RenderViewBase
    ...
    */
    public VolumeRenderer(ModelImage _imageA, ModelImage _imageB,
     ...
     * surfaces.  Initializes the surface dialog and the mouse recorder
     * dialog, so that this original view is saved.  When the user opens these
     ...
     */
    public VolumeRenderer( ModelImage _imageA, ModelLUT _LUTa,
   ...
   public void setMaterialShininess(float value) {
     ...
     * image planes.  The surfaces that can be added would * be children of
     * the transform group.  Each image plane has a transform group associated
     * with it * and beneath that, a box shape where the texture maps are
    ...
    /* createTexturedImage is added so that the size of the texture used to
     * display the rendered image on the texture-mapped polygon can be changed
     * dynamically. Instead of recreating the scene graph, this function
     * re-initializes the texture-size dependent components of the scene graph
     * and can be accessed dynamically through the user-interface. */
    private void createTexturedImage()
    {
        ...
        Appearance kAppearance = (Appearance)
                kDefaultAppearance.cloneNodeComponent( true );
        ...
        TexCoordGeneration kTexCoordGeneration = new TexCoordGeneration( TexCoordGeneration.OBJECT_LINEAR,
                TexCoordGeneration.TEXTURE_COORDINATE_2, m_kCoordMapX, m_kCoordMapY );
        ...
        boxSliceX =
            new ViewJComponentBoxSlice( 0, yBox, zBox, ViewJComponentBoxSlice.X_SLICE );
     ...
     * @param slice Indicates image time-slice (4th dimension) to be
     * displayed.
     */
     ...
     * @param flag If <code>true</code> the image/VOIs can be modified; if
     * <code>false</code> image/VOIs can NOT be modified.
     */
    ...
    /**
     * Change the camera model.
     * @return bParallel true for a parallel camera, false for a perspective
     *   camera
     */
     ...
     * @return int[] Array of three values containing the X, Y, and Z
     ...
     * @param forceShow Forces show to reimport image and calculate java
     * image.
    ...
    /**
     *	Tells the mouse dialog that the transform has changed.  Does not use
     *	the type parameter.
    ...
    /**
     * Called by the volume surface render to update the mouse
     * zoom transform.   Set the tranform matrix to scale matrix.
    ...
    /**
     * Called by the surface volume render to update the tranlation
     * behavior.   Set the transform matrix to translate matrix.
     ...
     * rotate the virtual track ball.  The initial mouse location is recorded
     * for use by 'moveTrackBall', stored as a point (x0,y0) in [-1,1]^2.
     *
    ...
    /**
     * When a mouse button is released, the application is finishing a drag
     * operation to rotate the virtual track ball.  The final mouse location
     * is recorded for use by 'moveTrackBall', stored as a point (x1,y1) in
     * [-1,1]^2.  The virtual track ball is moved into its final position and
     * a ray trace is performed at highest resolution.
     *
     * @param kEvent the mouse event generated by a mouse release
     */
    ...
    /**
     * Override the parent resetImage method to update image volume in the MIP
     * volume renderer.
     */
    ...
    /**
     * Override the parent resetImage method to update image volume in
     * the MIP volume renderer.
     */
    ...
    /**
     *	Does nothing but must instantiate for this to be a subclass of
     *	ViewJFrameBase.
     */
    ...
    /*
     * set the MouseBehavior enabled states.
     * this is used to turn on and off the mouse behavior
    ...
    /* setMaxRenExtent changes the render target image by changing the
     * pixBufferA_XY size and by changing the render target image of the
     * componentImageXY member variable. createTexturedImage (local) is called
     * to reinitialize the size of the texture displayed on the texture-mapped
     * polygon. */
    ...
    /** Buffer that holds image A pixel data for the XY plane, used to build
     * displayable image. */
    protected int[] pixBufferA_XY;

    /** Buffer that holds image B pixel data for the XY plane, used to build
     * displayable image. */
    ...
    /* Member variables used to display the rendered image on the
     * texture-mapped polygon, stored so that they can be updated as the
     * render target image size is changed by the user-interface: */
