...
public class JDialogMipavOptions
    ...
    // ui must be set to access the list of images to set image-specfic
    // options (ie,. log mode)
    ...
    // MIPAV global options
    //  private JPanel globalChangesPanel;
    ...
    /**
     * makes the "Save dialog defaults" option line in the
     * otherPanel
    ...
    /**
     * Makes the "Perform LAX check" option line in the globalChangesPanel
     * If checked, the initial heap size and maximum heap size in the LAX
     * startup file is checked against what is in the preferences for this
     * option.
     * If unchecked, it signifies that no check should be made.
     * @param gbc the constraints used in the globalChangesPanel
     * @param gbl the layout used in the globablChangesPanel
     */
    ...
    /**
     * Makes the "Check on frame close" option line in the globalChangesPanel
     * If checked the user is required to reply to a dialog to close the frame.
     * If unchecked the frame is closed and data my be lost.
     * @param gbc the constraints used in the globalChangesPanel
     * @param gbl the layout used in the globablChangesPanel
     */
    ...
    /** makes the active-colour option line in the
     *	 globalChangesPanel, to allow user to select the colour
     *	 used to denote the active image.  Sets the colour
     *	 to either the colour in the preferences file or to the
     *	 MIPAV default.
     *	 @param gbc the constraints used in the globalChangesPanel
     *	 @param gbl the layout used in the globablChangesPanel
     */
    protected void makeActiveColorOptions(GridBagConstraints gbc, GridBagLayout gbl) {
        ...
        if (Preferences.getProperty("ActiveImageColor") == null) {
            Preferences.setProperty("ActiveImageColor", MipavUtil.makeColorString(ViewJComponentEditImage.ACTIVE_IMAGE_COLOR));
    ...
    /** makes the Apply/Close button panel, with the Apply button
     *	 on the left and the Close button on the right.
     *	 <p>
    ...
    /** when the image is available, there is an option
     *   there is an option to have processes operating on the image
     *   send logging information to the Log pane of the output window.
     *   This method would create the Log checkbox inside the MIPAV
     *   options dialog.  Note that this operation was not finished,
     *   and there is no way to ensure the image and the log
     *   checkbox are truly connected.
     */
    /*public void includeImageSettingsBoxGG() {
        //  if the user interface isn't already avail, ignore
        if (userInterface == null || userInterface.getImageFrameVector().size() == 0) {
            return;
        }
        // image specific variables
        localChangesPanel = new JPanel();
        localChangesPanel.setBorder(buildTitledBorder("Image Settings"));
            // panel gets a grid layout
        GridBagLayout gbl = new GridBagLayout();
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.anchor = GridBagConstraints.WEST;
        localChangesPanel.setLayout(gbl);
        imageChooser = new JComboBox();
        fillChooser();
        imageChooser.setFont(MipavUtil.font12);
        gbc.gridheight = GridBagConstraints.REMAINDER;
        gbc.gridwidth = 1;
        gbc.anchor = GridBagConstraints.NORTH;
        gbc.insets = new Insets(0, 0, 0, 10);
        gbl.setConstraints(imageChooser, gbc);
        localChangesPanel.add(imageChooser);
        logMode = new JCheckBox("Use Log Mode");
        logMode.setFont(MipavUtil.font12);
        gbc.gridheight = 1;
        gbc.gridwidth = 1;
        gbc.anchor = GridBagConstraints.NORTH;
        gbl.setConstraints(logMode, gbc);
        localChangesPanel.add(logMode);
        this.getContentPane().add(localChangesPanel, BorderLayout.CENTER);
        pack();
        setVisible(true);
         }*/
    ...
    //*****  ACTION LISTENER
     ...
     public void actionPerformed(ActionEvent event) {
         ...
         if (command.equalsIgnoreCase("color")) {
             ...
             colorChooser = new ViewJColorChooser(null,
                                                  "Pick Active Color",
                                                  new ActionListener() { //OKAY listener
             ...
             // "apply" sets all the preferences in the dialog and then makes itself unset-able
             // global preferences
 ...
 //            Preferences.setProperty("DEBUG", new Integer(debugLevel.getSelectedIndex()));
             ...
             if (userInterface != null) {
                 ...
                 for (i = 0; i < imageframelist.size(); i++) { // for all viewJFrames listed in the the user interface list
                     ...
                     try {
                         ...
                         if (preferredCrosshair.equalsIgnoreCase("default")) {
                             ...
                             try {
                                 ...
                                 ( (ViewJFrameImage) userInterface.getImageFrameVector().elementAt(i)).getComponentImage().
                                     setCrosshairCursor(toolkit.createCustomCursor(MipavUtil.getIcon(
                                     preferredCrosshair).getImage(), new Point(12, 12), preferredCrosshair));
                             }
                             catch (NullPointerException noIcon) {
                                 // specfied icon cannot be found.  Instead, we set default:
                                 ( (ViewJFrameImage) userInterface.getImageFrameVector().elementAt(i)).getComponentImage().
                                     setCrosshairCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
                                 Preferences.debug("JDialogMipavOptions: Crosshair icon \"" +
                                                   preferredCrosshair + "\" cannot be found.  " +
     ...
     *  numbers, backspace, period and delete-key entries.
     */
