...
/**
 * This class maintains a set of SoftwareLight instances where each
 * light can be of type ambient, directional, point, or spot.  Furthermore,
 * each light can have position and direction in model coordinates or
 * in world coordinates where a transformation can be specified which
 * converts world coordinates to model coordinates.  This class is also
 * used to apply the complete set of lighting to a "cell" in model coordinates
 * which consists of a set of coordinates, a normal vector, and a color
 * along with some other default "material" properties.
 */
public class SoftwareLightSet {
     ...
     * @param akLights SoftwareLight[] Array of software lights
     * in model coordinats.
     */
     ...
     *
     * Note: If a transformation was applied previously to convert world
     * lights to model coordinates, this transformation will have
     * to be respecified to convert these world lights to model
     * coordinates.
     *
     * @param akLights SoftwareLight[] Array of software lights
    ...
    public void setWorldLights(SoftwareLight[] akLights) {
        ...
        for ( int i = 0; i < akLights.length; i++ ) {
            ...
            // Directional light - copy direction
                ...
                SoftwareLightDirectional kLightWorldInModelSpace = (SoftwareLightDirectional) m_akLightsWorldInModelSpace[i];
    ...
    /**
     * Apply a 3x3 transformation to the software world light position
     * and direction which are in world coordinates so that they are
     * converted to model coordinates.
    ...
    public void applyWorldToModelTransform(Matrix3f kTransform) {
        ...
        for ( int i = 0; i < m_akLightsWorldPosition.length; i++ ) {
            ...
            if ( null != kPosition ) {
                ...
                kTransform.transform(
                    kPosition,
                    m_akLightsWorldPositionInModelSpace[i]);
            }
        }
        for ( int i = 0; i < m_akLightsWorldDirection.length; i++ ) {
            ...
            if ( null != kDirection ) {
                ...
                kTransform.transform(
                    kDirection,
                    m_akLightsWorldDirectionInModelSpace[i]);
            }
        }
    }

    /**
     * Apply a 3x3 transformation to the software world light position and
     * direction which are in world coordinates so that they are converted
    ...
    public void applyWorldToModelTransform(Vector3f kAxisX, Vector3f kAxisY, Vector3f kAxisZ) {
        ...
        for ( int i = 0; i < m_akLightsWorldPosition.length; i++ ) {
            ...
            if ( null != kPosition ) {
                m_kV.set(kPosition);
                m_akLightsWorldPositionInModelSpace[i].set(
                    kAxisX.dot(m_kV),
                    kAxisY.dot(m_kV),
                    kAxisZ.dot(m_kV));
            }
        ...
        for ( int i = 0; i < m_akLightsWorldDirection.length; i++ ) {
            ...
            if ( null != kDirection ) {
                m_kV.set(kDirection);
                m_akLightsWorldDirectionInModelSpace[i].set(
                    kAxisX.dot(m_kV),
                    kAxisY.dot(m_kV),
                    kAxisZ.dot(m_kV));
            }
    ...
    /**
     * Compute the "lighted" color of the cell at the specified position
     * with the specified normal vector and base color.  This is based on
     * application of all the world and model lights to this cell.
     * @param kMaterial SoftwareMaterial Contains default material properties
     * for lighting some of which may be overridden by properties specified
     * in the vertex propertyies.
     * @param kVertexProperty SoftwareVertexProperty Contains the position,
     * normal, and optional diffuse/specular colors defined for this cell.
     * @param kEyeModel Point3f Position of the eyepoint in model coordinates.
     * @return Color3f Computed "lighted" color clamped so as not to
     * exceed white.
     */
    public Color3f getCellColor(SoftwareMaterial kMaterial,
    ...
    /**
     * Array of software lights converted from world coordinates to model
     * coordinates.
     */
     ...
     * An array item may be null if that software light does not
     * have a position (e.g., ambient or directional).
     */
    private Point3f[] m_akLightsWorldPosition = new Point3f[0];

    /**
     * Array of model coordinate positions for software world lights
     * corresponding to the entries in the m_akLightsWorldPosition array.
     */
     ...
     * An array item may be null if that software light does not
     * have a direction (e.g., ambient or point).
     */
    private Vector3f[] m_akLightsWorldDirection = new Vector3f[0];

    /**
     * Array of model coordinate vectors for software world lights
     * corresponding to the entries in the m_akLightsWorldDirection array.
     */
