...
public class JDialogThresholdRGB
        ...
        if (algo.isCompleted()) {
            if (userInterface.isScriptRecording()) {
                ...
                userInterface.getScriptDialog().append(Boolean.toString(useChannel[0]) + " " +
                    Boolean.toString(useChannel[1]) + " " + Boolean.toString(useChannel[2]) + " ");


                if (useChannel[0]) {
                    userInterface.getScriptDialog().append(thresholdR[0] + " " +
                        thresholdR[1] + " " + fillValues[0] + " " );
                }
                if (useChannel[1]) {
                    userInterface.getScriptDialog().append(thresholdG[0] + " " +
                        thresholdG[1] + " " + fillValues[1] + " " );
                }
                if (useChannel[2]) {
                    userInterface.getScriptDialog().append(thresholdB[0] + " " +
                        thresholdB[1] + " " + fillValues[2] + " ");
                }
    ...
    private JPanel buildThresholdPanel(int panelNum) {
        ...
        tempStr = new String("Lower limit: ( " +
                             makeString(mins[panelNum], 2) + " - " +
                             makeString(maxs[panelNum], 2) + " )");
    ...
    /**
     *	Once all the necessary variables are set, call the threshold rgb
    ...
    private void callAlgorithm() {
        ...
        if (displayLoc == NEW) {
            try {
                ...
                // Make algorithm
                thresholdAlgoRGB = new AlgorithmThresholdDualRGB(resultImage, image, thresholdR,
                    thresholdG, thresholdB, fillValues, useChannel, regionFlag, isInverse);
            ...
            try {
                ...
                thresholdAlgoRGB = new AlgorithmThresholdDualRGB(image, thresholdR,
                    thresholdG, thresholdB, fillValues, useChannel, regionFlag, isInverse);
    ...
    //*******************************************************************
     //************************* Focus Events ****************************
      //*******************************************************************
        ...
        *   resets the necessary variables.  In this case,
        *   it also handles a bug involving when the focusGained and
        *   focusLost functions are called by turning off the ability
        *   of the text fields to call those functions.
       ...
       public void focusLost(FocusEvent event) {
           ...
           if (testParameter(text, mins[panelNum], maxs[panelNum])) {
               tempStr = new String("Upper limit: ( " +
                                    makeString(Float.parseFloat(text),3) + " - " +
                                    makeString(maxs[panelNum],3) + " ).");
               thresholdLabels2[panelNum].setText(tempStr);
               thres1 = stringConv.valueOf(text).floatValue();
               text = textThreshold2[panelNum].getText();
               thres2 = stringConv.valueOf(text).floatValue();
               if (thres2 < thres1) {
                   thres2 = (thres1 + maxs[panelNum]) / 2;
                   textThreshold2[panelNum].setText(String.valueOf(thres2));
               }
           }
           else {
               field.requestFocus();
