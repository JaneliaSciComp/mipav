...
/**
 *  Plots a surface of an image as a function as height surface.  If the image
 *	is 3D, the current slice is plotted.
 *
 ...
 *		@see ModelQuadMesh
 */
public class JDialogPlotSurface
    extends JDialogBase
  ...
  private void init() {
    ...
    fileTF.setPreferredSize(new Dimension(fileTF.getHorizontalVisibility().
                                          getMaximum() + 5,
   ...
   *	<li>OK Button		- gets the file name from the text box and the subsample and mesh parameters
   *						from their respective combo boxes and calls the algorithm</li>
    ...
    if (command.equals("Choose")) {
      try {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
          ...
          userInterface.setDefaultDirectory("" + chooser.getCurrentDirectory() +
                                            File.separatorChar);
        }
        fileTF.setPreferredSize(new Dimension(fileTF.getHorizontalVisibility().
                                              getMaximum() + 5,
  ...
  private boolean setVariables() {
    ...
    subSample = Integer.valueOf( (String) subSampleCBox.getSelectedItem()).
        intValue();
  ...
  /**
   *	Once all the necessary variables are set, call the Plot surface
   *	algorithm.
   */
  private void callAlgorithm() {
    try {
      // Make algorithm
      extractSurAlgo = new AlgorithmHeightFunction(image, subSample, fileName,
          slice, mesh);
      ...
      MipavUtil.displayError(
          "Dialog extract surface: unable to allocate enough memory");
      return;
    }
  }
