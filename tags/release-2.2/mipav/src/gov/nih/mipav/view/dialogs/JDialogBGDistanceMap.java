 ...
 */
public class JDialogBGDistanceMap extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "BGDistanceMap " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
     ...
     *	@param flag		<code>true</code> indicates the whole image is used
     *                   for a background distance map, <code>false</code> indicates a region.
     */
            ...
            if ( displayLoc == NEW ) {
                try {
                    ...
                    distanceMapAlgo2D = new AlgorithmMorphology2D( resultImage, kernel, 0,
                            AlgorithmMorphology2D.BG_DISTANCE_MAP, 0, 0, 0, 0, regionFlag );
            ...
            if ( displayLoc == NEW ) {
                try {
                    ...
                    distanceMapAlgo3D = new AlgorithmMorphology3D( resultImage, kernel, 0,
                            AlgorithmMorphology3D.BG_DISTANCE_MAP, 0, 0, 0, 0, regionFlag );
                    if ( regionFlag == false ) {
