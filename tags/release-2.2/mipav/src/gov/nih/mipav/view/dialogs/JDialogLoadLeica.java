...
public class JDialogLoadLeica
  ...
  private void init() {
    ...
    if (buildList()) {
      ...
      scrollPane = new JScrollPane(seriesList,
                                   JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
                                   JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
      mainPanel.add(scrollPane);
      mainPanel.setBorder(buildTitledBorder(
          "Available Leica series listed in header"));
  ...
  private boolean buildList() {
    ...
    try {
      ...
      while (keepGoing) {
        ...
        if (currentLine == null) {
          if (series != null && !vec.contains(series) &&
              seriesExists(series)) {
            vec.add(series);
          }
          break;
          ...
          String unitsStr = currentLine.substring(currentLine.indexOf("[") + 1,
                                                  currentLine.indexOf("]"));
          ...
          if (currentLine.indexOf("[") != -1) {
            String unitsStr = currentLine.substring(currentLine.indexOf("[") +
                1,
                currentLine.indexOf("]"));
            currentLine = currentLine.substring(currentLine.indexOf("]") + 1,
                                                currentLine.length()).trim();
    ...
    seriesList = new JList(vec) {
        ...
        public String getToolTipText(MouseEvent evt) {
            ...
            tipString += "X Dim: " + series.getExtents()[0] + ", Y Dim: " +
                series.getExtents()[1];
            ...
            tipString += ", X Res: " + series.getResolutions()[0] +
                ", Y Res: " + series.getResolutions()[1];
            if (series.getResolutions().length > 2) {
    ...
    if (directoryFile.isDirectory()) {
      ...
      File[] allFiles = directoryFile.listFiles(new FileFilter() {
        public boolean accept(File f) {
          if (f.getPath().endsWith("tif") ||
              f.getParent().endsWith("TIF")) {
            return true;
          }
          else {
            return false;
  ...
  private boolean sortLeicaFiles(LeicaSeries series) {
    ...
    if (fileNames != null) {
      LeicaFileComparator lComp = new LeicaFileComparator(series.getChannels(),
          series.getName());
  ...
  /**
   * Loads the selected Leica series into a new ViewJFrameImage
   * uses the FileTiff to read each Tiff image and construct
   * the appropriate model image based on the LeicaSeries attributes
   * which were read in from the text header
  ...
  private void loadLeica(LeicaSeries series) {
    ...
    try {
      ...
      if (series.getNumImages() == 1) {
       ...
       tempName = tempPath.substring(tempPath.lastIndexOf(File.separator) + 1,
                                     tempPath.length());
  ...
  public class LeicaFileComparator
    ...
    public int compare(Object a, Object b) {
      ...
      aEnd = aEnd.substring(aEnd.indexOf(seriesName + "_") + seriesName.length() +
                            1, aEnd.length() - 4);
      bEnd = bEnd.substring(bEnd.indexOf(seriesName + "_") + seriesName.length() +
                            1, bEnd.length() - 4);
      ...
      if (aEnd.indexOf("_") != -1) {
        ...
        channelA = Integer.parseInt(aEnd.substring(aEnd.indexOf("_") + 3,
            aEnd.length()));
        channelB = Integer.parseInt(bEnd.substring(bEnd.indexOf("_") + 3,
            bEnd.length()));
