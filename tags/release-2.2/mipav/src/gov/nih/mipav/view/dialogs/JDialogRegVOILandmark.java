...
*   Calls AlgorithmRegVOILandmark.  The user is able to control
...
*   xy resolutions (intra-plane). The user has the
...
*   VOI regions. Adapted from JDialogGradientMag
*
...
public class JDialogRegVOILandmark extends JDialogBase implements AlgorithmInterface {
    ...
    */
    private void init() {
        ...
        gbc.anchor = gbc.WEST; gbc.insets = new Insets(0,5,0,5); gbc.weightx = 1;
        optionPanel.add(labelOpt, gbc);
        gbc.gridx = 1; gbc.fill = gbc.HORIZONTAL;
        optionPanel.add(comboBoxOpt, gbc);
        gbc.gridx = 0; gbc.gridy = 1; gbc.fill = gbc.NONE;
        ...
        gbc.gridx = 0; gbc.gridy = 0; gbc.gridwidth = 1; gbc.fill = gbc.HORIZONTAL;
    ...
    */
    public ModelImage   getResultImage()    {return resultImage;}
    ...
    */
    public void actionPerformed(ActionEvent event) {
        ...
        if ( source == OKButton ) {
            ...
            try{
                Preferences.debug(minTx+","+maxTx+","+minTy+","+maxTy+","+minRz+","+maxRz+","+step+"\n");
                ...
                for(j = 0; j < nCurves; j++){
                    if ( ((VOIContour)(curves[0].elementAt(j))).isActive() ) break;
                }
                ...
                int length = (int)Math.round(((VOIContour)(curves[0].elementAt(j))).
                                        getLengthPtToPt(resolutions)/resolutions[0]);
                ...
                length = image.getVOIs().VOIAt(VOIindex).getPositionAndIntensity(0, j,
                        tmpPosition, tmpIntensity, volBuffer, xdim);
                ...
                algoRegVOILankmark = new AlgorithmRegVOILandmark(image, resultImage, sigmas, true, position, minTx,
                                                            maxTx, minTy, maxTy,minRz, maxRz, step, opt, costFunc);
    ...
    public void algorithmPerformed(AlgorithmBase algorithm) {
        ...
        if ( algorithm instanceof AlgorithmRegVOILandmark) {
            if(algoRegVOILankmark.isCompleted() == true){
                ...
                if (parentFrame != null) UI.registerFrame(parentFrame);
