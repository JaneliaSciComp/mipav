...
public class JDialogCardiology
    extends JDialogBase
    ...
    private void createSections() {
        ...
        for (int i = 0; i < numSections; i++, angle += angleInc) {
            ...
            for (int j = 0; !foundIntersection && j < (numPtsInner - 1); j++) {
                ...
                foundIntersection = VOICardiology.intersects(centerPt, secondPt,
                                                             (Point3Df) innerContour.elementAt(j),
            ...
            foundIntersection = VOICardiology.intersects(centerPt, secondPt,
                                                         ...
                                                         (Point3Df) outerContour.elementAt(0),
                                                         intersectionPt);
            for (int j = 0; !foundIntersection && j < (numPtsOuter - 1); j++) {

                foundIntersection = VOICardiology.intersects(centerPt, secondPt,
                                                             (Point3Df) outerContour.elementAt(j),
                                                             (Point3Df) outerContour.elementAt(j + 1),
                                                             intersectionPt);
            }
        ...
        for (firstPt = 0; firstPt < numPtsInner; firstPt++) {
            ...
            if ( (int) currentPt.x == xPtsInner[0] &&
                (int) currentPt.y == yPtsInner[0]) {
        ...
        for (int i = firstPt; i < numPtsInner; i++) {
            ...
            for (int j = startPt; j < numSections; j++) {
                if ( (int) currentPt.x == xPtsInner[j] &&
                    (int) currentPt.y == yPtsInner[j]) {
                    isIntersection = true;
        ...
        for (firstPt = 0; firstPt < numPtsOuter; firstPt++) {
            ...
            if ( (int) currentPt.x == xPtsOuter[0] &&
                (int) currentPt.y == yPtsOuter[0]) {
        ...
        for (int i = firstPt; i < numPtsOuter; i++) {
            ...
            //see if it is one of the intersection points (already found pt 1)
            for (int j = startPt; j < numSections; j++) {
                if ( (int) currentPt.x == xPtsOuter[j] &&
                    (int) currentPt.y == yPtsOuter[j]) {
                    isIntersection = true;
                ...
                for (int m = 0; m < numSections; m++) {
                    ...
                    if (currentVector != null) {
                        for (int k = 0; k < currentVector.size(); k++) {
                            if ( (int) currentPt.x ==
                                (int) ( (Point3Df) currentVector.elementAt(k)).x &&
                                (int) currentPt.y ==
                                (int) ( (Point3Df) currentVector.elementAt(k)).y) {
        ...
        if (image.getNDims() == 3) {
            newImage = new ModelImage(image.getType(), new int[] {dimX, dimY},
                                      image.getImageName() + "_CardioVOI", userInterface);
    ...
    /**
     * Make sure the number of sections field is okay, if so
     * then check the VOIs
    ...
    /**
     * Determine if there are THREE VOIs, Two closed (one completely inside the other)
     * and a third that is unclosed (the infarction)
     * @return boolean
     */
    private boolean checkVOIs() {
        ...
        if (image.getVOIs() != null && image.getVOIs().size() == 3) {
            ...
            if (image.getVOIs().VOIAt(innerGuess).isBinaryMaskContained(mask, image.getExtents()[0], image.getExtents()[1])) {
