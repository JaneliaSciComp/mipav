...
*
*/
public class JDialogFRET extends JDialogBase implements AlgorithmInterface, ItemListener,
                                                        WindowListener {
	...
	*/
	private void init() {
        ...
        if (image.getNDims() == 2) {
          imageSelectionPanel.setBorder(buildTitledBorder(
              "Select postbleached image"));
          ...
          imageSelectionPanel.setBorder(buildTitledBorder(
              "Image with pre and post bleaching"));
          GridBagConstraints gbc5 = new GridBagConstraints();
          gbc5.gridwidth = 1;
          gbc5.gridheight = 1;
          gbc5.anchor = gbc5.WEST;
   ...
   */
   public void actionPerformed(ActionEvent event) {
    	...
    	else if ((source == donorButton) || (source == backgroundButton) ||
                 (source == signalButton)) {
    ...
    */
           ...
           for (i = 0; i < nVOIs; i++) {
              if ((VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) ||
                  ...
                  if ((Math.abs(hue - 0.0f)) < 0.0001f) {
                      ...
                      MipavUtil.displayError("Contour VOI with illegal hue = " + hue +
                                             " is present");
                      return false;
                  }
              }
           ...
           if (image.getNDims() == 2) {
             if (VOIs.VOIAt(donorIndex).getCurves()[0].size() == 0) {
               MipavUtil.displayError(
                   "Must have donor VOI present in postbleached " +
             ...
             if (backgroundIndex >= 0) {
               if (VOIs.VOIAt(backgroundIndex).getCurves()[0].size() == 0) {
                 MipavUtil.displayError(
                     "Must have background VOI present in postbleached "
                     + postImage.getImageName());
                 return false;
               }
             } // if (backgroundIndex >= 0)
             if (signalIndex >= 0) {
               if (VOIs.VOIAt(signalIndex).getCurves()[0].size() == 0) {
                 MipavUtil.displayError(
                     "Must have signal VOI present in postbleached "
                     + postImage.getImageName());
                 return false;
               }
               ...
               if (backgroundIndex < 0) {
                 MipavUtil.displayError(
                     "Cannot use signal VOI without a background VOI");
                 return false;
               }
             } // if (signalIndex >= 0)
           } // if (image.getNDims() == 2)
             ...
             if (VOIs.VOIAt(donorIndex).getCurves()[1].size() == 0) {
               MipavUtil.displayError(
                   "Must have donor VOI present in postbleached slice of " +
                    image.getImageName());
               return false;
             }
             if (backgroundIndex >= 0) {
               if (VOIs.VOIAt(backgroundIndex).getCurves()[1].size() == 0) {
                 MipavUtil.displayError(
                     "Must have background VOI present in postbleached slice of "
                     + image.getImageName());
                 return false;
               }
             } // if (backgroundIndex >= 0)
             if (signalIndex >= 0) {
               if (VOIs.VOIAt(signalIndex).getCurves()[1].size() == 0) {
                 MipavUtil.displayError(
                     "Must have signal VOI present in postbleached slice of "
                     + image.getImageName());
                 return false;
               }
               if (backgroundIndex < 0) {
    ...
    private void callAlgorithm() {
        try {
            ...
            // Make algorithm
                fretAlgo= new AlgorithmFRETAcceptorPhotobleach(image,postImage,
