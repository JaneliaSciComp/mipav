...
/**
 *	Dialog to control the view of the 3D surface viewer and renderer.  User can
 *	switch between standard view and fly mode.  Standard view has rotating with standard
 *	mouse motions and zooming with the mouse and the alt key down.  Fly mode has zooming
 *	with the left mouse button, translation with the right mouse button, and pitch and roll
 *	with the middle mouse button.  There are also buttons on screen for Mac users.  The user
 *	can warp to different views using the warp controls and record the flight path using the
 *	mouse event recorder.
 *
 *	@author	Neva Cherniavsky
 */
public class JPanelView extends JPanelRendererBase
    ...
    private void init() {
        ...
        scroller = new JScrollPane( scrollPanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED );
     ...
     *	<ul><li>Fly - changes the view platform to fly mode, so that the left mouse button
     *				  is zoom, the right mouse button is translate, and the middle mouse
     *				  button is pitch and roll.</li>
     *		<li>Standard - changes the view platform back to the standard, with the left
     *					   mouse button grabbing the object and rotating it and the left
     *					   mouse button with the alt mask doing a zoom.</li>
    ...
    /**
     *	Listens for changes in the velocity slider.  If the user changes the
     *	velocity, it is changed within the parent class of FlyBehavior.
    ...
    /**
     *	Creates the mouse control panels.  There are four arrows for each mouse button,
    ...
    /**
     *	Sets the elements associated with "fly" behavior to be enabled or
     *	disabled, depending on <code>flag</code>.
     *	@param flag		<code>true</code> indicates that these elements should be
     *					enabled, <code>false</code> that they should be disabled.
     */
    ...
    /**
     *   Sets the pressed or unpressed icon for the button, depending on
     *   which type it is.
     *   @param source   The button that was pressed or released.
     *   @param press    <code>true</code> indicates the button was pressed, <code>false</code> that
     *                   it was released.
     */
    ...
    /**
     *	Translates the mouse event received from the mouse control buttons
     *	to its equivalent on the canvas.
     *	@param event	Original mouse event.
    ...
    /**
     *	Class used to send Standard mouse events to the canvas.  Must subclass Thread
     *	because a single <code>mousePressed</code> event on one of the mouse
     *	buttons needs to generate <code>mouseDragged</code> events on the canvas until
    ...
    /**
     *   Helper class which sets the appropriate icons for the
     *   up button.  Will use "instanceof" keyword to determine
     *   icons later.
     */
    class UpButton extends JButton {

        /**
         *   Creates new button with correct icon and
         *   given tool tip.
    ...
    /**
     *   Helper class which sets the appropriate icons for the
     *   down button.  Will use "instanceof" keyword to determine
     *   icons later.
     */
    ...
    /**
     *   Helper class which sets the appropriate icons for the
     *   right button.  Will use "instanceof" keyword to determine
     *   icons later.
     */
    ...
    /**
     *   Helper class which sets the appropriate icons for the
     *   left button.  Will use "instanceof" keyword to determine
     *   icons later.
     */
