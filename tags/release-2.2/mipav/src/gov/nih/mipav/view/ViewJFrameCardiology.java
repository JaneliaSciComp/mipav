 ...
 */
public class ViewJFrameCardiology extends ViewJFrameBase
    ...
    private void init(ModelLUT LUTa, Dimension loc, boolean logMagDisplay) throws OutOfMemoryError {
       ...
       controls.buildToolbar(menuBuilder.isMenuItemSelected("Image toolbar"),
                             false,
                             false,
                             false, componentImage.getVOI_ID());
    ...
    private void initComponentImage(int[] extents) throws OutOfMemoryError {
        ...
        componentImage = new ViewJComponentCardiology(this, imageA, LUTa, imageBufferA,
    ...
    public void updateFrame(float sX, float sY) {
        ...
        if (componentImage.getSize(null).width + 200 > xScreen || componentImage.getSize(null).height + 200 > yScreen) {
            ;
        } else if (getSize().width > componentImage.getSize(null).width || getSize().height > componentImage.getSize(null).height) {
    ...
    /* public boolean setZoomA() {
     ...
     float imageTemp[]       = null;
     int   pixBufferTemp[]   = null;
     int   paintBufferTemp[] = null;
     try   {
     if (imageA != null) {
     int bufferFactor = 1;
     if ( imageA.isColorImage() ) {
     ...
     imageTemp       = new float [bufferFactor * xDim * yDim];
     pixBufferTemp   = new int[xDim * yDim];
     paintBufferTemp = new int[xDim * yDim];
     }
     }
     catch (OutOfMemoryError error){
     imageTemp      = null;
     pixBufferTemp   = null;
     paintBufferTemp = null;
     System.gc();
     throw(error);
     }
     imageBufferA  = imageTemp;
     pixBuffer     = pixBufferTemp;
     paintBuffer   = paintBufferTemp;
    ...
    protected void updateHistoLUTFrame( int imageAorB, ViewJComponentEditImage compImg ) {
        ...
        if ( compImg.getImageA().getHistoLUTFrame() != null && imageAorB == IMAGE_A ) {
            compImg.getImageA().getHistoLUTFrame().updateHistoLUT( compImg.getImageA(), compImg.getLUTa(), null, null, true );
        } else if ( compImg.getImageA().getHistoLUTFrame() != null && imageAorB == IMAGE_B ) {
            compImg.getImageA().getHistoLUTFrame().updateHistoLUT( null, null, compImg.getImageB(), compImg.getLUTb(), true );
        } else if ( compImg.getImageA().getHistoLUTFrame() != null && imageAorB == IMAGE_A_B ) {
            compImg.getImageA().getHistoLUTFrame().updateHistoLUT( compImg.getImageA(), compImg.getLUTa(), compImg.getImageB(), compImg.getLUTb(), true );
        }
    }
    ...
    public void close() {
        ...
        if ( userInterface.isScriptRecording() ) {
            userInterface.getScriptDialog().append("CloseFrame " + userInterface.getScriptDialog().getVar( getActiveImage().getImageName() ) + "\n" );
        }
            ...
            if ( !getActiveImage().getImageName().equals( name ) ) {
                try {
                    ...
                    if ( getActiveImage().getNDims() == img.getNDims()
                            || ( isForCalculator
                                    && ( getActiveImage().getNDims() == 3 && img.getNDims() == 2 ) ) ) {
     ...
     */
    private ModelImage prepareImages( ModelImage image2load, ModelLUT lut,
            boolean isFromFrame ) {
        ...
        res[0] = getActiveImage().getFileInfo()[0].getResolutions()[
                0];
        res[1] = getActiveImage().getFileInfo()[0].getResolutions()[
                1];
    ...
    public void keyReleased( KeyEvent e ) {
        ...
        switch ( keyCode ) {
            ...
            case KeyEvent.VK_B:
                ...
                Preferences.setProperty("ShowPaintBorder", String.valueOf("" + !Preferences.is(Preferences.PREF_SHOW_PAINT_BORDER)));
                updateImages(true);
                break;
    ...
    private ViewJFrameCardiology subSampleImage( ModelImage _imageA, ModelImage _imageB ) {
        ...
        /*
         xDim = dimPowerOfTwo(xDim);
         yDim = dimPowerOfTwo(yDim);
         zDim = dimPowerOfTwo(zDim);
         */
