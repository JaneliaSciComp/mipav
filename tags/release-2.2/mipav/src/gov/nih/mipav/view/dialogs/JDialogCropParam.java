...
public class JDialogCropParam extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "CropParam " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
                if ( displayLoc == NEW ) {
                ...
                if ( resultImage.getExtents().length == 2 ) {
                    ...
                    userInterface.getScriptDialog().append(
                            "" + leftSide + " " + rightSide + " " + topSide + " " + bottomSide + " " + front + " "
                            + back + "\n" );
                }
            }
        }
    ...
    private void init() {
        ...
        /*
         JPanel loopingTestPanel = new JPanel();
         loopingTestPanel.setBorder(buildTitledBorder("Looping test"));
         Box loopingTestBox = new Box(BoxLayout.Y_AXIS);
         loopingGroup = new ButtonGroup();
         noBuffer = new JRadioButton("No Buffer", true);
         noBuffer.setFont(serif12);
         noBuffer.addActionListener(this);
         loopingGroup.add(noBuffer);
         loopingTestBox.add(noBuffer);
         usingBuffer = new JRadioButton("Uses Buffer", false);
         usingBuffer.setFont(serif12);
         usingBuffer.addActionListener(this);
         loopingGroup.add(usingBuffer);
         loopingTestBox.add(usingBuffer);
         loopingTestPanel.add(loopingTestBox);
         contentBox.add(loopingTestPanel);
         */
     ...
     *	@param scale	Value to set right side value to.
     */
     ...
     *	@param scale	Value to set left side value to.
     */
     ...
     *	@param scale	Value to set top side value to.
     */
     ...
     *	@param scale	Value to set bottom side value to.
     */
     ...
     *	@param scale	Value to set front value to.
     */
     ...
     *	@param scale	Value to set back value to.
     */
         ...
         if ( algorithm instanceof AlgorithmCrop) {
             if((cropAlgo.isCompleted() == true ) && (resultImage != null)) { // in StoreInDest; "new Image"
               ...
               insertScriptLine(algorithm);
                 ...
                 if (resultImage != null)resultImage.disposeLocal();  // clean up memory
                 resultImage = null;
    ...
    /**
     *   When one of the text inputs has been left blank, trying to convert them
     *	to ints results in throwing a null pointer exception.  This method
     *	determines which one of the JTextFields threw the null pointer Exception.
    ...
    public void callAlgorithm() {
     if (displayLoc == NEW) {
       try {
         ...
         cropAlgo = new AlgorithmCrop(resultImage, image, borderSize, xBounds,
                                      yBounds, zBounds);
       ...
       try {
         cropAlgo = new AlgorithmCrop(image, borderSize, xBounds,
                                      yBounds, zBounds);
         cropAlgo.addListener( this );
