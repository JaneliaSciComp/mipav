...
/**
 *       Component LUT - the vertical or horizontal bar
 *       that gives a visual representation of the lookup table.
 *
...
public class ViewJComponentLUTTable extends ViewJComponentBase {
     ...
     *   @param _LUT         Model that the image will be filter through to
    ...
    /**
     *   Filters the model LUT through the LUT and produces
     *   an int array (aRGB) that can be convert to a java image
    ...
    private void drawString( Graphics g, String str, int x, int y ) {
        ...
        /*
         ...
         g.drawString(str, 5 + x, y - 6);
         g.drawString(str, 5 + x, y - 4);
         g.drawString(str, 6 + x, y - 5);
         g.drawString(str, 4 + x, y - 5);
         */
        // g.setColor(Color.white);
        g.drawString( str, 5 + x, y - 5 );
    ...
    private void drawLine( Graphics g, int x, int y ) {
        ...
        /*
         g.drawLine(offsetX - 30, y0 + 1, offsetX - 33, y0 + 1);
         g.drawLine(offsetX - 30, y0 - 1, offsetX - 33, y0 - 1);
         g.drawLine(offsetX - 31, y0, offsetX - 34, y0);
         g.drawLine(offsetX - 29, y0, offsetX - 32, y0);
         g.setColor(Color.white); */
    ...
    /*
     ...
     g.drawImage(img, 0, 0, Math.round(zoomX*img.getWidth(this)*resolutionX)-80,
     Math.round(zoomY*img.getHeight(this)*resolutionY),
     0, 0, img.getWidth(this)-80, img.getHeight(this), null);
     }

     g.setFont(MipavUtil.font12);
     string = "ruida";
     if ((int)(zoomX*imageDim.width+0.5)-40 > 0 && string != null){
     g.setColor(Color.black);
     g.drawString(string, 5+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-5);
     g.drawString(string, 5+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-6);
     g.drawString(string, 5+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-4);
     g.drawString(string, 6+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-5);
     g.drawString(string, 4+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-5);
     g.setColor(Color.white);
     g.drawString(string, 5+offsetX, (int)(zoomY*resolutionY*imageDim.height+0.5)-5);
     }
