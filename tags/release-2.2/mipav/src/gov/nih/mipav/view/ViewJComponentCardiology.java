...
public class ViewJComponentCardiology extends ViewJComponentEditImage
    ...
    /** opacity value used by the paint brush.  The opacity value may range
     * from 0 to 1, where 1 is completely opaque, and 0 is completely invisible.
     * <p>
     * Opacity definitions:
     * <ol>
     *  <ul>Opaque is 1.0</ul>
     *  <ul>default is 0.25</ul>
     *  <ul>Invisible is 0.0</ul>
     * </ol>
     * <p>Values outside the bounds of 0.0 to 1.0 are not defined.
     */
    ...
    protected Vector paintBorderPolygons = new Vector();
    protected boolean paintChangeFlag = false;*/
     ...
     */
    public ViewJComponentCardiology( ViewJFrameBase _frame,
            ModelImage _imageA,
            ModelLUT _LUTa,
            float[] imgBufferA,
            int[] pixelBuffer,
    ...
    public void calculateVOILengths() {
      ...
      for (int i = 0; i < activeArray.length; i++) {
        if (activeArray[i]) {
          ...
          cardioVOI.calcAverageLength(i, imageActive.getExtents()[0],
              imageActive.getExtents()[1], imageActive.getFileInfo()[0].getResolutions(),
    ...
    public void calculateVOIIntensities() {
      ...
      for (int i = 0; i < activeArray.length; i++) {
        if (activeArray[i]) {
            ...
            System.err.println(i + " entire section: " +avgIntensities[0] + ", infarction: " +
                               avgIntensities[1]);

  //        System.err.println("i: " + cardioVOI.calcAverageIntensity(graphImgBuff, i, imageActive.getExtents()[0],
  //                                                                  imageActive.getUserInterface()));
        }
      }
    }
     ...
     */
    public void mouseEntered( MouseEvent mouseEvent ) {/* if (System.getProperties().getProperty("os.name").equals("MacOS")) {  //??????
         frame.setVisible(true);
         frame.windowActivated(fnull);
         }*/}
                            ...
                            for ( j = 0; j < VOIs.VOIAt( i ).getCurves()[slice].size(); j++ ) {
                                if ( ( (VOILine) ( VOIs.VOIAt( i ).getCurves()[slice].elementAt( j ) ) ).nearLine( xS,
                                    ...
                                    if ( imageActive.isColorImage() == true ) {
                                        ...
                                        for ( int c = 0; c < 3; c++ ) {
                                            int pt = ( (VOILine) ( VOIs.VOIAt( i ).getCurves()[slice].elementAt( j ) ) ).findPositionAndIntensityRGB(
    ...
    public void mouseReleased( MouseEvent mouseEvent ) {
            ...
            if ( imageActive.getFileInfo( 0 ).getOrigin()[0] != 0 || imageActive.getFileInfo( 0 ).getOrigin()[1] != 0
                ...
                if ( values != null ) {
                    frame.setMessageText(
                            "  X: " + String.valueOf( xS ) + " Y: " + String.valueOf( yS ) + "  Intensity:  "
                ...
                if ( imageActive.isColorImage() == true ) {
                    frame.setMessageText(
                            "  X: " + String.valueOf( xS ) + " Y: " + String.valueOf( yS ) + "  R:  "
        ...
        if ( ( mouseEvent.getModifiers() & mouseEvent.BUTTON2_MASK ) != 0 ) {
          ...
          if ( VOIs != null ) {
            ...
            if (( mouseEvent.getModifiers() & mouseEvent.BUTTON1_MASK ) != 0  ) {
              if (mode == NEW_POINT) {
                ( (VOICardiology) (VOIs.VOIAt(0).getCurves()[slice].elementAt(0))).
                    insertElement(xS, yS, slice);
                ...
                int section = ( (VOICardiology) VOIs.VOIAt(0).getCurves()[0].
                               elementAt(0)).getSection(xS, yS);
               if (section >= 0) {
                 boolean isActive = ( (VOICardiology) VOIs.VOIAt(0).getCurves()[0].
                                     elementAt(0)).isActive(section);
                 ( (VOICardiology) VOIs.VOIAt(0).getCurves()[0].elementAt(0)).
        ...
        if ( mode == SELECT ) {// paintComponent(getGraphics());
            ...
            return;
            // setMode(DEFAULT);
        } else if ( mode == LEVELSET ) {} else if ( mode == RECTANGLE ) {} else if ( mode == RECTANGLE3D ) {} else if ( mode
                == ELLIPSE ) {} else if ( mode == LINE ) {} else if ( mode == PROTRACTOR ) {} else if ( mode
                                                ...
                                                for ( int c = 0; c < 3; c++ ) {
                                                    ...
                                                    for ( j = 0, intensitySum = 0; j
                                                        ...
                                                        if ( ( (VOIContour) VOIs.VOIAt( i ).getCurves()[s].elementAt( j ) ).isActive()
                                                            ...
                                                            intensitySum += ( (VOIContour) ( VOIs.VOIAt( i ).getCurves()[s].elementAt( j ) ) ).calcRGBIntensity(
    ...
    public void mouseDragged( MouseEvent mouseEvent ) {
        ...
        if ( mode == MOVE_INTERSECTION_POINT ||
             mode == MOVE_POINT) {
    ...
    public boolean getShowMagIntensityEnabled() {
        ...
        return super.getShowMagIntensityEnabled( g, MAGR_WIDTH, MAGR_HEIGHT, MAGR_MAG,
                imageActive.getType(), getActiveImage().getMin(), getActiveImage().getMax() );
     ...
     */
    protected String[] setMouseOverlayData( FileInfoBase fileInfo,
            MouseEvent mouseEvent, int zSlice ) {
    ...
    /**
     *    Grows a region based on a starting supplied. A voxel is added to the
     *    the paintBitmap mask if its intensity is between the the bounds where
     *    are also supplied.
     * <p>
     ...
     */
    public void regionGrow( short x, short y, short z, float value, String str,
            boolean click ) {
        ...
        try {
            ...
            if ( imageActive.getNDims() == 2 ) {
                ...
                count = regionGrowAlgo.regionGrow3D( seedPaintBitmap, new Point3Ds( saveX, saveY, saveZ ),
                        ...
                        maxDistance, variableThresholds, timeSlice, null );
                showRegionInfo( count, str );
            }
        } catch ( OutOfMemoryError error ) {
            System.gc();
