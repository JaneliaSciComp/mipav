...
public class JDialogExtractBrain extends JDialogBase
     ...
     *	because no user input is necessary at present.
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "ExtractBrain " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );
    ...
    /**
     * Set the stiffness of the mesh that is part of the surface normal term
    ...
    /**
     * Set the maximum depth that is part of the image term in
    ...
    /**
     * Sets whether or not to use only an estimate of sphere to
     *    nitialize the boundary evolution
     * @param useSphere if true estimate boundary using sphere model. If false the
     *                  initial boundary is estimated using an ellipsoid, which is more
    ...
    public void algorithmPerformed( AlgorithmBase algorithm ) {
        ...
        if ( algorithm instanceof AlgorithmBrainExtractor ) {
            ...
            userInterface.setGlobalDataText(
                    image.getImageName() + " volume = \t" + extractBrainAlgo.getBrainVolume() / 1000.0f + " cc\n" );
    ...
    public void itemStateChanged( ItemEvent event ) {
        if ( event.getSource() == useCenterOfMassCheckBox ) {
            ...
            if ( oldPoint.equals( centerOfMass ) || useCenterOfMass ) {
                // user hasn't changed the initial point from the center of mass (or has chosen to just use the center of mass), then we can change the values
