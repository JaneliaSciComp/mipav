...
/**
 *       This is a frame which holds a graph in one panel and a
 *       set of user buttons in the other.  It uses swing components
 *       for all of the graphics and calls ViewJComponentGraph for the
 *       graph.
 *
...
public class ViewJFrameGraph extends JFrame
    ...
    /**
     *  Constructor   Constructs the frame, with the graph component
     *                in one panel and the user options in the other.
     *                No funciton is loaded by default, but the user
     *                can open a new function to be displayed
     */
     ...
     *  Constructor   Constructs the frame, with the graph component
     *                in one panel and the user options in the other.
     *                Draws initial default graph of desired function / intensity plot.
     ...
     */
    public ViewJFrameGraph( float[] xInit, float[] yInit, String title,
            String labelX, String labelY, Color color ) {
        super( title );
        ViewJComponentFunct[] functArray;
        ViewJComponentFunct[] fittedFuncts;
     ...
     */
    public ViewJFrameGraph( float[][] xInit, float[][] yInit, String title,
            String labelX, String labelY, Color[] colorArray ) {
        super( title );
        voi = null;
        updateRGBIndex = 0;
        ViewJComponentFunct[] functArray;
     ...
     *  Constructor   Constructs the frame, with the graph component
     *                in one panel and the user options in the other.
     *                Graph is assumed to be made from a VOIPoint.
     *                Draws initial default graph of desired function / intensity plot.
    ...
    public void saveNewFunction( float[] newX, float[] newY, int index ) {
        ...
        for ( int i = 0; i < graph.getFittedFuncts().length; i++ ) {
            graph.getFittedFuncts()[i].setXs( graph.getFittedFuncts()[i].getOriginalXs() ); //gets the original x and y point values
            graph.getFittedFuncts()[i].setYs(
                    graph.getFittedFuncts()[i].getOriginalYs() );
        }
     ...
     *  @param ptIndex   the index of the point to be updated (will not do anything if trying to update
     *                   of an index different than updateRGBIndex)
     */
    ...
    /**
     *  Function that builds the menu of various tools to be
     *  used in conjunction with the graph. ie: opening / saving files,
     *  changing gridlines, title / axis labels, etc.
     *
     */
    ...
    /**
     *  Takes the action commands and paints if the apply
     *  button is pressed, closes if the close button is
     *  pressed.
        ...
        if ( command.equals( "CloseGraph" ) ) {
            ...
            try {
                ...
                for ( int i = 0; i < functions.length; i++ ) {
                    ...
                    messageGraph.append(
                            " y = " + String.valueOf( params[0] ) + " + " + String.valueOf( params[1] ) + " * exp("
                            + String.valueOf( params[2] ) + " * x)\n" );
                    messageGraph.append( "\n" );
                }
            } catch ( OutOfMemoryError error ) {
            ...
            try {
                ...
                for ( int i = 0; i < functions.length; i++ ) {
                    ...
                    messageGraph.append(
                            " y = " + String.valueOf( params[0] ) + " + " + String.valueOf( params[1] ) + " * x\n" );
                    messageGraph.append( "\n" );
                }
            } catch ( OutOfMemoryError error ) {
                MipavUtil.displayError( "Graph :  Out of memory " );
            ...
            String xText = xGridLineField.getText(); //stores the input for the number of gridlines along the x and y axis
            ...
            if ( testParameter( xText, 1, 50 ) && testParameter( yText, 1, 50 ) ) { //tests to see if the number inputted is
                ...
                graph.setNumberOfXGridLines(
                        Integer.parseInt( xText ) ); //sets the appropriate number of gridlines
    ...
    /**
     *  This class is called by the color chooser when the user selects a
     *  a color from the background color chooser dialog box.  It changes the background color.
     */
    class bColorListener
    ...
    /**
     *   This class is called by the color chooser when the user selects a
     *   a color from the function line color chooser dialog.  It changes the
     *                   selected line (found by the variable index) to the color returned by
     *                   the color chooser.
     */
    class ColorListener
        implements ActionListener {
    ...
    /**
     *  This class is called by the color chooser when the user cancels.  It
     *  disposes of the dialog if the dialog still exists.
     */
    ...
    /**
     *  Creates a dialog box in which the user can modify various features
     *  of the graph
     */
    ...
    /* Creates a panel to edit various fitted function features. (Panel is to be used in
     *  the tabbed pane of the Modify Graph Dialog box
     */
    ...
    /* Creates a panel where various function fetaures such as visibility, color, and points
     *  can be toggled.  This panel is used in the tabbed pane of the Modify Graph Dialog box.
     */
    private void buildFunctionPanel() {
        ...
        for ( int index = 0; index < 5; index++ ) {
            ...
            functVisibleCheckbox[index].addActionListener(
                    new ActionListener() {
                public void actionPerformed( ActionEvent event ) {
                    JCheckBox c = (JCheckBox) ( event.getSource() );
                    for ( int i = 0; i < graph.getFuncts().length; i++ ) {
                        if ( c == functVisibleCheckbox[i] ) {
                            ...
                            if ( minRangeLabel != null ) {
                                minRangeLabel.setText(
                                        "Min. for Range (<" + Float.toString( graph.getDefaultMinRange() ) + ")" );
                            }
                            if ( maxRangeLabel != null ) {
                                maxRangeLabel.setText(
                                        "Max. for Range (>" + Float.toString( graph.getDefaultMaxRange() ) + ")" );
                            }
                            break;
                        }
                    }
    ...
    /* Creates a panel where various features of the legend can be edited, such as showing
     *  the legend, and changing function names.  This panel is used by the tabbed pane in
     *  Modify Graph Dialog Box.
     */
        ...
        for ( int i = 0; i < 5; i++ ) { //sets the bounds for each of the text fields and label for
            ...
            try {
                if ( i < graph.getFuncts().length ) {
                    legendField[i] = new JTextField( graph.getFuncts()[i].getFunctName() ); //sets the default string in the text field to
                } else {
                    legendField[i] = new JTextField();
                }

                legendLabel[i] = new JLabel( "Function " + ( i + 1 ) + " Name" ); //sets the label to the corresponding funciton
                //index number
            } catch ( OutOfMemoryError error ) {
                MipavUtil.displayError( "Out of memory: ViewJComponentGraph.buildLegendPanel" );
                return;
    ...
    /* Creats a panel where various featuers of the graph can be edited, such as visbility of
     *  gridlines and minor tick marks, number of gridlines, background color, range, and
     *  labels for the title, x and y axiis.
     */
    ...
    /**
     * Updates Fitted Functions to accurately estimate the functions in the graph
     * (Function data may have chaned due to realtime updating, and therefore
     *  the fitted functions must be updated)
     */
    ...
    /**
     * Updates fields of the Modify Graph Dialog which may have changed due to things
     * such as adding/deleting functions.
     */
    ...
    public void componentResized( ComponentEvent event ) {
        ...
        graph.resetBounds( new Rectangle( 0, 0, rect.width, rect.height - 60 ) ); //changes the graph's bounds appropriately
        update(
                getGraphics() );
    ...
    /**
     *  Saves this graph as Excel data so that Excel can read
     *  in the data and graph it as an Excel file.  The file can also be saved
     *  to be viewed at a later time with MIPAV
     */
    private void save()
        throws IOException {
        ...
        try {
            ...
            chooser.addChoosableFileFilter( new ViewImageFileFilter( ViewImageFileFilter.PLOT ) ); //adds a choosable filter
    ...
    /**
     *	Disposes of error dialog, then frame.  Nullifies pointer to this in
     *   VOIPoint, if this graph was made from a VOIPoint.
     */
