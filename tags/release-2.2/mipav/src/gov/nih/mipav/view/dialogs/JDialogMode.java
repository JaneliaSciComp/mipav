...
/**
 *   Dialog to get user input, then call AlgorithmMode. The user has the
...
public class JDialogMode
    ...
    public JDialogMode(Frame theParentFrame, ModelImage im) {
        ...
        if (im.getType() != ModelImage.BYTE && im.getType() != ModelImage.UBYTE &&
            ...
            im.getType() != ModelImage.INTEGER &&
            im.getType() != ModelImage.UINTEGER) {
        ...
        if (image.getNDims() == 2) { // source image is 2D
            ...
            if (displayLoc == NEW) { // (2D)
                try {
                    ...
                    // Make algorithm
                    modeAlgo = new AlgorithmMode(resultImage, image,
                                                 kernelSize, kernelShape,
                                                 entireImageFlag);
                ...
                try {
                    ...
                    modeAlgo = new AlgorithmMode(image, kernelSize,
                                                 kernelShape, entireImageFlag);
