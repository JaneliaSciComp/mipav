...
/**
 *   Dialog to get user input, then call the algorithm.  The algorithm
 *   concatenates the images.  The user has the option to generate a
 *   new image or replace the source image.  It should be noted that the
 *   algorithms are executed in their own thread.  See AlgorithmConcat
...
public class JDialogConcat
      ...
      if (algo.isCompleted()) {
          if (userInterface.isScriptRecording()) {
              ...
              if (displayLoc == NEW) {
                  ...
                  userInterface.getScriptDialog().append(userInterface.
                                                         getScriptDialog().getVar(imageA.getImageName()) + "\n");
  ...
  private void init() {
    ...
    if (comboBoxImage.getItemCount() == 0) {
      MipavUtil.displayError(
          "There are no images with the proper dimensions\nto concatenate to"
          + imageA.getImageName() + ".");
      return;
    }
      ...
      if (UI.getFrameContainingImage(img) != null) {
        if (!imageA.getImageName().equals(name)) {
          if (imageA.isColorImage() == img.isColorImage()) {
            ...
            else if ( ( (imageA.getNDims() == 4) || (img.getNDims() == 4)) &&
                     (imageA.getNDims() >= 3) && (img.getNDims() >= 3) &&
                     (imageA.getExtents()[0] == img.getExtents()[0]) &&
                     (imageA.getExtents()[1] == img.getExtents()[1]) &&
  ...
  /**
   *	Once all the necessary variables are set, call the Concat algorithm
   *   based on what type of image this is and whether or not there
  ...
  private void callAlgorithm() {
    ...
    if (imageA.getNDims() <= 4 && imageB.getNDims() <= 4) {
      ...
      if (imageA.getNDims() == 2 && imageB.getNDims() == 2) {
        ...
        MipavUtil.displayError(
            "The two images are not of the proper\ndimensionality to be concatenated.");
        return;
      }
      destImage = new ModelImage(imageA.getType(), destExtents,
                                 makeImageName(imageA.getImageName(), "_concat"),
                                 userInterface);
      ...
      try {
        ...
        System.gc();
        MipavUtil.displayError(
            "Dialog Concatenation: unable to allocate enough memory");
        return;
      }
    }
  }
