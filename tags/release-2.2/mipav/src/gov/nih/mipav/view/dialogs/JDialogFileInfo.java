...
import javax.swing.*;
   ...
   /**
    *   This class shows the dialog which conatains the file-info header information
    *   as used in the FileInfoBase class.
    *   <p>
    *   it builds two tables, and any row can be made editable when supplied with
    *   the appropriate editor to use.  Entries that are edited okay, reports updates
    *   to file info.
    *   <p>
    *   It merely brings up a JDalogEditor when "edit" button is clicked.
    *   <p>
    *   <i>17 January 2002</i>: Right now, this class is set up to handle <i>only</i>
    *   FileInfoAnalyze edits.  This is because the
    *
    *   @author     parsonsd;
    *   @version    0.2
    */
 ...
 public class JDialogFileInfo extends JDialogBase implements ActionListener {
   ...
   /**
    *   This method displays all the valid variables, that
    *   is, the ones that are no longer equal to their default
    *   values.  It calls parseDate and parseTime when needed
    *   and translates other strings.  However, this method
    *   does not yet translate every single DICOM tag, only
    *   those most used.  The others it outputs as strings.
    *
    *   @param numEditableColumns   the number of columns
    *                       an editable table should have
    *                       (either 2 or 3 is allowed).
    */
    ...
    *   @param String name file info parameter (ie., dimensions, extents, &c).
    *   @param St                                                                                                                                                                                                                                                                                                                       ring value value assigned to a fileinfo parameter
    */
    ...
    /** appends a row to the end of the primary info table.  assigns this
    *   name/value pair to be editable and adds the fileinfo to
    ...
    *   @param editor The value of editor is the editor interface to be used.
    *                 Eg., a JPanelEditDefault.  Specified by <ul>
    ...
    *   @param String value value assigned to a fileinfo parameter
    *   @param editor The value of editor is the editor interface to be used.
    *                 Eg., a JPanelEditDefault.  For example, specified by <ul>
    ...
    */
    ...
    /** permits the caller to get a value out of the primary table
    *   by using the name given to the fileInfo.
    *   @return the value returned is the first value which
    *           keys to this name; any other instances of the name
    *           will be ignored.  <code>Null</code> is returned if the
    ...
    *   window close;
    *   <p>
    *   will alert any open window (frame) to set title as that information may
    *   have changed.
    *   <p>
            ...
            for (i = 0; i < rows.length; i++) {
                ...
                if (primaryTypeHolder.containsKey(named)) { // if named
                    ...
                    if (obj != null) {
                        ...
                        String values[] = new String[1];    // this is unclean design now to cover something i didn't foresee
                        ...
                        editor.addWindowListener(new WindowAdapter() {
                            ...
                            public void windowClosed(WindowEvent e) {
                                ...
                                if (ed.wasDialogOkay()){
                                    ...
                                    //  Also suggest that a distinct datatype (other than Vector)
                                    //  be created to handle the special needs.
                                    ((FileInfoAnalyze)fileinfo).stateChanged(changed); // what to do to update fileinfo!?!?!
                                    ...
                                    for (int q=0; q < image.getFileInfo().length; q++) {
                                        ((FileInfoAnalyze)fileinfo).updateFileInfos((FileInfoAnalyze)image.getFileInfo(q));
                                    }
                                }
                                else {
                                    primaryEditorHolder.remove(edID);
            ...
            for (i = 0; i < rows.length; i++) {
                ...
                if (secondaryTypeHolder.containsKey(named)) { // if named
                    ...
                    if (obj != null) {
                        ...
                        int editors[] = new int[objs.size()]; // because we tell an editor to do something based on an int code
                        for (int j = 0; j < objs.size(); j++) {
                            editors[j] = ((Integer)(objs.elementAt(j))).intValue();   // what the user asked for
                        }
                        editor = new JDialogEditor(this, named, values, editors);
                        ...
                        editor.addWindowListener(new WindowAdapter() {
                            ...
                            public void windowClosed(WindowEvent e) {
                                ...
                                if (ed.wasDialogOkay()) {
                                  ...
                                  //  be created to handle the special needs.
                                  ( (FileInfoAnalyze) fileinfo).stateChanged(changed); // what to do to update fileinfo!?!?!
                                  secondaryModel.setValueAt(ed.getDisplayValue(),
                                                            edID.intValue(), 1);
                                  ...
                                  if (image != null && image.getFileInfo() != null) {
                                    for (int q = 0; q < image.getFileInfo().length; q++) {
                                      ( (FileInfoAnalyze) fileinfo).updateFileInfos( (
                                          FileInfoAnalyze) image.getFileInfo(q));
                                    }
                                  }
                                }
                                else {
    ...
    *
    *   @param  tagKey the tag's Key.  Used to dtermine if this tag already has
    *                  an editor associated with it.
    *   @return true if both a tag with the tagkey existed in the list and the
    *                associated dialog was brought to front.
    */
