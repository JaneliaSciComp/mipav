 ...
 * With 4D images only the first 3 dimensions are subsampled.
 * @author Sir Benjamin Link
 * @version 1.0
 */
public class JDialogSubsample extends JDialogBase
        ...
        if ( algo.isCompleted() ) {
            if ( userInterface.isScriptRecording() ) {
                ...
                userInterface.getScriptDialog().append(
                        "Subsample " + userInterface.getScriptDialog().getVar( image.getImageName() ) + " " );

                userInterface.getScriptDialog().putVar( resultImage.getImageName() );
                userInterface.getScriptDialog().append(
                        userInterface.getScriptDialog().getVar( resultImage.getImageName() ) + " " + denom + " "
                        + doVOI + " " + processIndep + " " + lockZ + "\n" );
            }
        }
    }
    ...
    private void init() {
        ...
        if ((image.getVOIs() != null) && (!image.getVOIs().isEmpty()) &&
             (image.getNDims() <= 3)) {
    ...
    private boolean setVariables() {
        ...
        if (doVOI) {
            oXres = image.getFileInfo(0).getResolutions()[0] *
                    (float)(image.getExtents()[0]-1) / (float)(newExtents[0]-1);
            oYres = image.getFileInfo(0).getResolutions()[1] *
                    (float)(image.getExtents()[1]-1)/ (float)(newExtents[1]-1);
            //Sx = ( (float) (newExtents[0] - 1) * oXres) /
                   //( (float) (image.getExtents()[0] - 1) * image.getFileInfo(0).getResolutions()[0]);
            Sx = 1.0f;
            //Sy = ( (float) (newExtents[1] - 1) * oYres) /
            ...
            if ( processIndep || (image.getNDims() == 2)) {
                ...
                if (lockZ) {
                    ...
                    oZres = image.getFileInfo(0).getResolutions()[2] *
                            (float)(image.getExtents()[2]-1)/ (float)(newExtents[2]-1);
                    //Sz = ( (float) (newExtents[2] - 1) * oZres) /
                           //( (float) (image.getExtents()[2] - 1) * image.getFileInfo(0).getResolutions()[2]);
                    Sz = 1.0f;
                }
    ...
    private void callAlgorithm() {
        ...
        resultImage = new ModelImage( image.getType(), newExtents, image.getImageName() + "_subsample_" + denom, userInterface );

        algoSub = new AlgorithmSubsample( image, resultImage, newExtents, sigmas, processIndep,
                                          doVOI, xfrm );
