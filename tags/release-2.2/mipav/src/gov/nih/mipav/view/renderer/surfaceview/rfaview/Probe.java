...
/**
 * <p>Title: Probe </p>
 * <p>Description: This class defines the basic geometry structures of the probe, such as
 * Java3D image scene graph, detection entry point, bone detection point, and probe detection
 * guide line.   The class also constructs three types of probe, default type, thermal heat type,
 * and regular type.  Each probe has its own geometry shape and appearance.  The probe
 * detection is used only by the 3D texture render for the current development phase.  Raycast
 * based renderer will have the RFA probe detection later. </p>
 * @author Ruida Cheng
 */
public class Probe
    ...
    /** The probe's root transform group, which holds the red target sign, guide line,
     * and the actual probe geometry shape. */
    ...
    /** Probe detection ending point transform group that represents the probe
     *  guide line ending point. */
    protected TransformGroup endPointTG;

    /** Probe detection starting point transform group that represents the probe
     *  guide line starting point. */
    ...
    /** The green sphere branch group that branches underneath the entryPointBG,
     *  and represents the green sphere.
     */
    ...
    /** The yellow sphere branch group that branches underneath the boneBG,
     *  and repreents the yellow sphere branch group.
     */
    ...
    /** The blue sphere branch group that branches underneath the vasculatureBG,
     *  and represents the blue sphere branch group.
     */
    protected BranchGroup blueBG;

    /** The root branch group of the indicator.   The group is the parent root of
     *  boneBG, entryPointBG, vascaulatureBG.
     */
    ...
    /**
     * The probe contructor.  Initialize the image scene graph and build the three types
     * of probe.
     ...
     * JPanelProbe call this method during probe detection and burning.
    ...
    /**
     * Build the three types of probe. The basic geometry structure of the probes include
     * the red target, entry and ending point, the red guide line, etc.
     ...
     * @param startPtTransVector Vector3f
     * @param endPtTransVector Vector3f
     */
    ...
    /**
     * Get the end point transform.  Used by the burning process to decide
     * the recorded probing path.
    ...
    /**
     * Get the entry pont transform group.  Used by the burning process to decide
     * the recorded probing entry point.
    ...
    /**
     * Get the start point transform.  Used by the burning process to decide
     * the recorded probing path.
    ...
    /**
     * When the probe is picked, change the probe geometry shape color to green
     * to indicate picked.
    ...
    /**
     * The red target sign's x cylinder transform group, which regards as the
     * center of the burning point.
    ...
    /**
     * When the probe is picked and rotated by mouse, detecting the probing tissue
    ...
    /**
     * Called when the JPanelProbe twist slider is changed. The twist
     * transform is updated based on the input value fValue, and passed to the
     * SurfaceRender class for display.
