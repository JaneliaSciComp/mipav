...
/**
 *   Adaptive Histogram Equlization<p>
...
public class JDialogAHE
  ...
  private void init() {
    ...
    comboBoxWidthDiv.setMaximumSize(comboBoxWidthDiv.getPreferredSize()); // don't let it get any bigger than what it prefers
    ...
    clampValueLabel = new JLabel(
        "Fraction of most frequent pixel intensity (%)");
    ...
    colorPanel.setToolTipText(
        "Each color channel can be equalized in any combination");
  ...
  /**
   *   Creates the combo-box that allows user to select the number of
   *   divisions in the image when building the histogram.
   */
  ...
  private boolean setVariables() {
    ...
    try {
      heightDivisions = Integer.parseInt( (String) comboBoxHeightDiv.
                                         getSelectedItem());
      widthDivisions = Integer.parseInt( (String) comboBoxWidthDiv.
                                        getSelectedItem());

      if (heightDivisions >= maxH) {
        MipavUtil.displayError(
            "Too many divisions.  Choose a number smaller than " + maxH);
        fld = (JTextField) (comboBoxHeightDiv.getEditor().getEditorComponent());
        fld.requestFocus();
        fld.selectAll();
        return false;
      ...
      if (widthDivisions >= maxW) {
        MipavUtil.displayError(
            "Too many divisions.  Choose a number smaller than " + maxW);
        fld = (JTextField) (comboBoxWidthDiv.getEditor().getEditorComponent());
        fld.requestFocus();
        fld.selectAll();
        return false;
  ...
  private void callAlgorithm() {
    ...
    if (displayLoc == NEW) {
      try {
        ...
        // Make algorithm
        aheAlgo = new AlgorithmAHE(resultImage, image, heightDivisions,
                                   widthDivisions);
        ...
        MipavUtil.displayError(
            "Dialog Adaptive Histogram Equalization: unable to allocate enough memory");
        if (resultImage != null) {
