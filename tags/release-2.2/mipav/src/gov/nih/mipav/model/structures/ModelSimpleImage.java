...
/**
 *   This is very simple class to store a float type image of up to
 *   four dimensions. For speed all class variables are public. This
 *   class is used extensively in Registration algorithms where we
 *   needs many copies of the image where the data does not need to
 *   be visible to the UserInterface. With the data array as public
 *   speed is achieved but at a cost that the buffer is always of the
 *   type float.
 *
...
public class ModelSimpleImage extends ModelSerialCloneable  {
    ...
    /**
     * Creates a class to hold the minimum information about an image -
     * the extents, the resolutions, and optionall, the image data.
     * @param image ModelImage Structure where the input image data is stored.
     * In this particular call, this parameter must not be null as the
     * extents and resolutions are derived from the input image.
     */
    ...
    /**
     * Creates a class to hold the minimum information about an image -
     * the extents, the resolutions, and the image data.  Used for a 4D image
     * to make a 3D ModelSimpleImage.
     * @param image ModelImage Structure where the input image data is stored.
     * In this particular call, this parameter must not be null as the
     * extents and resolutions are derived from the input image.
    ...
    /**
    *	Creates a class to hold the minimum information about an image -
    *	the extents, the resolutions, and optionally, the image data.
    *	@param dimExtents	Extents of the image; must be non null.
    *	@param voxRes		Resolutions of the image; must be non null.
    *	@param image		Structure where image data is stored; can be null.
    *						If it is null, the buffer associated with this class
    *						will be empty.  The ModelImage is not otherwise saved
    *						within this class.
    */
    ...
    */
    ...
    /**
    *	Creates a class to hold the minimum information about an image -
    *	the extents with the resolutions defaulted to 1.
    *	@param dimExtents	Extents of the image; must be non null.
    */
    ...
    /**
    *	Creates a class to hold the minimum information about an image -
    *	the extents and the resolutions.
    *	@param dimExtents	Extents of the image; must be non null.
    *	@param voxRes		Resolutions of the image; must be non null.
    */
     ...
     * which contains the intensity value from this image if it is
     * a color image or which contains the same values if the image
     * is already an intensity image.  Scale factors are equally
     * weighted for each of the RGB channels.
     * @return ModelSimpleImage New instance created with intensity values
     * only.
     */
     ...
     * which contains the intensity value from this image if it is
     * a color image or which contains the same values if the image
     * is already an intensity image.  Scale factors are specified
     * for how to compute the intensity image as a linear combination
     * of the RGB channels.
    ...
    public ModelSimpleImage createIntensityImage (float fScaleR, float fScaleG, float fScaleB) {
        ...
        ModelSimpleImage kIntensityImage = new ModelSimpleImage(
            this.extents, this.resolutions);
        ...
        // from 'this' image into the intensity value for the new image.
            ...
            for (int i = 0; i < kIntensityImage.data.length; i++) {
                int iColorIndex = 4*i;
                kIntensityImage.data[i] =
                    (this.data[iColorIndex+0] * fScaleR) +
    ...
    /**
     * Create a mapping of this image.  The mapping is defined by a set of
     * floating point coordinates for each sample in the image.  The
     * coordinates are actuallys stored in separate images, one for each
     * dimension.  The number dimensions and the resolutions of these
     * coordinate images must be the same so that either one can be used
     * to determine the dimensions and resolutions of the new image
     * where the mapped source image values will be stored using
     * bilinear interpolation.
     * @param kImageMapX ModelSimpleImage Image contains the x coordinate
     * for the mapping.  Each value in the image must be in the range [0,1].
     * @param kImageMapY ModelSimpleImage Image contains the y coordinate
     * for the mapping.  Each value in the image must be in the range [0,1].
     * @return ModelSimpleImage A new instance created with the mapping
     * of this image.
     */
    public ModelSimpleImage createMappedImage2d(
        ModelSimpleImage kImageMapX, ModelSimpleImage kImageMapY) {

        ModelSimpleImage kMappedImage = new ModelSimpleImage(
            kImageMapX.extents, kImageMapX.resolutions, this.isColor);
        ...
        for (int iY = 0; iY < iNumSamplesTrgY; iY++) {
            for (int iX = 0; iX < iNumSamplesTrgX; iX++) {
                ...
                for (int iC = 0; iC < iNumChannelsSrc; iC++) {
                    ...
                    float fSrcY0 =
                        this.data[iC+iX0Y0] * fX0 +
                        this.data[iC+iX1Y0] * fX1;
                    float fSrcY1 =
                        this.data[iC+iX0Y1] * fX0 +
                        this.data[iC+iX1Y1] * fX1;
     ...
     * @param kImageMapY ModelSimpleImage Image contains the y coordinate
     * for the mapping.  Each value in the image must be in the range [0,1].
     * @param kImageMapZ ModelSimpleImage Image contains the z coordinate
     * for the mapping.  Each value in the image must be in the range [0,1].
     * @return ModelSimpleImage A new instance created with the mapping
     * of this image.
     */
    public ModelSimpleImage createMappedImage3d(
        ModelSimpleImage kImageMapX, ModelSimpleImage kImageMapY,
            ...
            for (int iY = 0; iY < iNumSamplesTrgY; iY++) {
                for (int iX = 0; iX < iNumSamplesTrgX; iX++) {
                    ...
                    for (int iC = 0; iC < iNumChannelsSrc; iC++) {
                        ...
                        // interpolate across X
                        float fSrcY0Z0 =
                            this.data[iC+iX0Y0Z0] * fX0 +
                            this.data[iC+iX1Y0Z0] * fX1;
                        float fSrcY1Z0 =
                            this.data[iC+iX0Y1Z0] * fX0 +
                            this.data[iC+iX1Y1Z0] * fX1;
                        float fSrcY0Z1 =
                            this.data[iC+iX0Y0Z1] * fX0 +
                            this.data[iC+iX1Y0Z1] * fX1;
                        float fSrcY1Z1 =
                            this.data[iC+iX0Y1Z1] * fX0 +
                            this.data[iC+iX1Y1Z1] * fX1;
    ...
    public ModelSimpleImage subSample2dBy2(ModelSimpleImage resultImage, boolean isColor) {
        ...
        for (int y = 0, by = 1; by < yStop; y++, by += 2) {
            for (int x = 0, bx = 1; bx < xStop; x++, bx += 2) {
                ...
                if (isColor) {
                    for (c = 0; c <= 3; c++) {
                        resultImage.data[4 * (y * rowSize + x) + c] =
                            (float)
                            ( (0.208 * srcImage.data[4 * (currentRow + currentCol) + c])
                             + 0.122 * (srcImage.data[4 * (currentRow + nextCol) + c] +
                                        srcImage.data[4 * (currentRow + previousCol) + c] +
                                        srcImage.data[4 * (nextRow + currentCol) + c] +
                                        srcImage.data[4 * (previousRow + currentCol) + c])
                             + 0.076 * (srcImage.data[4 * (nextRow + nextCol) + c] +
                                        srcImage.data[4 * (previousRow + nextCol) + c] +
                                        srcImage.data[4 * (nextRow + previousCol) + c] +
                                        srcImage.data[4 * (previousRow + previousCol) + c]));
                    } // for (c = 0; c <= 3; c++)
                } // if (isColor)
                else { // not color
                    resultImage.data[y * rowSize + x] =
                        (float)
                        ( (0.208 * srcImage.data[currentRow + currentCol])
                         + 0.122 * (srcImage.data[currentRow + nextCol] +
                                    srcImage.data[currentRow + previousCol] +
                                    srcImage.data[nextRow + currentCol] +
                                    srcImage.data[previousRow + currentCol])
                         + 0.076 * (srcImage.data[nextRow + nextCol] +
    ...
    public ModelSimpleImage subsample3dBy2(boolean isColor) {
        ...
        ModelSimpleImage resultImage = new ModelSimpleImage(extents, resolutions,
                                                            isColor);
        ...
        for (int z = 0, bz = 1; bz < zStop; z++, bz += 2) {
            for (int y = 0, by = 1; by < yStop; y++, by += 2) {
                for (int x = 0, bx = 1; bx < xStop; x++, bx += 2) {
                    ...
                    if (isColor) {
                        for (c = 0; c <= 3; c++) {
                            resultImage.data[4 * (z * sliceSize + y * rowSize + x) + c] =
                                (float)
                                (0.0924 *
                                 (srcImage.data[4 * (currentSlice + currentRow + currentCol) + c]) +
                                 0.0560 *
                                 ...
                                 0.0206 * (srcImage.data[4 * (nextSlice + nextRow + nextCol) + c] +
                        ...
                        resultImage.data[z * sliceSize + y * rowSize + x] =
                            (float)
                            (0.0924 * (srcImage.data[currentSlice + currentRow + currentCol])
                             + 0.0560 * (srcImage.data[currentSlice + currentRow + nextCol] +
                             ...
                             + 0.0339 * (srcImage.data[currentSlice + nextRow + nextCol] +
                             ...
                             + 0.0206 * (srcImage.data[nextSlice + nextRow + nextCol] +
        ...
        for (int z = 0; z < zStop; z++) {
            ...
            for (int y = 0, by = 1; by < yStop; y++, by += 2) {
                for (int x = 0, bx = 1; bx < xStop; x++, bx += 2) {
                    ...
                    if (isColor) {
                        for (c = 0; c <= 3; c++) {
                            resultImage.data[4 * (resultZ + y * rowSize + x) + c] =
                                (float)
                                ( (0.208 * srcImage.data[4 * (srcZ + currentRow + currentCol) + c])
                                 + 0.122 * (srcImage.data[4 * (srcZ + currentRow + nextCol) + c] +
                                            srcImage.data[4 * (srcZ + currentRow + previousCol) + c] +
                                            srcImage.data[4 * (srcZ + nextRow + currentCol) + c] +
                                            srcImage.data[4 * (srcZ + previousRow + currentCol) + c])
                                 + 0.076 * (srcImage.data[4 * (srcZ + nextRow + nextCol) + c] +
                                            srcImage.data[4 * (srcZ + previousRow + nextCol) + c] +
                                            srcImage.data[4 * (srcZ + nextRow + previousCol) + c] +
                                            srcImage.data[4 * (srcZ + previousRow + previousCol) + c]));
                        } // for (c = 0; c <= 3; c++)
                    } // if (isColor)
                    else { // not color
                        resultImage.data[resultZ + y * rowSize + x] =
                            (float)
                            ( (0.208 * srcImage.data[srcZ + currentRow + currentCol])
                             + 0.122 * (srcImage.data[srcZ + currentRow + nextCol] +
                                        srcImage.data[srcZ + currentRow + previousCol] +
                                        srcImage.data[srcZ + nextRow + currentCol] +
                                        srcImage.data[srcZ + previousRow + currentCol])
                             + 0.076 * (srcImage.data[srcZ + nextRow + nextCol] +
                                        srcImage.data[srcZ + previousRow + nextCol] +
                                        srcImage.data[srcZ + nextRow + previousCol] +
                                        srcImage.data[srcZ + previousRow + previousCol]));
                    } // else not color
                }
            }
        } // for (int z = 0; z < zStop; z++)
    ...
    */
    ...
    /**
    *	Calculates the min and max values for the image array, so
