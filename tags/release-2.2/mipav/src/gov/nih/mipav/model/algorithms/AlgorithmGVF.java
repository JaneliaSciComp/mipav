...
/**
 *  Snake-like algorithm derivative using BSplines. The algorithm is supplied
 *  a polygon (VOI - contour) and that polygon is allowed to evolve to edge of
 *  the object generated by calculating the the gradient vector field.
 ...
 *  at http://iacl.ece.jhu.edu/~chenyang
 *
...
public class AlgorithmGVF extends AlgorithmBase {
     ...
     */
    public AlgorithmGVF( ModelImage destImage, ModelImage srcImg, float[] sigmas,
            int gvfIterations, int boundaryIterations,
            float k, VOI srcVOI, boolean do25D ) {
     ...
     *   energy is too much extra computation to be worth computing
     *   energy is given by the sum over 0.5*alpha*|first order derivative|**2 + Eext,
     *   where v = -grad(Eext).
     *   The first derivative of the curve and Eext would also have to be calculated
    ...
    private void runSnake( float[] xPoints, float[] yPoints, float[] u, float[] v, Polygon resultGon ) {
        ...
        for ( z = 0; ( z < boundaryIterations ) && ( !finished ) && ( !threadStopped ); z++ ) {
            ...
            for ( i = 0; i < nPts; i++ ) {
                ...
                // The first and second derivatives were seen to be unscaled
                // deriv2Dir   = bSpline.bSplineJetXY(2, index, xPoints, yPoints);

                position = (int) interpPt.x + xDim * (int) interpPt.y;

                newXPts[i + 2] = interpPt.x
                        + getBilinear( position, interpPt.x - (int) interpPt.x, interpPt.y - (int) interpPt.y, extents,
                        u );
    ...
    /**
     *   Prepares the data and runs the algorithm for a 3D image on
            ...
            if ( destFlag == true ) {
                ...
                try {
                    ...
                    MipavUtil.displayError(
                            "AlgorithmGVF: IOException on destImage" + ".importData(slice*length,gvfBuffer,false)" );
                    progressBar.dispose();
                    setCompleted( false );
                    return;
                }
