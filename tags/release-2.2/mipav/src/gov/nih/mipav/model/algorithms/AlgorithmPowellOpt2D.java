...
/**
 *      Runs Powell's method for a 2D image.  Check the parent class comment
...
public class AlgorithmPowellOpt2D extends AlgorithmPowellOptBase {
    ...
    /**
     *	Constructs a new algorithm with the given centers of mass
     *   (needed for setting the transformations), the given cost
     *   function (which was constructed with the proper images),
     *   the initial point we're looking at, some tolerance within that
     *   point to look for the minimum, and the maximum number of iterations.
     ...
     *	@param initial			Initial point to start from, length of 7 for linear
     *                           or more for nonlinear.
     ...
     */
    public AlgorithmPowellOpt2D( AlgorithmBase parent, Point2Dd com, int degreeOfFreedom, AlgorithmOptimizeFunctionBase costFunc,
            double[] initial, double[] tols, int maxIter, boolean _rigid,
            int bracketBound ) {
     ...
     *	along that line using golden search.  It then resets the point and minimizes again,
     *	until the point moves by less than the tolerance.  This method starts with the initial point defined in
    ...
    public void runAlgorithm() {
        ...
        while ( count < maxIterations && keepGoing && nDims > 0 ) {
            ...
            for ( int i = 0; i < nDims && !parent.isThreadStopped(); i++ ) {
                if ( myProgressBar != null ) {
                    myProgressBar.updateValue(
                            progressBegin + ( ( nDims * count + ( i + 1 ) ) * progressMax ) / ( nDims * maxIterations ),
                            activeImage );
                }
        ...
        // Preferences.debug("Exited loop with count= " +count +" > maxIters" +maxIterations +" and keepGoing= " +keepGoing +".\n");
        return;
    ...
    /**
     *	Helper method to take the "point" we've been working with as a vector, and
     *	convert it into a transformation matrix.  The length of the vector should
     *	be equal to the global variable nDims, which in turn was initialized by
     *	the degrees of freedom originally sent to this algorithm.
     *   Therefore, if there are 2 degrees of freedom, we set only the translations; 3
     *   and no rigid means translation and global scaling; 3 and rigid means rotation
     *   and translation; 4 means rotation, translation, and global scaling; 5 means rotation,
     *   translation, and scaling; and 6 means translation, scaling, and skewing.
     *	@param vector	Vector that represented a "point" in the algorithm which needs to be
