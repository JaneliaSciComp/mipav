...
public int readBinary( byte[] data, int count ) throws DICOM_Exception {
    ...
    try {
        ...
        if( actual < 0 ) {
            close();
            if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
                Preferences.debug( DICOM_Util.timeStamper() + " DICOMSocket.readBinary( Trying to read nBytes = " + count + " ) actual = " + actual + " \n");
        ...
        System.out.println("Is connectd ? " + socket.isConnected() + " Is closesd ? " + socket.isClosed());
        */
...
public void writeBinary( byte[] data, int offset, int count ) throws DICOM_Exception {
    ...
    try {
        ...
        close();
        throw new DICOM_Exception( "DICOMSocket sendBinary( " + data + ", " + offset + ", " +
                                                                count + " ):" + e );
    }
...
*/
private final static char convertToHex(int num) {
...
*/
private final static char convertToASCII(int num) {

    if(num < 32 || num > 126) return ('_');
