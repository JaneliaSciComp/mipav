...
import gov.nih.mipav.view.Preferences;
  ...
  *
  *  Methods for Non-linear curve fitting in Java
  *  single class to hold all functions&#59 portable to any calling class
  *  with function to be fitted defined in the subclass 
  *  See &quotNumerical Recipes&quot, WH Press et al, Cambridge University Press, 1988.
  *  Levenberg-Marquardt Method, Chap 15, section 5 p. 683
  *  and Marquardt  D, 1963, J Soc Ind Appl Math, 11:431-441.
  *
  */
...
//	public static final int EXPONENTIAL         = 2;
	...
	public void driver() {
		...
		try {
			...
			/* Nonzero entries of ia give those parameters that are to be fitted.
			   Zero entries of ia give those parameters that are to be held constant.
			   mfit of the ma parameters are to be fitted.  ma - mfit of the 
			   parameters are to be held constant. */
	...
	*/
	...
	/* This routine calls mrqcof and covsrt.
	   Levenberg-Marquardt method, attempting to reduce the value chisquare of a fit
	   ...
	   a[1..ma].  The input array ia[1..ma] indicates by nozero entries those components
	   of a that should be fitted for, and by zero entries those components that 
	   should be held fixed at their input values.  The program returns the current
	   best-fit values for the parameters a[1..ma] and chisq.  The arrays covar[1..ma][1..ma],
	   alpha[1..ma][1..ma] are used as working space during most iterations.  mrqcof calls 
	   fitLine(x1,atry,dyda), fitExponential(x1,atry,dyda), or some other fitting function.
	   mrqcof suplies the x value x1 and the matrix with the current best 
	   guess of the parameters atry[].  function calulates the derivatives dyda[1..ma] 
	   with respect to the fitting parameters atry at x.  function also returns the y value
	   ymod generated by these parameters.
	     
	     The first call to mrqmin sets alamda equal to -1 for initialization.  The routine then 
	   sets alamda = .001.  If a step succeeds chisq becomes smaller and alamda decreases by a
	   factor of 10.  If a step fails alamda grows by a factor of 10.  The routine is called 
	   repeatedly until convergence is achieved.  Then, a final call with alamda = 0 is made
	   to generate the covariance matrix covar[1..ma][1..ma] and the curvature matrix 
	   alpha[1..ma][1..ma]. */
...
/*	public double fitLine(double x1, double atry[], double dyda[]) {
  ...
  */  
    ...
    */
	...
	/**
	*   covsrt - used to rearrange the alpha and covariance matrices into the order of all
	*            ma parameters
	*   @param mat
	*/
    ...
    */
    ...
    /**
    *   gser    - returns the incomplete gamma function P(a,x) evaluated in its series
    *             representation as gamser.  T(a) is also returned
    *   @param a
    *   @param x
    */
    ...
    /**
    *   gcf - returns the incomplete gamma function P(a,x) evaluated in its series
    *         representation as gamser.  T(a) is also returned
    *   @param a
    *   @param x
    */
