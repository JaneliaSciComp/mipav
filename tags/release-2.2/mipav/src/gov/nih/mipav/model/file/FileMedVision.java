...
/**
*   The class reads MedVision files that have been saved as
*   MAC bin files. The MAC files have needed image resolutions encoded in the
*   in the resource section at the end of the file. MedVision also has
*   method of saving images to the Intel world but pixel resolutions are
*   not stored. Here I decode MAC MedVision files. It is easy to write
*   a Intel MedVision reader or one could directly read in the file using
*   using MIPA because they have a fixed offset and can be read in using
*   the RAW file reader.
*
...
public class FileMedVision extends FileBase {
    ...
    */
    public FileMedVision(ViewUserInterface _UI, String fName, String fDir) {
        ...
        fileDir         = fDir;
        if (true){
            progressBar = new ViewJProgressBar("Loading MedVision image",
    ...
    /**
    *   readImage             - reads a MedVision file in MAC format (i.e.,
    *                           decodes resource info)
    ...
    */
    public ModelImage readImage() throws IOException {
        ...
        for ( i = 0; i < fileInfoCom.totalSlices; i++){
            ...
            try {
                reader.readImage(fileInfo.getDataType(),
                                 (int)raFile.getFilePointer(),
                                 extents[0]*extents[1]);
            }
    ...
    */
    ...
    /**
    *   decodeResourceFooter  - This is the tricky function that decodes the
    *                           the resource footer at the end of the MAC file
    *                           The data is coded is very odd manner. I was unable
    *                           to find MAC documentation and decoded it by "hand"
    *   @param endianess        boolean describing byte order
    *   @exception IOException  if there is an error reading the file
    */
    ...
    */
    ...
    /**
    *   copyFileInfoCommon - copy common file info into new header
    *                        Better must exist but this works
    */
