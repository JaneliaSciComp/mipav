...
/**
 *  Model of a Lookup Table (LUT).   At present, one can construct 10 different
 *  LUTs: GRAY, SPECTRUM, HOTMETAL, COOLHOT, SKIN, STRIPED, RED, GREEN, BLUE, GRAY_BR,
 *  and BONE. This class allows the programmer to specify the number of colors for the LUT.
 *  The LUT extents should be 4 x 256 or problems may arise with the differing colormaps
 *  (This class can be extended to handle arbitrary size LUTs but 256 is almost universal).
 *  There are 256 locations with alpha, red, green, and blue values as floats in order to store
 *  both RGB or HSI or etc. The LUTs are calculated from the corresponding transfer
 *  functions for each of a, r, g, and b.
 *
 *  @version 1.0
...
public class ModelLUT
    ...
    /**
     * Special int array where the LUT is packed with alpha in the most significant
     *  byte of the int, followed red, green, and blue.
     */
     ...
     *   @return               the transfer function that describes how to remap the image
     *                         intensities into display values from the LUT.
     */
     ...
     * (blue -> light blue -> green -> yellow -> orange -> red)
     *
     */
    ...
    /**
     * Creates the R, G, and B transfer functions to produce a "Cool hot"
     * scale LUT. (blue -> pink -> red -> orange -> yellow -> white)
     *
     */
     ...
     * (blue -> pink -> red -> orange -> yellow -> white)
     */
    ...
    /**
     *   Calculates the color band (i.e. red, green, blue) for the LUT using the
    ...
    /**
     *   Inverts the LUT using the values stored in the band arrays and
     *   not the transfer functions
     */
    ...
    /**
     * This is a method to export a special int array where the alpha
     * is stored in the most significant byte, then red, green and blue.
     * Note that the transfer function has been applied to it.
     *
     ...
     * This is a method to export a special int array where the alpha
     * is stored in the most significant byte, then red, green and blue.
     * Without the transfer function applied to the LUT
     *
    ...
    /**
     * This is a method to export a 2D float array of the LUT. This method is used by the ViewJComponent
     * show methods to blend between two images.
     *
    ...
    public String toString(boolean displayInHex, boolean displayAlphaValues) {
        ...
        for (int i = 0; i < height; i++) {
            ...
            if (displayInHex) {
                ...
                outStr.append(
                    "[" + alphaValue + Integer.toHexString(get(1, i).intValue()) + ", "
                ...
                outStr.append(
                    "[" + alphaValue + Integer.toString(get(1, i).intValue()) + ", "
    ...
    /**
     * The purpose of this method is to adjust the zero index of the LUT from (0, 0, 0) to (1, 1, 1)
     * The reason is so image A and image B are blended throughout by default.
     */
    ...
    /**
     * The purpose of this method is to adjust the zero index of the LUT from (1, 1, 1) to (0, 0, 0)
     * The reason is so image A and image B are not blended throughout by default.
     */
