 ...
 *   the reference volume, and so on until the first volume is registered
 ...
 *   simply registered to the average of all the volumes.
 *
 ...
 *        Subsampling can be in x, y, and z or in only x and y.
 *        We loop thru a number of iterations equal to the number of volumes - 1
 *        for adjacent mode or equal to the number of volumes in average or reference mode with
 *        one volume as the input volume and one volume as the reference volume.  For each
 *   iteration:
 *   4.)  With the volumes that were subsampled by 8 or 16, we call levelEight.  This function
 ...
 *   9.)  We now call levelFour with the volumes subsampled by 4 and the vectors of minima.  We measure the
 *        costs of the minima on the new volumes and sort them.  We take the top numMinima in each vector
 ...
 *   12.) We now call levelTwo with the volumes subsampled by 2.  We measure the costs of the minima
 *        at the new volumes.  We optimize the best minimum with 7 degrees of freedom, then 9, then 12.
 ...
 *   13.) We call levelOne with the un-subsampled volumes.  At levelOne, one optimization run is performed,
 ...
 *        is used to register the input volume to the reference volume.  The registered input volume
 *        data is imported into inputImage.<br>
 *
 *   Only subsample if 16 or more z slices are present, so that the number of z slices will
 *   not be reduced below 8.
 *
 *
...
public class AlgorithmRegOAR35D extends AlgorithmBase {
    ...
    /** If true use adjacent image for registration. If false use the average volume or
     use the image volume reference number to indicate the image volume to register
     all images to.*/
    ...
    /* Produce 2 output graphs - 1 for 3 rotations and 1 for
     * 3 translations.  Only can be used for DOF == 6 and
     * register to average or reference volume
     */
     ...
     *  intialize the translation. If false, images are pretty much aligned and
     *  don't calculate COG.
     */
    ...
    /** Arrays used for producing graphs for DOF == 6
     * and register to average or reference image
     */
     ...
     * @param refImageNum    If mode == REFERENCE, the volume all other volumes are registered
     *                       to.  If mode == ADJACENT, the first volume used as a reference.
     ...
     * @param doGraph        If true produce 2 output graphs - one for 3 rotations
     *                       and one for 3 translations
     ...
     */
    public AlgorithmRegOAR35D( ModelImage _image, int _costChoice, int _DOF,
            int _interp, int _interp2,
            int mode, int refImageNum,
     ...
     *	@param _coarseRate	Point at which coarse samples should be taken (i.e., every 45 degrees).
     *	@param _fineRate	Point at which fine samples should be taken (i.e., every 15 degrees).
     *    @param doGraph      If true produce 2 output graphs - 1 for 3 rotations and
     *                       one for 3 translations
     *   @param doSubsample  If true subsample
     *   @param fastMode     If true skip subsample and go directly to level 1 optimization
     *    @param _bracketBound The bracket size around the minimum in multiples of unit_tolerance
     *                         for the first iteration of Powell's algorithm.
     ...
     *   @param _numMinima Number of minima from level 8 to test at level 4
     */
    public AlgorithmRegOAR35D( ModelImage _image, ModelImage _inputWeight,
            ...
            float _coarseRate, float _fineRate,
            boolean doGraph,
            boolean doSubsample, boolean fastMode, int _bracketBound,
     ...
     *   of the two images after the xyz isotropic transformation will be the same in the x, y,
     *   and z dimensions.  That resolution will equal the minimum resolution.
     ...
     *   down to the smallest level; if it is too big, it will be subsampled to 8 or 16.
     *   The same is done with the weight image if necessary.  The program loops thru
     *   levelEight, levelFour, levelTwo, and levelOne with one volume as the reference
     *   volume and one volume as the input volume.  The function levelEight
     *   is called with the volumes subsampled by 8 or 16; it returns two vectors with minima.  Then
     *   the function levelFour is called with volumes subsampled by 4 and the two vectors; it
     *   returns one vector of minima.  The function levelTwo is called with volumes subsampled
     ...
     *   minimum, which is used to register the input volume to the reference volume.  The
     *   registered input volume data is imported into inputImage.
     */
    public void runAlgorithm() {
        ...
        if ( ( iResols[0] == iResols[1] ) && ( iResols[0] == iResols[2] ) ) {
            ...
            transform = new AlgorithmTransform( inputImage, new TransMatrix( 4 ), interp, resIso[0], resIso[1],
                    resIso[2], extentsIso[0], extentsIso[1], extentsIso[2], false, true, false );
        ...
        if ( useOutsideReferenceVolume && resample ) {
            transform = new AlgorithmTransform( outsidePreReferenceVolume, new TransMatrix( 4 ), interp, resIso[0],
                    resIso[1], resIso[2], extentsIso[0], extentsIso[1], extentsIso[2], false, true, false );
            transform.setProgressBarVisible( false );
            transform.run();
        ...
        if ( weighted ) {
            ...
            if ( !fastMode ) {
                if ( simpleWeightR_1.dataSize > subMinFactor && doSubsample && simpleWeightR_1.zDim >= minimumZForSub ) {
    ...
    private Vector[] levelEight( ModelSimpleImage ref, ModelSimpleImage input ) {
         ...
         */
        Preferences.debug(
                "Total number of positions at coarse sampling to be tested: " + coarseNum * coarseNum * coarseNum
                + ".\n" );
        Preferences.debug(
                "Total number of positions at fine sampling to be tested: " + fineNum * fineNum * fineNum + ".\n" );
        ...
        MatrixListItem[][][] matrixList = new MatrixListItem[fineNum][fineNum][
                fineNum];
        ...
        for ( int i = 0; i < fineNum; i++ ) {
            for ( int j = 0; j < fineNum; j++ ) {
                for ( int k = 0; k < fineNum; k++ ) {
                    ...
                    for ( int itest = -1; itest <= 1 && minimum; itest++ ) { // as long as still possible minimum, check neighbors one degree off
    ...
    private Vector levelFour( ModelSimpleImage ref, ModelSimpleImage input,
        ...
        if ( minimumIndex < total ) {
            ...
            Preferences.debug(
                    "New minimum is from optimized minima at index " + ( minimumIndex - total + 1 ) + " and is "
                    + currentMinimum + "\n" );
        }
        ...
        for ( int j = 0; j < 7 && !threadStopped; j++ ) {
            for ( int i = 0; i < ( 2 * total ) && !threadStopped; i++ ) {
                ...
                if ( j == 0 ) {
                    ...
                    Preferences.debug(
                            " by adding " + sign * fineDelta + " to initial[" + ( (int) ( j - 1 ) / 2 ) + "] ("
        ...
        Preferences.debug(
                "Best minimum from level four is a perturbed version of " + ( minimumIndex + 1 ) + " and is "
                + currentMinimum + ".\n" );
        ...
        for ( int z = 0, bz = 1; bz < zStop; z++, bz += 2 ) {
            for ( int y = 0, by = 1; by < yStop; y++, by += 2 ) {
                for ( int x = 0, bx = 1; bx < xStop; x++, bx += 2 ) {
                    ...
                    if ( isColor ) {
                        for ( c = 0; c <= 3; c++ ) {
                            resultImage.data[4 * ( z * sliceSize + y * rowSize + x ) + c] = (float)
                                    ( 0.0924 * ( srcImage.data[4 * ( currentSlice + currentRow + currentCol ) + c] )
                                    + 0.0560
                                            * ( srcImage.data[4 * ( currentSlice + currentRow + nextCol ) + c]
                                                    + srcImage.data[4 * ( currentSlice + currentRow + previousCol ) + c]
                                                    + srcImage.data[4 * ( currentSlice + nextRow + currentCol ) + c]
                                                    + srcImage.data[4 * ( currentSlice + previousRow + currentCol ) + c]
                                                    + srcImage.data[4 * ( nextSlice + currentRow + currentCol ) + c]
                                                    + srcImage.data[4 * ( previousSlice + currentRow + currentCol ) + c] )
                                                    + 0.0339
                                                            * ( srcImage.data[4 * ( currentSlice + nextRow + nextCol ) + c]
                                                                    + srcImage.data[4 * ( currentSlice + previousRow + nextCol ) + c]
                                                                    + srcImage.data[4 * ( currentSlice + nextRow + previousCol ) + c]
                                                                    + srcImage.data[4 * ( currentSlice + previousRow + previousCol ) + c]
                                                                    + srcImage.data[4 * ( nextSlice + currentRow + nextCol ) + c]
                                                                    + srcImage.data[4 * ( previousSlice + currentRow + nextCol ) + c]
                                                                    + srcImage.data[4 * ( nextSlice + currentRow + previousCol ) + c]
                                                                    + srcImage.data[4 * ( previousSlice + currentRow + previousCol ) + c]
                                                                    + srcImage.data[4 * ( nextSlice + nextRow + currentCol ) + c]
                                                                    + srcImage.data[4 * ( previousSlice + nextRow + currentCol ) + c]
                                                                    + srcImage.data[4 * ( nextSlice + previousRow + currentCol ) + c]
                                                                    + srcImage.data[4 * ( previousSlice + previousRow + currentCol ) + c] )
                                                                    + 0.0206
                                                                            * ( srcImage.data[4 * ( nextSlice + nextRow + nextCol ) + c]
                                                                                    + srcImage.data[4 * ( previousSlice + nextRow + nextCol ) + c]
                                                                                    + srcImage.data[4 * ( nextSlice + previousRow + nextCol ) + c]
                                                                                    + srcImage.data[4 * ( previousSlice + previousRow + nextCol ) + c]
                                                                                    + srcImage.data[4 * ( nextSlice + nextRow + previousCol ) + c]
                                                                                    + srcImage.data[4 * ( previousSlice + nextRow + previousCol ) + c]
                                                                                    + srcImage.data[4 * ( nextSlice + previousRow + previousCol ) + c]
                                                                                    + srcImage.data[4 * ( previousSlice + previousRow + previousCol ) + c] ) );
                        } // for (c = 0; c <= 3; c++)
                    } // if (isColor)
                    else { // black and white
                        resultImage.data[z * sliceSize + y * rowSize + x] = (float)
                                ( 0.0924 * ( srcImage.data[currentSlice + currentRow + currentCol] )
                                + 0.0560
                                        * ( srcImage.data[currentSlice + currentRow + nextCol]
                                                + srcImage.data[currentSlice + currentRow + previousCol]
                                                + srcImage.data[currentSlice + nextRow + currentCol]
                                                + srcImage.data[currentSlice + previousRow + currentCol]
                                                + srcImage.data[nextSlice + currentRow + currentCol]
                                                + srcImage.data[previousSlice + currentRow + currentCol] )
                                                + 0.0339
                                                        * ( srcImage.data[currentSlice + nextRow + nextCol]
                                                                + srcImage.data[currentSlice + previousRow + nextCol]
                                                                + srcImage.data[currentSlice + nextRow + previousCol]
                                                                + srcImage.data[currentSlice + previousRow + previousCol]
                                                                + srcImage.data[nextSlice + currentRow + nextCol]
                                                                + srcImage.data[previousSlice + currentRow + nextCol]
                                                                + srcImage.data[nextSlice + currentRow + previousCol]
                                                                + srcImage.data[previousSlice + currentRow + previousCol]
                                                                + srcImage.data[nextSlice + nextRow + currentCol]
                                                                + srcImage.data[previousSlice + nextRow + currentCol]
                                                                + srcImage.data[nextSlice + previousRow + currentCol]
                                                                + srcImage.data[previousSlice + previousRow + currentCol] )
                                                                + 0.0206
                                                                        * ( srcImage.data[nextSlice + nextRow + nextCol]
                                                                                + srcImage.data[previousSlice + nextRow + nextCol]
                                                                                + srcImage.data[nextSlice + previousRow + nextCol]
                                                                                + srcImage.data[previousSlice + previousRow + nextCol]
                                                                                + srcImage.data[nextSlice + nextRow + previousCol]
                                                                                + srcImage.data[previousSlice + nextRow + previousCol]
                                                                                + srcImage.data[nextSlice + previousRow + previousCol]
                                                                                + srcImage.data[previousSlice + previousRow + previousCol] ) );
                    }
                }
            }
        }
    ...
    private static ModelSimpleImage subSampleBy2XY( ModelSimpleImage srcImage,
            ModelSimpleImage resultImage, boolean isColor ) {
