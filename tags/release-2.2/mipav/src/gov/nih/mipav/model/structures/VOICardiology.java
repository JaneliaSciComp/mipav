...
public class VOICardiology extends VOIBase {
     ...
     */
    private final double getLength(float x0, float y0, float x1, float y1, float[] res) {
        double length;
    ...
    public void trimPoints(double constraint, boolean tFlag) {
        ...
        while (flag == true) {
            ...
            for (i = 0; i < end - 2; i++ ) {
                ...
                if (testDistance(Math.round(bx), Math.round(ax), Math.round(cx), Math.round(by), Math.round(ay), Math
                        .round(cy), constraint)) {
                    removeElementAt(i + 1);
                    end = size();
                    i = i - 1;
    ...
    public double calcAverageLength(int sectionNum, int dimX, int dimY, float[] resolutions, ViewUserInterface ui) {
                    ...
                    if (foundInf) {
                        ...
                        if (intersectionPtInfarctions[j].x != centerPt.x
                            ...
                            distanceInfarction = MipavMath.length(intersectionPtInfarctions[j].x,
                                    intersectionPtInfarctions[j].y, centerPt.x, centerPt.y, resolutions);
                ...
                for (int m = 0; m < foundIntersectionInfarction.length; m++ ) {
                    if (foundIntersectionInfarction[m]) {
                        distanceInfarction = MipavMath.length(intersectionPtInfarctions[m].x,
                                intersectionPtInfarctions[m].y, centerPt.x, centerPt.y, resolutions);
        ...
        ui.getMessageFrame().getData().append(
                "Section: " + sectionNum + " average length (inner to outer): " + differenceTotal
                        / numIntersectionsEntire + "\n");
        for (int b = 0; b < numInfSections; b++ ) {
            ...
            ui.getMessageFrame().getData().append(
                    "\t Infarction section (clockwise): " + b + " average length: " + differenceTotalInf[b]
    ...
    public void drawSelf(float zoomX, float zoomY, float resolutionX, float resolutionY, float originX, float originY,
        ...
        for (int i = 0; i < numSections; i++ ) {
            ...
            if (activeArray[secondPt.getSection()]) {
                g.setColor(Color.yellow);
            } else if ( (i > 0 && activeArray[secondPt.getSection() - 1]) || (i == 0 && activeArray[numSections - 1])) {
                g.setColor(Color.white);
            } else {
                g.setColor(Color.gray);
            }
            g.drawLine((int) Math.round(firstPt.x * zoomX * resolutionX), (int) Math.round(firstPt.y * zoomY
                    * resolutionY), (int) Math.round(secondPt.x * zoomX * resolutionX), (int) Math.round(secondPt.y
                    * zoomY * resolutionY));
            if (showCenter) {
                g.setColor(Color.lightGray);
                g.drawLine((int) Math.round(centerPt.x * zoomX * resolutionX), (int) Math.round(centerPt.y * zoomY
                        * resolutionY), (int) Math.round(firstPt.x * zoomX * resolutionX), (int) Math.round(firstPt.y
                        * zoomY * resolutionY));
            }
        }
        g.setColor(Color.CYAN);
        ...
        if (active == true) {
            ...
            for (j = 0; j < size(); j++ ) {
                ...
                if (nearPoint == j) { // Highlight Active point
                    ...
                    // g.fillRect( (int) ( gon.xpoints[j] - 1.5 + 0.5f ), (int) ( gon.ypoints[j] - 1.5 + 0.5f ), 3, 3 );
                    // g.setColor( Color.black );
                    // g.fillRect( (int) ( gon.xpoints[j] - 1.5 + 0.5f ), (int) ( gon.ypoints[j] - 1.5 + 0.5f ), 4, 4 );
                    // g.setColor( Color.white );
    ...
    public float[] calcAverageIntensity(float[] data, int sectionNum, int imgXDim, ViewUserInterface ui) {
        ...
        ui.getMessageFrame().getData().append(
                "Section: " + sectionNum + "\n\taverage intensity: " + results[0] + "\n\ttotal num pixels: " + numPix
                        + "\n\tinfarction average intensity: " + results[1] + "\n");
        return results;
    }
    ...
    private int getIndex(int type, int section, boolean isIntersection, boolean isShared) {
        ...
        for (int i = 0; i < size(); i++ ) {
            tempPt = (CardioPoint3Df) elementAt(i);
            if (tempPt.getType() == type && tempPt.getSection() == section && tempPt.isIntersection() == isIntersection
                    && tempPt.isShared() == isShared) {
                return i;
            }
        }
        return index;
    ...
    private int getSharedIndex(int type, int section, Point3Df sharedPt) {
        ...
        for (int i = 0; i < size(); i++ ) {
            tempPt = (CardioPoint3Df) elementAt(i);
            if (tempPt.isShared() && tempPt.getType() == type && tempPt.x == sharedPt.x && tempPt.y == sharedPt.y) {
                return i;
            }
        }
        return index;
     ...
     */
    private final float area2(Point3Df ptA, Point3Df ptB, float ptCx, float ptCy) {
