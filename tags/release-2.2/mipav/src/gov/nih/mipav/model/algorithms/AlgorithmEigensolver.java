...
/**
 * A numerical eigensolver for symmetric matrices.  The code is a minor
 * variation of that found in Numerical Recipes in C.
 */
 
public class AlgorithmEigensolver
{
    /**
     * Create an eigensolver object.  The matrices that the solver
     * processes must be square and symmetric.
     *
     ...
     * @param iRow the row index (0 <= iRow < iSize)
     * @param iCol the column index (0 <= iCol < iSize)
     * @param fValue the value to add to the matrix value in that row and
     *   column
     */
    ...
    /**
     * Solve for the eigenvalues and eigenvectors of the matrix.  This
     * function should be called only after the matrix entries are set and
     * the matrix is ensured to be symmetric.  The eigenvalues are stored
     * in increasing order and may be accessed by getEigenvalue(i).  The
     * corresponding eigenvectors are stored as columns of a matrix.  The
     * eigenvector for the i-th eigenvalue is stored in the i-th column of
     * the matrix.  Eigenvectors are accessed by getEigenvector(iRow,iCol).
    ...
    /**
     * Householder reduction to convert a symmetric matrix to a tridiagonal
     * matrix.  This code is a minor modification of that from Numerical
     * Recipes in C.
     */
    ...
    /**
     * Iteratively reduce the tridiagonal matrix obtained from Householder
     * reduction to a diagonal matrix.  This code is a minor modification of
    ...
    protected boolean reduceQLAlgorithm ()
    {
        ...
        for (int i0 = 0; i0 < m_iSize; i0++)
        {
            ...
            for (i1 = 0; i1 < iMaxIter; i1++)
            {
                ...
                {
                    double fTmp = Math.abs(m_afDiag[i2]) +
    ...
    protected double m_aafMat[/* m_iSize */][/* m_iSize */];
    
    // Storage for the eigenvalues of the matrix.
    protected double m_afDiag[/* m_iSize */];
    
    // Temporary storage for the reduction to a diagonal matrix.
    protected double m_afSubd[/* m_iSize */];
}
