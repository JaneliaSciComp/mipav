...
/**
 *   This class extends the generic buffer class ModelStorageArray and
 *   is used to store n-dimensional images and buffer class that supports
 *   boolean, byte, short, int, long, float, double, etc. data types. After
 *   the buffer is created the minimum and maximum parameters are
 *   calculated. ModelImage is a specific buffer to addressing issues
 *   relating to images.
 *
...
public class ModelImage extends ModelStorageBase {
    ...
    /** Reference to a transformation matix that can be applied to the image and
     *   tranform (warp) the image in some manner.
     */
    ...
    /** List of frames where this image is displayed. This is an important list, for example
     *  when the LUT of table is changed the list is used to notify all the frames displaying
     *  this image that they should update their view of the image. The transient keyword is
     *  used to indicate that the frame list should NOT be cloned when the image is cloned.
     *  The new cloned image should build up its own list of frame(s) where it is displayed. */
    ...
    /**
     *   Mask is a binary object that is true interior to a VOI and false otherwise
     *   used in algorithms to process only on VOIs if indicated by the user.
     */
    private BitSet mask;
    ...
    /**
     * ImageName is patient's name when using DICOM images.  It is filename for any other image
     * something other than filename must be used for DICOM because one DICOM image is made of
     * many seperate files. ** Not necessarily the file name.
     */
    private String imageName;

    /**
     *   The user interface has a vector of all image models loaded into
     *   MIPAV. I put the reference to it here so that when an image is created it
     *   can added tothe vector in the user interface.
     */
     ...
     *
     * The only difference with this constructor is that it does not require the ViewUserInterface param.
     */
    ...
    /**
     *   Copies the image and all data associated with the image (i.e. VOIs).
     *   Sets the name of the new image to newName.
     *   @param newName  String containing the name for the cloned image.
     *                   If null then 'this' image name is appended with "_clone".
     *
    ...
    /**
     *   Copies the image and all data associated with the image (i.e. VOIs).
     *   Invokes the clone(String newName) method with newName set to null;
     *   @return the new copy of the image
     */
    public Object clone() {
     ...
     *                   if DICOM image then ImageName is the patients' name
     *                   else, imageName is the file name (see the contructor for more)
     */
    ...
    /**
     *   Calculates translation offset for transforming image
    ...
    /**
     *   Calculates translation offset for transforming image
     *   about the center of the image in the resolution space.
    ...
    /**
     * Accessor that sets the name of the image. NOT TO BE USED BY ANYONE
     * EXCEPT ViewUserInterface.registerImage. Use setImageName instead!!!
     * @param name           the String representing the filename
     */
    ...
    /**
     *   Give the image a new image name, updates frame (if not null), and
     *   file infos
     */
     ...
     * @param _mask mask in the form of a BitSet, 1 indicates pixel should be processed
     *                                            0 indicates pixel should not be processed
     */
    ...
    /**
     * Sets the mask which indicate which pixels/voxels to process.
     * @param mask mask in the form of a BitSet, 1 indicates pixel should be processed
     *                                            0 indicates pixel should not be processed
     */
    ...
    /**
     *   Add a listener to this class so that notifyListener can
     *   be used to notify all listeners to update the display of the image.
     ...
     *   @param forceShow  force the display method(s) to reload image data and display image
     *                     slower but needed if image model changes.
     *   @param alphaBlend indicates the amount of blending between two images (image 1's blending value)
     *                     1.0 - all of image 1; 0.5 - half image 1 and half image 2
     *   @param interpMode image interpolation method (Nearest or Smooth)
     */
    public void notifyImageDisplayListeners( ModelLUT LUT, boolean forceShow,
            int alphaBlend, int interpMode ) {
        ...
        for ( int i = 0; i < frameList.size(); i++ ) {
            if ( ( frameList.elementAt( i ) instanceof ViewJFrameBase ) ) {
                ...
                if ( this == imgA ) {
                    ( (ViewImageUpdateInterface) frameList.elementAt( i ) ).updateImages( LUT, null, forceShow,
                            interpMode );
                } else if ( this == imgB ) {
                    ( (ViewImageUpdateInterface) frameList.elementAt( i ) ).updateImages( null, LUT, forceShow,
                            interpMode );
                }
            }
        }
     ...
     *   @param RGBT       ModelRGB
     */
    public void notifyImageDisplayListeners( boolean forceShow, int alphaBlend,
            ModelRGB RGBT ) {
    ...
    public void notifyImageDisplayListeners( ModelLUT LUT, boolean forceShow ) {
        ...
        if ( frameList != null ) {
            for ( int i = 0; i < frameList.size(); i++ ) {
                ...
                } /* LUT update of a non-ModelImage data strucuture: */ else if ( ( frameList.elementAt( i )
                        instanceof
                        gov.nih.mipav.view.renderer.surfaceview.brainflattenerview.JPanelBrainSurfaceFlattener ) ) {
                    ModelImage imgA = ( (gov.nih.mipav.view.renderer.surfaceview.brainflattenerview.JPanelBrainSurfaceFlattener) frameList.elementAt( i ) ).getImageA();
                    ModelImage imgB = ( (gov.nih.mipav.view.renderer.surfaceview.brainflattenerview.JPanelBrainSurfaceFlattener) frameList.elementAt( i ) ).getImageB();
    ...
    /**
     *   Used to notify all frames that display this image model that this image's
     *   extents have changed.  The display of this image may also need to be updated.
     */
    ...
    */
    ...
    /**
     *   Deep copies the file type info from the fromImage to the current image object.
     *   But does not copy over image buffer type.
     *   @param fromImage   image from which to copy file type info
     */
    ...
    /**
     *   Fixes file information to resultant image structure. When one image's file information is copied
     *   to anothers this method sets the modality to OTHER and file directory to an empty string.
     *   @param image       source image
     *   @param resultImage resultant image
     */
    ...
    public short[] generateVOIMask( short[] mask, int index, boolean XOR ) {
        ...
        if ( voiVector.size() != 0 ) {
            return ( ( (VOI) voiVector.elementAt( index ) ).createShortMask( getExtents()[
                    0], getExtents()[1], mask, XOR ) );
        } else {
            return null;
        }
    }
    ...
    /**
     * Forms a single VOI structure from all the
     *             VOIs presently loaded in the imageModel
     */
    public void groupVOIs() {
        ...
        for ( i = 0; i < nVOIs; i++ ) {
            if ( voiVector.VOIAt( i ).getCurveType() == VOI.CONTOUR && voiVector.VOIAt( i ).isActive() ) {
                if ( newVOI == null ) {
                    newVOI = new VOI( (short) 0, "joinedVOI.voi", slices, VOI.CONTOUR, -1.0f );
            ...
            } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POLYLINE && voiVector.VOIAt( i ).isActive() ) {
                if ( newPLineVOI == null ) {
            ...
            } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.LINE && voiVector.VOIAt( i ).isActive() ) {
                if ( newLineVOI == null ) {
            ...
            } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POINT && voiVector.VOIAt( i ).isActive() ) {
                if ( newPtVOI == null ) {
                ...
                for ( j = 0; j < nSlices; j++ ) {
                    ...
                    for ( k = nPoints - 1; k >= 0; k-- ) {
                        if ( ( (VOIBase) voiVector.VOIAt( i ).getCurves()[j].elementAt( k ) ).isActive() ) {
                            point1[0] = points[k];
                            newPtVOI.importCurve( point1, j );
                            voiVector.VOIAt( i ).getCurves()[j].removeElementAt( k );
                        }
                    }
            ...
            } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.PROTRACTOR && voiVector.VOIAt( i ).isActive() ) {
        ...
        int newSize = voiVector.size();
        int counter;
        Vector[] curves = null;
        for ( i = newSize - 1; i >= 0; i-- ) {
            counter = 0;
            curves = voiVector.VOIAt( i ).getCurves();
            for ( j = 0; j < curves.length; j++ ) {
                counter += curves[j].size();
            }
            if ( counter == 0 ) {
                //System.err.println("removed one");
                voiVector.removeElementAt( i );
            }
        }
    ...
    public void groupVOIs( ViewVOIVector newVOIVector, int[] where, String name ) {
        ...
        for ( i = 0; i < nVOIs; i++ ) {
            if ( newVOIVector.VOIAt( i ).getCurveType() == VOI.CONTOUR ) {
                if ( newVOI == null ) {
                    newVOI = new VOI( (short) 0, name, slices, VOI.CONTOUR, -1.0f );
                }
        ...
        for ( voiNum = 0; voiNum < voiVector.size(); voiNum++ ) {
            if ( ( (VOI) voiVector.elementAt( voiNum ) ).getName().equals( name ) ) {
                for ( sliceNum = 0; sliceNum < ( (VOI) voiVector.elementAt( voiNum ) ).getCurves().length; sliceNum++ ) {
                    for ( curveNum = 0; curveNum < ( (VOI) voiVector.elementAt( voiNum ) ).getCurves()[sliceNum].size(); curveNum++ ) {
                        ( (VOIBase) ( (VOI) voiVector.elementAt( voiNum ) ).getCurves()[
                                sliceNum].elementAt( curveNum ) ).setLabel( String.valueOf( index++ ) );
                    }
                }
    ...
    public void ungroupVOIs() {
        ...
        nVOIs = voiVector.size();
        for ( i = 0; i < nVOIs; i++ ) {
            if ( voiVector.VOIAt( i ).isActive() == true ) {
                if ( voiVector.VOIAt( i ).getCurveType() == VOI.CONTOUR ) {
                    nSlices = voiVector.VOIAt( i ).getCurves().length;
                    for ( j = 0; j < nSlices; j++ ) {
                        numContours += voiVector.VOIAt( i ).getCurves()[j].size();
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POINT ) {
                    nSlices = voiVector.VOIAt( i ).getCurves().length;
                    for ( j = 0; j < nSlices; j++ ) {
                        numPoints += voiVector.VOIAt( i ).exportPoints( j ).length;
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POLYLINE ) {
                    nSlices = voiVector.VOIAt( i ).getCurves().length;
                    for ( j = 0; j < nSlices; j++ ) {
                        numPLines += voiVector.VOIAt( i ).getCurves()[j].size();
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.LINE ) {
                    nSlices = voiVector.VOIAt( i ).getCurves().length;
                    for ( j = 0; j < nSlices; j++ ) {
                        numLines += voiVector.VOIAt( i ).getCurves()[j].size();
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.PROTRACTOR ) {
                    nSlices = voiVector.VOIAt( i ).getCurves().length;
                    for ( j = 0; j < nSlices; j++ ) {
                        numProtractors += voiVector.VOIAt( i ).getCurves()[j].size();
                    }
                }
            } // if (voiVector.VOIAt(i).isActive() == true)
        } // for (i = 0; i < nVOIs; i++)

        if ( ( numContours == 0 ) && ( numPoints == 0 ) && ( numPLines == 0 ) && ( numLines == 0 )
                && ( numProtractors == 0 ) ) {
            ...
            if ( voiVector.VOIAt( i ).isActive() == true ) {
                if ( voiVector.VOIAt( i ).getCurveType() == VOI.CONTOUR ) {
                    ...
                    for ( j = 0; j < nSlices; j++ ) {
                        nContours = contours[j].size();
                        for ( k = 0; k < nContours; k++ ) {
                            newVOI[n].getCurves()[j].addElement( contours[j].elementAt( k ) );
                            n++;
                        }
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POLYLINE ) {
                    ...
                    for ( j = 0; j < nSlices; j++ ) {
                        nContours = contours[j].size();
                        for ( k = 0; k < nContours; k++ ) {
                            newPLineVOI[nPLine].getCurves()[j].addElement( contours[j].elementAt( k ) );
                            nPLine++;
                        }
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.LINE ) {
                    ...
                    for ( j = 0; j < nSlices; j++ ) {
                        nContours = contours[j].size();
                        for ( k = 0; k < nContours; k++ ) {
                            newLineVOI[nLine].getCurves()[j].addElement( contours[j].elementAt( k ) );
                            nLine++;
                        }
                    }
                } else if ( voiVector.VOIAt( i ).getCurveType() == VOI.POINT ) {
                    ...
                    for ( j = 0; j < nSlices; j++ ) {
                        nContours = contours[j].size();
                        for ( k = 0; k < nContours; k++ ) {
                            newProtractorVOI[nProtractor].getCurves()[j].addElement( contours[
                                    j].elementAt( k ) );
                            nProtractor++;
                        }
                    }
        ...
        for ( i = 0; i < numContours; i++ ) {
            voiVector.addElement( newVOI[i] );
        }
        for ( i = 0; i < numPoints; i++ ) {
            voiVector.addElement( newPtVOI[i] );
        }
        for ( i = 0; i < numPLines; i++ ) {
            voiVector.addElement( newPLineVOI[i] );
        }
        for ( i = 0; i < numLines; i++ ) {
            voiVector.addElement( newLineVOI[i] );
        }
        for ( i = 0; i < numProtractors; i++ ) {
            voiVector.addElement( newProtractorVOI[i] );
        }
     ...
     *   @param  z          index of file information -- each image slice can have separate
     *                      file information (i.e. like DICOM).
     ...
     */
    public void displayAboutInfo( JDialogBase dialog, int z, int t,
            boolean dicom, boolean xml ) {
        int index;
     ...
     *                   Different from super(...) in that  this method also
     *                   sets the imageName.
     *
     *                   <p>DICOM images set their image name based on the
     *                   patient name held in the first slice
     *
     * @param fInfo      fileInfo structure.
     */
    //public void setFileInfo(FileInfoBase fInfo, int i)  {
    //    super.setFileInfo(fInfo, i);        // make sure the ModelStorageBase sets file info first

    // doing this when reading in, not sure it's necessary here.

    // DICOM files must be given an imagename different from its filename because
    // DICOM images are made of many different files.
    ...
    public void anonymize( boolean[] list, boolean doRename ) {
        ...
        if ( this.isDicomImage() ) { // if image is DICOM,
            if ( getNDims() == 2 ) { // and if image is a single slice
                ...
                this.setFileInfo( fileInfo[0], 0 ); // and then make sure (by resetting) the fileInfo in this image is the same as the sanitised version
                ...
                for ( i = 0; i < getExtents()[2]; i++ ) { // then for all slices in this image,
                    ...
                    this.setFileInfo(
                            fileInfo[i], i ); // and then make sure (by resetting) the ith fileInfo in this image is the same as the sanitised version
                }
            }
    ...
    /**
     *   Save the image to a file. The file type the image is to be save in is
     ...
     */
    public boolean saveImage( String directory, String fileName, int fileType,
            boolean isActive ) {
     ...
     *
     *   @param composite if true make a composite matrix of the by multipling this
     *           matrix with the one to be read from the file.
     *           If false replace this object matrix with a new matrix read from the file.
     *
     */
     ...
     *  R = Right - first axis is positive to the right
     *  A = Anterior - second axis is positive to the anterior
     *  S = Superior - third axis is positive to the superior
     ...
     *  L = Left - first axis is positive to the left
     *  P = Posterior - second axis is positive to the posterior
     *  S = Superior - third axis is positive to the superior
     *	@param	x	Absolute x value in slice.
     *	@param	y	Absolute y value in slice.
     *	@param  z	Absolute z value in slice.
    ...
    public String toString() {
        ...
        for ( int i = 0; i < getNDims(); i++ ) {
            s += "\n\t Units:      \t" + FileInfoBase.getUnitsOfMeasureAbbrevStr
                    ( getFileInfo()[0].getUnitsOfMeasure()[0] ); // possibly expand to see all measurements
        }
        s += "\n\t Orientation:\t" + FileInfoBase.getImageOrientationStr( getImageOrientation() );
        for ( int i = 0; i < getNDims(); i++ ) {
            s += "\n\t Axis: " + i + "      \t"
                    + FileInfoBase.getAxisOrientationStr( fileInfo[0].getAxisOrientation( i ) );
        }
        //s += "\n\t Orientation:\t" + FileInfoBase.getImageOrientationStr(getImageOrientation());
        for ( int i = 0; i < getNDims(); i++ ) {
     ...
     * Performs bounds checking and returns a full plane of the image (up to the slicesize * 2).
    ...
    /**
     *  If no LUT or RGB color table is defined, this returns the packed int
     * value for the color at iIndexs for the input ModelImage kImage:
