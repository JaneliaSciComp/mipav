 ...
 * cubes, assuming a linear interpolation on the cubes, and
 * extracting triangular level sets for those cubes.  The resulting
 * level surface is a triangle mesh.
 */
...
public class ModelSurfaceExtractorCubes {
    ...
    public ModelSurfaceExtractorCubes(int iXBound, int iYBound, int iZBound, int[] aiData, float fXDelta, float fYDelta,
                                      float fZDelta, int[] direction, float[] startLocation,
     ...
     * containing the input point is determined.  A weighted combination
     * of the gradient values at the vertices is used for the interpolated
     * value.
    ...
    public ModelTriangleMesh getLevelSurface(int iLevel, ViewJProgressBar progressBar) {
        // The extraction assumes linear interpolation by decomposition of
        // image domain into cubes.
    ...
    /**
     * Called to add the specified point to the hashmap<Point3f,Integer>.
     * If the specified point already exists in the hashmap, then it is not
     * added.  If it is added, the Integer value of the hashmap is set
     * to the order of insertion index.  The Integer value of this point
     * in the hashmap, if added or if already found in the hashmap, is
     * returned.
     * @param kV Point3f sample space coordinates of the point
     * @param kVMap HashMap hashmap with Point3f key and Integer value
     * for adding the point if not already present
     * @return int insertion order index for the specified point
     */
    static private int addVertex (Point3f kV, HashMap kVMap) {
    ...
    /**
     * Determine which edges of the voxel are intersected by the
     * level surface.
     ...
     * @param rkTable VETable vertex-edge table containing the point
     * of intersection for edge intersected by the level surface
     * @return int bit mask containing flag indicating which edges
     * of the voxel are intersected by the level surface
     */
     ...
     * @param iY int y sample space coordinate for "bottom/left" voxel corner
     * @param iZ int z sample space coordinate for "bottom/left" voxel corner
     * @param iType int bit mask containing flag indicating which edges
     * of the voxel are intersected by the level surface
     * @param rkTable VETable vertex-edge table containing the point
     * of intersection for edge intersected by the level surface
     */
    private void getXMinEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMIN_YMIN)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMIN_YMAX)) iFaceType |= 0x02;
        if (0 != (iType & EB_XMIN_ZMIN)) iFaceType |= 0x04;
        if (0 != (iType & EB_XMIN_ZMAX)) iFaceType |= 0x08;
    ...
    private void getXMaxEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMAX_YMIN)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMAX_YMAX)) iFaceType |= 0x02;
        if (0 != (iType & EB_XMAX_ZMIN)) iFaceType |= 0x04;
        if (0 != (iType & EB_XMAX_ZMAX)) iFaceType |= 0x08;
    ...
    private void getYMinEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMIN_YMIN)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMAX_YMIN)) iFaceType |= 0x02;
        if (0 != (iType & EB_YMIN_ZMIN)) iFaceType |= 0x04;
        if (0 != (iType & EB_YMIN_ZMAX)) iFaceType |= 0x08;
   ...
   private void getYMaxEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMIN_YMAX)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMAX_YMAX)) iFaceType |= 0x02;
        if (0 != (iType & EB_YMAX_ZMIN)) iFaceType |= 0x04;
        if (0 != (iType & EB_YMAX_ZMAX)) iFaceType |= 0x08;
    ...
    private void getZMinEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMIN_ZMIN)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMAX_ZMIN)) iFaceType |= 0x02;
        if (0 != (iType & EB_YMIN_ZMIN)) iFaceType |= 0x04;
        if (0 != (iType & EB_YMAX_ZMIN)) iFaceType |= 0x08;
    ...
    private void getZMaxEdges(int iX, int iY, int iZ, int iType, VETable rkTable) {
        int iFaceType = 0;
        if (0 != (iType & EB_XMIN_ZMAX)) iFaceType |= 0x01;
        if (0 != (iType & EB_XMAX_ZMAX)) iFaceType |= 0x02;
        if (0 != (iType & EB_YMIN_ZMAX)) iFaceType |= 0x04;
        if (0 != (iType & EB_YMAX_ZMAX)) iFaceType |= 0x08;
     ...
     * actual vertex locations.  The class extends Object for hashing support.
     */
    private class Triangle
        ...
        public boolean equals(Object kObject) {
            ...
            return (m_iV0 == kT.m_iV0) && ( (m_iV1 == kT.m_iV1 && m_iV2 == kT.m_iV2) || (m_iV1 == kT.m_iV2 && m_iV2 == kT.m_iV1));
        }
    }
    ...
    private class VETable {
         ...
         * @return Triangle new index containing the indices into
         * the array of edge vertices, one index for each vertex
         * in the triangle.
         */
        ...
        /**
         * Mark the specified edge vertex as no longer being used
         * to create triangles
        ...
        /**
         * Vertex class which stores the coordinates of an edge-vertex
         * along with the indices of adjacent vertices which comprise
         * the edges intersected by the level surface.
         */
