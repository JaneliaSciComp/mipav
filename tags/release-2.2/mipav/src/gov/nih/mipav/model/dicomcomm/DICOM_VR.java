...
public class DICOM_VR {
    ...
    */
    public static DICOM_VR readData( int group, int element, int length, DICOM_Comms vrBuffer )
    ...
    */
    public final static void writeData( DICOM_VR vr, DICOM_Comms vrBuffer ){
    ...
    */
    public String toString( String str ) {
        ...
        return( str + ": VR( " +
                DICOM_Util.toHexString(            (short) group           ) + ", "    +
                DICOM_Util.toHexString(            (short) element         ) + " ) "   +
                "("  + DICOM_Util.padAndOrTruncate( description,        40 ) + " ) \t\t\t" +
                "["  + DICOM_Util.padAndOrTruncate( typeCodeName,        2 ) + " ] "   +
                "["  + strDataLength                                         + " ] "   +
    ...
    */
    public final static long unsignedInt(int value )  { return( ( (long)(value) ) & 0xffffffff ); }

    /**
    *   Converts value to an unsigned short and returns it as an integer
    *   @param value    value to be converted to an unsigned short
    */
    public final static int unsignedShort(int value ) { return( (  (int)(value) ) & 0xffff ); }
    ...
    */
    private final static boolean isAlpha( byte ch ) {
        if( ((ch >= ' ') && (ch <= 125)) || ch == 0) return (true);
        return (false);
    }
    ...
    */
    private final static boolean isDataAlpha( byte[] data ) {
    ...
    private final static String dataToString( DICOM_VR vr, int typeCode ) {
        ...
        if( data.length < 1 ) {
            ...
            //case DICOM_RTC.TYPE_US_or_SS_or_OW:
                if( vr.group == 0x0000 && ( vr.element == 0x0100 || vr.element == 0x0800 || vr.element == 0x0900 ) ) {
                    for( index = 0; index < data.length; index += 2 ) {
                        str += DICOM_Util.toHexString( (short)DICOM_Comms.bufferToInt16( data, index, DICOM_Comms.LITTLE_ENDIAN ) );
                        str += "h ";
                    }
                }
                else {
    ...
    */
    private final void makeDataEven( byte[] _data ) {
        int index ;
       // \\00;
