 ...
 *  <p>
 *	Segmentation divides an image into distinct classes or types, such as segmenting the brain into 3
 *	tissue types: gray matter, white matter, and cerebrospinal fluid.  There are 2 types of segmentation:
 *	hard and soft or fuzzy.  In hard segmentation a pixel is simply assigned to 1 of the classes.
 *	However, often in medical images, there cannot be absolute classification of a pixel because of
 *	partial volume effects where multiple tissues contribute to a pixel or voxel causing intensity
 *	blurring across boundaries.  Fuzzy segmentation allows for the uncertainty in the location of object
 *	boundaries.  In fuzzy segmentation a membership function exists for each class at every pixel location.
 *	At each pixel location a class membership function will have a value of 0 if there is absolutely no chance
 *	of that pixel belonging to the class.  At each pixel location a class membership function will have a value
 *	of 1 if the pixel belongs to the class with absolute certainty.  Membership functions can vary from 0 to 1,
 *	with the constraint that at any pixel location the sum of the membership functions of all the classes must
 *	add up to 1.  The fuzzy membership function reflects the similarity between the data value at that pixel and
 *	the value of the class centroid.  As a pixel data value becomes closer to the class centroid, the class
 *	membership function approaches unity.
 *	<p>
 *	The fuzzy C-Means algorithm is an unsupervised method; it works without the use of training data.
 *	This algorithm, allowing for soft segmentation based on fuzzy set theory, generalizes the K-means
 *	algorithm. The technique clusters data by iteratively computing a fuzzy membrship function and mean value
 *	estimates for each tissue class.  The algorithm works by minimizing the sum over all pixels j and all classes k of:
 *  (ujk**q) * ((yj - vk)**2)
 *	<p>
 *	nClass = number of classes.  ujk is the membership value at pixel location j for class k such that that sum over k from k = 1 to
 *	k = nClass for ujk = 1.  q is a weighing exponent on each membership value and determines the
 *	amount of "fuzziness" of the resulting segmentation.  q is required to be greater than 1 and is
 *	typically set to 2.  yj is the observed single channel image intensity at location j.
 *	vk is the centroid of class k.
 *	<p>
 ...
 *	</pre></code>
 *	<p>
 *	Minimization is achieved by an interative process which: <br>
 ...
 *	<p>
 *	The iteration continues until either the user specified maximum number of iterations has occcured
 *	or until convergence has been detected.  Convergence occurs when all membership functions over all pixel
 *	locations j change by less than the tolerance value between 2 iterations.  The default maximum iteration
 *	number is 100.  The default tolerance is 0.01.
 *	<p>
 /**
 *
 *	<b>Fuzzy C-Means Segmentation algorithm</b>
 *  <p>
 *	Segmentation divides an image into distinct classes or types, such as segmenting the brain into 3
 *	tissue types: gray matter, white matter, and cerebrospinal fluid.  There are 2 types of segmentation:
 *	hard and soft or fuzzy.  In hard segmentation a pixel is simply assigned to 1 of the classes.
 ...
 *	number is 100.  The default tolerance is 0.01.
 *	<p>
 *	A dialog for each image will ask the user for a threshold value for that image.
 *	The default value is the image minimum value.  In the centroid calculation only those pixels whose
 *	values equal or exceed threshold in all of the images are used in the centroid calculation.
 *	In the hard segmentation pixels whose values are less than threshold in any of the images
 *	or not in the selected first image VOI if the whole image is not used are set to a
 *	to a segmentation value of 0, meaning that these pixels are outside of the specified classes.
 *	<p>
 *	The initial dialog has a checkbox for Boundary noise cropping which is unchecked by default.
 *	This function finds the smallest bounding box outside of which all first image pixel values
 *	are below the first image threshold.  Values inside the bounding box are copied to a smaller
 *	array to save space and calculations are performed with this reduced array.  However, these
 *	pixels outside the box are restored for the production of the hard and fuzzy segmented images.
 *	In the hard segmentation case values outisde the box all have a value of 0.  In the fuzzy
 *	segmentation case values outside the box all have a value equal to the first image minimum.  If
 *	the cropping checkbox is selected, the user is constrained to select the whole image rather than
 *	VOI regions.
 *	<p>
 *	There are 3 choices for produced output images:<br>
 *	1.)	HARD ONLY<br>
 *	2.)	FUZZY ONLY<br>
 *	3.)	HARD & FUZZY BOTH<br>
 *  <p>
 *	Hard segmentation produces only 1 unsigned byte output image which assigns pixels
 *	which do not meet threshold requirements values of 0. The first class is assigned a value of 255/nClass,
 *	the second class is assigned a pixel value of  2*255/nClass, and so on.  The last class has a value of 255.
 *	<p>
 *	Fuzzy segmentation produces 1 image of the same type as the source image for every segmentation class.
 *	The membership function is scaled so that the minimum membership value scales to the source image minimum
 *	value and the maximum membership value scales to the source image maximum value.  If boundary cropping is used,
 *	pixels outside the bounding box are all assigned the source image minimum value.
 *	<p>
 *
 *	The first dialog provides buttons for loading and removing images.  The multispectral dialog has a list
 *	of loaded images.  The button labeled Load another image causes an Open File dialog to appear.
 *	An opened file will only cause an image to be added to the loaded images list if it has the same
 *	dimensionality as the original image and if the length of each dimension is the same as that of
 *	the original image.  Also, if the original image is not color, then the later loaded images cannot be
 *	color.
 *	<p>
 *	The button labeled Remove selected image can be used to remove one image from
 *	the list.  However, the original image that was present when the multispectral file menu command
 *	was invoked(the first image in the list) cannot be removed.  This button will only be enabled when
 *	at least 2 images are in the list.
 *	<p>
 *	If the first loaded image is color, checkboxes appear allowing the user to select red, green, and blue
 *	components.  The default is all components selected.
 *	<p>
 *	A later dialog appears for every black and white image and for every selected component of every color
 *	image and the signal thresholds and initial centroids are entered on these later dialogs.
 *	<p>
 *	Image Types:<br>
 *	This algorithm can be applied to 2D and 3D data sets that are not COMPLEX.<br>
 *  <p>
 *	References:
 *	This code is not original - it is simply a ported subset of code written and kindly provided by Dzung Pham.
 *	Note that our code is not the adaptive fuzzy C-means segmentation, which the Dzung Pham code is.  Our code
 *	is not adaptive - it does not correct intensity inhomogeneities, also known as shading artifacts.<br>
 ...
 *	3.)	Dzung L. Pham and Jerry L. Prince, "Adaptive Fuzzy Segmentation of Magnetic Resonance Images",
 *	IEEE Transactions on Medical Imaging, Vol. 18, No. 9, September, 1999, pp. 737 - 752.<br>
 *
 *		@version 0.1 June 15, 2000
 *		conversion of Dzung Pham's C code
...
public class AlgorithmMSpectralFuzzyCMeans extends AlgorithmBase {
     ...
     *   @param _smooth1
     *   @param _smooth2
     *   @param _outputGainField
     *   @param _segmentation
     *   @param _cropBackground
     *   @param _maxIter
     *   @param _tolerance
     *   @param _doRed
     *   @param _doGreen
     ...
     *   @param _wholeImage
     */
    public AlgorithmMSpectralFuzzyCMeans( ModelImage[] destImg, ModelImage[] srcImg,
            int _nClass, int _pyramidLevels,
            int _jacobiIters1, int _jacobiIters2,
            float _q, float _smooth1, float _smooth2,
            boolean _outputGainField,
            int _segmentation,
            boolean _cropBackground,
            int _maxIter, float _tolerance,
            boolean _doRed, boolean _doGreen, boolean _doBlue,
            boolean _wholeImage ) {
     ...
     *   out puts the string to the messsage frame if the logging
     *   procedure is turned on.
     */
    private void constructLog() {
        historyString = new String(
                "Fuzzy C-means(" + String.valueOf( nClass ) + ", " + String.valueOf( pyramidLevels ) + ", "
                + String.valueOf( jacobiIters1 ) + ", " + String.valueOf( jacobiIters2 ) + ", " + String.valueOf( qVal )
                + ", " + String.valueOf( exponent ) + ", " + String.valueOf( smooth1 ) + ", "
                + String.valueOf( smooth2 ) + ", " + String.valueOf( outputGainField ) + ", "
                + String.valueOf( segmentation ) + ", " + String.valueOf( cropBackground ) + ", "
                + String.valueOf( threshold ) + ", " + String.valueOf( maxIter ) + ", " + String.valueOf( tolerance )
                + ", " + String.valueOf( doRed ) + ", " + String.valueOf( doGreen ) + ", " + String.valueOf( doBlue )
            ...
            if (destImage != null) {
                for ( int i = 0; i < destImage.length; i++ ) {
                    if (destImage[i] != null && destImage[i].getHistoryArea() != null) {
                        if (srcImage != null) {
                            destImage[i].getHistoryArea().setText(srcImage[
                                    Math.min(i,srcImage.length-1)].getHistoryArea().getText());
                        }
    ...
    private void cMeans2() {
            ...
            for ( i = 0; i < nClass; i++ ) {
                ...
                try {
                    ...
                    MipavUtil.displayError(
                            "MSpectralFuzzyCMeans: IOException on destImage[" + destNum
        ...
        if ( segmentation != FUZZY_ONLY ) {
            try {
                ...
                cleanUp();
                MipavUtil.displayError(
                        "MSpectralFuzzyCMeans: IOException on destImage[" + destNum + "].importData(0,segBuffer,true)" );
                progressBar.dispose();
                setCompleted( false );
                return;
            } catch ( OutOfMemoryError e ) {
                ...
                System.gc();
                MipavUtil.displayError(
                        "MSpectralFuzzyCMeans: Out of memory on destImage[" + destNum + "].importData(0,segBuffer,true)" );
                progressBar.dispose();
                setCompleted( false );
                return;
            }
    ...
    /* private void computeMultiplier2D(float[] buffer, float[] centroids, float[] mems, float[] mult,
     float smooth1, float smooth2, float qVal) {
     // Only use scalar multiplier field here
     float[] f;
     float[] w;
     int x,y;
     int i;

     f = new float[sliceSize];
     w = new float[sliceSize];

     for (x = 0; x < xDim; x++) {
     for (y = 0; y < yDim; y++) {
     ComputeFW2(f,w,buffer,centroids,mems,x,y,qVal);
     } // for (y = 0; y < yDim; y++)
     } // for (x = 0; x < xDim; x++)
     fmvtrunc2(mult,f,w,smooth1,smooth2,jacobiIters1,jacobiIters2,truncLevel);

     // Compute max and minimum of multiplier field
     maxMult = 0.0f;
     minMult = 1e6f;
     for (i = 0; i < sliceSize; i++) {
     if (mult[i] > maxMult)
    ...
    /*
     ...
     for (c = 0; c < nClass; c++) {
     indexM = c*sliceSize + index;
     // Compute dot products: cTC and yTc
     temp1 = centroids[c] * centroids[c];
     temp2 = buffer[index] * centroids[c];
     w[index] += (float)Math.pow((double)mems[indexM],(double)qVal) * temp1;
     f[index] += (float)Math.pow((double)mems[indexM],(double)qVal) * temp2;
     } // for (c = 0; c < nClass; c++)
    ...
    /** This function performs a truncated FMV cycle.  It stops performing
     V-cycles at the pyramid level given by 'truncLevel'. */

    /* private void fmvtrunc2(float[] mult, float[] f, float[] w, float smooth1, float smooth2,
     int jacobiIters1, int jacobiIters2, int truncLevel) {
     float[] fpyr,wpyr;
     int i;
     float[] temp;
     float[] u2;
     ...
     // Allocate pyramids
     fpyr = new float[pyramidLevels * sliceSize];
     wpyr = new float[pyramidLevels * sliceSize];
     // Keep track of the sizes of each level
     sizesX = new int[pyramidLevels];
     sizesY = new int[pyramidLevels];
     for (i = 0; i < sliceSize; i++) {
     fpyr[i] = f[i];
     wpyr[i] = w[i];
     } // for (i = 0; i < sliceSize; i++)
     sizesX[0] = xDim;
     sizesY[0] = yDim;
     temp = new float[sliceSize];
     for (i = 0; i < sliceSize; i++) {
     temp[i] = mult[i];
     } // for (i = 0; i < sliceSize; i++)

     u2 = new float[(xDim/2 + xDim%2) * (yDim/2 + yDim%2)];
     u2x = xDim/2 + xDim%2;
     u2y = yDim/2 + yDim%2;
     for (i = 1; i < pyramidLevels; i++) {
     u2 = reduce2d(temp,sizesX,sizesY,i-1);
     u2x = sizesX[i];
     u2y = sizesY[i];
     u2Length = u2x * u2y;
     temp = null;
     temp = new float[u2Length];
     for (j = 0; j < u2Length; j++) {
     temp[j] = u2[j];
     }
     reduce2db(fpyr,sizesX,sizesY,i-1);
     reduce2db(wpyr,sizesX,sizesY,i-1);
     } // for (i = 1; i < pyramidLevels; i++)
     fp = new float[sliceSize];
     index = (pyramidLevels - 1) * sliceSize;
     for (i = 0; i < sliceSize; i++) {
     fp[i] = fpyr[index + i];
     }
     vcycle2(u2,u2x,u2y,fp,wpyr,smooth1,smooth2,pyramidLevels-1,jacobiIters1,jacobiIters2);
     for (i = pyramidLevels-2; i>=0; i--) {
     temp = expand2d(u2,u2x,sizesX,sizesY,i);
     u2x = sizesX[i];
     u2y = sizesY[i];
     u2Length = u2x * u2y;
     u2 = null;
     u2 = new float[u2Length];
     for (j = 0; j < u2Length; j++) {
     u2[j] = temp[j];
     }
     if (i >= truncLevel) { // Stop doing V-cycles if necessary
     index = sliceSize*i;
     for (j = 0; j < sliceSize; j++) {
     fp[j] = fpyr[index + j];
     }
     vcycle2(u2,u2x,u2y,fp,wpyr,smooth1,smooth2,i,jacobiIters1,jacobiIters2);
     }
     } // for (i = pyramidLevels-2; i>=0; i--)
    ...
    /*
     ...
     int level, int jacobiIters1, int jacobiIters2) {
     float h;
     float mask[];
     float smooth1b,smooth2b;
     float r[];
     float temp[];
     float dummy;
     int x,y;
     ...
     float u2[];
     float v[];
     int uLength,u2Length;
     int index;

     // Initialize
     h = (float)Math.pow(2.0,(double)level);
     smooth1b = smooth1/(h*h);
     smooth2b = smooth2/(h*h);
     mask = new float[25];
     setMask52(mask,smooth1b,smooth2b);

     jacobi2(u,ux,uy,f,wpyr,level,smooth1b,smooth2b,jacobiIters1);
     uLength = ux * uy;
     if (level != (pyramidLevels-1)) {
     // Compute residual r = f - wpyr*u - smooth*Laplace(u)
     r = new float[uLength];
     temp = conv2d5(u,ux,uy,mask);
     dummy = 0.0f;
     for (i = 0; i < uLength; i++) {
     index = level*sliceSize + i;
     r[i] = f[i] - wpyr[index] * u[i] -temp[i];
     }
     ...
     sizesXU[0] = ux;
     sizesYU[0] = uy;
     r2 = reduce2d(r,sizesXU,sizesYU,0);
     temp = null;
     r = null;
     u2x = sizesXU[1];
     u2y = sizesYU[1];
     ...
     // Recursive call
     vcycle2(u2,u2x,u2y,r2,wpyr,smooth1,smooth2,level+1,jacobiIters1,jacobiIters2);
     v = expand2d(u2,u2x,sizesX,sizesY,level);
     ...
     jacobi2(u,ux,uy,f,wpyr,level,smooth1,smooth2,jacobiIters2);
     }
     */
    ...
    /* private float[] expand2d(float[] in, int inx, int sizesX[], int sizesY[], int level) {
     ...
     outx = sizesX[level];
     outy = sizesY[level];
     out = new float[outx * outy];
     for (x = 0; x < outx; x++) {
     l = (int)((double)x/2.0);
     for (y = 0; y < outy; y++) {
     m = (int)((double)y/2.0);
     out[x + y*outx] = in[l + m*inx];
     }
     }
     return(out);
     }
    ...
    /*
     ...
     int level, float smooth1, float smooth2, int iter) {
     float alpha;
     float[] mask;
     int x,y;
     float[] temp;
     ...
     // Set up the mask
     mask = new float[25];
     setMask52(mask, smooth1, smooth2);
     for (i = 0; i < 25; i++) {
     mask[i] *= -1.0f;
     }
     mask[12] = 0.0f;
     ...
     uLength = ux*uy;
     for (i = 0; i < uLength; i++) {
     Dinv = 1.0f/(wpyr[level*sliceSize + i] + 6.0f*smooth1 + 42.0f*smooth2);
     u[i] = (1.0f - alpha) *u[i] +
     alpha * Dinv * temp[i] +
     alpha * Dinv * f[i];
     }
    ...
    /*
     ...
     for (l = -2; l <=2; l++) {
     if ((x+l) < 0) lind = 0;
     else if ((x+l) > (inx-1)) lind = inx-1;
     else lind = (x+l);
     for (m = -2; m <= 2; m++) {
     if ((y+m) < 0) mind = 0;
     else if ((y+m) > (iny-1)) mind = iny-1;
     else mind = (y+m);
     out[x + y*inx] += mask[l+2 + 5*(m+2)]*in[lind + mind*inx];
     }
     }
     }
     }
    ...
    /*
     ...
     mask1[12] = 4.0f;
     mask1[7] = mask1[11] = mask1[17] = mask1[13] = -1.0f;
     ...
     for (i = 0; i < 25; i++) {
     mask[i] = smooth1*mask1[i] + smooth2*mask2[i];
     }
     }
     */
    ...
    /*
     ...
     xNew = sizesX[iLevel]/2 + sizesX[iLevel]%2;
     yNew = sizesY[iLevel]/2 + sizesY[iLevel]%2;
     out = new float[xNew * yNew];
     sizesX[iLevel+1] = xNew;
     sizesY[iLevel+1] = yNew;
     for (x = 0; x < xNew; x++) {
     x2 = 2*x;
     x2p = x2+1;
     if (x2p > (sizesX[iLevel]-1)) {
     x2p = sizesX[iLevel] - 1;
     }
     for (y = 0; y < yNew; y++) {
     y2 = 2*y;
     y2p = y2+1;
     if (y2p > (sizesY[iLevel]-1)) {
     y2p = sizesY[iLevel] - 1;
     }
     out[x + y*xNew] = (in[x2 + y2*sizesX[iLevel]]  + in[x2p + y2*sizesX[iLevel]] +
     in[x2 + y2p*sizesX[iLevel]] + in[x2p + y2p*sizesX[iLevel]])/4;
     } // for (y = 0; y < yNew; y++)
     } // for (x = 0; x < xNew; x++)
    ...
    /*
     ...
     xNew = sizesX[iLevel+1];
     yNew = sizesY[iLevel+1];
     ...
     tmp  = iLevel*sliceSize;
     tmp1 = y2*sizesX[iLevel];
     tmp2 = y2p*sizesX[iLevel];

     i = (iLevel+1)*sliceSize + x + y*xNew;
     i1 = tmp + x2  + tmp1;
     i2 = tmp + x2p + tmp1;
     i3 = tmp + x2  + tmp2;
     i4 = tmp + x2p + tmp2;
     in[i] = (in[i1] + in[i2] + in[i3] + in[i4])/4;
     } // for (y = 0; y < yNew; y++)
     } // for (x = 0; x < xNew; x++)
     }
     */
