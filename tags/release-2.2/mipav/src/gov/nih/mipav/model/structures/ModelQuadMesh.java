...
/**
 *	A simple quadrilateral mesh that represents a level surface.  The mesh only
 *	stores vertex positions and vertex normals.  The surface viewer creates
 *	a Shape3D object whose geometry is an object from ModelQuadMesh.  The
 *	surface color is provided through the LUT.  To save memory, everything is done by
 *	reference - the connection array, the vertex array, the normal array, and the color
 *	array are all stored here and modified directly.
 *
...
public class ModelQuadMesh
    extends IndexedQuadArray implements GeometryUpdater {
    ...
    /**
     *	A quad mesh whose vertex normal vectors are computed from the
     *	geometry of the mesh itself.  The normal at a vertex is the normalized
     *	average of normals of the triangles that share the vertex.  This constructor
     *	makes a quad mesh whose vertices and colors are set by reference.
     *
    ...
    public ModelQuadMesh(Point3f[] akVertex, int[] aiConnect, int length1, int length2, float[][] LUT, float hgtRange) {
        ...
        for (int i = 0; i < connect.length; i++) connect[i] = aiConnect[i];
    ...
    public ModelQuadMesh(Point3f[] akVertex, Vector3f[] akNormal, int[] aiConnect) {
        ...
        for (int i = 0; i < vertex.length; i++) {
            vertex[i] = new Point3f(akVertex[i]);
            if (vertex[i].z < min) min = vertex[i].z;
            if (vertex[i].z > max) max = vertex[i].z;
        }
...
//        setNormalRef3f(normal);
    ...
    /**
     * Save an array of quad meshes to a text file.  The format for the
     * file is
     * <pre>
     * int type;  // 2 = ModelQuadMesh
     ...
     *            writing.
     */
    static public void print(String kName, ModelTriangleMesh[] akComponent) throws IOException {
    ...
    /**
     *	Save an array of quad meshes to a binary file.  The format for the
     *	file is
     *	<pre>
     *	int type;        // 2 = ModelQuadMesh
     *	int aCount;      // number of array elements
     ...
     *	@exception IOException if there is an error writing to the file.
     */
    static public void save(String kName, ModelQuadMesh[] akComponent, ViewJProgressBar progress) throws IOException {
        if (akComponent.length == 0)
            return;
     ...
     *	'void save (String, ModelQuadMesh[])'.  ModelQuadMesh uses this
    ...
    protected void save(RandomAccessFile kOut, ViewJProgressBar progress, int added, int total) throws IOException {
        ...
        byte[] bufferByte = new byte[vertex.length * 24]; // 24 = 3 (vertices) * 4 (quad) * 2 (both normal and vertex arrays)
        int i, index, tmpInt;
    ...
    /**
     *	Load the quad mesh from a binary file.  The caller
...
/**** I don't think both are needed in the new Model LUT design.
    /**
     *	Accessor that sets the LUT for the colors of the indices.
     ...
     *	It is not called directly by the application.  It sets the colors of the vertices
     *	based on the LUT.  The System.gc() call is necessary because otherwise the application
     *	eats up memory.
