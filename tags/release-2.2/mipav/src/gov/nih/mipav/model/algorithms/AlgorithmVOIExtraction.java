...
public class AlgorithmVOIExtraction
                        ...
                        if ( (!mask.get(smallPos)) && (objBuffer[smallPos] == obj)) {
                            ...
                            for (i = 0; ( (newGrayScale) && (i < grayScaleNumber)); i++) {
                                if (grayScaleArray[i] == expImgBuffer[scanPos]) {
                                    ...
                                    ( (VOIContour) (VOIs.VOIAt(VOIIndexArray[i]).getCurves()[z].lastElement())).trimPoints(
                            ...
                            if (newGrayScale) {
                                ...
                                ( (VOIContour) (VOIs.VOIAt(VOIIndexArray[grayScaleNumber]).getCurves()[z].lastElement())).trimPoints(
                                    Preferences.getTrim(), Preferences.getTrimAdjacient());
                                grayScaleNumber++;
                    ...
                    for (x = 0; x < xDimE && !threadStopped; x++) {
                        ...
                        if ( (!outMask.get(smallPos)) && (!mask.get(smallPos)) && (objBuffer[smallPos] == 0)) {
                            ...
                            for (i = 0; ( (newGrayScale) && (i < grayScaleNumber)); i++) {
                                ...
                                for (j = 0; j < nCurves; j++) {
                                    if ( ( (VOIContour) (VOIs.VOIAt(VOIIndexArray[i]).getCurves()[z].elementAt(j))).contains(x / 2, y / 2, true) &&
                                       voiID == obj  ) {
                                        newGrayScale = false;
                                        // add the polygon to an existing VOI
                                        VOIs.VOIAt(VOIIndexArray[i]).importPolygon(contourPolygon, z);
