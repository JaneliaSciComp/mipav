...
/**
 * Algorithm to apply a median filter to an image, placing it in a new ModelImage,
 * or returning the changed picture to the same image.  Algorithm takes values in the
 * neighborhood that correspond to the "on" values in the kernel (mask) and places
 * them into a sorted list.  If the variance (square of standard Deviation) isn't 0 and the
 * pixel's value is outside the bounds given by the given fraction of the standard
 * deviation, then the median of the list is computed and
 * placed into that element of the image.  If the variance is 0, the median of the
 * list is always computed and placed into that element of the image.
 *
 * <p>If any element in the kernel which does not have a corresponding element in the
 * image, then the pixel is unfiltered. This results in a border of unfiltered pixels
 * in the image 1/2 of the kernel-size large.
 *
...
public class AlgorithmMedian extends AlgorithmBase {
     ...
     *   @param stdDev       Inner-bounds by which to process pixels (pixel values outside this bound
     *                       will be median filtered).
     *   @param maskFlag     Flag that indicates that the median filtering will be
     *                       performed for the whole image if equal to true.
     */
    public AlgorithmMedian( ModelImage srcImg, int iters, int kSize, int kShape,
            float stdDev, boolean maskFlag ) {
        super( null, srcImg );
     ...
     *                       performed for the whole image if equal to true.
     */
    public AlgorithmMedian( ModelImage destImg, ModelImage srcImg, int iters,
            int kSize, int kShape,
            float stdDev, boolean maskFlag ) {
     ...
     *   @param stdDev       Inner-bounds by which to process pixels (pixel values outside this bound
     *                       will be median filtered).
     *   @param sliceBySlice Each slice in a volume image is to be filtered separately (when true),
     *   					else the volume will use a kernel with 3 dimensions.
     *   @param maskFlag     Flag that indicates that the median filtering will be
     *                       performed for the whole image if equal to true.
     */
    public AlgorithmMedian( ModelImage srcImg, int iters, int kSize, int kShape,
     ...
     *   @param stdDev       Inner-bounds by which to process pixels (pixel values outside this bound
     *                       will be median filtered).
     *   @param sliceBySlice Each slice in a volume image is filtered separately (when true),
     *                       else the volume will use a kernel with 3 dimensions.
     *   @param maskFlag     Flag that indicates that the median filtering will be
     *                       performed for the whole image if equal to true.
     */
    public AlgorithmMedian( ModelImage destImg, ModelImage srcImg, int iters,
            int kSize, int kShape,
            float stdDev, boolean sliceBySlice, boolean maskFlag ) {
    ...
    /**
     *	RGB images are median filtered by 'channel.'  That is, each color,
     *   red, blue and green, is filtered independently of the other two colors.
     *   This median filter permits selectively filtering any combination of the
     *   three channels instead of simply filtering all three.
    ...
    private void constructLog() {
        ...
        historyString = new String(
                "Median(" + String.valueOf( kernelShape ) + ", " + String.valueOf( kernelSize ) + ", "
     ...
     *
     * Make a copy of the srcImage in a buffer with borders for the kernel.  Copy the closest
     * rows and columns data into the added borders.
     */
    ...
    /**
     * Method copies the data in srcBuffer centered in the border buffer
     * That is, the src image is completely surrounded by a border.  The
     * values in the border locations are copies of nearby srcBuffer values.
     ...
     */
    private void copy2DSrcBufferToBdrBuffer( float[] srcBuffer, float[] bdrBuffer,
        ...
        for ( row = 0; row < halfK; ++row ) {
            ...
            for ( col = 0; col < halfK; ++col ) {
                ...
                bdrBuffer[row * bdrBufferWidth + bdrBufferWidth - 1 - col + bdrBufferOffsetIndex] = srcBuffer[srcBufferWidth - 1];
            }
        }
            ...
            for ( currentSlice = 0; currentSlice < numberOfSlices && !threadStopped; currentSlice++ ) {
                ...
                if (progressBar != null) {
                    progressBar.updateValue(Math.round
                                         ((((float) (currentSlice) / (srcBufferDepth - 1)) *
                                           100)), activeImage);
                }
                sliceFilterBorder( bdrBuffer, resultBuffer, currentSlice * bdrSliceLength, currentSlice * srcSliceLength );
            }
    ...
    /**
     * Method copies the data in srcBuffer so it is centered in the border buffer
     * That is, the src image is completely surrounded by a border.  The
     * values in the border locations are copies of nearby srcBuffer values.
     * @param srcBuffer float []   the source image buffer
     * @param bdrBuffer float []   the border image buffer
    ...
    private void copy3DSrcBufferToBdrBuffer( float[] srcBuffer, float[] bdrBuffer ) {
        ...
        for ( sliceIndex = 0; sliceIndex < srcBufferDepth; sliceIndex++ ) {
            ...
            if (progressBar != null) {
                progressBar.updateValue(Math.round
                                     ((((float) (sliceIndex) / (srcBufferDepth - 1)) * 100)),
                                     activeImage);
            }
    ...
    private void calcStoreInDestBorder3D() {
        ...
        if ( sliceFiltering ) {
            for ( currentSlice = 0; currentSlice < numberOfSlices && !threadStopped; currentSlice++ ) {
                ...
                if (progressBar != null) {
                    progressBar.updateValue(Math.round
                                         ((((float) (currentSlice) / (srcBufferDepth - 1)) * 100)),
                                         activeImage);
                }
                sliceFilterBorder( bdrBuffer, resultBuffer, currentSlice * bdrSliceLength, currentSlice * srcSliceLength );
            }
    ...
    /**
     * Method performs median filtering on a volume that has been centerend in a
     * border buffer and places the result in the destination buffer
    ...
    private void volumeFilterBorder( float[] srcBdrBuffer, float[] destBuffer ) {
        ...
        for ( destSlice = 0; destSlice < srcBufferDepth && !threadStopped; destSlice++ ) {
            if (progressBar != null) {
                progressBar.updateValue(Math.round((((float) (destSlice) /
                                                  (srcBufferDepth - 1)) * 100)), activeImage);
            }
            ...
            for ( destRow = 0; destRow < srcBufferHeight; destRow++ ) {
                ...
                for ( destCol = 0; destCol < srcBufferWidth; destCol++, srcBdrBufferIdx++,
                        destBufferIdx++ ) {
     ...
     *   Image can be filtered by filtering each slice individually, or by filtering using a kernel-volume.
     */
     ...
     *   @param  msgString           A text message that can be displayed as a message text
     *                               in the progressBar.
     */
    private final void sliceFilter( float[] srcBuffer,
            float[] destBuffer,
        ...
        for ( pass = 0; pass < iterations && !threadStopped; pass++ ) {
            ...
            if ( isColorImage ) { // color image dealt with in special way
                ...
                while ( initialIndex < 3 && !threadStopped ) { // alpha:0, R:1, G:2, B:3.  But alpha must be copied
                    ...
                    if ( numberOfSlices > 1 && pBarVisible == true ) { // 3D image     update progressBar
                        ...
                        progressBar.updateValue(
                                Math.round
                                (
                                        ( ( (float) ( currentSlice + pass * numberOfSlices )
                                                / ( iterations * numberOfSlices ) )
                                                        * 100 ) ),
                                                        activeImage );
                    }
                    ...
                    if ( !rChannel && initialIndex == 1 ) {
                        ...
                        if ( pBarVisible == true ) {
                            progressBar.setMessage(
                                    "Filtering " + msgString + " (pass " + String.valueOf( pass + 1 ) + " of "
                                    + iterations + ") ..." );
                        }
                            ...
                            if ( numberOfSlices == 1 ) { // 2D image     update progressBar
                                if ( ( i - initialIndex ) % mod == 0 && pBarVisible == true ) {
                                    progressBar.updateValue(
                                            Math.round
                                            (
                                                    (float) ( 3 * ( pass * sliceLength )
                                                            + ( initialIndex - 1 ) * sliceLength + i / 4 )
                ...
                if ( pBarVisible ) {
                    progressBar.setMessage( "Filtering " + msgString + " (pass " + String.valueOf( pass + 1 ) + " of " + iterations + ") ..." );
                    if ( numberOfSlices > 1 ) { // 3D image     update progressBar
                        // do a progress bar update
                        progressBar.updateValue( Math.round( ( ( (float) ( currentSlice + pass * numberOfSlices ) / ( iterations * numberOfSlices ) ) * 100 ) ), activeImage );
                    }
                }
                for ( i = 0; i < imageSliceLength && !threadStopped; i++ ) {
                    if ( numberOfSlices == 1 ) { // 2D image     update progressBar
                        if ( i % mod == 0 && pBarVisible == true ) {
                            progressBar.updateValue( Math.round( ( (float) ( ( pass * imageSliceLength ) + i ) / ( iterations * imageSliceLength ) * 100 ) ), activeImage );
                        }
                    }
     ...
     */
    private final void sliceFilterBorder( float[] srcBdrBuffer,
            float[] destBuffer,
    ...
    /**
     *   Compiles a list of the values neighboring the desired pixel, that are defined in the
     *   kernel.
     *   @param  i       The central pixel to find neighbors for.
     *   @param  data float []  Image data
     *   @param  is2D    True indicates that the neighbors are found along a
     *                   2D slice (or 2D image) instead of neighbors in a 3D volume.
     *   @return 		The neighboring pixel list
     *                   corresponds to the kernel chosen.
     */
    private final float[] getBorderBufferNeighborList( int i, float[] data,
            boolean is2D ) {
        int row, col;
    ...
    /**
     *   Filter a 3D image with a 3D kernel.  Allows median filtering to include the picture elements
     *	at greater depths than only the current slice.
     *   <p><em>Note that this volume filter will correctly filter color images on all bands (aRGB) because
     *	the neighbor list is correct (see getNeighborList()).  This means, however, it will not selectivly
     *	filter any bands (one may not filter only the Red channel, for instance), and will also filter
     *   all alpha values as well.  Of course, progress bar updates will not include any color information.
     *	For these reasons it a useable, but limited color filter.</em>
    ...
    private void volumeFilter( float[] srcBuffer, float[] destBuffer ) {
        ...
        rightBound = sliceWidth - valuesPerPixel * halfK - 1; // in color: (4*width - 4*halfK - 1); mono: (width - halfK - 1)
        ...
        for ( pass = 0; pass < iterations && !threadStopped; pass++ ) {
            if ( pBarVisible == true ) {
                progressBar.setMessage(
                        "Filtering image (pass " + String.valueOf( pass + 1 ) + " of " + iterations + ") ..." );
            }
            for ( i = 0; i < imageLength && !threadStopped; i++ ) {
                if ( i % mod == 0 && pBarVisible == true ) {
                    progressBar.updateValue( Math.round
                            ( ( (float) ( ( pass * imageLength ) + i ) / ( iterations * imageLength ) * 100 ) ),
                            activeImage );
                }
    ...
    /**
     *   Filter a Color 3D image with a 3D kernel.  Allows median filtering to
     *   include the picture elements at greater depths than only the current
     *   slice.  This method allows selected band filtering, and does not filter
     *   the alpha band.
     *   @param  srcBuffer float []	Source image.
     *   @param  destBuffer float []	Destination image.
     *   @see    volumeFilter
            ...
            if ( !rChannel && initialIndex == 1 ) {
                ...
                for ( pass = 0; pass < iterations && !threadStopped; pass++ ) {
                    if ( pBarVisible == true ) {
                        if ( initialIndex == 1 ) {
                            progressBar.setMessage(
                                    "Filtering red channel (pass " + String.valueOf( pass + 1 ) + " of " + iterations
                                    + ") ..." );
                        } else if ( initialIndex == 2 ) {
                            progressBar.setMessage(
                                    "Filtering green channel (pass " + String.valueOf( pass + 1 ) + " of " + iterations
                                    + ") ..." );
                        } else if ( initialIndex == 3 ) {
                            progressBar.setMessage(
                                    "Filtering blue channel (pass " + String.valueOf( pass + 1 ) + " of " + iterations
                                    + ") ..." );
                        }
                    }
                    // if we needed to filter the image, we dropped through the selection to filter the
                    ...
                    for ( i = initialIndex; i < imageLength && !threadStopped; i += 4 ) {
                        if ( ( i - initialIndex ) % mod == 0 && pBarVisible == true ) {
                            progressBar.updateValue(
                                    Math.round
                                    (
                                            ( (float) ( iterations * ( initialIndex - 1 ) * imageSize + imageSize * pass
                                            + i / 4 )
                                                    / ( 3 * iterations * imageSize )
                                                    * 100 ) ),
                                                    activeImage );
                        }
     ...
     *  The kernel mask is the list of values pulled from the
     *  image which will be used to find the median of the
     *  central pixel.  Its length is
     *  <i>(number of pixels to be used to determine median)</i>.
     *  <p>
     *  Thus the kernel center (decided here), has the value of
     *  the location of the central pixel shown in the window.
     *  The value of the kernel center is the number of pixels picked
     *  up to median sort.
     *
     *  Note that for symmetric masks always have kernelCenter = count/2
     *  and maskCenter = count/2.
     *
     */
    private void makeKernelMask() {
        halfK = kernelSize / 2;
        // figure how many kernel elements are actually in the kernel-mask
        int count = 0; // start counting from one, since sort starts with element 1 (even empty mask must have 1 element!)
     ...
     *   kernel.
     *   <p>
     *   Color images are processed differently from the monochrome images because although colour images
     *   use the same size kernel as mono images, it fills the kernel with brightness levels that are
     *	spread out in the data set.  The Neighbor list still reports the monochromatic brightness values.
     *	That is, for a color image: the neighbors of the central pixel with the same color are
     *   returned in the neighbor list's kernel.
     *   @param  i       The central pixel to find neighbors for.
     *   @param  data float []   Image data
     *   @param  is2D    True indicates that the neighbors are found along a
     *                   2D slice (or 2D image) instead of neighbors in a 3D volume.
     ...
     *                   corresponds to the kernel chosen.
     */
    private final float[] getNeighborList( int i, float[] data, boolean is2D ) {
        int row, col;
        int kCenter = kernelCenter; // index to the central element of the kernel
        // (this is the mask for which elements in data are used.)
        int width = 0; // width of slice in number of pixels
        ...
        if ( isColorImage ) { // 2D filtering of color images is a little different than of mono images
            ...
            if ( is2D ) {
                for ( row = -halfK; row <= halfK; row++ ) { // go through all rows
                    for ( col = leftBound, kcol = -halfK; col <= rightBound; col += 4,
                            kcol++ ) { // go through every 4th column
                        if ( kernel[kCenter + kcol + row * kernelSize] != 0 ) { // but don't bother copying into the list if
                            // we don't want that element (the kernel's pixl is zero)
     ...
     *   @author parsonsd
     */
    private final float mean( float[] list ) {
        int i;
        float sum = 0;
    ...
    /**
     *   Finds the standard deviation of the values in the input list
     *   (defined as:   s = [(1/(N-1))*SUM (from 0 to N-1)[ (Xi - <bold>X</bold>)^2]]^(1/2))
     *   @param list float []		The list of numbers.
     *   @param average  Arithmetic mean of the values in list.
     *   @return 		The standard deviation.
     */
    private final float standardDeviation( float[] list, float average ) {
        int i;
        int N = list.length;
    ...
    /**
     *	If the progress bar is visible, sets the text to: <br><tt>Copying all <i>color</i> values ... </tt>
