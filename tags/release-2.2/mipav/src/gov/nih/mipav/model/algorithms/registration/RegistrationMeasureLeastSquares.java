...
/**
 * Concrete implementation of the RegistrationMeasure class based on sum
 * of the squared differences between the target image and the registered
 * source image.  The error value is normalized by computing the root
 * mean squared error.  The differences between corresponding target
 * image and registered source image are maintained in a separate image
    ...
    extends RegistrationMeasure
{
    ...
    // Current error at each sample (registered-target).  Has the same
    // number of samples as the target and registered images.
