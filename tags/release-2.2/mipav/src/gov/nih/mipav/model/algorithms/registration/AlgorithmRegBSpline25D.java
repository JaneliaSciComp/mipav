...
public class AlgorithmRegBSpline25D
     ...
     * image must have the same dimensions as the input source image.
     * @param kImageSource ModelImage
     * Reference to the original input image to use as the registration source.
     * @param iSliceTarget int Index of the slice in the input source
     * image that is to be used as the reference.  If this index is -1,
     * then each slice is registered to the one before it
     ...
     * the same dimensions as the input source image.
    ...
    public AlgorithmRegBSpline25D(ModelImage kImageResult,
        ...
        super(kImageResult, kImageSource, kImageSource, kImageDeformation,
    ...
    public void runAlgorithm() {
        ...
        iteration: for (int iSlice = 0; iSlice < iNumSlices; iSlice++) {

            // Show the slice number in the progress.
            String kStringSlice = new String(
                "Slice: " +
                String.valueOf(iSlice+1)+
                "/" +
                String.valueOf(iNumSlices) +
                "  ");
            ...
            ModelSimpleImage kSimpleImageTarget = new ModelSimpleImage (
                aiSliceExtents,
                afSliceResolutions,
                m_kImageSource,
                (-1 == m_iSliceTarget)
            ...
            ModelSimpleImage kSimpleImageSource = new ModelSimpleImage(
                aiSliceExtents,
                afSliceResolutions,
                m_kImageSource,
                iSlice);
            ...
            try {
                ...
                ModelSimpleImage[] akSimpleImageSourceMap =
                    akReg[iSlice].createImageSourceMap();
                ModelSimpleImage kSimpleImageResult =
                    kSimpleImageSourceOrig.createMappedImage2d(
                ...
                if (null != m_kImageDeformation) {
                    ModelSimpleImage kImageDataDeformation =
                        akReg[iSlice].createImageDeformation();
                    m_kImageDeformation.importData(
                        iStartImageDeformation,
                        kImageDataDeformation.data,
                        true); // compute min/max
     ...
     */
    protected BSplineRegistration2Df runPass(
        ...
        try {
            ...
            if (null != kRegPrev) {
                ...
                kReg = new BSplineRegistration2Df(
                    kSource,
                    kTargetUse,
                    kBasisX,
                    kBasisY,
                ...
                for (int iIteration = 0; iIteration < kOptions.iMaxIterations; iIteration++) {
                    ...
                    for (int iControlX = 1; iControlX < kOptions.iBSplineNumControlPoints - 1; iControlX++) {
                        for (int iControlY = 1; iControlY < kOptions.iBSplineNumControlPoints - 1; iControlY++) {
                            ...
                            kReg.minimizeControlPoint(iControlX, iControlY,
     ...
     * of control points.
     * @param akReg BSplineRegistration2Df[] Contains the parameters which
     * define the BSpline for registering each slice of the source image
     * to the particular reference slice.
     */
    private void writeResults(BSplineRegistration2Df[] akReg) {
        ...
        try {
            ...
            lineString = new String(
                String.valueOf(m_kImageSource.getFileInfo(0).getResolutions()[0]) + " " +
            ...
            for (int iSlice = 0; iSlice < m_kImageSource.getExtents()[2]; iSlice++) {
                lineString = new String("# Final values of control points for slice " +
                                        String.valueOf(iSlice) + "\n");
                line = lineString.getBytes();
                raFile.write(line);
                ...
                for (int iControlX = 0; iControlX < kBasisX.getNumControlPoints(); iControlX++) {
                    for (int iControlY = 0; iControlY < kBasisY.getNumControlPoints(); iControlY++) {
                        ...
                        lineString = new String(Float.toString(control2D.x) + "  " +
                                                Float.toString(control2D.y) + "\n");
                        line = lineString.getBytes();
                        raFile.write(line);
                    }
                }
     ...
     * If the value is -1, then each slice is registered to its adjacent one.
     */
