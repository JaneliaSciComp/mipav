...
public class DICOM_CResponse extends DICOM_CRequest {
    ...
    * @exception DICOMException  indicates an error
    */
    public void write (DICOM_PDUService   pdu,         DICOM_Object dco,         String       UID,
                       int                status,      String       errComment,  DICOM_Object ddo,
                       int                nRemaining,  int          nComplete,   int          nFailed,
                       int                nWarning )
        ...
        while( (vr = dco.pop()) != null ) {
            if( vr.group == 0x0000 ) {
                switch( vr.element ) {
                    case    0x0010:                      dcoRsp.push (vr); break;
                    case    0x0110: vr.element = 0x0120; dcoRsp.push (vr); break;
                    case    0x0200: vr.element = 0x0300; dcoRsp.push (vr); break;
                    case    0x0300: vr.element = 0x0200; dcoRsp.push (vr); break;
                    case    0x1000:                      dcoRsp.push (vr); break;
                }
            }
        }

        if( ddo != null ) { dcoRsp.setInt16( DICOM_RTC.DD_DataSetType, DICOM_Constants.DSTYPE_DATAPRESENT   ); }
        else              { dcoRsp.setInt16( DICOM_RTC.DD_DataSetType, DICOM_Constants.DSTYPE_NODATAPRESENT ); }
        ...
        if( errComment != null ) { dcoRsp.setStr( DICOM_RTC.DD_ErrorComment, errComment ); }
        ...
        if( ddo != null ) pdu.write(ddo, UID, (byte)0 );
    }
