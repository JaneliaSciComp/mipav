...
/**
 *	Performs original to ACPC, original to Talairach, ACPC to original,
 *	ACPC to Talairach, Talairach to original, or Talairach to ACPC
 *	transformation.
 *	@version    May 26, 2005
 *	@author     Pilou Bazin
 */
public class AlgorithmTalairachTransform extends AlgorithmBase {
    ...
    public AlgorithmTalairachTransform(ModelImage destImage, ModelImage srcImage,
                                       TalairachTransformInfo tInfo,
                                       int transformType, int interpolation,
                                       boolean doClip, boolean doVOI) {
    ...
    */
    ...
    public void runAlgorithm() {
        ...
        if (destImage != null){     // if there exists a destination image
            ...
            if ( (nix!=srcImage.getExtents()[0]) || (niy!=srcImage.getExtents()[1]) || (niz!=srcImage.getExtents()[2]) ) {
    ...
    */
        ...
        for (x=0;x<nrx && !threadStopped;x++) {
            for (y=0;y<nry && !threadStopped;y++) {
                for (z=0;z<nrz && !threadStopped;z++) {
                    if (n%mod==0) progressBar.updateValue(n/mod, activeImage);
                    n++;

                    // according to type
                    if (transformType == ORIG_TO_ACPC) tInfo.acpcToOrig(x,y,z, pt);
                    else if (transformType == ACPC_TO_ORIG) tInfo.origToAcpc(x,y,z, pt);
                    else if (transformType == ACPC_TO_TLRC) tInfo.tlrcToAcpc(x,y,z, pt);
                    else if (transformType == TLRC_TO_ACPC) tInfo.acpcToTlrc(x,y,z, pt);
                    else if (transformType == ORIG_TO_TLRC) tInfo.tlrcToOrig(x,y,z, pt);
                    else if (transformType == TLRC_TO_ORIG) tInfo.origToTlrc(x,y,z, pt);
                    ...
                    for (x = 0; x < nrx && !threadStopped; x++) {
                        for (y = 0; y < nry && !threadStopped; y++) {
                            for (z = 0; z < nrz && !threadStopped; z++) {
                                ...
                                if ((xi > 0) && (xi < nix - 1) && (yi > 0) &&
                                    (yi < niy - 1) && (zi > 0) && (zi < niz - 1)) {
     ...
     */
    private void computeTrilinearImage(float img[], float xi, float yi, float zi, float[] result, int xr, int yr, int zr) {
     ...
     */
    private void computeNearestImage(float img[], float xi, float yi, float zi, float[] result, int xr, int yr, int zr) {
        int xa,ya,za;
       ...
       if ( (xi>=0) && (xi<=nix-1) && (yi>=0) && (yi<=niy-1) && (zi>=0) && (zi<=niz-1) ) {
            ...
            ya = (int)Math.floor(yi);
            za = (int)Math.floor(zi);
                        if (xi-xa>0.5) xa = xa+1;
                        if (yi-ya>0.5) ya = ya+1;
                        if (zi-za>0.5) za = za+1;
    ...
    private void computeBSplineImage(float img[], float xi, float yi, float zi, float[] result,
    ...
    private void computeCubicLagrangian(float img[], float xi, float yi, float zi, float[] result,
                                     int xr, int yr, int zr) {
        if ( (xi>=0) && (xi<=nix-1) && (yi>=0) && (yi<=niy-1) && (zi>=0) && (zi<=niz-1) ) {
    ...
    private void computeQuinticLagrangian(float img[], float xi, float yi, float zi, float[] result,
                                     int xr, int yr, int zr) {
        if ( (xi>=0) && (xi<=nix-1) && (yi>=0) && (yi<=niy-1) && (zi>=0) && (zi<=niz-1) ) {
    ...
    private void computeHepticLagrangian(float img[], float xi, float yi, float zi, float[] result,
                                     int xr, int yr, int zr) {
        if ( (xi>=0) && (xi<=nix-1) && (yi>=0) && (yi<=niy-1) && (zi>=0) && (zi<=niz-1) ) {
    ...
    private void computeWSincImage(float img[], float xi, float yi, float zi, float[] result,
                                     int xr, int yr, int zr) {
        if ( (xi>=0) && (xi<=nix-1) && (yi>=0) && (yi<=niy-1) && (zi>=0) && (zi<=niz-1) ) {
