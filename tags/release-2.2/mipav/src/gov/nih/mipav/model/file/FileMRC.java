 ...
 * I am not familiar with the latest standards for MRC files, but
 * typically the extra header information has been used to save information
 * about each section in the file.   Our own implementation is largely
 * derived from that of the Agard group at UCSF, from which the Deltavision
 * software originates.  The Deltavision standard uses two fields called
 * &quot;nreal&quot; and &quot;nint&quot; to describe the number of 4-byte reals and integers
 * stored per section.  I deviated from that and used &quot;nint&quot; to hold the
 * number of bytes of data per section, and &quot;nreal&quot; to hold a set of
 * flags describing which kinds of data were present (tilt angle, frame X-Y-Z
 * coordinates for a montage, stage position, magnification, all stored as
 * 2-byte integers.).  This information is present only for data acquired on
 * our microscope here.
 */
public class FileMRC extends FileBase {
    ...
    */
    public ModelImage readImage() throws IOException {
        ...
        try {
            progressBar = new ViewJProgressBar(fileName, "Reading MRC file...",
         ...
         progressBar.setLocation((int)Toolkit.getDefaultToolkit().getScreenSize().getWidth() / 2,
            ...
            if (s.equals("MAP ")) {
                ...
                if (stamp == 17) {
                    ...
                    MipavUtil.displayError("Stamp byte at location 212 is an illegal " +
                                            stamp + "\n");
            ...
            if (fileSize > raFile.length()) {
                MipavUtil.displayError("Expected file size of " + fileSize +
                        " excceds actual file size of " + raFile.length());
            }
    ...
    private void readBuffer(int slice, float buffer[]) throws IOException {
        ...
        switch (mode) {
            case 0: // unsigned char
               ...
               /* j = 0;
                for (y = nYStart; y <nYStart + imgExtents[1]; y++) {
                    for (x = nXStart; x < nXStart + imgExtents[0]; x++) {
                        raFile.seek(1024 + x + imgExtents[0]*y +
                                    imgExtents[0]*imgExtents[1]*(nZStart+slice));
                        raFile.read(byteBuffer,j*imgExtents[0],imgExtents[0]);
                        j++;
                    }
                } */
            ...
            case 1: // short
                ...
                for (y = nYStart; y <nYStart + imgExtents[1]; y++) {
                    for (x = nXStart; x < nXStart + imgExtents[0]; x++) {
                        raFile.seek(1024 + 2*(x + imgExtents[0]*y +
                                    imgExtents[0]*imgExtents[1]*(nZStart+slice)));
                        raFile.read(byteBuffer,2*j*imgExtents[0],2*imgExtents[0]);
                        j++;
                    }
                }*/
                progress = slice*buffer.length;
            ...
            case 2: // float
                ...
                for (y = nYStart; y <nYStart + imgExtents[1]; y++) {
                    for (x = nXStart; x < nXStart + imgExtents[0]; x++) {
                        raFile.seek(1024 + 4*(x + imgExtents[0]*y +
                                    imgExtents[0]*imgExtents[1]*(nZStart+slice)));
                        raFile.read(byteBuffer,4*j*imgExtents[0],4*imgExtents[0]);
                        j++;
                    }
                }*/
                progress = slice*buffer.length;
                ...
                case 16: // unsigned char * 3, used for rgb data
                    ...
                    for (y = nYStart; y <nYStart + imgExtents[1]; y++) {
                        for (x = nXStart; x < nXStart + imgExtents[0]; x++) {
                            raFile.seek(1024 + 3*(x + imgExtents[0]*y +
                                        imgExtents[0]*imgExtents[1]*(nZStart+slice)));
                            raFile.read(byteBuffer,3*j*imgExtents[0],3*imgExtents[0]);
                            j++;
                        }
                    }*/
    ...
    private void readComplexBuffer(int slice, float buffer[], float buffer2[]) throws IOException {
        ...
        switch (mode) {
            ...
            case 4: // float * 2, used for complex data
                ...
                for (y = nYStart; y <nYStart + imgExtents[1]; y++) {
                    for (x = nXStart; x < nXStart + imgExtents[0]; x++) {
                        raFile.seek(1024 + 8*(x + imgExtents[0]*y +
                                    imgExtents[0]*imgExtents[1]*(nZStart+slice)));
                        raFile.read(byteBuffer,8*j*imgExtents[0],8*imgExtents[0]);
                        j++;
                    }
                }*/
                progress = slice*buffer.length;
    ...
    */
    public void writeImage(ModelImage image, FileWriteOptions options) throws IOException {
        ...
        progressBar = new ViewJProgressBar(fileName, "Writing MRC file...",
                                           0, 100, true, null, null);
        ...
        if ((resZUnit == FileInfoBase.INCHES)     || (resZUnit == FileInfoBase.CENTIMETERS) ||
           ...
           (resZUnit == FileInfoBase.MICROMETERS) ||
           (resZUnit == FileInfoBase.MILLIMETERS) || (resZUnit == FileInfoBase.METERS) ||
           (resZUnit == FileInfoBase.KILOMETERS)  || (resZUnit == FileInfoBase.MILES)) {
