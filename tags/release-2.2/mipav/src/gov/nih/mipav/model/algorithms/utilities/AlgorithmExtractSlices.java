...
/**
 *     Algorithm that extracts the slices indicated in the list from the srcImage and puts
 *     them into the destImage.
 *
 *      Note that an image is a set of slices; each slice is XxY, with Z slices.
 *
 *      @author          Lynne Pusanik
 *                          based on AlgorithmRemoveSlices
...
public class AlgorithmExtractSlices extends AlgorithmBase {
     ...
     *   @param  extractSlices   list of slice numbers indicating which slices
     *                           in source should be extracted to destination.
     *
     */
        ...
        if ( ( srcImage.getFileInfo()[0] ).getFileFormat() == FileBase.DICOM ) {
            ...
            if ( srcImage.getFileInfo( 1 ) != null ) {
                imagePositionCoords = convertIntoFloat(
                        ( (FileInfoDicom) srcImage.getFileInfo( 0 ) ).parseTagValue( "0020,0032" ) );
                nextPositionCoords = convertIntoFloat(
                        ( (FileInfoDicom) srcImage.getFileInfo( 1 ) ).parseTagValue( "0020,0032" ) );
                ...
                if ( ( nextPositionCoords[0] != imagePositionCoords[0] )
                    ...
                    MipavUtil.displayWarning(
                            "Extract Slices does not support changes in\n" + "image position (DICOM tag 0020,0032)\n"
                ...
                for ( zSrc = 0; zSrc < oldZdim && !threadStopped; zSrc++ ) { // for all slices in the src image
                    ...
                    if ( isProgressBarVisible() ) {
                        progressBar.updateValue(
                                Math.round( (float) ( t * oldZdim + zSrc ) / ( tDimSrc * oldZdim - 1 ) * 100 ),
                                activeImage );
                    }
                    ...
                    if ( extractList.contains( Integer.toString( zSrc ) ) ) {
                        ...
                        if ( ( srcImage.getFileInfo()[0] ).getFileFormat() == FileBase.DICOM ) {
                            ...
                            fileInfoBuffer.setValue(
                                    "0020,0013", String.valueOf( zDest + 1 ),
                            ...
                            // copy the "image position (patient)" info so that axis-position left by extracting slices (zSrc)
                            // will change by sliding the next included slice forward (to the zDest position).
                            // So: the X&Y info for the zSrc (the one not removed with some large index)
                            // and we use the zDest info from the index of the copy-to slice (which has a low index).
                            imagePositionCoords = convertIntoFloat(
                                    ( (FileInfoDicom)
                                    srcImage.getFileInfo( zSrc ) ).parseTagValue( "0020,0032" ) );
                            nextPositionCoords = convertIntoFloat( ( (FileInfoDicom)
                                    srcImage.getFileInfo( zDest ) ).parseTagValue( "0020,0032" ) );
    ...
    /**
     *   Method to return the extract list since local copies may change
