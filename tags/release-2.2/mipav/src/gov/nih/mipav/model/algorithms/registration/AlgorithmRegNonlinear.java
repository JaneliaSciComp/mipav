 ...
 *  AIR WWW site (currently <a href="http://bishopw.loni.ucla.edu/AIR5/">
 *  http://bishopw.loni.ucla.edu/AIR5/</a>. <p>
 *  References:<br>
 ...
 *      Tomography 1998; 22:155-165.<br>
 *  <p>
 *  The nonlinear registration algorithm is designed so that second
 *  order nonlinear transformations are initialized with results of
 *  an affine registration, and successively higher models are
 *  sequentially initialized with the best results obtained at lower
 *  orders.
 *
 *  From reference 2:
 *  For nonlinear models the usual AIR procedure for removing biases
 *  associated with defining one image as the reference image and the
 *  other as the reslice image cannot be used because the nonlinear
 *  transformations are not analytically invertible.  To compensate, the
 *  nonlinear algorithm includes in its cost function not only those voxels
 *  that are supratrheshold in the reference file but also those that are
 *  subthreshold in the reference file but that are suprathreshold in the
 *  corresponding voxels of the interpolated reslice image.  Without this
 *  compensation, optimization of the cost function tends to enlarge the
 *  reslice image until it completely covers all the edges of the reference
 *  image.
 *
...
public class AlgorithmRegNonlinear extends AlgorithmBase {
    ...
    public AlgorithmRegNonlinear( ModelImage resultImage, ModelImage targetImage, boolean entireTarget,
            ...
            int interpolation, boolean clipOutput,
            boolean interaction, boolean transformVOI, float sourceThreshold,
            float targetThreshold, float sourceGaussX, float sourceGaussY,
            float sourceGaussZ, float targetGaussX, float targetGaussY,
            float targetGaussZ, int sampleFactor2,
            int costFxn, float precision,
            int iterations, boolean writeParameters, double[][] esInitial ) {
    ...
    /**
     *   AlgorithmRegNonlinear - constructor
     *   This constructor used for runs aligning slices within a given 3D image
     *   to a chosen reference slice number
     *   If doAdjacent is true:
     *   First referenceSlice + 1 is registered to referenceSlice, then referenceSlice + 2
     *   is registered to referenceSlice + 1, and so on until the last slice is registered
     *   to the slice just before the last slice.  The referenceSlice - 1 is registered to
     *   referenceSlice, referenceSlice - 2 is registered to referenceSlice - 1, and so on
     *   until slice 0 is registered to slice 1.
     *   If doAdjacent is false:
     *   Register all slices to the referenceSlice
     *   @param resultImage
     *   @param referenceSlice  slice to which the other slices are aligned
     *   @param doAdjacent      if true register to adjacent slice
     *                          if false register to reference slice
     *   @param entireTarget    if false only selected VOI in target image is used
     *   @param sourceImage     image which is resliced to match the target image
     *   @param entireSource    if false only selected VOI in source image is used
     *   @param transformation  models using different numbers of parameters
     ...
     *   @param writeParameters
     */
    public AlgorithmRegNonlinear( ModelImage resultImage, int referenceSlice,
            boolean doAdjacent, boolean entireTarget,
            ModelImage sourceImage, boolean entireSource, int transformation,
            int interpolation, boolean clipOutput,
            boolean interaction, boolean transformVOI, float sourceThreshold,
            float targetThreshold, float sourceGaussX, float sourceGaussY,
            ...
            float targetGaussZ, int sampleFactor2,
            int costFxn, float precision,
            int iterations, boolean writeParameters ) {
        this.resultImage = resultImage;
     ...
     *   @param transformVOI
     */
    public AlgorithmRegNonlinear( ModelImage sourceImage,
            int targetXDim, int targetYDim, int targetZDim,
            float[] resultResolutions, int[] targetUnits,
            int fmodel, double[][] es, int interpolation, boolean clipOutput,
    ...
    public void paramRun() {
        ...
        if ( inPlane ) { // 2D
            if ( sourceImage.isColorImage() ) {
                dataOut = rWarp2DC( sourceArray, sourceXDim, sourceYDim, (float) sourceImage.getMin(),
                        (float) sourceImage.getMax(), targetXDim, targetYDim, targetZDim, es, fmodel - 20, interpolation );
            } else {
            ...
            dataOut = rWarp3D( sourceArray, sourceXDim, sourceYDim, sourceZDim, (float) sourceImage.getMin(),
                    (float) sourceImage.getMax(), targetXDim, targetYDim, targetZDim, es, fmodel, interpolation );
        }
        ...
        try {
            ...
            MipavUtil.displayError(
                    "AlgorithmRegNonlinear: Out of memory on resultImage" + ".importData(0,dataOut,true)" );
            disposeProgressBar();
            setCompleted( false );
            return;
        }
        ...
        if ( transformVOI ) {
            ...
            try {
                ...
                finalize();
                MipavUtil.displayError(
                        "AlgorithmRegNonlinear: IOException on maskImage" + ".importData(0,dataOut,true)" );
                disposeProgressBar();
                setCompleted( false );
                return;
            } catch ( OutOfMemoryError e ) {
                finalize();
                MipavUtil.displayError(
                        "AlgorithmRegNonlinear: Out of memory on maskImage" + ".importData(0,dataOut,true)" );
                disposeProgressBar();
                setCompleted( false );
                return;
            }
    ...
    public void runAlgorithm() {
        ...
        if ( series2D ) {
            ...
            try {
                ...
                finalize();
                MipavUtil.displayError(
                        "AlgorithmRegNonlinear: IOException on sourceImage.exportData"
            ...
            try {
                ...
                finalize();
                MipavUtil.displayError(
                        "AlgorithmRegNonlinear: IOException on resultImage"
            ...
            if ( !doAdjacent ) {
                ...
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "AlgorithmRegNonlinear: IOException on targetImage" + ".importData(0,targetArray,true)" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
        ...
        for ( series = 0; series < seriesNumber; series++ ) {
            if ( series2D ) {
                ...
                if ( doAdjacent ) {
                    if ( tNumber == referenceSlice ) {
                        ...
                        try {
                            ...
                            finalize();
                            MipavUtil.displayError(
                                    "AlgorithmRegNonlinear: IOException on resultImage.exportData"
                ...
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "AlgorithmRegNonlinear: IOException on sImage" + ".importData(0,sourceArray,true)" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
            ...
            if ( ( sourceGaussX != 0.0f ) || ( sourceGaussY != 0.0f ) || ( sourceGaussZ != 0.0f ) ) {
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "Algorithm Registration Nonlinear: unable to allocate enough memory for sourceGaussianImage" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
            ...
            if ( ( targetGaussX != 0.0f ) || ( targetGaussY != 0.0f ) || ( targetGaussZ != 0.0f ) ) {
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "Algorithm Registration Nonlinear: unable to allocate enough memory for targetGaussianImage" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
            ...
            while ( model <= fmodel ) {
                ...
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "Algorithm Registration Nonlinear: unable to allocate enough memory for matrices" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
                ...
                /*for (j = 0; j < coords; j++) {
                 for (k = 0; k < coeffp; k++) {
                 es[j][k] = tps[k+j*coeffp];
                 }
                 }
                 scale = tps[coords*coeffp];*/
                ...
                while ( sampleFactor >= sampleFactor2 ) {
                    ...
                    do { // while ((Math.abs(deltacf) > precision) && (!threadStopped))
                        ...
                        switch ( uvderivsN ) {
                        case UVDERIV2DWARP:
                            cf = uvderiv2Dwarp( parameters - 1, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceThreshold, targetArray, targetMask, targetXDim, targetYDim,
                                    targetZDim, targetThreshold, dcf, ecf, dx, scale );
                            break;

                        case UVDERIV3DWARP:
                            cf = uvderiv3Dwarp( parameters - 1, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceZDim, sourceThreshold, targetArray, targetMask, targetXDim,
                                    targetYDim, targetZDim, targetThreshold, dcf, ecf, dx, scale );
                            break;

                        case QVDERIV2DWARP:
                            cf = qvderiv2Dwarp( parameters - 1, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceThreshold, targetArray, targetMask, targetXDim, targetYDim,
                                    targetZDim, targetThreshold, dcf, ecf, dx, scale );
                            break;

                        case QVDERIV3DWARP:
                            cf = qvderiv3Dwarp( parameters - 1, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceZDim, sourceThreshold, targetArray, targetMask, targetXDim,
                                    targetYDim, targetZDim, targetThreshold, dcf, ecf, dx, scale );
                            break;
                        ...
                        case UVDERIVCOR2DWARP:
                            cf = uvderivCOR2Dwarp( parameters, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceThreshold, targetArray, targetMask, targetXDim, targetYDim,
                                    targetZDim, targetThreshold, dcf, ecf, dx, scale, q1, q2, q3, q4, q5, q6, q7, 256,
                                    maxActualValue, minActualValue );
                            break;

                        case QVDERIVCOR2DWARP:
                            cf = qvderivCOR2Dwarp( parameters, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceThreshold, targetArray, targetMask, targetXDim, targetYDim,
                                    targetZDim, targetThreshold, dcf, ecf, dx, scale, q1, q2, q3, q4, q5, q7, 256,
                                    maxActualValue, minActualValue );
                            break;
                        ...
                        case UVDERIVCOR3DWARP:
                            cf = uvderivCOR3Dwarp( parameters, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceZDim, sourceThreshold, targetArray, targetMask, targetXDim,
                                    targetYDim, targetZDim, targetThreshold, dcf, ecf, dx, scale, q1, q2, q3, q4, q5, q6,
                                    q7, 256, maxActualValue, minActualValue );
                            break;

                        case QVDERIVCOR3DWARP:
                            cf = qvderivCOR3Dwarp( parameters, es, sampleFactor, sourceArray, sourceMask, sourceXDim,
                                    sourceYDim, sourceZDim, sourceThreshold, targetArray, targetMask, targetXDim,
                                    targetYDim, targetZDim, targetThreshold, dcf, ecf, dx, scale, q1, q2, q3, q4, q5, q7,
                                    256, maxActualValue, minActualValue );
                            break;
                        } // switch(uvderivsN)
                        if ( error != 0 ) {
                            break;
                        ...
                        if ( dpofa( ecf2, parameters ) == parameters ) {
                            ...
                            if ( posdefreq ) {
                                ...
                                if ( sampleFactor / sfFactor >= sampleFactor2 ) {
                                    System.out.println(
                                            "Increasing sampling density due to non-positive definite Hessian\n" );
                                    break;
                                } else {
                        ...
                        Preferences.debug(
                                "iterations completed: " + iters + " sampling: " + sampleFactor + " parameters: "
                                + parameters + "\n" );
            ...
            if ( series2D ) {
                ...
                try {
                    ...
                    finalize();
                    MipavUtil.displayError(
                            "AlgorithmRegNonlinear: IOException on resultImage" + ".importData(0,dataOut,true)" );
                    disposeProgressBar();
                    setCompleted( false );
                    return;
                }
     ...
     */
    public final void writeInt( int data, boolean endianess )
        throws IOException {
     ...
     *   @exception IOException  if there is an error writing the file
     */
    public final void writeFloat( float data, boolean endianess )
        throws IOException {
        int tmpInt;
     ...
     *   @exception IOException  if there is an error writing the file
     */
    public final void writeDouble( double data, boolean endianess )
        throws IOException {
     ...
     *   @exception IOException  if there is an error writing the file
     */
    public final void writeLong( long data, boolean endianess )
        throws IOException {
    ...
    private double uvderiv2Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask,
            int x_dim1, int y_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask, int x_dim2, int y_dim2,
            int z_dim2, float threshold5, double[] dcf, double[][] ecf,
    ...
    private double uvderivCOR2Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask,
            int x_dim1, int y_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask, int x_dim2, int y_dim2,
            int z_dim2, float threshold5, double[] dcf, double[][] ecf, double[] dx, double scale,
            int[] count, double[] mean, double[] square, double[][] dmean,
            double[][] dsquare, double[][][] emean, double[][][] esquare,
    ...
    private double qvderivCOR2Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask,
            int x_dim1, int y_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask, int x_dim2, int y_dim2,
            int z_dim2, float threshold5, double[] dcf, double[][] ecf, double[] dx, double scale,
            int[] count, double[] mean, double[] square, double[][] dmean,
            double[][] dsquare, double[][][] esquare,
            int partitions, double maxActualValue, double minActualValue ) {
    ...
    private double uvderiv3Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask, int x_dim1,
            int y_dim1, int z_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask,
    ...
    private double uvderivCOR3Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask, int x_dim1,
            int y_dim1, int z_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask,
            int x_dim2, int y_dim2, int z_dim2, float threshold5, double[] dcf, double[][] ecf,
            double[] dx, double scale,
            int[] count, double[] mean, double[] square, double[][] dmean,
    ...
    private double qvderivCOR3Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask, int x_dim1,
            int y_dim1, int z_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask,
            int x_dim2, int y_dim2, int z_dim2, float threshold5, double[] dcf, double[][] ecf,
            double[] dx, double scale,
            int[] count, double[] mean, double[] square, double[][] dmean,
    ...
    private double qvderiv2Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask, int x_dim1,
            int y_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask, int x_dim2, int y_dim2,
            int z_dim2, float threshold5, double[] dcf, double[][] ecf,
            double[] dx, double scale ) {
        int counttotal = 0;
        double cf = 0.0; /* intermediate value */
    ...
    private double qvderiv3Dwarp( int parameters, double[][] es, int sampleFactor, float[] pixel2, boolean[] pixel2Mask, int x_dim1,
            int y_dim1, int z_dim1, float threshold2, float[] pixel5, boolean[] pixel5Mask,
            int x_dim2, int y_dim2, int z_dim2, float threshold5, double[] dcf, double[][] ecf,
            double[] dx, double scale ) {
     ...
     * Trilinear interpolation used
     */


    private float[] rWarp2D( float[] volume, int x_dim1, int y_dim1, float minLimit, float maxLimit,
            int x_dim2, int y_dim2, int z_dim2, double[][] es, int order,
            int interpolation ) {
        float[] new_volume;
        double x_max1;
        double y_max1;
     ...
     *         null if unsuccessful
     *
     * This routine will reslice a color image file based on the unit vectors es
     * Trilinear interpolation used
     */


    private float[] rWarp2DC( float[] volume, int x_dim1, int y_dim1, float minLimit, float maxLimit,
            int x_dim2, int y_dim2, int z_dim2, double[][] es, int order,
            int interpolation ) {
        float[] new_volume;
        double x_max1;
    ...
    private float[] rWarp3D( float[] volume, int x_dim1, int y_dim1, int z_dim1,
    ...
    /**
     *   From material Copyright 1995 Roger P. Woods, M.D.
     *   Modified 11/28/95
