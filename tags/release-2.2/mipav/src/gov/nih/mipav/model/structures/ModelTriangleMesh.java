...
/**
 * A simple triangle mesh that represents a level surface.  The mesh only
 * stores vertex positions and vertex normals.  The surface viewer creates
 * a Shape3D object whose geometry is an object from ModelTriangleMesh.  The
 * surface color is provided by attaching to the Shape3D object an appearance
 * that contains a material.  The vertex normals in ModelTriangleMesh are
 * used by the lighting system in conjunction with the surface material to
 * produce the surface color.
 */
...
public class ModelTriangleMesh extends IndexedTriangleArray {
    ...
    /**
     * A triangle mesh whose vertex normal vectors are computed from the
     * geometry of the mesh itself.  The normal at a vertex is the normalized
     * average of normals of the triangles that share the vertex.
     *
     * @param akVertex  array of vertices in the mesh
     * @param aiConnect  Connectivity array for the triangles.  Each triple of
     *   indices represents one triangle.  The triangle is counterclockwise
     *   ordered as viewed by an observer outside the mesh.
     */
     ...
     *   ordered as viewed by an observer outside the mesh.
     */
    public ModelTriangleMesh( Point3f[] akVertex, Vector3f[] akNormal,
            int[] aiConnect ) {
        super( akVertex.length, IndexedTriangleArray.COORDINATES | IndexedTriangleArray.NORMALS, aiConnect.length );
    ...
    /**
     * Support for regenerating an ModelTriangleMesh from an ModelClodMesh whenever
     * the level of detail is changed for the ModelClodMesh.  If the mesh was
     * generated from an ModelClodMesh, the generator member refers to that
     * ModelClodMesh object.  If the mesh was not generated from an ModelClodMesh,
     * the generator member is null.
     *
     ...
     *
     * @return A copy of the array of connectivity indices of the triangle
     *         mesh.
     */
     ...
     *
     * @return An array of connected components of the triangle mesh.  Each
     *         component has its own vertex and connectivity index arrays.
     */
    ...
    /**
     *	The input 4x4 homogeneous matrix H is assumed to multiply vectors V as
     *	H*V where V = (x,y,z,1).  Let the upper left 3x3 block of H be A.  Let
     *	the upper right 3x1 block of H be B.  The last row is [0 0 0 1].  Let
     *	the upper 3x1 block of V be X.  The lower 1x1 block is 1.  The product
     *	H*V = A*X + B.
     *	<p>
    ...
    /**
     *	Rotation R is 3x3, translation T is 3x1, and scale S is 3x1.  The
     *	product with 3x1 vector X is Diag(S)*R*X + T where Diag(S) is the
     *	diagonal matrix whose diagonal entries are the values of S.  As a
     *	homogeneous matrix (see notation above), A = diag(S)*R and B = T.
     *	<p>
    ...
    public void affineTransform( float[][] afRot, float[] afTrn, float[] afScale ) {
        ...
        for ( int iV = 0; iV < getVertexCount(); iV++ ) {
            ...
            // transform within image space
            float fX = afScale[0] * ( afRot[0][0] * m_kV0.x + afRot[0][1] * m_kV0.y + afRot[0][2] * m_kV0.z ) + afTrn[0];
            float fY = afScale[1] * ( afRot[1][0] * m_kV0.x + afRot[1][1] * m_kV0.y + afRot[1][2] * m_kV0.z ) + afTrn[1];
            float fZ = afScale[2] * ( afRot[2][0] * m_kV0.x + afRot[2][1] * m_kV0.y + afRot[2][2] * m_kV0.z ) + afTrn[2];
    ...
    /**
     *	Calculates volume of triangle mesh.  The mesh consists of triangle
     *   faces and encloses a bounded region.  Face j, 0 <= j <= n-1 has
     *   verticies P0, P1, and P2.  The order of the verticies is counterclockwise
     *   as you view the face from outside the bounded region.  The mesh is closed
     *   and manifold in the sense that each edge is shared by exactly two
     *   triangles.  The volume of the bounded region is:<br>
     *   <pre>
     *       V = 1/6 (Sum from j=0 to n-1 of {P0 dot P1 cross P2})
     *   </pre>
     *   The terms of the summation can be positive, negative, or zero.  The term
     *   is positive if the face is counterclockwise when viewed from the zero
     *   vector, or zero if the face appears to be a line segment when viewed from
     *   the zero vector. NOTICE THAT THERE ARE 2 DIFFERENT DEFINITIONS OF
     *   COUNTERCLOCKWISE, COUNTERCLOCKWISE AS VIEWED FROM OUTSIDE THE BOUNDED
     *   REGION AND COUNTERCLOCKWISE AS VIEWED FROM THE ZERO VECTOR.
     *   <p>
     *   A 3D image on a rectangular lattice contains points (i0, i1, i2) where
     *   0 <= ik < Bk for specified dimension bounds Bk.  These are just indicies.
     *   The actual physical measurements are provided by scaling factors Dk > 0.
     *   For example, a 256x256x256 MRI has B0 = B1 = B2 = 256.  If each voxel is
     *   1 mm in x, 1 mm in y, and 5 mm in z, then D0 = D1 = 1 and D2 = 5.  The
     *   3D image encloses a rectangular region [0,C0] x [0,C1] x [0,C2] in
     *   <em>physical space</em> where Ck = Dk*Bk.  In the example, C0 = D0*B0 =
     *   256 mm in x, C1 = D1*B1 = 256 mm in y, and C2 = D2*B2 = 1280 mm in z.
     *   Volume calculations are required to use physical measurements.  In the
     *   example, volume will be in cubic millimeters.
     *   <p>
     *   The surface extraction is performed by mapping [0,C0] x [0,C1] x [0,C2]
     *   into [-1,1] x [-1,1] x [-1,1] using uniform scaling.  This is done to
     *   keep the floating point values within order 1 to avoid the floating
     *   point errors that occur if you were to use the index values themselves.
     *   The topology of a level surface is invariant under any scaling (not
     *   just uniform), but the continuous level of detail algorithm for triangle
     *   decimation does edge collapses based on various geometric measurements of
     *   the mesh representing the level surface.  The geometric measurements are
     *   not invariant under nonuniform scaling.  Map the image into a cube using
     *   uniform scaling so that the triangle collapse order is invariant.  The
     *   uniform scaling is done so that the largest image dimension [0,M] is
     *   mapped to [-1,1].  The other ranges are mapped to intervals of the form
     *   [-L,L) where L < 1.  If (i0,i1,i2) is in [0,B0) x [0,B1) x [0,B2), the
     *   corresponding (x0,x1,x2) in [-1,1) is: <br>
     *   <pre>
     *               2*Dk*ik - Ck
     *       xk =    ------------
     *               max{C0,C1,C2}
     *   </pre>
     *   <p>
     *   However, we want to map from [0,Bk) to an inclusive interval [-Rk,Rk], where
     *   0 < Rk < 1 and Rk = 1 - Dk/max{C0,C1,C2}.  This ensures that surfaces begin in
     *   the center of a voxel rather than at the (0,0,0) corner of the voxel.  The problem
     *   is easiest to see in the Z direction: a surface that should cover the full Z range
     *   will end before the last slice.  Therefore, the formula should be: <br>
     *   <pre>
     *               2*Dk*ik - Ck + Dk
     *       xk =    -----------------
     *                 max{C0,C1,C2}
     *   </pre>
     *   <p>
     *   Once a closed manifold triangle mesh is extracted, the problem is now to
     *   compute its volume in physical space.  Note that ik are indicies in the
     *   original image, but the true physical length that is measured (relative
     *   to other index locations) is yk = Dk*ik.  Any triangle mesh (x0,x1,x2)
     *   must be mapped to (y0,y1,y2) before the volume calculation.  The mapping
     *   is: <br>
     *   <pre>
     ...
     *   </pre>
     *   <p>
     *   The volume calculations use the previously mentioned formula where the P
     *   points are the (y0,y1,y2) values.
     *
    ...
    /**
     * Calculate the surface mesh area.   Each surface mesh is composed of triangles.
     * Calculate the surface area from the summation of the each triangle area.
     * Based on the Area by Stokes' Theorem.
     *   Area(S) = 1/2 * Normal dot ( Sum from i = 0 to n-1 of ( V1 cross V2) )
     *   ==>
     *   Area(S) = 1/2 * ( Sum from i = 0 to n-1 of ( Normal dot ( V1 cross V2) ) )
     * @return float
     */
    public float area() {
         ...
         for ( int iT = 0; iT < getIndexCount(); /**/) {
            ...
            /* The surface mesh is generated by the triangular isosurfaces.
               The surface area of the object is just the sum of the area of these triangular isosurfaces.
               Formula to calculate the area of an arbitrary triangle, assume the length of the sides of
               an arbitrary triangle are a, b and c.
                Area = Sqrt(p(p-a)(p-b)(p-c)), where p = 1/2(a + b + c)
               To find the length of the sides of a triangle, use the vector formula as the vertices
               of the triangle are in 3D space.
               Let the vertices of one side of the triangle are (x1, y1, z1) and (x2, y2, z2).
               The length of the side is equal to
                 side = Sqrt((x2-x1)^2+(y2-y1)^2+(z2-z1)^2)
             */
            /*double a, b, c, p;
            a = Math.sqrt((double)((m_kV1.x-m_kV0.x)*(m_kV1.x-m_kV0.x) + (m_kV1.y-m_kV0.y)*(m_kV1.y-m_kV0.y) + (m_kV1.z-m_kV0.z)*(m_kV1.z-m_kV0.z)));
            b = Math.sqrt((double)((m_kV2.x-m_kV1.x)*(m_kV2.x-m_kV1.x) + (m_kV2.y-m_kV1.y)*(m_kV2.y-m_kV1.y) + (m_kV2.z-m_kV1.z)*(m_kV2.z-m_kV1.z)));
            c = Math.sqrt((double)((m_kV0.x-m_kV2.x)*(m_kV0.x-m_kV2.x) + (m_kV0.y-m_kV2.y)*(m_kV0.y-m_kV2.y) + (m_kV0.z-m_kV2.z)*(m_kV0.z-m_kV2.z)));
            p = ( a + b + c ) / 2;
            float fProd = (float)(Math.sqrt(p*(p-a)*(p-b)*(p-c)));
    ...
    /**
     *  Smooth mesh.  The formula can be found in "The Visualization Toolkit" by
     *  Will Schoeder, Ken Martin, and Bill Lorensen, p. 389.  Mesh smoothing moves the
     *  verticies of the mesh closer to an average of the points.  Each point is
     *  moved so that it is the average of the points around it.  The formula is:
     *  <pre>
     *      xi+1 = xi + (alpha * (Sum from j=0 to n of {xj - xi}))
     *  </pre>
     *  where xi+1 is the new point, xi is the orginal point, and the xjs are points
     *  that are connected to xi.  Alpha is some smoothing factor between .01 and .10.
     *  This formula is run for a number of iterations to obtain a smooth mesh.  Usually
     *  the iterations will be between 50 and 100.
     *  @param iteration    Number of times to run smoothing formula on data set.
     *  @param alpha        Smoothing factor.
     *  @param volumeLimit if true stop iterations when the present volume is
     *                      volumePercent or more different from the initial
     *  @param volumePercent percentage from initial volume for stopping
     *                       iterations
     *  @param pVisible      if true display progress bar
     */
    public void smoothMesh( int iteration, float alpha, boolean volumeLimit,
            float volumePercent, boolean pVisible ) {
        int i;
        int num;
    ...
    /**
     *  Smooth mesh.
     *  This method smoothes without shrinking by 2 Gaussian smoothing steps.
     *  First, a Gaussian smoothing is performed with a positive scale factor
     *  lambda.  Second, a Gaussian smoothing is performed with a negative scale
     *  factor mu, which is greater in magnitude than lambda.  To produce a
     *  significant smoothing, these steps must be repeated a number of times.
     *  3 references for this smoothing:
     *  1.) Curve and Surface Smoothing Without Shrinkage by Gabriel Taubin,
     *  Technical Report RC-19536, IBM Research, April, 1994. (also in
     *  Proceedings, Fifth International Conference on Computer Vision, pages
     *  852-857, June, 1995).
     *  2.) A Signal Processing Approach to Fair Surface Design by Gabriel Taubin,
     *  Computer Graphics, pages 351-358, August, 1995 (Proceedings SIGGRAPH '95).
     *  3.) Optimal Surface Smoothing as Filter Design by Gabriel Taubin,
     *  Tong Zhang, and Gene Golub, IBM Research Report RC-20404 (#90237).
     *  Usually the iterations will be between 30 and 100.
     *  @param iteration    Number of times to run smoothing formula on data set.
    ...
    /**
     *  Builds a list of cross-references, so that connections[i]
     *  contains all the verticies that are connected to vertex at i.
     */
    ...
    /**
     * Derived from the first 2 of the 3 components of AlgorithmBrainExtraction
     * Note that m_fStiffness does not increase and then decrease as in
     * AlgorithmBrainExtraction but instead remains constant.  This is because
     * the change in  m_fStiffness only applies to the third component of
     * AlgorithmBrainExtraction which uses image intensities.
     *  @param iterations
     *  @param m_fStiffness
     *  @param volumeLimit if true stop iterations when the present volume is
     *                      volumePercent or more different from the initial
     ...
     *  @param pVisible      if true display progress bar
     */
    public void smoothTwo( int iterations, float m_fStiffness, boolean volumeLimit,
            float volumePercent, boolean pVisible ) {
        int i;
        float initialVolume = 0.0f;
        float presentVolume;
     ...
     * evolution of the triangle mesh that approximates the brain surface.
     ...
     * @param akComponent
     * @param flip if the y and z axes should be flipped - true in extract and
     *			   in save of JDialogSurface
     * To have proper orientations in surface file if flip is true flip y and
     * z on reading.
     * param direction 1 or -1 for each axis
     * param startLocation
     * param box (dim-1)*resolution
     * @exception IOException if the specified file could not be opened for
     *            writing
     */
    static public void saveAsVRML( String kName, ModelTriangleMesh[] akComponent, boolean flip,
            int[] direction, float[] startLocation,
            float[] box, Color3f color )
        throws IOException {
     ...
     *            writing
     */
    public void saveAsVRML( String kName, boolean flip, int numberOfShapes,
     ...
     */
    protected void saveAsVRML( PrintWriter kOut, boolean flip, int[] direction,
            float[] startLocation, float[] box, Color3f color )
        throws IOException {
     ...
     *            writing
     */
    static public void saveAsPortableVRML( PrintWriter kOut, ModelTriangleMesh[] akComponent,
            boolean flip,
            int[] direction, float[] startLocation,
            float[] box, Color3f color )
        throws IOException {
     ...
     *            writing
     */
    public void saveAsPortableVRML( PrintWriter kOut, boolean flip, int numberOfShapes,
            int[] direction, float[] startLocation, float[] box, Color3f color )
        throws
                IOException {
        // kOut.println("#VRML V2.0 utf8"); // object is ModelTriangleMesh
     ...
     * @exception IOException if there is an error writing to the file
     */
    protected void saveAsPortableVRML( PrintWriter kOut, boolean flip, int[] direction,
            float[] startLocation, float[] box, Color3f color )
        throws
                IOException {
        Point3f kVertex = new Point3f();
     ...
     */
    static public int parseVRMLMesh( RandomAccessFile kIn )
    ...
    /**
     * Load the triangle mesh from a VRML file specifically written by MIPAV!.  The caller must have
     * already opened the file. Must be made more robust to better parse the file. It only reads
     * VRML 2.0 that MIPAV has written.
     *
     ...
     */
    static public ModelTriangleMesh loadVRMLMesh( RandomAccessFile kIn, ViewJProgressBar progress, int added, int total, boolean flag )
        throws IOException {
            ...
            if ( flag ) {
                if ( kIn.getFilePointer() == 0 ) {
                    ...
                    if ( !str.equals( "#VRML V2.0 utf8" ) || !str2.equals( "#MIPAV" )
                        ...
                        Preferences.debug(
                                "ModelTriangleMesh.loadVRMLMesh: File doesn't appear to be a VRML 2.0 file written by MIPAV.\n" );
                        // or throw error.
                        return null;
                    }
     ...
     *            writing
     */
    public void saveAsTextFile( String kName )
        throws IOException {
        PrintWriter kOut = new PrintWriter( new FileWriter( kName ) );
    ...
    /**
     * Save an array of triangle meshes to a text file.  The format for the
     * file is
     * <pre>
     ...
     *            writing
     */
    static public void saveAsTextFile( String kName, ModelTriangleMesh[] akComponent )
        throws IOException {
        if ( akComponent.length == 0 ) {
            return;
        }
    ...
    /**
     * Internal support for 'void print (String)' and
     * 'void print (String, ModelTriangleMesh[])'.  ModelTriangleMesh uses this
     * function to print vertices, normals, and connectivity indices to the
     * file.  ModelClodMesh overrides this to additionally print collapse
     * records to the file
     *
     * @param kOut the file to which the triangle mesh is saved
     * @exception IOException if there is an error writing to the file
     */
    protected void print( PrintWriter kOut )
        throws IOException {
        Point3f kVertex = new Point3f();
        Vector3f kNormal = new Vector3f();
    ...
    /**
     * Save an array of triangle meshes to a binary file.  The format for the
     * file is
     * <pre>
     * int type;  // 0 = ModelTriangleMesh, 1 = ModelClodMesh
     * int aCount;  // number of array elements
     ...
     * @param startLocation
     * @param box (dim-1)*res
     * @param inverseDicomArray
     * @exception IOException if there is an error writing to the file
     */
    static public void save( String kName, ModelTriangleMesh[] akComponent, boolean flip,
            int[] direction, float[] startLocation, float[] box,
        ...
        if (i > 0 && i < kName.length() - 1) {
          ...
          if (extension.equals("txt")) {
            ...
            for ( i = 0; i < akComponent.length; i++ ) {
                akComponent[i].save( kOut, flip, direction, startLocation, box,
                                     inverseDicomArray);
            }
          }
        }
        ...
        if ( Preferences.isDebug() ) {
            print( JDialogBase.makeImageName( kName, "_sur.txt" ), akComponent );
        } */
    }
    ...
    public void save( String kName, boolean flip, int[] direction, float[] startLocation,
            float[] box, double[][] inverseDicomArray)
        throws IOException {
        ...
        /*
        if ( Preferences.isDebug() ) {
            saveAsTextFile( JDialogBase.makeImageName( kName, "_sur.txt" ) );
        } */
    }
     ...
     *
     * @param kOut the file to which the triangle mesh is saved
     * @param flip if the y and z axes should be flipped - true in extraction algorithms
     *             and in JDialogSurface.
     * To have proper orientations in surface file if flip is true flip y and
     * z on reading.
     * @param direction either 1 or -1 for each axis
     * @param startLocation
     ...
     * @exception IOException if there is an error writing to the file
     */
    protected void save( RandomAccessFile kOut, boolean flip, int[] direction,
            float[] startLocation, float[] box, double[][] inverseDicomArray)
        throws IOException {
    ...
    /**
     * Load the triangle mesh from a binary file.  The caller must have
     * already opened the file and read the mesh type (0 = ModelTriangleMesh,
     * 1 = ModelClodMesh) and the number of meshes in the file.  The caller
     * then calls this function for each mesh.  The format for a mesh is
     * <pre>
     * int vCount;  // number of vertices
     * Point3f vertices[vCount];
     * Point3f normals[vCount];
     ...
     * @exception IOException if there is an error reading from the file
     */
    static public ModelTriangleMesh loadTMesh( RandomAccessFile kIn, ViewJProgressBar progress, int added, int total, boolean isVisible )
        throws IOException {
        try {
            int i, index, tmpInt;
            ...
            if (dicom) {
              ...
              for (i = 0; i <= 3; i++) {
                for (j = 0; j <= 3; j++) {
                  ...
                  tmpLong = ((c1 << 56) | (c2 << 48) | (c3 << 40) | (c4 << 32) |
                             (c5 << 24) | (c6 << 16) | (c7 << 8) | c8);
     ...
     * @exception IOException if there is an error reading from the file
     */
    static public ModelTriangleMesh loadTMesh( RandomAccessFile kIn, JProgressBar pBar, int added, int total )
        throws IOException {
        try {
            int i, index, tmpInt;
            int b1 = 0, b2 = 0, b3 = 0, b4 = 0;
    ...
    // The ModelClodMesh object that generated the ModelTriangleMesh object, null
    // otherwise.
