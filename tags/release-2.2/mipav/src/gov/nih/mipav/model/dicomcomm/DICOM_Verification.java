...
/**
 *  This is the DICOM verification class that defines functions to perform Verification
 *  to a DICOM image file archive such as the Kodak PACS image file server located
 *  in NIH's Clinical Center.  ("Pings" the server)
 *
 *      @version    1.0
   ...
   */
    ...
    */
    private void verifySOPClass( DICOM_Object dco ) throws DICOM_Exception {
        ...
        if( dco == null ) { throw new DICOM_Exception( "DCO is null" ); }

        dcoUID = dco.getStr( DICOM_RTC.DD_AffectedSOPClassUID );

        if( dcoUID == null ) { throw new DICOM_Exception( "SOP class UID is null" ); }
