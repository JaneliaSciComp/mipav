...
public abstract class AlgorithmPowellOptBase extends AlgorithmBase {
    ...
    protected double lineMinimization( double initial, double boundguess, double[] directions ) {
        ...
        while ( ( ( ++count ) < 100 ) && ( Math.abs( bracket.c - bracket.a ) > unit_tolerance )
                && !parent.isThreadStopped() ) {
            if ( count > 0 ) {
    ...
    /**
     *   Creates a good "bracket" for the minimum value.  A "bracket" is a set of three points
    ...
    private Bracket minimumBracket( Bracket bracket, double[] unit_directions ) {
        ...
        while ( bracket.functionAtB > bracket.functionAtC ) { // note: must maintain bracket.functionAtA >= bracket.functionAtB
    ...
    /**
     *   Takes the estimated minimum of the quadratic, checks to see
     *   if it is valid; if so, returns it, if not, returns extrapolated
     *   point instead.
