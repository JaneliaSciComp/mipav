 ...
 *
 * Routines that wish to sample all integer x,y values along a line must sample the line at 1/2 unit increments. As an
 * example, suppose we have a line with the following (x,y) values: (0.2,0.7),(0.7,1.2),(1.2,1.7), (1.7,2.2). If we
 * sample at unit increments with rounding we could either obtain the points (0,1),(1,2) or (1,1),(2,2). By sampling at
 * 1/2 unit increments along the line we sample at (0,1),(1,1),(1,2),(2,2).
 *
 ...
 * @see VOIPoint
 *
 * $Logfile: /mipav/src/gov/nih/mipav/model/structures/VOIContour.java $ $Revision: 114 $ $Date: 2/24/06 3:34p $
 */
public class VOIContour extends VOIBase {
     ...
     *
     * @param constraint factor that controls the number of points removed. A larger constraint removes more points 0.50
     *            typical - removes most "almost/and collinear" points 0.10 - removes only "collinear" points
    ...
    public boolean isCounterClockwise() {
        ...
        for (i = 1; i < length; i++ ) {
            if ( ((Point3Df) (elementAt(i))).x <= ((Point3Df) (elementAt(k))).x
                    && ( ((Point3Df) (elementAt(i))).x < ((Point3Df) (elementAt(k))).x || ((Point3Df) (elementAt(i))).y < ((Point3Df) (elementAt(k))).y)) {
                k = i;
            }
        }
    ...
    public void drawSelf(float zoomX, float zoomY, float resolutionX, float resolutionY, float originX, float originY,
        ...
        if (active == true) {
            ...
            if (nearPoint == NOT_A_POINT && lastPoint != NOT_A_POINT && lastPoint >= 0 && this.size() > lastPoint) {
    ...
    public void transformContour(TransMatrix tMatrix, boolean doRound) {
        ...
        for (i = 0; i < size(); i++ ) {
            ...
            if (doRound) {
                setElementAt(
                        new Point3Df(MipavMath.round(point.x), MipavMath.round(point.y), MipavMath.round(point.z)), i);
            } else {
                setElementAt(new Point3Df(point.x, point.y, point.z), i);
            }
        }
     ...
     *            http://www.caip.rutgers.edu/~xswang/object-segment/objhtml/node4.html Object Quantification by Simon
     *            Xin Wang
     */
    public void secondOrderAttributes(int xDim, int yDim, float xRes, float yRes, int xUnits, int yUnits,
            float[] pAxis, float[] eccentricity, float[] majorAxis, float[] minorAxis) {
        int nPts = 0;
        BitSet mask;
        try {
    ...
    public void secondOrderAttributeslsq(float xRes, float yRes, int xUnits, int yUnits, float[] pAxis,
        ...
        par[5] = A[0] * sy * sy * mx * mx + A[1] * sx * sy * mx * my + A[2] * sx * sx * my * my - A[3] * sx * sy * sy
                * mx - A[4] * sx * sx * sy * my + A[5] * sx * sx * sy * sy;
    ...
    public float calcPerimeter(float xRes, float yRes) {
        ...
        for (idx = 0; idx < xPts.length - 1; idx++ ) {
            perimeter += MipavMath.distance(xPts[idx] * xRes, xPts[idx + 1] * xRes, yPts[idx] * yRes, yPts[idx + 1]
                    * yRes);
        }
        if (closed) {
    ...
    public boolean contains(int _x, int _y, boolean forceReload) {
        ...
        for (i = 0; i < nPts; i++ ) {
            if (yPts[j] <= y && y < yPts[i] && areaTwice(xPts[i], yPts[i], xPts[j], yPts[j], x, y) >= 0 || yPts[i] <= y
                    && y < yPts[j] && areaTwice(xPts[j], yPts[j], xPts[i], yPts[i], x, y) >= 0) {
                isInside = !isInside;
            }
            j = i;
        }
     ...
     */
    private final float areaTwice(float ptAx, float ptAy, float ptBx, float ptBy, float ptCx, float ptCy) {
