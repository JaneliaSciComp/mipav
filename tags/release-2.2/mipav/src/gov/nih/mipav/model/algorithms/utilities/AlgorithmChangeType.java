...
/**
 *      This is a convenience function to convert from one image type to
 *      another and remap the data into a new range. It assumes that that
 *      calling function has checked that the range values are in the range
 *      of the image type. For example, an Unsigned Byte image type can
 *      represent data from 0 - 255 and therefore if converting from a Short
 *      type (-32768 to 32767) the range parameters should fall between 0 and
 *      255. This class can also be used to remap data to a different range
...
public class AlgorithmChangeType extends AlgorithmBase {
     ...
     */
    public AlgorithmChangeType( ModelImage destImg, ModelImage srcImg,
            double stRge1, double endRge1,
            double stRge2, double endRge2,
            boolean img25D ) {
     ...
     *   @param endRge2   ending value of intensities of the destination image
     */
    public AlgorithmChangeType( ModelImage srcImg, int newType,
            double stRge1, double endRge1,
            double stRge2, double endRge2,
            boolean img25D ) {
    ...
    private void constructLog( boolean destinationFlag ) {
        historyString = new String(
                "ChangeType(" + stRange1 + ", " + endRange1 + ", " + stRange2 + ", " + endRange2 + ")" + "\n" );
    }
    ...
    /**
     *   This function replaces the source image with the
     *   new type image and new data range.
     */
    ...
    /**
     *   This function produces a new image with the
     *   new data type and new data range.
     */
                ...
                for ( j = 0; j < length && !threadStopped; j++ ) {
                    if ( ( ( i * length ) + j ) % mod == 0 && isProgressBarVisible() ) {
                        progressBar.updateValue(
                                Math.round( (float) ( ( i * length ) + j ) / ( ( numSlices * length ) - 1 ) * 100 ),
                                activeImage );
                    }
