...
/**
 *  Quantify image data based on a mask image. At present only calculates the
 *  the number of voxels, volume, and total intensity for an object defined
 *  in a mask image.
 *
...
public class AlgorithmQuantify extends AlgorithmBase {
    ...
    /**
    *   The mask image. The range of the image [0, positive integer value]
    *   where zero is the background and a positive number indicates that
    *   is part of an object that has the same positive integer number.
    */
    ...
    */
    ...
    /**
    *   Private class to AlgorithmQuantify used to store the number of voxels, volume, and
    *   total intensity for an object defined in a mask image. This class also has a function
    *   to output the statistics calculated.
    */
    private class MaskObject {
        ...
        /**
        *   Outputs the area (2D) or volume (3D) the user interface for the specificed
        *   image and object
        *
        ...
        */
        public void output(ModelImage image, int objNo) {
           ...
           ViewUserInterface UI = image.getUserInterface();
            ...
            if(image.getNDims() == 2) {
                area = (nVoxels * image.getFileInfo(0).getResolutions()[0]  *
                                  image.getFileInfo(0).getResolutions()[1]);
