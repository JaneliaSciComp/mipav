...
/**
 *
 *  An optional registration may be performed before colocalization.  In this
 *  registration both images or colors are moved halfway to a common center
 *  point.  AlgorithmRegOAR2D is used with the cost function being the only
 *  registration parameter the user can vary in the dialog box.  Correlation
 *  ratio is the default, but the user can also select least squares,
 *  normalized cross correlation, or normalized mutual information.  The
 *  colocalization will be performed on the registered image or images rather
 *  than on the original image.  If the maximum data range is decreased,
 *  the bin1 or bin2 will be decreased if necessary to keep them from
 *  exceeding the actual number of channels in the data range.
 *
 *
 * A histogram buffer is generated with bin1 counts along the x axis and bin2 counts
 * along the y axis.  bin1 and bin2 are user selectable in the dialog.  leftPad,
 * rightPad, bottomPad, and topPad spaces are left around the data area in histBuffer
 * to allow room for axis lines and labels.
 *
 *
 * The user specifies the number of gaussian distributions.  For each distribution the
 * initial values of the means, standard deviations, and correlation are assigned using
 * a random number generator. Each distribution is initially assigned an equal probability.
 *
 * The ellipse corresponds to a contour of constant probabliity density on which
 * the density is smaller by a factor of exp[-0.5] than it is at the mean point.
 * The probability density function is a jointly gaussian or bivariate gaussian
 * density function. In each iteration:
 * 1.) Find the gaussian distribution j with the form:
 * p(x,y/j) = (1/(2*PI*stdxj*stdyj*sqrt(1 - rhoj**2))) *
 *          exp{(-1/(2*(1 - rhoj**2)))[((x-meanxj)**2)/(stdxj**2)
 *              - (2*rhoj*(x-meanxj)*(y-meanyj))/(stdxj*stdyj) + ((y - meanyj)**2)/(stdyj**2)]}
 * where stdxj and stdyj are the x and y standard deviations and rhoj is the correlation
 * coefficient.
 *
 * 2.) p(x,y) are found from p(x,y) = sum over j of p(x,y/j)*p(j).
 * 3.) Pold(j/(x,y) = (sum over all j of Pold(x,y/j)*Pold(j))/Pold(x,y)
 *
 * The update equations for the values of the new variables for each gaussian distribution j are:
 * 4.) meanxj = (sum over x,y of Pold(j/(x,y))*x)/(sum over x,y of Pold(j/(x,y)))
 * 5.) meanyj = (sum over x,y of Pold(j/(x,y))*y)/(sum over x,y of Pold(j/(x,y)))
 * 6.) stdxj**2 = (sum over x,y of Pold(j/(x,y)) * (x - meanxj)**2)/
 (sum over x,y of Pold(j/(x,y)))
 * 7.) stdyj**2 = (sum over x,y of Pold(j/(x,y)) * (y - meanyj)**2)/
 (sum over x,y of Pold(j/(x,y)))
 * 8.) rhoj = (sum over x,y of Pold(j/(x,y)) * (x - meanxj) * (y - meanyj)/(stdjx * stdjy))/
 (sum over x,y of Pold(j/(x,y)))
 *
 * 9.) Each new p[j] is found from (sum over x,y of Pold(j/(x,y)))/number of (x,y) points
 * These 9 basic steps are iterated the number of times specified by the user.
 *
 * The calculated data is passed to ViewJFrameColocalization.
 *
 * References:
 * 1.) Expectation Maximization Algorithm by Ismet Bayraktaroglu
 *     http://www-cse.ucsd.edu/users/ibayrakt/java/em
 *
 * 2.) Neural Networks for Pattern Recognition by Christopher M. Bishop,
 *     Oxford University Press, 1995, Section 2.6.2 The EM algorithm,
 *     pp. 65 - 72.
 *
 * 3.) Probability, Random Variables, and Random Signal Principles, Second Edition
 *     by Peyton Z. Peebles, Jr., McGraw-Hill Book Company, 1987, Section 5.3
 *     Jointly Gaussian Random Variables, pp. 124 - 128.
 *
 * 4.) Clustering Algorithms by John A. Hartigan, Wiley, John & Sons, Incorporated,
 *     1975, Chapter 5, Mixtures, pp. 113 - 129.
 *
 * 5.) Principles of Data Mining by David Hand, Heikki Mannila, and Padhraic Smith,
 *     A Bradford Book The MIT Press, 2001, pp. 260 - 265, 281 - 284, 315 - 323.
 *
 */
...
public class AlgorithmColocalizationEM extends AlgorithmBase {
     ...
     *   destination image.  Used for 2 black and white images
     ...
     *   @param register     If true register 2 black and white images
     *                       before colocalization
     *   @param cost         Cost function used in registration
     *   @param gaussians    Number of gaussians
     *   @param iterations   Number of iterations
     */
    public AlgorithmColocalizationEM( ModelImage destImg, ModelImage segImage,
            ModelImage srcImg, ModelImage baseImage,
            int bin1, int bin2, float threshold1,
            float threshold2, boolean doOr, int leftPad, int rightPad,
            int bottomPad, int topPad,
            boolean entireImage,
            boolean register, int cost,
            int gaussians, int iterations ) {
    ...
    /**
     *   Constructor for images in which 2D histogram is placed in a predetermined
     *   destination image.  Used for 1 color image.
     *   @param destImg      image model where result image is to stored
     *   @param segImage     image to display classes to which pixels belong
    ...
    private void constructLog() {
        historyString = new String(
                "ColocalizationEM(" + bin1 + ", " + bin2 + ", " + threshold1 + ", " + threshold2 + ", " + doOr + ", "
                + useRed + ", " + useGreen + ", " + useBlue + ", " + entireImage + ", " + register + ", " + cost + ", "
    ...
    /**
     *   This function produces a 2D histogram image with srcImage values
     *   represented across the x axis and baseImage values represented
     *   across the y axis.
     */
    private void calc2D() {
        int i;
        ...
        try {
            try {
                ...
                if ( threadStopped ) {
                    Preferences.debug(
                            "somehow you managed to cancel the algorithm "
        ...
        frameColocalize = new ViewJFrameColocalizationEM( registeredSrcImage, null, registeredBaseImage, null, null,
                destImage, controlFrame, useRed, useGreen, useBlue, min1, max1, min2, max2,
                scale1, scale2, leftPad, rightPad, bottomPad, topPad, mean, halfMajor, halfMinor, theta );
    ...
    /**
     *   This function produces a 2D histogram image with first color values
     *   represented across the x axis and second color values represented
     *   across the y axis.
     */
    private void calcColor2D() {
        ...
        frameColocalize = new ViewJFrameColocalizationEM( registeredSrcImage, null, baseImage, null, null, destImage,
                controlFrame, useRed, useGreen, useBlue, min1, max1, min2, max2, scale1,
                scale2, leftPad, rightPad, bottomPad, topPad, mean, halfMajor, halfMinor, theta );
                        ...
                        for ( i = 0; i < xDim; i++ ) {
                            ...
                            for ( c = 0; c <= 3; c++ ) {
                                ...
                                if ( c == secondColor ) {
                                    ...
                                    registeredSrcImage.setUShort( 4 * totPos + c, srcImage.getUShort( 4 * totPos + c ) );
                                }
                            }
                        }
                    }
    ...
    private void emfunction() {
        ...
        for ( iter = 0; iter < iterations; iter++ ) {
            ...
            for ( j = 0; j < gaussians; j++ ) {
                ...
                for ( i = 0; i < count; i++ ) {
                    rho[j] += probjxOld( i, j ) * ( b1[i] - mean[j][0] ) * ( b2[i] - mean[j][1] )
                            / ( std[j][0] * std[j][1] );
                }
                rho[j] /= denom;
                if ( rho[j] >= 0.99 ) {
                ...
                /*
                 ...
                 std[j][0]*std[j][0]        rho[j]*std[j][0]*std[j][1]
                 rho[j]*std[j][0]*std[j][1]    std[j][1]*std[j][1]
                 lambda1 and lambda2 are the eigenvalues of the covariance matrix
                 The corresponding eigenvalues give the variances along the
                 respective principal directions.
                 Then the eigenvector corresponding to the major axis is
                 (std[j][0]*std[j][0] - lambda1, rho[j]*std[j][0]*std[j][1])
                 */
                lpart = Math.sqrt(
                        ( std[j][0] * std[j][0] + std[j][1] * std[j][1] )
                                * ( std[j][0] * std[j][0] + std[j][1] * std[j][1] )
                                        - 4.0 * std[j][0] * std[j][0] * std[j][1] * std[j][1]
                                        * ( 1.0 - rho[j] * rho[j] ) );
            ...
            for ( j = 0; j < gaussians; j++ ) {
                ...
                Preferences.debug(
                        "Major half axis = " + halfMajor[j] + " " + " Minor half axis = " + halfMinor[j] + "\n" );
    ...
    private double probXjOld( int i, int j ) {
        ...
        e = ( b1[i] - meanOld[j][0] ) * ( b1[i] - meanOld[j][0] ) / ( stdOld[j][0] * stdOld[j][0] )
                - 2.0 * rhoOld[j] * ( b1[i] - meanOld[j][0] ) * ( b2[i] - meanOld[j][1] )
                / ( stdOld[j][0] * stdOld[j][1] )
                + ( b2[i] - meanOld[j][1] ) * ( b2[i] - meanOld[j][1] ) / ( stdOld[j][1] * stdOld[j][1] );
        e /= ( -2.0 * ( 1.0 - rhoOld[j] * rhoOld[j] ) );
        return Math.exp( e )
                / ( 2.0 * Math.PI * stdOld[j][0] * stdOld[j][1] * Math.sqrt( 1.0 - rhoOld[j] * rhoOld[j] ) );
    }
