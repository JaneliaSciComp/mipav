 ...
 *    based on input parameters that specify the min/max voxel values
 *    for the face voxels, and the maximum skin thinkness.
 *    The algorithm also blurs the face voxels based on input parameter
 *    for blur factor.
 *    The face and blured face are stored in the ModelImage so the
 *    user can see the results.
 *    On getting an OK message, the blurred face is recombined with the
 *    rest of the head volume.
 *    On getting a Cancel message the original volume is restored.
 *
...
public class AlgorithmFaceAnonymizer extends AlgorithmBase {
    ...
    /**
     *  m_iMinValue is used to set the minimum value in the
     *  face and blurred face images so that the image is displayed
     ...
     */
    public AlgorithmFaceAnonymizer( ModelImage srcImg, int[] aiAxis,
            int iMin, int iMax, int iRange ) {
        // source image:
    ...
    /**
     *  Called when the dialog OK button is pressed.  this sets up the
     *  Algorithm state to commit the changes in the image and to tell the
     *  dialog to cleanup and delete this algorithm.
     */
    public void setOK() {
        m_iState = OK;
    }
    ...
    /**
     * Called when the Find Face button is pressed.  the algorithm computes
     * the face region of the volume and sets the displayed image to be that
     * region.
     */
    public void setFind() {
        m_iState = FIND;
    ...
    /**
     *  Called when the Blur Face button is pressed.  the algorithm computes
     *  the blurred face and sets the displayed image to be the blurred region.
     */
    public void setBlur() {
        m_iState = BLUR;
    }
    ...
    /**
     *  Called when the dialog Cancel button is pressed.  this sets up the
     *  Algorithm state to revert to the original copy of the image and to tell
     *  the dialog to cleanup and delete this algorithm.
     */
    ...
    private void constructLog() {
        historyString = new String( "AnonymizeFace(" + m_iState +
                                    ", (" + m_aiAxis[0] + ", " + m_aiAxis[1] + ", " + m_aiAxis[2] + "), " +
                                    this.m_iMin + ", " + m_iMax + ", " + m_iRange + ")\n" );
    }
    ...
    /**
     * The head points along the X axis: rays are traced in the +/- x
     * direction:
     */
     ...
     * This is done based on the min/max voxel values for the face,
     * and the maximum face thickness value, which are set by the
     * FaceAnonymizer dialog.
     * The face volume is copied into the ModelImage and displayed
     * to the user.
     */
    ...
    /**
     *  Blurs the face mask voxels The blurred face volume is copied into the
     *  ModelImage and displayed to the user.
     */
     ...
     * Copy the blurred face volume and the original data
     * into the ModelImage.
     */
    public void okFace() {
    ...
    /**
     * Cancel Button has been pressed: Copy the unaltered original data into
     * the ModelImage.
     */
    ...
    /**
     * Find face, blur face, commit, or cancel based on which buttons in
     * the dialog are pressed.
     */
