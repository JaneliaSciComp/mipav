 ...
 *
 *  This code is a port of the MATLAB routine
 *  TwoD_FRET_and_2FP_bleed_thru_from_1FP provided by Dr. Stephen Lockett.
 *
 *  The code here requires 3 images - image with a FP1 dye taken with a
 *  FP1 filter, image with a FP1 dye taken with a FRET filter, and image
 *  with a FP1 dye taken with an FP2 filter.  The code is run with a set of
 *  3 images with donor dye only to obtain the DFP to FRET bleed thru and
 *  the DFP to AFP bleed thru.  The code is run with a set of 3 images with
 *  acceptor dye only to obtain the AFP to FRET bleed thru and the AFP to DFP
 *  bleed thru.  When these 4 bleed thru parameters have been obtained, then
 *  a set of 3 images containing both donor and acceptor dye is used with
 *  AlgorithmFRETEfficiency to obtain a FRET efficiency on each active VOI.
 *  So a set of 9 images is used to obtain the FRET efficiency.
 *  Only 1 user selected color will be used from a color image.
 *
 ...
 *  bleedthrough calculations.  If a pixel is saturated in any of the 3 images,
 *  then that pixel will be excluded from all calculations.
 *  The VOIs must all be placed in the source image and the source image must
 *  be an image of the 1FP taken with a 1FP filter.  That is, the source image
 *  must either be an image with donor dye only taken with a donor fluorescent
 *  peak filter or an image with acceptor dye only taken with an acceptor
 *  fluorescent peak filter.
 ...
 *  active region.
 *
 *  denom = mean(FP1 dye with FP1 filter active VOI) -
 *          mean(FP1 dye with FP1 filter background VOI)
 *
 *  FRET bleed through = [mean(FP1 dye with FRET filter active VOI) -
 *                        mean(FP1 dye with FRET filter background VOI)]/denom
 *
 *  FP2 bleed through = [mean(FP1 dye with FP2 filter active VOI) -
 *                            mean(FP1 dye with FP2 filter background VOI)]/
 *                            denom
 *
 *  The code outputs the FP1 to FRET bleed thru and the FP1 to FP2 bleed thru
 *  for each active VOI region.
 *
 *  Reference:
...
public class AlgorithmFRETBleedThrough extends AlgorithmBase {
    ...
    // If acceptorRun is false, run on 3 images with donor dye only.
     ...
     *   @param acceptorRun  If true, run on 3 images with only acceptor dye
     *                       If false, run on 3 images with only donor dye
     */
    public AlgorithmFRETBleedThrough( ModelImage FP1Image, ModelImage FRETImage,
            ModelImage FP2Image,
    ...
    private void constructLog() {
        historyString = new String(
                "FRETBleedThrough(" + useRed + ", " + useGreen + ", " +
                                      useBlue + ", " +
                                      acceptorRun + "\n" );
    }
    ...
    public void runAlgorithm() {
      ...
      for (i = 0; i < imageSize; i++) {
        if ((FP1Buffer[i] == saturation) || (FRETBuffer[i] == saturation) ||
            (FP2Buffer[i] == saturation)) {
          mask[i] = -1;
        }
      } // for (i = 0; i < imageSize; i++)
      ...
      Preferences.debug("1FP image with 1FP filter\t" +
                        "1FP image with FRET filter\t" +
                        "1FP image with 2FP filter\n");
      Preferences.debug(srcImage.getImageName() + "\t\t" +
                        FRETImage.getImageName() + "\t\t" +
                        FP2Image.getImageName() + "\n\n");
      UI.setDataText("1FP image with 1FP filter\t" +
                     "1FP image with FRET filter\t" +
                     "1FP image with 2FP filter\n");
      ...
      for (i = 0; i < nBoundingVOIs - 1; i++) {
        ...
        Preferences.debug(activeNumber[i] + "\t\t" +
                       String.valueOf(nf.format(FRETBleed[i])) + "\t\t" +
            ...
            if (files[i].getName().startsWith("acceptor_")) {
              if (files[i].exists()) {
                try {
                  ...
                  MipavUtil.displayError("Security error occurs while trying to " +
                                         "delete " + files[i].getName());
                }
              }
            }
          } // if (acceptorRun)
      ...
      if (acceptorRun) {
        writeFile    = new File(srcImage.getFileInfo(0).getFileDirectory() +
                                "acceptor_" +
      ...
      if (nBoundingVOIs > 2) {
        Preferences.debug("Weighted average\t" +
                          String.valueOf(nf.format(averageFRETBleed)) + "\t\t" +
                          String.valueOf(nf.format(averageFP2Bleed)) + "\n\n");
        UI.setDataText("Weighted average\t" +
                          String.valueOf(nf.format(averageFRETBleed)) + "\t\t" +
                          String.valueOf(nf.format(averageFP2Bleed)) + "\n\n");
      } // if (nBoundingVOIs > 2)
    ...
    private void buildProgressBar() {
        try {
            if ( pBarVisible == true ) {
                progressBar = new ViewJProgressBar( srcImage.getImageName(), "Performing FRET bleed thru...", 0, 100, true, this,
                        this );
