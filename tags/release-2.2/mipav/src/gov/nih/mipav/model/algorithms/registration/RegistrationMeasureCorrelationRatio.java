 ...
 * correlation ratio computed as follows:
 *
 * sum_k { variance(Y_k) * n_k / N } / variance(Y)
 *
 * where
 * - X is the target image
 * - Y is the registered source image
 * - N is the total number of samples
 * - k is in the range [0,nbins)
 * - variance(Y) is the variances for every sample in the image Y
 * - variance(Y_k) is the variance of the k-th iso-set defined as the set
 * of intensities in image Y at positions where the intensity in image X
 * is in the k-th intensity bin
 * - n_k is the number of samples in the k-th iso-set
 *
 * The cummulative summations used to compute the variances are
 * maintained along with the index of the corresponding bin
 * for each sample.
 */
    ...
    extends RegistrationMeasure
{
    ...
    public double getError() {
        ...
        for (int i = 0; i < m_iNumBins; i++) {
            dWeightedVarianceSum +=
                getVariance (m_aiBinSum0[i], m_adBinSum1[i], m_adBinSum2[i]) *
    ...
    /**
     * Compute the variance given the total number of samples along with the
     * sum of values and sum of squared values.
     * @param N int Number of samples.
     * @param dSum1 double Sum of sample values.
     * @param dSum2 double Sum of sample squared values.
     * @return double Computed variance.  Zero is returned if there are
     * not at least two samples.
     */
    static protected double getVariance (int N, double dSum1, double dSum2) {
        if (N > 1) {
