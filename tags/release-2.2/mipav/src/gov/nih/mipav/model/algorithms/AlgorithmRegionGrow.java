 ...
 *     A VOI is used to calc an intensity within the region. The region
 *     grows if the itensity is bounded by average +/- the standard deviation.
 *     The a new VOI should be generated but for now the image is filled with
 *     with a flood value.
 *
 *     The variableThresholds option of region grow is not used with fuzzy
 *     connectedness.  If variableThresholds is true,
 *     upperDelta = initialValue of upperBound - value of seed point
 *     lowerDelta = Value of seed point - initialValue of lowerBound
 *     Each time a new pixel is added to the region recalculate the
 *     mean and standard deviation of the region
 *     mean = (sum of the pixel values)/(pixel number)
 *     standard deviation =
 *     ((Sum of the squared values - (sum of values)*(sum of values)/(pixel number))/(pixel number)
 *     Then recalculate upperBound and lowBound as:
 *     upperBound = initialValue + (1.0 - Math.min(0.8, stdDev/mean)) * upperDelta
 *     lowBound = initialValue - (1.0 - Math.min(0.8, stdDev/mean)) * lowerDelta
 *     upperBound cannot become higher than its initial value, but it can become lower
 *     than its initial value.  lowBound cannot become lower than its initial value,
 *     but it can become higher than its initial value.  That is, upperBound - lowBound
 *     cannot exceed its initial value, but it can become lower than its initial value.
 *     Adaptive thresholding is useful in preventing bleeding across smooth image
 *     gradients.
 *     Reference:
 *     Automatic Image-To-Map-Registration of Remote Sensing Data by Heiner Hild,
 *     Photogrammetric Week '01', D. Fritsch & R. Spiller, Eds., Wichmann Verlag,
 *     Heidelberg, 2001.
 *
 *     The fuzzy connectedness option of the region growing code is ported with
 *     some modifications from NLM's Insight ITK toolkit.  Fuzzy connectedness
 *     requires a seed point and a fuzzy threshold value ranging from 0.0 to 1.0
 *     as inputs.  The fuzzy scene that is generated depends only on the seed point
 *     and is independent of the fuzzy threshold specified.  Thus, once a fuzzy scene
 *     is created by clicking the mouse at a seed point, new paint bit masks corresponding
 *     to the same seed point but different fuzzy thresholds can be created very quickly
 *     and easily by simply entering a new fuzzy threshold with the slider and clicking
 *     the new threshold button.  The user has the option of creating a fuzzy image.
 *
 *     2 significant changes are made from the original code.  In the original code the
 *     variance of the segment corresponding to the seed point is arbitrarily taken as
 *     2500.  Here the total variance is calculated and the object variance is roughly
 *     estimated as 1/5 of the total variance.  In the original code a point could be
 *     pushed on the stack even if it was already present on the stack.  In this modified
 *     code a boolean array is used to prevent points currently on the stack from being
 *     pushed onto the stack a second time.
 *
 *     The reference that Insight gives for their fuzzy connectedness code is:
 *     "Fuzzy Connectedness and Object Definition: Theory, Algorithms, and Applications
 *     in Image Segmentation" by Jayaram K. Udupa and Supun Samarasekera, Graphicals
 *     Models and Image Processing, Vol. 58, No. 3, pp. 246-261, 1996.
 *
 *     A summary of the algorithm used follows:
 *     1.) Create an integer fuzzy scene of the same total length as the original source
 *         image.  Set the seed point to 65535 and set all the other elements of the
 *         fuzzy scene to 0.
 *     2.) Push the indices of the 4 or 6 nearest neighbors of the seed point onto the stack.
 *     Note that this is a LIFO stack - that is indices will be popped from the front of the
 *     stack.  While the stack is not empty do:
 *        3.) Pop the first index from the stack into current
 *        4.) Find fmax
 *            Fuzzy affinity of 2 neighboring points measures how close the average of
 *            their 2 gray scale values is to the gray scale value of the seed point.
 *            Fuzzy affinity reaches a maximum of 65535 when the average of the 2 gray
 *            scale values equals the gray scale value of the seed point and fuzzy affinity
 *            approaches a minimum of 0 as the average of the 2 neighbors differs greatly
 *            from the seed value.
 *
 *            For each neighbor get the smaller of the neighbor's fuzzy scene value and
 *            the fuzzy afinity of current and the neighbor.  Thus, for a single path
 *            the strength of the path is determined by the weakest link in the path.
 *            For the first neighbor this value is assigned to tmp and for all following
 *            neighbors this value if assigned to tmp2.  Then, for fmax choose the maximum
 *            of the 4 or 6 paths.  Thus, the fuzzy connectedness between 2 points is
 *            determined by the strongest of the multiple paths between the 2 points.
 *        5.) if (fmax > fuzzyScene[current])
 *             6.) Set fuzzyScene[current] = fmax
 *             7.) Push the 4 or 6 indices of the nearest neighbors of current onto the stack
 *                 if they are not already present on the stack.
 *            endif
 *        endwhile
 *        8.) Create an active threshold by multiplying the fuzzy threshold by 65535.  Set the
 *            paint bit map if the fuzzy scene element >= activeThreshold.  Otherwise, clear
 *            the paint bit map.
 *
 *      ********   Needs a lot of work ... 2D in dest "WORKS"
 *
 *      ** regionGrow methods -  they do WORK!!!!
 *
...
public class AlgorithmRegionGrow extends AlgorithmBase {
     ...
     */
    public AlgorithmRegionGrow( ModelImage destImg, ModelImage srcImg,
            float _multiplier, float _floodValue ) {
     ...
     *   @param fuzzyThreshold if negative fuzzy connectedness is not used
     *                      value ranges from 0 to 1 if fuzzy connectedness is used
     ...
     *   @param maxDistance max distance from the seed point (in pixels) that the region is
     *                      allowed to grow.
     *   @param variableThresholds If true vary thresholds as region grows
     *   @return returns the area region
     */
    public int regionGrow2D( BitSet paintMask, Point seedPt, float fuzzyThreshold,
            boolean useVOI, boolean displayFuzzy,
            RegionGrowDialog growDialog, float lowBound, float upperBound,
            int sizeLimit, float maxDistance, boolean variableThresholds ) {
            ...
            if (fuzzyThreshold < 0.0f  && variableThresholds) { // don't use fuzzy connectedness
                ...
                while ( !stack.isEmpty() ) {
                    ...
                    if ( x + 1 < xDim ) {
                        ...
                        if ( maxDistance > 0 ) {
                            distance = Math.sqrt(
                                    ( x - xInit ) * ( x - xInit ) * resX + ( y - yInit ) * ( y - yInit ) * resY );
                        }
                        if ( paintMask.get( idx ) == false && val >= lowBound && val <= upperBound
                            ...
                            upperBound = (float)(initialValue +
                                         (1.0 - Math.min(0.8, stdDev/mean))*upperDelta);
                            lowBound = (float)(initialValue -
                                       (1.0 - Math.min(0.8, stdDev/mean))*lowerDelta);
                ...
                varInverse[0][1] = -( objectVar[1][0] * objectVar[2][2] - objectVar[2][0] * objectVar[1][2] ) / varDet;
                varInverse[0][2] = ( objectVar[1][0] * objectVar[2][1] - objectVar[2][0] * objectVar[1][1] ) / varDet;
                varInverse[1][0] = -( objectVar[0][1] * objectVar[2][2] - objectVar[2][1] * objectVar[0][2] ) / varDet;
                varInverse[1][1] = ( objectVar[0][0] * objectVar[2][2] - objectVar[2][0] * objectVar[0][2] ) / varDet;
                varInverse[1][2] = -( objectVar[0][0] * objectVar[2][1] - objectVar[2][0] * objectVar[0][1] ) / varDet;
                varInverse[2][0] = ( objectVar[0][1] * objectVar[1][2] - objectVar[1][1] * objectVar[0][2] ) / varDet;
                varInverse[2][1] = -( objectVar[0][0] * objectVar[1][2] - objectVar[1][0] * objectVar[0][2] ) / varDet;
    ...
    public int regionGrow2D( BitSet paintMask, Point seedPt, float fuzzyThreshold,
            boolean useVOI, boolean displayFuzzy,
            RegionGrowDialog growDialog, float lowBoundR, float upperBoundR,
            float lowBoundG, float upperBoundG,
            float lowBoundB, float upperBoundB,
            int sizeLimit, float maxDistance) {
                ...
                while ( !stack.isEmpty() ) {
                    ...
                    if ( x + 1 < xDim ) {
                        ...
                        if ( paintMask.get( idx ) == false && valR >= lowBoundR && valR <= upperBoundR
                                && valG >= lowBoundG && valG <= upperBoundG &&
                                valB >= lowBoundB && valB <= upperBoundB && distance <= maxDistance ) {
                            stack.push( idx );
                            paintMask.set( idx );
                            count++;
                            if ( sizeLimit != -1 && ( count + 1 ) * volRes > sizeLimit ) {
     ...
     */
    public int regionGrow3D( BitSet paintMask, Point3Ds seedPt, float fuzzyThreshold,
            boolean useVOI, boolean displayFuzzy,
            RegionGrowDialog growDialog, float lowBound, float upperBound,
            int sizeLimit, float maxDistance, boolean variableThresholds,
            int timeSlice, CubeBounds regionBounds ) {
     ...
     * @param displayFuzzy
     */
    public void setNewThreshold( float fuzzyThreshold, PaintGrowListener compImage,
     ...
     * @return tmp
     */
    private final double findStrongPath( int current, int xDim, int yDim ) {
        double tmp = 0;
        double tmp1;
        double tmp2;
        double centerPixel;
     ...
     * @return tmp
     */
    private final double findStrongPathColor( int current, int xDim, int yDim ) {
        double tmp = 0;
        double tmp1;
        double tmp2;
     ...
     * @return tmp
     */
    private final double findStrongPath( int current, int xDim, int yDim, int zDim ) {
        double tmp = 0;
        double tmp1;
        double tmp2;
        float centerPixel;
     ...
     * @return tmp
     */
    private final double findStrongPathColor( int current, int xDim, int yDim, int zDim ) {
        double tmp = 0;
        double tmp1;
        double tmp2;
        double[] centerPixel = new double[3];
     ...
     * @return result
     */
    private final double fuzzyAffinity( double f1, double f2 ) {
        double tmp1;
        double result;
     ...
     * @return result
     */
    private final double fuzzyAffinityColor( double[] f1, double[] f2 ) {
