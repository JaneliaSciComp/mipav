...
public class AlgorithmWaveletThreshold extends AlgorithmBase {
    ...
    // is zero padding going past each original boundary by the coefficient
    // number - 2.   The image is transformed into wavelet coefficients.

    // In hard thresholding a wavelet coefficient whose magnitude is below the
    // product of threshold and the maximum wavelet magnitude is zeroed, and
    // a wavelet coefficient whose magnitude is greater than or equal to the product
    // is left unchanged.  In soft thresholding a wavelet coefficient whose
    // magnitude is below the product of threshold and the maximum wavelet magnitude
    // is zeroed, and a wavelet coefficient whose magnnitude is greater than or
    // equal to the product has its magnitude decreased by the product.  Generally,
    // soft thresholding is a better choice.  The signal reconstructed with the
    // coefficients left after hard thresholding may have undesirable artifacts
    // including sidelobes.

    // Then, an inverse wavelet transform is performed.  The image is stripped
    // down to its original size.  The data is clamped so as not to exceed the
    // bounds of the data type of the image into which it is imported.
    // This is a simple method of reducing white noise.

    // The forward and inverse Daubechies wavelet transform routines are taken
    // from Numerical Recipes in C The Art of Scientific Computing, 2nd edition,
    // by William H. Press, Saul A. Teukolsky, William T. Vetterling, and
    // Brian P. Flannery, Cambridge University Press, 1997, Chapter 13.10,
    // pp. 591 - 606.

    // The wavelet transform is mathematically defined only within a signal; image
    // applications need to solve the boundary problem.  4 different techniques
    // exist:
    // 1. Mirror image replication
    // 2. Zero padding
    // 3. Linear extrapolation
    // 4. Circular convolution
    // In this module zero padding is used.

    // As stated in the Medx 3.4 User's Guide in Chapter 17 on Advanced Image Processing
    // Techniques: "The mirror condition can only be used with symmetric wavelet basis."
    // "Daubechies wavelet bases are non-symmetrical orthogonal wavelets with compact
    // support and are good for data suppression.  Only the periodic boundary condition
    // should be used when using the Daubechies wavelet basis."

    // In Empirical Evaluation of Boundary Policies for Wavelet-based Image Coding
    // by Claudia Schremmer zero padding, circular convolution, and mirror image
    // padding are compared.  She concludes that mirror padding works best with
    // regard to quality, zero padding performs worst with regard to quality, and
    // circular convolution is midway between the 2.

    // The matlab wavelet toolbox literature says that "the disadvantage of zero padding
    // is that discontinuities are artificially created at the border."  Mirror padding "has
    // the disadvantage of artificially creating discontinuities of the first derivative at
    // the border, but this method works well in general for images."  Extrapolation "works
    // well in general for smooth signals."  The wavelet toolbox uses mirror padding as its
    // default mode.

    // How much padding is required?  Claudia
    // Schremmer states: " With padding, the coefficients of the signal on either side
    // of the border are padded with filter_length - 2 coefficients.  Consequently,
    // each signal coefficient enters into filter_length/2 calculations of convolution,
    ...
    // underlying wavelet filter
    // For 4 coefficients the routine daub4 is considerably
    // faster than pwt.
    // DAUB4 implements a Daubechies 4-coefficient wavelet filter.
    // PWT implements Daubechies filters with 4, 12, and 20
    // coefficients.  DAUB4 and PWT use different default
    // centerings.  In spite of the faster speed of DAUB4,
    // the dialog is designed only to call PWT so that
    // the user only sees changes due to the number of
    // coefficients and is not confused by changes in centering.
    public final static int DAUB4 = 1;
    public final static int PWT = 2;

    // coefficents for DAUB4 wavelet filter
    public final static double C0 = 0.4829629131445341;
    public final static double C1 = 0.8365163037378079;
    public final static double C2 = 0.2241438680420134;
    public final static double C3 = -0.1294095225512604;

    // coefficients for the PWT filter
    public final static double c4[] = {0.0,0.4829629131445341,0.8365163037378079,
                        0.2241438680420134,-0.1294095225512604};
    public final static double c12[] = {0.0,0.111540743350,0.494623890398,0.751133908021,
    ...
    public final static double c20[] = {0.0, 0.026670057901,0.188176800078,0.527201188932,
	 ...
	 *  @param threshold        fraction of maximum magnitude below which coefficients
	 *                          are zeroed
	 *  @param doWaveletImage   display log maagnitude wavelet transform image if true
	 *  @param userInterface
	 */
    public AlgorithmWaveletThreshold(ModelImage destImg, ModelImage srcImg, int filterType,
                            int cNum, int thresholdType, float threshold, boolean doWaveletImage,
	 ...
	 *  @param userInterface
	 */
    public AlgorithmWaveletThreshold(ModelImage srcImg, int filterType,
                            int cNum, int thresholdType, float threshold, boolean doWaveletImage,
                            ViewUserInterface userInterface) {
        super(null, srcImg);
    ...
    */
    public ModelImage   getWaveletImage()    {return waveletImage;}
    ...
    */
    public void runAlgorithm() {
        ...
        if (nDims == 2) {
            ...
            for (x = 0; x < xDim; x++) {
                for (y = 0; y < yDim; y++) {
                    aExp[x + offsetX + newXDim*(y + offsetY)] =
                    aArray[x + xDim*y];
                }
            }
          ...
          //  }
           ...
           // }
            ...
            for (x = 0; x < xDim; x++) {
                for (y = 0; y < yDim; y++) {
                    for (z = 0; z < zDim; z++) {
                        aExp[x + offsetX + newXDim*(y + offsetY) + newXY*(z + offsetZ)] =
                        aArray[x + xDim*y + xy*z];
                    }
                }
            }
    ...
    */
    private void constructLog() {
        ...
        historyString = new String( "WaveletThreshold(" +
                                    String.valueOf(cNum) +
                                    String.valueOf(thresholdType) +
                                    String.valueOf(threshold)+
                                    String.valueOf(doWaveletImage) + ")\n");
    }


    /**
    *  @param a data replaced by its wavelet transform for transformDir == FORWARD
    *           or by its inverse wavelet transform for transformDir == INVERSE
    *  One dimensional discrete wavelet transform.  Note that the length of a
    ...
    *  @param a data replaced by its wavelet transform for transformDir == FORWARD
    *           or by its inverse wavelet transform for transformDir == INVERSE
    *  n-dimensional discrete wavelet transform.  Note that the length of each dimension of a
    *  must be an integer power of 2
    */
    ...
    /**
    *  @param a data vector to which Daubechies 4-coefficient wavelet filter or
    *  its transpose (for transformDir == INVERSE) is applied
    *  @param n
    */
    ...
    /**
    *  @param a data to which wavelet filter (for transformDir == FORWARD) or
    *           to which transpose wavelet filter is applied (for
    *           transformDir == INVERSE)
    *  @param n
