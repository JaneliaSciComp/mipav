...
/**
 *  This algorithm operates on 3D black and white images and on 2D and 3D
 *  color images.  The user can select 1 or both of 2 output images:  The
 *  first image is a filtered 3D image containing only the first n principal
 *  components, where n is selected by the user after viewing images of the
 *  first 10 (or the number that exist if less than 10) principal components.
 *  The second image is a 2D image created by averaging together the images
 *  in the 3D image.  Note that red, green, and blue are treated as separate
 *  2D planes.  This implementation assumes that the noise variance is equal
 *  in every slice and that the noise is uncorrelated between slices.  Principal
 *  components maximizes data variance in components; it concentrates the information
 *  content of the data in as small a number of components as possible.
 *  This algorithm is based on:
 *  1.)  Digital Image Processing by Rafael C. Gonzalez
 *  and Richard E. Woods, Section 3.6 The Hotelling Transform,
 *  pp. 148 - 157, 1992.
 *  2.)  "The statistical properites of three noise removal
 *  procedures for multichannel remotely sensed data" by
 *  M. Berman, CSIRO Division of Mathemetics and Statistics
 *  P.O. Box 218, Lindfield, N.S.W., 2070
 *  Consulting Report NSW/85/31/MB9
 *  Note that this algorithm has 4 different names: Hotelling, eigenvector,
 *  principal component, and Karhunen-Loeve transform.  Also note that
 *  when the noise variance is the same in all bands and the noise is
 *  uncorrelated between bands the principal component transformation
 *  is equivalent to the maximum noise fraction transformation.
 *  In the more general case, the MNF transform is equivalent to a transformation
 *  of the data to a coordinate system in which the noise covariance matrix is
 *  the identity matrix, followed by a principal components transformation.
 *  The transform in in this form is the noise-adjusted principal components
 *  transform.  In forward order MNF maximizes the noise content in successive
 *  components that are uncorrelated over the dataset or equivalently
 *  in reverse order MNF maximizes the signal to noise ratio of each component.
 *  Information about the signal to noise ratio improvement is found in "Information
 *  Extraction, SNR Improvement, and Data Compression in MultiSpectral
 *  Imagery", Patrick J. Ready and Paul A. Wintz, IEEE Transactions on
 *  Communications, Vol. COM-21, No. 10, October, 1973, pp. 1123-1130.
 */
public class AlgorithmPrincipalComponents extends AlgorithmBase
    ...
    /**
     * if true create an image averaging together the
     * slices in the filtered version
     */
     ...
     * 10 or less than 10 if fewer exist
     */
     ...
     *   @param doAveraging    if true create an image averaging together the
     *                         slices in the filtered version
     *   @param displayAndAsk  if true display principal component images and
     *                         ask how many to retain
     *   @param pNumber        if displayAndAsk if false pNumber is the number
     *                         of principal component images to retain
     */
    public AlgorithmPrincipalComponents( ModelImage[] destImg, ModelImage srcImg,
            boolean doFilter, boolean doAveraging,
    ...
    /**
     * Writes the logString to the appropriate log area.
     * Overrides the AlgorithmBase <code>writeLog()</code> to append the history
     * to all of the destination images we've created.
     */
    ...
    private void pComponent() {
        ...
        // Reconstruct the data with the inverse transform using only
        // the number of principal components chosen
        // Since the rows of eigenvector are orthonormal vectors, the inverse
        // of eignevector is simply equal to the transpose of eigenvector
        // Also remember that we only want the first pNumber rows of
        // eigenvector or the first pNumber columns of eigenInverse
            ...
            for ( j = 0; j < samples; j++ ) {
                for ( z = 0; z < zDim; z++ ) {
                    for ( i = 1; i < 4; i++ ) {
                        ...
                        for ( k = 0; k < pNumber; k++ ) {
                            values[4 * z * samples + 4 * j + i] += eigenInverse[3 * z + i - 1][k] * pTrunc[k][j]
                                    + mean[3 * z + i - 1];
                        }
                    }
                }
    ...
    private void createPNumberDialog( ActionListener al ) {
        ...
        pNumberDialog.setLocation(
                Toolkit.getDefaultToolkit().getScreenSize().width / 2 - pNumberDialog.getBounds().width / 2,
                Toolkit.getDefaultToolkit().getScreenSize().height / 2 - pNumberDialog.getBounds().height / 2 );
