...
/**
 *	Algorithm to place an image into the center of a larger image,
 *	as if the image was given margins or a border around its outside.
 *
 */
public class AlgorithmAddMargins
    ...
    /**
     *	Using buffers creates a buffer in the algo to do image
     *	copies, then transfers the lot back to the model image
    ...
    /**
     *	Import source and destination images into the class (shorthand for a 2d image).
     *	Used for noncolor.
     ...
     *
     */
    public AlgorithmAddMargins(ModelImage srcImage, ModelImage destImage, double n,
    ...
    /**
     *	Import source image into the class (shorthand for a 2d image).
     *	Used for noncolor.
     *	@param	srcImage		Source image (image to clip from)
     *	@param	n			Value to use in portions of the destination not clipped from the source
     *	@param	width			Border or margin to be added on each side
     ...
     *
     */
    public AlgorithmAddMargins(ModelImage srcImage, double n,
     ...
     */
    public AlgorithmAddMargins(ModelImage srcImage, ModelImage destImage, double n,
                                    int leftWidth, int rightWidth, int height) {
        super(destImage, srcImage);
        if (srcImage.getNDims() != 2) {
            setCompleted(false);
            notifyListeners(this);
     ...
     */
    public AlgorithmAddMargins(ModelImage srcImage, double n,
                                    int leftWidth, int rightWidth, int top,
                                    int bottom) {
        super(null, srcImage);
        if (srcImage.getNDims() != 2) {
            setCompleted(false);
     ...
     *
     */
    public AlgorithmAddMargins(ModelImage srcImage, ModelImage destImage, double r, double g, double b,
                                    int width, int height) {
        super(destImage, srcImage);
        if (srcImage.getNDims() != 2) {
            setCompleted(false);
     ...
     *
     */
    public AlgorithmAddMargins(ModelImage srcImage, double r, double g, double b,
                                    int width, int top, int bottom) {
        super(null, srcImage);
        if (srcImage.getNDims() != 2) {
            setCompleted(false);
     ...
     */
    public AlgorithmAddMargins(ModelImage srcImage, ModelImage destImage, double n,
                                    int width, int height, int front, int back) {
        super(destImage, srcImage);
        if (srcImage.isColorImage()) {
          colorFactor = 4;
          redValue = n;
     ...
     */
    public AlgorithmAddMargins(ModelImage srcImage, double n,
                                    int width, int top, int bottom, int front,
                                    int back) {
        super(null, srcImage);
        if (srcImage.isColorImage()) {
          colorFactor = 4;
     ...
     */
    public AlgorithmAddMargins(ModelImage srcImage, double r, double g, double b,
                                    int width, int top, int bottom, int front, int back) {
        super(null, srcImage);
        colorFactor = 4;
    ...
    private void constructLog() {
        String margin;
        if (colorFactor == 1) {
            margin = "AddImageMargins(left side: " + leftMargin
                + ", right side: " + rightMargin
                + ", top/bottom: " + topMargin
                + ", front: " + frontMargin
                + ", back: " + backMargin
                + " with pixel value " + marginValue + ")";
        }
        else {
            margin = "AddImageMargins(left side: " + leftMargin
                + ", right side: " + rightMargin
                ...
                + ", front: " + frontMargin
                + ", back: " + backMargin
                + " with red pixel value " + redValue
                + ", green pixel value " + greenValue
                + ", blue pixel value " + blueValue + ")";
        }
    ...
    private void calcStoreInDest() {
        ...
        int topRows = destWidth * topMargin; // number of elements in the top of the destImage image but are not in srcImage
        ...
        if (!useBuffers) { //	do not use local buffer
            ...
            try { // TRY to make import/export for 2d & 3d when not using buffers
                ...
                if (srcImage.getNDims() == 2) {
                    ...
                    // IMAGE LATITUDES: skip through the added 'top' margin, then copy the source image into offset place
                    for (i = 0; i < srcHeight && !threadStopped; i++) {
                        if (isProgressBarVisible())
                            progressBar.updateValue(Math.round( (float) (i + topMargin) / destHeight * 100), activeImage);
                        ...
                        destImage.importData(colorFactor * startDraw, values, false); // place the srcImage image, copying row of img data by row.
                    ...
                    for (t = 0; t < tDim && !threadStopped; t++) {
                        ...
                        for (z = 0; z < frontMargin && !threadStopped; z++) {
                            ...
                            if (isProgressBarVisible())
                                progressBar.updateValue(Math.round( ( (float) (t * destDepth + z)) / (tDim * destDepth) * 100),
                                    activeImage);
                        ...
                        for (z = 0; z < srcDepth && !threadStopped; z++) { // for all slices in the old image
                            ...
                            if ( (isProgressBarVisible()) && (tDim == 1))
                                progressBar.updateValue
                                    (Math.round( (float) (z + frontMargin) / destDepth * 100), activeImage);
                            ...
                            for (i = 0; i < srcHeight && !threadStopped; i++) {
                                ...
                                srcImage.exportData(colorFactor * (tOldOffset + srcPlate + srcWidth * i),
                                    colorFactor * srcWidth, values); // pull out img data row by row from each plate
                                startDraw = destPlate + topRows + destWidth * i + leftMargin; // start skips top and left margin to
                                destImage.importData(colorFactor * (tNewOffset + startDraw),
                                    values, false); // place the srcImage image, copying row of img data by row.
                                destImage.importData(colorFactor * (tNewOffset + startDraw + srcWidth),
                                    rightMargins, false); // right margin
                            }
                            // BOTTOM MARGIN LATITUDES: over the bottom margin, copy in the default values
                        ...
                        if (tDim == 1 && isProgressBarVisible()) {
                            progressBar.setMessage("Updating File Info...");
                            //int fillLength = Math.round((float)z/destDepth);
                            //int piece = (1 - fillLength);
                        }
                        ...
                        for (Z = 0; Z < destDepth && !threadStopped; Z++) {
                           ...
                           if ( (srcImage.getFileInfo()[0]).getFileFormat() == FileBase.DICOM) {
                                ...
                                fileInfoBuffer.setExtents(destImage.getExtents()); // modify extents to use the extents of destImage img
        						...
        						//stringForDicom = String.valueOf(destImage.getExtents()[0]);
        						//fileInfoBuffer.setValue("0028,0011", stringForDicom);
        						fileInfoBuffer.setValue("0028,0010", new Short( (short) fileInfoBuffer.getExtents()[1]), 2);
        						fileInfoBuffer.setValue("0028,0011", new Short( (short) fileInfoBuffer.getExtents()[0]), 2);
                            ...
                            if (! (Z < frontMargin || Z >= (srcDepth + frontMargin - 1))) {
                            	...
                            	dont update the srcImage counter.  This way:
                            	For new slices before the start of the original image set, copy the first FileInfoBuffer and modify.
                            	For new slices that correspond to an existing image, copy that FileInfoBuffer and modify.
                            	For new slices after the end of the original image set, copy the last FileInfoBuffer and modify.
                            	*/
    ...
    /**
     *	 Adds image margins and stores result in srcImage
    ...
    private void calcStoreInPlace() {
        ...
        useLocalBufferForSource(colorFactor * srcWidth, srcHeight,
                                    colorFactor * destWidth, destHeight);
        if (threadStopped) {
            finalize();
            return;
        }
    ...
    /**
     *	Alert the algorithm whther or not to perform the algorithm by copying the
     *	image data using a locally-held buffer, or by import the image data directly
     *	to the destination model image.
    ...
    /**
     *	Does the translation and copy of the source image in a local buffer.
     *	This method could be inline code, but is run as a method
     *	to simplify reading the algorithm.	Likewise, the arguments are
     *	generated from class-defined variables, which could be easily be made
     *	local variables, but this makes reading this method a little easier.
     *
     ...
     */
    protected void useLocalBuffer(int srcWidth, int srcHeight,
        ...
        int row, col;
        int topMarginRow = topMargin,
            bottomMarginRow = srcHeight + topMargin,
            leftMarginCol = leftMargin,
        ...
        if (srcImage.getNDims() == 2) {
            ...
            for (i = 0; i < destSliceArea && !threadStopped; i++) {
                ...
                if ( ( (row <= topMarginRow) || (row >= bottomMarginRow)) ||
                    ( (col <= leftMarginCol) || (col >= rightMarginCol))) { // if out of bounds on the top-or-bottom, or on the left-or-right side
                    destSlice[i] = (float) marginValue;
                }
                else {
                    // srcImage pixel location := current destImage pixel - margins on both sides down to this row + the margin on the right - all the elements in the top margin
                    destSlice[i] = sourceSlice[i + ( -2 * leftMargin * (row + 1) + rightMargin) - topMargin * srcWidth];
                }
            }
            if (threadStopped) {
                sourceSlice = null;
            ...
            try {
                ...
                for (z = 0; z < destDepth && !threadStopped; z++) { // for all slices in the old image
                    if ( (z < frontMargin) || (z >= srcDepth + frontMargin)) {
                        ...
                        // skip through the added 'top' margin, then copy the source image into offset placerogressBar.updateValue(Math.round( (float)(z*destSliceArea + i)/(imageLength) * 100));
                        srcImage.exportSliceXY(z - frontMargin, sourceSlice);
                        for (i = 0; i < destSliceArea && !threadStopped; i++) {
                            if (isProgressBarVisible() && (i % mod == 0))
                                progressBar.updateValue(Math.round( (float) (z * destSliceArea + i) / (imageLength) * 100),
                                    activeImage);
                            row = i / destWidth;
                            col = i % destWidth;
     ...
     *	@param destHeight	height of the destination image (or that slice of the image)
     */
    protected void useLocalBufferForSource(int srcWidth, int srcHeight,
                                  int destWidth, int destHeight) {
        int i, z, t;
        int Z;
        int srcSliceArea = srcWidth * srcHeight;
        ...
        int topMarginRow = topMargin,
            bottomMarginRow = srcHeight + topMargin,
            leftMarginCol = colorFactor * leftMargin,
            ...
            try {
                ...
                for (t = 0; t < tDim && !threadStopped; t++) {
                  ...
                  for (z = 0; z < destDepth && !threadStopped; z++) { // for all slices in the old image
                    if ( (z < frontMargin) || (z >= srcDepth + frontMargin)) {
                      for (i = 0; i < destSliceArea; i++) { // leading & trailing slices
                        if (colorFactor == 1) {
                            destSlice[tNewOffset + z * destSliceArea + i] =
                                    (float) marginValue; // are filled with the margin value
                        }
                        else if ((i % 4) == 0) {
                      ...
                      // skip through the added 'top' margin, then copy the source image into offset placerogressBar.updateValue(Math.round( (float)(z*destSliceArea + i)/(imageLength) * 100));
                        srcImage.exportData( (t * srcDepth + z - frontMargin) *
                                            srcSliceArea,
                                            srcSliceArea, sourceSlice);
                      for (i = 0; i < destSliceArea && !threadStopped; i++) {
                        if (isProgressBarVisible() && (i % mod == 0))
                          progressBar.updateValue(Math.round( (float) (tNewOffset +
                           z * destSliceArea + i) / (imageLength) * 100),
                                               activeImage);
                        row = i / destWidth;
                        col = i % destWidth;
                ...
                for (t = 0; t < tDim && !threadStopped; t++) {
                  ...
                  z = 0; // z is the counter for the orig image
                      for (Z = 0; Z < destDepth && !threadStopped; Z++) {
                          ...
                          // DICOM
                          if ( (srcImage.getFileInfo()[0]).getFileFormat() == FileBase.DICOM) {
                              fileInfoDicomBuffer[t*destDepth + Z] =
                                  (FileInfoDicom) srcImage.getFileInfo(z).clone();

                              fileInfoDicomBuffer[t*destDepth + Z].setExtents(newExtents); // modify extents to use the extents of destImage img

                              // change the slice number ("0020,0013"):
                              // Image slice numbers start at 1; index starts at 0, so compensate by adding 1
                              // Reset the image (slice) number with the new number ordering
                              stringForDicom = Integer.toString(Z + 1);
                              fileInfoDicomBuffer[t*destDepth + Z].setValue(
                                  "0020,0013", stringForDicom, stringForDicom.length());
                              ...
                              if (newOriginLPS != null) {
                                  ...
                                  stringForDicom = Float.toString(newOriginLPS[0]) + "\\" + Float.toString(newOriginLPS[1]) + "\\" +
                                                   Float.toString(newOriginLPS[2]);
                                  fileInfoDicomBuffer[t*destDepth + Z].setValue(
                                     "0020,0032", stringForDicom, stringForDicom.length());
                              ...
                              if (imgOriginLPS != null) {
                                  stringForDicom = String.valueOf(imgOriginLPS[2]);
                                  fileInfoDicomBuffer[t*destDepth + Z].setValue(
                                      "0020,1041", stringForDicom, stringForDicom.length());
                              }
                              ...
                              fileInfoDicomBuffer[t*destDepth + Z].setValue(
                                  "0028,0010", new Short( (short) fileInfoDicomBuffer[t*destDepth + Z].getExtents()[1]), 2);
                              fileInfoDicomBuffer[t*destDepth + Z].setValue(
                                  "0028,0011", new Short( (short) fileInfoDicomBuffer[t*destDepth + Z].getExtents()[0]), 2);
                              ...
                              isDicom = true;
                                ...
                                fileInfoBuffer[t*destDepth + Z] = (FileInfoBase) srcImage.getFileInfo(t * srcDepth + z).clone();
                ...
                for (t = 0; t < tDim; t++) {
                  for (Z = 0; Z < destDepth; Z++) {
                    if (isDicom) {
                      srcImage.setFileInfo(fileInfoDicomBuffer[t * destDepth + Z],
                                           (t * destDepth + Z));
                    }
                    else {
                      srcImage.setFileInfo(fileInfoBuffer[t * destDepth + Z],
                                           (t * destDepth + Z));
                    }
                  }
                } // for (t = 0; t < tDim; t++)
    ...
    */
	...
	private float[] calculateNewOrigin(ModelImage srcImg, int[] margin) {
		...
		for (int i=0; i<3; i++) {
    	    ...
    	    if (axisOrient == FileInfoBase.ORI_A2P_TYPE ||
        		axisOrient == FileInfoBase.ORI_R2L_TYPE ||
            	axisOrient == FileInfoBase.ORI_I2S_TYPE) {
