...
/**
 *  This class controls the file input/output of most formats that MIPAV
 *  supports, including tiff, raw, analyze, DICOM, and Medvision.  It switches
 *  based on file type and calls the file constructors, readers, and writers
 *  for the specific file type.  However, note the FileAvi and FileQT are called directly
 *  from ViewJFrameBase for file writes.
 *
...
public class FileIO {
     ...
     * If the type of file to read is unknown (ie., the suffix doesn't match
     * one of the known types, build and display the unknown file dialog so the
     * user can try to identify the image type so the correct reader can be used
     */
    ...
    /**
     * Link to either progress bar frame or panel passed in to
     * update progress while reading an image.  Which must then be passed into
     * read function for specific file type.
     */
    private ProgressBarInterface pInterface = null;

    /**
     * Creates the FileIO and displays <q>Choose File Type</q> unknown file
     * dialog.
     * Constructs a new FileIO object, sets the user interface, and
     * initializes the unknown file dialog.
     */
    public FileIO() {
     ...
     *              FileIO() - ViewUserInterface parameter no longer required
     * Creates the FileIO and displays <q>Choose File Type</q> unknown file
     * dialog.
     * Constructs a new FileIO object, sets the user interface, and
     * initializes the unknown file dialog.
     ...
     * Creates the FileIO and displays <q>Choose File Type</q> unknown file
     * dialog.
     *   Constructs a new FileIO object, sets the user interface, sets the LUT,
     * and initializes the unknown file dialog.
    ...
    /**
     * Sets the progress bar (either panel or frame) to be used in image
     * opening to update status.
    ...
    /**
     *   Returns address of TIFF header of second image if present in
     * CZ-Private Tag of LSM 510 file
    ...
    /**
     *   Reads file, determines file type, and calls a
     *   read function specific to the file.  That read
     *   function returns an image and this function
     *   stores it in ModelImage.  File is not multi file,
     *   file info is not previously known, there's no "second
     *   address" for AFNI, and this is not an image B.
     ...
     */
    public ModelImage readImage( String fileName, String fileDir,
            boolean multiFile, FileInfoBase fileInfo ) {
     ...
     */
    public ModelImage readImage( String fileName, String fileDir,
            boolean multiFile, FileInfoBase fileInfo, boolean loadB ) {
     ...
     *   stores it in ModelImage.  File is not multi file,
     *   file info is not previously known, there's no "second
     *   address" for AFNI, and this is not an image B.  This is
     *   just one image regardless of how many slices there are; the
     *   middle slice of a 3D dataset will be displayed.
     *   @param fileName      File name where image is located.
     *   @param fileDir       File directory where image is located.
     *   @return              The image that was read in from the file.
     */
    ...
    /**
     * This method examines parameter <i>fileList<i> and loads TIFF files according
     * to the order in the list. It can load 2, 3, or 4 channel images. The method will
     * examine channelMap to determine in which order the ARGB channels should be
     * interleaved.
     *
     ...
     * @param channelMap int[] - a mapping of channels (ARGB) to positions. For example,
     * a channelMap parameter of {2, 1, 0, 3} means the first channel will be G,
     * the second will be R, the third
     * will be A and the last will be B.
     * @param showOrderedProgressBar boolean - This parameter is used to control whether the local progress
     * bar is shown. Note this is different than FileIO's global progress bar. The reason for the
     * difference is because this method uses FileIO's readOneImage() method. That method uses the global
     * progress bar. Its useless to have two progress bars, and FileIO isn't set up in a way that
     * allows this method to directly control the global progress bar, hence the need for its own local one.
     ...
     * @return ModelImage
     */
    public ModelImage readOrderedARGB(File [] fileList, int numChannels, int [] channelMap, boolean showOrderedProgressBar, Dimension subsampleDimension, boolean forceUBYTE)
    {
        ...
        ViewJProgressBar progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + "files ...", UI.getProgressBarPrefix() + "...", 0, 100, false, null, null );
        ...
        for (int n = 0; n < fileList.length; n++)
        {
            ...
            if (fileList[n].exists())
            {
                try
                {
                    modelImageTemp = readOneImage(fileList[n].getName(), fileList[n].getParentFile().getAbsolutePath() + File.separator);
                    ...
                    modelImageTemp.exportData(0, oneSliceBuffer.length, oneSliceBuffer); // export one slice at a time to result ModelImage
    ...
    /**
     * This method will load a group of files in the order of <i>fileList<i>. The result
     * will be a grayscale ModelImage.
     *
     ...
     * @return ModelImage
     */
    public ModelImage readOrderedGrayscale(File [] fileList, boolean showLocalProgressBar, Dimension subsampleDimension, boolean forceUBYTE)
    {
        ModelImage modelImageTemp = null;
        ModelImage modelImageResult = null;
        float [] oneSliceBuffer;
        ...
        try
        {
            ...
            // the first slice has already been read. instead of re-reading it in the loop, export to buffer and save an iteration
    ...
    public static ModelImage convertToARGB(ModelImage modelImage)
    {
        ...
        ModelImage modelImageResultARGB = new ModelImage(ModelStorageBase.ARGB, modelImage.getExtents(), modelImage.getImageName());
        ...
        try
        {
            for (int i = 0; i < modelImage.getExtents()[2]; i++) // loop through images
            {
                ...
                modelImageResultARGB.importData(i * oneSliceBuffer.length, oneSliceBuffer, false); // import into modelImageResultUB
    ...
    public static ModelImage convertToUBYTE(ModelImage modelImageResult)
    {
        ...
        ModelImage modelImageResultUB = new ModelImage(ModelStorageBase.UBYTE, modelImageResult.getExtents(), modelImageResult.getImageName());
        ...
        try
        {
            for (int i = 0; i < modelImageResult.getExtents()[2]; i++) // loop through images
            {
                ...
                modelImageResultUB.importData(i * oneSliceBuffer.length, oneSliceBuffer, false); // import into modelImageResultUB
     ...
     * parameter.
     *
     * @param modelImage ModelImage - the image to be subsampled. This image will be destroyed in the course of the algorithm
    ...
    public static ModelImage subsample(ModelImage modelImage, Dimension subsampleDimension)
    {
        ...
        ModelImage modelImageResult = new ModelImage(modelImage.getType(), new int [] {subsampleDimension.getSize().width, subsampleDimension.getSize().height}, modelImage.getImageName() + "_subsampled");

        AlgorithmSubsample algorithmSubsample = new AlgorithmSubsample(modelImage, modelImageResult,
                subsampledExtents, new float[] {1.0f, 1.0f, 1.0f}, false, false, null);
     ...
     *   @param fileName      File name where image is located.
     *   @param fileDir       File directory where image is located.
     *   @param multiFile     Flag indicating multi file loading of images from
     *                        directory.  <code>true</code> is load images of
     *                        this filetype to be loaded as a set (when
     *                        this feature is supported).
     *   @param fileInfo      File info already known; will usually be null, but valid if called from script parser.
     *   @param secondAddress Address of second TIFF header
     *   @param loadB         Flag indicating if this is an image B.
     *   @param one           A load-single flag.  <code>true</code> indicates
     *                        that the method is to load only the file that is
     *                        defined by the <code>fileName</code>, rather than
     *                        a multiple file load of a 3d Image.
     *   @return              The image that was read in from the file.
     */
    public ModelImage readImage( String fileName, String fileDir,
            boolean multiFile, FileInfoBase fileInfo,
        ...
        try
        {
            if (fileType != FileBase.BRUKER)
            {
                ...
                if (fileType == FileBase.UNDEFINED)
                { // if image type not defined by extension, popup
    ...
    /**
     * Reads a thumbnail image from an XML file.
     * if thumbnail is empty, will returned FileImageXML will be null.
     * @param name filename
    ...
    /**
     *  Determines file type from the file name and calls
     *  different write functions based on the file type.
     *  Stores file in the specified file name and directory given by the options.
     *  Calls appropriate dialogs if necessary.
     *  Supports files of type raw, analyze, and DICOM.
        ...
        if ( UI.isScriptRecording() ) {
            if ( options.isSaveAs() ) {
                UI.getScriptDialog().append(
                        "SaveImageAs " + UI.getScriptDialog().getVar( image.getImageName() ) + " "
                        + getSuffix( fileType ) + " " );
                ...
                UI.getScriptDialog().append(
                        "SaveImage " + UI.getScriptDialog().getVar( image.getImageName() ) + " " + getSuffix( fileType )
                        + "\n" );
            }
        }
    }
     ...
     *   @return    The image file type entered by the user,
     *              or FileBase.ERROR if FileIO is quiet or the dialog is
     *              cancelled.
     *   see        FileBase.ERROR
     *   see        isQuiet()
     ...
     *   Not all file types are supported to handle multifiles.  Those that
     *   do support building a 3D image out of a series of 2D images are
     *   defined to have that input capability here.
     *   @param fileType    Input file type.  File types which MIPAV does not allow multifile images
     *                      get the file type returned as sent.
     *   @param multiFile   If <code>true</code>, returns a new filetype
     *                      corresponding to the input filetype + MULTIFILE.
     ...
     *	@return     The suffix or file-extension.  For example, <q>.jpg</q>.
    ...
    /**
     *   Only used for COR volume files with hyphen in name
     *	Breaks the filename into basename and suffix, then returns the suffix.
     *	@param fn	The filename.
     *	@return     The suffix or file-extension.  For example, <q>-info</q>.
     *              Note that suffix includes the separator '-'
     */
    ...
    /**
     *	Looks in the image directory and returns all images with the same suffix as <code>fileName</code>,
     *	sorted in lexicographical order.
     *	@param	fileDir		Directory to look for images.
     *	@param	fileName	File name of the image.
     *	@return     An array of the image names to be read in or saved as.
     */
    public String[] getFileList( String fileDir, String fileName )
        ...
        for ( i = 0; i < nImages; i++ ) { // (bubble sort? ... )
            for ( j = i + 1; j < nImages; j++ ) {
                result = FilenameSorter.compareToLastNumericalSequence(fileList2[i], fileList2[j]); // compare based on last numerical sequence
//              result = fileList2[i].compareTo( fileList2[j] );
    ...
    /**
     *   Only for FreeSurfer COR volume files
     *	Looks in the image directory and returns all images with the same root up to the hyphen,
     *	sorted in lexicographical order.  Will set the number of images (<code>nImages</code>) for
     *   the calling program.
     *	@param	fileDir		Directory to look for images.
     *	@param	fileName	File name of the image.
     *	@return     An array of the image names to be read in or saved as.
     */
    public String[] getCORFileList( String fileDir, String fileName )
        throws OutOfMemoryError {
        int i;
        int j = 0;
        int k;
     ...
     * An index might be 0001, or 140, for example.
     ...
     *   Numerics and some special characters <code>[ -  _ .</code> are removed
     *  from the end of the file.
     *   @param fName  File name where the last characters are alpha-numerics
     ...
     * Any numbers or <q>.info</q> or <q>.info~</q> will be removed from after
     * a hyphen in the given fname.
     *   @param fName  File name where the last characters are alpha-numerics
     *                 indicating the image number or .info or .info~
     *   @return 	  File name without numbers on the end.
     */
    ...
    /**
     *	Sorts an array of floats (using insertion sort), turns into array of
     *  ints used to sort images by slice location; thus, the final value of
     *  B[i] is for the ith image read in, put that image at location B[i] in
     *  the image buffer.  This is necessary for images labeled dicom1, dicom2,
     *  etc because dicom11, dicom12, ... will come before dicom2.  Also our
     *  only indication of the "true" ordering is slice location.
    ...
    /**
     *	Sorts an array of ints (using insertion sort), turns into array of ints
     *   used to sort images by image number; thus, the final value of B[i]
     *   is for the ith image read in, put that image at location B[i] in the
     *   image buffer.  This is necessary for images labeled dicom1, dicom2, etc
     *   because dicom11, dicom12, ... will come before dicom2.  Also our only
     *   indication of the "true" ordering is image number.
     *	@param A      Array to be sorted.
     *	@param B      Array it goes into.
     *	@param size   Size of the array (both arrays are the same size).
     *    @return       <code>false</code> only if any of the numbers in the array are equal.
     ...
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *  @param one      Indicates that only the named file should be read, as
     *                  opposed to reading the matching files in the directory,
     *                  as defined by the filetype.  <code>true</code> if only
     *                  want to read one image from 3D dataset.
     *	@return			The image that was read in, or null if failure.
     */
    ...
    private ModelImage readMagnetomVision( String fileName, String fileDir,
        ...
        try {
            ...
            progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "Magnetom Vision files ...", 0, 100,
                    false, null, null );
            if ( !quiet ) {
    ...
    private ModelImage readGEGenesis5X( String fileName, String fileDir ) {
        ...
        try {
            ...
            progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + "" + fileName, UI.getProgressBarPrefix() + "GE formatted image(s) ...", 0, 100,
                    false, null, null );
            if ( !quiet ) {
                progressBar.setVisible( true );
            }
        ...
        for ( i = 0; i < nImages; i++ ) {
            try {
                ...
                if ( fileList[i] != null ) {
                    ...
                    myFileInfo.setExtents( extents );
                    myFileInfo.setOrigin(
                            ( (FileInfoGESigna5X) ( myFileInfo ) ).getOriginAtSlice( imageFile.getImageNumber() - 1 ) );
    ...
    /**
     *	Reads a multi TIFF file by first reading the headers then reading in
     *  each separate file.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@return			The image that was read in, or null if failure.
    ...
    private ModelImage readTiffMulti( String fileName, String fileDir ) {
        ...
        try {
            ...
            progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + "" + fileName, UI.getProgressBarPrefix() + "TIFF image(s) ...", 0, 100, false, null,
                    null );
            if ( !quiet ) {
                progressBar.setVisible( true );
            }
        ...
        for ( i = 0; i < nImages; i++ ) {
            try {
                ...
                if ( !quiet ) {
                    MipavUtil.displayError(
                            "Unable to read images: the image\n" + "number in the file " + myFileInfo.getFileName()
                            + " is corrupted." );
                }
                if ( image != null ) {
                    image.disposeLocal();
    ...
    /**
     *	Reads a COR file by first reading the header file then reading in each
     *  separate slice file.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@return			The image that was read in, or null if failure.
    ...
    private ModelImage readCOR( String fileName, String fileDir ) {
        ...
        try {
            ...
            image = new ModelImage( myFileInfo.getDataType(), extents, myFileInfo.getFileName(), UI );
            // Progress bar shows what % of images have been read.
            progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "COR image(s) ...", 0, 100, false, null,
                    null );
            if ( !quiet ) {
                progressBar.setVisible( true );
            }
    ...
    /**
     *	Reads a BRUKER file by first reading in the d3proc header file,
     *   second the reco header file, third the acqp file,
     ...
     *	@return			The image that was read in, or null if failure.
     */
    private ModelImage readLSM( String fileName, String fileDir,
            int secondAddress, boolean one ) {
        ModelImage image = null;
        FileLSM imageFile;
    ...
    private ModelImage readLSMMulti( String fileName, String fileDir ) {
        ...
        try {
            ...
            image = new ModelImage( myFileInfo.getDataType(), extents, myFileInfo.getFileName(), UI );
            // Progress bar shows what % of images have been read.
            progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "LSM image(s) ...", 0, 100, false, null,
                    null );
            if ( !quiet ) {
                progressBar.setVisible( true );
            }
     ...
     *                  as defined by the filetype.  <code>true</code> if only
     *                  want to read one image from 3D dataset.
     *  @param readQT   Indicates that a QuickTime movie file is being read.
     *                  <code>true</code> if this file represents QuickTime.
     *	@return			The image that was read in, or null if failure.
     */
    ...
    private ModelImage readQT( String fileName, String fileDir ) {
        ...
        /*
         ...
         ModelImage image = null;
         try {
         imageFile = new FileQT(UI, fileName, fileDir);
         image = imageFile.readImage();
         //LUT      = ((FileQT)imageFile).getModelLUT();
         }
         catch (IOException error) {
         if (image != null) {
         image.disposeLocal();
    ...
    /**
     *	Reads a RAW file by calling the read method of the file.  Gets the necessary information
     *	from a dialog, if the <code>fileInfo</code> parameter is null, and otherwise relies on the
     *	information stored in <code>fileInfo</code> to properly read in the file.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@param fileInfo File info of the image file; usually null, but if defined no dialog appears.
     *	@return			The image that was read in, or null if failure.
     */
    private ModelImage readRaw( String fileName, String fileDir,
            FileInfoBase fileInfo ) {
        ModelImage image = null;
        FileRaw imageFile;
        ...
        if ( UI.isScriptRecording() ) {
            ...
            UI.getScriptDialog().append(
                    "\tAttributes " + fileInfo.getDataType() + " " + fileInfo.getEndianess() + " "
                    + fileInfo.getOffset() + " " + fileInfo.getExtents().length + " " );
            for ( i = 0; i < fileInfo.getExtents().length; i++ ) {
                UI.getScriptDialog().append(
                        fileInfo.getExtents()[i] + " " + fileInfo.getResolutions()[i] + " "
     ...
     *	@return			The image that was read in, or null if failure.
     */
    private ModelImage readRawMulti( String fileName, String fileDir,
            FileInfoBase fileInfo ) {
        ModelImage image = null;
        FileRaw imageFile;
        ...
        progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "Raw image(s) ...", 0, 100, false, null, null );
        if ( !quiet ) {
            progressBar.setVisible( true );
        }
        if ( !UI.isAppFrameVisible() ) {
    ...
    /**
     *	Reads an analyze file by calling the read method of the file.  Also checks
     *    if it's a Cheshire, Interfile, or NIFTI and
     *	if so, calls that method instead.
     *  This method contains special code to not display the progress bar should
    ...
    /**
     *	Reads a NIFTI file by calling the read method of the file.
     *	if so, calls that method instead.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *  @param one      Indicates that only the named file should be read, as
     ...
     * This method contains special code to not display the progress bar
     * should it load <q>splash.xml</q>.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *  @param one      Indicates that only the named file should be read, as
     *                  opposed to reading the matching files in the directory,
    ...
    /**
     *	Reads a multi Analyze file.  Gets a list of the images from the file
     *  directory and reads them each in.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@return			The image that was read in, or null if failure.
     */
    private ModelImage readAnalyzeMulti( String fileName, String fileDir ) {
        ...
        progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + "" + fileName, UI.getProgressBarPrefix() + "Analyze image(s) ...", 0, 100, false, null,
                null );
        if ( !quiet ) {
            progressBar.setVisible( true );
        }
        ...
        for ( i = 0; i < nImages; i++ ) {
            try {
                ...
                if ( !quiet ) {
                    MipavUtil.displayError(
                            "Unable to read images: the image\nnumber in the file " + fileInfo.getFileName()
                            + " is corrupted." );
                }
                if ( image != null ) {
                    image.disposeLocal();
    ...
    /**
     *	Reads a multi NIFTI file.  Gets a list of the images from the file
     *  directory and reads them each in.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@return			The image that was read in, or null if failure.
    ...
    private ModelImage readNIFTIMulti( String fileName, String fileDir ) {
        ...
        progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "NIFTI image(s) ...", 0, 100, false, null,
                null );
        if ( !quiet ) {
            progressBar.setVisible( true );
        }
    ...
    /**
     *	Reads a multi XML file.  Gets a list of the images from the file
     *  directory and reads them each in.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *	@return			The image that was read in, or null if failure.
    ...
    private ModelImage readXMLMulti( String fileName, String fileDir ) {
        ...
        progressBar = new ViewJProgressBar( UI.getProgressBarPrefix() + fileName, UI.getProgressBarPrefix() + "XML image(s) ...", 0, 100, false, null, null );
        if ( !quiet ) {
            progressBar.setVisible( true );
        }
        if ( !UI.isAppFrameVisible() ) {
            ...
            if ( fileName.endsWith( ".ct" ) ) {
                ...
                for ( i = 0; i < fileList.length; i++ ) {
                    if ( fileList[i].endsWith( ".log" ) ) {
                        ...
                        if ( imageFile.trimmer( fileName ).equals(
    ...
    /**
     * Reads a single DICOM image by calling the read method of the file.
     * fileDir is set and
     * If it cannot read the header, null will be returned.
     *	@param fileName	Name of the image file to read.
     *	@param fileDir Directory of the image file to read.
     *	@return	The image that was read in, or null if failure.
     */
    ...
    /** Reads a list of DICOM files.  DICOM images have all their slices in
     * separate files (except multi-frame), with different information in each
     * header.  Slices are positioned as they are listed in the
     * <code>fileList</code>. without regard to the positional data stored in
     * the image. However, some positional data must be included in the image
     * -- even if not acted upon -- so the headers are read twice.
     * This method is identical (indeed, it calls)
    ...
    /**
     * Reads a DICOM file.  DICOM images store slices in separate files
     * or as a 'multi-frame' image,  with different information in
     * each header.  The position of the slice in the image is
     * determined by information found in the header so
     * all the headers must be read before the images can be read.
     * <p>
     * This method does not read any other images in the
     * directory; returns a modelImage with 3 dimensions
     * read in only when a multiple slices are stored as a
     * multi-frame image.
     * @return    The image that was read in, or null if failure.
     * @exception IllegalArgumentException  if File f is null, does not exist,
     *                                      or cannot be read.
     */
    public ModelImage readDicom( File f )
        throws IOException,
    ...
    /**
     * Reads a list of DICOM files.  DICOM images have all their slices in separate
     * files (except multi-frame), with different information in each header.
     * The position of the slice in the image is determined by information found
     * in the header so all the headers must be read before the images can be
     * read.  That's why this method goes through all the images twice.
     *
     * @param fileList     Names of the image files residing in
     *                     <code>this#fileDir</code> to read.
     * @param performSort  <code>true</code> if this method is to sort the
     *                     files in the list, or, <code>false</code> will
     *                     apply the images in each file in the order they
     *                     appear as the order to use in the ModelImage.
     * @return             The image that was read in, or null if failure.
     */
     ...
     * read.  That's why this method goes through all the images twice.
     *
     * @param selectedFileName Name of the image file selected to be readin
     *                 <code>this#fileDir</code> to read. Used to ID study and series number
     * @param fileList List of all the files to be read in.
     * @param performSort <code>true</code> if this method is to sort the
     *                    files in the list, or,
     *                    <code>false</code> will apply the images in each
     *                    file in the order they appear as the order to
     *                    use in the ModelImage.
     *    @return The image that was read in, or null if failure.
     */
    public ModelImage readDicom( String selectedFileName, String[] fileList, boolean performSort ) {
        ...
        try {
            ...
            if ( pInterface == null ) {
                // Progress bar shows what % of images have been read.
                pInterface = new ViewJProgressBar( UI.getProgressBarPrefix() + "" + fileName, UI.getProgressBarPrefix() + "DICOM image(s) ...", 0, 100, false,
                        null, null );
            }
        ...
        if ( !( (FileInfoDicom) ( myFileInfo ) ).isMultiFrame() ) {
             ...
             * needed for displaying overlay information properly
             * we look at the first two images and see which position varies by
             * the third resolution.  that position indicates which axis the
             * image is sliced on, which indicates the orientation
             * first go through headers and find out where in the
            ...
            for ( i = 0, nImages = 0; i < nListImages; i++ ) {
                ...
                try {
                    ...
                    if ( performSort ) {
                        if (seriesNo.equals(seriesNoMaster) && studyID.equals(studyIDMaster)) {// && acqNo.equals(acqNoMaster)) {
                            ...
                            if (matrix != null) {
                                 ...
                                 * from the Image Position tag, by the matrix.  The tPt array
                                 * now has the three numbers arranged as if this
                                 * image had been transformed.  The third place in the array
                                 * holds the number that the axis is being sliced along.
                                 * xlocation, ylocation, zlocation are from the DICOM tag
                                 * 0020,0032 patient location;
                                 */
             ...
             * we will first try to order the image set based on orientation of
             * slices.  If there are slices in the image set which have the same
             * location on the Z-axis, the data may be a time-based set.  If we
             * guess that it is (and there aren't nearly enough test-datasets),
             * we attempt to re-order the ordered Z-locations based on instance
             * number to pull out the time-data (ie., of two images with the same
             * Z-axis, a lower instance number was taken at an earlier time; all
             * such earlier-time images are grouped).  If neither solution seemed
             * to work, then we order all the images strictly on the instance
             * number.  There might be some problems if we had to do this.
             * If the list sent to this method was found using the getFileList()
             * method, all the names are in lexicographical order.  This is the
             * default ordering if neither sorting method works, or if performSort
             * is false.
             */
            ...
            if ( performSort ) {
                ...
                if ( nImages > 1 && !valid ) {
                    ...
                    while ( orientsList.size() > 0 ) {
                        ...
                        Preferences.debug(
                                "Loading, and making comparison to: " + ref0.getIndex() + ".." + ref0.getLocation()
                                + "\n", 4 );
                        ...
                        for ( Enumeration e = orientsClone.elements(); e.hasMoreElements(); ) {
                            ...
                            if ( !refi.equals( ref0 ) ) {
                                ...
                                Preferences.debug(
                                        "!!  Comparison to: " + ref0.getIndex() + ".." + ref0.getLocation() + "\n", 4 );
            ...
            // A separate and equally valid explanation of the Image Orientation matrix is:
            // X patient  Y patient  Z patient
            // X image     1          0         0
            // Y image     0          1         0
            // Z image     0          0         1
            // In this case, the number "1" at X image, X patient means the x in the image is the
            // same as the x in the patient and goes in the same direction; thus, the image x goes from
            // right to left.  If it were a "-1", that would mean the image x goes from left to right.
            // If it were a "0", and there was a "1" in X image, Y patient, that would mean that the image
            // x goes from the anterior of the patient to the posterior.  This is the case in sagittal images,
            // for example.
            //
            // We're concerned with what the z image is, because that tells us what the orientation is.  If there
            // is a "1" in X patient, the patient x is the image's z-axis, making this a sagittal image.  If there is a "1"
            // in Y patient, the patient y is the image's z-axis, making this a coronal image.  And if there is
            // a "1" in Z patient, the patient z is the image's z-axis, making this an axial image.  We look at
            // absolute value because here we are not concerned about right to left vs left to right, only which
            // patient axis the slices were taken along.
           ...
           float sliceSpacing =   -1;
            ...
            if ( ( (FileInfoDicom) image.getFileInfo(0)).getTagsList().get("0018,0050") != null ||
               ...
               if ( (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo(0)).getTagsList().get("0018,0050")).
                   getValue( true) != null) {
                  sliceThickness = Float.parseFloat(
                      (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo(0)).getTagsList().get("0018,0050")).
                      getValue(true));
               }
               ...
               if ( (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo(0)).getTagsList().get("0018,0088")).
                   getValue(true) != null) {
                  sliceSpacing = Float.parseFloat(
                      (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo(0)).getTagsList().get("0018,0088")).
                      getValue(true));
               }
//               System.err.println("Slice Spacing: " + sliceSpacing);
               if (sliceSpacing != -1 ) sliceThickness = sliceSpacing;
//               System.err.println("Slice Thickness: " + sliceThickness);
            ...
            if ( ( (FileInfoDicom) image.getFileInfo( 0 ) ).getTagsList().get( "0020,0032" ) != null ) {
                ...
                res3Dim = ( xLoc0 - xLoc1 ) * ( xLoc0 - xLoc1 ) +
                          ( yLoc0 - yLoc1 ) * ( yLoc0 - yLoc1 ) +
            ...
            if (  ( (FileInfoDicom) image.getFileInfo( 0 ) ).getTagsList().get( "0020,1041" ) != null && sliceThickness == -1 ) {
                if ( (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo( 0 ) ).getTagsList().get( "0020,1041" ) ).getValue(
                        true ) != null ) {

                    sliceDifference = Float.parseFloat(
                            (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo( 1 ) ).getTagsList().get( "0020,1041" ) ).getValue(
                                    true ) )- Float.parseFloat(
                                                    (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo( 0 ) ).getTagsList().get( "0020,1041" ) ).getValue(
                                                            true ) );
                    ...
                    if ( Math.abs(sliceDifference) < sliceThickness  && Math.abs(sliceDifference) > 0.001 ) {
                        ...
                        for ( int m = 0; m < nImages - 1; m++ ) {
                            ( (FileInfoDicom) image.getFileInfo( m ) ).getResolutions()[2] = Float.parseFloat(
                                    (String) ( (FileDicomTag) ( (FileInfoDicom) image.getFileInfo( m + 1 ) ).getTagsList().get( "0020,1041" ) ).getValue(
                                            true ) ) - Float.parseFloat((String) (
                                            (FileDicomTag) ( (FileInfoDicom) image.getFileInfo( m ) ).getTagsList().get( "0020,1041" ) ).getValue( true ) );
                        }
                        if ( nImages > 2 ) {
                            ( (FileInfoDicom) image.getFileInfo( nImages - 1 ) ).getResolutions()[2] = ( (FileInfoDicom) image.getFileInfo( nImages - 2 ) ).getResolutions()[2];
                        }
                    }
                }
            }
     ...
     *	@param fileName	Name of the image file to read.
     *	@param fileDir	Directory of the image file to read.
     *  @param multifile Indication whether this file is be read alone, or if
     *                  the reader is to read all matching filenames as a part
     *                  of the dataset.  <code>true</code> if want to read all
     *                  matching files to form an image into a 3D dataset.
     *	@return			The image that was read in, or null if failure.
     */
    private ModelImage readJimi( String fileName,
                                 String fileDir,
                                 boolean multifile )
    {
    ...
    private boolean writeAvi( ModelImage image, FileWriteOptions options ) {
        ...
        try {
            ...
            if ( !aviFile.writeImage( image, options.getImageB(), options.getLUTa(), options.getLUTb(),
                    options.getRGBTa(), options.getRGBTb(), options.getRed(), options.getGreen(), options.getBlue(),
                    options.getOpacity(), options.getAlphaBlend(), options.getPaintBitmap(), options.getAVICompression() ) ) {
    ...
    private boolean writeXML( ModelImage image, FileWriteOptions options ) {
        ...
        try {
            ...
            /**
             * Set the LUT (for grayscale) and ModelRGB (for color)
             * doesn't matter if either is null
             */
    ...
    /** Provides a method of conversion from <code>FileInfoLSM</CODE> to
     * <CODE>FileInfoImageXML</CODE>, by filling the <CODE>FileInfoImageXML</CODE>
     * with sets of chosen image information.
    ...
    /** Provides a method of conversion from <code>FileInfoDicom</CODE> to
     * <CODE>FileInfoImageXML</CODE>, by filling the <CODE>FileInfoImageXML</CODE>
     * with sets of chosen image information (from the DICOM tags).
     * <p>
     * The XML format suggests that each DICOM tag become a seperate set,
     * and any multiple values stored in the DICOM tag, becomes parameter data.
     * The set name is the DICOM tag name.  Each value of the tag is stored as a
     * separate parameter in the set, and its description is stored as
     * the DICOM key (group and element number, as displayed in the file,
     * &quot;dicom.dictionary&quot;) along with its position in the
     * value multiplicity.  All values stored are stored as value-type
     * &quot;string&quot;, and neither date nor time are set.  Order of entries
     * is not guaranteed, as order is not meaningful in XML.
     * <p>
     * Be sure to see the image.xsd file for more information.
     * @see #getDicomSaveList(FileInfoDicom)
     * @see JDialogDicom2XMLSelection
     * @param sourceInfo The FileInfoDicom which is the source for
     *                   user-selectable tags
     * @param destInfo The FileInfoImageXML that holds the image information to be
                ...
                try {
                    if ( tagValues[q].toString().indexOf( 0 ) != -1 ) {
                        ...
                        try {
                            ...
                            System.err.println(
                                    "(" + tagKey + ") Trying to output " + "current string bounded by "
     ...
     * This method chooses the tags to be saved by presenting a dialog for the
     * user to select a list of tags or by reading the
     * <q>dicomsave.dictionary</q> file.
    ...
    protected Hashtable getDicomSaveList( FileInfoDicom sourceInfo )
    {
        ...
        if ( quiet ) { // don't bother asking user when running a macro.
            ...
            if (tags2save == null) System.out.println("tags2save is null");
        } else {
    ...
    private boolean writeTiff( ModelImage image, FileWriteOptions options ) {
        ...
        try { // Construct a new file object
            ...
            if (LUT == null) {
              ...
              extents[0] = 4;
              extents[1] = 256;
              ( (FileTiff) imageFile).writeImage(image, new ModelLUT(1, 256, extents),
                                                 options);
            }
            else {
    ...
    /**
     *	Writes a DICOM file to store the image.  Calls a dialog if the source
     * isn't a DICOM image.  DICOM images
     *	are each written to a separate file with a different header.
     *	@param image	The image to write.
     *	@param options	The options to use to write the image.
     *	@return		Flag indicating that this was a successful write.
     */
    private boolean writeDicom( ModelImage image, FileWriteOptions options ) {
        ...
        if ( image.isDicomImage() ) {
            ...
            if ( image.getExtents().length > 2 ) { // This sets the fileinfo to the same for all slices !!
                ...
                for ( int k = 0; k < image.getExtents()[2]; k++ ) { // I think this is OK for saving via secondary capture.
    ...
    private boolean writeJimi( ModelImage image, FileWriteOptions options ) {
        ...
        for ( int i = beginSlice; i <= endSlice; i++ ) {
            if ( image.isColorImage() ) {
                ( (ViewJFrameImage) ( image.getImageFrameVector().firstElement() ) ).getComponentImage().setColorPaintBuffers(
                        0, i, true );
            } else {
                ( (ViewJFrameImage) ( image.getImageFrameVector().firstElement() ) ).getComponentImage().setPaintBuffers(
                        0, i, null, null, true );
            }
        ...
        ( (ViewJFrameImage) ( image.getImageFrameVector().firstElement() ) ).getComponentImage().show( 0, slice, null,
                null, true, -1 );
        return true;
    }
    ...
    /**
     *	Calls GUI dialogs based on what type of image this is and the number of
     * dimensions
     *	@param extents	Extents of the image, used to determine what type of
     *                  dialog to call and initialize variables.
     *	@param isTiff	Flag indicating if this is a TIFF file; TIFF files
     *                  require more options.
     *	@param options	Options to get from the dialog and save.
     */
    private boolean callDialog( int[] extents, boolean isTiff,
    ...
    private void recordOptions( FileWriteOptions options, int[] extents ) {
        ...
        if ( nDims == 3 ) {
            ...
            if ( options.getFileType() == FileBase.TIFF || options.getFileType() == FileBase.MINC ) {
                UI.getScriptDialog().append(
                        options.getBeginSlice() + " " + options.getEndSlice() + " " + options.getTimeSlice() + " " );
                if ( options.isMultiFile() ) {
                    UI.getScriptDialog().append( options.getStartNumber() + " " + options.getDigitNumber() + " " );
                }
            } else {
                UI.getScriptDialog().append(
                        options.getBeginSlice() + " " + options.getEndSlice() + " " + options.getBeginTime() + " "
                        + options.getEndTime() );
            }
        }
        if ( options.getFileType() == FileBase.MINC ) {
            UI.getScriptDialog().append(
                    options.getOrientation() + " " + options.getXStart() + " " + options.getXSpace() + " "
                    + options.getYStart() + " " + options.getYSpace() + " " + options.getZStart() + " "
                    + options.getZSpace() + " " + options.isLeftToRight() + " " + options.isPosToAnt() + " "
                    + options.isInfToSup() + " " );
        }
    }
    ...
    /**
     *   Refers to whether or not the FileIO will send alerts
     *   to the user about progress or errors.
     */
     ...
     *   Refers to whether or not the FileIO will send alerts
     *   to the user about progress or errors.
     * @param q Indicates if the output from the methods in this object are to
     *          display dialogs interrupting flow in a locally-defined manner.
     *          <code>true</code> indicates that the process is to NOT inform
     *          the user and therefore be <q>quiet</q>.
     */
     ...
     *	@return	Index of argument that is the biggest.  That is,
     *          if <code>zero</code> is the largest of the three numbers, the
     *          value 0 is returned; alternatively, if the second argument
     *          is the largest of the three numbers, the value 1 is returned;
     *          and so on.
     */
