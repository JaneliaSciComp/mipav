...
public class AlgorithmImageHessian extends AlgorithmBase {
    ...
    private void run2D() {
        ...
        for ( row = 0; row < numRows; row++ ) {
            progressBar.updateValue( Math.round
                    ( ( (float) ( row ) / ( numRows - 1 ) * 100 ) ), activeImage );
    ...
    private void run3D() {
        ...
        for ( int scale = 1; scale < 2; scale++ ) {
            ...
            for ( plane = 0; plane < numPlanes; plane++ ) {
                progressBar.updateValue( Math.round
                        ( ( (float) ( plane ) / ( numPlanes - 1 ) * 100 ) ), activeImage );
                for ( row = 0; row < numRows; row++ ) {
                    for ( col = 0; col < numCols; col++ ) {
                        ...
                        structureness = Math.sqrt(
                                evals[lut[0]] * evals[lut[0]] + evals[lut[1]] * evals[lut[1]]
                        ...
                        resultBuffer[plane * numRows * numCols + row * numCols + col] = (float) ( vesselness * 100.0 );
            ...
            for ( plane = 0; plane < numPlanes; plane++ ) {
                ...
                for ( row = 0; row < numRows; row++ ) {
                    for ( col = 0; col < numCols; col++ ) {
                        ...
                        if ( row == 36 && col == 92 ) {
                            System.out.println(
                                    "expRa: " + fltFmt.format( expRa ) + "  (1.0 - expRa): "
                                    + fltFmt.format( ( 1.0 - expRa ) ) );
                            System.out.println( "expRb: " + fltFmt.format( expRb ) );
                            System.out.println(
                                    "expS: " + fltFmt.format( expS ) + "  (1.0 - expS): "
    ...
    private void runInDest3D() {
        ...
        for ( int scale = 1; scale < 2; scale++ ) {
            ...
            for ( plane--; plane < ( ( numPlanes / 2 ) + 2 ); plane++ ) { // do 3 center slices
                ...
                for ( row = 0; row < numRows; row++ ) {
                    for ( col = 0; col < numCols; col++ ) {
                        ...
                        // output result on different slices
                        resultBuffer[plane * numRows * numCols + row * numCols + col] = (float) ( vesselness * 1000.0 );
            ...
            /*      PFH: I put this here to run the algorithm with different
             scale values in batch mode.  Thes code saves the solution
             ...
             srcImage.getImageName() + "_vsGauss" + scale,FileBase.XML);
             */
        ...
        /*
         System.out.println("Max maxExpRa: " + maxExpRa);
         System.out.println("Max maxExpRb: " + maxExpRb);
         System.out.println("Max maxExpS: " + maxExpS);
         System.out.println("Max S: " + maxS);
    ...
    // Algorithm is given a scale range and scale increment and computed the
    // 1) hessian, 2) eigenvalues, 3) eigenvectors of the hessian.  These
    // values are saved in individual files for each scale
