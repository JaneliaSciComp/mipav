 ...
 *
 */
public class AlgorithmCostFunctions
     ...
     *   @param smoothSize
     */
    public AlgorithmCostFunctions( ModelSimpleImage rImage, ModelSimpleImage iImage, int functionID,
            int nBins, float smoothSize ) {
    ...
    /**
     * Sets the reference weight image. If the weight values are outside the range [0:1] then
     * the weigthts will be remapped to be between 0:1.
    ...
    /**
     * Sets the input weight image. If the weight values are outside the range [0:1] then
     * the weigthts will be remapped to be between 0:1.
    ...
    /**
     *   Precalculates information and allocates buffers used in the calculation
     *   of mutual information statistics.
    ...
    /**
     *	Calculates the cost (dependent on the selected cost function) based on the reference image
     *   and the input image.
    ...
    /**
     *   Correlation ratio cost function using weighting functions to mask out areas that
     *   should not be included in the cost function calculations
     *   @param tMatrix
     */
        ...
        for ( z = 0; z <= zEnd; z++ ) {
            ...
            for ( y = 0; y <= yEnd; y++ ) {
                ...
                for ( x = minMaxPt.x; x <= minMaxPt.y; x++ ) {
                    ...
                    for ( c = 1; c <= 3; c++ ) {
                        b1 = dy1
                                * ( dx1 * inputImage.data[4 * position1 + c] + dx * inputImage.data[4 * position11 + c] )
                                        + dy
                                                * ( dx1 * inputImage.data[4 * position1x + c]
                                                        + dx * inputImage.data[4 * position11x + c] );

                        b2 = dy1
                                * ( dx1 * inputImage.data[4 * position2 + c] + dx * inputImage.data[4 * position21 + c] )
                                        + dy
    ...
    private final double hanning( double x, int w ) {
        if ( Math.abs( x ) > w ) {
            return 0.0;
        } else {
    ...
    private final double kernelVal( double x, int w ) {
    ...
    private final double getSinc( float[] data, double x, double y, double z ) {
     ...
     *
     */
    private void calcEntropy( TransMatrix tMatrix, double[] jointEntropy,
            double[] margEntropyR, double[] margEntropyI ) {
    ...
    private void calcEntropySmoothed( TransMatrix tMatrix, double[] jointEntropy,
            double[] margEntropyR, double[] margEntropyI ) {
    ...
    private void calcEntropySmoothedWgt( TransMatrix tMatrix, double[] jointEntropy,
            double[] margEntropyR, double[] margEntropyI ) {
        int x, y, z;
        double tmpZ1, tmpZ2, tmpZ3;
        int indexZ;
        ...
        for ( z = 0; z <= zEnd; z++ ) {
            ...
            for ( y = 0; y <= yEnd; y++ ) {
                ...
                for ( x = minMaxPt.x; x <= minMaxPt.y; x++ ) {
                    ...
                    // Find iCenter and indexValueR.  iCenter is bin number for this pixel in the Input image (y coordinate
                    // of joint histogram image).  indexValueR in the bin number for this pixel in the Reference image (x
                    // coordiante of joint histogram image).
                    iCenter = (int) ( ( value - inputImage.min ) * constantI );
                    indexValueR = (int) ( ( refImage.data[index] - refImage.min ) * constantR );
     ...
     *
     */
    private final void findRangeX( Point minMaxPt ) {
