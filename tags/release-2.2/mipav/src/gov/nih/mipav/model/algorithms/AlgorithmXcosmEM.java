...
/**
 * <p>XCOSM is an algorithm for Computational Optical Sectioning
 * Microscopy (COSM) algorithms to remove out-of-focus light in 3-D
 * volumes collected plane by plane using either widefield or confocal
 * fluorescence microscopy.</p>
 *
 * <p>References:<br>
 * Available fro the web site: http://www.essrl.wustl.edu/~preza/xcosm/
 *
 * <ol>
 * <li># J.-A. Conchello, "Super-resolution and convergence properties of the
 * expectation-maximization algorithm for maximum-likelihood deconvolution of
 * incoherent images", J. Opt. Soc. Am. A, Vol 15, (10), pp. 2609-2620, 1998.</li>
 *
 * <li> J.-A. Conchello, "Fluorescence photobleaching correction for
 * expectation maximization algorithm" in Three-Dimensional microscopy: image
 * acquisition and processing T. Wilson and C. J. Cogswell editors,
 * SPIE  2412-21, pp138-146, 1995. </li>
 *
 * <li> J.-A. Conchello and J. G. McNally "Fast regularization technique for
 * expectation maximization algorithm for computational optical sectioning
 * microscopy" in Three-Dimensional microscopy: image acquisition and processing
 * C. J. Cogswell, G. S. Kino, and T. Wilson, editors, SPIE 2655, pp.199-208,
 * 1996. </li>
 *
 * <li> J. Markham and J.-A. Conchello, "Tradeoffs in regularized
 * maximum-likelihood image restoration" in 3D Microscopy: Image Acquisition
 * and Processing IV. C. J. Cogswel, J.-A. Conchello, and T. Wilson, editors,
 * SPIE 2984-18,1997. </li>
 * </ol>
 * </p>
 *
 * <p> The code in this file is a direct port of the freely-available <b>C</b>
 * code for the COSM project, linked to the website identified above.  The X
 * signifies an X-Windows interface.  The code in this file computes a restored
 * volume of the test data, that is also available on the website that is
 * similar to the results of the compiled <b>C</b> code.  The differences are
 * small (< 0.01) and may be because of different representations for integer
 * and float data between Java and <b>C</b>, which when iterated yield the small
 * differeces that have been noted.
 *
...
public class AlgorithmXcosmEM extends AlgorithmBase {
    ...
    public AlgorithmXcosmEM(ModelImage destImg, ModelImage srcImg,
                            ModelImage psfImg, int lowZ,
                            boolean estimateDecay, float decayConstant,
                            int percentIterationsOriginalSize,
                            int percentIterationsHalfSize,
                            int percentIterationsQuaterSize, int maxIterations,
                            int backIterations, boolean penaltyIntensity,
                            float penaltyValue) {
        ...
        if ((origPx < origInx) || (origPy < origIny)) {
            MipavUtil.displayError(
                "PSF must have NX >= Input NX, and NY >= Input NY)");
        }
        ...
        if ((origPz > origRz) || (origPx > origRx) || (origPy > origRy)) {
            MipavUtil.displayError(
                "PSF must have dimensions <= output dimensions");
        }
        ...
        if ((_1xperc + _2xperc + _4xperc) != 100) {
            MipavUtil.displayError(
                "AlgorithmXcosmEM.  1x, 1/2x, and 1/4x percentages do NOT add up to 100");
            return;
        }
        ...
        try {
            ...
            work = fwork = null;
            errorCleanUp(
                "AlgorithmXcosmEM: Out of memory when creating work buffers", true);
            return;
        } // end try{}-catch{}
    }
    ...
    public void runAlgorithm() {
        ...
        if (originalImage == null) {
            MipavUtil.displayError(
                "AlgorithmXcosmEM.runAlgorithm()  Source Image is null");
            return;
        }

        if (originalImage.getNDims() == 2) {
            MipavUtil.displayError(
                "AlgorithmXcosmEM.runAlgorithm()  Source Image must be 3D");
            return;
        }
...
//        constructLog();
        ...
        if (originalImage.isColorImage()) {
            MipavUtil.displayError(
                "AlgorithmXcosmEM.runAlgorithm()  Source Image must be GrayScale");
            return;
        }
...
//        int diffrLength = (origPx + 2) * (origPy + 2) * (2 * origPz);
        ...
        try {
            ...
            errorCleanUp(
                "AlgorithmXcosmEM: Out of memory when creating image buffers", true);
            return;
        } // end try{}-catch{}
        ...
        processEM(sourceBuffer, origInx, origIny, origInz,
                  resultBuffer, origRx, origRy, origRz,
                  psfBuffer, origPx, origPy, origPz,
                  diffr, origLowZ, origHighZ);
    ...
    void processEM(float[] image, int Inx, int Iny, int Inz,
                   float[] result, int Rx, int Ry, int Rz,
                   float[] psf, int Px, int Py, int Pz,
                   float[] diffr, int lowZ, int highZ) {
        ...
        for (int loop = 1; loop <= itMax; loop++) {
            ...
            // diffr is essentially the same here.  C version does not initialize the
            // allocated space for the diffr array and mirror does not copy to the last
            // columns and rows.  Therefore, an incomplete border around the right and
            // bottom of the image differs between these versions.
            ...
            // saveWASHU(diffr, Rx+2, Ry+2, 2*Rz, "mydiffrJ.wu");
            // diffr is essentially the same here.  Difference is in the bottom row for all the image planes


            cmultiply(diffr, halfRx + 1, Ry + 2, OTF, halfRx + 1, Ry + 2,
                      halfRx + 1, Ry, twoRz);
            // saveWASHU(diffr, Rx+2, Ry+2, 2*Rz, "mydiffrJ.wu");
            // diffr is essentially the same here.
            ...
            cmultiplyConjg(diffr, halfRx + 1, Ry + 2, OTF, halfRx + 1, Ry + 2,
                          halfRx + 1, Ry, twoRz);
           ...
           // saveWASHU(diffr, Rx+2, Ry+2, twoRz + 2, "mydiffrJ.wu");
...
//        saveWASHU(result, Rx, Ry, Rz, "myResultJ.wu");
    ...
    void firstOrder(float[] ain, int anx, int any,
                    float[] sol, int snx, int sny,
...
//        saveWASHU(sol, Rx+2, Ry+2, 2*Rz+2, "mydiffrJ.wu");
            ...
            for (i = 2; i <= nx1; i++) {
                for (j = 2; j <= ny1; j++) {
                    for (k = 2; k <= nz1; k++) {
                        ...
                        tmpNew = conb * fTmp + con2 * (-2.0f * tmpDev * fTmp +
                                 conB1 * (sol1 + sol2 + sol3 + sol4 +
                                 ratDel * (sol5 + sol6)));
                        solIdx = (i - 1) + (j - 1) * snx + (k - 1) * splSize;
                        sol[solIdx] = tmpNew;
                        if (tmpNew < tol) continue;
    ...
    void saveWASHU(float[] array, int rx, int ry, int rz, String fileName) {
        ...
        FileWriteOptions fileOptions = new FileWriteOptions(fileName, "C:/Documents and Settings/phemler/My Documents/Research/xcosm/myBuild/currBuild/", true);
...
//      f2 = endarray-iz*pl_size;
    ...
    /**********************************************************************
     AddGamma:
       ...
       Add 1.0 - (plane#)*decay to 'array' over (Inx,Iny,Inz = [lowz..highz])
    **********************************************************************/
    void addGamma(float[] array, int nx1, int ny1,
            int Inx, int Iny, int zstart, int zend) {
    float factor, ftmp;
    ...
    /****************************************************************
     ...
     Perform ratio over nx,ny,(nz = [zstart,zend])

     (set array1 = 1.0 where no ratio calculated)
    ***************************************************************/

    void EMRatio(float[] array1, int nx1, int ny1, int nz1,
                 float[] array2, int nx2, int ny2,
...
*/
    ...
    void setConstant(float[] array, int nx1, int ny1,
...
// array1[idx1] = array1[idx1] * array[idx2]
// dimensions of array2 >= dimensions of array1
   void mult3D(float[] array1, int nx1, int ny1,
               float[] array2, int nx2, int ny2,
   ...
   for(iz = 0; iz < nz; iz++) {
    ...
    void copyArrays(float[] array1, int nx1, int ny1,
                    float[] array2, int array2Idx,
                    int nx2, int ny2, int nx, int ny, int nz) {
    int ix, iy, iz, pl_size1, pl_size2, deltx1, deltx2;
    int idx1, idx2;
...
//              System.out.println("idx1: " + idx1 + "  idx2: " + idx2);
  ...
  float emError2(float[] array1, int nx1, int ny1,
                float[] array2, int array2Idx,
                int nx2, int ny2, int nx, int ny, int nz) {
     int ix, iy, iz, pl_size1, pl_size2, deltx1, deltx2;
     int idx1, idx2;
    ...
    void updateDecay(float tolerance, float imagesum, float[] diffr,
                     int nx1, int ny1, int lowz, int highz,int Inx, int Iny, int Inz) {
    ...
    /**********************************************************************
     WeightedSum ::
       Calculate sum of each plane in array weighted by plane number
       Loop only over [zstart,zend] planes
    **********************************************************************/
    void weightedSum(float[] sumarray, float[] array, int nx1, int ny1,
                     int nx, int ny, int zstart, int zend) {
    ...
    /***************************************************************
     Weighted sum of array:
      ...
      sum = 0
      for each plane
        sum += (plane_num * sum_of_plane)

      array allocated dimensions: nx1,ny1,*
      Perform sum over nx,ny,nz
    ***************************************************************/
    ...
    void cmultiply(float[] array1, int nx1, int ny1, float[] array2, int nx2,
                   int ny2, int nx, int ny, int nz) {
            ...
            for (iy = 0; iy < ny; iy++, offset1 += delt1, offset2 += delt2) {
                for (ix = 0; ix < nx; ix++, offset1 += 2, offset2 += 2) {
                    re = array1[offset1]     * array2[offset2] -
                         array1[offset1 + 1] * array2[offset2 + 1];
                    im = array1[offset1]     * array2[offset2 + 1] +
                         array1[offset1 + 1] * array2[offset2];
                    array1[offset1] = re;
                    array1[offset1 + 1] = im;
                } // end for (ix = 0; ...)
            } // end for (iy = 0; ...)
    ...
    void cmultiplyConjg(float[] array1, int nx1, int ny1, float[] array2, int nx2,
                   int ny2, int nx, int ny, int nz) {
        int pl_size1 = 2 * (nx1 * ny1);
        int pl_size2 = 2 * (nx2 * ny2);
        int delt1 = 2 * (nx1 - nx); // 2 compensates for the fcomplex type in the C code
    ...
    float[] calcOTF(float[] psf, int nx, int ny, int nz, int Rx, int Ry, int Rz) {
        ...
        try {
            ...
            retotf = null;
            errorCleanUp(
                "AlgorithmXcosmEM: Out of memory when creating otf buffer", true);
            return null;
        } // end try{}-catch{}
        ...
        for (int iz = 0; iz < nzo2; iz++) {
            ...
            for (int iy = 0; iy < nyo2; iy++) {
                for (int ix = 0; ix < nxo2; ix++) {
                    ...
                    retotf[o1 + Rx - ix - 1 +
                        iy * (Rx + 2)] = psf[p1 + nx - ix - 1 + iy * nx];

                    // Top plane: lower left
                    retotf[o1 + ix +
                        (Ry - iy - 1) *
                        (Rx + 2)] = psf[p1 + ix + (ny - iy - 1) * nx];

                    // Top plane: lower right
                    retotf[o1 + Rx - ix - 1 +
                        (Ry - iy - 1) *
                        (Rx + 2)] = psf[p1 + nx - ix - 1 + (ny - iy - 1) * nx];
                    ...
                    retotf[o2 + Rx - ix - 1 +
                        iy * (Rx + 2)] = psf[p2 + nx - ix - 1 + iy * nx];

                    // Bottom Plane: lower left
                    retotf[o2 + ix +
                        (Ry - iy - 1) *
                        (Rx + 2)] = psf[p2 + ix + (ny - iy - 1) * nx];

                    // Bottom Plane: lower right
                    retotf[o2 + Rx - ix - 1 +
                        (Ry - iy - 1) *
                        (Rx + 2)] = psf[p2 + nx - ix - 1 + (ny - iy - 1) * nx];
        ...
        /*
         ...
         ModelImage otf = new ModelImage(ModelImage.FLOAT, exts, "mypsf",
                 destImage.getUserInterface());
                          try {
                              otf.importData(0, retotf, true);
                          } catch (IOException error) {
                              retotf = null;
         errorCleanUp("AlgorithmXcosmEM: could NOT import otf image", true);
                              return null;
                          } // end try{}-catch{}

         FileIO fio = new FileIO(destImage.getUserInterface());
         FileWriteOptions fileOptions = new FileWriteOptions("mypsf.wu", "C:/", true);
                          fio.writeImage(otf, fileOptions);
         */
        ...
        /*
         ...
         errorCleanUp("AlgorithmXcosmEM: could NOT import otf image", true);
                ...
                FileIO fio = new FileIO(destImage.getUserInterface());
                FileWriteOptions fileOptions = new FileWriteOptions("myfftJ.wu", "C:/Documents and Settings/phemler/My Documents/Research/xcosm/myBuild/currBuild/", true);
                fio.writeImage(otf, fileOptions);
         */
    ...
    void real_fft3d(float[] xx, int nx, int ny, int nz, int xdim, int ydim,
                    int dir) {
        int halfNx = nx / 2;
        int cx = xdim;
        int cxy = cx * ydim;
            ...
            if (nz != 1) {
                ...
                for (i1 = 0; i1 <= nx; i1 += 2) {
                    for (i2 = 0; i2 < ny; i2++) {
                        ...
                        /*
                                                if (i1 == 0 && i2 == 0) {
                         System.out.println("Before the first IFFT");
                         System.out.println("i2: " + i2 + "  i3: " + i3);
                         for (i3 = 0; i3 < nx; i3 += 2) {
                         System.out.println("wk[" + (i3 / 2) + "]  " +
                         nf.format(work[i3]) + "   " +
                         nf.format(work[(i3 + 1)]));
                                                    } // end for (i3 = 0; ...)
                         } // end if (i1 == 0 && i2 == 0)
...
//                        if (i1 == 0 && i2 == 0) {
                         if (i1 == (nx-4) && i2 == (ny-2)) {
                         System.out.println("After the first IFFT");
                         System.out.println("i1: " + i1 + "  i2: " + i2);
                         for (i3 = 0; i3 < nx; i3 += 2) {
                         System.out.println("wk[" + (i3 / 2) + "]  " +
                         nf.format(work[i3]) + "   " +
...
*/
            ...
            for (i3 = 0; i3 < nz; i3++) {
                for (i2 = 0; i2 < ny; i2++) {
                    ...
                    /*
                                        if (i2 == ny/2 && i3 == nz/2) {
                     System.out.println("Before the third IFFT");
                     System.out.println("i2: " + i2 + "  i3: " + i3);
                                            for (i1 = 0; i1 < nx; i1 += 2) {
                                                System.out.println("wk[" + (i1/2) + "]  " + nf.format(work[i1]) + "   " + nf.format(work[(i1+1)]));
                                            } // end for (i1 = 0; ...)
...
//            System.out.println("work[" + i + "]: " + shiftedwork[i]);
        ...
        while (n > mmax) {
            ...
            for (m = 1; m < mmax; m += 2) {
                for (i = m; i <= n; i += istep) {
                    j = i + mmax;
                    tempr = (float) (wr * shiftedwork[j] - wi * shiftedwork[j +
                                     1]);
                    tempi = (float) (wr * shiftedwork[j + 1] +
                                     wi * shiftedwork[j]);
    ...
    void mirror(float[] in, int nx, int ny, int nz, float[] out, int ox, int oy,
                int oz) {
...
/*
    ...
    private void buildProgressBar() {
        try {
            if (pBarVisible == true) {
                progressBar = new ViewJProgressBar(srcImage.getImageName(),
                    "Xcosm Expectation Maximization ...", 0, 100,
                    true, this, this);
                int xScreen = Toolkit.getDefaultToolkit().getScreenSize().width;
                int yScreen = Toolkit.getDefaultToolkit().getScreenSize().
                              height;
            ...
            Preferences.debug(
                    "AlgorithmXcosmEM: NullPointerException found while building progress bar.", Preferences.DEBUG_ALGORITHM);
        }
