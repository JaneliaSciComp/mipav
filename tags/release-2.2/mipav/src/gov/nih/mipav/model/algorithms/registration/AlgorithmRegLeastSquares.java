...
/**
 *  "Least-Squares Fitting of 2 3-D Point Sets", K. S. Arun
 *	PAMI-9(5), September, 1987 (698-700)
 *
 *  Rotation and translation but not scale. That is, this is a
 *  rigid transformation based on two homologous datasets.
 *
 *  @version 1.0 October 1998
 *  @author  Delia McGarry
 *	idea to use point matching for surgical cases is from the
 Virginia Neurological Institute, Charlottesville, VA
 *	Registers
 *      1. 2 sets of 2D points (number of points in sets should be
 *         equal and greater or equal to 3 and non-colinear)
 *      2. 2 sets of 3D points (number of points in sets should be
 *         equal and greater or equal to 4 and non-colinear)
 *         Software can handle cases which are are coplanar but are
 *         not colinear.
 *
 *  USAGE:
 *	AlgorithmleastSquares LSMatch = new AlgorithmLeastSquares(coordsA, coordsB);
 *	LSMatch.run();
 *	where coordsA = double[3][n] or Point3Dd[n]
 *
 *  USAGE FOR SUBSEQUENT EXECUTIONS:
 *	LSMatch.setup(coordsA, coordsB);
 *	LSMatch.run();
 *
 */
public class AlgorithmRegLeastSquares extends AlgorithmBase {
    ...
    /**
     * Checks if pointSetA is same dimension as pointSetB
     *		sets pointSetA and pointSetB
    ...
    /**
     *   matchBtoA
     *   pA and pB are 2 3D point sets (pointSetA and B)
     *
     *   p1[i]=(1/numCoords)*(sum from j = 0 to j = numCoords - 1 of pB[i][j])
     *   p2[i]=(1/numCoords)*(sum from j = 0 to j = numCoords - 1 of pA[i][j])
     *
     *
