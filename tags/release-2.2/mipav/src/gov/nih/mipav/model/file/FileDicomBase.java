...
/**
*	  FileDICOMBase is an  class that supports the reading/writing of
*     DICOM files. It reads in a buffer of tags that can be parsed more
*     quickly than continued random accesses to the harddrive.
*
...
public class FileDicomBase {
    ...
    /** Integer variable used to read/write in data so that they
    ...
    */
    ...
    public void loadTagBuffer() {
        try {
           bPtr = 0;
           if (raFile != null)  fLength = raFile.length();
    ...
    */
    public final double getDouble(boolean endianess) throws IOException {
        ...
        if (endianess == BIG_ENDIAN) {
            tmpLong =(((long)b1 << 56) | ((long)b2 << 48) |
                      ((long)b3 << 40) | ((long)b4 << 32) |
                      ((long)b5 << 24) | ((long)b6 << 16) |
                      ((long)b7 << 8) | b8);
        }
        else {
            tmpLong =(((long)b8 << 56) | ((long)b7 << 48) |
    ...
    *   @param str     String containing integer data which is broken down in
    *                  bytes and stored in the byte buffer.
