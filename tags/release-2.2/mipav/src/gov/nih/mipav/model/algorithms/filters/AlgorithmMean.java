...
/**
 *	Algorithm to apply a mean filter to an image, placing it in a new ModelImage
 *  if New image is selected or returning the changed picture to the same image if
 *  Replace image is selected.  Define a region including the target pixel and neigbors.
 *  Then the mean is given by the sum of all the pixels in the region divided by
 *  the total number of pixels in the region.  The target regions are either odd
 *  lengthed squares for slice averaging or odd lengthed cubes for volume
 *  multislice averaging.  For 2D imaging only slice averaging can be used
 *  since there is only 1 slice.  For 3D averaging slice averaging is the default,
 *  but slice averaging can be selected with the Apply slice kernel button and
 *  volume averaging can be selected with the Apply volume kernel button. If slice
 *  averaging is selected, a 3 x 3 kernel size is the default, but a 3 x 3, 5 x 5,
 *  7 x 7, 9 x 9, or 11 x 11 size may be selected.  If volume averaging is selected,
 *  a 3 x 3 x 3 kernel size is the default, but a 3 x 3 x 3, 5 x 5 x 5, 7 x 7 x 7,
 *  9 x 9 x 9, or 11 x 11 x 11 kernel size may be selected.  With slice averaging
 *  the time requirement will grow as the square of the kernel length and with
 *  volume averaging the time requirement will grow as the cube of the kernel
 *  length.
 *
 *  Whole image application of the mean is the default, but the user can select
 *  either whole image or voi region(s).  If voi region(s) is selected, then the
 *  mean value of the pixel will replace the original value of the pixel inside
 *  the voi regions(s) and the pixel values outside the voi regions(s) will remain
 *  unchanged.
 *
 *  For color images the default is to obtain the means of the red, green, and
 *  blue channels.  However, if Red Channel, Green Channel, or Blue
 *  Channel is unchecked, then that color channel will pass thru with no mean
 *  averaging performed.
 *
 *  Note the the term average does not always represent mean.  If fact, there are
 *  three types of average, the commonly used mean and median and the infrequently
 *  used mode.  The mean is defined as the sum of the values in a group divided
 *  by the number of values in a group.  The median value is defined as the value
 *  which has the same number of values greater than it and less than it.  The
 *  mode is defined as the value with the greatest number of occurrences.
 *
 *  If part of the neighborhood kernel falls outside the image when the pixel is
 *  on or near the boundary, simply only use that portion of the kernel inside the
 *  image.  Consider, for example, the upper right corner pixel in a 2D image using
 *  a 3 x 3 kernel.  In this case only the target pixel and its lower, left, and
 *  diagonal left low neighbors will be used.  The sum of these 4 pixels will be
 *  divided by 4.
 */
...
public class AlgorithmMean extends AlgorithmBase {
    ...
    *   @param maskFlag     Flag that indicates that the mean filtering will be
    *                       performed for the whole image if equal to true.
    */
	public AlgorithmMean(ModelImage destImg, ModelImage srcImg, int kSize,
    ...
    *                       performed for the whole image if equal to true.
    */
    public AlgorithmMean(ModelImage srcImg, int kSize,
                           boolean sliceBySlice, boolean maskFlag) {
        super(null, srcImg);
	    if ( srcImg.isColorImage() ) {
	            isColorImage = true;
    ...
    */
	...
	/**
	*	RGB images are mean filtered by 'channel.'  That is, each color,
	*   red, blue and green, is filtered independantly of the other two colors.
	*   This mean filter permits selectively filtering any combination of the
    ...
    */
    private void calcStoreInDest3D(){
        ...
        try { resultBuffer = new float[length];}
        catch(OutOfMemoryError e){
            buffer = null;
            resultBuffer = null;
        ...
        try{destImage.importData(0, resultBuffer, true);}
        catch  (IOException e)
        {
            buffer = null;
            resultBuffer = null;
    ...
    */
                ...
                while (initialIndex < 3 && !threadStopped) {   // alpha:0, R:1, G:2, B:3.  But alpha must be copied
                    ...
                    if (numberOfSlices > 1 && pBarVisible == true) {    // 3D image     update progressBar
                        ...
                        progressBar.updateValue(Math.round
                            (( (  (float)(currentSlice)/numberOfSlices)* 100)), activeImage);
                    }
                        ...
                        for (i = initialIndex; i < imageSliceLength && !threadStopped; a+=4, i+=4){
                            if (numberOfSlices == 1) {   // 2D image     update progressBar
                                if ((i-initialIndex)%mod == 0 && pBarVisible == true) {
                                    progressBar.updateValue(Math.round
                                    ( (float)( (initialIndex-1)*sliceLength + i/4)/(3*sliceLength)*100) , activeImage);
                                }
                            }
                ...
                for ( i = 0; i < imageSliceLength && !threadStopped; i++){
                    if (numberOfSlices == 1) {   // 2D image     update progressBar
                        if (i%mod == 0 && pBarVisible == true) {
                            progressBar.updateValue(Math.round
                           ((float) i/(imageSliceLength)*100), activeImage);
                        }
                    }
    ...
    /**
    *   Filter a 3D image with a 3D kernel.  Allows mean filtering to include the picture elements
    ...
    */
    ...
    private void    volumeFilter(float srcBuffer[], float destBuffer[]) {
        ...
        int mod = (imageLength)/100;  // mod is 1 percent of length of slice * the number of slices
            ...
            for ( i = 0; i < imageLength && !threadStopped; i++){
                if (i%mod == 0 && pBarVisible == true) {
                    progressBar.updateValue(Math.round
                    ( (((float) (i)/(imageLength-1))*100)) , activeImage);
                }
    ...
    /**
    *   Filter a Color 3D image with a 3D kernel.  Allows mean filtering to
            ...
            if ((initialIndex==1)&& (!rChannel)) {
                ...
                for (i = initialIndex; i < imageLength; i+=4) {
                    ...
                    for ( i = initialIndex; i < imageLength && !threadStopped; i+=4){
                        if ((i-initialIndex)%mod == 0 && pBarVisible == true) {
                            progressBar.updateValue(Math.round
                            ( (  (float)((initialIndex - 1)*imageSize + i/4)/(3*imageSize)*100)) , activeImage);
                        }
    ...
    */
    private void buildProgressBar(){
        try {
            if (pBarVisible == true) {
                progressBar = new ViewJProgressBar(srcImage.getImageName(), "Filtering image ...",
