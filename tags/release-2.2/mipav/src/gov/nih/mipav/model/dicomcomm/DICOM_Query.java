...
/**
 *  This is the DICOM query class that defines functions to compose and send a patient root,
 *  patient level DICOM query request to the image file server located in NIH's Clinical Center.
 *
...
public class DICOM_Query extends DICOM_SOP implements Runnable {
    ...
    */
    public void setStudyRootQuery()         { UID = DICOM_Constants.UID_StudyRootQuery;           }

    /**
    *   setPatientRootQuery   - sets query to Patient Root
    */
    public void setPatientRootQuery()       { UID = DICOM_Constants.UID_PatientRootQuery;         }

    /**
    *   setPatientStudyOnlyQuery   - sets query to Patient Study Only
    */
    public void setPatientStudyOnlyQuery()  { UID = DICOM_Constants.UID_PatientStudyOnlyQuery;    }


    /**
    *   Sets up the data object (IOD) for a patient root patient level C-Find request
    *   on key patient name.
    ...
    */
    ...
    /**
    *   Sets up the data object (IOD) for a patient root study level C-Find request
    *   on key patient ID.
    ...
    */
    ...
    /**
    *   Sets up the data object (IOD) for a patient root study
    *   level C-Find request on key patient ID.
    *   @param patientID        key attribute for study level query
    *   @param studyID          key attribute for study level query/ can be empty string ""
    *   @return                 return patient root study level query data object
    */
    public DICOM_Object setQueryStudyData(String patientName, String patientID,
                                             String studyID,     String studyDate,
    ...
    /**
    *   Sets up the data object (IOD) for a patient root series
    *   level C-Find request on key patient ID.
    *   @param patientID        key attribute for study level query
    ...
    */
    ...
    /**
    *   Sets up the data object (IOD) for a patient root image
    *                           level C-Find request on key patient ID.
    ...
    */
    public DICOM_PDUService connectToServer(String AETitle) {
        ...
        try                       { pdu.connectClientToServer( AETitle, false ); }
        catch( DICOM_Exception e ) { pdu = null; }
    ...
    */
    public void sendQuery(DICOM_PDUService pdu, DICOM_Object ddo) {
        ...
        if (Preferences.debugLevel(Preferences.DEBUG_COMMS) )
            Preferences.debug( "DICOMQuery.sendQuery: " +
                pdu.findResults.toString("Display query results") + " \n");
        pdu.close();
        if (cancelFlag == false) {
    ...
    */
    public void write(DICOM_PDUService pdu, DICOM_Object ddo) throws DICOM_Exception {
        ...
        while( pdu.readInObject(dco) && keepGoing) {
            ...
            if( dco.getInt16( DICOM_RTC.DD_DataSetType ) == DICOM_Constants.DSTYPE_NODATAPRESENT ) { return; }
