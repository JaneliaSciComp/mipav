...
public class PlugInAlgorithmNCI_LiepinshSeg extends AlgorithmBase {
    ...
    */
    private void calcStoreInDest(){
        ...
        if (srcImage.getNDims() == 5) f = srcImage.getExtents()[4];
        else f = 1;

        if (srcImage.getNDims() >= 4) t = srcImage.getExtents()[3];
        else t = 1;

        if (srcImage.getNDims() >= 3) z = srcImage.getExtents()[2];
        else z = 1;
        int maskCnt = 0;
        ...
        for (m = 0; m < f && !threadStopped; m++) {
            for (k = 0; k < t && !threadStopped; k++) {
                for (j = 0; j < z && !threadStopped; j++) {
                    ...
                    for ( i = 0, id = 0; i < lengthIn && !threadStopped; i += 4, id++){
                        ...
                        if ( mask.get(id) ) {
                            maskCnt++;
                            if (id%mod==0 && isProgressBarVisible()) progressBar.updateValue(
                                        Math.round((float)(i+offsetIn)/(totalLength-1) * 100), activeImage);
                            ...
                            if ( bufferDest[id] != 50 && buffer[i+1] >= buffer[i+2] && buffer[i+1] >= buffer[i+3]) {
                            ...
                            /*
                             if ( bufferDest[id] != 50 && bufferDest[id] != 100 && (buffer[i+1] > 10) &&
                                  (buffer[i+2] > 10) && (buffer[i+3] > 10) ) {
                                //System.out.println(");
                               if ( (Math.abs(buffer[i + 3] - buffer[i + 2]) +
                                     Math.abs(buffer[i + 3] - buffer[i + 1]) +
                                     Math.abs(buffer[i + 2] - buffer[i + 1])) < 50) {

                                   if ( Math.abs(buffer[i + 3] - buffer[i + 2]) < 25 &&
                                        Math.abs(buffer[i + 3] - buffer[i + 1]) < 25 &&
                                        Math.abs(buffer[i + 2] - buffer[i + 1]) < 25) {
                    ...
                    for ( i = xDim ; i < bufferDest.length-xDim && !threadStopped; i++){
                        if (bufferDest[i] != 0) {
                            sum += bufferDest[i];
                            if ( bufferDest[i] == 50 ) counter50++;
                            else if ( bufferDest[i] == 100 ) counter100++;
                        }
                    }
		    ...
		    if (threadStopped) {
                    ...
                    destImage.getUserInterface().getMessageFrame().getData().append("Pixels with blue label  = " + counter50 +
                                              "   total pixels " + maskCnt +
                                              "   percent pixels = " + bluePercent + "\n");
                    destImage.getUserInterface().getMessageFrame().getData().append("Pixels with brown label  = " + counter100 +
                                            "   total pixels " + maskCnt +
                                            "   percent pixels = " + brownPercent + "\n");
