 ...
 *   @see        AlgorithmBase
 *
 *   $Logfile: /mipav/src/plugins/PlugInAlgorithmFISHAnalysis.java $
 *   $Revision: 3 $
 *   $Date: 11/18/05 1:12p $
 *
 */
public class PlugInAlgorithmFISHAnalysis extends AlgorithmBase {
    ...
    private void calc2D(){
        ...
        /**
         * Added by Prabhakar (Reddy) Gudla, NCI/SAIC-Frederick
         * Date: 06/03/2005
         *
         */
        ...
        blueImage = new ModelImage(ModelStorageBase.FLOAT, srcImage.getExtents(),
                srcImage.getImageName() + "_blue",
        ...
        greenImage = new ModelImage(ModelStorageBase.FLOAT, srcImage.getExtents(),
                srcImage.getImageName() + "_green",
                srcImage.getUserInterface());
        ...
        redImage = new ModelImage(ModelStorageBase.FLOAT, srcImage.getExtents(),
                srcImage.getImageName() + "_red",
                srcImage.getUserInterface());
        ...
        blueSegImage[0] = new ModelImage(ModelStorageBase.UBYTE,
                srcImage.getExtents(),
        ...
        endTolerance = 0.01f;
        wholeImage = true;
        fcmAlgo = new AlgorithmFuzzyCMeans(blueSegImage, blueImage, nClasses, nPyramid,
        ...
        greenSegImage[0] = new ModelImage(ModelStorageBase.UBYTE,
                srcImage.getExtents(),
        ...
        endTolerance = 0.01f;
        wholeImage = true;
        fcmAlgo = new AlgorithmFuzzyCMeans(greenSegImage, greenImage, nClasses, nPyramid,
                oneJacobiIter, twoJacobiIter, q,
                oneSmooth, twoSmooth,
                outputGainField, segmentation,
                cropBackground,
        ...
        redSegImage[0] = new ModelImage(ModelStorageBase.UBYTE,
                srcImage.getExtents(),
        ...
        endTolerance = 0.01f;
        wholeImage = true;
        fcmAlgo = new AlgorithmFuzzyCMeans(redSegImage, redImage, nClasses, nPyramid,
                oneJacobiIter, twoJacobiIter, q,
                oneSmooth, twoSmooth,
                outputGainField, segmentation,
                cropBackground,
        ...
        try {
            imageFrameG=
                    new ViewJFrameImage(greenSegImage[0], null,
                    new Dimension(610, 200),
                    srcImage.getUserInterface());
        } catch (OutOfMemoryError error) {
            System.gc();
            UI.setDataText("Out of memory: unable to open new frame");
        }
         */
        /*
        try {
            imageFrameR=
                    new ViewJFrameImage(redSegImage[0], null,
                    new Dimension(610, 200),
                    srcImage.getUserInterface());
        } catch (OutOfMemoryError error) {
            System.gc();
        ...
        fillValue = 0.0f;
        binaryFlag = true;
        thresholdAlgo = new AlgorithmThresholdDual(blueSegImage[0], thresholds,
        ...
        numPruningPixels = 0;
        edgingType = 0;
        openAlgo = new AlgorithmMorphology2D(blueSegImage[0], kernel, circleDiameter,
        ...
        closeAlgo = new AlgorithmMorphology2D(blueSegImage[0], kernel, circleDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
        ...
        itersDilation = 0;
        itersErosion = 0;
        idObjectsAlgo2D = new AlgorithmMorphology2D(blueSegImage[0], kernel, circleDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
        ...
        for (id = 1; id <= numObjects; id++) {
            ...
            dilateAlgo = new AlgorithmMorphology2D(blueSegImage[0], kernel, circleDiameter,
                    method, itersDilation, itersErosion, numPruningPixels,
                    edgingType, wholeImage);
            ...
            Preferences.debug("Center of mass = (" + xCenter[id-1] + ", " + yCenter[id-1]
                    + ")\n");
            Preferences.debug("Distance from center of mass to edge = " +
                    centerToEdge[id-1] + "\n");
        } // for (id = 1; id <= numObjects; id++)
        ...
        for (i = 0; i < nVOIs; i++) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                regionToCenter = (float)Math.sqrt((xPos - xCenter[objectID-1])*
                        (xPos - xCenter[objectID-1])*
                        xRes*xRes +
                        (yPos - yCenter[objectID-1])*
                        (yPos - yCenter[objectID-1])*
                        yRes*yRes);
                lowestSqr = Float.MAX_VALUE;
                for (j = 0, y = 0; y < yDim; y++, j += xDim) {
                ...
                if (redCount >= greenCount) {
                    ...
                    rfCnt++;
                    UI.setDataText("VOI ID = " + i + ", Name = " + VOIs.VOIAt(i).getName() + " with red weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ")\n");
...
//                                "(" + xPos + ", " + yPos + ")\n");
                    ...
                    gfCnt++;
                    UI.setDataText("VOI ID = " + i + ", Name = " + VOIs.VOIAt(i).getName() +  " with green weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ")\n");
//                    UI.setGlobalDataText("VOI ID = " + i + " with green weighted voi center of mass = " +
//                                "(" + xPos + ", " + yPos + ")\n");
                }
                UI.setDataText("Center of cell mass = (" + xCenter[objectID-1] + ", " +
                        yCenter[objectID-1] + ")\n");
...
//                                        regionToCenter + " " + unitsString + "\n");
                    ...
                    UI.setDataText("Distance to center of cell mass = " +
                            regionToCenter +"\n");
//                    UI.setGlobalDataText("Distance to center of cell mass = " +
//                                        regionToCenter + "\n");
                }
...
//                                        regionToEdge + "\n\n");
         ...
         * Date: 06/03/2005
         *
         * Process the binary segmented image (blueSegImage) into a VOI.
         * This is help us get the statistics of the segmented image, i.e. cell nucleus
         */

        /*
        try {
            imageFrame=
                    new ViewJFrameImage(blueSegImage[0], null,
                    new Dimension(610, 200),
                    srcImage.getUserInterface());
        } catch (OutOfMemoryError error) {
            System.gc();
        ...
        if(nVOIs > 1){
            ...
            for(int oCnt=0; oCnt < nVOIs; oCnt++){
                for(int iCnt = oCnt+1; iCnt < nVOIs; iCnt++){
                    distBtwVOIs[distCnt] = (float)Math.sqrt((xPosVOIs[oCnt] - xPosVOIs[iCnt])*(xPosVOIs[oCnt] - xPosVOIs[iCnt])*xRes*xRes +
                            (yPosVOIs[oCnt] - yPosVOIs[iCnt])*(yPosVOIs[oCnt] - yPosVOIs[iCnt])*yRes*yRes);
                    UI.setDataText("\n" + VOIs.VOIAt(oCnt).getName() + "--" + VOIs.VOIAt(iCnt).getName() + " = " + distBtwVOIs[distCnt]);
                    distCnt++;
                }
            }
            UI.setDataText("\n--------------------------------------------------------\n");
        ...
        setCompleted(true);
        /*
        // trying the VOI from AlgorithmMorphology2D
        short ts = 99;
        String tSt = new String("Try_VOI");
        UI.setDataText("T1");
        Polygon tryPolygon = new Polygon();
        tryPolygon = AlgorithmMorphology2D.genContour(blueSegImage[0], xDim, yDim, new Point(10, 20));
        UI.setDataText("T2");
        VOI tryVOI = new VOI(ts, tSt.toString(), 1);
        UI.setDataText("T3");
        tryVOI.setCurveType(VOI.CONTOUR);
        UI.setDataText("T4");
        tryVOI.importPolygon(tryPolygon, 1);
        UI.setDataText("T5");
        tryVOI.setAllActive(true);
        UI.setDataText("T6");
        UI.setDataText("\nArea === " + tryVOI.area());

        //allRGVOIs.add(tryVOI);
        //end trying
        */
    ...
    private void calc3D(){
        ...
        numPruningPixels = 0;
        edgingType = 0;
        openAlgo = new AlgorithmMorphology3D(blueSegImage[0], kernel, sphereDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
        openAlgo.setProgressBarVisible(false);
        openAlgo.run();
        ...
        closeAlgo = new AlgorithmMorphology3D(blueSegImage[0], kernel, sphereDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
        closeAlgo.setProgressBarVisible(false);
        closeAlgo.run();
        ...
        itersDilation = 0;
        itersErosion = 0;
        idObjectsAlgo3D = new AlgorithmMorphology3D(blueSegImage[0], kernel, sphereDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
        ...
        for (id = 1; id <= numObjects; id++) {
            ...
            dilateAlgo = new AlgorithmMorphology3D(blueSegImage[0], kernel, sphereDiameter,
                    method, itersDilation, itersErosion, numPruningPixels,
                    edgingType, wholeImage);
            dilateAlgo.setProgressBarVisible(false);
            dilateAlgo.run();
        ...
        for (i = 0; i < nVOIs; i++) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                regionToEdge = (float)Math.sqrt(lowestSqr);
                if (redCount >= greenCount) {
                    UI.setDataText("VOI ID = " + i + " with red weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ", " + zPos + ")\n");
                    UI.setGlobalDataText("VOI ID = " + i + " with red weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ", " + zPos + ")\n");
                } else {
                    UI.setDataText("VOI ID = " + i + " with green weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ", " + zPos + ")\n");
                    UI.setGlobalDataText("VOI ID = " + i + " with green weighted voi center of mass = " +
                            "(" + xPos + ", " + yPos + ", " + zPos + ")\n");
                }
                UI.setDataText("Center of cell mass = (" + xCenter[objectID-1] + ", " +
                        yCenter[objectID-1] + ", " + zCenter[objectID-1] + ")\n");
                UI.setGlobalDataText("Center of cell mass = (" + xCenter[objectID-1] + ", " +
                        yCenter[objectID-1] + ", " + zCenter[objectID-1] + ")\n");
                if (unitsString != null) {
                    UI.setDataText("Distance to center of cell mass = " + regionToCenter +
    ...
    private int factorial(int n) {
        ...
        else if (n > 10) throw new RuntimeException("Overflow error in factorial");
