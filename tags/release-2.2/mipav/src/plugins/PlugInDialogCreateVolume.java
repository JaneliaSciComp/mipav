    ...
    private void generateNewImages() {
        ...
        try {
            for (int pN = 1; pN <= numPos; pN++ ) { // pn is the position number
                newName = rootName + pN;
                resultImageSet[pN - 1] = new ModelImage(image.getType(), destExtents, newName, image.getUserInterface());
        ...
        try {
            ...
            if (runInSeparateThread) {
                ...
                if (createVolumeAlgo.startMethod(Thread.MIN_PRIORITY) == false) {
                    MipavUtil
                            .displayError("Create Volume reports: A thread is already running on this object [createVolumeAlgo]");
                }
            } else {
