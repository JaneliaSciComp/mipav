 ...
 *   @see        AlgorithmBase
 *
 *   $Logfile: /mipav/src/plugins/PlugInAlgorithmRegionDistance.java $
 *   $Revision: 72 $
 *   $Date: 2/06/06 5:50p $
 * PlugInAlgorithmRegionDistance is used to find the distances between geometric cell centers
 * and cell boundaries, geometric cell centers and voi geometric centers, geometric cell
 * centers and voi centers of mass, voi geometric centers and cell boundaries, and
 * voi centers of mass and cell boundaries.  The geometric center is a zero order moment
 * not using intensity multiplications.  The center of mass if a first order moment mutliplying
 * position by intensity.
 * The cells are blue and the chromosome regions of interest outlined by the vois are predominantly
 * red or green.  Images can be 2D or 3D.  VOIs can be supplied by the user, but if no VOI is
 * supplied by the user the program will automatically generate red VOIs and green VOIs in each cell.
 * The user can input the number of red and green VOIs to be looked for in each cell.  The
 * default is 2 red VOIs and 2 green VOIs.
 *
 *  1.) Only the blue portion of the red, green, blue color image is used to create the blueImage.
 *
 *  2.) Fuzzy C means segmentation is performed on the blueImage to create a blueSegImage, in which
 *      all of the blueImage pixel intensities are assigned to 1 of 2 classes. The lower intensity
 *      value pixels corresponding to the background in blueImage are all given the intensity value
 *      127 in blueSegImage and the higher intensity value pixels in blueImage corresponding to the
 *      nucleus are all given the intensity value 254 in blueSegImage.  In fuzzy c means segmentation
 *      pixels less than the threshold are excluded from the classification process and assigned the
 *      intensity value of 0 in the segmentation image.  Here, threshold is set to 0.0, so no pixels
 *      are less than threshold, so all the pixels in blueImage are assigned to either 127 or 254 in
 *      blueSegImage.  Why 127 and 254?  The fuzzy c means program assigns values of
 *      255/(number of classes) to the classes in the segmented image.  With 2 classes, the resulting
 *      values are 127 and 254.
 *
 *  3.) AlgorithmThresholdDual is used to set all the pixels with intensity = 254 to pixels with
 *      intensity = 1 and all the pixels with intensity = 127 to pixels with intensity = 0.
 *
 *  4.) A slice by slice hole filling is performed on the blue segmented image.
 *
 *  5.) blueSegImage is smoothed with a morphological opening followed by a morphological closing.
 *      An opening consists of an erosion operation followed by a dilation operation.  A closing
 *      consists of a dilation operation followed by an erosion operation.
 *
 *  6.) Erode blueSegImage with iters erosion iterations so as to be able to ID separate cells
 *      which are touching.  iters is a user specified variable with a default of 6.
 *
 *  7.) Assign positive integer IDs to each object between blueMin and 200,000 pixels in size in the
 *      2D program or between blueMin and 2,000,000 pixels in size in the 3D program.  blueSegImage now
 *      has 0 for background and a positive integer ID for each object between the minimum and
 *      maximum size.  blueMin has a default of 1,000.
 *
 *  8.) Dilate blueSegImage with 6*iters dilation operations so as to reverse the previous erosion.
 *      iters is a user specified variable with a default of 8 for 2D and 6 for 3D.
 *
 *  9.) Zero all portions of the dilated objects that are zero in the blue smoothed image from
 *      the end of step 5.
 *
 * 10.) Set all the pixels in any blue object touching an edge to 0.
 *
 * 11.) Export the red portion of the image to buffer and the green portion of the image
 *      to greenBuffer.
 *
 * 12.) If no user supplied contour VOIs are present, the program automatically generates VOIs.
 *
 * 13.) Process each VOI one at a time.
 *   a.) For each VOI find the sum of the red intensity values, the sum of the green
 *       intensity values, and the pixel count for each ID object.
 *   b.) Look at all the object pixel counts and assign the ID of the object with the
 *       most pixels in the VOI to objectID.
 *   c.) Find the center of the VOI.  If redCount >= greenCount, find the red center,
 *       otherwise find the green center.  Both the zero order geometric centers and
 *       the first order centers of mass are found.
 *   d.) Expand the nucleus to which the VOI belongs to include the VOI.
 *
 * 14.) Process IDArray objects one at a time.
 *    a.) Find the xCenter, yCenter, zCenter of each ID object where these centers are
 *        zero order moments or geometric centers.
 *    b.) Create a byteArray with pixels for that ID object equal to 1 and all the other pixels = 0.
 *    c.) Import byteArray into blueSegImage.
 *    d.) Run a dilation on blueSegImage and export the result to dilateArray.
 *    e.) At every point where dilateArray = 1 and byteArray = 0, set boundaryArray = to the id
 *        of that object.  For every point on the boundary find the square of the distance to
 *        the center of the object and if this square distance is the lowest yet noted, put the
 *        value in lowestSquare.  If this square distance is the highest yet noted, put the value
 *        in highestSquare.
 *    f.) After examining all points in the image, the program records the Rmin distance from the center
 *        to edge as the square root of lowestSquare and the Rmax distance from the center to the edge
 *        as the square root of highestSquare.
 *    g.) The program finds the radius that a sphere of the same volume would have using
 *        radius = cube root of (volume/(4/3*PI)).
 * 15.) Determine nucleus-VOI distances
 *   a.) Determine the distance from the center of the ID object to the zero order center
 *       of the VOI.  Find the distance of the line segment that passes thru these 2 points and
 *       terminates at the cell surface. Find the distance from the center of the ID object to the
 *       first order center of the VOI.  Find the distance of the line segment that passes thru
 *       these 2 points and terminates at the cell surface.
 *   b.) For each point in boundaryArray equal to obectID calculate the square of the distance
 *       between the center of the VOI and the current boundary point and if the square distance
 *       is the lowest yet noted, put the value in lowestSquare.  Done for both zero and
 *       first order VOI centers.
 *   c.) After examining all the points in the image, the program records the distance from the
 *       center of the VOI to the edge of the object as the square root of lowestSquare.
 *
 * Scheme for automatic VOI generation:
 *  1.) Create a red image.
 *  2.) Median filter the red image.
 *  3.) Obtain histogram information on the green image.  Set the threshold so that the
 *      greenFraction portion of the cumulative histogram is at or above threshold for
 *      the fuzzy c means.
 *  4.) Perform a 3 level fuzzy c means segmentation on the red image.
 *  5.) Convert the red max to 1 and the other values to 0.
 *  6.) Remove holes from red VOIs.
 *  7.) Smooth red with a morphological opening followed by a closing.
 *  8.) ID objects in red segmented image which have at least redMin pixels.
 *      Split redIDArray objects apart into 2 or 3 redIDArray objects
 *      if they go between 2 or 3 cells.
 *  9.) Create a green image.
 * 10.) Median filter the green image.
 * 11.) Obtain histogram information on the green image.  Set the threshold so that the
 *      greenFraction portion of the cumulative histogram is at or above threshold for
 *      the fuzzy c means.
 * 12.) Perform a 3 level fuzzy c means segmentation on the green image.
 * 13.) Convert the green max to 1 and the other values to 0.
 * 15.) Remove holes from green VOIs.
 * 15.) Smooth green with a morphological opening followed by a closing.
 * 16.) ID objects in green segmented image which have at least greenMin pixels.
 *      Split redIDArray objects apart into 2 or 3 redIDArray objects
 *      if they go between 2 or 3 cells.
 * 17.) Sort red objects by red intensity count into a sorted red array.
 *      Have a separate sorted array for each nucleus.  Put no more than
 *      10 red objects into the sorted red array for each nucleus.
 * 18.) Sort green objects by green intensity count into a sorted green array.
        Have a separate sorted green array for each nucleus.
 * 19.) Create byteBuffer with value = 0 if no sorted object is present at that position
 *      and use a separate positive index for each red or green voi. Create an accompanying
 *      color table to set for each nucleus the largest red voi to color yellow, the other
 *      red vois to a color yellow.darker(), the largest green voi to color pink, and the
 *      other green vois to color pink.darker().  Create an accompanying name table to
 *      set the name for each voi.  The largest red voi in the fifth nucleus has name 5R1.
 *      The second largest red voi in the fifth nucleus has the name 5R2.  The third largest
 *      green voi in the fourth nucleus has the name 4G3.
 * 20.) Extract VOIs from the red_green image.
 * 21.) Set the source image VOIs to the VOIs obtained from the red_green image.
 */
public class PlugInAlgorithmRegionDistance
    ...
    // Iterations used in erosion before IDIng = iters
    // Iterations used in dilation after IDing = 6*iters
    private int iters = 6;
     ...
     *   @param iters
     */
    public PlugInAlgorithmRegionDistance(ModelImage srcImg, int redMin, float redFraction,
                                         int redNumber, int greenMin, float greenFraction,
                                         int greenNumber, int blueMin,
                                         int iters) {
        super(null, srcImg);
        this.redMin = redMin;
    ...
    private void calc2D() {
        ...
        grayImage = new ModelImage(ModelStorageBase.FLOAT, srcImage.getExtents(),
                                   srcImage.getImageName() + "_gray",
        ...
        fcmAlgo = new AlgorithmFuzzyCMeans(grayImage, nClasses, nPyramid,
        ...
        fillValue = 0.0f;
        binaryFlag = true;
        thresholdAlgo = new AlgorithmThresholdDual(grayImage, thresholds,
            fillValue, binaryFlag, wholeImage, true);
        ...
        fillHolesAlgo2D = new AlgorithmMorphology2D(grayImage, 0, 0,
        ...
        numPruningPixels = 0;
        edgingType = 0;
        openAlgo = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
        ...
        closeAlgo = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
                                              method, itersDilation, itersErosion, numPruningPixels,
                                              edgingType, wholeImage);
        ...
        itersDilation = 0;
        itersErosion = iters;
        erosionAlgo = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
                                                method, itersDilation, itersErosion, numPruningPixels,
                                                edgingType, wholeImage);
        ...
        itersDilation = 0;
        itersErosion = 0;
        idObjectsAlgo2D = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
            method, itersDilation, itersErosion, numPruningPixels,
            edgingType, wholeImage);
        ...
        itersErosion = 0;
        dilationAlgo = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
                                                 method, itersDilation, itersErosion, numPruningPixels,
                                                 edgingType, wholeImage);
        ...
        if (nVOIs == 0) {
            ...
            stdDev = 0.0f;
            wholeImage = true;
            algoMedian = new AlgorithmMedian(grayImage, medianIters, kernelSize,
                                             kernelShape, stdDev, wholeImage);
            ...
            wholeImage = true;
            // grayImage enters ModelStorageBase.FLOAT and returns ModelStorageBase.UBYTE
            fcmAlgo = new AlgorithmFuzzyCMeans(grayImage, nClasses,
                                               nPyramid,
                                               oneJacobiIter, twoJacobiIter, q,
                                               oneSmooth, twoSmooth,
                                               outputGainField, segmentation,
                                               cropBackground,
                                               threshold, maxIter, endTolerance,
                                               wholeImage);
            ...
            numPruningPixels = 0;
            edgingType = 0;
            openAlgo = new AlgorithmMorphology2D(grayImage, kernel,
                                                 circleDiameter,
                                                 method, itersDilation, itersErosion,
                                                 numPruningPixels,
                                                 edgingType, wholeImage);
            openAlgo.setProgressBarVisible(false);
            openAlgo.run();
            ...
            method = AlgorithmMorphology2D.CLOSE;
            closeAlgo = new AlgorithmMorphology2D(grayImage, kernel,
                                                  circleDiameter,
                                                  method, itersDilation, itersErosion,
                                                  numPruningPixels,
                                                  edgingType, wholeImage);
            ...
            itersDilation = 0;
            itersErosion = 0;
            idObjectsAlgo2D = new AlgorithmMorphology2D(grayImage, kernel,
                circleDiameter,
                method, itersDilation,
                itersErosion, numPruningPixels,
                edgingType, wholeImage);
                    ...
                    if (secondMaxCount >= redMin) {
                        ...
                        if (thirdMaxCount > 0) {
                            if (thirdMaxCount >= redMin) {
                                ...
                                for (j = 0; j < length; j++) {
                                    if ((redIDArray[j] == i) && (IDArray[j] != 0) &&
                                        (IDArray[j] != maxNucleus) && (IDArray[j] != secondMaxNucleus)) {
                                        redIDArray[j] = (byte)0;
                                    }
                                } // for (j = 0; j < totLength; j++)
                            } // else thirdMaxCount < redMin
                        ...
                        if (secondMaxCount >= greenMin) {
                            ...
                            if (thirdMaxCount > 0) {
                                if (thirdMaxCount >= greenMin) {
                                    ...
                                    for (j = 0; j < length; j++) {
                                        if ((greenIDArray[j] == i) && (IDArray[j] != 0) &&
                                            (IDArray[j] != maxNucleus) && (IDArray[j] != secondMaxNucleus)) {
        ...
        for (i = 0; i < nVOIs; i++) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                newPtVOI = new VOI( (short) (i + nVOIs), "point2D" + i + ".voi", 1,
                                   VOI.POINT, -1.0f);
                ...
                ( (VOIPoint) (newPtVOI.getCurves()[0].elementAt(0))).
                    setFixed(true);
                ((VOIPoint) (newPtVOI.getCurves()[0].elementAt(0))).
                    setLabel(voiName);
        ...
        for (id = 1; id <= numObjects; id++) {
            progressBar.setMessage("Processing object " + id + " of " + numObjects +
                                   " in blue segmented image");
            if (nVOIs > 0) {
            ...
            dilateAlgo = new AlgorithmMorphology2D(grayImage, kernel, circleDiameter,
                method, itersDilation, itersErosion, numPruningPixels,
                edgingType, wholeImage);
            ...
            Preferences.debug("Center of mass = (" + xCenter[id - 1] + ", " + yCenter[id - 1]
                              + ")\n");
            Preferences.debug("Distance from center of mass to near edge = " +
                              centerToNearEdge[id - 1] + "\n");
            Preferences.debug("Distance from center of mass to far edge = " +
                              centerToFarEdge[id - 1] + "\n");
        } // for (id = 1; id <= numObjects; id++)
        ...
        for (i = 0; i < numObjects; i++) {
            newPtVOI = new VOI( (short) (i + 2 * nVOIs), "point2D" + i + ".voi", 1,
                               VOI.POINT, -1.0f);
        ...
        if (xUnits != FileInfoBase.UNKNOWN_MEASURE) {
            UI.setDataText("X resolution = " + xRes + "  " +
                           FileInfoBase.getUnitsOfMeasureStr(xUnits) + "\n");
        }
        else {
            UI.setDataText("X resolution = " + xRes + "\n");
        }
        if (yUnits != FileInfoBase.UNKNOWN_MEASURE) {
            UI.setDataText("Y resolution = " + yRes + "  " +
                           FileInfoBase.getUnitsOfMeasureStr(yUnits) + "\n\n");
        }
        else {
            UI.setDataText("Y resolution = " + yRes + "\n\n");
        }
        ...
        for (i = 0; i < numObjects; i++) {
            ...
            for (j = 0; j <= nVOIs - 1; j++) {
                if (objectID[j] == (i+1)) {
                    ...
                    geoToCenter = (float) Math.sqrt( (xPosGeo[j] - xCenter[i]) *
                                             (xPosGeo[j] - xCenter[i]) * xRes * xRes +
                                             (yPosGeo[j] - yCenter[i]) *
                                             (yPosGeo[j] - yCenter[i]) * yRes * yRes);
                    ...
                    centerToGeoToEdge = (float) Math.sqrt( (lastEdgeX - xCenter[i]) *
                                             (lastEdgeX - xCenter[i]) * xRes * xRes +
                                             (lastEdgeY - yCenter[i]) *
                                             (lastEdgeY - yCenter[i]) * yRes * yRes);
                    ...
                    geoToEdge = (float) Math.sqrt((lastEdgeX - xPosGeo[j]) *
                                    (lastEdgeX - xPosGeo[j]) * xRes * xRes +
                                    (lastEdgeY - yPosGeo[j]) *
                                    (lastEdgeY - yPosGeo[j]) * yRes * yRes);
                    UI.setDataText("\t" + nf.format(geoToEdge));


                    gravToCenter = (float) Math.sqrt( (xPosGrav[j] - xCenter[i]) *
                                             (xPosGrav[j] - xCenter[i]) * xRes * xRes +
                                             (yPosGrav[j] - yCenter[i]) *
                                             (yPosGrav[j] - yCenter[i]) * yRes * yRes);
                    ...
                    centerToGravToEdge = (float) Math.sqrt( (lastEdgeX - xCenter[i]) *
                                             (lastEdgeX - xCenter[i]) * xRes * xRes +
                                             (lastEdgeY - yCenter[i]) *
                                             (lastEdgeY - yCenter[i]) * yRes * yRes);
                    ...
                    gravToEdge = (float) Math.sqrt((lastEdgeX - xPosGrav[j]) *
                                    (lastEdgeX - xPosGrav[j]) * xRes * xRes +
                                    (lastEdgeY - yPosGrav[j]) *
                                    (lastEdgeY - yPosGrav[j]) * yRes * yRes);
                    UI.setDataText("\t" + nf.format(gravToEdge));


                    /*lowestSqr = Float.MAX_VALUE;
                    for (j1 = 0, y = 0; y < yDim; y++, j1 += xDim) {
                        for (x = 0; x < xDim; x++) {
                            index = x + j1;
                            if (boundaryArray[index] == objectID[j]) {
                                distSqr = (xPosGeo[j] - x) * (xPosGeo[j] - x) *
                                          xRes * xRes +
                                          (yPosGeo[j] - y) * (yPosGeo[j] - y) *
                                          yRes * yRes;
                                if (distSqr < lowestSqr) {
                                    lowestSqr = distSqr;
                                    xEdge = x;
                    ...
                    geoToEdge = (float) Math.sqrt(lowestSqr);
                    UI.setDataText("\t\t" + nf.format(geoToEdge));
                    geoToEdge *= 100.0f /centerToFarEdge[i];
                    UI.setDataText("\t" + nf.format(geoToEdge));
                    ...
                    for (j1 = 0, y = 0; y < yDim; y++, j1 += xDim) {
                        for (x = 0; x < xDim; x++) {
                            index = x + j1;
                            if (boundaryArray[index] == objectID[j]) {
                                distSqr = (xPosGrav[j] - x) * (xPosGrav[j] - x) *
                                          xRes * xRes +
                                          (yPosGrav[j] - y) * (yPosGrav[j] - y) *
                                          yRes * yRes;
                                if (distSqr < lowestSqr) {
                                    lowestSqr = distSqr;
                                    xEdge = x;
                    ...
                    gravToEdge = (float) Math.sqrt(lowestSqr);
                    UI.setDataText("\t\t" + nf.format(gravToEdge));
                    gravToEdge *= 100.0f /centerToFarEdge[i];
                    UI.setDataText("\t" + nf.format(gravToEdge));*/
                } // if (objectID[j] == (i+1))
            } // for (j = 0; j <= nVOIs - 1; j++)
            UI.setDataText("\n");
    ...
    private void calc3D() {
        ...
        numPruningPixels = 0;
        edgingType = 0;
        openAlgo = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
                                             method, itersDilation, itersErosion, numPruningPixels,
                                             edgingType, wholeImage);
        openAlgo.setProgressBarVisible(false);
        openAlgo.run();
        ...
        closeAlgo = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
                                              method, itersDilation, itersErosion, numPruningPixels,
                                              edgingType, wholeImage);
        closeAlgo.setProgressBarVisible(false);
        closeAlgo.run();
        ...
        itersDilation = 0;
        itersErosion = iters;
        erosionAlgo = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
                                                method, itersDilation, itersErosion, numPruningPixels,
                                                edgingType, wholeImage);
        erosionAlgo.setProgressBarVisible(false);
        erosionAlgo.run();
        ...
        itersDilation = 0;
        itersErosion = 0;
        idObjectsAlgo3D = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
            method, itersDilation, itersErosion, numPruningPixels,
            edgingType, wholeImage);
        ...
        itersDilation = 6*iters;
        itersErosion = 0;
        dilationAlgo = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
                                                 method, itersDilation, itersErosion, numPruningPixels,
                                                 edgingType, wholeImage);
        dilationAlgo.setProgressBarVisible(false);
        dilationAlgo.run();
        ...
        if (nVOIs == 0) {
            if (redNumber > 0) {
                ...
                wholeImage = true;
                algoMedian = new AlgorithmMedian(grayImage, medianIters, kernelSize,
                                                 kernelShape, stdDev, sliceBySlice,
                                                 wholeImage);
                algoMedian.setProgressBarVisible(false);
                algoMedian.run();
                algoMedian.finalize();
                ...
                progressBar.setMessage("Removing holes from red VOIs");
                progressBar.updateValueImmed(37);
                fillHolesAlgo3D = new AlgorithmMorphology3D(grayImage, 0, 0,
                        AlgorithmMorphology3D.FILL_HOLES,
                        0, 0, 0, 0, wholeImage);
                ...
                numPruningPixels = 0;
                edgingType = 0;
                openAlgo = new AlgorithmMorphology3D(grayImage, kernel,
                        sphereDiameter,
                        method, itersDilation, itersErosion,
                        numPruningPixels,
                        edgingType, wholeImage);
                ...
                progressBar.updateValueImmed(42);
                method = AlgorithmMorphology3D.CLOSE;
                closeAlgo = new AlgorithmMorphology3D(grayImage, kernel,
                        sphereDiameter,
                        method, itersDilation, itersErosion,
                        numPruningPixels,
                        edgingType, wholeImage);
                ...
                itersDilation = 0;
                itersErosion = 0;
                idObjectsAlgo3D = new AlgorithmMorphology3D(grayImage, kernel,
                        sphereDiameter,
                        method, itersDilation,
                        itersErosion, numPruningPixels,
                        edgingType, wholeImage);
            ...
            if (greenNumber > 0) {
                ...
                for (j = 1; j <= numGreenObjects; j++) {
                    progressBar.setMessage("Sorting green object " + j +
                                           " of " + numGreenObjects + " by intensity count");
        ...
        for (i = 0; i < nVOIs; i++) {
            ...
            if (VOIs.VOIAt(i).getCurveType() == VOI.CONTOUR) {
                ...
                newPtVOI = new VOI( (short) (i + nVOIs), "point3D" + i + ".voi", zDim,
                                   VOI.POINT, -1.0f);
                newPtVOI.setColor(Color.white);
                xArr[0] = xPosGrav[i];
                yArr[0] = yPosGrav[i];
                ...
                ( (VOIPoint) (newPtVOI.getCurves()[ (int) zPosGrav[i]].elementAt(0))).
                    setFixed(true);
                ( (VOIPoint) (newPtVOI.getCurves()[ (int) zPosGrav[i]].elementAt(0))).
                    setLabel(voiName);
                srcImage.registerVOI(newPtVOI);
        ...
        for (id = 1; id <= numObjects; id++) {
            ...
            dilateAlgo = new AlgorithmMorphology3D(grayImage, kernel, sphereDiameter,
                    method, itersDilation, itersErosion, numPruningPixels,
                    edgingType, wholeImage);
            dilateAlgo.setProgressBarVisible(false);
            dilateAlgo.run();
        ...
        for (i = 0; i < numObjects; i++) {
            newPtVOI = new VOI( (short) (i + 2 * nVOIs), "point3D" + i + ".voi", zDim,
                               VOI.POINT, -1.0f);
            newPtVOI.setColor(Color.magenta);
            xArr[0] = xCenter[i];
            yArr[0] = yCenter[i];
            ...
            ( (VOIPoint) (newPtVOI.getCurves()[ (int) zArr[0]].elementAt(0))).
                setFixed(true);
            ( (VOIPoint) (newPtVOI.getCurves()[ (int) zArr[0]].elementAt(0))).
                setLabel("N" + (i+1));
            srcImage.registerVOI(newPtVOI);
        ...
        if (zUnits != FileInfoBase.UNKNOWN_MEASURE) {
            UI.setDataText("Z resolution = " + zRes + "  " +
                           FileInfoBase.getUnitsOfMeasureStr(zUnits) + "\n\n");
        }
        else {
            UI.setDataText("Z resolution = " + zRes + "\n\n");
        }
        ...
        for (i = 0; i < numObjects; i++) {
            ...
            for (j = 0; j <= nVOIs - 1; j++) {
                if (objectID[j] == (i+1)) {
                    ...
                    if ((Math.abs(initialdx) >= Math.abs(initialdy)) &&
                        (Math.abs(initialdx) >= Math.abs(initialdz))) {
                        if (initialdx >= 0.0f) {
                            incx = 0.1f;
                        }
                        else {
                    ...
                    gravToCenter = (float) Math.sqrt( (xPosGrav[j] - xCenter[i]) *
                                             ...
                                             (yPosGrav[j] - yCenter[i]) *
                                             (yPosGrav[j] - yCenter[i]) * yRes * yRes +
                                             (zPosGrav[j] - zCenter[i]) *
                                             (zPosGrav[j] - zCenter[i]) * zRes * zRes);
                    ...
                    gravToEdge = (float) Math.sqrt((lastEdgeX - xPosGrav[j]) *
                                    ...
                                    (lastEdgeY - yPosGrav[j]) *
                                    (lastEdgeY - yPosGrav[j]) * yRes * yRes +
                                    (lastEdgeZ - zPosGrav[j]) *
                                    (lastEdgeZ - zPosGrav[j]) * zRes * zRes);
                    UI.setDataText("\t" + nf.format(gravToEdge));


                    /*lowestSqr = Float.MAX_VALUE;
                    for (k1 = 0, z = 0; z < zDim; z++, k1 += sliceLength) {
                        for (j1 = k1, y = 0; y < yDim; y++, j1 += xDim) {
                            for (x = 0; x < xDim; x++) {
                                index = x + j1;
                                if (boundaryArray[index] == objectID[j]) {
                                    distSqr = (xPosGeo[j] - x) * (xPosGeo[j] - x) *
        ...
        System.out.println("VOIs = " + VOIs);
        for (i = 0; i < nVOIs; i++) {
            System.out.println("id = " + VOIs.VOIAt(i).getID());
            System.out.println(VOIs.VOIAt(i).getName());
