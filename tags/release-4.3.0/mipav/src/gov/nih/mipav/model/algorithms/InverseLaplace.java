 ...
 * pp. 357-366.</p>
 *
 * <p>Hollenbeck algorithm modification: The time vector is split in segements of equal magnitude which are inverted
 * individually. This gives better overall accuracy.</p>
 */
 ...
 * InverseLaplace with an analytically known inverse. The // function to be transformed is exp(a*t) // parameter a in
 * exp(a*t) double a = 1.0; // An estimate for the maximum of the real parts of the singularities // of F. If unknown,
 ...
 * double[n]; // true value of the function exp(a*t) double ftrue[] = new double[n]; int i; FitExpModel lmod; double
 * timeFunction[] = null; for (i = 1; i <= n; i++) { t[i-1] = Math.pow(10.0,0.5*(double)i - 13.0); ftrue[i-1] =
 * Math.exp(a*t[i-1]); } lmod = new FitExpModel(t, largestPole, tol); lmod.driver(); timeFunction =
 ...
 * double [imagS.length][2]; double a = 1.0; int i; double denom; double realDenom; realDenom = (realS - a) * (realS -
 * a); for (i = 0; i < imagS.length; i++) { denom = realDenom + imagS[i] * imagS[i]; // real part ans[i][0] = (realS -
        ...
        try {
            ...
            for (iLogT = iMinLogT; iLogT <= iMaxLogT; iLogT++) {
                ...
                if (segPoints > 0) {
                    ...
                    for (i = 0; i < segPoints; i++) {
                        realH2M = .5 *
                                      (1.0 + ((realD[(2 * M) - 1] - realD[2 * M]) * realZ[i]) -
                                           ((imagD[(2 * M) - 1] - imagD[2 * M]) * imagZ[i]));
                        imagH2M = .5 *
                                      (((realD[(2 * M) - 1] - realD[2 * M]) * imagZ[i]) +
