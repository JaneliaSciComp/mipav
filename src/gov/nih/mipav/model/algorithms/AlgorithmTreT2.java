...
public class AlgorithmTreT2 extends AlgorithmTProcess {
	...
	/*
	private final int[] ssfpImageIndex_phase0;

	private final int[] ssfpImageIndex_phase180;

	private final int t1ImageIndex;

	private final int b1ImageIndex;
	*/
	...
	/*
	public AlgorithmTreT2(final JDialogTreT2 dialog, final double[] treFAPhase0, final double[] treFAPhase180, final int[] ssfpImageIndexPhase0,
			final int[] ssfpImageIndexPhase180, final int t1ImageIndex, final int b1ImageIndex, final double[] simplexLineValues,
			final double[] simplexResiduals, final double[] simplexCentre, final double[] reflection, final double[] expansion, final double[] contraction,
			final double[] shrink, final double[][] simplex, final double[] twoPSimplexLineValues, final double[] twoPSimplexResiduals,
			final double[] twoPSimplexCentre, final double[] twoPReflection, final double[] twoPExpansion, final double[] twoPContraction,
			final double[] twoPShrink, final double[][] twoPSimplex, final int[] bestToWorst, final String[] wList) {
		super();
		this.dialog = dialog;
	...
	*/
	...
	public AlgorithmTreT2(final JDialogTreT2 dialog, final double[] treFAPhase0, final double[] treFAPhase180, final String[] ssfpImageNamesPhase0,
    final String[] ssfpImageNamesPhase180, final String t1ImageName, final String b1ImageName, final double[] simplexLineValues,
    final double[] simplexResiduals, final double[] simplexCentre, final double[] reflection, final double[] expansion, final double[] contraction,
    final double[] shrink, final double[][] simplex, final double[] twoPSimplexLineValues, final double[] twoPSimplexResiduals,
    final double[] twoPSimplexCentre, final double[] twoPReflection, final double[] twoPExpansion, final double[] twoPContraction,
    final double[] twoPShrink, final double[][] twoPSimplex, final int[] bestToWorst, final String[] wList) {
		super();
		this.dialog = dialog;
		this.treFA_phase0 = treFAPhase0;
		this.treFA_phase180 = treFAPhase180;
		this.ssfpImageNames_phase0 = ssfpImageNamesPhase0;
		this.ssfpImageNames_phase180 = ssfpImageNamesPhase180;
		this.t1ImageName = t1ImageName;
		this.b1ImageName = b1ImageName;
		this.simplexLineValues = simplexLineValues;
		this.simplexResiduals = simplexResiduals;
		this.simplexCentre = simplexCentre;
		this.reflection = reflection;
	...
	public AlgorithmTreT2(final ModelImage destImage, final ModelImage srcImage, final JDialogTreT2 dialog, final double[] treFAPhase0,
			final double[] treFAPhase180, final String[] ssfpImageNamesPhase0, final String[] ssfpImageNamesPhase180, final String t1ImageName, 
			final String b1ImageName, final double[] simplexLineValues, final double[] simplexResiduals, final double[] simplexCentre, final double[] reflection,
			final double[] expansion, final double[] contraction, final double[] shrink, final double[][] simplex, final double[] twoPSimplexLineValues,
			final double[] twoPSimplexResiduals, final double[] twoPSimplexCentre, final double[] twoPReflection, final double[] twoPExpansion,
			final double[] twoPContraction, final double[] twoPShrink, final double[][] twoPSimplex, final int[] bestToWorst, final String[] wList) {
	...
	public void runAlgorithm() {
		if (dialog.isPerformFullModeling() == true || dialog.isPerformApproxModeling() == true) {
			if (dialog.isPerformFullModeling() == true) {
				calculateT2withFullModeling();
			} else {
				calculateT2withApproximateModeling();
			}
		} else {
			if (dialog.isPerformConventionalWith0Phase() == true) {
	...
	public void calculateT2with0Phase() {
		...
		image = ViewUserInterface.getReference().getRegisteredImageByName(ssfpImageNames_phase0[0]);
		...
		for (angle = 0; angle < dialog.getNfa_phase0(); angle++) {
			image = ViewUserInterface.getReference().getRegisteredImageByName(ssfpImageNames_phase0[angle]);
		...
		for (t = 0; t < tSeries; t++) {
			...
			for (k = 0; k < nSlices; k++) {
				...
				image = ViewUserInterface.getReference().getRegisteredImageByName(t1ImageName);
				...
				if (dialog.isIncludeB1Map() == true) {
					image = ViewUserInterface.getReference().getRegisteredImageByName(b1ImageName);
				...
				for (x = 0; x < width; x++) {
					for (y = 0; y < height; y++) {
						...
						if (t1PixelValues[pixelIndex] > 0.00) {
							...
							for (p = 0; p < dialog.getNfa_phase0(); p++) {
								phase0Data[p] = ssfpPixelValues_phase0[p][pixelIndex];
							}
				...
				try {
					final int startVal = image.getSliceSize() * nSlices * t + image.getSliceSize() * k;
					if (dialog.isCalculateT2()) {
	...
	public void calculateT2with180Phase() {
		...
		image = ViewUserInterface.getReference().getRegisteredImageByName(ssfpImageNames_phase180[0]);
		width = image.getExtents()[0];
		height = image.getExtents()[1];
		if (image.getNDims() > 2) {
			nSlices = image.getExtents()[2];
		...
		for (angle = 0; angle < dialog.getNfa_phase180(); angle++) {
			image = ViewUserInterface.getReference().getRegisteredImageByName(ssfpImageNames_phase180[angle]);
			if (image.getNDims() > 3 && !do4D) { // clause is only entered once
				do4D = true;
				tSeries = image.getExtents()[3];
				t2ResultStack = new ModelImage(ModelImage.DOUBLE, image.getExtents(), "t2_results");
	...
	public void calculateT2withFullModeling() {
		...
		for (t = 0; t < tSeries; t++) {
			...
			for (k = 0; k < nSlices; k++) {
				...
				// now that we have all the information, perform the calculate the initial estimates of B1, T2 and M0
				// pixel-wise
				pixelIndex = 0;
				...
				for (y = 0; y < height; y++) {
					for (x = 0; x < width; x++) {
						...
						if (t1 > 10.00) {
							/*
							 * if (t1 > 2500) { b0Values[k-1][y][x] = 0.00; m0Values[k-1][y][x] = maxM0;
							 * t2Values[k-1][y][x] = maxT2; r2Values[k-1][y][x] = 1.00/maxT2;
							 * 
							 * }
							 * 
							 * else {
							...
							threePDownHillSimplex(optimization, initialGuess, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements, dialog.getNfa_phase0()
									+ dialog.getNfa_phase180());
			...
			for (k = 0; k < nSlices; k++) {
				...
				fireProgressStateChanged(40 + (int) ( (k + 1.0) / nSlices * 20.0));
				if (interrupted()) {
					hardInterrupt = true;
					return;
				}
				...
				for (y = 0; y < height; y++) {
					for (x = 0; x < width; x++) {
						...
						if (y > 2 && y < height - 2 && x > 2 && x < width - 2) {
							...
							for (p1 = 0; p1 < 5; p1++) {
								for (p2 = 0; p2 < 5; p2++) {
									smoothedB0 += b0Values[k][y - 2 + p2][x - 2 + p1] * Gaussian[p1][p2];
								}
							}
							...
							b0Field[k][pixelIndex] = (float) b0Values[k][y][x];
						}
			...
			for (k = 0; k < nSlices; k++) {
				...
				fireProgressStateChanged(60 + (int) ( (k + 1.0) / nSlices * 15.0));
				if (interrupted()) {
					hardInterrupt = true;
					return;
				}
				...
				for (y = 0; y < height; y++) {
					for (x = 0; x < width; x++) {
						...
						if (b0Field[k][pixelIndex] > 0.00) {
							...
							twoPDownHillSimplex(twoPOptimization, twoPInitialGuess, b0, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements,
									dialog.getNfa_phase0() + dialog.getNfa_phase180());
	...
	public void twoPDownHillSimplex(final double[] optimization, final double[] initialGuess, final double b0, final double t1, final double tr,
			final double[] ssfpSampleData, final double[] sina, final double[] cosa, final double[] phaseIncrements, final int N) {
		...
		for (i = 0; i < numParams; i++) {
			twoPSimplex[0][i] = twoPSimplexLineValues[i];
		}
		...
		for (i = 0; i < numParams; i++) {
			...
			if (twoPSimplexLineValues[i] != 0.00) {
				twoPSimplexLineValues[i] = (1.00 + usual_delta) * twoPSimplexLineValues[i];
			} else {
				twoPSimplexLineValues[i] = zero_term_delta;
			}
			residual = calculateTwoPResiduals(twoPSimplexLineValues, b0, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements, N);
		...
		for (i = 0; i < numVertices; i++) {
			twoPSimplexResiduals[i] = twoPSimplex[i][numParams];
		}
		...
		while (rtol > maxError && iterations < NMAX) {
			...
			for (i = 0; i < numParams; i++) {
				twoPSimplexCentre[i] = 0.00;
			}
			for (i = 0; i < numVertices; i++) {
				if (i != bestToWorst[numParams - 1]) {
					for (j = 0; j < numParams; j++) {
						twoPSimplexCentre[j] += twoPSimplex[i][j];
					}
				}
			}
			for (i = 0; i < numParams; i++) {
				twoPSimplexCentre[i] = twoPSimplexCentre[i] / numParams;
			}

			// reflect the twoPSimplex through the face of the high point
			// for (i=0; i<numVertices; i++) twoPReflection[i] = 0.00;
			for (i = 0; i < numParams; i++) {
				twoPReflection[i] = (1.00 + RHO) * twoPSimplexCentre[i] - RHO * twoPSimplex[worst][i];
			}
			...
			if (twoPReflection[numParams] < twoPSimplex[best][numParams]) {
				...
				for (i = 0; i < numParams; i++) {
					twoPExpansion[i] = (1.00 + RHO * CHI) * twoPSimplexCentre[i] - RHO * CHI * twoPSimplex[worst][i];
				}
				...
				if (twoPExpansion[numParams] < twoPReflection[numParams]) {
					for (i = 0; i < numVertices; i++) {
						twoPSimplex[worst][i] = twoPExpansion[i];
					}
				} else {
					for (i = 0; i < numVertices; i++) {
						twoPSimplex[worst][i] = twoPReflection[i];
					}
				}
			} else {
				if (twoPReflection[numParams] < twoPSimplex[secondWorst][numParams]) {
					...
					if (twoPReflection[numParams] < twoPSimplex[worst][numParams]) {
						...
						for (i = 0; i < numParams; i++) {
							twoPContraction[i] = (1.00 + PSI * RHO) * twoPSimplexCentre[i] - RHO * PSI * twoPSimplex[worst][i];
						}
						...
						if (twoPContraction[numParams] <= twoPReflection[numParams]) {
							for (i = 0; i < numVertices; i++) {
								twoPSimplex[worst][i] = twoPContraction[i];
							}
						} else {
						...
						// for (i=0; i<numVertices; i++) twoPContraction[i] = 0.00;
						for (i = 0; i < numParams; i++) {
							twoPContraction[i] = (1.00 - PSI) * twoPSimplexCentre[i] + PSI * twoPSimplex[worst][i];
						}
						twoPContraction[numParams] = calculateTwoPResiduals(twoPContraction, b0, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements, N);
			...
			for (i = 0; i < numVertices; i++) {
				twoPSimplexResiduals[i] = twoPSimplex[i][numVertices - 1];
			}
			calculateBestToWorst(twoPSimplexResiduals, bestToWorst, numVertices);
			best = bestToWorst[0];
			worst = bestToWorst[2];
	...
	public void threePDownHillSimplex(final double[] optimization, final double[] initialGuess, final double t1, final double tr,
			final double[] ssfpSampleData, final double[] sina, final double[] cosa, final double[] phaseIncrements, final int N) {
		...
		for (i = 0; i < numParams; i++) {
			simplex[0][i] = simplexLineValues[i];
		}
		...
		for (i = 0; i < numParams; i++) {
			...
			if (simplexLineValues[i] != 0.00) {
				simplexLineValues[i] = (1.00 + usual_delta) * simplexLineValues[i];
			} else {
				simplexLineValues[i] = zero_term_delta;
			}
			residual = calculateResiduals(simplexLineValues, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements, N);
		...
		for (i = 0; i < numVertices; i++) {
			simplexResiduals[i] = simplex[i][numParams];
		}
		...
		while (rtol > maxError && iterations < NMAX) {
			...
			for (i = 0; i < numParams; i++) {
				simplexCentre[i] = 0.00;
			}
			for (i = 0; i < numVertices; i++) {
				if (i != bestToWorst[numParams - 1]) {
					for (j = 0; j < numParams; j++) {
						simplexCentre[j] += simplex[i][j];
					}
				}
			}
			for (i = 0; i < numParams; i++) {
				simplexCentre[i] = simplexCentre[i] / numParams;
			}

			// reflect the simplex through the face of the high point
			// for (i=0; i<numVertices; i++) reflection[i] = 0.00;
			for (i = 0; i < numParams; i++) {
				reflection[i] = (1.00 + RHO) * simplexCentre[i] - RHO * simplex[worst][i];
			}
			...
			if (reflection[numParams] < simplex[best][numParams]) {
				...
				for (i = 0; i < numParams; i++) {
					expansion[i] = (1.00 + RHO * CHI) * simplexCentre[i] - RHO * CHI * simplex[worst][i];
				}
				...
				if (expansion[numParams] < reflection[numParams]) {
					for (i = 0; i < numVertices; i++) {
						simplex[worst][i] = expansion[i];
					}
				} else {
					for (i = 0; i < numVertices; i++) {
						simplex[worst][i] = reflection[i];
					}
				}
			} else {
				if (reflection[numParams] < simplex[secondWorst][numParams]) {
					...
					if (reflection[numParams] < simplex[worst][numParams]) {
						...
						for (i = 0; i < numParams; i++) {
							contraction[i] = (1.00 + PSI * RHO) * simplexCentre[i] - RHO * PSI * simplex[worst][i];
						}
						...
						// for (i=0; i<numVertices; i++) contraction[i] = 0.00;
						for (i = 0; i < numParams; i++) {
							contraction[i] = (1.00 - PSI) * simplexCentre[i] + PSI * simplex[worst][i];
						}
						contraction[numParams] = calculateResiduals(contraction, t1, tr, ssfpSampleData, sina, cosa, phaseIncrements, N);
			...
			for (i = 0; i < numVertices; i++) {
				simplexResiduals[i] = simplex[i][numVertices - 1];
			}
			calculateBestToWorst(simplexResiduals, bestToWorst, numVertices);
			best = bestToWorst[0];
			worst = bestToWorst[2];
	...
	public double calculateTwoPResiduals(final double[] simplexLineValues, final double b0, final double t1, final double tr, final double[] Signal,
			final double[] sina, final double[] cosa, final double[] phaseIncrements, final int N) {
		...
		for (i = 0; i < N; i++) {
			...
			mx = m0 * (1.00 - e1) * e2 * sina[i] * (Math.cos(phasePrecession) - e2)
					/ ( (1.00 - e1 * cosa[i]) * (1.00 - e2 * Math.cos(phasePrecession)) - e2 * (e1 - cosa[i]) * (e2 - Math.cos(phasePrecession)));
			my = m0 * (1.00 - e1) * e2 * sina[i] * Math.sin(phasePrecession)
					/ ( (1.00 - e1 * cosa[i]) * (1.00 - e2 * Math.cos(phasePrecession)) - e2 * (e1 - cosa[i]) * (e2 - Math.cos(phasePrecession)));
	...
	public double calculateResiduals(final double[] simplexLineValues, final double t1, final double tr, final double[] Signal, final double[] sina,
			final double[] cosa, final double[] phaseIncrements, final int N) {
	...
	public double calculate2PResiduals(final double[] simplexLineValues, final double b0, final double t1, final double tr, final double[] Signal,
			final double[] sina, final double[] cosa, final double[] phaseIncrements, final int N) {
	...
	public void calculateBestToWorst(final double[] simplexResiduals, final int[] bestToWorst, final int numVertices) {
		...
		for (i = 0; i < numVertices; i++) {
			if (simplexResiduals[i] < simplexResiduals[best]) {
				best = i;
			}
			if (simplexResiduals[i] > simplexResiduals[worst]) {
				worst = i;
			}
		}
		...
		for (i = 0; i < numVertices; i++) {
			if (i != worst) {
				if (simplexResiduals[i] > simplexResiduals[secondWorst]) {
					secondWorst = i;
				}
			}
		}
	...
	public void reduceB0Field(final double[][] b0Field, final double resonancePeriod, final int width, final int height) {
		int i, j;
		...
		for (i = 0; i < width; i++) {
			for (j = 0; j < height; j++) {
				...
				if (fraction >= 1) {
					repetitionCycle = (int) (Math.floor(fraction));
				}
				if (fraction < 1 && fraction >= 0.5) {
					repetitionCycle = 1;
				}
				if (fraction < 0.5) {
					repetitionCycle = 0;
				...
				if (offResonanceMod < 0.00) {
					offResonanceMod = -1.00 * offResonanceMod;
				}
	...
	public void smoothField(final double[][] field, final float[][] fieldValues, final int width, final int height, final int k) {
		int x, y, p1, p2;
		int pixelIndex;
		...
		for (x = 0; x < width; x++) {
			for (y = 0; y < height; y++) {
				...
				if (y > 2 && y < height - 2 && x > 2 && x < width - 2) {
					...
					fieldValues[k - 1][pixelIndex] = (float) 0.00;
				}
	...
	public void smoothFieldB(final double[][] field, final int width, final int height) {
		int x, y, p1, p2;
		double smoothedValue;
	...
	public void resetSliceToZero(final double[][] field, final int width, final int height) {
		int x, y;
	...
	public void swapMatrixForVector(final double[][] fieldField, final float[][] fieldValues, final int k, final int width, final int height) {
		int x, y, pixelIndex;
	...
	public void swapMatrixForVectorB(final double[][] fieldField, final float[][] fieldValues, final int k, final int width, final int height) {
		int x, y, pixelIndex;
