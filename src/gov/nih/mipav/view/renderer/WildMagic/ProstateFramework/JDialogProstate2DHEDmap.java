 ...
 * 
 */
public class JDialogProstate2DHEDmap extends JDialogBase
	 ...
	 */
	public JDialogProstate2DHEDmap(Frame theParentFrame) {
	...
	private void sortImageTable() {
		for (int i = 0; i < 50; i++ ) {
			...
			for ( int j = 0; j < stringVec.size(); j++ ) {
				...
				int indEnd = fullName.lastIndexOf("-");
				String subName = fullName.substring(0, indEnd);
				int stopIndex = subName.lastIndexOf("-");
	...
	private void readKeyImageDir() {
		...
		*/
		...
		// File fileDir_4 = new File("/scratch/aam_test/fold5");
		// traverse_folder_4(fileDir_4);
		// cp -a */voi*.xml test1
		File fileDir_5 = new File("/scratch/aam_test/fold2");
		traverse_folder_5(fileDir_5, true);
		File fileDir_5_test = new File("/data/ruida/prostateHED/fold2Test/test_prostate_mask_iter_25000");
	...
	public void readFile() {
		...
		try {
			...
			// for (int i = 0; i < keyImageVector.size(); i++) {
			int start = 0;
			int end = keyImageVector5.size();
			// int end = 50; 
			
			int currentIndex = 0;

			// end = keyImageVector5.size();
			for (int imageIndex = start; imageIndex < end; imageIndex++) {
				// read key image
