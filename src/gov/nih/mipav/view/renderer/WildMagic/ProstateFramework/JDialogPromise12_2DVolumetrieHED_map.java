 ...
 * 
 */
public class JDialogPromise12_2DVolumetrieHED_map extends JDialogBase
	 ...
	 */
	public JDialogPromise12_2DVolumetrieHED_map(Frame theParentFrame) {
	...
	public void sortKeyImage_1() {
		...
		for (i = 0; i < len; i++) {
			...
			// System.err.println(voiName);
			start = voiName.lastIndexOf(File.separator) + 1;
			end = voiName.lastIndexOf("_");
			// System.err.println(voiName.substring(start+4, end));
	...
	*/
	...
	private void readKeyImageDir() {
		...
		File fileDir_1 = new File("/scratch/miccai/2D_Volumetric/N4TestCropScale");
		traverse_folder_1(fileDir_1);
		...
		File fileDir_5_test = new File("/scratch/miccai/2D_Volumetric/result_ced/coronal/");
...
//			coronalList.add(imageIndex, coronal_orient.getResultImage());
	...
	private void generateContours() {
        ...
        for(int i = 0; i < size; i++ ){
		    ...
		    ModelImage srcImage = coronalList.get(i);
		    ...
		    int[] srcExt = srcImage.getExtents();
	...
	public curvePair optimizeContours(ModelImage maskImage, ModelImage gradMagImage ) {
		float maxScore = 0;
		int curveNum = 0;
		curvePair curve1 = getScore(maskImage, gradMagImage, 240, 255);
