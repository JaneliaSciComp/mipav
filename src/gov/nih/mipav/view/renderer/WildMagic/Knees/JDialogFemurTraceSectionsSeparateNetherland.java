 ...
 * 
 */
public class JDialogFemurTraceSectionsSeparateNetherland extends JDialogBase implements AlgorithmInterface {
	...
	private JFileChooser greCEDImageChooser = new JFileChooser();
	private String greCEDImageName;
	private String greCEDImageDirectory;
	
	private JFileChooser fatCEDImageChooser = new JFileChooser();
	...
	private Vector<Vector<ModelImage>> imageStackFuzzyCfat = new Vector<Vector<ModelImage>>();
	private Vector<Vector<ModelImage>> imageStackFuzzyCgre = new Vector<Vector<ModelImage>>();
	 ...
	 */
	public JDialogFemurTraceSectionsSeparateNetherland(Frame theParentFrame, ModelImage srcImage) {
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (command.equals("OK")) {
			...
			readFATImage();
		} else if (command.equals("ChooseGRECEDImage")) {
			readGREImage();
		} else if (command.equals("ChooseFATCEDImage")) {
			readFATCEDImage();
		}
	}
	...
	public void init() {
		...
		// target image GRE CED directory
		labelImageGRECED = new JLabel("GRE ced Image: ");
		labelImageGRECED.setFont(serif12);
		labelImageGRECED.setForeground(Color.black);

		imageSelectionPanel.add(labelImageGRECED, gbc);

		textFieldImageGRECED = new JTextField(20);
		textFieldImageGRECED.setFont(serif12);

		gbc.gridx = 1;
		imageSelectionPanel.add(textFieldImageGRECED, gbc);

		buttonImageGRECED = new JButton("Choose");
		buttonImageGRECED.addActionListener(this);
		buttonImageGRECED.setActionCommand("ChooseGRECEDImage");
		buttonImageGRECED.setFont(serif12B);
		buttonImageGRECED.setPreferredSize(MipavUtil.defaultButtonSize);

		gbc.gridx = 2;
		imageSelectionPanel.add(buttonImageGRECED, gbc);
		
		
		// target image FAT CED directory
		labelImageFATCED = new JLabel("FAT ced Image: ");
		labelImageFATCED.setFont(serif12);
		labelImageFATCED.setForeground(Color.black);

		imageSelectionPanel.add(labelImageFATCED, gbc);

		textFieldImageFATCED = new JTextField(20);
		textFieldImageFATCED.setFont(serif12);

		gbc.gridx = 1;
		imageSelectionPanel.add(textFieldImageFATCED, gbc);

		buttonImageFATCED = new JButton("Choose");
		buttonImageFATCED.addActionListener(this);
		buttonImageFATCED.setActionCommand("ChooseFATCEDImage");
		buttonImageFATCED.setFont(serif12B);
		buttonImageFATCED.setPreferredSize(MipavUtil.defaultButtonSize);

		gbc.gridx = 2;
		imageSelectionPanel.add(buttonImageFATCED, gbc);
	...
	public void doSegmentation() {
		...
		new ViewJFrameImage(cedImageGRE);
		new ViewJFrameImage(cedImageFAT);
		
		// ModelImage cedImageGRE = runCED(greImage);
		doFuzzyCmean(cedImageGRE, imageStackFuzzyCgre);
		
		// ModelImage cedImageFAT = runCED(fatImage);
		doFuzzyCmean(cedImageFAT, imageStackFuzzyCfat);
	...
	public void segmentationAuto() {
		...
		// tracingDFS_mid(greImageSliceMid, fatImageSliceMid, midVOIBoundary,
		// midVOIInner, midVOIOuter, true);
	   ...
	   // for (int i = startSlice - 1; i >= startSlice - 45; i--) {
			...
			ModelImage class1ImageFat = imageStackFuzzyCfat.get(i).get(0);
			ModelImage class2ImageFat = imageStackFuzzyCfat.get(i).get(1);
			ModelImage class3ImageFat = imageStackFuzzyCfat.get(i).get(2);
			ModelImage fuzzyCImageFat = imageStackFuzzyCfat.get(i).get(3);

			ModelImage class1ImageGre = imageStackFuzzyCgre.get(i).get(0);
			ModelImage class2ImageGre = imageStackFuzzyCgre.get(i).get(1);
			ModelImage class3ImageGre = imageStackFuzzyCgre.get(i).get(2);
			ModelImage fuzzyCImageGre = imageStackFuzzyCgre.get(i).get(3);

			tracingDFS(fatImageSlice, fuzzyCImageFat, class1ImageFat, class2ImageFat, class3ImageFat, 
					   greImageSlice, fuzzyCImageGre, class1ImageGre, class2ImageGre, class3ImageGre,
					   i, resultVOIBoundary, resultVOIInner, resultVOIOuter,
					   leftLine, rightLine);
		...
		/*
		 * // Track back, Right Leg, for GROUP_4 if ( whichLeg == RIGHT_LEG ) {
		 * traceBackRightLegGroup4(fileInfosFAT, fileInfosGRE); //
		 * traceBackRightLegGroup7to6(fileInfosFAT, fileInfosGRE); } else if (
		 * whichLeg == LEFT_LEG ) { traceBackLeftLegGroup7to6(fileInfosFAT,
		 * fileInfosGRE); // traceBackLeftLegGroup4(fileInfosFAT, fileInfosGRE);
		 * } voiNewFinal.update();
		 */
	...
	private void traceTwoIslandsOnLeft(FileInfoImageXML[] fileInfosFAT, FileInfoImageXML[] fileInfosGRE) {
		...
		for (int i = startSlice - 1; i >= endSlice; i--) {
			...
			ModelImage class1Image = imageStackFuzzyCfat.get(i).get(0);
			ModelImage class2Image = imageStackFuzzyCfat.get(i).get(1);
			ModelImage class3Image = imageStackFuzzyCfat.get(i).get(2);
			ModelImage fuzzyCImage = imageStackFuzzyCfat.get(i).get(3);

			traceTwoIslands(fatImageSlice, fuzzyCImage, class1Image, class2Image, class3Image, greImageSlice, i, currentSliceVOI, endSliceVOI, false);
	...
	private void traceTwoIslandsOnRight(FileInfoImageXML[] fileInfosFAT, FileInfoImageXML[] fileInfosGRE) {
		...
		for (int i = startSlice + 1; i <= endSlice; i++) {
			...
			if ((xBound[1] - xBound[0]) >= 200 || (yBound[1] - yBound[0]) >= 200)
				break;
	...
	private void traceBackLeftLegGroup4(FileInfoImageXML[] fileInfosFAT, FileInfoImageXML[] fileInfosGRE) {
		...
		for (int i = startSlice - 1; i >= endSlice; i--) {
			...
			tracingDFSBackward(fatImageSlice, fuzzyCImage, class1Image, class2Image, class3Image, greImageSlice, i, currentSliceVOI, endSliceVOI, false);
	...
	private void traceBackLeftLegGroup7to6(FileInfoImageXML[] fileInfosFAT, FileInfoImageXML[] fileInfosGRE) {
		...
		for (int i = startSlice + 1; i <= endSlice; i++) {
			...
			tracingDFSBackward(fatImageSlice, fuzzyCImage, class1Image, class2Image, class3Image, greImageSlice, i, currentSliceVOI, endSliceVOI, true);
	...
	private void traceTwoIslands(ModelImage fatImageSlice, ModelImage fuzzyCImage, ModelImage class1Image, ModelImage class2Image, ModelImage class3Image,
			ModelImage greImageSlice, int sliceNumber, VOI currentVOI, VOI endVOI, boolean extendOuter) {
		...
		// ******************************************
		/*
		 * VOIBaseVector current_curves =
		 * fatImageSlice.getVOIs().VOIAt(0).getCurves(); int numCurves =
		 * current_curves.size();
		 * 
		 * int[] extents = new int[3]; extents = fatImage.getExtents();
		 * 
		 * int size = extents[0] * extents[1]; short[] buffer = new short[size];
		 * 
		 * int[] newExtents = new int[2]; newExtents[0] = extents[0];
		 * newExtents[1] = extents[1];
		 * 
		 * ModelImage testImage = new ModelImage(ModelStorageBase.SHORT,
		 * newExtents, makeImageName(fatImage.getImageName(), "testImage"));
		 * 
		 ...
		 * BitSet seedPaintBitmap = new BitSet(); Point seedPt = new
		 * Point((int)contourCenter.X, (int)contourCenter.Y); float
		 * fuzzyThreshold = -1.0f; boolean useVOI = false; boolean displayFuzzy
		 * = false;
		 * 
		 * float saveValue = 10.0f; float less = 10.0f; float more = 10.0f; int
		 * sizeLimit = -1; float maxDistance = -1.0f; boolean variableThresholds
		 * = false;
		 * 
		 * 
		 * AlgorithmRegionGrow regionGrowAlgo = new
		 * AlgorithmRegionGrow(fatImageSlice, 1.0f, 1.0f);
		 * regionGrowAlgo.setRunningInSeparateThread(false);
		 * 
		 * // under segment so that we do not get the blanket
		 * regionGrowAlgo.regionGrow2D(seedPaintBitmap, seedPt, -1, false,
		 * false, null,saveValue - less, saveValue + more, -1, -1, false);
		 * 
		 * paintBitmap.or(seedPaintBitmap);
		 * 
		 * // make the abdominal label image from the volume BitSet determined
		 * in the region grow for (int idx = 0; idx < size; idx++) { if
		 * (paintBitmap.get(idx)) { buffer[idx] = 1; } else { buffer[idx] = 0; }
		 * } // end for (int idx = 0; ...)
		 * 
		 * // save the sliceBuffer into the abdomenImage try {
		 * testImage.importData(0, buffer, false); } catch (IOException ex) {
		 * System.err.println("labelAbdomen2D(): Error importing data"); }
		 * 
		 * 
		 * } // end curveIndex loop
		 * 
		 * boolean wholeImage = true; AlgorithmMorphology2D idObjectsAlgo2D; int
		 * method = AlgorithmMorphology2D.ID_OBJECTS;
		 * 
		 * idObjectsAlgo2D = new AlgorithmMorphology2D(testImage, 0, 0, method,
		 * 0, 0, 0, 0, wholeImage); idObjectsAlgo2D.setMinMax(1,
		 * Integer.MAX_VALUE); idObjectsAlgo2D.run();
		 * idObjectsAlgo2D.finalize(); idObjectsAlgo2D = null;
		 * 
		 * testImage.calcMinMax(); AlgorithmVOIExtraction VOIExtractionAlgo =
		 * new AlgorithmVOIExtraction(testImage); VOIExtractionAlgo.run();
		 * 
		 * VOIVector voiVector = testImage.getVOIs(); int VOIsSize =
		 * voiVector.size();
		 * 
		 ...
		 * 
		 * for ( int i = 0; i < VOIsSize; i++ ) { VOI currVOI =
		 * voiVector.VOIAt(i); VOIBaseVector vArray = currVOI.getCurves();
	...
	public void equalDistanceSmooth(ModelImage greImageSlice) {
		...
		for (int i = 0; i < vVector.size(); i++) {
			...
			/*
			 * if (group == GROUP_1 || group == GROUP_2 || group == GROUP_9 ||
			 * group == GROUP_10) { constant_length = 1.0d; } else {
	...
	public ModelImage findTwoIslands(ModelImage image, int[] xBounds, int[] yBounds, int[] zBounds, VOI filteredVOI) {
		...
		// if ( (boxXmax - boxXmin) >= 10 || ( boxYmax - boxYmin ) >= 150 )
		// return null;
		...
		for (int k = 0; k < measure_va.size(); k++) {
			...
			for (i = 0; i < numVOIs; i++) {
				...
				if (distance > 0) {
					...
					/*
					 * int nPtsCurrent = current_v.size(); float[] xPtsCurrent =
					 * new float[nPtsCurrent]; float[] yPtsCurrent = new
					 * float[nPtsCurrent]; float[] zPtsCurrent = new
					 * float[nPtsCurrent]; current_v.exportArrays(xPtsCurrent,
					 * yPtsCurrent, zPtsCurrent);
					 * 
					 * for (int j = 0; j < nPtsCurrent; j++) { xPtsCurrent[j] +=
					 * xBounds[0]; yPtsCurrent[j] += yBounds[0]; zPtsCurrent[j]
					 * = 0; }
					 * 
					 * VOIBase vTemp = (VOIBase)image_v.clone();
					 * vTemp.removeAllElements();
					 * vTemp.importArrays(xPtsCurrent, yPtsCurrent, zPtsCurrent,
					 * nPtsCurrent);
					 */
		...
		for (int curveIndex = 0; curveIndex < numCurves; curveIndex++) {
			...
			// under segment so that we do not get the blanket
			regionGrowAlgo.regionGrow2D(seedPaintBitmap, seedPt, -1, false, false, null, saveValue - less, saveValue + more, -1, -1, false);
	...
	private void tracingDFSBackward(ModelImage fatImageSlice, ModelImage fuzzyCImage, ModelImage class1Image, ModelImage class2Image, ModelImage class3Image,
		...
		// System.err.println("inside trackDFSBackward nPtsCurrentOuter = " +
		// nPtsCurrentOuter);
		...
		for (i = 0; i < minPts; i++) {
			...
			stepX = (xInner - center.X) / 1200;
			stepY = (yInner - center.Y) / 1200;
			...
			if (degree >= section4_degree_start - 15 && degree < section4_degree_end + 15) {
				...
				resultPos = traceSection4Backward(posX, posY, xInner, yInner, stepX, stepY, center, degree, sliceNumber, currentStep, distCurrent, distInner,
						greImageSlice, class1Image, class2Image, class3Image, fuzzyCImage);
		...
		// ********************************** Re-orient the points according to
		// polar degree ***********************
	...
	private Vector2f traceSection4Backward(float posX, float posY, float stopX, float stopY, float stepX, float stepY, Vector3f center, float degree,
			int sliceNumber, int currentStep, float distCurrent, float distInner, ModelImage greImageSlice, ModelImage class1Image, ModelImage class2Image,
			ModelImage class3Image, ModelImage fuzzyCImage) {
	...
	private void findBoundingContour(int sliceNumber, Vector3f center, ModelImage fatImageSlice, VOI endVOI) {
		...
		for (i = 0; i < nPtsCurrent; i++) {
			...
			/*
			 * stepX = (posX - centerX) / 600f; stepY = (posY - centerY) / 600f;
			 * 
			 * traceX = posX; traceY = posY;
			 * 
			 * distCurrent = Math.sqrt((traceX - centerX) * (traceX - centerX) +
			 * (traceY - centerY) * (traceY - centerY)); while (distCurrent >
			 * distanceInner) { traceX -= stepX; traceY -= stepY; //
			 * index_inner++; distCurrent = Math.sqrt((traceX - centerX) *
			 * (traceX - centerX) + (traceY - centerY) * (traceY - centerY)); }
			 * pt_inner[i] = new Vector3f(traceX, traceY, 0f);
			 * 
			 * traceX = posX; traceY = posY;
			 * 
			 * distCurrent = Math.sqrt((traceX - centerX) * (traceX - centerX) +
			 * (traceY - centerY) * (traceY - centerY)); while (distCurrent <
			 * distanceOuter) { traceX += stepX; traceY += stepY; distCurrent =
			 * Math.sqrt((traceX - centerX) * (traceX - centerX) + (traceY -
			 * centerY) * (traceY - centerY)); // index_outer++; } pt_outer[i] =
			 * new Vector3f(traceX, traceY, 0f);
			 */
		} // end i loop;
	...
	public void readGREImages() {
		FileIO greImageIO = null;

		try {
			// read target image
			greImageIO = new FileIO();
			cedImageGRE = greImageIO.readImage(greCEDImageName, greCEDImageDirectory);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void readFATCEDImages() {
		...
		try {
			// read target image
			fatImageIO = new FileIO();
			cedImageFAT = fatImageIO.readImage(fatCEDImageName, fatCEDImageDirectory);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void readGREImage() {
		greCEDImageChooser.setDialogTitle("Open GRE ced Image");

		if (UI.getDefaultDirectory() != null) {
			final File file = new File(UI.getDefaultDirectory());

			if (file != null) {
				greCEDImageChooser.setCurrentDirectory(file);
			}
		} else {
			greCEDImageChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
		}

		greCEDImageChooser.addChoosableFileFilter(new ViewImageFileFilter(new String[] { ".xml", ".XML" }));

		final int returnValue = greCEDImageChooser.showOpenDialog(UI.getMainFrame());

		if (returnValue == JFileChooser.APPROVE_OPTION) {
			greCEDImageName = greCEDImageChooser.getSelectedFile().getName();
			greCEDImageDirectory = String.valueOf(greCEDImageChooser.getCurrentDirectory()) + File.separatorChar;
			textFieldImageGRECED.setText(greCEDImageName);
	...
	private void readFATCEDImage() {
		fatCEDImageChooser.setDialogTitle("Open FAT ced Image");

		if (UI.getDefaultDirectory() != null) {
			final File file = new File(UI.getDefaultDirectory());

			if (file != null) {
				fatCEDImageChooser.setCurrentDirectory(file);
			}
		} else {
			fatCEDImageChooser.setCurrentDirectory(new File(System.getProperty("user.dir")));
		}

		fatCEDImageChooser.addChoosableFileFilter(new ViewImageFileFilter(new String[] { ".xml", ".XML" }));

		final int returnValue = fatCEDImageChooser.showOpenDialog(UI.getMainFrame());

		if (returnValue == JFileChooser.APPROVE_OPTION) {
			fatCEDImageName = fatCEDImageChooser.getSelectedFile().getName();
			fatCEDImageDirectory = String.valueOf(fatCEDImageChooser.getCurrentDirectory()) + File.separatorChar;
			textFieldImageFATCED.setText(fatCEDImageName);
	...
	private void readFATImage() {
		fatImageChooser.setDialogTitle("Open FAT Image");
	...
	private Vector2f traceSection6(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {

		double fuzzyCIntensityFat;
		double class1IntensityFat;
		double class2IntensityFat;
		double class3IntensityFat;
		
		double fuzzyCIntensityGre;
		double class1IntensityGre;
		double class2IntensityGre;
		double class3IntensityGre;
		...
		*/ 
		...
		while (/* fuzzyCIntensityFat != 3 && fuzzyCIntensityFat != 2 && */ distCurrent < distOuter) {
			...
			if ((whichLeg == RIGHT_LEG && (group <= GROUP_3 || group >= GROUP_8)) || (whichLeg == LEFT_LEG && (group >= GROUP_3 || group < GROUP_8))) {
				while (distCurrent < distOuter) {
					...
					if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) {
						...
						findHoles = findHolesOnImage((int) posX, (int) posY, fuzzyCImageFat, holesDiameter, 0, FuzzyC);
						if (findHoles) {
							// skip holes
							while ((fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) && distCurrent < distOuter) {
								posX += stepX;
								posY += stepY;
			...
			boolean findGREfuzzyCClass3Pattern = false;
			findGREfuzzyCClass3Pattern = findGreyRegionOnImage((int) posX, (int) posY, fuzzyCImageGre, 5, 0, GRE_FUZZY_CLASS3);
			if (findGREfuzzyCClass3Pattern) {
				posX += stepX;
				posY += stepY;
				distCurrent = (float) Math.sqrt((posX - center.X) * (posX - center.X) + (posY - center.Y) * (posY - center.Y));
				fuzzyCIntensityFat = fuzzyCImageFat.getDouble((int) posX, (int) posY);
			...
			boolean findGREfuzzyCClass1Pattern = false;
			findGREfuzzyCClass1Pattern = findGreyRegionOnImage((int) posX, (int) posY, fuzzyCImageGre, 5, 0, GRE_FUZZY_CLASS1);
			if (findGREfuzzyCClass1Pattern) {
				posX += stepX;
				posY += stepY;
				distCurrent = (float) Math.sqrt((posX - center.X) * (posX - center.X) + (posY - center.Y) * (posY - center.Y));
				fuzzyCIntensityFat = fuzzyCImageFat.getDouble((int) posX, (int) posY);
			...
			if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) {
				boolean findHoles = false;
				// find holes on fuzzyC image
				findHoles = findHolesOnImage((int) posX, (int) posY, fuzzyCImageFat, 15, 0, FuzzyC);
				if (findHoles) {
					// skip holes
					while ((fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) && distCurrent < distOuter) {
						posX += stepX;
			...
			if ( fuzzyCIntensityGre == 2  || fuzzyCIntensityGre == 3 ) {
				while ((fuzzyCIntensityGre == 2 || fuzzyCIntensityGre == 3 ) && distCurrent < distOuter) {
					posX += stepX;
					posY += stepY;
			...
			if ( fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.7f || class3IntensityGre <= 0.1f ) {
				while ((fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.7f || class3IntensityGre <= 0.1f ) && distCurrent < distOuter) {
					posX += stepX;
					posY += stepY;
					...
					if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3 || class1IntensityFat <= 0.1f)
						break;
				...
				if ( fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3 || 
						class1IntensityFat <= 0.1f || class3IntensityFat >= 0.9f ||  class2IntensityFat >= 0.9f ) {
					break;
				}
				
				if ( fuzzyCIntensityGre == 2  || fuzzyCIntensityGre == 3 || 
						class1IntensityGre <= 0.1f || class3IntensityFat >= 0.9f || class2IntensityGre >= 0.7f) { 
					break;
				}
			...
			if ( class1IntensityGre <= 0.1f || class3IntensityGre >= 0.7f )
				break;
	...
	private Vector2f traceSection1(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private Vector2f traceSection2(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		while (/* fuzzyCIntensityFat != 3 && fuzzyCIntensityFat != 2 && */  distCurrent < distOuter) {
			...
			if (findGREfuzzyCPattern) {
				fuzzyCIntensityFat = fuzzyCImageFat.getDouble((int) posX, (int) posY);
				if ( fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3 ) break; 
				else {
					posX += stepX;
					posY += stepY;
					distCurrent = (float) Math.sqrt((posX - center.X) * (posX - center.X) + (posY - center.Y) * (posY - center.Y));
			...
			if ( fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3 || 
					class1IntensityFat <= 0.1f || class3IntensityFat >= 0.9f ||  class2IntensityFat >= 0.5f ) {
				break;
			}
			...
			if ( fuzzyCIntensityGre == 1  || class1IntensityGre >= 0.9f || class3IntensityFat <= 0.1f ) {
				while ((fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f || class3IntensityFat <= 0.1f ) && distCurrent < distOuter) {
					posX += stepX;
					posY += stepY;
					...
					if ( fuzzyCIntensityGre == 2 || fuzzyCIntensityGre == 3) break;
				...
				if ( fuzzyCIntensityGre == 2  || fuzzyCIntensityGre == 3 || class1IntensityGre <= 0.1f || class3IntensityFat >= 0.7f ) { 
					break;
				}
			...
			boolean findEdgeOnGREFuzzyC = findEdgeOnImage((int) posX, (int) posY, fuzzyCImageGre, 10, 50, FuzzyC_GRE_LOW);
			...
			if (whichLeg == LEFT_LEG) {
				if (group == GROUP_6 && (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3))
					break;
				else if (group == GROUP_7 && fuzzyCIntensityFat == 3)
					break;
				else if (group == GROUP_8 && fuzzyCIntensityFat == 3)
					break;
			}
			...
			xResult[0] = -1;
			findWeakEdge = findWeakConnectedEdgeOnImageVertical((int) posX, (int) posY, fuzzyCImageFat, 7, FuzzyC, true, xResult);
			findWeakEdgeClass2 = findWeakConnectedEdgeOnImageVertical((int) posX, (int) posY, class2ImageFat, 7, Class2, false, xResult);
			findWeakEdgeClass3 = findWeakConnectedEdgeOnImageVertical((int) posX, (int) posY, class3ImageFat, 7, Class3, false, xResult);
			...
			if (findWeakEdge || findWeakEdgeClass2) {
				...
				while (distCurrent < distOuter) {
					...
					fuzzyCIntensityFat = fuzzyCImageFat.getDouble((int) testX, (int) testY);
					class3IntensityFat = class3ImageFat.getDouble((int) testX, (int) testY);
					if (fuzzyCIntensityFat == 3 || fuzzyCIntensityFat == 2) {
						lookFor3 = true;
						break;
					}
					testX += stepX;
			...
			findEdgeOnClass1 = findEdgeOnImage((int) posX, (int) posY, class1ImageFat, 7, 50, Class1);
			findLowIntenClass1 = findEdgeOnImage((int) posX, (int) posY, class1ImageFat, 7, 50, Class1_lowInten);
			findEdgeOnClass2 = findEdgeOnImage((int) posX, (int) posY, class2ImageFat, 7, 50, Class2);
			findEdgeOnClass3 = findEdgeOnImage((int) posX, (int) posY, class3ImageFat, 7, 50, Class3);
			...
			class2IntensityFat = class2ImageFat.getDouble((int) posX, (int) posY);
			class3IntensityFat = class3ImageFat.getDouble((int) posX, (int) posY);
			if (class2IntensityFat >= 0.9f || class1IntensityFat < 0.1f || class3IntensityFat >= 0.09) {
				boolean findHolesClass1 = false;
				findHolesClass1 = findHolesOnImage((int) posX, (int) posY, class1ImageFat, 25, 0, Class1);
				boolean findHolesClass2 = false;
				findHolesClass2 = findHolesOnImage((int) posX, (int) posY, class2ImageFat, 25, 0, Class2);
				boolean findHolesClass3 = false;
				findHolesClass3 = findHolesOnImage((int) posX, (int) posY, class3ImageFat, 25, 0, Class3);
				if (findHolesClass1 || findHolesClass2 || findHolesClass3) {
					// skip holes
					while ((class1IntensityFat <= 0.1f || class2IntensityFat >= 0.9f || class3IntensityFat >= 0.09) && distCurrent < distOuter) {
						posX += stepX;
						posY += stepY;
	...
	private Vector2f traceSection3(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		while (/* fuzzyCIntensityFat != 3 && fuzzyCIntensityFat != 2 && */   distCurrent < distOuter) {
			...
			if ((whichLeg == RIGHT_LEG && (group <= GROUP_3 || group >= GROUP_8)) || (whichLeg == LEFT_LEG && (group >= GROUP_3 || group < GROUP_8))) {
				while (distCurrent < distOuter) {
					...
					if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) {
						boolean findHoles = false;
						// find holes on fuzzyC image
						findHoles = findHolesOnImage((int) posX, (int) posY, fuzzyCImageFat, 20, 0, FuzzyC);
						if (findHoles) {
							// skip holes
							while ((fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) && distCurrent < distOuter) {
								posX += stepX;
			...
			if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3) {
				boolean findHoles = false;
				// find holes on fuzzyC image
				findHoles = findHolesOnImage((int) posX, (int) posY, fuzzyCImageFat, 25, 0, FuzzyC);
			...
			if ( fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f ) {
				while ((fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f ) && distCurrent < distOuter) {
					posX += stepX;
					posY += stepY;
				...
				if ( fuzzyCIntensityGre == 3 || class1IntensityGre <= 0.1f ) 
					break;
	...
	private Vector2f traceSection4(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		while (/* fuzzyCIntensityFat != 3  && fuzzyCIntensityFat != 2 && */ distCurrent < distOuter) {
			...
			if ((whichLeg == RIGHT_LEG && (group < GROUP_2 || group > GROUP_9)) || (whichLeg == LEFT_LEG && (group > GROUP_2 || group < GROUP_9))) {
				if (fuzzyCIntensityFat == 2 || fuzzyCIntensityFat == 3 || 
						class1IntensityFat <= 0.1f || class2IntensityFat >= 0.9f || class3IntensityFat >= 0.9f)
					break;
			} else {
				if (fuzzyCIntensityFat == 3 || fuzzyCIntensityFat == 2)
					break;
			...
			if ( fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f || class3IntensityGre <= 0.1f ) {
				while ((fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f || class3IntensityGre <= 0.1f ) && distCurrent < distOuter) {
					posX += stepX;
					posY += stepY;
				...
				if ( fuzzyCIntensityGre == 3 || class1IntensityGre <= 0.1f || class3IntensityGre >= 0.9f ) 
					break;
	...
	private Vector2f traceSection5_upper(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		while ( fuzzyCIntensityFat != 3 && fuzzyCIntensityFat != 2 && distCurrent < distOuter) {
			...
			if ( fuzzyCIntensityGre == 1 || class1IntensityGre >= 0.9f /* || class3IntensityGre <= 0.1f */ ) {
				...
				if ( fuzzyCIntensityGre == 3 ||  fuzzyCIntensityFat == 2 || class1IntensityGre <= 0.1f || class3IntensityGre >= 0.9f ) 
					break;
			...
			int[] xResult = new int[1];
			xResult[0] = -1;
			findWeakEdge = findWeakConnectedEdgeOnImageVertical((int) posX, (int) posY, fuzzyCImageFat, 10, FuzzyC, true, xResult);
			// findWeakEdge10 = findWeakConnectedEdgeOnImageVertical((int)posX, (int) posY, fuzzyCImage, 10, FuzzyC, true, xResult);
			findWeakEdgeClass3 = findWeakConnectedEdgeOnImageVertical((int) posX, (int) posY, class3ImageFat, 10, Class3, true, xResult);
			...
			if (findWeakEdge) {
				...
				if (findWeakEdgeClass3) {
					...
					while (distCurrent < distOuter) {
						...
						fuzzyCIntensityFat = fuzzyCImageFat.getDouble((int) testX, (int) testY);
						class3IntensityFat = class3ImageFat.getDouble((int) testX, (int) testY);
						if (fuzzyCIntensityFat == 3 || fuzzyCIntensityFat == 2 || class3IntensityFat >= 0.04f) {
							lookFor3 = true;
							break;
						}
						testX += stepX;
	...
	private Vector2f traceSection5_lower(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat,
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		while (/* fuzzyCIntensityFat != 3 && */ distCurrent < distOuter) {
	...
	private Vector2f traceSection6_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private Vector2f traceSection1_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private Vector2f traceSection2_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
		...
		double fuzzyCIntensityGre = fuzzyCImageGre.getDouble((int) posX, (int) posY);
		double class1IntensityGre = class1ImageGre.getDouble((int) posX, (int) posY);
		...
		while (fuzzyCIntensityFat != 3 && fuzzyCIntensityFat != 2 && distCurrent < distOuter) {
			...
			/*
			if ( fuzzyCIntensityFat == 1 || class1IntensityFat >= 0.9 || class3IntensityFat <= 0.1f ||  class2IntensityFat <= 0.25f ) {
				continue;
			}
			*/ 
	...
	private Vector2f traceSection3_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private Vector2f traceSection4_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber, float distCurrent,
			float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private Vector2f traceSection5_upper_condyle(float posX, float posY, float stepX, float stepY, Vector3f center, float degree, int sliceNumber,
			float distCurrent, float distOuter, ModelImage greImageSlice, 
			ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, ModelImage fuzzyCImageFat, 
			ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, ModelImage fuzzyCImageGre) {
	...
	private void tracingDFS_condyle(ModelImage fatImageSlice, ModelImage fuzzyCImageFat, ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat,
			ModelImage greImageSlice, ModelImage fuzzyCImageGre, ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre,
			int sliceNumber, VOI resultVOIBoundary, VOI resultVOIInner, VOI resultVOIOuter, VOILine leftLine, VOILine rightLine) {
		...
		for (i = 0; i < nPts; i++) {
			...
			Vector2f resultPos = new Vector2f(0, 0);
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				// System.err.println("trace section 1 condyle");
				resultPos = traceSection1_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat,
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 1 condyle end");
			} else if (degree >= section2_degree_start && degree < section2_degree_end) {
				// System.err.println("trace section 2 condyle");
				resultPos = traceSection2_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat,
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 2 condyle end");
			} else if (degree >= section3_degree_start && degree < section3_degree_end) {
				// System.err.println("trace section 3 condyle");
				resultPos = traceSection3_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 3 condyle end");
			} else if (degree >= section4_degree_start && degree < section4_degree_end) {
				// System.err.println("trace section 4 condyle");
				resultPos = traceSection4_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat,
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 4 condyle end");
			} else if (degree > section5_degree_upperHalf_start && degree <= section5_degree_upperHalf_end) {
				// System.err.println("trace section 5 upper condyle");
				resultPos = traceSection5_upper_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 upper condyle end");
			} else if (degree >= section5_degree_lowerHalf_start && degree < section5_degree_lowerHalf_end) {
				// System.err.println("trace section 5 lower condyle");
				resultPos = traceSection5_lower_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 lower condyle end");
			} else { // section 6
				// System.err.println("trace section 6 condyle");
				resultPos = traceSection6_condyle(posX, posY, stepX, stepY, center, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 6 condyle end");
			}
		 ...
		 * constant_length = 0.5d; } else if (((sliceNumber - startPt) > 2 &&
		 * (sliceNumber - startPt) <= 5) || (endPt - sliceNumber) > 2 && (endPt
		 * - sliceNumber) <= 5) { constant_length = 0.8d; } else {
		...
		for (k = 0; k < m; k++) {
			pts[k] = new Vector3f(xNew[k], yNew[k], 0);
		}
		...
		// generate inner and outer contours
		// findBoundingContour(sliceNumber, resultVOIBoundary, resultVOIInner,
		// resultVOIOuter, center, fatImageSlice);
		...
		fuzzyCImageFat.addVOIs(voiVectorNew);
		class1ImageFat.addVOIs(voiVectorNew);
		class2ImageFat.addVOIs(voiVectorNew);
		class3ImageFat.addVOIs(voiVectorNew);
		// new ViewJFrameImage(greImageSlice); // Cheng1
	...
	private void tracingDFS(ModelImage fatImageSlice, ModelImage fuzzyCImageFat, ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat,
							ModelImage greImageSlice, ModelImage fuzzyCImageGre, ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre,
							int sliceNumber, VOI resultVOIBoundary, VOI resultVOIInner, VOI resultVOIOuter, VOILine leftLine, VOILine rightLine) {
		...
		// ****************************** Check the two lines lower points
		// distance *************************
		float shaftDistance = (float) Math.sqrt((upperRightPt.X - upperLeftPt.X) * (upperRightPt.X - upperLeftPt.X) + (upperRightPt.Y - upperLeftPt.Y)
		...
		if (shaftDistance <= close_length   /* || traceCondyle */  ) {
			tracingDFS_condyle(fatImageSlice, fuzzyCImageFat, class1ImageFat, class2ImageFat, class3ImageFat, 
					greImageSlice, fuzzyCImageGre, class1ImageGre, class2ImageGre, class3ImageGre, 
					sliceNumber, resultVOIBoundary, resultVOIInner, resultVOIOuter, leftLine, rightLine);
		} else {
			...
			if ((whichLeg == RIGHT_LEG && (group <= GROUP_3 || group >= GROUP_8)) || (whichLeg == LEFT_LEG && (group >= GROUP_3 || group <= GROUP_8))) {
				...
				tracingDFS_shaftCondyle(fatImageSlice, fuzzyCImageFat, class1ImageFat, class2ImageFat, class3ImageFat, 
										greImageSlice, fuzzyCImageGre, class1ImageGre, class2ImageGre, class3ImageGre, 
										sliceNumber, resultVOIBoundary, resultVOIInner, resultVOIOuter, leftLine, rightLine);
			}
		}
        ...
        */ 
		...
		/*
		 * VOIBase v = resultVOIBoundary.getCurves().get(0); int nPts =
		 * v.size(); float[] xPts = new float[nPts]; float[] yPts = new
		 * float[nPts]; float[] zPts = new float[nPts]; v.exportArrays(xPts,
		 * yPts, zPts);
		 * 
		 * float min = 512;
		 * 
		 * for ( int i = 0; i < nPts; i++ ) { if ( yPts[i] < min ) { min =
		 * yPts[i]; } }
		 * 
		 * if ( min >= 20 ) { tracingDFS_condyle(fatImageSlice, fuzzyCImage,
		 * class1Image, class2Image, class3Image, greImageSlice,
		 * gaussianImageSlice, sliceNumber, resultVOIBoundary, resultVOIInner,
		 * resultVOIOuter, leftLine, rightLine); } else {
		 * tracingDFS_shaftCondyle(fatImageSlice, fuzzyCImage, class1Image,
		 * class2Image, class3Image, greImageSlice, gaussianImageSlice,
		 * sliceNumber, resultVOIBoundary, resultVOIInner, resultVOIOuter,
		 * leftLine, rightLine); }
		 */

	}

	private void tracingDFS_shaftCondyle(ModelImage fatImageSlice, ModelImage fuzzyCImageFat, ModelImage class1ImageFat, ModelImage class2ImageFat, ModelImage class3ImageFat, 
										 ModelImage greImageSlice, ModelImage fuzzyCImageGre, ModelImage class1ImageGre, ModelImage class2ImageGre, ModelImage class3ImageGre, 
										 int sliceNumber, VOI resultVOIBoundary, VOI resultVOIInner, VOI resultVOIOuter, VOILine leftLine, VOILine rightLine) {
		...
		for (i = 0; i < lowerIndex; i++) {
			...
			stepX = (xInner - condyleCenter.X) / 1200f;
			stepY = (yInner - condyleCenter.Y) / 1200f;
			...
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				// System.err.println("trace section 1");
				resultPos = traceSection1(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
					    greImageSlice, 
					    class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
					    class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 1 end");
			} else if (degree >= section2_degree_start && degree < section2_degree_end) {
				// System.err.println("trace section 2");
				resultPos = traceSection2(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 2 end");
			} else if (degree >= section3_degree_start && degree < section3_degree_end) {
				// System.err.println("trace section 3");
				resultPos = traceSection3(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 3 end");
			} else if (degree >= section4_degree_start && degree < section4_degree_end) {
				// System.err.println("trace section 4");
				resultPos = traceSection4(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 4 end");
			} else if (degree > section5_degree_upperHalf_start && degree <= section5_degree_upperHalf_end) {
				// System.err.println("trace section 5 upper");
				resultPos = traceSection5_upper(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 upper end");
			} else if (degree >= section5_degree_lowerHalf_start && degree < section5_degree_lowerHalf_end) {
				// System.err.println("trace section 5 lower");
				resultPos = traceSection5_lower(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 lower end");
			} else { // section 6
				// System.err.println("trace section 6");
				resultPos = traceSection6(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat,
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 6 end");
			}
		...
		for (i = 0; i < leftIndex; i++) {
			...
			stepX = (xInner - condyleCenter.X) / 1200;
			stepY = (yInner - condyleCenter.Y) / 1200;
			...
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				// System.err.println("trace section 1 left line");
				resultPos = traceSection1(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 1 left line end");
			} else if (degree >= section2_degree_start && degree < section2_degree_end) {
				// System.err.println("trace section 2 left line");
				resultPos = traceSection2(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 2 left line end");
			} else if (degree >= section3_degree_start && degree < section3_degree_end) {
				// System.err.println("trace section 3 left line");
				resultPos = traceSection3(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 3 left line end");
			} else if (degree >= section4_degree_start && degree < section4_degree_end) {
				// System.err.println("trace section 4 left line");
				resultPos = traceSection4(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 4 left line end");
			} else if (degree > section5_degree_upperHalf_start && degree <= section5_degree_upperHalf_end) {
				// System.err.println("trace section 5 upper left line");
				resultPos = traceSection5_upper(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 upper left line end");
			} else if (degree >= section5_degree_lowerHalf_start && degree < section5_degree_lowerHalf_end) {
				// System.err.println("trace section 5 lower left line");
				resultPos = traceSection5_lower(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 lower left line end");
			} else { // section 6
				// System.err.println("trace section 6 left line");
				resultPos = traceSection6(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 6 left line end");
			}
		...
		if ((whichLeg == LEFT_LEG && group <= GROUP_6) || (whichLeg == RIGHT_LEG && group <= GROUP_5)) {
			...
			if (upperLeftX > (lowerLeftX + 10)) {
		    	upperLeftX = lowerLeftX + 10;
			}
		...
		*/
		...
		for (i = 0; i < rightIndex; i++) {
			...
			Vector2f resultPos = new Vector2f(xOuter, yOuter);
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				// System.err.println("trace section 1 right line");
				resultPos = traceSection1(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat,
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre
						);
				// System.err.println("trace section 1 right line end");
			} else if (degree >= section2_degree_start && degree < section2_degree_end) {
				// System.err.println("trace section 2 right line");
				resultPos = traceSection2(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 2 right line end");
			} else if (degree >= section3_degree_start && degree < section3_degree_end) {
				// System.err.println("trace section 3 right line");
				resultPos = traceSection3(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice, 
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 3 right line end");
			} else if (degree >= section4_degree_start && degree < section4_degree_end) {
				// System.err.println("trace section 4 right line");
				resultPos = traceSection4(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
			} else if (degree > section5_degree_upperHalf_start && degree <= section5_degree_upperHalf_end) {
				// System.err.println("trace section 5 upper right line");
				resultPos = traceSection5_upper(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 upper right line end");
			} else if (degree >= section5_degree_lowerHalf_start && degree < section5_degree_lowerHalf_end) {
				// System.err.println("trace section 5 lower right line");
				resultPos = traceSection5_lower(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 5 lower right line end");
			} else { // section 6
				// System.err.println("trace section 6 right line");
				resultPos = traceSection6(posX, posY, stepX, stepY, condyleCenter, degree, sliceNumber, distCurrent, distOuter, 
						greImageSlice,
						class1ImageFat, class2ImageFat, class3ImageFat, fuzzyCImageFat, 
						class1ImageGre, class2ImageGre, class3ImageGre, fuzzyCImageGre);
				// System.err.println("trace section 6 right line end");
			}
        ...
        */ 
		 ...
		 * walkStepX = ( upperRightX - upperLeftX ) / 10.0f; walkStepY = (
		 * upperRightY - upperLeftY ) / 10.0f;
		 * 
		 * currX = upperLeftX; currY = upperLeftY;
		 * 
		 * for ( k = 0; k <= 10; k++ ) { ptList.add(new Vector3f(currX, currY,
		 * 0f)); linePoints.put(linePointsIndex, new Line(condyleCenter.X,
		 * condyleCenter.Y, currX, currY, linePointsIndex)); linePointsIndex++;
		 * 
		 * currX += walkStepX; currY += walkStepY; }
		 */
		...
		fuzzyCImageFat.addVOIs(voiVectorNew);
		fuzzyCImageFat.setImageName("fuzzyCFat_" + sliceNumber + "_");
		class1ImageFat.addVOIs(voiVectorNew);
		class1ImageFat.setImageName("class1Fat_" + sliceNumber + "_");
		class2ImageFat.addVOIs(voiVectorNew);
		class2ImageFat.setImageName("class2Fat_" + sliceNumber + "_");
		class3ImageFat.addVOIs(voiVectorNew);
		class3ImageFat.setImageName("class3Fat_" + sliceNumber + "_");
		...
		fuzzyCImageGre.addVOIs(voiVectorNew);
		fuzzyCImageGre.setImageName("fuzzyCGRE_" + sliceNumber + "_");
		class1ImageGre.addVOIs(voiVectorNew);
		class1ImageGre.setImageName("class1GRE_" + sliceNumber + "_");
		class2ImageGre.addVOIs(voiVectorNew);
		class2ImageGre.setImageName("class2GRE_" + sliceNumber + "_");
		class3ImageGre.addVOIs(voiVectorNew);
		class3ImageGre.setImageName("class3GRE_" + sliceNumber + "_");
		// new ViewJFrameImage(class1ImageGre);
		// new ViewJFrameImage(class2ImageGre);
		// new ViewJFrameImage(class3ImageGre);
		// new ViewJFrameImage(fuzzyCImageGre);
	...
	private void findBoundingContour(int sliceNumber, VOI resultVOIBoundary, VOI resultVOIInner, VOI resultVOIOuter, Vector3f center, ModelImage fatImageSlice,
		...
		// double distanceCutoff = Math.sqrt(( cutOffPoint.X - centerX) *
		// (cutOffPoint.X - centerX) + (cutOffPoint.Y - centerY) *
		// (cutOffPoint.Y - centerY));
			...
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				...
				if (whichLeg == LEFT_LEG) {
					if (group == GROUP_1) {
						...
						distanceOuter = distance * (1d + 0.12d);
						distanceInner = distance * (1d - 0.02d);
					} else if (group == GROUP_7) {
				...
				if (whichLeg == LEFT_LEG) {
					...
					if (group == GROUP_1) {
						...
						distanceOuter = distance * (1d + 0.30d);
						distanceInner = distance * (1d - 0.09d);
					} else if (group == GROUP_5) {
						distanceOuter = distance * (1d + 0.20d);
						distanceInner = distance * (1d - 0.08d);
					} else if (group == GROUP_6) {
	...
	private void findBoundingContour_condyle(int sliceNumber, VOI resultVOIBoundary, VOI resultVOIInner, VOI resultVOIOuter, Vector3f center,
			ModelImage fatImageSlice, VOILine leftLine, VOILine rightLine) {
			...
			if (degree >= section1_degree_start && degree < section1_degree_end) {
				if (whichLeg == LEFT_LEG) {
					if (group == GROUP_1) {
						...
						distanceOuter = distance * (1d + 0.05d);
						distanceInner = distance * (1d - 0.2d);
					} else if (group == GROUP_5) {
						distanceOuter = distance * (1d + 0.05d);
						distanceInner = distance * (1d - 0.20d);
					} else if (group == GROUP_6) {
				...
				if (whichLeg == LEFT_LEG) {
					if (group == GROUP_1) {
						...
						distanceOuter = distance * (1d + 0.10d);
						distanceInner = distance * (1d - 0.60d);
					} else if (group == GROUP_9) {
						distanceOuter = distance * (1d + 0.10d);
						distanceInner = distance * (1d - 0.60d);
					} else if (group == GROUP_10) {
	...
	private void findSections(Vector3f[] inContour, int contourSize, float[][] sections, Vector3f center) {
		...
		if (contourSize > 0) {
			...
			// System.err.println("section1_degree_start = " + section1_start +
			// "  section1_degree_end = " + section1_end);
			...
			// System.err.println("section2_degree_start = " + section2_start +
			// "  section2_degree_end = " + section2_end);
			...
			// System.err.println("section3_degree_start = " + section3_start +
			// "  section3_degree_end = " + section3_end);
			...
			// System.err.println("section4_degree_start = " + section4_start +
			// "  section4_degree_end = " + section4_end);
			...
			// System.err.println("section6_degree_start = " + section6_start +
			// "  section6_degree_end = " + section6_end);
	...
	private void tracingDFS_mid(ModelImage greImageSlice, ModelImage fatImageSliceMid, VOI boundary, VOI inner, VOI outer, boolean first) {
		...
		/*
		 * // VOI resultVOI = new VOI((short) 0, "result-VOI", VOI.CONTOUR, -1);
		 * VOIVector voiVectorNew = new VOIVector(); voiVectorNew.add(boundary);
		 * 
		 * greImageSlice.addVOIs(voiVectorNew);
		 * smoothVOI150Single(greImageSlice, greImageSlice); double
		 * constant_length = 2.5d; boundary = greImageSlice.getVOIs().VOIAt(0);
		 * VOIBaseVector vVector = boundary.getCurves();
		 * 
		 * v = vVector.get(0); double perimeter =
		 * v.getLengthPtToPt(greImageSlice.getFileInfo(0).getResolutions()); //
		 * System.err.println("perimeter = " + perimeter); int numberPoints =
		 * (int) Math.round(perimeter / constant_length);
		 * 
		 * float res[] = greImageSlice.getFileInfo(0).getResolutions(); float
		 * xRes = res.length > 0 ? res[0] : 1; float yRes = res.length > 1 ?
		 * res[1] : 1; float zRes = res.length > 2 ? res[2] : 1;
		 * 
		 * boundary = greImageSlice.getVOIs().VOIAt(0); vArray =
		 * boundary.getSortedCurves(VOIBase.ZPLANE, 1); VOIBase result_v =
		 * vArray[0].get(0); nPts = result_v.size(); xPts = new float[nPts];
		 * yPts = new float[nPts]; zPts = new float[nPts];
		 * 
		 * float[] xNew = new float[numberPoints]; float[] yNew = new
		 * float[numberPoints]; float[] zNew = new float[numberPoints];
		 * 
		 * // System.err.println(" numberPoints = " + numberPoints);
		 * result_v.exportArrays(xPts, yPts, zPts);
		 * 
		 * float x1, y1, x2, y2, distance; int m = 0; int k = 0; boolean find =
		 * false;
		 * 
		 * while (k < nPts - 5) { x1 = xPts[k]; y1 = yPts[k]; find = false; for
		 * (int j = k + 1; j < k + 6; j++) { x2 = xPts[j]; y2 = yPts[j];
		 * distance = (float) Math.sqrt((x2 - x1) * (x2 - x1) * (xRes * xRes) +
		 * (y2 - y1) * (y2 - y1) * (yRes * yRes)); if (distance >=
		 * constant_length) { xNew[m] = x2; yNew[m] = y2; zNew[m] = 0f;
		 * 
		 * m++; k = j; find = true; break; } } if (find) continue; k++; }
		 * 
		 * while (k < nPts) { x1 = xPts[k]; y1 = yPts[k]; find = false; int
		 * endPt = k + 6; if (k + 6 > nPts) { endPt = nPts; } for (int j = k +
		 * 1; j < endPt; j++) { x2 = xPts[j]; y2 = yPts[j]; distance = (float)
		 * Math.sqrt((x2 - x1) * (x2 - x1) * (xRes * xRes) + (y2 - y1) * (y2 -
		 * y1) * (yRes * yRes)); if (distance >= constant_length) { xNew[m] =
		 * x2; yNew[m] = y2; zNew[m] = 0f;
		 * 
		 * m++; k = j; find = true; break; } } if (find) continue; k++; }
		 * 
		 * linePoints.clear();
		 * 
		 * for (int z = 0; z < m; z++) { posX = xNew[z]; posY = yNew[z]; Line
		 * line = new Line(condyleCenter.X, condyleCenter.Y, posX, posY, z);
		 * linePoints.put(z, line); }
		 */
	...
	private void tracingDFS_mid_cutoffLine(ModelImage greImageSlice, ModelImage fatImageSliceMid, VOI boundary, VOI inner, VOI outer, boolean first,
		...
		// **************** find the cut off point on the right line points
		...
		for (i = 0; i < lowerIndex; i++) {
			...
			// kLineTemp.add(new Vector3f(condyleCenter.X, condyleCenter.Y,
			// 0f));
		...
		// linePoints.put(i, new Line(condyleCenter.X, condyleCenter.Y,
		// lowerLeftX, lowerLeftY, i));
	...
	void generateMap(int x, int y, ModelImage imageSlice, int visited[][], int map[][], int Xmin, int Xmax, int Ymin, int Ymax, int type) {
		...
		if (type == GRE_FUZZY_CLASS1 ) {
			if (imgIntensity < 0.8f ) {
				if ((y - Ymin) >= 0 && (y - Ymin) <= yDim && (x - Xmin) >= 0 && (x - Xmin) <= xDim)
					map[y - Ymin][x - Xmin] = 1;
			} else { // fuzzyC class 1
				if ((y - Ymin) >= 0 && (y - Ymin) <= yDim && (x - Xmin) >= 0 && (x - Xmin) <= xDim)
